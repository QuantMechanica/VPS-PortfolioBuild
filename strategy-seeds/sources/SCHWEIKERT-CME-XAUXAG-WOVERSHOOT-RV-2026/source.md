...
title: Gold-silver completed-week reversal-overshoot reversion extraction
...
approval_basis: decisions/2026-08-20_xauxag_weekly_reversal_overshoot_reversion_source_approval.md
parent_source_ids:
...
cards_extracted:
  - xauxag-wovershoot-rv
---

# XAU/XAG Completed-Week Reversal-Overshoot Reversion Source Packet
...
A fresh generic-reader request for the CME URL returned
`DEFERRED:SOURCE_POLICY`; no bypass, inferred page content, or new online claim
is used.
...
They do not establish that a dominant opposite-sign weekly move overshoots,
that the inverse package will converge, or that a Darwinex CFD package is
neutral or uncorrelated.
...
`abs(r_new)>abs(r_old)`. A dominant positive newest reversal opens SELL XAU /
BUY XAG. A dominant negative newest reversal opens BUY XAU / SELL XAG. Close
...
CFD carrier, equal-notional target, aggregate fixed-risk cap, ATR stops,
spread caps, consumed-attempt ledger, next-week exit, and stale guard are
transparent QM choices. They are not attributed to the sources. No source
return, alpha, drawdown, density, CFD equivalence, neutrality, or portfolio-
...
The deterministic pre-allocation checker returned `CLEAN` across 4,562
...
weekly NR7 breakouts, or strict same-sign weekly deceleration. None requires
exactly two adjacent completed broker-week relative returns to have opposite
signs while the newest is strictly larger, then fades only that newest move
for exactly the next broker week.

The three synchronized week ends, chronological return orientation, strict
sign opposition, strict newest absolute dominance, inverse newest-return
sides, consumed weekly attempt, and next-week exit are jointly load-bearing.
Changing any one creates a different identity.

## Reputable-Source Criteria

- R1: `PASS_WITH_WEEKLY_OVERSHOOT_TRANSLATION_RISK`. One bounded source ID
...
weekly reversal-overshoot fade's efficacy. Q02 must retire the card below five
...
may be rescued by changing the number of weeks, accepting equality or same-
sign moves, changing direction, adding a threshold or fitted center, changing
the hold, or adding a trend/calendar filter.
