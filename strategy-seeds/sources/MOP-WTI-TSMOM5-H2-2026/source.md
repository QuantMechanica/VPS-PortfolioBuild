...
title: WTI exact five-completed-month trend with a fixed two-month holding clock
...
approval_basis: decisions/2026-09-08_wti_five_month_momentum_two_month_hold_source_approval.md
...
# WTI Five-Month Trend / Two-Month Hold
...
and volatility scaling. It does not publish a standalone WTI `k=5, h=2`
...
six consecutive completed month-end closes `C[0]..C[5]`, oldest to newest.
Compute `r5 = ln(C[5] / C[0])`. Buy if `r5 > 0`, sell if `r5 < 0`, and consume
...
completed months on that lifecycle. `QM5_41379` through `QM5_41384` observe
three, one, nine, six, four, and two completed months. The new identity
requires the exact combination of six consecutive endpoints, five-month
