#!/usr/bin/env python3
"""Deterministic H-PY pilot fire count — Dukascopy USATECHIDXUSD ticks -> H1.

Motivation-only computed output for QM-RESEARCH-2026-0007 (TAIL_RISK Family A:
bounded positive pyramiding, session-flat). Counts how often the preregistered
cash-session pyramid fires on an NDX-class index CFD and what the basket outcome
shape looks like (level-2/level-3 reach, giveback-stop rate, expectancy) on the
discovery window 2018-2020. NOT farm .DWX data, NOT proof of edge.
...
Session/parameter defaults are EXACTLY the preregistered card defaults:
    ...
    add2_trigger_atr=1.0, add3_trigger_atr=2.0,
    level2_size_mult=0.75, level3_size_mult=0.5,
...
= -2R. Aggregate basket <= 2.25 L1 legs by construction.
...
Execution model (closed-bar, deterministic):
    ...
    flatten-hour bar); one add per bar maximum; no adds on Friday after cutoff:
      ...
      L3 when (close-entry) >= add3_trigger_atr*ATR AND a second new favourable
         extreme prints after the L2 fill.
    ...
    identical to the H-MR pilot convention.
...
OUT = Path(os.environ.get(
    "HPY_OUT",
    str(Path(__file__).resolve().parent / "h_py_pilot.json"),
))
...
def run_year(year):
    ...
    while i < n:
        ...
        for s in block:
            ...
            pending_add = 0  # 2 or 3, fills at next bar open
            exit_r = None
            reason = None
            held = 0
            t = b
            while t < n:
                ...
                if held >= tsb:
                    ...
                    reason = "time_stop"
                    break
                # add triggers (evaluated on close; fill next bar)
...
def main():
    ...
    for year in YEARS:
        ...
        print(f"[{year}] days={st['days_evaluated']} baskets={st['baskets']} "
              f"l2={st['l2_fills']} l3={st['l3_fills']} r_sum={st['r_sum']:.2f}",
              flush=True)
    ...
    result = {
        "schema": "qm.computed-output/h-py-pilot/v1",
        ...
        "totals": {
            **agg,
            ...
            "l3_reach_rate": round(agg["l3_fills"] / baskets, 4) if baskets else None,
