...
title: Mehlitz-Auer memory-enhanced momentum - WTI R12-q13 extraction
...
strategy_ids:
  - MEHLITZ-AUER-MEM-2024_XTI_R12Q13_S04
---

# Mehlitz-Auer WTI R12-q13 Source Packet
...
to reversal. This extraction selects the source-declared `R12-q13` pair.
...
```text
w = [24/13, 22/13, 20/13, 18/13, 16/13, 14/13, 12/13, 10/13, 8/13, 6/13, 4/13, 2/13]
```

Require `abs(z_vr) > 1.64485362695147`. Let `R12` be the sum of the latest twelve
monthly log returns. The source direction matrix is:

```text
direction = sign(R12) * sign(z_vr)
```

- positive direction: long WTI;
- negative direction: short WTI;
- insignificant `z_vr`, zero `R12`, or invalid arithmetic: flat.
...
locked `R12-q13` rule, and allows at most one consumed entry attempt per broker
...
- R1: PASS. The primary citation is a peer-reviewed journal article with a DOI
  and a completely reviewed open precursor; WTI and `R12-q13` are explicit in
  the source lineage.
- R2: PASS. The 33 endpoints, 32 returns, twelve-month ranking return, twelve lag
...
The deterministic pre-allocation checker scanned 4,314 registry rows and 431
...
- `QM5_20256_wti-vr6-mom` implements `R6-q7`, with six-month direction, six
  lags, and weights `12/7` through `2/7`.
- This extraction implements `R12-q13`, with twelve-month direction, twelve lags,
  weights `24/13` through `2/13`, and twelve squared robust weights.
- `QM5_12603_wti-tsmom12m` is plain twelve-month sign momentum without a memory
...
- `QM5_20245_wti-vr-rsm` applies `q=2` memory to twelve binary monthly signs,
  not the source's cumulative `R12-q13` pair.
