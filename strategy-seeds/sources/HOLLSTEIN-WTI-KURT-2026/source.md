...
title: WTI historical-kurtosis premium extraction from Anomalies in Commodity Futures Markets
publisher: Quarterly Journal of Finance
source_type: bounded_peer_reviewed_paper_extraction
status: approved_source_complete
approval_basis: OWNER commodity/energy portfolio mission delivered 2026-08-13
created: 2026-08-13
...
# WTI Historical-Kurtosis Premium Source Packet

## Approval And Complete-Read Boundary

The durable OWNER mission dated 2026-08-13 authorizes one new structural,
low-frequency commodity/energy card, non-live V5 build, and paced Q02 enqueue.
The governed parent packet records an end-to-end read of the 57-page accepted
article and online appendix, including definitions, portfolio sorts, factor
tests, alternate portfolio counts, subperiods, annual holds, tables, and WTI
membership. This bounded packet was read completely before card drafting.
...
- Table 1 explicitly includes WTI crude oil in the source universe.

The article therefore supplies the statistic, high-minus-low orientation,
monthly decision clock, and WTI membership. It does not supply a profitable
single-WTI time-series threshold rule.
...
pivot: buy WTI above `3.0 + 1e-12`, sell WTI below `3.0 - 1e-12`, and consume
the month flat inside the tolerance or on invalid state. Hold until the next
broker-month transition, subject to a forty-calendar-day stale guard and a
frozen `3.5 * ATR(20,D1)` hard stop.

The absolute benchmark-3 map is a transparent QM hypothesis. The source ranks
at least six collateralized futures and does not claim that Pearson kurtosis
above three predicts positive WTI returns or that a value below three predicts
negative WTI returns. The sample estimator is also not exactly unbiased around
three in finite samples. No source alpha, return, Sharpe, drawdown, WTI-only
effect, CFD equivalence, cost, density, correlation, or portfolio result is
imported. Q02 is a low-prior falsification.

## Non-Duplicate Boundary

The canonical pre-allocation checker scanned 4,360 registry rows and 471 root
cards. It found no exact slug or strategy identity and returned seven expected
...
- `QM5_13131_energy-kurt-rank` computes the same source estimator separately
  ...
  two-leg package. This candidate reads only WTI and maps it around three.
...
- `QM5_20290_wti-skew-prem` maps a centered third standardized moment around
...
- `QM5_13130_xti-xng-lowmax` and `QM5_20294_xauxag-max-rk` average only the five
  largest daily returns and buy the lower-MAX leg; they do not measure the full
  return distribution's fourth moment.
- Legacy kurtosis EAs combine skew, daily scaling, or intraday composites and
  do not implement a pure monthly outright-WTI Pearson-kurtosis state.
- `QM5_12567_cum-rsi2-commodity` is a short-horizon long-only oscillator
  pullback, not a monthly fourth-moment premium.

The exact 252 simple returns, source denominators, Pearson fourth moment,
fixed benchmark three, high-kurtosis long/low-kurtosis short map, outright WTI
...
- R3: PASS with futures-to-CFD risk. Registered `XTIUSD.DWX` D1 OHLC and native
...
moment denominator, excess rather than Pearson kurtosis, fitted pivot, reversed
direction, repeated monthly attempt, missing hard stop, risk mismatch, stale
hold, or nondeterminism. No post-result threshold, formula, direction, stop,
