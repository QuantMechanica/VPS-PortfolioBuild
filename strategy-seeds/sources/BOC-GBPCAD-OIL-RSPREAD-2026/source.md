...
title: Oil price and Canadian dollar structural linkage for WTI/GBPCAD relative value
...
relative-value rule: compare completed WTI returns with completed GBPCAD
...
- `xti-gbpcad-rspr`: `XTIUSD.DWX` and `GBPCAD.DWX` D1 inverse-CAD
...
uses WTI plus GBPCAD, trades both legs as a two-leg basket, and opens from a
...
- R3 data available: PASS. `XTIUSD.DWX` and `GBPCAD.DWX` are present in
