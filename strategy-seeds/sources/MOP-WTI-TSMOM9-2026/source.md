...
title: WTI exact nine-completed-month own-return extraction from Time Series Momentum
...
approval_basis: decisions/2026-08-12_qm5_20293_wti_tsmom9m_g0.md
...
# WTI Nine-Month Time-Series Momentum Source Packet
...
`decisions/2026-08-12_qm5_20293_wti_tsmom9m_g0.md`. No new online page,
...
return sign. The paper does not report a standalone nine-month WTI result.
...
ten consecutive completed `XTIUSD.DWX` month-end closes, oldest to newest.
...
The fixed nine-completed-month interval is selected and locked before any Q02
result. The continuous-CFD carrier, broker-month endpoint reconstruction,
...
For ten positive finite completed month-end closes `C[0]..C[9]`, oldest to
newest:
...
All ten month keys must be consecutive, timestamps must increase strictly,
and `C[9]` must be the immediately prior completed broker month. Interior
endpoints prove continuity and may not be interpolated or omitted. There is
no 189-D1 approximation, three-month confirmation, neutral threshold, sign
vote, average, sort, clipping, regression, fitted parameter, moving average,
oscillator, calendar direction, external series, or prior pipeline result.
Signal magnitude never scales risk.

## Non-Duplicate Boundary

The deterministic pre-allocation checker scanned 4,358 EA-registry rows and
469 cards. It found no exact identity and returned eleven expected fuzzy
matches from the shared MOP source and `tsmom` tokens. Manual registry and
mechanic review found no WTI exact nine-completed-month carrier without a
confirmation or neutral band.

`QM5_12616` uses a 189-D1 approximation, a nonzero neutral threshold, and a
63-D1 same-sign confirmation. Existing pure WTI sign carriers use one, two,
three, four, six, or twelve months. `QM5_20281` changes both the formation and
holding clock; `QM5_20056` combines six and twelve months; `QM5_20258` votes
three horizons. Regression, rank, robust-return, path, sign-run, weighted,
calendar, event, XNG, and XAU/XAG systems observe different state objects or
carriers.

The ten endpoints, continuity, exact endpoint orientation, lack of a
confirmation or threshold, symmetric sign, consumed attempt, and monthly
...
nine-month horizon. Q02 must retire the card below five completed packages per
