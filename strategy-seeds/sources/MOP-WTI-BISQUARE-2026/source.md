...
title: WTI fixed-step redescending bisquare robust-location trend extraction
...
approval_basis: decisions/2026-08-12_qm5_20286_wti_bisquare_mom_g0.md
...
# WTI Redescending Bisquare Robust-Location Source Packet
...
`decisions/2026-08-12_qm5_20286_wti_bisquare_mom_g0.md`. No blocked page,
...
information. They do not establish that a redescending bisquare location
improves the signal.
...
chronological log returns. Compute their even-sample median and even-sample raw
MAD. Freeze `cutoff = 4.685 * 1.4826 * MAD`, initialize location at the median,
and run exactly 32 redescending bisquare reweighted-mean updates. Buy for a
positive final location, sell for a negative final location, and consume the
month flat when it is exactly zero or invalid. Renew at the next broker-month
boundary.

The robust estimator, cutoff, scale normalization, fixed update count,
endpoint convention, continuous-CFD carrier, broker-month reconstruction,
one-attempt ledger, `RISK_FIXED` sizing, ATR hard stop, spread ceiling, and
stale exit are transparent QM mechanizations. No source return, alpha, Sharpe
ratio, drawdown, trade count, cost, WTI-only result, CFD equivalence, or
...
The scale and cutoff freeze before the first update. All 32 updates execute.
At least one positive weight is required at every update. There is no early-
stop tolerance, return deletion or replacement, fallback center, alternate
cutoff, magnitude sizing, or runtime fit.

## Non-Duplicate Boundary

The deterministic pre-allocation checker scanned 4,351 EA-registry rows and
463 root cards. It found no exact identity and surfaced one fuzzy match,
`QM5_20285_wti-huber-mom`.

That neighbor uses a Huber score whose finite tail weights remain positive as
`delta / residual`. The bisquare score here is redescending: its weights fall
smoothly to zero at the cutoff and every observation at or beyond the cutoff
has exactly zero influence. `QM5_20282` applies a one-shot median/MAD cap;
`QM5_20283` is a fixed quartile trimean; `QM5_20277` uses fixed-order-statistic
Winsor replacement; and `QM5_20270` trims a fixed number of observations.
None performs the locked redescending update. Verdict:
...
- R2: PASS. Endpoint count/order, return orientation, median/MAD convention,
  constants, frozen cutoff, support and weight equation, update count,
...
efficacy of the bisquare transformation. Q02 must retire the card below five
...
may be rescued by changing scale, cutoff, iteration count, horizon, direction,
