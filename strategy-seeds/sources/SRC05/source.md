...
budget_tracking:
  heartbeats_used: 2                          # h1 scaffold + 4-card extraction batch (S01, S02, S05, S06); h2 single-symbol-futures batch (S07, S08, S09)
  cards_drafted: 7                            # S01 chan-at-bb-pair, S02 chan-at-kf-pair, S05 chan-at-fx-coint-pair, S06 chan-at-cal-spread (h1 commits 23bd5a7 + 30e048c + a2d2f2a) + S07 chan-at-ts-mom-fut, S08 chan-at-roll-arb-etf, S09 chan-at-vx-es-roll-mom (h2 batch this commit)
  cards_passed_g0: 0
  cards_killed_pre_p1: 0
extraction_pass_status: h2_first_pass_in_progress  # 7/12 unconditional cards drafted; remaining h3-h4 batch: S03/S04/S10-S12 (5 cards) + S13/S14 conditional pending CEO ratification
...
| S03 | `chan-at-buy-on-gap` | `strategy-seeds/cards/chan-at-buy-on-gap_card.md` | TBD | DRAFT-PENDING | PDF p. 94 (Ex 4.1) | Cross-sectional gap-down + above-MA + top-N selection on SPX universe. Buy at open if open < prev_low × (1 - entryZscore × 90d_stdev) AND open > 20d_MA; pick top-N most-negative-gap stocks; exit at close. APR 8.7%, Sharpe 1.5. Multi-stock-universe (V5-architecture-CHALLENGED — same as SRC02 chan-khandani-lo-mr). |
| S04 | `chan-at-spy-arb` | `strategy-seeds/cards/chan-at-spy-arb_card.md` | TBD | DRAFT-PENDING | PDF p. 98 (Ex 4.2) | Cointegration arbitrage between SPY and a Johansen-selected long-only basket of SPX-component stocks that individually cointegrate with SPY. Linear MR on the log market value of the long-short portfolio (lookback=5, training set 2007-01-01 to 2007-12-31). APR 4.5%, Sharpe 1.3. Multi-stock-universe (V5-architecture-CHALLENGED). |
...
| S10 | `chan-at-xs-mom-fut` | `strategy-seeds/cards/chan-at-xs-mom-fut_card.md` | TBD | DRAFT-PENDING | PDF p. 145 (inline, Daniel-Moskowitz) | Cross-sectional futures momentum: rank universe of 52 commodity futures by 252-day lagged return; long top-1 + short bottom-1 (or top/bottom decile), hold 25 days. APR 18% (2005-07), then -33% (2008-09 crisis). **Vocab gap candidate: `cross-sectional-momentum`** (sibling of existing `cross-sectional-decile-sort` MR — opposite direction). |
| S11 | `chan-at-xs-mom-stock` | `strategy-seeds/cards/chan-at-xs-mom-stock_card.md` | TBD | DRAFT-PENDING | PDF p. 146 (Ex 6.2, Daniel-Moskowitz S&P 500 adaptation) | Cross-sectional stock momentum: rank S&P 500 by 252-day lagged return; long top-50 + short bottom-50, hold 25 days. APR 37% (May-Dec 2007), then -30% (2008-09 crisis). Reuses S10 `cross-sectional-momentum` flag with stock-universe parameter. Distinct from S10 by universe class (stocks vs futures). |
| S12 | `chan-at-fstx-gap-mom` | `strategy-seeds/cards/chan-at-fstx-gap-mom_card.md` | TBD | DRAFT-PENDING | PDF p. 156 (Ex 7.1) | Opening-gap momentum (go-with the gap, not fade): long if open > prev_high × (1 + 0.1·90d_stdev); short mirror; exit at close. FSTX source case + GBPUSD generalization. APR 13%, Sharpe 1.4 (FSTX); APR 7.2%, Sharpe 1.3 (GBPUSD). **Vocab gap candidate: `opening-gap-momentum`** (sibling of existing `gap-fade-stop-entry` — opposite direction; AND distinct from `vol-expansion-breakout` because referenced extreme is prior_high/prior_low, not next-bar-open + N%·prior-range). |
