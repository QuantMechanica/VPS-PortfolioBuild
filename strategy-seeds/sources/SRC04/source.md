...
| S02b | `lien-dbb-trend-join` | `cards/lien-dbb-trend-join_card.md` | QUA-341 (ready_for_board_close) | DRAFT (h2) | Ch 9 PDF pp. 107-110 | Double BB **trend-mode** trend-join entry (close back across 1st-σ band after K-bar opposite-side dwell). Sibling of S02a; shares proposed `bband-reclaim` flag with `precondition_mode=n-bars-opposite-1sigma`. Co-regime-fire suppression vs S02a documented in card § 6. |
