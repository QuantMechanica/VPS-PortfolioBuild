...
title: WTI exact nine-completed-month trend with a fixed two-month holding clock
...
approval_basis: decisions/2026-09-07_wti_nine_month_momentum_two_month_hold_source_approval.md
...
# WTI Nine-Month Trend / Two-Month Hold
...
and volatility scaling. It does not publish a standalone WTI `k=9, h=2`
...
ten consecutive completed month-end closes `C[0]..C[9]`, oldest to newest.
Compute `r9 = ln(C[9] / C[0])`. Buy if `r9 > 0`, sell if `r9 < 0`, and consume
...
`QM5_20293_wti-tsmom9m` observes the same nine-month return but closes and
renews monthly. `QM5_20281_wti-tsmom-h2` shares the fixed odd-month lifecycle
but observes twelve completed months. `QM5_41379` and `QM5_41380` observe
three and one completed months. The new identity requires the exact
combination of ten endpoints, nine-month orientation, fixed odd-month epoch,
