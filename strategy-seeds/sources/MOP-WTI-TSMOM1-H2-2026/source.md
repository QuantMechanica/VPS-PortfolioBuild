...
title: WTI exact one-completed-month trend with a fixed two-month holding clock
...
approval_basis: decisions/2026-09-07_wti_one_month_momentum_two_month_hold_source_approval.md
...
# WTI One-Month Trend / Two-Month Hold
...
and volatility scaling. It does not publish a standalone WTI `k=1, h=2`
...
two consecutive completed month-end closes `C[0]..C[1]`, oldest to newest.
Compute `r1 = ln(C[1] / C[0])`. Buy if `r1 > 0`, sell if `r1 < 0`, and consume
...
`QM5_20187_wti-tsmom1m` observes the same one-month return but closes and
renews monthly. `QM5_20281_wti-tsmom-h2` shares the fixed odd-month lifecycle
but observes twelve completed months, while `QM5_41379_wti-tsmom3-h2` uses
three months. The new identity requires the exact combination of two
endpoints, one-month orientation, fixed odd-month epoch, no even-month action,
one consumed bimonthly attempt, and two-month renewal.

## R1-R4 And Falsification

- R1: `PASS_WITH_WTI_SPECIFIC_EFFICACY_UNPROVEN`.
- R2: `PASS`; all signal, clock, attempt, stop, risk, and lifecycle rules are fixed.
