...
title: WTI completed-week same-sign deceleration momentum extraction
...
approval_basis: decisions/2026-08-20_wti_weekly_deceleration_momentum_source_approval.md
parent_source_ids:
  - MOP-TSMOM-2012
parent_sha256:
...
# WTI Completed-Week Deceleration Momentum Source Packet
...
continued returns after the newest absolute weekly move is smaller than the
...
`abs(r_new)<abs(r_old)`. Two positive decelerating weeks open BUY WTI. Two
negative decelerating weeks open SELL WTI. Close at the first later Monday
anchor or after ten calendar days.

The weekly horizon, exact endpoint reconstruction, two-return count, strict
same-sign and deceleration conditions, continuation side, CFD carrier,
...
The deterministic pre-allocation checker returned `CLEAN` across 4,557
registry rows and 625 root cards. The nearest direct-WTI weekly siblings are
disjoint: `QM5_41068` requires a strictly larger same-sign newest move,
`QM5_41069` requires opposed signs and follows the older sign, and
`QM5_41065` requires an opposed-sign handoff and follows the newest sign.
`QM5_41066` applies a smaller-magnitude condition to gold-minus-silver
relative returns and fades a two-leg basket; it does not continue a WTI own-
return sign. Existing one-week WTI systems use volatility ranks, within-week
segments, ranges, or event windows rather than this exact two-return state.

The three completed week ends, chronological return orientation, strict
same-sign test, strict absolute deceleration, continuation side, consumed
weekly attempt, and next-week exit are jointly load-bearing. Changing any one
creates a different identity.

## Reputable-Source Criteria

- R1: `PASS_WITH_WEEKLY_DECELERATION_TRANSLATION_RISK`. One bounded source ID
...
weekly deceleration gate's efficacy. Q02 must retire the card below five
...
acceleration, or opposed signs, changing direction or hold, or adding a
