...
title: WTI exact two-completed-month trend with a fixed two-month holding clock
...
approval_basis: decisions/2026-09-08_wti_two_month_momentum_two_month_hold_source_approval.md
...
# WTI Two-Month Trend / Two-Month Hold
...
and volatility scaling. It does not publish a standalone WTI `k=2, h=2`
...
three consecutive completed month-end closes `C[0]..C[2]`, oldest to newest.
Compute `r2 = ln(C[2] / C[0])`. Buy if `r2 > 0`, sell if `r2 < 0`, and consume
...
`QM5_20064_wti-tsmom2m` observes a two-month return but closes and renews
monthly. `QM5_20281_wti-tsmom-h2` shares the fixed odd-month lifecycle but
observes twelve completed months. `QM5_41379` through `QM5_41383` observe
three, one, nine, six, and four completed months. The new identity requires
the exact combination of three endpoints, two-month orientation, fixed
odd-month epoch, no even-month action, one consumed bimonthly attempt, and
two-month renewal.
