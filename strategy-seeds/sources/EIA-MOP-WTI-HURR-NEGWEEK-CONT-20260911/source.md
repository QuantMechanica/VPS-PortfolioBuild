...
title: WTI hurricane-season negative-week continuation
...
approval_basis: decisions/2026-09-11_wti_hurricane_negative_week_continuation_source_approval.md
...
cards_extracted: [wti-hurr-negweek-cont]
---

# WTI Hurricane-Season Negative-Week Continuation

## Complete-Read Record

Both governed parents named above were read end to end before the durable
approval at
`decisions/2026-09-11_wti_hurricane_negative_week_continuation_source_approval.md`.
...
short-only selection, a one-week hold, or a Darwinex continuous WTI CFD. The
exact rule is a transparent pre-result QM translation. A hurricane can also
reduce refinery demand, but this packet does not infer the cause of a negative
week. No performance or diversification claim transfers.
...
5. When `r < 0`, sell. Equality, nonnegative return, malformed history, stale
...
The canonical pre-allocation scan found no exact identity and eight fuzzy
family neighbors. `QM5_41421` uses the same negative-week/short continuation
state only in the refinery-maintenance calendar of February, March, September,
and October; the August hurricane month and exclusion of February-March are
load-bearing. `QM5_41430` requires a positive week and buys continuation;
`QM5_41431` requires the same negative week but buys reversion. `QM5_41424`
fades positive weeks in refinery-maintenance months. `QM5_12591` uses a daily
Donchian breakout with trend confirmation and `QM5_12754` fades a daily failed
spike. Natural-gas shoulder-week rules use another carrier and regime.

The manual verdict is
`CLEAN_WTI_HURRICANE_SEASON_NEGATIVE_WEEK_SHORT_CONTINUATION_AFTER_FAMILY_REVIEW`.
...
packages, nonnegative-week entry, any long, retry, missing stop, wrong
