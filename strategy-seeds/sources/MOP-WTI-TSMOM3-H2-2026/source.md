...
title: WTI exact three-completed-month trend with a fixed two-month holding clock
...
approval_basis: decisions/2026-09-07_wti_three_month_momentum_two_month_hold_source_approval.md
...
# WTI Three-Month Trend / Two-Month Hold
...
defines time-series-momentum positions by the sign of an instrument's own
past `k`-month return and expresses the family with formation horizon `k` and
...
and volatility scaling. It does not publish a standalone WTI `k=3, h=2`
...
four consecutive completed month-end closes `C[0]..C[3]`, oldest to newest.
Compute `r3 = ln(C[3] / C[0])`. Buy if `r3 > 0`, sell if `r3 < 0`, and consume
the bimonthly attempt flat for equality or invalid state. Hold to the next odd
month boundary; even-month transitions cannot close or replace the package.
...
`QM5_20055_wti-tsmom3m` observes the same three-month return but closes and
renews monthly. `QM5_20281_wti-tsmom-h2` shares the fixed odd-month lifecycle
but observes twelve completed months. The new identity requires the exact
combination of four endpoints, three-month orientation, fixed odd-month epoch,
no even-month action, one consumed bimonthly attempt, and two-month renewal.

## R1-R4 And Falsification

- R1: `PASS_WITH_WTI_SPECIFIC_EFFICACY_UNPROVEN`.
- R2: `PASS`; all signal, clock, attempt, stop, risk, and lifecycle rules are
