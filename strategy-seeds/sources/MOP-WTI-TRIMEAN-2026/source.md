...
title: WTI monthly quartile-trimean return-location extraction from Time Series Momentum
...
approval_basis: decisions/2026-08-11_qm5_20283_wti_trimean_mom_g0.md
...
# WTI Quartile-Trimean Return Trend Source Packet
...
`decisions/2026-08-11_qm5_20283_wti_trimean_mom_g0.md`. No new online page,
...
They do not establish a quartile trimean or its performance.
...
twelve adjacent close-to-close monthly log returns and sort an independent
copy ascending. Estimate the lower quartile as the mean of indexes 2 and 3,
the median as the mean of indexes 5 and 6, and the upper quartile as the mean
of indexes 8 and 9. The signal statistic is the `1:2:1` weighted mean of those
three locations. Buy when positive, sell when negative, and consume the month
flat when it is exactly zero or invalid. Renew at the next broker-month
boundary.

The quartile-trimean statistic, exact even-half index convention, continuous-
CFD carrier, broker-month endpoint reconstruction, one-attempt ledger,
...
signal = BUY  when trimean > 0
         SELL when trimean < 0
         FLAT when trimean == 0 or state is invalid
```

The current decision month contributes no endpoint. There is no interpolation
or fallback to a cumulative endpoint return, arithmetic mean, raw median,
fixed-count trimmed or Winsorized mean, MAD-capped mean, pairwise pseudomedian,
binary sign count, price slope, rank, moving average, oscillator, calendar
direction, external series, or prior pipeline result.

## Non-Duplicate Boundary

The deterministic pre-allocation checker scanned 4,348 EA-registry rows and
459 cards. It found no exact identity and surfaced four expected same-source
fuzzy matches. Manual review separates the candidate from raw-median direction,
the equal-weight middle-eight trimmed mean, fixed two-per-tail Winsorization,
and chronological linear weighting.

The trimean ignores sorted indexes 0, 1, 4, 7, 10, and 11; gives indexes 2, 3,
8, and 9 weight `1/8` each; and gives indexes 5 and 6 weight `1/4` each. No
existing card uses that exact six-order-statistic functional. The thirteen
endpoints, twelve adjacent intervals, ascending sort, exact indexes, `1:2:1`
aggregation, divisor four, symmetric direction, monthly attempt, and renewal
...
- R2: PASS. Endpoint count and order, return intervals, sort, quartile and
  median indexes, weights, divisor, direction, attempt, fixed risk, hard stop,
...
- R3: PASS. Registered `XTIUSD.DWX` D1 history plus native MT5 calendar,
  ...
  input.
- R4: PASS. Deterministic logarithm, sorting, weighted arithmetic, and native
...
quartile-trimean rule. Q02 must retire the card below five completed packages
...
changing the horizon, index convention, weights, estimator, direction,
