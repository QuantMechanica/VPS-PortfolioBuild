...
title: WTI winter weekly range-contraction body reversion
status: approved_source_complete
source_type: governed_peer_reviewed_reputable_and_academic_cross_source_mechanization
approval_basis: decisions/2026-09-11_wti_winter_nr2_body_reversion_source_approval.md
primary_instrument: XTIUSD.DWX
decision_timeframe: D1
strategy_ids:
  - BURAKOV-CRABEL-YANG-WTI-WINTER-NR2-BODY-FADE-20260911_S01
parent_sources:
...
# WTI Winter Weekly Range-Contraction Body Reversion

## Complete-Read Record

The three bounded parent records named above were read end to end before the durable approval at
`decisions/2026-09-11_wti_winter_nr2_body_reversion_source_approval.md`. Burakov, Freidin, and
...
lineage. Yang, Goncu, and Pantelous provide academic commodity-futures reversal lineage.

## Claim And Translation Boundary

No parent tests the exact rule below. The papers use monthly reference prices, different range
patterns, or exchange-traded futures; they do not establish a two-week weekly contraction filter,
weekly body-sign fade, fixed-dollar ATR risk, or one-week holding period on the Darwinex
...
relatives. `QM5_41444` uses the same body fade only after strict range expansion, the opposite
volatility state. `QM5_41442` is also expansion-gated and fades only outer-quartile final-close
states rather than every nonzero body. `QM5_41445` uses the same strict contraction and body
predicate but follows rather than fades it. Certified `QM5_12567` is an XNG cumulative-RSI
pullback. Carrier, November-May clock, strict contraction, body-only direction, immediate weekly
entry, symmetric reversion, and one-week lifecycle are jointly load-bearing.

Manual verdict:
`DISTINCT_WTI_NOVEMBER_MAY_NR2_OWN_BODY_SYMMETRIC_REVERSION_AFTER_FAMILY_REVIEW`.
