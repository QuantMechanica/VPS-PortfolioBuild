...
title: Gold-silver weekly ratio range-contraction body continuation
status: approved_source_complete
source_type: governed_peer_reviewed_exchange_and_range_state_mechanization
approval_basis: decisions/2026-09-12_xauxag_nr2_body_momentum_source_approval.md
primary_instruments: [XAUUSD.DWX, XAGUSD.DWX]
decision_timeframe: D1
strategy_ids:
  - FMR-CRABEL-CME-XAUXAG-NR2-BODY-MOM-20260912_S01
parent_sources:
...
# Gold-Silver Weekly Ratio Range-Contraction Body Continuation

## Complete-Read Record

The three bounded approved parent records named above were read end to end before the durable
approval at `decisions/2026-09-12_xauxag_nr2_body_momentum_source_approval.md`.
...
a contracting weekly ratio-close range followed by continuation of that week's body is profitable.
...
- R2 `PASS`: synchronization, two completed weeks, strict range contraction, strict body sign,
...
The canonical pre-allocation checker used the current Vault path and found no exact identity
across 4,935 registry rows, 1,544 repository cards, and 45 Strategy Wiki records. It raised four
expected fuzzy relatives. Its durable receipt is
`artifacts/qm5_candidate_xauxag_nr2_body_mom_dedup_preallocation_20260912.json`.

Manual review distinguishes this candidate from `QM5_41449`, which uses expansion with body
continuation; `QM5_41450`, which uses contraction with body reversion; `QM5_41451`, which uses
contraction with outer-quartile CLV continuation; and `QM5_41452`, which uses expansion with
outer-quartile CLV continuation. The carrier, range object, two-week contraction, strict body
direction, continuation side, and one-week package are jointly load-bearing.
