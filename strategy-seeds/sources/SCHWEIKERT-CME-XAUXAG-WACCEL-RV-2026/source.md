...
title: Gold-silver completed-week relative acceleration overshoot reversion extraction
...
approval_basis: decisions/2026-08-20_xauxag_weekly_acceleration_overshoot_reversion_source_approval.md
parent_source_ids:
...
# XAU/XAG Completed-Week Acceleration-Overshoot Reversion Source Packet
...
They do not establish that two same-sign weekly moves overshoot when the
newest magnitude is larger, that the inverse package will converge, or that a
...
XAU / BUY XAG. Two negative, accelerating relative moves open BUY XAU / SELL
XAG. Close at the first later Monday anchor or after ten calendar days.

The weekly horizon, exact endpoint reconstruction, two-return count, strict
same-sign and acceleration conditions, inverse sides, CFD carrier, equal-
notional target, aggregate fixed-risk cap, ATR stops, spread caps, consumed-
attempt ledger, next-week exit, and stale guard are transparent QM choices.
They are not attributed to the sources. No source return, alpha, drawdown,
density, CFD equivalence, neutrality, or portfolio-correlation statistic is
imported.
...
The deterministic pre-allocation checker returned `CLEAN` across 4,563
...
weekly NR7 breakouts, same-sign weekly deceleration, or opposite-sign weekly
reversal overshoot. None requires exactly two adjacent completed broker-week
relative returns to have the same sign while the newest magnitude is strictly
larger, then fades that shared direction for exactly the next broker week.

The three synchronized week ends, chronological return orientation, strict
same-sign test, strict absolute acceleration, inverse sides, consumed weekly
...
weekly acceleration fade's efficacy. Q02 must retire the card below five
...
deceleration or opposed signs, changing direction, adding a threshold or
