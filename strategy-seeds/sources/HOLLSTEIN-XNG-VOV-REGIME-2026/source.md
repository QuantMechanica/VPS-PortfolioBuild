...
title: XNG Self-Relative Realized Volatility-of-Volatility Regime
source_type: bounded_strategy_extraction
status: approved_source_bounded
approval_ref: decisions/2026-08-13_qm5_20299_xng_vov_regime_g0.md
created: 2026-08-13
...
# Hollstein-Prokopczuk-Tharann XNG Self-Relative VoV Extraction

## Durable Approval And Complete-Read Basis

The OWNER commodity/energy portfolio mission is durably bounded in
`decisions/2026-08-13_qm5_20299_xng_vov_regime_g0.md`. The governed parent
...
- accepted-manuscript pp. 5-9: the 26-commodity universe, XNG membership,
...
The new single-XNG carrier needs a fixed comparison state. It compares two
...
BUY  XNGUSD.DWX when vov[0] < vov[271] - 1e-12
SELL XNGUSD.DWX when vov[0] > vov[271] + 1e-12
...
- The source ranks a broad cross-section; the EA compares XNG with its own
...
No return, alpha, Sharpe ratio, drawdown, significance value, XNG-only result,
...
- `QM5_13146_energy-vov` ranks concurrent XTI and XNG VoV and trades a paired
  package; it has no preceding XNG block or outright topology.
- `QM5_20236_xauxag-vov-rank` ranks two metals and trades a paired package.
- `QM5_20298_wti-vov-regime` preserves the same estimator, disjoint offsets,
  direction, and lifecycle on WTI. This XNG carrier has its own registered
  history, contract economics, spread guard, magic, and independent Q02
  verdict; no WTI efficacy transfers.
...
- `QM5_20297_xng-kurt-prem` measures a fourth central return moment around a
  fixed normal benchmark, not nested realized VoV or state change.
- XNG trend, robust-return, calendar, variance-ratio, event, breakout, and
...
The deterministic checker found no exact identity across 4,364 registry rows
and 475 cards. Ten expected lexical/source-family fuzzy matches were manually
resolved. The two exact block offsets, disjoint support, nested denominators,
self-relative direction, single XNG carrier, and monthly attempt are jointly
...
- R1: PASS with caveats. Tier-A peer-reviewed primary paper, DOI,
  ...
  robustness context, and explicit natural-gas membership.
