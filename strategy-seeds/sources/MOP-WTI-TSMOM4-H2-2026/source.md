...
title: WTI exact four-completed-month trend with a fixed two-month holding clock
...
approval_basis: decisions/2026-09-08_wti_four_month_momentum_two_month_hold_source_approval.md
...
# WTI Four-Month Trend / Two-Month Hold
...
and volatility scaling. It does not publish a standalone WTI `k=4, h=2`
...
five consecutive completed month-end closes `C[0]..C[4]`, oldest to newest.
Compute `r4 = ln(C[4] / C[0])`. Buy if `r4 > 0`, sell if `r4 < 0`, and consume
...
`QM5_20280_wti-tsmom4m` observes the same four-month return but closes and
renews monthly. `QM5_20281_wti-tsmom-h2` shares the fixed odd-month lifecycle
but observes twelve completed months. `QM5_41379` through `QM5_41382` observe
three, one, nine, and six completed months. The new identity requires the exact
combination of five endpoints, four-month orientation, fixed odd-month epoch,
