# Strategy Card (mechanized) — H-PY2L: bounded two-level pyramid on cash-session index trend (session-flat)

## Research provenance
Variant lineage of H-PY (`strategy-seeds/sources/QM-RESEARCH-2026-0007/H_PY_card.md`,
QM5_41479), minted as QM-RESEARCH-2026-0008 (parent 0007) under the interim
OWNER delegation of 2026-09-15/16, per `docs/research/TAIL_RISK_PROGRAMME_2026-09-16.md`
§4 and the machine-readable bounds in
`tools/strategy_farm/config/tail_risk_families.v1.json`. The post-pilot L3
root-cause analysis (`QM-RESEARCH-2026-0007/ANALYSIS.md`, labelled
ANALYSIS_NOT_PREREGISTERED, in-sample 2018-2020 only) proved that the parent
card's third pyramid level is structurally unreachable inside the FTMO
session-flat envelope: entries fill at 17:00-18:00 UTC, the earliest level-2
fill is the 19:00 bar, adds may not fill into the flatten-hour bar (20:00),
and a third level may only trigger on a bar strictly after the level-2 fill —
so the level-3 fill is barred by construction (the trigger's price condition
was met on 13 of 22 level-2 baskets and the fill was mechanically excluded
every time; single-rule counterfactuals disabling the giveback stop, the trail
ratchet, the time stop, or the flatten exit all still produce zero level-3
fills). The correct design response is the Family A contract's own tightening
clause: this card reduces `max_levels` 3 → 2 and the progression
1.0/0.75/0.5 → 1.0/0.75 (aggregate cap 2.25 → 1.75 legs), and removes the
level-3 machinery (`add3_trigger_atr`, `level3_size_mult`). Nothing is
widened; every other rule is identical to the frozen 0007 card. Offline
statistical instruments were used in research only (directive sec41); every
rule below is fully mechanical with no runtime model (directive sec42/sec51).
Pilot motivation (not proof) comes from a deterministic fire count of the
exact rules below on a Dukascopy USATECHIDXUSD tick feed (NDX-class index
CFD, 2018-2020 in-sample; computed output `h_py2l_pilot.json`), which
reproduces the parent pilot's realized path exactly (the parent never filled
level 3): 185 baskets over 29 months (6.4/month), raw PF(R) 1.26 at
+0.12R/basket, level-2 reach 11.9%, giveback-before-level-2 15.1%, average
peak +0.77R, max +19.1R — in-sample, zero-cost, single-feed feasibility
motivation only; the frozen 2021-2024 holdout and the governed Q00-Q17
pipeline are the judges.
...
the close. The mechanism is convexity, not prediction: the add concentrates
exposure only after the tape has already moved in favour (an ATR-scaled open
profit plus a new favourable extreme is required), so the position is largest
exactly when the session trend has confirmed itself, and the never-widening
basket stop converts parabolic reversals into small realised givebacks rather
than open-ended losses. Confining the whole construction to the cash window
and forcing flat before the thin rollover removes the two tails that kill
FTMO accounts — overnight gap and swap — which is precisely the tail a
built-out basket fears most. The two-level form is the honest shape of this
mechanism inside the session-flat envelope: the session clock leaves room for
exactly one add, and the third level the family envelope permits would be dead
code.
...
side of a short intraday EMA. A favourable close stack first prints +1.0x ATR
of open profit with a new favourable extreme (level 2 fills). The signature
ends in one of three shapes: a vertical extension whose giveback from the peak
triggers the basket exit at +50% retained profit, a drift that the trailing
stop later harvests, or a chop that stops the base leg at its ATR stop or runs
into the session-flat flatten.
...
cost on every session that breaks out; the add is paid for only by sessions
...
base-leg version (H-CW shape) only if cash-session trends persist long enough
to print a new favourable extreme after +1.0x ATR more often than they reverse
violently through the giveback stop first.
...
There is no third level. Hard bound: never more than 2 legs and never more
than 1.75 base legs aggregate per basket; the add logic fails closed at the
bound (no further adds even if the trigger repeats).
...
Level 2 sizes at ``level2_size_mult`` of the base size (Family A tightening
1.0 / 0.75), so a fully built basket carries 1.75 base legs and at most 5%
margin. Aggregate basket exposure never exceeds 1.75 base legs; one pyramid
per symbol per day; at most ``max_positions_total`` concurrent baskets across
the EA's symbol slots, so worst-case planned stop risk across the EA is
bounded by the daily and weekly breakers even if every basket is fully built.
Family-contract values (deliberately not free parameters): level weights
1.0 / 0.75, aggregate 1.75 legs, margin cap 5%, level cap 2 (a tightening of
the family 3-level envelope, never a widening); the emergency-exit bounds
0.5% adverse and 50% giveback are upper bounds (the card may tighten via
...
no contract is required at intake; the complete contract is carried anyway per
the programme doctrine (uniformity + the joint-tail engine needs declared
bounds for every sleeve it measures):
...
```json
{
  ...
  "max_levels": 2,
  "sizing_progression": {"type": "custom", "multiplier": null, "notes": "per-level multipliers [1.0, 0.75] of the initial leg; aggregate = 1.75 initial legs (Family A 3-level envelope tightened, never widened)"},
  "max_open_positions": 3,
  "max_basket_exposure": "1.75",
  "max_gross_notional": "1.75",
  "max_margin_pct": 5.0,
...
in-sample, computed output ``h_py2l_pilot.json``) recorded: 489 evaluated
session days (154 shock-skipped at 3.0x), 187 gross signals, 185 taken
baskets, 6.4 baskets/month and ~6.4 active days/month over 29 months; exits
73 base stops, 37 giveback stops, 75 session-flat flattens; level 2 filled in
11.9% of baskets; raw outcome PF(R) 1.26 at +0.12R/basket, average peak open
basket profit +0.77R, maximum +19.1R. These figures are identical to the
parent 0007 pilot because the parent's third level never filled — the two
 runners share the same realized path (recorded as the parent consistency
check in ``h_py2l_pilot.json``). These are honest pilot measurements used for
density calibration and feasibility only: they are IN-SAMPLE on one proxy feed
with zero costs, and the preregistered validation must treat them as
motivation, not evidence — the governed pipeline (filters, .DWX execution,
Q00-Q17) is the judge.
...
## Regime of failure
Trend exhaustion and clustered session reversals. A parabolic reversal after
the add prints converts the largest aggregate position at the worst prices —
bounded by the breakeven-plus trail and the giveback stop, but repeated
giveback exits are death by sequence. Correlated index symbols breaking out
together concentrate the same-session giveback across baskets (NDX/GDAXI/SP500
share the cash-session factor). Choppy range sessions produce repeated small
base-stop and giveback losses. The regime of failure is exactly what the
preregistered kill criteria measure.

## Invalidation conditions
Refuted if, out of sample, the bounded two-level pyramid does not beat its own
flat base-leg baseline (H-CW shape) on expectancy or worst-day loss, or if
the edge depends on a single symbol or single period. Refuted at programme
level if the joint-tail protocol shows the family's worst days systematically
coincide with the passive book's or the H-CW/H-MR sleeves' worst days (hidden
common mode — the family is the tail carrier and must not join a multi-sleeve
book).
...
7. The tightened mechanism never fires out of sample: level-2 reach < 2% of
   sealed baskets (the add machinery is dead weight and the card collapses to
   H-CW with extra parameters).
...
high-impact news blackout keeps FOMC/NFP/CPI windows closed. The session-flat
envelope is precisely the defence against the -10.26% demo breach pattern
(overnight/swap tails on carried positions), and the bounded two-level pyramid
adds convex trend capture inside it rather than a new tail. As the aggressive
