...
title: WTI monthly linear-recency weighted-return extraction from Time Series Momentum
...
approval_basis: decisions/2026-08-11_qm5_20278_wti_linw_mom_g0.md
...
# WTI Linear-Recency Return Momentum Source Packet
...
`decisions/2026-08-11_qm5_20278_wti_linw_mom_g0.md`. No new online page,
...
establish linear recency weighting, the exact integer vector, or performance.
...
twelve non-overlapping chronological monthly log returns. Multiply the oldest
return by one, the next by two, and so on through weight twelve for the newest
return. Divide the weighted sum by the fixed weight total 78. Buy when the
linear-recency mean is positive, sell when it is negative, and consume the
month flat when it is exactly zero or invalid. Renew at the next broker-month
boundary.

The linear weights, continuous-CFD carrier, broker-month endpoint
reconstruction, one-attempt ledger, `RISK_FIXED` sizing, ATR hard stop, spread
ceiling, and stale exit are transparent QM mechanizations. The paper does not
prescribe them. No source return, alpha, Sharpe ratio, drawdown, trade count,
...
w[i] = i + 1
weight_total = 1 + 2 + ... + 12 = 78
linear_weighted_mean = sum(w[i] * r[i]) / 78

signal = BUY  when linear_weighted_mean > 0
         SELL when linear_weighted_mean < 0
         FLAT when linear_weighted_mean == 0 or state is invalid
```

Chronology is load-bearing: weight one belongs to the oldest return and weight
twelve to the newest. The current decision month contributes no endpoint.
There is no sort and no fallback to an equally weighted cumulative return,
single horizon, nested horizon vote, quarterly sign vote, median, trimmed
mean, Winsorized mean, pairwise estimator, price-level regression, rank,
moving average, oscillator, calendar direction, external series, or prior
pipeline result.

## Non-Duplicate Boundary

The deterministic pre-allocation checker scanned 4,343 EA-registry rows and
454 cards. It found no exact identity and returned three expected shared-
source fuzzy matches to the WTI median-return, trimmed-mean, and Winsorized-
mean cards.

Those cards sort the twelve returns and therefore discard chronology before
aggregation. This rule never sorts: it gives every adjacent return a distinct
fixed chronological weight. The quarterly-block vote discards magnitude and
gives each block one vote; the OLS rule fits log-price levels and requires an
`R^2` gate. Exact-horizon WTI systems use one endpoint interval or sign
agreement. The index MAC(5) rule uses four daily SP500 returns, a contrarian
direction, and weights `4,3,2,1`; it is a different carrier, clock, horizon,
direction, and source contract.

The thirteen endpoints, twelve adjacent intervals, oldest-to-newest order,
weights `1..12`, total 78, symmetric trend direction, monthly attempt, and
...
- R2: PASS. Endpoint count and order, return intervals, every integer weight,
  divisor, direction, attempt, fixed risk, hard stop, spread cap, rollover,
  and stale exit are exact and mechanical.
- R3: PASS. Registered `XTIUSD.DWX` D1 history plus native MT5 calendar,
  ...
  input.
- R4: PASS. Deterministic logarithm, multiplication, addition, and native
...
linear-recency rule. Q02 must retire the card below five completed packages per
...
the horizon, weights, divisor, direction, carrier, stop, hold, spread cap, or
retry contract.
