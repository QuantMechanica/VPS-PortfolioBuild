...
title: XTI/XNG two-week common-shock relative-leader-switch continuation
...
approval_basis: decisions/2026-09-06_xtixng_common_shock_leader_switch_continuation_source_approval.md
parent_source_ids:
  - FMR-MOMTS-2010
...
cards_extracted:
  - xtixng-cs-leadswitch-cont
---

# XTI/XNG Two-Week Common-Shock Leader-Switch Continuation Source Packet
...
The sources support a falsifiable relative-value continuation question, but
they do not test a two-week leader-switch rule, a weekly CFD package, or this
specific continuation state. No source alpha, hedge ratio, frequency, cost,
...
Thus the package follows the newest relative winner only after two consecutive
...
decision-week price enters the signal, and return magnitude never scales risk.

The clock, three synchronized completed weeks, strict sign tests, strict
leader switch, newest-winner continuation, continuous-CFD carrier,
...
The canonical receipt covered 4,851 registry rows and 1,464 repository cards,
...
- `QM5_41368_xtixng-cs-leadswitch-rv` uses the same two-week common-shock
  formation but fades the newest winner. This candidate takes the opposite,
  source-aligned continuation direction.
- `QM5_41369_xtixng-cs-leadpersist-cont` follows the newest winner only when
  the same relative leader persists across both weeks. This candidate requires
  the winner to switch.
- `QM5_41367_xtixng-commonshock-cont` follows after one common-sign completed
  week and has no prior-week leader-rotation condition.
- `QM5_41366_xtixng-decouple-cont` requires opposite signs inside one
  completed week. This candidate requires same signs inside both completed
  weeks.
- `QM5_41362_xtixng-wdecel-cont` follows shared direction after a relative
  spread deceleration; it does not require a leader switch and does not trade
  the relative winner/loser pair.
...
- R2 `PASS`: synchronized endpoints, consecutive-week membership, two strict
  common-sign states, strict leader rotation, newest-winner continuation,
