...
title: WTI monthly fixed-tail Winsorized return-location extraction from Time Series Momentum
...
approval_basis: decisions/2026-08-11_qm5_20277_wti_winsor_mom_g0.md
...
# WTI Winsorized-Mean Momentum Source Packet
...
`decisions/2026-08-11_qm5_20277_wti_winsor_mom_g0.md`. No new online page,
...
They do not establish fixed-tail Winsorization or its performance.
...
twelve non-overlapping close-to-close monthly log returns and sort a copy
ascending. Replace sorted indexes 0 and 1 with sorted index 2, and replace
indexes 10 and 11 with sorted index 9. Average all twelve resulting terms with
divisor twelve. Buy when the Winsorized mean is positive, sell when it is
negative, and consume the month flat when it is exactly zero or invalid.
Renew at the next broker-month boundary.

The two-per-tail Winsorization, exact boundary indexes, continuous CFD
carrier, broker-month endpoint reconstruction, one-attempt ledger,
`RISK_FIXED` sizing, ATR hard stop, spread ceiling, and stale exit are
transparent QM mechanizations. The paper does not prescribe them. No source
...
signal = BUY  when winsor_mean > 0
         SELL when winsor_mean < 0
         FLAT when winsor_mean == 0 or state is invalid
```

The lowest and highest two observations remain in the twelve-term statistic
but are capped at the nearest retained boundary. This is not deletion: sorted
indexes 2 and 9 each contribute three times, while indexes 3 through 8 each
contribute once. The current decision month contributes no endpoint. There is
no fallback to a cumulative endpoint return, raw-return median, middle-eight
trimmed mean, pairwise pseudomedian, binary sign count, fixed-block vote,
price slope, regression, moving average, oscillator, calendar direction,
external series, or prior pipeline result.

## Non-Duplicate Boundary

The deterministic pre-allocation checker scanned 4,342 EA-registry rows and
453 cards. It found no exact identity and surfaced only the expected fuzzy
matches to the same-source trimmed-mean and raw-median cards.

Manual review separates this rule from `QM5_20270`, which deletes four tail
observations and divides the middle-eight sum by eight. The new rule retains a
twelve-term denominator and caps rather than deletes the tails, giving the
boundary observations three weights each. `QM5_20269` uses only two central
observations, while `QM5_20276` forms and takes the median of 78 inclusive
pairwise averages. Other WTI trend cards use cumulative endpoints, signs,
blocks, slopes, regression, rank, or path efficiency.

The thirteen endpoints, twelve disjoint intervals, ascending sort, boundary
indexes 2 and 9, two replacements per tail, twelve-term divisor, symmetric
direction, monthly attempt, and renewal clock are jointly load-bearing.
Verdict: `CLEAN_FIXED_TAIL_WINSORIZED_RETURN_LOCATION`.
...
- R2: PASS. Endpoint count and order, return intervals, sort, replacement
  count, boundary indexes, divisor, direction, attempt, fixed risk, hard stop,
...
- R3: PASS. Registered `XTIUSD.DWX` D1 history plus native MT5 calendar,
  ...
  input.
- R4: PASS. Deterministic logarithm, sorting, fixed tail replacement, and
  native execution state only; no trained output, prohibited signal
...
Winsorized return-location rule. Q02 must retire the card below five completed
...
rescued by changing the horizon, tail count, boundary indexes, estimator,
