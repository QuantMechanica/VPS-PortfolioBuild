...
title: WTI exact six-completed-month trend with a fixed two-month holding clock
...
approval_basis: decisions/2026-09-07_wti_six_month_momentum_two_month_hold_source_approval.md
...
# WTI Six-Month Trend / Two-Month Hold
...
and volatility scaling. It does not publish a standalone WTI `k=6, h=2`
...
seven consecutive completed month-end closes `C[0]..C[6]`, oldest to newest.
Compute `r6 = ln(C[6] / C[0])`. Buy if `r6 > 0`, sell if `r6 < 0`, and consume
...
`QM5_20059_wti-tsmom6m` observes the same six-month return but closes and
renews monthly. `QM5_20281_wti-tsmom-h2` shares the fixed odd-month lifecycle
but observes twelve completed months. `QM5_41379`, `QM5_41380`, and
`QM5_41381` observe three, one, and nine completed months. The new identity
requires the exact combination of seven endpoints, six-month orientation,
fixed odd-month epoch, no even-month action, one consumed bimonthly attempt,
and two-month renewal.
