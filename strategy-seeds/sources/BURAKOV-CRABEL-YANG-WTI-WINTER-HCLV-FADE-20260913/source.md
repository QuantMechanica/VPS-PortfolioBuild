...
title: WTI winter upper-tercile weekly-close reversion
status: approved_source_complete
source_type: governed_peer_reviewed_reputable_and_academic_cross_source_mechanization
approval_basis: decisions/2026-09-13_wti_winter_upper_clv_reversion_source_approval.md
...
# WTI Winter Upper-Tercile Weekly-Close Reversion
...
positive WTI seasonal leg. The governed Crabel composite provides reproducible
...
prices, different weekly range patterns, or exchange-traded futures and do not
establish an upper-tercile completed-week settlement, a one-week WTI CFD short,
fixed-dollar ATR risk, or Darwinex execution. Selling inside the source's
positive winter regime is explicitly counter-seasonal: seasonality defines the
structural state while reversal lineage supplies only the hypothesis class.
This is a transparent pre-result QM hypothesis. No source return,
...
- R1 `PASS_WITH_CROSS_SOURCE_HORIZON_AND_COUNTER_SEASONAL_TRANSLATION_RISK`:
  ...
  lineage, and academic commodity reversal; the exact conjunction and
  counter-seasonal direction are explicitly untested.
...
The canonical pre-allocation checker found no exact identity across 4,948
registry rows and 1,557 repository cards. It reported five fuzzy family
matches and recorded the unavailable external Strategy Wiki mount.
`QM5_41442` and `QM5_41456` aggregate two completed weeks and impose strict
range-rank predicates with symmetric outer-quartile signals. `QM5_41444` and
`QM5_41446` use two-week range state plus body direction. This candidate
aggregates one week, does not rank range, ignores body direction, and keys only
on a strict upper-tercile close.

The closest one-week siblings are deliberately distinct. `QM5_41462` sells
upper-tercile closes only during the disjoint June-October summer interval.
`QM5_41463` buys the same winter upper-tercile state under continuation
lineage. Carrier, November-May clock, one-week package, strict upper-tercile
settlement, short-only orientation, and one-week lifecycle are jointly
load-bearing.
