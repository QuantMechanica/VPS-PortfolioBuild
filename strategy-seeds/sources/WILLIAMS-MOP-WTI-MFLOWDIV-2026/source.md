...
title: WTI monthly public/session information-flow divergence rule
...
# WTI Monthly Public/Session Flow Divergence Source Packet
...
new-month entry, session-following direction, or a one-month hold. Moskowitz,
...
`WILLIAMS-MOP-WTI-MFLOWDIV-2026_S01` is one predeclared direct-WTI package:
...
The canonical pre-card checker scanned 4,522 EA-registry rows and 618 card
...
- `QM5_41033_wti-flow-dom` follows the reconciled total/dominant component of
  one opposed-flow week; this packet always follows the session component and
  may oppose the completed-month total;
- `QM5_20187_wti-tsmom1m` follows every nonzero completed-month total; this
  packet rejects agreement months and does not use total-flow direction;
...
opposition, reconciliation, session-following direction, first-new-month
decision, and next-month renewal are the auditable identity. A failed result
may not be rescued by admitting agreement, following total flow, adding a
...
`decisions/2026-08-17_wti_monthly_flow_divergence_source_approval.md` authorize
...
| v1-q02 | 2026-08-17 | one paced target-only WTI baseline row created | Q02 | ENQUEUED |
