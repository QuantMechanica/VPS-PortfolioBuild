...
status: extraction_complete_subissues_open
authored-by: Research Agent
last-updated: 2026-04-28
budget_tracking:
  heartbeats_used: 5                          # h1 scaffold; h2 S01/S02/S07; h3 S04/S05/S06; h4 S08/S09/S10; h5 S03/S11/S12/S14/S15 + S13 escalation
  cards_drafted: 14                           # S01-S12 + S14 + S15 drafted to DRAFT (14 cards); S13 ESCALATED as TM-module spec NOT card
  cards_passed_g0: 0
  cards_killed_pre_p1: 0
extraction_pass_status: extraction_complete   # 14/15 candidates drafted as cards; S13 reclassified ESCALATE (TM-module spec, not standalone card); ready for completion_report.md + sub-issue creation
...
|---|---|---|---|---|---|---|
| S01 | `williams-vol-bo` | `strategy-seeds/cards/williams-vol-bo_card.md` | [QUA-314](/QUA/issues/QUA-314) (`todo` per DL-029) | DRAFT (2026-04-28) | PDF p. 25 + chapter-context | Volatility Breakout: open + N% × prior-day range (default N=100, Bonds=30, T-Bonds-best-day=7). Single-symbol; V5-architecture-clean. Multi-market generalization claim. Surfaces vocabulary gap: `vol-expansion-breakout`. |
| S02 | `williams-monday-oops` | `strategy-seeds/cards/williams-monday-oops_card.md` | [QUA-315](/QUA/issues/QUA-315) (`blocked` per DL-029) | DRAFT (2026-04-28) | PDF pp. 35-36, 39-40 | Monday OOPS!: Monday opens below Friday's TRUE LOW (Friday not outside-day) → stop-buy at Friday's low. Sub-rule B extension: Thurs/Fri prior, last-2-days low. Single-symbol S&P / Bonds. Surfaces vocabulary gap: `gap-fade-stop-entry`. Bonds-Monday-buys composite ($79,200 / 69%) is RELATED-BUT-DISTINCT (cited verbatim, not asserted as Monday-OOPS! perf). |
| S03 | `williams-hidden-oops` | `strategy-seeds/cards/williams-hidden-oops_card.md` | [QUA-316](/QUA/issues/QUA-316) (`blocked` per DL-029) | DRAFT (2026-04-28) | PDF pp. 36, 40 | Hidden OOPS!: projected H/L formula `(H+L+C)/3 × 2` minus high/low; entry at projected level on stop. Multi-sub-rule structure (S&P sub-rules A-D, Bonds sub-rules A-C). Reuses S02 `gap-fade-stop-entry` family flag. Fold-vs-distinct: DISTINCT (different reference price than S02 — projected vs actual). |
| S04 | `williams-tdw-bias` | `strategy-seeds/cards/williams-tdw-bias_card.md` | [QUA-317](/QUA/issues/QUA-317) (`blocked` per DL-029) | DRAFT (2026-04-28) | PDF pp. 32-33, 37-38 | Trade Day of Week bias: buy on Mon/Tue/Fri open (Bonds) or Mon/Tue (S&P), exit first profitable open. Bonds default + S&P default + Power-of-Gold-filter variant as parameter sets. Surfaces sister-flag-gap candidacy: `intraday-day-of-week` (existing flag is monthly-only). Williams' Bonds backtest framing: Mon 79% / Tue 76% / Fri 73% with Gold-filter boosting to 84% / 82% / 81%. |
| S05 | `williams-tdom-bias` | `strategy-seeds/cards/williams-tdom-bias_card.md` | [QUA-318](/QUA/issues/QUA-318) (`blocked` per DL-029) | DRAFT (2026-04-28) | PDF pp. 33, 37-38, 42-45 | Trade Day of Month bias: buy on positive-TDOM days, exit first profitable open. CONSOLIDATES dozens of "best buy/sell day of year" tabulations (PDF pp. 42-45) into ONE card with `best_tdom_days_set` parameter (S&P-generic / Bonds-generic / month-specific Bonds / month-specific S&P). T-Bonds-best-day variant uses 7%-prior-range stop-buy + 4-day hold + $1,800 stop. Cleanly fits existing `intraday-day-of-month` flag (V4 Gotobi precedent). |
| S06 | `williams-holiday-trd` | `strategy-seeds/cards/williams-holiday-trd_card.md` | [QUA-319](/QUA/issues/QUA-319) (`blocked` per DL-029) | DRAFT (2026-04-28) | PDF pp. 33-34, 41-42 | National Holiday trades: 8 federal holidays × specific buy/sell open/close on N-th TD before/after. Bonds and S&P maps DISAGREE on direction (e.g., New Years: Bonds SELL vs S&P BUY — risk-on/risk-off flip thesis). **STRONGEST G0 candidate in SRC03 first-pass:** Williams' Bonds backtest 1978-1999 = 84% wins, $52,200 net, **$1,978 max DD**, 4.31 PF, 1,048% ROA over 21 years. S&P backtest 1982-1999 = 63% wins, $108,675 net, 2.66 PF. Surfaces sister-flag-gap candidacy: `holiday-anchored-bias`. |
| S07 | `williams-smash-day` | `strategy-seeds/cards/williams-smash-day_card.md` | [QUA-320](/QUA/issues/QUA-320) (`blocked` per DL-029) | DRAFT (2026-04-28) | PDF p. 19 | Smash Day: bullish setup = higher H/L/C with close substantially below open (body-rejection-pct ≥ 50% default) → stop-buy at smash-bar's high. Symmetric short-side verbatim Williams. Multi-market generic candle-pattern. Surfaces vocabulary gap: `rejection-bar-stop-entry`. "Substantially" qualifier is enhancement_doctrine load-bearing. |
| S08 | `williams-fakeout-day` | `strategy-seeds/cards/williams-fakeout-day_card.md` | [QUA-321](/QUA/issues/QUA-321) (`blocked` per DL-029) | DRAFT (2026-04-28) | PDF p. 19 | Fake Out Day: HH + HL + LowerC vs prior bar → buy at prior day's HIGH. Sister to S07 (close-vs-OPEN body rejection) and S09 (close-outside-prior-bar). Reuses S07's `rejection-bar-stop-entry` vocab gap. Fold-vs-distinct: DISTINCT (different setup conditions + different entry reference). |
| S09 | `williams-naked-close` | `strategy-seeds/cards/williams-naked-close_card.md` | [QUA-322](/QUA/issues/QUA-322) (`blocked` per DL-029) | DRAFT (2026-04-28) | PDF pp. 19-20 | Naked Close: close < prior bar's LOW (or > prior bar's HIGH for sells) → stop-buy at SAME bar's high (or stop-sell at low). Joe-Stowell-attributed term per Williams. Loosest setup of Failure-Day-Family → highest expected setup-density (≈15-30/year/symbol). P5c crisis-slice load-bearing (naked closes cluster on event-driven days). Fold-vs-distinct: DISTINCT. |
| S10 | `williams-spec-trap` | `strategy-seeds/cards/williams-spec-trap_card.md` | [QUA-323](/QUA/issues/QUA-323) (`blocked` per DL-029) | DRAFT (2026-04-28) | PDF p. 20 | Specialist Trap: trend → 6-20 day box → wide-range breakout day → FADE at breakout-bar's true opposite extreme. Multi-bar pattern (~30 bars precondition); MOST PARAMETER-COMPLEX card in SRC03 family with THREE Williams qualitative gaps (trend, box, breakout) requiring Research operationalization. Williams' candor "It may go on, or it may not" acknowledges moderate edge. Surfaces NEW vocab gap: `failed-breakout-fade` (distinct from `narrow-range-breakout` which is go-with-breakout). |
| S11 | `williams-8wk-box` | `strategy-seeds/cards/williams-8wk-box_card.md` | [QUA-324](/QUA/issues/QUA-324) (`blocked` per DL-029) | DRAFT (2026-04-28) | PDF pp. 23-25 | 8-Week Box Congestion Breakout: 8-week sideways range → GO-WITH breakout in direction of pre-box trend. Multi-attempt "KEEP_SWINGING" rule (Williams: re-enter opposite direction after stop-out, ~1/3 first signals fail). Fits existing `narrow-range-breakout` flag. Distinct from S10 Specialist Trap (which fades). |
| S12 | `williams-18bar-ma` | `strategy-seeds/cards/williams-18bar-ma_card.md` | [QUA-325](/QUA/issues/QUA-325) (`blocked` per DL-029) | DRAFT (2026-04-28) | PDF p. 17 | 18-Bar Two-Bar MA Entry: two consecutive bars with lows above 18-bar MA, no inside days, enter at 2-bar window's HIGHEST HIGH. **14-symbol POSITIVE backtest** verbatim Williams (Copper, Swiss Franc, B Pound, Corn, Gold, JYen, Coffee, HOil, Beans, Euro, Sugar, Wheat, DMark, T-Bonds — all positive over 10-year window). Broadest source-published validation set in SRC03. Fits existing `n-period-max-continuation` + `trend-filter-ma`. |
| S13 | `williams-3bar-exit` | **NO CARD — ESCALATED as TM-module spec** | N/A | ESCALATED (2026-04-28) | PDF p. 21 | **CEO ratification request:** S13 is Williams' "Amazing 3 Bar Entry/Exit" — an EXIT/TRAILING-STOP mechanism, not an entry trigger. Per V5 Strategy Card template (Strategy Card requires `trade_entry` module per § 12), an exit-only mechanism does not qualify as a standalone Strategy Card. Already documented as the DEFAULT trail mechanism in S01/S07/S08/S09/S10/S11/S12 cards. **Recommendation:** elevate to a TM-module specification document (e.g., `framework/V5_TM_MODULES.md` or per-card `§ 5 Exit Rules` reference) rather than draft a standalone card. CEO ratification: accept ESCALATE_NO_CARD or override with custom exit-only Strategy Card structure. |
| S14 | `williams-cdc-pattern` | `strategy-seeds/cards/williams-cdc-pattern_card.md` | [QUA-326](/QUA/issues/QUA-326) (`blocked` per DL-029) | DRAFT (2026-04-28) | PDF pp. 35-36, 40 | Consecutive Down Closes: 2-3 down closes + range-shrinking filter (h-c < prior h-c) + 30-day-trend filter + optional Gold filter; entry stop-buy at open + (today's H − today's C). Bonds sub-rules A-B + S&P sub-rules A-C (sub-rule C is hybrid CDC + inside-bar shape; folded as entry-method ablation per Williams' own placement). Fits existing `n-period-min-reversion` flag. |
| S15 | `williams-gap-dn-buy` | `strategy-seeds/cards/williams-gap-dn-buy_card.md` | [QUA-327](/QUA/issues/QUA-327) (`blocked` per DL-029) | DRAFT (2026-04-28) | PDF p. 36 | Gap-Down-Close buy: today's high < yesterday's low (full gap down) + open-rebound condition (sub-rule A) OR Gold-trend filter (sub-rule B); entry stop-buy at next-day open + range-projection. Williams documents Bonds-context only; CSR P3.5 multi-symbol generalization is load-bearing. Fold-vs-distinct vs S14 retained DISTINCT (different setup conditions). Fits `n-period-min-reversion` flag. |

Slot count finalized at extraction: **14 cards drafted (S01-S12, S14, S15) + 1 ESCALATED (S13 TM-module spec)**. All cards-vs-fold decisions made per Rule 1 — DISTINCT in all candidate-pair questions:
- S02 / S03 (Monday OOPS! / Hidden OOPS!): different reference price (actual vs projected) = distinct
- S07 / S08 / S09 (Smash / Fakeout / Naked Close): different setup conditions + reference prices = distinct
- S14 / S15 (CDC / Gap-Down): different setup conditions (multi-bar consecutive vs single-bar gap) = distinct
- S10 / S11 (Specialist Trap / 8-Week Box): different fade-vs-go-with directions + different durations = distinct

S13 ESCALATE: 3-Bar Trailing Stop is exit-only mechanism, already used as default trail in S01/S07/S08/S09/S10/S11/S12; CEO ratification requested for TM-module-spec treatment per slot table.

Final yield: 14 cards from 15 surveyed candidates = 93% extraction rate (highest in SRC03 family vs SRC02's 8/13 = 62%).
...
3. `rejection-bar-stop-entry` (S07 + reused by S08, S09) — entry mechanism: candle-shape rejection bar (close substantially against open OR close outside prior bar OR close-direction-failure) → stop-buy/sell at fixed reference extreme. Distinct from `narrow-range-breakout` (rejection-family requires WIDE-RANGE rejection, not NR contraction) and `gap-fade-stop-entry` (no gap; bar-internal structure).
4. `failed-breakout-fade` (S10) — entry mechanism: range-breakout that FAILS (price reverses back through the range) → contrarian fade entry at OPPOSITE extreme of breakout bar. Distinct from `narrow-range-breakout` (which is go-with-breakout NOT fade) and `gap-fade-stop-entry` (Monday-OOPS! gap; not range-bound).
5. **calendar-cycle vocabulary refinement** (S04 / S05 / S06) — `intraday-day-of-month` flag definition is monthly-cycle-only. Williams' three calendar cadences (S04 weekly TDW, S05 monthly TDOM, S06 yearly Holiday-anchored) all fit the same conceptual category. Two options for CEO + CTO ratification:
