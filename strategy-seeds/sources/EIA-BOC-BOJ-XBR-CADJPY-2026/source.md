...
# Brent/CADJPY Oil-FX Relative-Value Source
...
- `xbr-cadjpy-rspr`: D1 two-leg `XBRUSD.DWX` / `CADJPY.DWX` return-spread
  z-score reversion. The basket treats Brent as the energy leg and CADJPY as an
...
- R3 data available: PASS. `XBRUSD.DWX` and `CADJPY.DWX` exist in the DWX
  symbol matrix.
