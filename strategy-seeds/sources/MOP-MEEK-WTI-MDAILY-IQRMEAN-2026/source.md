...
title: WTI completed-month daily-return interquartile-mean momentum extraction
...
approval_basis: decisions/2026-08-23_wti_monthly_daily_iqr_mean_momentum_source_approval.md
parent_source_ids:
  - MOP-TSMOM-2012
...
cards_extracted:
  - wti-mdaily-iqrmean-mom
---

# WTI Completed-Month Daily-Return Interquartile-Mean Momentum Source Packet
...
committed before this extraction at `81ca87515`. No blocked page,
...
while exposing whether its direction survives removal of the completed
month's large positive and negative daily shocks. They do not establish the
exact interquartile mean, a WTI-only next-month result, or a Darwinex CFD
...
The fail-closed canonical checker scanned 4,633 registry identities, 1,301
...
- `QM5_20270_wti-trimmean-mom` averages the middle eight of twelve disjoint
  monthly returns spanning a year. This extraction averages the dynamically
  sized middle band of one completed month's 17-23 daily returns.
- `QM5_41111_wti-mdaybreadth-mom` counts daily signs and requires endpoint
  agreement; this extraction retains central magnitudes and has no endpoint
  gate.
- `QM5_41127_wti-mdaily-persist-mom` estimates adjacent demeaned-return
  ...
  after exact reconstruction and follows an order-statistic average.
- `QM5_41131_wti-mdaily-tailtrim-mom` deletes exactly one observation per
  tail and sums 15-21 remaining returns. This extraction removes four or five
  observations per tail and averages only 9-13 central observations.
- `QM5_41132_wti-mweekday-med-mom` averages five ending-weekday buckets and
  takes their median. This extraction has no bucket or weekday state.
- `QM5_41133_wti-mdaily-median-mom` uses only the exact one- or two-return
  center. This extraction uses a materially wider 9-13-observation band.
- earlier trim, Winsor, MAD-cap, trimean, pseudomedian, Huber, and bisquare
  WTI cards transform twelve monthly returns rather than the latest month's
  daily sample.
...
risk, and next-month lifecycle are jointly load bearing. Verdict:
...
- R1: `PASS_WITH_WITHIN_MONTH_IQR_MEAN_TRANSLATION_RISK`. The lineage
  ...
  complete-read receipt, durable PDF hash, and explicit WTI membership plus
  a named-author, peer-reviewed open-access WTI daily-return paper with
  complete-read evidence. The exact central-band estimator is untested.
...
sample, trim formula, direction, carrier, risk, hold, or by adding endpoint
