...
title: WTI monthly information-flow agreement continuation rule
...
approval_commit: ddb43e0da
strategy_ids:
  - WILLIAMS-MOP-WTI-MFLOWAGREE-2026_S01
cards_extracted:
  - wti-mflow-agree
parent_sources:
  - SRC03
...
# WTI Monthly Information-Flow Agreement Source Packet
...
1. Larry R. Williams (1999), *Long-Term Secrets to Short-Term Trading*, Wiley
   ...
   discusses separately accumulated lines, divergences, and crossings.
...
Williams does not test WTI, monthly aggregation, strict component agreement,
new-month entry, or a one-month hold. Moskowitz, Ooi, and Pedersen do not
decompose returns by information time or support the agreement gate. Neither
source tests the conjunction, Darwinex continuous CFDs, normalized broker
labels, fixed cash risk, or an ATR stop. No source performance, significance,
cost, density, drawdown, WTI-only efficacy, CFD equivalence, correlation, or
portfolio result transfers.

## Bounded Mechanization

`WILLIAMS-MOP-WTI-MFLOWAGREE-2026_S01` is one predeclared direct-WTI package:
...
- require both component sums to share one strict sign and reconcile their sum
  to the completed month-end-to-month-end log return;
- BUY for two positive sums and SELL for two negative sums; opposition, exact
...
agreement, reconciliation, direction, 180-minute attachment boundary, risk,
stop, spread, and lifecycle are disclosed QM choices. The sources do not test
this interaction or Darwinex carrier.
...
bar enters no signal term. Signal magnitude never changes size.
...
- R2 `PASS`: exact month identity, normalized labels, all completed
  close/open endpoints, strict agreement, reconciliation, direction, attempt
...
files. It found no exact identity and raised the expected weekly
`QM5_41029_wti-flow-agree` family neighbor. Manual review returned
`CLEAN_WTI_MONTHLY_INFORMATION_FLOW_AGREEMENT_AFTER_FAMILY_REVIEW`:

- `QM5_41029_wti-flow-agree` forms on one exact Monday-Friday week, decides the
...
- `QM5_20187_wti-tsmom1m` follows every nonzero completed-month total; this
  packet rejects all months whose information-time components oppose;
- `QM5_41032_wti-flow-div` and `QM5_41033_wti-flow-dom` require weekly
  component opposition, a disjoint state and cadence;
- `QM5_41023_wti-mends-mom` compares two close-to-close boundary segments and
...
agreement, reconciliation, first-new-month decision, and next-month renewal
are the auditable identity. A failed result may not be rescued by admitting
opposition, adding a threshold/filter, moving the clock, changing the
formation month, or shortening/extending the hold.

## Safety And Extraction Boundary

The OWNER mission and
`decisions/2026-08-17_wti_monthly_flow_agreement_source_approval.md` authorize
...
identity or endpoints, current-bar leakage, entry on component opposition,
...
| v1-capacity | 2026-08-17 | exact-path factory sample reached 7/7; queue left untouched | Q02 | NOT_ENQUEUED_CPU_CEILING |
