...
title: WTI summer upper-tercile weekly-close reversion
status: approved_source_complete
source_type: governed_peer_reviewed_reputable_and_academic_cross_source_mechanization
approval_basis: decisions/2026-09-12_wti_summer_upper_clv_reversion_source_approval.md
...
# WTI Summer Upper-Tercile Weekly-Close Reversion
...
prices or exchange-traded futures and do not establish an upper-tercile
completed-week settlement, a one-week WTI CFD short, fixed-dollar ATR risk, or
Darwinex execution. This is a transparent pre-result QM hypothesis. No source
return, significance, causality, portfolio correlation, or live-fitness claim
transfers.
...
5. Sell only when `CLV > 2/3`. Equality, zero range, malformed history, or an
   ineligible month is flat. There is no long side, weekly range rank, or
...
The five-month interval contains roughly twenty-two weekly decisions. An
upper-tercile close is expected to produce about seven positions per full
...
- R1 `PASS_WITH_CROSS_SOURCE_AND_HORIZON_TRANSLATION_RISK`: peer-reviewed WTI
  ...
  commodity reversal; the exact conjunction is explicitly untested.
...
The canonical pre-allocation checker found no exact identity across 4,943
registry rows and 1,552 repository cards. It reported only `QM5_41457` and
`QM5_41458` as fuzzy family matches and recorded the unavailable external
Strategy Wiki mount. Both siblings aggregate two completed weeks, compare
newest range with prior range, and require a positive newest-week body. This
candidate aggregates one week, does not rank range, ignores body direction,
and keys only on a strict upper-tercile close.

`QM5_20093` is unconditional in each summer month. The closest CLV summer
sibling, `QM5_41461`, uses the disjoint lower tercile and continuation
lineage; it cannot signal on the same valid observation. Carrier,
June-October clock, one-week package, strict upper-tercile settlement,
