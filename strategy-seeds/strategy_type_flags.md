...
### bband-reclaim
- **Definition**: Entry mechanism on a CLOSE back across a Bollinger band (typically the inner ±N·σ band) AFTER a multi-bar precondition that established the price as having dwelled on the OUTER side of the band. Long: price was outside the lower band per the precondition, then closes back across (above) it; short mirror at the upper band. Strategy-Card-level parameter: `precondition_mode ∈ {outer-band-zone, n-bars-opposite-1sigma}` — `outer-band-zone` (S02a-style: most recent bar(s) trade between the inner-σ and outer-σ bands, defining a range-bound "outer-band-zone dwell") vs `n-bars-opposite-1sigma` (S02b-style: K consecutive prior bars closed on the OUTER side of the inner-σ band). Distinct from `zscore-band-reversion` because the trigger is a CLOSE BACK INSIDE the band, not the original CROSS OUT — opposite direction of the same band geometry.
- **V4 examples**: None — V4 surviving sleeves had no Bollinger-reclaim entry EAs per the Mining-provenance table below; this flag was added in the SRC04 batch (CEO ratified 2026-04-28 in QUA-333 closeout) on first deployment.
- **SRC04 example**: `lien-dbb-pick-tops` (SRC04_S02a, `strategy-seeds/cards/lien-dbb-pick-tops_card.md` Header / § 4 Entry Rules) — Lien Ch 9 PDF pp. 103-107 "Double Bollinger Band PICK TOPS" range-mode mean-reversion entry: close back across 1st-σ band after outer-band-zone dwell (`precondition_mode = outer-band-zone`). Also `lien-dbb-trend-join` (SRC04_S02b, `strategy-seeds/cards/lien-dbb-trend-join_card.md`) — Lien Ch 9 PDF pp. 107-110 "Double Bollinger Band TREND JOIN" trend-mode breakout entry: close back across 1st-σ band after K consecutive prior bars closed on outer side (`precondition_mode = n-bars-opposite-1sigma`). Sibling cards share this flag with per-card disambiguation via `precondition_mode` parameter; co-regime-fire suppression documented in S02b card § 6.
- **Disambiguation from**: `zscore-band-reversion` (entry on the original CROSS OUT of ±N·σ; bband-reclaim is the opposite — entry on the CLOSE BACK INSIDE after a dwell precondition); `n-period-min-reversion` (N-bar minimum-extreme reference, not Bollinger-band reference; long-bias only, not long-and-short symmetric); `failed-breakout-fade` (multi-bar trend + box + breakout precondition with contrarian fade at the OPPOSITE breakout extreme; bband-reclaim is the band itself reclaimed, not a fade of a separate breakout structure); `gap-fade-stop-entry` (calendar-pattern + gap-through reference; bband-reclaim has no calendar or gap requirement).

### round-num-fade
- **Definition**: Entry mechanism placing a stop-buy / stop-sell at a fixed pip offset from an ABSOLUTE psychological round-number price (e.g., xx.00 / x.x000 / x.x500 in major-FX quotes), conditioned on a counter-trend MA-direction precondition (long fades a downmove approaching a round number from above; short mirror). Strategy-Card-level parameters: `round_number_modulus` (e.g., 100 pips, 50 pips, 1.0000), `entry_offset_pips` (Lien default 10–15p), `counter_trend_ma_period` (Lien default 20MA on entry timeframe), `stop_pips` (Lien default 20p). Reference price is the absolute round-number level itself, not a derived range or N-bar extreme.
- **V4 examples**: None — V4 surviving sleeves had no round-number-anchored stop-entry EAs per the Mining-provenance table below; this flag was added in the SRC04 batch (CEO ratified 2026-04-28 in QUA-333 closeout) on first deployment.
- **SRC04 example**: `lien-fade-double-zeros` (SRC04_S03, `strategy-seeds/cards/lien-fade-double-zeros_card.md` Header / § 4 Entry Rules) — Lien Ch 10 PDF pp. 112-115 "Fading the Double Zeros" round-number psychological-level fade on M15 with 20MA counter-trend filter, 10–15-pip entry offset from the round-number level, 20-pip stop. Per-trade pip P&L on 3 worked examples (no aggregate backtest).
- **Disambiguation from**: `vol-expansion-breakout` (forward stop-entry at next-bar open + N% × prior-bar range; round-num-fade uses an ABSOLUTE round-number anchor, no range scaling and no go-with direction); `gap-fade-stop-entry` (calendar-pattern + gap-through reference; round-num-fade has no calendar-pattern or gap-through requirement); `n-period-min-reversion` (N-bar minimum extreme as reference; round-num-fade uses an absolute psychological level independent of recent bar history); `failed-breakout-fade` (multi-bar trend + box + breakout precondition; round-num-fade requires only the round-number proximity + counter-trend MA filter); `zscore-band-reversion` (statistical band of the symbol's own moving statistics; round-num-fade is anchored to fixed psychological levels independent of recent volatility).

### ma-stack-entry
- **Definition**: Entry trigger fires when K consecutive moving averages of increasing periods are in MONOTONIC SEQUENTIAL ORDER on the entry timeframe (long: `SMA(P1) > SMA(P2) > ... > SMA(PK)` for `P1 < P2 < ... < PK`; short mirror with reversed inequality). The entry typically fires `N` candles AFTER initial stack formation if the stack still holds (Lien's lag confirms the regime is durable, not a one-bar fluke). Strategy-Card-level parameters: `ma_periods` (Lien canonical case K=5 with periods 10/20/50/100/200), `ma_type` (SMA vs EMA — Lien default SMA), `confirmation_lag_bars` (default N=2 per Lien). Distinct from a single-MA filter because the signal is the ORDER of K MAs, not crossing of price vs one MA.
- **V4 examples**: None — V4 surviving sleeves had no MA-stack-as-entry EAs per the Mining-provenance table below (V4 used MAs as filters / overlays, not as the entry trigger); this flag was added in the SRC04 batch (CEO ratified 2026-04-28 in QUA-333 closeout) on first deployment.
- **SRC04 example**: `lien-perfect-order` (SRC04_S09, `strategy-seeds/cards/lien-perfect-order_card.md` Header / § 4 Entry Rules) — Lien Ch 16 "The Perfect Order Trade" using the canonical 5-MA stack (10, 20, 50, 100, 200) on the entry timeframe with N-candle confirmation lag once the perfect-order stack first prints.
- **Disambiguation from**: `trend-filter-ma` (single MA filter — e.g., long-only when Close > SMA(200) — used as a regime overlay over a different base entry; ma-stack-entry IS the entry trigger, not an overlay); `cross-sectional-decile-sort` (universe-ranked relative-strength across symbols; ma-stack-entry is single-instrument time-series geometry); `vol-regime-gate` (vol-bucket classifier as a filter, not an MA structure); `regime-filter-multi` (composite multi-signal regime classifier; ma-stack-entry is the specific MA-order-as-entry mechanism).
...
### SRC04 batch (3 entry flags) — CEO ratified 2026-04-28 (QUA-333 closeout, QUA-351 back-port)

CEO ratified the following 3 flag additions on the QUA-333 (SRC04 Lien extraction) closeout (comment `6457c9fc`), applying the addition-process documented above (V4-or-new-source citation + Research issue + CEO/CTO sign-off). Back-port into this file tracked under QUA-351. Source: Lien, Kathy (2009). *Day Trading and Swing Trading the Currency Market: Technical and Fundamental Strategies to Profit from Market Moves*, 2nd ed. Wiley Trading.

| Section | Flag | Card sourcing it | Notes |
|---|---|---|---|
| A. Entry-mechanism | `bband-reclaim` | SRC04_S02a `lien-dbb-pick-tops`, SRC04_S02b `lien-dbb-trend-join` | Close back across inner ±1·σ Bollinger band after a multi-bar dwell on the OUTER side. Per-card disambiguation via `precondition_mode ∈ {outer-band-zone, n-bars-opposite-1sigma}`; co-regime-fire suppression S02a→S02b documented in S02b card § 6. |
| A. Entry-mechanism | `round-num-fade` | SRC04_S03 `lien-fade-double-zeros` | Stop-buy/sell at fixed pip offset from absolute round-number price (xx.00 / x.x000 / x.x500), with counter-trend MA filter; major-FX-calibrated pip thresholds. |
| A. Entry-mechanism | `ma-stack-entry` | SRC04_S09 `lien-perfect-order` | K consecutive moving averages in monotonic sequential order (Lien canonical K=5 periods 10/20/50/100/200); entry fires N candles AFTER initial stack formation if stack still holds. |

CTO technical-correctness ratification of the SRC04 §A definitions remains pending (rolls up with the prior-batch CTO ratification).

---

## Future-vocab-watches

This section records single-anchor candidate flags that appeared in the most recent source extraction but have NOT been ratified, pending a second-instance deployment precedent in a future SRC. Per the addition-process at the top of this section, a flag generally requires either (a) an explicit V4 SM_XXX deployment example or (b) a clear V5 net-new entry with at least two card anchors across sources (or a single anchor with a strong CEO ratification on first deployment, used sparingly). The watches below are deferred to the second-anchor pattern: they will be ratified when a future SRC0N+ card adds the missing second instance.

### Watch (i) — `adx-trend-confirm-gate` + `adx-range-mr-gate` (paired)

- **Provisional definition (paired)**: ADX-derived regime gates that pair an ADX-rising-and-above-threshold "trend-confirm" gate with an ADX-falling-or-below-threshold "range-MR" gate, used as filters distinguishing trend-mode entries from range-mode mean-reversion entries on the same market. The two flags are PAIRED (proposed together) so a card using one can declare the symmetrical complement.
- **Single-anchors (SRC04)**: `lien-channels` (SRC04_S06, `strategy-seeds/cards/lien-channels_card.md`) — Lien Ch 13 channel-trade range-mode MR using ADX as a range-confirm gate. `lien-perfect-order` (SRC04_S09, `strategy-seeds/cards/lien-perfect-order_card.md`) — Lien Ch 16 perfect-order trend-confirm using ADX-rising as a trend-mode gate. Two SRC04 anchors but on the SAME-source / SAME-author basis; CEO deferred ratification pending a third instance from a different source.
- **Ratification trigger**: A SRC0N (N ≥ 5) card surfacing either ADX-trend-confirm or ADX-range-MR gating from a non-Lien source — this provides cross-source confirmation that the paired-gate idea is not an idiosyncratic Lien convention. On that third instance, ratify both flags as a paired addition.
- **Holding pattern in cards**: SRC04_S06 and SRC04_S09 cards record the ADX gate usage in their § 6 Filters as a card-level parameter without a controlled-vocabulary flag; cross-link to this watch.

### Watch (ii) — `yield-spread-regime-filter`

- **Provisional definition**: Filter that gates entry on the level or sign of a sovereign yield-spread (e.g., 2Y/10Y, 10Y minus front-end, intra-curve relative-value) used as a macro-regime overlay on a single-instrument FX or rates strategy. Distinct from `carry-direction` (which uses BROKER swap differential at the FX-pair level, not a separately observed yield-spread series).
- **Single-anchor (SRC04)**: `lien-carry-trade` (SRC04_S11, `strategy-seeds/cards/lien-carry-trade_card.md`) — Lien Ch 18 / Ch 24 carry-trade with a yield-differential overlay framed as a regime filter rather than as the entry direction itself. Single-anchor — needs second instance.
- **Ratification trigger**: A SRC0N (N ≥ 5) card surfacing a yield-spread or yield-differential filter as a regime overlay from any source. On that second instance, ratify the flag.
- **Holding pattern in card**: SRC04_S11 card records the yield-spread filter at card-level § 6 / § 8 without a controlled-vocabulary flag; cross-link to this watch.
