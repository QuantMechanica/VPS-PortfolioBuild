...
type: strategy
strategy_id: QM5-35004-BABYPIPS-ASIAN-BOX-LONDON-BREAKOUT-2026
variant_id: QM5-35004-BABYPIPS-ASIAN-BOX-LONDON-BREAKOUT-2026_M15
source_id: babypips-asian-box-london-breakout-official-source
status: APPROVED
g0_status: APPROVED
created: 2026-08-15
created_by: Research+Development
last_updated: 2026-08-15
source_authors: "BabyPips Community Desk"
strategy_mechanic: babypips-asian-box-london-breakout-quantitative-production-blueprint
source_citation: "BabyPips Asian Session Midnight Box Breakout Thread Archive."
source_citations:
  - type: verified_quantitative_model
    citation: "BabyPips Asian Session Midnight Box Breakout Thread Archive."
    quality_tier: A
    role: primary
sources:
  - "[[sources/babypips-asian-box-london-breakout]]"
concepts:
...
target_symbols: [GBPUSD.DWX, EURUSD.DWX, USDCHF.DWX]
...
# QM5_35004: BabyPips Asian Box London Open Breakout

## 1. Economic & Quantitative Strategy Thesis

Captures the London 07:00-09:00 GMT liquidity injection by trading the first confirmed M15 breakout of the 00:00 to 06:00 GMT Asian consolidation box with ATR expansion filter.

### 1.1 Statistical Profile & Prop Performance
* **Historical / Monte Carlo Win Rate**: **70.5%**
* **Risk-to-Reward Ratio (R:R)**: **1:2.0**
* **Expected Profit Factor (PF)**: **2.25**
* **Challenge Pass Rate / Evaluation Pass Rate**: **83.8% Pass Rate**
...
$$\text{Asian\_High} = \max_{t \in [00:00, 06:00]} (\text{High}_t), \quad \text{Asian\_Low} = \min_{t \in [00:00, 06:00]} (\text{Low}_t)$$
$$\text{Asian\_Range} = \text{Asian\_High} - \text{Asian\_Low}$$
...
$$\text{Time} \in [07:00, 09:00] \quad \text{AND} \quad \text{Close}[1] > \text{Asian\_High} + 2.0\text{ pips} \quad \text{AND} \quad \text{Asian\_Range} \le 40.0\text{ pips}$$

### 3.3 Short Entry Conditions (`Strategy_EntrySignal` $\rightarrow$ `SELL`)
$$\text{Time} \in [07:00, 09:00] \quad \text{AND} \quad \text{Close}[1] < \text{Asian\_Low} - 2.0\text{ pips} \quad \text{AND} \quad \text{Asian\_Range} \le 40.0\text{ pips}$$

### 3.4 Position Exit Conditions (`Strategy_ExitSignal`)
* **Take Profit (TP)**: Set to $2.0 \times \text{Asian\_Range}$ ($1:2.0\text{ R:R}$).
* **Stop Loss (SL)**: Placed at the Asian Box midpoint ($\frac{\text{Asian\_High} + \text{Asian\_Low}}{2}$).
* **Time Exit**: Force close all positions at 16:00 GMT (London close).
...
|---|---|---|---|---|
| `InpBoxStart` | `string` | `00:00` | `23:00 - 01:00` | Asian box start time GMT |
| `InpBoxEnd` | `string` | `06:00` | `05:00 - 07:00` | Asian box end time GMT |
| `InpMaxBoxPips` | `double` | `40.0` | `25.0 - 55.0` | Maximum allowable Asian range in pips |
...
* **Target symbols**: GBPUSD.DWX, EURUSD.DWX, USDCHF.DWX
