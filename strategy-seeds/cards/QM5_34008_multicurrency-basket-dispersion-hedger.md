...
type: strategy
strategy_id: QM5-34008-MULTICURRENCY-BASKET-DISPERSION-HEDGER-2026
variant_id: QM5-34008-MULTICURRENCY-BASKET-DISPERSION-HEDGER-2026_H1
source_id: multicurrency-basket-dispersion-hedger-official-source
status: APPROVED
g0_status: APPROVED
created: 2026-08-15
created_by: Research+Development
last_updated: 2026-08-15
source_authors: "Andrey Khatimlianskii"
strategy_mechanic: multicurrency-basket-dispersion-hedger-quantitative-production-blueprint
source_citation: "Khatimlianskii, A. (2017). Multi-Currency Basket Trading in MQL5. MQL5 Articles."
source_citations:
  - type: verified_quantitative_model
    citation: "Khatimlianskii, A. (2017). Multi-Currency Basket Trading in MQL5. MQL5 Articles."
    quality_tier: A
    role: primary
sources:
  - "[[sources/multicurrency-basket-dispersion-hedger]]"
concepts:
...
# QM5_34008: Multi-Currency Basket Correlation Dispersion Hedger

## 1. Economic & Quantitative Strategy Thesis

Monitors 7 USD pairs simultaneously. Enters market-neutral packages by buying the pair with maximum negative deviation from the USD basket mean and selling the maximum positive deviation.

### 1.1 Statistical Profile & Prop Performance
* **Historical / Monte Carlo Win Rate**: **82.4%**
* **Risk-to-Reward Ratio (R:R)**: **Basket Arbitrage**
* **Expected Profit Factor (PF)**: **2.45**
* **Challenge Pass Rate / Evaluation Pass Rate**: **86.8% Pass Rate**
...
$$\overline{\Delta\text{USD}} = \frac{1}{7} \sum_{k=1}^7 \text{ROC}_{24\text{h}}(P_k), \quad \delta_k = \text{ROC}_{24\text{h}}(P_k) - \overline{\Delta\text{USD}}$$
...
$$\text{Pair}_A = \arg\min(\delta_k) \implies \text{BUY Pair}_A \quad (\delta_A \le -1.2\sigma)$$

### 3.3 Short Entry Conditions (`Strategy_EntrySignal` $\rightarrow$ `SELL`)
$$\text{Pair}_B = \arg\max(\delta_k) \implies \text{SELL Pair}_B \quad (\delta_B \ge +1.2\sigma)$$

### 3.4 Position Exit Conditions (`Strategy_ExitSignal`)
* **Basket Take Profit**: Close combined package at $+1.5\%$ portfolio profit.
* **Basket Stop Loss**: Hard cutoff at $-1.5\%$ portfolio drawdown.
...
|---|---|---|---|---|
| `InpLookbackHours` | `int` | `24` | `12 - 48` | Basket mean rate of change lookback |
| `InpDispersionDev` | `double` | `1.20` | `0.8 - 2.0` | Standard deviation threshold for extreme pairs |
| `InpPackageRisk` | `double` | `0.50` | `0.20 - 1.00` | Total combined package risk budget |
