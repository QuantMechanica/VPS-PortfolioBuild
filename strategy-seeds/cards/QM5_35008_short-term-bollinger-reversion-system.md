...
type: strategy
strategy_id: QM5-35008-SHORT-TERM-BOLLINGER-REVERSION-SYSTEM-2026
variant_id: QM5-35008-SHORT-TERM-BOLLINGER-REVERSION-SYSTEM-2026_M15
source_id: short-term-bollinger-reversion-system-official-source
status: APPROVED
g0_status: APPROVED
created: 2026-08-15
...
strategy_mechanic: short-term-bollinger-reversion-system-quantitative-production-blueprint
source_citation: "Robopip (2018-2024). Short-Term Bollinger Reversion System. BabyPips.com."
source_citations:
  - type: verified_quantitative_model
    citation: "Robopip (2018-2024). Short-Term Bollinger Reversion System. BabyPips.com."
    quality_tier: A
    role: primary
sources:
  - "[[sources/short-term-bollinger-reversion-system]]"
concepts:
...
target_symbols: [EURUSD.DWX, USDCAD.DWX, EURCHF.DWX]
...
# QM5_35008: Short-Term Bollinger Band Reversion (Robopip)

## 1. Economic & Quantitative Strategy Thesis

Fades extreme 2.5-sigma Bollinger Band probes during quiet evening hours (18:00 to 22:00 GMT) when RSI(14) exhibits divergence, targeting reversion to the 20 SMA midline.
...
* **Maximum Expected Portfolio Drawdown**: **<2.6%**
...
$$\text{UpperBB} = \text{SMA}(20) + 2.5 \sigma, \quad \text{LowerBB} = \text{SMA}(20) - 2.5 \sigma$$
...
$$\text{Time} \in [18:00, 22:00] \quad \text{AND} \quad \text{Low}[1] \le \text{LowerBB}[1] \quad \text{AND} \quad \text{Close}[1] > \text{Open}[1] \quad \text{AND} \quad \text{RSI}[1] \le 30.0$$

### 3.3 Short Entry Conditions (`Strategy_EntrySignal` $\rightarrow$ `SELL`)
$$\text{Time} \in [18:00, 22:00] \quad \text{AND} \quad \text{High}[1] \ge \text{UpperBB}[1] \quad \text{AND} \quad \text{Close}[1] < \text{Open}[1] \quad \text{AND} \quad \text{RSI}[1] \ge 70.0$$

### 3.4 Position Exit Conditions (`Strategy_ExitSignal`)
* **Take Profit (TP)**: Target at the 20-period SMA middle band.
* **Stop Loss (SL)**: Placed at entry $\mp 1.5 \times \text{ATR}(14, \text{M15})[1]$.
* **Time Exit**: Close position at 23:00 GMT before rollover.
...
|---|---|---|---|---|
| `InpBBPeriod` | `int` | `20` | `14 - 30` | Bollinger Bands moving average period |
| `InpBBDeviation` | `double` | `2.50` | `2.0 - 3.0` | Standard deviation for outer bands |
| `InpRSIPeriod` | `int` | `14` | `7 - 21` | RSI oscillator period |
...
* **Target symbols**: EURUSD.DWX, USDCAD.DWX, EURCHF.DWX
