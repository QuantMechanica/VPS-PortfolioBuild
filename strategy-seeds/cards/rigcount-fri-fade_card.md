...
type: strategy
strategy_id: BAKERHUGHES-RIGCOUNT-FRI-FADE-2026
...
source_citations:
  - type: official_industry_data
    ...
    location: "Rig Count overview and FAQ source packet in strategy-seeds/sources/BAKERHUGHES-RIGCOUNT-2026/source.md."
    quality_tier: A
    role: primary
sources:
...
concepts:
  ...
  - "[[concepts/weekly-event-exhaustion-fade]]"
indicators:
  - "[[indicators/atr]]"
strategy_type_flags: [calendar-seasonality, n-period-min-reversion, atr-hard-stop, time-stop, symmetric-long-short, low-frequency]
target_symbols: [XTIUSD.DWX]
primary_target_symbols: [XTIUSD.DWX]
markets: [XTIUSD.DWX]
single_symbol_only: true
logical_symbol: QM5_12912_XTI_RIGCOUNT_FRI_FADE_D1
period: D1
expected_trade_frequency: "D1 WTI last-workday rig-count displacement exhaustion fade; estimate 6-14 trades/year after return, close-location, spread, and framework filters."
...
g0_approval_reasoning: "R1 PASS official Baker Hughes rig-count source packet; R2 PASS deterministic D1 first-new-week fade after large last-workday WTI displacement with close-location confirmation, ATR stop, closed-bar reversion/adverse exits, and time exit; R3 PASS XTIUSD.DWX available; R4 PASS no ML/grid/martingale/external runtime data."
---

# Baker Hughes Rig-Count Friday Fade
...
The edge is a short-horizon exhaustion sleeve: when WTI closes the final
broker-week bar with an unusually large directional move and at that day's
extreme, enter opposite the move at the first bar of the new broker week and
exit after a brief reversion window, an adverse closed-bar continuation, or an
ATR hard stop.

This is deliberately different from:

- `QM5_12858_rigcount-fri-mom`: same source cadence, opposite entry side.
  `rigcount-fri-mom` follows the displacement; this card fades only more
  extended closes with a closed-bar reversion exit.
...
- Symbol: `XTIUSD.DWX`.
- Period: `D1`.
- Expected frequency: about 6-14 trades/year before Q02 validation.
...
- name: strategy_min_signal_return_pct
  default: 1.10
  sweep_range: [0.80, 1.10, 1.50, 2.00]
- name: strategy_min_atr_return_mult
  default: 0.60
  sweep_range: [0.45, 0.60, 0.80, 1.00]
...
Darwinex `XTIUSD.DWX` price-reaction proxy has tradable exhaustion-fade value.
...
- [x] Non-duplicate: not the existing Baker Hughes momentum card, not RSI
  commodity logic, not a static WTI weekday/month anomaly, not a weekend-gap
  rule, not WPSR/Cushing/refinery/hurricane/OPEC/SPR/expiry/ETF-roll/
  seasonality, and not an energy or metals ratio basket.
...
- trade_entry: D1 WTI exhaustion fade after a large completed last-workday
...
|---|---|---|---|
| v1 | 2026-07-02 | initial Baker Hughes rig-count Friday fade build | Q02 | ENQUEUED |
...
| Q01 Build Validation | 2026-07-02 | PASS | `D:\QM\reports\framework\21\build_check_20260702_094227.json`; compile log `C:\QM\repo\framework\build\compile\20260702_094227\QM5_12912_rigcount-fri-fade.compile.log` |
| Q02 Baseline Screening | 2026-07-02 | QUEUED | `D:\QM\strategy_farm\state\farm_state.sqlite` work item `b9e8ac9c-ae9b-4708-96de-df5f50b58e1d`; enqueue evidence `D:\QM\reports\state\claude_sweep_enqueue_2026-06-10.json` |
