...
- 2026-05-15: **P2 zero-trade verdict (QUA-1552) = EA_CODE_DRIFT, not STRATEGY_DRIFT.** Zero-Trades-Specialist preflight (`docs/ops/evidence/2026-05-15_zero_trades_p2_baseline_verdicts.csv` row 5) flagged QM5_1017 P2 as `STRATEGY_DRIFT` after 8-of-8 deterministic 0-trade runs on D1 2024 across mixed symbol classes. Research review of `framework/EAs/QM5_1017_chan_pairs_stat_arb/QM5_1017_chan_pairs_stat_arb.mq5` shows the EA is a self-declared P1 scaffold: `Strategy_EntrySignal` (line 135) unconditionally `return false` after computing a placeholder z-score with `hedge_ratio = 1.0`; `Strategy_ManageOpenPosition` (line 141) is empty; `Strategy_ExitSignal` (line 146) `return false`; no actual cadf augmented Dickey-Fuller test runs — the `cadf_gate_enabled` flag is a non-functional input. 0 trades is by construction, not by hypothesis failure. Strategy card hypothesis remains source-supported (Chan 2009 Ex 3.6 / 7.2 / 7.3 / 7.5 + Ch 7 pp. 126-133); no card delta required. Recommended downstream: re-classify CSV row as `EA_CODE_DRIFT`, open Development child issue under QUA-1548 for full scaffold→production implementation per Card §4/§5/§7/§13, and confirm the two-symbol magic-formula slot allocation flagged in §12 hard_rules_at_risk → one_position_per_magic_symbol. Full Research verdict: `docs/ops/QUA-1552_RESEARCH_VERDICT_2026-05-15.md`. Heartbeat could not post to issue thread — local-agent JWT secret missing for this run (`6117b674-08c4-47e8-b6bf-e36bb29c7c02`); verdict committed to `agents/research` Git for next-run mirroring.
```
