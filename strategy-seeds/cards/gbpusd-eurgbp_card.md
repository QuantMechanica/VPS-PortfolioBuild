---
strategy_id: AI-CODEX-FX-COINT66-20260609-GBPUSD-EURGBP
ea_id: QM5_20223
slug: gbpusd-eurgbp
type: strategy
status: APPROVED
...
markets: [forex]
timeframes: [D1]
primary_target_symbols: [GBPUSD.DWX, EURGBP.DWX]
target_symbols: [GBPUSD.DWX, EURGBP.DWX]
logical_symbol: QM5_20223_GBPUSD_EURGBP_COINTEGRATION_D1
period: D1
...
# QM5_20223 GBPUSD/EURGBP D1 Cointegration Basket
...
low-frequency daily implementation. Chan makes no claim for GBPUSD/EURGBP.
...
GBPUSD/EURGBP ranks forty-fourth of 66 by OOS net Sharpe. Rank 43 is already
represented by dedicated fixed-D1 basket `QM5_12766`. The deterministic
repository dedup check, exact-pair searches, and an unordered traded-symbol
manifest reconciliation found no dedicated fixed-beta GBPUSD/EURGBP D1 card,
EA, registry allocation, or logical basket manifest before this card.
...
The negative DEV and OOS scores are adverse evidence, and the approximately
149-bar half-life implies a very long holding horizon with material swap and
regime risk. This card authorizes one low-frequency frontier test, not a
certified edge. A terminal economic failure retires the sleeve; it does not
authorize a filter, beta refit, or parameter rescue.

## Non-Duplicate Boundary

Broad multi-symbol or cross-region FX systems may mention both symbols, but
they do not trade this fixed pair, beta, daily residual, and logical package.
Sibling fixed-scan sleeves use different instruments, residuals, magics, and
logical identities.

## 2. Concept

Trade temporary deviations in the fixed log-price residual
`ln(GBPUSD) - (-0.399228065) * ln(EURGBP)`. The negative fitted coefficient
...
The package retains GBP, USD, EUR, carry, and broad risk-sentiment exposure.
That sign-sensitive structure, negative beta, negative window scores, and slow
half-life are explicit high-risk caveats rather than permission to add a
filter or refit the spread.

## Hypothesis

The fixed scan found negative GBPUSD/EURGBP residual results in both DEV and
OOS. This card tests the OWNER-requested next-best unbuilt row as a deliberately
strict falsification candidate: large deviations must survive real tick costs,
swap, and the binding cadence floor in the canonical D1 implementation. The
hypothesis is weak by construction; any terminal economic, cadence, history,
...
`GBPUSD.DWX` is the tester host and first traded leg. `EURGBP.DWX` is the
companion traded leg. The GBPUSD traded history also supplies the GBP-to-USD
conversion path for the EURGBP contract, so the manifest requires no
...
- Compute `spread = ln(GBPUSD) - strategy_beta * ln(EURGBP)`, with frozen
  `strategy_beta = -0.399228065`.
...
Chan supplies the pair-trading method but reports no GBPUSD/EURGBP result.
...
| R3 | PASS | Both traded `.DWX` histories are native factory symbols and jointly supply required USD conversion. |
...
```yaml
target_modules:
  no_trade: "Select and warm both traded histories; reject wrong host, timeframe, slot, unaligned history, or invalid normalized volume."
...
data_requirements: "GBPUSD.DWX and EURGBP.DWX D1 histories"
```
