...
indicators:
  - rolling-zscore
  - atr-stop
target_symbols: [GBPUSD.DWX, EURAUD.DWX]
...
g0_approval_reasoning: "R1 PASS because the method comes from Chan cointegration pair-trading and pair selection comes from the OWNER-requested in-house 66-pair scan; R2 PASS deterministic fixed-pair z-score basket; R3 PASS GBPUSD.DWX and EURAUD.DWX data exist in the exported scan universe; R4 PASS no ML/grid/martingale. Marked high-risk because DEV Sharpe was negative and OOS Sharpe was very low."
expected_pf: 1.01
expected_dd_pct: 30.0
portfolio_scope: basket
---

# Edge Lab GBPUSD/EURAUD Cointegration Basket
...
QM5_12747, QM5_12749, QM5_12751, and QM5_12756 already cover later
exploratory baskets. This card is the strongest remaining unbuilt OOS-positive
candidate by OOS net Sharpe: GBPUSD/EURAUD had DEV Sharpe -0.27, OOS net
Sharpe 0.11, OOS return +1.03%, 17 OOS state changes, hedge 0.03, and 107-day
half-life in the same rerun.

## Concept

GBPUSD and EURAUD do not share a direct currency leg, so this is a low-conviction
macro relative-value residual rather than a common-leg pair. The weak scan result
suggests occasional mean reversion between cable and EUR/AUD risk/commodity
expression, but the negative DEV Sharpe and very low sub-threshold OOS Sharpe
make this a very high-risk exploratory sleeve. The EA trades the spread, not
either cross as a standalone directional system.

## Markets And Timeframe

- Host symbol: GBPUSD.DWX.
- Basket legs: GBPUSD.DWX and EURAUD.DWX.
...
- Compute `spread = ln(GBPUSD) - 0.03 * ln(EURAUD)`.
- Compute a 60-bar rolling z-score of the spread.
- If no pair package is open and z > +2.0, open a short-spread package: short GBPUSD, long EURAUD.
- If no pair package is open and z < -2.0, open a long-spread package: long GBPUSD, short EURAUD.
...
- Host chart must be GBPUSD.DWX or EURAUD.DWX on D1/H1, with slot 0 used for the logical host.
...
No external performance claim is taken from Chan for GBPUSD/EURAUD specifically.
The in-house scan rerun found DEV Sharpe -0.27 and OOS net Sharpe 0.11 after
...
- [x] R3 testable: GBPUSD.DWX and EURAUD.DWX are Darwinex-native `.DWX` symbols in the exported scan data.
...
| v1-q02 | 2026-06-28 | build task db148c56-e737-413a-9f55-f771cf3abefa recorded | Q02 | PENDING work item 0e925c0c-5a27-459c-86d2-cdaddd2b2e9f |
| v1-priority | 2026-06-29 | pending logical-basket Q02 row marked priority and runtime multisymbol hint refreshed | Q02 | PENDING unique work item 0e925c0c-5a27-459c-86d2-cdaddd2b2e9f |
