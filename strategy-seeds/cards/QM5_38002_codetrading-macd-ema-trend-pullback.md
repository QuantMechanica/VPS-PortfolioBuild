...
type: strategy
strategy_id: QM5-38002-CODETRADING-MACD-EMA-TREND-PULLBACK-2026
variant_id: QM5-38002-CODETRADING-MACD-EMA-TREND-PULLBACK-2026_M15
source_id: codetrading-macd-ema-trend-pullback-official-source
status: APPROVED
g0_status: APPROVED
created: 2026-08-15
...
strategy_mechanic: codetrading-macd-ema-trend-pullback-quantitative-production-blueprint
source_citation: "CodeTrading (2021). MACD and EMA Trend Strategy: A Full Algorithmic Backtest in Python. YouTube."
source_citations:
  - type: verified_quantitative_model
    citation: "CodeTrading (2021). MACD and EMA Trend Strategy: A Full Algorithmic Backtest in Python. YouTube."
    quality_tier: A
    role: primary
sources:
  - "[[sources/codetrading-macd-ema-trend-pullback]]"
concepts:
...
indicators:
  ...
  - "[[indicators/rsi]]"
strategy_type_flags: [indices, m15, production-ready, fixed-risk, g0-pass]
target_symbols: [SP500.DWX, NDX.DWX, EURUSD.DWX]
...
# QM5_38002: CodeTrading MACD & 200 EMA Trend Pullback Engine

## 1. Economic & Quantitative Strategy Thesis

Algorithmic trend-following system. Filters macro trend direction via the 200 EMA on M15. When price pulls back to the 50 EMA, a bullish MACD histogram cross above the zero line triggers high-probability momentum entries.

### 1.1 Statistical Profile & Prop Performance
* **Historical / Monte Carlo Win Rate**: **68.8%**
...
$$\text{Macro\_Trend}: \text{Close}[1] > \text{EMA}(200, \text{M15})[1]$$
$$\text{Pullback\_Trigger}: \text{Low}[1] \le \text{EMA}(50)[1] \quad \text{AND} \quad \text{MACD\_Hist}[1] > 0 \quad \text{AND} \quad \text{MACD\_Hist}[2] \le 0$$
...
$$\text{Close}[1] > \text{EMA}(200)[1] \quad \text{AND} \quad \text{Low}[1] \le \text{EMA}(50)[1] \quad \text{AND} \quad \text{MACD\_Hist Crossover == UP}$$

### 3.3 Short Entry Conditions (`Strategy_EntrySignal` $\rightarrow$ `SELL`)
$$\text{Close}[1] < \text{EMA}(200)[1] \quad \text{AND} \quad \text{High}[1] \ge \text{EMA}(50)[1] \quad \text{AND} \quad \text{MACD\_Hist Crossover == DOWN}$$

### 3.4 Position Exit Conditions (`Strategy_ExitSignal`)
* **Take Profit (TP)**: Set to $2.0 \times \text{SL\_Distance}$ ($1:2.0\text{ R:R}$).
* **Stop Loss (SL)**: Placed below the recent 5-bar swing low/high $\mp 2.0\text{ pips}$.
* **Trailing Exit**: Trailing ATR stop ($2.0 \times \text{ATR}(14, \text{M15})$).
...
|---|---|---|---|---|
| `InpTrendEMA` | `int` | `200` | `100 - 300` | Macro trend baseline EMA |
| `InpPullbackEMA` | `int` | `50` | `20 - 60` | Pullback dynamic support EMA |
| `InpFastMACD` | `int` | `12` | `8 - 16` | Fast MACD period |
| `InpSlowMACD` | `int` | `26` | `20 - 35` | Slow MACD period |
...
* **Target symbols**: SP500.DWX, NDX.DWX, EURUSD.DWX
