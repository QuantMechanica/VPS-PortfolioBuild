...
type: strategy
strategy_id: QM5-38003-CODETRADING-BOLLINGER-ENGULFING-REVERSAL-2026
variant_id: QM5-38003-CODETRADING-BOLLINGER-ENGULFING-REVERSAL-2026_H1
source_id: codetrading-bollinger-engulfing-reversal-official-source
status: APPROVED
g0_status: APPROVED
created: 2026-08-15
...
strategy_mechanic: codetrading-bollinger-engulfing-reversal-quantitative-production-blueprint
source_citation: "CodeTrading (2022). How To Trade Candles Patterns | Strategy Backtest In Python. YouTube."
source_citations:
  - type: verified_quantitative_model
    citation: "CodeTrading (2022). How To Trade Candles Patterns | Strategy Backtest In Python. YouTube."
    quality_tier: A
    role: primary
sources:
  - "[[sources/codetrading-bollinger-engulfing-reversal]]"
concepts:
...
target_symbols: [EURUSD.DWX, GBPJPY.DWX, AUDUSD.DWX]
...
# QM5_38003: CodeTrading Bollinger Bands & Engulfing Rejection

## 1. Economic & Quantitative Strategy Thesis

Mathematical price action model. Systematically identifies Engulfing candlestick rejections occurring at the outer 2.0-sigma Bollinger Band bounds, confirmed by RSI momentum filters.
...
$$\text{Bullish\_Engulfing}: \text{Close}[1] > \text{Open}[2] \quad \text{AND} \quad \text{Open}[1] < \text{Close}[2] \quad \text{AND} \quad \text{Close}[2] < \text{Open}[2]$$
$$\text{Band\_Touch}: \text{Low}[1] \le \text{LowerBB}(20, 2.0)[1]$$
...
$$\text{Bullish\_Engulfing == TRUE} \quad \text{AND} \quad \text{Band\_Touch == TRUE} \quad \text{AND} \quad \text{RSI}(14)[1] \le 35.0$$

### 3.3 Short Entry Conditions (`Strategy_EntrySignal` $\rightarrow$ `SELL`)
$$\text{Bearish\_Engulfing == TRUE} \quad \text{AND} \quad \text{High}[1] \ge \text{UpperBB}[1] \quad \text{AND} \quad \text{RSI}(14)[1] \ge 65.0$$

### 3.4 Position Exit Conditions (`Strategy_ExitSignal`)
* **Take Profit (TP)**: Set to $2.0 \times \text{SL\_Distance}$ ($1:2.0\text{ R:R}$).
* **Stop Loss (SL)**: Placed below the low of the engulfing candle $\mp 2.0\text{ pips}$.
* **Middle Band Exit**: Close 50% volume at the 20 SMA middle band.
...
| `InpBBDev` | `double` | `2.00` | `1.5 - 2.5` | Standard deviation multiplier |
| `InpRSIPeriod` | `int` | `14` | `7 - 21` | RSI filter period |
...
* **Target symbols**: EURUSD.DWX, GBPJPY.DWX, AUDUSD.DWX
