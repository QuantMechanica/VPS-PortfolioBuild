...
type: strategy
strategy_id: MOP-TSMOM-2012_XTI_BISQUARE12_S34
variant_id: MOP-TSMOM-2012_XTI_BISQUARE12_S34
source_id: MOP-WTI-BISQUARE-2026
ea_id: QM5_20286
slug: wti-bisquare-mom
status: APPROVED
execution_contract_ref: strategy-seeds/cards/approved/QM5_20286_wti-bisquare-mom_card.md
...
source_citations:
  - type: peer_reviewed_trading_paper
    ...
    location: "DOI https://doi.org/10.1016/j.jfineco.2011.11.003; complete-paper evidence strategy-seeds/sources/MOP-TSMOM-2012/source.md; bounded extraction strategy-seeds/sources/MOP-WTI-BISQUARE-2026/source.md"
    quality_tier: A
    role: primary_own_price_direction_and_monthly_cadence
strategy_mechanic: monthly-wti-fixed-step-redescending-bisquare-m-location-of-twelve-completed-monthly-log-returns
sources:
  - "[[sources/MOP-WTI-BISQUARE-2026]]"
concepts:
...
period: D1
timeframe: D1
expected_trade_frequency: "Approximately eleven to twelve monthly WTI packages/year after thirteen completed month ends because only nonpositive-MAD, exact-zero, zero-total-weight, or invalid states stay flat; Q02 must prove at least five completed positions/year or retire."
...
hard_rules_at_risk: [completed_month_reconstruction, chronological_log_return_orientation, even_sample_median, even_sample_mad, mad_normalization, frozen_bisquare_cutoff, strict_support_boundary, redescending_weight_equation, zero_total_weight_fail_closed, exactly_32_updates, monthly_attempt_state, risk_mode_dual, friday_close_disabled, cfd_futures_basis, q02_frequency_floor, portfolio_correlation]
g0_approval_reasoning: "APPROVED under decisions/2026-08-12_qm5_20286_wti_bisquare_mom_g0.md: R1 one complete-read peer-reviewed WTI trading source; R2 fixed thirteen endpoints, twelve adjacent returns, median/MAD, cutoff, strict support, squared weights, 32 updates, direction, and lifecycle; R3 registered WTI D1 route; R4 deterministic native arithmetic. The canonical checker found no exact identity and manual review separated the sole Huber fuzzy neighbor and the closest one-shot robust estimators."
---

# QM5_20286 WTI Redescending Bisquare Return Trend
...
twelve-month return can be dominated by one oil shock. A Huber score limits
but never removes finite tail influence. This card instead tests whether the
central direction of twelve separate completed monthly returns is more useful
when sufficiently remote observations receive exactly zero influence through
a fixed redescending bisquare score.
...
The bisquare statistic is a transparent QM robust-location hypothesis. The
paper does not test it. The scale normalization, cutoff, fixed update count,
Darwinex continuous CFD, broker-month reconstruction, fixed-dollar sizing,
ATR hard stop, spread cap, attempt ledger, and lifecycle controls are QM
...
The canonical checker scanned 4,351 EA-registry rows and 463 root cards. It
found no exact identity and surfaced one fuzzy match:

- `QM5_20285_wti-huber-mom` uses a Huber score whose finite tail
  observations always retain positive influence `delta / residual`; this
  card's bisquare weight falls to exactly zero at the cutoff and remains zero
  beyond it;
- `QM5_20282_wti-madcap-mom` freezes the median as a cap center, clips each
  observation once, and takes an equal-weight mean;
- `QM5_20283_wti-trimean-mom` is a fixed quartile summary rather than an
  iterative redescending score;
- `QM5_20277`, `QM5_20270`, and `QM5_20276` use fixed Winsor
  replacement, fixed trimming, and a pairwise pseudomedian; and
- cumulative, sign/vote/run, weighting, regression, rank, path-efficiency,
  and skip-month systems use different functionals or endpoint objects.

The two sorts, even-sample median/MAD, `1.4826` normalization, `4.685`
frozen cutoff, strict `abs(u)<1` support, squared bisquare weight, zero-tail
influence, exactly 32 re-centering updates, zero-total-weight failure,
exact-zero rejection, consumed attempt, and monthly renewal are jointly
load-bearing. Verdict:
`CLEAN_AFTER_MANUAL_HUBER_NEIGHBOR_REVIEW`.

## Markets, Timeframe, And Cadence

- Exact host and traded symbol: `XTIUSD.DWX`.
- Timeframe: D1; magic slot 0; intended magic `202860000`.
...
- Signal state: sign of the locked 32-step redescending location.
...
BUY when `mu[32] > 0`. SELL when `mu[32] < 0`. An exact-zero,
nonpositive-MAD, zero-total-weight, or invalid state remains flat. The
statistic's magnitude never scales risk.
...
trimmed mean, Winsorized mean, MAD-capped mean, quartile trimean,
Hodges-Lehmann pseudomedian, Huber location, sign statistic, vote, regression,
rank, moving average, oscillator, calendar direction, external series, or
previous pipeline result.

## 4. Entry Rules

1. Require exact EA ID `20286`, `XTIUSD.DWX` D1, magic slot 0, and every
...
8. Reject nonpositive or invalid MAD. Freeze
   `cutoff = 4.685 * 1.4826 * MAD`; initialize `mu` at the median.
9. Run exactly 32 updates. At each update normalize every residual by the
   frozen cutoff. Give `abs(u)<1` the weight `(1-u^2)^2` and every other
   observation weight zero. Reject a nonpositive or invalid total weight or
...
12. Open at most one market position with a frozen `3.5 * ATR(20,D1)`
    broker hard stop and no take-profit.
...
- Reject a consumed attempt, owned exposure, same-month entry history,
  ...
  close, invalid return, wrong sort/median/MAD, nonpositive scale/cutoff,
  wrong strict-support comparison or weight, nonpositive total weight, wrong
  update count, exact-zero signal, excessive spread, invalid quote,
  unavailable ATR, invalid stop, or invalid metadata.
...
| `strategy_bisquare_cutoff` | 4.685 | [4.685] | fixed normalized redescending cutoff |
| `strategy_mad_normalizer` | 1.4826 | [1.4826] | fixed raw-MAD scale normalization |
| `strategy_bisquare_steps` | 32 | [32] | exact re-centering update count |
...
support boundary, weight equation, update ordering, direction, entry clock,
...
WTI in their commodity universe. They do not claim this exact estimator works,
that a continuous CFD reproduces rolling futures, or that the candidate
...
small-sample scale instability, multiple local redescending solutions, and
correlation with XNG or risk assets can dominate the premise. Zero tail
influence can discard a genuine regime change and does not guarantee edge or
neutrality.
...
- Fail on wrong endpoint order, nonconsecutive months, current-month leakage,
  wrong adjacent pairs or orientation, wrong median/MAD/scale, mutable cutoff,
  an inclusive rather than strict support boundary, wrong squared weight,
  nonzero influence outside the cutoff, update count other than 32, fallback
  after zero total weight or exact zero, wrong-side entry, repeated attempt,
...
- Retire on nonpositive governed economics or later portfolio-correlation
  rejection.
- Do not rescue failure by changing horizon, estimator, cutoff, normalizer,
  iteration count, direction, entry clock, stop, hold, spread, retry, or
  carrier.

## Strategy Allowability Check

- [x] R1: PASS. One canonical Tier-A peer-reviewed trading source with DOI,
  complete-paper evidence, durable retrieval hash, and explicit WTI
  membership.
- [x] R2: PASS. Fixed endpoints, returns, median/MAD, constants, strict
  support, weights, update count, direction, attempt, hard stop, rollover, and
  stale exit.
- [x] R3: PASS. Registered `XTIUSD.DWX` D1 plus native V5 execution state
  only.
- [x] R4: PASS. Deterministic logarithm, sorting, and fixed arithmetic; no
  ...
  martingale.
- [x] Dedup: no exact identity; the Huber fuzzy neighbor and one-shot robust
  estimators were manually resolved.
...
- trade_entry: month-attempt persistence, endpoint reconstruction, adjacent
  returns, median/MAD, fixed-step bisquare location, spread/quote/ATR/stop
...
| Q02 Baseline Screening | 2026-08-12 | ENQUEUED; pending at immediate readback, attempt 0, no verdict | work item `db894631-d726-4ff2-98c3-ef8ab043d0ff`; `docs/ops/evidence/2026-08-12_qm5_20286_wti_bisquare_mom_q01_q02_enqueue.md` |
