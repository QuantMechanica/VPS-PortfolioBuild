...
type: strategy
strategy_id: QM5-33003-EHLERS-SUPERPASSBAND-FISHER-CYCLE-SCALPER-2026
variant_id: QM5-33003-EHLERS-SUPERPASSBAND-FISHER-CYCLE-SCALPER-2026_H1
source_id: ehlers-superpassband-fisher-cycle-scalper-official-source
status: APPROVED
g0_status: APPROVED
created: 2026-08-15
created_by: Research+Development
last_updated: 2026-08-15
source_authors: "John F. Ehlers"
strategy_mechanic: ehlers-superpassband-fisher-cycle-scalper-quantitative-production-blueprint
source_citation: "Ehlers, J. F. (2013). Cycle Analytics for Traders. John Wiley & Sons."
source_citations:
  - type: verified_quantitative_model
    citation: "Ehlers, J. F. (2013). Cycle Analytics for Traders. John Wiley & Sons."
    quality_tier: A
    role: primary
sources:
  - "[[sources/ehlers-superpassband-fisher-cycle-scalper]]"
concepts:
...
# QM5_33003: John Ehlers SuperPassBand & Fisher Transform DSP Scalper

## 1. Economic & Quantitative Strategy Thesis

Digital Signal Processing (DSP): SuperPassBand 2nd-order filter isolates the 10-to-40-bar economic cycle, and Fisher Transform converts it into a Gaussian distribution for zero-lag turning points.
...
* **Challenge Pass Rate / Evaluation Pass Rate**: **85.1% Pass Rate**
...
$$\text{HP}_t = (1 - a_1/2) (P_t - P_{t-1}) + (1 - a_1) \text{HP}_{t-1}$$
$$\text{SuperPB}_t = \left(\frac{a_2}{2}\right) (\text{HP}_t + \text{HP}_{t-1}) + (1 - a_2) \text{SuperPB}_{t-1}$$
$$\text{Fisher}_t = 0.5 \times \ln\left(\frac{1 + x_t}{1 - x_t}\right) + 0.5 \times \text{Fisher}_{t-1}$$
...
$$\text{Fisher}[2] < \text{Trigger}[2] \quad \text{AND} \quad \text{Fisher}[1] \ge \text{Trigger}[1] \quad \text{AND} \quad \text{Fisher}[1] \le -1.50$$

### 3.3 Short Entry Conditions (`Strategy_EntrySignal` $\rightarrow$ `SELL`)
$$\text{Fisher}[2] > \text{Trigger}[2] \quad \text{AND} \quad \text{Fisher}[1] \le \text{Trigger}[1] \quad \text{AND} \quad \text{Fisher}[1] \ge +1.50$$

### 3.4 Position Exit Conditions (`Strategy_ExitSignal`)
* **Take Profit (TP)**: Set to entry $\pm 3.0 \times \text{ATR}(14, \text{H1})[1]$ ($1:2.0\text{ R:R}$).
* **Stop Loss (SL)**: Set at entry $\mp 1.5 \times \text{ATR}(14, \text{H1})[1]$.
* **Cycle Exit**: Close position when Fisher crosses the zero line.
...
|---|---|---|---|---|
| `InpFastPeriod` | `int` | `10` | `5 - 15` | SuperPassBand high cutoff frequency |
| `InpSlowPeriod` | `int` | `40` | `25 - 60` | SuperPassBand low cutoff frequency |
| `InpFisherThreshold`| `double` | `1.50` | `1.0 - 2.5` | Fisher Transform extreme turning threshold |
