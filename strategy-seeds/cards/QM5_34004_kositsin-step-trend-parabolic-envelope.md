...
type: strategy
strategy_id: QM5-34004-KOSITSIN-STEP-TREND-PARABOLIC-ENVELOPE-2026
variant_id: QM5-34004-KOSITSIN-STEP-TREND-PARABOLIC-ENVELOPE-2026_H1
source_id: kositsin-step-trend-parabolic-envelope-official-source
status: APPROVED
g0_status: APPROVED
created: 2026-08-15
created_by: Research+Development
last_updated: 2026-08-15
source_authors: "Nikolay Kositsin"
strategy_mechanic: kositsin-step-trend-parabolic-envelope-quantitative-production-blueprint
source_citation: "Kositsin, N. (2015). Open-Source Production Trading Systems. MQL5 CodeBase Library."
source_citations:
  - type: verified_quantitative_model
    citation: "Kositsin, N. (2015). Open-Source Production Trading Systems. MQL5 CodeBase Library."
    quality_tier: A
    role: primary
sources:
  - "[[sources/kositsin-step-trend-parabolic-envelope]]"
concepts:
...
target_symbols: [EURUSD.DWX, XAUUSD.DWX, GBPJPY.DWX]
...
# QM5_34004: Nikolay Kositsin Step-Trend Parabolic Envelope

## 1. Economic & Quantitative Strategy Thesis

Discrete step quantization prevents whipsaws in flat markets by keeping the trend line completely horizontal until price moves by at least one full ATR-derived step size.

### 1.1 Statistical Profile & Prop Performance
* **Historical / Monte Carlo Win Rate**: **71.2%**
...
$$\text{Step\_Size} = 0.80 \times \text{ATR}(14, \text{H1})[1]$$
$$\text{Step\_MA}_t = \text{Step\_MA}_{t-1} \pm \text{Step\_Size} \quad (\text{when } |\Delta P| \ge \text{Step\_Size})$$
...
$$\text{Close}[1] > \text{Step\_MA}[1] \quad \text{AND} \quad \text{Step\_MA}[1] > \text{Step\_MA}[2] \quad \text{AND} \quad \text{Parabolic\_SAR}[1] < \text{Low}[1]$$

### 3.3 Short Entry Conditions (`Strategy_EntrySignal` $\rightarrow$ `SELL`)
$$\text{Close}[1] < \text{Step\_MA}[1] \quad \text{AND} \quad \text{Step\_MA}[1] < \text{Step\_MA}[2] \quad \text{AND} \quad \text{Parabolic\_SAR}[1] > \text{High}[1]$$

### 3.4 Position Exit Conditions (`Strategy_ExitSignal`)
* **Take Profit (TP)**: Set to $2.0 \times \text{SL\_Distance}$ ($1:2.0\text{ R:R}$).
* **Stop Loss (SL)**: Set at $\text{Step\_MA}[1] \mp 0.5 \times \text{Step\_Size}$.
...
|---|---|---|---|---|
| `InpATRPeriod` | `int` | `14` | `10 - 30` | ATR volatility lookback period |
| `InpStepMultiplier` | `double` | `0.80` | `0.50 - 1.50` | ATR multiplier for discrete step size |
| `InpSARStep` | `double` | `0.02` | `0.01 - 0.05` | Parabolic SAR acceleration step |
...
* **Target symbols**: EURUSD.DWX, XAUUSD.DWX, GBPJPY.DWX
