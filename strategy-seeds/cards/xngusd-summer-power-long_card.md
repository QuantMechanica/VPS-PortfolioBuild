...
period: D1
expected_trade_frequency: "Summer electric-sector demand XNG sleeve on D1; estimate 2-4 monthly entries/year after trend confirmation and framework filters."
...
g0_approval_reasoning: "R1 PASS official EIA Today in Energy source; R2 PASS deterministic June-August XNG long-only calendar window with SMA confirmation, ATR stop, and time/season exits; R3 PASS XNGUSD.DWX available; R4 PASS no ML/grid/martingale/external data."
expected_pf: 1.10
expected_dd_pct: 20.0
---

# XNG Summer Power Long
...
The EIA source describes natural-gas consumption as seasonally shaped, with a summer consumption peak tied to electric-sector demand for power generation. This card isolates that regime as a low-frequency `XNGUSD.DWX` D1 sleeve: long only during June through August, only when price confirms above a slow D1 mean, and flat outside the window.

This is deliberately different from:

- `QM5_12567_cum-rsi2-commodity`: no RSI, oscillator, or short-horizon pullback logic.
- `QM5_12575_eia-xng-season`: this card is summer-only and long-only, not the broad winter/summer/shoulder calendar map.
- `QM5_12588_eia-xng-sum-sqz`: this card does not trade compression breakout logic; it is a monthly seasonal allocation with SMA confirmation.
...
- Eligible months are June, July, and August.
...
- Exit when the broker-calendar month is outside June-August.
...
No performance claim is imported into QM. The EIA source is used only for structural lineage around natural-gas seasonal consumption and summer electric-sector demand. The Q02+ pipeline tests the mechanical Darwinex `XNGUSD.DWX` port.
...
- [x] R2 mechanical: fixed June-August calendar window, SMA confirmation, ATR hard stop, trend exit, season exit, and max-hold exit.
...
- trade_entry: monthly summer-power long entry after D1 SMA confirmation.
...
|---|---|---|---|---|
| v1 | 2026-06-28 | initial structural XNG summer power long build | G0 | APPROVED |
