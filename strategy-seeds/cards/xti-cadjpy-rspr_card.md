...
sources:
  - "[[sources/EIA-BOC-BOJ-XTI-CADJPY-2026]]"
concepts:
...
target_symbols: [XTIUSD.DWX, CADJPY.DWX]
basket_symbols: [XTIUSD.DWX, CADJPY.DWX]
markets: [XTIUSD.DWX, CADJPY.DWX]
primary_target_symbols: [XTIUSD.DWX, CADJPY.DWX]
single_symbol_only: false
logical_symbol: QM5_13010_XTI_CADJPY_RSPREAD_D1
period: D1
timeframes: [D1]
expected_trade_frequency: "D1 XTI/CADJPY return-spread z-score reversion; estimate 8-14 paired packages/year before Q02 validates synchronized history and fills."
...
g0_approval_reasoning: "Mission-directed G0 approval 2026-07-04: R1 PASS official EIA/central-bank source packet; R2 PASS deterministic D1 two-leg XTI/CADJPY return-spread z-score reversion with spread caps, mean exit, max-hold exit, and ATR hard stops; R3 PASS XTIUSD.DWX and CADJPY.DWX exist in the DWX matrix; R4 PASS no ML/grid/martingale/external runtime data. Non-duplicate because this is a paired WTI/CADJPY return-spread basket, not the Singh CADJPY leading-indicator build, WTI/USDCAD, WTI/USDJPY, XTI/NZD, XTI/XNG, Brent/WTI, metal-ratio, XNG, or index logic."
---

# XTI/CADJPY D1 Return-Spread Reversion

## Source

- Source: [[sources/EIA-BOC-BOJ-XTI-CADJPY-2026]]
...
`return_spread = ln(XTI[t] / XTI[t-L]) - beta_cadjpy * ln(CADJPY[t] / CADJPY[t-L])`

When WTI has unusually outperformed CADJPY over the fixed D1 return window, the
basket shorts WTI and buys CADJPY. When WTI has unusually underperformed
CADJPY, it buys WTI and sells CADJPY. The bet is a temporary return-spread
...
- `QM5_1040_singh-cmd-corr`: that EA trades CADJPY from oil support/resistance
  breakouts and does not trade WTI as a paired basket leg.
...
- `QM5_13006_xti-nzd-rspread`, `QM5_12840_xti-xng-rspread`, Brent/WTI,
  ...
  sleeves: this is a two-leg WTI/CADJPY return-spread basket.
...
- Logical symbol: `QM5_13010_XTI_CADJPY_RSPREAD_D1`.
- Host symbol: `XTIUSD.DWX`.
- Basket legs: `XTIUSD.DWX` and `CADJPY.DWX`.
- Period: D1.
...
- Copy completed D1 closes for `XTIUSD.DWX` and `CADJPY.DWX`.
...
- Compute `return_spread = xti_ret - strategy_beta_cadjpy * cadjpy_ret`.
...
`XTIUSD.DWX` / `CADJPY.DWX` basket on Darwinex bars.
...
- [x] R3 testable: `XTIUSD.DWX` and `CADJPY.DWX` exist in the Darwinex symbol
  matrix.
- [x] R4 compliant: no ML, adaptive PnL fitting, grid, martingale, external
  runtime feed, or more than one position per magic slot.
- [x] Non-duplicate: paired WTI/CADJPY return-spread mean reversion, not Singh
  oil-to-CADJPY breakout, WTI/USDCAD, WTI/USDJPY, XTI/NZD, XTI/XNG,
...
- Slot 0: `XTIUSD.DWX`.
- Slot 1: `CADJPY.DWX`.
- Use the logical basket setfile `QM5_13010_XTI_CADJPY_RSPREAD_D1` for Q02.
...
- trade_entry: D1 standardized XTI/CADJPY return-spread reversion.
...
trade, if Q02 PF is below 1.0 after costs, if synchronized XTI/CADJPY history
...
|---|---|---|---|---|
| v1 | 2026-07-04 | initial XTI/CADJPY return-spread basket build | Q02 | PENDING |
...
| Q01 Build Validation | 2026-07-04 | PENDING | `artifacts/qm5_13010_build_result.json` |
