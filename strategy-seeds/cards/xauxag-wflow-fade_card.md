...
type: strategy
strategy_id: WILLIAMS-SCHWEIKERT-XAUXAG-WFLOWFADE-2026_S01
variant_id: WILLIAMS-SCHWEIKERT-XAUXAG-WFLOWFADE-2026_S01
source_id: WILLIAMS-SCHWEIKERT-XAUXAG-WFLOWFADE-2026
ea_id: QM5_41040
slug: xauxag-wflow-fade
status: APPROVED
execution_contract_ref: strategy-seeds/cards/approved/QM5_41040_xauxag-wflow-fade_card.md
execution_contract_status: APPROVED
created: 2026-08-17
created_by: Research+Development
last_updated: 2026-08-17
g0_status: APPROVED
g0_decision: decisions/2026-08-17_xauxag_weekly_flow_conditioned_reversion_g0.md
...
strategy_mechanic: exact-prior-monday-friday-synchronized-xau-minus-xag-close-open-versus-open-close-strict-opposition-session-dominant-fade-completed-relative-week-next-monday-paired-friday-flat
sources:
  - "[[sources/WILLIAMS-SCHWEIKERT-XAUXAG-WFLOWFADE-2026]]"
concepts:
...
strategy_type_flags: [commodity, precious-metals, relative-value, price-flow-decomposition, weekly-flow-conditioned-reversion, session-dominance, logical-basket, weekly-entry, friday-close, atr-hard-stop, low-frequency]
markets: [commodities, precious_metals, gold, silver]
timeframes: [D1]
target_symbols: [XAUUSD.DWX, XAGUSD.DWX]
primary_target_symbols: [XAUUSD.DWX, XAGUSD.DWX]
single_symbol_only: false
logical_symbol: QM5_41040_XAU_XAG_WFLOWFADE_D1
...
companion_magic: 410400001
period: D1
timeframe: D1
expected_trade_frequency: "Approximately 7-15 completed XAU/XAG packages per full post-warm-up year after exact-week, synchronization, strict flow opposition, and strict session-dominance gates; Q02 must prove at least five/year or retire."
...
hard_rules_at_risk: [exact_synchronized_week, completed_close_open_endpoints, cross_metal_subtraction, strict_flow_opposition, strict_session_dominance, completed_week_fade, flow_reconciliation, monday_decision_clock, weekly_attempt_state, no_current_bar_leakage, no_late_restart_entry, basket_atomicity, aggregate_fixed_risk, notional_mismatch, magic_schema, paired_friday_close, cfd_futures_basis, q02_frequency_floor, portfolio_correlation]
g0_approval_reasoning: "R1 complete OWNER-supplied Tier-A flow decomposition plus peer-reviewed and exchange gold/silver carrier lineage and complete governed weekly endpoint lineage with disclosed conjunction risk; R2 exact sequence, endpoints, subtraction, opposition, dominance, reconciliation, fade sides, attempt, timing, joint sizing, atomicity, and lifecycle; R3 native synchronized XAU/XAG D1 only; R4 deterministic arithmetic without a banned signal or trained logic; canonical dedup raised only the expected weekly and monthly flow neighbors and manual review fixed the state, direction, cadence, and lifecycle boundaries."
---
...
monetary, safe-haven, and industrial information. When their exact completed
prior-week close-to-open relative flow opposes but is smaller than their
open-to-close relative flow, the session-dominated relative move may represent
a crowded displacement rather than a durable common-metal trend. The candidate
fades that completed XAU-minus-XAG week with opposite legs from the next Monday
through Friday.
...
objects and discusses their separate accumulation and disagreement. Schweikert
...
The five-session sums, gold-minus-silver subtraction, strict opposition, strict
session-dominance gate, completed-week fade, exact calendar sequence,
...
The deterministic pre-allocation checker scanned 4,527 EA-registry rows and
624 root-card files. It found no exact identity and two expected fuzzy flow-
family neighbors. Manual review fixes the load-bearing boundaries:

- `QM5_41030_xauxag-flowdiv` trades every strict-opposition week and follows
  the session-relative sign. This card trades only the strict subset in which
  session-relative flow dominates and takes the opposite sides by fading the
...
- `QM5_41039_xauxag-mflow-div` consumes a complete broker month, follows
  ...
  week, a Monday decision, a contrarian dominance gate, and Friday flat.
...
- Host/traded slot 0: `XAUUSD.DWX`, D1, magic `410400000`.
- Companion/traded slot 1: `XAGUSD.DWX`, D1, magic `410400001`.
...
- Expected cadence: approximately 7-15 completed packages/year.
...
1. Evaluate entry only on a new `XAUUSD.DWX` D1 bar while attached to exact
   `XAUUSD.DWX`, D1, EA ID 41040, slot 0.
...
10. Require strict component opposition and
    `abs(session_relative) > abs(overnight_relative)`. If `week_relative > 0`,
    SELL XAU and BUY XAG. If `week_relative < 0`, BUY XAU and SELL XAG.
    Agreement, exact zero, equal magnitude, absent session dominance, invalid
    arithmetic, or failed reconciliation consumes the week.
...
- Strict flow opposition, strict session dominance, endpoint reconciliation,
  and completed-week fade direction are load-bearing.
...
- Own at most one logical package: one XAU position under magic `410400000`
  and one XAG position under magic `410400001` in opposite directions.
...
No parameter sweep, after-result threshold, dominance ratio, flow-sign, fade
...
subtraction, opposition, session dominance, reconciliation, or fade sides;
...
No weak result may be rescued by dropping the dominance gate, following the
session component, adding a magnitude threshold, changing the carrier/clock,
...
2. all sign and magnitude orderings, equality, and invalid prices select only
   strict opposition with strict session dominance and the correct completed-
   week fade sides;
