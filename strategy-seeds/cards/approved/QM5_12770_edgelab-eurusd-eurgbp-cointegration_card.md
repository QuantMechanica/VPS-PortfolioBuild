...
indicators:
  - rolling-zscore
  - atr-stop
target_symbols: [EURUSD.DWX, EURGBP.DWX]
logical_symbol: QM5_12770_EURUSD_EURGBP_COINTEGRATION_D1
period: D1
...
g0_approval_reasoning: "R1 PASS Chan cointegration method plus OWNER-requested in-house 66-pair FX scan; R2 PASS deterministic fixed-pair z-score basket; R3 PASS EURUSD.DWX and EURGBP.DWX data exist in scan universe; R4 PASS no ML/grid/martingale. Marked very high-risk because DEV and OOS net Sharpe were both negative."
expected_pf: 0.95
expected_dd_pct: 35.0
portfolio_scope: basket
---

# Edge Lab EURUSD/EURGBP Cointegration Basket
...
QM5_12764, QM5_12765, QM5_12766, and QM5_12768 already cover stronger
exploratory baskets. This card is the next unbuilt rank-22 tail candidate by OOS
net Sharpe: EURUSD/EURGBP had DEV Sharpe -0.0833, OOS net Sharpe -0.1761, OOS
return -1.4936%, 17 OOS state changes, hedge 0.601215, and 149.27-day half-life
in the same rerun.

## Concept

EURUSD and EURGBP share the EUR base side but differ by USD and GBP quote-side
macro/rate risk. This is a low-conviction relative-value residual rather than a
hard-bar scan survivor. The scan found negative DEV and OOS Sharpe, making this
a very high-risk exploratory sleeve. The EA trades the spread, not either pair
as a standalone directional system.

## Hypothesis

Temporary dislocations between the EURUSD and EURGBP D1 log-price spread can
mean-revert because both pairs express EUR base-side pressure through different
USD and GBP macro channels. The negative scan result makes this a tail test
only; Q02+ must reject it if live-cost mechanics confirm no edge.

## Markets And Timeframe

- Host symbol: EURUSD.DWX.
- Basket legs: EURUSD.DWX and EURGBP.DWX.
- Logical symbol: QM5_12770_EURUSD_EURGBP_COINTEGRATION_D1.
- Period: D1.
...
- The EA trades only the fixed EURUSD/EURGBP basket; it does not reselect pairs
...
- Compute `spread = ln(EURUSD) - 0.601215 * ln(EURGBP)`.
- Compute a 60-bar rolling z-score of the spread.
- If no pair package is open and z > +2.0, open a short-spread package: short EURUSD, long EURGBP.
- If no pair package is open and z < -2.0, open a long-spread package: long EURUSD, short EURGBP.
...
- Host chart must be EURUSD.DWX or EURGBP.DWX on D1/H1, with slot 0 used for the logical host.
- The EA selects GBPUSD.DWX as conversion history for the EURGBP leg in USD-denominated tests.
...
- name: strategy_beta
  default: 0.601215
  sweep_range: [0.45, 0.601215, 0.75]
...
No external performance claim is taken from Chan for EURUSD/EURGBP specifically.
The in-house scan rerun found DEV Sharpe -0.0833 and OOS net Sharpe -0.1761
after cost, below zero and below the original 0.8 survivor threshold. Pipeline
gates are the judge.
...
- risk_class: very high because this is a rank-22 negative DEV/OOS tail candidate after the rank-21 USDJPY/EURJPY exploratory basket.
- gridding: false.
- scalping: false.
- ml_required: false.
...
- [x] R3 testable: EURUSD.DWX and EURGBP.DWX are Darwinex-native `.DWX` symbols in the exported scan data.
...
|---|---|---|---|---|
| v1 | 2026-06-29 | initial rank-22 next-unbuilt FX cointegration basket card | G0 | APPROVED |
| v1-q02 | 2026-06-29 | build task dd64c930-550b-41e6-9d3a-807b184f9cb8 recorded and logical-basket Q02 enqueued | Q02 | PENDING 118ab93b-8df8-47a2-85d1-79b4ab0e1eaa |
