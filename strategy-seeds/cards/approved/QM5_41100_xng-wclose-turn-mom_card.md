...
type: strategy
strategy_id: BIANCHI-MOP-XNG-WCLOSE-TURN-MOM-2026_S01
variant_id: BIANCHI-MOP-XNG-WCLOSE-TURN-MOM-2026_S01
source_id: BIANCHI-MOP-XNG-WCLOSE-TURN-MOM-2026
ea_id: QM5_41100
slug: xng-wclose-turn-mom
status: APPROVED
execution_contract_ref: strategy-seeds/cards/approved/QM5_41100_xng-wclose-turn-mom_card.md
execution_contract_status: APPROVED
created: 2026-08-22
created_by: Research+Development
last_updated: 2026-08-21
g0_status: APPROVED
g0_decision: decisions/2026-08-22_qm5_41100_xng_weekly_close_turn_momentum_g0.md
source_approval: decisions/2026-08-22_xng_weekly_close_turn_momentum_source_approval.md
...
source_citations:
  - type: academic_paper
    ...
    quality_tier: A
    role: commodity_reversal_and_natural_gas_carrier_lineage
  - type: academic_paper
    ...
    location: "DOI 10.1016/j.jfineco.2011.11.003; complete-read record strategy-seeds/sources/MOP-TSMOM-2012/source.md; bounded translation strategy-seeds/sources/BIANCHI-MOP-XNG-WCLOSE-TURN-MOM-2026/source.md"
    quality_tier: A
    role: own_return_continuation_and_natural_gas_carrier_lineage
strategy_mechanic: normalized-week-boundary-xng-one-immediately-completed-three-to-five-session-week-every-chronological-close-strict-single-interior-turn-strict-monotone-legs-final-close-full-recovery-beyond-first-close-continuation-one-week-hold
sources:
  - "[[sources/BIANCHI-MOP-XNG-WCLOSE-TURN-MOM-2026]]"
concepts:
...
strategy_type_flags: [commodity, energy, natural-gas, completed-week-close-path, single-turn-recovery, time-series-momentum, symmetric-long-short, atr-hard-stop, time-stop, low-frequency]
...
period: D1
timeframe: D1
expected_trade_frequency: "Approximately 6-18 completed XNG positions per full post-warm-up year after the strict single-turn and full-recovery gates; Q02 must prove at least five/year or retire."
...
review_focus: "Falsify a second-XNG completed-week close-turn recovery sleeve whose symmetric weekly state and one-week lifecycle differ from certified QM5_12567. Verify uniform energy labels, exact Monday anchors, one immediately completed 3-5-session week, every chronological close, exactly one strict interior turn, strict monotone legs, final close beyond the first close in the recovery direction, one attempt, fixed risk, and next-week exit. Q09 alone may establish realized decorrelation."
...
g0_approval_reasoning: "OWNER-authorized second-XNG structural sleeve; R1 two complete-read peer-reviewed commodity sources with natural-gas membership and weekly translation disclosed; R2 exact path, attempt, risk and lifecycle; R3 native XNG D1; R4 deterministic, no banned or trained logic"
---

# QM5_41100 XNG Completed-Week Close-Turn Recovery Momentum

## Hypothesis

A completed XNG broker week can contain an internal reversal without ending
...
The symmetric weekly close-path state, boundary clock, and full-week hold are
mechanically different from certified `QM5_12567`'s long-only two-day XNG
oscillator pullback. This is a diversification hypothesis only: the shared
carrier remains a material overlap risk, Q02 owns frequency and baseline
...
`decisions/2026-08-22_xng_weekly_close_turn_momentum_source_approval.md` at
commit `e0fd6935a`. The bounded packet SHA-256 is
`E040B331076E4AA7E8DD3F223831DCCC15585AA120CA943F5A523A53A5A1882C`.
...
include natural gas in their futures universe. Neither paper tests a within-week
...
Before allocation, the canonical checker scanned 4,589 registry identities,
1,268 repository cards, and 45 Strategy-Wiki nodes. It returned the expected
fuzzy hit on exact WTI carrier sibling
`QM5_41099_wti-wclose-turn-mom`, with no exact candidate identity. The current
OWNER mission explicitly permits a second XNG strategy when its logic differs
from `QM5_12567`, and the established `QM5_41080`/`QM5_41081` precedent gives
separate governed identities to predeclared WTI and XNG carrier
falsifications. No WTI result transfers. Manual family review fixes the
mechanical boundaries:

- certified `QM5_12567_cum-rsi2-commodity` uses a long-only two-day
  cumulative-RSI2 pullback below a slow mean with a five-bar maximum hold.
  This card has no oscillator or slow mean, is symmetric, evaluates one
  completed week, and holds to the next weekly boundary.
- `QM5_41081_xng-wclose-location-mom` uses two completed weekly packages, a
  ...
  location. This card reads only one week's chronological closes, never highs
  or lows, and requires a strict single-turn/full-recovery path.
- `QM5_41094_xng-wbody-dominance-mom` compares aggregate weekly body size with
  high-low range. This card reads neither the weekly open nor intraday
  extremes and imposes no body-share threshold.
- `QM5_41067_xng-wflip-mom` classifies two adjacent week-end return signs.
  This card uses the within-week close path of one package and reads no older
  weekly return.
- `QM5_41063_xng-week-nr7-brk` ranks seven completed weekly ranges and waits
  for a current-week breakout. This card ranks nothing and excludes every
  current-week price from the signal.
- `QM5_41099_wti-wclose-turn-mom` is the exact WTI carrier sibling. It is the
  expected fuzzy match and provides no XNG efficacy, cost, density, basis, or
  correlation prior.
...
Verdict:
`NO_EXACT_XNG_WEEKLY_CLOSE_TURN_RECOVERY_MOMENTUM_DUPLICATE_AFTER_FAMILY_REVIEW`.

## Markets, Timeframe, And Cadence

- Host and target: exact `XNGUSD.DWX`.
- Timeframe: exact D1; magic slot 0; planned magic `411000000`.
...
2. Require exact `XNGUSD.DWX`, D1, EA ID `41100`, slot zero, fixed-risk mode,
...
- Own at most one `XNGUSD.DWX` position under magic `411000000`.
...
Ooi, and Pedersen supply own-return continuation and XNG membership. Neither
source supplies the weekly horizon, close-path shape, or recovery condition.

## QM Interpretations

`BIANCHI-MOP-XNG-WCLOSE-TURN-MOM-2026_S01` fixes the weekly horizon, one
...
Exact `XNGUSD.DWX` native D1 closes and timestamps, broker time, symbol
...
|---|---|---|
| R1 | PASS_WITH_WEEKLY_CLOSE_PATH_TRANSLATION_RISK | Two named-author, peer-reviewed DOI papers with complete-read evidence and explicit XNG membership; weekly close-path translation risk disclosed. |
...
| R3 | PASS_WITH_ENERGY_LABEL_AND_CFD_BASIS_RISK | Registered native `XNGUSD.DWX` D1 history and MT5 state provide every runtime field; energy-label and CFD-basis risks remain Q02 falsification items. |
...
Changing the XNG carrier, weekly aggregation, close sequence, strict-turn
...
|---|---|---|---|---|
| v1 | 2026-08-22 | initial XNG completed-week close-turn recovery card | G0 | APPROVED |

## Pipeline Phase Status

| Phase | Date | Status | Evidence |
|---|---|---|---|
| G0 Research Intake | 2026-08-22 | APPROVED | `decisions/2026-08-22_qm5_41100_xng_weekly_close_turn_momentum_g0.md` |
| Q01 Build Validation | 2026-08-22 | PENDING | build not started |
| Q02 Baseline Screening | 2026-08-22 | NOT_QUEUED | Q01 build required first |
