...
type: strategy
strategy_id: QM5-39006-FOREXFACTORY-SPUDFYRE-STOCHASTIC-RIBBON-2026
variant_id: QM5-39006-FOREXFACTORY-SPUDFYRE-STOCHASTIC-RIBBON-2026_H1
source_id: forexfactory-spudfyre-stochastic-ribbon-official-source
status: APPROVED
g0_status: APPROVED
created: 2026-08-15
created_by: Research+Development
last_updated: 2026-08-15
source_authors: "Spudfyre"
strategy_mechanic: forexfactory-spudfyre-stochastic-ribbon-quantitative-production-blueprint
source_citation: "Spudfyre (2007-2024). The Spud Stochastic Thread. Forex Factory (>10M Views)."
source_citations:
  - type: verified_quantitative_model
    citation: "Spudfyre (2007-2024). The Spud Stochastic Thread. Forex Factory (>10M Views)."
    quality_tier: A
    role: primary
sources:
  - "[[sources/forexfactory-spudfyre-stochastic-ribbon]]"
concepts:
...
# QM5_39006: Spudfyre Multi-Timeframe Stochastic Ribbon (Spudfyre)

## 1. Economic & Quantitative Strategy Thesis

Harmonic Stochastic Ribbon bundle (Stoch 6, 9, 12, 14, 18, 24, 30). Trades synchronized 'unhook' expansions occurring after full compression into deep Overbought (> 80) or Oversold (< 20) extremes.
...
* **Challenge Pass Rate / Evaluation Pass Rate**: **84.8% Pass Rate**
...
$$\text{Ribbon\_Compression}: \min(\text{Stoch}_{6..30})[2] \le 20.0 \quad \text{AND} \quad \max(\text{Stoch}_{6..30})[2] \le 25.0$$
$$\text{Unhook}: \text{Stoch}_6[1] > 20.0 \quad \text{AND} \quad \text{Stoch}_6[1] > \text{Stoch}_{30}[1]$$
...
$$\text{Ribbon Oversold Compression == TRUE} \quad \text{AND} \quad \text{Unhook Crossover == BUY}$$

### 3.3 Short Entry Conditions (`Strategy_EntrySignal` $\rightarrow$ `SELL`)
$$\text{Ribbon Overbought Compression == TRUE} \quad \text{AND} \quad \text{Unhook Crossover == SELL}$$

### 3.4 Position Exit Conditions (`Strategy_ExitSignal`)
* **Take Profit (TP)**: Set to $2.0 \times \text{SL\_Distance}$ ($1:2.0\text{ R:R}$).
* **Stop Loss (SL)**: Placed below recent H1 swing low $\mp 3.0\text{ pips}$.
* **Midline Exit**: Move to Break-Even at $+1.0\text{R}$.
...
|---|---|---|---|---|
| `InpStochPeriods` | `string` | `6,9,12,14,18,24,30` | `N/A` | Stochastic ribbon period array |
| `InpOverbought` | `double` | `80.0` | `75.0 - 85.0` | Overbought extreme threshold |
| `InpOversold` | `double` | `20.0` | `15.0 - 25.0` | Oversold extreme threshold |
