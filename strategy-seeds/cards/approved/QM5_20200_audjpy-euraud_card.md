---
strategy_id: AI-CODEX-FX-COINT66-20260609-AUDJPY-EURAUD
ea_id: QM5_20200
slug: audjpy-euraud
type: strategy
status: APPROVED
...
markets: [forex]
timeframes: [D1]
primary_target_symbols: [AUDJPY.DWX, EURAUD.DWX]
target_symbols: [AUDJPY.DWX, EURAUD.DWX]
logical_symbol: QM5_20200_AUDJPY_EURAUD_COINTEGRATION_D1
period: D1
...
# QM5_20200 AUDJPY/EURAUD D1 Cointegration Basket
...
low-frequency daily implementation. Chan makes no claim for AUDJPY/EURAUD.
...
AUDJPY/EURAUD ranks seventeenth of 66 by OOS net Sharpe. Rank 16
EURJPY/EURAUD is already built as `QM5_20199` and has one logical Q02 row
pending. Exact-pair searches found no dedicated
fixed-beta AUDJPY/EURAUD D1 card, EA, or logical basket manifest.

| Pair | DEV net Sharpe | OOS net Sharpe | OOS return | OOS state changes | Fixed DEV beta | Half-life |
|---|---:|---:|---:|---:|---:|---:|
| AUDJPY / EURAUD | 0.808691 | 0.640798 | 10.433804% | 21 | -2.073289367 | 27.702 D1 bars |

The sub-0.8 OOS Sharpe is adverse evidence; the approximately 28-bar half-life
still implies multi-week holding and material swap exposure. This card
authorizes one low-frequency frontier test, not a certified edge. A terminal
economic failure retires the sleeve; it does not authorize a filter, beta
refit, or parameter rescue.

## Non-Duplicate Boundary

Broad multi-symbol AUD strength systems and conversion-history declarations
may mention both symbols, but they do not trade this fixed pair, beta, daily
residual, and logical package. Sibling fixed-scan sleeves use different
instruments, residuals, magics, and logical identities.

## 2. Concept

Trade temporary deviations in the fixed log-price residual
`ln(AUDJPY) - (-2.073289367) * ln(EURAUD)`. The negative fitted coefficient
...
The pair shares AUD in opposite quotation positions and retains EUR, JPY,
carry, and risk-sentiment exposure. That sign-sensitive structure and the
negative beta are explicit high-risk caveats rather than permission to add a
filter.

## Hypothesis

The fixed scan suggests large deviations in the AUDJPY/EURAUD residual may
...
`AUDJPY.DWX` is the tester host and first traded leg. `EURAUD.DWX` is the
companion traded leg. The three conversion-only histories are selected and
warmed for JPY/AUD P/L and EUR margin under the USD tester account; they
receive no order and no magic slot.
...
- Compute `spread = ln(AUDJPY) - strategy_beta * ln(EURAUD)`, with frozen
  `strategy_beta = -2.073289367`.
...
- Permit entry only when the host is `AUDJPY.DWX`, the configured period is D1
...
Chan supplies the pair-trading method but reports no AUDJPY/EURAUD result.
...
```yaml
target_modules:
  no_trade: "Select and warm five declared histories; reject wrong host, timeframe, slot, unaligned traded history, or invalid normalized volume."
...
data_requirements: "AUDJPY.DWX, EURAUD.DWX, USDJPY.DWX, AUDUSD.DWX, and EURUSD.DWX D1 histories"
```
...
| v2 | 2026-08-01 | initial sign-aware two-leg implementation | Q01 | PASS |
...
| Q01 Build Validation | 2026-08-01 | PASS | `D:\QM\reports\framework\21\build_check_20260801_201048.json` |
| Q02 Baseline Screening | 2026-08-01 | PENDING | work item `47fc8c32-fe57-4f98-828e-d8cb8d0f4a25` |

## 16. Lessons Captured

- 2026-08-01: Rank 17 is below the original survivor bar; it is admitted only
