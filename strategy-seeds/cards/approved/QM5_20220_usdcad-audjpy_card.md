---
strategy_id: AI-CODEX-FX-COINT66-20260609-USDCAD-AUDJPY
ea_id: QM5_20220
slug: usdcad-audjpy
type: strategy
status: APPROVED
...
markets: [forex]
timeframes: [D1]
primary_target_symbols: [USDCAD.DWX, AUDJPY.DWX]
target_symbols: [USDCAD.DWX, AUDJPY.DWX]
logical_symbol: QM5_20220_USDCAD_AUDJPY_COINTEGRATION_D1
period: D1
...
# QM5_20220 USDCAD/AUDJPY D1 Cointegration Basket
...
low-frequency daily implementation. Chan makes no claim for USDCAD/AUDJPY.
...
USDCAD/AUDJPY ranks forty-second of 66 by OOS net Sharpe. Rank 40 is already
represented by `QM5_20219`, and rank 41 by `QM5_12765`. The deterministic
research dedup check returned CLEAN; exact-pair searches, registry searches,
and unordered traded-symbol manifest reconciliation found no dedicated
fixed-beta USDCAD/AUDJPY D1 card, EA, allocation, or logical basket.

| Pair | DEV net Sharpe | OOS net Sharpe | OOS return | OOS state changes | Fixed DEV beta | Half-life |
|---|---:|---:|---:|---:|---:|---:|
| USDCAD / AUDJPY | 0.285291 | -0.060279 | -0.374459% | 15 | -0.186232670 | 73.380 D1 bars |

The negative OOS Sharpe and return are adverse evidence. The approximately
73-bar half-life also implies long holds with material swap and regime risk.
...
`ln(USDCAD) - (-0.186232670) * ln(AUDJPY)`. The negative fitted coefficient
...
The package retains USD, CAD, AUD, JPY, carry, and broad risk-sentiment
exposure. That sign-sensitive structure and the negative OOS result are
explicit high-risk caveats rather than permission to add a filter or refit.

## Hypothesis

The fixed scan found a positive DEV score but a slightly negative OOS result.
This card tests whether large residual deviations can nevertheless survive
real tick costs and swap in the canonical D1 implementation. The hypothesis is
deliberately weak and falsifiable: economic, cadence, history, or execution
...
`USDCAD.DWX` is the tester host and first traded leg. `AUDJPY.DWX` is the
companion traded leg. `AUDUSD.DWX` and `USDJPY.DWX` are declared and warmed
only for USD-account conversion; they receive no order and no magic slot.
...
- Compute `spread = ln(USDCAD) - strategy_beta * ln(AUDJPY)`, with frozen
  `strategy_beta = -0.186232670`.
...
- Permit entry only when the host is `USDCAD.DWX`, the configured period is D1
  or the supported H1 tester wrapper, and the host magic slot is 0.
- Require both traded symbols and both conversion-only symbols to be selected
  and warm.
...
- Conversion-only symbols receive neither an order nor a magic allocation.
...
Chan supplies the pair-trading method but reports no USDCAD/AUDJPY result.
...
```yaml
target_modules:
  no_trade: "Select and warm both traded histories plus conversion histories; reject wrong host, timeframe, slot, unaligned history, or invalid normalized volume."
...
data_requirements: "USDCAD.DWX, AUDJPY.DWX, AUDUSD.DWX, and USDJPY.DWX D1 histories"
```
