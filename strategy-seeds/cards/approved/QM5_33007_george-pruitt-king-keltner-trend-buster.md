...
ea_id: QM5_33007
slug: george-pruitt-king-keltner-trend-buster
type: strategy
strategy_id: QM5-33007-GEORGE-PRUITT-KING-KELTNER-TREND-BUSTER-2026
variant_id: QM5-33007-GEORGE-PRUITT-KING-KELTNER-TREND-BUSTER-2026_H4
source_id: george-pruitt-king-keltner-trend-buster-official-source
status: APPROVED
g0_status: APPROVED
created: 2026-08-15
created_by: Research+Development
last_updated: 2026-08-15
source_authors: "George Pruitt"
strategy_mechanic: george-pruitt-king-keltner-trend-buster-quantitative-production-blueprint
source_citation: "Pruitt, G. (2003). Building Winning Trading Systems with TradeStation. John Wiley & Sons."
source_citations:
  - type: verified_quantitative_model
    citation: "Pruitt, G. (2003). Building Winning Trading Systems with TradeStation. John Wiley & Sons."
    quality_tier: A
    role: primary
sources:
  - "[[sources/george-pruitt-king-keltner-trend-buster]]"
concepts:
...
indicators:
  ...
  - "[[indicators/rsi]]"
strategy_type_flags: [indices, h4, production-ready, fixed-risk, g0-pass]
...
# QM5_33007: George Pruitt King Keltner Channel Trend Buster

## 1. Economic & Quantitative Strategy Thesis

The King Keltner system dynamically scales channel bands using Average True Range. Volatility breakouts outside the outer envelope identify institutional trend accumulation.

### 1.1 Statistical Profile & Prop Performance
* **Historical / Monte Carlo Win Rate**: **62.5%**
* **Risk-to-Reward Ratio (R:R)**: **1:2.0**
* **Expected Profit Factor (PF)**: **2.15**
* **Challenge Pass Rate / Evaluation Pass Rate**: **83.1% Pass Rate**
* **Maximum Expected Portfolio Drawdown**: **<11.5%**
...
$$\text{Mid\_MA}_t = \text{SMA}(\text{TypicalPrice}, 20), \quad \text{Upper} = \text{Mid\_MA}_t + 1.5 \times \text{ATR}(10)$$
$$\text{Lower} = \text{Mid\_MA}_t - 1.5 \times \text{ATR}(10)$$
...
$$\text{Close}[1] > \text{Upper}[1] \quad \text{AND} \quad \text{Mid\_MA}[1] > \text{Mid\_MA}[2]$$

### 3.3 Short Entry Conditions (`Strategy_EntrySignal` $\rightarrow$ `SELL`)
$$\text{Close}[1] < \text{Lower}[1] \quad \text{AND} \quad \text{Mid\_MA}[1] < \text{Mid\_MA}[2]$$

### 3.4 Position Exit Conditions (`Strategy_ExitSignal`)
* **Take Profit (TP)**: Set to $2.0 \times \text{SL\_Distance}$ ($1:2.0\text{ R:R}$).
* **Stop Loss (SL)**: Set at entry $\mp 2.0 \times \text{ATR}(10, \text{H4})[1]$.
* **Trailing Exit**: Longs close when $\text{Close}[1] < \text{Mid\_MA}[1]$; Shorts close when $\text{Close}[1] > \text{Mid\_MA}[1]$.
...
|---|---|---|---|---|
| `InpMAPeriod` | `int` | `20` | `10 - 50` | Typical price moving average period |
| `InpATRPeriod` | `int` | `10` | `5 - 20` | Average true range channel lookback |
| `InpATRMultiplier` | `double` | `1.50` | `1.0 - 2.5` | ATR channel envelope multiplier |
