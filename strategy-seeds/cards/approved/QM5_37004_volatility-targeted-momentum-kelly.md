...
type: strategy
strategy_id: QM5-37004-VOLATILITY-TARGETED-MOMENTUM-KELLY-2026
variant_id: QM5-37004-VOLATILITY-TARGETED-MOMENTUM-KELLY-2026_D1
source_id: volatility-targeted-momentum-kelly-official-source
status: APPROVED
g0_status: APPROVED
created: 2026-08-15
created_by: Research+Development
last_updated: 2026-08-15
source_authors: "Marcos López de Prado"
strategy_mechanic: volatility-targeted-momentum-kelly-quantitative-production-blueprint
source_citation: "López de Prado, M. (2018). Advances in Financial Machine Learning. Backtrader Engine."
source_citations:
  - type: verified_quantitative_model
    citation: "López de Prado, M. (2018). Advances in Financial Machine Learning. Backtrader Engine."
    quality_tier: A
    role: primary
sources:
  - "[[sources/volatility-targeted-momentum-kelly]]"
concepts:
...
target_symbols: [SP500.DWX, NDX.DWX, XTIUSD.DWX, XAUUSD.DWX]
...
# QM5_37004: Volatility-Targeted Momentum with Half-Kelly Sizing

## 1. Economic & Quantitative Strategy Thesis

Inverse-volatility position scaling pinned to an exact 10.0% annualized portfolio risk target. Combines 12-month exponential momentum with half-Kelly bet sizing to maximize compounding Sharpe ratio.
...
$$\sigma_{\text{ann}} = \sigma_{20} \times \sqrt{252}, \quad w_i = \frac{\sigma_{\text{target}}}{\sigma_{\text{ann}, i}} \times \left(0.5 \times \frac{p - (1-p)/b}{1}\right)$$
...
$$\text{Momentum}_{12\text{M}}[1] > 0 \quad \text{AND} \quad \text{Close}[1] > \text{SMA}(200, \text{D1})[1]$$

### 3.3 Short Entry Conditions (`Strategy_EntrySignal` $\rightarrow$ `SELL`)
$$\text{Momentum}_{12\text{M}}[1] < 0 \quad \text{AND} \quad \text{Close}[1] < \text{SMA}(200, \text{D1})[1]$$

### 3.4 Position Exit Conditions (`Strategy_ExitSignal`)
* **Take Profit (TP)**: Trailing Chandelier ATR Stop ($3.0 \times \text{ATR}(14, \text{D1})$).
...
|---|---|---|---|---|
| `InpVolTargetPct` | `double` | `10.0` | `5.0 - 15.0` | Target annualized volatility percent |
| `InpMomentumDays` | `int` | `252` | `100 - 300` | Momentum lookback in trading days |
| `InpKellyFraction`| `double` | `0.50` | `0.25 - 0.75` | Fractional Kelly sizing multiplier |
| `InpRiskPercent` | `double` | `0.50` | `0.20 - 1.00` | Base capital risk percent |
...
* **Target symbols**: SP500.DWX, NDX.DWX, XTIUSD.DWX, XAUUSD.DWX
