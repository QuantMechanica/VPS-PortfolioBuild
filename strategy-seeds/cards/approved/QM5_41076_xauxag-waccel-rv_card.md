...
type: strategy
strategy_id: SCHWEIKERT-CME-XAUXAG-WACCEL-RV-2026_S01
variant_id: SCHWEIKERT-CME-XAUXAG-WACCEL-RV-2026_S01
source_id: SCHWEIKERT-CME-XAUXAG-WACCEL-RV-2026
ea_id: QM5_41076
slug: xauxag-waccel-rv
status: APPROVED
execution_contract_ref: strategy-seeds/cards/approved/QM5_41076_xauxag-waccel-rv_card.md
execution_contract_status: APPROVED
created: 2026-08-20
created_by: Research+Development
last_updated: 2026-08-20
g0_status: APPROVED
g0_decision: decisions/2026-08-20_qm5_41076_xauxag_weekly_acceleration_overshoot_reversion_g0.md
source_approval: decisions/2026-08-20_xauxag_weekly_acceleration_overshoot_reversion_source_approval.md
...
strategy_mechanic: synchronized-completed-two-adjacent-week-xau-minus-xag-relative-returns-same-sign-newest-strictly-larger-fade-newest-direction-one-week-equal-notional-basket
sources:
  - "[[sources/SCHWEIKERT-CME-XAUXAG-WACCEL-RV-2026]]"
concepts:
  - "[[concepts/gold-silver-relative-value]]"
  - "[[concepts/weekly-acceleration-overshoot-reversion]]"
  - "[[concepts/market-neutral-basket]]"
indicators:
  - "[[indicators/adjacent-completed-week-relative-log-returns]]"
  - "[[indicators/atr-risk-stop]]"
strategy_type_flags: [commodity, precious-metals, gold-silver-ratio, market-neutral-basket, weekly-acceleration-overshoot, mean-reversion, symmetric-long-short, atr-hard-stop, time-stop, low-frequency]
markets: [commodities, precious_metals]
timeframes: [D1]
target_symbols: [XAUUSD.DWX, XAGUSD.DWX]
primary_target_symbols: [XAUUSD.DWX, XAGUSD.DWX]
single_symbol_only: false
logical_symbol: QM5_41076_XAU_XAG_WACCEL_RV_D1
symbol: QM5_41076_XAU_XAG_WACCEL_RV_D1
host_symbol: XAUUSD.DWX
companion_symbol: XAGUSD.DWX
symbol_slots: [0, 1]
magic_numbers: [410760000, 410760001]
period: D1
timeframe: D1
expected_trade_frequency: "Approximately eight to eighteen completed paired packages per full post-warm-up year after exact synchronized week, strict sign agreement, strict newest acceleration, and execution gates; Q02 must prove at least five/year or retire."
expected_trades_per_year_per_symbol: 13
expected_pf: 1.01
expected_dd_pct: 30.0
risk_class: high
ml_required: false
r1_track_record: PASS_WITH_WEEKLY_ACCELERATION_TRANSLATION_RISK
r2_mechanical: PASS
...
hard_rules_at_risk: [exact_xau_xag_carrier, synchronized_week_endpoints, consecutive_monday_anchors, nonoverlapping_relative_returns, strict_sign_agreement, strict_newest_absolute_acceleration, inverse_shared_return_basket_direction, persistent_week_attempt, equal_notional_basket, aggregate_fixed_risk, hard_stops_present, atomic_package_repair, next_week_exit, risk_mode_dual, q02_frequency_floor, portfolio_correlation]
g0_approval_reasoning: "R1 uses one bounded child source with named peer-reviewed DOI and official exchange lineages while disclosing the weekly acceleration-overshoot fade as an untested QM translation; R2 locks exact synchronized weeks, chronological relative returns, strict sign agreement, strict newest acceleration, inverse-shared-return side, durable attempt, aggregate fixed risk, equal notional, hard stops, spreads, and next-week lifecycle; R3 uses registered native XAU/XAG D1 histories with synchronization and CFD-basis risks explicit and requires active slots 0/1 before build; R4 is deterministic timestamp, price, logarithm, ATR, quote, position, deal, and terminal-state arithmetic without a banned signal, trained output, external feed, grid, or martingale; canonical dedup and manual family review found no exact identity."
---

# QM5_41076 XAU/XAG Completed-Week Acceleration-Overshoot Reversion

## Hypothesis

When the gold/silver log ratio moves in the same direction for two completed
broker weeks and the newest move is strictly larger, the accelerating relative
move may have overshot. Fading that shared direction for the next broker week
may capture convergence without taking an outright single-metal signal.
...
`decisions/2026-08-20_xauxag_weekly_acceleration_overshoot_reversion_source_approval.md`
at commit `9d8585d00`.
...
source tests adjacent same-sign weekly returns, strict newest-magnitude
acceleration, this contrarian direction, a continuous-CFD package, equal-
...
The canonical pre-allocation checker scanned 4,563 registry rows and 625 root
...
- `QM5_41066_xauxag-wdecay-rv` uses two strict same-sign weekly relative
  ...
  the same strict sign with a strictly larger newest move, so the states are
  mutually exclusive.
- `QM5_41075_xauxag-wovershoot-rv` requires opposite weekly signs before
  fading a dominant newest reversal. This card requires two same-sign weeks,
  so the states are mutually exclusive.
- `QM5_20275_gsr-runfade` uses five consecutive same-sign D1 relative returns
  ...
  exactly two non-overlapping completed broker weeks and enters only after a
  exactly two accelerating completed weekly moves before entry.
...
- Exact host: `XAUUSD.DWX`, D1, slot 0, magic `410760000`.
- Exact companion: `XAGUSD.DWX`, D1, slot 1, magic `410760001`.
...
- Signal: two strict same-sign weekly relative returns with strictly
...
1. Evaluate only once on a new exact `XAUUSD.DWX` D1 bar under EA 41076 and
...
6. Compute `s1`, `s2`, `s3`, `r_new`, and `r_old` exactly as specified. Require
   both returns finite and non-zero, the same strict sign, and
   `abs(r_new)>abs(r_old)`. Opposed signs, either zero, equality, or a smaller
   newest move remains flat.
7. SELL XAU/BUY XAG only when `r_old>0`, `r_new>0`, and the newest move
   strictly accelerates. BUY XAU/SELL XAG only when `r_old<0`, `r_new<0`, and
   the newest move strictly accelerates. Signal magnitude never changes risk.
...
- Exact host, D1, EA 41076, slot zero, and both registered magics.
...
- First-week-bar clock, synchronized timestamps, three consecutive week
  anchors, positive finite prices, strict sign agreement, strict newest
  acceleration, durable attempt, side-specific trade mode, spread, quote, ATR,
...
- Own exactly one `XAUUSD.DWX` position under magic `410760000` and one
  opposite-side `XAGUSD.DWX` position under magic `410760001`.
...
source supplies the weekly horizon or acceleration-overshoot fade.

## QM Interpretations

`SCHWEIKERT-CME-XAUXAG-WACCEL-RV-2026_S01` fixes the exact weekly endpoint
construction, two-return count, strict state, inverse-shared-return sides,
...
intervals, current-week leakage, opposed signs, zero, equality, or non-
accelerating entry, wrong inverse-shared-return side, duplicate attempt,
one-leg survivor,
...
Changing the carrier, endpoint count, weekly horizon, sign or acceleration
...
non-overlapping relative returns; both strict same-sign newest-acceleration
directions; opposed signs, zero, equality, and non-acceleration flat states; no
...
| Q01 Build Validation | 2026-08-20 | PENDING | build not started |
| Q02 Baseline Screening | 2026-08-20 | NOT_ENQUEUED | Q01 required first |
