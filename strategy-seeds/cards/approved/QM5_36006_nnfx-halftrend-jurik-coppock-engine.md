...
type: strategy
strategy_id: QM5-36006-NNFX-HALFTREND-JURIK-COPPOCK-ENGINE-2026
variant_id: QM5-36006-NNFX-HALFTREND-JURIK-COPPOCK-ENGINE-2026_D1
source_id: nnfx-halftrend-jurik-coppock-engine-official-source
status: APPROVED
g0_status: APPROVED
created: 2026-08-15
...
strategy_mechanic: nnfx-halftrend-jurik-coppock-engine-quantitative-production-blueprint
source_citation: "No Nonsense Forex Non-Lagging Volatility Algorithms."
source_citations:
  - type: verified_quantitative_model
    citation: "No Nonsense Forex Non-Lagging Volatility Algorithms."
    quality_tier: A
    role: primary
sources:
  - "[[sources/nnfx-halftrend-jurik-coppock-engine]]"
concepts:
...
# QM5_36006: NNFX HalfTrend & Jurik Velocity Engine

## 1. Economic & Quantitative Strategy Thesis

HalfTrend (non-repainting ATR step baseline) paired with Jurik Velocity (JMA-filtered momentum C1), Coppock Curve C2, and Chaikin Money Flow (CMF 20) volume confirmation.
...
* **Maximum Expected Portfolio Drawdown**: **<4.1%**
...
$$\text{HalfTrend} = \text{EMA}(P, 2) \pm 2.0 \times \text{ATR}(100), \quad \text{JurikVelocity} = \Delta \text{JMA}(P, 14)$$
...
$$\text{Close}[1] > \text{HalfTrend}[1] \quad \text{AND} \quad \text{JurikVel}[1] > 0 \quad \text{AND} \quad \text{Coppock}[1] > 0 \quad \text{AND} \quad \text{CMF}(20)[1] > +0.05$$
...
* **Runner Exit**: HalfTrend direction flip.
...
|---|---|---|---|---|
| `InpHalfTrendAmp` | `int` | `2` | `1 - 4` | HalfTrend amplitude setting |
| `InpJurikPeriod` | `int` | `14` | `8 - 21` | Jurik JMA smoothing period |
| `InpCMFPeriod` | `int` | `20` | `14 - 30` | Chaikin Money Flow lookback |
