...
type: strategy
strategy_id: MOP-WTI-WCLOSE-LOCATION-MOM-2026_S01
variant_id: MOP-WTI-WCLOSE-LOCATION-MOM-2026_S01
source_id: MOP-WTI-WCLOSE-LOCATION-MOM-2026
ea_id: QM5_41080
slug: wti-wclose-location-mom
status: APPROVED
execution_contract_ref: strategy-seeds/cards/approved/QM5_41080_wti-wclose-location-mom_card.md
execution_contract_status: APPROVED
created: 2026-08-21
created_by: Research+Development
last_updated: 2026-08-21
g0_status: APPROVED
g0_decision: decisions/2026-08-21_qm5_41080_wti_completed_week_close_location_momentum_g0.md
source_approval: decisions/2026-08-21_wti_completed_week_close_location_momentum_source_approval.md
...
source_citations:
  - type: academic_paper
    ...
    location: "DOI 10.1016/j.jfineco.2011.11.003; complete-read packet strategy-seeds/sources/MOP-TSMOM-2012/source.md; bounded translation strategy-seeds/sources/MOP-WTI-WCLOSE-LOCATION-MOM-2026/source.md"
    quality_tier: A
    role: own_return_sign_continuation_and_wti_carrier_lineage
strategy_mechanic: normalized-week-boundary-wti-two-consecutive-completed-weekly-packages-parent-close-to-new-close-strict-return-sign-confirmed-by-newest-week-own-high-low-strict-close-location-outer-fifth-continuation-one-week-hold
sources:
  - "[[sources/MOP-WTI-WCLOSE-LOCATION-MOM-2026]]"
concepts:
...
strategy_type_flags: [commodity, energy, wti-crude, time-series-momentum, completed-week-close-location, symmetric-long-short, atr-hard-stop, time-stop, low-frequency]
...
period: D1
timeframe: D1
expected_trade_frequency: "Approximately 10-25 completed WTI positions per full post-warm-up year after strict history, close-location, and execution gates; Q02 must prove at least five/year or retire."
...
g0_approval_reasoning: "OWNER-authorized build; R1 named-author peer-reviewed source with complete-read evidence and weekly close-location translation risk disclosed; R2 exact clock, anchors, OHLC aggregation, endpoints, return sign, close-location thresholds, side, attempt, risk, and lifecycle; R3 registered native WTI D1 only; R4 deterministic arithmetic without banned signal or trained logic; canonical dedup CLEAN and manual family review separated exact weekday segments, outside-parent settlement, multi-week sign/magnitude paths, current-week breakouts, and rolling volatility-gated momentum."
---

# QM5_41080 WTI Completed-Week Close-Location Momentum

## Hypothesis

The sign of WTI's immediately completed broker-week return may persist into
the next week when that week also settles near the matching edge of its own
realized high-low range. At the first tradable bar of the next week, the
strategy follows a positive return only after a strict upper-fifth close and
follows a negative return only after a strict lower-fifth close.

The source establishes broad own-return continuation and WTI membership, not
this weekly close-location condition, standalone continuous-CFD result, or
...
`strategy-seeds/sources/MOP-WTI-WCLOSE-LOCATION-MOM-2026/source.md`, approved
...
test weekly WTI, a completed-week close location, `0.80` / `0.20` thresholds,
continuous-CFD weekly packages, fixed-dollar ATR risk, or the QM book. Every
weekly clock, range-state, execution, and risk choice below is a declared QM
interpretation.

## Non-Duplicate Decision

The canonical pre-allocation checker included author and mechanic fields,
scanned 4,567 registry rows and 625 root cards, and returned `CLEAN`, with no
exact or fuzzy match. Manual review fixes the load-bearing boundaries:

- `QM5_41020_wti-wclose-mom` forms on exact prior Tuesday and Friday closes,
  enters Monday, exits Wednesday, and never aggregates a completed weekly
  high-low range. This card uses full completed weekly packages and owns the
  complete next week.
- `QM5_41073_wti-woutside-settle` requires the newest weekly range to be
  strictly outside its parent's range, its close beyond the parent extreme,
  and its open-to-close direction aligned. This card does not inspect parent
  highs/lows or the newest open; it confirms strict parent-close-to-new-close
  sign with the newest week's own range position.
- `QM5_41065` through `QM5_41074` otherwise classify multi-week return signs,
  ...
  completed week's own high-low close location.
- `QM5_41061_wti-week-nr7-brk` and `QM5_12965_wti-week-orb` wait for a
  current-week price break after compression or an opening-range state. This
  card excludes all current-week price from the signal and enters only at the
  boundary.
- `QM5_13049_xti-1w-mom-vol` uses a rolling five-D1 magnitude threshold and a
  volatility-rank gate with a different decision clock and lifecycle.
- `QM5_12567_cum-rsi2-commodity` is a long-only two-day oscillator pullback,
  not symmetric weekly WTI continuation.

Verdict:
`CLEAN_WTI_COMPLETED_WEEK_RETURN_SIGN_WITH_OWN_RANGE_CLOSE_LOCATION_CONFIRMATION_AFTER_FAMILY_REVIEW`.

## Markets, Timeframe, And Cadence

- Host and target: exact `XTIUSD.DWX`.
- Timeframe: exact D1; magic slot 0; magic `410800000`.
...
1. Evaluate only once on a new exact `XTIUSD.DWX` D1 bar under EA 41080 and
   magic slot zero.
...
8. Aggregate newest-week `H0=max(high)`, `L0=min(low)`, and `C0=chronologically
   final close`; select parent `C1=chronologically final close`. Compute `r`
   and `clv` exactly as above and require both finite.
...
- Exact host, D1, EA 41080, slot zero, and registered magic.
...
- Own at most one `XTIUSD.DWX` position under magic `410800000`.
...
membership. They do not supply the weekly horizon or range-position gate.

## QM Interpretations

`MOP-WTI-WCLOSE-LOCATION-MOM-2026_S01` fixes the weekly horizon, completed
...
Changing the WTI carrier, weekly packages, return interval, either
close-location threshold, direction, attempt clock, risk, stop, or lifecycle
