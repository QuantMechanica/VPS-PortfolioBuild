...
indicators:
  - rolling-zscore
  - atr-stop
target_symbols: [GBPJPY.DWX, AUDJPY.DWX]
...
g0_approval_reasoning: "R1 PASS Chan cointegration method plus OWNER-requested in-house 66-pair FX scan; R2 PASS deterministic fixed-pair z-score basket; R3 PASS GBPJPY.DWX and AUDJPY.DWX data exist in scan universe; R4 PASS no ML/grid/martingale."
expected_pf: 0.98
expected_dd_pct: 30.0
portfolio_scope: basket
---

# Edge Lab GBPJPY/AUDJPY Cointegration Basket
...
This card is the next unbuilt rank-23 tail candidate by OOS net Sharpe:
GBPJPY/AUDJPY had DEV Sharpe 0.5062, OOS net Sharpe -0.1777, OOS return
-1.3359%, 19 OOS state changes, hedge 0.725275, and 38.85-day half-life in the
same rerun.

## Concept

GBPJPY and AUDJPY share the JPY quote side but differ by GBP and AUD base-side
...
Temporary dislocations between the GBPJPY and AUDJPY D1 log-price spread can
...
- The EA trades only the fixed GBPJPY/AUDJPY basket; it does not reselect pairs
...
- Compute `spread = ln(GBPJPY) - 0.725275 * ln(AUDJPY)`.
- Compute a 60-bar rolling z-score of the spread.
- If no pair package is open and z > +2.0, open a short-spread package: short GBPJPY, long AUDJPY.
- If no pair package is open and z < -2.0, open a long-spread package: long GBPJPY, short AUDJPY.
...
- Host chart must be GBPJPY.DWX or AUDJPY.DWX on D1/H1, with slot 0 used for the logical host.
...
- name: strategy_beta
  default: 0.725275
  sweep_range: [0.55, 0.725275, 0.90]
...
No external performance claim is taken from Chan for GBPJPY/AUDJPY specifically.
The in-house scan rerun found DEV Sharpe 0.5062 and OOS net Sharpe -0.1777
...
- risk_class: very high because this is a rank-23 negative-OOS tail candidate after the rank-22 EURUSD/EURGBP exploratory basket.
- gridding: false.
- scalping: false.
- ml_required: false.
...
- [x] R3 testable: GBPJPY.DWX and AUDJPY.DWX are Darwinex-native `.DWX` symbols in the exported scan data.
...
|---|---|---|---|---|
| v1 | 2026-06-29 | initial rank-23 next-unbuilt FX cointegration basket card | G0 | APPROVED |
