...
- Require synchronized bars, valid symbol metadata, valid ATR and acceptable
  spreads before any entry.
- Framework news policy gates entries only; framework kill switch remains
  authoritative for both registered magics.
...
- One Monday-only logical package: SELL XTI and BUY XNG together.
- Close the complete package at the next D1 boundary; never trade either leg
  alone and never retry within the same broker week.
- Target 1:1 absolute USD notionals while keeping the two frozen ATR stops
  inside one combined fixed-risk budget.
- Treat any orphan, duplicate, same-direction or materially unbalanced pair as
  malformed and close all owned exposure.
...
## 10. Initial Risk Profile And Kill Criteria
...
Q02 and research use exactly `RISK_FIXED=1000`, `RISK_PERCENT=0`, and
`PORTFOLIO_WEIGHT=1` for the logical two-leg package. Live risk, deployment
and portfolio admission are not authorized.
...
## 13. Implementation And Safety Boundary
