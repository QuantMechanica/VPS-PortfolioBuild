---
ea_id: QM5_13076
slug: xti-nzdcad-rspr
type: strategy
strategy_id: EIA-BOC-RBNZ-XTI-NZDCAD-2026
source_id: EIA-BOC-RBNZ-XTI-NZDCAD-2026
source_citation: "EIA oil/exchange-rate working paper plus official Bank of Canada commodity-CAD note and RBNZ commodity-price monetary-policy context."
source_citations:
  - type: government_research
    citation: "Beckmann, Czudaj, and Arora. The Relationship between Oil Prices and Exchange Rates. U.S. Energy Information Administration Working Paper, 2017."
    location: "https://www.eia.gov/workingpapers/pdf/oil_exchangerates_61317.pdf"
    quality_tier: A
    role: primary
  - type: central_bank_research
    citation: "Bank of Canada Staff Analytical Note 2017-1. The Link Between the Canadian Dollar and Commodity Prices: Has It Broken?"
    location: "https://www.bankofcanada.ca/2017/02/staff-analytical-note-2017-1/"
    quality_tier: A
    role: cad_channel
  - type: central_bank_research
    citation: "Reserve Bank of New Zealand. Commodity prices and implications for monetary policy. 2011."
    location: "https://www.rbnz.govt.nz/research-and-publications/research/our-research-and-analysis/additional-research/commodity-prices-and-implications-for-monetary-policy"
    quality_tier: A
    role: nzd_commodity_channel
sources:
  - "[[sources/EIA-BOC-RBNZ-XTI-NZDCAD-2026]]"
concepts:
  - "[[concepts/oil-fx-relative-value]]"
  - "[[concepts/market-neutral-basket]]"
  - "[[concepts/energy-sleeve]]"
indicators:
  - "[[indicators/rolling-return-spread]]"
  - "[[indicators/zscore]]"
  - "[[indicators/atr]]"
strategy_type_flags: [oil-fx-return-spread, market-neutral-basket, zscore-reversion, atr-hard-stop, time-stop, low-frequency, energy]
target_symbols: [XTIUSD.DWX, NZDCAD.DWX]
basket_symbols: [XTIUSD.DWX, NZDCAD.DWX]
markets: [XTIUSD.DWX, NZDCAD.DWX]
primary_target_symbols: [XTIUSD.DWX, NZDCAD.DWX]
single_symbol_only: false
logical_symbol: QM5_13076_XTI_NZDCAD_RSPREAD_D1
period: D1
timeframes: [D1]
expected_trade_frequency: "D1 XTI/NZDCAD return-spread z-score reversion; estimate 8-14 paired packages/year before Q02 validates synchronized history and fills."
expected_trades_per_year_per_symbol: 10
g0_status: APPROVED
status: APPROVED
r1_track_record: PASS
r2_mechanical: PASS
r3_data_available: PASS
r4_ml_forbidden: PASS
pipeline_phase: Q02
last_updated: 2026-07-09
expected_pf: 1.08
expected_dd_pct: 20.0
risk_class: medium-high
ml_required: false
modules_used: [no_trade, trade_entry, trade_management, trade_close]
target_modules: [Strategy_NoTradeFilter, Strategy_EntrySignal, Strategy_ManageOpenPosition, Strategy_ExitSignal, Strategy_NewsFilterHook]
hard_rules_at_risk: [symbol_history_sufficiency, basket_execution, friday_close, magic_schema, risk_mode_dual]
g0_approval_reasoning: "Mission-directed G0 approval 2026-07-09: R1 PASS official EIA oil/exchange-rate research plus Bank of Canada CAD commodity-channel and RBNZ NZ commodity-channel support; R2 PASS deterministic D1 two-leg XTI/NZDCAD return-spread z-score reversion with spread caps, mean exit, max-hold exit, and ATR hard stops; R3 PASS XTIUSD.DWX and NZDCAD.DWX exist in the DWX matrix; R4 PASS no ML/grid/martingale/external runtime data. Non-duplicate because this is a paired WTI/NZDCAD inverse-CAD return-spread basket, not WTI/USDCAD, XTI/NZDUSD, XTI/CADJPY, XTI/CADCHF, XTI/AUDCAD, XTI/GBPCAD, XTI/EURCAD, XTI/AUDJPY, XTI/AUDUSD, XTI/XNG, Brent/WTI, metal-ratio, XNG, or index logic."
---

# XTI/NZDCAD D1 Return-Spread Reversion

## Source

- Source: [[sources/EIA-BOC-RBNZ-XTI-NZDCAD-2026]]
- Primary citation: Beckmann, Czudaj, and Arora, "The Relationship between Oil
  Prices and Exchange Rates", U.S. Energy Information Administration Working
  Paper, 2017.
- Support: Bank of Canada Staff Analytical Note 2017-1 for the commodity/CAD
  channel; Reserve Bank of New Zealand commodity-price research for the New
  Zealand commodity-exporter terms-of-trade channel.

## Concept

Oil and CAD have a structural macro relationship, but this card deliberately
does not add another USDCAD or CADJPY sleeve. It uses `NZDCAD.DWX`, where CAD
strength normally pushes the cross lower, to build an inverse-CAD relative-value
package against WTI:

`return_spread = ln(XTI[t] / XTI[t-L]) + beta_nzdcad * ln(NZDCAD[t] / NZDCAD[t-L])`

If WTI rallies while NZDCAD also rises, CAD is not confirming the crude move;
the basket fades the high return spread by selling both WTI and NZDCAD. If WTI
falls while NZDCAD also falls, CAD strength has outpaced crude; the basket buys
both WTI and NZDCAD. The bet is a temporary return-spread dislocation between
the crude leg and a direct NZD/CAD commodity-FX leg.

This is deliberately different from:

- `QM5_12609_wti-cad-spread-mr` and `QM5_12722_wti-cad-brk`: those use USDCAD,
  not NZDCAD, and different spread definitions.
- `QM5_13006_xti-nzd-rspread`: that uses NZDUSD, not the direct CAD cross.
- `QM5_13010_xti-cadjpy-rspr`, `QM5_13011_xti-cadchf-rspr`,
  `QM5_13034_xti-audcad-rspr`, `QM5_13057_xti-gbpcad-rspr`,
  `QM5_13060_xti-eurcad-rspr`, `QM5_13059_xti-audjpy-rspr`, and
  `QM5_13073_xti-audusd-rspr`: all use different FX legs or quotation logic.
- Brent/WTI, XTI/XNG, oil/gold, oil/silver, XAU/XAG, XNG, index, and outright
  WTI calendar/event sleeves: this is a two-leg WTI/NZDCAD return-spread
  basket.
- `QM5_12567_cum-rsi2-commodity`: no RSI, oscillator, short-horizon pullback,
  or single-symbol commodity reversal logic is used.

## Markets And Timeframe

- Logical symbol: `QM5_13076_XTI_NZDCAD_RSPREAD_D1`.
- Host symbol: `XTIUSD.DWX`.
- Basket legs: `XTIUSD.DWX` and `NZDCAD.DWX`.
- Period: D1.
- Expected trade frequency: about 8-14 paired packages/year before Q02.
- Backtest risk mode: `RISK_FIXED`.
- Runtime data: Darwinex MT5 D1 OHLC, spread metadata, ATR, broker time, and
  V5 framework state only. No EIA, Bank of Canada, RBNZ, futures-curve, CFTC,
  macro CSV, API, analyst forecast, or ML model is consumed at runtime.

## Entry Rules

- Evaluate only on a new D1 bar of the `XTIUSD.DWX` host chart.
- Copy completed D1 closes for `XTIUSD.DWX` and `NZDCAD.DWX`.
- Compute `xti_ret = ln(XTI close[1] / XTI close[1 + strategy_return_lookback_d1])`.
- Compute `nzdcad_ret = ln(NZDCAD close[1] / NZDCAD close[1 + strategy_return_lookback_d1])`.
- Compute `return_spread = xti_ret + strategy_beta_nzdcad * nzdcad_ret`.
- Standardize the latest completed return spread against the prior
  `strategy_z_lookback_d1` completed return spreads.
- Short spread: if z-score is above `strategy_entry_z`, sell `XTIUSD.DWX` and
  sell `NZDCAD.DWX`.
- Long spread: if z-score is below `-strategy_entry_z`, buy `XTIUSD.DWX` and
  buy `NZDCAD.DWX`.
- No entry if either basket leg already has an open position for this EA magic.
- No entry if either symbol's current spread exceeds its configured spread cap.

## Exit Rules

- Stop loss: each leg receives a fixed hard SL at ATR(`strategy_atr_period_d1`)
  times `strategy_atr_sl_mult` from entry.
- Exit both legs when absolute spread z-score falls below `strategy_exit_z`.
- Exit both legs after `strategy_max_hold_days`.
- If only one basket leg is open, close it immediately as a broken package.
- Friday close remains enabled by the V5 framework and closes both basket legs.

## Filters

- Only run from the `XTIUSD.DWX` D1 host chart with `qm_magic_slot_offset=0`.
- Skip entries when `XTIUSD.DWX` spread exceeds `strategy_xti_max_spread_pts`.
- Skip entries when `NZDCAD.DWX` spread exceeds `strategy_nzdcad_max_spread_pts`.
- Skip entries when either close series or either ATR series is unavailable.
- Framework news, kill-switch, magic, and Friday-close guards remain active.

## Trade Management Rules

- No pyramiding, gridding, martingale, partial close, or trailing stop.
- One open two-leg package at a time.
- Package integrity repair is deterministic: if one leg is missing, close the
  remaining leg.

## Parameters To Test

- name: strategy_return_lookback_d1
  default: 20
  sweep_range: [10, 20, 40]
- name: strategy_z_lookback_d1
  default: 120
  sweep_range: [80, 120, 180]
- name: strategy_beta_nzdcad
  default: 0.55
  sweep_range: [0.35, 0.55, 0.80]
- name: strategy_entry_z
  default: 1.9
  sweep_range: [1.6, 1.9, 2.2]
- name: strategy_exit_z
  default: 0.4
  sweep_range: [0.25, 0.4, 0.6]
- name: strategy_atr_period_d1
  default: 20
  sweep_range: [14, 20, 30]
- name: strategy_atr_sl_mult
  default: 3.0
  sweep_range: [2.5, 3.0, 4.0]
- name: strategy_max_hold_days
  default: 30
  sweep_range: [20, 30, 45]
- name: strategy_xti_max_spread_pts
  default: 1000
  sweep_range: [700, 1000, 1500]
- name: strategy_nzdcad_max_spread_pts
  default: 140
  sweep_range: [90, 140, 220]
- name: strategy_deviation_points
  default: 20
  sweep_range: [10, 20, 50]

## Author Claims

The source packet establishes structural lineage for oil/exchange-rate,
commodity-CAD, and New Zealand commodity-price channels only. This card imports
no source performance number. Q02 and later phases must validate or reject the
`XTIUSD.DWX` / `NZDCAD.DWX` basket on Darwinex bars.

## Initial Risk Profile

- expected_pf: 1.08.
- expected_dd_pct: 20.
- expected_trade_frequency: approximately 8-14 paired packages/year.
- risk_class: medium-high because crude volatility, NZDCAD liquidity/spread,
  and synchronized basket fills need Q02 proof.
- gridding: false.
- scalping: false.
- ml_required: false.

## Strategy Allowability Check

- [x] R1 reputable source: official EIA research plus official Bank of Canada
  and RBNZ central-bank support.
- [x] R2 mechanical: fixed D1 return spread, rolling z-score entry/exit, ATR
  hard stops, spread caps, max-hold exit, and broken-package repair.
- [x] R3 testable: `XTIUSD.DWX` and `NZDCAD.DWX` exist in the Darwinex symbol
  matrix with D1 history.
- [x] R4 compliant: no ML, adaptive PnL fitting, grid, martingale, external
  runtime feed, or more than one position per magic slot.
- [x] Non-duplicate: paired WTI/NZDCAD return-spread mean reversion, not the
  existing XTI/CADJPY, XTI/CADCHF, XTI/AUDCAD, XTI/GBPCAD, XTI/EURCAD,
  XTI/AUDJPY, XTI/AUDUSD, XTI/NZDUSD, WTI/USDCAD, XTI/XNG, Brent/WTI,
  oil/metal, XNG, metal, index, or commodity-RSI logic.

## Risk

Backtests use `RISK_FIXED=1000`, `RISK_PERCENT=0`, and one logical basket
setfile. Live risk is intentionally not configured here; any future live
allocation must come from the portfolio process. The EA does not touch
`T_Live`, AutoTrading, deploy manifests, portfolio admission, or the portfolio
gate.

## Registry And Queue Notes

- Slot 0: `XTIUSD.DWX`.
- Slot 1: `NZDCAD.DWX`.
- Use the logical basket setfile `QM5_13076_XTI_NZDCAD_RSPREAD_D1` for Q02.
- Keep Q02 setfile on `RISK_FIXED=1000` and `RISK_PERCENT=0`.

## Framework Alignment

- no_trade: host chart guard, D1 guard, parameter guard, spread caps, news,
  Friday close, and valid data checks.
- trade_entry: D1 standardized XTI/NZDCAD return-spread reversion.
- trade_management: broken-package repair and max-hold tracking.
- trade_close: z-score mean exit, max-hold exit, Friday close, and ATR hard
  stops.

## Kill Criteria

Kill or recycle the card if Q02 cannot produce at least one valid logical-basket
trade, if Q02 PF is below 1.0 after costs, if synchronized XTI/NZDCAD history
is insufficient, or if the basket preflight cannot execute both legs under the
V5 one-position-per-magic model.

## Pipeline History

| version | date | rebuild reason | phase reached | verdict |
|---|---|---|---|---|
| v1 | 2026-07-09 | initial XTI/NZDCAD return-spread basket build | Q02 | PENDING |

## Pipeline Phase Status

| Phase | Date | Verdict | Evidence path |
|---|---|---|---|
| G0 Research Intake | 2026-07-09 | APPROVED | this card |
| Q01 Build Validation | 2026-07-09 | PENDING | `artifacts/qm5_13076_build_result.json` |
| Q02 Baseline Screening | 2026-07-09 | PENDING | enqueue after compile |
