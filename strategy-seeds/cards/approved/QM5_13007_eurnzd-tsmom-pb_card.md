...
period: D1
expected_trade_frequency: "Monthly D1 rebalance; enters only when 6-month and 12-month return signs agree, approximately 5-8 position changes/year."
...
r2_mechanical: PASS
r2_reasoning: "On the first new D1 bar of each month, compare closed EURNZD.DWX price to 126- and 252-D1-bar closes; hold the agreed direction or flatten on disagreement."
...
# EURNZD Persistent-Bias Time-Series Momentum
...
tests whether medium-term and long-term price direction agree. Agreement
...
- Evaluate only on the first new D1 bar of each broker-calendar month.
- Compute `signal_6m = sign(close[1] - close[1 + 126])`.
- Compute `signal_12m = sign(close[1] - close[1 + 252])`.
...
- Monthly disagreement exits flatten the position.
- Monthly opposite agreement flips direction.
...
- name: strategy_mid_lookback_d1_bars
  default: 126
  sweep_range: [105, 126, 147]
- name: strategy_long_lookback_d1_bars
  default: 252
...
- [x] R2 mechanical: fixed monthly cadence, fixed lookbacks, deterministic flat/long/short state.
...
- trade_entry: monthly dual-horizon return-sign agreement.
- trade_management: no discretionary management beyond framework stop/Friday/kill controls.
- trade_close: monthly disagreement/opposite-signal exit handled on the rebalance bar.
