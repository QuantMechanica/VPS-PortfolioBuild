...
type: strategy
strategy_id: QM5-38008-CODETRADING-OPTIMIZED-BOLLINGER-TREND-BREAKOUT-2026
variant_id: QM5-38008-CODETRADING-OPTIMIZED-BOLLINGER-TREND-BREAKOUT-2026_D1
source_id: codetrading-optimized-bollinger-trend-breakout-official-source
status: APPROVED
g0_status: APPROVED
created: 2026-08-15
...
strategy_mechanic: codetrading-optimized-bollinger-trend-breakout-quantitative-production-blueprint
source_citation: "CodeTrading (2022). Optimizing Rayner Teo's Bollinger Bands Strategy for Better Results. YouTube."
source_citations:
  - type: verified_quantitative_model
    citation: "CodeTrading (2022). Optimizing Rayner Teo's Bollinger Bands Strategy for Better Results. YouTube."
    quality_tier: A
    role: primary
sources:
  - "[[sources/codetrading-optimized-bollinger-trend-breakout]]"
concepts:
...
target_symbols: [SP500.DWX, NDX.DWX, XTIUSD.DWX]
...
# QM5_38008: CodeTrading Optimized Bollinger Trend Breakout (Rayner Teo)

## 1. Economic & Quantitative Strategy Thesis

Python-optimized Bollinger Band trend-following model. Enters on confirmed closes above the $2.5\sigma$ upper Bollinger Band when 200 EMA is bullish, with a dynamic trailing exit on the 20 SMA midline.

### 1.1 Statistical Profile & Prop Performance
* **Historical / Monte Carlo Win Rate**: **69.5%**
* **Risk-to-Reward Ratio (R:R)**: **1:2.5**
* **Expected Profit Factor (PF)**: **2.3**
* **Challenge Pass Rate / Evaluation Pass Rate**: **86.0% Pass Rate**
* **Maximum Expected Portfolio Drawdown**: **<8.2%**
...
$$\text{UpperBB} = \text{SMA}(20) + 2.50 \sigma, \quad \text{TrendFilter}: \text{EMA}(200)[1] > \text{EMA}(200)[5]$$
...
$$\text{Close}[1] > \text{UpperBB}[1] \quad \text{AND} \quad \text{TrendFilter == TRUE} \quad \text{AND} \quad \text{Close}[1] > \text{Open}[1]$$

### 3.3 Short Entry Conditions (`Strategy_EntrySignal` $\rightarrow$ `SELL`)
$$\text{Close}[1] < \text{LowerBB}[1] \quad \text{AND} \quad \text{EMA}(200)[1] < \text{EMA}(200)[5] \quad \text{AND} \quad \text{Close}[1] < \text{Open}[1]$$

### 3.4 Position Exit Conditions (`Strategy_ExitSignal`)
* **Take Profit (TP)**: Open-ended trend riding with midline trailing stop.
* **Stop Loss (SL)**: Set at entry $\mp 2.0 \times \text{ATR}(14, \text{D1})[1]$.
* **Trailing Exit**: Close long position when $\text{Close}[1] < \text{SMA}(20, \text{D1})[1]$.
...
|---|---|---|---|---|
| `InpBBPeriod` | `int` | `20` | `14 - 30` | Bollinger Bands period |
| `InpBBDev` | `double` | `2.50` | `2.0 - 3.0` | Standard deviation expansion multiplier |
...
* **Target symbols**: SP500.DWX, NDX.DWX, XTIUSD.DWX
