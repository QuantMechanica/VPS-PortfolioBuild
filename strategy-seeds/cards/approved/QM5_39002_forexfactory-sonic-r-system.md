...
type: strategy
strategy_id: QM5-39002-FOREXFACTORY-SONIC-R-SYSTEM-2026
variant_id: QM5-39002-FOREXFACTORY-SONIC-R-SYSTEM-2026_M15
source_id: forexfactory-sonic-r-system-official-source
status: APPROVED
g0_status: APPROVED
created: 2026-08-15
created_by: Research+Development
last_updated: 2026-08-15
source_authors: "sonicdeejay"
strategy_mechanic: forexfactory-sonic-r-system-quantitative-production-blueprint
source_citation: "sonicdeejay (2008-2024). Sonic R. System. Forex Factory (>10M Views)."
source_citations:
  - type: verified_quantitative_model
    citation: "sonicdeejay (2008-2024). Sonic R. System. Forex Factory (>10M Views)."
    quality_tier: A
    role: primary
sources:
  - "[[sources/forexfactory-sonic-r-system]]"
concepts:
...
# QM5_39002: Sonic R System (Dragon Tunnel & 89 EMA Trend Wave)

## 1. Economic & Quantitative Strategy Thesis

The Sonic R system trades wave pullbacks into the 'Dragon Tunnel' (34 EMA High/Low/Close) in the direction of the 89 EMA macro trend wave, confirmed by volume rejection pinbars.

### 1.1 Statistical Profile & Prop Performance
* **Historical / Monte Carlo Win Rate**: **73.0%**
...
$$\text{Dragon\_High} = \text{EMA}(34, \text{High})[1], \quad \text{Dragon\_Low} = \text{EMA}(34, \text{Low})[1]$$
$$\text{TrendWave} = \text{EMA}(89, \text{Close})[1]$$
...
$$\text{Close}[1] > \text{TrendWave}[1] \quad \text{AND} \quad \text{Low}[1] \le \text{Dragon\_High}[1] \quad \text{AND} \quad \text{Close}[1] > \text{Dragon\_High}[1]$$

### 3.3 Short Entry Conditions (`Strategy_EntrySignal` $\rightarrow$ `SELL`)
$$\text{Close}[1] < \text{TrendWave}[1] \quad \text{AND} \quad \text{High}[1] \ge \text{Dragon\_Low}[1] \quad \text{AND} \quad \text{Close}[1] < \text{Dragon\_Low}[1]$$

### 3.4 Position Exit Conditions (`Strategy_ExitSignal`)
* **Take Profit (TP)**: Set to $2.5 \times \text{SL\_Distance}$ ($1:2.5\text{ R:R}$).
* **Stop Loss (SL)**: Placed beyond the outer edge of the Dragon Tunnel $\mp 3.0\text{ pips}$.
...
|---|---|---|---|---|
| `InpDragonPeriod` | `int` | `34` | `21 - 50` | Dragon Tunnel EMA period |
| `InpTrendWavePeriod`| `int`| `89` | `55 - 144` | Macro Trend Wave EMA period |
