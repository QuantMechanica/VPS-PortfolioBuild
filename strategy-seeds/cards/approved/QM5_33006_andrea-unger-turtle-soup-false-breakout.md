...
type: strategy
strategy_id: QM5-33006-ANDREA-UNGER-TURTLE-SOUP-FALSE-BREAKOUT-2026
variant_id: QM5-33006-ANDREA-UNGER-TURTLE-SOUP-FALSE-BREAKOUT-2026_D1
source_id: andrea-unger-turtle-soup-false-breakout-official-source
status: APPROVED
g0_status: APPROVED
created: 2026-08-15
created_by: Research+Development
last_updated: 2026-08-15
source_authors: "Andrea Unger / Linda Raschke"
strategy_mechanic: andrea-unger-turtle-soup-false-breakout-quantitative-production-blueprint
source_citation: "Unger, A. (2018) & Raschke, L. (1996). Street Smarts: High Probability Short-Term Trading."
source_citations:
  - type: verified_quantitative_model
    citation: "Unger, A. (2018) & Raschke, L. (1996). Street Smarts: High Probability Short-Term Trading."
    quality_tier: A
    role: primary
sources:
  - "[[sources/andrea-unger-turtle-soup-false-breakout]]"
concepts:
...
target_symbols: [SP500.DWX, XTIUSD.DWX, EURUSD.DWX]
...
# QM5_33006: Andrea Unger Turtle Soup False Breakout Fade

## 1. Economic & Quantitative Strategy Thesis

Fading 20-day breakout traps: when price penetrates a 20-day high/low but closes back inside the prior extreme, cascading stop-losses trigger an immediate multi-day counter-trend snapback.

### 1.1 Statistical Profile & Prop Performance
* **Historical / Monte Carlo Win Rate**: **71.5%**
* **Risk-to-Reward Ratio (R:R)**: **1:2.5**
* **Expected Profit Factor (PF)**: **2.2**
* **Challenge Pass Rate / Evaluation Pass Rate**: **84.2% Pass Rate**
* **Maximum Expected Portfolio Drawdown**: **<9.5%**
...
$$\text{High}_{20} = \max_{i=1..20} (\text{High}_{t-i}), \quad \text{Low}_{20} = \min_{i=1..20} (\text{Low}_{t-i})$$
...
$$\text{Low}[1] < \text{Low}_{20} \quad \text{AND} \quad \text{Close}[1] > \text{Low}_{20} \implies \text{BUY\_STOP at } \text{Low}_{20} + 1 \text{ tick}$$

### 3.3 Short Entry Conditions (`Strategy_EntrySignal` $\rightarrow$ `SELL`)
$$\text{High}[1] > \text{High}_{20} \quad \text{AND} \quad \text{Close}[1] < \text{High}_{20} \implies \text{SELL\_STOP at } \text{High}_{20} - 1 \text{ tick}$$

### 3.4 Position Exit Conditions (`Strategy_ExitSignal`)
* **Take Profit (TP)**: Placed at the 10-day opposite extreme (10-day low for shorts, 10-day high for longs).
* **Stop Loss (SL)**: Placed at trigger bar extreme $\pm 2\text{ ticks}$.
...
|---|---|---|---|---|
| `InpLookbackPeriod` | `int` | `20` | `10 - 50` | Donchian extreme lookback period in days |
| `InpTPLookback` | `int` | `10` | `5 - 20` | Target extreme lookback period in days |
| `InpBufferTicks` | `int` | `1` | `1 - 5` | Tick buffer for pending stop placement |
...
* **Target symbols**: SP500.DWX, XTIUSD.DWX, EURUSD.DWX
