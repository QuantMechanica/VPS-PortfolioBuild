...
slug: xng-lr-trend
status: APPROVED
execution_contract_ref: strategy-seeds/cards/approved/QM5_20262_xng-lr-trend_card.md
...
source_citations:
  - type: peer_reviewed_paper
    ...
    location: "DOI https://doi.org/10.1016/j.jfineco.2011.11.003; complete-paper evidence strategy-seeds/sources/MOP-TSMOM-2012/source.md; bounded extraction strategy-seeds/sources/MOP-XNG-LRTREND-2026/source.md"
    quality_tier: A
...
indicators:
  ...
  - "[[indicators/atr]]"
strategy_type_flags: [commodity, energy, natural-gas, structural-trend, path-consistency, monthly-rebalance, atr-hard-stop, time-stop, symmetric-long-short, low-frequency]
...
period: D1
timeframe: D1
expected_trade_frequency: "Estimated 6-10 completed monthly XNG positions/year after thirteen completed month ends; Q02 must prove at least five/year or retire."
...
g0_approval_reasoning: "APPROVED under decisions/2026-08-07_qm5_20262_xng_lr_trend_g0.md: R1 one tier-A peer-reviewed complete-read source with explicit natural-gas membership; R2 locked thirteen completed month ends, oldest-to-newest log-price OLS slope, fixed R-squared 0.50 gate, monthly attempt, ATR stop, rollover, and stale exit; R3 registered XNGUSD.DWX D1 history; R4 deterministic native arithmetic only. Deterministic dedup scanned 4,319 registry rows and 436 cards with no exact collision; six lexical/source-family fuzzy neighbors were manually resolved, and a content scan found no existing XNG OLS slope plus fit-gate mechanic. No source efficacy or decorrelation transfers."
---
...
Natural gas can sustain slow price paths as production, storage, transport,
LNG export, hedging, weather-demand, and power-burn regimes adjust. A simple
endpoint return cannot distinguish a persistent path from one dominated by a
single jump. This card therefore trades monthly XNG trend direction only when
an ordinary least-squares fit to thirteen completed log-price endpoints has a
fixed minimum explanatory fit.

The carrier is shared with the certified XNG sleeve, but the state variable,
clock, symmetry, and holding period are deliberately different from its daily
long-only cumulative-RSI2 pullback. That does not prove decorrelation,
profitability, or portfolio suitability. Q02 owns density and economics;
unchanged downstream gates, including Q09, own robustness and realized book
overlap.

## Source Traceability And Claim Boundary

The sole source of record is the governed bounded packet
`strategy-seeds/sources/MOP-XNG-LRTREND-2026/source.md`. Its complete-read
...
through twelve lags and includes natural gas among its commodity futures.
...
The canonical checker scanned 4,319 EA-registry rows and 436 intake cards. It
found no exact slug or strategy-ID collision and returned six expected fuzzy
matches from shared XNG/trend language or the source family. Manual review
fixes the boundary:

- `QM5_20261_wti-lr-trend` uses the same path-quality rule on WTI. The exact
  XNG carrier is load-bearing and no WTI result transfers.
- `QM5_20259_xng-mom-vote` combines one-, three-, and twelve-month cumulative
  return signs rather than fitting the path between endpoints.
- `QM5_20204_xng-tsmom1m`, `QM5_20063_xng-tsmom3m`, and
  `QM5_12804_xng-tsmom12m-atr` use endpoint returns rather than a regression
  path.
- `QM5_13116_xng-signmom` counts monthly return signs; it does not use slope
  or regression residual dispersion.
...
- `QM5_12567_cum-rsi2-commodity` is a long-only daily RSI(2) pullback aligned
  with SMA(200), with a five-bar maximum hold. This card is symmetric,
  oscillator-free, and monthly.
- XNG moving-average, channel, variance-ratio, calendar, pullback, reversal,
  event, and relative-value EAs use different state variables or clocks.

A content scan found no XNG card requiring both log-price OLS slope and fixed
...
- Timeframe: D1; magic slot 0; intended magic `202620000`.
...
1. Require exact EA ID `20262`, `XNGUSD.DWX` D1, magic slot 0, and every
...
futures and identify natural gas in their commodity universe. They do not claim that
...
sensitivity to large endpoints, stop-outs, and correlation with the incumbent
XNG sleeve or risk assets can dominate the premise. `R^2` is a descriptive
in-sample path measure, not a forecast-confidence guarantee.
...
- [x] R3: registered `XNGUSD.DWX` D1 plus native V5 execution state only.
