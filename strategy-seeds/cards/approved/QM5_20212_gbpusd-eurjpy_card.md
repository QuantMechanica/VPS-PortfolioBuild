---
strategy_id: AI-CODEX-FX-COINT66-20260609-GBPUSD-EURJPY
ea_id: QM5_20212
slug: gbpusd-eurjpy
type: strategy
status: APPROVED
...
markets: [forex]
timeframes: [D1]
primary_target_symbols: [GBPUSD.DWX, EURJPY.DWX]
target_symbols: [GBPUSD.DWX, EURJPY.DWX]
logical_symbol: QM5_20212_GBPUSD_EURJPY_COINTEGRATION_D1
period: D1
...
# QM5_20212 GBPUSD/EURJPY D1 Cointegration Basket
...
low-frequency daily implementation. Chan makes no claim for GBPUSD/EURJPY.
...
GBPUSD/EURJPY ranks thirty-second of 66 by OOS net Sharpe. Rank 31,
GBPJPY/EURAUD, is already represented by dedicated fixed-D1 basket
`QM5_20211`. The deterministic repository dedup check, exact-pair searches,
and an unordered traded-symbol manifest reconciliation found no dedicated
fixed-beta GBPUSD/EURJPY D1 card, EA, registry allocation, or logical basket
...
`ln(GBPUSD) - (-0.080732288) * ln(EURJPY)`. The negative fitted coefficient
...
The fixed scan suggests large deviations in the GBPUSD/EURJPY residual may
...
`GBPUSD.DWX` is the tester host and first traded leg. `EURJPY.DWX` is the
companion traded leg. `EURUSD.DWX` and `USDJPY.DWX` are conversion-only
...
- Compute `spread = ln(GBPUSD) - strategy_beta * ln(EURJPY)`, with frozen
  `strategy_beta = -0.080732288`.
...
Chan supplies the pair-trading method but reports no GBPUSD/EURJPY result.
...
data_requirements: "GBPUSD.DWX, EURJPY.DWX, EURUSD.DWX, and USDJPY.DWX D1 histories"
```
...
- 2026-08-04: Rank 32 has negative DEV performance, a sub-0.8 OOS score, and
