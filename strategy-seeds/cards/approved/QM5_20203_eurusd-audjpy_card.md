---
strategy_id: AI-CODEX-FX-COINT66-20260609-EURUSD-AUDJPY
ea_id: QM5_20203
slug: eurusd-audjpy
type: strategy
status: APPROVED
...
# QM5_20203 EURUSD/AUDJPY D1 Cointegration Basket
...
low-frequency daily implementation. Chan makes no claim for EURUSD/AUDJPY.
...
EURUSD/AUDJPY ranks twenty-first of 66 by OOS net Sharpe. Ranks 19 and 20,
EURGBP/EURAUD and NZDUSD/GBPJPY, are already represented by dedicated legacy
basket builds `QM5_12712` and `QM5_12728`. The deterministic repository dedup
check and exact-pair searches found no dedicated fixed-beta EURUSD/AUDJPY D1
...
The sub-0.8 OOS Sharpe is adverse evidence, and the approximately 152-bar
...
`ln(EURUSD) - (-0.160071209) * ln(AUDJPY)`. The negative fitted coefficient
...
The package retains EUR, USD, AUD, JPY, carry, and risk-sentiment exposure.
...
The fixed scan suggests large deviations in the EURUSD/AUDJPY residual may
...
`EURUSD.DWX` is the tester host and first traded leg. `AUDJPY.DWX` is the
...
- Compute `spread = ln(EURUSD) - strategy_beta * ln(AUDJPY)`, with frozen
  `strategy_beta = -0.160071209`.
...
Chan supplies the pair-trading method but reports no EURUSD/AUDJPY result.
...
data_requirements: "EURUSD.DWX, AUDJPY.DWX, AUDUSD.DWX, and USDJPY.DWX D1 histories"
```
...
- 2026-08-02: Rank 21 is below the original survivor bar and has a very slow
