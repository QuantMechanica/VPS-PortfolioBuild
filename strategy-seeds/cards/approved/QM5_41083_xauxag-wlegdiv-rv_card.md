...
type: strategy
strategy_id: SCHWEIKERT-CME-XAUXAG-WLEGDIV-RV-2026_S01
variant_id: SCHWEIKERT-CME-XAUXAG-WLEGDIV-RV-2026_S01
source_id: SCHWEIKERT-CME-XAUXAG-WLEGDIV-RV-2026
ea_id: QM5_41083
slug: xauxag-wlegdiv-rv
status: APPROVED
execution_contract_ref: strategy-seeds/cards/approved/QM5_41083_xauxag-wlegdiv-rv_card.md
execution_contract_status: DRAFT
created: 2026-08-21
created_by: Research+Development
last_updated: 2026-08-21
g0_status: APPROVED
g0_decision: decisions/2026-08-21_qm5_41083_xauxag_weekly_leg_divergence_reversion_g0.md
source_approval: decisions/2026-08-21_xauxag_weekly_leg_divergence_reversion_source_approval.md
...
source_citations:
  - type: peer_reviewed_paper
    ...
    location: "DOI 10.1016/j.jbankfin.2017.11.010; complete-read governed packet strategy-seeds/sources/SCHWEIKERT-XAUXAG-RATIO-2026/source.md; bounded extraction strategy-seeds/sources/SCHWEIKERT-CME-XAUXAG-WLEGDIV-RV-2026/source.md"
    quality_tier: A
...
strategy_mechanic: synchronized-two-consecutive-completed-week-end-pairs-individual-gold-and-silver-log-returns-strict-opposite-sign-fade-relative-winner-one-week-equal-notional-basket
sources:
  - "[[sources/SCHWEIKERT-CME-XAUXAG-WLEGDIV-RV-2026]]"
concepts:
  - "[[concepts/gold-silver-relative-value]]"
  - "[[concepts/completed-week-leg-sign-divergence]]"
  - "[[concepts/market-neutral-basket]]"
indicators:
  - "[[indicators/completed-week-log-return-sign]]"
  - "[[indicators/atr-risk-stop]]"
strategy_type_flags: [commodity, precious-metals, gold-silver-ratio, relative-value-basket, completed-week-leg-divergence, mean-reversion, symmetric-long-short, atr-hard-stop, time-stop, low-frequency]
markets: [commodities, precious_metals]
timeframes: [D1]
target_symbols: [XAUUSD.DWX, XAGUSD.DWX]
primary_target_symbols: [XAUUSD.DWX, XAGUSD.DWX]
single_symbol_only: false
logical_symbol: QM5_41083_XAU_XAG_WLEGDIV_RV_D1
symbol: QM5_41083_XAU_XAG_WLEGDIV_RV_D1
host_symbol: XAUUSD.DWX
companion_symbol: XAGUSD.DWX
symbol_slots: [0, 1]
magic_numbers: [410830000, 410830001]
period: D1
timeframe: D1
expected_trade_frequency: "Approximately eight to eighteen completed paired packages per full post-warm-up year after exact synchronized completed weeks, strict individual leg-sign opposition, and execution gates; Q02 must prove at least five/year or retire."
expected_trades_per_year_per_symbol: 12
expected_pf: 1.01
expected_dd_pct: 35.0
risk_class: high
ml_required: false
r1_track_record: PASS_WITH_WEEKLY_LEG_STATE_TRANSLATION_RISK
r2_mechanical: PASS
...
hard_rules_at_risk: [exact_xau_xag_carrier, immediately_preceding_two_monday_anchors, synchronized_completed_d1_closes, three_to_five_week_sessions, exact_week_end_endpoints, strict_individual_leg_sign_opposition, contrarian_relative_winner_direction, persistent_week_attempt, equal_notional_basket, aggregate_fixed_risk, hard_stops_present, atomic_package_repair, next_week_exit, risk_mode_dual, q02_frequency_floor, portfolio_correlation]
g0_approval_reasoning: "R1 uses one bounded child source with named peer-reviewed DOI and official exchange lineages while disclosing the completed-week individual-leg sign fade as an untested QM translation; R2 locks synchronized consecutive weeks, endpoint selection, individual log-return orientation, strict sign opposition, contrarian sides, durable attempt, aggregate fixed risk, equal notional, hard stops, spreads, and next-week lifecycle; R3 uses registered native XAU/XAG D1 histories with synchronization and CFD-basis risks explicit and requires active slots 0/1 before build; R4 is deterministic timestamp, price, logarithm, comparison, ATR, quote, position, deal, and terminal-state arithmetic without a banned signal, trained output, external feed, grid, or martingale; canonical dedup and manual family review found no exact identity."
---

# QM5_41083 XAU/XAG Completed-Week Leg-Divergence Reversion

## Hypothesis

When gold and silver finish the same completed broker week with strictly
opposite signed returns, the intermetal move reflects a clean divergence rather
than a shared precious-metal direction. Selling the weekly winner and buying
the weekly loser for the next broker week may capture re-convergence in their
state-dependent long-run relation without taking an outright one-leg signal.
...
source tests opposite individual weekly return signs, their contrarian side, a
continuous-CFD package, equal-notional sizing, fixed cash risk, ATR stops, or a
one-week hold.
...
- `QM5_12577`, `QM5_20157`, `QM5_20161`, `QM5_20263`, and `QM5_20268` use a
  ...
  card estimates none.
- `QM5_41030_xauxag-flowdiv` compares close/open and open/close relative-flow
  components inside a week. This card ignores session decomposition and uses
  the two metals' individual full-week endpoint returns.
- `QM5_41031_xauxag-goldlead` requires a thresholded one-day gold shock and
  bounded silver underresponse. This card is symmetric, weekly, and has no
  magnitude threshold.
- `QM5_41040` and `QM5_41057` classify relative session/overnight flow. This
  card uses only synchronized completed week-end closes.
- `QM5_41066` and `QM5_41075` through `QM5_41078` classify sequences of one
  gold-minus-silver relative return over two or more weeks. This card instead
  requires the individual gold and silver returns over one common week to have
  opposite signs.
- `QM5_41079_xauxag-wclose-extreme-rv` ranks synchronized ratio closes within
  one week. This card has no within-week rank and uses only two week-end pairs.
...
The exact XAU/XAG carrier, common completed weekly interval, strict opposite
individual metal-return signs, contrarian relative-winner side, durable weekly
...
- Exact host: `XAUUSD.DWX`, D1, slot 0, active magic `410830000`.
- Exact companion: `XAGUSD.DWX`, D1, slot 1, active magic `410830001`.
- Logical symbol: `QM5_41083_XAU_XAG_WLEGDIV_RV_D1`.
- Formation: the synchronized week-end close pairs of the two immediately
  preceding consecutive Monday-anchored broker weeks.
- Decision: first tradable bar of a new Monday-anchored broker week, within
  180 elapsed raw-session minutes.
- Signal: gold and silver have strictly opposite nonzero log-return signs over
  the exact same completed weekly interval; fade the relative winner.
- Ordinary exit: first tick whose broker Monday anchor is later than the
  package-open anchor.
- Expected cadence: eight to eighteen completed packages/year; retire below
  five.
...
Let the older and newer synchronized completed week-end close pairs be
`(G0,S0)` and `(G1,S1)` with positive finite prices and exact increasing
timestamps. Define:
...
high, low, or close never enters the calculation. Equality and either zero
return are flat. Return magnitude never changes signal or risk.
...
1. Evaluate only once on a new exact `XAUUSD.DWX` D1 bar under EA 41083 and
...
5. Within a fixed 30-bar buffer, collect every positive finite synchronized
   close pair belonging to each of the two immediately preceding consecutive
   Monday anchors. Require exactly three to five pairs in each week, identical
   timestamps across legs, unique increasing timestamps, and no pair from
   another anchor. Select the chronologically latest pair from each week.
6. Compute gold and silver log returns between the older and newer selected
   pairs. Require both returns to be finite, strictly nonzero, and of opposite
   sign. Any equality, zero, same-sign state, invalid arithmetic, missing
   session, or more than five sessions remains flat.
7. SELL XAU/BUY XAG when gold is positive and silver negative. BUY XAU/SELL
   XAG when gold is negative and silver positive. Magnitude never changes
   eligibility, direction, or risk.
...
- Exact host, D1, EA 41083, slot zero, and both governed magics.
...
- First-week-bar clock, exact consecutive prior anchors, synchronized
  timestamps, three-to-five session counts, unique endpoints, positive finite
  prices, strict nonzero opposite leg signs, durable attempt, side-specific
  trade mode, spread, quote, ATR, sizing, stop geometry, and notional match all
  fail closed.
...
- Own exactly one `XAUUSD.DWX` position under active magic `410830000` and one
  opposite-side `XAGUSD.DWX` position under active magic `410830001`.
...
| `strategy_history_bars_d1` | 30 | bounded two-week buffer |
...
source supplies the individual weekly leg-sign state or one-week fade.

## QM Interpretations

`SCHWEIKERT-CME-XAUXAG-WLEGDIV-RV-2026_S01` fixes exact consecutive prior
weeks, synchronized endpoint selection, strict individual sign opposition,
...
position/deal state, and persistent terminal global-variable attempt state. No
finite external dataset or calendar exists.
...
leakage, accepting zero or same-sign leg returns, wrong contrarian side,
...
zero or same-sign state, adding a magnitude threshold, changing direction or
hold, changing the history or session bounds, fitting a center or hedge ratio,
or adding a volatility, volume, calendar, external-data, or prior-result gate.
No post-result parameter salvage is authorized.
...
| synchronized endpoint pairs, individual return signs, sides, attempt, spread, ATR | Trade Entry | `Strategy_EntrySignal` plus deterministic helpers |
...
completed weeks with three-to-five synchronized sessions; latest endpoint
selection; both opposite-sign directions; same-sign, zero, equality,
asynchrony, and incomplete-week flat states; no current-bar leakage;
persistent weekly attempts; fixed-risk frozen-stop sizing; atomic repair;
next-week and stale exits; card lint; strict compile; setfile schema; resolver
