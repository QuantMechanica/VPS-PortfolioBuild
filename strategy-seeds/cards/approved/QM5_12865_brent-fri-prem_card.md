...
g0_approval_reasoning: "R1 PASS peer-reviewed crude-oil day-of-week source with explicit Brent/WTI lineage; R2 PASS deterministic Friday D1 long rule with ATR stop and framework Friday-close/next-bar flatten; R3 PASS wide-net DWX route via XBRUSD.DWX farm symbol map/prior XBR setfiles with Q02 validating history sufficiency; R4 PASS no ML/grid/martingale/external runtime data."
expected_pf: 1.08
...
and WTI return samples. This card isolates the Friday premium on the Brent
benchmark rather than adding another WTI, XNG, index, or metal leg: buy
`XBRUSD.DWX` only on the broker-calendar Friday D1 bar and flatten through the
framework Friday close or the first subsequent D1 bar.

This is deliberately different from:

- `QM5_12841_brent-thu-prem`: same Brent benchmark, but Thursday rather than
  Friday.
- `QM5_12856_brent-mon-fade`: Brent short Monday weakness, opposite side and
  weekday.
- `QM5_12597_wti-fri-prem`: same weekday thesis but WTI benchmark
...
- Period: D1.
- Expected trade frequency: about 40-52 entries/year before Q02 proves or kills
  the route.
...
- Current broker-calendar D1 bar must be Friday, where Sunday=0 and Friday=5
  in MQL5 broker time.
...
- Close through the V5 framework Friday-close guard.
- Close any residual position on the first new D1 bar after Friday.
...
in Brent and WTI returns, including the Brent Friday side. No source
...
- expected_pf: 1.08
- expected_dd_pct: 18
- expected_trade_frequency: approximately 40-52 entries/year.
- risk_class: medium-high due Brent CFD history, Friday close interaction, and
  crude gap risk.
- gridding: false.
- scalping: false.
- ml_required: false.
...
- [x] R1 reputable source: peer-reviewed Soft Computing crude-oil seasonality
  paper with DOI.
- [x] R2 mechanical: fixed broker-calendar Friday, single D1 long entry, ATR
  stop, Friday close, and residual next-bar exit.
...
- [x] Non-duplicate: Brent Friday premium is not Brent Thursday, Brent Monday,
  WTI Friday, WTI event/month/roll logic, XNG, XAU/XAG, XTI/XNG, or commodity
  RSI logic.

## Framework Alignment

- no_trade: D1 and `XBRUSD.DWX` guard, slot guard, parameter guard, spread cap.
- trade_entry: Friday broker-calendar long entry.
- trade_management: first non-Friday D1 bar and max-hold stale-position exits.
- trade_close: hard ATR stop plus deterministic time exits and framework Friday
  close.
...
|---|---|---|---|
| v1 | 2026-07-02 | initial structural Brent Friday calendar-premium card | Q02 | ENQUEUED |
...
| Q01 Build Validation | 2026-07-01 | PASS | `artifacts/qm5_12865_build_result.json` |
| Q02 Baseline Screening | 2026-07-01 | QUEUED | `D:\QM\strategy_farm\state\farm_state.sqlite` work item `b84d3cb8-2ecf-4efa-bdc5-ef0cdacbdf2e` |
