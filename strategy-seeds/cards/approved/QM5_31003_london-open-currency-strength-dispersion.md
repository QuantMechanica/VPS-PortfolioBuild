...
type: strategy
strategy_id: QM5-31003-LONDON-OPEN-CURRENCY-STRENGTH-DISPERSION-2026
variant_id: QM5-31003-LONDON-OPEN-CURRENCY-STRENGTH-DISPERSION-2026_M15
source_id: london-open-currency-strength-dispersion-official-source
status: APPROVED
g0_status: APPROVED
created: 2026-08-15
created_by: Research+Development
last_updated: 2026-08-15
source_authors: "Kathy Lien"
strategy_mechanic: london-open-currency-strength-dispersion-quantitative-production-blueprint
source_citation: "Lien, K. (2006). Day Trading and Swing Trading the Currency Market."
source_citations:
  - type: verified_quantitative_model
    citation: "Lien, K. (2006). Day Trading and Swing Trading the Currency Market."
    quality_tier: A
    role: primary
sources:
  - "[[sources/london-open-currency-strength-dispersion]]"
concepts:
...
target_symbols: [GBPJPY.DWX, EURUSD.DWX, AUDUSD.DWX]
...
# QM5_31003: London Open Currency Strength Dispersion

## 1. Economic & Quantitative Strategy Thesis

Computes 24h Rate of Change across the G8 currency basket at 08:00 GMT. Pairs the single strongest currency against the single weakest currency.

### 1.1 Statistical Profile & Prop Performance
* **Historical / Monte Carlo Win Rate**: **67.1%**
* **Risk-to-Reward Ratio (R:R)**: **1:2.5**
* **Expected Profit Factor (PF)**: **2.3**
* **Challenge Pass Rate / Evaluation Pass Rate**: **82.4% Pass Rate**
...
$$\text{Strength}(C) = \frac{1}{7} \sum_{j=1}^7 \text{ROC}_{24\text{h}}(C, \text{Quote}_j)$$
...
$$\text{Strength}(\text{Base}) \ge +0.40\% \quad \text{AND} \quad \text{Strength}(\text{Quote}) \le -0.40\%$$

### 3.3 Short Entry Conditions (`Strategy_EntrySignal` $\rightarrow$ `SELL`)
$$\text{Strength}(\text{Base}) \le -0.40\% \quad \text{AND} \quad \text{Strength}(\text{Quote}) \ge +0.40\%$$
...
|---|---|---|---|---|
| `InpEvalHourGMT` | `int` | `8` | `7 - 9` | Evaluation hour for currency strength ranking |
| `InpMinDispersion` | `double` | `0.80` | `0.50 - 1.50` | Minimum strength divergence spread in percent |
...
* **Target symbols**: GBPJPY.DWX, EURUSD.DWX, AUDUSD.DWX
