...
type: strategy
strategy_id: MOP-MEEK-WTI-MDAILY-IQRMEAN-2026_S01
variant_id: MOP-MEEK-WTI-MDAILY-IQRMEAN-2026_S01
source_id: MOP-MEEK-WTI-MDAILY-IQRMEAN-2026
ea_id: QM5_41134
slug: wti-mdaily-iqrmean-mom
status: APPROVED
execution_contract_ref: strategy-seeds/cards/approved/QM5_41134_wti-mdaily-iqrmean-mom_card.md
execution_contract_status: APPROVED
created: 2026-08-23
created_by: Research+Development
last_updated: 2026-08-23
g0_status: APPROVED
g0_decision: decisions/2026-08-23_qm5_41134_wti_monthly_daily_iqr_mean_momentum_g0.md
source_approval: decisions/2026-08-23_wti_monthly_daily_iqr_mean_momentum_source_approval.md
...
source_citations:
  - type: peer_reviewed_paper_bounded_packet
    ...
    location: "DOI 10.1016/j.jfineco.2011.11.003; complete-read packet strategy-seeds/sources/MOP-TSMOM-2012/source.md; bounded composite strategy-seeds/sources/MOP-MEEK-WTI-MDAILY-IQRMEAN-2026/source.md"
    quality_tier: A
  ...
  - type: peer_reviewed_open_access_paper_bounded_packet
    ...
    location: "DOI 10.1080/23322039.2023.2213876; complete-read packet strategy-seeds/sources/MEEK-HOELSCHER-WTI-DOW-2023/source.md; bounded composite strategy-seeds/sources/MOP-MEEK-WTI-MDAILY-IQRMEAN-2026/source.md"
    quality_tier: A
    role: wti_close_to_close_daily_log_return_lineage
strategy_mechanic: normalized-month-boundary-wti-immediately-completed-seventeen-to-twenty-three-session-daily-log-returns-ascending-sort-floor-quarter-trim-each-tail-central-band-arithmetic-mean-sign-continuation-one-month-hold
sources:
  - "[[sources/MOP-MEEK-WTI-MDAILY-IQRMEAN-2026]]"
concepts:
...
strategy_type_flags: [commodity, energy, wti-crude, structural-trend, completed-month-daily-return-interquartile-mean, robust-location-direction, symmetric-long-short, atr-hard-stop, time-stop, low-frequency]
...
period: D1
timeframe: D1
expected_trade_frequency: "Approximately 10-12 completed WTI positions per full post-warm-up year after exact month, interquartile-mean arithmetic, and execution gates; Q02 must prove at least five/year or retire."
...
g0_approval_reasoning: "OWNER mission 2026-08-23; R1 peer-reviewed WTI own-return and complete-read daily-return sources with the within-month interquartile-mean translation disclosed; R2 exact month/return/endpoint/sort/integer-trim/retained-mean/direction/attempt/risk/lifecycle; R3 native XTI D1 with label and CFD-basis risk; R4 deterministic arithmetic without a trained or banned signal; canonical pre-allocation dedup CLEAN and manual family review separates raw endpoint, daily breadth, daily persistence, single-tail trim, ordinary median, weekday-bucket median, cross-month robust estimators, and certified XNG RSI logic."
---

# QM5_41134 WTI Completed-Month Daily-Return Interquartile-Mean Momentum
...
can be dominated by a small number of extreme sessions. Following the mean of
the central daily-return band after removing the integer outer quartiles tests
whether the ordinary completed-month path carries a direction that persists
into the next month.
...
at commit `81ca87515` and committed as a bounded packet at `6af0cbdae`.
...
Neither source tests a WTI-only within-month interquartile mean, a continuous
CFD, fixed-dollar ATR risk, or the QM book. The exact integer-quartile trim,
execution, and risk rules below are declared QM interpretations.
...
Before allocation, the fail-closed canonical checker scanned 4,633 registry
identities, 1,301 cards, and 45 Strategy Wiki nodes using the actual Company
...
- `QM5_20270_wti-trimmean-mom` averages the middle eight of twelve completed
  monthly returns spanning a year, not daily returns inside one month.
- `QM5_41111_wti-mdaybreadth-mom` counts daily signs and requires raw endpoint
  agreement. This card keeps central magnitudes and has no endpoint gate.
- `QM5_41127_wti-mdaily-persist-mom` estimates adjacent demeaned-return
  persistence and follows the endpoint. This card has no adjacency state.
- `QM5_41131_wti-mdaily-tailtrim-mom` removes exactly one observation from
  each tail and sums all 15-21 retained returns. This card removes four or
  five from each tail and averages only 9-13 central returns.
- `QM5_41132_wti-mweekday-med-mom` takes a median of five weekday-bucket
  means. This card has no weekday buckets.
- `QM5_41133_wti-mdaily-median-mom` uses only one or two exact center order
  statistics. This card uses a 9-13-observation central band.
...
daily return ending in the month, full-sample ascending sort, integer outer-
quartile deletion, central-band arithmetic mean, symmetric direction,
...
- Host and traded symbol: exact `XTIUSD.DWX`, D1, slot 0, magic `411340000`.
...
central_mean > 0 => BUY XTIUSD.DWX
central_mean < 0 => SELL XTIUSD.DWX
otherwise        => FLAT
```
...
2. Require exact `XTIUSD.DWX`, D1, EA `41134`, slot zero, registered magic,
...
- Runtime cannot read current-month signal prices, futures curves, inventory,
  volume, open interest, fitted output, external files/APIs, trained output,
  prior pipeline results, or manual signals.
...
- Backtest mode only: `RISK_FIXED=1000`, `RISK_PERCENT=0`, and
  `PORTFOLIO_WEIGHT=1`.
- One exact `XTIUSD.DWX` D1 position at a time under slot zero and registered
  magic `411340000`.
