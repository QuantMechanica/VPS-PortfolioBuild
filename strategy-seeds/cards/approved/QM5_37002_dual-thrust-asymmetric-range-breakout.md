...
type: strategy
strategy_id: QM5-37002-DUAL-THRUST-ASYMMETRIC-RANGE-BREAKOUT-2026
variant_id: QM5-37002-DUAL-THRUST-ASYMMETRIC-RANGE-BREAKOUT-2026_D1
source_id: dual-thrust-asymmetric-range-breakout-official-source
status: APPROVED
g0_status: APPROVED
created: 2026-08-15
created_by: Research+Development
last_updated: 2026-08-15
source_authors: "Michael Chalek / QuantConnect Desk"
strategy_mechanic: dual-thrust-asymmetric-range-breakout-quantitative-production-blueprint
source_citation: "Chalek, M. & QuantConnect Research. Dual Thrust Algorithmic Performance Benchmarks."
source_citations:
  - type: verified_quantitative_model
    citation: "Chalek, M. & QuantConnect Research. Dual Thrust Algorithmic Performance Benchmarks."
    quality_tier: A
    role: primary
sources:
  - "[[sources/dual-thrust-asymmetric-range-breakout]]"
concepts:
...
# QM5_37002: Dual Thrust Asymmetric Range Breakout (Michael Chalek)

## 1. Economic & Quantitative Strategy Thesis

One of the top 10 most deployed strategies on QuantConnect LEAN. Calculates asymmetric range bounds using multi-day high-close and close-low differentials with $k_1/k_2$ multipliers.
...
$$\text{Range} = \max(\text{HH}_N - \text{LC}_N, \text{HC}_N - \text{LL}_N)$$
$$\text{Buy\_Trigger} = \text{Open}_t + k_1 \times \text{Range}, \quad \text{Sell\_Trigger} = \text{Open}_t - k_2 \times \text{Range}$$
...
* **Stop Loss (SL)**: Set at opposite trigger boundary (or entry $\mp 0.50 \times \text{Range}$).
* **EOD Exit**: Close all open positions at market before daily settlement.
...
|---|---|---|---|---|
| `InpLookbackDays` | `int` | `4` | `2 - 8` | Lookback period in days |
| `InpK1` | `double` | `0.50` | `0.30 - 0.80` | Long breakout range multiplier |
| `InpK2` | `double` | `0.50` | `0.30 - 0.80` | Short breakout range multiplier |
