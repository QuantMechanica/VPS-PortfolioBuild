...
type: strategy
strategy_id: BOC-GBPCAD-OIL-RSPREAD-2026
source_id: BOC-GBPCAD-OIL-RSPREAD-2026
...
source_citations:
  ...
  - type: government_energy_context
    ...
    role: supplement
sources:
  - "[[sources/BOC-GBPCAD-OIL-RSPREAD-2026]]"
concepts:
...
target_symbols: [XTIUSD.DWX, GBPCAD.DWX]
basket_symbols: [XTIUSD.DWX, GBPCAD.DWX]
markets: [XTIUSD.DWX, GBPCAD.DWX]
primary_target_symbols: [XTIUSD.DWX, GBPCAD.DWX]
single_symbol_only: false
logical_symbol: QM5_13057_XTI_GBPCAD_RSPREAD_D1
period: D1
timeframes: [D1]
expected_trade_frequency: "D1 XTI/GBPCAD inverse-CAD return-spread z-score reversion; estimate 6-12 paired packages/year before Q02 validates synchronized history and fills."
...
g0_approval_reasoning: "Mission-directed G0 approval 2026-07-08: R1 PASS EIA and Bank of Canada source packet; R2 PASS deterministic D1 XTI/GBPCAD inverse-CAD return-spread basket; R3 PASS XTIUSD.DWX and GBPCAD.DWX exist in the DWX matrix; R4 PASS no ML/grid/martingale/external runtime data."
---

# XTI/GBPCAD D1 Return-Spread Reversion

## Source

- Source: [[sources/BOC-GBPCAD-OIL-RSPREAD-2026]]
...
`return_spread = ln(XTI[t] / XTI[t-L]) + beta_gbpcad * ln(GBPCAD[t] / GBPCAD[t-L])`

When WTI has unusually outperformed while GBPCAD has also risen, the package
fades a "oil up while CAD weak" divergence by selling both WTI and GBPCAD. When
WTI has unusually underperformed while GBPCAD has also fallen, it buys both WTI
and GBPCAD.
...
- Logical symbol: `QM5_13057_XTI_GBPCAD_RSPREAD_D1`.
- Host symbol: `XTIUSD.DWX`.
- Basket legs: `XTIUSD.DWX` and `GBPCAD.DWX`.
- Period: D1.
...
- Copy completed D1 closes for `XTIUSD.DWX` and `GBPCAD.DWX`.
...
- Compute `return_spread = xti_ret + strategy_beta_gbpcad * gbpcad_ret`.
...
- [x] R3 testable: `XTIUSD.DWX` and `GBPCAD.DWX` exist in the Darwinex symbol
  matrix.
- [x] R4 compliant: no ML, adaptive PnL fitting, grid, martingale, external
  runtime feed, or more than one position per magic slot.
- [x] Non-duplicate: paired WTI/GBPCAD inverse-CAD return-spread mean
  reversion, not WTI/USDCAD, WTI/CADJPY, WTI/CADCHF, WTI/AUDCAD,
  GBPCAD/GBPNZD cointegration, XTI/NZD, XTI/XNG, Brent/CAD, Brent/WTI,
...
- Slot 0: `XTIUSD.DWX`.
- Slot 1: `GBPCAD.DWX`.
- Use the logical basket setfile `QM5_13057_XTI_GBPCAD_RSPREAD_D1` for Q02.
...
- trade_entry: D1 standardized XTI/GBPCAD inverse-CAD return-spread reversion.
...
trade, if Q02 PF is below 1.0 after costs, if synchronized XTI/GBPCAD history
...
|---|---|---|---|
| v1 | 2026-07-08 | initial XTI/GBPCAD return-spread basket build | Q02 | PENDING |
...
| Q01 Build Validation | 2026-07-08 | PENDING | `artifacts/qm5_13057_build_result.json` |
