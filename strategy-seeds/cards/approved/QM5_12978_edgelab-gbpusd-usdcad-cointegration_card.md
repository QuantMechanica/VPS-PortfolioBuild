...
indicators:
  - rolling-zscore
  - atr-stop
target_symbols: [GBPUSD.DWX, USDCAD.DWX]
logical_symbol: QM5_12978_GBPUSD_USDCAD_COINTEGRATION_D1
period: D1
...
# Edge Lab GBPUSD/USDCAD Cointegration Basket
...
GBPUSD/USDCAD as the top unbuilt strict row: DEV Sharpe 0.2612, OOS net Sharpe
1.5477, OOS return 9.0401%, 19 OOS state changes, hedge -1.140460, and
62.39-day half-life.

## Concept

GBPUSD and USDCAD are both USD-linked majors, but their quote orientation is
...
Temporary dislocations in `ln(GBPUSD) - beta * ln(USDCAD)` can mean-revert
because both pairs express broad USD pressure while retaining GBP and CAD
macro/commodity-rate residuals. The scan result is in-house and low-sample, so
the pipeline is the judge.

## Markets And Timeframe

- Host symbol: GBPUSD.DWX.
- Basket legs: GBPUSD.DWX and USDCAD.DWX.
...
- The EA trades only the fixed GBPUSD/USDCAD basket; it does not reselect pairs
...
- Host chart must be GBPUSD.DWX or USDCAD.DWX on D1/H1, with slot 0 used for the logical host.
...
- name: strategy_beta
  default: -1.140460285727
  sweep_range: [-1.35, -1.140460285727, -0.95]
...
No external performance claim is taken from Chan for GBPUSD/USDCAD specifically.
The in-house full-scan rerun found DEV Sharpe 0.2612 and OOS net Sharpe 1.5477
...
- [x] R3 testable: GBPUSD.DWX and USDCAD.DWX are Darwinex-native `.DWX` symbols in the exported scan data.
