...
slug: wti-tsmom4m
status: APPROVED
execution_contract_ref: strategy-seeds/cards/approved/QM5_20280_wti-tsmom4m_card.md
...
source_citations:
  - type: peer_reviewed_paper
    ...
    location: "DOI https://doi.org/10.1016/j.jfineco.2011.11.003; complete-paper evidence strategy-seeds/sources/MOP-TSMOM-2012/source.md; bounded extraction strategy-seeds/sources/MOP-WTI-TSMOM4-2026/source.md"
    quality_tier: A
...
indicators:
  ...
  - "[[indicators/atr]]"
strategy_type_flags: [commodity, energy, crude-oil, structural-trend, time-series-momentum, exact-four-month-return, monthly-rebalance, atr-hard-stop, time-stop, symmetric-long-short, low-frequency]
...
period: D1
timeframe: D1
expected_trade_frequency: "Approximately eleven to twelve monthly WTI packages/year after five completed month ends because only exact-zero or invalid four-month states stay flat; Q02 must prove at least five completed positions/year or retire."
...
hard_rules_at_risk: [completed_month_reconstruction, exact_four_month_orientation, monthly_attempt_state, risk_mode_dual, friday_close_disabled, cfd_futures_basis, q02_frequency_floor, portfolio_correlation]
g0_approval_reasoning: "APPROVED under decisions/2026-08-11_qm5_20280_wti_tsmom4m_g0.md: R1 one complete-read peer-reviewed WTI source; R2 fixed five endpoints, exact four-month log return, symmetric direction, monthly attempt/renewal, stop, and lifecycle; R3 registered WTI D1 route; R4 deterministic native arithmetic. The canonical checker found no exact identity; expected shared-source fuzzy matches were manually separated from the absent four-completed-month mechanic."
---
...
exact return across four completed broker months may carry enough of that slow
state to avoid reacting only to the newest month while remaining more current
than six- or twelve-month trend.
...
`strategy-seeds/sources/MOP-WTI-TSMOM4-2026/source.md`. Its complete-read
...
The source does not report a standalone WTI four-month result. The exact
...
The canonical checker scanned 4,345 EA-registry rows and 456 cards. It found
no exact identity and returned expected fuzzy same-source/token matches for
manual review. The registry has WTI own-return carriers at one, two, three,
six, nine, and twelve months, but no four-completed-month carrier.

- `QM5_20187`, `QM5_20064`, `QM5_20055`, `QM5_20059`, `QM5_12616`, and
  `QM5_12603` use different horizons; the older three-month build also uses a
  63-D1-bar approximation rather than exact broker-month endpoints;
- `QM5_20056` requires six/twelve-month agreement and `QM5_20258` votes
  one/three/twelve-month signs;
- `QM5_20261`, `QM5_20264`, `QM5_20269` through `QM5_20274`, and
  `QM5_20276` through `QM5_20279` use regression, rank, robust-return,
  pairwise, path, run, linear-weight, or exponential-weight state; and
- calendar, event, XNG, XAU/XAG, index, and metal systems do not use this
  exact WTI five-endpoint/four-completed-month state.

The five endpoints, monthly continuity, exact `(C[0], C[4])` orientation,
symmetric sign direction, consumed attempt, and monthly renewal are jointly
load-bearing. Verdict: `CLEAN_AFTER_EXPECTED_SHARED_SOURCE_FUZZY_REVIEW`.

## Markets, Timeframe, And Cadence

- Exact host and traded symbol: `XTIUSD.DWX`.
- Timeframe: D1; magic slot 0; intended magic `202800000`.
- Decision clock: first processed D1 bar after a genuine broker-month change.
- Formation: five consecutive completed broker-month closes spanning exactly
  four completed months.
...
At the start of month `t`, let `C[0]..C[4]` be completed month-end closes from
months `t-5..t-1`, ordered oldest to newest:
...
no fallback to a D1-bar approximation, different horizon, sign vote, return
average, sort, clipping, regression, moving average, oscillator, calendar
direction, external series, or previous pipeline result.

## 4. Entry Rules

1. Require exact EA ID `20280`, `XTIUSD.DWX` D1, magic slot 0, and every
...
5. Reconstruct exactly five completed month-end closes from bounded D1
...
7. Calculate one finite log return `ln(C[4]/C[0])`. Buy when positive and sell
...
|---|---:|---|---|
| `strategy_return_months` | 4 | [4] | exact completed broker-month interval |
| `strategy_history_bars_d1` | 400 | [400] | bounded endpoint reconstruction |
...
WTI in their commodity universe. They do not claim this four-month WTI CFD
rule works, that four months is optimal, that a continuous CFD reproduces
...
correlation with XNG or risk assets can dominate the premise. The four-month
...
- Fail on endpoint leakage, missing/interpolated/nonconsecutive months,
  reversed return orientation, a horizon other than four completed months,
  wrong-side entry, repeated monthly attempt, missing hard stop, hold beyond
  forty days, invalid risk mode, or nondeterminism.
...
- [x] Dedup: no exact identity; expected same-source fuzzy matches are
  manually separated from the absent four-completed-month mechanic.

## Framework Alignment

- no_trade: exact WTI/D1/EA/slot, locked inputs, fixed risk/news/Friday
  contract, and cheap parameter guards.
- trade_entry: month-attempt persistence, five-endpoint reconstruction, exact
  four-month return sign, spread/quote/ATR/stop checks, and one fixed-risk
  order.
