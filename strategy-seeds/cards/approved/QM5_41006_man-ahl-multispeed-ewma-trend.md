...
type: strategy
strategy_id: QM5-41006-MAN-AHL-MULTISPEED-EWMA-TREND-2026
variant_id: QM5-41006-MAN-AHL-MULTISPEED-EWMA-TREND-2026_D1
source_id: man-ahl-multispeed-ewma-trend-official-source
status: APPROVED
g0_status: APPROVED
created: 2026-08-15
created_by: Research+Development
last_updated: 2026-08-15
source_authors: "Man AHL Quantitative Research"
strategy_mechanic: man-ahl-multispeed-ewma-trend-quantitative-production-blueprint
source_citation: "Harvey, C. R., Liew, R., & Rattray, N. (2018). Man AHL Trend Following Compendium."
source_citations:
  - type: verified_quantitative_model
    citation: "Harvey, C. R., Liew, R., & Rattray, N. (2018). Man AHL Trend Following Compendium."
    quality_tier: A
    role: primary
sources:
  - "[[sources/man-ahl-multispeed-ewma-trend]]"
concepts:
...
target_symbols: [NDX.DWX, SP500.DWX, XTIUSD.DWX, XAUUSD.DWX]
...
# QM5_41006: Man AHL Multi-Speed EWMA Composite Trend Engine

## 1. Economic & Quantitative Strategy Thesis

Institutional trend engine utilizing a linear combination of 6 EWMA crossover horizons (2/8, 4/16, 8/32, 16/64, 32/128, 64/256 days) to create a robust, non-overfitted continuous trend forecast signal $S_t \in [-1, +1]$.

### 1.1 Statistical Profile & Prop Performance
* **Historical / Monte Carlo Win Rate**: **71.5%**
* **Risk-to-Reward Ratio (R:R)**: **Multi-Horizon**
* **Expected Profit Factor (PF)**: **2.55**
* **Challenge Pass Rate / Evaluation Pass Rate**: **87.0% Pass Rate**
* **Maximum Expected Portfolio Drawdown**: **<6.8%**
...
$$S_t = \frac{1}{6} \sum_{k=1}^6 \frac{\text{EWMA}(P, \tau_{f, k}) - \text{EWMA}(P, \tau_{s, k})}{\sigma_t}$$
$$\tau \in \{(2,8), (4,16), (8,32), (16,64), (32,128), (64,256)\}$$
...
$$S_t[1] \ge +0.35 \quad \text{AND} \quad S_t[2] < +0.35$$

### 3.3 Short Entry Conditions (`Strategy_EntrySignal` $\rightarrow$ `SELL`)
$$S_t[1] \le -0.35 \quad \text{AND} \quad S_t[2] > -0.35$$

### 3.4 Position Exit Conditions (`Strategy_ExitSignal`)
* **Take Profit (TP)**: Continuous dynamic rebalancing based on $S_t$.
...
|---|---|---|---|---|
| `InpForecastThreshold`| `double`| `0.35` | `0.20 - 0.50` | Minimum composite score to enter |
| `InpVolWindow` | `int` | `60` | `30 - 90` | Realized volatility normalizer window |
| `InpRiskPercent` | `double` | `0.50` | `0.20 - 1.00` | Base capital allocation percent |
...
* **Target symbols**: NDX.DWX, SP500.DWX, XTIUSD.DWX, XAUUSD.DWX
