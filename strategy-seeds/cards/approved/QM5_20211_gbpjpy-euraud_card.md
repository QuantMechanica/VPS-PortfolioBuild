---
strategy_id: AI-CODEX-FX-COINT66-20260609-GBPJPY-EURAUD
ea_id: QM5_20211
slug: gbpjpy-euraud
type: strategy
status: APPROVED
...
markets: [forex]
timeframes: [D1]
primary_target_symbols: [GBPJPY.DWX, EURAUD.DWX]
target_symbols: [GBPJPY.DWX, EURAUD.DWX]
logical_symbol: QM5_20211_GBPJPY_EURAUD_COINTEGRATION_D1
period: D1
expected_trade_frequency: "Approximately 4 completed two-leg packages per year per traded symbol, inferred from 19 OOS basket state changes across 2023-2024; Q02 must retire the sleeve if realized frequency is below the binding floor."
...
# QM5_20211 GBPJPY/EURAUD D1 Cointegration Basket
...
low-frequency daily implementation. Chan makes no claim for GBPJPY/EURAUD.
...
GBPJPY/EURAUD ranks thirty-first of 66 by OOS net Sharpe. Rank 30,
GBPUSD/AUDUSD, is already represented by dedicated fixed-D1 basket
`QM5_12739`. Exact strategy and slug searches, card and registry searches,
and an unordered traded-symbol manifest reconciliation found no dedicated
fixed-beta GBPJPY/EURAUD D1 card, allocation, EA, or logical basket before
this card.
...
The sub-0.8 OOS Sharpe is adverse evidence. The positive DEV result and
approximately 34-bar half-life do not override the weak OOS score or the
material swap and cross-currency exposure. This card authorizes one
low-frequency frontier test, not a certified edge. A terminal economic
failure retires the sleeve; it does not authorize a filter, beta refit, or
parameter rescue.
...
`ln(GBPJPY) - (-1.386054145) * ln(EURAUD)`. The negative fitted coefficient
...
The package retains GBP, JPY, EUR, AUD, carry, and risk-sentiment exposure.
That sign-sensitive structure and the negative beta are explicit high-risk
caveats rather than permission to add a filter or refit the relationship.

## Hypothesis

The fixed scan suggests large deviations in the GBPJPY/EURAUD residual may
mean-revert slowly enough for a D1 two-leg implementation to survive costs.
Because OOS performance missed the original survivor bar, the hypothesis is
deliberately weak and falsifiable: real-tick cost, swap, cadence, or
profitability failure retires this exact sleeve.
...
`GBPJPY.DWX` is the tester host and first traded leg. `EURAUD.DWX` is the
companion traded leg. `USDJPY.DWX`, `GBPUSD.DWX`, `EURUSD.DWX`, and
`AUDUSD.DWX` are conversion-only histories selected and warmed for the JPY,
GBP, EUR, and AUD legs under the USD tester account; they receive no order and
no magic slot.
...
- Compute `spread = ln(GBPJPY) - strategy_beta * ln(EURAUD)`, with frozen
  `strategy_beta = -1.386054145`.
...
- Permit entry only when the host is `GBPJPY.DWX`, the configured period is D1
...
Chan supplies the pair-trading method but reports no GBPJPY/EURAUD result.
...
```yaml
target_modules:
  no_trade: "Select and warm six declared histories; reject wrong host, timeframe, slot, unaligned traded history, or invalid normalized volume."
...
data_requirements: "GBPJPY.DWX, EURAUD.DWX, USDJPY.DWX, GBPUSD.DWX, EURUSD.DWX, and AUDUSD.DWX D1 histories"
```
...
| Q01 Build Validation | 2026-08-04 | PASS | `D:\QM\reports\framework\21\build_check_20260804_023205.json` |
| Q02 Baseline Screening | 2026-08-04 | PENDING | work item `21db772c-c974-4a05-8e21-5ec78659e988` |

## 16. Lessons Captured

- 2026-08-04: Rank 31 is below the original survivor bar despite positive DEV
  evidence and a moderate estimated half-life; it is admitted only as the
  explicitly requested next-best one-shot sleeve, with retirement rather than
  rescue after economic failure.
