...
One restart-safe long attempt is permitted at the broker Friday 21:00 H1
boundary. A valid position is held only until the first Monday H1 boundary, the
four-day stale guard, or the fixed ATR hard stop.
...
Weekend gaps can exceed the requested stop, and the source's historical
settlement boundary may not match Darwinex CFD bars. Fixed-risk sizing bounds
the requested stop loss but cannot remove gap slippage. Q02 must use the
canonical backtest risk mode; no percentage-risk or live configuration belongs
in this build.
