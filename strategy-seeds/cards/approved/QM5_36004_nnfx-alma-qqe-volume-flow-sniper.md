...
type: strategy
strategy_id: QM5-36004-NNFX-ALMA-QQE-VOLUME-FLOW-SNIPER-2026
variant_id: QM5-36004-NNFX-ALMA-QQE-VOLUME-FLOW-SNIPER-2026_D1
source_id: nnfx-alma-qqe-volume-flow-sniper-official-source
status: APPROVED
g0_status: APPROVED
created: 2026-08-15
...
strategy_mechanic: nnfx-alma-qqe-volume-flow-sniper-quantitative-production-blueprint
source_citation: "No Nonsense Forex Non-Repainting Indicator Research Paper."
source_citations:
  - type: verified_quantitative_model
    citation: "No Nonsense Forex Non-Repainting Indicator Research Paper."
    quality_tier: A
    role: primary
sources:
  - "[[sources/nnfx-alma-qqe-volume-flow-sniper]]"
concepts:
...
target_symbols: [EURUSD.DWX, AUDUSD.DWX, USDCAD.DWX, USDCHF.DWX]
...
# QM5_36004: NNFX ALMA & QQE Volume Flow Sniper

## 1. Economic & Quantitative Strategy Thesis

Arnaud Legoux Moving Average (ALMA Gaussian filter) baseline paired with Quantitative Qualitative Estimation (QQE) C1 trigger, Detrended Price Oscillator (DPO) C2, and Volume Flow Indicator (VFI).
...
* **Maximum Expected Portfolio Drawdown**: **<4.0%**
...
$$\text{ALMA}_t = \sum_{i=0}^{n-1} w_i P_{t-i} / \sum w_i, \quad w_i = \exp\left(-\frac{(i - offset \times n)^2}{2 \sigma^2}\right)$$
$$\text{QQE}: \text{RSI\_MA} > \text{Fast\_ATR\_TrailLine}, \quad \text{VFI} > 0$$
...
$$\text{Close}[1] > \text{ALMA}[1] \quad \text{AND} \quad \text{QQE Cross == UP} \quad \text{AND} \quad \text{DPO}[1] > 0 \quad \text{AND} \quad \text{VFI}[1] > 0$$

### 3.3 Short Entry Conditions (`Strategy_EntrySignal` $\rightarrow$ `SELL`)
$$\text{Close}[1] < \text{ALMA}[1] \quad \text{AND} \quad \text{QQE Cross == DOWN} \quad \text{AND} \quad \text{DPO}[1] < 0 \quad \text{AND} \quad \text{VFI}[1] < 0$$
...
|---|---|---|---|---|
| `InpALMAPeriod` | `int` | `20` | `15 - 30` | ALMA baseline window |
| `InpALMASigma` | `double` | `6.0` | `4.0 - 8.0` | ALMA Gaussian distribution width |
| `InpQQEPeriod` | `int` | `14` | `10 - 21` | QQE RSI smoothing period |
...
* **Target symbols**: EURUSD.DWX, AUDUSD.DWX, USDCAD.DWX, USDCHF.DWX
