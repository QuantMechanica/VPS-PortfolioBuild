...
slug: wti-hl-mom
status: APPROVED
execution_contract_ref: strategy-seeds/cards/approved/QM5_20276_wti-hl-mom_card.md
...
source_citations:
  - type: peer_reviewed_paper
    ...
    location: "DOI https://doi.org/10.1016/j.jfineco.2011.11.003; complete-paper evidence strategy-seeds/sources/MOP-TSMOM-2012/source.md; bounded extraction strategy-seeds/sources/MOP-WTI-HLRET-2026/source.md"
    quality_tier: A
...
indicators:
  ...
  - "[[indicators/hodges-lehmann-pseudomedian]]"
  - "[[indicators/atr]]"
...
period: D1
timeframe: D1
expected_trade_frequency: "Approximately eleven to twelve monthly WTI packages/year after thirteen completed month ends because only exact-zero or invalid pseudomedian states stay flat; Q02 must prove at least five completed positions/year or retire."
...
hard_rules_at_risk: [completed_month_reconstruction, chronological_log_return_orientation, inclusive_pair_enumeration, exact_pair_count, ascending_sort, even_sample_median, monthly_attempt_state, risk_mode_dual, friday_close_disabled, cfd_futures_basis, q02_frequency_floor, portfolio_correlation]
g0_approval_reasoning: "APPROVED under decisions/2026-08-11_qm5_20276_wti_hl_mom_g0.md: R1 one complete-read peer-reviewed WTI source; R2 fixed thirteen endpoints, twelve disjoint returns, exact 78 inclusive pairwise averages, sort, center indexes, direction, and lifecycle; R3 registered WTI D1 route; R4 deterministic native arithmetic. The deterministic checker found no exact identity and manual review separated the two fuzzy robust-return siblings and the pairwise price-slope system."
---
...
twelve-month return can be dominated by one shock; a raw median discards most
return magnitudes. The one-sample pairwise-average pseudomedian uses every
observed month both alone and in cross-pairs to estimate a robust central
monthly return before following its sign.
...
`strategy-seeds/sources/MOP-WTI-HLRET-2026/source.md`. Its complete-read
...
The source does not use a Hodges-Lehmann estimator. The 78 inclusive pairwise
averages, even-sample center rule, Darwinex continuous CFD, broker-month
...
The canonical checker scanned 4,341 EA-registry rows and 451 cards. It found
no exact identity and surfaced two expected 0.50 same-source fuzzy matches.
Manual mechanic review resolves the closest systems:

- `QM5_20269_wti-medret-mom` uses only the two center observations from a
  sorted twelve-return sample;
- `QM5_20270_wti-trimmean-mom` removes two returns per tail and averages the
  eight directly observed middle returns;
- `QM5_20271_wti-theilsen-tr` uses 78 forward slopes between thirteen log-
  price levels with `i < j` and mandatory `j-i` time denominators; this rule
  uses 78 averages of twelve adjacent returns with `i <= j`;
- cumulative-return, sign-count/run, fixed-block-vote, OLS/rank, and path-
  efficiency systems estimate different functionals.

The thirteen endpoints, twelve disjoint intervals, inclusive pair enumeration,
78 averages, sort, center indexes 38 and 39, direction, consumed attempt, and
...
- Timeframe: D1; magic slot 0; intended magic `202760000`.
...
- Signal state: sign of the even-sample median of all 78 inclusive pairwise
  averages of those returns.
...
BUY when `hl > 0`. SELL when `hl < 0`. Exact zero or invalid state remains
flat. Every self-pair is present, so `w(i,i)=r[i]`; no pair is duplicated or
omitted. The statistic's magnitude never scales risk.

## Rules

These are the complete authorized baseline. There is no signal-parameter
sweep and no fallback to a cumulative return, raw median, trimmed mean, sign
statistic, vote, regression, rank statistic, moving average, oscillator,
calendar direction, external series, or previous pipeline result.

## 4. Entry Rules

1. Require exact EA ID `20276`, `XTIUSD.DWX` D1, magic slot 0, and every
...
7. Calculate exactly twelve finite adjacent log returns from pairs `(0,1)`
   ...
   allowed.
8. Enumerate `(i,j)` in nested ascending order with `i=0..11` and `j=i..11`.
   Form `(r[i]+r[j])/2`, require finite arithmetic, and require exactly 78
   values. Self-pairs must remain included.
9. Sort the 78 values ascending and compute exactly
   `(sorted[38]+sorted[39])/2`. Buy when positive and sell when negative;
   exact zero remains flat.
...
- Reject a consumed attempt, owned exposure, same-month entry history,
  ...
  omitted or duplicated inclusive pair, wrong pair count, nonfinite average,
  wrong sort or center indexes, zero signal, excessive spread, invalid quote,
...
inclusive pair convention, average divisor, pair count, sort, median indexes,
...
WTI in their commodity universe. They do not claim this pairwise-average
pseudomedian works, that a continuous CFD reproduces rolling futures, or that
the candidate diversifies the QM book.
...
correlation with XNG or risk assets can dominate the premise. Robust location
is descriptive and does not guarantee stable edge or neutrality.
...
- Fail on wrong endpoint order, nonconsecutive months, current-month leakage,
  wrong adjacent pairs or log orientation, excluding self-pairs, wrong nested
  pair bounds, pair count other than 78, wrong average divisor, wrong sort or
  median indexes, wrong-side entry, repeated monthly attempt, hold beyond
...
- Retire on nonpositive governed economics or later portfolio-correlation
  rejection.
- Do not rescue failure by changing horizon, estimator, pair convention,
...
- [x] R2: PASS. Fixed endpoints, returns, inclusive pairs, averages, sort,
  center indexes, direction, attempt, hard stop, rollover, and stale exit.
...
- [x] Dedup: no exact identity; raw-median, trimmed-mean, pairwise price-slope,
  cumulative, sign/vote, rank/regression, and path-efficiency systems were
  manually resolved.
...
- trade_entry: month-attempt persistence, endpoint reconstruction, adjacent
  returns, exact inclusive pairwise averages, sort, pseudomedian, spread/quote/
  ATR/stop checks, and one fixed-risk order.
