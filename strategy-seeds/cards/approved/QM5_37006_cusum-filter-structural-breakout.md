...
type: strategy
strategy_id: QM5-37006-CUSUM-FILTER-STRUCTURAL-BREAKOUT-2026
variant_id: QM5-37006-CUSUM-FILTER-STRUCTURAL-BREAKOUT-2026_M15
source_id: cusum-filter-structural-breakout-official-source
status: APPROVED
g0_status: APPROVED
created: 2026-08-15
...
strategy_mechanic: cusum-filter-structural-breakout-quantitative-production-blueprint
source_citation: "López de Prado, M. (2018). Advances in Financial Machine Learning. Symmetric CUSUM Filter."
source_citations:
  - type: verified_quantitative_model
    citation: "López de Prado, M. (2018). Advances in Financial Machine Learning. Symmetric CUSUM Filter."
    quality_tier: A
    role: primary
sources:
  - "[[sources/cusum-filter-structural-breakout]]"
concepts:
...
target_symbols: [NDX.DWX, SP500.DWX, XTIUSD.DWX]
...
# QM5_37006: Marcos López de Prado CUSUM Structural Breakout

## 1. Economic & Quantitative Strategy Thesis

Quality control Cumulative Sum (CUSUM) filter. Detects structural shifts in the mean value of continuous price returns exceeding a dynamic volatility threshold $h = 1.5 \times \sigma$.
...
$$S_t^+ = \max(0, S_{t-1}^+ + y_t - \mathbb{E}[y_t]), \quad S_t^- = \min(0, S_{t-1}^- + y_t - \mathbb{E}[y_t])$$
$$\text{Threshold } h = 1.50 \times \text{std}(\Delta P, 50)$$
...
$$S_t^+ \ge h \implies \text{BUY (Structural Bullish Regime Shift)}$$

### 3.3 Short Entry Conditions (`Strategy_EntrySignal` $\rightarrow$ `SELL`)
$$S_t^- \le -h \implies \text{SELL (Structural Bearish Regime Shift)}$$
...
* **CUSUM Reset**: Reset $S_t = 0$ upon trade execution.
...
|---|---|---|---|---|
| `InpVolWindow` | `int` | `50` | `20 - 100` | Rolling return volatility window |
| `InpThresholdH` | `double` | `1.50` | `1.0 - 2.5` | Standard deviation multiplier for CUSUM threshold |
...
* **Target symbols**: NDX.DWX, SP500.DWX, XTIUSD.DWX
