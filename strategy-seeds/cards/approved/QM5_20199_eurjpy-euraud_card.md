---
strategy_id: AI-CODEX-FX-COINT66-20260609-EURJPY-EURAUD
ea_id: QM5_20199
slug: eurjpy-euraud
type: strategy
status: APPROVED
...
markets: [forex]
timeframes: [D1]
primary_target_symbols: [EURJPY.DWX, EURAUD.DWX]
target_symbols: [EURJPY.DWX, EURAUD.DWX]
logical_symbol: QM5_20199_EURJPY_EURAUD_COINTEGRATION_D1
period: D1
...
# QM5_20199 EURJPY/EURAUD D1 Cointegration Basket
...
low-frequency daily implementation. Chan makes no claim for EURJPY/EURAUD.
...
EURJPY/EURAUD ranks sixteenth of 66 by OOS net Sharpe. Rank 15
EURJPY/EURGBP is already built as `QM5_20197`; its Q01 build passed and its
Q02 enqueue remains capacity-governed. Exact-pair searches found no dedicated
fixed-beta EURJPY/EURAUD D1 card, EA, or logical basket manifest.
...
Broad multi-symbol EUR strength systems and conversion-history declarations
...
`ln(EURJPY) - (-1.073345776) * ln(EURAUD)`. The negative fitted coefficient
...
Both pairs share EUR as the base and retain AUD, JPY, carry, and risk-sentiment
exposure. The shared-base structure and negative beta are explicit high-risk
caveats rather than permission to add a filter.

## Hypothesis

The fixed scan suggests large deviations in the EURJPY/EURAUD residual may
...
`EURJPY.DWX` is the tester host and first traded leg. `EURAUD.DWX` is the
...
- Compute `spread = ln(EURJPY) - strategy_beta * ln(EURAUD)`, with frozen
  `strategy_beta = -1.073345776`.
...
- Permit entry only when the host is `EURJPY.DWX`, the configured period is D1
...
Chan supplies the pair-trading method but reports no EURJPY/EURAUD result.
...
data_requirements: "EURJPY.DWX, EURAUD.DWX, USDJPY.DWX, AUDUSD.DWX, and EURUSD.DWX D1 histories"
```
...
| Q01 Build Validation | 2026-08-01 | PASS | `D:\QM\reports\framework\21\build_check_20260801_192533.json` |
| Q02 Baseline Screening | 2026-08-01 | PENDING | work item `40bb7cc0-05f8-453a-a03e-515e7a916b13` |

## 16. Lessons Captured

- 2026-08-01: Rank 16 is below the original survivor bar; it is admitted only
