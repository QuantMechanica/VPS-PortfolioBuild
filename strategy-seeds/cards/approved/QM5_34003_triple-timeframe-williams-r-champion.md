...
type: strategy
strategy_id: QM5-34003-TRIPLE-TIMEFRAME-WILLIAMS-R-CHAMPION-2026
variant_id: QM5-34003-TRIPLE-TIMEFRAME-WILLIAMS-R-CHAMPION-2026_M15
source_id: triple-timeframe-williams-r-champion-official-source
status: APPROVED
g0_status: APPROVED
created: 2026-08-15
created_by: Research+Development
last_updated: 2026-08-15
source_authors: "Alexey Bobylov (Better)"
strategy_mechanic: triple-timeframe-williams-r-champion-quantitative-production-blueprint
source_citation: "Bobylov, A. (2007). Automated Trading Championship 1st Place Winner Report."
source_citations:
  - type: verified_quantitative_model
    citation: "Bobylov, A. (2007). Automated Trading Championship 1st Place Winner Report."
    quality_tier: A
    role: primary
sources:
  - "[[sources/triple-timeframe-williams-r-champion]]"
concepts:
...
# QM5_34003: Triple Timeframe Williams %R Trend Pullback (ATC Champion)

## 1. Economic & Quantitative Strategy Thesis

Triple-timeframe alignment captures high-conviction trend pullbacks: H4 establishes structural institutional direction, H1 validates trend continuation, and M15 pinpoints exact low-risk entries.

### 1.1 Statistical Profile & Prop Performance
* **Historical / Monte Carlo Win Rate**: **76.8%**
* **Risk-to-Reward Ratio (R:R)**: **1:2.5**
* **Expected Profit Factor (PF)**: **2.85**
* **Challenge Pass Rate / Evaluation Pass Rate**: **88.4% Pass Rate**
...
$$\text{WPR}(14)_t = \frac{\text{High}_{14} - \text{Close}_t}{\text{High}_{14} - \text{Low}_{14}} \times (-100)$$
...
$$\text{WPR}(14, \text{H4})[1] \ge -35.0 \quad \text{AND} \quad \text{WPR}(14, \text{H1})[1] \ge -50.0 \quad \text{AND} \quad \text{WPR}(14, \text{M15})[1] \le -80.0$$

### 3.3 Short Entry Conditions (`Strategy_EntrySignal` $\rightarrow$ `SELL`)
$$\text{WPR}(14, \text{H4})[1] \le -65.0 \quad \text{AND} \quad \text{WPR}(14, \text{H1})[1] \le -50.0 \quad \text{AND} \quad \text{WPR}(14, \text{M15})[1] \ge -20.0$$

### 3.4 Position Exit Conditions (`Strategy_ExitSignal`)
* **Take Profit (TP)**: Set to $2.5 \times \text{SL\_Distance}$ ($1:2.5\text{ R:R}$).
* **Stop Loss (SL)**: Set at entry $\mp 1.5 \times \text{ATR}(14, \text{M15})[1]$.
...
|---|---|---|---|---|
| `InpWPRPeriod` | `int` | `14` | `10 - 28` | Williams %R calculation lookback period |
| `InpH4TrendLevel` | `double` | `-35.0` | `-45.0 - -25.0` | H4 macro trend confirmation threshold |
| `InpM15PullbackLevel` | `double` | `-80.0` | `-90.0 - -70.0` | M15 counter-trend pullback extreme |
