...
`XTIUSD.DWX` price after controlled pullbacks during the May 15 through August
31 window.

The mechanical expression is long-only: buy D1 pullback/reversal candles during
the jet fuel summer window only when crude is already above a rising 100-day D1
trend SMA. Exit when the seasonal window ends, trend breaks, the short Donchian
exit low breaks, or a time stop is reached.
