...
Approved copy of `strategy-seeds/cards/wti-brent-brk_card.md`.

This card implements a market-neutral D1 Brent/WTI channel-breakout basket on
`XTIUSD.DWX` and `XBRUSD.DWX`. It buys Brent and sells WTI on upside spread
breakouts, sells Brent and buys WTI on downside spread breakouts, and exits on
a shorter opposite-channel break, max-hold, Friday close, broken-package repair,
or hard per-leg ATR stops.

It is deliberately not `QM5_12843_wti-brent-spread`: that card fades
Brent/WTI z-score extremes, while this card follows continuation after a
completed-bar spread channel break.
