...
type: strategy
strategy_id: QM5-41001-KEITH-FITSCHEN-ABERRATION-TRADING-SYSTEM-2026
variant_id: QM5-41001-KEITH-FITSCHEN-ABERRATION-TRADING-SYSTEM-2026_D1
source_id: keith-fitschen-aberration-trading-system-official-source
status: APPROVED
g0_status: APPROVED
created: 2026-08-15
created_by: Research+Development
last_updated: 2026-08-15
source_authors: "Keith Fitschen"
strategy_mechanic: keith-fitschen-aberration-trading-system-quantitative-production-blueprint
source_citation: "Fitschen, K. (1993). Aberration Trading System. Futures Truth Magazine #1 All-Time System."
source_citations:
  - type: verified_quantitative_model
    citation: "Fitschen, K. (1993). Aberration Trading System. Futures Truth Magazine #1 All-Time System."
    quality_tier: A
    role: primary
sources:
  - "[[sources/keith-fitschen-aberration-trading-system]]"
concepts:
...
target_symbols: [XTIUSD.DWX, XAUUSD.DWX, SP500.DWX]
...
# QM5_41001: Keith Fitschen Aberration Commodity Trend System

## 1. Economic & Quantitative Strategy Thesis

Ranked #1 All-Time Trading System by Futures Truth Magazine. Uses 3.0-standard-deviation 30-day Bollinger Bands to capture massive commodity and energy super-trends, exiting on the 30-day SMA midline.
...
$$\text{UpperBand} = \text{SMA}(30, \text{D1}) + 3.0 \sigma, \quad \text{LowerBand} = \text{SMA}(30, \text{D1}) - 3.0 \sigma$$
...
$$\text{Close}[1] > \text{UpperBand}[1] \quad \text{AND} \quad \text{Close}[2] \le \text{UpperBand}[2]$$

### 3.3 Short Entry Conditions (`Strategy_EntrySignal` $\rightarrow$ `SELL`)
$$\text{Close}[1] < \text{LowerBand}[1] \quad \text{AND} \quad \text{Close}[2] \ge \text{LowerBand}[2]$$

### 3.4 Position Exit Conditions (`Strategy_ExitSignal`)
* **Take Profit (TP)**: Open-ended trend riding with midline trailing exit.
* **Stop Loss (SL)**: Set at entry $\mp 2.5 \times \text{ATR}(14, \text{D1})[1]$.
* **Midline Exit**: Close long when $\text{Close}[1] < \text{SMA}(30, \text{D1})[1]$.
...
|---|---|---|---|---|
| `InpSMAPeriod` | `int` | `30` | `20 - 45` | Aberration SMA baseline lookback |
| `InpDevMultiplier`| `double` | `3.00` | `2.5 - 3.5` | Standard deviation multiplier |
...
* **Target symbols**: XTIUSD.DWX, XAUUSD.DWX, SP500.DWX
