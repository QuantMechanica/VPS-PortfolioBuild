...
period: D1
timeframes: [D1]
expected_trade_frequency: "Weekly D1 Brent Thursday-calendar premium sleeve; estimate 40-52 trades/year after broker holidays, XBR history availability, and framework filters."
...
g0_approval_reasoning: "R1 PASS peer-reviewed crude-oil day-of-week source with explicit Brent/WTI lineage; R2 PASS deterministic Thursday D1 long/next-bar flat rule with ATR stop; R3 PASS wide-net DWX route via XBRUSD.DWX farm symbol map/prior XBR setfiles with Q02 validating history sufficiency; R4 PASS no ML/grid/martingale/external runtime data."
expected_pf: 1.08
...
and WTI return samples. This card isolates the Thursday premium on the Brent
benchmark instead of adding another WTI leg: buy `XBRUSD.DWX` only on the
broker-calendar Thursday D1 bar and flatten on the first subsequent D1 bar.

This is deliberately different from:

- `QM5_12771_wti-thu-prem`: same weekday thesis but WTI benchmark
  (`XTIUSD.DWX`), not Brent.
- `QM5_12596_wti-mon-fade`, `QM5_12610_wti-tue-fade`, `QM5_12597_wti-fri-prem`,
  and `QM5_12775_wti-wed-prem`: different weekday and/or direction.
...
in Brent and WTI returns, including the Thursday premium. No source performance
...
- [x] Non-duplicate: Brent benchmark calendar premium is not another XTIUSD
  WTI build, not XNG, not XAU/XAG, not XTI/XNG, and not commodity RSI logic.
...
|---|---|---|---|
| v1 | 2026-07-01 | initial structural Brent Thursday calendar-premium card | Q02 | ENQUEUED |
...
| Q01 Build Validation | 2026-07-01 | PASS | `artifacts/qm5_12841_build_result.json` |
| Q02 Baseline Screening | 2026-07-01 | QUEUED | `D:\QM\strategy_farm\state\farm_state.sqlite` work item `9b381b73-9cf8-42d7-9f68-d00e400f4d3b` |
