...
type: strategy
strategy_id: SCHWEIKERT-CME-XAUXAG-WDECAY-RV-2026_S01
variant_id: SCHWEIKERT-CME-XAUXAG-WDECAY-RV-2026_S01
source_id: SCHWEIKERT-CME-XAUXAG-WDECAY-RV-2026
ea_id: QM5_41066
slug: xauxag-wdecay-rv
status: APPROVED
execution_contract_ref: strategy-seeds/cards/approved/QM5_41066_xauxag-wdecay-rv_card.md
execution_contract_status: APPROVED
created: 2026-08-20
created_by: Research+Development
last_updated: 2026-08-20
g0_status: APPROVED
g0_decision: decisions/2026-08-20_qm5_41066_xauxag_weekly_deceleration_reversion_g0.md
source_approval: decisions/2026-08-20_xauxag_weekly_deceleration_reversion_source_approval.md
...
strategy_mechanic: synchronized-completed-two-adjacent-week-xau-minus-xag-relative-returns-same-sign-newest-strictly-smaller-fade-cumulative-direction-one-week-equal-notional-basket
sources:
  - "[[sources/SCHWEIKERT-CME-XAUXAG-WDECAY-RV-2026]]"
concepts:
  - "[[concepts/gold-silver-relative-value]]"
  - "[[concepts/weekly-deceleration-reversion]]"
  - "[[concepts/market-neutral-basket]]"
indicators:
  - "[[indicators/adjacent-completed-week-relative-log-returns]]"
  - "[[indicators/atr-risk-stop]]"
strategy_type_flags: [commodity, precious-metals, gold-silver-ratio, market-neutral-basket, weekly-deceleration, mean-reversion, symmetric-long-short, atr-hard-stop, time-stop, low-frequency]
markets: [commodities, precious_metals]
timeframes: [D1]
target_symbols: [XAUUSD.DWX, XAGUSD.DWX]
primary_target_symbols: [XAUUSD.DWX, XAGUSD.DWX]
single_symbol_only: false
logical_symbol: QM5_41066_XAU_XAG_WDECAY_RV_D1
symbol: QM5_41066_XAU_XAG_WDECAY_RV_D1
host_symbol: XAUUSD.DWX
companion_symbol: XAGUSD.DWX
symbol_slots: [0, 1]
magic_numbers: [410660000, 410660001]
period: D1
timeframe: D1
expected_trade_frequency: "Approximately eight to eighteen completed paired packages per full post-warm-up year after exact synchronized week, same-sign, strict deceleration, and execution gates; Q02 must prove at least five/year or retire."
expected_trades_per_year_per_symbol: 13
expected_pf: 1.01
expected_dd_pct: 30.0
risk_class: high
ml_required: false
r1_track_record: PASS_WITH_WEEKLY_EXHAUSTION_TRANSLATION_RISK
r2_mechanical: PASS
...
hard_rules_at_risk: [exact_xau_xag_carrier, synchronized_week_endpoints, consecutive_monday_anchors, nonoverlapping_relative_returns, strict_same_sign, strict_newest_magnitude_decay, inverse_basket_direction, persistent_week_attempt, equal_notional_basket, aggregate_fixed_risk, hard_stops_present, atomic_package_repair, next_week_exit, risk_mode_dual, q02_frequency_floor, portfolio_correlation]
g0_approval_reasoning: "R1 uses one bounded child source with named peer-reviewed DOI and official exchange lineages while disclosing the weekly deceleration fade as an untested QM translation; R2 locks exact synchronized weeks, chronological relative returns, strict same sign, strict magnitude decay, inverse side, durable attempt, aggregate fixed risk, equal notional, hard stops, spreads, and next-week lifecycle; R3 uses registered native XAU/XAG D1 histories and active slots 0/1 with synchronization and CFD-basis risks explicit; R4 is deterministic timestamp, price, logarithm, ATR, quote, position, deal, and terminal-state arithmetic without a banned signal, trained output, external feed, grid, or martingale; canonical dedup and manual family review found no exact identity."
---

# QM5_41066 XAU/XAG Completed-Week Deceleration Reversion

## Hypothesis

When the gold/silver log ratio moves in the same direction for two completed
broker weeks but the second move is strictly smaller, the relative trend may
be losing force. Fading that shared direction for the next broker week may
capture convergence without taking an outright single-metal signal.
...
source tests adjacent weekly returns, strict same-sign magnitude decay, this
contrarian direction, a continuous-CFD package, equal-notional sizing, fixed
cash risk, ATR stops, or a one-week hold.
...
returns, strict same sign, strict newest-magnitude decay, inverse side, weekly
...
- Exact host: `XAUUSD.DWX`, D1, slot 0, magic `410660000`.
- Exact companion: `XAGUSD.DWX`, D1, slot 1, magic `410660001`.
...
- Signal: two strict same-sign weekly relative returns with strictly smaller
  newest absolute magnitude; fade their shared direction.
...
1. Evaluate only once on a new exact `XAUUSD.DWX` D1 bar under EA 41066 and
...
6. Compute `s1`, `s2`, `s3`, `r_new`, and `r_old` exactly as specified. Require
   ...
   `abs(r_new)<abs(r_old)`. Sign opposition, either zero, equality, or
   acceleration remains flat.
7. SELL XAU/BUY XAG only when both returns are positive and decelerating. BUY
   XAU/SELL XAG only when both are negative and decelerating. Signal magnitude
   never changes side or risk.
...
- Exact host, D1, EA 41066, slot zero, and both registered magics.
...
- First-week-bar clock, synchronized timestamps, three consecutive week
  anchors, positive finite prices, strict same sign, strict deceleration,
  durable attempt, side-specific trade mode, spread, quote, ATR, sizing, stop
  geometry, and notional match all fail closed.
...
- Own exactly one `XAUUSD.DWX` position under magic `410660000` and one
  opposite-side `XAGUSD.DWX` position under magic `410660001`.
...
source supplies the weekly horizon or deceleration fade.

## QM Interpretations

`SCHWEIKERT-CME-XAUXAG-WDECAY-RV-2026_S01` fixes the exact weekly endpoint
construction, two-return count, strict state, inverse sides, continuous-CFD
clock, durable attempt, equal-notional aggregate risk, spread caps, stops, and
lifecycle.
...
intervals, current-week leakage, opposed-sign, zero, equality, or acceleration
entry, wrong inverse side, duplicate attempt, one-leg survivor, aggregate-risk
breach, notional mismatch above 20 percent, missing hard stop, wrong next-week
exit, nondeterminism, or invalid fixed-risk mode.

Changing the carrier, endpoint count, weekly horizon, sign or deceleration
...
non-overlapping relative returns; both strict same-sign deceleration
directions; sign opposition, zero, equality, and acceleration flat states; no
...
| Q01 Build Validation | - | NOT RUN | pending build |
| Q02 Baseline Screening | - | NOT ENQUEUED | Q01 required |
