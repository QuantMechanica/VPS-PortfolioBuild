...
indicators:
  - rolling-zscore
  - atr-stop
target_symbols: [AUDUSD.DWX, GBPJPY.DWX]
logical_symbol: QM5_12776_AUDUSD_GBPJPY_COINTEGRATION_D1
period: D1
...
# Edge Lab AUDUSD/GBPJPY Cointegration Basket
...
QM5_12764, QM5_12765, QM5_12766, QM5_12768, QM5_12770, and QM5_12772 already
cover stronger exploratory baskets. This card is the next unbuilt rank-24 tail
candidate by OOS net Sharpe: AUDUSD/GBPJPY had DEV Sharpe 0.1257, OOS net
Sharpe -0.2072, OOS return -2.3438%, 19 OOS state changes, hedge 0.189701, and
117.50-day half-life in the same rerun.

## Concept

AUDUSD and GBPJPY are not a tight same-complex pair; the basket tests whether a
weak residual between an AUD/USD risk-currency expression and a sterling-yen
...
Temporary dislocations between the AUDUSD and GBPJPY D1 log-price spread can
...
- Basket legs: AUDUSD.DWX and GBPJPY.DWX.
...
- The EA trades only the fixed AUDUSD/GBPJPY basket; it does not reselect pairs
...
- Compute `spread = ln(AUDUSD) - 0.189701 * ln(GBPJPY)`.
- Compute a 60-bar rolling z-score of the spread.
- If no pair package is open and z > +2.0, open a short-spread package: short AUDUSD, long GBPJPY.
- If no pair package is open and z < -2.0, open a long-spread package: long AUDUSD, short GBPJPY.
...
- Host chart must be AUDUSD.DWX or GBPJPY.DWX on D1/H1, with slot 0 used for the logical host.
- The EA selects USDJPY.DWX as conversion history for USD-denominated GBPJPY accounting.
...
- name: strategy_beta
  default: 0.189701
  sweep_range: [0.10, 0.189701, 0.30]
...
No external performance claim is taken from Chan for AUDUSD/GBPJPY
specifically. The in-house scan rerun found DEV Sharpe 0.1257 and OOS net
Sharpe -0.2072 after cost, below zero and below the original 0.8 survivor
...
- risk_class: very high because this is a rank-24 negative-OOS tail candidate after the rank-23 GBPJPY/AUDJPY exploratory basket.
- gridding: false.
- scalping: false.
- ml_required: false.
...
- [x] R3 testable: AUDUSD.DWX and GBPJPY.DWX are Darwinex-native `.DWX` symbols in the exported scan data.
