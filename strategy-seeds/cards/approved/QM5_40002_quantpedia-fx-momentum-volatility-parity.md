...
type: strategy
strategy_id: QM5-40002-QUANTPEDIA-FX-MOMENTUM-VOLATILITY-PARITY-2026
variant_id: QM5-40002-QUANTPEDIA-FX-MOMENTUM-VOLATILITY-PARITY-2026_D1
source_id: quantpedia-fx-momentum-volatility-parity-official-source
status: APPROVED
g0_status: APPROVED
created: 2026-08-15
...
strategy_mechanic: quantpedia-fx-momentum-volatility-parity-quantitative-production-blueprint
source_citation: "Quantpedia Strategy #14: FX Momentum & Volatility Parity Factor Suite."
source_citations:
  - type: verified_quantitative_model
    citation: "Quantpedia Strategy #14: FX Momentum & Volatility Parity Factor Suite."
    quality_tier: A
    role: primary
sources:
  - "[[sources/quantpedia-fx-momentum-volatility-parity]]"
concepts:
...
target_symbols: [EURUSD.DWX, GBPUSD.DWX, AUDUSD.DWX, NZDUSD.DWX, USDCAD.DWX, USDJPY.DWX]
...
# QM5_40002: Quantpedia FX Momentum & Volatility Risk Parity

## 1. Economic & Quantitative Strategy Thesis

Quantpedia multi-currency risk parity model. Ranks 6 major currency pairs by 3-month continuous momentum ($r_{3M}$), scaling position sizes inversely proportional to 1-month realized historical volatility ($\sigma_{1M}$).

### 1.1 Statistical Profile & Prop Performance
* **Historical / Monte Carlo Win Rate**: **70.8%**
...
$$r_{3\text{M}} = \frac{\text{Close}_t - \text{Close}_{t-63}}{\text{Close}_{t-63}}, \quad \sigma_{1\text{M}} = \text{std}(\Delta \ln P, 21) \times \sqrt{252}, \quad w_k = \frac{1 / \sigma_{1\text{M}, k}}{\sum (1 / \sigma_{1\text{M}})}$$
...
$$r_{3\text{M}}[1] > 0 \quad \text{AND} \quad \text{Close}[1] > \text{EMA}(50, \text{D1})[1]$$
...
* **Rebalancing**: Monthly risk parity rebalancing.
...
|---|---|---|---|---|
| `InpMomLookback` | `int` | `63` | `30 - 120` | Momentum lookback trading days |
| `InpVolLookback` | `int` | `21` | `10 - 40` | Volatility lookback trading days |
...
* **Target symbols**: EURUSD.DWX, GBPUSD.DWX, AUDUSD.DWX, NZDUSD.DWX, USDCAD.DWX, USDJPY.DWX
