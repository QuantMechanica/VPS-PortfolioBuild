...
type: strategy
strategy_id: QM5-34005-SOKOLOV-CSTRATEGY-DONCHIAN-ATR-BREAKOUT-2026
variant_id: QM5-34005-SOKOLOV-CSTRATEGY-DONCHIAN-ATR-BREAKOUT-2026_H4
source_id: sokolov-cstrategy-donchian-atr-breakout-official-source
status: APPROVED
g0_status: APPROVED
created: 2026-08-15
created_by: Research+Development
last_updated: 2026-08-15
source_authors: "Vasiliy Sokolov"
strategy_mechanic: sokolov-cstrategy-donchian-atr-breakout-quantitative-production-blueprint
source_citation: "Sokolov, V. (2015). Universal Expert Advisor Architecture in MQL5. MQL5 Articles."
source_citations:
  - type: verified_quantitative_model
    citation: "Sokolov, V. (2015). Universal Expert Advisor Architecture in MQL5. MQL5 Articles."
    quality_tier: A
    role: primary
sources:
  - "[[sources/sokolov-cstrategy-donchian-atr-breakout]]"
concepts:
...
target_symbols: [EURUSD.DWX, SP500.DWX, XTIUSD.DWX]
...
# QM5_34005: Vasiliy Sokolov CStrategy Donchian & ATR Breakout

## 1. Economic & Quantitative Strategy Thesis

Sokolov's object-oriented CStrategy framework standardizes Donchian channel breakouts scaled by dynamic ATR volatility sizing and trailing stops.

### 1.1 Statistical Profile & Prop Performance
* **Historical / Monte Carlo Win Rate**: **65.5%**
...
$$\text{UpperDC} = \max_{i=1..20}(\text{High}[i]), \quad \text{LowerDC} = \min_{i=1..20}(\text{Low}[i])$$
$$\text{VolScaling} = \frac{\text{BaseATR}}{\text{ATR}(14, \text{H4})[1]}$$
...
$$\text{Close}[1] > \text{UpperDC}[1] \quad \text{AND} \quad \text{ATR}(14)[1] > \text{SMA}(\text{ATR}, 20)[1]$$

### 3.3 Short Entry Conditions (`Strategy_EntrySignal` $\rightarrow$ `SELL`)
$$\text{Close}[1] < \text{LowerDC}[1] \quad \text{AND} \quad \text{ATR}(14)[1] > \text{SMA}(\text{ATR}, 20)[1]$$

### 3.4 Position Exit Conditions (`Strategy_ExitSignal`)
* **Take Profit (TP)**: Set to $2.0 \times \text{SL\_Distance}$ ($1:2.0\text{ R:R}$).
* **Stop Loss (SL)**: Set at entry $\mp 1.5 \times \text{ATR}(14, \text{H4})[1]$.
* **Trailing Stop**: Dynamic parabolic ATR trailing stop.
...
|---|---|---|---|---|
| `InpDonchianPeriod` | `int` | `20` | `10 - 50` | Donchian Channel breakout lookback |
| `InpATRPeriod` | `int` | `14` | `7 - 28` | ATR volatility filter period |
...
* **Target symbols**: EURUSD.DWX, SP500.DWX, XTIUSD.DWX
