...
type: strategy
strategy_id: QM5-30003-MULTISYSTEM-GOLD-SCALPER-FOREX-GOLD-INVESTOR-2026
variant_id: QM5-30003-MULTISYSTEM-GOLD-SCALPER-FOREX-GOLD-INVESTOR-2026_M15
source_id: multisystem-gold-scalper-forex-gold-investor-official-source
status: APPROVED
g0_status: APPROVED
created: 2026-08-15
created_by: Research+Development
last_updated: 2026-08-15
source_authors: "Forex Gold Investor Development Team"
strategy_mechanic: multisystem-gold-scalper-forex-gold-investor-quantitative-production-blueprint
source_citation: "Forex Gold Investor Live Performance. FXBlue & Myfxbook Audited."
source_citations:
  - type: verified_quantitative_model
    citation: "Forex Gold Investor Live Performance. FXBlue & Myfxbook Audited."
    quality_tier: A
    role: primary
sources:
  - "[[sources/multisystem-gold-scalper-forex-gold-investor]]"
concepts:
...
# QM5_30003: Multi-System Intraday Gold Scalper (Forex Gold Investor)
...
* **Expected Profit Factor (PF)**: **1.55**
...
$$\text{Module 1: LinReg Channel Fade, Module 2: Session Range Break, Module 3: 4H Momentum Surge}$$
...
$$\text{Any Module Long Trigger == TRUE} \quad \text{AND} \quad \text{Current Open Positions} == 0$$

### 3.3 Short Entry Conditions (`Strategy_EntrySignal` $\rightarrow$ `SELL`)
$$\text{Any Module Short Trigger == TRUE} \quad \text{AND} \quad \text{Current Open Positions} == 0$$

### 3.4 Position Exit Conditions (`Strategy_ExitSignal`)
* **Take Profit (TP)**: Fixed $+30.0\text{ pips}$ (Hidden on client side).
* **Stop Loss (SL)**: Hard $-25.0\text{ pips}$ (Hidden on client side).
* **Stale Exit**: Force close if position is open for more than 12 hours.
...
|---|---|---|---|---|
| `InpEnableMod1` | `bool` | `true` | `true/false` | Enable Linear Regression Channel module |
| `InpEnableMod2` | `bool` | `true` | `true/false` | Enable Session Breakout module |
| `InpMaxHoldHours` | `int` | `12` | `4 - 24` | Stale position maximum hold time in hours |
