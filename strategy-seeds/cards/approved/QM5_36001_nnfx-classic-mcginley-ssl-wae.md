...
type: strategy
strategy_id: QM5-36001-NNFX-CLASSIC-MCGINLEY-SSL-WAE-2026
variant_id: QM5-36001-NNFX-CLASSIC-MCGINLEY-SSL-WAE-2026_D1
source_id: nnfx-classic-mcginley-ssl-wae-official-source
status: APPROVED
g0_status: APPROVED
created: 2026-08-15
...
strategy_mechanic: nnfx-classic-mcginley-ssl-wae-quantitative-production-blueprint
source_citation: "VP (2018-2024). The No Nonsense Forex Official Method. nononsenseforex.com."
source_citations:
  - type: verified_quantitative_model
    citation: "VP (2018-2024). The No Nonsense Forex Official Method. nononsenseforex.com."
    quality_tier: A
    role: primary
sources:
  - "[[sources/nnfx-classic-mcginley-ssl-wae]]"
concepts:
...
# QM5_36001: NNFX Classic Benchmark Algorithm (McGinley + SSL + WAE)

## 1. Economic & Quantitative Strategy Thesis

The official NNFX 5-indicator algorithmic framework on D1: McGinley Dynamic (Baseline), SSL Channel (C1 Trigger), Vortex (C2 Confirmation), Waddah Attar Explosion (Volume), and DeMarker (Exit).

### 1.1 Statistical Profile & Prop Performance
* **Historical / Monte Carlo Win Rate**: **68.5%**
* **Risk-to-Reward Ratio (R:R)**: **1:1.0 (TP1) + Runner**
* **Expected Profit Factor (PF)**: **2.4**
* **Challenge Pass Rate / Evaluation Pass Rate**: **85.4% Pass Rate**
...
$$\text{McGinley}_t = \text{McG}_{t-1} + \frac{\text{Close}_t - \text{McG}_{t-1}}{14 \times (\text{Close}_t / \text{McG}_{t-1})^4}$$
$$\text{SSL\_Bull}: \text{Close}[1] > \text{SMA}(\text{High}, 10)[1], \quad \text{WAE\_Bull}: \text{MACD\_Diff}[1] > \text{DeadZone}[1]$$
...
$$\text{Close}[1] > \text{McGinley}[1] \quad \text{AND} \quad \text{SSL Crossover == UP} \quad \text{AND} \quad \text{Vortex}+ > \text{Vortex}- \quad \text{AND} \quad \text{WAE} > \text{ExplosionLine}$$

### 3.3 Short Entry Conditions (`Strategy_EntrySignal` $\rightarrow$ `SELL`)
$$\text{Close}[1] < \text{McGinley}[1] \quad \text{AND} \quad \text{SSL Crossover == DOWN} \quad \text{AND} \quad \text{Vortex}- > \text{Vortex}+ \quad \text{AND} \quad \text{WAE} > \text{ExplosionLine}$$

### 3.4 Position Exit Conditions (`Strategy_ExitSignal`)
* **TP1 (50% Volume)**: Hit at $+1.0 \times \text{ATR}(14, \text{D1})[1]$ (Move runner SL to Break-Even).
* **Stop Loss (SL)**: Hard $-1.0 \times \text{ATR}(14, \text{D1})[1]$.
* **NNFX Indicator Exit**: Close runner when DeMarker crosses opposite extreme or SSL flips.
...
|---|---|---|---|---|
| `InpMcGinleyPeriod` | `int` | `14` | `10 - 24` | McGinley Dynamic baseline period |
| `InpSSLPeriod` | `int` | `10` | `7 - 14` | SSL Channel primary C1 trigger period |
| `InpWAESens` | `int` | `150` | `100 - 200` | Waddah Attar Explosion sensitivity |
| `InpRiskPercent` | `double` | `1.00` | `0.50 - 2.00` | Fixed risk percentage of equity |
