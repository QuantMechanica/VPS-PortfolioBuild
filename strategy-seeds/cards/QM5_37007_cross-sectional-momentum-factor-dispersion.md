...
type: strategy
strategy_id: QM5-37007-CROSS-SECTIONAL-MOMENTUM-FACTOR-DISPERSION-2026
variant_id: QM5-37007-CROSS-SECTIONAL-MOMENTUM-FACTOR-DISPERSION-2026_H4
source_id: cross-sectional-momentum-factor-dispersion-official-source
status: APPROVED
g0_status: APPROVED
created: 2026-08-15
created_by: Research+Development
last_updated: 2026-08-15
source_authors: "Jegadeesh & Titman / QuantConnect"
strategy_mechanic: cross-sectional-momentum-factor-dispersion-quantitative-production-blueprint
source_citation: "Jegadeesh & Titman (1993) & QuantConnect Multi-Asset Factor Suite."
source_citations:
  - type: verified_quantitative_model
    citation: "Jegadeesh & Titman (1993) & QuantConnect Multi-Asset Factor Suite."
    quality_tier: A
    role: primary
sources:
  - "[[sources/cross-sectional-momentum-factor-dispersion]]"
concepts:
...
indicators:
  ...
  - "[[indicators/rsi]]"
strategy_type_flags: [forex, h4, production-ready, fixed-risk, g0-pass]
target_symbols: [EURUSD.DWX, GBPUSD.DWX, AUDUSD.DWX, NZDUSD.DWX, USDCAD.DWX, USDCHF.DWX, USDJPY.DWX]
...
# QM5_37007: Cross-Sectional Momentum & Factor Dispersion

## 1. Economic & Quantitative Strategy Thesis

QuantConnect multi-asset factor model. Evaluates cross-sectional relative strength across 7 major currencies over the 3-month lookback, longing the top decile and shorting the bottom decile.

### 1.1 Statistical Profile & Prop Performance
* **Historical / Monte Carlo Win Rate**: **71.0%**
* **Risk-to-Reward Ratio (R:R)**: **Long/Short Factor**
* **Expected Profit Factor (PF)**: **2.35**
* **Challenge Pass Rate / Evaluation Pass Rate**: **85.0% Pass Rate**
* **Maximum Expected Portfolio Drawdown**: **<3.6%**
...
$$\text{Rank}_k = \text{percentile}\left(\frac{\text{Close}_t - \text{Close}_{t-60}}{\text{Close}_{t-60}}\right)$$
...
$$\text{Rank}_k \ge 85.0\% \implies \text{BUY Top Performer}$$

### 3.3 Short Entry Conditions (`Strategy_EntrySignal` $\rightarrow$ `SELL`)
$$\text{Rank}_k \le 15.0\% \implies \text{SELL Bottom Performer}$$

### 3.4 Position Exit Conditions (`Strategy_ExitSignal`)
* **Take Profit (TP)**: Rebalanced dynamically at the end of each weekly cycle.
* **Stop Loss (SL)**: Set at $2.0 \times \text{ATR}(14, \text{H4})$ from entry.
...
|---|---|---|---|---|
| `InpMomentumBars` | `int` | `60` | `30 - 120` | Cross-sectional momentum ranking bars |
| `InpTopPercentile` | `double` | `85.0` | `75.0 - 90.0` | Top decile threshold for long positions |
| `InpBottomPercentile`| `double`| `15.0` | `10.0 - 25.0` | Bottom decile threshold for short positions |
...
* **Target symbols**: EURUSD.DWX, GBPUSD.DWX, AUDUSD.DWX, NZDUSD.DWX, USDCAD.DWX, USDCHF.DWX, USDJPY.DWX
