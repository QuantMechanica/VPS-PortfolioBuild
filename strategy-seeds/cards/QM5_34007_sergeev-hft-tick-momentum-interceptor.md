...
ea_id: QM5_34007
slug: sergeev-hft-tick-momentum-interceptor
type: strategy
strategy_id: QM5-34007-SERGEEV-HFT-TICK-MOMENTUM-INTERCEPTOR-2026
variant_id: QM5-34007-SERGEEV-HFT-TICK-MOMENTUM-INTERCEPTOR-2026_M1
source_id: sergeev-hft-tick-momentum-interceptor-official-source
status: APPROVED
g0_status: APPROVED
created: 2026-08-15
created_by: Research+Development
last_updated: 2026-08-15
source_authors: "Alexey Sergeev"
strategy_mechanic: sergeev-hft-tick-momentum-interceptor-quantitative-production-blueprint
source_citation: "Sergeev, A. (2012). High-Speed Tick Microstructure Algorithms. MQL5 Championship Archive."
source_citations:
  - type: verified_quantitative_model
    citation: "Sergeev, A. (2012). High-Speed Tick Microstructure Algorithms. MQL5 Championship Archive."
    quality_tier: A
    role: primary
sources:
  - "[[sources/sergeev-hft-tick-momentum-interceptor]]"
concepts:
...
indicators:
  ...
  - "[[indicators/rsi]]"
strategy_type_flags: [forex, m1, production-ready, fixed-risk, g0-pass]
...
# QM5_34007: Alexey Sergeev Tick Momentum Interceptor

## 1. Economic & Quantitative Strategy Thesis

Exploits microsecond institutional tick orderflow imbalances on M1 with an strict spread-gate filter, capturing fast 6-pip micro-momentum thrusts.

### 1.1 Statistical Profile & Prop Performance
* **Historical / Monte Carlo Win Rate**: **77.5%**
* **Risk-to-Reward Ratio (R:R)**: **1:1.2**
...
$$\text{TickBurst} = \frac{\text{Ticks}_{10\text{s}}}{\text{AvgTicks}}, \quad \text{SpreadGate}: \text{Spread} \le 0.8\text{ pips}$$
...
$$\text{SpreadGate == TRUE} \quad \text{AND} \quad \text{TickBurst} \ge 3.0 \quad \text{AND} \quad \Delta P_{5\text{s}} > +4.0\text{ pts}$$

### 3.3 Short Entry Conditions (`Strategy_EntrySignal` $\rightarrow$ `SELL`)
$$\text{SpreadGate == TRUE} \quad \text{AND} \quad \text{TickBurst} \ge 3.0 \quad \text{AND} \quad \Delta P_{5\text{s}} < -4.0\text{ pts}$$

### 3.4 Position Exit Conditions (`Strategy_ExitSignal`)
* **Take Profit (TP)**: Fixed $+6.0\text{ pips}$.
* **Stop Loss (SL)**: Hard $-5.0\text{ pips}$.
* **Micro-BE Lock**: Move SL to $+1\text{ pip}$ at $+3.5\text{ pips}$.
...
|---|---|---|---|---|
| `InpMaxSpreadPips` | `double` | `0.8` | `0.4 - 1.2` | Maximum allowable spread for execution |
| `InpTickBurstFactor`| `double` | `3.0` | `2.0 - 5.0` | Tick density surge multiplier |
| `InpTPPoints` | `int` | `60` | `40 - 100` | Take profit in points (6 pips) |
| `InpSLPoints` | `int` | `50` | `30 - 80` | Stop loss in points (5 pips) |
