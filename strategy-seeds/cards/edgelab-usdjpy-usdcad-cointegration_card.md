---
ea_id: QM5_12762
slug: edgelab-usdjpy-usdcad-cointegration
type: strategy
...
indicators:
  - rolling-zscore
  - atr-stop
target_symbols: [USDJPY.DWX, USDCAD.DWX]
...
g0_approval_reasoning: "R1 PASS because the method comes from Chan cointegration pair-trading and pair selection comes from the OWNER-requested in-house 66-pair scan; R2 PASS deterministic fixed-pair z-score basket; R3 PASS USDJPY.DWX and USDCAD.DWX data exist in the exported scan universe; R4 PASS no ML/grid/martingale. Marked high-risk because DEV Sharpe was negative and OOS Sharpe was very low."
expected_pf: 1.01
expected_dd_pct: 30.0
portfolio_scope: basket
---

# Edge Lab USDJPY/USDCAD Cointegration Basket
...
QM5_12747, QM5_12749, QM5_12751, QM5_12756, QM5_12758, and QM5_12760 already cover later
exploratory baskets. This card is the strongest remaining unbuilt OOS-positive
candidate by OOS net Sharpe: USDJPY/USDCAD had DEV Sharpe -0.12, OOS net
Sharpe 0.10, OOS return +1.20%, 15 OOS state changes, hedge 0.196, and 381-day
half-life in the same rerun.

## Concept

USDJPY and USDCAD are both USD-base pairs but express different quote-side macro
drivers: yen funding/risk-off pressure versus CAD commodity and North-American
rate pressure. This is a low-conviction macro relative-value residual rather
than a strong same-complex pair like EURJPY/GBPJPY. The weak scan result
suggests occasional mean reversion between those USD-base expressions, but the
...
- Basket legs: USDJPY.DWX and USDCAD.DWX.
...
- Compute `spread = ln(USDJPY) - 0.196 * ln(USDCAD)`.
- Compute a 60-bar rolling z-score of the spread.
- If no pair package is open and z > +2.0, open a short-spread package: short USDJPY, long USDCAD.
- If no pair package is open and z < -2.0, open a long-spread package: long USDJPY, short USDCAD.
...
- Host chart must be USDJPY.DWX or USDCAD.DWX on D1/H1, with slot 0 used for the logical host.
...
No external performance claim is taken from Chan for USDJPY/USDCAD specifically.
The in-house scan rerun found DEV Sharpe -0.12 and OOS net Sharpe 0.10 after
...
- [x] R3 testable: USDJPY.DWX and USDCAD.DWX are Darwinex-native `.DWX` symbols in the exported scan data.
...
| v1-q02 | 2026-06-29 | build passed and logical-basket Q02 enqueued | Q02 | PENDING work item ec04e440-5ee4-440e-b6bc-d78898d233ee |
