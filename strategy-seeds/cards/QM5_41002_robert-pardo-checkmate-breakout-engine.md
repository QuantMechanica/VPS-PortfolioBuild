...
type: strategy
strategy_id: QM5-41002-ROBERT-PARDO-CHECKMATE-BREAKOUT-ENGINE-2026
variant_id: QM5-41002-ROBERT-PARDO-CHECKMATE-BREAKOUT-ENGINE-2026_H4
source_id: robert-pardo-checkmate-breakout-engine-official-source
status: APPROVED
g0_status: APPROVED
created: 2026-08-15
created_by: Research+Development
last_updated: 2026-08-15
source_authors: "Robert Pardo"
strategy_mechanic: robert-pardo-checkmate-breakout-engine-quantitative-production-blueprint
source_citation: "Pardo, R. (2008). The Evaluation and Optimization of Trading Strategies. John Wiley & Sons."
source_citations:
  - type: verified_quantitative_model
    citation: "Pardo, R. (2008). The Evaluation and Optimization of Trading Strategies. John Wiley & Sons."
    quality_tier: A
    role: primary
sources:
  - "[[sources/robert-pardo-checkmate-breakout-engine]]"
concepts:
...
# QM5_41002: Robert Pardo Checkmate Currency Breakout Engine

## 1. Economic & Quantitative Strategy Thesis

Walk-forward optimized breakout system from Pardo Applied Research. Enters on expansion beyond the 10-bar Donchian channel filtered by an expanding ATR volatility slope.
...
$$\text{DonchianUpper} = \max_{i=1..10}(\text{High}[i]), \quad \text{ATRSlope} = \text{ATR}(14, \text{H4})[1] - \text{ATR}(14, \text{H4})[5]$$
...
$$\text{Close}[1] > \text{DonchianUpper}[1] \quad \text{AND} \quad \text{ATRSlope} > 0$$

### 3.3 Short Entry Conditions (`Strategy_EntrySignal` $\rightarrow$ `SELL`)
$$\text{Close}[1] < \min_{1..10}(\text{Low})[1] \quad \text{AND} \quad \text{ATRSlope} > 0$$
...
* **Trailing Stop**: Trailed by the opposite 10-bar Donchian boundary.
...
|---|---|---|---|---|
| `InpDonchianBars` | `int` | `10` | `6 - 20` | Donchian channel lookback bars |
| `InpATRPeriod` | `int` | `14` | `8 - 21` | ATR volatility lookback |
