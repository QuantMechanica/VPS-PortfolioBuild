...
type: strategy
strategy_id: QM5-41003-KAUFMAN-READY-SET-GO-MOMENTUM-2026
variant_id: QM5-41003-KAUFMAN-READY-SET-GO-MOMENTUM-2026_H1
source_id: kaufman-ready-set-go-momentum-official-source
status: APPROVED
g0_status: APPROVED
created: 2026-08-15
created_by: Research+Development
last_updated: 2026-08-15
source_authors: "Perry Kaufman"
strategy_mechanic: kaufman-ready-set-go-momentum-quantitative-production-blueprint
source_citation: "Kaufman, P. J. (2013). Trading Systems and Methods, 5th Edition. John Wiley & Sons."
source_citations:
  - type: verified_quantitative_model
    citation: "Kaufman, P. J. (2013). Trading Systems and Methods, 5th Edition. John Wiley & Sons."
    quality_tier: A
    role: primary
sources:
  - "[[sources/kaufman-ready-set-go-momentum]]"
concepts:
...
target_symbols: [SP500.DWX, NDX.DWX, GDAXI.DWX]
...
# QM5_41003: Perry Kaufman Ready-Set-Go Momentum Acceleration

## 1. Economic & Quantitative Strategy Thesis

Kaufman's 3-stage momentum filter: Stage 1 'Ready' (Volatility compression $\text{ATR} < \text{SMA}(\text{ATR}, 20)$), Stage 2 'Set' (Price above 50 EMA), Stage 3 'Go' (Momentum acceleration breakout).

### 1.1 Statistical Profile & Prop Performance
* **Historical / Monte Carlo Win Rate**: **72.0%**
...
$$\text{Ready}: \text{ATR}(10)[1] < \text{SMA}(\text{ATR}, 30)[1], \quad \text{Set}: \text{Close}[1] > \text{EMA}(50)[1]$$
$$\text{Go}: \text{Close}[1] - \text{Close}[5] > 1.5 \times \text{ATR}(10)[1]$$
...
$$\text{Ready == TRUE} \quad \text{AND} \quad \text{Set == TRUE} \quad \text{AND} \quad \text{Go == TRUE}$$

### 3.3 Short Entry Conditions (`Strategy_EntrySignal` $\rightarrow$ `SELL`)
$$\text{Ready == TRUE} \quad \text{AND} \quad \text{Close}[1] < \text{EMA}(50)[1] \quad \text{AND} \quad \text{Close}[5] - \text{Close}[1] > 1.5\text{ATR}$$

### 3.4 Position Exit Conditions (`Strategy_ExitSignal`)
* **Take Profit (TP)**: Set to $2.0 \times \text{SL\_Distance}$ ($1:2.0\text{ R:R}$).
* **Stop Loss (SL)**: Set at entry $\mp 1.5 \times \text{ATR}(10, \text{H1})[1]$.
...
|---|---|---|---|---|
| `InpFastATR` | `int` | `10` | `5 - 15` | Fast ATR compression period |
| `InpSlowATR` | `int` | `30` | `20 - 50` | Slow baseline ATR period |
| `InpTrendEMA` | `int` | `50` | `30 - 80` | Directional trend EMA |
...
* **Target symbols**: SP500.DWX, NDX.DWX, GDAXI.DWX
