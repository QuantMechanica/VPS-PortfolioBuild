...
type: strategy
strategy_id: MACROTRENDS-SILVER-OIL-RATIO-2026_BRENT_S04
...
indicators:
  ...
  - "[[indicators/atr]]"
strategy_type_flags: [market-neutral-basket, oil-silver-relative-value, brent-silver-relative-value, return-spread-zscore, mean-reversion-exit, atr-hard-stop, time-stop, low-frequency, energy]
target_symbols: [XBRUSD.DWX, XAGUSD.DWX]
primary_target_symbols: [XBRUSD.DWX, XAGUSD.DWX]
markets: [XBRUSD.DWX, XAGUSD.DWX]
timeframes: [D1]
logical_symbol: QM5_13053_XBR_XAG_RSPREAD_D1
single_symbol_only: false
period: D1
expected_trade_frequency: "Low-frequency D1 XBR/XAG return-spread z-score reversion; estimate 6-14 paired packages/year before Q02 validates history and fills."
...
hard_rules_at_risk: [basket_execution, friday_close, magic_schema, risk_mode_dual, xbr_history_sufficiency]
g0_approval_reasoning: "Mission-directed G0 approval 2026-07-08: R1 PASS existing approved Macrotrends oil/silver ratio source packet plus Chan/Wiley pair-spread implementation lineage; R2 PASS deterministic D1 Brent-minus-silver return-spread z-score entry, mean exit, max-hold exit, spread caps, and ATR hard stops; R3 PASS XBRUSD.DWX local Brent route and XAGUSD.DWX in DWX matrix, with Q02 validating synchronized history sufficiency; R4 PASS no ML/grid/martingale/external runtime feed. Non-duplicate because this is Brent/silver return-spread reversion, not WTI/silver price-ratio or return-spread, Brent/gold, XBR/XNG, XTI/XNG, Brent/WTI, XAU/XAG, directional Brent/WTI/XNG/XAU, calendar/event, or commodity RSI logic."
---

# Brent/Silver Return-Spread Reversion
...
that exposure market-neutral and deliberately avoids another absolute
oil/silver price-ratio sleeve. It measures short fixed-window relative return
divergence:

`return_spread = ln(XBR_t / XBR_t-N) - beta * ln(XAG_t / XAG_t-N)`

The current return spread is standardized against recent completed-D1 history.
If Brent has overrun silver over the return window, the basket sells Brent and
buys silver. If silver has overrun Brent, the basket buys Brent and sells
silver. The thesis is short-horizon relative-return snapback inside a structural
Brent/silver pair, not outright Brent trend, not outright metal exposure, and
not a WTI duplicate.

This is deliberately different from:

- `QM5_12864_oilsilver-rspr`: that EA uses WTI as the oil leg. This card uses
  Brent as the crude benchmark leg and will be validated as its own logical
  basket.
- `QM5_12606_oil-silver-ratio`: that EA fades the absolute XTI/XAG log price
  ratio level. This card fades a short-window Brent-minus-silver return-spread
  shock.
...
- `QM5_12867_brentgold-rspread`: that card trades Brent versus gold. This card
  trades Brent versus silver, adding industrial-metal behavior.
- `QM5_12862_xauxag-rspread`: that card is intra-metals. This card introduces
  energy versus silver relative value.
- Directional WTI, Brent, XNG, XAU, index, calendar, event, inventory, roll,
  and `QM5_12567_cum-rsi2-commodity` sleeves: every entry is a paired package
  and uses no RSI, oscillator pullback, ML, grid, martingale, or external feed.

## Markets And Timeframe

- Logical symbol: `QM5_13053_XBR_XAG_RSPREAD_D1`.
...
- Compute the latest `strategy_return_lookback_d1` D1 log return for
  `XBRUSD.DWX` and `XAGUSD.DWX`.
- Compute `return_spread = return_XBR - beta * return_XAG`.
- Standardize the return spread using the last `strategy_z_lookback_d1`
  completed return-spread observations.
- If z-score is above `strategy_entry_z`, Brent has outperformed silver
  sharply: sell `XBRUSD.DWX` and buy `XAGUSD.DWX`.
- If z-score is below negative `strategy_entry_z`, silver has outperformed
  Brent sharply: buy `XBRUSD.DWX` and sell `XAGUSD.DWX`.
...
- Two-leg basket only.
- Symmetric long/short Brent/silver relative-value package.
- No pyramiding.
- No gridding.
- No martingale.
- No partial close.
...
reject the mechanical rule on Darwinex `XBRUSD.DWX` and `XAGUSD.DWX` bars.
...
- risk_class: high because Brent gaps, silver volatility, and basket execution
...
- [x] R1 reputable source: existing approved Macrotrends oil/silver ratio
  source packet plus Chan/Wiley pair-spread implementation lineage.
- [x] R2 mechanical: fixed D1 return-spread z-score entry, normalization exit,
  time stop, spread caps, and ATR hard stops.
- [x] R3 testable: `XBRUSD.DWX` and `XAGUSD.DWX` exist in the DWX symbol
  universe and require OHLC only. Q02 validates synchronized history.
- [x] R4 compliant: no ML, adaptive PnL fitting, grid, martingale, external
  runtime feed, or more than one package per magic.
- [x] Non-duplicate: this is XBR/XAG return-spread reversion, not XTI/XAG
  price-ratio level reversion, XTI/XAG return-spread, oil/silver ratio
  breakout, Brent/gold return-spread, XAU/XAG return-spread, commodity RSI, or
...
- trade_entry: D1 standardized XBR/XAG return-spread reversion.
...
|---|---|---|---|
| v1 | 2026-07-08 | initial XBR/XAG return-spread basket build | Q02 | ENQUEUED |
...
| Q01 Build Validation | 2026-07-08 | PASS | `artifacts/qm5_13053_build_result.json` |
| Q02 Baseline Screening | 2026-07-08 | QUEUED | paced farm work item recorded in build result |
