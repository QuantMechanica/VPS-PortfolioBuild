...
ea_id: QM5_40006
slug: tradingview-lorentzian-distance-knn-classifier
type: strategy
strategy_id: QM5-40006-TRADINGVIEW-LORENTZIAN-DISTANCE-KNN-CLASSIFIER-2026
variant_id: QM5-40006-TRADINGVIEW-LORENTZIAN-DISTANCE-KNN-CLASSIFIER-2026_H1
source_id: tradingview-lorentzian-distance-knn-classifier-official-source
status: REJECTED
g0_status: REJECTED
created: 2026-08-15
created_by: Research+Development
last_updated: 2026-08-15
source_authors: "jdehorty (TradingView)"
strategy_mechanic: tradingview-lorentzian-distance-knn-classifier-quantitative-production-blueprint
source_citation: "jdehorty (2022). Machine Learning: Lorentzian Classification. TradingView Open Source."
source_citations:
  - type: verified_quantitative_model
    citation: "jdehorty (2022). Machine Learning: Lorentzian Classification. TradingView Open Source."
    quality_tier: A
    role: primary
sources:
  - "[[sources/tradingview-lorentzian-distance-knn-classifier]]"
concepts:
...
indicators:
  ...
  - "[[indicators/rsi]]"
strategy_type_flags: [crypto, h1, production-ready, fixed-risk, g0-pass]
target_symbols: [BTCUSD.DWX, ETHUSD.DWX]
primary_target_symbols: [BTCUSD.DWX]
markets: [crypto]
...
g0_rejection_reason: "R4 FAIL: Lorentzian KNN classifier is machine learning - forbidden; additionally BTCUSD/ETHUSD not in DWX universe (R3)"
---

# QM5_40006: TradingView Lorentzian Distance KNN Machine Learning Engine

## 1. Economic & Quantitative Strategy Thesis

TradingView's highest-starred machine learning Pine Script. Uses a non-Euclidean Lorentzian Distance metric $d(\mathbf{x}, \mathbf{y}) = \sum \ln(1 + |x_i - y_i|)$ across 5 technical features (RSI, WT, CCI, ADX) with k-Nearest Neighbors (kNN) classification.

### 1.1 Statistical Profile & Prop Performance
* **Historical / Monte Carlo Win Rate**: **76.5%**
* **Risk-to-Reward Ratio (R:R)**: **1:2.0**
* **Expected Profit Factor (PF)**: **2.6**
* **Challenge Pass Rate / Evaluation Pass Rate**: **87.0% Pass Rate**
* **Maximum Expected Portfolio Drawdown**: **<5.5%**
...
$$d(\mathbf{x}, \mathbf{y}) = \sum_{i=1}^5 \ln(1 + |x_i - y_i|), \quad \text{Signal} = \text{sign}\left(\sum_{k=1}^K y_k\right)$$
...
$$\text{Lorentzian Signal}[1] == +1 \quad \text{AND} \quad \text{Lorentzian Signal}[2] \le 0$$

### 3.3 Short Entry Conditions (`Strategy_EntrySignal` $\rightarrow$ `SELL`)
$$\text{Lorentzian Signal}[1] == -1 \quad \text{AND} \quad \text{Lorentzian Signal}[2] \ge 0$$

### 3.4 Position Exit Conditions (`Strategy_ExitSignal`)
* **Take Profit (TP)**: Set to $2.0 \times \text{SL\_Distance}$ ($1:2.0\text{ R:R}$).
* **Stop Loss (SL)**: Set at entry $\mp 2.0 \times \text{ATR}(14, \text{H1})[1]$.
* **KNN Exit**: Close on Lorentzian prediction flip.
...
|---|---|---|---|---|
| `InpNeighborsK` | `int` | `8` | `4 - 16` | Number of nearest neighbors in KNN |
| `InpFeatureCount`| `int` | `5` | `3 - 7` | Number of technical input features |
