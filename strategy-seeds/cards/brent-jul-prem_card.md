...
logical_symbol: QM5_13052_XBR_JUL_PREM_D1
period: D1
timeframes: [D1]
expected_trade_frequency: "July-only D1 Brent source-window seasonal sleeve; estimate 18-23 entries/year after weekends, broker holidays, and framework filters."
...
g0_approval_reasoning: "Mission-directed G0 approval 2026-07-08: R1 PASS peer-reviewed oil-seasonality paper covering Brent and WTI; R2 PASS deterministic July Brent D1 long/time-flat rule with ATR hard stop; R3 PASS XBRUSD.DWX local Brent route used by prior builds, with Q02 validating current history sufficiency; R4 PASS no ML/grid/martingale/external runtime data. Non-duplicate because this isolates daily Brent July exposure inside the source-defined February-September window, not WTI July, broad Brent February-September first-month-bar exposure, Brent September terminal exposure, Brent weak-month fades, Brent weekday effects, WTI event/calendar, XTI/XNG, XNG, XAU/XAG, gas-metal, trend, carry, or commodity RSI logic."
---

# Brent July Calendar Premium
...
seasonal allocation window. This card isolates the mid-window July segment on
the Brent benchmark as a separate Darwinex-native energy sleeve: long-only
`XBRUSD.DWX` exposure during broker-calendar July D1 bars, flattened on the
...
- `QM5_12962_wti-jul-prem`: same month concept on WTI, not Brent.
- `QM5_12981_brent-febsep-prem`: broad February-September Brent source window
  using only the first tradable D1 bar of each month, not daily July bars.
- `QM5_12982_brent-sep-prem`: terminal September segment, not mid-window July.
...
July sits inside the source-defined February-September crude-oil allocation
window. Testing Brent July separately from WTI and from the broad first-day
...
- Direction: long only.
- Entry gate: broker-calendar D1 bar is in July and no position exists for this
  EA magic.
- Exit gate: first new D1 bar after entry, outside July, max-hold expiry,
...
- [x] Non-duplicate: Brent July exposure is not WTI July, not the broad Brent
...
|---|---|---|---|
| v1 | 2026-07-08 | initial structural Brent July calendar-premium build | Q02 | QUEUED |
