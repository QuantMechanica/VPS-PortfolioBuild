---
strategy_id: AI-CLAUDE-FX-COINT66-20260609-GBPUSD-USDCHF
ea_id: QM5_20183
...
markets: [forex]
timeframes: [D1]
primary_target_symbols: [GBPUSD.DWX, USDCHF.DWX]
target_symbols: [GBPUSD.DWX, USDCHF.DWX]
logical_symbol: QM5_20183_GBPUSD_USDCHF_COINTEGRATION_D1
period: D1
expected_trade_frequency: "Approximately 5-7 two-leg state changes per year at basket level; Q02 must retire the sleeve if realized frequency is below the binding floor."
...
# QM5_20183 GBPUSD/USDCHF D1 Cointegration Basket
...
The sign-aware rerun ranks GBPUSD/USDCHF seventh of 66 pairs by OOS net Sharpe
...
The negative DEV Sharpe is adverse evidence, not something to optimize away.
It makes this a regime-unstable frontier experiment with a strict no-rescue
kill criterion.

## Concept

GBPUSD and USDCHF are liquid USD majors with opposite USD quote orientation.
For a negative fitted beta, a long spread buys both pairs: the short-USD
exposure in GBPUSD is partly offset by the long-USD exposure in USDCHF. The
...
`ln(GBPUSD) - (-0.651268047) * ln(USDCHF)` can mean-revert because both legs
load on broad USD pressure while retaining GBP and CHF residual risk. The OOS
scan result supports a low-frequency test, while the negative DEV result says
the relationship may be regime-specific. Q02 onward is the judge; no adaptive
...
The host chart is `GBPUSD.DWX` D1. `USDCHF.DWX` is the second traded leg and
...
- Compute `spread = ln(GBPUSD) - strategy_beta * ln(USDCHF)`, with
  `strategy_beta = -0.651268047`.
...
- Permit execution only when the chart symbol is `GBPUSD.DWX` or
...
- name: strategy_beta
  default: -0.651268047
  sweep_range: [-0.75, -0.651268047, -0.55]
...
Chan does not make a performance claim for GBPUSD/USDCHF. His source supplies
...
expected_pf: 1.05
expected_dd_pct: 30.0
expected_trade_frequency: "5-7 two-leg state changes/year at basket level"
risk_class: high
gridding: false
scalping: false
ml_required: false
...
| R3 | PASS | `GBPUSD.DWX` and `USDCHF.DWX` are in the exported 66-pair D1 universe and are native factory symbols. |
...
```yaml
target_modules:
  no_trade: "Select and warm GBPUSD.DWX/USDCHF.DWX; reject wrong host, timeframe, slot, or unaligned history."
