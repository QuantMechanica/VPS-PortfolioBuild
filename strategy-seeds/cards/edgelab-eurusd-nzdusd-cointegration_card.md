...
indicators:
  - rolling-zscore
  - atr-stop
target_symbols: [EURUSD.DWX, NZDUSD.DWX]
logical_symbol: QM5_12735_EURUSD_NZDUSD_COINTEGRATION_D1
period: D1
...
EURUSD and NZDUSD are liquid USD-denominated major pairs. Temporary divergence
between the euro-USD and New Zealand dollar-USD legs can mean-revert, but the sub-threshold
...
- Basket legs: EURUSD.DWX and NZDUSD.DWX.
- Logical symbol: QM5_12735_EURUSD_NZDUSD_COINTEGRATION_D1.
- Period: D1.
...
- Compute `spread = ln(EURUSD) - 0.81 * ln(NZDUSD)`.
- Compute a 60-bar rolling z-score of the spread.
- If no pair package is open and z > +2.0, open a short-spread package: short EURUSD, long NZDUSD.
- If no pair package is open and z < -2.0, open a long-spread package: long EURUSD, short NZDUSD.
...
- Host chart must be EURUSD.DWX or NZDUSD.DWX on D1/H1, with slot 0 used for the logical host.
...
- [x] R3 testable: EURUSD.DWX and NZDUSD.DWX are Darwinex-native `.DWX` symbols in the exported scan data.
...
| v1 | 2026-06-28 | pending Q02 row payload repaired with explicit basket tester deposit, RISK_FIXED, full-history window, priority flag, and non-duplicate guard evidence | Q02 | PENDING |
