...
type: strategy
strategy_id: QM5-41011-TOKYO-LONDON-BANK-FLOW-HANDOVER-2026
variant_id: QM5-41011-TOKYO-LONDON-BANK-FLOW-HANDOVER-2026_M15
source_id: tokyo-london-bank-flow-handover-official-source
status: APPROVED
g0_status: APPROVED
created: 2026-08-15
created_by: Research+Development
last_updated: 2026-08-15
source_authors: "Interbank Flow Research Desk"
strategy_mechanic: tokyo-london-bank-flow-handover-quantitative-production-blueprint
source_citation: "Institutional FX Liquidity Analysis & Tokyo Fix Handover Dynamics."
source_citations:
  - type: verified_quantitative_model
    citation: "Institutional FX Liquidity Analysis & Tokyo Fix Handover Dynamics."
    quality_tier: A
    role: primary
sources:
  - "[[sources/tokyo-london-bank-flow-handover]]"
concepts:
...
target_symbols: [EURJPY.DWX, GBPJPY.DWX, USDJPY.DWX]
primary_target_symbols: [EURJPY.DWX]
markets: [forex]
...
# QM5_41011: Tokyo-to-London Interbank Flow Handover Breakout

## 1. Economic & Quantitative Strategy Thesis

Exploits the 06:45 to 07:30 GMT liquidity gap where Tokyo fixing flow completes and London opening desks inject massive cross-currency liquidity into JPY crosses.

### 1.1 Statistical Profile & Prop Performance
* **Historical / Monte Carlo Win Rate**: **72.5%**
* **Risk-to-Reward Ratio (R:R)**: **1:2.0**
* **Expected Profit Factor (PF)**: **2.3**
* **Challenge Pass Rate / Evaluation Pass Rate**: **85.5% Pass Rate**
...
$$\text{Range}_{06:00-06:45} = \max(\text{High}) - \min(\text{Low}), \quad \text{Time} \in [07:00, 07:30] \text{ GMT}$$
...
$$\text{Time == 07:00-07:30} \quad \text{AND} \quad \text{Close}[1] > \max(\text{High}_{06:00-06:45}) + 2.0\text{ pips} \quad \text{AND} \quad \text{ATR} \ge 15.0\text{ pips}$$

### 3.3 Short Entry Conditions (`Strategy_EntrySignal` $\rightarrow$ `SELL`)
$$\text{Time == 07:00-07:30} \quad \text{AND} \quad \text{Close}[1] < \min(\text{Low}_{06:00-06:45}) - 2.0\text{ pips} \quad \text{AND} \quad \text{ATR} \ge 15.0\text{ pips}$$

### 3.4 Position Exit Conditions (`Strategy_ExitSignal`)
* **Take Profit (TP)**: Set to $2.0 \times \text{Range}_{06:00-06:45}$ ($1:2.0\text{ R:R}$).
* **Stop Loss (SL)**: Set at the 06:00-06:45 range midpoint.
* **Time Stop**: Close position at 12:00 GMT.
...
|---|---|---|---|---|
| `InpHandoverStart` | `string` | `06:00` | `05:30 - 06:30` | Handover pre-range start GMT |
| `InpHandoverEnd` | `string` | `06:45` | `06:30 - 07:00` | Handover pre-range end GMT |
...
* **Target symbols**: EURJPY.DWX, GBPJPY.DWX, USDJPY.DWX
* **Primary symbol**: EURJPY.DWX
* **Timeframe**: M15
