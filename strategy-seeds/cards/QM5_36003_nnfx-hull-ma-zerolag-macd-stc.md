...
type: strategy
strategy_id: QM5-36003-NNFX-HULL-MA-ZEROLAG-MACD-STC-2026
variant_id: QM5-36003-NNFX-HULL-MA-ZEROLAG-MACD-STC-2026_D1
source_id: nnfx-hull-ma-zerolag-macd-stc-official-source
status: APPROVED
g0_status: APPROVED
created: 2026-08-15
...
strategy_mechanic: nnfx-hull-ma-zerolag-macd-stc-quantitative-production-blueprint
source_citation: "No Nonsense Forex High-Speed Indicator Combination Suite."
source_citations:
  - type: verified_quantitative_model
    citation: "No Nonsense Forex High-Speed Indicator Combination Suite."
    quality_tier: A
    role: primary
sources:
  - "[[sources/nnfx-hull-ma-zerolag-macd-stc]]"
concepts:
...
# QM5_36003: NNFX Hull MA & ZeroLag MACD Fast Trend Engine

## 1. Economic & Quantitative Strategy Thesis

High-speed NNFX variant using Hull Moving Average (HMA 20) baseline, ZeroLag MACD (C1 trigger), Schaff Trend Cycle (STC 23,50,10 as C2), and Better Volume filter.

### 1.1 Statistical Profile & Prop Performance
* **Historical / Monte Carlo Win Rate**: **66.8%**
...
$$\text{HMA}_t = \text{WMA}\left(2 \times \text{WMA}(P, 10) - \text{WMA}(P, 20), \sqrt{20}\right)$$
$$\text{ZL\_MACD} = \text{EMA}(\text{Price} + (\text{Price} - \text{EMA}(12)), 12) - \text{EMA}(\text{Price} + (\text{Price} - \text{EMA}(26)), 26)$$
...
$$\text{Close}[1] > \text{HMA}[1] \quad \text{AND} \quad \text{ZL\_MACD}[1] > \text{ZL\_Signal}[1] \quad \text{AND} \quad \text{STC}[1] \ge 75.0 \quad \text{AND} \quad \text{BetterVol == HIGH}$$

### 3.3 Short Entry Conditions (`Strategy_EntrySignal` $\rightarrow$ `SELL`)
$$\text{Close}[1] < \text{HMA}[1] \quad \text{AND} \quad \text{ZL\_MACD}[1] < \text{ZL\_Signal}[1] \quad \text{AND} \quad \text{STC}[1] \le 25.0 \quad \text{AND} \quad \text{BetterVol == HIGH}$$
...
* **Runner Exit**: ZeroLag MACD signal line crossover.
...
|---|---|---|---|---|
| `InpHMAPeriod` | `int` | `20` | `14 - 30` | Hull Moving Average baseline period |
| `InpFastMACD` | `int` | `12` | `8 - 15` | ZeroLag MACD fast EMA |
| `InpSlowMACD` | `int` | `26` | `20 - 35` | ZeroLag MACD slow EMA |
