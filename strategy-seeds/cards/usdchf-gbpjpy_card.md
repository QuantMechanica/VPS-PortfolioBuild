---
strategy_id: AI-CODEX-FX-COINT66-20260609-USDCHF-GBPJPY
ea_id: QM5_20240
slug: usdchf-gbpjpy
type: strategy
status: APPROVED
...
markets: [forex]
timeframes: [D1]
primary_target_symbols: [USDCHF.DWX, GBPJPY.DWX]
target_symbols: [USDCHF.DWX, GBPJPY.DWX]
logical_symbol: QM5_20240_USDCHF_GBPJPY_COINTEGRATION_D1
period: D1
...
# QM5_20240 USDCHF/GBPJPY D1 Cointegration Basket
...
low-frequency daily implementation. Chan makes no claim for USDCHF/GBPJPY.
...
`QM5_1156_caldeira-cointegration-pairs-fx`. USDCHF/GBPJPY ranks fifty-ninth of
66 by OOS net Sharpe and is the first relationship-level build gap after that
duplicate. The deterministic dedup check found no exact collision across
4,297 registry rows and 413 direct cards. None of 256 tracked basket manifests
declares this exact two-leg relationship.
...
The negative DEV and OOS results, small absolute beta, and long half-life are
adverse evidence. This card authorizes a one-shot low-frequency falsification
test, not a certified edge. Terminal economic, cadence, or minimum-volume
failure retires the exact sleeve without a filter, beta refit, or parameter
rescue.

## Non-Duplicate Boundary

`QM5_1156` explicitly mechanizes rank 58 and is therefore not duplicated.
Broad momentum, carry, trend, and strength baskets that contain USDCHF and
GBPJPY as unrelated universe members do not trade this frozen-beta D1 residual
or logical two-leg package. Sibling fixed-scan sleeves use different pairs,
coefficients, magics, and logical identities.

## 2. Concept

Trade temporary deviations in the fixed log-price residual
`ln(USDCHF) - (-0.070133022445) * ln(GBPJPY)`. The negative fitted
...
package retains USD, CHF, GBP, JPY, carry, and broad risk-sentiment exposure.

## Hypothesis

The frozen scan found negative DEV and OOS performance. This card tests whether
large fixed-residual deviations survive native tick costs and swap in the
canonical D1 implementation. Economic, cadence, history, minimum-volume, or
...
`USDCHF.DWX` is the tester host and first traded leg. `GBPJPY.DWX` is the
...
- Compute `spread = ln(USDCHF) - strategy_beta * ln(GBPJPY)`, with frozen
  `strategy_beta = -0.070133022445`.
...
Chan supplies the pair-trading method but reports no USDCHF/GBPJPY result. The
...
- RETIRE if the small normalized GBPJPY leg cannot meet broker minimum volume.
...
data_requirements: "USDCHF.DWX, GBPJPY.DWX, and USDJPY.DWX D1 histories"
```
...
- 2026-08-06: Rank 58 already exists as an explicit pair slot in QM5_1156;
  rank 59 is the first unbuilt relationship after applying that stronger
  duplicate guard.
- 2026-08-06: Rank 59 is DEV- and OOS-negative, has a small absolute beta, and
  is below the expected Q02 cadence floor; it is admitted only as the
  explicitly requested next-best one-shot sleeve, with retirement rather than
  rescue after economic or minimum-volume failure.
