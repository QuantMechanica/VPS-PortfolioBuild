...
pipeline_phase: Q02
review_focus: "Falsify the source-defined unconditional two-month XNG sign fade on the DWX CFD carrier; the paper assumes zero costs and does not establish CFD basis, risk-adjusted efficacy, drawdown, or correlation to the certified book."
modules_used: [no_trade, trade_entry, trade_management, trade_close]
target_modules: [Strategy_NoTradeFilter, Strategy_EntrySignal, Strategy_ManageOpenPosition, Strategy_ExitSignal, Strategy_NewsFilterHook]
hard_rules_at_risk: [friday_close, risk_mode_dual, cfd_futures_basis, low_frequency]
g0_approval_reasoning: "APPROVED under the 2026-07-20 OWNER commodity-sleeve mission: R1 peer-reviewed single-source lineage; R2 unconditional fixed two-month contrarian sign rule with an ex-ante odd-month anchor; R3 registered XNG D1 carrier and six decisions/year; R4 deterministic, ML-free and one position per magic."
---

# XNG Two-Month Unconditional Contrarian State

## Hypothesis

Natural-gas spot and futures prices can mean-revert at a fixed two-month
horizon. Mishra and Smyth test that structural hypothesis with the simplest
possible state rule: after a two-month fall, hold long for the next two-month
...
margin or short-financing model for Table 10. Its unusually strong two-month
...
Nov-Dec. On the first tradable D1 bar of each odd-numbered month, reconstruct
the latest three completed month-end closes:

`C0 = just-completed month`, `C1 = one month earlier`, `C2 = two months earlier`.

- BUY when `C0 < C2`, fading the just-completed two-month decline.
- SELL when `C0 > C2`, fading the just-completed two-month rise.
...
odd-month anchor and observable package renewal are frozen ex-ante portability
...
volatility percentile. `QM5_13139_energy-cv-rank` shares a bimonthly clock but
ranks XTI against XNG on a 36-month coefficient of variation. None implements
an unconditional two-month XNG time-series sign fade.

## Markets And Timeframe

- Target: `XNGUSD.DWX` only, D1, magic slot 0.
- Decision: first tradable D1 bar of each odd broker month.
- Expected cadence: six decisions/packages per complete year; Q02 is
...
- Reconstruct three distinct completed broker-month closes from a bounded D1
  scan and compare `C0` strictly with `C2`; `C1` is retained only to prove that
  the endpoints span two distinct completed months.
- BUY for `C0 < C2`; SELL for `C0 > C2`; equality creates no new order.
...
- Require valid history/prices/ATR, acceptable spread, no open current-period
  package and no earlier entry deal in the current bimonthly period.
...
- On exact equality, retain the prior position as the source states; the V5
  70-day stale guard remains a safety override.
- Close at 70 calendar days as a fail-safe or at the broker hard stop.
- No intraperiod signal, target, trail, break-even move or discretionary exit.

## 6. Filters (No-Trade Module)

- Exact `XNGUSD.DWX`, D1, slot-0 host guard.
- Locked two-month holding period and odd-month epoch; parameter, history,
...
- One position per magic and at most one entry package per bimonthly period.
...
- Friday close is disabled to preserve the fixed two-month holding horizon.
...
| `strategy_rebalance_month_parity` | 1 | [1] | odd-month broker-calendar epoch |
| `strategy_atr_period` | 20 | [20] | V5 frozen hard-stop estimate |
| `strategy_atr_sl_mult` | 4.0 | [4.0] | frozen hard-stop distance |
| `strategy_max_hold_days` | 70 | [70] | stale safety override |
...
- [x] R3 testable: registered `XNGUSD.DWX` D1 carrier and six decisions/year.
...
- trade_entry: source-defined opposite sign of the completed two-month move,
  with one frozen V5 ATR hard stop.
- trade_management: fixed-period renewal plus 70-day stale safety override.
