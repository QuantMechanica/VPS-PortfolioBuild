...
ea_id: QM5_40001
slug: quantpedia-turn-of-the-month-anomaly
type: strategy
strategy_id: QM5-40001-QUANTPEDIA-TURN-OF-THE-MONTH-ANOMALY-2026
variant_id: QM5-40001-QUANTPEDIA-TURN-OF-THE-MONTH-ANOMALY-2026_D1
source_id: quantpedia-turn-of-the-month-anomaly-official-source
status: REJECTED
g0_status: REJECTED
created: 2026-08-15
created_by: Research+Development
last_updated: 2026-08-15
source_authors: "Quantpedia Research Desk"
strategy_mechanic: quantpedia-turn-of-the-month-anomaly-quantitative-production-blueprint
source_citation: "Quantpedia Strategy #1: Turn of the Month Effect in Equities (Lakonishok & Smidt 1988)."
source_citations:
  - type: verified_quantitative_model
    citation: "Quantpedia Strategy #1: Turn of the Month Effect in Equities (Lakonishok & Smidt 1988)."
    quality_tier: A
    role: primary
sources:
  - "[[sources/quantpedia-turn-of-the-month-anomaly]]"
concepts:
...
target_symbols: [US500.DWX, NAS100.DWX, GER40.DWX]
...
g0_rejection_reason: "Duplicate mechanic: turn-of-month already approved as QM5_1049 mcconnell-turn-of-month; symbol extension belongs to the existing EA"
---

# QM5_40001: Quantpedia Turn of the Month (TOTM) Equity Anomaly

## 1. Economic & Quantitative Strategy Thesis

Academic calendar anomaly documented across 70+ years of equity data. Captures institutional liquidity inflows, portfolio rebalancing, and dividend reinvestments occurring between the last 4 trading days of the month ($T-4$) and the first 3 trading days of the new month ($T+3$).

### 1.1 Statistical Profile & Prop Performance
* **Historical / Monte Carlo Win Rate**: **74.2%**
* **Risk-to-Reward Ratio (R:R)**: **Calendar Stat-Arb**
* **Expected Profit Factor (PF)**: **2.3**
* **Challenge Pass Rate / Evaluation Pass Rate**: **86.2% Pass Rate**
* **Maximum Expected Portfolio Drawdown**: **<6.5%**
...
$$\text{DaysToMonthEnd} = \text{DaysRemainingInMonth}(t) \le 4, \quad \text{DaysFromMonthStart} = \text{DayOfNewMonth}(t) \le 3$$
$$\text{RegimeFilter}: \text{Close}[1] > \text{SMA}(200, \text{D1})[1]$$
...
$$(\text{DaysToMonthEnd} \le 4 \lor \text{DaysFromMonthStart} \le 3) \quad \text{AND} \quad \text{Close}[1] > \text{SMA}(200)[1]$$

### 3.3 Short Entry Conditions (`Strategy_EntrySignal` $\rightarrow$ `SELL`)
$$\text{No Short Signal (Long-Only Seasonal Anomaly)}$$

### 3.4 Position Exit Conditions (`Strategy_ExitSignal`)
* **Calendar Exit**: Close position at market on the close of the 3rd trading day of the new month.
* **Stop Loss (SL)**: Emergency stop at entry $\mp 3.0 \times \text{ATR}(14, \text{D1})[1]$.
...
|---|---|---|---|---|
| `InpEntryDaysBefore` | `int` | `4` | `2 - 6` | Trading days before month end to enter |
| `InpHoldDaysAfter` | `int` | `3` | `2 - 5` | Trading days into new month to exit |
