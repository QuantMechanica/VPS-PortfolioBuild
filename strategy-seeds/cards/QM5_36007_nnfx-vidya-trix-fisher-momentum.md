...
type: strategy
strategy_id: QM5-36007-NNFX-VIDYA-TRIX-FISHER-MOMENTUM-2026
variant_id: QM5-36007-NNFX-VIDYA-TRIX-FISHER-MOMENTUM-2026_D1
source_id: nnfx-vidya-trix-fisher-momentum-official-source
status: APPROVED
g0_status: APPROVED
created: 2026-08-15
...
strategy_mechanic: nnfx-vidya-trix-fisher-momentum-quantitative-production-blueprint
source_citation: "No Nonsense Forex Dynamic Variable Index Models."
source_citations:
  - type: verified_quantitative_model
    citation: "No Nonsense Forex Dynamic Variable Index Models."
    quality_tier: A
    role: primary
sources:
  - "[[sources/nnfx-vidya-trix-fisher-momentum]]"
concepts:
...
target_symbols: [EURUSD.DWX, GBPJPY.DWX, NZDCAD.DWX]
...
# QM5_36007: NNFX VIDYA & TRIX Momentum System

## 1. Economic & Quantitative Strategy Thesis

Variable Index Dynamic Average (VIDYA adjusted by Chande Momentum Oscillator) baseline, TRIX (Triple Exponential Derivative) C1 trigger, Fisher Transform C2, and Money Flow Index (MFI).

### 1.1 Statistical Profile & Prop Performance
* **Historical / Monte Carlo Win Rate**: **68.9%**
...
* **Maximum Expected Portfolio Drawdown**: **<4.3%**
...
$$\text{VIDYA}_t = \alpha \times |\text{CMO}(12)| \times P_t + (1 - \alpha \times |\text{CMO}(12)|) \text{VIDYA}_{t-1}$$
$$\text{TRIX} = \frac{\text{EMA3}_t - \text{EMA3}_{t-1}}{\text{EMA3}_{t-1}} \times 10000$$
...
$$\text{Close}[1] > \text{VIDYA}[1] \quad \text{AND} \quad \text{TRIX}[1] > \text{TRIX\_Signal}[1] \quad \text{AND} \quad \text{Fisher}[1] > 0 \quad \text{AND} \quad \text{MFI}(14)[1] \ge 50.0$$

### 3.3 Short Entry Conditions (`Strategy_EntrySignal` $\rightarrow$ `SELL`)
$$\text{Close}[1] < \text{VIDYA}[1] \quad \text{AND} \quad \text{TRIX}[1] < \text{TRIX\_Signal}[1] \quad \text{AND} \quad \text{Fisher}[1] < 0 \quad \text{AND} \quad \text{MFI}(14)[1] \le 50.0$$
...
* **Runner Exit**: TRIX signal line re-cross.
...
|---|---|---|---|---|
| `InpVIDYAPeriod` | `int` | `9` | `5 - 15` | VIDYA base smoothing period |
| `InpCMOPeriod` | `int` | `12` | `8 - 20` | Chande Momentum volatility period |
| `InpTRIXPeriod` | `int` | `14` | `9 - 21` | TRIX triple EMA period |
...
* **Target symbols**: EURUSD.DWX, GBPJPY.DWX, NZDCAD.DWX
