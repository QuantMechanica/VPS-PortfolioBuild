...
type: strategy
strategy_id: MOP-TSMOM-2012_XTI_MK12_S16
variant_id: MOP-TSMOM-2012_XTI_MK12_S16
source_id: MOP-WTI-RANKTREND-2026
ea_id: QM5_20264
slug: wti-rank-trend
status: APPROVED
execution_contract_ref: strategy-seeds/cards/approved/QM5_20264_wti-rank-trend_card.md
...
source_citations:
  - type: peer_reviewed_paper
    ...
    location: "DOI https://doi.org/10.1016/j.jfineco.2011.11.003; complete-paper evidence strategy-seeds/sources/MOP-TSMOM-2012/source.md; bounded extraction strategy-seeds/sources/MOP-WTI-RANKTREND-2026/source.md"
    quality_tier: A
    role: primary_own_price_trend_and_monthly_cadence
strategy_mechanic: monthly-wti-thirteen-month-end-mann-kendall-pairwise-rank-trend-with-fixed-score-gate
sources:
  - "[[sources/MOP-WTI-RANKTREND-2026]]"
concepts:
...
indicators:
  ...
  - "[[indicators/atr]]"
strategy_type_flags: [commodity, energy, crude-oil, structural-trend, rank-statistic, path-consistency, monthly-rebalance, atr-hard-stop, time-stop, symmetric-long-short, low-frequency]
...
period: D1
timeframe: D1
expected_trade_frequency: "Estimated 5-9 completed monthly WTI positions/year after thirteen completed month ends; Q02 must prove at least five/year or retire."
...
g0_approval_reasoning: "APPROVED under decisions/2026-08-07_qm5_20264_wti_rank_trend_g0.md: R1 complete-read peer-reviewed WTI source; R2 fixed all-pairs score and lifecycle; R3 registered XTI D1; R4 deterministic native arithmetic. Exact dedup clean; expected OLS family neighbors manually resolved."
---

# QM5_20264 WTI Pairwise Rank Trend

## Hypothesis

WTI can sustain slow directional regimes as production policy, investment,
inventories, transport, refining, hedging, and demand adjust. A twelve-month
endpoint return can be dominated by one jump, while an OLS path gate remains
sensitive to the magnitudes and squared residuals of large monthly moves. This
card instead asks whether newer monthly endpoints tend to rank above or below
older endpoints across every pair.

The direct crude-oil carrier is economically different from the certified XAU,
...
The canonical checker scanned 4,321 EA-registry rows and 438 intake cards. It
found no exact identity and returned only expected shared-source WTI/XNG OLS
neighbors for manual review. A content scan found no Mann-Kendall, Kendall tau,
concordant-minus-discordant, or equivalent all-pairs WTI rank-trend card.

The nearest WTI systems use one endpoint return, adjacent monthly-return signs,
votes across cumulative-return horizons, OLS slope plus `R^2`, moving averages,
channels, variance ratios, or calendar states. None compares every older/newer
month-end pair while discarding return magnitude. The thirteen endpoints, all
78 ordered comparisons, no-tie rejection, integer score, fixed `abs(S) >= 28`
gate, consumed attempt, and monthly renewal are jointly load-bearing. Verdict:
`CLEAN_AFTER_EXPECTED_FAMILY_AND_MANUAL_REVIEW`.

## Markets, Timeframe, And Cadence

- Exact host and traded symbol: `XTIUSD.DWX`.
- Timeframe: D1; magic slot 0; intended magic `202640000`.
...
1. Require exact EA ID `20264`, `XTIUSD.DWX` D1, magic slot 0, and every
...
this rank-trend rule works, that score 28 is optimal, that a continuous CFD
...
single-name concentration, sparse qualified paths, stale rank trends, hard-stop
slippage, and correlation with XNG or risk assets can dominate the premise.
The rank score ignores move magnitude and is not a forecast-confidence
guarantee.
...
- [x] Dedup: no exact identity; expected OLS family neighbors manually resolved.

## Framework Alignment

- no_trade: exact WTI/D1/EA/slot, locked inputs, fixed risk/news/Friday contract,
