...
type: strategy
strategy_id: EIA-BOC-RBNZ-XBR-NZDCAD-2026
source_id: EIA-BOC-RBNZ-XBR-NZDCAD-2026
source_citation: "EIA oil/exchange-rate working paper plus official Bank of Canada commodity-CAD note and RBNZ commodity-price monetary-policy context."
source_citations:
  ...
  - type: central_bank_research
    ...
    role: cad_channel
  - type: central_bank_research
    citation: "Reserve Bank of New Zealand. Commodity prices and implications for monetary policy. 2011."
    ...
    quality_tier: A
    role: nzd_commodity_channel
sources:
  - "[[sources/EIA-BOC-RBNZ-XBR-NZDCAD-2026]]"
concepts:
...
indicators:
  ...
  - "[[indicators/zscore]]"
  - "[[indicators/atr]]"
strategy_type_flags: [oil-fx-return-spread, market-neutral-basket, zscore-reversion, atr-hard-stop, time-stop, low-frequency, energy]
target_symbols: [XBRUSD.DWX, NZDCAD.DWX]
basket_symbols: [XBRUSD.DWX, NZDCAD.DWX]
markets: [XBRUSD.DWX, NZDCAD.DWX]
primary_target_symbols: [XBRUSD.DWX, NZDCAD.DWX]
single_symbol_only: false
logical_symbol: QM5_13082_XBR_NZDCAD_RSPREAD_D1
period: D1
timeframes: [D1]
expected_trade_frequency: "D1 XBR/NZDCAD return-spread z-score reversion; estimate 8-14 paired packages/year before Q02 validates synchronized history and fills."
...
g0_approval_reasoning: "Mission-directed G0 approval 2026-07-09: R1 PASS official EIA oil/exchange-rate research plus Bank of Canada CAD commodity-channel and RBNZ NZ commodity-channel support; R2 PASS deterministic D1 two-leg XBR/NZDCAD return-spread z-score reversion with spread caps, mean exit, max-hold exit, and ATR hard stops; R3 PASS XBRUSD.DWX and NZDCAD.DWX exist in the DWX matrix; R4 PASS no ML/grid/martingale/external runtime data. Non-duplicate because this is a paired Brent/NZDCAD inverse-CAD return-spread basket, not Brent/USDCAD, XBR/NZDUSD, XBR/CADJPY, XBR/CADCHF, XBR/AUDCAD, XBR/GBPCAD, XBR/EURCAD, XBR/AUDJPY, XBR/AUDUSD, XBR/XNG, Brent/WTI, metal-ratio, XNG, or index logic."
---

# XBR/NZDCAD D1 Return-Spread Reversion

## Source

- Source: [[sources/EIA-BOC-RBNZ-XBR-NZDCAD-2026]]
...
- Support: Bank of Canada Staff Analytical Note 2017-1 for the commodity/CAD
  channel; Reserve Bank of New Zealand commodity-price research for the New
  Zealand commodity-exporter terms-of-trade channel.

## Concept

Oil and CAD have a structural macro relationship, but this card deliberately
does not add another USDCAD or CADJPY sleeve. It uses `NZDCAD.DWX`, where CAD
strength normally pushes the cross lower, to build an inverse-CAD relative-value
package against Brent:

`return_spread = ln(XBR[t] / XBR[t-L]) + beta_nzdcad * ln(NZDCAD[t] / NZDCAD[t-L])`

If Brent rallies while NZDCAD also rises, CAD is not confirming the crude move;
the basket fades the high return spread by selling both Brent and NZDCAD. If Brent
falls while NZDCAD also falls, CAD strength has outpaced crude; the basket buys
both Brent and NZDCAD. The bet is a temporary return-spread dislocation between
the crude leg and a direct NZD/CAD commodity-FX leg.

This is deliberately different from:

- `QM5_12609_wti-cad-spread-mr` and `QM5_12722_wti-cad-brk`: those use USDCAD,
  not NZDCAD, and different spread definitions.
- `QM5_13006_xti-nzd-rspread`: that uses NZDUSD, not the direct CAD cross.
- `QM5_13010_xti-cadjpy-rspr`, `QM5_13011_xti-cadchf-rspr`,
  `QM5_13034_xti-audcad-rspr`, `QM5_13057_xti-gbpcad-rspr`,
  `QM5_13060_xti-eurcad-rspr`, `QM5_13059_xti-audjpy-rspr`, and
  `QM5_13073_xti-audusd-rspr`: all use different FX legs or quotation logic.
- Brent/WTI, XBR/XNG, oil/gold, oil/silver, XAU/XAG, XNG, index, and outright
  Brent calendar/event sleeves: this is a two-leg Brent/NZDCAD return-spread
  basket.
...
- Basket legs: `XBRUSD.DWX` and `NZDCAD.DWX`.
- Period: D1.
...
- Runtime data: Darwinex MT5 D1 OHLC, spread metadata, ATR, broker time, and
  V5 framework state only. No EIA, Bank of Canada, RBNZ, futures-curve, CFTC,
...
- Copy completed D1 closes for `XBRUSD.DWX` and `NZDCAD.DWX`.
- Compute `xbr_ret = ln(XBR close[1] / XBR close[1 + strategy_return_lookback_d1])`.
- Compute `nzdcad_ret = ln(NZDCAD close[1] / NZDCAD close[1 + strategy_return_lookback_d1])`.
- Compute `return_spread = xbr_ret + strategy_beta_nzdcad * nzdcad_ret`.
...
- Skip entries when `NZDCAD.DWX` spread exceeds `strategy_nzdcad_max_spread_pts`.
...
- name: strategy_nzdcad_max_spread_pts
  default: 140
  sweep_range: [90, 140, 220]
...
commodity-CAD, and New Zealand commodity-price channels only. This card imports
no source performance number. Q02 and later phases must validate or reject the
`XBRUSD.DWX` / `NZDCAD.DWX` basket on Darwinex bars.
...
- risk_class: medium-high because crude volatility, NZDCAD liquidity/spread,
...
- [x] R3 testable: `XBRUSD.DWX` and `NZDCAD.DWX` exist in the Darwinex symbol
  matrix with D1 history.
- [x] R4 compliant: no ML, adaptive PnL fitting, grid, martingale, external
  runtime feed, or more than one position per magic slot.
- [x] Non-duplicate: paired Brent/NZDCAD return-spread mean reversion, not the
  existing XBR/CADJPY, XBR/CADCHF, XBR/AUDCAD, XBR/GBPCAD, XBR/EURCAD,
  XBR/AUDJPY, XBR/AUDUSD, XBR/NZDUSD, Brent/USDCAD, XBR/XNG, Brent/WTI,
  oil/metal, XNG, metal, index, or commodity-RSI logic.
...
- Slot 0: `XBRUSD.DWX`.
- Slot 1: `NZDCAD.DWX`.
- Use the logical basket setfile `QM5_13082_XBR_NZDCAD_RSPREAD_D1` for Q02.
...
- trade_entry: D1 standardized XBR/NZDCAD return-spread reversion.
...
trade, if Q02 PF is below 1.0 after costs, if synchronized XBR/NZDCAD history
...
|---|---|---|---|---|
| v1 | 2026-07-09 | initial XBR/NZDCAD return-spread basket build | Q02 | PENDING |
...
| Q01 Build Validation | 2026-07-09 | PENDING | `artifacts/qm5_13082_build_result.json` |
