...
slug: brent-apr-prem
type: strategy
strategy_id: ARENDAS-OIL-SEASON-2018_BRENT_S01
...
period: D1
timeframes: [D1]
expected_trade_frequency: "April-only D1 Brent month-of-year positive-return sleeve; estimate 18-22 entries/year after weekends, broker holidays, and framework filters."
...
g0_approval_reasoning: "R1 PASS peer-reviewed oil-seasonality paper covering both Brent and WTI; R2 PASS deterministic April D1 long/time-flat rule with ATR stop; R3 PASS XBRUSD.DWX is locally routed by prior Brent builds with Q02 validating current history sufficiency; R4 PASS no ML/grid/martingale/external runtime data."
expected_pf: 1.08
...
and identifies April as one of the positive-return months. This card isolates
that source claim as a clean Brent-only energy sleeve: long-only exposure to
`XBRUSD.DWX` during broker-calendar April D1 bars, with each position flattened
on the next D1 bar unless the ATR hard stop, month-end, stale guard, or
framework Friday close acts first.

This is deliberately different from:

- `QM5_12727_wti-apr-prem`: same April thesis, but WTI benchmark; this card
  targets Brent exposure.
- `QM5_12853_brent-may-prem`, `QM5_12854_brent-dec-fade`, and
  `QM5_12855_brent-nov-fade`: Brent calendar cards, but different months and
  seasonal direction.
...
- `QM5_12849_brent-tsmom12m` and `QM5_12859_brent-52w-anchor`: Brent trend or
  anchor logic, not a fixed April calendar premium.
...
one-bar long Brent exposure during broker-calendar April can add energy exposure
that differs from the current XAU, index, XNG, WTI, and paired-spread sleeves.

## rules

- Trade only `XBRUSD.DWX` on D1.
- Enter long only during broker-calendar April.
...
The source reports April as one of the positive average-return months in its
...
- [x] R1 source lineage: peer-reviewed oil-seasonality paper covering Brent and
  WTI seasonality.
- [x] R2 mechanical: fixed broker-calendar April, single D1 long entry, ATR
...
- [x] R4 compliant: no ML, no adaptive PnL fitting, no grid, no martingale, one
  position per magic.
- [x] Non-duplicate: Brent April calendar premium is not WTI April, Brent
  May/November/December calendar logic, Brent weekday, Brent TSMOM, Brent/WTI
  spread, XTI/XNG, XNG, XAU/XAG, gas-metal, WTI event, or commodity RSI logic.
...
|---|---|---|---|---|
| v1 | 2026-07-02 | initial Brent April calendar-premium build | Q02 | ENQUEUED |
...
| Q01 Build Validation | 2026-07-02 | PASS | `D:\QM\reports\framework\21\build_check_20260701_234015.json` |
| Q02 Baseline Screening | 2026-07-02 | QUEUED | `D:\QM\strategy_farm\state\farm_state.sqlite` work item `347cc5d0-ac86-4b99-b959-b680da56d49e` |
