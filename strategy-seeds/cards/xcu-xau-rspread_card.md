...
source_citation: "Parnes 2024 copper-to-gold ratio paper, State Street gold/copper ratio explainer, and CME metals market context."
source_citations:
  ...
  - type: asset_manager_research
    ...
    quality_tier: A-
    role: copper_gold_market_context
...
sources:
  - "[[sources/PARNES-SSGA-COPPERGOLD-2026]]"
concepts:
  - "[[concepts/copper-gold-ratio]]"
...
markets: [XCUUSD.DWX, XAUUSD.DWX]
primary_target_symbols: [XCUUSD.DWX, XAUUSD.DWX]
single_symbol_only: false
logical_symbol: QM5_13098_XCU_XAU_RSPREAD_D1
period: D1
timeframes: [D1]
expected_trade_frequency: "D1 XCU/XAU copper-gold return-spread z-score reversion; estimate 5-10 paired packages/year before Q02 validates synchronized history and fills."
...
g0_approval_reasoning: "Mission-directed G0 approval 2026-07-09: R1 PASS peer-reviewed copper/gold ratio source plus SSGA and CME market references; R2 PASS deterministic D1 XCU/XAU return-spread basket; R3 PASS because XAUUSD.DWX is widely registered and existing XCU builds use XCUUSD.DWX, with synchronized multi-symbol history and fills to be proven by Q02; R4 PASS no ML/grid/martingale/external runtime data. Non-duplicate because this is paired copper/gold return-spread mean reversion, not solo copper trend/reversal, XCU/AUDUSD, XTI/XCU, XTI/XAU oil/gold, XNG/XAU gas/gold, XAU/XAG, energy calendar/event, index, or commodity-RSI logic."
---

# XCU/XAU D1 Return-Spread Reversion

## Source

- Source: [[sources/PARNES-SSGA-COPPERGOLD-2026]]
...
The source packet treats copper/gold as a structural relative-value signal:
copper is the cyclical industrial-metal leg, while gold is the defensive
safe-haven leg. This card does not forecast macro data and does not consume any
external source at runtime. It mechanizes only the tradable local expression of
temporary copper-minus-gold return dislocations.

The Darwinex-native package is:

`return_spread = ln(XCU[t] / XCU[t-L]) - beta_xau * ln(XAU[t] / XAU[t-L])`

When copper has unusually outperformed gold, the package sells `XCUUSD.DWX`
and buys `XAUUSD.DWX`. When copper has unusually underperformed gold, it buys
`XCUUSD.DWX` and sells `XAUUSD.DWX`. The thesis is temporary return-spread
normalization between a growth-sensitive industrial metal and a defensive
precious metal, not an outright copper or gold forecast.
...
- Basket legs: `XCUUSD.DWX` and `XAUUSD.DWX`.
- Period: D1.
...
- Copy completed D1 closes for `XCUUSD.DWX` and `XAUUSD.DWX`.
- Compute `xcu_ret = ln(XCU close[1] / XCU close[1 + strategy_return_lookback_d1])`.
- Compute `xau_ret = ln(XAU close[1] / XAU close[1 + strategy_return_lookback_d1])`.
- Compute `return_spread = xcu_ret - strategy_beta_xau * xau_ret`.
...
- Skip entries when `XAUUSD.DWX` spread exceeds `strategy_xau_max_spread_pts`.
...
The source packet establishes structural lineage for copper/gold as an
...
reject the `XCUUSD.DWX` / `XAUUSD.DWX` basket on Darwinex bars.
...
- risk_class: medium-high because copper history/fills, gold hedge behavior,
...
- [x] R1 reputable source: peer-reviewed copper/gold ratio paper plus SSGA and
  CME market references.
...
- [x] R3 testable: `XAUUSD.DWX` is widely registered in V5 and existing XCU
  EAs build against `XCUUSD.DWX`; Q02 must validate synchronized XCU/XAU
  history and fills.
- [x] R4 compliant: no ML, adaptive PnL fitting, grid, martingale, external
  runtime feed, or more than one position per magic slot.
- [x] Non-duplicate: paired copper/gold return-spread mean reversion, not solo
...
- Slot 0: `XCUUSD.DWX`.
- Slot 1: `XAUUSD.DWX`.
- Use the logical basket setfile `QM5_13098_XCU_XAU_RSPREAD_D1` for Q02.
...
- trade_entry: D1 standardized XCU/XAU return-spread reversion.
...
trade, if Q02 PF is below 1.0 after costs, if synchronized XCU/XAU history is
...
|---|---|---|---|
| v1 | 2026-07-09 | initial XCU/XAU return-spread basket card | Q02 | QUEUED |
