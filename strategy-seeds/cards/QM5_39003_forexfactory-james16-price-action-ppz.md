...
type: strategy
strategy_id: QM5-39003-FOREXFACTORY-JAMES16-PRICE-ACTION-PPZ-2026
variant_id: QM5-39003-FOREXFACTORY-JAMES16-PRICE-ACTION-PPZ-2026_D1
source_id: forexfactory-james16-price-action-ppz-official-source
status: APPROVED
g0_status: APPROVED
created: 2026-08-15
created_by: Research+Development
last_updated: 2026-08-15
source_authors: "James16 Group"
strategy_mechanic: forexfactory-james16-price-action-ppz-quantitative-production-blueprint
source_citation: "James16 (2005-2024). All Things Price Action. Forex Factory (>30M Views)."
source_citations:
  - type: verified_quantitative_model
    citation: "James16 (2005-2024). All Things Price Action. Forex Factory (>30M Views)."
    quality_tier: A
    role: primary
sources:
  - "[[sources/forexfactory-james16-price-action-ppz]]"
concepts:
...
# QM5_39003: James16 Price Action & Price Pivot Zone (PPZ) Rejection

## 1. Economic & Quantitative Strategy Thesis

Forex Factory's longest-running thread (>30M views). Trades clean Pinbar and DBLHC candlestick rejections occurring at major multi-week Price Pivot Zones (PPZ) aligned with 21 EMA support.
...
$$\text{PPZ\_Zone}: |\text{Price} - \text{PivotLevel}| \le 0.50 \times \text{ATR}(14, \text{D1})[1]$$
$$\text{Pinbar}: \text{Nose\_Wick} \ge 0.65 \times \text{TotalRange}, \quad \text{Body} \le 0.25 \times \text{TotalRange}$$
...
$$\text{PPZ Support == TRUE} \quad \text{AND} \quad \text{Bullish Pinbar == TRUE} \quad \text{AND} \quad \text{Close}[1] > \text{EMA}(21)[1]$$

### 3.3 Short Entry Conditions (`Strategy_EntrySignal` $\rightarrow$ `SELL`)
$$\text{PPZ Resistance == TRUE} \quad \text{AND} \quad \text{Bearish Pinbar == TRUE} \quad \text{AND} \quad \text{Close}[1] < \text{EMA}(21)[1]$$

### 3.4 Position Exit Conditions (`Strategy_ExitSignal`)
* **Take Profit (TP)**: Target next institutional PPZ resistance ($1:2.5\text{ R:R}$).
* **Stop Loss (SL)**: Placed $2.0\text{ pips}$ beyond the extreme Pinbar tail.
* **Trailing Stop**: Dynamic swing high/low trailing stop.
...
|---|---|---|---|---|
| `InpPPZLookback` | `int` | `20` | `10 - 50` | Pivot zone detection lookback bars |
| `InpTrendEMA` | `int` | `21` | `14 - 34` | Dynamic baseline EMA period |
