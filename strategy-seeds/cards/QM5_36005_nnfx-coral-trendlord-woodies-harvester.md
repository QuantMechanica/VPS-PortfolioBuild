...
type: strategy
strategy_id: QM5-36005-NNFX-CORAL-TRENDLORD-WOODIES-HARVESTER-2026
variant_id: QM5-36005-NNFX-CORAL-TRENDLORD-WOODIES-HARVESTER-2026_D1
source_id: nnfx-coral-trendlord-woodies-harvester-official-source
status: APPROVED
g0_status: APPROVED
created: 2026-08-15
...
strategy_mechanic: nnfx-coral-trendlord-woodies-harvester-quantitative-production-blueprint
source_citation: "No Nonsense Forex Trend Harvester Methodology."
source_citations:
  - type: verified_quantitative_model
    citation: "No Nonsense Forex Trend Harvester Methodology."
    quality_tier: A
    role: primary
sources:
  - "[[sources/nnfx-coral-trendlord-woodies-harvester]]"
concepts:
...
target_symbols: [GBPJPY.DWX, EURJPY.DWX, AUDNZD.DWX]
...
# QM5_36005: NNFX Coral & Trend Lord Momentum Harvester

## 1. Economic & Quantitative Strategy Thesis

Coral Trend Indicator (SMMA smoothed baseline) combined with Trend Lord (C1 color filter), Woodies CCI (C2 zero-line cross), and Waddah Attar Explosion.

### 1.1 Statistical Profile & Prop Performance
* **Historical / Monte Carlo Win Rate**: **69.2%**
...
* **Maximum Expected Portfolio Drawdown**: **<4.4%**
...
$$\text{Coral}_t = \text{SMMA}(P, 20, coeff=0.4), \quad \text{TrendLord}: \text{BarColor} == \text{Green/Red}$$
...
$$\text{Close}[1] > \text{Coral}[1] \quad \text{AND} \quad \text{TrendLord}[1] == \text{GREEN} \quad \text{AND} \quad \text{Woodies\_CCI}[1] > 0 \quad \text{AND} \quad \text{WAE} > \text{ExplosionLine}$$

### 3.3 Short Entry Conditions (`Strategy_EntrySignal` $\rightarrow$ `SELL`)
$$\text{Close}[1] < \text{Coral}[1] \quad \text{AND} \quad \text{TrendLord}[1] == \text{RED} \quad \text{AND} \quad \text{Woodies\_CCI}[1] < 0 \quad \text{AND} \quad \text{WAE} > \text{ExplosionLine}$$
...
* **Runner Exit**: Trend Lord color change.
...
|---|---|---|---|---|
| `InpCoralPeriod` | `int` | `20` | `14 - 30` | Coral SMMA smoothing period |
| `InpWoodiesCCIPeriod`| `int`| `14` | `10 - 20` | Woodies CCI period |
| `InpWAESens` | `int` | `150` | `100 - 200` | WAE sensitivity |
...
* **Target symbols**: GBPJPY.DWX, EURJPY.DWX, AUDNZD.DWX
