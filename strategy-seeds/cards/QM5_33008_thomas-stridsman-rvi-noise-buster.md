...
ea_id: QM5_33008
slug: thomas-stridsman-rvi-noise-buster
type: strategy
strategy_id: QM5-33008-THOMAS-STRIDSMAN-RVI-NOISE-BUSTER-2026
variant_id: QM5-33008-THOMAS-STRIDSMAN-RVI-NOISE-BUSTER-2026_H1
source_id: thomas-stridsman-rvi-noise-buster-official-source
status: APPROVED
g0_status: APPROVED
created: 2026-08-15
created_by: Research+Development
last_updated: 2026-08-15
source_authors: "Thomas Stridsman"
strategy_mechanic: thomas-stridsman-rvi-noise-buster-quantitative-production-blueprint
source_citation: "Stridsman, T. (2001). Trading Systems That Work. John Wiley & Sons."
source_citations:
  - type: verified_quantitative_model
    citation: "Stridsman, T. (2001). Trading Systems That Work. John Wiley & Sons."
    quality_tier: A
    role: primary
sources:
  - "[[sources/thomas-stridsman-rvi-noise-buster]]"
concepts:
...
# QM5_33008: Thomas Stridsman RVI Volatility Noise Buster

## 1. Economic & Quantitative Strategy Thesis

The Relative Volatility Index (RVI) measures the standard deviation of directional price movements. Filtering trend breakouts with RVI eliminates over 70% of false whipsaws.
...
* **Challenge Pass Rate / Evaluation Pass Rate**: **82.8% Pass Rate**
...
$$\text{RVI}_t = \frac{\text{EMA}(\sigma_{\text{up}}, 14)}{\text{EMA}(\sigma_{\text{up}}, 14) + \text{EMA}(\sigma_{\text{down}}, 14)} \times 100$$
...
$$\text{Close}[1] > \text{EMA}(50, \text{H1})[1] \quad \text{AND} \quad \text{RVI}[2] < 50.0 \quad \text{AND} \quad \text{RVI}[1] \ge 50.0$$

### 3.3 Short Entry Conditions (`Strategy_EntrySignal` $\rightarrow$ `SELL`)
$$\text{Close}[1] < \text{EMA}(50, \text{H1})[1] \quad \text{AND} \quad \text{RVI}[2] > 50.0 \quad \text{AND} \quad \text{RVI}[1] \le 50.0$$

### 3.4 Position Exit Conditions (`Strategy_ExitSignal`)
* **Take Profit (TP)**: Set to $2.0 \times \text{SL\_Distance}$ ($1:2.0\text{ R:R}$).
* **Stop Loss (SL)**: Set at entry $\mp 1.8 \times \text{ATR}(14, \text{H1})[1]$.
* **Trailing Stop**: At $+1.0\text{R}$, trail at $1.5 \times \text{ATR}(14, \text{H1})$ behind highest high.
...
|---|---|---|---|---|
| `InpRVIPeriod` | `int` | `14` | `10 - 28` | Relative Volatility Index lookback period |
| `InpEMAPeriod` | `int` | `50` | `20 - 100` | Trend filter exponential moving average |
| `InpCenterLine` | `double` | `50.0` | `45.0 - 55.0` | RVI directional momentum threshold |
