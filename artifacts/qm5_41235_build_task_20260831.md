...
Task ID: `6a252d26-7752-4928-8aa8-7788761370b8`
EA ID: `QM5_41235`
Slug: `wti-samecal-hampel5`
Card path: `D:\QM\strategy_farm\artifacts\cards_approved\QM5_41235_wti-samecal-hampel5_card.md`
Source ID: ``
Target EA directory: `C:\QM\repo\framework\EAs\QM5_41235_wti-samecal-hampel5`
Build artifact target: `D:\QM\strategy_farm\artifacts\builds\6a252d26-7752-4928-8aa8-7788761370b8.json`
...
- Directory: **`C:\QM\repo\framework\EAs\QM5_41235_wti-samecal-hampel5`** — use this EXACT path. Do NOT strip the `QM5_` prefix.
  The directory name must be `QM5_<NNNN>_<slug>` (e.g. `QM5_1044_vpmacd-us-indices`),
  NOT `<NNNN>_<slug>` (e.g. `1044_vpmacd-us-indices`).
- `.mq5` file: **`QM5_41235_wti-samecal-hampel5.mq5`** (e.g. `QM5_1044_vpmacd-us-indices.mq5`)
- `.ex5` file: same basename with `.ex5` extension after compile.
- Setfile naming: **`QM5_41235_wti-samecal-hampel5_<SYMBOL>_<TF>_<env>.set`**
...
- **Reworks / improvements (DL-069):** to improve an existing EA, REBUILD IN PLACE —
  overwrite `QM5_41235_wti-samecal-hampel5` (git keeps the history) and re-enqueue from Q02. Do NOT
  drop a new `QM5_41235_wti-samecal-hampel5_v2` dir unless you ALSO register that `_v2` slug (active)
...
1. Read `D:\QM\strategy_farm\artifacts\cards_approved\QM5_41235_wti-samecal-hampel5_card.md` fully. Extract Entry, Exit, Stop Loss, Position Sizing,
...
2. Reserve an ea_id row in `framework\registry\ea_id_registry.csv` if not already
   present. Append `QM5_41235,wti-samecal-hampel5` row (preserve CSV header + line endings).

3. Compute magic_base = numeric suffix of `QM5_41235` × 10000. For each symbol the
...
4. Create directory `C:\QM\repo\framework\EAs\QM5_41235_wti-samecal-hampel5`. Copy `framework/templates/EA_Skeleton.mq5`
   to `C:\QM\repo\framework\EAs\QM5_41235_wti-samecal-hampel5/QM5_41235_wti-samecal-hampel5.mq5` as a starting point — it has the
   framework wiring + 5 Strategy_ hook stubs pre-populated.

5. Edit `QM5_41235_wti-samecal-hampel5.mq5` to fill the 5 Strategy_ hooks against the
...
5a. **Write `C:\QM\repo\framework\EAs\QM5_41235_wti-samecal-hampel5/SPEC.md`** from `framework/templates/SPEC.md.template`.
    ...
    - Header: `**EA ID:** QM5_{{ea_id_suffix}}`, `**Slug:** wti-samecal-hampel5`,
    ...
    - § 6 Source Citation: source_id + pointer + "R1 lineage recorded and
      R2–R4 PASS per `artifacts/cards_approved/QM5_41235_wti-samecal-hampel5.md`"
    ...
    Validate with:
    ```powershell
    python C:\QM\repo\framework\scripts\validate_spec_doc.py C:\QM\repo\framework\EAs\QM5_41235_wti-samecal-hampel5
    ```
...
6. Resolve the latest real smoke logger sample, then run `build_check.ps1`.
   ...
   $buildCheckArgs = @(
     ...
     "-EALabel", "QM5_41235_wti-samecal-hampel5"
   )
...
7. Run `pwsh -File C:\QM\repo\framework\scripts\compile_one.ps1 -EALabel QM5_41235_wti-samecal-hampel5`.
   Must produce `.ex5`.
...
8. **Generate P2 setfiles before smoke** using `gen_setfile.ps1`.
   ...
   pwsh -File C:\QM\repo\framework\scripts\gen_setfile.ps1 `
     -EaSlug QM5_41235_wti-samecal-hampel5 -Symbol <SYMBOL.DWX> -TF <CARD_TIMEFRAME> -Env backtest
   ```
   This populates `framework/EAs/QM5_41235_wti-samecal-hampel5/sets/` with one setfile per
   symbol named `QM5_41235_wti-samecal-hampel5_<SYMBOL>_<TF>_backtest.set`. Without these the
...
9. Run exactly one smoke test on the first registered symbol and its generated setfile.
   ...
   pwsh -File C:\QM\repo\framework\scripts\run_smoke.ps1 `
     -EALabel QM5_41235_wti-samecal-hampel5 -Symbol <FIRST_REGISTERED_SYMBOL.DWX> `
     -Year 2024 -Terminal any -Period <CARD_TIMEFRAME> `
     -SetFile C:\QM\repo\framework\EAs\QM5_41235_wti-samecal-hampel5\sets\<GENERATED_SETFILE>.set `
...
Write **exactly one JSON object** to `D:\QM\strategy_farm\artifacts\builds\6a252d26-7752-4928-8aa8-7788761370b8.json` AND echo it to stdout.
No prose around it. Schema:

```json
{
  "task_id": "6a252d26-7752-4928-8aa8-7788761370b8",
  "ea_id": "QM5_41235",
  "ea_dir": "framework/EAs/QM5_41235_wti-samecal-hampel5",
...
Advisor will read the file at `D:\QM\strategy_farm\artifacts\builds\6a252d26-7752-4928-8aa8-7788761370b8.json`.
