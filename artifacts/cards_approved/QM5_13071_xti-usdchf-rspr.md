...
type: strategy
strategy_id: EIA-SNB-XTI-USDCHF-RSPREAD-2026
source_id: EIA-SNB-XTI-USDCHF-RSPREAD-2026
source_citation: "EIA oil/exchange-rate working paper and Swiss National Bank CHF safe-haven working paper."
source_citations:
  ...
  - type: central_bank_research
    citation: "Swiss National Bank Working Paper 2013-04. On financial risk and the safe haven characteristics of Swiss franc exchange rates."
    location: "https://www.snb.ch/en/publications/research/working-papers/2013/working_paper_2013_04"
    quality_tier: A
    role: chf_safe_haven
sources:
  - "[[sources/EIA-SNB-XTI-USDCHF-RSPREAD-2026]]"
concepts:
...
indicators:
  ...
  - "[[indicators/zscore]]"
  - "[[indicators/atr]]"
strategy_type_flags: [oil-fx-return-spread, market-neutral-basket, chf-safe-haven, zscore-reversion, atr-hard-stop, time-stop, low-frequency]
target_symbols: [XTIUSD.DWX, USDCHF.DWX]
basket_symbols: [XTIUSD.DWX, USDCHF.DWX]
markets: [XTIUSD.DWX, USDCHF.DWX]
primary_target_symbols: [XTIUSD.DWX, USDCHF.DWX]
single_symbol_only: false
logical_symbol: QM5_13071_XTI_USDCHF_RSPREAD_D1
period: D1
timeframes: [D1]
expected_trade_frequency: "D1 XTI/USDCHF CHF-terms return-spread z-score reversion; estimate 5-10 paired packages/year before Q02 validates synchronized history and fills."
...
g0_approval_reasoning: "Mission-directed G0 approval 2026-07-08: R1 PASS EIA and SNB official source packet; R2 PASS deterministic D1 XTI/USDCHF return-spread basket; R3 PASS XTIUSD.DWX and USDCHF.DWX exist in the DWX symbol universe; R4 PASS no ML/grid/martingale/external runtime data."
---

# XTI/USDCHF D1 Return-Spread Reversion

## Source

- Source: [[sources/EIA-SNB-XTI-USDCHF-RSPREAD-2026]]
...
- CHF channel: Swiss National Bank Working Paper 2013-04, "On financial risk
  and the safe haven characteristics of Swiss franc exchange rates", URL
...
the literature frames the link as bidirectional and time-varying rather than a
simple one-way forecast. The Swiss franc also has safe-haven characteristics,
with risk-factor sensitivity changing during stress. This card expresses a WTI
energy/CHF relative-value sleeve as a two-leg return-spread reversion package
instead of an outright WTI trend or a channel breakout.

`USDCHF` rises when CHF weakens against USD. The implemented dislocation is:

`return_spread = ln(XTI[t] / XTI[t-L]) + beta_usdchf * ln(USDCHF[t] / USDCHF[t-L])`

When WTI has unusually outperformed while USDCHF has also risen, the package
fades a "oil up while CHF weak" divergence by selling both WTI and USDCHF.
When WTI has unusually underperformed while USDCHF has also fallen, it buys
both WTI and USDCHF.

This is deliberately different from:

- `QM5_12835_wti-usdchf-brk`: same broad source family, but that EA trades a
  CHF-terms log-price channel breakout. This card trades fixed-window
  return-spread z-score reversion.
- `QM5_12834_wti-jpy-spread`: different safe-haven currency and mean-reversion
  definition.
- `QM5_13057_xti-gbpcad-rspr`, `QM5_13059_xti-audjpy-rspr`, and
  `QM5_13060_xti-eurcad-rspr`: different FX legs and different macro channel.
- `QM5_13011_xti-cadchf-rspr`: CAD/CHF commodity-currency cross, not direct
  WTI in USDCHF CHF terms.
- XAU/XAG, XNG RSI, XTI/XNG, Brent/WTI, oil/metals, index, and single-symbol
  WTI trend/seasonality sleeves.

## Markets And Timeframe

- Logical symbol: `QM5_13071_XTI_USDCHF_RSPREAD_D1`.
- Host symbol: `XTIUSD.DWX`.
- Basket legs: `XTIUSD.DWX` and `USDCHF.DWX`.
- Period: D1.
...
- Runtime data: Darwinex MT5 D1 OHLC, spread metadata, ATR, broker time, and
  V5 framework state only. No EIA, SNB, futures-curve, rate, macro CSV, API,
  analyst forecast, or ML model is consumed at runtime.

## Entry Rules

- Evaluate only on a new D1 bar of the `XTIUSD.DWX` host chart.
- Copy completed D1 closes for `XTIUSD.DWX` and `USDCHF.DWX`.
- Compute `xti_ret = ln(XTI close[1] / XTI close[1 + strategy_return_lookback_d1])`.
- Compute `usdchf_ret = ln(USDCHF close[1] / USDCHF close[1 + strategy_return_lookback_d1])`.
- Compute `return_spread = xti_ret + strategy_beta_usdchf * usdchf_ret`.
...
- Skip entries when `USDCHF.DWX` spread exceeds `strategy_usdchf_max_spread_pts`.
...
CHF safe-haven channel only. This card imports no source performance number.
Q02 and later phases must validate or reject the `XTIUSD.DWX` / `USDCHF.DWX`
...
- risk_class: medium-high because crude volatility, USDCHF liquidity, and
...
- [x] Non-duplicate: paired WTI/USDCHF safe-haven return-spread mean
  reversion, not the existing WTI/USDCHF channel breakout, WTI/JPY spread,
  XTI/CAD-cross baskets, XTI/XNG, Brent/WTI, metal-ratio, XNG, index, or
  outright WTI logic.
...
- Slot 0: `XTIUSD.DWX`.
- Slot 1: `USDCHF.DWX`.
- Use the logical basket setfile `QM5_13071_XTI_USDCHF_RSPREAD_D1` for Q02.
...
- trade_entry: D1 standardized XTI/USDCHF CHF-terms return-spread reversion.
...
trade, if Q02 PF is below 1.0 after costs, if synchronized XTI/USDCHF history
...
|---|---|---|---|
| v1 | 2026-07-08 | initial XTI/USDCHF return-spread basket build | Q02 | PENDING |
...
| Q01 Build Validation | 2026-07-08 | PENDING | `artifacts/qm5_13071_build_result.json` |
