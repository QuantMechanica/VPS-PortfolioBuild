...
This is deliberately different from existing WTI calendar, month, weekday, weekend-gap, WPSR, refinery, hurricane, OPEC, expiry, ETF-roll, SPR, CAD/oil, XTI/XNG, oil/gold, oil/silver, seasonal pseudo-price, 52-week anchor, box breakout, Donchian, RSI pullback, commodity reversal, XAU/XAG ratio, and commodity RSI logic.
...
- Long entry: SMA(`pro_change`, `strategy_flow_ma_period`) crosses above SMA(`public_change`, `strategy_flow_ma_period`) on the prior completed D1 bar.
- Short entry: SMA(`pro_change`, `strategy_flow_ma_period`) crosses below SMA(`public_change`, `strategy_flow_ma_period`) on the prior completed D1 bar.
- Enter at market on the next D1 bar after the confirmed cross.
- No entry if already open for this EA magic or if spread exceeds `strategy_max_spread_points`.

## 5. Exit Rules

- Stop loss: fixed hard SL at ATR(`strategy_atr_period`) * `strategy_atr_sl_mult`, frozen at entry.
- Signal reversal exit: close on the opposite Pro-Go line cross.
- Time exit after `strategy_max_hold_days` calendar days.
- Friday close remains enabled by the V5 framework.

## Strategy Allowability Check

- [x] R1 reputable source: SRC03 is an owner-approved Tier-A Williams source.
