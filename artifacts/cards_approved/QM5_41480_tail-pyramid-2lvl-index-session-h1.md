---
ea_id: QM5_41480
slug: tail-pyramid-2lvl-index-session-h1
type: strategy
source_id: QM-RESEARCH-2026-0008
source_type: internal_research
preregistration_sha256: c4f63ce4055df1b1fc3331571063cb35605559f317ee3779a81bd74ac97084db
source_hash: 91dd994d662e8feee88fb17ca67888a323c3adaeeffdbeb86c03b641c8fc50de
...
risk_contract: ../../strategy-seeds/sources/QM-RESEARCH-2026-0008/risk_contract.json
expected_dd_pct: 2.0
...
g0_approval_reasoning: "OWNER_DIRECT_SESSION_DELEGATION to Kimi, 2026-09-15/16 (interim strategy-engineering build of the fully mechanized QM-RESEARCH-2026-0008 H-PY2L card, TAIL_RISK Family A tightened variant of QM5_41479 / QM-RESEARCH-2026-0007). The parent H-PY card's third pyramid level was proven structurally unreachable inside the session-flat envelope by the 0007 L3 root-cause analysis (ANALYSIS_NOT_PREREGISTERED); this variant tightens the Family A bounds (max_levels 3->2, progression 1.0/0.75/0.5 -> 1.0/0.75, aggregate 2.25 -> 1.75 legs), never widens. Independent non-Kimi critique pending: claude disabled until 2026-09-17, codex on hold until 2026-09-19, agy quota-dead. Build-only authorization; no pipeline phase, no gate verdict, no live use. No registry row allocated for ea_id 41480 (verified free); the central allocator runs later (as 41475-77 did)."
...
# H-PY2L: bounded two-level pyramid on cash-session index trend (session-flat, H1)

## Source
- Source: [[sources/QM-RESEARCH-2026-0008]] — tightened TAIL_RISK Family A variant (bounded positive pyramiding, two levels), authored by Kimi (research role, offline) under the interim OWNER delegation 2026-09-15/16, per `docs/research/TAIL_RISK_PROGRAMME_2026-09-16.md` §4 and `tools/strategy_farm/config/tail_risk_families.v1.json`; variant lineage of [[sources/QM-RESEARCH-2026-0007]] (H-PY, QM5_41479, parent).
- Provenance: the base entry reuses the H-CW session-flat envelope ([[sources/QM-RESEARCH-2026-0002]], QM5_41475) — same cash-session opening-range breakout on NDX/GDAXI/SP500 H1, same shock/spread/news filter class, same flatten hour and breakers. The parent H-PY card added the Family A 3-level pyramid; its pilot filled level 2 in 11.9% of baskets and level 3 in 0%. The 0007 L3 root-cause analysis (`ANALYSIS.md`, ANALYSIS_NOT_PREREGISTERED, in-sample only) proved the third level is structurally unreachable inside the session-flat envelope (entries fill 17:00-18:00 UTC, earliest L2 fill is the 19:00 bar, adds may not fill into the flatten-hour bar, and L3 may only trigger on a bar strictly after the L2 fill — the trigger's price condition was met 13 times and the fill was mechanically barred every time; single-rule counterfactuals disabling the giveback stop, trail ratchet, time stop, or flatten exit all still produce zero L3 fills). H-PY2L is the family contract's own tightening response: max_levels 3 → 2, progression 1.0/0.75/0.5 → 1.0/0.75 (aggregate cap 2.25 → 1.75 legs), level-3 machinery removed; nothing widened.
- Pilot motivation (not proof): deterministic fire count of these exact two-level rules on an NDX-class feed (2018-2020 in-sample) — 6.4 baskets/month, ~6.4 active days/month, raw PF(R) 1.26 at +0.12R/basket, level-2 reach 11.9%, giveback-before-level-2 15.1% (computed output `h_py2l_pilot.json`, sha256 in the source manifest). Every figure equals the parent pilot because the parent's third level never filled (parent consistency check all_match in the JSON).
- Preregistration: `c4f63ce4055df1b1fc3331571063cb35605559f317ee3779a81bd74ac97084db` (strategy-seeds/sources/QM-RESEARCH-2026-0008/preregistration.json, schema qm.research-preregistration/v1, version 1; mechanical spec sha256 b8a85a48...f681b over H_PY2L_card.md).
...
- Machine-readable risk contract: `risk_contract.json` (same directory as the source store; byte-identical to the fenced contract in H_PY2L_card.md; validated with `strategy_risk_contract.py`: `validate_contract=[]`, `is_unbounded=False`).

## Structural cause
Equity-index CFDs exhibit persistent intraday order flow during the cash-session overlap (deepest liquidity, tightest spread, strongest autocorrelated tape). When the session opening range breaks and the breakout holds above/below a short intraday EMA, the directional flow that funded the break frequently persists for several more hours — long enough to pay a trailed pyramid, and long enough that a giveback stop set at 50% of peak open basket profit monetizes the fat right tail of those sessions instead of donating it back at the close. The mechanism is convexity, not prediction: the add concentrates exposure only after the tape has already moved in favour (an ATR-scaled open profit and a new favourable extreme are required), so the position is largest exactly when the session trend has confirmed itself, and the never-widening basket stop converts parabolic reversals into small realised givebacks rather than open-ended losses. Confining the whole construction to the cash window and forcing flat before the thin rollover removes the two tails that kill FTMO accounts — overnight gap and swap — which is precisely the tail a built-out basket fears most. The two-level form is the honest shape of the mechanism inside the session-flat envelope: the session clock leaves room for exactly one add.

## Price signature
During the cash-session window price breaks the range of the first session bars and continues in the breakout direction while holding on the confirmed side of a short intraday EMA. A favourable close stack first prints +1.0x ATR of open profit with a new favourable extreme (level 2 fills). The signature ends in one of three shapes: a vertical extension whose giveback from the peak triggers the basket exit at +50% retained profit, a drift that the trailing stop later harvests, or a chop that stops the base leg at its ATR stop or runs into the session-flat flatten.
...
- There is no third level. Hard bound: never more than 2 legs, never more than 1.75 base legs aggregate; the add logic fails closed at the bound.
...
### Position sizing
Level 1 risks `risk_per_trade_pct` of equity at its reference stop (RISK_FIXED in backtest, RISK_PERCENT live; family bound: never above 0.25%). Level 2 sizes at `level2_size_mult` of base (Family A tightening 1.0 / 0.75) — a fully built basket carries 1.75 base legs and at most 5% margin. One pyramid per symbol per day; at most `max_positions_total` concurrent baskets, so worst-case planned stop risk is bounded by the daily/weekly breakers even with every basket fully built. Family-contract values: level weights 1.0 / 0.75, 1.75-leg aggregate, 5% margin cap, 2-level cap (a tightening of the family 3-level envelope, never a widening); the 0.5% adverse / 50% giveback bounds are upper bounds (the card may tighten, never widen).
...
## Deterministic risk contract (Family A tightened to two levels — carried for programme uniformity)
Positive pyramiding is NOT tail-amplifying under STRATEGY_ELIGIBILITY_V2 §9, so no contract is required at intake; the complete contract is carried anyway (programme uniformity + the joint-tail engine reads declared bounds from every sleeve). Machine copy: `risk_contract.json` in the source store.

| Field | Value |
|---|---|
| schema | qm.strategy-risk-contract/v1 |
| tail_amplifying | false (mechanism_flags: [positive_pyramiding]) |
| max_levels | 2 (family 3-level envelope tightened, never widened) |
| sizing_progression | custom [1.0, 0.75]; aggregate 1.75 initial legs |
| max_open_positions | 3 (per basket) |
| max_basket_exposure / max_gross_notional | 1.75 (initial-leg units) |
| max_margin_pct | 5.0 |
...
## Expected frequency
At most one basket per day per symbol by construction. The deterministic pilot fire count of these exact two-level rules (NDX-class Dukascopy feed, 2018-2020 in-sample) recorded: 489 evaluated session days (154 shock-skipped at 3.0x), 187 gross signals, 185 taken baskets, 6.4 baskets/month and ~6.4 active days/month over 29 months; exits 73 base stops, 37 giveback stops, 75 session-flat flattens; level 2 filled in 11.9% of baskets; raw outcome PF(R) 1.26 at +0.12R/basket, average peak open basket profit +0.77R, maximum +19.1R. These figures equal the parent 0007 pilot because the parent's third level never filled (parent consistency check all_match in `h_py2l_pilot.json`). These are honest pilot measurements for density calibration and feasibility only — IN-SAMPLE, ZERO-COST, SINGLE-FEED. Expect roughly 6-7 baskets per month per symbol, about 12-20 baskets per month across the three index symbols, with at least 8 active days per month — the same density order as the H-CW shell it reuses.

## Regime of failure
Trend exhaustion and clustered session reversals. A parabolic reversal after the add prints converts the largest aggregate position at the worst prices — bounded by the breakeven-plus trail and the giveback stop, but repeated giveback exits are death by sequence. Correlated index symbols breaking out together concentrate the same-session giveback across baskets (NDX/GDAXI/SP500 share the cash-session factor). Choppy range sessions produce repeated small base-stop and giveback losses. The regime of failure is exactly what the preregistered kill criteria measure.

## Invalidation conditions
Refuted if, out of sample, the bounded two-level pyramid does not beat its own flat base-leg baseline (H-CW shape) on expectancy or worst-day loss, or if the edge depends on a single symbol or single period. Refuted at programme level if the joint-tail protocol shows the family's worst days systematically coincide with the passive book's or the H-CW/H-MR sleeves' worst days (hidden common mode — the family is the tail carrier and must not join a multi-sleeve book).
...
7. The tightened mechanism never fires out of sample: level-2 reach < 2% of sealed baskets.
...
## FTMO fit
Zero overnight exposure (swap ~ 0, no gap-through-stop on a built-out basket), a hard 0.5% equity basket bound plus a -1.0% daily / -2.0% weekly breaker stack far inside the FTMO 5% daily and 10% total drawdown limits, and the high-impact news blackout keeps FOMC/NFP/CPI windows closed. The session-flat envelope is precisely the defence against the -10.26% demo breach pattern (overnight/swap on carried positions), and the bounded two-level pyramid adds convex trend capture inside it rather than a new tail. As the aggressive family admitted to FTMO evaluation at programme stage (Family A is FTMO-primary per the programme), it stacks inside the 4% effective joint daily-loss budget with the passive book — but only after the joint-tail protocol clears it against the H-CW/H-MR streams.
...
|-----------|--------|------------|
| R1 Source-Link | PASS | Internal research source QM-RESEARCH-2026-0008 (variant of QM-RESEARCH-2026-0007) with lineage, manifest-backed numeric provenance, and preregistration hash (c4f63ce4...84db). Cross-vendor critic PENDING (REVIEW_PENDING). |
| R2 Mechanical | PASS | Opening-range breakout base leg, ATR/EMA-gated add, never-widening ratchet basket stop, giveback stop, equity adverse bound, time stop, flatten hour, filters, and breakers are all explicit closed-bar rules. |
...
| R4 No ML | PASS | Fixed mechanical parameters, no ML, no online adaptation, one basket per symbol per day, the add only at confirmed profit; no martingale/grid/negative pyramiding. |
...
## Pipeline history
- G0: 2026-09-16, APPROVED under OWNER_DIRECT_SESSION_DELEGATION to Kimi (review_status REVIEW_PENDING; independent non-Kimi critique gated: claude disabled until 2026-09-17, codex on hold until 2026-09-19, agy quota-dead). ea_id 41480 reserved by filename convention only — verified free before writing (zero references in artifacts/, framework/, deploy-manifests/, docs/); no registry row allocated, matching the 41479 card convention — the central allocator runs later.

## Related strategies
- QM5_41479 tail-pyramid-index-session-h1 (H-PY 3-level parent; QM-RESEARCH-2026-0007 — this card tightens its Family A bounds to two levels, never widens).
- QM5_41475 cash-window-index-continuation-h1 (H-CW; the base entry envelope this card reuses).
- QM-RESEARCH-2026-0006 H-MR cash-open index mean reversion (session sibling; same feed/pilot conventions).
- QM-RESEARCH-2026-0002 (H-CW parent campaign context).

## Lessons Learned
- The parent's level-3 reach (0/185) looked like rarity but was a structural degeneracy: entry clock + "no add fills into the flatten-hour bar" + "L3 only after the L2 fill bar" jointly make the third level unreachable in the session-flat envelope. Counterfactual single-rule replays (no giveback / no trail / no time stop / no flatten exit) all still produced zero L3 fills — attribution before redesign. A design-level dead level is tightened out of the contract, not optimized around.
