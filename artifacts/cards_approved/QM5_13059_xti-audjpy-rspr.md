---
ea_id: QM5_13059
slug: xti-audjpy-rspr
type: strategy
strategy_id: EIA-RBA-JAPAN-XTI-AUDJPY-2026
source_id: EIA-RBA-JAPAN-XTI-AUDJPY-2026
source_citation: "EIA oil/exchange-rate working paper, RBA AUD exchange-rate explainer, and EIA Japan energy importer context."
source_citations:
  ...
  - type: government_energy_context
    ...
    location: "https://www.eia.gov/international/content/analysis/countries_long/japan/japan.pdf"
    quality_tier: A
    role: jpy_importer_context
sources:
  - "[[sources/EIA-RBA-JAPAN-XTI-AUDJPY-2026]]"
concepts:
  - "[[concepts/oil-audjpy-relative-value]]"
...
indicators:
  ...
  - "[[indicators/zscore]]"
  - "[[indicators/atr]]"
strategy_type_flags: [oil-fx-return-spread, market-neutral-basket, commodity-currency, importer-fx-leg, audjpy-positive-quote, zscore-reversion, atr-hard-stop, time-stop, low-frequency]
target_symbols: [XTIUSD.DWX, AUDJPY.DWX]
basket_symbols: [XTIUSD.DWX, AUDJPY.DWX]
markets: [XTIUSD.DWX, AUDJPY.DWX]
primary_target_symbols: [XTIUSD.DWX, AUDJPY.DWX]
single_symbol_only: false
logical_symbol: QM5_13059_XTI_AUDJPY_RSPREAD_D1
period: D1
timeframes: [D1]
expected_trade_frequency: "D1 XTI/AUDJPY return-spread z-score reversion; estimate 6-12 paired packages/year before Q02 validates synchronized history and fills."
...
g0_approval_reasoning: "Mission-directed G0 approval 2026-07-08: R1 PASS EIA and RBA source packet; R2 PASS deterministic D1 XTI/AUDJPY return-spread basket; R3 PASS XTIUSD.DWX and AUDJPY.DWX exist in the DWX matrix; R4 PASS no ML/grid/martingale/external runtime data."
---

# XTI/AUDJPY D1 Return-Spread Reversion

## Source

- Source: [[sources/EIA-RBA-JAPAN-XTI-AUDJPY-2026]]
...
- Support: EIA Japan Country Analysis Brief, URL
...
the relationship is regime-dependent and not a one-way predictor. This card
trades a two-leg relative-value package instead of an outright WTI forecast.

`AUDJPY` gives the basket a positive AUD commodity/risk leg against an
energy-importer currency. The implemented dislocation is:

`return_spread = ln(XTI[t] / XTI[t-L]) - beta_audjpy * ln(AUDJPY[t] / AUDJPY[t-L])`

When WTI has unusually outperformed AUDJPY, the package fades that divergence
by selling WTI and buying AUDJPY. When WTI has unusually underperformed
AUDJPY, it buys WTI and sells AUDJPY.
...
- `QM5_13010_xti-cadjpy-rspr`, `QM5_13011_xti-cadchf-rspr`,
  `QM5_13034_xti-audcad-rspr`, and `QM5_13057_xti-gbpcad-rspr`: CAD-cross
  oil/FX baskets with different quote mechanics.
- `QM5_13029_gbpcad-gbpnzd-coint` and `QM5_13058_audcad-gbpnzd-coint`:
  FX-only cointegration, not an energy/FX WTI sleeve.
...
- Basket legs: `XTIUSD.DWX` and `AUDJPY.DWX`.
- Period: D1.
...
- Runtime data: Darwinex MT5 D1 OHLC, spread metadata, ATR, broker time, and
  V5 framework state only. No EIA, RBA, futures-curve, inventory, import,
...
- Copy completed D1 closes for `XTIUSD.DWX` and `AUDJPY.DWX`.
- Compute `xti_ret = ln(XTI close[1] / XTI close[1 + strategy_return_lookback_d1])`.
- Compute `audjpy_ret = ln(AUDJPY close[1] / AUDJPY close[1 + strategy_return_lookback_d1])`.
- Compute `return_spread = xti_ret - strategy_beta_audjpy * audjpy_ret`.
...
- Skip entries when `AUDJPY.DWX` spread exceeds `strategy_audjpy_max_spread_pts`.
...
- name: strategy_beta_audjpy
  default: 0.70
  sweep_range: [0.45, 0.70, 0.95]
...
- name: strategy_audjpy_max_spread_pts
  default: 80
  sweep_range: [50, 80, 140]

## Author Claims

The source packet establishes structural lineage for oil/exchange-rate,
Australian-dollar commodity exposure, and Japan energy-import context only.
This card imports no source performance number. Q02 and later phases must
validate or reject the `XTIUSD.DWX` / `AUDJPY.DWX` basket on Darwinex bars.
...
- risk_class: medium-high because crude volatility, AUDJPY regime shifts, and
...
- [x] R1 reputable source: U.S. government energy research plus RBA central
  bank AUD commodity-currency research and EIA Japan energy context.
...
- [x] R3 testable: `XTIUSD.DWX` and `AUDJPY.DWX` exist in the Darwinex symbol
  matrix.
- [x] R4 compliant: no ML, adaptive PnL fitting, grid, martingale, external
  runtime feed, or more than one position per magic slot.
- [x] Non-duplicate: paired WTI/AUDJPY return-spread mean reversion, not
  WTI/USDCAD, WTI/CADJPY, WTI/CADCHF, WTI/AUDCAD, WTI/GBPCAD, WTI/AUDUSD,
  XTI/NZD, XTI/XNG, Brent/CAD, Brent/WTI, metal-ratio, XNG, index, or outright
  WTI logic.
...
- Slot 0: `XTIUSD.DWX`.
- Slot 1: `AUDJPY.DWX`.
- Use the logical basket setfile `QM5_13059_XTI_AUDJPY_RSPREAD_D1` for Q02.
...
- trade_entry: D1 standardized XTI/AUDJPY return-spread reversion.
...
trade, if Q02 PF is below 1.0 after costs, if synchronized XTI/AUDJPY history
...
|---|---|---|---|
| v1 | 2026-07-08 | initial XTI/AUDJPY return-spread basket build | Q02 | PENDING |
...
| Q01 Build Validation | 2026-07-08 | PENDING | `artifacts/qm5_13059_build_result.json` |
