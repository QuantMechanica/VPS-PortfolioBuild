---
strategy_id: AI-CODEX-FX-COINT66-20260609-GBPJPY-EURGBP
ea_id: QM5_20201
slug: gbpjpy-eurgbp
type: strategy
status: APPROVED
...
markets: [forex]
timeframes: [D1]
primary_target_symbols: [GBPJPY.DWX, EURGBP.DWX]
target_symbols: [GBPJPY.DWX, EURGBP.DWX]
logical_symbol: QM5_20201_GBPJPY_EURGBP_COINTEGRATION_D1
period: D1
...
# QM5_20201 GBPJPY/EURGBP D1 Cointegration Basket
...
low-frequency daily implementation. Chan makes no claim for GBPJPY/EURGBP.
...
GBPJPY/EURGBP ranks eighteenth of 66 by OOS net Sharpe. Rank 17
AUDJPY/EURAUD is already built as `QM5_20200` and has terminal Q02 FAIL
evidence. The deterministic repository dedup check and exact-pair searches
found no dedicated fixed-beta GBPJPY/EURGBP D1 card, EA, registry allocation,
or logical basket manifest before this card.
...
The sub-0.8 OOS Sharpe is adverse evidence, and the approximately 83-bar
half-life implies a long holding horizon with material swap and regime risk.
...
Broad multi-symbol GBP or European FX systems and conversion-history
...
`ln(GBPJPY) - (-1.681438352) * ln(EURGBP)`. The negative fitted coefficient
...
GBP appears as the base of GBPJPY and the quote of EURGBP, but the package
retains EUR, JPY, carry, and risk-sentiment exposure. That sign-sensitive
structure and the negative beta are explicit high-risk caveats rather than
permission to add a filter.

## Hypothesis

The fixed scan suggests large deviations in the GBPJPY/EURGBP residual may
...
half-life is very slow, the hypothesis is deliberately weak and falsifiable:
...
`GBPJPY.DWX` is the tester host and first traded leg. `EURGBP.DWX` is the
...
- Compute `spread = ln(GBPJPY) - strategy_beta * ln(EURGBP)`, with frozen
  `strategy_beta = -1.681438352`.
...
Chan supplies the pair-trading method but reports no GBPJPY/EURGBP result.
...
data_requirements: "GBPJPY.DWX, EURGBP.DWX, USDJPY.DWX, GBPUSD.DWX, and EURUSD.DWX D1 histories"
```
...
| Q01 Build Validation | 2026-08-01 | PASS | `D:\QM\reports\framework\21\build_check_20260801_205745.json` |
| Q02 Baseline Screening | 2026-08-01 | PENDING | work item `3de84ccb-ded5-4289-9283-a52df4e3551e` |

## 16. Lessons Captured

- 2026-08-01: Rank 18 is below the original survivor bar and has a slow
