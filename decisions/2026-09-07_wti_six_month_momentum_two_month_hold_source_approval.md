# WTI Six-Month Momentum / Two-Month Hold — Source Approval
...
- Proposed slug: `wti-tsmom6-h2`
- Strategy ID: `MOP-TSMOM-2012_XTI_K6H2_S33`
- Parent source: `strategy-seeds/sources/MOP-TSMOM-2012/source.md`
- Dedup receipt: `artifacts/qm5_wti_tsmom6_h2_preallocation_dedup_20260907.json`
...
WTI `k=6, h=2` result. No paper return, significance, alpha, Sharpe ratio, CFD
equivalence, trade count, or diversification result transfers.

## Locked Mechanic

1. Trade preset-bound `XTIUSD.DWX` on D1 only.
2. At the first D1 bar of each odd-numbered broker month, reconstruct seven
   consecutive completed broker-month-end closes.
3. Compute `ln(C[6] / C[0])`; buy for a strict positive value, sell for a
...
The canonical checker scanned 4,862 registry rows and 1,475 cards. It found no
exact identity and returned expected family matches; the unavailable external
Wiki root remains explicit in the receipt. `QM5_20059` uses six-month
formation but renews every month. `QM5_20281` uses the same fixed odd-month
two-month clock but twelve-month formation; `QM5_41379` through `QM5_41381`
use three-, one-, and nine-month formation. This identity requires both exact
six-month endpoints and the non-overlapping two-month lifecycle.

Verdict: `DISTINCT_WTI_K6H2_NONOVERLAPPING_MONTHLY_TSMOM`.
