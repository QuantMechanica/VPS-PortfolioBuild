# WTI Same-Calendar Arctan-Weighted Location — Source Approval
...
- proposed slug: `wti-samecal-arctan5`
- proposed strategy ID:
  `KELOHARJU-SCIPY-WTI-SAMECAL-ARCTAN5-2026_S01`
- proposed source ID: `KELOHARJU-SCIPY-WTI-SAMECAL-ARCTAN5-2026`
...
- state: a fixed 32-update arctangent-loss iteratively reweighted location
  over the exact prior five matching-calendar-month WTI log returns,
  initialized at their median with one frozen rescaled-MAD scale
...
The following durable repository records and bounded primary record were read
...
3. The complete `scipy.optimize.least_squares` reference page maintained by
   ...
   reviewed again 2026-08-31. The retrieved 93,197-byte HTML had SHA-256
   `CD8BCEEF256035736DDDE8E0F690C2487EFEFD5AFA773AEB50C822E5AF632435`.
   It fixes the arctangent loss as `rho(z)=arctan(z)`, states that it bounds
   the loss contributed by one residual, and fixes scale application as
   `C^2*rho(f^2/C^2)`.
4. The complete governed Cauchy-source packet
   ...
   It preserves the already reviewed rescaled-MAD, median-start, frozen-scale,
   fixed-iteration robust-location convention and its local-solution risk.
...
arctangent loss and scale convention. The governed Cauchy packet supplies
only the already reviewed deterministic robust-location plumbing.
...
3. Preserve returns in chronological year order and sort only copies:
   ...
   The weight is the first derivative of SciPy's documented
   `rho(z)=arctan(z)` under `z=u^2`: `rho'(z)=1/(1+z^2)`. Reject nonpositive
   or nonfinite MAD, scale, weight, weight sum, weighted sum, or intermediate
   location.
...
8. Never retry, scale in, pyramid, grid, martingale, optimize, refit scale,
   ...
   substitute a raw mean, median, Cauchy, Huber, Hampel, bisquare, trim,
   Winsor, capped, or order-statistic location.

Exact calendar-year membership, return orientation, median and raw MAD,
rescaled frozen scale, arctangent derivative weight, 32 updates, epsilon,
...
- R1 `PASS_WITH_ARCTAN_CONJUNCTION_AND_SINGLE_CFD_TRANSLATION_RISK`: two
  ...
  documentation fixes the arctangent loss and scale convention. The
...
scanned 4,737 registry identities, 1,375 cards, and all 45 current Strategy
Wiki nodes. It found no exact identity and surfaced expected fuzzy
same-calendar, Cauchy, Hampel, and Huber relatives for mandatory manual
review.

For sorted returns `[-0.095,-0.045,-0.005,+0.050,+0.060]`, raw MAD is
`0.055`, frozen scale is `0.081543`, and the locked arctangent iteration ends
at approximately `+0.006280955600`, so this card buys. On the otherwise
identical fixed-scale path, the Cauchy sibling ends at approximately
`-0.002436516741`; the raw mean is `-0.007` and the median is `-0.005`, so all
three sell. Sign reflection reverses every strict mapping.

The arctangent derivative weight is `1/(1+u^4)`, whereas Cauchy is
`1/(1+u^2)`. Both remain positive for every finite residual, but arctangent
suppresses large standardized residuals quartically rather than quadratically
and converges to the opposite side on the locked fixture. Huber has
inverse-linear tails and ten exact years; bisquare and Hampel reach exact zero
tail weight under different curves. The curve and fixed local path therefore
alter actual participation rather than rename a parameter.

Verdict:
`FUZZY_FAMILY_MATCHES_RESOLVED_AS_SEMANTICALLY_DISTINCT_WTI_EXACT_FIVE_YEAR_SAME_CALENDAR_FROZEN_SCALE_ARCTAN_LOCATION_SIGN_MONTHLY_SLEEVE`.
