# WTI Five-Month Momentum / Two-Month Hold — Source Approval
...
- Proposed slug: `wti-tsmom5-h2`
- Strategy ID: `MOP-TSMOM-2012_XTI_K5H2_S36`
- Parent source: `strategy-seeds/sources/MOP-TSMOM-2012/source.md`
- Dedup receipt: `artifacts/qm5_wti_tsmom5_h2_preallocation_dedup_20260908.json`
...
WTI `k=5, h=2` result. No paper return, significance, alpha, Sharpe ratio, CFD
equivalence, trade count, or diversification result transfers.

## Locked Mechanic

1. Trade preset-bound `XTIUSD.DWX` on D1 only.
2. At the first D1 bar of each odd-numbered broker month, reconstruct six
   consecutive completed broker-month-end closes.
3. Compute `ln(C[5] / C[0])`; buy for a strict positive value, sell for a
...
The canonical checker scanned 4,865 registry rows, 1,478 cards, and 45
...
formation; `QM5_41379` through `QM5_41384` use three-, one-, nine-, six-,
four-, and two-month formation. This identity requires both exact five-month
endpoints and the non-overlapping two-month lifecycle.

Verdict: `DISTINCT_WTI_K5H2_NONOVERLAPPING_MONTHLY_TSMOM`.
