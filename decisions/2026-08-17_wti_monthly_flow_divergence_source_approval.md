# WTI Monthly Public/Session Flow Divergence - Source Approval
...
- proposed slug: `wti-mflow-div`
- proposed strategy ID: `WILLIAMS-MOP-WTI-MFLOWDIV-2026_S01`
- proposed source ID: `WILLIAMS-MOP-WTI-MFLOWDIV-2026`
...
`strategy-seeds/sources/WILLIAMS-MOP-WTI-MFLOWDIV-2026/source.md` was read
...
2. Moskowitz, Ooi, and Pedersen (2012), "Time Series Momentum," *Journal of
   ...
   returns by information time or validate following session flow during
...
8. BUY only when `session_flow > 0` and `overnight_flow < 0`. SELL only when
   ...
   magnitude never changes size or direction.
...
selector, moving line, crossover, or total-flow direction is authorized.
...
- `QM5_41034_wti-mflow-agree` uses the same completed-month endpoints but
  ...
  opposition, and follows session flow.
...
- `QM5_41033_wti-flow-dom` trades one exact opposed-flow week in the sign of
  the reconciled total/dominant component. This candidate always follows the
  session component and can therefore take the opposite side of its completed
  month when overnight magnitude dominates.
