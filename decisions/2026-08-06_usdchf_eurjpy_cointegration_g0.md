# USDCHF/EURJPY Cointegration G0 Authorization
...
umbrella cointegration EA: `USDCHF.DWX` / `EURJPY.DWX`, D1. Development may
...
implementation. Chan makes no USDCHF/EURJPY performance claim.
...
across 4,312 registry rows and 429 direct cards. No pair-named EA directory,
exact-pair setfile, or dedicated two-leg basket manifest implements
USDCHF/EURJPY.

Rank 63 (`USDCHF.DWX` / `EURAUD.DWX`) is already built as `QM5_20252`.
Six broad basket manifests contain both rank-64 symbols only as unrelated
universe members. The dynamic cointegration umbrellas `QM5_1156` and
`QM5_1257` do not contain EURJPY in their candidate universes and therefore do
not implement this relationship.

## Structural and kill boundary

- Fixed spread: `ln(USDCHF) - (-0.075286902527) * ln(EURJPY)`.
...
The path-aware sample at `2026-08-06T22:01:13Z` observed five factory
terminals (`T1`, `T2`, `T5`, `T6`, and `T8`), below the binding ceiling of
