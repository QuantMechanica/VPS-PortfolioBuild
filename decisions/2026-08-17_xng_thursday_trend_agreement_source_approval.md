# XNG Standard-Thursday Event / Slow-Trend Agreement — Source Approval
...
- proposed slug: `xng-thu-trend-agree`
- proposed strategy ID: `EIA-MOP-XNG-THUTRENDAGREE-2026_S01`
- source ID: `EIA-MOP-XNG-THUTRENDAGREE-2026`
...
- decision clock: first executable broker-Friday tick after an exact standard
  Thursday
- signal: strict sign agreement between the completed Thursday close-to-close
...
`strategy-seeds/sources/EIA-MOP-XNG-THUTRENDAGREE-2026/source.md` was read
...
agreement conjunction or establish an XNG-specific result for it. No
...
7. Require both returns finite, nonzero, and strictly equal in sign. Positive
   agreement buys XNG; negative agreement sells XNG. Disagreement or equality
   consumes Friday flat. Magnitude never scales size.
...
The canonical checker scanned 4,535 registry rows and 625 root card files. It
...
- `QM5_41047_xng-thu-trend-pb` requires event/slow-trend opposition and trades
  the slow-trend sign; this candidate requires agreement and follows the common
  sign.
- `QM5_20163_xng-thu-trend` enters before Thursday, is short-only, and uses no
  completed event return; this candidate waits until Friday and is symmetric.
- `QM5_41043_xng-thu-flow-agree` compares Thursday close-to-open and
  open-to-close components; this candidate compares the whole completed event
  return with a non-overlapping pre-event 252-session trend.
- `QM5_41044_xng-thu-flow-fade` requires opposed Thursday components and fades
  their dominant move; this candidate requires cross-horizon agreement and
  continues it.
- `QM5_12584_eia-xng-storage` uses event-day reaction magnitude rather than a
  separate slow-trend sign.
- `QM5_20124/20128/20132` use M30 release-window price sequences.
