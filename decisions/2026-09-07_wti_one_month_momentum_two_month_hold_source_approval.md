# WTI One-Month Momentum / Two-Month Hold — Source Approval
...
- Proposed slug: `wti-tsmom1-h2`
- Strategy ID: `MOP-TSMOM-2012_XTI_K1H2_S31`
- Parent source: `strategy-seeds/sources/MOP-TSMOM-2012/source.md`
- Dedup receipt: `artifacts/qm5_wti_tsmom1_h2_preallocation_dedup_20260907.json`
...
a standalone WTI `k=1, h=2` result. No paper return, significance, alpha,
Sharpe ratio, CFD equivalence, trade count, or diversification result transfers.

## Locked Mechanic

1. Trade preset-bound `XTIUSD.DWX` on D1 only.
2. At the first D1 bar of each odd-numbered broker month, reconstruct two
   consecutive completed broker-month-end closes.
3. Compute `ln(C[1] / C[0])`; buy for a strict positive value, sell for a
...
The canonical checker scanned 4,860 registry rows and 1,473 cards. It found no
exact identity and returned expected family matches; the unavailable external
Wiki root remains explicit in the receipt. `QM5_20187` uses the same one-month
formation but renews every month. `QM5_20281` uses the same fixed odd-month
two-month clock but a twelve-month formation; `QM5_41379` uses three months.
This identity requires both exact one-month endpoints and the non-overlapping
two-month lifecycle.

Verdict: `DISTINCT_WTI_K1H2_NONOVERLAPPING_MONTHLY_TSMOM`.
