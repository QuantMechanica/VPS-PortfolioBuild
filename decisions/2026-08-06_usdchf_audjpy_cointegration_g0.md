# USDCHF/AUDJPY Cointegration G0 Authorization
...
umbrella cointegration EA: `USDCHF.DWX` / `AUDJPY.DWX`, D1. Development may
...
implementation. Chan makes no USDCHF/AUDJPY performance claim.
...
The negative DEV and OOS results, roughly 97-bar half-life, and sub-floor
inferred cadence are adverse findings, not parameters to repair. A terminal
Q02 economic or cadence failure retires the exact sleeve.

## Non-duplicate decision

The deterministic research check found no exact slug or strategy-ID collision
across 4,307 registry rows and 424 direct cards. Three fuzzy matches were
manually resolved as sibling relationships sharing only one leg. A separate
review of tracked basket manifests found no exact USDCHF/AUDJPY traded pair.

Rank 60 (`USDJPY.DWX` / `EURGBP.DWX`) is already built as `QM5_20246`.
`QM5_1156_caldeira-cointegration-pairs-fx` contains USDCHF but its six-symbol
universe does not contain AUDJPY. Broad baskets containing both symbols as
unrelated universe members do not implement this fixed D1 residual.

## Structural and kill boundary

- Fixed spread: `ln(USDCHF) - (-0.027722525061) * ln(AUDJPY)`.
...
- Two ATR(20) x 2.0 hard stops, atomic package entry, partial-entry rollback,
  and orphan cleanup.
- `AUDUSD.DWX` and `USDJPY.DWX` are conversion-history-only; neither receives
...
The path-aware sample at `2026-08-06T17:47:16Z` observed five factory
terminals (`T2`, `T3`, `T5`, `T6`, and `T9`), below the binding ceiling of
