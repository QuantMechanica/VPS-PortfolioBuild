# Q00 Decision - QM5_41116 XAU/XAG Completed-Month Three-Block Vote Reversion
...
Approved card:
`strategy-seeds/cards/approved/QM5_41116_xauxag-mthirdvote-rv_card.md`.

## Identity

- EA ID: `QM5_41116`, allocated atomically in the deterministic registry and
  committed at `3bd9a13a9`;
- slug: `xauxag-mthirdvote-rv`;
- strategy ID: `SCHWEIKERT-CME-XAUXAG-MTHIRDVOTE-RV-2026_S01`;
- source ID: `SCHWEIKERT-CME-XAUXAG-MTHIRDVOTE-RV-2026`;
- source authorization: `d853ac635`;
- bounded source extraction: `8da1fe0e4`;
- host: exact `XAUUSD.DWX`, D1, slot 0, planned magic `411160000`;
- companion: exact `XAGUSD.DWX`, D1, slot 1, planned magic `411160001`; and
- mechanic: fade the strict majority sign of three exhaustive chronological
  cumulative relative-return blocks inside the immediately completed month.
...
`approved=true` for `QM5_41116` after the registered custom-history admission
check and stamped expected frequency eleven/year, PF prior 1.01, drawdown prior
...
- R1 `TIER_A_WITH_MONTHLY_THREE_BLOCK_TRANSLATION_RISK`: the bounded child
  source preserves named peer-reviewed DOI and official-exchange lineage. The
  within-month three-block vote is disclosed as an untested QM translation.
- R2 `PASS`: exact synchronized month clock, two consecutive completed
  ...
  `floor(n/3)` and `floor(2*n/3)` endpoints, exhaustive adjacent-return
  partition, strict two-of-three sign vote, zero abstention, inverse pair
  sides, durable attempt, aggregate risk, hard stops, atomicity, spread gates,
...
The fail-closed pre-allocation checker scanned 4,612 EA-registry identities,
1,284 repository cards, and 45 Strategy-Wiki nodes. It found no exact
collision and one fuzzy family neighbor,
`QM5_41112_xauxag-mdaybreadth-rv`. Manual semantic review separates the
candidate from:

- `QM5_41112_xauxag-mdaybreadth-rv`, which counts every individual daily
  relative-return sign and requires a strict sign majority plus endpoint
  agreement. This card casts exactly three cumulative block votes and has no
  endpoint filter;
- `QM5_41113_xauxag-mhalfagree-rv`, which requires unanimity across two
  cumulative halves. This card accepts one opposing block through a strict
  two-of-three majority;
- `QM5_41115_wti-mthirdvote-mom`, which is single-symbol WTI continuation.
  This card computes XAU-minus-XAG relative returns, takes the inverse side,
  and owns an equal-notional two-leg basket;
- `QM5_20260_xauxag-mom-vote`, whose one-, three-, and twelve-month
  cross-sectional rank vote follows the winner rather than fading three
  blocks inside one month;
...
- rolling-center, residual, robust-score, and tail cards `QM5_12577`,
  `QM5_20157`, `QM5_20161`, `QM5_20263`, and `QM5_20268`; and
...
strict two-of-three sign vote, magnitude-blind inverse package side, consumed
...
45 Wiki nodes. Its only exact hits are the newly reserved `QM5_41116` slug and
...
- `a=floor(n/3)`, `b=floor(2*n/3)`, first block `Q[a-1]-P`, second block
  `Q[b-1]-Q[a-1]`, third block `Q[n-1]-Q[b-1]`, and zero abstention;
- SELL XAU/BUY XAG only when at least two blocks are positive and BUY XAU/SELL
  XAG only when at least two blocks are negative;
...
- both news axes OFF, Friday close OFF, next-month closure, and a forty-day
  stale guard; and
- deterministic partition/mechanic tests, strict compile, set/registry checks,
  `basket_manifest.json`, and static Q01 validation before Q02 handoff.

No current-month signal price, alternate partition, daily-sign count,
full-month endpoint filter, fitted center, regression, magnitude threshold,
signal-strength sizing, volatility/volume/season/weekday/event filter,
external data, retry, pending entry, target, trail, scale-in, grid, martingale,
pyramid, overlay hedge, or after-result rescue is approved.
...
asynchronous or duplicated endpoints, wrong chronology or partition indices,
duplicated or omitted adjacent returns, wrong zero handling or vote, wrong
relative side, current-month leakage, repeated attempt, invalid risk mode,
incomplete package, missing stop, wrong month lifecycle, or nondeterminism.
