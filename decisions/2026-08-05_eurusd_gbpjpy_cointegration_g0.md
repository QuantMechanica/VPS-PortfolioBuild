# EURUSD/GBPJPY Cointegration G0 Authorization
...
the current OOS-Sharpe frontier: `EURUSD.DWX` / `GBPJPY.DWX`, D1. Development
...
implementation. Chan makes no EURUSD/GBPJPY performance claim.
...
The deterministic research dedup check scanned 4,282 registry rows and 398
cards with no exact slug or strategy-ID collision. Its sole fuzzy hit was the
rank-46 EURUSD/EURJPY sibling, manually resolved as a different second leg,
residual, and logical basket. Exact text, EA-directory, registry, and all 246
unordered traded-symbol manifest searches found no dedicated two-leg
EURUSD/GBPJPY fixed-beta D1 card, allocation, EA, or logical basket.

Rank 45 is already `QM5_12768`, and rank 46 is already `QM5_20224`. Rank 47 is
therefore the first current gap after the latest mechanized frontier row.

## Structural and kill boundary

- Fixed spread: `ln(EURUSD) - (-0.242481866) * ln(GBPJPY)`.
