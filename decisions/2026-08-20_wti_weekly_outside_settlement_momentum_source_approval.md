# WTI Completed-Week Outside-Settlement Momentum - Source Approval

Date: 2026-08-20

Decision: `APPROVED_SOURCE` for one bounded V5 Strategy Card, deterministic
EA-ID and magic allocation, one branch-only non-live build, strict Q01
validation, and one paced target-only Q02 enqueue if tester and host-CPU
ceilings permit. This decision does not authorize a manual tester dispatch.

Authority: OWNER commodity/energy portfolio mission delivered to Codex on the
`agents/board-advisor` branch on 2026-08-20. The mission requires one new,
non-duplicate, structural low-frequency commodity edge with reputable-source
criteria and `RISK_FIXED` backtests; explicitly permits a structural
`XTIUSD` trend edge; and forbids live and portfolio-gate mutation.

## Candidate Identity

- proposed slug: `wti-woutside-settle`
- proposed strategy ID: `MOP-WTI-WOUTSIDE-SETTLE-2026_S01`
- proposed source ID: `MOP-WTI-WOUTSIDE-SETTLE-2026`
- carrier: exact `XTIUSD.DWX`, D1, single slot
- state: the immediately completed broker week has a strict higher high and
  strict lower low than its consecutive parent week, then settles beyond the
  parent range, in its own directional outer quartile, and on the matching
  side of its own first-session open
- action: follow the completed outside week's direction for one broker week
- lifecycle: one persisted attempt per broker week and first-later-week flat

The deterministic allocator owns the EA ID. This record neither reserves nor
predicts an ID.

## Approved Source Basis

The governed record below was read completely before this approval:

1. `strategy-seeds/sources/MOP-TSMOM-2012/source.md`, SHA-256
   `C8B07ECD62C1D5EF9E3D7975EEE6A3B6C46C1C566D0B20B42237613D9B3A7042`,
   which records a complete-paper review and durable retrieval identity for
   Tobias J. Moskowitz, Yao Hua Ooi, and Lasse Heje Pedersen (2012), "Time
   Series Momentum," *Journal of Financial Economics* 104(2), 228-250, DOI
   `10.1016/j.jfineco.2011.11.003`.

The bounded child extraction is
`strategy-seeds/sources/MOP-WTI-WOUTSIDE-SETTLE-2026/source.md`.

Moskowitz, Ooi, and Pedersen document positive own-return continuation,
mechanically map past-return sign to future position direction, and include
WTI crude oil in their commodity universe. Their tested formation and holding
horizons are monthly. They do not test a WTI-only weekly horizon, outside-week
range geometry, settlement beyond a parent range, or an outer-quartile close.
Those are predeclared QM timing and state hypotheses. No source return,
density, cost, continuous-CFD equivalence, or portfolio-correlation result
transfers.

## Locked Mechanic

1. Require exact `XTIUSD.DWX`, D1, slot zero, fixed-risk backtest inputs, both
   news axes OFF, and Friday close OFF.
2. On the first tradable D1 bar of a new Monday-anchored broker week, within
   180 elapsed minutes of its executable open, reconstruct the immediately
   completed broker week and its consecutive parent week from completed D1
   OHLC. Each week must contain three to five strictly ordered sessions.
3. Apply one uniform energy-label convention to the current bar and every
   historical bar: raw labels when the current D1 date equals broker date, or
   a `+1`-calendar-day normalization only when the raw label is exactly one
   day behind. Reject every other or mixed convention.
4. Require the newer completed week to have `high_new > high_parent` and
   `low_new < low_parent`. Define
   `clv = (close_new-low_new)/(high_new-low_new)`. Buy only when
   `close_new > open_new`, `close_new > high_parent`, and `clv > 0.75`.
   Sell only when `close_new < open_new`, `close_new < low_parent`, and
   `clv < 0.25`. Equality, zero range, inside settlement, or any other state
   consumes the week flat.
5. Persist the exact Monday week-anchor attempt before every fallible
   downstream gate. Rejection, order failure, or restart cannot retry that
   broker week.
6. Size one position to `RISK_FIXED=1000`, `RISK_PERCENT=0`, against a frozen
   `3.5 * ATR(20,D1)` hard stop. Use no target and cap spread at 1,500 points.
7. Close on the first tick of a later broker week or after ten calendar days.
   Never trail, partially close, scale in, grid, martingale, pyramid, or add an
   external runtime dependency.

The strict outside range, settlement beyond the parent extreme, outer-quartile
close, and completed-week open-to-close sign must all agree. This is a
filtered own-return continuation trade, not a generic weekly return rule and
not an outside-week exhaustion fade.

## Non-Duplicate Decision

The canonical checker scanned 4,560 registry rows and 625 root cards and
returned `CLEAN`, with no exact or fuzzy match. Manual review separates:

- `QM5_13095_xti-outweek-fade`, which waits for a separate current-week D1
  reversal/reclaim, uses SMA and ATR-range context, and fades the completed
  outside-week extreme; this candidate decides only at the first new-week bar,
  requires settlement beyond the parent extreme, and follows that direction
  with no separate reversal bar or SMA signal;
- `QM5_41061_wti-week-nr7-brk`, which requires the newest completed range to
  be the narrowest of seven weeks and then waits for a current-week breakout;
  this candidate requires two-week outside expansion already completed and
  enters at the next boundary;
- `QM5_12965_wti-week-orb`, which builds an opening range from the current
  week's first D1 bar and waits for its later break, not a completed
  outside-parent settlement;
- `QM5_41068_wti-waccel-mom` and `QM5_41070_wti-wdecel-mom`, which compare
  two adjacent weekly close-to-close return signs and magnitudes without
  completed-week high/low containment or settlement geometry;
- `QM5_41065_wti-wflip-mom`, `QM5_41069_wti-wpull-trend`,
  `QM5_41071_wti-wresume-dom`, and `QM5_41072_wti-wcounter-dom`, which use
  completed-week return paths rather than a strict outside range and
  parent-extreme settlement; and
- `QM5_12567_cum-rsi2-commodity`, which is a long-only two-day oscillator
  pullback rather than symmetric weekly WTI range-expansion continuation.

The exact WTI carrier, two consecutive completed weekly OHLC aggregates,
strict outside range, completed-week own-return sign, settlement beyond the
parent extreme, strict 0.75/0.25 close-location boundary, consumed weekly
attempt, and full-week ownership are jointly load-bearing. Verdict:
`CLEAN_WTI_COMPLETED_OUTSIDE_WEEK_SETTLEMENT_CONTINUATION_AFTER_MANUAL_REVIEW`.

## Reputable-Source Criteria

- R1 `PASS_WITH_WEEKLY_RANGE_TRANSLATION_RISK`: named authors, peer-reviewed
  JFE paper, DOI, complete-read evidence, durable retrieval identity, explicit
  WTI membership, and the untested weekly range condition disclosed.
- R2 `PASS`: exact week anchors, session counts, OHLC aggregates, strict
  outside state, own-return sign, settlement, close-location boundary, side,
  attempt, risk, stop, spread, and lifecycle are locked before testing.
- R3 `PASS_WITH_ENERGY_LABEL_AND_CFD_BASIS_RISK`: registered
  `XTIUSD.DWX` D1 and MT5-native state provide every runtime input; Q02 owns
  history, label, density, and CFD-basis falsification.
- R4 `PASS`: deterministic timestamps and completed OHLC arithmetic with no
  trained logic, banned signal, external feed, grid, martingale, scale-in, or
  pyramid.

## Portfolio Claim Boundary

The candidate carries direct WTI physical-energy price risk outside the
certified XAU/SP500/NDX/XNG book and differs mechanically from certified
`QM5_12567`'s long-only two-day cumulative-RSI2 pullback. Carrier and mechanic
difference do not prove low correlation. Q09 alone may establish realized
portfolio overlap; this approval makes no decorrelation or admission claim.

## Kill And Safety Boundary

Expected cadence is approximately three to ten completed positions per full
post-warm-up year. Q02 must retire below three trades per year, at zero trades
or nonpositive governed economics, or on any label, week-anchor, aggregation,
outside-range, settlement, close-location, side, attempt, risk, lifecycle, or
determinism defect. No weak result may be rescued by accepting equality,
moving the quartile boundary, dropping the parent-settlement condition,
reversing the side, changing the hold, or adding a volatility, volume,
calendar, moving-average, or external state.

This approval excludes manual backtests; live, demo, shadow, stress, and
optimization presets; terminal dispatch or control; AutoTrading; `T_Live`;
deploy or T_Live manifests; portfolio-gate changes; portfolio admission;
decorrelation claims; and correlation waivers. Q02 may be enqueued once only
after fresh exact-path tester and host-CPU checks are below their ceilings. At
the ceiling, stop before queue mutation and record a non-live handoff.

