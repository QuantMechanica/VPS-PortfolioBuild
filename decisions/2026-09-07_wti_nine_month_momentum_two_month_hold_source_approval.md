# WTI Nine-Month Momentum / Two-Month Hold — Source Approval
...
- Proposed slug: `wti-tsmom9-h2`
- Strategy ID: `MOP-TSMOM-2012_XTI_K9H2_S32`
- Parent source: `strategy-seeds/sources/MOP-TSMOM-2012/source.md`
- Dedup receipt: `artifacts/qm5_wti_tsmom9_h2_preallocation_dedup_20260907.json`
...
WTI `k=9, h=2` result. No paper return, significance, alpha, Sharpe ratio, CFD
equivalence, trade count, or diversification result transfers.

## Locked Mechanic

1. Trade preset-bound `XTIUSD.DWX` on D1 only.
2. At the first D1 bar of each odd-numbered broker month, reconstruct ten
   consecutive completed broker-month-end closes.
3. Compute `ln(C[9] / C[0])`; buy for a strict positive value, sell for a
...
The canonical checker scanned 4,861 registry rows and 1,474 cards. It found no
exact identity and returned expected family matches; the unavailable external
Wiki root remains explicit in the receipt. `QM5_20293` uses nine-month
formation but renews every month. `QM5_20281` uses the same fixed odd-month
two-month clock but twelve-month formation; `QM5_41379` and `QM5_41380` use
three- and one-month formation. This identity requires both exact nine-month
endpoints and the non-overlapping two-month lifecycle.

Verdict: `DISTINCT_WTI_K9H2_NONOVERLAPPING_MONTHLY_TSMOM`.
