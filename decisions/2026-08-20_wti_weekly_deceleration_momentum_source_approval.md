# WTI Completed-Week Deceleration Momentum - Source Approval
...
- proposed slug: `wti-wdecel-mom`
- proposed strategy ID: `MOP-WTI-WDECEL-MOM-2026_S01`
- proposed source ID: `MOP-WTI-WDECEL-MOM-2026`
...
- action: follow the shared completed-week sign for one broker week
...
same-sign weeks, or an absolute-return deceleration condition. Those are
...
3. Compute the older and newest adjacent, non-overlapping weekly log returns.
   ...
   non-deceleration consume the week flat.
...
The canonical checker scanned 4,557 registry rows and 625 root cards and
returned `CLEAN`, with no exact or fuzzy match. Manual review separates:

- `QM5_41068_wti-waccel-mom`, which requires the same strict sign but admits
  only a strictly larger newest move; this candidate rejects acceleration and
  requires a strictly smaller newest move;
- `QM5_41069_wti-wpull-trend`, which requires opposed weekly signs and follows
  the older sign after a smaller countermove; this candidate rejects every
  opposed-sign pair and follows the shared sign;
- `QM5_41065_wti-wflip-mom`, which requires opposed signs and follows the
  newest sign without a magnitude comparison;
- `QM5_41066_xauxag-wdecay-rv`, which applies a same-sign smaller-magnitude
  rule to two synchronized gold-minus-silver relative returns and fades their
  cumulative direction as an equal-notional basket; this candidate uses one
  WTI own-return series and continues its shared sign;
- `QM5_13049_xti-1w-mom-vol` and `QM5_21503_xti-weekly-tsmom-lowvol`, which
  condition one weekly return on volatility; this candidate uses two returns
  and no volatility field; and
- `QM5_20239_wti-pulltrend`, which combines a twelve-month trend ending one
  month before an opposed one-month move; this candidate uses two adjacent
  full weeks with the same sign and no slow annual state.

The exact WTI carrier, three week-end closes, two non-overlapping return
intervals, strict same sign, strict absolute deceleration, shared-sign
continuation, consumed weekly attempt, and full-week ownership are jointly
load-bearing. Verdict:
`CLEAN_WTI_TWO_WEEK_SAME_SIGN_DECELERATION_CONTINUATION_AFTER_MANUAL_REVIEW`.

## Reputable-Source Criteria

- R1 `PASS_WITH_WEEKLY_DECELERATION_TRANSLATION_RISK`: named authors,
  peer-reviewed JFE paper, DOI, complete-read evidence, durable retrieval
  identity, explicit WTI membership, and the untested weekly deceleration
  gate disclosed.
...
acceleration, changing direction or hold, adding a return threshold, or
