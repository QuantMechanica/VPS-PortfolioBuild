# XAU/XAG Weekly Reversal-Overshoot Reversion - Source Approval
...
- proposed slug: `xauxag-wovershoot-rv`
- proposed strategy ID:
  `SCHWEIKERT-CME-XAUXAG-WOVERSHOOT-RV-2026_S01`
- proposed source ID: `SCHWEIKERT-CME-XAUXAG-WOVERSHOOT-RV-2026`
...
- state: the two immediately preceding, synchronized, non-overlapping weekly
  gold-minus-silver log returns have strict opposite signs and the newest
  absolute move is strictly larger than the older move
- direction: fade the newest dominant reversal with an equal-notional
...
A fresh generic-reader request for the CME page on 2026-08-20 returned
`DEFERRED:SOURCE_POLICY`; no bypass or new online-content claim is used. The
approval relies only on the already governed complete-read packets above.
...
sign reversal, newest-move dominance, an inverse one-week package, Darwinex
...
4. For completed week-end index 1 newest through 3 oldest, compute
   ...
   signs, and `abs(r_new)>abs(r_old)`. Equality, same sign, a smaller newest
   move, zero, missing weeks, or timestamp disagreement remains flat.
5. If `r_new` is positive, SELL XAU and BUY XAG. If `r_new` is negative, BUY
   XAU and SELL XAG. Signal magnitude does not scale risk.
...
- R1 `PASS_WITH_WEEKLY_OVERSHOOT_TRANSLATION_RISK`: one bounded `source_id`
  carries named peer-reviewed DOI and official exchange lineages; the weekly
  opposite-sign dominance fade is explicitly an untested QM hypothesis.
...
The canonical checker scanned 4,562 registry rows and 625 root cards and
...
- `QM5_41066_xauxag-wdecay-rv`, which requires two strict same-sign weekly
  relative returns and a strictly smaller newest move; this candidate requires
  opposite signs and a strictly larger newest reversal, so their entry states
...
- `QM5_20184`, `QM5_20194`, `QM5_20202`, and `QM5_20260`, which rebalance on
  completed monthly cross-sectional return ranks or votes; this candidate uses
  adjacent weekly spread returns and no rank; and
...
Expected cadence is ten to twenty completed packages per full post-warm-up
...
opposition, dominance, direction, attempt, basket, risk, lifecycle, or
determinism defect. No weak result may be rescued by adding a threshold,
accepting equality or same-sign moves, changing direction, changing the hold,
or adding a fitted center, beta, calendar, trend, or volatility filter.
