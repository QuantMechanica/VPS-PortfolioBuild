# USDJPY/NZDUSD Cointegration G0 Authorization
...
the current OOS-Sharpe frontier: `USDJPY.DWX` / `NZDUSD.DWX`, D1. Development
...
implementation. Chan makes no USDJPY/NZDUSD performance claim.
...
The OOS result is nearly flat, the fixed beta is negative, and the fitted
half-life is extremely slow. Those are adverse findings, not parameters to
repair. A terminal Q02 economic or cadence failure retires the exact sleeve.

## Non-duplicate decision

An exact text search, deterministic EA-registry search, and unordered
`traded_symbols` / `basket_symbols` manifest reconciliation found no
dedicated two-leg USDJPY/NZDUSD fixed-beta D1 card, registry allocation, EA,
or logical basket. Broad FX baskets and conversion-history declarations that
contain both symbols do not trade this exact residual.

Ranks 34 through 39 are already dedicated builds:
`QM5_12751`, `QM5_12756`, `QM5_12758`, `QM5_12760`, `QM5_12762`,
and `QM5_12764`. Rank 40 is therefore the first current gap after
`QM5_20216` rank 33.

## Structural and kill boundary

- Fixed spread: `ln(USDJPY) - (-0.782302979) * ln(NZDUSD)`.
