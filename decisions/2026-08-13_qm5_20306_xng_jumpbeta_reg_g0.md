# QM5_20306 XNG Self-Relative Common-Jump-Beta Regime G0 Authorization
...
`QM5_20306_xng-jumpbeta-reg`. On the first processed natural-gas D1 bar after
a genuine broker-month transition, the candidate estimates XNG's incremental
sensitivity to a price-native common-energy jump proxy over two consecutive,
non-overlapping 252-return blocks. It buys XNG when the recent jump beta is
lower than the preceding beta and sells XNG when it is higher.

This is a locked carrier extension of the governed aggregate-jump
characteristic, not a rescue or performance transfer from another EA. It may
proceed through bounded source/card extraction, schema and G0 lint,
deterministic registry and magic allocation, resolver regeneration, strict
compile, one `RISK_FIXED` backtest setfile, Q01 validation, and one paced Q02
enqueue. Authorization does not pre-approve efficacy, diversification,
...
the accepted manuscript, online appendix, aggregate-jump characteristic, and
translation limits. Nguyen and Prokopczuk (2019), "Jumps in Commodity
Markets," *Journal of Commodity Markets* 13, 55-70, DOI
`10.1016/j.jcomm.2018.10.002`, supplies only peer-reviewed energy co-jump
context.
...
does not test an outright XNG continuous CFD, a two-name realized energy
factor, or an own-history beta comparison.
...
failed hard on runs-test `p=0.04487`; its low- and normal-volatility regime P&L
were negative. Its Q08 baseline recorded PF 1.10 and 83 trades. The WTI carrier
`QM5_20304_wti-jumpbeta-reg` is a same-method sibling on a different traded
return stream. No sibling result transfers performance or correlation to XNG.
...
positive finite closes, a newest completed endpoint before the decision bar
and no more than ten calendar days stale, and no shared return between blocks.
Do not substitute log returns, population deviations, equal or pooled weights,
a different jump threshold, dropped non-jump rows, total beta, ridge
regression, a trend/calendar filter, score-sized risk, or an alternate
direction.
...
a frozen `3.5 * ATR(20,D1)` hard stop, a 3,000-point entry-spread cap, and no
take-profit. Close before monthly replacement or after forty calendar days.
Friday close and both news axes are OFF. XTI is read-only and must never be
...
- R1: PASS with translation and adverse-family caveats retained: named peer-
  ...
  exact aggregate-jump direction, peer-reviewed energy-jump supplement,
  paired Q08 failure, and the WTI carrier disclosed.
...
The canonical pre-allocation checker scanned 4,371 EA-registry rows and 482
root cards. It found no exact slug or strategy-ID identity and surfaced four
expected source/mechanic family neighbors. Manual review resolves them:

- `QM5_13147_energy-jumpbeta` estimates concurrent XTI and XNG coefficients in
  one block, ranks the two values, opens opposite legs, and splits package
  risk. This candidate estimates XNG's coefficient in two disjoint blocks,
  compares recent with preceding state, owns one XNG position, and treats XTI
  as a read-only factor input.
- `QM5_20304_wti-jumpbeta-reg` uses the identical locked estimator and
  lifecycle on WTI. This predeclared carrier extension changes the traded
  return stream to natural gas and inherits no sibling result; it is not a
  parameter variant.
...
- `QM5_12567_cum-rsi2-commodity`, the certified XNG sleeve, is a short-horizon
  ...
  monthly, symmetric long/short, and driven by a two-year common-energy
  jump-sensitivity state.
- XNG skewness, kurtosis, volatility-of-volatility, ALIQ, seasonality,
  weekday, storage-event, trend, variance-ratio, and relative-value EAs use
  different information objects or clocks.

The XNG carrier, synchronized inputs, block-local inverse-volatility weights,
fixed two-sigma jump factor, intercept plus two-regressor OLS, offsets `0/252`,
source low-beta direction, one-leg topology, and consumed monthly lifecycle
...
- intended EA ID: `QM5_20306`, subject to deterministic registry allocation;
- slug: `xng-jumpbeta-reg`;
- strategy ID: `HOLLSTEIN-AGGJUMP-2021_XNG_TS_S03`;
- intended symbol/slot/magic: `XNGUSD.DWX` / 0 / `203060000`;
- read-only factor symbol: `XTIUSD.DWX`, with no magic or order authority;
...
- fail on timestamp misalignment, wrong block support, pooled block weights,
  ...
  OLS, reversed direction, retry, XTI order, missing stop, risk mismatch, hold
  beyond forty days, or nondeterminism.

The slow symmetric jump-sensitivity state is structurally different from the
incumbent short-horizon long-only XNG pullback, but realized independence is
unproven. Q09 alone may establish overlap with the XAU/SP500/NDX/XNG book.
