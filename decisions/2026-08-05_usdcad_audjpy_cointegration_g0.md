# USDCAD/AUDJPY Cointegration G0 Authorization
...
the current OOS-Sharpe frontier: `USDCAD.DWX` / `AUDJPY.DWX`, D1. Development
...
implementation. Chan makes no USDCAD/AUDJPY performance claim.
...
no dedicated two-leg USDCAD/AUDJPY fixed-beta D1 card, allocation, EA, or
logical basket. Broad FX baskets and conversion-history declarations that
contain either symbol do not trade this exact residual.

Rank 40 is already `QM5_20219` and rank 41 is already `QM5_12765`. Rank 42 is
therefore the first current gap after the latest mechanized frontier row.

## Structural and kill boundary

- Fixed spread: `ln(USDCAD) - (-0.186232670) * ln(AUDJPY)`.
