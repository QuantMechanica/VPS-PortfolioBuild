# G0 Decision — QM5_41231 WTI Same-Calendar Five-Sample Bisquare
...
`decisions/2026-08-30_wti_same_calendar_bisquare5_source_approval.md` and the
complete candidate packet committed at `6bb6ec9fa`.
...
- EA ID: `QM5_41231`, pending atomic deterministic registry verification
...
- host / slot 0: exact `XTIUSD.DWX`, D1, intended magic `412310000`
- mechanic: at each genuine normalized broker-month transition, reconstruct
  the exact prior five matching-calendar-month WTI returns, compute their
  raw median and MAD, freeze a `4.685 * 1.4826 * MAD` cutoff, execute exactly
  32 redescending bisquare updates, follow the fitted location sign, and
  renew next month
...
- R1 `PASS_WITH_ROBUST_LOCATION_AND_SINGLE_CFD_TRANSLATION_RISK`: two
  ...
  bisquare arithmetic. The five-sample trading conjunction remains untested.
- R2 `PASS`: month clock, label normalization, exact endpoints, exact five
  years, median/MAD indexes, constants, frozen cutoff, strict support, squared
  weights, update count, epsilon, side, attempt, risk, stop, spread, and
...
- R4 `PASS`: deterministic timestamps, logarithms, finite arithmetic,
  sorting, absolute deviations, fixed reweighting, comparisons, and V5
  execution plumbing only; no trained signal, prohibited runtime feed, grid,
...
- On sorted `[-6,-1,-0.5,+0.5,+2]`, this card buys from final bisquare
  location approximately `+0.124940938`; raw mean, median, middle-three trim,
  endpoint Winsor, trimean, midhinge, shortest-three, and inclusive-pair
  siblings are zero or negative.
- Sign reflection produces the exact opposite mapping.
- The existing same-calendar mean, median, trim, Winsor, pseudomedian,
  chronological-block, shortest-interval, trimean, and midhinge EAs use
  different fixed functionals and do not re-center a redescending score.
- `QM5_41204_wti-samecal-huber10` requires ten years and retains positive
  influence for finite tails; this candidate requires five and assigns zero
  influence at and beyond its frozen cutoff.
...
- exact `XTIUSD.DWX` D1 slot 0 under registered magic `412310000`;
...
- odd median index `2`, raw-MAD index `2`, normalizer `1.4826`, cutoff `4.685`,
  frozen support, strict `abs(u)<1`, squared weights, positive total weight,
  and exactly 32 updates over the original five returns;
...
No raw mean, ordinary median, fixed trim, endpoint Winsorization, trimean,
midhinge, pairwise pseudomedian, shortest interval, Huber location, fitted or
mutable cutoff, early iteration stop, signed-rank or confidence score, recency
weight, regime gate, current-month price, fixed-month direction, recent
trend, curve, storage, inventory, event, volume, optimizer output, trained
signal, external runtime input, retry, scale-in, grid, martingale, pyramid, or
...
wrong return orientation, median/MAD/scale defect, mutable cutoff, inclusive
support, wrong weight, wrong update count, zero-weight fallback,
