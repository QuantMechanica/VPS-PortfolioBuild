# WTI completed-month daily-return median momentum - Source Approval
...
- proposed source ID: `MOP-MEEK-WTI-MDAILY-MED-2026`
- proposed strategy ID: `MOP-MEEK-WTI-MDAILY-MED-2026_S01`
- proposed slug: `wti-mdaily-median-mom`
...
Neither source tests the exact ordinary median of every daily return inside
one completed WTI month. The primary paper supports own-price monthly
continuation; the secondary paper supplies direct WTI daily-return and
ending-session lineage. The within-month median, continuous CFD carrier,
...
3. Reverse the selected closes into chronological order. Form exactly one log
   ...
   `1e-10`.
4. Sort all 17 through 23 finite daily returns ascending without rounding.
   For an odd count, select index `n/2`. For an even count, use the arithmetic
   average of indexes `n/2-1` and `n/2`. Reject any invalid index, nonfinite
   center, or count outside the frozen bounds.
5. BUY when the ordinary daily-return median is strictly positive, SELL when
   it is strictly negative, and consume the month flat when it is exactly
   zero or any state is invalid. The raw endpoint return is an identity check
   and diagnostic only; it may agree or disagree with the median. Neither
   statistic scales risk.
...
- R1: `PASS_WITH_WITHIN_MONTH_MEDIAN_TRANSLATION_RISK`. A named-author,
  ...
  record. The exact within-month median rule is explicitly untested.
- R2: `PASS`. Month selection, older boundary, observation limits, return
  endpoints, identity tolerance, ascending sort, odd/even center formula,
...
- R4: `PASS`. Deterministic timestamps, logarithms, addition, sorting,
...
`artifacts/qm5_wti_mdaily_median_mom_preallocation_dedup_20260823.json`. It
returned `CLEAN` across 4,632 registry identities, 1,300 cards, and 45
...
- `QM5_20269_wti-medret-mom` takes the even-sample median of twelve disjoint
  completed *monthly* returns. This extraction takes the ordinary median of
  all 17-23 daily returns inside one immediately completed month.
- `QM5_41111_wti-mdaybreadth-mom` counts positive versus negative daily
  ...
  retains return magnitudes, has no sign count, and does not gate on the
  endpoint.
- `QM5_41127_wti-mdaily-persist-mom` estimates adjacent demeaned-return
  persistence and then follows the raw endpoint. This extraction discards
  adjacency after reconstructing the month and follows the daily median.
- `QM5_41131_wti-mdaily-tailtrim-mom` deletes exactly one minimum and one
  maximum daily return and sums every retained observation. This extraction
  uses only the one or two ordinary center order statistics.
- `QM5_41132_wti-mweekday-med-mom` first groups returns by ending weekday,
  averages each of five buckets, and takes the median of those five means.
  This extraction has no weekday buckets or intermediate averaging and gives
  every daily observation one place in the ordinary order statistic.
- `QM5_20270`, `QM5_20276`, `QM5_20282`, `QM5_20283`, `QM5_20285`, and
  `QM5_20286` apply trim, pairwise-pseudomedian, MAD cap, trimean, Huber, or
  bisquare transforms to twelve monthly returns rather than one month's daily
  sample.
...
daily return ending in the month, ascending full-sample sort, ordinary
odd/even median, symmetric continuation, consumed month, fixed risk, and
...
at zero trades, with nonpositive governed economics, or on any label, month,
return, sort, median, side, attempt, risk, lifecycle, or determinism defect.
...
sample, center formula, direction, carrier, risk, hold, or by adding an
endpoint-agreement, weekday, seasonal, event, volatility, external, or
prior-result state.
