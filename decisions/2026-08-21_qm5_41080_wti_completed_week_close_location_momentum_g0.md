# G0 Decision - QM5_41080 WTI Completed-Week Close-Location Momentum
...
Approved card:
`strategy-seeds/cards/approved/QM5_41080_wti-wclose-location-mom_card.md`.

## Identity

- EA ID: `QM5_41080`, allocated deterministically at commit `dfd62d52d`
- slug: `wti-wclose-location-mom`
- strategy ID: `MOP-WTI-WCLOSE-LOCATION-MOM-2026_S01`
- source approval commit: `123294145`
- magic allocation and resolver commit: `9ce17323a`
- active host: exact `XTIUSD.DWX`, D1, slot 0, magic `410800000`
- mechanic: follow the immediately completed broker week's strict
  parent-close-to-new-close return sign only when the new close finishes
  strictly above `0.80` or below `0.20` of that week's own high-low range

The governed allocator created the active slot-zero magic row after the EA
directory existed; the regenerated resolver retained all 17,560 active rows
with zero drops.
...
- R2 `PASS`: exact first-week-bar clock, label convention, two consecutive
  ...
  attempt, risk, spreads, and lifecycle are mechanical.
...
The canonical checker included author and mechanic fields, scanned 4,567
registry rows and 625 root cards, and returned `CLEAN`, with no exact or fuzzy
match. Manual review separates the exact weekday segment in `QM5_41020`, the
outside-parent settlement geometry in `QM5_41073`, the multi-week sign and
magnitude path family through `QM5_41074`, current-week range breakouts, and
rolling five-D1 magnitude/volatility systems.

This card alone combines two consecutive completed Monday-anchored WTI
...
- exact XTI D1 host slot 0 under magic `410800000`;
...
Approval authorizes the branch-only non-live build, one XTI D1 `RISK_FIXED`
