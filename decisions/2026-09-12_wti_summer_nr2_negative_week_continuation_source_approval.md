# WTI Summer NR2 Negative-Week Continuation - Source Approval
...
2. `strategy-seeds/sources/MOP-TSMOM-2012/source.md`, the academic commodity-futures
   momentum record; and
...
None of the parents tests this exact June-October, two-week contraction, negative-week,
...
wider than its predecessor and its close to be strictly below its open. Sell only, consume the
...
- R1 `PASS_WITH_CROSS_SOURCE_AND_HORIZON_TRANSLATION_RISK`: peer-reviewed WTI summer-season
  evidence, academic commodity momentum, and governed reputable range-state lineage; the exact
  conjunction is untested.
- R2 `PASS`: calendar, completed-week construction, strict contraction and negative-body
...
The canonical checker scanned 4,941 registry rows and 1,550 repository cards, found no exact
identity, reported seven fuzzy family matches, and failed closed because the external Strategy Wiki
...
`QM5_41443`, a symmetric November-May NR2 body-continuation rule. The closest match,
`QM5_41459`, requires strict range expansion whereas this candidate requires strict contraction,
so their volatility states are mutually exclusive. `QM5_41457` shares contraction but requires a
strictly positive body and fades it, so its signal predicate and economic direction differ. Verdict:
