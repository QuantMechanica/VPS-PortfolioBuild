# WTI Eleven-Month Momentum / Two-Month Hold — Source Approval
...
- Proposed slug: `wti-tsmom11-h2`
- Strategy ID: `MOP-TSMOM-2012_XTI_K11H2_S40`
- Parent source: `strategy-seeds/sources/MOP-TSMOM-2012/source.md`
- Dedup receipt: `artifacts/qm5_wti_tsmom11_h2_preallocation_dedup_20260909.json`
...
standalone WTI `k=11, h=2` result. No paper return, significance, alpha,
Sharpe ratio, CFD equivalence, trade count, or diversification result transfers.

## Locked Mechanic

1. Trade preset-bound `XTIUSD.DWX` on D1 only.
2. At the first D1 bar of each odd-numbered broker month, reconstruct twelve
...
The canonical checker scanned 4,870 registry rows, 1,483 cards, and 45
Strategy Wiki nodes. It found no exact identity and returned expected family
matches. Monthly-renewal WTI carriers use a different lifecycle;
`QM5_20281` shares the fixed odd-month two-month clock but uses twelve-month
formation; `QM5_41379` through `QM5_41388` use three-, one-, nine-, six-,
four-, two-, five-, seven-, eight-, and ten-month formation. This identity requires
both exact eleven-month endpoints and the non-overlapping two-month lifecycle.

Verdict: `DISTINCT_WTI_K11H2_NONOVERLAPPING_MONTHLY_TSMOM`.
