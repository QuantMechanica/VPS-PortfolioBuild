# WTI Winter NR2 Body Momentum - Source Approval
...
3. `strategy-seeds/sources/MOP-TSMOM-2012/source.md`, the complete-read peer-reviewed futures
   momentum record with explicit WTI membership.

The source-of-record translation is
`strategy-seeds/sources/BURAKOV-CRABEL-MOP-WTI-WINTER-NR2-BODY-MOM-20260911/source.md`. None of
the parents tests this exact November-May, two-week range-contraction, own-body continuation rule
...
narrower than its predecessor. Buy after a strictly positive newest-week body and sell after a
...
- R1 `PASS_WITH_CROSS_SOURCE_AND_HORIZON_TRANSLATION_RISK`: peer-reviewed WTI seasonality,
  governed reputable range-state lineage, and peer-reviewed own-return continuation; exact
...
The canonical checker scanned 4,925 registry rows and 1,535 repository cards, found no exact
...
`artifacts/qm5_candidate_wti_winter_nr2_body_mom_dedup_preallocation_20260911.json`.

Manual review distinguishes `QM5_41443`, which uses the opposite volatility state (strict range
expansion); `QM5_41441`, which waits for a later current-week completed-close box breakout rather
than entering from the completed week's body; and `QM5_41440`, which is expansion-gated,
upper-quartile, and long-only. Certified `QM5_12567` is a two-day cumulative-RSI XNG pullback.
Verdict: `DISTINCT_WTI_NOVEMBER_MAY_NR2_OWN_BODY_SYMMETRIC_CONTINUATION_AFTER_FAMILY_REVIEW`.
