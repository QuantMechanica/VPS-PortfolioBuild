---
name: DL-031 — Projects Formalization + Issue Routing Convention
description: Goals/Projects/Issues hierarchy formalized — 4 V5 projects under top goal; every new issue created with projectId; CEO has unilateral authority on routing
type: decision-log
---

# DL-031 — Projects Formalization + Issue Routing Convention

Date: 2026-04-27
Source directive: OWNER directive 2026-04-27 ~15:00 local (relayed via Board Advisor)
Recording issue: QUA-254 (CEO routing + convention task)
Owner: CEO (`7795b4b0-8ecd-46da-ab22-06def7c8fa2d`)
Authority basis: DL-023 (CEO broadened-authority waiver — class 4: internal process choices → issue-tree shape) plus OWNER directive making the formalization itself.
Status: Active.
...
2. **Convention recorded** in `processes/process_registry.md` § "Issue Routing".
...
- **Doc-KM** mirrors the project structure to Notion via `infra/notion-sync/` so the public-facing Doc-KM mirror reflects the same hierarchy.
...
- T6 routing changes nothing about the T6 hard rule (DL-025). All T6 issues still require OWNER manifest + AutoTrading-OFF discipline; the project just gives them a home.
- No retro-routing of done/cancelled issues. Cosmetic only.

## Cross-links

- **Recording task:** QUA-254 — CEO reroute + convention task.
- **Source directive:** OWNER 2026-04-27 ~15:00 local (relayed via Board Advisor).
- **Authority basis:** DL-023 (CEO broadened-authority waiver).
- **Related:** DL-025 (T6 boundary) carries forward unchanged. DL-029 (Strategy Research workflow) maps cleanly onto the new V5 Strategy Research project. QUA-216 (Approval-only execution policy) attaches to T6 project when CEO configures.
- **Registry:** `decisions/REGISTRY.md` — DL-031 row.
- **Process doc:** `processes/process_registry.md` § "Issue Routing".
