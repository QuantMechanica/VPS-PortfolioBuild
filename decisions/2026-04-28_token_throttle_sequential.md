...
## Amendment A1 (2026-04-29 06:30 local) — Option B single-symbol matrix lift

- **Authority:** OWNER directive 2026-04-29 06:30 local
- **Recording task:** [QUA-508](/QUA/issues/QUA-508)
- **Scope:** amends DL-040's "first-matrix-dispatch hold" boundary only

Decision:
- The first-execution hold on full 36-symbol matrix dispatch remains in force.
- A narrow exception is approved for **Option B single-symbol matrix lift**:
  - Pipeline-Operator may run single-symbol matrix progression for one active strategy at a time.
  - Sequential throttles from DL-040 stay binding: one active source, one active strategy, one active phase.
  - No cross-strategy or cross-source parallelism is reintroduced.

Control notes:
- This amendment does not change DL-038's permanent rule that 36-symbol matrix execution is the standard production pattern after OWNER green-light.
- This amendment does not relax any V5 hard-rule gates (Model 4 every real tick, `.DWX`, risk mode discipline, magic schema, module boundaries).

## Amendment A2 (2026-04-29 07:35 local) — Reverse A1; full 36-symbol matrix dispatch green-lit

- **Authority:** OWNER directive 2026-04-29 07:35 local
- **Recording task:** [QUA-512](/QUA/issues/QUA-512)
- **Scope:** supersedes A1's single-symbol exception and amends DL-040 first-matrix-dispatch hold

Decision:
- A1 Option B single-symbol matrix lift is reversed and no longer active.
- OWNER explicitly green-lights the first full **36-symbol matrix dispatch** under DL-038 Rule 2.
- Sequential throttles from DL-040 remain binding: one active source, one active strategy, one active phase.
- No cross-strategy or cross-source parallelism is reintroduced.

Control notes:
- DL-038 Rule 2 execution returns to canonical production shape for the active strategy: 36 symbols across T1-T5.
- All V5 hard rules remain fully binding (Model 4 every real tick, `.DWX` suffix discipline, risk enum discipline, magic schema, module boundaries, no external APIs, no ML imports).
