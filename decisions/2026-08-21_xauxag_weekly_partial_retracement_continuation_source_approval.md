# XAU/XAG Weekly Partial-Retracement Continuation - Source Approval
...
- proposed slug: `xauxag-wretr-rv`
- proposed strategy ID: `SCHWEIKERT-CME-XAUXAG-WRETR-RV-2026_S01`
- proposed source ID: `SCHWEIKERT-CME-XAUXAG-WRETR-RV-2026`
- carrier: exact `XAUUSD.DWX` / `XAGUSD.DWX` D1 paired basket
- state: an immediately preceding completed-week gold-minus-silver impulse is
  followed by a strict opposite-sign completed-week retracement whose absolute
  magnitude is strictly smaller, leaving the ratio between the impulse extreme
  and its pre-impulse anchor
- direction: follow the newest retracement for one additional broker week with
...
carrier. Neither source tests two adjacent opposite-sign weekly relative
returns, strict smaller-retracement geometry, newest-direction continuation,
Darwinex continuous CFDs, equal-notional sizing, ATR stops, or this lifecycle.
...
4. For completed week-end index 1 newest through 3 oldest, compute
   ...
   Require positive finite prices, finite non-zero returns, strict opposite
   signs, and `abs(r_new)<abs(r_old)`. Equality, same signs, a larger newest
   move, zero, missing weeks, or timestamp disagreement remains flat.
5. If `r_old>0` and `r_new<0`, SELL XAU and BUY XAG. If `r_old<0` and
   `r_new>0`, BUY XAU and SELL XAG. The package follows the smaller newest
   retracement toward the pre-impulse ratio anchor; signal magnitude does not
   scale risk.
...
- R1 `PASS_WITH_WEEKLY_RETRACEMENT_TRANSLATION_RISK`: one bounded `source_id`
  carries named peer-reviewed DOI and official exchange lineages; the partial-
  retracement continuation is explicitly an untested QM hypothesis.
...
The canonical `research_dedup_check.py` scan covered 4,564 registry rows and
...
- `QM5_41066_xauxag-wdecay-rv`, which requires two strict same-sign weekly
  relative returns; this candidate requires strict sign opposition;
- `QM5_41075_xauxag-wovershoot-rv`, which requires an opposite-sign newest
  move whose magnitude is strictly larger and then fades it; this candidate
  requires a strictly smaller newest move and follows it, so the states and
  sides differ;
- `QM5_41076_xauxag-waccel-rv`, which requires two same-sign weekly relative
  returns and strict newest acceleration before fading; this candidate is a
  bounded opposite-sign partial retracement;
- `QM5_41069_wti-wpull-trend`, a single-leg WTI sibling which follows the
  older dominant impulse after a smaller opposite week; this candidate uses a
  two-metal relative-value carrier and follows the smaller newest retracement
...
- `QM5_20275_gsr-runfade`, which requires a fresh run of five same-sign D1
  ...
  exactly two non-overlapping completed broker weeks; and
- existing rolling ratio, fitted residual, robust-score, empirical-tail,
  monthly-rank, flow-decomposition, weekend-gap, and weekly-range systems,
  none of which use this exact anchored partial-retracement state.

The exact carrier, three synchronized week ends, two adjacent non-overlapping
relative returns, strict sign opposition, strict smaller newest magnitude,
newest-direction package, weekly attempt, equal-notional aggregate-risk
...
Expected cadence is eight to twenty completed packages per full post-warm-up
...
sign, magnitude, direction, attempt, basket, risk, lifecycle, or determinism
defect. No weak result may be rescued by accepting equality or same signs,
changing direction or hold, or adding a threshold, fitted center, beta,
calendar, trend, or volatility filter.
