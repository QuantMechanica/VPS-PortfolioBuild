# WTI Completed-Week Countershock Dominance - Source Approval
...
- proposed slug: `wti-wcounter-dom`
- proposed strategy ID: `MOP-WTI-WCOUNTER-DOM-2026_S01`
- proposed source ID: `MOP-WTI-WCOUNTER-DOM-2026`
- carrier: exact `XTIUSD.DWX`, D1, single slot
- state: three immediately preceding, non-overlapping completed broker-week
  log returns have an outer-sign / opposed-middle / restored-outer-sign path,
  while the middle countershock is strictly larger than both outer moves
  combined
- action: follow the sign of the dominant middle shock, which is also the
  strict sign of the cumulative three-week return, for one broker week
...
`strategy-seeds/sources/MOP-WTI-WCOUNTER-DOM-2026/source.md`.
...
outer/counter/restoration path, or the countershock-dominance condition. Those
are predeclared QM timing hypotheses. No source return, density, cost, CFD
...
3. Compute three adjacent, non-overlapping weekly log returns in chronological
   order. Require the oldest and newest returns to have the same strict sign
   and the middle return to have the opposite strict sign. Require the absolute
   middle return to be strictly larger than the sum of the absolute oldest and
   newest returns. Follow the middle return: a positive/negative/positive path
   sells; a negative/positive/negative path buys. Zero, equality, another path,
   or failed combined dominance consumes the week flat.
...
The strict combined-dominance condition proves that the cumulative return
`r_oldest + r_middle + r_newest` has the middle return's sign. The position is
therefore a filtered own-return continuation trade, not an unbounded reversal
of the most recent week.

## Non-Duplicate Decision

The canonical checker scanned 4,559 registry rows and 625 root cards and
returned `CLEAN`, with no exact or fuzzy match. Manual review separates:

- `QM5_41071_wti-wresume-dom`, which requires the newest restored move to
  dominate only the middle countermove and follows the newest sign; this
  candidate requires the middle countermove to dominate both same-sign outer
  moves combined and follows the opposite, three-week-net sign;
- `QM5_41065_wti-wflip-mom`, which uses only two opposed completed weeks and
  follows every newest sign; this candidate requires a third older week, outer
  sign restoration, and strict combined middle dominance before trading
  opposite the newest week;
- `QM5_41069_wti-wpull-trend`, which enters in the older sign immediately
  after a strictly smaller newest counterweek; this candidate waits for a
  separate completed restoration week, proves the prior countershock still
  dominates the full three-week path, and follows that countershock;
- `QM5_41068_wti-waccel-mom` and `QM5_41070_wti-wdecel-mom`, which require the
  newest two returns to share a sign; this candidate requires them to oppose;
- `QM5_41064_wti-mflip-mom`, which uses completed broker months rather than
  four synchronized week-end closes; and
- `QM5_20273_wti-signrun-tr`, `QM5_13049_xti-1w-mom-vol`, and
  `QM5_21503_xti-weekly-tsmom-lowvol`, which use multi-month sign runs or a
  single weekly return plus volatility state rather than a strict
  three-interval path and combined-magnitude proof.

The exact WTI carrier, four week-end closes, three chronological return
intervals, outer-sign equality, opposed middle, strict middle-over-sum
dominance, middle/net-sign direction, consumed weekly attempt, and full-week
...
- R2 `PASS`: exact week anchors, endpoints, chronological state, strict
  combined dominance, side, attempt, risk, stop, spread, and lifecycle are
...
combined-dominance, direction, attempt, risk, lifecycle, or determinism
defect. No weak result may be rescued by accepting equality, weakening the
sum condition, removing an outer week, changing direction or hold, or adding
a threshold, volatility, volume, calendar, or external state.
