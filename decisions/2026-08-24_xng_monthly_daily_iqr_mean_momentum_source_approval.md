# XNG completed-month daily-return interquartile-mean momentum - Source Approval
...
low-frequency commodity edge and explicitly permits a second `XNGUSD.DWX`
edge when its logic differs from certified `QM5_12567`. It requires reputable
source criteria, a `RISK_FIXED` backtest setfile, committed non-duplicate work,
and one paced Q02 enqueue. It forbids `T_Live`, AutoTrading, portfolio-gate,
and T_Live-manifest changes.

This decision approves bounded source intake for:

- proposed source ID: `MOP-MEEK-XNG-MDAILY-IQRMEAN-2026`
- proposed strategy ID: `MOP-MEEK-XNG-MDAILY-IQRMEAN-2026_S01`
- proposed EA identity: `QM5_41136`
- proposed slug: `xng-mdaily-iqrmean-mom`
- instrument: `XNGUSD.DWX`
...
|---|---|---|
| `strategy-seeds/sources/MOP-TSMOM-2012/source.md` | Peer-reviewed own-price monthly momentum evidence; natural gas is explicitly in the commodity universe | `C8B07ECD62C1D5EF9E3D7975EEE6A3B6C46C1C566D0B20B42237613D9B3A7042` |
| `strategy-seeds/sources/MEEK-HOELSCHER-WTI-DOW-2023/source.md` | Peer-reviewed complete-read evidence for close-to-close daily natural-gas log returns and heterogeneous energy behavior | `0C6BBF1285C7C196F4D04FEB2254A62D9A9D89EDCA9E4DBBAC3D003EB3E88FDE` |
...
`k=1,h=1` implementation, and includes natural gas in its commodity universe.
...
reviewed in the governed packet. The paper studies natural-gas futures,
constructs synchronized close-to-close log returns, and documents
heterogeneous daily energy behavior.

Neither source tests a natural-gas-only central-half mean of the daily returns
inside one completed month. The primary paper supports own-price monthly
continuation; the secondary paper supplies direct natural-gas daily-return and
...
4. Sort all `n` finite daily returns ascending without rounding. Define
   ...
   exactly once. For the allowed 17-23 sessions this removes four or five
   returns from each tail and retains 9-13 central observations.
...
7. Permit at most one owned natural-gas position. Use `RISK_FIXED=1000`,
   `RISK_PERCENT=0`, `PORTFOLIO_WEIGHT=1`, a frozen
   `3.5 * ATR(20,D1)` hard stop, no target, and a 3,000-point entry-spread
   ceiling.
...
- R1: `PASS_WITH_WITHIN_MONTH_IQR_MEAN_TRANSLATION_RISK`. A named-author,
  ...
  PDF hash, and explicit natural-gas membership is joined to a named-author,
  peer-reviewed open-access daily-energy-return paper with a complete-read
...
- R3: `PASS_WITH_ENERGY_LABEL_AND_CONTINUOUS_CFD_BASIS_RISK`. Registered
  `XNGUSD.DWX` D1 history and native MT5 calendar/execution state supply every
...
The fail-closed pre-allocation report is
`artifacts/qm5_xng_mdaily_iqrmean_mom_preallocation_dedup_20260824.json`.
It scanned 4,635 registry identities, 1,303 cards, and 45 Strategy Wiki nodes
from the current Company Reference root. It found no exact slug or strategy-ID
collision and raised one expected fuzzy carrier sibling,
`QM5_41134_wti-mdaily-iqrmean-mom`, for manual review.

Manual mechanic review resolves the fuzzy result:

- `QM5_41134_wti-mdaily-iqrmean-mom` applies the same source-family statistic
  to outright WTI. The proposed card owns exact natural-gas routing, the XNG
  return stream and spread basis, and cannot execute on WTI. Repository
  precedent treats source-pure WTI and XNG carriers as separate single-symbol
  hypotheses: `QM5_20187_wti-tsmom1m` and `QM5_20204_xng-tsmom1m`.
- `QM5_20204_xng-tsmom1m` follows the unpartitioned completed-month endpoint.
  This extraction sorts every daily return ending in the month, removes the
  dynamic outer quartiles, and follows only the retained central-band mean;
  the raw endpoint is not a signal gate.
- existing XNG weekly range, close-location, flow, calendar, reversal, and
  multi-month trend cards do not select a 9-13-observation order-statistic
  band from all daily returns inside exactly one completed month.
- certified `QM5_12567_cum-rsi2-commodity` is a long-only two-day cumulative
  RSI(2) pullback above SMA(200), with a five-bar maximum hold. This extraction
  is symmetric, oscillator-free, monthly, and holds to the next month.
...
month, fixed risk, and next-month exit are jointly load bearing. Manual
verdict:
...
This logic is materially different from the certified XNG RSI sleeve but does
not prove low realized correlation. Q09 alone owns the portfolio result. No
failure may be rescued by changing the sample, quartile rule, direction,
