# WTI Eleven-Month Momentum / One-Month Hold — Source Approval
...
- Proposed slug: `wti-tsmom11-h1`
- Strategy ID: `MOP-TSMOM-2012_XTI_K11H1_S42`
- Parent source: `strategy-seeds/sources/MOP-TSMOM-2012/source.md`
- Dedup receipt: `artifacts/qm5_wti_tsmom11_h1_preallocation_dedup_20260919.json`

## Authority And Source Quality

The current explicit OWNER PACER mission authorizes one reputable-source,
structural, low-frequency commodity/energy card and build under the binding
input-pin and capacity guards. Moskowitz, Ooi, and Pedersen (2012), *Time
...
standalone WTI `k=11, h=1` result. No paper return, significance, alpha,
Sharpe ratio, CFD equivalence, trade count, or diversification result transfers.

## Locked Mechanic

1. Trade preset-bound `XTIUSD.DWX` on D1 only.
2. At the first D1 bar of every new broker month, reconstruct twelve
   consecutive completed broker-month-end closes.
3. Compute `ln(C[11] / C[0])`; buy for a strict positive value, sell for a
...
- R4 `PASS`: native calendar/OHLC/logarithm/ATR arithmetic only; no trained or
  prohibited mechanic.

## Duplicate Decision

The canonical checker scanned 4,965 registry rows, 1,560 cards, and 45
Strategy Wiki nodes. It found no exact identity and returned expected family
matches. Manual review resolved the closest systems:

- `QM5_41390_wti-tsmom11-h2` uses the same exact eleven-month formation but
  only at odd-month boundaries and holds through the intervening even month;
- `QM5_41391_wti-tsmom10-h1` renews every month but uses eleven endpoints and
  an exact ten-month return;
- `QM5_12603_wti-tsmom12m` uses a 252-D1 proxy and neutral-band/parameter
  surface rather than twelve exact broker-month endpoints; and
- `QM5_20284_wti-skip1-trend` deliberately excludes the newest completed
  month and measures an older twelve-month interval.

This identity requires both exact eleven-month endpoints and every-month
one-month renewal. Verdict: `DISTINCT_WTI_K11H1_MONTHLY_TSMOM`.
