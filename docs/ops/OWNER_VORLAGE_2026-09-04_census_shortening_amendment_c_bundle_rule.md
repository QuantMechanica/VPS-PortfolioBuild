...
## Correction 20:38Z (evidence audit by Codex Astra, task 1ff3fa26)

The census ledger already excludes whole years: of 10,788 done census cells fleet-wide, 5,559 are MEASURED and 5,229 are `SKIPPED_EXCLUDED` receipts (schema qm.dl089-skipped-as-excluded/v1, no run). Measured cells per program therefore range from 165 (11910/NZDUSD, 11421/EURUSD: only 2019 measured, 2020-2025 excluded) to the full 1,085 (XAUUSD programs such as 21507: 824 measured + 260 pending). The statement above that 132 of 154 arms on 11421 were identical to the baseline in every year holds only for the single measured year (2019, 3 trades); the other years were excluded by the factory, not measured. Astra's fire-count tool (deployed d29fac723f, 552 tests) predicts over all seven baseline years 87 of 154 zero-count arms for 11421 and, on 10706/GBPUSD (32-37 trades/yr, 3 measured years), 160 zero versus 166 positive arm/years; on the 480 measured cells prediction and outcome agree 100 % with zero false never-fires. Revised expectation for Option B5: roughly -50 % cells on high-frequency (XAU/GBP) programs, i.e. the ones that currently gate the counter; low-frequency FX programs are already mostly excluded by the year rule. Open before B5 can be wired: declare the D1-bar source (native CopyRates export used by the tool, or a TKC decoder for the tick archive) so tester parity is provable; the tool keeps `safe_to_skip=false` until then.
