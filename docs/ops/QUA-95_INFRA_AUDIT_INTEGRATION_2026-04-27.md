...
Issue: `QUA-95`  
Scope: confirm blocker task health, issue-transition payload consistency, blocked invariant enforcement, handoff integrity, blocked assertion freshness, ops-bundle manifest integrity, and blocked-heartbeat wrapper validation are present in central infra audit output.
...
```text
Infra audit completed: status=critical, checks=19, issues=2
Report: C:\QM\repo\infra\reports\infra_audit_latest.json
```
...
- QUA-95 scheduler health, transition payload consistency, blocked invariant enforcement, handoff integrity, blocked assertion freshness, ops-bundle manifest integrity, and blocked-heartbeat wrapper validation are now audited in the same report as disk, terminal liveness, Drive sync, and stale index-lock checks.
