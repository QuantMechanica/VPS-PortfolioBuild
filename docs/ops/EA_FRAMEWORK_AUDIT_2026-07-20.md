...
| P0.3 | **KillSwitch halt-coverage 13/24 root cause = recompile debt**: binaries predating commit 47f1d9709 carry the dead absolute-path default → KS_MANUAL / KS_PORTFOLIO_DD silently ignored. 11 live sleeves cannot be halted via the file channel. | `QM_KillSwitch.mqh:458-473` (correct post-fix) | T_Live .ex5-vintage audit (compile timestamp vs fix commit), then recompile wave |
