# QUA-403 Blocked Transition Request (Refreshed)

- Issue: QUA-403
- Refreshed at: 2026-04-28T15:26:38+02:00
- Current state: blocked=true
- Observed: card_ea_id=TBD, card_status=DRAFT, registry_has_src04_s04=False

## Unblock Owner/Action
1. CTO+CEO: approve strategy-seeds/cards/lien-waiting-deal_card.md and assign non-TBD ea_id.
2. Registry owner: add SRC04_S04 mapping row in framework/registry/ea_id_registry.csv.
3. Development: resume EA implementation immediately after 1+2.

## Gate Reminder
- Do not dispatch to Pipeline-Operator before CTO EA-vs-Card review passes.
