...
| `Q16` | Best-Settings Head-to-Head | **`Q14`** | 2 | SEALED_BEST_SETTINGS_VS_BASELINE_AND_INCUMBENT_Q11 | RENUMBER (name byte-preserved; evidence_role suffix `_Q10`→`_Q11` tracks the renumbered incumbent gate — no new criterion, no holdout). **Terminal Phase-2 gate: `next=null`.** Reference baseline = `Q09` (pre-news full run). Outcomes `CHALLENGER_PROMOTED` / `KEEP_INCUMBENT`. Removes the `Q16→Q11` back-edge. |
