...
| `QM_StrategyFarm_TesterCachePurge` | 20 min | `tester_cache_purge.ps1` — first reclaims only aged, exclusively-openable `bar*.tmp` under `T1-T10\Tester\Agent-*\temp` without stopping busy slots; then, if D: free <150GB, preserves protected active slots and captured Factory ON/OFF state, purges only idle regenerable `T*\Tester\bases`+`Agent-*` caches, and requests only missing workers. Controller runs as SYSTEM; worker launch remains INTERACTIVE qm-admin. Never touches T_Live/FTMO/source ticks/reports. |
...
| `QM_StrategyFarm_UnbuiltCardsDisposition_Hourly` | hourly | `unbuilt_cards_disposition.py` — read-only: classifies the approved-but-unbuilt card backlog into READY/NEEDS_SOURCE/DATA_BLOCKED buckets and writes a timestamped snapshot; `chk_unbuilt_cards_count` (health.py) reads the latest snapshot to enrich its WARN/FAIL detail with the bucket split. Never creates build tasks or touches pipeline state. Install: `install_unbuilt_cards_disposition_scheduled_task.ps1`. MNT-013 (2026-08-21). |
