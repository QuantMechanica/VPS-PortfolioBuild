# OWNER-Vorlage 2026-09-05 — FTMO Positive-Evidence-Abnahmetest (Revision R5)

Status: **RATIFIED (OWNER, Mission Control receipt `a2057610-baf2-4c8f-b493-84ec80421ada`, 2026-09-05 12:41Z).** Ratified R5 body LF sha256 `de549514fd75e68adb6f972a39451c89b43d7f7e35961e6e3da197903bfa923d` (= receipt-sealed value; this status line is the only post-ratification edit). Seal-table re-verification at ratification (13:45Z): 10/12 explicit LF digests match the current tree; two moved AFTER the R5 table was written and BEFORE the seal ceremony (no seal exists yet, so no contamination): item 5 `ftmo_timebox_eval.py` 5a3f522d… → 9e47f00a… (commit 4902ea050b, +8 lines: the `inspect-cost-version` read-only subcommand only, evaluation path untouched) and item 8 `opt_census.py` 1c0ffccd… → c99508e2… (commits 97bebb43f5/82d45dacfb/27bb879e6e: DL-089 D1 pre-screen staging on the enqueue path; declared trial count stays 154). The seal ceremony recomputes items 1, 5, 8 and 18 on the then-current bytes. Items 1 and 18 are recomputed at the seal ceremony on the then-current bytes (this line and the contract V1 status mirror move them). No evaluation stream opened; NO-BUY unchanged. Klasse: **ROT** (kauf-nah), **kein Auffangregel**.
...
> **Revisionsstand R5 (CEO, 2026-09-05) — rein mechanisch.** R4 wurde ein drittes Mal unabhängig geprüft (`docs/ops/evidence/2026-09-05_review3_ftmo_positive_evidence_test.md`; Verdikt **FAIL** auf den exakten R4-Bytes — roh `d669aa1e4a1ff7882f982f90f9e3302bf63f384ecb5758030125e18bd59dde8a`, LF `1e821203c31860530a117f852cddd1ad5769433ca832cef4443648129f6835d4`, 500 Zeilen — die Methode ist nach einer mechanischen Korrektur ratifizierbar). R5 setzt **genau diese drei Befunde** um und ändert sonst nichts: **(F1)** die 59 horizont-verkürzten Starts sind **keine** erzwungenen `TIMEOUT`s — `evaluate_phase()` prüft Breach- und Zielbedingung an jedem verfügbaren Tag und kehrt beim ersten terminalen Ereignis sofort zurück (`ftmo_timebox_eval.py:858-890`); `TIMEOUT` fällt nur, wenn bis zum Ende der verfügbaren Daten nichts eintrat (`:891-895`). Die Zahl der tatsächlich rechts-zensierten Starts ist ergebnisabhängig, folgt **nicht** aus `96 − 37` und wird vor dem Siegel nicht geöffnet (§1 Punkt 2, §C.4 R-2d, §G). **(F2)** Alle Siegel-Digests sind auf dem aktuellen Tip `c1d72603fb` neu berechnet; sechs statische Identitäten waren nach dem geordneten Portabilitäts-Fix `7f56e1c825` veraltet. **OQ-11 ist damit erledigt**, und die Digest-Konvention dieses Dokuments deckt sich jetzt mit der Engine, statt ihr nur eine Regel entgegenzuhalten (§E). **(F3)** Die 1:1-Kopplung `W_min = ESS_min` ist als **konservative OWNER-Wahl** ausgewiesen, nicht als Unabhängigkeits-Identität; die Empfehlung bleibt unverändert (§C.4 R-2). Keine Schwelle, keine statistische Konstante und kein Kriterium wurde geändert.
>
...
2. **Die „37 rollierenden Starts" aus R3 waren für diese Engine falsch — die Engine zählt mehr Starts, aber die zusätzlichen sind nicht chancenlos.** `rolling_outcomes()` iteriert über **jeden** Tag mit `eligible_start` und verlangt **nicht**, dass der 60-Tage-Horizont noch in die Spanne passt (`ftmo_timebox_eval.py:898-902`). Auf 96 Tagen sind das **96 gezählte Starts**: **37 mit vollem 60-Tage-Horizont, 59 mit verkürztem Horizont** (37+59 = 96). Ein verkürzter Start ist **nicht** automatisch ein `TIMEOUT` — `evaluate_phase()` prüft die Breach- und die Zielbedingung an jedem verfügbaren Tag und kehrt beim ersten terminalen Ereignis sofort zurück (`:858-890`); `TIMEOUT` entsteht nur, wenn bis zum Ende der verfügbaren Daten weder Breach noch Ziel eintrat (`:891-895`, Regel `censoring` `:94`). **Rechts-zensiert sind also ausschließlich die unaufgelösten verkürzten Starts**; ihre Zahl ist ergebnisabhängig, folgt **nicht** aus `96 − 37` und wird vor dem Siegel nicht geöffnet. Gesichert ist damit nur die **Zählkorrektur** an `rolling_starts` (`:1145`) und `p1_hac.n` (`:1147`); Richtung und Größe der Wirkung auf `p1_raw_rate` (`:1146`) sind aus dem Kalender allein **nicht** ableitbar.
...
2. **Diesen vorab-deklarierten, widerlegbaren Abnahmetest in der R5-Fassung ratifizieren** — Population / Metrik / Unsicherheit / **operative Power-Regel** / Evidenzklasse / Refutationsklausel / Siegel —, BEVOR ein Ergebnis gesehen wird.
...
# ENGLISH CONTRACT — FTMO Positive-Evidence Acceptance Test (predeclared, refutable) — R5
...
| DL-083 Q09 marginal-eval reject | 0.40 | `:75-76` (comment only); actually enforced in `ftmo_timebox_eval.py:112-114`, applied `:1115-1117` (`REFUSED_DL083_CORRELATION_AT_OR_ABOVE_0P40` `:61`). *R4 reported the contract JSON's own citation `ftmo_probability_contract.v1.json:30` as stale ("applied :1092"); it was corrected by `7f56e1c825` and now cites `ftmo_timebox_eval.py:112-114` / applied `:1115-1117`. OQ-11 is retired.* |
...
*Note on the contract's formerly stale line: `ftmo_probability_contract.v1.json:29` carried `migration_note: "builder currently consumes zeros-DROPPED point Pearson (:184-197); migrate to Layer-A certified CI"` although that migration had already landed in `f68ce8f338`. `7f56e1c825` corrected the note in place — it now records the completed migration citing `build_book_ftmo.py:171-205`. Never a behavioural difference; OQ-11 is retired.*
...
**R-2 · Minimum sealed span (binding, numeric, computed before any result is opened).** The moving-block bootstrap's own resampling unit is the **60-calendar-day block** (`ftmo_timebox_eval.py:104` block length; `_resampled_days` draws blocks of `min(n, 60)` at `:994` from `n − block + 1` admissible starts at `:997`; `moving_block_bootstrap` rebuilds `rolling_outcomes` per replicate at `:1020-1022`). Those blocks are drawn **with replacement from heavily overlapping start positions**, so a drawn block is a conditionally independent RNG selection and **not** an independent market observation; no independence identity follows from the implementation. R-2 therefore takes the unit the bootstrap itself resamples and adopts it as an explicit **OWNER-chosen conservative coverage policy** (OQ-2c): **one non-overlapping 60-day source block counts as one `W` unit.** So stated, the sealed span must supply at least as many disjoint blocks as R-1 demands effective observations:
...
D = 2026-01-01 .. 2026-04-06 inclusive       = 96 calendar days
  ...
  of those, HORIZON-TRUNCATED starts (fewer than 60 days observable)  = 96 - 37 = 59
        a truncated start still PASSES or breaches on any observed day (:858-890);
        it is RIGHT-CENSORED only when no terminal event occurred before the data end
        (:891-895, rule :94), so the censored count is RESULT-DEPENDENT, is not 96 - 37,
        and is not opened before the seal
...
**One.** At the recommended `p* = 0.90` / `bar = 0.80`, R-2 demands `W_min = 35` and `D_min = 2,100`; the window supplies `W_sealed = 1` and `D = 96`. It is short by a factor of **35** in blocks and **21.9** in days. This now *follows* from a predeclared numeric criterion instead of being asserted. **R3's "37 rolling starts" figure was itself wrong for this engine** and is corrected above: 37 is the count of *full-horizon* starts, while `rolling_starts` (`:1145`) counts every eligible day and therefore also carries the 59 horizon-truncated starts. Those 59 are **not** forced non-passes: `evaluate_phase()` returns PASS or a breach as soon as the condition is met on an observed day (`:858-890`) and yields `TIMEOUT` only for a start left unresolved when the data end (`:891-895`). How many of them are actually right-censored, and in which direction they move `p1_raw_rate` (`:1146`), is a property of the opened result and is deliberately not declared here. **None of this changes `D = 96`, `W_sealed = 1`, or the R-2 verdict**, which rest on the sealed calendar alone.
...
**Digest convention (binding, demonstrated, and since `7f56e1c825` identical to the engine's own).** All seal digests are SHA-256 over **LF-normalized bytes** — re-verified in R5 to equal the git blob content (`git cat-file -p HEAD:<path> | sha256sum` reproduces the LF digest exactly). The working-copy digest is recorded beside it and is **never** the seal identity, because it is checkout-dependent. **The convention is no longer only this document's:** the contract loader now stamps the LF-normalized digest as `sha256` and retains the exact on-disk digest as `raw_sha256` (`ftmo_probability_contract.py:153,:154`), which the evaluator publishes as a pair (`ftmo_timebox_eval.py:1482,:1483`) — so the identity this seal records and the identity the engine stamps are the same value:

> **Two-checkout evidence (re-measured in R5 at tip `c1d72603fb`).** For the identical commit, `C:\QM\repo` and this agent worktree produce **different working-copy digests** for the same four P7a modules while producing **identical LF digests**. Example: `ftmo_timebox_eval.py` → working copy `8de5458042020cdd23fd79dcc5facf2993a154a9eebfae31a14a1b7996ce938b` (main checkout) vs `c01ca9798d828e0a5948a34a24602698c2bf2e889fad53ef07de8030dfcf7291` (this worktree) — LF `5a3f522de35d3d21f3c4089fa2e0adec83e9bba4db2cabefd98a0cd37ecd85ca` in both. The same holds for `build_book_ftmo.py` (`39481dbc…` vs `25d84f91…`, LF `5cf70d3f…`), `ftmo_p1_mc.py` (`934409ba…` vs `f71eb6aa…`, LF `80e57bce…`) and `challenge_firstpassage.py` (`79002fe3…` vs `d25b7d54…`, LF `fc3b58c9…`). **A seal quoting working-copy digests is not portable; only the LF digest is.**

> **Corollary that touched the engine itself — CLOSED by `7f56e1c825`.** In R4, `load_probability_contract()` hashed the file's **raw on-disk bytes** and the evaluator stamped that checkout-dependent value into every result, while `ftmo_probability_contract.v1.json` was not listed in `.gitattributes` as `-text`; the stamped digest therefore differed between checkouts (`54cb80fd…` vs `7328b8b1…`), which R4 raised as **OQ-11**. The ordered portability fix `7f56e1c825` closed both halves: the contract path is now `-text` (`git check-attr text -- …` → `text: unset`), and the loader stamps the **LF-normalized** digest as `sha256` while retaining the exact on-disk digest as `raw_sha256` (`ftmo_probability_contract.py:153,:154`), published as a pair by the evaluator (`ftmo_timebox_eval.py:1482,:1483`) and mirrored by the builder and both diagnostic engines. Re-measured in R5 at tip `c1d72603fb`: the contract file is byte-identical in `C:\QM\repo` and in this worktree, raw = LF = `5b4e24eb60cd175f0973ddf7c8ea3f9db0daf2965379f1fedff6a8d816f79362`. **The seal records the LF digest as authoritative and the raw digest beside it; naming a canonical runtime host is no longer needed for the contract identity.** Evidence: `docs/ops/evidence/2026-09-05_contract_json_digest_portability.md`. **OQ-11 is RETIRED** — see the open-questions list.

**Seal procedure — before the first result is opened, compute and record a sha256 over each of the following.** Items 5–8 were the Review-F5 extension; **items 15–18 are the R4 extension** binding the delivered P7a dependencies. **All digests below were recomputed in R5 at tip `c1d72603fb`**, which includes the ordered portability fix `7f56e1c825`; six static identities (evaluator, `ftmo_p1_mc.py`, `challenge_firstpassage.py`, builder, contract JSON, contract loader) moved relative to R4 and are restated below. That commit changed **no** probability, correlation, risk or acceptance threshold (`docs/ops/evidence/2026-09-05_contract_json_digest_portability.md`). **LF is authoritative, working copy is the parenthetical.**

| # | Sealed input | Path / digest (LF authoritative) |
|---|--------------|-------------|
| 1 | This contract text (R5) | `docs/ops/OWNER_VORLAGE_2026-09-05_ftmo_positive_evidence_test.md` — digest computed on the ratified bytes |
...
| 5 | **Evaluator module** `tools/strategy_farm/portfolio/ftmo_timebox_eval.py` — LF `5a3f522de35d3d21f3c4089fa2e0adec83e9bba4db2cabefd98a0cd37ecd85ca` (worktree `c01ca979…`) | 1,561 lines; resampling, outcomes, HAC, refusals, INERT publication. **Moved since R4 by `7f56e1c825`** (publishes `raw_sha256` beside `sha256`, `:1483`); R4's `c3b2fcd8…` is stale |
| 6 | **Supporting engines** — LF digests: `ftmo_p1_mc.py` `80e57bce0fcccf9730da5d4a781faccd9ccb8412bea5ff8b096fe5388e8119ba`; `challenge_firstpassage.py` `fc3b58c91aebed95cf49fdabcda888576c3972c2056a1c3fc7e655dc2fad68bc`; `ftmo_rules_engine.py` `2c79ccd274d00bc1d1c5299c9700cf3ff51185429b99acb5a32497d90f5e7e51`; `ftmo_rule_contract.py` `02adb2ebe566e142ef7e5cb7e22873c365f76f305ff688f94a27fc9264373aad` | the first two were changed by P7a and **again by `7f56e1c825`** (R4's `a4a0a841…` / `7ff679d6…` are stale); the last two are unchanged (their R3 values were the working-copy form) |
| 7 | **Builder** `build_book_ftmo.py` LF `5cf70d3f9e54ea4f512149d9b90c9d4a1e0431a7f387d1d0586ad57f984987bd` (worktree `25d84f91…`; R4's `f371632e…` is stale — moved by `7f56e1c825`); **scorer** `fund_score.py` LF `cb9c3989923f4370bfb30d8f46b18547afa65414db81f354b8eda5026dd23a56`; **counter** `book_build_guard.py` LF `e3ba47378a605848db2694ba662b64e0eb9a31df1fcab7849e4e0fe66d2f6015` | floors `:58,:59-61`; formula `:94-103`; counter `:28,:236-238` |
...
| 15 | **Probability contract JSON** `tools/strategy_farm/config/ftmo_probability_contract.v1.json` — LF `5b4e24eb60cd175f0973ddf7c8ea3f9db0daf2965379f1fedff6a8d816f79362` (**raw identical**: the path is `-text` since `7f56e1c825`, so every checkout carries these bytes; R4's `54cb80fd…` and the CRLF `7328b8b1…` are both stale) plus the contract `schema` and `status` strings (`:2,:3`) | **the source of the design bar, horizons, bootstrap params, correlation caps and the INERT statuses**; loaded at evaluator import (`ftmo_timebox_eval.py:72`) and NOT pinned by `prepare-config`. The loader stamps exactly this LF value as `sha256` and the raw digest as `raw_sha256`, so seal identity = stamped identity |
| 16 | **Contract loader** `tools/strategy_farm/portfolio/ftmo_probability_contract.py` — LF `c6c8ddf6f525bfac0dbddbbf8a2643a9ec7c725fb5fb4a0ed48d25c158943b0f` (worktree `f7827c37…`; R4's `a8596709…` is stale — moved by `7f56e1c825`) | enforces C-6 inertness, the 0.80 P1 bound, DSR N = 369 and rulepack parity, and since `7f56e1c825` emits the portable `sha256` / raw `raw_sha256` pair (`:153,:154`). *That commit shifted its validation anchors by +6; the citations `:117-120 / :121-122 / :123-124 / :126-143` used elsewhere in this document are one commit behind while remaining behaviourally correct — refreshed under OQ-8 at seal time.* |
...
| `build_book_ftmo.py:68-69` DL-083 comment | now `:75-76`; the 0.40 is actually enforced in `ftmo_timebox_eval.py:112-114`, applied `:1115-1117` | corrected + enforcement site named (the contract JSON's `:30` said `:1092`; `7f56e1c825` corrected it to `:112-114` / applied `:1115-1117`, retiring OQ-11) |
...
| "rolling 60-day starts = 96 − 60 + 1 = 37" | wrong for this engine: `rolling_outcomes` (`:898-902`) enumerates every eligible day; 37 is the *full-horizon* subset, the other 59 are **horizon-truncated** — they can PASS or breach on an observed day (`:858-890`) and are right-censored only when unresolved at the data end (`:891-895`) | corrected §C.4 R-2d. **R5 further corrects R4's own reading**, which called all 59 forced TIMEOUTs (review 3, F1); the right-censored count is result-dependent and is not opened before the seal |
...
- **OQ-11 — RETIRED (resolved by `7f56e1c825`).** Both halves of R4's complaint are closed. (i) `tools/strategy_farm/config/ftmo_probability_contract.v1.json` is now `-text` in `.gitattributes` (`git check-attr text -- …` → `text: unset`), and `load_probability_contract()` stamps the **LF-normalized** digest as `sha256` while retaining the exact on-disk digest as `raw_sha256` (`ftmo_probability_contract.py:153,:154`); the evaluator publishes the pair (`ftmo_timebox_eval.py:1482,:1483`), as do the builder and both diagnostic engines. Re-measured in R5 at tip `c1d72603fb`: raw = LF = `5b4e24eb…` in both `C:/QM/repo` and this worktree, so the engine's stamped identity equals the §E item 15 seal identity and §E's digest convention now **matches** the engine instead of standing against it. (ii) The three stale citations inside the contract JSON were corrected in the same commit: `migration_note` (`:29`) now records the completed Layer-A migration citing `build_book_ftmo.py:171-205`, and `:30` now cites `ftmo_timebox_eval.py:112-114` with applied `:1115-1117`. Evidence: `docs/ops/evidence/2026-09-05_contract_json_digest_portability.md`. **One residual, declared rather than silently inherited:** that commit shifted the loader's validation anchors by +6 and the builder's post-import anchors by +1, so those line citations elsewhere in this document are one commit behind while behaviourally correct; **OQ-8's seal-time §G re-verification covers the refresh**, and no threshold, gate or population is affected.
