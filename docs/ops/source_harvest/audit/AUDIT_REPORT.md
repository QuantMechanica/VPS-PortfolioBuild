...
- **Real latent bug:** `QM_Branding.mqh` exists twice with **divergent content**; the compiled copy is `framework/include/QM_Branding.mqh` (via `QM_ChartUI.mqh:4` `..\QM_Branding.mqh`), while the **newer 05-07 edit landed on `framework/include/QM/QM_Branding.mqh`, which nothing includes** — silently ineffective edits. → FB-11.
