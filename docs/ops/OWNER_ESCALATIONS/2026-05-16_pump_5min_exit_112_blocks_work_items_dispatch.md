...
- 2026-05-19T15:47Z (observe wake) — pattern converging without further intervention. Hourly `no_work_items_created` (backtest_p2 failed) rate since the 13:53Z entry: 11Z=3, 12Z=3, 13Z=1, 14Z-15Z=0 (07Z spike of 24 from the morning burst is reflected by the same `t.kind+t.status='backtest_p2 failed'` self-heal filter). Previously-stuck-cards check at 15:47Z: QM5_1387 + QM5_1554 quiet since 07:45Z (dead-lettered, no auto-re-fire); QM5_1383 + QM5_1435 last re-fired 09:52Z; QM5_1406 last re-fired 13:38Z. Self-heal filter still surfaces 60 real failures (25 `backtest_p2` + 35 `build_ea`, mix of `no_work_items_created` and yesterday's REPORT_MISSING/METATESTER_HUNG/MODEL4_MARKER_REQUIRED smoke cluster from `2026-05-17_smoke_first_run_intermittent_report_missing.md`). No new structural issue this wake — fixes (1)/(2)/(3) still pending OWNER selection; no Board Advisor action taken beyond this recurrence log.
