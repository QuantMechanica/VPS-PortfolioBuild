...
| research **critic** | scratch dir (writable) | **scratch dir only, with read access to the repo** — never the live tree granted write | **scratch dir only** | **primary** posture + **secondary** hash guard + **tertiary** code guards (below) |
...
**Secondary (detection) — adapter hash guard (R-D):** the adapter computes a `git status`/tree hash of `C:/QM/repo` **and** a content hash of the `D:/QM` verdict/evidence state paths (`farm_state.sqlite`, `D:/QM/strategy_farm/state/`, `D:/QM/reports/state/`) **before and after** every critic run. Any change = **failed run**: the output is **discarded**, `agent_chain` marks the stage status **`critic_wrote`**, and **no formatter run** follows.
...
| `farmctl` subcommands | **No `farmctl` subcommand is callable by a Kimi task**: the read-only critic posture (§3.5) denies shell, and the creator worktree carries no authority to run `farmctl approve-card` / `close-review` (those are orchestrator/human paths). |
