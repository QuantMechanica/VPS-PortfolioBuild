...
- **D1 batch-1 (agent-26, `27bab3ecc5`+`ff953f79e0`):** 8/8 fresh rows claimable, proofs PASS, 8/8 processed (7× FAIL_HARD + 10211 INVALID setfile-defect — honest economics); governed path = `enqueue-backtest --phase Q08 --from-work-item-id <Q07> --expected-current-ex5-sha256` (the briefed fresh-enqueue script never existed; V2 execution record is the authority).
