...
- **E1-C Option B — APPLIED** (`b85f9d3084`, pushed): NEWS_TAINT_TOTAL=99 · UNCHANGED_EQUIVALENT=91 · VERDICT_FLIP=0 · DELTA_NOT_EXACT=0 · EVIDENCE_INCOMPLETE=8 · HOLDS_RELEASED=91 · OPTION_A=8 · NEW_RUNNABLE=91 · FORECAST_HOURS_ADDED=0. Tainted and clean calendars have IDENTICAL event sets over every window (both empty in the shared upstream gap) — equivalence proven with the gates' own adjudication code. Condition-6 semantics test revealed plain per-row releases get re-applied by the sweep → implemented the code's own tamper-evident counter-marker pattern (91/91 released, fail-closed re-arm). Taint config byte-untouched; receipt-chain repair separate/untouched; 8 Option-A rows auto-route to full remeasurement; one drift hold (9973) held for next pass. Evidence `docs/ops/evidence/2026-09-16_e1c_optionb/REPORT.md`.

- **D1 FINAL (16:10Z): cohort complete.** 15/16 executed + 10280 parked pre-burn (setfile class). Tally: **FAIL_HARD 13 · FAIL_SOFT 1 · INVALID 2** (10211 `empty_strategy_params`; 10269 `DSR_V2_TRADE_OUTSIDE_SEALED_CALENDAR` — NEW evidence-integrity mechanism, Monte-Carlo FDR found a trade outside the sealed calendar window; diagnosed class, parked for the D6-adjacent repair wave, not chased). Cohort economics per the failure analysis: §8.2 DSR dominant; trend/MA all failed, MR escaped.
- **D1 COMPLETE — both batches** (agent-29, commits `f4feed3bb0`+`d94caff4d4`; evidence `2026-09-16_q08_amend_v3/batch2/`): all 8 batch-2 cards amended + dispositions (all 8 needed them — fresh enqueue does NOT auto-supersede) + fresh Q08 enqueues with full identity. **Newly claimable 8/8; proofs a–d PASS; 3 already ran (10287 FAIL_HARD end-to-end with DSR SEALED — legitimate; 9576 on T3; 1230 on T7), 5 queued.** Tool hardening committed: disposition backup moved inside FactoryMutationLock (lost the race ~8× vs the compile wave; 8/8 after). Pre-existing defect flagged for Fable, NOT patched per no-recursive-patching rule: `strategy_wiki_sync.py` build() folds the second-chance register into inputs_sha256 while lint() omits it — REJECTED-class nodes with register entries read permanently stale under lint (9576 proof: digest-with-register == writer output, card hash current). Excluded-5 + batch-1 byte-untouched attested. **D1 status: APPLIED (16/16 cards, decision fully executed).**
...
- **D2B/D4 (11731):** D2B originally STOPped; **D4 APPLIED in full** (agent-30, commits `98207717da`/`b314e812ec`/`b1a880812d`/`502a0cd95f`): code-bound force-rebuild authority for QM5_11731 only (fail-closed: hardcoded id + decision doc must agree) → COMPILE_EA `528c42c5` **COMPILE_OK** (0/0, hardening clean, 4 setfiles, fresh ex5 `6e6fbf44…`; old ex5 `d4c18bdc…` and done build task never touched) → exclusion lifted hash-bound `3f092f0a→a614bfc2` only after fresh identity existed → **EURUSD.DWX M5 canary `55d28c4a` PASS 12:17Z, pilot a–e CLEAN** (Model-4 Every Real Tick, 1,784 trades, sha-bound; economics weak PF 0.65 — for later gates) → **governed fanout RELEASED: GBPUSD (T10)/USDCHF (T8)/USDJPY (T7) active** → EURUSD Q04 economic FAIL (legitimate, taxonomy strategy). Incident: hourly `clean_repo_worktree -RestoreTrackedEx5` hygiene reverted the fresh ex5 mid-chain; the intake hash guard correctly REFUSED until bytes were recovered hash-verified from the sweep's own archive — guard proven. Runbook recommendation recorded: EA-scoped hygiene or commit ex5 with governed receipt.
...
- **MC truth reconciliation DONE** (agent-32, commit `5c88281890`, pushed): `factory_population.py` read-model (13 tests green; supersedes three-numbers while keeping its output shape) — live 12:42Z: OPEN=987 · TRUE_CLAIMABLE=1,077 (41478 census cells materialized post-Q02-PASS) · RESOURCE_FEASIBLE=4 · ACTIVE=4 · forecast 83h · BUFFER_LOW=GREEN. MC render relabeled (Queue cell = OPEN_PIPELINE_ROWS; new Fabrik-Population strip with four counts + parked split + forecast). The "981 open / 2011 parked" explained exactly: 981 = 12:06Z `pending_executable` count (raw census incl. governance/technical blocks); 2011 matches no measured state (parked swings ±800 with census materialization; dominated by 2,555 PRESCREEN_SKIPPED inert cells). Full row-exact classification at `docs/ops/evidence/2026-09-16_factory_reconciliation/RECONCILIATION_2026-09-16.md`. Top unlocks: RAM-44GB calibration (46h), news E1-C (93h, OWNER), news-runner spawn repair (18h), Q08 DSR cohort (11h), Q09 sealed-plan fix (4h).
- **11:10Z steady state (superseded by the unlock wave):** 11121 finished **FAIL_HARD** (09-14 pair fully resolved: 13137 INVALID-neighborhood, 11121 economic FAIL_HARD). ACTIVE=0 across two consecutive checks with true_claimable=1 (RAM-heavy Q06, legitimately resource-gated) = **NO_RUNNABLE_WORK (healthy, governance-gated)** — watchdog `no_runnable:True` agrees; no recovery action exists within standing authority (never force the heavy row). 11561 (D2A): Q02 PASS → **Q04 FAIL** (legitimate economic stop). Buffer forming for post-batch-2: Q07 done today for 9579, 10269, 10280 (future Q08-eligible once their batch-2 cards are amended). Remaining unlocks: OWNER Clarification 6 (batch 2), D4 (11731), D5 (41478), second-chance reviews (09-19), critic window (09-17/09-19).
