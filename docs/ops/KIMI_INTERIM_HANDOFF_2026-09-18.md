...
- **D3 (41478 seal):** PARTIAL + STOP at re-compile authority. Seal DONE (DRAFT `798d2eeb`→APPROVED `7d6b37e6`, g0_authority OWNER-DEC-Q12-SIBLING-41478-20260916, scope-recorded as measurement sibling only) · allocation `9784505ad8` (magic 414780000) · promotion `577907f208` · governed local compile ran **0 errors/0 warnings** and the deterministic `build_gate_hardening` gate fail-closed on a REAL latent defect in the staged source (`EA_TRADE_REQUEST_UNINITIALIZED`, bare struct) → behavior-preserving fix committed `89980c99cb` (source `b0771317…`). STOP: re-compile needs OWNER/router-ratified authority (fresh enqueue refused WORK_ITEMS_EXIST_AT_APPLY; repair-successor refused BUILD_TASK_BINDING_NOT_REQUESTED; force-rebuild allowlists exclude 41478). Service dry PROVES the flip (refusal now "no COMPILE_OK receipt" — seal/allocation/promotion all recognized); `--apply` correctly not run. Provider determination: codex NOT mandated (identical to 11731 lane). Evidence `docs/ops/evidence/2026-09-16_seal_41478/RECEIPT.md`, commit `c5f024cf91`. **Queued OWNER decision D5: ratify re-compile authority for 41478** (force-rebuild entry or register the sibling decision as source-repair authority).
