...
| **P0-21** | **Verify Tick Data Manager DarwinexZero GMT/DST settings** (Notion-canonical) | DevOps + Pipeline-Operator + Quality-Tech | **PASS on T1 — 2026-04-25.** Other Claude session produced `D:\QM\reports\setup\tick-data-timezone\REPORT_2026-04-25_test_eurusd_dst_match.md`: TEST-EURUSD verified against broker EURUSD across winter/US-DST-only/both-DST windows; 0-second offset, sub-pip OHLC differences. Plus DWX import automation operational (`docs/ops/DWX_IMPORT_AUTOMATION.md`) — hourly cron stages exported CSVs to `.DWX` custom symbols on T1. ~30 symbols (~500GB) exported by 2026-04-26. **Open follow-ups (do not block Phase 0 close):** (a) T2-T5 propagation via `Bases\Custom\` copy + per-terminal verification, (b) `portable.txt` drop in `D:\QM\mt5\T1\`, (c) AppData artifact `…BA21BF\` rename to `.disabled`, (d) capture TDS recipe (GMT base + DST + tick source + date range + contract spec) into `TICK_DATA_MANAGER_DARWINEX_TIME.md`. Owners: Pipeline-Operator (a), DevOps (b/c), Documentation-KM (d) — all post-Wave-0. |
...
| P0-32 | Sweep 1: Migrate critical Wave-0 bootstrap material | Claude Board Advisor | DONE 2026-04-26: 13 Paperclip agent system prompts byte-equivalent in `paperclip-prompts/`. `docs/ops/PROJECT_CHARTER.md`, `docs/ops/RESEARCH_METHODOLOGY_V2.md`, `docs/ops/CODEX_AUDIT_V5_UPGRADE_PLAN.md`, `docs/ops/PAPERCLIP_V2_COMPANY_DESIGN.md` from Notion. `strategy-seeds/cards/_TEMPLATE.md` authored. |
| P0-33 | Sweep 2: Migrate Wave-0/1 reference material | Claude Board Advisor | DONE 2026-04-26: Notion mirrors → `docs/ops/PAPERCLIP_OPERATING_SYSTEM_NOTION.md`, `docs/ops/EPISODE_GUIDE.md`, `docs/ops/GITHUB_REPO_PLAN.md`, `docs/ops/PROMPTS_AND_SCRIPTS_LIBRARY_INDEX.md`, `docs/ops/QUANTMECHANICA_DASHBOARD_SPEC.md`, `docs/ops/WEBSITE_RELAUNCH_PLAN.md`. Public Expense Log + CSV → `expenses/PUBLIC_EXPENSE_LOG.md` + `expenses/expenses.csv`. Brand assets (favicon.svg, og-image.svg, logo PNGs) → `branding/assets/`. |
| P0-34 | Sweep 3: Migrate V4 reference material | Claude Board Advisor | DONE 2026-04-26: V4 scripts → `reference/v4_scripts/` (8 Python+PS files: aggregator loop, dashboard refresh, processes builder, feed guard, scheduler scripts). V4 infra → `reference/v4_infra/` (9 files: MT5 watchdog, load shaping, WinRM bootstrap, GitHub issues sync, all with READMEs). V4 Controlling → `reference/v4_controlling/` (KPI builder, dashboard evidence map, refresh JS). V4 doc → `reference/v4_doc/` (decision-history, star-ea-reference). Voice samples → `branding/voice/` (Brand_Voice_Samples, Key_Messaging) for episode-script consistency. Each folder has README explaining read rule + V4-vs-V5 boundary. Reference material — not V5 input; Wave 0 reviews + decides port/redesign/discard per file. |
| P0-35 | OWNER decision sweep + DXZ-live-only redesign | Claude Board Advisor | DONE 2026-04-26: News Compliance ADR ACCEPTED Option A (P8 expanded, P9 not gated). DarwinexZero confirmed live-only — P10 redesigned as Live Burn-In Window (no demo intermediary, minimum-lot live + KS-test kill-switch). Brand Guide § 10 confirmed defaults (sync_brand_tokens.ps1 yes, mascot in framework no). TDS renewal SKIP. P0-21 marked PASS-on-T1. PHASE_FINAL_FOUNDER_COMMS + DWX_IMPORT_AUTOMATION committed. ADRs: `2026-04-26_dxz_live_only_and_p10_live_burn_in.md`, `2026-04-26_tds_renewal_skip.md`, plus `2026-04-25_news_compliance_variants_TBD.md` updated to ACCEPTED. |
