...
| P0-25 | Decide fate of V4 news-impact tooling | DevOps + Development + CTO | PENDING: locate `run_news_impact_tests.py` on laptop (Codex Task A). After locate, CTO decides port / rewrite / replace for V5 framework. No automatic adoption — V5 builds its own tooling per `V5_RESTART_SCOPE_BOUNDARY.md`. |
| P0-26 | Establish V5 EA framework | CTO + Development | New: define V5 Include lib, magic-number schema, set-file convention, risk-input standard, EA template, compile harness. Must be MT5-native, work on T1-T5 portable terminals, and be reviewed by Quality-Tech before first V5 EA build. Targets in `decisions/2026-04-26_v5_restart_clean_slate.md` and `docs/ops/V5_RESTART_SCOPE_BOUNDARY.md`. Repo skeleton expected under `framework/` (does not yet exist). |
