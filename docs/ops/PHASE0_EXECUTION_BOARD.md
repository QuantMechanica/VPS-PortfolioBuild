...
| P0-26 | Establish V5 EA framework | CTO + Development | DESIGN DONE + DEFAULTS CONFIRMED 2026-04-26: spec at `framework/V5_FRAMEWORK_DESIGN.md` (15-step Codex implementation order, 6 confirmed defaults). Decision: `decisions/2026-04-26_v5_framework_design.md`. **Implementation pending Codex**: ready to start, no further OWNER round-trip required. |
| P0-27 | Reconstruct V5 sub-gate spec | CTO + Quality-Tech | DONE 2026-04-26: `docs/ops/PIPELINE_V5_SUB_GATE_SPEC.md` written from surviving evidence after Codex confirmed `CODEX_PIPELINE_V2.1_SPEC/IMPACT/DIFF.md` are MISSING. Per-phase defaults for P3.5, P5, P5b, P5c, P6, P7, P10. Quality-Tech recalibration triggers documented; first pass blocked on first V5 EA distributions reaching the corresponding phase. |
| P0-28 | Migrate brand system + write V5 brand guide | OWNER + Documentation-KM | DONE 2026-04-26: `branding/QM_BRANDING_GUIDE.md` (V5-application-focused brand guide) and `branding/brand_tokens.json` (machine-readable design tokens) written from canonical Drive sources (`ClaudeDesign_Upload/01_Brand_System/`, `03_Website/style.css`, `04_Voice_Content/Brand_Voice_Samples.md`). Hard-rule enforcement wired into `framework/scripts/build_check.ps1` planned per P0-26 implementation. |
| P0-29 | Extend framework with trade-mgmt + chart UI | CTO + Development | DESIGN DONE 2026-04-26: `framework/V5_FRAMEWORK_DESIGN.md` extended with 7 new include modules — `QM_Branding`, `QM_OrderTypes`, `QM_Entry`, `QM_Exit`, `QM_StopRules`, `QM_TradeManagement`, `QM_ChartUI`. Implementation order updated to 25 steps. Per-EA in-chart dashboard widget specced (header + 6 stat tiles + status row + log footer, ChartObject API, OnTimer 1s refresh, brand-conformant). Implementation pending Codex (continues from P0-26). |
