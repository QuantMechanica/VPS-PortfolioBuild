# QM5_9103 build preflight — deterministic refusal

- Router task: `4382291e-4a39-4eda-a38d-745805885817`
- Task type / priority: `build_ea` / `10`
- Assigned agent: `codex`
- EA / expected slug: `9103` / `mql5-ichi-ten-ki`
...
PowerShell `Import-Csv` and filtered by exact string equality on `ea_id=9103`.

| Gate | Result |
|---|---|
| `ea_id_registry.csv` row count | `1` — active row, slug `mql5-ichi-ten-ki` |
| `magic_numbers.csv` row count | `0` — FAIL |
| Canonical EA directory | `framework/EAs/QM5_9103_mql5-ichi-ten-ki/` exists |
| Files beneath directory | only `QM5_9103_mql5-ichi-ten-ki.mq5`; no `.ex5`, setfile, or build-time card copy |
...
allocate all required symbol-slot magic rows for EA 9103. After those records
