...
def analyze(pair,term_source,native):
...
def main():
    ...
    for pair in intake['pairs']:
        row,series=analyze(pair,terms,native);rows.append(row)
    ...
    result={'schema':'qm.ftmo-exposed-shortlist-review/v1','task_id':'54729be7-8082-4a8c-afcf-e4497e6d9f4a','input_bindings':{str(p):sha(p) for p in (INTAKE,TERMS,NATIVE)},'native_spec_observed_at':native['checked_at_utc'],'native_spec_freshness':'HISTORICAL_SEPT6_NOT_CURRENT_AT_USE','native_specs':native['symbols'],'native_units_and_gaps':units,'account_type':'STANDARD_2STEP_100K_FREE_TRIAL_HISTORICAL_OWNER_RECORD','cost_class':'NATIVE_SEPT6_SWAP_PLUS_PROVISIONAL_PROVIDER_COMMISSION_NO_MATCHED_SPREAD','pairs':rows,'correlations':correlations,'sensitivity':sensitivity,'selected_pairs':[],'selection_reason':'ABSTAIN_UNTIL_CURRENT_IDENTITY_AND_FULL_COST_EVIDENCE','investigation_pairs':['QM5_10706:GBPUSD.DWX','QM5_11421:EURUSD.DWX','QM5_11422:USDCAD.DWX','QM5_13054:XTIUSD.DWX'],'reuse_tasks':['3032534e-eaf0-5b68-b09f-2127ebb315b0','f6d18a6e-0170-47c7-bbfd-e1b33d9d01c8'],'new_downloads':0,'native_runs':0}
