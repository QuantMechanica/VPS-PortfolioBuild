...
input datetime InpFromServer = D'2026.05.26 00:00';
...
bool HarvestSymbol(const string symbol, const string output_tag)
  {
   ...
   datetime last_bar = 0;
   const datetime available_start = ResolveHistoryStart(symbol);
   const datetime cached_last = (datetime)SeriesInfoInteger(symbol, PERIOD_M1, SERIES_LASTBAR_DATE);
   if(cached_last < InpFromServer)
      return WriteEmptyCoverage(symbol,output_tag,coverage_path,cached_last,"NO_CACHED_M1_IN_REQUESTED_WINDOW");
