...
tag = 'M08B_' + dt.datetime.now(dt.UTC).strftime('%Y%m%dT%H%M%SZ')
owner = 'ftmo_m1_bootstrap:' + tag
terminal = None
reservation = None
start = time.monotonic()
receipt = {'schema': 'qm.m08b-measurement/v1', 'tag': tag, 'started_utc': m.utc_now(),
           'timeout_seconds': 600, 'maximum_factory_budget_seconds': 3600,
           'symbols': list(m.DXZ_SYMBOLS), 'source': m.file_binding(SOURCE),
...
try:
    with m.exclusive_bootstrap_lock():
        ...
        terminal = m.select_free_factory_terminal(rows, claims, reservations)
        ...
        reservation = m.reserve_factory_terminal(terminal, owner, minutes=20)
        ...
        try:
            ...
            startup = m.prepare_startup_files(terminal_root=root, run_root=run,
                symbols=m.DXZ_SYMBOLS, output_tag=tag, login=login, server=server, chart_symbol='EURUSD')
            receipt['startup_bindings'] = startup
            expected = [m.local_harvest_paths(root, tag, s)[1] for s in m.DXZ_SYMBOLS]
            receipt['execution'] = m.launch_and_wait(terminal_root=root,
                ...
                timeout_seconds=600, challenge_identity=snapshot['challenge'][0])
