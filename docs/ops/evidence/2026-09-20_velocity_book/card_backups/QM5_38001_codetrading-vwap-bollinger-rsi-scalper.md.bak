...
type: strategy
strategy_id: QM5-38001-CODETRADING-VWAP-BOLLINGER-RSI-SCALPER-2026
variant_id: QM5-38001-CODETRADING-VWAP-BOLLINGER-RSI-SCALPER-2026_M5
source_id: codetrading-vwap-bollinger-rsi-scalper-official-source
status: APPROVED
g0_status: APPROVED
created: 2026-08-15
...
strategy_mechanic: codetrading-vwap-bollinger-rsi-scalper-quantitative-production-blueprint
source_citation: "CodeTrading (2022). Python Backtest of a Scalping Strategy with VWAP, Bollinger Bands and RSI. YouTube Channel Archive."
source_citations:
  - type: verified_quantitative_model
    citation: "CodeTrading (2022). Python Backtest of a Scalping Strategy with VWAP, Bollinger Bands and RSI. YouTube Channel Archive."
    quality_tier: A
    role: primary
sources:
  - "[[sources/codetrading-vwap-bollinger-rsi-scalper]]"
concepts:
...
# QM5_38001: CodeTrading VWAP & Bollinger Band Scalper

## 1. Economic & Quantitative Strategy Thesis

High-frequency mean-reversion scalper deconstructed from the CodeTrading Python backtests. Fades touches of the lower Bollinger Band when price is below intraday VWAP and RSI < 30, capturing mean-reverting snapbacks to VWAP equilibrium.
...
$$\text{VWAP}_t = \frac{\sum (P_i \times V_i)}{\sum V_i}, \quad \text{LowerBB} = \text{SMA}(20) - 2.0 \sigma, \quad \text{UpperBB} = \text{SMA}(20) + 2.0 \sigma$$
...
$$\text{Low}[1] \le \text{LowerBB}[1] \quad \text{AND} \quad \text{Close}[1] < \text{VWAP}[1] \quad \text{AND} \quad \text{RSI}(14)[1] \le 30.0 \quad \text{AND} \quad \text{Close}[1] > \text{Open}[1]$$

### 3.3 Short Entry Conditions (`Strategy_EntrySignal` $\rightarrow$ `SELL`)
$$\text{High}[1] \ge \text{UpperBB}[1] \quad \text{AND} \quad \text{Close}[1] > \text{VWAP}[1] \quad \text{AND} \quad \text{RSI}(14)[1] \ge 70.0 \quad \text{AND} \quad \text{Close}[1] < \text{Open}[1]$$

### 3.4 Position Exit Conditions (`Strategy_ExitSignal`)
* **Take Profit (TP)**: Set at the Session VWAP line (or $1.8 \times \text{SL\_Distance}$).
* **Stop Loss (SL)**: Set at entry $\mp 1.5 \times \text{ATR}(14, \text{M5})[1]$.
* **Trailing Stop**: Move to Break-Even once profit reaches $+1.0\text{R}$.
