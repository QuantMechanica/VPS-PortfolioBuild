...
type: strategy
strategy_id: QM5-38005-CODETRADING-ASCENDING-TRIANGLE-BREAKOUT-2026
variant_id: QM5-38005-CODETRADING-ASCENDING-TRIANGLE-BREAKOUT-2026_H1
source_id: codetrading-ascending-triangle-breakout-official-source
status: APPROVED
g0_status: APPROVED
created: 2026-08-15
...
strategy_mechanic: codetrading-ascending-triangle-breakout-quantitative-production-blueprint
source_citation: "CodeTrading (2022). How to Detect and Trade Triangle Patterns in Python. YouTube."
source_citations:
  - type: verified_quantitative_model
    citation: "CodeTrading (2022). How to Detect and Trade Triangle Patterns in Python. YouTube."
    quality_tier: A
    role: primary
sources:
  - "[[sources/codetrading-ascending-triangle-breakout]]"
concepts:
...
target_symbols: [XAUUSD.DWX, SP500.DWX, EURUSD.DWX]
...
# QM5_38005: CodeTrading Automated Ascending Triangle Breakout

## 1. Economic & Quantitative Strategy Thesis

Programmatically detects Ascending Triangle patterns: horizontal resistance line connecting at least 2 swing pivot highs and an ascending trendline connecting higher swing pivot lows. Triggers on confirmed breakout bar close.
...
$$\text{Resistance\_Slope}: |\beta_{\text{res}}| \le 0.05, \quad \text{Support\_Slope}: \beta_{\text{supp}} \ge +0.10$$
$$\text{Breakout}: \text{Close}[1] > \text{Resistance\_Level} + 2.0\text{ pips}$$
...
$$\text{Ascending Triangle Detected == TRUE} \quad \text{AND} \quad \text{Close}[1] > \text{Res\_Level} \quad \text{AND} \quad \text{Vol}[1] > 1.3 \times \text{SMA}(\text{Vol}, 20)[1]$$

### 3.3 Short Entry Conditions (`Strategy_EntrySignal` $\rightarrow$ `SELL`)
$$\text{Descending Triangle Detected == TRUE} \quad \text{AND} \quad \text{Close}[1] < \text{Supp\_Level} \quad \text{AND} \quad \text{Vol}[1] > 1.3 \times \text{SMA}(\text{Vol}, 20)[1]$$

### 3.4 Position Exit Conditions (`Strategy_ExitSignal`)
* **Take Profit (TP)**: Set to $1.0 \times \text{Triangle\_Height}$ ($1:2.0\text{ R:R}$).
* **Stop Loss (SL)**: Placed at the most recent swing pivot low $\mp 2.0\text{ pips}$.
* **Trailing Exit**: Trail with swing pivot lows.
...
|---|---|---|---|---|
| `InpPivotWindow` | `int` | `5` | `3 - 10` | Pivot high/low detection window |
| `InpMaxResSlope` | `double` | `0.05` | `0.02 - 0.10` | Maximum slope for flat resistance line |
...
* **Target symbols**: XAUUSD.DWX, SP500.DWX, EURUSD.DWX
