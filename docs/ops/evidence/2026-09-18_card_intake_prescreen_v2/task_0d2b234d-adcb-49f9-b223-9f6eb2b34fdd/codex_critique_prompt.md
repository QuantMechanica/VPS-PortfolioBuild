...
You are Codex, acting as an **independent cross-vendor critic**. This module was authored by
Claude and, by the module's own design, **cannot be self-activated**:
...
`farmctl` or `agent_router.py` commands that mutate state.
...
2. `tools/strategy_farm/tests/test_card_intake_prescreen_g0_economics.py` — its test suite
   ...
   read-only sandbox permitting test execution).
...
- **False positive** = the check would flag/FAIL a card that is actually fine (wastes
  reviewer time, or would incorrectly block a genuinely sound card once this becomes a
  blocking gate).
...
- Does `no_op_filter_lint`'s `NOT_APPLICABLE` result for all 208 worklog cards (vs. its `FAIL` on
  the QM5_41477 reference card) indicate the two shipped rules
  (`trade_cap_vs_window_count`, `time_stop_vs_flat_minute`) are too narrowly matched to one
  card's specific phrasing to catch the general defect class, i.e. a false-negative risk by
...
- Is the `0.03R` `COST_TO_TARGET_MAX_R` floor itself well-grounded (cite where it comes from), and
  is the commission figure sourced correctly from the OWNER-ratified registry with no
  hand-rolled fallback path that could silently mis-price cost?
...
You are running in a **read-only sandbox** — you cannot write any file, and you must not try.
Produce your full critique as your **final chat message** (markdown), structured as:
...
entire deliverable is the final message text.
