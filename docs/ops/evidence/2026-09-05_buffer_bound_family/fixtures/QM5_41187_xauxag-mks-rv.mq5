...
#property description "QM5_41187 XAU/XAG Monthly Signed-ECDF Distribution-Shift Reversion"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_41187 - XAU/XAG Monthly Signed-ECDF Distribution-Shift Reversion
// -----------------------------------------------------------------------------
...
// completed broker months. Split the gold-minus-silver log ratios into fixed
// older/newer blocks of six, scan their combined strict order, and measure
// both signed empirical-distribution count gaps. Fade only one dominant gap
// at the inclusive three-count boundary. Exposure is an opposite,
// equal-notional package held to the next month. Runtime is native,
// completed-price-only, deterministic, and one attempt per month.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41187;
...
input int    strategy_min_gap_count           = 3;
...
struct Strategy_SignedEcdfMetrics
  {
   int      month_count;
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41187 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_MKS_RV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41187_XAUXAG_MKS_RV";
...
bool Strategy_LoadMonthlySignedEcdf(
      const int current_month_key,
      Strategy_SignedEcdfMetrics &metrics)
  {
   ZeroMemory(metrics);
   metrics.label_csv = "";
   metrics.delta_csv = "";
   ...
   while(metrics.month_count < strategy_endpoint_count)
     {
      if(expected_month <= 0)
         return false;
      bool found = false;
      while(xau_index < xau_copied && xag_index < xag_copied)
        {
         ...
         if(!Strategy_LogRatio(xau_bars[xau_index].close,
                               ...
                               ratio))
            return false;
         if(metrics.month_count < 0 ||
            metrics.month_count >= ArraySize(newest_first_ratios) ||
            metrics.month_count >= ArraySize(newest_first_times))
            return false;
         newest_first_ratios[metrics.month_count] = ratio;
         newest_first_times[metrics.month_count] = xau_time;
         if(metrics.month_count == 0)
            metrics.newest_endpoint_time = xau_time;
         ++metrics.month_count;
         ++xau_index;
         ++xag_index;
         found = true;
   ...
   if(metrics.month_count != strategy_endpoint_count ||
      metrics.newest_endpoint_time <= 0 ||
      (long)(g_current_host_bar - metrics.newest_endpoint_time) < 0L ||
      (long)(g_current_host_bar - metrics.newest_endpoint_time) >
         (long)strategy_max_endpoint_gap_days * 86400L)
      return false;
   ...
   metrics.endpoint_displacement =
      chronological_ratios[strategy_endpoint_count - 1] -
      chronological_ratios[0];
   if(!MathIsValidNumber(metrics.endpoint_displacement))
      return false;

   // Exact ties invalidate the ECDF label order. The broker-month attempt was
   // already persisted and therefore remains consumed.
   ...
   if(metrics.tie_count > 0)
      return false;

   double sorted_ratios[];
   int sorted_labels[];
   if(ArrayResize(sorted_ratios, strategy_endpoint_count) !=
         strategy_endpoint_count ||
      ArrayResize(sorted_labels, strategy_endpoint_count) !=
         strategy_endpoint_count)
      return false;

   for(int index = 0; index < strategy_endpoint_count; ++index)
     {
      sorted_ratios[index] = chronological_ratios[index];
      sorted_labels[index] = (index < strategy_block_size) ? -1 : 1;
     }

   // Stable insertion sort is deterministic; ties were rejected above.
   for(int index = 1; index < strategy_endpoint_count; ++index)
     {
      const double key_value = sorted_ratios[index];
      const int key_label = sorted_labels[index];
      int cursor = index - 1;
      while(cursor >= 0 && sorted_ratios[cursor] > key_value)
        {
         sorted_ratios[cursor + 1] = sorted_ratios[cursor];
         sorted_labels[cursor + 1] = sorted_labels[cursor];
         --cursor;
        }
      sorted_ratios[cursor + 1] = key_value;
      sorted_labels[cursor + 1] = key_label;
     }
   ...
   for(int index = 0; index < strategy_endpoint_count; ++index)
     {
      if(index > 0 && !(sorted_ratios[index] > sorted_ratios[index - 1]))
         return false;
      if(sorted_labels[index] < 0)
         ++old_seen;
      else if(sorted_labels[index] > 0)
         ++new_seen;
      else
         return false;

      const int delta = old_seen - new_seen;
      metrics.dplus_count = MathMax(metrics.dplus_count, delta);
      metrics.dminus_count = MathMax(metrics.dminus_count, -delta);
      ...
      metrics.label_csv += (sorted_labels[index] < 0) ? "O" : "N";
      metrics.delta_csv += IntegerToString(delta);
     }

   metrics.old_count = old_seen;
   metrics.new_count = new_seen;
   if(metrics.scanned_count != strategy_endpoint_count ||
      metrics.old_count != strategy_block_size ||
      metrics.new_count != strategy_block_size ||
      metrics.dplus_count < 0 || metrics.dplus_count > strategy_block_size ||
      metrics.dminus_count < 0 || metrics.dminus_count > strategy_block_size ||
      StringLen(metrics.label_csv) <= 0 ||
      StringLen(metrics.delta_csv) <= 0)
      return false;

   metrics.direction = 0;
   // direction > 0 means BUY XAU / SELL XAG. A newer distribution displaced
   // high is faded short-ratio; a displaced-low distribution is faded long.
   if(metrics.dplus_count >= strategy_min_gap_count &&
      metrics.dplus_count > metrics.dminus_count)
      metrics.direction = -1;
   else if(metrics.dminus_count >= strategy_min_gap_count &&
           metrics.dminus_count > metrics.dplus_count)
      metrics.direction = 1;
   return true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41187_XAUXAG_MKS_RV_HOST";
   ...
   Strategy_SignedEcdfMetrics metrics;
   const bool valid =
      Strategy_LoadMonthlySignedEcdf(g_signal_month_key, metrics);

   string state = "monthly_signed_ecdf_failed";
   if(!valid && metrics.tie_count > 0)
      state = "ratio_tie_consumed_flat";
   else if(valid && metrics.direction > 0)
      state = "newer_low_distribution_fade_long_xau";
   else if(valid && metrics.direction < 0)
      state = "newer_high_distribution_fade_short_xau";
   else if(valid && metrics.dplus_count >= strategy_min_gap_count &&
           metrics.dplus_count == metrics.dminus_count)
      state = "tied_gap_consumed_flat";
   else if(valid)
      state = "weak_gap_consumed_flat";

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"month_count\":%d,\"newest_endpoint_time\":%I64d,\"endpoint_displacement\":%.12e,\"scanned_count\":%d,\"old_count\":%d,\"new_count\":%d,\"label_csv\":\"%s\",\"delta_csv\":\"%s\",\"dplus_count\":%d,\"dminus_count\":%d,\"min_gap_count\":%d,\"tie_count\":%d,\"direction\":%d,\"state\":\"%s\"}",
                            g_signal_month_key,
                            metrics.month_count,
                            (long)metrics.newest_endpoint_time,
                            metrics.endpoint_displacement,
                            metrics.scanned_count,
                            metrics.old_count,
                            metrics.new_count,
                            ...
                            metrics.delta_csv,
                            metrics.dplus_count,
                            metrics.dminus_count,
                            strategy_min_gap_count,
                            metrics.tie_count,
                            metrics.direction,
                            state));
   if(!valid || metrics.direction == 0)
      return false;

   Strategy_OpenPair(metrics.direction);
   return false;
  }
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG twelve-month signed-ECDF distribution-shift reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41187\",\"ea\":\"xauxag-mks-rv\"}");
