...
#property description "QM5_41186 XTI/XNG Monthly Median-Runs Ratio Reversion"
...
// completed broker months. Strict-rank the oil-minus-gas log ratios, omit
// the unique median rank, count every chronological low/high regime run,
// and fade the newest nonmedian regime only when the run count is <= 7.
// Exposure is an opposite, equal-notional package held to the next month.
// Runtime is native, completed-price-only, deterministic, and one attempt per
// month.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41186;
...
struct Strategy_MedianRunsMetrics
  {
   int      month_count;
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XTIXNG_MEDRUN_RV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41186_XTIXNG_MEDRUN_RV";
...
bool Strategy_LoadMonthlyMedianRuns(
      const int current_month_key,
      Strategy_MedianRunsMetrics &metrics)
  {
   ZeroMemory(metrics);
   metrics.omitted_index = -1;
   metrics.rank_csv = "";
   metrics.sign_csv = "";
      ...
      while(xti_index < xti_copied && xng_index < xng_copied)
        {
         ...
         newest_first_times[metrics.month_count] = xti_time;
         if(metrics.month_count == 0)
            metrics.newest_endpoint_time = xti_time;
         ++metrics.month_count;
         ++xti_index;
         ++xng_index;
         found = true;
   ...
   // A strict rank permutation is mandatory. Exact ties invalidate the
   // signal and the already-persisted broker-month attempt stays consumed.
   ...
   if(ArrayResize(ranks, strategy_endpoint_count) != strategy_endpoint_count ||
      ArrayResize(signs, strategy_endpoint_count - 1) != 12)
      return false;
   bool seen_ranks[14];
   ...
   for(int index = 0; index < strategy_endpoint_count; ++index)
     {
      ...
      if(index > 0)
         metrics.rank_csv += ",";
      metrics.rank_csv += IntegerToString(rank);
     }
   ...
   if(metrics.rank_sum != 91 || StringLen(metrics.rank_csv) <= 0)
      return false;

   int sign_count = 0;
   for(int index = 0; index < strategy_endpoint_count; ++index)
     {
      const int rank = ranks[index];
      if(rank == 7)
        {
         ++metrics.median_count;
         metrics.omitted_index = index;
         continue;
        }
      const int sign = (rank < 7) ? -1 : 1;
      if(sign_count < 0 || sign_count >= ArraySize(signs))
         return false;
      ...
      if(sign < 0)
         ...
         ++metrics.high_count;
      if(StringLen(metrics.sign_csv) > 0)
         metrics.sign_csv += ",";
      metrics.sign_csv += IntegerToString(sign);
     }

   if(sign_count != 12 || metrics.median_count != 1 ||
      metrics.omitted_index < 0 || metrics.omitted_index >= 13 ||
      metrics.low_count != 6 || metrics.high_count != 6 ||
      StringLen(metrics.sign_csv) <= 0)
      return false;
   ...
   if(metrics.run_count < 2 || metrics.run_count > 12)
      return false;

   metrics.newest_rank = ranks[strategy_endpoint_count - 1];
   metrics.direction = 0;
   // direction > 0 means BUY XTI / SELL XNG. The qualified persistent
   // median regime is traded contrarian, never magnitude-sized.
   if(metrics.run_count <= strategy_max_runs)
     {
      if(metrics.newest_rank > 7)
         metrics.direction = -1;
      else if(metrics.newest_rank < 7)
         metrics.direction = 1;
     }
   return true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41186_XTIXNG_MEDRUN_RV_HOST";
   ...
   Strategy_MedianRunsMetrics metrics;
   const bool valid =
      Strategy_LoadMonthlyMedianRuns(g_signal_month_key, metrics);

   string state = "monthly_median_runs_failed";
   if(!valid && metrics.tie_count > 0)
      state = "ratio_tie_consumed_flat";
   else if(valid && metrics.direction > 0)
      state = "persistent_low_ratio_fade_long_xti";
   else if(valid && metrics.direction < 0)
      state = "persistent_high_ratio_fade_short_xti";
   else if(valid && metrics.newest_rank == 7)
      state = "newest_median_flat";
   else if(valid && metrics.run_count > strategy_max_runs)
      state = "excessive_runs_flat";
   else if(valid)
      state = "median_runs_flat";

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"month_count\":%d,\"newest_endpoint_time\":%I64d,\"endpoint_displacement\":%.12e,\"rank_sum\":%d,\"rank_csv\":\"%s\",\"median_count\":%d,\"omitted_index_zero_based\":%d,\"sign_csv\":\"%s\",\"low_count\":%d,\"high_count\":%d,\"run_count\":%d,\"max_runs\":%d,\"newest_rank\":%d,\"tie_count\":%d,\"direction\":%d,\"state\":\"%s\"}",
                            g_signal_month_key,
                            metrics.month_count,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XTI/XNG thirteen-month median-runs ratio-reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41186\",\"ea\":\"xtixng-median-runs-rv\"}");
