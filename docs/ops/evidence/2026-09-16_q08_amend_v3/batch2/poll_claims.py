...
while time.time() < deadline:
    ...
    act = sum(1 for nid in BY_NEW if states.get(nid) and states[nid]["status"] == "active")
    fin = sum(1 for nid in BY_NEW if states.get(nid) and states[nid]["status"] in ("done", "failed"))
    print(f"[{now[:19]}] active={act} finished={fin} claims={len(claims)}/8 -> { {k[:8]: (v['terminal']) for k, v in claims.items()} }", flush=True)
    if len(claims) >= 8:
