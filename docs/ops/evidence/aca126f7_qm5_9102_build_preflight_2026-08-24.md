# QM5_9102 build preflight — deterministic refusal

- Router task: `aca126f7-1caf-4769-828a-129b2e315d4a`
- Task type / priority: `build_ea` / `10`
- Assigned agent: `codex`
- EA / expected slug: `9102` / `mql5-ichi-price-ki`
...
PowerShell `Import-Csv` and filtered by exact string equality on `ea_id=9102`.

| Gate | Result |
|---|---|
| `ea_id_registry.csv` row count | `1` — active row, slug `mql5-ichi-price-ki` |
| `magic_numbers.csv` row count | `0` — FAIL |
| Canonical EA directory | `framework/EAs/QM5_9102_mql5-ichi-price-ki/` exists |
| Files beneath directory | only `QM5_9102_mql5-ichi-price-ki.mq5`; no `.ex5`, setfile, or build-time card copy |
...
allocate all required symbol-slot magic rows for EA 9102. After those records
