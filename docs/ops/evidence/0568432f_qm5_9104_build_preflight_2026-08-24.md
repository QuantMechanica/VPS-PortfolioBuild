# QM5_9104 build preflight — deterministic refusal

- Router task: `0568432f-2e9e-4ebc-9a91-808eac072508`
- Task type / priority: `build_ea` / `10`
- Assigned agent: `codex`
- EA / expected slug: `9104` / `mql5-bb-sideways`
...
PowerShell `Import-Csv` and filtered by exact string equality on `ea_id=9104`.

| Gate | Result |
|---|---|
| `ea_id_registry.csv` row count | `1` — active row, slug `mql5-bb-sideways` |
| `magic_numbers.csv` row count | `0` — FAIL |
| Canonical EA directory | `framework/EAs/QM5_9104_mql5-bb-sideways/` exists |
| Files beneath directory | only `QM5_9104_mql5-bb-sideways.mq5`; no `.ex5`, setfile, or build-time card copy |
...
allocate all required symbol-slot magic rows for EA 9104. After those records
