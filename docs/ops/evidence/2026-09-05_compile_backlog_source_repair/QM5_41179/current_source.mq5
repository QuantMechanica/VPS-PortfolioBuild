...
#property description "QM5_41179 XTI/XNG Fourteen-Month Cox-Stuart Paired-Sign Reversion"
...
// latest exactly matched XTI/XNG close pair in each of fourteen consecutive
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41179 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XTIXNG_MCOXSTUART_RV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41179_XTIXNG_MCOXSTUART_RV";
...
bool Strategy_LoadMonthlyPairedSign(
      ...
      int &direction)
  {
   ...
   const int xti_copied =
      CopyRates(g_leg_xti, // perf-allowed: one bounded fourteen-month scan behind a consumed monthly attempt.
                PERIOD_D1, 1, strategy_history_bars_d1, xti_bars);
   const int xng_copied =
      CopyRates(g_leg_xng, // perf-allowed: one bounded fourteen-month scan behind a consumed monthly attempt.
   ...
   // direction > 0 means BUY XTI / SELL XNG. Fade only a strict five-of-seven
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41179_XTIXNG_MCOXSTUART_RV_HOST";
   ...
   if(valid && direction > 0)
      state = "five_negative_fade_long_xti";
   else if(valid && direction < 0)
      state = "five_positive_fade_short_xti";
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XTI/XNG fourteen-month Cox-Stuart paired-sign reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41179\",\"ea\":\"xtixng-mcoxstuart-rv\"}");
