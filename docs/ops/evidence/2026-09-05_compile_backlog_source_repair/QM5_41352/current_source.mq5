...
#property description "QM5_41352 WTI Monthly ADF and Robust Variance-Ratio Agreement Trend"
...
//   - require ADF t >= -2.594 and robust q=4 VR z > 1.64485362695147
...
input int    strategy_vr_return_count       = 32;
input int    strategy_vr_q                  = 4;
input double strategy_vr_z_min              = 1.64485362695147;
...
struct Strategy_SignalMetrics
  {
   ...
   double adf_t;
   int vr_return_count;
   double vr_return_mean;
   double vr_squared_sum;
   double variance_ratio;
   double robust_variance;
   double vr_z;
   double momentum_12;
   string level_path;
...
bool Strategy_VarianceRatioCore(const double &values[],
                                double &return_mean,
                                double &squared_sum,
                                double &variance_ratio,
                                double &robust_variance,
                                ...
                                string &vr_path)
  {
   ...
   if(value_count != 32 || strategy_vr_return_count != 32 ||
      strategy_vr_q != 4 ||
      MathAbs(strategy_vr_z_min - 1.64485362695147) >
         0.000000000000001 ||
      strategy_energy_floor != 0.000000000000000001)
      return false;
   ...
   for(int index = 0; index < value_count; ++index)
     {
      ...
      if(!MathIsValidNumber(centered) || !MathIsValidNumber(square) ||
         ...
         return false;
      squared_sum += square;
      if(StringLen(vr_path) > 0)
         vr_path += ",";
      vr_path += DoubleToString(values[index], 12);
     }
   if(!MathIsValidNumber(squared_sum) ||
      squared_sum <= strategy_energy_floor)
      return false;

   const double squared_sum_sq = squared_sum * squared_sum;
   if(!MathIsValidNumber(squared_sum_sq) ||
      squared_sum_sq <= strategy_energy_floor)
      return false;
   ...
   for(int lag = 1; lag < strategy_vr_q; ++lag)
     {
      ...
      for(int index = lag; index < value_count; ++index)
        {
         const double current_delta = values[index] - return_mean;
         const double prior_delta = values[index - lag] - return_mean;
         const double cross = current_delta * prior_delta;
         const double robust_term = current_delta * current_delta *
                                    prior_delta * prior_delta;
         if(!MathIsValidNumber(cross) ||
            !MathIsValidNumber(robust_term) || robust_term < 0.0)
            return false;
         lag_cross_sum += cross;
         robust_numerator += robust_term;
        }
      ...
      if(!MathIsValidNumber(rho) || !MathIsValidNumber(delta) ||
         delta < 0.0 || !MathIsValidNumber(vr_weight) ||
         !MathIsValidNumber(robust_weight))
         return false;
      variance_ratio += vr_weight * rho;
      robust_variance += robust_weight * robust_weight * delta;
     }
   if(!MathIsValidNumber(variance_ratio) ||
      !MathIsValidNumber(robust_variance) ||
      robust_variance <= strategy_energy_floor)
      return false;

   const double robust_se = MathSqrt(robust_variance);
   if(!MathIsValidNumber(robust_se) ||
      robust_se <= strategy_energy_floor)
      return false;
   vr_z = (variance_ratio - 1.0) / robust_se;
   return (MathIsValidNumber(vr_z) && StringLen(vr_path) > 0);
  }
bool Strategy_AgreementSignal(const double &closes[],
                              Strategy_SignalMetrics &metrics)
  {
   ...
   metrics.vr_return_count = strategy_vr_return_count;
   metrics.direction = 0;
   metrics.persistence_qualified = false;
   metrics.adf_qualified = false;
   metrics.vr_qualified = false;
   ...
   double returns[32];
   for(int index = 0; index < strategy_vr_return_count; ++index)
     {
      const int right_index = index + 28;
      const int left_index = index + 27;
      if(index < 0 || index >= 32 || left_index < 0 ||
   ...
   if(!Strategy_VarianceRatioCore(
         returns,
         metrics.vr_return_mean,
         metrics.vr_squared_sum,
         metrics.variance_ratio,
         metrics.robust_variance,
         metrics.vr_z,
         metrics.vr_path))
      return false;
   metrics.vr_qualified = (metrics.vr_z > strategy_vr_z_min);
   metrics.persistence_qualified =
      (metrics.adf_qualified && metrics.vr_qualified);
...
bool Strategy_AgreementReferenceSelfTest()
  {
   double persistent_up[60];
   double persistent_down[60];
   double adf_only[60];
   double vr_only[60];
   double constant_closes[60];
   double up_level = 4.0;
   double down_level = 5.0;
   double up_return = 0.005;
   double down_return = -0.005;
   for(int index = 0; index < 60; ++index)
     {
      if(index > 0)
        {
         const double innovation =
            0.0015 * MathSin(0.71 * (double)index) +
            0.0010 * MathCos(1.37 * (double)index);
         up_return = 0.7 * up_return + 0.0009 + innovation;
         down_return = 0.7 * down_return - 0.0009 - innovation;
         up_level += up_return;
         down_level += down_return;
        }
      persistent_up[index] = MathExp(up_level);
      persistent_down[index] = MathExp(down_level);
      adf_only[index] =
         MathExp(4.0 + 0.012 * (double)index +
                 0.025 * MathSin(0.73 * (double)index) +
                 0.009 * MathCos(1.91 * (double)index));
      vr_only[index] =
         MathExp(4.0 + 0.080 * MathSin(0.70 * (double)index) +
                 0.030 * MathCos(0.41 * (double)index));
      constant_closes[index] = MathExp(4.2);
     }

   Strategy_SignalMetrics metrics;
   if(!Strategy_AgreementSignal(persistent_up, metrics) ||
      !metrics.adf_qualified || !metrics.vr_qualified ||
      !metrics.persistence_qualified || metrics.direction != 1 ||
      MathAbs(metrics.adf_t - (-0.36016068469282697)) >
         0.000000001 ||
      MathAbs(metrics.variance_ratio - 1.8128203309434223) >
         0.000000001 ||
      MathAbs(metrics.robust_variance - 0.10997537120057366) >
         0.000000001 ||
      MathAbs(metrics.vr_z - 2.451019913634225) >
         0.000000001 ||
      MathAbs(metrics.momentum_12 - 0.03751537658058446) >
         0.000000001)
      return false;

   if(!Strategy_AgreementSignal(persistent_down, metrics) ||
      !metrics.adf_qualified || !metrics.vr_qualified ||
      !metrics.persistence_qualified || metrics.direction != -1 ||
      MathAbs(metrics.adf_t - (-0.36016068469305773)) >
         0.000000001 ||
      MathAbs(metrics.vr_z - 2.451019913634226) >
         0.000000001 ||
      MathAbs(metrics.momentum_12 - (-0.03751537658058357)) >
         0.000000001)
      return false;

   if(!Strategy_AgreementSignal(adf_only, metrics) ||
      !metrics.adf_qualified || metrics.vr_qualified ||
      metrics.persistence_qualified || metrics.direction != 0 ||
      MathAbs(metrics.adf_t - (-0.28754973622603336)) >
         0.000000001 ||
      MathAbs(metrics.vr_z - 0.5572010656725933) >
         0.000000001)
      return false;

   if(!Strategy_AgreementSignal(vr_only, metrics) ||
      metrics.adf_qualified || !metrics.vr_qualified ||
      metrics.persistence_qualified || metrics.direction != 0 ||
      MathAbs(metrics.adf_t - (-32.508096212950704)) >
         0.000000001 ||
      MathAbs(metrics.vr_z - 3.1643986637281003) >
         0.000000001)
      return false;
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   else
     {
      ...
      if(!g_signal_valid)
         g_signal_state = "agreement_validation_failed";
              ...
              !g_signal_metrics.vr_qualified)
         g_signal_state = "both_gates_reject_consumed_flat";
      else if(!g_signal_metrics.adf_qualified)
         g_signal_state = "adf_rejects_vr_only_consumed_flat";
      else if(!g_signal_metrics.vr_qualified)
         g_signal_state = "vr_rejects_adf_only_consumed_flat";
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"endpoint_count\":%d,\"level_count\":%d,\"vr_return_count\":%d,\"mean_y\":%.16f,\"mean_z\":%.16f,\"mean_w\":%.16f,\"szz\":%.16f,\"sww\":%.16f,\"szw\":%.16f,\"szy\":%.16f,\"swy\":%.16f,\"determinant\":%.16f,\"alpha\":%.16f,\"gamma\":%.16f,\"phi\":%.16f,\"sse\":%.16f,\"residual_variance\":%.16f,\"se_gamma\":%.16f,\"adf_t\":%.12f,\"adf_qualified\":%s,\"vr_return_mean\":%.12f,\"vr_squared_sum\":%.16f,\"variance_ratio\":%.12f,\"robust_variance\":%.16f,\"vr_z\":%.12f,\"vr_qualified\":%s,\"persistence_qualified\":%s,\"momentum_12\":%.12f,\"levels\":\"%s\",\"returns\":\"%s\",\"regression\":\"%s\",\"vr_returns\":\"%s\",\"oldest_close\":%.10f,\"newest_close\":%.10f,\"oldest_endpoint\":%I64d,\"newest_endpoint\":%I64d,\"state\":\"%s\"}",
                            ...
                            g_signal_metrics.vr_return_count,
                            ...
                            g_signal_metrics.vr_return_mean,
                            g_signal_metrics.vr_squared_sum,
                            g_signal_metrics.variance_ratio,
                            g_signal_metrics.robust_variance,
                            g_signal_metrics.vr_z,
                            g_signal_metrics.vr_qualified ? "true" : "false",
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41352 ||
      qm_magic_slot_offset != 0 ||
      QM_FrameworkMagic() != 413520000 ||
      qm_rng_seed != 42)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41352_WTI_ADF_VR";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41352 ||
      qm_magic_slot_offset != 0)
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI monthly ADF and robust variance-ratio agreement trend holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41352_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   g_entry_month_state_key =
      StringFormat("QM5_41352_ENTRY_MONTH_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41352\",\"ea\":\"wti-adf-vr-agree-tr\"}");
