   ...
   while(cursor<InpEndBrokerEpoch && ok)
     {
      ...
      // written_rows only advances when FlushBar sees the *next* minute, so
      // the chunk's final in-progress bar is still pending here and gets
      // attributed to the next chunk (or the trailing FlushBar below for the
      // symbol's last chunk) -- a known +/-1 bar boundary carry-over, dwarfed
      // by the thousands-of-minutes chunk floor this row feeds downstream.
      JournalChunk(journal_handle,symbol,chunk_start,chunk_end,"bars",0,
                   (int)(written_rows-rows_before_chunk),0,"CHUNK_ROWS");
      cursor=chunk_end;
     }
