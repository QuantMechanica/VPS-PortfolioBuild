...
review-and-approve act, not a rebuild. Before signing, note two items — item 1 (epoch) is now **DECIDED**; item 2 (ordering) still needs sequencing:

1. **`deployment_epoch_utc` — DECIDED** `2026-07-19T13:50:00Z` (go-live, "went-live"
   semantics) per OWNER receipt `decisions/2026-09-04_owner_receipts_briefing_2_4.md`
   (`OWNER-DEC-BOOK-V2V4V6-EPOCH-20260904`); supersedes the pointer's current
   `2026-07-24T06:42:00Z`. §6 Risk-1.
...
| 8 | Epoch semantics | `deployment_epoch_utc` | **`2026-07-19T13:50:00+00:00`** (DECIDED — receipt `2026-09-04_owner_receipts_briefing_2_4.md`) | Downstream "days-live" math shifts 5 days once re-minted from the current 07-24 pointer (§6 Risk-1). |
...
**Pre-req A — epoch DECIDED (§6 Risk-1).** OWNER adopted the go-live epoch
`2026-07-19T13:50:00Z` ("went-live" semantics) — receipt
`decisions/2026-09-04_owner_receipts_briefing_2_4.md` (`OWNER-DEC-BOOK-V2V4V6-EPOCH-20260904`);
go-live source `decisions/2026-07-19_t_live_dxz_sunday_final_book.md`. The decided value is set
verbatim in `--deployment-epoch-utc` below (`2026-07-19T13:50:00+00:00`).
...
python tools/strategy_farm/generate_live_deployment_pointer.py \
  ...
  --expected-server "Darwinex-Live" \
  --expected-phase "DXZ_LIVE" \
  --deployment-epoch-utc "2026-07-19T13:50:00+00:00" \
...
**Risk-1 — epoch DECIDED (OWNER 2026-09-04).** The pointer's
...
burn-in / "days-live" math shifts by **5 days** once the pointer is re-minted with 07-19 (07-19 is 5 days before the pointer's current 07-24 epoch, so the book counts as live 5 days longer; `scripts/sunday_livevsbook_compare.ps1` reads the epoch from the pointer, so this changes only after the OWNER re-mint). Full analysis:
`docs/ops/evidence/2026-08-22_sp-a1_deployment_epoch_discrepancy_note.md`. **DECIDED (OWNER 2026-09-04):**
`deployment_epoch_utc = 2026-07-19T13:50:00Z` (go-live) with "went-live" semantics — receipt
`decisions/2026-09-04_owner_receipts_briefing_2_4.md` (`OWNER-DEC-BOOK-V2V4V6-EPOCH-20260904`).
