...
with sync_playwright() as p:
 ...
 for name,url in PAGES.items():
  for width in (1440,375):
   ...
   page.goto('http://127.0.0.1:8772'+url,wait_until='load',timeout=30000)
   page.evaluate('document.fonts.ready')
   if name=='archive': page.wait_for_function("document.querySelector('#mx-count').textContent.includes('showing first 200')")
   page.screenshot(path=str(OUT/f'after_{name}_{width}.png'))
   if name=='archive':
    page.locator('.mx-scroll').scroll_into_view_if_needed();page.screenshot(path=str(OUT/f'after_matrix_{width}.png'))
   if name=='detail':
    page.locator('.gate-step').first.scroll_into_view_if_needed();page.screenshot(path=str(OUT/f'after_ledger_{width}.png'))
