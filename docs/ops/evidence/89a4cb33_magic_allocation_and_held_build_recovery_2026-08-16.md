...
Router ownership confirmation: a brief attempted BACKLOG transition was identified as conflicting with the cycle's higher-priority “ignore REVIEW” rule and corrected within the same cycle. Before the exact rollback, SQLite backup API wrote `D:\QM\strategy_farm\state\backups\farm_state_before_review_task_exact_restore_20260816T120103Z.sqlite`. The six mutable fields (`state`, `assigned_agent`, `artifact_path`, `verdict`, `payload_json`, and `updated_at`) were restored for all 10/10 tasks from the pre-change snapshot, with zero field mismatches. Thus every task is again assigned to Codex in `REVIEW`, with artifact `C:\QM\repo\docs\ops\evidence\fa922736_stalled_build_recovery_2026-08-16.md` and verdict `PRECHECK_DEFERRED: governed magic allocation overlaps unrelated dirty canonical registry transaction`. No implementation or source mutation was made for these ten EAs.
