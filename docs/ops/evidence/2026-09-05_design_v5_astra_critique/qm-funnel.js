...
(function () {
  ...
  function styles() {
    ...
    s.textContent = `
      ...
      #gate-funnel .funnel__control:hover{background:var(--steel-soft,rgba(41,84,212,.1))}
      #gate-funnel :is(button,summary,.qm-funnel__viewport):focus-visible{outline:2px solid var(--steel,#2954d4);outline-offset:4px}
  ...
  function Funnel(el, m) {
    ...
    this.change = function () { self.sync(); };
  ...
  Funnel.prototype.fit = function () {
    ...
    this.colors = { ink: css.getPropertyValue('--ink').trim() || '#0f1720',
      ...
      steel: css.getPropertyValue('--steel').trim() || '#2954d4',
      wash: css.getPropertyValue('--steel-soft').trim() || 'rgba(41,84,212,.10)' };
    this.draw();
  };
  Funnel.prototype.half = function (x) {
    return x >= this.neck ? this.tube : this.rim + (this.tube - this.rim) * Math.max(0, (x - this.left) / (this.neck - this.left));
  };
  Funnel.prototype.outline = function () {
    var c = this.ctx;
    c.beginPath(); c.moveTo(this.left, this.cy - this.rim);
    c.lineTo(this.neck, this.cy - this.tube); c.lineTo(this.right, this.cy - this.tube);
    c.lineTo(this.right, this.cy + this.tube); c.lineTo(this.neck, this.cy + this.tube);
    c.lineTo(this.left, this.cy + this.rim); c.closePath();
  };
  Funnel.prototype.draw = function () {
    ...
    // Short parallel outlet and open elliptical mouth; no closed capsule cap.
    c.strokeStyle = col.ink; c.lineWidth = 1.4; c.globalAlpha = 0.72;
    c.beginPath(); c.moveTo(this.left, this.cy - this.rim); c.lineTo(this.neck, this.cy - this.tube); c.lineTo(this.right, this.cy - this.tube);
    c.moveTo(this.left, this.cy + this.rim); c.lineTo(this.neck, this.cy + this.tube); c.lineTo(this.right, this.cy + this.tube); c.stroke();
    c.beginPath(); c.ellipse(this.left, this.cy, 10, this.rim, 0, 0, Math.PI * 2); c.stroke();
    c.globalAlpha = 1;
    ...
    for (var i = 0; i < total; i++) {
      ...
      var alpha = 0.8, tail = 9;
      if (x > stop) {
        var age = (x - stop) / 35;
        if (age >= 1) continue;
        x = stop - age * 3; y += age * 20; alpha *= 1 - age; tail = 5;
        c.strokeStyle = col.ink;
      } else c.strokeStyle = col.steel;
      c.globalAlpha = alpha; c.lineWidth = 1.25;
  ...
  Funnel.prototype.sync = function () {
    ...
    function tick(now) {
      ...
      if (self.last) self.t += Math.min((now - self.last) / 1000, 0.08) * (self.media.matches ? 0.52 : 1);
