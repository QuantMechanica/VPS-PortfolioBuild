...
**Date:** 2026-08-25 UTC (`2026-08-25T20:46:33Z`), Europe/Berlin

**Branch:** `agents/board-advisor`

**Observation base:** `3b9feccecb7ef8cfc149e98e5634834ebc7d9612`
...
The supported `farmctl mt5-slots` snapshot at `2026-08-25T20:45:56Z`
observed six governed factory terminals actively testing: T2, T5, T6, T7,
T8, and T9. All ten terminal-worker daemons were present, six reservations
were active, and no orphaned factory terminal was reported.

Five fresh one-second whole-host CPU readings were all `100.00%`. Their
average and maximum were both `100.00%`. The explicit ceiling binds when
either the average or maximum is at least `97%`; both measures triggered the
stop. `T_Live` and the unrelated FTMO terminal were observed only to exclude
them from the factory count; neither was controlled.
...
`artifacts/fx_cointegration_frontier_hard_cpu_stop_20260825T204633Z_board_advisor.json`.

## Non-duplicate operational delta

The preceding FX receipt at `2026-08-25T19:46:33Z` observed seven active
factory terminals and seven reservations. The current supported census records
six active terminals and reservations with a changed cohort: T2 and T5 joined,
while T3, T4, and T10 departed. Average CPU increased from `99.03%` to
`100.00%`; the maximum remained `100.00%`. This receipt captures that changed
governed occupancy and fresh capacity measurement while reconfirming the
single QM5_20219 Q03 lineage without adding another work item.
