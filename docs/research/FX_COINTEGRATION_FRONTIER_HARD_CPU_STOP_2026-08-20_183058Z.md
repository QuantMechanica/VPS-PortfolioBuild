...
basket manifest and an APPROVED in-house Chan/cointegration Card snapshot. No
enqueue, requeue, priority mutation, or timestamp restamp was made because
each would duplicate or disturb the existing governed successor.

## Binding capacity stop

From `18:30:52Z` through `18:30:58Z`, five one-second whole-host CPU samples
were `99.415584%`, `93.091359%`, `94.239312%`, `94.091670%`, and
`99.030335%`. The `99.415584%` maximum exceeded the explicit `97%` hard
ceiling, so the ceiling was binding even though the sample average was
`95.973652%`.

Six canonical work items were active at that observation on T1, T2, T3, T5,
T6, and T7. This is a fresh capacity snapshot 57.02 minutes after the
preceding FX-frontier observation: the active set fell from ten to six, but
the host still hit the hard ceiling. `T_Live` and the unrelated FTMO terminal
were observed only to exclude them and were not controlled. Per the mission
stop condition, no Q02 or Q04 dispatch, queue mutation, tester launch,
terminal reservation, terminal reconciliation, or terminal control followed.

Machine-readable evidence is
`artifacts/fx_cointegration_frontier_hard_cpu_stop_20260820T183058Z_board_advisor.json`.
