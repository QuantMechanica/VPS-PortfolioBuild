...
Five fresh whole-host CPU samples at `03:30Z` were 100%, 97%, 96%, 100%, and
99%. Their 98.4% average and 100% maximum exceeded the explicit 97% ceiling.

The canonical database contained six active work items and 2,225 pending
items. A nearby path-filtered process census found factory terminals T4, T5,
T6, T7, and T10. The process and database observations were not treated as an
atomic slot reconciliation because the CPU stop was already binding. T_Live
was excluded by path; no live manifest or AutoTrading surface was queried or
changed.

This is a fresh observation 57.06 minutes after the preceding FX-frontier
evidence. Two prior work-item identities departed, none arrived, active work
decreased from eight to six, and pending work increased from 2,220 to 2,225.
The selected Q04 row itself did not change state.
...
`artifacts/fx_cointegration_frontier_hard_cpu_stop_20260821T033132Z_board_advisor.json`.
