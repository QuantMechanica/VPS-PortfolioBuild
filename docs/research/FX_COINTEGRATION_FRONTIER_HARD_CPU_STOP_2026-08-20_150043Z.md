...
At `15:00Z`, five one-second whole-host CPU samples were `98.05%`, `98.64%`,
`98.35%`, `98.06%`, and `98.83%`. Their `98.39%` average and `98.83%` maximum
were above the mission's explicit `97%` hard ceiling.
...
followed. This is a fresh capacity observation 59.37 minutes after the prior
FX-frontier observation. Machine-readable evidence is
`artifacts/fx_cointegration_frontier_hard_cpu_stop_20260820T150043Z_board_advisor.json`.
