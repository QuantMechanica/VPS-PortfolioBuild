...
attempt zero. The row was observed read-only and was not duplicated,
requeued, reprioritized, or dispatched.
...
The read-only `farmctl.py mt5-slots` sample at `2026-08-13T07:02:39Z` found
five factory terminals running: `T4`, `T5`, `T8`, `T9`, and `T10`. The paced
launch maximum in `D:/QM/strategy_farm/state/launch_gate_max.txt` is `1`;
five active factory jobs exceed the binding ceiling. The FTMO and `T_Live`
...
`artifacts/fx_cointegration_frontier_cpu_stop_20260813T070239Z_board_advisor.json`.
