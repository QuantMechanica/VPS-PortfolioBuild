...
At `08:46Z`, five whole-host CPU samples taken two seconds apart were `100%`,
`100%`, `100%`, `100%`, and `97%`. The average was `99.4%` and the maximum was
`100%`, at or above the mission's explicit `97%` hard ceiling.
...
followed. This is a new capacity observation 46 minutes after the prior
`08:00Z` binding observation, not a duplicate queue work item. Machine-readable
evidence is
`artifacts/fx_cointegration_frontier_hard_cpu_stop_20260820T084633Z_board_advisor.json`.
