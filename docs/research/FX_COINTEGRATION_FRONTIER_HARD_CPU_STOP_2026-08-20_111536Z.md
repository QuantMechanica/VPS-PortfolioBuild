...
At `11:15Z`, five whole-host CPU samples were `97.80%`, `95.51%`, `96.50%`,
`98.93%`, and `100.00%`. The average was `97.75%` and the maximum was `100.00%`;
the average exceeded the mission's explicit `97%` hard ceiling.
...
followed. This is a new capacity observation about 73 minutes after the prior
`10:02Z` binding observation, not a duplicate queue work item. Machine-readable
evidence is
`artifacts/fx_cointegration_frontier_hard_cpu_stop_20260820T111536Z_board_advisor.json`.
