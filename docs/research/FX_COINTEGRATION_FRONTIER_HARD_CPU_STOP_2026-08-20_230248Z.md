...
The five whole-host CPU samples ending at `23:02:48Z` were each `100.0%`.
Their `100.0%` average and maximum exceeded the explicit `97%` ceiling; the
paced launch gate remains one.

Nine canonical work items were active on T1 through T9, and a path-anchored
process snapshot matched those nine factory terminals. `T_Live` and the
unrelated FTMO terminal were observed only to exclude them and were not
controlled.

This is a fresh observation 60.17 minutes after the preceding FX-frontier
evidence. The active set changed materially: three preceding work-item
identities departed, four arrived, active work increased from eight to nine,
and pending work decreased from 2,224 to 2,222. The selected Q04 row itself
...
`artifacts/fx_cointegration_frontier_hard_cpu_stop_20260820T230248Z_board_advisor.json`.
