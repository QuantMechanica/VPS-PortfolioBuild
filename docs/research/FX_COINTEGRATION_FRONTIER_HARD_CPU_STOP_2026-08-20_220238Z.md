...
The five whole-host CPU samples ending at `22:02:38Z` were `100.000000%`,
`95.022278%`, `95.592250%`, `99.515644%`, and `99.514209%`. Their
`97.928876%` average and `100.000000%` maximum both exceeded the explicit
`97%` ceiling.

Eight canonical work items were active on T1, T2, T3, T4, T5, T6, T8, and
T10. This is a fresh observation 46.01 minutes after the preceding
FX-frontier evidence. The active count stayed at eight, but the active set
rotated from the T7 `QM5_10796` Q07 row to the T4 `QM5_12935` Q07 row;
pending work increased from 2221 to 2224. A path-anchored terminal-process
snapshot matched those eight factory terminals. `T_Live` and the unrelated
FTMO terminal were excluded from the factory count and were not controlled.
...
`artifacts/fx_cointegration_frontier_hard_cpu_stop_20260820T220238Z_board_advisor.json`.
