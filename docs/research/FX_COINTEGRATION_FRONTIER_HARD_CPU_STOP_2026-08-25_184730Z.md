# FX cointegration frontier: expanded factory load / hard CPU stop

**Date:** 2026-08-25 UTC (`2026-08-25T18:47:30Z`), Europe/Berlin

**Branch:** `agents/board-advisor`

**Observation base:** `6fd78af075de0ed7618817b78b0100c34cba4fdd`
...
Fresh supported `farmctl work-items` queries also confirm that the preferred
...
`basket_manifest.json`, and keeps `RISK_FIXED=1000`, `RISK_PERCENT=0`, and
`PORTFOLIO_WEIGHT=1`. The unique Q03 successor was left intact; no duplicate
enqueue or priority mutation was made.

## Binding capacity stop

The supported `farmctl mt5-slots` snapshot at `2026-08-25T18:47:08Z`
observed eight governed factory terminals actively testing: T2, T3, T4, T6,
...
Five fresh one-second whole-host CPU readings were `100.00%`, `89.87%`,
`99.81%`, `99.91%`, and `93.49%`. Their average was `96.61%` and their
maximum was `100.00%`. The explicit ceiling binds when either the average or
maximum is at least `97%`; the maximum triggered the stop. `T_Live` and the
...
`artifacts/fx_cointegration_frontier_hard_cpu_stop_20260825T184730Z_board_advisor.json`.

## Non-duplicate operational delta

The preceding FX receipt at `2026-08-25T18:00:53Z` observed five active
factory terminals and five reservations. The current supported census records
eight of each. Average CPU moved from `99.83%` to `96.61%`, while the maximum
remained `100.00%`. This receipt records the material expansion in governed
factory occupancy and freshly reconfirms the unique QM5_20219 lineage without
