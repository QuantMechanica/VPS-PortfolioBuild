...
- 2026-05-01 — [`processes/21-issue-discipline.md`](21-issue-discipline.md) (Quality-Tech + Documentation-KM enforcement; CEO ratification on disputed cancellations) — codifies issue-vs-comment discipline (separable + distinct acceptance + new workstream required for new issue), per-heartbeat issue-creation caps (IC 2 soft / 5 hard; Manager 5 soft / 10 hard), mandatory `<!-- unblock_owner: ... -->` + `<!-- block_class: ... -->` encoding on every `blocked` transition, and sequencing-blocked vs. capacity-blocked taxonomy with escalation SLAs. Smoke-test iterations go as comments on the parent backtest issue, not new child issues. API patterns in companion [`docs/ops/PAPERCLIP_API_DISCIPLINE.md`](../docs/ops/PAPERCLIP_API_DISCIPLINE.md). Decision record: [DL-057](../decisions/2026-05-01_issue_inflation_discipline.md). Authored under [QUA-647](/QUA/issues/QUA-647), triggered by [QUA-644](/QUA/issues/QUA-644) audit (12,200 done issues / 69 unowned-blocked in 7 days). Layers on DL-051 (gate-test requirement) + DL-046 (anti-theater).
