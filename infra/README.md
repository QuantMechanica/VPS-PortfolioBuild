...
- `scripts/Install-QUA95TaskHealthTask.ps1`
  ...
  - Passes explicit `-TransitionPayloadCheckScript`, `-UnblockReadinessCheckScript`, `-AuditSignalCheckScript`, `-CanonicalSnapshotCheckScript`, and `-CustomVisibilityProofCheckScript` paths to avoid hidden default/path drift.
  - Safe to re-run (`Register-ScheduledTask -Force`).
...
- `monitoring/Test-QUA95BlockedHeartbeatWrapper.ps1`
  - Runs `Invoke-QUA95BlockedHeartbeat.ps1` in non-recursive validation mode
    ...
    `QUA-95_AUDIT_SIGNAL_2026-04-27.json` signal status.
