...
DESKTOP_CSV_NUDGE = Path(r"C:\Users\Administrator\Desktop\CSV_TAIL_MISMATCH.txt")
...
def main() -> int:
    ...
    try:
        ...
        try:
            staged = 0
            skipped = 0
            csv_tail_mismatches: list[tuple[str, str]] = []
            for root, tick, m1 in pairs:
                ...
                if already_imported(target):
                    ...
                    if not tails_ok:
                        log(
                            f"  warn {root}: already imported, but csv tail alignment failed ({tails_why})",
                            fp,
                        )
                        csv_tail_mismatches.append((root, tails_why))
            ...
            if csv_tail_mismatches:
                lines = [
                    "ACTION REQUIRED — CSV tail mismatch detected.",
                    "",
                    "These symbols have stale/misaligned tick vs M1 tails and should not be reimported",
                    "until fresh aligned exports are provided in D:\\QM\\reports\\setup\\tick-data-timezone.",
                    "",
                ]
                for sym, why in csv_tail_mismatches:
                    lines.append(f"- {sym}: {why}")
                lines += [
                    "",
                    f"Generated by dwx_hourly_check.py at {now_iso()}",
                ]
                DESKTOP_CSV_NUDGE.write_text("\n".join(lines), encoding="utf-8")
                log(f"  wrote csv-tail nudge to {DESKTOP_CSV_NUDGE}", fp)
            elif DESKTOP_CSV_NUDGE.exists():
                DESKTOP_CSV_NUDGE.unlink()
                log("  csv tails aligned -- removed csv-tail nudge.", fp)
        finally:
            mt5.shutdown()
