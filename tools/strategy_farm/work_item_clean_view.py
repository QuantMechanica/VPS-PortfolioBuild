...
def install_clean_view(connection: sqlite3.Connection) -> None:
    ...
    connection.execute(
        ...
        SELECT
            ...
            {source('ex5_sha256')} AS ex5_sha256,
            {source('setfile_sha256')} AS setfile_sha256,
            {source('mq5_sha256')} AS mq5_sha256,
            {source('include_closure_sha256')} AS include_closure_sha256,
            {source('build_id')} AS build_id,
            {source('data_window_start')} AS data_window_start,
            {source('data_window_end')} AS data_window_end,
            {source('news_calendar_sha256')} AS news_calendar_sha256,
