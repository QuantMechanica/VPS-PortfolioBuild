...
def _candidate(raw: dict, stage: str, repo: Path) -> Candidate | None:
    ...
    return Candidate(ea_id, slug, stage, directory, card, symbols)
...
def load_candidates(repo: Path, fleet_worklist: Path, century_worklist: Path) -> list[Candidate]:
    ...
    raw_stages: list[tuple[str, Sequence[dict]]] = [
        ("compiled_ready", list((fleet.get("groups") or {}).get("compiled_ready") or [])),
        ("century", list(century.get("ready_to_allocate") or [])),
        ("sourced_not_compiled", list((fleet.get("groups") or {}).get("sourced_not_compiled") or [])),
        ("card_only", list((fleet.get("groups") or {}).get("card_only") or [])),
    ]
    ...
    for stage, rows in raw_stages:
        for raw in rows:
            if not isinstance(raw, dict):
                continue
            item = _candidate(raw, stage, repo)
