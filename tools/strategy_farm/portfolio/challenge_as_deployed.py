...
LEVERAGES = (1.0,)  # what the framework ACTUALLY permits: QM_RiskSizerSetCapPct(1.0) clamps every leg to 1% of equity, and no set file carries an override
...
for path in sorted(STREAMS.glob("*.jsonl")):
    ...
    with path.open(encoding="utf-8", errors="replace") as fh:
        for line in fh:
            ...
            raw_span.append((entry, close, net, mae))
            rows.append((close, net, mae))
    rows.sort()
    span = sum(1 for e, c, _, _ in raw_span if (c - e).days >= 1)
    if raw_span and 100.0 * span / len(raw_span) > 1.0:
        continue          # multi-day positions: this model cannot bound them
