"""Sealed M15 screen for 2-hour and 4-hour session displacement."""
    ...
    *,
    ...
    for indices in m15.session_days(frame, instrument):
        ...
        if opening is None or len(indices) <= window_bars:
            continue
        decision_bar = opening[-1]
        if int(years[decision_bar]) > end_year:
            continue
        atr = float(atrs[decision_bar])
        displacement = float(closes[decision_bar]) - float(opens[opening[0]])
        if (
            not np.isfinite(atr)
            or atr <= 0.0
            or abs(displacement) < displacement_atr * atr
            or displacement == 0.0
        ):
            continue
        ...
        trade = m15.make_trade(
            ...
            entry_reason=f"mid_session_displacement_{mode}",
        )
...
def parameter_grid() -> Iterable[dict[str, Any]]:
    for values in itertools.product(
        ...
        yield dict(
            zip(
                ("window_bars", "mode", "displacement_atr", "stop_atr", "target_r"),
                values,
            )
        )
...
def screen(instruments: Sequence[m15.Instrument]) -> dict[str, Any]:
    ...
    if eligible:
        ...
        trades = displacement_trades(
            frames[instrument.symbol], instrument, **winner["parameters"], end_year=2025
        )
        metrics = base.split_metrics(trades)
        selected.append(
            {
                "symbol": winner["symbol"],
                "family": "mid_session_displacement",
    ...
    return {
        ...
        "predeclaration": "artifacts/ftmo_m15_mid_session_displacement_predeclaration_2026-07-12.json",
        "selection_contract": {
            ...
            "entry_rule": "next_bar_open_after_completed_displacement_window",
