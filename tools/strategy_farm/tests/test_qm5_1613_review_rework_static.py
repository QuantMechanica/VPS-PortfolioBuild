...
EA_DIR = REPO / "framework" / "EAs" / "QM5_1613_aa-dsp-atsmom"
SOURCE = EA_DIR / "QM5_1613_aa-dsp-atsmom.mq5"
SPEC = EA_DIR / "SPEC.md"
...
EXPECTED_ACTIVE_SLOTS = {
    0: ("GDAXI.DWX", 16130000),
    1: ("NDX.DWX", 16130001),
    2: ("SP500.DWX", 16130002),
    3: ("UK100.DWX", 16130003),
    4: ("WS30.DWX", 16130004),
    5: ("XAUUSD.DWX", 16130005),
    6: ("EURUSD.DWX", 16130006),
    7: ("GBPUSD.DWX", 16130007),
    8: ("USDJPY.DWX", 16130008),
    9: ("USDCHF.DWX", 16130009),
    10: ("AUDUSD.DWX", 16130010),
    11: ("USDCAD.DWX", 16130011),
    12: ("NZDUSD.DWX", 16130012),
    13: ("XTIUSD.DWX", 16130013),
}
...
def _spread_allows(current: int, history: list[int], multiplier: float = 2.5) -> bool:
    if current <= 0 or len(history) < 2 or any(value <= 0 for value in history):
        return False
    return current <= multiplier * statistics.median(history)


def test_entry_is_card_faithful_current_sign_state_not_zero_cross() -> None:
    entry = _function_body(
        SOURCE.read_text(encoding="utf-8"),
        "bool Strategy_EntrySignal(QM_EntryRequest &req)",
    )

    assert "if(atsmom1 > 0.0)" in entry
    assert "else if(atsmom1 < 0.0)" in entry
    assert "atsmom2" not in entry
    assert "ComputeATSMOM(2" not in entry


def test_spread_guard_is_dynamic_bounded_and_fail_closed() -> None:
...
def test_spread_threshold_allows_at_limit_and_refuses_above_limit() -> None:
    history = [10] * 20

    assert _spread_allows(25, history)
    assert not _spread_allows(26, history)
    assert not _spread_allows(10, [10, 0])
...
def test_exit_and_friday_close_precede_entry_only_news_blackout() -> None:
    ...
    assert mae < friday < manage < new_bar < exit_signal < news < entry
    assert on_tick.count("QM_IsNewBar(_Symbol, PERIOD_D1)") == 1
    assert "ZeroMemory(req);" in on_tick


def test_existing_registry_allocations_are_preserved_and_sets_are_card_exact() -> None:
    with MAGIC_REGISTRY.open(encoding="utf-8-sig", newline="") as handle:
        rows = [
            row
            for row in csv.DictReader(handle)
            if row["ea_id"] == "1613" and row["status"].lower() == "active"
        ]
    actual = {
        int(row["symbol_slot"]): (row["symbol"], int(row["magic"])) for row in rows
    }
    assert actual == EXPECTED_ACTIVE_SLOTS

    expected_names = {
        f"QM5_1613_aa-dsp-atsmom_{symbol}_D1_backtest.set"
        for symbol in EXPECTED_SET_SLOTS.values()
    }
    assert {path.name for path in SETS.glob("*.set")} == expected_names
    for slot, symbol in EXPECTED_SET_SLOTS.items():
        path = SETS / f"QM5_1613_aa-dsp-atsmom_{symbol}_D1_backtest.set"
...
def test_build_identity_uses_canonical_truthful_schema() -> None:
    ...
    assert result["task_id"] == "6cb2150d-fd37-4068-a0cd-5a11e9d90b32"
    assert result["ea_id"] == "QM5_1613"
    assert result["magic_base"] == 16130000
    assert result["symbols_registered"] == [
        symbol for _, (symbol, _) in sorted(EXPECTED_ACTIVE_SLOTS.items())
    ]
    assert result["mq5_sha256"] == hashlib.sha256(SOURCE.read_bytes()).hexdigest()
    assert len(result["setfiles_generated"]) == len(EXPECTED_SET_SLOTS)
