...
def test_checkpoint_wal_persistent_busy_warns_not_raises(tmp_path, monkeypatch) -> None:
    # 2026-08-15: a busy exhaustion is reported as a warning, never raised —
    # the committed transaction is WAL-durable regardless, and three activation
    # ceremonies failed closed here without any integrity fact in question.
    ...
    assert out["busy"] == 1
    # R10 forensics: a frozen (log, checkpointed) progression must still label
    # itself as a persistent reader pin (the af2c60a38 defect class).
    assert "persistent reader pin" in out["warning"]
    assert "[(3, 1)]" in out["warning"]
...
def test_checkpoint_wal_moving_busy_labels_churn(tmp_path, monkeypatch) -> None:
    ...
    assert "moving reader churn" in out["warning"]
    assert "(3, 1)" in out["warning"]
    assert "(5, 3)" in out["warning"]
