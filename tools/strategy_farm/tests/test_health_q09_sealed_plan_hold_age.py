...
def test_old_q09_sealed_plan_hold_is_health_failure() -> None:
    ...
    con.execute(
        ...
        ("row-old", "QM5_100", "EURUSD.DWX", health.NEWS_PHASE, "pending", _stamp(8), _stamp(8)),
    )
    con.execute(
...
def test_fresh_or_inactive_hold_does_not_alarm() -> None:
    ...
    for row_id, age, active in (("fresh", 2, 1), ("released", 20, 0)):
        con.execute(
            "INSERT INTO work_items VALUES(?,?,?,?,?,?,?)",
            (row_id, "QM5_101", "USDJPY.DWX", health.NEWS_PHASE, "pending", _stamp(age), _stamp(age)),
        )
        con.execute(
            "INSERT INTO work_item_holds VALUES(?,?,?,?)",
...
def test_other_hold_codes_are_not_misclassified() -> None:
    ...
    con.execute(
        "INSERT INTO work_items VALUES(?,?,?,?,?,?,?)",
        ("other", "QM5_102", "XAUUSD.DWX", health.NEWS_PHASE, "pending", _stamp(24), _stamp(24)),
    )
    con.execute(
        "INSERT INTO work_item_holds VALUES(?,?,?,?)",
