...
def _fixture(tmp_path: Path, monkeypatch) -> dict:
    ...
    official_snapshot = repo / "docs/ops/evidence/2026-09-18_ftmo_official_rules_snapshot.json"
    ...
    rulepack.write_text(
        json.dumps({
            "official_sources": [{
                "snapshot_path": "docs/ops/evidence/2026-09-18_ftmo_official_rules_snapshot.json",
...
def test_prepare_plan_is_exact_six_item_t10_contract(tmp_path: Path, monkeypatch) -> None:
    ...
    for row in plan["operations"]:
        ...
        assert payload["ftmo_official_rules_snapshot_path"].endswith(
            "2026-09-18_ftmo_official_rules_snapshot.json"
        )
