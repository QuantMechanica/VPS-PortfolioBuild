...
def test_derivation_freezes_every_other_parameter():
    ...
    assert s.values(s.decode(data))['RISK_PERCENT']=='0.1'
    assert s.values(s.decode(data))['RISK_FIXED']=='0'
...
def test_seal_and_source_drift(tmp_path):
    ...
    source=tmp_path/'source.set';source.write_bytes(BASE)
    ...
    assert s.sealed_source(conn,10706,'GBPUSD')[0]==BASE
