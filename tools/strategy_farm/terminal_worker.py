...
def _work_item_history_symbols(
    ...
    diagnostic_symbols = payload.get("diagnostic_history_symbols")
    if isinstance(diagnostic_symbols, list):
        symbols.extend(diagnostic_symbols)
...
def _find_work_item_summary_data(item: sqlite3.Row, payload: dict[str, Any]) -> tuple[Path, dict[str, Any]] | None:
    ...
    if farmctl._is_dwx_tick_tail_probe_item(item, payload):
        ...
        return (summary_path, summary) if summary is not None else None
...
def _finish_dwx_tick_tail_probe(
    conn: sqlite3.Connection,
    item: sqlite3.Row,
    payload: dict[str, Any],
    exit_code: int | None,
    ...
    """Classify the read-only utility without emitting a pipeline verdict."""

    try:
        from tools.strategy_farm import dwx_tick_tail_probe_work_item as _probe_item
    except ModuleNotFoundError:
        import dwx_tick_tail_probe_work_item as _probe_item
    ...
    if summary is not None:
        try:
            validation = _probe_item.validate_summary(
                summary,
                payload,
                str(item["id"]),
                verify_files=True,
            ...
            if exit_code != 0:
                ...
                reason = "governed_tick_tail_probe_requires_review"
...
def _run_claimed_dwx_tick_tail_probe(
    root: Path,
    row: sqlite3.Row,
    terminal: str,
    timeout_seconds: int,
) -> dict[str, Any]:
    """Run the exact T1 diagnostic without EA staging or gate advancement."""
    ...
    payload = _json_loads(row["payload_json"])
    try:
        _probe_item.validate_payload(payload, terminal=terminal, verify_files=True)
    except (OSError, TypeError, ValueError) as exc:
        return {
...
def _run_claimed_item(
    ...
    *,
    ...
    if farmctl._is_dwx_tick_tail_probe_item(row):
        # This utility is not an EA: skip setfile/EX5 staging, strategy history
