"""Staggered idle-worker reload chunk 85 (Orchestrator 2026-09-15 10:2xZ): CODE reload of the three workers still on pre-fix code (T5 started 09-14 21:31 local = before cc33300782 prescreen-skip and 3601a2974d disk-bomb guard; T8/T10 started 09-15 01:10 local = before 3601a2974d). T1-T4/T6/T7/T9 were already relaunched by tester_cache_purge / earlier chunks after both commits. Purpose: the append-only Q08 reruns for 21507/20266 must not land on a worker without the prescreen-skip rule. Same mechanics as chunk 84: stop only at an idle claim attempt, wait for the respawn via start_terminal_workers.py, spacing >= 150 s.
Chunk-84 header follows.
Staggered idle-worker reload chunk 84 (Orchestrator 2026-09-14 16:0xZ, after chunk 83): CODE reload of all ten workers for the dsr_cohort evidence fixes (DL-090 aged .gz siblings for summary/report, content-hash comparison, timeframe fallback M1..MN1) so the Q08 DSR context preflight assembles the class-A/C rows of OWNER-DEC-Q08-CONTEXT-REPAIR-V2 at claim. Same env.

Chunk-83 header follows.

Staggered idle-worker reload chunk 83 (Orchestrator 2026-09-14 15:xxZ, after chunk 82): CODE reload of all ten workers for (a) the EXCLUSIVE drain lane for >=40 GB rows (SP500 single_index_tick, heavy multisymbol; pre-drain + empty-fleet arithmetic + duty cycle; commit e246cc0dab; kill switch QM_DRAIN_EXCLUSIVE=0) and (b) the two-leg metal basket class (24 GB provisional, commit ab37b2f8c2). OWNER 2026-09-14: kein RAM-Zukauf, SP500/Multisymbol laufen exklusiv. Same env.

Chunk-82 header follows.

Staggered idle-worker reload chunk 82 (Orchestrator 2026-09-14 14:1xZ): CODE reload of all ten workers for (a) index reservation table step 2 (NDX/WS30 12 GB on measured peaks, commit 4607ac1d63) and (b) for T3/T5 additionally the evidence-class-aware DL-089 lane view (commit 3283767561) they missed in chunk 80. Supersedes the unstarted chunk 81. Same env as chunk 80.

Chunk-80 header follows.

Staggered idle-worker reload chunk 80 (Orchestrator 2026-09-14 14:0xZ): CODE reload for the per-symbol index-tick RAM reservation table (terminal_worker INDEX_TICK_RESERVATION_GB_BY_BASE: SP500 44 GB measured, NDX/GDAXI/WS30/UK100 provisional 24 GB; rollback env QM_INDEX_TICK_RESERVATION_TABLE=0). OWNER 2026-09-14 "Los gehts, alles freigegeben und gemaess Vorschlag entschieden" (control review, lever 1). Same env as chunk 79.

Chunk-79 header follows.

Staggered idle-worker reload chunk 79 (Orchestrator 2026-09-14 04:2xZ): CODE reload for dsr_cohort bare-year windows (a0e04bd168), config_sweep promotion/preflight shared chain authenticator (3851f385c8) and the claim_result diagnostic (194fffa917); all ten terminals.

Chunk-77 header follows.

 (Orchestrator 2026-09-14 02:5xZ): CODE reload so every worker imports the config_sweep fix for chained PRESCREEN rerun amendments (commit after 905c29f2c7: authenticate_ledger walks the sealed reruns/ chain; second-hop reruns of the 41405 PRESCREEN matrix were failing the lane preflight and suppressing the program). Same env as chunk 76 (incl. the PRESCREEN allow-list entry); all ten terminals, one at a time, idle-aware.

Chunk-76 header follows.
...
TERMINALS = ["T5", "T8", "T10"]  # chunk 82: T3/T5 first (they also need the lever-3 code)  # chunk 77: all ten  # chunk 76: reloaded by chunk 75 before the flag
