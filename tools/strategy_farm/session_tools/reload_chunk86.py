"""Staggered idle-worker reload chunk 86 (Orchestrator 2026-09-15 11:0xZ): CODE reload of all ten workers for (a) the sticky row-bound exclusive pre-drain (a08a28f071) and (b) the head-of-line Q08 DSR-context precheck (f160a0937c) activated by the machine-scope env QM_Q08_DSR_CONTEXT_PREFLIGHT=1 set 11:0xZ (inert without it; rollback = unset + reload). Same mechanics as chunk 84: stop only at an idle claim attempt, respawn via start_terminal_workers.py, spacing >= 150 s.
