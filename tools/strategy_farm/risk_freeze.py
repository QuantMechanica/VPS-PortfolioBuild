...
LIFT_CONDITIONS = [
    {
        ...
        "status": "SATISFIED_BY_OWNER_ATTESTATION",
        "blocked_by": "2026-09-14: SATISFIED by OWNER attestation receipt 424fb9d6-38f9-4c8e-914c-70dbdd7fbf02 (OWNER chat 2026-09-14 'alles freigegeben und gemaess Vorschlag entschieden', card OWNER-DEC-LIVE-IDENTITY-CURRENT-8b06c6c8..., declared effect ATTEST_CURRENT_PROPOSAL_SHA256=8b06c6c8...;FREEZE=ACTIVE;NO_ACTIVATION): live_identity_consumer reports OWNER_ATTESTED_CURRENT_IDENTITY / condition_1_satisfied for proposal r5 (docs/ops/evidence/2026-09-13_m06_attest_current/consumer_receipt_424fb9d6.json); the guard's identity exception recognises it; the freeze itself stays ACTIVE until the OWNER's written lift sentence. History 2026-09-13: the signing route is circular (generate_live_deployment_pointer.py calls this guard without the identity exception; refusal captured). Lift-free route = identity exception: a fresh read-only observation (24 sleeves, 9.7499 %, problems=[]) and an inert proposal are minted under docs/ops/evidence/2026-09-13_m06_attest_current/; live_deployment_pointer_auth.authenticate_deploy_stamp(identity_exception_enabled=True) and verify_live_deployment_contract --allow-attested-current-identity then read OWNER_ATTESTED_CURRENT_IDENTITY. Consumers refuse today with exactly one reason: RECEIPT_NOT_OWNER_ATTESTATION_OF_THIS_PROPOSAL. Remaining = one OWNER YES receipt with decision_id OWNER-DEC-LIVE-IDENTITY-CURRENT-<proposal_sha256> and selected_effect ATTEST_CURRENT_PROPOSAL_SHA256=<sha>;FREEZE=ACTIVE;NO_ACTIVATION (no pointer signature, no lift, no risk change). Any edit of risk_freeze.py voids the proposal -> re-mint after edits.",
    },
    {
        "id": "NEWS-CONTRACT-V2",
        "requirement": "news impact taxonomy implemented under qm.news_impact_mapping.v1",
        "status": "PARTIAL",
        "blocked_by": "2026-09-13: sections 2/3/4/5/7/9 implemented and the consumers are cut over behind ONE Default-OFF flag QM_NEWS_IMPACT_MAPPING_V2=1 (p8_news_driver impact resolver, news_calendar_gate preflight declaration, q09_news_runner Q10_NEWS run self-report carrying mapping_version + authoritative_source + dst_rule_version); flag off = byte-identical pre-V2 behaviour, live path untouched (DL-080). Review lanes closed append-only: historical Q09_NEWS 64/64, active Q10_NEWS 84/95 (4 via farmctl readjudicate-news-8cell, 80 dispositions; evidence docs/ops/evidence/2026-09-13_q10_news_review_lane_dispositions.md). Still open, all four named precisely: (1) section 6 known_at_utc is not implemented anywhere; (2) section 8's cross-language MQL5<->Python DST sweep and the 3,502-row/7.30% regression re-run do not exist (python side only); (3) 11 Q10_NEWS rows remain REVIEW_REQUIRED because the CURRENT rule still returns control_or_policy_off_not_qualifiable - closing them needs a rerun or a ROT gate change; (4) the flag is not activated anywhere, which needs an OWNER go plus a worker reload. Not blocked by the Q10_NEWS pending wave: 52 pending rows are held (28 NEWS_CALENDAR_TAINTED, 12 NEWS_CALENDAR_TIMESTAMP_DEFECT, 9 NEWS_RUNNER_SPAWN_SILENT_ABORT, 2 Q09_AWAITING_SEALED_PLAN), no run is active.",
    },
    {
