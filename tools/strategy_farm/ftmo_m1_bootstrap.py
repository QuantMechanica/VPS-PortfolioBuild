...
def compile_harvest_script(terminal_root: Path, run_root: Path) -> dict[str, Any]:
    ...
    if staged_source.exists() and sha256_file(staged_source) != source_binding["sha256"]:
        raise BootstrapError(f"refusing to overwrite a different staged MQL5 source: {staged_source}")
...
$actualCreation = ([datetime]$row.CreationDate).ToUniversalTime().ToString('yyyy-MM-ddTHH:mm:ss.fffffffZ')
$expectedCreationNormalized = ([datetime]$expectedCreation).ToUniversalTime().ToString('yyyy-MM-ddTHH:mm:ss.fffffffZ')
