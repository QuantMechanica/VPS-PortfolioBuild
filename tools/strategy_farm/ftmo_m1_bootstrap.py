    ...
    *,
    ...
    ini_text = "\n".join(
        [
            ...
            # Without an explicit [Common] Login/Server a first-start portable
            # terminal never attempts a connection and every CopyRates call
            # dies with 4401 (proven live 2026-08-08). The password stays in
            # the lane's Config/accounts.dat - never in this file.
            "[Common]",
            f"Login={FTMO_DEMO_LOGIN}",
            f"Server={FTMO_DEMO_SERVER}",
            "[Experts]",
            "Enabled=0",
            "AllowLiveTrading=0",
            "AllowDllImport=0",
