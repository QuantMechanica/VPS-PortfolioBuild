...
class TestAggregatePhaseResults(unittest.TestCase):
    def test_ready_when_all_required_phases_pass(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            log_records = _read_jsonl(root / "QM5_1001" / "aggregate_phase_results_log.jsonl")
        ...
        self.assertEqual(len(log_records), 8)
        for key in ("phase", "ea_id", "verdict", "criterion", "evidence_path"):
            self.assertIn(key, log_records[0])
        self.assertEqual(log_records[-1]["phase"], "PIPELINE")
        self.assertEqual(log_records[-1]["verdict"], "READY")
