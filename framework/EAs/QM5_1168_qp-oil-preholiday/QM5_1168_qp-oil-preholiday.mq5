...
#property description "QM5_1168 Quantpedia Oil Pre-Holiday Drift"

#include <QM/QM_Common.mqh>

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 1168;
...
datetime NthWeekdayOfMonth(const int year, const int month, const int weekday, const int ordinal)
  {
   datetime month_start = MakeDate(year, month, 1);
   int seen = 0;
   for(int i = 0; i < 31; ++i)
     {
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   const datetime prior_trading_day = AddTradingDays(current_d1, -1);
   if(prior_trading_day <= 0)
      return false;

   const datetime holiday = FirstConfiguredHolidayAfter(prior_trading_day);
   if(holiday <= 0)
      return false;

   const datetime entry_day = AddTradingDays(holiday, -5);
   if(entry_day <= 0 || !SameDate(prior_trading_day, entry_day))
      return false;
   ...
   req.price = ask;
   req.tp = 0.0;
   req.reason = "QP_OIL_PREHOLIDAY_LONG";
   return true;
  }
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_1168\",\"ea\":\"qp_oil_preholiday\"}");
