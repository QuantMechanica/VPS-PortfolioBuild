...
#property description "QM5_20165 Energy Leverage Divergence Convergence"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_20165 - Energy Leverage Divergence Convergence
// -----------------------------------------------------------------------------
// D1 two-leg energy basket conditioned on opposite leverage asymmetries:
//   shock = log(XTI[t] / XTI[t-1]) - beta * log(XNG[t] / XNG[t-1])
//   enter only when WTI fell, XNG rose, and shock z < -entry:
//   buy WTI and sell natural gas for convergence.
...
input double strategy_exit_z             = 0.3;
...
bool Strategy_RefreshSpreadState()
  {
   ...
   for(int i = 0; i < z_lookback; ++i)
     {
      ...
      if(xti[i] <= 0.0 || xng[i] <= 0.0 || xti[past_idx] <= 0.0 || xng[past_idx] <= 0.0)
         return false;

      const double xti_ret = MathLog(xti[i] / xti[past_idx]);
      const double xng_ret = MathLog(xng[i] / xng[past_idx]);
      spreads[i] = xti_ret - strategy_beta * xng_ret;
   ...
   g_xti_return = MathLog(xti[0] / xti[return_lookback]);
   g_xng_return = MathLog(xng[0] / xng[return_lookback]);
...
bool Strategy_OpenPair(const int spread_direction)
  {
   ...
   const QM_OrderType xng_type = long_spread ? QM_SELL : QM_BUY;
   const string reason = long_spread ? "QM5_20165_LONG_ENERGY_LEV_CONV"
                                     : "QM5_20165_SHORT_ENERGY_LEV_CONV";
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart())
      return true;
   if(strategy_return_lookback_d1 != 1 || strategy_z_lookback_d1 < 40 || strategy_beta <= 0.0)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20165_RSPREAD_HOST";
   ...
   if(g_xti_return < 0.0 && g_xng_return > 0.0 && g_spread_z < -strategy_entry_z)
      Strategy_OpenPair(1);
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_20165\",\"ea\":\"energy-lev-conv\"}");
