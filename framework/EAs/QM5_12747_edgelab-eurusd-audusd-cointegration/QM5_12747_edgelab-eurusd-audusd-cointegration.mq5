...
#property description "QM5_12747 Edge Lab EURUSD AUDUSD Cointegration"
...
input double strategy_beta              = 0.73;
...
bool Strategy_IsHostSymbol()
  {
   return (_Symbol == g_leg_eurusd || _Symbol == g_leg_audusd);
  }
...
bool Strategy_EnsureBasketScope()
  {
   ...
   string allowed[2] = {"EURUSD.DWX", "AUDUSD.DWX"};
   for(int i = 0; i < 2; ++i)
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(!Strategy_EnsureBasketScope())
      return false;
   if(!QM_SymbolAssertOrLog(g_leg_eurusd) || !QM_SymbolAssertOrLog(g_leg_audusd))
      return false;
   ...
   for(int i = 0; i < lookback; ++i)
     {
      if(eurusd[i] <= 0.0 || audusd[i] <= 0.0)
         return false;
      spreads[i] = MathLog(eurusd[i]) - strategy_beta * MathLog(audusd[i]);
...
bool Strategy_OpenPair(const int spread_direction)
  {
   ...
   const double weight_sum = eurusd_weight + audusd_weight;
   if(weight_sum <= 0.0)
      return false;

   const bool long_spread = (spread_direction > 0);
   const QM_OrderType eurusd_type = long_spread ? QM_BUY : QM_SELL;
   const QM_OrderType audusd_type = long_spread ? QM_SELL : QM_BUY;
   const string reason = long_spread ? "QM5_12747_LONG_SPREAD_Z_LT_NEG_ENTRY"
                                     : "QM5_12747_SHORT_SPREAD_Z_GT_POS_ENTRY";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12747_PAIR_HOST";
