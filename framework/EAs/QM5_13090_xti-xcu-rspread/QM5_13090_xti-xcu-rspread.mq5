...
#property description "QM5_13090 XTI XCU Return Spread Reversion"
...
// -----------------------------------------------------------------------------
// D1 two-leg WTI/Copper commodity basket:
//   rspread = log(XTI[t] / XTI[t-L]) - beta_xcu * log(XCU[t] / XCU[t-L])
//   z > entry: short return spread = sell WTI, buy copper
//   z < -entry: long return spread = buy WTI, sell copper
   ...
   for(int i = 0; i < z_lookback; ++i)
     {
      ...
      if(xti[i] <= 0.0 || xcu[i] <= 0.0 || xti[past_idx] <= 0.0 || xcu[past_idx] <= 0.0)
         return false;

      const double xti_ret = MathLog(xti[i] / xti[past_idx]);
      const double xcu_ret = MathLog(xcu[i] / xcu[past_idx]);
      spreads[i] = xti_ret - strategy_beta_xcu * xcu_ret;
...
bool Strategy_OpenPair(const int spread_direction)
  {
   ...
   const QM_OrderType xcu_type = long_spread ? QM_SELL : QM_BUY;
   const string reason = long_spread ? "QM5_13090_LONG_XTI_XCU_RSPREAD"
                                     : "QM5_13090_SHORT_XTI_XCU_RSPREAD";
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart())
      return true;
   if(strategy_return_lookback_d1 < 5 || strategy_z_lookback_d1 < 40 || strategy_beta_xcu <= 0.0)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_13090_RSPREAD_HOST";
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_13090\",\"ea\":\"xti-xcu-rspread\"}");
