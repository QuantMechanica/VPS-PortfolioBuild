# QM5_10107_gh-novts-sidus — Strategy Spec
...
`artifacts/cards_approved/QM5_10107_gh-novts-sidus.md`. See that card's body for
...
`QM5_10107_gh-novts-sidus.mq5`. Framework wiring (risk, magic, news, Friday close)
...
|---|---|---|---|
| `strategy_ema_fast_period` | 18 | (see source) | (see strategy logic) |
| `strategy_ema_slow_period` | 28 | (see source) | (see strategy logic) |
| `strategy_lwma_fast_period` | 5 | (see source) | (see strategy logic) |
| `strategy_lwma_slow_period` | 8 | (see source) | (see strategy logic) |
| `strategy_open_lookback` | 5 | (see source) | (see strategy logic) |
| `strategy_exit_lookback` | 5 | (see source) | (see strategy logic) |
| `strategy_min_bars` | 100 | (see source) | (see strategy logic) |
| `strategy_max_spread_points` | 5.0 | (see source) | (see strategy logic) |
| `strategy_stop_points` | 1000 | (see source) | (see strategy logic) |
| `strategy_take_profit_rr` | 1.0 | (see source) | (see strategy logic) |
...
| Cadence note | "H1 four-moving-average state flip; selective trend-change cadence. Estimate 20-45 trades/year/symbol." |
...
`artifacts/cards_approved/QM5_10107_gh-novts-sidus.md`
