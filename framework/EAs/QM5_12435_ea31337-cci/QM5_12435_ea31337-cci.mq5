...
//   - Call iATR / iMA / iRSI / iMACD / iADX / iBands / iCCI or CopyBuffer
...
// Card §Entry: CCI(period) on typical price below -level and rising over the
// last two closed bars (shift 1 vs shift 2).
bool Strategy_LongSignal()
  {
   const ENUM_TIMEFRAMES tf = (ENUM_TIMEFRAMES)_Period;
   const double cci_now  = QM_CCI(_Symbol, tf, strategy_cci_period, 1, PRICE_TYPICAL);
   const double cci_prev = QM_CCI(_Symbol, tf, strategy_cci_period, 2, PRICE_TYPICAL);

   return (cci_now < -strategy_signal_open_level && cci_now > cci_prev);
  }

// Card §Entry: mirror of the long rule above +level, falling over two bars.
