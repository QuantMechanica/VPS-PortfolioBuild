...
The EA trades a raw EA31337 CCI extreme-reversal rule on closed bars. A long signal
occurs when CCI(20) on typical price is below `-90` and rising over the last two
closed bars (current vs prior). A short signal mirrors that rule above `+90`, with
CCI falling over the same two-bar window. Positions use fixed SL/TP and close
earlier when the opposite CCI extreme signal appears or after 30 bars.
...
|---|---|---|---|
| `strategy_cci_period` | 20 | 2+ | CCI period on typical price from the EA31337 source default. |
| `strategy_signal_open_level` | 90.0 | >0 | Absolute CCI threshold for long (below -level) and short (above +level). |
...
**Pointer:** https://github.com/EA31337/Strategy-CCI/blob/master/Stg_CCI.mqh
...
|---|---|---|---|
| v1 | 2026-08-10 | Initial build from card | task c2941420-efec-4ddb-84af-5a9cb7d0102c |
