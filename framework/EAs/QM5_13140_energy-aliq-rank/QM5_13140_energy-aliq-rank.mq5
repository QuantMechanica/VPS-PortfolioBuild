...
#property description "QM5_13140 XTI XNG Amihud-Illiquidity Rank"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_13140 - XTI/XNG Monthly Amihud-Illiquidity Rank
// -----------------------------------------------------------------------------
// Monthly structural energy basket:
//   - use D1 bars from the 12 fully completed broker calendar months
//   - proxy ALIQ as mean(abs(log return) / tick volume * 1,000,000)
//   - buy the higher-ALIq leg and short the lower-ALIq leg
// Tick volume is a native activity proxy, not exchange dollar volume.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 13140;
...
input int    strategy_aliq_window_months       = 12;
...
double   g_cache_xti_aliq = 0.0;
double   g_cache_xng_aliq = 0.0;
...
bool Strategy_AmihudProxy(const string symbol,
                          ...
                          int &observation_count)
  {
   ...
   if(ArrayResize(expected_month_keys, strategy_aliq_window_months) != strategy_aliq_window_months ||
      ArrayResize(month_observations, strategy_aliq_window_months) != strategy_aliq_window_months)
      return false;

   for(int month_index = 0; month_index < strategy_aliq_window_months; ++month_index)
     {
   ...
   for(int i = 0; i < count - 1; ++i)
     {
      ...
      const long tick_volume = rates[i].tick_volume;
      if(close_now <= 0.0 || close_prior <= 0.0 || tick_volume <= 0 ||
      ...
      if(!MathIsValidNumber(daily_return))
         return false;
      const double daily_aliq = MathAbs(daily_return) / (double)tick_volume * 1000000.0;
      if(daily_aliq < 0.0 || !MathIsValidNumber(daily_aliq))
         return false;
...
bool Strategy_LoadSignalState(const datetime decision_bar_time,
  ...
  {
   pair_direction = 0;
   g_cache_xti_aliq = 0.0;
   g_cache_xng_aliq = 0.0;
   ...
   if(decision_month_key <= 0)
      return false;
   if(!Strategy_AmihudProxy(g_leg_xti,
                            decision_month_key,
                            g_cache_xti_aliq,
                            g_cache_xti_observations))
      return false;
   if(!Strategy_AmihudProxy(g_leg_xng,
                            decision_month_key,
                            g_cache_xng_aliq,
                            g_cache_xng_observations))
      return false;

   g_cache_aliq_difference = g_cache_xti_aliq - g_cache_xng_aliq;
   if(!MathIsValidNumber(g_cache_aliq_difference))
      return false;
   if(g_cache_aliq_difference > 1.0e-12)
      pair_direction = 1;
   else if(g_cache_aliq_difference < -1.0e-12)
      pair_direction = -1;
   return true;
  }
...
bool Strategy_OpenPair(const int pair_direction)
  {
   ...
   const string reason = long_xti_short_xng ? "QM5_13140_LONG_XTI_SHORT_XNG_HIGH_ALIQ"
                                            : "QM5_13140_SHORT_XTI_LONG_XNG_HIGH_ALIQ";
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart())
      return true;
   if(strategy_aliq_window_months != 12)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_13140_XTI_XNG_ALIQ_HOST";
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_13140\",\"ea\":\"energy-aliq-rank\"}");
