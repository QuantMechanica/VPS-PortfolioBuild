...
input int    strategy_entry_hour        = 16;
input int    strategy_entry_minute      = 30;
input int    strategy_close_hour        = 22;
...
string g_strategy_symbols[STRATEGY_UNIVERSE_SIZE] = {"SP500.DWX", "NDX.DWX", "WS30.DWX", "GDAXI.DWX"};
...
bool Strategy_PriorReturn(const string symbol, double &ret)
  {
   ret = 0.0;
   ...
   if(close_1 <= 0.0 || close_2 <= 0.0)
      return false;
   ret = (close_1 / close_2) - 1.0;
   return true;
  }

int Strategy_RankSignal()
  {
   const int idx = Strategy_SymbolIndex(_Symbol);
   if(idx < 0)
      return 0;

   double rets[STRATEGY_UNIVERSE_SIZE];
   for(int i = 0; i < STRATEGY_UNIVERSE_SIZE; ++i)
      if(!Strategy_PriorReturn(g_strategy_symbols[i], rets[i]))
         return 0;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   const datetime now = iTime(_Symbol, (ENUM_TIMEFRAMES)_Period, 0);
   if(now <= 0)
      return false;
   ...
   const int now_min = Strategy_MinutesOfDay(now);
   const int entry_min = MathMax(0, MathMin(23, strategy_entry_hour)) * 60
                         + MathMax(0, MathMin(59, strategy_entry_minute));
   const int close_min = MathMax(0, MathMin(23, strategy_close_hour)) * 60
                         + MathMax(0, MathMin(59, strategy_close_minute));
   if(now_min < entry_min || now_min >= close_min)
      return false;

   const int rank_signal = Strategy_RankSignal();
   if(rank_signal == 0)
      return false;

   const double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
   const double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   const double entry = (rank_signal > 0) ? ask : bid;
   if(entry <= 0.0)
      return false;

   const QM_OrderType side = (rank_signal > 0) ? QM_BUY : QM_SELL;
   ...
   req.tp = 0.0;
   req.reason = (rank_signal > 0) ? "CHAN_INTRADAY_REV_LONG_WORST" : "CHAN_INTRADAY_REV_SHORT_BEST";
   req.symbol_slot = g_strategy_slots[Strategy_SymbolIndex(_Symbol)];
