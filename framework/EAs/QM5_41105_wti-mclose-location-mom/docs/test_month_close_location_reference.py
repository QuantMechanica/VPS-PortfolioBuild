"""Deterministic reference checks for QM5_41105 monthly close location."""
...
def close_location_signal(
    current_month_key: int,
    completed_newest_first: list[Bar],
    offset: timedelta,
    minimum: int = 17,
    maximum: int = 23,
    long_clv_min: float = 0.75,
    short_clv_max: float = 0.25,
) -> tuple[bool, int, tuple[int, int], tuple[float, ...]]:
    """Mirror the bounded two-month final-close/range signal."""

    empty = (False, 0, (0, 0), (0.0,) * 6)
    ...
    new_bucket, parent_bucket = buckets
    new_close = new_bucket[0].close
    new_high = max(bar.high for bar in new_bucket)
    new_low = min(bar.low for bar in new_bucket)
    parent_close = parent_bucket[0].close
    new_range = new_high - new_low
    if (
        min(new_close, new_high, new_low, parent_close) <= 0.0
        or new_range <= 0.0
        or not all(
            math.isfinite(value)
            for value in (new_close, new_high, new_low, parent_close, new_range)
        )
    ):
        return empty

    return_value = math.log(new_close / parent_close)
    clv = (new_close - new_low) / new_range
    if not math.isfinite(return_value) or not math.isfinite(clv) or not 0 <= clv <= 1:
        return empty

    direction = 0
    if return_value > 0 and clv > long_clv_min:
        direction = 1
    elif return_value < 0 and clv < short_clv_max:
        direction = -1
    return (
        True,
        direction,
        (len(new_bucket), len(parent_bucket)),
        (new_close, new_high, new_low, parent_close, return_value, clv),
    )
...
def sample(
    new: tuple[float, float, float, float] = (100.0, 120.0, 80.0, 118.0),
    ...
    bars = (
        ...
        + make_month(2026, parent_month - 1, 1, (90.0, 105.0, 85.0, 95.0))
    )
...
class MonthCloseLocationReferenceTest(unittest.TestCase):
    def signal(self, bars: list[Bar], offset: timedelta = timedelta(0)):
        return close_location_signal(202608, bars, offset)
    ...
    def test_strict_long_and_short_paths(self) -> None:
        ...
        self.assertEqual(values[:4], (118.0, 120.0, 80.0, 100.0))
        self.assertGreater(values[4], 0.0)
        self.assertAlmostEqual(values[5], 0.95)

        _, _, bars = sample(new=(100.0, 105.0, 80.0, 82.0))
        valid, direction, _, values = self.signal(bars)
        self.assertTrue(valid)
        self.assertEqual(direction, -1)
        self.assertLess(values[4], 0.0)
        self.assertAlmostEqual(values[5], 0.08)
    ...
    def test_zero_return_threshold_equality_and_disagreement_are_flat(self) -> None:
        cases = (
            (100.0, 120.0, 80.0, 100.0),  # zero return
            (100.0, 120.0, 80.0, 110.0),  # strict long threshold equality
            (100.0, 120.0, 80.0, 90.0),   # strict short threshold equality
            (100.0, 120.0, 80.0, 105.0),  # positive return, interior close
            (100.0, 120.0, 80.0, 95.0),   # negative return, interior close
        )
        for aggregate in cases:
            _, _, bars = sample(new=aggregate)
            valid, direction, _, _ = self.signal(bars)
            self.assertTrue(valid)
            self.assertEqual(direction, 0)

    def test_chronological_final_closes_drive_return(self) -> None:
        _, _, bars = sample(
            new=(50.0, 120.0, 80.0, 118.0),
            parent=(150.0, 160.0, 90.0, 100.0),
        )
        valid, direction, _, values = self.signal(bars)
        self.assertTrue(valid)
        self.assertEqual(direction, 1)
        self.assertEqual((values[0], values[3]), (118.0, 100.0))
    ...
    def test_malformed_zero_range_nonconsecutive_and_current_month_rejected(self) -> None:
        ...
        bars.insert(
            0,
            Bar(datetime(2026, 8, 1, tzinfo=UTC), 118.0, 119.0, 117.0, 118.5),
        )
        self.assertFalse(self.signal(bars)[0])
    ...
    def test_static_build_contract_is_fixed_risk_and_completed_data_only(self) -> None:
        source = (EA_DIR / "QM5_41105_wti-mclose-location-mom.mq5").read_text(
            encoding="utf-8"
        )
        setfile = (
            EA_DIR
            / "sets"
            / "QM5_41105_wti-mclose-location-mom_XTIUSD.DWX_D1_backtest.set"
        ...
        for forbidden in (
            ...
            "WebRequest(",
            "moving_average",
            "strategy_return_threshold",
        ):
            self.assertNotIn(forbidden, source)
        self.assertLess(
            source.index("Strategy_RecordMonthAttempt(g_decision_month_key)"),
            source.index("Strategy_LoadCloseLocationSignal(g_decision_month_key"),
        )
        for marker in (
            "qm_ea_id=41105",
            ...
            "strategy_long_clv_min=0.75",
            "strategy_short_clv_max=0.25",
        ...
        approved_card = (
            ...
            / "cards"
            / "approved"
            / "QM5_41105_wti-mclose-location-mom_card.md"
        )
