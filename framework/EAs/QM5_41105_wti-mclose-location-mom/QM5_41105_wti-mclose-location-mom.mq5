...
#property description "QM5_41105 WTI Completed-Month Close-Location Momentum"
...
//   - follow the newest month's close-to-close return sign only when its final
//     close is in the matching strict outer quartile of its own high-low range
//   - own the position for the next broker month
//   - consume the month before fallible entry gates; never enter late or retry
// Runtime uses MT5-native OHLC, calendar, history, and framework state only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                      = 41105;
...
input double strategy_long_clv_min         = 0.75;
input double strategy_short_clv_max        = 0.25;
...
bool Strategy_LoadCloseLocationSignal(const int current_month_key,
                                      const int label_offset,
                                      int &direction,
                                      int &new_month_bars,
                                      ...
                                      double &close_location)
  {
   ...
   double month_highs[2] = {0.0, 0.0};
   double month_lows[2] = {0.0, 0.0};
   ...
   for(int index = 0; index < copied; ++index)
     {
      ...
      if(bucket < 0 || bucket > 1 || !Strategy_BarOHLCValid(bars[index]))
         return false;
      ...
      if(month_counts[bucket] == 0)
        {
         // Series order is newest to oldest, so this is the chronological
         // final close of the completed month.
         month_closes[bucket] = bars[index].close;
         month_highs[bucket] = bars[index].high;
         month_lows[bucket] = bars[index].low;
        }
      else
        {
         month_highs[bucket] =
            MathMax(month_highs[bucket], bars[index].high);
         month_lows[bucket] =
            MathMin(month_lows[bucket], bars[index].low);
        }
      ++month_counts[bucket];
   ...
   for(int bucket = 0; bucket < 2; ++bucket)
     {
      if(month_counts[bucket] < strategy_min_month_sessions ||
         ...
         return false;
      const double aggregate_range =
         month_highs[bucket] - month_lows[bucket];
      if(month_closes[bucket] <= 0.0 ||
         month_highs[bucket] <= 0.0 ||
         month_lows[bucket] <= 0.0 ||
         aggregate_range <= 0.0 ||
         !MathIsValidNumber(month_closes[bucket]) ||
         !MathIsValidNumber(month_highs[bucket]) ||
         !MathIsValidNumber(month_lows[bucket]) ||
         !MathIsValidNumber(aggregate_range))
         return false;
     }

   new_month_bars = month_counts[0];
   parent_month_bars = month_counts[1];
   new_close = month_closes[0];
   new_high = month_highs[0];
   new_low = month_lows[0];
   parent_close = month_closes[1];

   const double close_ratio = new_close / parent_close;
   const double new_range = new_high - new_low;
   if(close_ratio <= 0.0 || new_range <= 0.0 ||
      !MathIsValidNumber(close_ratio) ||
      !MathIsValidNumber(new_range))
      return false;

   month_return = MathLog(close_ratio);
   close_location = (new_close - new_low) / new_range;
   if(!MathIsValidNumber(month_return) ||
      !MathIsValidNumber(close_location) ||
      close_location < 0.0 || close_location > 1.0)
      return false;

   if(month_return > 0.0 &&
      close_location > strategy_long_clv_min)
      direction = 1;
   else if(month_return < 0.0 &&
           close_location < strategy_short_clv_max)
      direction = -1;
   return true;
  }
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   // Consume before completed-month aggregation, signal, news, spread,
   // quote, ATR, sizing, or order gates. The month clock is the prerequisite.
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadCloseLocationSignal(g_decision_month_key,
      ...
      if(!g_signal_valid)
         ...
         g_signal_state = "return_upper_quartile_long";
      else if(g_signal_direction < 0)
         g_signal_state = "return_lower_quartile_short";
      else if(g_month_return == 0.0)
         g_signal_state = "zero_return_flat";
      else if(g_close_location == strategy_long_clv_min ||
              g_close_location == strategy_short_clv_max)
         g_signal_state = "clv_threshold_equality_flat";
      else if((g_month_return > 0.0 &&
               g_close_location < strategy_short_clv_max) ||
              (g_month_return < 0.0 &&
               g_close_location > strategy_long_clv_min))
         g_signal_state = "return_location_disagreement_flat";
      else
         g_signal_state = "interior_close_location_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"new_month_bars\":%d,\"parent_month_bars\":%d,\"new_close\":%.8f,\"new_high\":%.8f,\"new_low\":%.8f,\"parent_close\":%.8f,\"return\":%.12f,\"clv\":%.12f,\"state\":\"%s\"}",
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41105 ||
   ...
   if(qm_friday_close_enabled ||
      ...
      return true;
   if(strategy_history_bars_d1 != 70 ||
      strategy_min_month_sessions != 17 ||
      strategy_max_month_sessions != 23 ||
      MathAbs(strategy_long_clv_min - 0.75) > 1.0e-12 ||
      MathAbs(strategy_short_clv_max - 0.25) > 1.0e-12 ||
      strategy_short_clv_max <= 0.0 ||
      strategy_long_clv_min >= 1.0 ||
      strategy_short_clv_max >= strategy_long_clv_min ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41105_WTI_MCLOSE_LOCATION_MOM";
   ...
   req.reason = (g_signal_direction > 0)
      ? "WTI_MCLOSE_LOCATION_MOM_LONG"
      : "WTI_MCLOSE_LOCATION_MOM_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41105 ||
   ...
   g_attempt_state_key =
      StringFormat("QM5_41105_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41105\",\"ea\":\"wti-mclose-location-mom\"}");
