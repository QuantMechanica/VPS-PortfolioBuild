...
#property description "QM5_20193 EURUSD USDCAD Cointegration"
...
//
// Card:
//   strategy-seeds/cards/approved/QM5_20193_eurusd-cad-coint_card.md
//
// Fixed spread:
//   ln(EURUSD.DWX) - beta * ln(USDCAD.DWX)
//
...
input double strategy_beta              = -0.839757300;
...
bool Strategy_IsHostSymbol()
  {
   return (_Symbol == g_leg_eurusd || _Symbol == g_leg_usdcad);
  }
...
bool Strategy_EnsureBasketScope()
  {
   ...
   string allowed[2] = {"EURUSD.DWX", "USDCAD.DWX"};
   for(int i = 0; i < 2; ++i)
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(!QM_SymbolAssertOrLog(g_leg_eurusd) ||
      !QM_SymbolAssertOrLog(g_leg_usdcad))
      return false;
   ...
   for(int i = 0; i < history_count; ++i)
     {
      if(eurusd_time[i] <= 0 || eurusd_time[i] != usdcad_time[i])
         return false;
      if(eurusd[i] <= 0.0 || usdcad[i] <= 0.0)
         return false;

      spreads[i] =
         MathLog(eurusd[i]) - strategy_beta * MathLog(usdcad[i]);
...
bool Strategy_OpenPair(const int spread_direction)
  {
   ...
   const double weight_sum = eurusd_weight + usdcad_weight;
   if(weight_sum <= 0.0)
      return false;

   const bool long_spread = (spread_direction > 0);
   const QM_OrderType eurusd_type = long_spread ? QM_BUY : QM_SELL;
   const QM_OrderType usdcad_type =
      long_spread
      ? (strategy_beta >= 0.0 ? QM_SELL : QM_BUY)
      : (strategy_beta >= 0.0 ? QM_BUY : QM_SELL);
   const string reason =
      long_spread ? "QM5_20193_LONG_SPREAD_Z_LT_NEG_ENTRY"
                  : "QM5_20193_SHORT_SPREAD_Z_GT_POS_ENTRY";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20193_PAIR_HOST";
