...
#property description "QM5_41490 clock-normalized session range-breakout research instrument"

#include <QM/QM_Common.mqh>

// QM-RESEARCH-2026-0013 research instrument. This is a parameterized,
// clock-normalized implementation of the QM5_13213 mechanics with the hardened
// fail-closed OCO behavior reviewed in QM5_41485. It carries no roster, FTMO,
// pipeline, deployment, T_Live, or AutoTrading authority.

enum Strategy_AnchorClock
  {
   SERVER_RAW = 0,
   GMT3_EQUIVALENT = 1,
   UTC = 2
...
input Strategy_AnchorClock anchor_clock       = GMT3_EQUIVALENT;
...
input int    strategy_flat_hour               = 18;
input int    strategy_flat_minute             = 0;
input int    strategy_grid_offset_minutes     = 0;
...
datetime Strategy_BrokerToClock(const datetime broker_time)
  {
   if(anchor_clock == SERVER_RAW)
      return broker_time;
   const datetime utc_time = QM_BrokerToUTC(broker_time);
   if(anchor_clock == GMT3_EQUIVALENT)
      return utc_time + 3 * 3600;
   return utc_time;
  }

datetime Strategy_ClockToBroker(const datetime clock_time)
  {
   if(anchor_clock == SERVER_RAW)
      return clock_time;
   datetime utc_time = clock_time;
   if(anchor_clock == GMT3_EQUIVALENT)
      utc_time -= 3 * 3600;
...
int Strategy_ClockDayKey(const datetime broker_time)
  {
   const datetime clock_time = Strategy_BrokerToClock(broker_time);
   MqlDateTime dt;
   TimeToStruct(clock_time, dt);
   return dt.year * 1000 + dt.day_of_year;
  }

datetime Strategy_ClockDayStart(const datetime clock_time)
  {
   MqlDateTime dt;
   TimeToStruct(clock_time, dt);
   dt.hour = 0;
...
int Strategy_ClockMinuteOfDay(const datetime broker_time)
  {
   const datetime clock_time = Strategy_BrokerToClock(broker_time);
   MqlDateTime dt;
   TimeToStruct(clock_time, dt);
...
datetime Strategy_TodayAtBroker(const datetime broker_time,
                                const int hour,
                                const int minute)
  {
   const datetime clock_time = Strategy_BrokerToClock(broker_time);
   const datetime clock_target = Strategy_ClockDayStart(clock_time) +
                                 hour * 3600 + minute * 60;
   return Strategy_ClockToBroker(clock_target);
  }
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_41490.%d.%s.attempt", QM_FrameworkMagic(), _Symbol);
  }
...
bool Strategy_BuildRangeAndAtr(const datetime range_start,
                               ...
                               double &atr)
  {
   ...
   // The range-start input is part of the measured contract, not a label.
   // Reject if the oldest requested grid bar does not begin at that instant.
   if(bars[strategy_range_bars - 1].start_time != range_start)
      return false;
...
datetime Strategy_MostRecentGridEnd(const datetime broker_now)
  {
   const datetime clock_now = Strategy_BrokerToClock(broker_now);
   datetime boundary_clock = Strategy_ClockDayStart(clock_now) +
                             strategy_grid_offset_minutes * 60;
   if(boundary_clock > clock_now)
      boundary_clock -= 86400;
   const long elapsed = (long)(clock_now - boundary_clock);
   boundary_clock += (datetime)((elapsed / 3600) * 3600);
   return Strategy_ClockToBroker(boundary_clock);
  }

bool Strategy_ReadTrailLevels(const datetime broker_now, double &trail_low, double &trail_high)
  {
   Strategy_GridBar newest;
   Strategy_GridBar prior;
   const datetime newest_end = Strategy_MostRecentGridEnd(broker_now);
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   const datetime range_start = Strategy_TodayAtBroker(now,
                                                       strategy_range_start_hour,
                                                       strategy_range_start_minute);
   const datetime anchor = Strategy_TodayAtBroker(now,
   ...
   double atr = 0.0;
   if(!Strategy_BuildRangeAndAtr(range_start, anchor, range_high, range_low, atr))
      return false;
   ...
   Strategy_PopulateEntry(buy_req, QM_BUY_STOP, range_high, range_low, "ANCHOR_RANGE_BUY_STOP");
   Strategy_PopulateEntry(sell_req, QM_SELL_STOP, range_low, range_high, "ANCHOR_RANGE_SELL_STOP");
   ...
   if(buy_ok != sell_ok)
     {
      ...
      Strategy_RemoveOurPendingOrders("ANCHOR_RANGE_ONE_SIDED_SEND");
...
void Strategy_ManageOpenPosition()
  {
   const datetime now = TimeCurrent();
   const int current_day = Strategy_ClockDayKey(now);
   const int flat_minute = strategy_flat_hour * 60 + strategy_flat_minute;

   bool stale_pending = (Strategy_ClockMinuteOfDay(now) >= flat_minute);
   for(int i = OrdersTotal() - 1; i >= 0 && !stale_pending; --i)
     {
      ...
      if(Strategy_ClockDayKey(setup_time) != current_day)
         stale_pending = true;
     }
   if(stale_pending)
      Strategy_RemoveOurPendingOrders("ANCHOR_RANGE_SESSION_FLAT");

   const bool has_position = Strategy_HasOurOpenPosition();
   const int pending_count = Strategy_OurPendingCount();
   if(has_position)
      Strategy_RemoveOurPendingOrders("ANCHOR_RANGE_OCO_PEER");
   else if(g_strategy_attempt_day_key == current_day && pending_count == 1)
      Strategy_RemoveOurPendingOrders("ANCHOR_RANGE_PAIR_RECONCILE");
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      if(target_sl > 0.0 && improves)
         QM_TM_MoveSL(ticket, target_sl, "ANCHOR_RANGE_2GRIDBAR_TRAIL");
     }
  }
...
bool Strategy_ExitSignal()
  {
   ...
   const int current_day = Strategy_ClockDayKey(now);
   const int flat_minute = strategy_flat_hour * 60 + strategy_flat_minute;
   if(Strategy_ClockMinuteOfDay(now) >= flat_minute)
      return true;
...
int OnInit()
  {
   const int range_start_minute = strategy_range_start_hour * 60 +
                                  strategy_range_start_minute;
   const int range_end_minute = strategy_range_end_hour * 60 +
                                strategy_range_end_minute;
   const int flat_minute = strategy_flat_hour * 60 + strategy_flat_minute;
   const int required_grid_bars = MathMax(strategy_range_bars,
                                          strategy_atr_period + 1);
   if(qm_ea_id != 41490 || _Period != PERIOD_M30 ||
      (int)anchor_clock < (int)SERVER_RAW || (int)anchor_clock > (int)UTC ||
      strategy_range_start_hour < 0 || strategy_range_start_hour > 23 ||
      strategy_range_start_minute < 0 || strategy_range_start_minute > 59 ||
      strategy_range_end_hour < 0 || strategy_range_end_hour > 23 ||
      strategy_range_end_minute < 0 || strategy_range_end_minute > 59 ||
      strategy_flat_hour < 0 || strategy_flat_hour > 23 ||
      strategy_flat_minute < 0 || strategy_flat_minute > 59 ||
      (strategy_grid_offset_minutes != 0 && strategy_grid_offset_minutes != 30) ||
      strategy_range_start_minute != strategy_grid_offset_minutes ||
      strategy_range_end_minute != strategy_grid_offset_minutes ||
      strategy_range_bars < 1 || strategy_range_bars > 24 ||
      range_start_minute >= range_end_minute ||
      range_end_minute >= flat_minute ||
      range_end_minute - range_start_minute != strategy_range_bars * 60 ||
      strategy_news_retry_window_minutes < 1 ||
      strategy_news_retry_window_minutes > 240 ||
      strategy_atr_period < 2 || strategy_atr_period > 100 ||
      strategy_range_scan_bars < required_grid_bars * 2 ||
      strategy_range_scan_bars > 10000 ||
     ...
     {
      Print("QM5_41490 bounded research-instrument input contract invalid");
...
void OnTradeTransaction(const MqlTradeTransaction &trans,
                        ...
                        const MqlTradeResult &result)
  {
   ...
   if(entry == DEAL_ENTRY_IN || entry == DEAL_ENTRY_INOUT)
      Strategy_RemoveOurPendingOrders("ANCHOR_RANGE_OCO_FILL_TRANSACTION");
  }
