...
SOURCE = EA_DIR / "QM5_11302_tc-m5-bb-stoch-extreme-reversal.mq5"
SPEC = EA_DIR / "SPEC.md"
...
def test_source_preserves_approved_mechanics_and_guardrails() -> None:
    ...
    required = (
        "QM_BB_Upper(_Symbol, strategy_timeframe",
        "QM_BB_Lower(_Symbol, strategy_timeframe",
        "QM_Stoch_K(_Symbol, strategy_timeframe",
        "bar2.high > bar3.high && bar3.high > bar4.high",
        "bar2.low < bar3.low && bar3.low < bar4.low",
        "QM_StopFixedPips(_Symbol, QM_BUY, entry, strategy_stop_pips)",
        "QM_TakeFixedPips(_Symbol, QM_BUY, entry, strategy_tp_pips)",
...
def test_backtest_sets_are_fixed_risk_and_match_registered_slots() -> None:
    expected = {"EURUSD.DWX": "0", "GBPUSD.DWX": "1", "USDJPY.DWX": "2"}
...
def test_spec_has_canonical_sections_and_identity() -> None:
    ...
    assert "**EA ID:** QM5_11302" in text
