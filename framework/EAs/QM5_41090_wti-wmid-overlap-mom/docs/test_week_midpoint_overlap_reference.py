"""Deterministic reference checks for QM5_41090 weekly midpoint overlap."""
...
def high_low_valid(bar: Bar) -> bool:
...
def midpoint_overlap_signal(
    ...
    """Mirror bounded weekly high/low aggregation and midpoint-overlap signal."""

    empty = (False, 0, (0, 0), (0.0,) * 8)
    ...
    new_high, new_low = weekly[0]
    parent_high, parent_low = weekly[1]
    ranges = (new_high - new_low, parent_high - parent_low)
    if any(value <= 0.0 or not math.isfinite(value) for value in ranges):
        return empty

    overlap_low = max(new_low, parent_low)
    overlap_high = min(new_high, parent_high)
    new_midpoint = new_low + 0.5 * ranges[0]
    parent_midpoint = parent_low + 0.5 * ranges[1]
    direction = 0
    if overlap_low >= overlap_high:
        pass
    elif new_midpoint > parent_midpoint:
        direction = 1
    elif new_midpoint < parent_midpoint:
        direction = -1
    values = (
    ...
    return True, direction, (len(buckets[0]), len(buckets[1])), values
...
class WeekMidpointOverlapReferenceTest(unittest.TestCase):
    def signal(self, bars: list[Bar], offset: timedelta = timedelta(0)):
        return midpoint_overlap_signal(20260824, bars, offset)
    ...
    def test_strict_long_and_short_paths(self) -> None:
        ...
        self.assertEqual(values, (120.0, 95.0, 110.0, 90.0, 95.0, 110.0, 107.5, 100.0))
    ...
    def test_equal_midpoint_touch_and_disjoint_ranges_are_flat(self) -> None:
        cases = [
            ((99.0, 115.0, 85.0, 101.0), (95.0, 110.0, 90.0, 100.0)),
            ((85.0, 90.0, 80.0, 89.0), (95.0, 110.0, 90.0, 100.0)),
            ((80.0, 89.0, 70.0, 88.0), (95.0, 110.0, 90.0, 100.0)),
        ]
    ...
    def test_one_endpoint_may_move_against_midpoint_direction(self) -> None:
        _, _, bars = sample(
            new=(100.0, 108.0, 95.0, 107.0),
            parent=(95.0, 110.0, 90.0, 100.0),
        )
        self.assertEqual(self.signal(bars)[0:2], (True, 1))

        _, _, bars = sample(
            new=(100.0, 105.0, 92.0, 93.0),
            parent=(95.0, 110.0, 90.0, 100.0),
        )
        self.assertEqual(self.signal(bars)[0:2], (True, -1))
    ...
    def test_opens_and_closes_do_not_enter_the_signal(self) -> None:
        _, _, bars = sample()
        baseline = self.signal(bars)
        changed = [
            Bar(bar.opened, -999.0, bar.high, bar.low, float("nan"))
            for bar in bars
        ]
        self.assertEqual(self.signal(changed), baseline)
    ...
    def test_static_build_contract_is_fixed_risk_and_completed_data_only(self) -> None:
        source = (EA_DIR / "QM5_41090_wti-wmid-overlap-mom.mq5").read_text(encoding="utf-8")
        setfile = (EA_DIR / "sets" / "QM5_41090_wti-wmid-overlap-mom_XTIUSD.DWX_D1_backtest.set").read_text(encoding="utf-8")
        for marker in (
            ...
            "new_range <= 0.0 || parent_range <= 0.0",
        ...
        self.assertNotIn("bars[index].open", source)
        self.assertNotIn("bars[index].close", source)
        self.assertLess(
            source.index("Strategy_RecordWeekAttempt(g_decision_week_key)"),
            source.index("Strategy_LoadMidpointOverlapSignal(g_decision_week_key"),
        )
        for marker in (
            "qm_ea_id=41090",
            "RISK_FIXED=1000",
