...
#property description "QM5_41090 WTI Weekly Overlap-Midpoint Momentum"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41090 - WTI Weekly Overlap-Midpoint Momentum
// -----------------------------------------------------------------------------
// D1 structural crude-oil sleeve:
//   - aggregate two consecutive completed broker-week high/low packages
//   - require their price intervals to overlap by a strictly positive amount
//   - follow the strict drift of their arithmetic high/low midpoints
//   - consume the week before fallible gates; never enter late or retry
//   - hold the completed overlapping-auction signal for one broker week
// Runtime uses MT5-native high/low, calendar, history, and framework state only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                      = 41090;
...
double   g_overlap_low            = 0.0;
double   g_overlap_high           = 0.0;
double   g_new_midpoint           = 0.0;
double   g_parent_midpoint        = 0.0;
...
bool Strategy_BarHighLowValid(const MqlRates &bar)
  {
   if(bar.time <= 0 ||
...
bool Strategy_LoadMidpointOverlapSignal(const int current_week_key,
                                       const int label_offset,
                                       int &direction,
                                       int &new_week_bars,
                                       ...
                                       double &parent_midpoint)
  {
   ...
   ArraySetAsSeries(bars, true);
   const int copied =
      CopyRates(_Symbol, // perf-allowed: bounded completed-week high/low scan behind the sole QM_IsNewBar branch.
                PERIOD_D1,
                1,
                strategy_history_bars,
                bars);
   ...
   int week_last_date_keys[2] = {0, 0};
   int found_weeks = 0;
   bool parent_boundary_seen = false;

   for(int index = 0; index < copied; ++index)
     {
      if(!Strategy_BarHighLowValid(bars[index]))
         return false;
      ...
      if(session_date_key <= 0 ||
         (week_last_date_keys[bucket] > 0 &&
          session_date_key >= week_last_date_keys[bucket]))
         return false;
      week_last_date_keys[bucket] = session_date_key;
   ...
   const double parent_range = parent_high - parent_low;
   if(new_range <= 0.0 || parent_range <= 0.0 ||
      !MathIsValidNumber(new_range) ||
      !MathIsValidNumber(parent_range))
      return false;

   overlap_low = MathMax(new_low, parent_low);
   overlap_high = MathMin(new_high, parent_high);
   new_midpoint = new_low + 0.5 * new_range;
   parent_midpoint = parent_low + 0.5 * parent_range;
   if(!MathIsValidNumber(overlap_low) ||
      !MathIsValidNumber(overlap_high) ||
      !MathIsValidNumber(new_midpoint) ||
      !MathIsValidNumber(parent_midpoint))
      return false;

   if(overlap_low >= overlap_high)
      return true;
   if(new_midpoint > parent_midpoint)
      direction = 1;
   else if(new_midpoint < parent_midpoint)
      direction = -1;
   return true;
  }
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_WeekAlreadyEntered(g_decision_week_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadMidpointOverlapSignal(g_decision_week_key,
                                           ...
                                           g_parent_midpoint);
      if(!g_signal_valid)
         g_signal_state = "weekly_high_low_validation_failed";
      else if(g_signal_direction > 0)
         g_signal_state = "higher_midpoint_positive_overlap_long";
      else if(g_signal_direction < 0)
         g_signal_state = "lower_midpoint_positive_overlap_short";
      else if(g_overlap_low >= g_overlap_high)
         g_signal_state = "nonpositive_overlap_flat";
      else
         g_signal_state = "midpoint_equality_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"week\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_week_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"new_week_bars\":%d,\"parent_week_bars\":%d,\"new_high\":%.8f,\"new_low\":%.8f,\"parent_high\":%.8f,\"parent_low\":%.8f,\"overlap_low\":%.8f,\"overlap_high\":%.8f,\"new_midpoint\":%.8f,\"parent_midpoint\":%.8f,\"state\":\"%s\"}",
                            g_decision_week_key,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41090 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41090_WTI_WMID_OVERLAP_MOM";
   ...
   req.reason = (g_signal_direction > 0)
      ? "WTI_WMID_OVERLAP_MOM_LONG"
      : "WTI_WMID_OVERLAP_MOM_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41090 ||
   ...
   g_attempt_state_key =
      StringFormat("QM5_41090_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41090\",\"ea\":\"wti-wmid-overlap-mom\"}");
