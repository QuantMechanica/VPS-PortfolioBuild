# QM5_41439_xng-winter-nr2-breakout - Strategy Spec

**EA ID:** QM5_41439

**Slug:** `xng-winter-nr2-breakout`

**Strategy ID:** `EIA-CRABEL-XNG-WINTER-NR2-BREAKOUT-20260911_S01`

## 1. Strategy Logic

During a November-March normalized broker week, aggregate the two immediately completed
...
Exact configured `XNGUSD.DWX` D1 carrier, slot zero, magic `414390000`. The symbol is an input;
...
Only November, December, January, February, and March Monday anchors qualify. Each of the two
immediately completed weeks must have three to five valid unique sessions. After strict
contraction, the first completed close above/below the newest completed-week box enters long/short.
Expected cadence is five to twelve positions per full year, with retirement below five in any full
scored year.

## 6. Source Citation And Non-Duplicate Boundary

The source packet is
`strategy-seeds/sources/EIA-CRABEL-XNG-WINTER-NR2-BREAKOUT-20260911/source.md`. It combines
official EIA winter-demand context with governed Crabel range-state lineage. The exact rule, CFD
efficacy, and book correlation are not source claims.

The WTI hurricane carrier `QM5_41437` uses the same chronology in another physical calendar.
Within XNG, `QM5_41063` is an all-year seven-week contraction rank, `QM5_41438` is immediate
expanding-range CLV momentum, and certified `QM5_12567` is a two-day long-only oscillator pullback.
...
financing, sparse seasonal sampling, delayed confirmation, false breakouts, and overlap with the
existing XNG sleeve can exceed modeled risk. No live set, deployment, portfolio admission,
