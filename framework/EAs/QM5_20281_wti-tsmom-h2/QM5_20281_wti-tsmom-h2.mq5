...
#property description "QM5_20281 WTI Twelve-Month Trend / Two-Month Hold"
...
//   1. derive thirteen consecutive completed WTI month-end closes from D1;
//   2. calculate the exact twelve-month return ln(C[12] / C[0]);
...
double g_cache_twelve_month_return = 0.0;
...
void Strategy_ResetCachedState()
  {
   ...
   g_cache_signal = 0;
   g_cache_twelve_month_return = 0.0;
   g_cache_state_reason = "not_evaluated";
  }

bool Strategy_TwelveMonthReturnSignal(const double &closes[],
                                      int &signal)
  {
   signal = 0;
   if(strategy_return_months != 12 ||
      ArraySize(closes) != 13)
     {
      g_cache_state_reason = "bad_twelve_month_contract";
      return false;
     }
   ...
   const double endpoint_ratio = closes[12] / closes[0];
   ...
   const double twelve_month_return = MathLog(endpoint_ratio);
   if(!MathIsValidNumber(twelve_month_return))
     {
      g_cache_state_reason = "invalid_twelve_month_return";
      return false;
     }
   ...
   if(!MathIsValidNumber(chained_return) ||
      MathAbs(twelve_month_return - chained_return) > 1.0e-10)
     {
      g_cache_state_reason = "endpoint_path_identity_failed";
      return false;
     }

   g_cache_twelve_month_return = twelve_month_return;
   if(twelve_month_return == 0.0)
     {
      g_cache_state_reason = "exact_zero_twelve_month_return";
      return true;
     }

   signal = (twelve_month_return > 0.0) ? 1 : -1;
...
bool Strategy_LoadSignalState(int &signal)
  {
   ...
   return Strategy_TwelveMonthReturnSignal(monthly_closes,
                                           signal);
  }
...
void Strategy_PrepareBimonthlySignal()
  {
   ...
   QM_LogEvent(QM_INFO,
               "MONTHLY_STATE",
               StringFormat("{\"month\":%d,\"valid\":%s,\"signal\":%d,\"return_12m\":%.10f,\"state\":\"%s\"}",
                            g_cache_month_key,
                            g_cache_signal_valid ? "true" : "false",
                            g_cache_signal,
                            g_cache_twelve_month_return,
                            g_cache_state_reason));
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20281_WTI_TSMOM_H2";
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_20281_BIMONTH_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_20281\",\"ea\":\"wti-tsmom-h2\"}");
