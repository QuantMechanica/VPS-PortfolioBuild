"""Independent deterministic fixtures for QM5_41229.

The suite covers normalized D1 labels, completed WTI month endpoints, exact
Y-5..Y-1 membership, fixed five-sample trimean arithmetic, sign boundaries,
...
def trimean_signal(observations: list[float]) -> TrimeanResult | None:
    ...
    location = (lower_hinge + 2.0 * median + upper_hinge) / 4.0
...
class SameCalendarTrimeanReferenceTests(unittest.TestCase):
    ...
    def test_fixed_hinges_weights_and_divisor_are_exact(self) -> None:
        result = trimean_signal([-2.0, -1.0, 0.375, 0.5, 2.0])
        self.assertIsNotNone(result)
        assert result is not None
        self.assertEqual(result.lower_hinge, -1.0)
        self.assertEqual(result.median, 0.375)
        self.assertEqual(result.upper_hinge, 0.5)
        self.assertEqual(result.location, 0.0625)
        self.assertEqual(result.direction, 1)

    def test_disagreement_fixture_opposes_mean_trim_and_winsor(self) -> None:
        values = [-2.0, -1.0, 0.375, 0.5, 2.0]
        result = trimean_signal(values)
        raw_mean = sum(values) / 5.0
        trimmed_mean = sum(sorted(values)[1:4]) / 3.0
        winsor_mean = sum([-1.0, -1.0, 0.375, 0.5, 0.5]) / 5.0
    ...
    def test_second_fixture_opposes_raw_mean_and_median(self) -> None:
        values = [-8.0, -4.0, 0.5, 1.0, 12.0]
        result = trimean_signal(values)
        self.assertIsNotNone(result)
        assert result is not None
    ...
    def test_sort_makes_year_order_irrelevant_after_membership(self) -> None:
        values = [-8.0, -4.0, 0.5, 1.0, 12.0]
        self.assertEqual(trimean_signal(values), trimean_signal(values[::-1]))

    def test_strict_epsilon_and_invalid_states_fail_closed(self) -> None:
        at_positive = trimean_signal([EPSILON] * 5)
        at_negative = trimean_signal([-EPSILON] * 5)
        beyond = trimean_signal([EPSILON + 1.0e-15] * 5)
        ...
        self.assertIsNone(trimean_signal([0.01] * 4))
        self.assertIsNone(trimean_signal([0.01, 0.01, math.nan, 0.01, 0.01]))
    ...
    def test_static_build_contract_matches_approved_card(self) -> None:
        source_path = EA_DIR / "QM5_41229_wti-samecal-trimean5.mq5"
        source = source_path.read_text(encoding="utf-8")
        set_path = (
            EA_DIR
            / "sets"
            / "QM5_41229_wti-samecal-trimean5_XTIUSD.DWX_D1_backtest.set"
        )
        ...
        for marker in (
            ...
            "lower_hinge + 2.0 * median_value + upper_hinge",
        ...
        self.assertLess(
            ...
            prepare.index("Strategy_LoadTrimeanSignal"),
        )
        ...
        approved = (
            ...
            / "cards"
            / "approved"
            / "QM5_41229_wti-samecal-trimean5_card.md"
        )
        ...
        self.assertIn(
            "41229,wti-samecal-trimean5,0,XTIUSD.DWX,412290000",
            magic_rows,
        )
        resolver = (
            REPO_ROOT
        ...
        self.assertIn("412290000", resolver)
