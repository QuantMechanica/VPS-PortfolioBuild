...
#property description "QM5_41229 WTI Same-Calendar Five-Sample Tukey Trimean"
...
//   - sort the five returns and read fixed order statistics 1, 2, and 3
//   - weight lower hinge, median, and upper hinge exactly 1:2:1
//   - follow only when the divisor-four trimean is beyond +/-1e-12
...
input int    strategy_median_index         = 2;
input int    strategy_upper_hinge_index   = 3;
input int    strategy_trimean_divisor     = 4;
...
// -----------------------------------------------------------------------------
// Exact completed-month endpoint and five-sample-trimean helpers.
// -----------------------------------------------------------------------------
...
bool Strategy_TrimeanSignal(const double &observations[],
                             ...
                             int &direction)
  {
   ...
   if(strategy_history_years != 5 ||
      ...
      strategy_median_index != 2 ||
      strategy_upper_hinge_index != 3 ||
      strategy_trimean_divisor != 4 ||
   ...
   const double median_value = sorted_values[2];
   ...
   if(!MathIsValidNumber(lower_hinge) ||
      ...
      lower_hinge > median_value ||
      median_value > upper_hinge)
      return false;

   location_value =
      (lower_hinge + 2.0 * median_value + upper_hinge) / 4.0;
...
bool Strategy_LoadTrimeanSignal(
   ...
   int &direction)
  {
   ...
   return Strategy_TrimeanSignal(observations,
                                              sample_count,
                                              location_value,
                                              direction);
  }
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   // Consume before history, trimean arithmetic, news, spread, quote,
   ...
   else
     {
      g_signal_valid =
          Strategy_LoadTrimeanSignal(
            ...
            g_signal_direction);
      if(!g_signal_valid)
         g_signal_state = "endpoint_sample_or_trimean_failed";
      else if(g_signal_direction > 0)
         g_signal_state = "tukey_trimean_long";
      else if(g_signal_direction < 0)
         g_signal_state = "tukey_trimean_short";
      else
         g_signal_state = "tukey_trimean_epsilon_flat";
     }
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(!QM_InputRequireLong("qm_ea_id", qm_ea_id, 41229) ||
      ...
      !QM_InputRequireLong("strategy_median_index", strategy_median_index, 2) ||
      !QM_InputRequireLong("strategy_upper_hinge_index", strategy_upper_hinge_index, 3) ||
      !QM_InputRequireLong("strategy_trimean_divisor", strategy_trimean_divisor, 4) ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "WTI_SAMECAL_TRIMEAN5";
   ...
   if(g_signal_direction > 0)
     {
      req.type = QM_BUY;
      req.reason = "WTI_SAMECAL_TRIMEAN5_LONG";
     }
   else
     {
      req.type = QM_SELL;
      req.reason = "WTI_SAMECAL_TRIMEAN5_SHORT";
     }
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41229 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI same-calendar five-sample trimean card holds to the next broker month"))
      return INIT_FAILED;
   ...
   g_attempt_state_key =
      StringFormat("QM5_41229_SAMECAL_TRIMEAN5_MONTH_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41229\",\"ea\":\"wti-samecal-trimean5\"}");
