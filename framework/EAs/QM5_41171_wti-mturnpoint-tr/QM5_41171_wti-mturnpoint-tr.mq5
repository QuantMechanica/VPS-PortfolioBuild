...
#property description "QM5_41171 WTI Monthly Turning-Point Persistence Trend"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41171 - WTI Monthly Turning-Point Persistence Trend
// -----------------------------------------------------------------------------
// D1 structural crude-oil sleeve:
//   - reconstruct thirteen consecutive completed broker-month-end closes
//   - count strict local peaks and troughs across eleven interior endpoints
//   - follow the endpoint direction only when the count is below its iid mean
...
bool Strategy_TurningPointSignal(const double &closes[],
                                 Strategy_SignalMetrics &metrics)
  {
   ZeroMemory(metrics);
   metrics.endpoint_count = ArraySize(closes);
   metrics.null_mean_numerator = 22;
   ...
   for(int index = 1; index < strategy_endpoint_count - 1; ++index)
     {
      ...
      const bool trough =
         (closes[index - 1] > closes[index] &&
          closes[index] < closes[index + 1]);
      if(peak || trough)
         ++metrics.turning_points;
     }

   if(metrics.turning_points < 0 || metrics.turning_points > 11)
      return false;
   metrics.comparison_lhs = 3 * metrics.turning_points;
   const bool below_null_mean =
      (metrics.comparison_lhs < metrics.null_mean_numerator);
   const bool within_locked_boundary =
      (metrics.turning_points <= strategy_max_turning_points);
   if(below_null_mean != within_locked_boundary)
      return false;
   ...
   if(below_null_mean)
     {
      if(metrics.newest_close > metrics.oldest_close)
         metrics.direction = 1;
      else if(metrics.newest_close < metrics.oldest_close)
         metrics.direction = -1;
      else
         return false;
     }
                                        ...
                                        int &direction)
  {
   ...
   const bool valid =
      Strategy_LoadMonthlyEndpoints(current_month_key,
                                    ...
                                    closes,
                                    endpoint_times) &&
      Strategy_TurningPointSignal(closes, metrics);
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
   ...
   else
     {
      ...
      g_signal_valid =
         endpoints_valid &&
         Strategy_TurningPointSignal(closes, g_signal_metrics);
      if(!g_signal_valid)
         g_signal_state = "turning_point_validation_failed";
      else if(g_signal_metrics.direction == 0)
         g_signal_state = "turning_point_count_not_below_null_mean_flat";
      else if(g_signal_metrics.direction > 0)
         g_signal_state = "turning_point_persistence_wti_long";
      else if(g_signal_metrics.direction < 0)
         g_signal_state = "turning_point_persistence_wti_short";
      else
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"endpoint_count\":%d,\"turning_points\":%d,\"comparison_lhs\":%d,\"null_mean_numerator\":%d,\"oldest_close\":%.10f,\"newest_close\":%.10f,\"oldest_endpoint\":%I64d,\"newest_endpoint\":%I64d,\"state\":\"%s\"}",
                            ...
                            g_signal_metrics.turning_points,
                            g_signal_metrics.comparison_lhs,
                            g_signal_metrics.null_mean_numerator,
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41171_WTI_TURNING_POINT";
   ...
   req.reason = (g_signal_metrics.direction > 0)
      ? "WTI_TURNING_POINT_LONG"
      : "WTI_TURNING_POINT_SHORT";
...
bool Strategy_NewsAllowsEntry(const datetime broker_time)
  {
   if(qm_news_temporal != QM_NEWS_TEMPORAL_OFF ||
      ...
      return QM_NewsAllowsTrade2(_Symbol,
                                 ...
                                 qm_news_temporal,
                                 qm_news_compliance);
   return QM_NewsAllowsTrade(_Symbol, broker_time, QM_NEWS_OFF);
  }
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41171 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI turning-point persistence trend holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41171_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41171\",\"ea\":\"wti-mturnpoint-tr\"}");
