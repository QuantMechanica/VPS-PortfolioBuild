...
#property description "QM5_41113 XAU/XAG Completed-Month Two-Half Agreement Reversion"
...
// the deterministic floor(n/2) observation boundary to partition the newest
// month's relative path into two cumulative legs. Fade only when both legs
// have the same strict sign, using one opposite equal-notional package held to
// the next month. Runtime is native, completed-price-only, deterministic, and
// one-attempt-per-month.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41113;
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41113 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
            QM_MagicSymbolCanonical(strategy_xag_symbol) == QM_MagicSymbolCanonical("XAGUSD.DWX") &&
            QM_MagicSymbolCanonical(strategy_xau_symbol) == QM_MagicSymbolCanonical("XAUUSD.DWX") &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_MHALFAGREE_RV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41113_XAUXAG_MHALFAGREE_RV";
...
bool Strategy_LoadMonthlyHalfAgreement(
      const int current_month_key,
      int &completed_month_sessions,
      int &parent_month_sessions,
      int &split_index,
      double &first_half_return,
      double &second_half_return,
      int &direction)
  {
   ...
   // Q[0]..Q[n-1] and k=floor(n/2), Q[k-1] maps to series index n-k.
   // The shared midpoint ratio is an endpoint/anchor, so the first cumulative
   // leg contains exactly k adjacent returns and the second contains n-k.
   split_index = completed_month_sessions / 2;
   const int midpoint_series_index =
   ...
   first_half_return = midpoint_ratio - parent_final_ratio;
   second_half_return = newest_ratios[0] - midpoint_ratio;
   if(!MathIsValidNumber(midpoint_ratio) ||
      !MathIsValidNumber(first_half_return) ||
      !MathIsValidNumber(second_half_return))
      return false;

   // direction > 0 means BUY XAU / SELL XAG. This is a contrarian package:
   // fade only when both cumulative chronological halves share a strict sign.
   if(first_half_return > 0.0 && second_half_return > 0.0)
      direction = -1;
   else if(first_half_return < 0.0 && second_half_return < 0.0)
      direction = 1;
   return true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41113_XAUXAG_MHALFAGREE_RV_HOST";
   ...
   int split_index = 0;
   int direction = 0;
   double first_half_return = 0.0;
   double second_half_return = 0.0;
   const bool valid =
      Strategy_LoadMonthlyHalfAgreement(g_signal_month_key,
                                       completed_month_sessions,
                                       parent_month_sessions,
                                       split_index,
                                       first_half_return,
                                       second_half_return,
                                       direction);
   string state = "monthly_ratio_half_agreement_failed";
   if(valid && direction > 0)
      state = "negative_half_agreement_fade_long_xau";
   else if(valid && direction < 0)
      state = "positive_half_agreement_fade_short_xau";
   else if(valid)
      state = "half_equality_or_disagreement_flat";

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"completed_month_sessions\":%d,\"parent_month_sessions\":%d,\"split_k\":%d,\"half_1\":%.12e,\"half_2\":%.12e,\"direction\":%d,\"state\":\"%s\"}",
                            g_signal_month_key,
                            completed_month_sessions,
                            parent_month_sessions,
                            split_index,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG completed-month two-half agreement reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41113\",\"ea\":\"xauxag-mhalfagree-rv\"}");
