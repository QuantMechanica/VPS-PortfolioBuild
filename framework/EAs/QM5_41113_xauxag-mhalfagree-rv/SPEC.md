# QM5_41113_xauxag-mhalfagree-rv - Strategy Spec
...
**Source:** `SCHWEIKERT-CME-XAUXAG-MHALFAGREE-RV-2026`
...
month's chronological final log ratio anchors the newest completed month, and
the deterministic observation midpoint partitions its full relative path:
...
half_1>0 and half_2>0 => SELL XAU, BUY XAG
half_1<0 and half_2<0 => BUY XAU, SELL XAG
otherwise             => FLAT
```

The first leg contains exactly the first `k` adjacent returns from `P`; the
second contains the remaining `n-k`. Their shared midpoint ratio is an
endpoint/anchor, not a duplicated return. The month is consumed flat on half
equality or disagreement, an invalid split, incomplete/nonconsecutive month,
asynchronous pair, or invalid close. Signal magnitude never changes
eligibility or risk.
...
- Host/traded slot 0: exact `XAUUSD.DWX`, D1, magic `411130000`.
- Companion/traded slot 1: exact `XAGUSD.DWX`, D1, magic `411130001`.
...
Q02 retires below five rather than changing the rule. Two positive cumulative
halves produce SELL XAU / BUY XAG; two negative halves produce BUY XAU / SELL
XAG. Equality or half-sign disagreement remains flat.
...
and the intermarket ratio carrier. The completed-month observation split,
two-half cumulative-return agreement, contrarian side, CFD mapping, risk, and
lifecycle are disclosed QM hypotheses. No source return, hedge ratio,
