"""Independent deterministic fixtures for QM5_41113's monthly two-half basket rule."""
...
def monthly_half_agreement(
    ...
    split_index = len(newest_ratios) // 2
    midpoint_series_index = len(newest_ratios) - split_index
    if not 0 < split_index < len(newest_ratios):
        return HalfAgreementResult()
    if not 0 < midpoint_series_index < len(newest_ratios):
        return HalfAgreementResult()

    midpoint_ratio = newest_ratios[midpoint_series_index]
    first_half = midpoint_ratio - parent_final_ratio
    second_half = newest_ratios[0] - midpoint_ratio
    direction = 0
...
class MonthlyHalfAgreementReferenceTest(unittest.TestCase):
    def signal(self, returns: list[float], **kwargs) -> HalfAgreementResult:
        xau, xag = sample(returns, **kwargs)
        completed_key = kwargs.get("completed_key", 202607)
        return monthly_half_agreement(next_month_key(completed_key), xau, xag)

    def test_positive_cumulative_halves_are_faded(self) -> None:
        result = self.signal([0.01] * 10 + [0.02] * 10)
        self.assertTrue(result.valid)
        self.assertEqual(result.direction, -1)
        self.assertEqual(result.split_index, 10)
        self.assertAlmostEqual(result.first_half, 0.10, places=10)
        self.assertAlmostEqual(result.second_half, 0.20, places=10)

    def test_negative_cumulative_halves_are_faded(self) -> None:
        result = self.signal([-0.01] * 10 + [-0.02] * 10)
        self.assertTrue(result.valid)
        self.assertEqual(result.direction, 1)
        self.assertEqual(result.split_index, 10)
    ...
    def test_half_sign_disagreement_is_flat(self) -> None:
        result = self.signal([0.02] * 10 + [-0.01] * 10)
        self.assertTrue(result.valid)
        self.assertGreater(result.first_half, 0.0)
        self.assertLess(result.second_half, 0.0)
        self.assertEqual(result.direction, 0)

    def test_half_equality_is_flat(self) -> None:
        result = self.signal([0.0] * 9 + [0.01] * 9)
        self.assertTrue(result.valid)
        self.assertEqual(result.completed_sessions, 18)
        self.assertEqual(result.split_index, 9)
        self.assertEqual(result.first_half, 0.0)
        self.assertGreater(result.second_half, 0.0)
        self.assertEqual(result.direction, 0)

    def test_even_and_odd_session_splits_are_floor_partitioned(self) -> None:
        for count in (17, 20, 23):
            split = count // 2
            result = self.signal([0.01] * split + [0.02] * (count - split))
            self.assertTrue(result.valid)
            self.assertEqual(result.completed_sessions, count)
            self.assertEqual(result.split_index, split)
            self.assertAlmostEqual(result.first_half, 0.01 * split, places=10)
            self.assertAlmostEqual(result.second_half, 0.02 * (count - split), places=10)
    ...
    def test_asynchronous_and_non_descending_pairs_are_rejected(self) -> None:
        ...
        self.assertFalse(monthly_half_agreement(202608, xau, xag).valid)
    ...
    def test_invalid_close_current_month_leak_and_short_buffer_are_rejected(self) -> None:
        ...
        xau, xag = sample([0.01] * 20)
        self.assertFalse(monthly_half_agreement(202608, xau[:-1], xag[:-1]).valid)
    ...
    def test_parent_chronological_final_is_the_anchor(self) -> None:
        ...
        self.assertEqual(result.direction, -1)
        self.assertAlmostEqual(result.first_half, 0.10, places=10)
        self.assertAlmostEqual(result.second_half, 0.10, places=10)
    ...
    def test_static_build_contract_matches_approved_card(self) -> None:
        source = (EA_DIR / "QM5_41113_xauxag-mhalfagree-rv.mq5").read_text(encoding="utf-8")
        setfile = (
            EA_DIR
            / "sets"
            / "QM5_41113_xauxag-mhalfagree-rv_QM5_41113_XAU_XAG_MHALFAGREE_RV_D1_D1_backtest.set"
        ...
        self.assertEqual(manifest["logical_symbol"], "QM5_41113_XAU_XAG_MHALFAGREE_RV_D1")
        ...
        approved = (
            ...
            / "cards"
            / "approved"
            / "QM5_41113_xauxag-mhalfagree-rv_card.md"
        )
