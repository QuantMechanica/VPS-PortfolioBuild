...
#property description "QM5_41183 WTI Monthly Signed-ECDF Distribution-Shift Trend"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41183 - WTI Monthly Signed-ECDF Distribution-Shift Trend
// -----------------------------------------------------------------------------
// D1 structural crude-oil sleeve:
//   - reconstruct twelve consecutive completed broker-month-end closes
//   - compare the fixed newer six against the fixed older six
//   - continue only when one signed ECDF count gap dominates at 3/6 or more
...
bool Strategy_LoadMonthlyEndpoints(const int current_month_key,
                                   ...
                                   datetime &endpoint_times[])
  {
   ...
   if(endpoint_count != strategy_endpoint_count || endpoint_count != 12)
      return false;
...
bool Strategy_SignedEcdfSignal(const double &closes[],
                               Strategy_SignalMetrics &metrics)
  {
   ...
   metrics.d_plus_count = 0;
   metrics.d_minus_count = 0;
   metrics.old_seen = 0;
   metrics.new_seen = 0;
   ...
   for(int rank_index = 0;
       ...
       ++rank_index)
     {
      ...
      string member = "N";
      if(selected < strategy_block_size)
        {
         ++metrics.old_seen;
         member = "O";
        }
      ...
      const int delta = metrics.old_seen - metrics.new_seen;
      metrics.d_plus_count = MathMax(metrics.d_plus_count, delta);
      metrics.d_minus_count = MathMax(metrics.d_minus_count, -delta);

      if(StringLen(metrics.sorted_membership_path) > 0)
         metrics.sorted_membership_path += ",";
      metrics.sorted_membership_path +=
         StringFormat("%s:%d", member, delta);
     }

   if(metrics.old_seen != strategy_block_size ||
      metrics.new_seen != strategy_block_size ||
      metrics.d_plus_count < 0 ||
      metrics.d_plus_count > strategy_block_size ||
      metrics.d_minus_count < 0 ||
      metrics.d_minus_count > strategy_block_size ||
   ...
   metrics.direction = 0;
   if(metrics.d_plus_count >= strategy_min_gap_count &&
      metrics.d_plus_count > metrics.d_minus_count)
      metrics.direction = 1;
   else if(metrics.d_minus_count >= strategy_min_gap_count &&
           metrics.d_minus_count > metrics.d_plus_count)
      metrics.direction = -1;
   return true;
  }
                                        ...
                                        int &direction)
  {
   ...
   const bool valid =
      Strategy_LoadMonthlyEndpoints(current_month_key,
                                    ...
                                    closes,
                                    endpoint_times) &&
      Strategy_SignedEcdfSignal(closes, metrics);
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
   ...
   else
     {
      ...
      g_signal_valid =
         endpoints_valid &&
         Strategy_SignedEcdfSignal(closes, g_signal_metrics);
      if(!g_signal_valid)
         g_signal_state = "signed_ecdf_validation_failed";
      else if(g_signal_metrics.direction == 0)
         g_signal_state = "signed_ecdf_central_or_tied_flat";
      else if(g_signal_metrics.direction > 0)
         g_signal_state = "signed_ecdf_newer_higher_wti_long";
      else if(g_signal_metrics.direction < 0)
         g_signal_state = "signed_ecdf_newer_lower_wti_short";
      else
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"endpoint_count\":%d,\"block_size\":%d,\"d_plus_count\":%d,\"d_minus_count\":%d,\"old_seen\":%d,\"new_seen\":%d,\"sorted_membership\":\"%s\",\"older_closes\":\"%s\",\"newer_closes\":\"%s\",\"oldest_close\":%.10f,\"newest_close\":%.10f,\"oldest_endpoint\":%I64d,\"newest_endpoint\":%I64d,\"state\":\"%s\"}",
                            ...
                            g_signal_metrics.d_plus_count,
                            g_signal_metrics.d_minus_count,
                            g_signal_metrics.old_seen,
                            g_signal_metrics.new_seen,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41183 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41183_WTI_SIGNED_ECDF_SHIFT_TREND";
   ...
   req.reason = (g_signal_metrics.direction > 0)
      ? "WTI_SIGNED_ECDF_NEWER_HIGHER_LONG"
      : "WTI_SIGNED_ECDF_NEWER_LOWER_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41183 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI signed-ECDF distribution-shift trend holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41183_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41183\",\"ea\":\"wti-mks-shift-tr\"}");
