...
#property description "QM5_10225 TradingView Keltner Golden-Cross Breakout"
...
input double strategy_breakout_atr_mult     = 1.0;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   const double basis_now = QM_EMA(_Symbol, _Period, strategy_keltner_ema_period, 1);
   ...
   const double upper_now = basis_now + atr_now * strategy_breakout_atr_mult;
   const double upper_prev = basis_prev + atr_prev * strategy_breakout_atr_mult;
   const double lower_now = basis_now - atr_now * strategy_breakout_atr_mult;
   const double lower_prev = basis_prev - atr_prev * strategy_breakout_atr_mult;

   if(fast_ma > slow_ma && close_prev <= upper_prev && close_now > upper_now)
     {
      ...
      req.reason = "QM5_10225_KELTNER_GC_LONG";
      return (req.sl > 0.0 && req.tp > 0.0);
     }

   if(fast_ma < slow_ma && close_prev >= lower_prev && close_now < lower_now)
     {
      ...
      req.reason = "QM5_10225_KELTNER_GC_SHORT";
      return (req.sl > 0.0 && req.tp > 0.0);
     }
...
void Strategy_ManageOpenPosition()
  {
   // Card specifies bracket exits at Keltner ATR stop/take-profit levels only.
  }
