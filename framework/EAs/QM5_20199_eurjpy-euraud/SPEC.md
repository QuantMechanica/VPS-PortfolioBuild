# QM5_20199_eurjpy-euraud — Strategy Spec
...
`ln(EURJPY.DWX) - (-1.073345776) * ln(EURAUD.DWX)`. Score the newest spread
...
| `strategy_beta` | -1.073345776 | fixed | DEV-fitted structural hedge coefficient |
...
- `EURJPY.DWX` — fixed tester host and first traded leg.
- `EURAUD.DWX` — fixed companion and second traded leg.
- `USDJPY.DWX`, `AUDUSD.DWX`, and `EURUSD.DWX` — conversion-history
  dependencies for JPY/AUD P/L and EUR margin under the USD-account tester;
  all three are order-free and have no magic slot.
...
| Bar gating | `QM_IsNewBar(EURJPY.DWX, PERIOD_D1)` consumed once per signal bar |
...
| Typical hold time | multi-day; scan half-life 27.755 D1 bars |
| Expected drawdown profile | high and regime-sensitive; conservative 30% prior |
| Regime preference | EURJPY/EURAUD fixed-residual mean reversion |
| Win rate target | unknown; Q02 is the first platform-economic judge |

The fixed scan recorded DEV net Sharpe `0.843337`, OOS net Sharpe `0.646172`,
and OOS return `10.514307%`. The sub-0.8 OOS score and multi-week half-life
are adverse priors, not a promotion claim.
...
`artifacts/cards_approved/QM5_20199_eurjpy-euraud_card.md`

Chan supplies the deterministic pair-trading method but makes no performance
claim for EURJPY/EURAUD.
...
`1.0` and `1.073345776`; each leg is independently bounded by its ATR stop.
...
|---|---|---|---|
| v1 | 2026-08-01 | Initial build from approved card | Next-ranked non-duplicate FX sleeve |
