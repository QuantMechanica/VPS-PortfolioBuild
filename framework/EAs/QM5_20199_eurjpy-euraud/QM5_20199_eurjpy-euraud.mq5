...
#property description "QM5_20199 EURJPY EURAUD Cointegration"
...
//
// Card:
//   strategy-seeds/cards/approved/QM5_20199_eurjpy-euraud_card.md
//
// Fixed spread:
//   ln(EURJPY.DWX) - beta * ln(EURAUD.DWX)
//
...
input double strategy_beta              = -1.073345776;
...
bool Strategy_EnsureBasketScope()
  {
   ...
   string allowed[5] = {"EURJPY.DWX", "EURAUD.DWX", "USDJPY.DWX", "AUDUSD.DWX", "EURUSD.DWX"};
   for(int i = 0; i < 5; ++i)
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(!Strategy_EnsureBasketScope())
      return false;
   if(!QM_SymbolAssertOrLog(g_leg_eurjpy) ||
   ...
   for(int i = 0; i < history_count; ++i)
     {
      if(eurjpy_time[i] <= 0 || eurjpy_time[i] != euraud_time[i])
         return false;
      if(eurjpy[i] <= 0.0 || euraud[i] <= 0.0)
         return false;

      spreads[i] =
         MathLog(eurjpy[i]) - strategy_beta * MathLog(euraud[i]);
...
bool Strategy_OpenPair(const int spread_direction)
  {
   ...
   const double eurjpy_weight = 1.0;
   const double euraud_weight = MathAbs(strategy_beta);
   const double weight_sum = eurjpy_weight + euraud_weight;
   if(weight_sum <= 0.0)
      return false;
   ...
   const double eurjpy_lots =
      Strategy_LotsForLeg(g_leg_eurjpy, eurjpy_weight, weight_sum);
   const double euraud_lots =
      Strategy_LotsForLeg(g_leg_euraud, euraud_weight, weight_sum);
   if(eurjpy_lots <= 0.0 || euraud_lots <= 0.0)
      return false;

   const bool long_spread = (spread_direction > 0);
   const QM_OrderType eurjpy_type = long_spread ? QM_BUY : QM_SELL;
   const QM_OrderType euraud_type =
      long_spread
      ? (strategy_beta >= 0.0 ? QM_SELL : QM_BUY)
      : (strategy_beta >= 0.0 ? QM_BUY : QM_SELL);
   const string reason =
      long_spread ? "QM5_20199_LONG_SPREAD_Z_LT_NEG_ENTRY"
                  : "QM5_20199_SHORT_SPREAD_Z_GT_POS_ENTRY";

   const bool eurjpy_ok =
      Strategy_OpenLeg(g_leg_eurjpy,
                       eurjpy_type,
                       eurjpy_lots,
                       reason);
   const bool euraud_ok =
      Strategy_OpenLeg(g_leg_euraud,
                       euraud_type,
                       euraud_lots,
                       reason);
   if(eurjpy_ok && euraud_ok)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20199_PAIR_HOST";
...
void OnTick()
  {
   ...
   const bool is_new_signal_bar =
      QM_IsNewBar(g_leg_eurjpy, PERIOD_D1);
