# QM5_20200_audjpy-euraud — Strategy Spec
...
`ln(AUDJPY.DWX) - (-2.073289367) * ln(EURAUD.DWX)`. Score the newest spread
...
| `strategy_beta` | -2.073289367 | fixed | DEV-fitted structural hedge coefficient |
...
- `AUDJPY.DWX` — fixed tester host and first traded leg.
...
| Bar gating | `QM_IsNewBar(AUDJPY.DWX, PERIOD_D1)` consumed once per signal bar |
...
| Typical hold time | multi-day; scan half-life 27.702 D1 bars |
| Expected drawdown profile | high and regime-sensitive; conservative 30% prior |
| Regime preference | AUDJPY/EURAUD fixed-residual mean reversion |
| Win rate target | unknown; Q02 is the first platform-economic judge |

The fixed scan recorded DEV net Sharpe `0.808691`, OOS net Sharpe `0.640798`,
and OOS return `10.433804%`. The sub-0.8 OOS score and multi-week half-life
...
`artifacts/cards_approved/QM5_20200_audjpy-euraud_card.md`

Chan supplies the deterministic pair-trading method but makes no performance
claim for AUDJPY/EURAUD.
...
`1.0` and `2.073289367`; each leg is independently bounded by its ATR stop.
