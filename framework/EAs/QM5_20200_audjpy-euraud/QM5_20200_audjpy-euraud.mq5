...
#property description "QM5_20200 AUDJPY EURAUD Cointegration"
...
//
// Card:
//   strategy-seeds/cards/approved/QM5_20200_audjpy-euraud_card.md
//
// Fixed spread:
//   ln(AUDJPY.DWX) - beta * ln(EURAUD.DWX)
//
...
input double strategy_beta              = -2.073289367;
...
bool Strategy_EnsureBasketScope()
  {
   ...
   string allowed[5] = {"AUDJPY.DWX", "EURAUD.DWX", "USDJPY.DWX", "AUDUSD.DWX", "EURUSD.DWX"};
   for(int i = 0; i < 5; ++i)
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(!Strategy_EnsureBasketScope())
      return false;
   if(!QM_SymbolAssertOrLog(g_leg_audjpy) ||
   ...
   for(int i = 0; i < history_count; ++i)
     {
      if(audjpy_time[i] <= 0 || audjpy_time[i] != euraud_time[i])
         return false;
      if(audjpy[i] <= 0.0 || euraud[i] <= 0.0)
         return false;

      spreads[i] =
         MathLog(audjpy[i]) - strategy_beta * MathLog(euraud[i]);
...
bool Strategy_OpenPair(const int spread_direction)
  {
   ...
   const double audjpy_weight = 1.0;
   const double euraud_weight = MathAbs(strategy_beta);
   const double weight_sum = audjpy_weight + euraud_weight;
   if(weight_sum <= 0.0)
      return false;
   ...
   if(audjpy_lots <= 0.0 || euraud_lots <= 0.0)
      return false;

   const bool long_spread = (spread_direction > 0);
   const QM_OrderType audjpy_type = long_spread ? QM_BUY : QM_SELL;
   const QM_OrderType euraud_type =
      long_spread
      ? (strategy_beta >= 0.0 ? QM_SELL : QM_BUY)
      : (strategy_beta >= 0.0 ? QM_BUY : QM_SELL);
   const string reason =
      long_spread ? "QM5_20200_LONG_SPREAD_Z_LT_NEG_ENTRY"
                  : "QM5_20200_SHORT_SPREAD_Z_GT_POS_ENTRY";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20200_PAIR_HOST";
...
void OnTick()
  {
   ...
   const bool is_new_signal_bar =
      QM_IsNewBar(g_leg_audjpy, PERIOD_D1);
