...
#property description "QM5_41399 XAU/XAG Ordinary Monthly Return-Median Reversion"
...
// changes, sort all twelve individual changes, and average sorted indexes
// five and six to obtain the ordinary even median. Fade that sign with an
// opposed, equal-target-notional package held to the
// next month. Runtime is deterministic, completed-price-only, and one
// consumed attempt per month.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41399;
...
input int    strategy_median_lower_index      = 5;
input int    strategy_median_upper_index      = 6;
...
bool Strategy_InputsValid()
  {
   return (Strategy_DoubleInputsFinite() &&
            ...
            g_leg_xau != g_leg_xag &&
            qm_ea_id == 41399 && qm_magic_slot_offset == 0 &&
            strategy_xag_symbol == "XAGUSD.DWX" &&
            strategy_endpoint_count == 13 &&
            strategy_return_count == 12 &&
            strategy_median_lower_index == 5 &&
            strategy_median_upper_index == 6 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_MEDRET_RV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41399_XAUXAG_MEDRET_RV";
...
bool Strategy_OrdinaryMedian(const double &returns[],
                             double &location,
                             string &sorted_path)
  {
   location = 0.0;
   ...
   for(int index = 0; index < strategy_return_count; ++index)
     {
      if(StringLen(sorted_path) > 0)
         sorted_path += ",";
      sorted_path += DoubleToString(sorted[index], 12);
     }
   location =
      (sorted[strategy_median_lower_index] +
       sorted[strategy_median_upper_index]) /
      (double)strategy_median_divisor;
   return MathIsValidNumber(location);
  }

bool Strategy_LoadMonthlyMedianReturnSignal(
      ...
      int &direction)
  {
   ...
   if(StringLen(return_path) <= 0)
      return false;
   if(!Strategy_OrdinaryMedian(relative_returns, location, sorted_path))
      return false;

   // direction > 0 means BUY XAU / SELL XAG. Fade only the ordinary median
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41399_XAUXAG_MEDRET_RV_HOST";
   ...
   string sorted_path = "";
   const bool valid =
      Strategy_LoadMonthlyMedianReturnSignal(
         g_signal_month_key,
         month_count,
   ...
   string state = "monthly_median_return_signal_failed";
   if(valid && direction > 0)
      state = "negative_median_return_fade_long_xau";
   else if(valid && direction < 0)
      state = "positive_median_return_fade_short_xau";
   else if(valid && MathAbs(location) <= strategy_signal_epsilon)
      state = "median_return_inside_epsilon_flat";
   else if(valid)
      state = "nonqualifying_median_return_flat";

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"month_count\":%d,\"newest_endpoint_time\":%I64d,\"median_return\":%.12e,\"direction\":%d,\"ratios\":\"%s\",\"returns\":\"%s\",\"sorted_returns\":\"%s\",\"state\":\"%s\"}",
                            g_signal_month_key,
                            month_count,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG ordinary monthly return-median reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41399\",\"ea\":\"xauxag-medret-rv\"}");
