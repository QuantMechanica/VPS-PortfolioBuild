...
#property description "QM5_11824 carter-m5-s14-wma10-sma20-stoch-rsi-macd-m5 — WMA/SMA cross + Stoch/RSI/MACD five-filter, ATR SL/TP (M5)"

#include <QM/QM_Common.mqh>

// =============================================================================
// QuantMechanica V5 EA — QM5_11824 carter-m5-s14-wma10-sma20-stoch-rsi-macd-m5
// -----------------------------------------------------------------------------
// Source: Thomas Carter, "20 Forex Trading Strategies (5 Minute Time Frame)",
//         2014, Strategy 14.
// Card: artifacts/cards_approved/QM5_11824_carter-m5-s14-wma10-sma20-stoch-rsi-macd-m5.md
//       (g0_status APPROVED).
//
...
//       Stoch  : K > D  AND  K rising   (K[1] > K[2])
//       RSI    : RSI(28) > 50  AND  RSI rising   (RSI[1] > RSI[2])
//       MACD   : MACD_Main - MACD_Signal > 0  (histogram positive)
//   Confirm STATE (SHORT): K < D AND K falling, RSI(28) < 50 AND RSI falling,
//                   histogram < 0.
//   Stop          : 2 x ATR(14) via QM_StopATRFromValue (card default).
//   Take profit   : 1:1 R/R via QM_TakeRR (rr = 1.0) — same ATR-derived stop.
//   No-Friday      : optional entry block on Fridays.
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   // Optional: no new entries on Friday.
   ...
   // --- Confirming STATES at shift 1 (with rising/falling momentum check) ---
   const double stoch_k1 = QM_Stoch_K(_Symbol, _Period, strategy_stoch_k, strategy_stoch_d, strategy_stoch_slowing, 1);
   const double stoch_d1 = QM_Stoch_D(_Symbol, _Period, strategy_stoch_k, strategy_stoch_d, strategy_stoch_slowing, 1);
   const double stoch_k2 = QM_Stoch_K(_Symbol, _Period, strategy_stoch_k, strategy_stoch_d, strategy_stoch_slowing, 2);
   ...
   if(stoch_k1 <= 0.0 || stoch_d1 <= 0.0 || stoch_k2 <= 0.0 || rsi1 <= 0.0 || rsi2 <= 0.0)
      return false;
   ...
   if(crossed_up)
     {
      // LONG confirmations: Stoch fast>slow & rising, RSI>level & rising, histogram>0.
      if(!(stoch_k1 > stoch_d1))
         return false;
      if(!(stoch_k1 > stoch_k2))           // Stoch %K rising
         return false;
      if(!(rsi1 > strategy_rsi_level))
         return false;
      if(!(rsi1 > rsi2))                   // RSI rising
         return false;
      if(!(macd_hist > 0.0))
         return false;
      dir = QM_BUY;
     ...
     {
      // SHORT confirmations: Stoch fast<slow & falling, RSI<level & falling, histogram<0.
      if(!(stoch_k1 < stoch_d1))
         return false;
      if(!(stoch_k1 < stoch_k2))           // Stoch %K falling
         return false;
      if(!(rsi1 < strategy_rsi_level))
         return false;
      if(!(rsi1 < rsi2))                   // RSI falling
         return false;
      if(!(macd_hist < 0.0))
         return false;
      dir = QM_SELL;
...
// Fixed ATR SL/TP only — no active management.
void Strategy_ManageOpenPosition()
  {
  }

// No discretionary exit; ATR stop and 1:1 RR take-profit handle the close.
