...
def direction(uk100_mtd_return: float) -> str | None:
    if uk100_mtd_return > 0:
        return "SELL"
    if uk100_mtd_return < 0:
        return "BUY"
    return None
...
class Qm541144ReferenceTests(unittest.TestCase):
    ...
    def test_only_calendar_quarter_ends_are_eligible(self) -> None:
        quarter_months = {3, 6, 9, 12}
        self.assertIn(dt.date(2024, 3, 28).month, quarter_months)
        self.assertNotIn(dt.date(2024, 8, 30).month, quarter_months)
