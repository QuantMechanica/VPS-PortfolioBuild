...
#property description "QM5_20013 XNG Two-Month Unconditional Contrarian"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_20013 - XNG Two-Month Unconditional Contrarian State
// -----------------------------------------------------------------------------
// Mishra-Smyth source carrier:
//   - fixed Jan-Feb, Mar-Apr, May-Jun, Jul-Aug, Sep-Oct, Nov-Dec periods
//   - latest completed month-end close versus the close two months earlier
//   - long after a two-month fall and short after a two-month rise
...
input int    strategy_holding_months           = 2;
...
int Strategy_PeriodKeyForTime(const datetime value)
  {
   ...
   if(value <= 0 || !TimeToStruct(value, parts))
      return 0;
   return parts.year * 10 + (parts.mon - 1) / 2;
  }
...
int Strategy_CurrentPeriodKey()
  {
   ...
   if(month < 1 || month > 12)
      return 0;
   return year * 10 + (month - 1) / 2;
  }

bool Strategy_IsBimonthlyRebalanceBar()
  {
...
bool Strategy_LoadContrarianState(double &latest_close,
                                  ...
                                  int &target_state)
  {
   ...
   two_month_close = month_closes[2];
   if(latest_close <= 0.0 || two_month_close <= 0.0 ||
      !MathIsValidNumber(latest_close) ||
      !MathIsValidNumber(two_month_close))
      return false;

   if(latest_close < two_month_close)
      target_state = 1;
   else if(latest_close > two_month_close)
      target_state = -1;
   else
      target_state = 0;
...
void Strategy_CloseExpiredPositions()
  {
   const bool is_boundary = Strategy_IsBimonthlyRebalanceBar();
   ...
   for(int index = PositionsTotal() - 1; index >= 0; --index)
     {
      ...
      // every non-equality decision renews the fixed two-month package.
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20013_XNG_2M_CONTR";
   req.symbol_slot = qm_magic_slot_offset;
   req.expiration_seconds = 0;

   if(!Strategy_IsBimonthlyRebalanceBar())
      return false;
   ...
   req.reason = (target_state > 0) ?
      "XNG_2M_CONTR_LONG" : "XNG_2M_CONTR_SHORT";
   return true;
  }
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Source rule requires an uninterrupted fixed two-month holding period"))
      return INIT_FAILED;

   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_20013\",\"ea\":\"xng-2m-contr\"}");
