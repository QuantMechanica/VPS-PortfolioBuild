# QM5_41334_tv-post-vwap-opt - Strategy Spec

**EA ID:** QM5_41334
**Slug:** tv-post-vwap-opt
**Parent EA:** QM5_10815_tv-post-vwap
...
VWAP. The stop is placed beyond the absorption swing by an ATR buffer and
capped at a maximum ATR distance; the default target is session VWAP with an
optional fixed-R variant. Exits are SL, TP, the M15/H1 time stops, and the
...
Parent parameters are unchanged from QM5_10815 (see that EA's SPEC.md). The
...
**Designed for:**
- GDAXI.DWX - the DL-089 measurement target for parent QM5_10815; the census
...
- All other parent symbols - the sibling is bound to the single Q12 deferral
  `expected one approved _opt sibling for QM5_10815/GDAXI.DWX`. Other symbols
...
The parent runs on M15 and H1; the DL-089 census runs on the H1 baseline.
...
| Metric | Expected |
|---|---|
| Trades / year / symbol | ~70 at the neutral baseline (parent expectation); lower when a predicate slot is active |
...
**R1-R4 verdict (Q00):** all PASS per `D:/QM/strategy_farm/artifacts/cards_approved/QM5_10815_tv-post-vwap.md`
...
|---|---|---|---|
| v1 | 2026-09-03 | DL-089 measurement sibling built from parent QM5_10815 (wave 3 recipe 5e6f19a61a); six opt_pp_* pattern veto inputs added | Prepared for CEO-run governed compile/allocation |
