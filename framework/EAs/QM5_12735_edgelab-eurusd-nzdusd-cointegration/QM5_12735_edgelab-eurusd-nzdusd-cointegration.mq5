...
#property description "QM5_12735 Edge Lab EURUSD NZDUSD Cointegration"
...
input double strategy_beta              = 0.81;
...
bool Strategy_IsHostSymbol()
  {
   return (_Symbol == g_leg_eurusd || _Symbol == g_leg_nzdusd);
  }
...
bool Strategy_EnsureBasketScope()
  {
   ...
   // Both legs settle through USD in the tester; select both legs explicitly
   // so the host run can warm and trade the complete basket.
   string allowed[2] = {"EURUSD.DWX", "NZDUSD.DWX"};
   for(int i = 0; i < 2; ++i)
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(!Strategy_EnsureBasketScope())
      return false;
   if(!QM_SymbolAssertOrLog(g_leg_eurusd) || !QM_SymbolAssertOrLog(g_leg_nzdusd))
      return false;
   ...
   if(CopyClose(g_leg_eurusd, PERIOD_D1, 1, lookback, eurusd) != lookback) // perf-allowed: Strategy_EntrySignal is called only after the framework QM_IsNewBar gate.
      return false;
   if(CopyClose(g_leg_nzdusd, PERIOD_D1, 1, lookback, nzdusd) != lookback) // perf-allowed: Strategy_EntrySignal is called only after the framework QM_IsNewBar gate.
      return false;
   ...
   for(int i = 0; i < lookback; ++i)
     {
      if(eurusd[i] <= 0.0 || nzdusd[i] <= 0.0)
         return false;
      spreads[i] = MathLog(eurusd[i]) - strategy_beta * MathLog(nzdusd[i]);
...
bool Strategy_OpenPair(const int spread_direction)
  {
   ...
   const double weight_sum = eurusd_weight + nzdusd_weight;
   if(weight_sum <= 0.0)
      return false;

   const bool long_spread = (spread_direction > 0);
   const QM_OrderType eurusd_type = long_spread ? QM_BUY : QM_SELL;
   const QM_OrderType nzdusd_type = long_spread ? QM_SELL : QM_BUY;
   const string reason = long_spread ? "QM5_12735_LONG_SPREAD_Z_LT_NEG_ENTRY"
                                     : "QM5_12735_SHORT_SPREAD_Z_GT_POS_ENTRY";

   bool eurusd_ok = Strategy_OpenLeg(g_leg_eurusd, eurusd_type, eurusd_weight, weight_sum, reason);
   bool nzdusd_ok = Strategy_OpenLeg(g_leg_nzdusd, nzdusd_type, nzdusd_weight, weight_sum, reason);
   if(eurusd_ok && nzdusd_ok)
     {
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12735_PAIR_HOST";
