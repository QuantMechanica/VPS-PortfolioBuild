...
#property description "QM5_13085 XCU AUDUSD Return Spread Reversion"
...
// -----------------------------------------------------------------------------
// D1 two-leg base-metal/FX basket:
//   rspread = log(XCU[t] / XCU[t-L]) - beta_audusd * log(AUDUSD[t] / AUDUSD[t-L])
//   z > entry: short return spread = sell copper, buy AUDUSD
//   z < -entry: long return spread = buy copper, sell AUDUSD
...
input double strategy_beta_audusd         = 1.20;
   ...
   for(int i = 0; i < z_lookback; ++i)
     {
      ...
      if(xcu[i] <= 0.0 || audusd[i] <= 0.0 || xcu[past_idx] <= 0.0 || audusd[past_idx] <= 0.0)
         return false;

      const double xcu_ret = MathLog(xcu[i] / xcu[past_idx]);
      const double audusd_ret = MathLog(audusd[i] / audusd[past_idx]);
      spreads[i] = xcu_ret - strategy_beta_audusd * audusd_ret;
...
bool Strategy_OpenPair(const int spread_direction)
  {
   if(spread_direction == 0 || Strategy_OpenPairLegCount() > 0)
      return false;
   if(!Strategy_SpreadAllowed(g_leg_xcu) || !Strategy_SpreadAllowed(g_leg_audusd))
      return false;

   const double xcu_weight = 1.0;
   const double audusd_weight = MathMax(0.1, MathAbs(strategy_beta_audusd));
   const double weight_sum = xcu_weight + audusd_weight;
   ...
   const string reason = long_spread ? "QM5_13085_LONG_XCU_AUDUSD_RSPREAD"
                                     : "QM5_13085_SHORT_XCU_AUDUSD_RSPREAD";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_13085_RSPREAD_HOST";
...
int OnInit()
  {
   ...
   string basket_symbols[2] = {g_leg_xcu, g_leg_audusd};
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_13085\",\"ea\":\"xcu-audusd-rspr\"}");
