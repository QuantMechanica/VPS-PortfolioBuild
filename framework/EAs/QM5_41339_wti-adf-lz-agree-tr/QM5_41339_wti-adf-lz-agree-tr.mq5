...
#property description "QM5_41339 WTI Monthly ADF and LZ76 Agreement Trend"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41339 - WTI Monthly ADF and LZ76 Agreement-Gated Trend
// -----------------------------------------------------------------------------
...
//   - require ADF t >= -2.594 and exact LZ76 sign complexity <= 6
...
input int    strategy_lz_return_count       = 20;
...
// Constants retained only for the inherited, unreachable von-Neumann
// reference helper below; live signal admission uses LZ76 exclusively.
const int    strategy_vn_return_count = 20;
const double strategy_vn_eta_max = 2.0;
...
struct Strategy_SignalMetrics
  {
   ...
   // Dead-helper scratch fields; never read by the strategy signal.
   int vn_return_count;
   double vn_return_mean;
   double vn_variance_sum;
   double vn_successive_sum;
   double vn_eta;
   double momentum_12;
   string level_path;
...
bool Strategy_VonNeumannCore(const double &values[],
                              double &return_mean,
                              double &variance_sum,
                              double &successive_sum,
                              double &eta,
                              string &vn_path)
  {
   return_mean = 0.0;
   ...
   if(value_count != 20 || strategy_vn_return_count != 20 ||
      MathAbs(strategy_vn_eta_max - 2.0) > 0.000000000000001 ||
      strategy_energy_floor != 0.000000000000000001)
      return false;
   ...
   for(int index = 0; index < value_count; ++index)
     {
      ...
      if(!MathIsValidNumber(centered) || !MathIsValidNumber(square) ||
         square < 0.0)
         return false;
      variance_sum += square;
      if(StringLen(vn_path) > 0)
         vn_path += ",";
      vn_path += DoubleToString(values[index], 12);
     }
   for(int index = 0; index < value_count - 1; ++index)
     {
      const double difference = values[index + 1] - values[index];
      const double square = difference * difference;
      if(!MathIsValidNumber(difference) || !MathIsValidNumber(square) ||
         square < 0.0)
         return false;
      successive_sum += square;
     }
   if(!MathIsValidNumber(variance_sum) ||
      variance_sum <= strategy_energy_floor ||
      !MathIsValidNumber(successive_sum) || successive_sum < 0.0)
      return false;

   eta = successive_sum / variance_sum;
   return (MathIsValidNumber(eta) && eta >= 0.0 &&
           StringLen(vn_path) > 0);
  }
...
bool Strategy_SubstringOccursBeforeTerminal(const string word,
  ...
  {
   const int word_length = StringLen(word);
   if(!Strategy_WordIsBinary(word) || phrase_start < 0 ||
      phrase_start >= word_length || phrase_length <= 0 ||
      phrase_start + phrase_length > word_length || phrase_start == 0)
      return false;
   const string phrase = StringSubstr(word, phrase_start, phrase_length);
   if(StringLen(phrase) != phrase_length)
      return false;
   for(int candidate_start = 0; candidate_start < phrase_start;
       ++candidate_start)
     {
...
bool Strategy_LZ76ParseWord(const string word,
                            ...
                            string &phrase_path)
  {
   ...
   while(phrase_start < word_length)
     {
      ...
      for(int phrase_length = 1; phrase_length <= remaining;
          ++phrase_length)
        {
   ...
   return (phrase_start == word_length && reconstruction == word &&
           complexity >= 2 && complexity <= 9 &&
...
bool Strategy_ADFLZReferenceSelfTest()
  {
   int complexity = 0;
   int exhaustive = 0;
   bool final_non_exhaustive = false;
   string phrases = "";
   if(!Strategy_LZ76ParseWord("0011011101110110", complexity, exhaustive,
                              final_non_exhaustive, phrases) ||
      complexity != 5 || phrases != "0|01|10|111|01110110")
      return false;
   if(!Strategy_LZ76ParseWord("00000001101110100100", complexity, exhaustive,
                              final_non_exhaustive, phrases) ||
      complexity != 6)
      return false;
   if(!Strategy_LZ76ParseWord("00000001101110101000", complexity, exhaustive,
                              final_non_exhaustive, phrases) ||
      complexity != 7)
      return false;
   return true;
...
bool Strategy_AgreementSignal(const double &closes[],
                              Strategy_SignalMetrics &metrics)
  {
   ...
   metrics.lz_return_count = strategy_lz_return_count;
   metrics.direction = 0;
   metrics.persistence_qualified = false;
   metrics.adf_qualified = false;
   metrics.lz_qualified = false;
   ...
   if(strategy_level_count != 60 ||
      ...
      MathAbs(strategy_adf_t_min - (-2.594)) >
         0.000000000000001 ||
      strategy_lz_return_count != 20 ||
      strategy_complexity_ceiling != 6 ||
      MathAbs(strategy_sign_epsilon - 0.000000000001) >
   ...
   double returns[20];
   for(int index = 0; index < strategy_lz_return_count; ++index)
     {
      const int right_index = index + 40;
      const int left_index = index + 39;
      if(index < 0 || index >= 20 || left_index < 0 ||
         right_index >= strategy_level_count)
         return false;
      returns[index] = levels[right_index] - levels[left_index];
      if(!MathIsValidNumber(returns[index]))
         return false;
      if(MathAbs(returns[index]) <= strategy_sign_epsilon)
         return false;
      metrics.sign_word +=
         (returns[index] > strategy_sign_epsilon) ? "1" : "0";
      if(StringLen(metrics.return_path) > 0)
         metrics.return_path += ",";
      metrics.return_path += DoubleToString(returns[index], 12);
     }
   if(StringLen(metrics.return_path) <= 0 ||
      StringLen(metrics.sign_word) != strategy_lz_return_count)
      return false;
...
bool Strategy_AgreementReferenceSelfTest()
  {
   double smooth_returns[20];
   double constant_returns[20];
   for(int index = 0; index < 20; ++index)
     {
      smooth_returns[index] = 0.001 * (double)index;
      constant_returns[index] = 0.01;
     }
   double return_mean = 0.0;
   double variance_sum = 0.0;
   double successive_sum = 0.0;
   double eta = 0.0;
   string vn_path = "";
   if(!Strategy_VonNeumannCore(
         smooth_returns, return_mean, variance_sum, successive_sum,
         eta, vn_path) ||
      MathAbs(return_mean - 0.0095) > 0.000000000001 ||
      MathAbs(eta - 0.02857142857142857) > 0.000000001)
      return false;
   if(Strategy_VonNeumannCore(
         constant_returns, return_mean, variance_sum, successive_sum,
         eta, vn_path))
      return false;

   double up_closes[60];
   double down_closes[60];
   double vn_only_closes[60];
   ...
   for(int index = 0; index < 60; ++index)
     {
      ...
      vn_only_closes[index] = MathExp(vn_only_level);
   ...
   if(!Strategy_AgreementSignal(adf_only_closes, metrics) ||
      ...
      MathAbs(metrics.adf_t - 0.4245704987551788) >
         0.000000001 ||
      MathAbs(metrics.vn_eta - 2.0442009296536146) >
         0.000000001 ||
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   else
     {
      ...
      if(!g_signal_valid)
         g_signal_state = "agreement_validation_failed";
              ...
              !g_signal_metrics.lz_qualified)
         g_signal_state = "both_gates_reject_consumed_flat";
      else if(!g_signal_metrics.adf_qualified)
         g_signal_state = "adf_rejects_lz_only_consumed_flat";
      else if(!g_signal_metrics.lz_qualified)
         g_signal_state = "lz_rejects_adf_only_consumed_flat";
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"endpoint_count\":%d,\"level_count\":%d,\"lz_return_count\":%d,\"mean_y\":%.16f,\"mean_z\":%.16f,\"mean_w\":%.16f,\"szz\":%.16f,\"sww\":%.16f,\"szw\":%.16f,\"szy\":%.16f,\"swy\":%.16f,\"determinant\":%.16f,\"alpha\":%.16f,\"gamma\":%.16f,\"phi\":%.16f,\"sse\":%.16f,\"residual_variance\":%.16f,\"se_gamma\":%.16f,\"adf_t\":%.12f,\"adf_qualified\":%s,\"lz_complexity\":%d,\"exhaustive_components\":%d,\"final_component_non_exhaustive\":%s,\"lz_qualified\":%s,\"persistence_qualified\":%s,\"momentum_12\":%.12f,\"levels\":\"%s\",\"returns\":\"%s\",\"regression\":\"%s\",\"sign_word\":\"%s\",\"phrases\":\"%s\",\"oldest_close\":%.10f,\"newest_close\":%.10f,\"oldest_endpoint\":%I64d,\"newest_endpoint\":%I64d,\"state\":\"%s\"}",
                            ...
                            g_signal_metrics.lz_return_count,
                            ...
                            g_signal_metrics.lz_qualified ? "true" : "false",
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41339 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41339_WTI_ADF_LZ76";
   ...
   req.reason = (g_signal_metrics.direction > 0)
      ? "WTI_ADF_LZ76_MOM_LONG"
      : "WTI_ADF_LZ76_MOM_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41339 ||
      qm_magic_slot_offset != 0)
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI monthly ADF and LZ76 agreement trend holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41339_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   g_entry_month_state_key =
      StringFormat("QM5_41339_ENTRY_MONTH_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41339\",\"ea\":\"wti-adf-lz-agree-tr\"}");
