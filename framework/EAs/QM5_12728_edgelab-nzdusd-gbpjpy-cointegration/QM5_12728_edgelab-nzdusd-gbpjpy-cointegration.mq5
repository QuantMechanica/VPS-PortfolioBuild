...
#property description "QM5_12728 Edge Lab NZDUSD GBPJPY Cointegration"
...
input double strategy_beta              = 0.04;
...
bool Strategy_IsHostSymbol()
  {
   return (_Symbol == g_leg_nzdusd || _Symbol == g_leg_gbpjpy);
  }
...
bool Strategy_EnsureBasketScope()
  {
   ...
   // GBPJPY needs GBP and JPY conversion history for USD-denominated tester
   // accounting; NZDUSD itself already settles in USD.
   string allowed[4] = {"NZDUSD.DWX", "GBPJPY.DWX", "GBPUSD.DWX", "USDJPY.DWX"};
   for(int i = 0; i < 4; ++i)
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(!Strategy_EnsureBasketScope())
      return false;
   if(!QM_SymbolAssertOrLog(g_leg_nzdusd) || !QM_SymbolAssertOrLog(g_leg_gbpjpy))
      return false;
   ...
   for(int i = 0; i < lookback; ++i)
     {
      if(nzdusd[i] <= 0.0 || gbpjpy[i] <= 0.0)
         return false;
      spreads[i] = MathLog(nzdusd[i]) - strategy_beta * MathLog(gbpjpy[i]);
...
bool Strategy_OpenPair(const int spread_direction)
  {
   ...
   const double weight_sum = nzdusd_weight + gbpjpy_weight;
   if(weight_sum <= 0.0)
      return false;
   ...
   const string reason = long_spread ? "QM5_12728_LONG_SPREAD_Z_LT_NEG_ENTRY"
                                     : "QM5_12728_SHORT_SPREAD_Z_GT_POS_ENTRY";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12728_PAIR_HOST";
