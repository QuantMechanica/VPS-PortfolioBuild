...
#property description "QM5_12770 Edge Lab EURUSD EURGBP Cointegration"
...
input double strategy_beta              = 0.601215;
...
bool Strategy_IsHostSymbol()
  {
   return (_Symbol == g_leg_eurusd || _Symbol == g_leg_eurgbp);
  }
...
bool Strategy_EnsureBasketScope()
  {
   ...
   // EURGBP settles in GBP, so MT5 also needs GBPUSD conversion history to
   // value profit and margin in the USD-denominated tester account.
   string allowed[3] = {"EURUSD.DWX", "EURGBP.DWX", "GBPUSD.DWX"};
   for(int i = 0; i < 3; ++i)
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(!Strategy_EnsureBasketScope())
      return false;
   if(!QM_SymbolAssertOrLog(g_leg_eurusd) || !QM_SymbolAssertOrLog(g_leg_eurgbp))
      return false;
   ...
   for(int i = 0; i < lookback; ++i)
     {
      if(eurusd[i] <= 0.0 || eurgbp[i] <= 0.0)
         return false;
      spreads[i] = MathLog(eurusd[i]) - strategy_beta * MathLog(eurgbp[i]);
...
bool Strategy_OpenPair(const int spread_direction)
  {
   ...
   const double weight_sum = eurusd_weight + eurgbp_weight;
   if(weight_sum <= 0.0)
      return false;
   ...
   const string reason = long_spread ? "QM5_12770_LONG_SPREAD_Z_LT_NEG_ENTRY"
                                     : "QM5_12770_SHORT_SPREAD_Z_GT_POS_ENTRY";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12770_PAIR_HOST";
