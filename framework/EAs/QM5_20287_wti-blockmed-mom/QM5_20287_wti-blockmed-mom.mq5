...
#property description "QM5_20287 WTI Chronological Block Median Trend"
...
//   3. partition them into four chronological non-overlapping blocks of three;
//   4. compute the arithmetic mean of each block;
//   5. trade the sign of the even median of the four block means.
// The robust block statistic is a transparent QM mechanization.
// Runtime uses native XTIUSD.DWX price and broker data only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 20287;
...
double g_cache_block0           = 0.0;
double g_cache_block1           = 0.0;
double g_cache_block2           = 0.0;
double g_cache_block3           = 0.0;
double g_cache_block_median     = 0.0;
...
void Strategy_ResetCachedState()
  {
   ...
   g_cache_block2 = 0.0;
   g_cache_block3 = 0.0;
   g_cache_block_median = 0.0;
   g_cache_state_reason = "not_evaluated";
  }

bool Strategy_BlockMedianSignal(const double &closes[], int &signal)
  {
   signal = 0;
   ...
   if(return_count != 12 ||
      ...
      strategy_block_months * strategy_block_count != return_count ||
      ArraySize(closes) != return_count + 1)
     {
      g_cache_state_reason = "bad_block_contract";
      return false;
     }
   ...
   double block_means[];
   double sorted_block_means[];
   ArrayResize(block_means, strategy_block_count);
   ArrayResize(sorted_block_means, strategy_block_count);
   for(int block = 0; block < strategy_block_count; ++block)
     {
      double block_sum = 0.0;
      const int block_start = block * strategy_block_months;
      for(int offset = 0; offset < strategy_block_months; ++offset)
         block_sum += monthly_returns[block_start + offset];
      const double block_mean = block_sum / (double)strategy_block_months;
      if(!MathIsValidNumber(block_sum) || !MathIsValidNumber(block_mean))
        {
         g_cache_state_reason = "invalid_block_mean";
         return false;
        }
      block_means[block] = block_mean;
      sorted_block_means[block] = block_mean;
     }

   ArraySort(sorted_block_means);
   for(int i = 0; i < strategy_block_count; ++i)
     {
      if(!MathIsValidNumber(sorted_block_means[i]) ||
         (i > 0 && sorted_block_means[i] < sorted_block_means[i - 1]))
        {
         g_cache_state_reason = "invalid_sorted_block_state";
         return false;
        }
     }

   const double location =
      (sorted_block_means[1] + sorted_block_means[2]) / 2.0;
   if(!MathIsValidNumber(location))
     {
      g_cache_state_reason = "invalid_block_median";
      return false;
     }

   g_cache_block0 = block_means[0];
   g_cache_block1 = block_means[1];
   g_cache_block2 = block_means[2];
   g_cache_block3 = block_means[3];
   g_cache_block_median = location;
   if(location == 0.0)
     {
      g_cache_state_reason = "exact_zero_block_median";
      return true;
     }
...
bool Strategy_LoadSignalState(int &signal)
  {
   ...
   return Strategy_BlockMedianSignal(monthly_closes, signal);
  }
...
void Strategy_PrepareMonthlySignal()
  {
   ...
   QM_LogEvent(QM_INFO,
               "MONTHLY_STATE",
               StringFormat("{\"month\":%d,\"valid\":%s,\"signal\":%d,\"block0\":%.10f,\"block1\":%.10f,\"block2\":%.10f,\"block3\":%.10f,\"block_median\":%.10f,\"state\":\"%s\"}",
                            g_cache_month_key,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 20287 ||
   ...
   if(strategy_return_months != 12 ||
      strategy_block_months != 3 ||
      strategy_block_count != 4 ||
      strategy_block_months * strategy_block_count != strategy_return_months)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20287_WTI_BLOCKMED_MOM";
   ...
   req.reason = (g_cache_signal > 0) ? "BLOCKMED_MOM_XTI_LONG" : "BLOCKMED_MOM_XTI_SHORT";
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_20287_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_20287\",\"ea\":\"wti-blockmed-mom\"}");
