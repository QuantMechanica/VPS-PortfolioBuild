...
#property description "QM5_20293 WTI Nine-Month Time-Series Momentum"
...
//   1. derive ten consecutive completed WTI month-end closes from D1 data;
//   2. calculate the exact nine-completed-month return ln(C[9] / C[0]);
//   3. buy a positive return or sell a negative return for the new month.
// The exact nine-month horizon, CFD mapping, ATR stop, and fixed-dollar sizing
// are transparent QM mechanizations. Runtime uses native XTIUSD.DWX data only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 20293;
...
bool Strategy_LoadMonthlyCloses(double &closes[])
  {
   ...
   if(strategy_return_months != 9 || strategy_history_bars_d1 != 500)
      return false;
   ...
   if(required_closes != 10 || month_count < required_closes)
      return false;
...
bool Strategy_NineMonthReturnSignal(const double &closes[], int &signal)
  {
   signal = 0;
   if(strategy_return_months != 9 || ArraySize(closes) != 10)
     {
...
bool Strategy_LoadSignalState(int &signal)
  {
   ...
   return Strategy_NineMonthReturnSignal(monthly_closes, signal);
  }
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 20293 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20293_WTI_TSMOM9M";
   ...
   req.reason = (g_cache_signal > 0) ? "TSMOM9M_XTI_LONG" : "TSMOM9M_XTI_SHORT";
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_20293_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_20293\",\"ea\":\"wti-tsmom9m\"}");
