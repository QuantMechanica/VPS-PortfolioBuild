...
#property description "QM5_20272 WTI Quarterly-Block Consensus Trend"
...
//   2. form four disjoint three-month log returns at indexes 0,3,6,9,12;
//   3. buy/sell only when at least three of four return signs agree;
//   4. consume ties, neutral blocks, and invalid states flat for that month.
// The quarterly-block vote is a transparent QM mechanization, not an author
// rule. Runtime uses native XTIUSD.DWX price and broker data only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 20272;
...
input int    strategy_block_months        = 3;
input int    strategy_consensus_required  = 3;
...
double g_cache_block_return_0   = 0.0;
double g_cache_block_return_1   = 0.0;
double g_cache_block_return_2   = 0.0;
double g_cache_block_return_3   = 0.0;
int    g_cache_positive_count   = 0;
int    g_cache_negative_count   = 0;
...
void Strategy_ResetCachedState()
  {
   ...
   g_cache_signal = 0;
   g_cache_block_return_0 = 0.0;
   g_cache_block_return_1 = 0.0;
   g_cache_block_return_2 = 0.0;
   g_cache_block_return_3 = 0.0;
   g_cache_positive_count = 0;
   g_cache_negative_count = 0;
   g_cache_state_reason = "not_evaluated";
  }

bool Strategy_QuarterBlockSignal(const double &closes[], int &signal)
  {
   signal = 0;
   const int point_count = strategy_endpoint_count;
   const int block_width = strategy_block_months;
   const int interval_count = point_count - 1;
   if(point_count != 13 ||
      block_width != 3 ||
      strategy_consensus_required != 3 ||
      interval_count != 12 ||
      interval_count % block_width != 0 ||
      ArraySize(closes) != point_count)
     {
      g_cache_state_reason = "bad_quarter_block_contract";
      return false;
     }

   const int block_count = interval_count / block_width;
   if(block_count != 4 || strategy_consensus_required > block_count)
     {
      g_cache_state_reason = "bad_block_count";
      return false;
     }

   double block_returns[4];
   int positive_count = 0;
   int negative_count = 0;
   for(int block = 0; block < block_count; ++block)
     {
      const int start_index = block * block_width;
      const int end_index = start_index + block_width;
      if(start_index < 0 || end_index >= point_count || end_index - start_index != 3)
        {
         g_cache_state_reason = "invalid_block_boundary";
         return false;
        }
      const double start_close = closes[start_index];
      const double end_close = closes[end_index];
      if(start_close <= 0.0 || end_close <= 0.0 ||
         !MathIsValidNumber(start_close) || !MathIsValidNumber(end_close))
        {
         g_cache_state_reason = "bad_block_close";
         return false;
        }
      const double ratio = end_close / start_close;
      if(ratio <= 0.0 || !MathIsValidNumber(ratio))
        {
         g_cache_state_reason = "bad_block_ratio";
         return false;
        }
      const double block_return = MathLog(ratio);
      if(!MathIsValidNumber(block_return))
        {
         g_cache_state_reason = "invalid_block_log_return";
         return false;
        }
      block_returns[block] = block_return;
      if(block_return > 0.0)
         ++positive_count;
      else if(block_return < 0.0)
         ++negative_count;
     }

   if(positive_count < 0 || negative_count < 0 ||
      positive_count + negative_count > block_count ||
      (positive_count >= strategy_consensus_required &&
       negative_count >= strategy_consensus_required))
     {
      g_cache_state_reason = "invalid_vote_state";
      return false;
     }

   g_cache_block_return_0 = block_returns[0];
   g_cache_block_return_1 = block_returns[1];
   g_cache_block_return_2 = block_returns[2];
   g_cache_block_return_3 = block_returns[3];
   g_cache_positive_count = positive_count;
   g_cache_negative_count = negative_count;

   if(positive_count >= strategy_consensus_required)
     {
      signal = 1;
      g_cache_state_reason = "qualified_long";
     }
   else if(negative_count >= strategy_consensus_required)
     {
      signal = -1;
      g_cache_state_reason = "qualified_short";
     }
   else
      g_cache_state_reason = "no_three_of_four_consensus";
   return true;
  }
...
bool Strategy_LoadSignalState(int &signal)
  {
   ...
   return Strategy_QuarterBlockSignal(monthly_closes, signal);
  }
...
void Strategy_PrepareMonthlySignal()
  {
   ...
   QM_LogEvent(QM_INFO,
               "MONTHLY_STATE",
               StringFormat("{\"month\":%d,\"valid\":%s,\"signal\":%d,\"q0\":%.10f,\"q1\":%.10f,\"q2\":%.10f,\"q3\":%.10f,\"positive\":%d,\"negative\":%d,\"state\":\"%s\"}",
                            g_cache_month_key,
                            g_cache_signal_valid ? "true" : "false",
                            g_cache_signal,
                            g_cache_block_return_0,
                            g_cache_block_return_1,
                            g_cache_block_return_2,
                            g_cache_block_return_3,
                            g_cache_positive_count,
                            g_cache_negative_count,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 20272 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20272_WTI_QUARTER_BLOCK_CONSENSUS";
   ...
   req.reason = (g_cache_signal > 0) ? "QTR_BLOCK_CONSENSUS_XTI_LONG" : "QTR_BLOCK_CONSENSUS_XTI_SHORT";
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_20272_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_20272\",\"ea\":\"wti-qtrvote-tr\"}");
