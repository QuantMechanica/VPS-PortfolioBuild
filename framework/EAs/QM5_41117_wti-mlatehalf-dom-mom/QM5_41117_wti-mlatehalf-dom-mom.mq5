...
#property description "QM5_41117 WTI Completed-Month Late-Half Dominance Momentum"
...
//   - split every adjacent return into two exhaustive chronological halves
//   - require the late half's absolute return to strictly exceed the early half
//   - consume the month before fallible gates; never enter late or retry
//   - follow the late half's sign for one month
// Runtime uses MT5-native completed closes, calendar, history, and framework state only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                      = 41117;
...
double   g_midpoint_close           = 0.0;
double   g_newest_final_close       = 0.0;
double   g_first_half_return        = 0.0;
double   g_second_half_return       = 0.0;
...
void Strategy_ResetDecisionState()
  {
   ...
   g_midpoint_close = 0.0;
   g_newest_final_close = 0.0;
   g_first_half_return = 0.0;
   g_second_half_return = 0.0;
   g_signal_state = "idle";
  }
...
bool Strategy_LoadLateHalfDominanceSignal(const int current_month_key,
                                          const int label_offset,
                                          int &direction,
                                          int &new_month_bars,
                                          ...
                                          double &second_half_return)
  {
   ...
   if(split_index < 1 || split_index >= new_month_bars)
      return false;

   // newest_closes[] is stored newest-first. Chronological C[k-1] is
   // therefore newest_closes[n-k], while C[n-1] is newest_closes[0].
   midpoint_close = newest_closes[new_month_bars - split_index];
   newest_final_close = newest_closes[0];
   if(midpoint_close <= 0.0 || newest_final_close <= 0.0 ||
      !MathIsValidNumber(midpoint_close) ||
   ...
   if(!MathIsValidNumber(first_half_return) ||
      !MathIsValidNumber(second_half_return))
      return false;

   if(MathAbs(second_half_return) > MathAbs(first_half_return))
     {
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   // Consume before close validation, split calculation, news, spread, quote,
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadLateHalfDominanceSignal(g_decision_month_key,
                                               ...
                                               g_first_half_return,
                                               g_second_half_return);
      if(!g_signal_valid)
         g_signal_state = "monthly_close_path_validation_failed";
      else if(g_signal_direction > 0)
         g_signal_state = "late_half_dominance_long";
      else if(g_signal_direction < 0)
         g_signal_state = "late_half_dominance_short";
      else if(g_second_half_return == 0.0)
         g_signal_state = "late_half_zero_flat";
      else if(MathAbs(g_second_half_return) ==
              MathAbs(g_first_half_return))
         g_signal_state = "half_magnitude_equality_flat";
      else
         g_signal_state = "late_half_not_dominant_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"new_month_bars\":%d,\"parent_month_bars\":%d,\"split_index\":%d,\"parent_final_close\":%.8f,\"midpoint_close\":%.8f,\"newest_final_close\":%.8f,\"first_half_return\":%.12e,\"second_half_return\":%.12e,\"state\":\"%s\"}",
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41117 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41117_WTI_MLATEHALF_DOM_MOM";
   ...
   req.reason = (g_signal_direction > 0)
      ? "WTI_MLATEHALF_DOM_MOM_LONG"
      : "WTI_MLATEHALF_DOM_MOM_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41117 ||
   ...
   g_attempt_state_key =
      StringFormat("QM5_41117_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
                "{\"card\":\"QM5_41117\",\"ea\":\"wti-mlatehalf-dom-mom\"}");
