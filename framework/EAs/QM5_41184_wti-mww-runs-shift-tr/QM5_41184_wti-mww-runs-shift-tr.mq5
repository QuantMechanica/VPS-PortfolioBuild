...
#property description "QM5_41184 WTI Monthly Two-Sample Label-Runs Shift Trend"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41184 - WTI Monthly Two-Sample Label-Runs Shift Trend
// -----------------------------------------------------------------------------
// D1 structural crude-oil sleeve:
//   - reconstruct ten consecutive completed broker-month-end closes
//   - fix the older five and newer five blocks before sorting
//   - count old/new membership runs in strict pooled price order
//   - continue the newer block median only when label_runs <= 6
...
input int    strategy_endpoint_count        = 10;
input int    strategy_block_size            = 5;
input int    strategy_max_label_runs        = 6;
...
bool Strategy_LoadMonthlyEndpoints(const int current_month_key,
                                   ...
                                   datetime &endpoint_times[])
  {
   ...
   ArraySetAsSeries(bars, true);
   const int copied =
      CopyRates(_Symbol, // perf-allowed: one bounded ten-month scan behind a consumed monthly attempt or monthly restart validation.
                PERIOD_D1,
                1,
                strategy_history_bars_d1,
                bars);
      ...
      if(endpoint_count <= 0 || month_key != last_month_key)
        {
         ...
         if(endpoint_count < 0 ||
            ...
            endpoint_count >= ArraySize(reverse_month_keys))
            return false;
   ...
   if(endpoint_count != strategy_endpoint_count || endpoint_count != 10)
      return false;
   ...
   for(int index = 0; index < endpoint_count; ++index)
     {
      ...
      if(reverse_index < 0 ||
         ...
         reverse_index >= ArraySize(reverse_month_keys))
         return false;
...
bool Strategy_SortAscending(double &values[])
  {
   ...
   for(int left = 0; left < count - 1; ++left)
     {
      ...
      for(int right = left + 1; right < count; ++right)
        {
         if(values[right] < values[selected])
            selected = right;
        }
      if(selected != left)
        {
         const double swap_value = values[left];
         values[left] = values[selected];
         values[selected] = swap_value;
        }
     }
...
bool Strategy_TwoSampleRunsSignal(const double &closes[],
                                  Strategy_SignalMetrics &metrics)
  {
   ...
   double older[];
   double newer[];
   if(ArrayResize(older, strategy_block_size) != strategy_block_size ||
      ArrayResize(newer, strategy_block_size) != strategy_block_size)
      return false;
   ...
   for(int index = 0; index < strategy_endpoint_count; ++index)
     {
      ...
      if(index < strategy_block_size)
        ...
        {
         const int newer_index = index - strategy_block_size;
         if(newer_index < 0 || newer_index >= ArraySize(newer))
            return false;
         newer[newer_index] = closes[index];
   ...
   if(!Strategy_SortAscending(older) ||
      !Strategy_SortAscending(newer))
      return false;
   metrics.old_median = older[2];
   metrics.new_median = newer[2];
   if(metrics.old_median <= 0.0 ||
      metrics.new_median <= 0.0 ||
      !MathIsValidNumber(metrics.old_median) ||
      !MathIsValidNumber(metrics.new_median) ||
      metrics.old_median == metrics.new_median)
      return false;

   bool used[10];
   ArrayInitialize(used, false);
   metrics.label_runs = 0;
   metrics.old_count = 0;
   metrics.new_count = 0;
   bool have_previous = false;
   bool previous_old = false;

   for(int rank_index = 0;
       rank_index < strategy_endpoint_count;
       ++rank_index)
     {
      int selected = -1;
      double selected_value = DBL_MAX;
      for(int candidate = 0;
          candidate < strategy_endpoint_count;
          ++candidate)
        {
         if(used[candidate])
            continue;
         if(selected < 0 || closes[candidate] < selected_value)
           {
            selected = candidate;
            selected_value = closes[candidate];
           }
        }
      if(selected < 0 || selected >= strategy_endpoint_count ||
         selected_value <= 0.0 || !MathIsValidNumber(selected_value))
         return false;
      used[selected] = true;

      const bool member_old = (selected < strategy_block_size);
      if(member_old)
         ++metrics.old_count;
      else
         ++metrics.new_count;
      if(!have_previous || member_old != previous_old)
         ++metrics.label_runs;
      previous_old = member_old;
      have_previous = true;

      if(StringLen(metrics.sorted_membership_path) > 0)
         metrics.sorted_membership_path += ",";
      if(member_old)
         metrics.sorted_membership_path += "O";
      else
         metrics.sorted_membership_path += "N";
     }

   if(metrics.old_count != strategy_block_size ||
      metrics.new_count != strategy_block_size ||
      metrics.label_runs < 2 ||
      metrics.label_runs > strategy_endpoint_count ||
   ...
   metrics.direction = 0;
   if(metrics.label_runs <= strategy_max_label_runs)
     {
      if(metrics.new_median > metrics.old_median)
         metrics.direction = 1;
      else if(metrics.new_median < metrics.old_median)
         metrics.direction = -1;
     }
   return true;
  }
                                        ...
                                        int &direction)
  {
   ...
   const bool valid =
      Strategy_LoadMonthlyEndpoints(current_month_key,
                                    ...
                                    closes,
                                    endpoint_times) &&
      Strategy_TwoSampleRunsSignal(closes, metrics);
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
   ...
   else
     {
      ...
      g_signal_valid =
         endpoints_valid &&
         Strategy_TwoSampleRunsSignal(closes, g_signal_metrics);
      if(!g_signal_valid)
         g_signal_state = "two_sample_runs_validation_failed";
      else if(g_signal_metrics.direction == 0)
         g_signal_state = "too_many_label_runs_flat";
      else if(g_signal_metrics.direction > 0)
         g_signal_state = "clustered_newer_median_higher_wti_long";
      else if(g_signal_metrics.direction < 0)
         g_signal_state = "clustered_newer_median_lower_wti_short";
      else
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"endpoint_count\":%d,\"block_size\":%d,\"label_runs\":%d,\"old_count\":%d,\"new_count\":%d,\"sorted_membership\":\"%s\",\"older_closes\":\"%s\",\"newer_closes\":\"%s\",\"old_median\":%.10f,\"new_median\":%.10f,\"oldest_close\":%.10f,\"newest_close\":%.10f,\"oldest_endpoint\":%I64d,\"newest_endpoint\":%I64d,\"state\":\"%s\"}",
                            ...
                            g_signal_metrics.label_runs,
                            g_signal_metrics.old_count,
                            g_signal_metrics.new_count,
                            g_signal_metrics.sorted_membership_path,
                            g_signal_metrics.older_path,
                            g_signal_metrics.newer_path,
                            g_signal_metrics.old_median,
                            g_signal_metrics.new_median,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41184 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41184_WTI_TWO_SAMPLE_RUNS_SHIFT_TREND";
   ...
   req.reason = (g_signal_metrics.direction > 0)
      ? "WTI_TWO_SAMPLE_RUNS_NEWER_MEDIAN_HIGHER_LONG"
      : "WTI_TWO_SAMPLE_RUNS_NEWER_MEDIAN_LOWER_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41184 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI two-sample label-runs shift trend holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41184_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41184\",\"ea\":\"wti-mww-runs-shift-tr\"}");
