...
indicators:
  - "[[indicators/atr]]"
strategy_type_flags: [calendar-seasonality, month-of-year, atr-hard-stop, time-stop, short-only]
target_symbols: [XTIUSD.DWX]
single_symbol_only: true
period: D1
expected_trade_frequency: "October-only D1 WTI month-of-year fade sleeve; estimate 18-23 entries/year after weekends, broker holidays, and framework filters."
...
g0_approval_reasoning: "R1 PASS academic WTI calendar-effects source; R2 PASS deterministic October D1 short/time-flat rule with ATR stop; R3 PASS XTIUSD.DWX available; R4 PASS no ML/grid/martingale/external data."
---

# WTI October Calendar Fade

## Source

- Source: [[sources/GORSKA-WTI-CAL-2015]]
- Primary citation: Gorska, A. and Krawiec, M., "Calendar Effects in the
  Market of Crude Oil", Quantitative Methods in Economics, 16(4), 2015, URL
...
crude-oil returns, including a late-year weakness cluster with October the
weakest average-return month in the studied WTI sample. This card isolates that
month-of-year effect as a low-frequency XTIUSD.DWX sleeve: take short-only D1
exposure during October and flatten each entry after one D1 bar unless the ATR
hard stop or framework Friday close acts first.

This is deliberately different from:

- `QM5_12596_wti-mon-fade` and `QM5_12610_wti-tue-fade`: this card is a
  month-of-year October fade, not a weekday short rule.
- `QM5_12597_wti-fri-prem`: opposite direction and weekday thesis.
- `QM5_12599_wti-feb-prem`: opposite month-of-year side and direction.
- `QM5_12576_eia-wti-season`: not broad EIA refined-product demand
  seasonality, not SMA/ROC-confirmed monthly hold logic.
...
- Current broker-calendar D1 bar must be in October.
...
- Close immediately if the current D1 bar is no longer in October.
...
WTI returns, including October as the weakest average-return month in the
studied sample. No source performance number is imported into QM; the Q02+
pipeline tests the rule on Darwinex XTIUSD.DWX bars.
...
- [x] R4 compliant: no ML, no adaptive PnL fitting, no grid, no martingale, one
  position per magic.
- [x] Non-duplicate: October month-of-year fade is not the existing weekday,
  February month, monthly demand, WPSR, refinery, hurricane, OPEC, expiry,
  reversal, ratio, XNG, XAU/XAG, or RSI commodity sleeve.

## Framework Alignment

- no_trade: D1 and XTIUSD.DWX guard, parameter guard, spread cap.
- trade_entry: October broker-calendar short entry.
...
|---|---|---|---|---|
| v1 | 2026-06-27 | initial structural WTI October calendar-fade build | G0 | DRAFT |

## Pipeline Phase Status

| Phase | Date | Verdict | Evidence path |
|---|---|---|---|
| G0 Research Intake | 2026-06-27 | DRAFT | this card |
