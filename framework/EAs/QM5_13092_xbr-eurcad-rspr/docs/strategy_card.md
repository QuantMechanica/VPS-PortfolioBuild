---
ea_id: QM5_13092
slug: xbr-eurcad-rspr
type: strategy
strategy_id: EIA-BOC-XBR-EURCAD-2026
...
source_citations:
  ...
  - type: government_energy_context
    ...
    role: supplement
sources:
  - "[[sources/EIA-BOC-XBR-EURCAD-2026]]"
concepts:
...
target_symbols: [XBRUSD.DWX, EURCAD.DWX]
basket_symbols: [XBRUSD.DWX, EURCAD.DWX]
markets: [XBRUSD.DWX, EURCAD.DWX]
primary_target_symbols: [XBRUSD.DWX, EURCAD.DWX]
single_symbol_only: false
logical_symbol: QM5_13092_XBR_EURCAD_RSPREAD_D1
period: D1
timeframes: [D1]
expected_trade_frequency: "D1 XBR/EURCAD inverse-CAD return-spread z-score reversion; estimate 6-12 paired packages/year before Q02 validates synchronized history and fills."
...
g0_approval_reasoning: "Mission-directed G0 approval 2026-07-09: R1 PASS EIA oil/exchange-rate research plus Bank of Canada oil/CAD systematic-variation support and EIA Canada energy context; R2 PASS deterministic D1 XBR/EURCAD inverse-CAD return-spread basket with fixed return window, rolling z-score, spread caps, mean exit, max-hold exit, and ATR hard stops; R3 PASS XBRUSD.DWX and EURCAD.DWX exist in the DWX matrix; R4 PASS no ML/grid/martingale/external runtime data. Non-duplicate because this is Brent/EURCAD, not XTI/EURCAD, XBR/USDCAD, XBR/AUDCAD, XBR/NZDCAD, XBR/CADJPY, XBR/CADCHF, XBR/XNG, FX-only EURCAD/GBPNZD cointegration, XAU/XAG, XNG, index, or outright Brent logic."
---

# XBR/EURCAD D1 Return-Spread Reversion

## Source

- Source: [[sources/EIA-BOC-XBR-EURCAD-2026]]
...
`return_spread = ln(XBR[t] / XBR[t-L]) + beta_eurcad * ln(EURCAD[t] / EURCAD[t-L])`

When Brent has unusually outperformed while EURCAD has also risen, the package
fades a "oil up while CAD weak" divergence by selling both Brent and EURCAD. When
Brent has unusually underperformed while EURCAD has also fallen, it buys both Brent
and EURCAD.

This is deliberately different from:

- `QM5_13057_xti-eurcad-rspr`: WTI/EURCAD, not Brent/EURCAD.
- `QM5_13005_xbr-cad-rspr`: USDCAD-specific Brent/CAD spread, not EURCAD.
...
- FX-only CAD-cross cointegration baskets: not an energy/FX oil-CAD sleeve.
- XAU/XAG, XNG, index, XBR/XNG, oil/metals, and single-symbol Brent
  trend/seasonality sleeves.

## Markets And Timeframe

- Logical symbol: `QM5_13092_XBR_EURCAD_RSPREAD_D1`.
- Host symbol: `XBRUSD.DWX`.
- Basket legs: `XBRUSD.DWX` and `EURCAD.DWX`.
- Period: D1.
...
- Copy completed D1 closes for `XBRUSD.DWX` and `EURCAD.DWX`.
...
- Compute `return_spread = xbr_ret + strategy_beta_eurcad * eurcad_ret`.
...
- [x] R3 testable: `XBRUSD.DWX` and `EURCAD.DWX` exist in the Darwinex symbol
  matrix.
- [x] R4 compliant: no ML, adaptive PnL fitting, grid, martingale, external
  runtime feed, or more than one position per magic slot.
- [x] Non-duplicate: paired Brent/EURCAD inverse-CAD return-spread mean
  reversion, not Brent/USDCAD, Brent/AUDCAD, Brent/NZDCAD, Brent/CADJPY,
  Brent/CADCHF, WTI/EURCAD, FX-only CAD-cross cointegration, XBR/XNG,
...
- Slot 0: `XBRUSD.DWX`.
- Slot 1: `EURCAD.DWX`.
- Use the logical basket setfile `QM5_13092_XBR_EURCAD_RSPREAD_D1` for Q02.
...
- trade_entry: D1 standardized XBR/EURCAD inverse-CAD return-spread reversion.
...
trade, if Q02 PF is below 1.0 after costs, if synchronized XBR/EURCAD history
...
|---|---|---|---|
| v1 | 2026-07-09 | initial XBR/EURCAD return-spread basket build | Q02 | PENDING |
...
| Q02 Baseline Screening | 2026-07-09 | PENDING | `work_items:ef700ff8-32d5-4144-acf1-3879ab6393ac` |
