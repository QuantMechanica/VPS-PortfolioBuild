...
#property description "QM5_41102 WTI Completed-Month Auction-Range Migration Momentum"
...
//   - compare only each completed month's aggregate high and low
...
//   - follow the completed auction-range migration for one broker month
// Runtime uses MT5-native OHLC, calendar, history, and framework state only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                      = 41102;
...
input int    strategy_required_months       = 2;
input int    strategy_min_month_bars        = 17;
input int    strategy_max_month_bars        = 23;
...
bool Strategy_LoadRangeMigrationSignal(const int current_month_key,
                                       const int label_offset,
                                       int &direction,
                                       int &new_month_bars,
                                       ...
                                       double &parent_low)
  {
   ...
   const int copied =
      CopyRates(_Symbol, // perf-allowed: bounded completed-month OHLC scan behind the sole QM_IsNewBar branch.
                ...
                strategy_history_bars,
                bars);
   if(copied < strategy_min_month_bars * 2 + 1)
      return false;
   ...
   for(int index = 0; index < copied; ++index)
     {
      ...
      ++month_counts[bucket];
      if(month_counts[bucket] > strategy_max_month_bars)
         return false;
     }
   ...
   for(int bucket = 0; bucket < 2; ++bucket)
     {
      if(month_counts[bucket] < strategy_min_month_bars ||
         month_counts[bucket] > strategy_max_month_bars)
         return false;
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadRangeMigrationSignal(g_decision_month_key,
      ...
      if(!g_signal_valid)
         ...
         g_signal_state = "endpoint_equality_flat";
              ...
              g_new_low > g_parent_low)
         g_signal_state = "inside_month_flat";
      else if(g_new_high > g_parent_high &&
              g_new_low < g_parent_low)
         g_signal_state = "outside_month_flat";
      else
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"new_month_bars\":%d,\"parent_month_bars\":%d,\"new_high\":%.8f,\"new_low\":%.8f,\"parent_high\":%.8f,\"parent_low\":%.8f,\"state\":\"%s\"}",
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41102 ||
   ...
   if(strategy_entry_grace_minutes != 180 ||
      ...
      strategy_required_months != 2 ||
      strategy_min_month_bars != 17 ||
      strategy_max_month_bars != 23 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41102_WTI_MRANGE_MIGRATE_MOM";
   ...
   req.reason = (g_signal_direction > 0)
      ? "WTI_MRANGE_MIGRATE_MOM_LONG"
      : "WTI_MRANGE_MIGRATE_MOM_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41102 ||
   ...
   g_attempt_state_key =
      StringFormat("QM5_41102_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41102\",\"ea\":\"wti-mrange-migrate-mom\"}");
