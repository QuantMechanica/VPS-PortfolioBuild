...
#property description "QM5_11396 connors-double7s-sma200-h4 — Connors Double 7's, SMA(200) trend + 7-bar close extreme (H4)"
...
// Work" (2009), "Double 7's Strategy" — Forex adaptation on H4. Card:
// artifacts/cards_approved/QM5_11396_connors-double7s-sma200-h4.md (APPROVED).
//
// Mechanics (H4, closed-bar reads at shift 1..N; gapless-safe = closes only):
...
//   P2 stop      : SL = ATR(14) * sl_atr_mult, capped at sl_max_pips (card: 50).
//                  No fixed TP — the Connors discretionary exits below close.
...
input int    strategy_extreme_lookback   = 7;      // N-bar close extreme lookback (Connors "7")
input int    strategy_atr_period         = 14;     // ATR period for P2 stop sizing
input double strategy_sl_atr_mult        = 2.0;    // SL distance = mult * ATR (card P2)
input int    strategy_sl_max_pips        = 50;     // SL hard cap in pips (card P2)
input int    strategy_max_hold_bars      = 12;     // time-stop: exit after N closed bars
   ...
   for(int s = 2; s <= lookback; ++s)
     {
      ...
      if(cs <= 0.0)
         return false;
      if(c1 > cs)
         return false; // a window close is lower -> not the N-bar low
     }
   return true;
  }
   ...
   for(int s = 2; s <= lookback; ++s)
     {
      ...
      if(cs <= 0.0)
         return false;
      if(c1 < cs)
         return false; // a window close is higher -> not the N-bar high
     }
   return true;
  }

// -----------------------------------------------------------------------------
// Helper: ATR-based stop PRICE with the card's pip cap applied to the distance.
// SL distance = min(ATR * sl_atr_mult, sl_max_pips). Returns 0.0 if unavailable.
// -----------------------------------------------------------------------------
double CappedAtrStopPrice(const QM_OrderType side,
                          const double entry,
                          const double atr_value)
  {
   ...
   double distance = strategy_sl_atr_mult * atr_value;
   if(distance <= 0.0)
      return 0.0;

   if(strategy_sl_max_pips > 0)
     {
...
// position per symbol/magic. Long in uptrend on a fresh 7-bar low close; short
// in downtrend on a fresh 7-bar high close.
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   // LONG: macro uptrend + 7-bar low close (pullback in uptrend).
   if(close1 > sma && IsNDayLowClose(strategy_extreme_lookback))
     {
      const double sl = CappedAtrStopPrice(QM_BUY, entry_ask, atr_value);
      if(sl <= 0.0)
         return false;
      req.type   = QM_BUY;
      req.price  = 0.0;   // framework fills market price at send
      req.sl     = sl;
      req.tp     = 0.0;   // no fixed TP — Connors discretionary exits close it
      req.reason = "connors_double7s_long";
      return true;
     }

   // SHORT: macro downtrend + 7-bar high close (rally in downtrend).
   if(close1 < sma && IsNDayHighClose(strategy_extreme_lookback))
     {
      const double sl = CappedAtrStopPrice(QM_SELL, entry_bid, atr_value);
      if(sl <= 0.0)
         return false;
      req.type   = QM_SELL;
      req.price  = 0.0;
...
// No active trade management beyond the fixed ATR stop plus the discretionary
// exits handled in Strategy_ExitSignal.
void Strategy_ManageOpenPosition()
  {
  }

// Discretionary exit (closed-bar logic; framework only fires this branch on the
// open-position magic). Returns TRUE to close. Connors exit = opposite N-bar
...
bool Strategy_ExitSignal()
  {
   ...
   // Exit reads are cheap closed-bar reads (shift 1) + handle-pooled SMA, so
   // they are O(1) and safe to evaluate per tick. The SL is enforced by the
   ...
   if(is_long)
     {
      ...
      if(close1 < sma)
         return true;
      // Connors exit: 7-bar high close reached.
   ...
   else
     {
      ...
      if(close1 > sma)
         return true;
      // Connors exit: 7-bar low close reached.
