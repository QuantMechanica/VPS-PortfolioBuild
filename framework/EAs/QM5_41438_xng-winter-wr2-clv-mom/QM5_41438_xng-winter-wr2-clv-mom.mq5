...
#property description "QM5_41438 XNG Winter WR2 CLV Momentum"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41438 - XNG Winter WR2 Close-Location Continuation
// -----------------------------------------------------------------------------
// D1 structural crude-oil sleeve:
//   - decide only in November through March during the winter-demand season
//   - aggregate two consecutive completed 3-to-5-session normalized weeks
//   - follow an expanding newest week only when its close is in an outer quartile
   ...
   else
     {
      ...
      if(!g_signal_valid)
         g_signal_state = "two_completed_weeks_validation_failed";
          ...
          g_signal_state = "wr2_upper_quartile_continuation_long";
       else if(g_signal_direction < 0)
          g_signal_state = "wr2_lower_quartile_continuation_short";
         ...
         g_signal_state = "close_location_equality_flat";
          ...
          g_signal_state = "close_not_outer_quartile_flat";
     }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41438_XNG_WINTER_WR2_CLV_MOM";
   ...
   req.reason = (g_signal_direction > 0
                 ? "XNG_WINTER_WR2_CLV_MOM_LONG"
                 : "XNG_WINTER_WR2_CLV_MOM_SHORT");
...
int OnInit()
  {
   if(!SymbolSelect(strategy_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41438 ||
      qm_magic_slot_offset != 0)
   ...
   g_attempt_state_key =
      StringFormat("QM5_41438_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41438\",\"ea\":\"xng-winter-wr2-clv-mom\"}");
