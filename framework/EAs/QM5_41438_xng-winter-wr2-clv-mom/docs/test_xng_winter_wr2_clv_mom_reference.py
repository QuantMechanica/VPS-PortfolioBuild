"""Deterministic reference checks for QM5_41438 XNG winter WR2/CLV momentum."""
...
class XngWinterWR2CLVReferenceTest(unittest.TestCase):
    ...
    def test_expansion_lower_quartile_is_short(self) -> None:
    ...
    def test_lower_clv_equality_is_flat(self) -> None:
        direction, clv = signal(self.current, *self.bars(0.25))
        self.assertEqual((direction, clv), (0, 0.25))

    def test_interior_clv_is_flat(self) -> None:
        self.assertEqual(signal(self.current, *self.bars(0.5))[0], 0)

    def test_all_five_winter_months_are_eligible(self) -> None:
        self.assertEqual(ELIGIBLE_MONTHS, {11, 12, 1, 2, 3})
