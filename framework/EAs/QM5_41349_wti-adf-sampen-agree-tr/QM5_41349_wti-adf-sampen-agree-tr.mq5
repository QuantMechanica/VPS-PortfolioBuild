...
#property description "QM5_41349 WTI Monthly ADF and Sample Entropy Agreement Trend"
...
//   - form sixty adjacent monthly log returns and newest sixty log levels
//   - require lag-one ADF t >= -2.594 and m=2 SampEn <= 2.5
//   - follow twelve-month WTI momentum only when both gates qualify
...
input int    strategy_embedding_dimension  = 2;
input int    strategy_embedding_lag        = 1;
input double strategy_radius_sd_fraction   = 0.2;
input double strategy_sd_floor             = 0.000000000001;
input double strategy_entropy_ceiling      = 2.5;
...
bool Strategy_LoadMonthlyEndpoints(const int current_month_key,
                                   ...
                                   datetime &endpoint_times[])
  {
   ...
   ArraySetAsSeries(bars, true);
   const int copied =
      CopyRates(_Symbol, // perf-allowed: one bounded sixty-one-month scan behind a consumed monthly attempt or monthly restart validation.
                PERIOD_D1,
                1,
                strategy_history_bars,
                bars);
...
bool Strategy_TemplatePairMatches(const double &values[],
                                  ...
                                  const double radius)
  {
   ...
   if(value_count <= 0 || left_start < 0 || right_start < 0 ||
      left_start >= right_start || dimension <= 0 || lag <= 0 ||
      radius <= 0.0 || !MathIsValidNumber(radius) ||
      left_start + (dimension - 1) * lag >= value_count ||
      right_start + (dimension - 1) * lag >= value_count)
      return false;

   for(int offset = 0; offset < dimension; ++offset)
     {
      const int left_index = left_start + offset * lag;
      const int right_index = right_start + offset * lag;
      if(left_index < 0 || right_index < 0 ||
         left_index >= value_count || right_index >= value_count)
         return false;
      const double distance =
         MathAbs(values[left_index] - values[right_index]);
      // The card locks strict Chebyshev distance: equality is not a match.
      if(!MathIsValidNumber(distance) || distance >= radius)
         return false;
     }
   return true;
  }

bool Strategy_SampleEntropyCore(const double &values[],
                                double &sample_mean,
                                double &sample_sd,
                                double &radius,
                                int &matches_m,
                                ...
                                double &sample_entropy)
  {
   ...
   radius = 0.0;
   matches_m = 0;
   matches_m_plus_one = 0;
   sample_entropy = 0.0;

   const int value_count = ArraySize(values);
   if(value_count != 60 || strategy_month_returns != 60 ||
      strategy_embedding_dimension != 2 || strategy_embedding_lag != 1 ||
      MathAbs(strategy_radius_sd_fraction - 0.2) >
         0.000000000000001 ||
      MathAbs(strategy_sd_floor - 0.000000000001) >
   ...
   for(int index = 0; index < value_count; ++index)
     {
      if(!MathIsValidNumber(values[index]))
         return false;
      sample_mean += values[index];
     }
   sample_mean /= (double)value_count;

   double centered_sum_squares = 0.0;
   for(int index = 0; index < value_count; ++index)
     {
      const double centered = values[index] - sample_mean;
      centered_sum_squares += centered * centered;
     }
   sample_sd = MathSqrt(centered_sum_squares /
                        (double)(value_count - 1));
   if(!MathIsValidNumber(sample_sd) || sample_sd <= strategy_sd_floor)
      return false;
   radius = strategy_radius_sd_fraction * sample_sd;
   if(!MathIsValidNumber(radius) || radius <= 0.0)
      return false;

   const int templates_m =
      value_count - (strategy_embedding_dimension - 1) *
                    strategy_embedding_lag;
   const int templates_m_plus_one =
      value_count - strategy_embedding_dimension *
                    strategy_embedding_lag;
   if(templates_m != 59 || templates_m_plus_one != 58)
      return false;

   for(int left = 0; left < templates_m; ++left)
      for(int right = left + 1; right < templates_m; ++right)
         if(Strategy_TemplatePairMatches(values,
                                         left,
                                         right,
                                         strategy_embedding_dimension,
                                         strategy_embedding_lag,
                                         radius))
            ++matches_m;

   for(int left = 0; left < templates_m_plus_one; ++left)
      for(int right = left + 1;
          right < templates_m_plus_one;
          ++right)
         if(Strategy_TemplatePairMatches(values,
                                         left,
                                         right,
                                         strategy_embedding_dimension + 1,
                                         strategy_embedding_lag,
                                         radius))
            ++matches_m_plus_one;

   if(matches_m <= 0 || matches_m_plus_one <= 0 ||
      matches_m_plus_one > matches_m)
      return false;
   sample_entropy =
      MathLog((double)matches_m / (double)matches_m_plus_one);
   return (MathIsValidNumber(sample_entropy) && sample_entropy >= 0.0);
  }

bool Strategy_SampleEntropyReferenceSelfTest()
  {
   double alternating[60];
   for(int index = 0; index < 60; ++index)
      alternating[index] = ((index % 2) == 0) ? -0.01 : 0.01;

   double sample_mean = 0.0;
   double sample_sd = 0.0;
   double radius = 0.0;
   int matches_m = 0;
   int matches_m_plus_one = 0;
   double sample_entropy = 0.0;
   if(!Strategy_SampleEntropyCore(alternating,
                                  sample_mean,
                                  sample_sd,
                                  radius,
                                  matches_m,
                                  matches_m_plus_one,
                                  sample_entropy) ||
      matches_m != 841 || matches_m_plus_one != 812 ||
      MathAbs(sample_mean) > 0.000000000000001 ||
      MathAbs(sample_entropy - 0.03509131981127019) >
         0.000000000001)
      return false;

   double strict_boundary[4];
   strict_boundary[0] = 0.0;
   strict_boundary[1] = 0.0;
   strict_boundary[2] = 1.0;
   strict_boundary[3] = 1.0;
   if(Strategy_TemplatePairMatches(strict_boundary, 0, 1, 2, 1, 1.0) ||
      !Strategy_TemplatePairMatches(strict_boundary,
                                    0,
                                    1,
                                    2,
                                    1,
...
bool Strategy_AgreementSignal(const double &closes[],
                                  Strategy_SignalMetrics &metrics)
  {
   ...
   if(strategy_level_count != 60 ||
      ...
      MathAbs(strategy_sd_floor - 0.000000000001) >
         0.000000000000000001 ||
      MathAbs(strategy_entropy_ceiling - 2.5) >
         0.000000000000001 ||
   ...
   // ADF uses the newest sixty completed log levels; the oldest endpoint
   // exists only to form the first sample-entropy return.
   double levels[60];
   for(int index = 0; index < strategy_level_count; ++index)
     {
      if(index < 0 || index + 1 >= metrics.endpoint_count)
         return false;
      levels[index] = MathLog(closes[index + 1]);
      if(!MathIsValidNumber(levels[index]))
         return false;
   ...
   if(!Strategy_SampleEntropyCore(returns,
                                  metrics.sample_mean,
                                  metrics.sample_sd,
                                  metrics.radius,
                                  metrics.matches_m,
                                  metrics.matches_m_plus_one,
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
   ...
   else
     {
      ...
      if(!g_signal_valid)
         ...
         g_signal_state = "adf_below_boundary_consumed_flat";
      else if(!g_signal_metrics.entropy_qualified)
         g_signal_state = "sample_entropy_above_ceiling_consumed_flat";
      else if(g_signal_metrics.direction == 0)
         g_signal_state = "agreement_neutral_momentum_consumed_flat";
      else if(g_signal_metrics.direction > 0)
         g_signal_state = "adf_sampen_agreement_positive_momentum_wti_long";
      else if(g_signal_metrics.direction < 0)
         g_signal_state = "adf_sampen_agreement_negative_momentum_wti_short";
      else
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"endpoint_count\":%d,\"return_count\":%d,\"adf_t\":%.12f,\"adf_qualified\":%s,\"sample_mean\":%.12f,\"sample_sd\":%.12f,\"radius\":%.12f,\"matches_m\":%d,\"matches_m_plus_one\":%d,\"sample_entropy\":%.12f,\"entropy_qualified\":%s,\"momentum_12\":%.12f,\"returns\":\"%s\",\"oldest_close\":%.10f,\"newest_close\":%.10f,\"oldest_endpoint\":%I64d,\"newest_endpoint\":%I64d,\"state\":\"%s\"}",
                            ...
                            g_signal_metrics.sample_mean,
                            g_signal_metrics.sample_sd,
                            g_signal_metrics.radius,
                            g_signal_metrics.matches_m,
                            g_signal_metrics.matches_m_plus_one,
                            g_signal_metrics.sample_entropy,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41349 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41349_WTI_ADF_SAMPEN_AGREE";
   ...
   req.reason = (g_signal_metrics.direction > 0)
      ? "WTI_ADF_SAMPEN_AGREE_LONG"
      : "WTI_ADF_SAMPEN_AGREE_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41349 ||
      qm_magic_slot_offset != 0)
      return INIT_PARAMETERS_INCORRECT;
   if(!Strategy_SampleEntropyReferenceSelfTest())
      return INIT_FAILED;
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI monthly ADF/sample-entropy agreement trend holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41349_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   g_entry_month_state_key =
      StringFormat("QM5_41349_ENTRY_MONTH_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41349\",\"ea\":\"wti-adf-sampen-agree-tr\"}");
