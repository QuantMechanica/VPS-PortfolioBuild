...
#property description "QM5_13034 XTI AUDCAD Return Spread Reversion"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_13034 - WTI/AUDCAD Return-Spread Reversion
// -----------------------------------------------------------------------------
// D1 two-leg energy/FX basket:
//   rspread = log(XTI[t] / XTI[t-L]) + beta_audcad * log(AUDCAD[t] / AUDCAD[t-L])
// AUDCAD is used as a CAD-through-AUDCAD proxy: CAD strength versus AUD lowers
// AUDCAD, so the CAD proxy is the negative AUDCAD return. Runtime uses MT5 OHLC
// only; no external source data is consumed by the EA.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 13034;
...
input double strategy_beta_audcad           = 0.65;
...
input int    strategy_audcad_max_spread_pts = 120;
input int    strategy_deviation_points      = 20;

string   g_leg_xti    = "XTIUSD.DWX";
string   g_leg_audcad = "AUDCAD.DWX";
...
bool Strategy_SpreadAllowed(const string symbol)
  {
   ...
   if(symbol == g_leg_audcad && strategy_audcad_max_spread_pts > 0)
      return (spread_points <= strategy_audcad_max_spread_pts);
   return true;
  }
...
bool Strategy_RefreshSpreadState()
  {
   ...
   ArraySetAsSeries(audcad, true);
   if(CopyClose(g_leg_xti, PERIOD_D1, 1, history_needed, xti) != history_needed) // perf-allowed: D1 new-bar/close refresh only.
      return false;
   if(CopyClose(g_leg_audcad, PERIOD_D1, 1, history_needed, audcad) != history_needed) // perf-allowed: D1 new-bar/close refresh only.
      return false;
   ...
   for(int i = 0; i < z_lookback; ++i)
     {
      ...
      if(xti[i] <= 0.0 || audcad[i] <= 0.0 || xti[past_idx] <= 0.0 || audcad[past_idx] <= 0.0)
         return false;

      const double xti_ret = MathLog(xti[i] / xti[past_idx]);
      const double audcad_ret = MathLog(audcad[i] / audcad[past_idx]);
      spreads[i] = xti_ret + strategy_beta_audcad * audcad_ret;
...
bool Strategy_OpenPair(const int spread_direction)
  {
   if(spread_direction == 0 || Strategy_OpenPairLegCount() > 0)
      return false;
   if(!Strategy_SpreadAllowed(g_leg_xti) || !Strategy_SpreadAllowed(g_leg_audcad))
      return false;

   const double xti_weight = 1.0;
   const double audcad_weight = MathMax(0.1, MathAbs(strategy_beta_audcad));
   const double weight_sum = xti_weight + audcad_weight;
   const bool long_spread = (spread_direction > 0);
   const QM_OrderType xti_type = long_spread ? QM_BUY : QM_SELL;
   const QM_OrderType audcad_type = long_spread ? QM_BUY : QM_SELL;
   const string reason = long_spread ? "QM5_13034_LONG_XTI_AUDCAD_RSPREAD"
                                     : "QM5_13034_SHORT_XTI_AUDCAD_RSPREAD";

   bool xti_ok = Strategy_OpenLeg(g_leg_xti, xti_type, xti_weight, weight_sum, reason);
   bool audcad_ok = Strategy_OpenLeg(g_leg_audcad, audcad_type, audcad_weight, weight_sum, reason);
   if(xti_ok && audcad_ok)
     {
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart())
      return true;
   if(strategy_return_lookback_d1 < 5 || strategy_z_lookback_d1 < 40 || strategy_beta_audcad <= 0.0)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_13034_RSPREAD_HOST";
   ...
   if(qm_news_temporal != QM_NEWS_TEMPORAL_OFF || qm_news_compliance != QM_NEWS_COMPLIANCE_NONE)
     {
      if(!QM_NewsAllowsTrade2(g_leg_xti, broker_time, qm_news_temporal, qm_news_compliance))
         return true;
      if(!QM_NewsAllowsTrade2(g_leg_audcad, broker_time, qm_news_temporal, qm_news_compliance))
         return true;
     }
   else
     {
      if(!QM_NewsAllowsTrade(g_leg_xti, broker_time, qm_news_mode_legacy))
         return true;
      if(!QM_NewsAllowsTrade(g_leg_audcad, broker_time, qm_news_mode_legacy))
         return true;
     }
   return false;
  }
...
int OnInit()
  {
   ...
   SymbolSelect(g_leg_audcad, true);
   ...
   string basket_symbols[2] = {g_leg_xti, g_leg_audcad};
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_13034\",\"ea\":\"xti-audcad-rspr\"}");
