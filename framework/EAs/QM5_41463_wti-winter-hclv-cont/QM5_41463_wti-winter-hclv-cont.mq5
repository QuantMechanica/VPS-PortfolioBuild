...
#property description "QM5_41463 WTI Winter Upper-CLV Continuation"
...
//   - buy only when that week closes in the upper third of its full range
...
input double strategy_clv_cutoff           = 0.666666666667;
...
bool Strategy_LoadUpperCLVSignal(const int current_week_key,
                                 const int label_offset,
                                 int &direction)
  {
   direction = 0;
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_WeekAlreadyEntered(g_decision_week_key))
     ...
     {
       g_signal_valid =
          Strategy_LoadUpperCLVSignal(g_decision_week_key,
                                      g_decision_label_offset,
                                      g_signal_direction);
       if(!g_signal_valid)
          g_signal_state = "completed_week_validation_failed";
       else if(g_signal_direction > 0)
          g_signal_state = "upper_tercile_continuation_long";
       else
          g_signal_state = "completed_week_not_upper_tercile_flat";
      }
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(strategy_symbol == "" ||
      ...
      MathAbs(strategy_clv_cutoff - 0.666666666667) > 1.0e-12 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41463_WTI_WINTER_HCLV_CONT";
   ...
   req.type = QM_BUY;
   req.reason = "WTI_WINTER_HCLV_CONT_LONG";
...
int OnInit()
  {
   if(!SymbolSelect(strategy_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41463 ||
      qm_magic_slot_offset != 0)
   ...
   g_attempt_state_key =
      StringFormat("QM5_41463_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41463\",\"ea\":\"wti-winter-hclv-cont\"}");
