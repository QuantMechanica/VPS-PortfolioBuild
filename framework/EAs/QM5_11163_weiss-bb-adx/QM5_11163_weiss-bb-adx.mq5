...
bool Strategy_NoTradeFilter()
  {
   ...
   if(_Symbol == "EURUSD.DWX")
      expected_slot = 0;
   else if(_Symbol == "EURJPY.DWX")
      expected_slot = 1;
   else if(_Symbol == "EURCHF.DWX")
      expected_slot = 2;
   else if(_Symbol == "AUDCAD.DWX")
      expected_slot = 3;
   else if(_Symbol == "SP500.DWX")
      expected_slot = 4;

   if(expected_slot < 0 || expected_slot != qm_magic_slot_offset)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   const int bb_now = QM_Sig_BB_MeanRev(_Symbol, PERIOD_D1,
                                        strategy_bb_period,
                                        strategy_bb_deviation,
                                        1);
   const int bb_prev = QM_Sig_BB_MeanRev(_Symbol, PERIOD_D1,
                                         strategy_bb_period,
                                         strategy_bb_deviation,
                                         2);
   ...
   if(stops_level > 0)
     {
      ...
      if(sl_points <= (double)stops_level || tp_points <= (double)stops_level)
         return false;
     }
...
void Strategy_ManageOpenPosition()
  {
   // Card specifies broker TP/SL only; no trailing, break-even, or partial close.
  }
...
bool Strategy_ExitSignal()
  {
   // Card specifies no discretionary exit beyond broker TP/SL and framework guards.
   return false;
  }
