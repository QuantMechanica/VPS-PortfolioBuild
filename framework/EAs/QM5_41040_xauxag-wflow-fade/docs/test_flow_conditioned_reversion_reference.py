"""Independent mechanic fixtures for QM5_41040.

The tests cover the exact synchronized six-bar calendar, strict relative-flow
opposition, strict session dominance, completed-week fade sides, endpoint
reconciliation, Monday grace, package risk/notional rounding, and weekly
lifecycle without invoking MT5 or copying framework order plumbing.
"""
...
def flow_conditioned_direction(
    ...
    session_dominant = abs(session_relative) > abs(overnight_relative)
    if strict_opposition and session_dominant and week_relative > 0:
        return -1
    if strict_opposition and session_dominant and week_relative < 0:
        return 1
    return 0
...
def relative_flow_direction(
    ...
    direction = flow_conditioned_direction(
...
class FlowConditionedReversionReferenceTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_positive_session_dominant_week_is_faded_by_selling_xau(self) -> None:
        ...
        self.assertGreater(abs(session), abs(overnight))

    def test_negative_session_dominant_week_is_faded_by_buying_xau(self) -> None:
    ...
    def test_opposition_without_session_dominance_remains_flat(self) -> None:
        xau = component_bars(100.0, 0.98, 1.01)
        xag = component_bars(100.0, 1.02, 0.995)
        direction, overnight, session, _, _ = relative_flow_direction(
            100.0, 100.0, xau, xag
        )
        ...
        self.assertLess(overnight, 0)
        self.assertGreater(session, 0)
        self.assertLessEqual(abs(session), abs(overnight))

    def test_equal_opposite_components_remain_flat(self) -> None:
        self.assertEqual(flow_conditioned_direction(-0.05, 0.05), 0)
        self.assertEqual(flow_conditioned_direction(0.05, -0.05), 0)
