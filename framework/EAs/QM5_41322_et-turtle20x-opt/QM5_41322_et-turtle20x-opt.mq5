...
#property description "QM5_41322 Elite Trader Turtle 20/10 Donchian Breakout - DL-089 opt sibling"
...
input bool   strategy_atr_regime_filter        = false;
input int    strategy_atr_regime_window        = 100;
input double strategy_atr_regime_percentile    = 50.0;
...
double PercentileATR(const int bars, const double percentile)
  {
   ...
   for(int i = 0; i < bars; ++i)
     {
      values[i] = QM_ATR(_Symbol, strategy_tf, strategy_atr_period, i + 2);
      if(values[i] <= 0.0)
         return 0.0;
     }
...
bool AtrRegimeAllowsEntry()
  {
   if(!strategy_atr_regime_filter)
      return true;

   const double atr = QM_ATR(_Symbol, strategy_tf, strategy_atr_period, 1);
   const double threshold = PercentileATR(strategy_atr_regime_window, strategy_atr_regime_percentile);
...
void InitRequest(QM_EntryRequest &req)
  {
   ...
   req.expiration_seconds = (seconds > 0) ? MathMax(seconds, strategy_pending_expiry_bars * seconds) : 86400;
  }
...
bool BuildStopRequest(const QM_OrderType side,
                      ...
                      QM_EntryRequest &req)
  {
   ...
   const double atr_stop = (side == QM_BUY_STOP)
                           ? entry_price - strategy_atr_stop_mult * atr
                           : entry_price + strategy_atr_stop_mult * atr;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(strategy_entry_channel < 2 || strategy_exit_channel < 1 ||
   ...
   const double entry_high = HighestHigh(1, strategy_entry_channel);
   const double entry_low = LowestLow(1, strategy_entry_channel);
   ...
   if(can_buy && can_sell)
     {
      if(Pattern_AllowsRequest(buy_req))
        {
         ulong buy_ticket = 0;
