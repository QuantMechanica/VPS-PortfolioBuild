...
#property description "QM5_41459 WTI Summer WR2 Negative-Week Continuation"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41459 - WTI Summer WR2 Negative-Week Continuation
// -----------------------------------------------------------------------------
// D1 structural crude-oil sleeve:
//   - decide only in June through October during the negative WTI summer leg
//   - aggregate two consecutive completed 3-to-5-session normalized weeks
//   - short only after an expanding newest week closes below its own open
...
input int    strategy_label_offset_seconds = 0;
input int    strategy_entry_grace_minutes  = 180;
input int    strategy_history_bars         = 30;
input int    strategy_summer_month_1      = 6;
input int    strategy_summer_month_2      = 7;
input int    strategy_summer_month_3      = 8;
input int    strategy_summer_month_4      = 9;
input int    strategy_summer_month_5      = 10;
...
bool Strategy_IsSummerMonth(const int month)
  {
   return (month == strategy_summer_month_1 ||
           month == strategy_summer_month_2 ||
           month == strategy_summer_month_3 ||
           month == strategy_summer_month_4 ||
           month == strategy_summer_month_5);
  }
...
bool Strategy_LoadWR2BodySignal(const int current_week_key,
                               ...
                               int &direction)
  {
   ...
   g_newer_week_body = MathLog(g_newer_week_close / g_newer_week_open);
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_WeekAlreadyEntered(g_decision_week_key))
      ...
      g_signal_state = "late_summer_consumed_flat";
   else if(!Strategy_IsSummerMonth(anchor_month))
      g_signal_state = "outside_summer_months";
   else
     {
      g_signal_valid =
      ...
      if(!g_signal_valid)
         ...
         g_signal_state = "newest_week_nonnegative_body_flat";
     }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41459_WTI_SUMMER_WR2_DOWNWEEK_CONT";
   ...
   req.type = QM_SELL;
   req.reason = "WTI_SUMMER_WR2_DOWNWEEK_CONT_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(strategy_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41459 ||
      qm_magic_slot_offset != 0)
   ...
   g_attempt_state_key =
      StringFormat("QM5_41459_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41459\",\"ea\":\"wti-summer-wr2-downweek-cont\"}");
