# QM5_41459_wti-summer-wr2-downweek-cont - Strategy Spec

**EA ID:** QM5_41459  
**Strategy ID:** `BURAKOV-CRABEL-MOP-WTI-SUMMER-WR2-DOWNWEEK-CONT-20260912_S01`
...
strictly wider. Sell only when that week's final close is strictly below its chronological
first open. Range equality, contraction, or a zero/positive body is flat. Consume the decision
...
Exact setfile-configured `XTIUSD.DWX` D1 carrier, slot zero, magic `414590000`. Runtime uses native
...
`strategy-seeds/sources/BURAKOV-CRABEL-MOP-WTI-SUMMER-WR2-DOWNWEEK-CONT-20260912/source.md`. It
...
range state), and the November-May WR2 family. Closest sibling `QM5_41458` uses the same strict
expansion state but requires a positive body and fades it; this candidate requires a negative
body and continues it, so their entry predicates are mutually exclusive. `QM5_41457` requires
range contraction and a positive body.
...
| entry | June-October calendar, two completed weeks, strict range expansion, negative-body short continuation, attempt, spread, ATR stop |
...
financing, summer instability, weekly-horizon translation, and overlap with WTI candidates can
