...
#property description "QM5_41380 WTI One-Month Trend / Two-Month Hold"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41380 - WTI One-Month Trend / Two-Month Hold
// -----------------------------------------------------------------------------
...
//   1. derive two consecutive completed WTI month-end closes from D1;
//   2. calculate the exact one-month return ln(C[1] / C[0]);
...
double g_cache_one_month_return   = 0.0;
...
void Strategy_ResetCachedState()
  {
   ...
   g_cache_signal = 0;
   g_cache_one_month_return = 0.0;
   g_cache_state_reason = "not_evaluated";
  }

bool Strategy_OneMonthReturnSignal(const double &closes[],
                                   int &signal)
  {
   signal = 0;
   if(strategy_return_months != 1 ||
      ArraySize(closes) != 2)
     {
      g_cache_state_reason = "bad_one_month_contract";
      return false;
     }
   ...
   const double endpoint_ratio = closes[1] / closes[0];
   ...
   const double one_month_return = MathLog(endpoint_ratio);
   if(!MathIsValidNumber(one_month_return))
     {
      g_cache_state_reason = "invalid_one_month_return";
      return false;
     }
   ...
   if(!MathIsValidNumber(chained_return) ||
      MathAbs(one_month_return - chained_return) > 1.0e-10)
     {
      g_cache_state_reason = "endpoint_path_identity_failed";
      return false;
     }

   g_cache_one_month_return = one_month_return;
   if(one_month_return == 0.0)
     {
      g_cache_state_reason = "exact_zero_one_month_return";
      return true;
     }

   signal = (one_month_return > 0.0) ? 1 : -1;
...
bool Strategy_LoadSignalState(int &signal)
  {
   ...
   return Strategy_OneMonthReturnSignal(monthly_closes,
                                        signal);
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41380_WTI_TSMOM1_H2";
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41380_BIMONTH_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41380\",\"ea\":\"wti-tsmom1-h2\"}");
