#!/usr/bin/env python3
"""Independent vectors for QM5_41380's locked statistic and clock."""
...
def consecutive(month_keys: list[int]) -> bool:
    return len(month_keys) == 2 and next_month(month_keys[0]) == month_keys[1]
...
def signal(closes: list[float]) -> tuple[int | None, float | None]:
    ...
    if any(not math.isfinite(value) or value <= 0.0 for value in closes):
        return None, None
    endpoint_return = math.log(closes[1] / closes[0])
    chained_return = math.log(closes[1] / closes[0])
...
def main() -> None:
    assert consecutive([202612, 202701])
    assert not consecutive([202611, 202701])
    ...
    assert signal([100.0, math.nan])[0] is None
    assert signal([100.0, 0.0])[0] is None

    direction, endpoint = signal([92.0, 104.0])
    assert direction == 1 and endpoint is not None
    assert abs(endpoint - math.log(104.0 / 92.0)) <= 1.0e-12
