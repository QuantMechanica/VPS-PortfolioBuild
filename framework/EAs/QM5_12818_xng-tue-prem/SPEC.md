# QM5_12818_xng-tue-prem - Strategy Spec
...
sleeve on `XNGUSD.DWX`. On each new D1 bar, it permits a long entry only when
the current broker-calendar day is Tuesday. The position is flattened on the
first subsequent non-Tuesday D1 bar or by a one-calendar-day stale-position
...
`QM5_12806_xng-rev-weekend` trades Monday long and Friday short, while this EA
trades Tuesday long only. It also uses no RSI, storage-report timing, weather
event trigger, month-of-year window, 52-week anchor, month-open breakout,
...
- Regime preference: Natural Gas Tuesday calendar-premium seasonality.
