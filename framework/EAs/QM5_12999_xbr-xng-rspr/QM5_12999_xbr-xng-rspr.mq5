...
#property description "QM5_12999 XBR XNG Return Spread Reversion"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_12999 - XBR/XNG Return-Spread Reversion
// -----------------------------------------------------------------------------
// D1 two-leg energy basket:
//   rspread = log(XBR[t] / XBR[t-L]) - beta * log(XNG[t] / XNG[t-L])
//   z > entry: short return spread = sell Brent, buy natural gas
//   z < -entry: long return spread = buy Brent, sell natural gas
...
input double strategy_entry_z            = 1.8;
   ...
   for(int i = 0; i < z_lookback; ++i)
     {
      ...
      if(xbr[i] <= 0.0 || xng[i] <= 0.0 || xbr[past_idx] <= 0.0 || xng[past_idx] <= 0.0)
         return false;

      const double xbr_ret = MathLog(xbr[i] / xbr[past_idx]);
      const double xng_ret = MathLog(xng[i] / xng[past_idx]);
      spreads[i] = xbr_ret - strategy_beta * xng_ret;
...
bool Strategy_OpenPair(const int spread_direction)
  {
   ...
   const string reason = long_spread ? "QM5_12999_LONG_XBR_XNG_RSPREAD"
                                     : "QM5_12999_SHORT_XBR_XNG_RSPREAD";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12999_RSPREAD_HOST";
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_12999\",\"ea\":\"xbr-xng-rspr\"}");
