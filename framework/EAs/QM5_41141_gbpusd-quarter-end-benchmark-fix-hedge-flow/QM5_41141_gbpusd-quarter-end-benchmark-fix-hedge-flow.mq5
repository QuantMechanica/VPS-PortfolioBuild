...
#property description "QM5_41141 GBPUSD quarter-end benchmark-fix hedge flow"

#include <QM/QM_Common.mqh>
#include <QM/QM_LondonCalendars.mqh>

// Mechanical scope from the APPROVED card: on the last London business day of
// each calendar quarter, use the sign of completed UK100.DWX month-to-date
// return at 14:00 London to hold GBPUSD.DWX into (but never beyond) the 16:00
// London fix.

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 41141;
...
const string STRATEGY_SIGNAL_SYMBOL = "UK100.DWX";
...
string g_strategy_symbols[2] = {"GBPUSD.DWX", "UK100.DWX"};
...
bool Strategy_IsLastLondonBusinessDay(const int date_key)
  {
   ...
   if(!QM_LondonCalendarDateKeyParts(date_key, today) ||
      (today.mon != 3 && today.mon != 6 &&
       today.mon != 9 && today.mon != 12) ||
...
string Strategy_ConsumedGlobalName()
  {
   return StringFormat("QM5_41141_QUARTEREND_FIX_%d", QM_FrameworkMagic());
  }
...
bool Strategy_LastLondonSessionBefore(const int current_month_start_key,
   ...
   for(int step = 0; step < 16; ++step)
     {
      ...
      const QM_LondonLseCashSessionType session_type =
         QM_LondonLseCashCalendarClassify(candidate);
      ...
      if(session_type == QM_LONDON_LSE_CASH_INVALID ||
         session_type == QM_LONDON_LSE_CASH_OUT_OF_COVERAGE)
         return false;
...
bool Strategy_Uk100MonthToDateReturn(const int london_date_key,
                                     ...
                                     double &return_value)
  {
   ...
   if(current_month_start_key <= 0 || previous_month_start_key <= 0 ||
      !Strategy_LastLondonSessionBefore(current_month_start_key,
                                        expected_prior_session_key))
      return false;

   datetime previous_month_start_utc = 0;
   datetime current_month_start_utc = 0;
   if(!QM_LondonCalendarLondonLocalToUTC(previous_month_start_key, 0, 0,
                                         previous_month_start_utc) ||
      !QM_LondonCalendarLondonLocalToUTC(current_month_start_key, 0, 0,
   ...
   const int actual_prior_session_key =
      QM_LondonCalendarDateKeyFromUTC(
...
bool Strategy_NoTradeFilter()
  {
   return (_Symbol != "GBPUSD.DWX" || _Period != PERIOD_M15);
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(!Strategy_Uk100MonthToDateReturn(date_key, entry_utc, mtd_return))
      return false;
   if(mtd_return == 0.0)
     {
      Strategy_ConsumeBeforeSubmission(date_key);
   ...
   req.sl = stop;
   req.tp = 0.0;
   req.reason = StringFormat("QUARTER_END_FIX_MTD_%+.6f", mtd_return);
   req.symbol_slot = 0;
...
int OnInit()
  {
   if(qm_ea_id != 41141 || qm_magic_slot_offset != 0 ||
      ...
      strategy_entry_lead_minutes != 120 || strategy_atr_period_h1 != 14 ||
      MathAbs(strategy_hard_stop_atr - 2.0) > 1e-9 ||
      _Symbol != "GBPUSD.DWX" || _Period != PERIOD_M15)
   ...
   if(!QM_LondonPublicHolidayCalendarLoad() ||
      !QM_LondonLseCashCalendarLoad() ||
      !SymbolSelect(STRATEGY_SIGNAL_SYMBOL, true))
     {
      QM_LogEvent(QM_ERROR, "SETUP_DATA_MISSING",
                  StringFormat("{\"london_public_calendar\":\"%s\",\"london_lse_calendar\":\"%s\",\"signal_symbol\":\"%s\"}",
   ...
   QM_BasketWarmupHistory(g_strategy_symbols, PERIOD_M15,
                          STRATEGY_WARMUP_BARS);
   Strategy_LoadConsumedState();
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41141_gbpusd-quarter-end-benchmark-fix-hedge-flow\"}");
