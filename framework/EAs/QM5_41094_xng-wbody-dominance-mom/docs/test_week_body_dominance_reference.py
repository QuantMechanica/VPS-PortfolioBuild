"""Deterministic reference checks for QM5_41094 weekly body dominance."""
...
def body_dominance_signal(
    ...
    """Mirror the bounded one-week OHLC and strict 3*body > 2*range rule."""
    ...
    dominant = 3 * week_body > 2 * week_range
    direction = 0
    if dominant and week_close > week_open:
        direction = 1
    elif dominant and week_close < week_open:
        direction = -1
    return (
        True,
        direction,
        len(bucket),
        (*values, week_range, week_body),
        dominant,
    )
...
def sample(
    completed: tuple[float, float, float, float] = (92.0, 120.0, 90.0, 116.0),
    count: int = 5,
...
class WeekBodyDominanceReferenceTest(unittest.TestCase):
    def signal(self, bars: list[Bar], offset: timedelta = timedelta(0)):
        return body_dominance_signal(20260824, bars, offset)
    ...
    def test_strict_long_and_short_paths(self) -> None:
        ...
        self.assertEqual(values, (92.0, 120.0, 90.0, 116.0, 30.0, 24.0))

        _, _, bars = sample(completed=(118.0, 120.0, 90.0, 94.0))
    ...
    def test_threshold_equality_subthreshold_and_body_equality_are_flat(self) -> None:
        cases = (
            ((100.0, 120.0, 90.0, 120.0), 30.0, 20.0),
            ((100.0, 120.0, 90.0, 119.0), 30.0, 19.0),
            ((105.0, 120.0, 90.0, 105.0), 30.0, 0.0),
        )
    ...
    def test_static_build_contract_is_fixed_risk_and_completed_data_only(self) -> None:
        source = (EA_DIR / "QM5_41094_xng-wbody-dominance-mom.mq5").read_text(
            encoding="utf-8"
        )
        setfile = (
            EA_DIR
            / "sets"
            / "QM5_41094_xng-wbody-dominance-mom_XNGUSD.DWX_D1_backtest.set"
        ...
        self.assertLess(
            ...
            source.index("Strategy_LoadBodyDominanceSignal(g_decision_week_key"),
        )
        for marker in (
            "qm_ea_id=41094",
            "RISK_FIXED=1000",
