...
#property description "QM5_13024 AUDCAD GBPAUD Cointegration"
...
input double strategy_beta              = -0.4667;
...
string   g_leg_gbpaud = "GBPAUD.DWX";
...
bool Strategy_IsHostSymbol()
  {
   return (_Symbol == g_leg_audcad || _Symbol == g_leg_gbpaud);
  }
...
bool Strategy_EnsureBasketScope()
  {
   ...
   string allowed[4] = {"AUDCAD.DWX", "GBPAUD.DWX", "USDCAD.DWX", "AUDUSD.DWX"};
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(!Strategy_EnsureBasketScope())
      return false;
   if(!QM_SymbolAssertOrLog(g_leg_audcad) || !QM_SymbolAssertOrLog(g_leg_gbpaud))
      return false;

   double audcad[];
   double gbpaud[];
   ArraySetAsSeries(audcad, true);
   ArraySetAsSeries(gbpaud, true);
   if(CopyClose(g_leg_audcad, PERIOD_D1, 1, lookback, audcad) != lookback) // perf-allowed: Strategy_EntrySignal is called only after the framework QM_IsNewBar gate.
      return false;
   if(CopyClose(g_leg_gbpaud, PERIOD_D1, 1, lookback, gbpaud) != lookback) // perf-allowed: Strategy_EntrySignal is called only after the framework QM_IsNewBar gate.
      return false;
   ...
   for(int i = 0; i < lookback; ++i)
     {
      if(audcad[i] <= 0.0 || gbpaud[i] <= 0.0)
         return false;
      spreads[i] = MathLog(audcad[i]) - strategy_beta * MathLog(gbpaud[i]);
...
bool Strategy_OpenPair(const int spread_direction)
  {
   ...
   const double audcad_weight = 1.0;
   const double gbpaud_weight = MathAbs(strategy_beta);
   const double weight_sum = audcad_weight + gbpaud_weight;
   if(weight_sum <= 0.0)
      return false;

   const bool long_spread = (spread_direction > 0);
   const QM_OrderType audcad_type = long_spread ? QM_BUY : QM_SELL;
   const QM_OrderType gbpaud_type = long_spread
                                    ? (strategy_beta >= 0.0 ? QM_SELL : QM_BUY)
                                    : (strategy_beta >= 0.0 ? QM_BUY : QM_SELL);
   const string reason = long_spread ? "QM5_13024_LONG_SPREAD_Z_LT_NEG_ENTRY"
                                     : "QM5_13024_SHORT_SPREAD_Z_GT_POS_ENTRY";

   bool audcad_ok = Strategy_OpenLeg(g_leg_audcad, audcad_type, audcad_weight, weight_sum, reason);
   bool gbpaud_ok = Strategy_OpenLeg(g_leg_gbpaud, gbpaud_type, gbpaud_weight, weight_sum, reason);
   if(audcad_ok && gbpaud_ok)
     {
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_13024_PAIR_HOST";
   ...
   if(qm_news_temporal != QM_NEWS_TEMPORAL_OFF || qm_news_compliance != QM_NEWS_COMPLIANCE_NONE)
     {
      if(!QM_NewsAllowsTrade2(g_leg_audcad, broker_time, qm_news_temporal, qm_news_compliance))
         return true;
      if(!QM_NewsAllowsTrade2(g_leg_gbpaud, broker_time, qm_news_temporal, qm_news_compliance))
         return true;
     }
   else
     {
      if(!QM_NewsAllowsTrade(g_leg_audcad, broker_time, qm_news_mode_legacy))
         return true;
      if(!QM_NewsAllowsTrade(g_leg_gbpaud, broker_time, qm_news_mode_legacy))
         return true;
     }
   return false;
  }
...
int OnInit()
  {
   ...
   SymbolSelect(g_leg_gbpaud, true);
