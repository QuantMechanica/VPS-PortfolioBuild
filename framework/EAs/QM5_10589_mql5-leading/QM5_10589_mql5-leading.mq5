...
bool Strategy_LeadingCrossSignal(int &signal)
  {
   signal = 0;
   if(strategy_calc_bars < 8 ||
      strategy_leading_alpha1 <= 0.0 ||
      strategy_leading_alpha2 <= 0.0 ||
      strategy_leading_alpha2 > 1.0)
      return false;

   MqlRates rates[];
   ArraySetAsSeries(rates, false);
   // Called only from Strategy_EntrySignal, which the framework gates by QM_IsNewBar().
   ...
   for(int i = 0; i < copied; ++i)
     {
      ...
      net_lead[i] = strategy_leading_alpha2 * lead
                    + (1.0 - strategy_leading_alpha2) * net_lead[i - 1];
      ema[i] = 0.5 * price + 0.5 * ema[i - 1];
     }

   const int now = copied - 1;
   const int prev = copied - 2;
   if(net_lead[prev] <= ema[prev] && net_lead[now] > ema[now])
      signal = 1;
   else if(net_lead[prev] >= ema[prev] && net_lead[now] < ema[now])
      signal = -1;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   int signal = 0;
   if(!Strategy_LeadingCrossSignal(signal) || signal == 0)
      return false;
...
bool Strategy_ExitSignal()
  {
   // Opposite Leading crosses are handled inside Strategy_EntrySignal under the
   // framework closed-bar gate so this hook stays O(1) on the per-tick path.
   return false;
  }
