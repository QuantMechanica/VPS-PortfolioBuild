...
#property description "QM5_41454 XAU/XAG Weekly NR2 Body Momentum"
...
// -----------------------------------------------------------------------------
// Reconstruct two synchronized completed broker weeks of XAU/XAG log-ratio
// closes. Trade only when the newest ratio-close range is strictly narrower than
// the prior range and the newest completed ratio-week body is strictly nonzero.
// Continue that body through an equal-notional opposite-leg package, consume
// one attempt per week, and flatten at the next week boundary.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41454;
...
input double strategy_body_epsilon            = 0.0000000001;
...
input int    strategy_host_max_spread_points  = 1500;
...
double   g_week_ratio_min[2];
double   g_week_ratio_max[2];
double   g_week_ratio_final[2]; // newest through prior completed week.
double   g_week_ratio_first[2]; // chronological first close in each week.
double   g_week_ratio_range[2];
int      g_week_key[2];
int      g_week_sessions[2];
double   g_newest_week_body = 0.0;
bool     g_range_contracted = false;
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41454 && qm_magic_slot_offset == 0 &&
            g_leg_xau != "" && g_leg_xag != "" &&
            g_leg_xau != g_leg_xag &&
            ...
            MathAbs(strategy_body_epsilon - 1.0e-10) <= 1.0e-15 &&
...
bool Strategy_SpreadAllowed(const string symbol)
  {
   ...
   if(symbol == g_leg_xau)
      return (spread_points <= (double)strategy_host_max_spread_points);
   if(symbol == g_leg_xag)
      return (spread_points <= (double)strategy_companion_max_spread_points);
   return false;
  }
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_NR2_BODY_MOM_ATTEMPT_WEEK", qm_ea_id);
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41454_XAUXAG_NR2_BODY_MOM";
...
bool Strategy_LoadNr2Body(const int current_week,
                         int &direction)
  {
   direction = 0;
   g_newest_week_body = 0.0;
   g_range_contracted = false;
   for(int slot = 0; slot < strategy_required_weeks; ++slot)
     {
      g_week_ratio_min[slot] = DBL_MAX;
      g_week_ratio_max[slot] = -DBL_MAX;
      g_week_ratio_final[slot] = 0.0;
      g_week_ratio_first[slot] = 0.0;
      g_week_ratio_range[slot] = 0.0;
      g_week_key[slot] = 0;
   ...
   const int xau_copied =
      CopyRates(g_leg_xau, // perf-allowed: decision-only bounded completed-week ratio scan.
                PERIOD_D1, 1, strategy_history_bars_d1, xau_bars);
   const int xag_copied =
      CopyRates(g_leg_xag, // perf-allowed: decision-only bounded completed-week ratio scan.
   ...
   for(int index = 0; index < strategy_history_bars_d1; ++index)
     {
      ...
      int slot = -1;
      for(int known = 0; known < strategy_required_weeks; ++known)
        {
      ...
      if(slot < 0)
        {
         int empty_slot = -1;
         for(int candidate = 0;
             candidate < strategy_required_weeks;
             ++candidate)
           {
            if(g_week_key[candidate] == 0)
              {
         ...
         const int expected_next =
            (empty_slot == 0) ? current_week : g_week_key[empty_slot - 1];
         if(Strategy_NextWeekKey(week_key) != expected_next)
            return false;
         g_week_key[empty_slot] = week_key;
         slot = empty_slot;
        }
      ...
      if(xau_close <= 0.0 || xag_close <= 0.0 ||
         !MathIsValidNumber(xau_close) ||
         !MathIsValidNumber(xag_close))
         return false;
      const double ratio_close = MathLog(xau_close) - MathLog(xag_close);
      if(!MathIsValidNumber(ratio_close))
         return false;

      if(g_week_sessions[slot] == 0)
         g_week_ratio_final[slot] = ratio_close;
      g_week_ratio_first[slot] = ratio_close;
      g_week_ratio_min[slot] = MathMin(g_week_ratio_min[slot], ratio_close);
      g_week_ratio_max[slot] = MathMax(g_week_ratio_max[slot], ratio_close);
   ...
   for(int slot = 0; slot < strategy_required_weeks; ++slot)
     {
      const int expected_next =
         (slot == 0) ? current_week : g_week_key[slot - 1];
      if(g_week_key[slot] <= 0 ||
         Strategy_NextWeekKey(g_week_key[slot]) != expected_next ||
         g_week_sessions[slot] < strategy_min_sessions_per_week ||
         g_week_sessions[slot] > strategy_max_sessions_per_week ||
         g_week_ratio_min[slot] == DBL_MAX ||
         g_week_ratio_max[slot] == -DBL_MAX ||
         !MathIsValidNumber(g_week_ratio_min[slot]) ||
         !MathIsValidNumber(g_week_ratio_max[slot]) ||
         !MathIsValidNumber(g_week_ratio_final[slot]) ||
         !MathIsValidNumber(g_week_ratio_first[slot]))
         return false;
      g_week_ratio_range[slot] =
         g_week_ratio_max[slot] - g_week_ratio_min[slot];
      if(g_week_ratio_range[slot] <= 0.0 ||
         !MathIsValidNumber(g_week_ratio_range[slot]))
         return false;
     }

   g_range_contracted =
      (g_week_ratio_range[0] < g_week_ratio_range[1]);
   if(!g_range_contracted)
      return true;

   g_newest_week_body =
      g_week_ratio_final[0] - g_week_ratio_first[0];
   if(!MathIsValidNumber(g_newest_week_body))
      return false;

   if(g_newest_week_body > strategy_body_epsilon)
      direction = 1;  // Positive ratio body: buy XAU, sell XAG.
   else if(g_newest_week_body < -strategy_body_epsilon)
      direction = -1; // Negative ratio body: sell XAU, buy XAG.
   return true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41454_XAUXAG_NR2_BODY_MOM_HOST";
   ...
   for(int slot = 0; slot < strategy_required_weeks; ++slot)
     {
      g_week_ratio_min[slot] = 0.0;
      g_week_ratio_max[slot] = 0.0;
      g_week_ratio_final[slot] = 0.0;
      g_week_ratio_first[slot] = 0.0;
      g_week_ratio_range[slot] = 0.0;
      g_week_key[slot] = 0;
   ...
   if(Strategy_OpenOwnedPositionCount() > 0 ||
     ...
     {
      signal_valid = Strategy_LoadNr2Body(g_signal_week_key, direction);
      if(!signal_valid)
         g_signal_state = "ratio_week_validation_failed";
      else if(direction > 0)
         g_signal_state = "positive_ratio_body_continued";
      else if(direction < 0)
         g_signal_state = "negative_ratio_body_continued";
      else if(!g_range_contracted)
         g_signal_state = "range_not_contracted_flat";
      else
         g_signal_state = "ratio_body_tied_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"week\":%d,\"decision_bar\":%I64d,\"late\":%s,\"valid\":%s,\"direction\":%d,\"prior_range\":%.12e,\"newest_range\":%.12e,\"newest_body\":%.12e,\"oldest_week\":%d,\"newest_week\":%d,\"oldest_sessions\":%d,\"newest_sessions\":%d,\"state\":\"%s\"}",
                            g_signal_week_key,
                            (long)g_current_host_bar,
                            g_late_decision ? "true" : "false",
                            signal_valid ? "true" : "false",
                            direction,
                            g_week_ratio_range[1],
                            g_week_ratio_range[0],
                            g_newest_week_body,
                            g_week_key[1],
                            g_week_key[0],
                            g_week_sessions[1],
...
int OnInit()
  {
   g_leg_xau = (strategy_host_symbol == "") ? _Symbol
                                             : strategy_host_symbol;
   g_leg_xag = strategy_companion_symbol;
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG weekly ratio range-contraction body continuation package exits at the next broker week"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41454\",\"ea\":\"xauxag-nr2-body-mom\"}");
