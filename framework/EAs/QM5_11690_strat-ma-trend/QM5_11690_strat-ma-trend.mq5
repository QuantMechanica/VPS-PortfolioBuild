...
#property description "QM5_11690 strat-ma-trend - Stratestic single-MA trend"
...
input bool   strategy_use_ema           = false;  // false = card seed SMA, true = static P3 EMA axis
...
input double strategy_sl_atr_mult       = 2.0;    // V5 catastrophic stop distance
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   if(strategy_ma_period <= 1 || strategy_atr_period <= 0 || strategy_sl_atr_mult <= 0.0)
      return false;
   ...
   const double stop_price = QM_StopATR(_Symbol, order_type, entry_price, strategy_atr_period, strategy_sl_atr_mult);
   if(stop_price <= 0.0)
      return false;
   ...
   req.sl = stop_price;
   req.tp = 0.0;
   req.reason = (direction > 0) ? "close_above_ma" : "close_below_ma";
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      if(position_type == POSITION_TYPE_BUY && close_value < ma_value)
         return true;
      if(position_type == POSITION_TYPE_SELL && close_value > ma_value)
         return true;
     }
