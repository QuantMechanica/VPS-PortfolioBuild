...
input int             strategy_cci_period      = 50;
...
input double          strategy_structure_cap_r = 2.5;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   const double atr_stop_dist = atr_1 * strategy_atr_sl_mult;
   double sl = QM_StopATRFromValue(_Symbol, side, entry, atr_1, strategy_atr_sl_mult);
   const double structure_sl = QM_StopStructure(_Symbol, side, entry, strategy_swing_lookback);
   if(structure_sl > 0.0)
     {
      const double structure_dist = MathAbs(entry - structure_sl);
      if(structure_dist > 0.0 &&
         structure_dist < atr_stop_dist &&
         structure_dist <= atr_stop_dist * strategy_structure_cap_r)
         sl = structure_sl;
     }
   if(sl <= 0.0)
      return false;
   ...
   req.tp = NormalizeDouble(long_signal ? (entry + sl_dist * strategy_tp_rr)
                                        : (entry - sl_dist * strategy_tp_rr), _Digits);
   req.reason = long_signal ? "CCI_MACD_EMA_LONG" : "CCI_MACD_EMA_SHORT";
   req.symbol_slot = qm_magic_slot_offset;
   req.expiration_seconds = 0;
   return true;
  }
