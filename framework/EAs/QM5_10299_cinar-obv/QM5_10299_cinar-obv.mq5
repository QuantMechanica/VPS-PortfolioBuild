...
#property description "QM5_10299 Cinar OBV SMA Cross"
...
input int    strategy_obv_sma_period    = 10;
...
bool Strategy_ObvVsSma(const int period, double &out_obv, double &out_sma)
  {
   out_obv = 0.0;
   out_sma = 0.0;
   if(period <= 1)
   ...
   const int copied = CopyRates(_Symbol, strategy_signal_tf, 1, period + 1, rates); // perf-allowed: bespoke closed-bar OBV/SMA window; caller is QM_IsNewBar-gated.
   if(copied < period + 1)
      return false;

   double obv_values[];
   ArrayResize(obv_values, period);

   double running_obv = 0.0;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   // Trade Entry: card requires D1 OBV above/below SMA(OBV, 10).
   if(_Period != strategy_signal_tf)
      return false;
   if(strategy_obv_sma_period <= 1 || strategy_atr_period <= 0 || strategy_atr_sl_mult <= 0.0)
      return false;

   double obv = 0.0;
   double obv_sma = 0.0;
   if(!Strategy_ObvVsSma(strategy_obv_sma_period, obv, obv_sma))
      return false;
   if(obv == obv_sma)
      return false;

   const QM_OrderType desired_side = (obv > obv_sma) ? QM_BUY : QM_SELL;
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      // Trade Close: stop-and-reverse on OBV/SMA relation flip.
   ...
   req.sl = sl;
   req.tp = 0.0;
   req.reason = (desired_side == QM_BUY) ? "OBV_GT_SMA_LONG" : "OBV_LT_SMA_SHORT";
   req.symbol_slot = qm_magic_slot_offset;
   req.expiration_seconds = 0;
   return true;
  }
