# QM5_10299_cinar-obv - Strategy Spec

**EA ID:** QM5_10299
**Slug:** `cinar-obv`
**Source:** `1b906e79-c619-5a61-90db-ee19ac95a19f` (see `artifacts/cards_approved/QM5_10299_cinar-obv.md`)
...
The EA computes On-Balance Volume from closed D1 close and MT5 tick-volume data. It then computes a 10-period simple moving average of OBV. It opens long when OBV is above its SMA and opens short when OBV is below its SMA. If an opposite position is already open, the EA closes it and opens the new direction on the same closed-bar evaluation.
...
|---|---|---|---|
| `strategy_signal_tf` | `PERIOD_D1` | MT5 timeframe enum | Timeframe used for OBV and ATR signal reads. |
| `strategy_obv_sma_period` | `10` | `2+` | Simple moving average period applied to OBV. |
...
- `NDX.DWX` - matrix-covered Nasdaq 100 index CFD with OHLC and tick-volume history.
...
- `WS30.DWX` - matrix-covered Dow 30 index CFD with OHLC and tick-volume history.
...
| Typical hold time | not specified in card frontmatter; expected to last until the opposite OBV/SMA relation or ATR stop |
| Expected drawdown profile | not specified in card frontmatter; catastrophic stop is 2.0 * ATR(14) |
| Regime preference | volume-confirmed trend / stop-and-reverse |
...
**Pointer:** `https://github.com/cinar/indicator/blob/master/strategy/volume/obv_strategy.go` and `https://github.com/cinar/indicator/blob/master/volume/obv.go`
**R1-R4 verdict (Q00):** all R1-R4 PASS per `artifacts/cards_approved/QM5_10299_cinar-obv.md`
...
|---|---|---|---|
| v1 | 2026-06-12 | Initial build from card | 2e864add-1a60-443d-b5ff-0059b41123cf |
