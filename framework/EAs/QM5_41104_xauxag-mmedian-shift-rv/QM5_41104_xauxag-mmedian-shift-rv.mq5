...
#property description "QM5_41104 XAU/XAG Monthly Ratio-Median Shift Reversion"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_41104 - XAU/XAG Monthly Ratio-Median Shift Reversion
// -----------------------------------------------------------------------------
// At the first synchronized D1 boundary of a broker month, reconstruct one
// ordinary sample median from the daily-close XAU/XAG log ratios in each of
// the two immediately completed calendar months. Fade the strict location
// shift with opposite equal-notional legs, and hold to the next month.
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41104 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_MMEDIAN_SHIFT_RV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41104_XAUXAG_MMEDIAN_SHIFT_RV";
...
bool Strategy_LoadMonthlyRatioMedians(
      ...
      int &direction)
  {
   ...
   MqlRates xag_bars[];
   double new_ratios[];
   double parent_ratios[];
   ArraySetAsSeries(xau_bars, true);
   ArraySetAsSeries(xag_bars, true);
   if(ArrayResize(new_ratios, strategy_max_month_sessions) !=
      strategy_max_month_sessions ||
      ArrayResize(parent_ratios, strategy_max_month_sessions) !=
      strategy_max_month_sessions)
      return false;
   ...
   if(!Strategy_OrdinarySampleMedian(new_ratios,
                                     new_month_sessions,
                                     new_median) ||
      !Strategy_OrdinarySampleMedian(parent_ratios,
                                     parent_month_sessions,
                                     parent_median))
      return false;

   // direction > 0 means BUY XAU / SELL XAG. Fade the strict displacement
   // between the two independent completed-month ratio medians.
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41104_XAUXAG_MMEDIAN_SHIFT_RV_HOST";
   ...
   const bool valid =
      Strategy_LoadMonthlyRatioMedians(g_signal_month_key,
                                       new_month_sessions,
                                       parent_month_sessions,
                                       new_median,
                                       parent_median,
                                       direction);
   string state = "monthly_ratio_median_failed";
   if(valid && direction > 0)
      state = "median_shifted_down_long_xau";
   else if(valid && direction < 0)
      state = "median_shifted_up_short_xau";
   else if(valid)
      state = "equal_medians_flat";

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"new_month_sessions\":%d,\"parent_month_sessions\":%d,\"new_median\":%.12e,\"parent_median\":%.12e,\"direction\":%d,\"state\":\"%s\"}",
                            g_signal_month_key,
                            new_month_sessions,
                            parent_month_sessions,
                            new_median,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG monthly ratio-median shift-reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41104\",\"ea\":\"xauxag-mmedian-shift-rv\"}");
