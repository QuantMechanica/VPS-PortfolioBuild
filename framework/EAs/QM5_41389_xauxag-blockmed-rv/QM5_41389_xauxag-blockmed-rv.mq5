...
#property description "QM5_41389 XAU/XAG Monthly Chronological Block-Median Reversion"
...
// completed broker months. Form twelve adjacent gold-minus-silver log-ratio
// changes, partition them chronologically into four non-overlapping blocks of
// three, sort only the four block means, and average the two inner values.
// Fade that sign with an opposed, equal-target-notional package held to the
// next month. Runtime is deterministic, completed-price-only, and one
// consumed attempt per month.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41389;
...
input int    strategy_block_count             = 4;
input int    strategy_block_width             = 3;
input int    strategy_block_divisor           = 3;
input int    strategy_median_divisor           = 2;
input double strategy_signal_epsilon          = 0.000000000001;
...
bool Strategy_InputsValid()
  {
   return (Strategy_DoubleInputsFinite() &&
            ...
            g_leg_xau != g_leg_xag &&
            qm_ea_id == 41389 && qm_magic_slot_offset == 0 &&
            strategy_xag_symbol == "XAGUSD.DWX" &&
            strategy_endpoint_count == 13 &&
            strategy_return_count == 12 &&
            strategy_block_count == 4 &&
            strategy_block_width == 3 &&
            strategy_block_divisor == 3 &&
            strategy_median_divisor == 2 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_BLOCKMED_RV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41389_XAUXAG_BLOCKMED_RV";
...
bool Strategy_Sort4(double &values[])
  {
   if(ArraySize(values) != strategy_block_count ||
      strategy_block_count != 4)
      return false;
   for(int index = 1; index < strategy_block_count; ++index)
     {
   ...
   for(int index = 0; index < strategy_block_count; ++index)
      if(!MathIsValidNumber(values[index]))
         return false;
   return true;
  }

bool Strategy_BlockMedian(const double &returns[],
                          double &location,
                          string &block_path)
  {
   location = 0.0;
   block_path = "";
   if(ArraySize(returns) != 12 ||
      strategy_return_count != 12 ||
      strategy_block_count != 4 ||
      strategy_block_width != 3 ||
      strategy_block_divisor != 3 ||
      strategy_median_divisor != 2 ||
      strategy_block_count * strategy_block_width != strategy_return_count)
      return false;

   double block_means[4];
   for(int block = 0; block < strategy_block_count; ++block)
     {
      double block_sum = 0.0;
      for(int offset = 0; offset < strategy_block_width; ++offset)
        {
         const int index = block * strategy_block_width + offset;
         if(index < 0 || index >= ArraySize(returns) ||
            !MathIsValidNumber(returns[index]))
            return false;
         block_sum += returns[index];
        }
      block_means[block] = block_sum / (double)strategy_block_divisor;
      if(!MathIsValidNumber(block_means[block]))
         return false;
      if(StringLen(block_path) > 0)
         block_path += ",";
      block_path += DoubleToString(block_means[block], 12);
     }

   double sorted[4];
   ArrayCopy(sorted, block_means);
   if(!Strategy_Sort4(sorted))
      return false;
   location = (sorted[1] + sorted[2]) /
              (double)strategy_median_divisor;
   return MathIsValidNumber(location);
  }

bool Strategy_LoadMonthlyBlockMedianSignal(
      ...
      int &direction)
  {
   ...
   if(StringLen(return_path) <= 0)
      return false;
   if(!Strategy_BlockMedian(relative_returns, location, block_path))
      return false;

   // direction > 0 means BUY XAU / SELL XAG. Fade only a robust location
   // outside the locked strict epsilon; magnitude never scales risk.
   if(location > strategy_signal_epsilon)
      direction = -1;
   else if(location < -strategy_signal_epsilon)
      direction = 1;
   return true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41389_XAUXAG_BLOCKMED_RV_HOST";
   ...
   string block_path = "";
   const bool valid =
      Strategy_LoadMonthlyBlockMedianSignal(
         g_signal_month_key,
         month_count,
   ...
   string state = "monthly_block_median_signal_failed";
   if(valid && direction > 0)
      state = "negative_block_median_fade_long_xau";
   else if(valid && direction < 0)
      state = "positive_block_median_fade_short_xau";
   else if(valid && MathAbs(location) <= strategy_signal_epsilon)
      state = "block_median_inside_epsilon_flat";
   else if(valid)
      state = "nonqualifying_block_median_flat";

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"month_count\":%d,\"newest_endpoint_time\":%I64d,\"block_count\":%d,\"block_width\":%d,\"block_median\":%.12e,\"direction\":%d,\"ratios\":\"%s\",\"returns\":\"%s\",\"blocks\":\"%s\",\"state\":\"%s\"}",
                            g_signal_month_key,
                            month_count,
                            (long)newest_endpoint_time,
                            strategy_block_count,
                            strategy_block_width,
                            location,
                            direction,
                            ratio_path,
                            return_path,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG monthly chronological block-median reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41389\",\"ea\":\"xauxag-blockmed-rv\"}");
