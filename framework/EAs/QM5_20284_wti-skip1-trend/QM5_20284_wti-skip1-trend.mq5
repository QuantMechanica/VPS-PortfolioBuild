...
#property description "QM5_20284 WTI Skip-One-Month Trend"
...
//   1. derive fourteen consecutive completed WTI month-end closes from D1 data;
//   2. validate but exclude the newest completed monthly return;
//   3. form the exact twelve-month return ending one month before the decision;
//   4. trade the sign of that delayed return for one monthly package.
// The skip-one construction is a transparent QM hypothesis, not an author rule.
// Runtime uses native XTIUSD.DWX price and broker data only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 20284;
...
input int    strategy_skip_months         = 1;
...
double g_cache_skipped_return   = 0.0;
...
bool Strategy_LoadMonthlyCloses(double &closes[])
  {
   ...
   if(strategy_trend_months != 12 ||
      strategy_skip_months != 1 ||
      strategy_history_bars != 500)
      return false;

   MqlRates rates[];
   ArraySetAsSeries(rates, false);
   const int copied = CopyRates(_Symbol, PERIOD_D1, 1, strategy_history_bars, rates); // perf-allowed: bounded bulk read reached only from the new-month D1 gate.
   if(copied <= 0)
      return false;
   ...
   const int required_closes =
      strategy_trend_months + strategy_skip_months + 1;
   if(required_closes != 14 || month_count < required_closes)
      return false;

   const int first = month_count - required_closes;
   if(!Strategy_AreConsecutiveMonths(month_keys, first, required_closes))
      return false;
   for(int i = first + 1; i < month_count; ++i)
   ...
   for(int i = 0; i < required_closes; ++i)
      closes[i] = month_end_closes[first + i];
   return true;
  }
...
void Strategy_ResetCachedState()
  {
   ...
   g_cache_skipped_return = 0.0;
   g_cache_state_reason = "not_evaluated";
  }

bool Strategy_DelayedTrendSignal(const double &closes[], int &signal)
  {
   signal = 0;
   const int required_closes =
      strategy_trend_months + strategy_skip_months + 1;
   if(strategy_trend_months != 12 ||
      strategy_skip_months != 1 ||
      required_closes != 14 ||
      ArraySize(closes) != required_closes)
     {
      g_cache_state_reason = "bad_skip1_contract";
      return false;
     }
   ...
   const int trend_end = required_closes - strategy_skip_months - 1;
   const int newest = required_closes - 1;
   if(trend_end != 12 || newest != 13)
     {
      g_cache_state_reason = "bad_endpoint_indexes";
      return false;
     }

   const double trend_return = MathLog(closes[trend_end] / closes[0]);
   const double skipped_return =
      MathLog(closes[newest] / closes[trend_end]);
   if(!MathIsValidNumber(trend_return) ||
      !MathIsValidNumber(skipped_return))
     {
      g_cache_state_reason = "invalid_return_state";
      return false;
     }

   g_cache_trend_return = trend_return;
   g_cache_skipped_return = skipped_return;
   if(trend_return == 0.0)
     {
      g_cache_state_reason = "exact_zero_delayed_trend";
      return true;
     }
...
bool Strategy_LoadSignalState(int &signal)
  {
   ...
   return Strategy_DelayedTrendSignal(monthly_closes, signal);
  }
...
void Strategy_PrepareMonthlySignal()
  {
   ...
   QM_LogEvent(QM_INFO,
               "MONTHLY_STATE",
               StringFormat("{\"month\":%d,\"valid\":%s,\"signal\":%d,\"trend_return\":%.10f,\"skipped_return\":%.10f,\"state\":\"%s\"}",
                            g_cache_month_key,
                            g_cache_signal_valid ? "true" : "false",
                            g_cache_signal,
                            g_cache_trend_return,
                            g_cache_skipped_return,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 20284 ||
   ...
   if(qm_stress_reject_probability != 0.0)
      return true;
   if(strategy_trend_months != 12 || strategy_skip_months != 1)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20284_WTI_SKIP1_TREND";
   ...
   req.reason = (g_cache_signal > 0) ? "SKIP1_TREND_XTI_LONG" : "SKIP1_TREND_XTI_SHORT";
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_20284_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_20284\",\"ea\":\"wti-skip1-trend\"}");
