...
#property description "QM5_10486 MQL5 Open Close Two-Candle Reversal"
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(strategy_atr_period <= 0 || strategy_atr_sl_mult <= 0.0 || strategy_target_rr <= 0.0)
      return false;
   ...
   const double open1 = iOpen(_Symbol, strategy_work_tf, 1);
   const double open2 = iOpen(_Symbol, strategy_work_tf, 2);
   const double close1 = iClose(_Symbol, strategy_work_tf, 1);
   const double close2 = iClose(_Symbol, strategy_work_tf, 2);
   if(open1 <= 0.0 || open2 <= 0.0 || close1 <= 0.0 || close2 <= 0.0)
      return false;
   ...
   req.sl = sl;
   req.tp = tp;
   req.reason = (signal > 0) ? "OPEN_CLOSE_LONG" : "OPEN_CLOSE_SHORT";
   return true;
  }
...
bool Strategy_ExitSignal()
  {
   ...
   if(ptype == POSITION_TYPE_BUY && open1 < open2 && close1 < close2)
      return true;
   if(ptype == POSITION_TYPE_SELL && open1 > open2 && close1 > close2)
      return true;
