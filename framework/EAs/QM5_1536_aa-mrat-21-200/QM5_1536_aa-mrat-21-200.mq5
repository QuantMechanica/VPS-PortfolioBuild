...
int  g_selected_side[STRATEGY_SYMBOL_COUNT];
bool g_selection_ready = false;
int  g_active_selected_count = 0;
int  g_last_eval_year = -1;
int  g_last_eval_month = -1;
bool g_rebalance_entry_bar = false;
...
bool Strategy_IsRebalanceBar()
  {
   ...
   if(dt.year == g_last_eval_year && dt.mon == g_last_eval_month)
      return false;
   g_last_eval_year = dt.year;
...
void Strategy_ClearSelection()
  {
   ...
   g_active_selected_count = 0;
   g_rebalance_entry_bar = false;
   for(int i = 0; i < STRATEGY_SYMBOL_COUNT; ++i)
      g_selected_side[i] = 0;
  }

void Strategy_AdvanceSelection()
  {
   ...
   int long_count = 0;
   int short_count = 0;
   int candidate_side[STRATEGY_SYMBOL_COUNT];
   for(int i = 0; i < STRATEGY_SYMBOL_COUNT; ++i)
      candidate_side[i] = 0;
   ...
   for(int i = 0; i < STRATEGY_SYMBOL_COUNT; ++i)
      g_selected_side[i] = candidate_side[i];
   g_active_selected_count = long_count + short_count;
   g_selection_ready = true;
   g_rebalance_entry_bar = true;
  }
...
bool Strategy_SelectionForCurrentSymbol(QM_OrderType &side, int &active_selected_count)
  {
   ...
   if(current_index < 0)
      return false;
   if(!g_selection_ready || g_active_selected_count <= 0)
      return false;

   active_selected_count = g_active_selected_count;
   if(g_selected_side[current_index] > 0)
     {
      side = QM_BUY;
      return true;
     }
...
void OnTick()
  {
   ...
   if(nb && Strategy_IsRebalanceBar())
      Strategy_AdvanceSelection();
