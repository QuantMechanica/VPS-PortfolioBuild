...
#property description "QM5_20188 ForexFactory Range Breakout GMT+3 H1 Tuesday Off"
...
// in behaviour). This new variant inherits no prop sizing or live authority
// from QM5_9936. Any future prop phase must receive its own evidence and cap.
// The selected value is validated in OnInit before the override is applied.
...
input bool   strategy_skip_tuesday          = true;
...
int Strategy_Gmt3Weekday(const datetime broker_time)
  {
   const datetime utc = QM_BrokerToUTC(broker_time);
   const datetime gmt3 = utc + 3 * 3600;
   MqlDateTime dt;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(g_strategy_orders_day_key == day_key || g_strategy_skip_day_key == day_key)
      return false;
   // Frozen QM5_20188 variant rule. MqlDateTime uses 0=Sunday, 2=Tuesday.
   // The decision is made from the same pre-entry GMT+3 clock as the range,
   // so it cannot depend on the future close time or trade outcome.
   if(strategy_skip_tuesday && Strategy_Gmt3Weekday(now) == 2)
     {
      g_strategy_skip_day_key = day_key;
      QM_LogEvent(QM_INFO, "STRATEGY_FILTER", "{\"filter\":\"gmt3_tuesday\"}");
      return false;
     }
...
int OnInit()
  {
   ...
   if(!strategy_skip_tuesday)
     {
      QM_LogEvent(QM_ERROR, "SETUP_CONFIG_INVALID",
                  "{\"component\":\"strategy_skip_tuesday_required\"}");
      return INIT_FAILED;
     }
