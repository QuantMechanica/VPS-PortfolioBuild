...
bool Strategy_ReadClosedBar(const int shift, MqlRates &bar)
  {
   ...
   // perf-allowed: one closed bar for hammer/exit checks, called only from closed-bar logic.
...
bool Strategy_IsHammer(const MqlRates &bar)
  {
   ...
   return (lower_shadow >= body * 2.0 && upper_shadow <= body);
  }
...
bool Strategy_ReadPSAR(const int shift, double &out_psar)
  {
   ...
   // perf-allowed: bounded SAR reconstruction only while open-position exit logic is on a new bar.
