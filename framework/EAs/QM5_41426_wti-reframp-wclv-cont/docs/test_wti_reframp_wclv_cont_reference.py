"""Deterministic checks for QM5_41426 refinery-ramp weekly continuation."""
...
class WtiRefineryRampCloseLocationReferenceTest(unittest.TestCase):
    ...
    def bars(self, newest_close: float = 82.0, parent_close: float = 80.0) -> list[Bar]:
        newest = package(self.new_anchor, [79.0, 80.0, 80.5, 81.0, newest_close], 76.0, 84.0)
        parent = package(self.parent_anchor, [78.0, 78.5, 79.0, 79.5, parent_close], 75.0, 82.0)
        return newest + parent

    def test_positive_return_and_upper_tercile_is_long(self) -> None:
    ...
    def test_positive_return_below_upper_tercile_is_flat(self) -> None:
        self.assertEqual(signal(self.current, self.bars(newest_close=80.0, parent_close=79.0))[0], 0)
    ...
    def test_exact_close_location_boundary_is_flat(self) -> None:
        ...
        direction, _, close_location = signal(
            self.current, self.bars(newest_close=boundary, parent_close=79.0)
        )
    ...
    def test_three_to_five_sessions_are_accepted(self) -> None:
        for count in (3, 4, 5):
            newest = package(self.new_anchor, [78.0] * (count - 1) + [82.0], 76.0, 84.0)
