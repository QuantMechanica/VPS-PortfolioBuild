...
#property description "QM5_41426 WTI Refinery-Ramp Weekly Close-Location Continuation"
...
//   - trade only April-July refinery-utilization-ramp anchor weeks
...
//   - hold one long position until the next normalized broker week
// Runtime uses MT5-native OHLC, calendar, history, and framework state only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                      = 41426;
...
input int    strategy_ramp_month_1           = 4;
input int    strategy_ramp_month_2           = 5;
input int    strategy_ramp_month_3           = 6;
input int    strategy_ramp_month_4           = 7;
...
bool Strategy_IsRampMonth(const int month)
  {
   return (month == strategy_ramp_month_1 ||
           month == strategy_ramp_month_2 ||
           month == strategy_ramp_month_3 ||
           month == strategy_ramp_month_4);
  }
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   else if(!Strategy_IsRampMonth(anchor_month))
      g_signal_state = "outside_refinery_ramp_months";
   else
     {
      g_signal_valid =
      ...
      if(!g_signal_valid)
         ...
         g_signal_state = "positive_return_upper_tercile_long";
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(strategy_symbol == "" ||
      ...
      strategy_ramp_month_1 != 4 ||
      strategy_ramp_month_2 != 5 ||
      strategy_ramp_month_3 != 6 ||
      strategy_ramp_month_4 != 7 ||
      strategy_required_weeks != 2 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41426_WTI_REFRAMP_WCLV_CONT";
   ...
   req.type = QM_BUY;
   req.reason = "WTI_REFRAMP_WCLV_CONT_LONG";
...
int OnInit()
  {
   if(!SymbolSelect(strategy_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41426 ||
      qm_magic_slot_offset != 0)
   ...
   g_attempt_state_key =
      StringFormat("QM5_41426_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41426\",\"ea\":\"wti-reframp-wclv-cont\"}");
