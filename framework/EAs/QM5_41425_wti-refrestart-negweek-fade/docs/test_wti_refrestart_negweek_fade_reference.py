"""Deterministic checks for QM5_41425 WTI restart negative-week reversion."""
...
def restart_week(anchor: date) -> bool:
...
def negative_week_signal(current_anchor: date, newest_first: list[Bar]) -> tuple[int, float]:
    ...
    if not restart_week(current_anchor):
...
class WtiRefineryRestartNegativeWeekReferenceTest(unittest.TestCase):
    def setUp(self) -> None:
        self.current = date(2026, 5, 18)
    ...
    def test_all_restart_anchor_months_are_eligible(self) -> None:
        for value in (
            date(2026, 4, 6),
            date(2026, 5, 4),
        ):
            self.assertTrue(restart_week(value))

    def test_nonrestart_months_are_ineligible(self) -> None:
        for value in (date(2026, 3, 2), date(2026, 6, 1), date(2026, 10, 5)):
            self.assertFalse(restart_week(value))

    def test_week_crossing_into_april_uses_monday_anchor(self) -> None:
        self.assertFalse(restart_week(date(2026, 3, 30)))

    def test_week_crossing_into_june_uses_monday_anchor(self) -> None:
        self.assertTrue(restart_week(date(2026, 5, 25)))
