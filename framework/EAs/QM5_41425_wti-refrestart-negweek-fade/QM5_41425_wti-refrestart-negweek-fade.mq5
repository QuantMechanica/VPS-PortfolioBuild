...
#property description "QM5_41425 WTI Refinery-Restart Negative-Week Reversion"
...
//   - decide only in April and May after the maintenance-season peak
...
input int    strategy_restart_month_1      = 4;
input int    strategy_restart_month_2      = 5;
...
bool Strategy_IsRestartMonth(const int month)
  {
   return (month == strategy_restart_month_1 ||
           month == strategy_restart_month_2);
  }
...
bool Strategy_LoadNegativeWeekSignal(const int current_week_key,
                                     ...
                                     int &direction)
  {
   ...
   // The refinery-restart sleeve is deliberately asymmetric: a strictly negative
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   else if(!Strategy_IsRestartMonth(anchor_month))
      g_signal_state = "outside_restart_months";
   else
     {
      g_signal_valid =
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(strategy_symbol == "" ||
      ...
      strategy_restart_month_1 != 4 ||
      strategy_restart_month_2 != 5 ||
      strategy_required_weeks != 1 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41425_WTI_REFRESTART_NEGWEEK_FADE";
   ...
   req.type = QM_BUY;
   req.reason = "WTI_REFRESTART_NEGWEEK_FADE_LONG";
...
int OnInit()
  {
   if(!SymbolSelect(strategy_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41425 ||
      qm_magic_slot_offset != 0)
   ...
   g_attempt_state_key =
      StringFormat("QM5_41425_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41425\",\"ea\":\"wti-refrestart-negweek-fade\"}");
