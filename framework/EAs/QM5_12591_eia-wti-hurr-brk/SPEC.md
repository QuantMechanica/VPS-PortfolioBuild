...
| v3 | 2026-07-05 | Rebuild in place: `Strategy_EntrySignal` was calling `QM_StopATR(...)` after `Strategy_AdvanceCachedState()` had already fetched ATR via the pooled `QM_ATR()` reader. `QM_StopATR`'s internal helper creates/copies/releases a fresh raw `iATR` handle in the same call, which the .DWX tester never back-fills on subsequent bars -- root-caused 2026-07-05 (see project memory `project_qm_calendar_fade_family_1trade_bug_2026-07-05`) as a 1-trade-then-permanent-silence defect affecting multiple WTI D1 EAs sharing this exact call pattern; the prior v2 smoke (1 trade in 2024, `20260705_211531`) is consistent with this defect having fired. Fixed by reusing the cached `g_atr_last` via `QM_StopATRFromValue(...)` instead. No change to Entry/Exit/Stop/Sizing mechanics vs the card. | task fb9242d4-5cab-41f7-aa3b-ea2b3de38349 |
