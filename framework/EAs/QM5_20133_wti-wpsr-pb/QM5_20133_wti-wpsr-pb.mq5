...
#property description "QM5_20133 EIA WTI WPSR M30 shallow-pullback continuation"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_20133 - EIA WTI WPSR M30 Shallow-Pullback Continuation
// -----------------------------------------------------------------------------
...
//   - require the completed 11:00-11:30 bar to pull back shallowly while
//     holding outside that prior range
//   - enter at 11:30 in the original impulse direction and flatten in the
...
input int    strategy_pullback_hhmm_ny           = 1100;
...
input double strategy_min_retrace_fraction       = 0.15;
input double strategy_max_retrace_fraction       = 0.50;
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 20133 &&
           qm_magic_slot_offset == 0 &&
           strategy_release_hhmm_ny == 1030 &&
           strategy_pullback_hhmm_ny == 1100 &&
           ...
           MathAbs(strategy_min_retrace_fraction - 0.15) <= 1.0e-12 &&
           MathAbs(strategy_max_retrace_fraction - 0.50) <= 1.0e-12 &&
...
void Strategy_AdvanceEventState()
  {
   ...
   if(!Strategy_BarMatches(1, day_key, strategy_pullback_hhmm_ny) ||
   ...
   const double pullback_open =
      iOpen(_Symbol, PERIOD_M30, 1); // perf-allowed: bounded pullback bar on one genuine new bar.
   const double pullback_high =
      iHigh(_Symbol, PERIOD_M30, 1); // perf-allowed: bounded pullback bar on one genuine new bar.
   const double pullback_low =
      iLow(_Symbol, PERIOD_M30, 1); // perf-allowed: bounded pullback bar on one genuine new bar.
   const double pullback_close =
      iClose(_Symbol, PERIOD_M30, 1); // perf-allowed: bounded pullback bar on one genuine new bar.
   ...
   if(release_open <= 0.0 || release_high <= 0.0 ||
      ...
      pullback_open <= 0.0 || pullback_high <= 0.0 ||
      pullback_low <= 0.0 || pullback_close <= 0.0 ||
      ...
      !MathIsValidNumber(pullback_open) ||
      !MathIsValidNumber(pullback_high) ||
      !MathIsValidNumber(pullback_low) ||
      !MathIsValidNumber(pullback_close) ||
   ...
   if(bullish_impulse)
     {
      const double retrace =
         (release_close - pullback_close) / release_range;
      if(pullback_close >= pullback_open ||
         pullback_close >= release_close ||
         pullback_close <= pre_high ||
         !MathIsValidNumber(retrace) ||
         retrace + 1.0e-12 < strategy_min_retrace_fraction ||
         retrace - 1.0e-12 > strategy_max_retrace_fraction)
         return;

      g_signal_type = QM_BUY;
      g_signal_stop_price =
         MathMin(release_low, pullback_low) -
         strategy_stop_buffer_atr * atr;
     }
   else if(bearish_impulse)
     {
      const double retrace =
         (pullback_close - release_close) / release_range;
      if(pullback_close <= pullback_open ||
         pullback_close <= release_close ||
         pullback_close >= pre_low ||
         !MathIsValidNumber(retrace) ||
         retrace + 1.0e-12 < strategy_min_retrace_fraction ||
         retrace - 1.0e-12 > strategy_max_retrace_fraction)
         return;
      ...
      g_signal_stop_price =
         MathMax(release_high, pullback_high) +
         strategy_stop_buffer_atr * atr;
     }
   else
      return;
   ...
   g_signal_atr = atr;
   g_signal_reference_price = pullback_close;
   g_entry_ready = true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_20133_WTI_WPSR_PB_M30";
   ...
   if(risk_distance <= 0.0 ||
      ...
      risk_distance + 1.0e-12 < strategy_min_stop_atr * g_signal_atr ||
      risk_distance - 1.0e-12 > strategy_max_stop_atr * g_signal_atr)
      return false;
   ...
   const double raw_target =
      ...
      entry_price - strategy_target_rr * risk_distance;
   request.tp = QM_StopRulesNormalizePrice(_Symbol, raw_target);
   ...
   request.reason =
      (request.type == QM_BUY) ?
      "WTI_WPSR_M30_PULLBACK_CONT_LONG" :
      "WTI_WPSR_M30_PULLBACK_CONT_SHORT";
   return true;
  }
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_20133_WTI_WPSR_PB_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   if(new_bar)
     {
      ...
      // Consume and derive the completed impulse/pullback state before
