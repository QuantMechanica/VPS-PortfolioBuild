...
#property description "QM5_9287 MQL5 Gator AD Volume Flip"
// Strategy Card: ba57d97a-0ee0-5a87-aa6d-fb5a37f08bdb (mql5-gator-ad-flip), G0 APPROVED 2026-05-19.
// Source: Stephen Njuki, MQL5 Wizard Techniques you should know (Part 78), Pattern 8.

#include <QM/QM_Common.mqh>

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 9287;
...
input double strategy_rr_take_profit       = 2.5;
...
bool Strategy_ReadClosedBars(MqlRates &rates[], const int count)
  {
   ...
   const int copied = CopyRates(_Symbol, (ENUM_TIMEFRAMES)_Period, 0, count, rates); // perf-allowed: bounded closed-bar OHLCV snapshot; strategy hooks are closed-bar oriented.
   return (copied >= count);
  }
...
bool Strategy_ReadADOsc(double &ad0, double &ad1, double &ad2)
  {
   ...
   MqlRates rates[];
   ArraySetAsSeries(rates, true);
   const int copied = CopyRates(_Symbol, (ENUM_TIMEFRAMES)_Period, 0, strategy_ad_warmup_bars, rates); // perf-allowed: bounded ADL EMA oscillator window, not a per-tick warmup loop for entry.
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(strategy_swing_lookback < 1)
      return false;

   if(!Strategy_GatorUpperRed(1) || !Strategy_GatorLowerRed(1))
      return false;
   ...
   MqlRates rates[];
   const int bars_needed = MathMax(strategy_swing_lookback + 1, 6);
   ...
   const double stddev = QM_StdDev(_Symbol, (ENUM_TIMEFRAMES)_Period, strategy_stddev_period, 1, PRICE_CLOSE, MODE_SMA);
   const double gator_up = Strategy_GatorUpperValue(1);
   const double gator_lo = MathAbs(Strategy_GatorLowerValue(1));
   if(stddev <= 0.0 || gator_up <= 0.0 || gator_lo <= 0.0)
      return false;

   if(stddev > MathMax(gator_up, gator_lo))
      return false;

   const double ad_range = MathMax(ad1, ad2) - MathMin(ad1, ad2);
   if(5.0 * stddev <= ad_range)
      return false;

   const double prior_mid = 0.5 * (rates[2].high + rates[2].low);
   const double atr = QM_ATR(_Symbol, (ENUM_TIMEFRAMES)_Period, strategy_atr_period, 1);
   if(prior_mid <= 0.0 || atr <= 0.0 || strategy_atr_sl_mult <= 0.0 || strategy_rr_take_profit <= 0.0)
      return false;
   ...
   if(rates[1].close > prior_mid && ad0 > ad1)
     {
      const double low_stop = Strategy_SwingLow(rates) - strategy_atr_sl_mult * atr;
      ...
      req.reason = "GATOR_AD_FLIP_LONG";
      return (req.tp > ask);
     }

   if(rates[1].close < prior_mid && ad0 < ad1)
     {
      const double high_stop = Strategy_SwingHigh(rates) + strategy_atr_sl_mult * atr;
      ...
      req.reason = "GATOR_AD_FLIP_SHORT";
...
bool Strategy_ExitSignal()
  {
   ...
   MqlRates rates[];
   if(!Strategy_ReadClosedBars(rates, 6))
      return false;
   ...
   const double mid1 = 0.5 * (rates[2].high + rates[2].low);
   const double mid2 = 0.5 * (rates[3].high + rates[3].low);
   if(mid1 <= 0.0 || mid2 <= 0.0)
      return false;

   if(ptype == POSITION_TYPE_BUY)
     {
      if(rates[1].close < mid1 && rates[2].close < mid2)
         return true;
      if(ad0 < ad1 && ad1 < ad2)
         return true;
     }
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"mql5-gator-ad-flip\",\"ea\":\"QM5_9287\"}");
