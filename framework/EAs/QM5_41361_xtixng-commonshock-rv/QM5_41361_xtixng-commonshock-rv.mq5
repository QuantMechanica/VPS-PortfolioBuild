...
#property description "QM5_41361 XTI/XNG Weekly Common-Shock Dispersion Reversion"
...
// natural gas log returns over that exact interval share a strict sign, fade the
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41361 && qm_magic_slot_offset == 0 &&
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41361_XTIXNG_COMMONSHOCK_RV";
...
bool Strategy_LoadWeeklyCommonShock(const int current_week,
                                    int &direction)
  {
   ...
   if((same_positive || same_negative) &&
      ...
      direction = -1; // WTI outperformed: sell XTI, buy XNG.
   else if((same_positive || same_negative) &&
           relative_return < -strategy_signal_epsilon)
      direction = 1; // natural gas outperformed: buy XTI, sell XNG.
   return true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41361_XTIXNG_COMMONSHOCK_RV_HOST";
   ...
   if(Strategy_OpenOwnedPositionCount() > 0 ||
   ...
   else
     {
      ...
      if(!signal_valid)
         ...
         g_signal_state = "wti_relative_outperformer_fade_short_xti";
      else if(direction > 0)
         g_signal_state = "xng_relative_outperformer_fade_long_xti";
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41361\",\"ea\":\"xtixng-commonshock-rv\"}");
