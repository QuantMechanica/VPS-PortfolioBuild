...
#property description "QM5_13069 JODI monthly WTI breakout"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_13069 - JODI Monthly Oil Data Breakout
// -----------------------------------------------------------------------------
// D1 structural WTI sleeve:
//   - official JODI/IEF monthly global oil-data update source lineage
//   - deterministic mid-to-late-month update-window proxy
//   - completed-bar Donchian breakout with SMA trend/slope confirmation
//   - ATR stop/target, max-hold exit, no external runtime data
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 13069;
...
input int    strategy_breakout_lookback  = 34;
...
bool Strategy_LoadJodiBreakoutState(int &direction,
                                    double &atr_last,
                                    int &signal_month_key)
  {
   ...
   const double trend_sma = QM_SMA(_Symbol, PERIOD_D1, strategy_trend_sma_period, 1, PRICE_CLOSE);
   const double trend_sma_prior = QM_SMA(_Symbol, PERIOD_D1, strategy_trend_sma_period,
                                        1 + strategy_sma_slope_shift, PRICE_CLOSE);
   if(atr_last <= 0.0 || trend_sma <= 0.0 || trend_sma_prior <= 0.0)
      return false;
   ...
   const double breakout_buffer = strategy_min_break_atr * atr_last;
   const bool long_breakout =
      event_close > event_open &&
      event_close >= breakout_high + breakout_buffer &&
      event_close > trend_sma &&
      trend_sma > trend_sma_prior;
   const bool short_breakout =
      event_close < event_open &&
      event_close <= breakout_low - breakout_buffer &&
      event_close < trend_sma &&
      trend_sma < trend_sma_prior;
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      if(close_last > 0.0 && trend_sma > 0.0)
        {
         if(pos_type == POSITION_TYPE_BUY && close_last < trend_sma)
            should_close = true;
         if(pos_type == POSITION_TYPE_SELL && close_last > trend_sma)
            should_close = true;
        }
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(strategy_breakout_lookback < 5)
      return true;
   if(strategy_trend_sma_period <= 1)
      return true;
   if(strategy_sma_slope_shift <= 0)
      return true;
   if(strategy_atr_period <= 1)
      return true;
   if(strategy_min_range_atr <= 0.0 || strategy_min_body_atr <= 0.0 || strategy_min_break_atr < 0.0)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_13069_XTI_JODI_BRK";
   ...
   if(!Strategy_LoadJodiBreakoutState(direction, atr_last, signal_month_key))
      return false;
   ...
   req.reason = (direction > 0) ? "XTI_JODI_BRK_LONG" : "XTI_JODI_BRK_SHORT";
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_13069\",\"ea\":\"xti-jodi-brk\"}");
