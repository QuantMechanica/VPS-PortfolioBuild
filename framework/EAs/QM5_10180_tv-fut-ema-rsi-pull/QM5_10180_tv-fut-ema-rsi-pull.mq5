...
bool Strategy_NoTradeFilter()
  {
   ...
   return (!Strategy_IsInsideSession(TimeCurrent()) && !Strategy_HasOpenPosition());
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(Strategy_HasOpenPosition())
      return false;
   if(strategy_ema_period <= 0 || strategy_rsi_period <= 0 || strategy_atr_period <= 0)
      return false;
   ...
   if(long_signal)
     {
      ...
      req.type = QM_BUY;
      req.price = entry;
      req.sl = NormalizeDouble(entry - (atr * strategy_atr_sl_mult), _Digits);
      req.tp = NormalizeDouble(entry + (atr * strategy_atr_tp_mult), _Digits);
      req.reason = "EMA100_RSI35_PULLBACK_LONG";
   ...
   req.sl = NormalizeDouble(entry + (atr * strategy_atr_sl_mult), _Digits);
   req.tp = NormalizeDouble(entry - (atr * strategy_atr_tp_mult), _Digits);
   req.reason = "EMA100_RSI65_PULLBACK_SHORT";
   return (entry > 0.0 && req.sl > 0.0 && req.tp > 0.0);
  }
...
void Strategy_ManageOpenPosition()
  {
   // Card baseline disables the optional source trailing stop.
  }
...
bool Strategy_ExitSignal()
  {
   const int max_bars = MathMax(1, strategy_max_hold_bars);
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      if(opened > 0 && now - opened >= max_bars * seconds)
         return true;

      if(strategy_session_enabled && !Strategy_IsInsideSession(now))
         return true;
     }
...
bool Strategy_NewsFilterHook(const datetime broker_time)
  {
   return false; // defer to the framework high-impact macro news filter.
  }
