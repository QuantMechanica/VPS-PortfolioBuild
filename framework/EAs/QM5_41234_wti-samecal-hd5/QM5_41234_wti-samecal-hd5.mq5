...
#property description "QM5_41234 WTI Same-Calendar Five-Sample Harrell-Davis Median"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41234 - WTI Same-Calendar Five-Sample Harrell-Davis Median Location
// -----------------------------------------------------------------------------
...
//   - apply fixed beta(3,3) interval masses to all five order statistics
//   - compute both exact-rational and independent decimal weighted locations
//   - require the two fixed-weight representations to agree within 1e-12
...
input double strategy_invariant_tolerance = 1.0e-12;
...
// -----------------------------------------------------------------------------
// Exact completed-month endpoint and five-sample Harrell-Davis helpers.
// -----------------------------------------------------------------------------
...
bool Strategy_HarrellDavisSignal(const double &observations[],
                                  ...
                                  int &direction)
  {
   ...
   if(strategy_history_years != 5 ||
      ...
      MathAbs(strategy_invariant_tolerance - 1.0e-12) > 1.0e-16 ||
   ...
   // For n=5 and p=0.5 the Harrell-Davis beta(3,3) interval masses
   // are exactly [181,811,1141,811,181]/3125. The independent decimal
   // representation is load-bearing and must agree with the rational form.
   const double rational_numerator =
      181.0 * sorted_values[0] +
      811.0 * sorted_values[1] +
      1141.0 * sorted_values[2] +
      811.0 * sorted_values[3] +
      181.0 * sorted_values[4];
   const double rational_location = rational_numerator / 3125.0;
   const double decimal_location =
      0.05792 * sorted_values[0] +
      0.25952 * sorted_values[1] +
      0.36512 * sorted_values[2] +
      0.25952 * sorted_values[3] +
      0.05792 * sorted_values[4];

   if(!MathIsValidNumber(rational_numerator) ||
      !MathIsValidNumber(rational_location) ||
      !MathIsValidNumber(decimal_location) ||
      MathAbs(rational_location - decimal_location) >
         strategy_invariant_tolerance)
      return false;

   location_value = rational_location;
...
bool Strategy_LoadHarrellDavisSignal(
   ...
   int &direction)
  {
   ...
   return Strategy_HarrellDavisSignal(observations,
                                   sample_count,
                                   location_value,
                                   direction);
  }
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   // Consume before history, Harrell-Davis arithmetic, news, spread, quote,
   ...
   else
     {
      g_signal_valid =
          Strategy_LoadHarrellDavisSignal(
            ...
            g_signal_direction);
      if(!g_signal_valid)
         g_signal_state = "endpoint_sample_or_hd_failed";
      else if(g_signal_direction > 0)
         g_signal_state = "fixed_hd_long";
      else if(g_signal_direction < 0)
         g_signal_state = "fixed_hd_short";
      else
         g_signal_state = "fixed_hd_epsilon_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"valid\":%s,\"samples\":%d,\"fixed_hd\":%.12g,\"direction\":%d,\"state\":\"%s\"}",
                            g_decision_month_key,
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(!QM_InputRequireLong("qm_ea_id", qm_ea_id, 41234) ||
      ...
      !QM_InputRequireDouble("strategy_invariant_tolerance", strategy_invariant_tolerance, 0.000000000001, 1.0e-16) ||
      ...
      !QM_InputRequireLong("strategy_deviation_points", strategy_deviation_points, 20))
      return true;
   return false;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(g_signal_direction > 0)
     {
      req.type = QM_BUY;
      req.reason = "WTI_SAMECAL_HD5_LONG";
     }
   else
     {
      req.type = QM_SELL;
      req.reason = "WTI_SAMECAL_HD5_SHORT";
     }
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41234 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI same-calendar five-sample Harrell-Davis card holds to the next broker month"))
      return INIT_FAILED;
   ...
   g_attempt_state_key =
      StringFormat("QM5_41234_SAMECAL_HD5_MONTH_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41234\",\"ea\":\"wti-samecal-hd5\"}");
