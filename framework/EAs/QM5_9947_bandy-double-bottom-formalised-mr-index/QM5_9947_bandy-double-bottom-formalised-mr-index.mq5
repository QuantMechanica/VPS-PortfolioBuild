...
#property description "QM5_9947 Bandy double-bottom formalised D1 MR"

#include <QM/QM_Common.mqh>

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 9947;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   const int copied = CopyRates(_Symbol, (ENUM_TIMEFRAMES)_Period, 1, need_bars, rates); // perf-allowed: bespoke double-bottom pivot scan, bounded and called only behind the framework new-bar gate.
   ...
   for(int i = strategy_pivot_lookback_bars; i <= last_candidate; ++i)
     {
      ...
      for(int j = 1; j <= strategy_pivot_lookback_bars && is_pivot; ++j)
        {
         if(l >= rates[i - j].low || l >= rates[i + j].low)
            is_pivot = false;
        }
      if(!is_pivot)
         continue;
   ...
   if(MathAbs(p2_low - p1_low) / p1_low > strategy_tolerance_pct)
      return false;

   double neckline_high = rates[p2_index + 1].high;
   for(int i = p2_index + 1; i <= p1_index - 1; ++i)
     {
      if(rates[i].high > neckline_high)
         neckline_high = rates[i].high;
     }
   const double pattern_low = MathMin(p1_low, p2_low);
   const double pattern_height = neckline_high - pattern_low;
   if(pattern_low <= 0.0 || neckline_high <= 0.0 || pattern_height <= 0.0)
      return false;
   ...
   double sl_raw = pattern_low - strategy_stop_buffer_atr * atr;
   const double sl_floor = ask - strategy_stop_cap_atr * atr;
   if(sl_raw < sl_floor)
      sl_raw = sl_floor;
