...
#property description "QM5_20233 XAU XAG Commodity Skewness Rank"
...
//     of D1 log returns
//   - long the lower-skew precious-metal leg and short the higher-skew leg
// Runtime is Darwinex-native D1 OHLC only; no external or futures-chain data.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 20233;
...
double   g_cache_xau_skew = 0.0;
double   g_cache_xag_skew = 0.0;
...
bool Strategy_LoadSignalState(const datetime decision_bar_time,
  ...
  {
   pair_direction = 0;
   g_cache_xau_skew = 0.0;
   g_cache_xag_skew = 0.0;
   ...
   if(!Strategy_FormationWindow(decision_bar_time, window_start, window_end))
      return false;
   if(!Strategy_PearsonSkewness(g_leg_xau, window_start, window_end,
                                g_cache_xau_skew, g_cache_xau_observations))
      return false;
   if(!Strategy_PearsonSkewness(g_leg_xag, window_start, window_end,
                                g_cache_xag_skew, g_cache_xag_observations))
      return false;

   g_cache_skew_difference = g_cache_xau_skew - g_cache_xag_skew;
...
bool Strategy_OpenPair(const int pair_direction)
  {
   ...
   const string reason = long_xau_short_xag ? "QM5_20233_LONG_XAU_SHORT_XAG_LOWSKEW"
                                            : "QM5_20233_SHORT_XAU_LONG_XAG_LOWSKEW";
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart())
      return true;
   if(qm_ea_id != 20233 || qm_magic_slot_offset != 0 || qm_rng_seed != 42)
      return true;
   ...
   if(qm_friday_close_enabled || qm_friday_close_hour_broker != 21 ||
      ...
      return true;
   if(strategy_lookback_months != 12 || strategy_history_bars != 500 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20233_XAU_XAG_SKEW_RANK_HOST";
...
int OnInit()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 20233 ||
