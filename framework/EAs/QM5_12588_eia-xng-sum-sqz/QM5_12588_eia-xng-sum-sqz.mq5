...
#property description "QM5_12588 EIA XNG Summer Power-Burn Squeeze"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_12588 - EIA XNG Summer Power-Burn Squeeze
// -----------------------------------------------------------------------------
// D1 structural natural-gas sleeve:
//   - trades only during the June-August summer electric-sector demand window
//   - enters long after D1 range compression and upside channel breakout
//   - exits on channel failure, SMA failure, season end, max hold, or ATR stop
// Runtime uses MT5 OHLC only; no EIA, storage, weather, or power-load feed.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 12588;
...
input int    strategy_compression_lookback = 10;
...
bool Strategy_InSummerPowerWindow(const datetime t)
  {
   MqlDateTime dt;
   TimeToStruct(t, dt);
   return (dt.mon >= 6 && dt.mon <= 8);
  }
...
bool Strategy_AverageRange(const int lookback, const int start_shift, double &avg_range)
  {
   avg_range = 0.0;
   if(lookback <= 0 || start_shift < 1)
      return false;
   ...
   for(int i = start_shift; i < start_shift + lookback; ++i)
     {
      const double bar_high = iHigh(_Symbol, PERIOD_D1, i); // perf-allowed: D1 compression gate.
      const double bar_low = iLow(_Symbol, PERIOD_D1, i);   // perf-allowed: D1 compression gate.
      if(bar_high <= 0.0 || bar_low <= 0.0 || bar_high <= bar_low)
         return false;
      sum += (bar_high - bar_low);
     }

   avg_range = sum / (double)lookback;
   return (avg_range > 0.0);
  }
...
void Strategy_CloseOpenPositionsIfNeeded()
  {
   ...
   const bool in_window = Strategy_InSummerPowerWindow(closed_time);
   const int magic = QM_FrameworkMagic();
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      bool should_close = (!in_window || close_last < exit_low || close_last < sma_last);
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(strategy_entry_channel <= 1 || strategy_exit_channel <= 1)
      return true;
   if(strategy_trend_period <= 1 || strategy_compression_lookback <= 1)
      return true;
   if(strategy_compression_atr_mult <= 0.0 || strategy_atr_period <= 0)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12588_EIA_XNG_SUMMER_SQZ";
   ...
   if(!Strategy_ClosedState(close_last, sma_last, atr_last, closed_time))
      return false;
   if(!Strategy_InSummerPowerWindow(closed_time))
      return false;

   const int day_key = Strategy_DayKey(closed_time);
   ...
   if(!Strategy_AverageRange(strategy_compression_lookback, 2, avg_range))
      return false;
   ...
   req.reason = "XNG_SUMMER_SQUEEZE_LONG";
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_12588\",\"ea\":\"eia-xng-sum-sqz\"}");
