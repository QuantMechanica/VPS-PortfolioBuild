...
The EA trades a Tokyo opening liquidity sweep breakout on M3 or M5 bars. Each Pakistan-time day it builds the Tokyo reference range from 05:00 PKT through the bar containing the 05:54 PKT magic-candle timestamp, then checks only the closed checkpoint candles from 06:30 PKT through 08:30 PKT at 15-minute intervals. A long entry requires the checkpoint candle to close above the reference high with its low also above the range; a short entry mirrors this below the reference low. The stop is placed on the opposite side of the reference range, the target is 1.5R, trades are skipped when stop distance exceeds 1.75 x ATR(14,M5), and any remaining position is closed at the configured same-day session-flat time.
...
|---|---:|---|---|
| `strategy_reference_start_pkt_hhmm` | 500 | 0000-2359 | PKT start time for accumulating the Tokyo opening reference range. |
| `strategy_magic_pkt_hhmm` | 554 | 0000-2359 | PKT timestamp of the magic candle; the bar containing this timestamp completes the reference range. |
...
| `strategy_session_flat_pkt_hhmm` | 900 | 0000-2359 | PKT time when an open trade is flattened if SL/TP did not hit. |
...
| Aspect | Value |
|---|---|
| Base timeframe | `M5` P2 baseline; `M3` also supported by the card timing |
| Multi-timeframe refs | `ATR(14,M5)` stop-distance cap |
...
| v2 | 2026-06-17 | Rebuild in place from card | 83f43419-f1b9-4f7c-ab7a-2cba7e44ebcb |
