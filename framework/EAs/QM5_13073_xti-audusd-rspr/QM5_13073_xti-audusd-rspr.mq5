...
#property description "QM5_13073 XTI AUDUSD Return Spread Reversion"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_13073 - WTI/AUDUSD Return-Spread Reversion
// -----------------------------------------------------------------------------
// D1 two-leg energy/FX basket:
//   rspread = log(XTI[t] / XTI[t-L]) - beta_audusd * log(AUDUSD[t] / AUDUSD[t-L])
//   z > entry: short return spread = sell WTI, buy AUDUSD
//   z < -entry: long return spread = buy WTI, sell AUDUSD
...
input double strategy_beta_audusd         = 1.00;
   ...
   for(int i = 0; i < z_lookback; ++i)
     {
      ...
      if(xti[i] <= 0.0 || audusd[i] <= 0.0 || xti[past_idx] <= 0.0 || audusd[past_idx] <= 0.0)
         return false;

      const double xti_ret = MathLog(xti[i] / xti[past_idx]);
      const double audusd_ret = MathLog(audusd[i] / audusd[past_idx]);
      spreads[i] = xti_ret - strategy_beta_audusd * audusd_ret;
...
bool Strategy_OpenPair(const int spread_direction)
  {
   if(spread_direction == 0 || Strategy_OpenPairLegCount() > 0)
      return false;
   if(!Strategy_SpreadAllowed(g_leg_xti) || !Strategy_SpreadAllowed(g_leg_audusd))
      return false;

   const double xti_weight = 1.0;
   const double audusd_weight = MathMax(0.1, MathAbs(strategy_beta_audusd));
   const double weight_sum = xti_weight + audusd_weight;
   ...
   const string reason = long_spread ? "QM5_13073_LONG_XTI_AUDUSD_RSPREAD"
                                     : "QM5_13073_SHORT_XTI_AUDUSD_RSPREAD";
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart())
      return true;
   if(strategy_return_lookback_d1 < 5 || strategy_z_lookback_d1 < 40 || strategy_beta_audusd <= 0.0)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_13073_RSPREAD_HOST";
...
int OnInit()
  {
   ...
   string basket_symbols[2] = {g_leg_xti, g_leg_audusd};
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_13073\",\"ea\":\"xti-audusd-rspr\"}");
