...
#property description "QM5_20229 WTI Season / One-Month Pullback"
...
//   - sign of the exact just-completed calendar-month WTI log return
//   - enter seasonally only when the completed return sign is opposite
   ...
   for(int index = PositionsTotal() - 1; index >= 0; --index)
     {
      ...
      const bool wrong_side =
         (seasonal_direction > 0 && position_type != POSITION_TYPE_BUY) ||
         (seasonal_direction < 0 && position_type != POSITION_TYPE_SELL);

      bool should_close = (seasonal_direction == 0 || wrong_side);
      if(current_month_key > 0 &&
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   const int seasonal_direction =
      Strategy_SeasonalDirection(month_key % 100);
   if(month_key <= 0 ||
      seasonal_direction == 0 ||
      month_key == g_last_attempt_month_key)
      return false;

   // Consume before history, signal, disagreement, spread, quote, news,
   // stop, sizing, or order gates.
   ...
   if(!Strategy_LoadCompletedMonthSignal(month_key,
                                         ...
                                         return_direction) ||
      return_direction == 0 ||
      return_direction == seasonal_direction)
      return false;
   ...
   req.type = (seasonal_direction > 0) ? QM_BUY : QM_SELL;
   ...
   req.reason = (seasonal_direction > 0)
      ? "WTI_SEAS_REV1_LONG"
      : "WTI_SEAS_REV1_SHORT";
   return true;
  }
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_20229_MONTH_ATTEMPT_%d",
                   QM_FrameworkMagic());
   Strategy_LoadAttemptState();

   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_20229\",\"ea\":\"wti-seas-rev1\"}");
