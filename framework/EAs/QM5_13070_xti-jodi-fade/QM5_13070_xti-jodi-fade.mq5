...
#property description "QM5_13070 JODI monthly WTI failed-probe fade"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_13070 - JODI Monthly Oil Data Failed-Probe Fade
// -----------------------------------------------------------------------------
// D1 structural WTI sleeve:
//   - official JODI/IEF monthly global oil-data update source lineage
//   - deterministic mid-to-late-month update-window proxy
//   - completed-bar Donchian failed-probe/reclaim with SMA mean confirmation
//   - ATR stop/target, max-hold and SMA mean exits, no external runtime data
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 13070;
...
input int    strategy_event_start_day     = 18;
input int    strategy_event_end_day       = 23;
input int    strategy_probe_lookback      = 34;
input int    strategy_mean_sma_period     = 100;
input int    strategy_atr_period          = 20;
input double strategy_min_range_atr       = 0.70;
input double strategy_min_body_atr        = 0.20;
input double strategy_min_probe_atr       = 0.10;
input double strategy_reclaim_buffer_atr  = 0.03;
...
bool Strategy_IsJodiProxyWindow(const datetime t)
  {
   if(t <= 0)
      return false;
...
bool Strategy_DonchianExcludingSignal(double &highest_high,
                                      double &lowest_low)
  {
   highest_high = -DBL_MAX;
   lowest_low = DBL_MAX;

   const int lookback = MathMax(5, strategy_probe_lookback);
...
bool Strategy_LoadJodiFadeState(int &direction,
                                double &atr_last,
                                int &signal_month_key)
  {
   ...
   if(event_time <= 0 || !Strategy_IsJodiProxyWindow(event_time))
      return false;

   const double event_open = iOpen(_Symbol, PERIOD_D1, 1);   // perf-allowed: completed D1 signal bar.
   const double event_high = iHigh(_Symbol, PERIOD_D1, 1);   // perf-allowed: completed D1 signal bar.
   const double event_low = iLow(_Symbol, PERIOD_D1, 1);     // perf-allowed: completed D1 signal bar.
   const double event_close = iClose(_Symbol, PERIOD_D1, 1); // perf-allowed: completed D1 signal bar.
   ...
   const double mean_sma = QM_SMA(_Symbol, PERIOD_D1, strategy_mean_sma_period, 1, PRICE_CLOSE);
   if(atr_last <= 0.0 || mean_sma <= 0.0)
      return false;
   ...
   if(!Strategy_DonchianExcludingSignal(context_high, context_low))
      return false;

   const double probe_distance = strategy_min_probe_atr * atr_last;
   const double reclaim_buffer = strategy_reclaim_buffer_atr * atr_last;

   const bool downside_reclaim =
      event_low <= context_low - probe_distance &&
      event_close >= context_low + reclaim_buffer &&
      event_close > event_open &&
      event_close < mean_sma;
   const bool upside_rejection =
      event_high >= context_high + probe_distance &&
      event_close <= context_high - reclaim_buffer &&
      event_close < event_open &&
      event_close > mean_sma;

   if(downside_reclaim)
      direction = 1;
   else if(upside_rejection)
      direction = -1;
   else
      return false;
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      if(close_last > 0.0 && mean_sma > 0.0)
        {
         if(pos_type == POSITION_TYPE_BUY && close_last >= mean_sma)
            should_close = true;
         if(pos_type == POSITION_TYPE_SELL && close_last <= mean_sma)
            should_close = true;
        }
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(strategy_event_end_day < strategy_event_start_day || strategy_event_end_day > 31)
      return true;
   if(strategy_probe_lookback < 5)
      return true;
   if(strategy_mean_sma_period <= 1)
      return true;
   if(strategy_atr_period <= 1)
      return true;
   if(strategy_min_range_atr <= 0.0 || strategy_min_body_atr <= 0.0)
      return true;
   if(strategy_min_probe_atr <= 0.0 || strategy_reclaim_buffer_atr < 0.0)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_13070_XTI_JODI_FADE";
   ...
   if(!Strategy_LoadJodiFadeState(direction, atr_last, signal_month_key))
      return false;
   ...
   req.reason = (direction > 0) ? "XTI_JODI_FADE_LONG" : "XTI_JODI_FADE_SHORT";
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_13070\",\"ea\":\"xti-jodi-fade\"}");
