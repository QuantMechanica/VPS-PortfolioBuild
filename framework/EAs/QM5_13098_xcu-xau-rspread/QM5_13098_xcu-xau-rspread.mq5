...
#property description "QM5_13098 XCU XAU Return Spread Reversion"
...
// -----------------------------------------------------------------------------
// D1 two-leg copper/gold commodity basket:
//   rspread = log(XCU[t] / XCU[t-L]) - beta_xau * log(XAU[t] / XAU[t-L])
//   z > entry: short return spread = sell copper, buy gold
//   z < -entry: long return spread = buy copper, sell gold
...
input double strategy_beta_xau           = 0.75;
   ...
   for(int i = 0; i < z_lookback; ++i)
     {
      ...
      if(xcu[i] <= 0.0 || xau[i] <= 0.0 || xcu[past_idx] <= 0.0 || xau[past_idx] <= 0.0)
         return false;

      const double xcu_ret = MathLog(xcu[i] / xcu[past_idx]);
      const double xau_ret = MathLog(xau[i] / xau[past_idx]);
      spreads[i] = xcu_ret - strategy_beta_xau * xau_ret;
...
bool Strategy_OpenPair(const int spread_direction)
  {
   if(spread_direction == 0 || Strategy_OpenPairLegCount() > 0)
      return false;
   if(!Strategy_SpreadAllowed(g_leg_xcu) || !Strategy_SpreadAllowed(g_leg_xau))
      return false;

   const double xcu_weight = 1.0;
   const double xau_weight = MathMax(0.1, MathAbs(strategy_beta_xau));
   const double weight_sum = xcu_weight + xau_weight;
   ...
   const string reason = long_spread ? "QM5_13098_LONG_XCU_XAU_RSPREAD"
                                     : "QM5_13098_SHORT_XCU_XAU_RSPREAD";
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart())
      return true;
   if(strategy_return_lookback_d1 < 5 || strategy_z_lookback_d1 < 40 || strategy_beta_xau <= 0.0)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_13098_RSPREAD_HOST";
...
int OnInit()
  {
   ...
   string basket_symbols[2] = {g_leg_xcu, g_leg_xau};
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_13098\",\"ea\":\"xcu-xau-rspread\"}");
