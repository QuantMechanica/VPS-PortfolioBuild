...
#property description "QM5_20282 WTI Median/MAD-Capped Return Trend"
...
//   2. form twelve disjoint chronological monthly log returns;
//   3. estimate their even-sample median and raw median absolute deviation;
//   4. cap each return at three raw MADs around the median;
//   5. trade the sign of the equal-weight mean of all twelve capped returns.
// The robust statistic is a transparent QM mechanization, not an author rule.
// Runtime uses native XTIUSD.DWX price and broker data only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 20282;
...
input double strategy_mad_cap_mult       = 3.0;
...
double g_cache_madcap_mean      = 0.0;
double g_cache_return_median    = 0.0;
double g_cache_raw_mad          = 0.0;
double g_cache_madcap_low       = 0.0;
double g_cache_madcap_high      = 0.0;
...
bool Strategy_LoadMonthlyCloses(double &closes[])
  {
   ...
   if(strategy_return_months != 12 || strategy_history_bars_d1 != 800)
      return false;
...
void Strategy_ResetCachedState()
  {
   ...
   g_cache_signal = 0;
   g_cache_madcap_mean = 0.0;
   g_cache_return_median = 0.0;
   g_cache_raw_mad = 0.0;
   g_cache_madcap_low = 0.0;
   g_cache_madcap_high = 0.0;
   g_cache_state_reason = "not_evaluated";
  }

bool Strategy_MadCappedMeanSignal(const double &closes[], int &signal)
  {
   signal = 0;
   const int return_count = strategy_return_months;
   if(return_count != 12 ||
      MathAbs(strategy_mad_cap_mult - 3.0) > 1.0e-12 ||
      ArraySize(closes) != return_count + 1)
     {
      g_cache_state_reason = "bad_madcap_contract";
      return false;
     }

   double monthly_returns[];
   double sorted_returns[];
   ArrayResize(monthly_returns, return_count);
   ArrayResize(sorted_returns, return_count);
   for(int i = 0; i < return_count; ++i)
     {
      const double older_close = closes[i];
      const double newer_close = closes[i + 1];
      if(older_close <= 0.0 || newer_close <= 0.0 ||
      ...
      const double monthly_return = MathLog(newer_close / older_close);
      if(!MathIsValidNumber(monthly_return))
        {
         g_cache_state_reason = "invalid_monthly_return";
         return false;
        }
      monthly_returns[i] = monthly_return;
      sorted_returns[i] = monthly_return;
     }
   ...
   for(int i = 0; i < return_count; ++i)
     {
      if(!MathIsValidNumber(sorted_returns[i]) ||
         (i > 0 && sorted_returns[i] < sorted_returns[i - 1]))
        {
         g_cache_state_reason = "invalid_sorted_return_state";
         return false;
        }
     }

   const double return_median =
      (sorted_returns[5] + sorted_returns[6]) / 2.0;
   if(!MathIsValidNumber(return_median))
     {
      g_cache_state_reason = "invalid_return_median";
      return false;
     }

   double sorted_deviations[];
   ArrayResize(sorted_deviations, return_count);
   for(int i = 0; i < return_count; ++i)
     {
      const double deviation = MathAbs(monthly_returns[i] - return_median);
      if(!MathIsValidNumber(deviation))
        {
         g_cache_state_reason = "invalid_absolute_deviation";
         return false;
        }
      sorted_deviations[i] = deviation;
     }

   ArraySort(sorted_deviations);
   for(int i = 0; i < return_count; ++i)
     {
      if(!MathIsValidNumber(sorted_deviations[i]) ||
         sorted_deviations[i] < 0.0 ||
         (i > 0 && sorted_deviations[i] < sorted_deviations[i - 1]))
        {
         g_cache_state_reason = "invalid_sorted_deviation_state";
         return false;
        }
     }

   const double raw_mad =
      (sorted_deviations[5] + sorted_deviations[6]) / 2.0;
   if(!MathIsValidNumber(raw_mad))
     {
      g_cache_state_reason = "invalid_raw_mad";
      return false;
     }

   g_cache_return_median = return_median;
   g_cache_raw_mad = raw_mad;
   if(raw_mad <= 0.0)
     {
      g_cache_state_reason = "nonpositive_raw_mad";
      return true;
     }

   const double madcap_low =
      return_median - strategy_mad_cap_mult * raw_mad;
   const double madcap_high =
      return_median + strategy_mad_cap_mult * raw_mad;
   if(!MathIsValidNumber(madcap_low) ||
      !MathIsValidNumber(madcap_high) ||
      madcap_low > madcap_high)
     {
      g_cache_state_reason = "invalid_madcap_boundary";
      return false;
     }

   double madcap_sum = 0.0;
   for(int i = 0; i < return_count; ++i)
     {
      double capped_return = monthly_returns[i];
      if(capped_return < madcap_low)
         capped_return = madcap_low;
      else if(capped_return > madcap_high)
         capped_return = madcap_high;

      if(!MathIsValidNumber(capped_return))
        {
         g_cache_state_reason = "invalid_capped_return";
         return false;
        }
      madcap_sum += capped_return;
      if(!MathIsValidNumber(madcap_sum))
        {
         g_cache_state_reason = "invalid_madcap_sum";
         return false;
        }
     }

   const double madcap_mean = madcap_sum / (double)return_count;
   if(!MathIsValidNumber(madcap_mean))
     {
      g_cache_state_reason = "invalid_madcap_mean";
      return false;
     }

   g_cache_madcap_low = madcap_low;
   g_cache_madcap_high = madcap_high;
   g_cache_madcap_mean = madcap_mean;
   if(madcap_mean == 0.0)
     {
      g_cache_state_reason = "exact_zero_madcap_mean";
      return true;
     }

   signal = (madcap_mean > 0.0) ? 1 : -1;
...
bool Strategy_LoadSignalState(int &signal)
  {
   ...
   return Strategy_MadCappedMeanSignal(monthly_closes, signal);
  }
...
void Strategy_PrepareMonthlySignal()
  {
   ...
   QM_LogEvent(QM_INFO,
               "MONTHLY_STATE",
               StringFormat("{\"month\":%d,\"valid\":%s,\"signal\":%d,\"return_median\":%.10f,\"raw_mad\":%.10f,\"madcap_mean\":%.10f,\"madcap_low\":%.10f,\"madcap_high\":%.10f,\"state\":\"%s\"}",
                            g_cache_month_key,
                            g_cache_signal_valid ? "true" : "false",
                            g_cache_signal,
                            g_cache_return_median,
                            g_cache_raw_mad,
                            g_cache_madcap_mean,
                            g_cache_madcap_low,
                            g_cache_madcap_high,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 20282 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20282_WTI_MADCAP_MOM";
   ...
   req.reason = (g_cache_signal > 0) ? "MADCAP_MOM_XTI_LONG" : "MADCAP_MOM_XTI_SHORT";
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_20282_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_20282\",\"ea\":\"wti-madcap-mom\"}");
