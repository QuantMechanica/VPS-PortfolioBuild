...
#property description "QM5_10452 (v2) MQL5 Triple Oscillator Divergence Reversal"
...
bool Strategy_IsPriceSwingLow(const int shift)
  {
   const double low = iLow(_Symbol, _Period, shift); // perf-allowed: structural/indicator series read; called from QM_IsNewBar-gated context
   if(low <= 0.0)
      return false;

   for(int offset = 1; offset <= strategy_min_bars_distance; ++offset)
     {
      if(iLow(_Symbol, _Period, shift - offset) <= low) // perf-allowed: structural/indicator series read; called from QM_IsNewBar-gated context
         return false;
      if(iLow(_Symbol, _Period, shift + offset) < low) // perf-allowed: structural/indicator series read; called from QM_IsNewBar-gated context
         return false;
     }
...
bool Strategy_IsPriceSwingHigh(const int shift)
  {
   const double high = iHigh(_Symbol, _Period, shift); // perf-allowed: structural/indicator series read; called from QM_IsNewBar-gated context
   if(high <= 0.0)
      return false;

   for(int offset = 1; offset <= strategy_min_bars_distance; ++offset)
     {
      if(iHigh(_Symbol, _Period, shift - offset) >= high) // perf-allowed: structural/indicator series read; called from QM_IsNewBar-gated context
         return false;
      if(iHigh(_Symbol, _Period, shift + offset) > high) // perf-allowed: structural/indicator series read; called from QM_IsNewBar-gated context
         return false;
     }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   const double close1 = iClose(_Symbol, _Period, 1); // perf-allowed: structural/indicator series read; called from QM_IsNewBar-gated context
   ...
   if(Strategy_FindSwingPair(true, recent_low, older_low))
     {
      const double recent_price_low = iLow(_Symbol, _Period, recent_low); // perf-allowed: structural/indicator series read; called from QM_IsNewBar-gated context
      const double older_price_low = iLow(_Symbol, _Period, older_low); // perf-allowed: structural/indicator series read; called from QM_IsNewBar-gated context
   ...
   if(Strategy_FindSwingPair(false, recent_high, older_high))
     {
      const double recent_price_high = iHigh(_Symbol, _Period, recent_high); // perf-allowed: structural/indicator series read; called from QM_IsNewBar-gated context
      const double older_price_high = iHigh(_Symbol, _Period, older_high); // perf-allowed: structural/indicator series read; called from QM_IsNewBar-gated context
