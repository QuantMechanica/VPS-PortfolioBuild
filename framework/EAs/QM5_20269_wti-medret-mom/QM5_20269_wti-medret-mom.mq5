...
#property description "QM5_20269 WTI Median-Return Momentum"
...
//   3. sort the returns and average zero-based center indexes 5 and 6;
//   4. trade the median sign, consuming an exact-zero/invalid month flat.
...
double g_cache_median_return   = 0.0;
...
void Strategy_ResetCachedState()
  {
   ...
   g_cache_signal = 0;
   g_cache_median_return = 0.0;
   g_cache_center_low = 0.0;
   g_cache_center_high = 0.0;
   g_cache_state_reason = "not_evaluated";
  }

bool Strategy_MedianReturnSignal(const double &closes[], int &signal)
  {
   signal = 0;
   ...
   const double center_low = sorted_returns[5];
   const double center_high = sorted_returns[6];
   const double median_return = 0.5 * (center_low + center_high);
   if(!MathIsValidNumber(center_low) ||
      !MathIsValidNumber(center_high) ||
      center_low > center_high ||
      !MathIsValidNumber(median_return))
     {
      g_cache_state_reason = "invalid_median_state";
      return false;
     }

   g_cache_center_low = center_low;
   g_cache_center_high = center_high;
   g_cache_median_return = median_return;
   if(median_return == 0.0)
     {
      g_cache_state_reason = "exact_zero_median";
      return true;
     }

   signal = (median_return > 0.0) ? 1 : -1;
...
bool Strategy_LoadSignalState(int &signal)
  {
   ...
   return Strategy_MedianReturnSignal(monthly_closes, signal);
  }
...
void Strategy_PrepareMonthlySignal()
  {
   ...
   QM_LogEvent(QM_INFO,
               "MONTHLY_STATE",
               StringFormat("{\"month\":%d,\"valid\":%s,\"signal\":%d,\"median_return\":%.10f,\"center_low\":%.10f,\"center_high\":%.10f,\"state\":\"%s\"}",
                            g_cache_month_key,
                            g_cache_signal_valid ? "true" : "false",
                            g_cache_signal,
                            g_cache_median_return,
                            g_cache_center_low,
                            g_cache_center_high,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 20269 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20269_WTI_MEDRET_MOM";
   ...
   req.reason = (g_cache_signal > 0) ? "MEDRET_MOM_XTI_LONG" : "MEDRET_MOM_XTI_SHORT";
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_20269_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_20269\",\"ea\":\"wti-medret-mom\"}");
