...
#property description "QM5_12586 EIA XNG Winter Withdrawal Breakout"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_12586 - EIA XNG Winter Withdrawal Breakout
// -----------------------------------------------------------------------------
// D1 structural natural-gas sleeve:
//   - trades only during the November-March winter withdrawal regime
//   - enters on prior-close Donchian breakouts with slow SMA confirmation
//   - exits on opposite-channel failure, season end, max hold, or ATR stop
// Runtime uses MT5 OHLC only; no storage, weather, or futures-curve feed.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 12586;
...
bool Strategy_InWinterWindow(const datetime t)
  {
   MqlDateTime dt;
   TimeToStruct(t, dt);
   return (dt.mon == 11 || dt.mon == 12 || dt.mon == 1 || dt.mon == 2 || dt.mon == 3);
  }
...
void Strategy_CloseOpenPositionsIfNeeded()
  {
   ...
   const bool in_window = Strategy_InWinterWindow(closed_time);
   const int magic = QM_FrameworkMagic();
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12586_EIA_XNG_WINTER";
   ...
   if(close_last > entry_high && close_last > sma_last)
      direction = 1;
   else if(close_last < entry_low && close_last < sma_last)
      direction = -1;
   else
      return false;
   ...
   req.reason = (direction > 0) ? "XNG_WINTER_BREAK_LONG" : "XNG_WINTER_BREAK_SHORT";
   return true;
  }
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_12586\",\"ea\":\"eia-xng-winter-brk\"}");
