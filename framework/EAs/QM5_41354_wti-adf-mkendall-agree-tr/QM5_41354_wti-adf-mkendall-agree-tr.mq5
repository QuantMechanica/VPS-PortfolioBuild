...
#property description "QM5_41354 WTI Monthly ADF-Mann-Kendall Agreement Trend"
...
//   - require ADF t >= -2.594 and newest-thirteen Mann-Kendall abs(S) >= 28
//   - use the all-pairs score sign only when both functions qualify
...
input int    strategy_rank_points           = 13;
input int    strategy_min_abs_score         = 28;
...
bool Strategy_MannKendallCore(const double &closes[],
                              int &score,
                              int &pair_count,
                              string &rank_path)
  {
   ...
   const int endpoint_count = ArraySize(closes);
   if(endpoint_count != 60 || strategy_level_count != 60 ||
      strategy_rank_points != 13 || strategy_min_abs_score != 28)
      return false;
   const int start = endpoint_count - strategy_rank_points;
   if(start != 47)
      return false;

   for(int i = start; i < endpoint_count - 1; ++i)
     {
      if(closes[i] <= 0.0 || !MathIsValidNumber(closes[i]))
         return false;
      for(int j = i + 1; j < endpoint_count; ++j)
        {
         if(closes[j] <= 0.0 || !MathIsValidNumber(closes[j]) ||
            closes[j] == closes[i])
            return false;
         score += (closes[j] > closes[i]) ? 1 : -1;
         ++pair_count;
        }
     }
   if(pair_count != 78 || score < -78 || score > 78)
      return false;
   rank_path = StringFormat("start=%d,pairs=%d,score=%d",
                            start, pair_count, score);
   return true;
  }
                              ...
                              Strategy_SignalMetrics &metrics)
  {
   ...
   if(strategy_level_count != 60 ||
      ...
      MathAbs(strategy_adf_t_min - (-2.594)) >
         0.000000000000001 ||
      strategy_rank_points != 13 ||
      strategy_min_abs_score != 28 ||
   ...
   if(!Strategy_MannKendallCore(closes,
                                metrics.rank_score,
                                metrics.rank_pairs,
                                metrics.rank_path))
      return false;
   metrics.rank_qualified =
      (MathAbs(metrics.rank_score) >= strategy_min_abs_score);
   metrics.persistence_qualified =
      (metrics.adf_qualified && metrics.rank_qualified);
   metrics.oldest_close = closes[0];
   metrics.newest_close = closes[metrics.endpoint_count - 1];

   if(!metrics.persistence_qualified)
      return true;
   metrics.direction = (metrics.rank_score > 0) ? 1 : -1;
   return true;
  }
bool Strategy_AgreementReferenceSelfTest()
  {
   ...
   double boundary_closes[60];
   double mirror_closes[60];
   double tied_closes[60];
   const double boundary_values[13] =
      {13,1,4,12,5,2,3,6,7,8,9,10,11};
   ...
   for(int index = 0; index < 60; ++index)
     {
      ...
      boundary_closes[index] = 100.0 + (double)index;
      mirror_closes[index] = 200.0 - (double)index;
      tied_closes[index] = 100.0 + (double)index;
     }
   for(int rank_index = 0; rank_index < 13; ++rank_index)
     {
      boundary_closes[47 + rank_index] = boundary_values[rank_index];
      mirror_closes[47 + rank_index] = 14.0 - boundary_values[rank_index];
      tied_closes[47 + rank_index] = boundary_values[rank_index];
     }
   tied_closes[59] = tied_closes[58];

   Strategy_SignalMetrics metrics;
   if(!Strategy_AgreementSignal(up_closes, metrics) ||
      !metrics.adf_qualified || !metrics.rank_qualified ||
      !metrics.persistence_qualified || metrics.direction != 1 ||
      metrics.rank_pairs != 78 || metrics.rank_score != 62 ||
   ...
   int pairs = 0;
   string path = "";
   if(!Strategy_MannKendallCore(boundary_closes, score, pairs, path) ||
      score != 28 || pairs != 78 || StringLen(path) <= 0)
      return false;
   if(!Strategy_MannKendallCore(mirror_closes, score, pairs, path) ||
      score != -28 || pairs != 78)
      return false;
   if(Strategy_MannKendallCore(tied_closes, score, pairs, path))
      return false;
   return true;
  }
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   else
     {
      ...
      if(!g_signal_valid)
         g_signal_state = "agreement_validation_failed";
              ...
              !g_signal_metrics.rank_qualified)
         g_signal_state = "both_gates_reject_consumed_flat";
      else if(!g_signal_metrics.adf_qualified)
         g_signal_state = "adf_rejects_rank_only_consumed_flat";
      else if(!g_signal_metrics.rank_qualified)
         g_signal_state = "rank_rejects_adf_only_consumed_flat";
      else if(g_signal_metrics.direction == 0)
         g_signal_state = "agreement_invalid_rank_direction_flat";
      else if(g_signal_metrics.direction > 0)
         g_signal_state = "agreement_positive_rank_wti_long";
      else if(g_signal_metrics.direction < 0)
         g_signal_state = "agreement_negative_rank_wti_short";
      else
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"endpoint_count\":%d,\"adf_t\":%.12f,\"adf_qualified\":%s,\"rank_score\":%d,\"rank_pairs\":%d,\"rank_qualified\":%s,\"agreement_qualified\":%s,\"levels\":\"%s\",\"regression\":\"%s\",\"rank_path\":\"%s\",\"oldest_endpoint\":%I64d,\"newest_endpoint\":%I64d,\"state\":\"%s\"}",
                            ...
                            g_signal_metrics.rank_score,
                            g_signal_metrics.rank_pairs,
                            g_signal_metrics.rank_qualified ? "true" : "false",
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41354 ||
      qm_magic_slot_offset != 0 ||
      QM_FrameworkMagic() != 413540000 ||
      qm_rng_seed != 42)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41354_WTI_ADF_MK_AGREE";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41354 ||
      qm_magic_slot_offset != 0)
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI monthly ADF-Mann-Kendall agreement trend holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41354_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   g_entry_month_state_key =
      StringFormat("QM5_41354_ENTRY_MONTH_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41354\",\"ea\":\"wti-adf-mkendall-agree-tr\"}");
