...
#property description "QM5_41034 WTI Monthly Information-Flow Agreement"
...
//   - require both accumulated components to share one strict sign
//   - reconcile their sum to the completed month-end-to-month-end log return
//   - follow the agreed direction until the next normalized broker month
...
bool Strategy_LoadMonthlyFlowSignal(const int current_month_key,
                                    ...
                                    double &total_flow)
  {
   ...
   const bool signs_agree_positive =
      (overnight_flow > 0.0 && session_flow > 0.0);
   const bool signs_agree_negative =
      (overnight_flow < 0.0 && session_flow < 0.0);
   if(signs_agree_positive && total_flow > 0.0)
      direction = 1;
   else if(signs_agree_negative && total_flow < 0.0)
      direction = -1;
   return true;
  }
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41034 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41034_WTI_MFLOW_AGREE";
   ...
   req.reason = (g_signal_direction > 0)
      ? "WTI_MFLOW_AGREE_LONG"
      : "WTI_MFLOW_AGREE_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_strategy_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41034 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved monthly flow-agreement card holds through Fridays until next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41034_MFLOW_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41034\",\"ea\":\"wti-mflow-agree\"}");
