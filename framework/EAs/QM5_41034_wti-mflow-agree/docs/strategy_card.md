...
type: strategy
strategy_id: WILLIAMS-MOP-WTI-MFLOWAGREE-2026_S01
variant_id: WILLIAMS-MOP-WTI-MFLOWAGREE-2026_S01
source_id: WILLIAMS-MOP-WTI-MFLOWAGREE-2026
ea_id: QM5_41034
slug: wti-mflow-agree
status: APPROVED
execution_contract_ref: strategy-seeds/cards/approved/QM5_41034_wti-mflow-agree_card.md
execution_contract_status: APPROVED
created: 2026-08-17
created_by: Research+Development
last_updated: 2026-08-17
g0_status: APPROVED
g0_decision: decisions/2026-08-17_wti_monthly_flow_agreement_g0.md
source_approval: decisions/2026-08-17_wti_monthly_flow_agreement_source_approval.md
...
strategy_mechanic: exact-immediately-completed-broker-month-wti-close-open-and-open-close-log-flow-strict-sign-agreement-entry-first-new-month-hold-to-next-month
sources:
  - "[[sources/WILLIAMS-MOP-WTI-MFLOWAGREE-2026]]"
concepts:
  - "[[concepts/price-flow-decomposition]]"
  - "[[concepts/monthly-flow-agreement]]"
  - "[[concepts/monthly-return-reconciliation]]"
indicators:
  - "[[indicators/completed-log-return]]"
  - "[[indicators/broker-calendar]]"
  - "[[indicators/atr-risk-stop]]"
strategy_type_flags: [commodity, energy, crude-oil, price-flow-decomposition, time-series-momentum, monthly-entry, monthly-hold, atr-hard-stop, low-frequency, symmetric-long-short]
...
period: D1
timeframe: D1
expected_trade_frequency: "Approximately 5-8 completed WTI positions per full post-warm-up year after strict monthly component agreement; Q02 must prove at least five/year or retire."
...
review_focus: "Falsify a direct-WTI monthly information-flow agreement sleeve outside the certified XAU/SP500/NDX/XNG book. Verify every completed prior-month close/open endpoint, strict component agreement, reconciliation, no late/repeated new-month entry, and next-month renewal; Q09 alone may establish realized decorrelation."
modules_used: [no_trade, trade_entry, trade_management, trade_close]
target_modules: [Strategy_NoTradeFilter, Strategy_EntrySignal, Strategy_ManageOpenPosition, Strategy_ExitSignal, Strategy_NewsFilterHook]
hard_rules_at_risk: [immediately_completed_broker_month, normalized_energy_label, completed_close_open_endpoints, strict_flow_sign_agreement, flow_reconciliation, no_current_bar_leakage, monthly_decision_clock, monthly_attempt_state, no_late_restart_entry, risk_mode_dual, friday_close_disabled, cfd_futures_basis, q02_frequency_floor, portfolio_correlation]
g0_approval_reasoning: "R1 complete OWNER-supplied Tier-A flow-decomposition extraction plus complete-read peer-reviewed WTI one-month carrier lineage with disclosed conjunction risk; R2 exact month, endpoints, agreement, reconciliation, direction, timing, retry, risk, and lifecycle; R3 native XTI D1 only; R4 deterministic arithmetic without banned signal or trained logic; canonical dedup raised the expected weekly flow-agreement neighbor and manual review fixed all material cadence and state boundaries."
---

# QM5_41034 WTI Monthly Information-Flow Agreement

## Hypothesis

A completed WTI month whose close-to-open public flow and open-to-close
session flow share one strict sign contains directionally concordant
information across both trading-time channels. Following the fully reconciled
month direction through the next broker month may isolate a structural
continuation state while excluding months whose total return masks internal
flow disagreement.

This is a falsifiable price-flow and calendar translation. The sources do not
test the exact monthly agreement gate, continuous CFD, fixed entry clock, ATR
stop, or QM portfolio.

## Source Traceability And Claim Boundary

The sole governed composite packet is
`strategy-seeds/sources/WILLIAMS-MOP-WTI-MFLOWAGREE-2026/source.md`, approved
before card extraction in
`decisions/2026-08-17_wti_monthly_flow_agreement_source_approval.md` at commit
`ddb43e0da`.
...
they do not validate this component-agreement gate.

The exact completed-month selection, component log sums, strict agreement,
telescoping reconciliation, normalized label convention, first-new-month
clock, continuous-CFD carrier, hard stop, spread cap, fixed-dollar risk,
Friday-close override, and attempt ledger are disclosed QM choices. No source
return, alpha, coefficient, significance, trade density, drawdown, cost, WTI-
only efficacy, CFD equivalence, decorrelation, or portfolio result transfers.
...
- Neither source defines monthly component agreement, a continuous-CFD label
  policy, an ATR stop, a spread ceiling, an attempt ledger, or a fixed cash
  risk budget.

## QM Interpretations

- The strict agreement-only eligibility state and direction from the
  reconciled total are pre-result QM mechanizations, not source findings.
...
The deterministic pre-allocation checker scanned 4,521 EA-registry rows and
617 root cards. It found no exact match and raised one expected fuzzy neighbor.
Manual review fixes the load-bearing boundaries:

- `QM5_41029_wti-flow-agree` uses one exact Monday-Friday formation and a
  Monday-Friday hold. This card uses every session in one completed broker
  month and holds until the next broker month.
- `QM5_20187_wti-tsmom1m` trades every nonzero completed-month total. This card
  rejects every month whose close-to-open and open-to-close sums oppose.
- `QM5_41032_wti-flow-div` and `QM5_41033_wti-flow-dom` require weekly
  opposition. This card requires monthly agreement, a disjoint eligible state
  at another cadence.
...
- Timeframe: D1; magic slot 0; planned magic `410340000`.
- Decision: first executable D1 tick of a new normalized broker month.
- Signal: same strict sign for all completed prior-month close-to-open and
  open-to-close log-return sums; direction equals reconciled total-flow sign.
...
- Expected cadence: approximately 5-8 completed positions/year after
  agreement, history, and entry-safety exclusions.
...
9. Require both accumulated component sums to share one strict nonzero sign.
10. Require their total to reconcile within `1e-10` to the anchor-to-prior-
    month-end log return.
11. BUY for two positive component sums and SELL for two negative sums.
    Opposition, exact zero, invalid arithmetic, failed reconciliation, or a
    missing endpoint consumes the month flat.
...
- Exact host `XTIUSD.DWX`, D1, `qm_ea_id=41034`, and slot 0 only.
...
- Fail closed on invalid label offset, date/month identity, history, OHLC,
  timestamps, component agreement, reconciliation, spread, ATR, quote, stop,
...
- trade_entry: monthly flow decomposition, strict agreement,
  reconciliation, symmetric direction, ATR hard stop, slot 0.
...
- wrong or nonconsecutive month identity, nonuniform label repair, or late
  entry;
- entry when component signs oppose or either is zero;
- direction different from both agreed component signs and total-flow sign;
...
2. Run deterministic reference cases covering zero and `+1` label offsets,
   ...
   component arithmetic, agreement/disagreement/zero, reconciliation,
   direction, attempt state, risk, and rollover.
...
|---|---|---|---|---|
| v1 | 2026-08-17 | initial monthly flow-agreement extraction | G0 | APPROVED |
| v1-build | 2026-08-17 | deterministic EA, fixed-risk set, strict compile, focused build check, and static P1 | Q01 | PASS |
| v1-capacity | 2026-08-17 | exact-path factory sample reached the 7/7 ceiling; no queue dry run or enqueue | Q02 | NOT_ENQUEUED_CPU_CEILING |

## Pipeline Phase Status

- G0: `APPROVED`.
- Q01: `PASS` — deterministic implementation, 15-case reference suite,
  strict compile, focused build checks, and static P1 all passed.
- Q02: `NOT_ENQUEUED_CPU_CEILING` — the exact-path T1-T10 sample was
  7/7, so the binding stop condition prevented both queue dry run and enqueue.
