"""Independent mechanic fixtures for QM5_41034.
...
agreement, month-return reconciliation, first-new-month grace, one-attempt
identity, fixed-risk inputs, and next-month stale repair. It does not invoke
MT5 or duplicate framework order plumbing.
"""
...
def select_direction(
    ...
    if abs(total_flow - month_return) > tolerance:
        return 0
    if overnight_flow > 0 and session_flow > 0 and total_flow > 0:
        return 1
    if overnight_flow < 0 and session_flow < 0 and total_flow < 0:
        return -1
    return 0
...
class MonthlyFlowAgreementReferenceTests(unittest.TestCase):
    ...
    def test_session_count_bounds_are_load_bearing(self) -> None:
        self.assertEqual(flow_direction(100.0, make_bars(100.0, 14, 1.01, 1.01))[0], 0)
        self.assertEqual(flow_direction(100.0, make_bars(100.0, 26, 1.01, 1.01))[0], 0)
        self.assertEqual(flow_direction(100.0, make_bars(100.0, 15, 1.01, 1.01))[0], 1)
        self.assertEqual(flow_direction(100.0, make_bars(100.0, 25, 1.01, 1.01))[0], 1)

    def test_positive_component_agreement_buys(self) -> None:
    ...
    def test_negative_component_agreement_sells(self) -> None:
    ...
    def test_component_opposition_is_flat(self) -> None:
        direction, overnight, session, month_return, _ = flow_direction(
            100.0, make_bars(100.0, 20, 1.01, 0.995)
        )
        self.assertGreater(overnight, 0)
        self.assertLess(session, 0)
        self.assertEqual(select_direction(overnight, session, month_return), 0)
    ...
    def test_failed_reconciliation_is_flat(self) -> None:
        self.assertEqual(select_direction(0.1, 0.2, 0.4), 0)
        self.assertEqual(
            select_direction(0.1, 0.2, 0.3 + 2 * RECONCILE_TOLERANCE),
            0,
        )

    def test_all_endpoints_telescope_to_month_return(self) -> None:
        bars = make_bars(100.0, 21, 1.003, 1.002)
    ...
    def test_invalid_completed_endpoint_consumes_flat(self) -> None:
        bars = list(make_bars(100.0, 20, 1.003, 1.002))
