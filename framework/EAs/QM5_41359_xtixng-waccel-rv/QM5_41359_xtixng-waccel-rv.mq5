...
#property description "QM5_41359 XTI/XNG Weekly Acceleration-Overshoot Reversion"
...
// -----------------------------------------------------------------------------
// Reconstruct three synchronized completed broker-week-end XTI/XNG close
// pairs and form two adjacent changes in ln(XTI)-ln(XNG). When the returns
// have strict same signs and the newest absolute move is strictly larger,
// fade that same-direction acceleration with an equal-notional opposite-leg package.
// Consume one attempt per week and flatten at the next week boundary.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41359;
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41359 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XTIXNG_WACCEL_ATTEMPT_WEEK", qm_ea_id);
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41359_XTIXNG_WACCEL_RV";
                                  ...
                                  int &direction)
  {
   ...
   for(int index = 0;
       ...
       ++index)
     {
      ...
      double spread = 0.0;
      if(!Strategy_LogRatio(xti_bars[index].close,
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41359_XTIXNG_WACCEL_RV_HOST";
   ...
   if(Strategy_OpenOwnedPositionCount() > 0 ||
   ...
   else
     {
      ...
      if(!signal_valid)
         ...
         g_signal_state = "positive_acceleration_overshoot_short_xti";
      else if(direction > 0)
         g_signal_state = "negative_acceleration_overshoot_long_xti";
      else if(g_new_relative_return == 0.0 ||
              g_old_relative_return == 0.0)
         g_signal_state = "exact_zero_flat";
      else if((g_new_relative_return > 0.0) !=
              (g_old_relative_return > 0.0))
         g_signal_state = "opposed_sign_flat";
      else
         g_signal_state = "no_strict_newest_dominance_flat";
     }
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XTI/XNG weekly acceleration-overshoot package exits at the next broker week"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41359\",\"ea\":\"xtixng-waccel-rv\"}");
