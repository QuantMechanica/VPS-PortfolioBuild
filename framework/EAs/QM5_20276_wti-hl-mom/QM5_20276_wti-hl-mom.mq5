...
#property description "QM5_20276 WTI Hodges-Lehmann Return Momentum"
...
//   3. form all 78 inclusive pairwise averages for 0 <= i <= j <= 11;
//   4. sort them and trade the average of zero-based center indexes 38 and 39.
// This robust return-location functional is a transparent QM mechanization,
// not an author rule. Runtime uses native XTIUSD.DWX broker data only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 20276;
...
input int    strategy_pair_count          = 78;
...
double g_cache_hl_return        = 0.0;
double g_cache_pair_center_low  = 0.0;
double g_cache_pair_center_high = 0.0;
...
bool Strategy_LoadMonthlyCloses(double &closes[])
  {
   ...
   if(required_closes != strategy_return_count + 1 ||
      required_closes != 13 ||
...
void Strategy_ResetCachedState()
  {
   ...
   g_cache_signal = 0;
   g_cache_hl_return = 0.0;
   g_cache_pair_center_low = 0.0;
   g_cache_pair_center_high = 0.0;
   g_cache_state_reason = "not_evaluated";
  }

bool Strategy_HodgesLehmannSignal(const double &closes[], int &signal)
  {
   signal = 0;
   const int endpoint_count = strategy_endpoint_count;
   const int return_count = strategy_return_count;
   const int expected_pair_count = strategy_pair_count;
   if(endpoint_count != 13 ||
      return_count != 12 ||
      expected_pair_count != 78 ||
      ArraySize(closes) != endpoint_count ||
      endpoint_count != return_count + 1)
     {
      g_cache_state_reason = "bad_endpoint_count";
      return false;
     }
   ...
   double sorted_pair_averages[];
   ArrayResize(sorted_pair_averages, expected_pair_count);
   int pair_index = 0;
   for(int i = 0; i < return_count; ++i)
     {
      for(int j = i; j < return_count; ++j)
        {
         if(pair_index >= expected_pair_count)
           {
            g_cache_state_reason = "pair_count_overflow";
            return false;
           }
         const double pair_average =
            0.5 * monthly_returns[i] + 0.5 * monthly_returns[j];
         if(!MathIsValidNumber(pair_average))
           {
            g_cache_state_reason = "invalid_pair_average";
            return false;
           }
         sorted_pair_averages[pair_index] = pair_average;
         ++pair_index;
        }
     }
   if(pair_index != expected_pair_count)
   ...
   ArraySort(sorted_pair_averages);
   const double center_low = sorted_pair_averages[38];
   const double center_high = sorted_pair_averages[39];
   const double hl_return = 0.5 * (center_low + center_high);
   if(!MathIsValidNumber(center_low) ||
      !MathIsValidNumber(center_high) ||
      center_low > center_high ||
      !MathIsValidNumber(hl_return))
     {
      g_cache_state_reason = "invalid_hl_state";
      return false;
     }

   g_cache_pair_center_low = center_low;
   g_cache_pair_center_high = center_high;
   g_cache_hl_return = hl_return;
   if(hl_return == 0.0)
     {
      g_cache_state_reason = "exact_zero_hl";
      return true;
     }

   signal = (hl_return > 0.0) ? 1 : -1;
...
bool Strategy_LoadSignalState(int &signal)
  {
   ...
   return Strategy_HodgesLehmannSignal(monthly_closes, signal);
  }
...
void Strategy_PrepareMonthlySignal()
  {
   ...
   QM_LogEvent(QM_INFO,
               "MONTHLY_STATE",
               StringFormat("{\"month\":%d,\"valid\":%s,\"signal\":%d,\"hl_return\":%.10f,\"pair_center_low\":%.10f,\"pair_center_high\":%.10f,\"state\":\"%s\"}",
                            g_cache_month_key,
                            g_cache_signal_valid ? "true" : "false",
                            g_cache_signal,
                            g_cache_hl_return,
                            g_cache_pair_center_low,
                            g_cache_pair_center_high,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 20276 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20276_WTI_HLRET_MOM";
   ...
   req.reason = (g_cache_signal > 0) ? "HLRET_MOM_XTI_LONG" : "HLRET_MOM_XTI_SHORT";
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_20276_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_20276\",\"ea\":\"wti-hl-mom\"}");
