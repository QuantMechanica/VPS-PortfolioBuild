...
#property description "QM5_11463 goodwin-j-session-high-breakout-usdjpy — NY-session range breakout + prior-D1-color filter (H1)"
...
//         (thetransparenttrader.com, ~2020), Strategy 3 "H2 Session Breakout".
// Card: artifacts/cards_approved/QM5_11463_goodwin-j-session-high-breakout-usdjpy.md
//       (g0_status APPROVED).
//
// Mechanics (USDJPY primary; H1):
//   The NY session opens 17:00 EST and the market builds a range over the first
   ...
   if(!in_accum && g_window_open)
     {
      ...
      if(!strategy_use_prior_bar_filter)
        {
         // Filter off: still need a side; default to the wider side is ambiguous,
         // so fall back to prior-bar color but treat a doji as long-bias.
         if(dir == 0) dir = +1;
        }
...
// The session range is STATE (already accumulated). The break of the locked
// extreme is the single EVENT — latched via g_fired so it never re-fires and
// never produces two crosses on one tick.
   ...
   if(g_dir == +1)
     {
      ...
      req.reason = "goodwin_session_high_break_long";
   ...
   if(g_dir == -1)
     {
      ...
      req.sl     = sl;
      req.tp     = 0.0;
      req.reason = "goodwin_session_low_break_short";
      g_fired    = true;
      return true;
     }
...
// EOD time exit: close any open position at/after the EOD exit minute (broker).
// We detect "we have reached or passed the EOD exit time and are no longer in
// the active trade leg" — i.e. the trade leg has ended for this session.
