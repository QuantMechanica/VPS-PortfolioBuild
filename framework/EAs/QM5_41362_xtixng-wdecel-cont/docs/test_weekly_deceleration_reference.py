"""Reference checks for QM5_41362 XTI/XNG weekly deceleration continuation."""
...
def weekly_deceleration_signal(
    current_week_key: int,
    completed_newest_first: list[PairBar],
) -> tuple[bool, int, float, float, tuple[float, float, float]]:
    """Mirror strict same signs and strictly smaller newest continuation."""
...
class WeeklyDecelerationReferenceTest(unittest.TestCase):
    def test_smaller_positive_same_sign_move_buys_xti(self) -> None:
        bars = pair_bars((5.20, 5.10, 4.80))
        valid, direction, new_value, old_value, spreads = weekly_deceleration_signal(20260824, bars)
        self.assertTrue(valid)
        self.assertEqual(direction, 1)
        self.assertAlmostEqual(new_value, 0.10)
        self.assertAlmostEqual(old_value, 0.30)
        self.assertGreater(spreads[0], spreads[2])
        self.assertGreater(spreads[0], spreads[1])

    def test_smaller_negative_same_sign_move_sells_xti(self) -> None:
        bars = pair_bars((4.80, 4.90, 5.20))
        valid, direction, new_value, old_value, spreads = weekly_deceleration_signal(20260824, bars)
        self.assertTrue(valid)
        self.assertEqual(direction, -1)
        self.assertAlmostEqual(new_value, -0.10)
    ...
    def test_equality_acceleration_opposite_sign_and_zero_are_flat(self) -> None:
        cases = (
            (5.20, 5.10, 5.00),
            (5.40, 5.10, 5.00),
            (5.00, 5.10, 5.00),
        ...
        for spreads in cases:
            with self.subTest(spreads=spreads):
                valid, direction, *_ = weekly_deceleration_signal(20260824, pair_bars(spreads))
                self.assertTrue(valid)
    ...
    def test_endpoint_validation_rejects_bad_order_gap_and_price(self) -> None:
        ...
        self.assertFalse(weekly_deceleration_signal(20260824, bars)[0])
        bars = pair_bars((5.20, 5.05, 5.10))
