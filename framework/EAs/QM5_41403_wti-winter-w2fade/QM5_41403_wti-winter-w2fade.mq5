...
#property description "QM5_41403 WTI Winter Two-Week Exhaustion Fade"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41403 - WTI Winter Two-Week Exhaustion Fade
// -----------------------------------------------------------------------------
// D1 structural crude-oil sleeve:
//   - decide only in November through May
//   - aggregate the two immediately completed 3-to-5-session normalized weeks
//   - fade only strict same-sign weekly return agreement
...
input int    strategy_history_bars         = 24;
...
int      g_newer_week_bars        = 0;
int      g_older_week_bars        = 0;
double   g_newer_week_open        = 0.0;
double   g_newer_week_close       = 0.0;
double   g_older_week_open        = 0.0;
double   g_older_week_close       = 0.0;
double   g_newer_week_log_return  = 0.0;
double   g_older_week_log_return  = 0.0;
...
void Strategy_ResetDecisionState()
  {
   ...
   g_older_week_open = 0.0;
   g_older_week_close = 0.0;
   g_newer_week_log_return = 0.0;
   g_older_week_log_return = 0.0;
   g_signal_state = "idle";
  }
...
bool Strategy_LoadTwoWeekFadeSignal(const int current_week_key,
                                         const int label_offset,
                                         int &direction)
  {
   direction = 0;
   ...
   ArraySetAsSeries(bars, true);
   const int copied =
      CopyRates(_Symbol, // perf-allowed: bounded completed-week scan behind the sole QM_IsNewBar branch.
                PERIOD_D1,
                1,
                strategy_history_bars,
                bars);
   if(copied < 2 * strategy_min_week_bars + 1)
      return false;
   ...
   for(int index = 0; index < copied; ++index)
     {
      ...
      if(week_keys[week_index] == 0)
        {
         ...
         else if(Strategy_NextWeekKey(week_key) != week_keys[0])
            return false;
         week_keys[week_index] = week_key;
        }
      else if(week_key != week_keys[week_index])
        {
         if(week_index == 0)
           {
            if(Strategy_NextWeekKey(week_key) != week_keys[0])
               return false;
            week_index = 1;
            week_keys[week_index] = week_key;
           }
      ...
      if(week_bars[week_index] == 0)
         week_closes[week_index] = bars[index].close;
      week_opens[week_index] = bars[index].open;
      ++week_bars[week_index];
      if(week_bars[week_index] > strategy_max_week_bars)
         return false;
     }

   if(!older_boundary_seen || week_index != 1)
      return false;
   for(int package_index = 0; package_index < 2; ++package_index)
     {
      if(week_bars[package_index] < strategy_min_week_bars ||
         week_bars[package_index] > strategy_max_week_bars ||
         week_opens[package_index] <= 0.0 ||
         week_closes[package_index] <= 0.0 ||
         !MathIsValidNumber(week_opens[package_index]) ||
         !MathIsValidNumber(week_closes[package_index]))
         return false;
     }

   g_newer_week_bars = week_bars[0];
   g_older_week_bars = week_bars[1];
   g_newer_week_open = week_opens[0];
   g_newer_week_close = week_closes[0];
   g_older_week_open = week_opens[1];
   g_older_week_close = week_closes[1];
   g_newer_week_log_return =
      MathLog(g_newer_week_close / g_newer_week_open);
   g_older_week_log_return =
      MathLog(g_older_week_close / g_older_week_open);
   if(!MathIsValidNumber(g_newer_week_log_return) ||
      !MathIsValidNumber(g_older_week_log_return))
      return false;

   // Both adjacent completed weeks must agree strictly. Trade the inverse
   // sign; disagreement and exact zero are valid flat states.
   if(g_newer_week_log_return > strategy_return_epsilon &&
      g_older_week_log_return > strategy_return_epsilon)
      direction = -1;
   else if(g_newer_week_log_return < -strategy_return_epsilon &&
           g_older_week_log_return < -strategy_return_epsilon)
      direction = 1;
   return true;
  }
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_WeekAlreadyEntered(g_decision_week_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadTwoWeekFadeSignal(g_decision_week_key,
                                             g_decision_label_offset,
                                             g_signal_direction);
      if(!g_signal_valid)
         g_signal_state = "two_week_validation_failed";
      else if(g_signal_direction > 0)
         g_signal_state = "two_negative_weeks_exhaustion_long";
      else if(g_signal_direction < 0)
         g_signal_state = "two_positive_weeks_exhaustion_short";
      else
         g_signal_state = "weekly_sign_disagreement_or_zero_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"week\":%d,\"anchor_month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_week_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"newer_week_bars\":%d,\"older_week_bars\":%d,\"newer_week_return\":%.12f,\"older_week_return\":%.12f,\"state\":\"%s\"}",
                            g_decision_week_key,
                            anchor_month,
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41403_WTI_WINTER_W2FADE";
   ...
   req.reason = (g_signal_direction > 0)
      ? "WTI_WINTER_W2_FADE_LONG"
      : "WTI_WINTER_W2_FADE_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(strategy_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41403 ||
      qm_magic_slot_offset != 0)
   ...
   g_attempt_state_key =
      StringFormat("QM5_41403_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41403\",\"ea\":\"wti-winter-w2fade\"}");
