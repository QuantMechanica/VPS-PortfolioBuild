...
#property description "QM5_13148 XTI XNG Fixed-Origin Rank Low-Minus-High"
...
double   g_cache_xng_normalized = 0.0;
double   g_cache_normalized_difference = 0.0;
int      g_cache_xti_observations = 0;
int      g_cache_xng_observations = 0;
datetime g_cache_xti_anchor_time = 0;
datetime g_cache_xng_anchor_time = 0;
datetime g_cache_xti_endpoint_time = 0;
datetime g_cache_xng_endpoint_time = 0;
...
bool Strategy_NormalizedPrice(const string symbol,
                              const datetime decision_bar_time,
                              const datetime anchor_origin,
                              double &normalized_price,
                              int &observation_count,
                              datetime &effective_anchor_time,
                              datetime &endpoint_time)
  {
   normalized_price = 0.0;
   observation_count = 0;
   effective_anchor_time = 0;
   endpoint_time = 0;
   if(decision_bar_time <= 0 || anchor_origin <= 0)
      return false;
   ...
   if(count <= strategy_min_anchor_age_bars)
      return false;

   endpoint_time = rates[0].time;
   if(endpoint_time <= 0 || endpoint_time >= decision_bar_time)
      return false;
   const long endpoint_gap_seconds = (long)(decision_bar_time - endpoint_time);
   const long max_endpoint_gap_seconds =
      (long)MathMax(1, strategy_max_endpoint_gap_days) * 86400;
   if(endpoint_gap_seconds < 0 || endpoint_gap_seconds > max_endpoint_gap_seconds)
      return false;

   int anchor_index = -1;
   for(int i = count - 1; i >= 0; --i)
     {
      if(rates[i].time < anchor_origin)
         continue;
      if(rates[i].time >= decision_bar_time)
         continue;
      anchor_index = i;
      break;
     }
   ...
   effective_anchor_time = rates[anchor_index].time;
   const long anchor_gap_seconds = (long)(effective_anchor_time - anchor_origin);
   const long max_anchor_gap_seconds =
      (long)MathMax(1, strategy_max_anchor_gap_days) * 86400;
   if(anchor_gap_seconds < 0 || anchor_gap_seconds > max_anchor_gap_seconds)
      return false;

   observation_count = anchor_index;
   if(observation_count < strategy_min_anchor_age_bars)
      return false;

   const double anchor_close = rates[anchor_index].close;
   const double latest_close = rates[0].close;
   if(anchor_close <= 0.0 || latest_close <= 0.0 ||
      !MathIsValidNumber(anchor_close) || !MathIsValidNumber(latest_close))
      return false;
   normalized_price = latest_close / anchor_close;
   return (normalized_price > 0.0 && MathIsValidNumber(normalized_price));
  }
...
bool Strategy_LoadSignalState(const datetime decision_bar_time,
  ...
  {
   pair_direction = 0;
   g_cache_xti_normalized = 0.0;
   g_cache_xng_normalized = 0.0;
   g_cache_normalized_difference = 0.0;
   g_cache_xti_observations = 0;
   g_cache_xng_observations = 0;
   g_cache_xti_anchor_time = 0;
   g_cache_xng_anchor_time = 0;
   g_cache_xti_endpoint_time = 0;
   g_cache_xng_endpoint_time = 0;

   if(decision_bar_time <= 0)
      return false;
   const datetime anchor_origin = StringToTime(strategy_anchor_date);
   if(anchor_origin <= 0)
      return false;
   if(!Strategy_NormalizedPrice(g_leg_xti,
                                decision_bar_time,
                                anchor_origin,
                                g_cache_xti_normalized,
                                g_cache_xti_observations,
                                g_cache_xti_anchor_time,
                                g_cache_xti_endpoint_time))
      return false;
   if(!Strategy_NormalizedPrice(g_leg_xng,
                                decision_bar_time,
                                anchor_origin,
                                g_cache_xng_normalized,
                                g_cache_xng_observations,
                                g_cache_xng_anchor_time,
                                g_cache_xng_endpoint_time))
      return false;

   if(g_cache_xti_anchor_time != g_cache_xng_anchor_time ||
      g_cache_xti_endpoint_time != g_cache_xng_endpoint_time)
      return false;

   g_cache_normalized_difference =
      g_cache_xti_normalized - g_cache_xng_normalized;
   if(!MathIsValidNumber(g_cache_normalized_difference))
      return false;
   if(g_cache_normalized_difference < -1.0e-12)
      pair_direction = 1;
   else if(g_cache_normalized_difference > 1.0e-12)
      pair_direction = -1;
   return true;
  }
...
bool Strategy_OpenPair(const int pair_direction)
  {
   ...
   const string reason = long_xti_short_xng ? "QM5_13148_LONG_XTI_LOW_NORMALIZED_RANK"
                                            : "QM5_13148_LONG_XNG_LOW_NORMALIZED_RANK";
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(strategy_max_anchor_gap_days != 7 || strategy_min_anchor_age_bars != 20)
      return true;
   if(strategy_history_bars < 2600 || strategy_history_bars > 3600)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_13148_XTI_XNG_RANK_LMH_HOST";
...
int OnInit()
  {
   ...
   QM_BasketWarmupHistory(basket_symbols,
                          PERIOD_D1,
                          MathMax(3000, strategy_history_bars));

   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_13148\",\"ea\":\"energy-rank-lmh\"}");
