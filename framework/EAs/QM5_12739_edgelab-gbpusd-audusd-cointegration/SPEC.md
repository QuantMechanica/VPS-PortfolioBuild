# QM5_12739_edgelab-gbpusd-audusd-cointegration - Strategy Spec
...
The EA trades the GBPUSD.DWX and AUDUSD.DWX D1 cointegration spread. It computes
`S = ln(GBPUSD) - beta * ln(AUDUSD)` with beta defaulting to 0.74, then
...
same script ranked GBPUSD/AUDUSD as the strongest remaining unbuilt positive
DEV/OOS candidate after the existing 12532/12533/12624/12712/12723/12728/12731/12732/12735 baskets, with
DEV Sharpe 0.52, OOS net Sharpe 0.30, OOS return +2.37%, 20 OOS state changes,
beta 0.74, and 57-day half-life.
...
| strategy_beta | 0.74 | >0 | Hedge coefficient in `ln(GBPUSD) - beta * ln(AUDUSD)` |
...
**Designed for:**
- GBPUSD.DWX - leg 1 of the fixed GBPUSD/AUDUSD spread and the spread numerator.
- AUDUSD.DWX - leg 2 of the fixed GBPUSD/AUDUSD spread and the beta-weighted spread denominator.

**Explicitly not for:**
- Other `.DWX` symbols. This card is a fixed two-leg FX-cross basket, not a portable multi-pair strategy.

The EA selects GBPUSD.DWX and AUDUSD.DWX only; both legs have direct USD
...
| Regime preference | mean-reverting GBP-vs-AUD USD-major relative-value spread |
