# QM5_20211_gbpjpy-euraud — Strategy Spec
...
`ln(GBPJPY.DWX) - (-1.386054145) * ln(EURAUD.DWX)`. Score the newest spread
...
| `strategy_beta` | -1.386054145 | fixed | DEV-fitted structural hedge coefficient |
...
- `GBPJPY.DWX` — fixed tester host and first traded leg.
- `EURAUD.DWX` — fixed companion and second traded leg.
- `USDJPY.DWX`, `GBPUSD.DWX`, `EURUSD.DWX`, and `AUDUSD.DWX` —
  conversion-history dependencies for JPY/GBP/EUR/AUD valuation under the
  USD-account tester; all four are order-free and have no magic slot.
...
| Bar gating | `QM_IsNewBar(GBPJPY.DWX, PERIOD_D1)` consumed once per signal bar |
...
| Typical hold time | multi-day; scan half-life 33.694 D1 bars |
| Expected drawdown profile | high and regime-sensitive; conservative 30% prior |
| Regime preference | GBPJPY/EURAUD fixed-residual mean reversion |
| Win rate target | unknown; Q02 is the first platform-economic judge |

The fixed scan recorded DEV net Sharpe `0.430254`, OOS net Sharpe `0.273190`,
and OOS return `4.833998%`. The sub-0.8 OOS score is an adverse prior, not a
promotion claim.
...
`1.0` and `1.386054145`; each leg is independently bounded by its ATR stop.
...
|---|---|---|---|
| v1 | 2026-08-04 | Initial build from approved card | Rank-31 non-duplicate FX sleeve |
