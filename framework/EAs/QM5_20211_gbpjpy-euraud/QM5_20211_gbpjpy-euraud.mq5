...
#property description "QM5_20211 GBPJPY EURAUD Cointegration"
...
//
// Card:
//   strategy-seeds/cards/approved/QM5_20211_gbpjpy-euraud_card.md
//
// Fixed spread:
//   ln(GBPJPY.DWX) - beta * ln(EURAUD.DWX)
//
// A negative beta makes the leg directions sign-aware: a long spread buys
// both pairs and a short spread sells both pairs. This is neutrality to the
// fitted residual only; the card preserves the cross-currency risk caveat.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 20211;
...
input double strategy_beta              = -1.386054145;
...
bool Strategy_EnsureBasketScope()
  {
   ...
   string allowed[6] = {"GBPJPY.DWX", "EURAUD.DWX", "USDJPY.DWX", "GBPUSD.DWX", "EURUSD.DWX", "AUDUSD.DWX"};
   for(int i = 0; i < 6; ++i)
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(!Strategy_EnsureBasketScope())
      return false;
   if(!QM_SymbolAssertOrLog(g_leg_gbpjpy) ||
      !QM_SymbolAssertOrLog(g_leg_euraud) ||
   ...
   double gbpjpy[];
   double euraud[];
   datetime gbpjpy_time[];
   datetime euraud_time[];
   ArraySetAsSeries(gbpjpy, true);
   ArraySetAsSeries(euraud, true);
   ArraySetAsSeries(gbpjpy_time, true);
   ArraySetAsSeries(euraud_time, true);

   if(CopyClose(g_leg_gbpjpy, PERIOD_D1, 1, history_count, gbpjpy) != // perf-allowed: called only by the gated spread refresh.
      history_count)
      return false;
   if(CopyClose(g_leg_euraud, PERIOD_D1, 1, history_count, euraud) != // perf-allowed: called only by the gated spread refresh.
      history_count)
      return false;
   if(CopyTime(g_leg_gbpjpy, PERIOD_D1, 1, history_count, gbpjpy_time) != // perf-allowed: called only by the gated spread refresh.
      history_count)
      return false;
   if(CopyTime(g_leg_euraud, PERIOD_D1, 1, history_count, euraud_time) != // perf-allowed: called only by the gated spread refresh.
      history_count)
      return false;
   ...
   for(int i = 0; i < history_count; ++i)
     {
      if(gbpjpy_time[i] <= 0 || gbpjpy_time[i] != euraud_time[i])
         return false;
      if(gbpjpy[i] <= 0.0 || euraud[i] <= 0.0)
         return false;

      spreads[i] =
         MathLog(gbpjpy[i]) - strategy_beta * MathLog(euraud[i]);
...
bool Strategy_PreparePair(const int spread_direction,
                          QM_EntryRequest &host_req)
  {
   ...
   const double gbpjpy_weight = 1.0;
   const double euraud_weight = MathAbs(strategy_beta);
   const double weight_sum = gbpjpy_weight + euraud_weight;
   if(weight_sum <= 0.0)
      return false;

   // Preflight both normalized legs before sending either order. An invalid
   // companion allocation rejects the complete package instead of leaving a
   // directional orphan.
   ...
   const double gbpjpy_lots =
      Strategy_LotsForLeg(g_leg_gbpjpy, gbpjpy_weight, gbpjpy_weight);
   const double euraud_lots =
      Strategy_LotsForLeg(g_leg_euraud, euraud_weight, gbpjpy_weight);
   if(gbpjpy_lots <= 0.0 || euraud_lots <= 0.0)
      return false;

   const bool long_spread = (spread_direction > 0);
   const QM_OrderType gbpjpy_type = long_spread ? QM_BUY : QM_SELL;
   const QM_OrderType euraud_type =
      long_spread
      ? (strategy_beta >= 0.0 ? QM_SELL : QM_BUY)
      : (strategy_beta >= 0.0 ? QM_BUY : QM_SELL);
   const string reason =
      long_spread ? "QM5_20211_LONG_SPREAD_Z_LT_NEG_ENTRY"
                  : "QM5_20211_SHORT_SPREAD_Z_GT_POS_ENTRY";

   const double host_entry =
      QM_OrderTypeIsBuy(gbpjpy_type)
      ? SymbolInfoDouble(g_leg_gbpjpy, SYMBOL_ASK)
      : SymbolInfoDouble(g_leg_gbpjpy, SYMBOL_BID);
   const double host_atr =
      QM_ATR(g_leg_gbpjpy, PERIOD_D1, strategy_atr_period_d1, 1);
   if(host_entry <= 0.0 || host_atr <= 0.0)
      return false;

   host_req.type = gbpjpy_type;
   host_req.price = 0.0;
   host_req.sl = QM_StopATRFromValue(g_leg_gbpjpy,
                                     gbpjpy_type,
                                     host_entry,
                                     host_atr,
                                     strategy_atr_sl_mult);
   ...
   g_companion_entry_type = euraud_type;
   g_companion_entry_lots = euraud_lots;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20211_PAIR_HOST";
...
bool Strategy_NewsFilterHook(const datetime broker_time)
  {
   if(qm_news_temporal != QM_NEWS_TEMPORAL_OFF ||
     ...
     {
      if(!QM_NewsAllowsTrade2(g_leg_gbpjpy,
                              broker_time,
                              qm_news_temporal,
                              qm_news_compliance))
         return true;
     ...
     {
      if(!QM_NewsAllowsTrade(g_leg_gbpjpy,
                             broker_time,
                             qm_news_mode_legacy))
         return true;
      if(!QM_NewsAllowsTrade(g_leg_euraud,
                             broker_time,
                             qm_news_mode_legacy))
         return true;
     }
...
void OnTick()
  {
   ...
   const bool is_new_signal_bar =
      QM_IsNewBar(g_leg_gbpjpy, PERIOD_D1);
