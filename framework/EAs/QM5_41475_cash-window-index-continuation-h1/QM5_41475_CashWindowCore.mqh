// =============================================================================
// QM5_41475 Cash-window index continuation (session-flat, H1) — strategy module
// -----------------------------------------------------------------------------
// All H-CW strategy logic lives here; the .mq5 carries inputs, framework
// wiring, and the Design-2 panel glue only. Rules are the mechanized
// QM-RESEARCH-2026-0002 H_CW card (preregistration cd661891...322e):
//
//   * Session in UTC hours (inputs; broker time mapped via QM_DSTAware).
//   * Long : closed H1 close > max high of the first N session bars (+buffer)
//            AND close > EMA(ema_period).  Short mirrored.
//   * One entry per symbol per day; max_positions_total across the EA family.
//   * Stop = atr_stop_mult * ATR(atr_period); TP = target_r * stop distance.
...
void HcwUtcStruct(const datetime broker_time, MqlDateTime &utc_dt)
  {
...
int HcwUtcDayKey(const MqlDateTime &utc_dt)
  {
...
int      g_hcw_slot              = -1;     // resolved symbol slot (== qm_magic_slot_offset)
int      g_hcw_day_key           = -1;     // UTC day key of the evaluated session
bool     g_hcw_shock_blocked     = false;  // first session bar range shock
bool     g_hcw_trade_taken_today = false;  // one entry per symbol per day
double   g_hcw_breakout_high     = 0.0;    // max high of the first N session bars
double   g_hcw_breakout_low      = 0.0;    // min low of the first N session bars
bool     g_hcw_window_ready      = false;  // at least N session bars closed today

int      g_hcw_spread_day_key    = -1;
double   g_hcw_spread_today[];             // today's H1 spread samples (points)
double   g_hcw_daily_median[];             // rolling last-20 daily medians (points)
double   g_hcw_spread_median     = 0.0;    // median of daily medians (0 = none yet)

bool     g_hcw_breaker_day_hit   = false;
bool     g_hcw_breaker_week_hit  = false;

datetime g_hcw_news_cache_bucket = -1;
bool     g_hcw_news_cache_blocked= false;

ulong    g_hcw_active_ticket     = 0;
bool     g_hcw_be_done           = false;

// ---------------------------------------------------------------------------
// Small helpers
// ---------------------------------------------------------------------------

bool HcwIsOurMagic(const long magic)
  {
   return magic >= (long)qm_ea_id * 10000 && magic < (long)(qm_ea_id + 1) * 10000;
  }

int HcwPortfolioPositionCount()
  {
   int count = 0;
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      if(t == 0 || !PositionSelectByTicket(t))
         continue;
      if(!HcwIsOurMagic(PositionGetInteger(POSITION_MAGIC)))
         continue;
      count++;
     }
   return count;
...
bool HcwSelectOurPosition(ulong &ticket, ENUM_POSITION_TYPE &ptype, double &open_price, double &sl)
  {
   ticket = 0;
...
double HcwMedian(double &values[], const int n)
  {
   if(n <= 0)
      return 0.0;
   double tmp[];
...
void HcwSpreadSample(const datetime closed_bar_time)
  {
   MqlDateTime utc;
   HcwUtcStruct(closed_bar_time, utc);
   const int day_key = HcwUtcDayKey(utc);

   if(day_key != g_hcw_spread_day_key)
     {
      // Finalize the previous day's median into the rolling window.
      const int n = ArraySize(g_hcw_spread_today);
      if(g_hcw_spread_day_key > 0 && n > 0)
        {
         const double med = HcwMedian(g_hcw_spread_today, n);
         const int m = ArraySize(g_hcw_daily_median);
         if(m >= 20)
            ArrayRemove(g_hcw_daily_median, 0, 1);
         ArrayResize(g_hcw_daily_median, ArraySize(g_hcw_daily_median) + 1);
         g_hcw_daily_median[ArraySize(g_hcw_daily_median) - 1] = med;
        }
      ArrayResize(g_hcw_spread_today, 0);
      g_hcw_spread_day_key = day_key;
     }
   ...
   if(ask <= 0.0 || bid <= 0.0)
      return;
   const int n = ArraySize(g_hcw_spread_today);
   ArrayResize(g_hcw_spread_today, n + 1);
   g_hcw_spread_today[n] = (ask - bid) / point;
  }

void HcwSpreadMedianRefresh()
  {
   const int m = ArraySize(g_hcw_daily_median);
   g_hcw_spread_median = (m > 0) ? HcwMedian(g_hcw_daily_median, m) : 0.0;
  }

bool HcwSpreadFilterBlocks()
  {
   const int days = ArraySize(g_hcw_daily_median);
   if(days < strategy_spread_min_days)
      return false; // warmup: not enough history to define a median yet
   HcwSpreadMedianRefresh();
   if(g_hcw_spread_median <= 0.0)
   ...
   if(point <= 0.0 || ask <= 0.0 || bid <= 0.0)
      return false;
   return ((ask - bid) / point) > strategy_spread_median_mult * g_hcw_spread_median;
  }
...
double HcwFamilyPnlSince(const datetime broker_from)
  {
   double pnl = 0.0;
   ...
   for(int i = HistoryDealsTotal() - 1; i >= 0; --i)
     {
      ...
      if(dt == 0)
         continue;
      if(!HcwIsOurMagic(HistoryDealGetInteger(dt, DEAL_MAGIC)))
         continue;
...
void HcwBreakersRefresh()
  {
   MqlDateTime utc;
   HcwUtcStruct(TimeCurrent(), utc);
   ...
   const double day_pnl = HcwFamilyPnlSince(day_start_broker);
   double anchor = AccountInfoDouble(ACCOUNT_EQUITY) - day_pnl;
   if(anchor <= 0.0)
      anchor = AccountInfoDouble(ACCOUNT_BALANCE);
   g_hcw_breaker_day_hit = (anchor > 0.0 && day_pnl / anchor * 100.0 <= strategy_daily_stop_pct);
   ...
   const double week_pnl = HcwFamilyPnlSince(QM_UTCToBroker(monday_utc));
   anchor = AccountInfoDouble(ACCOUNT_EQUITY) - week_pnl;
   if(anchor <= 0.0)
      anchor = AccountInfoDouble(ACCOUNT_BALANCE);
   g_hcw_breaker_week_hit = (anchor > 0.0 && week_pnl / anchor * 100.0 <= strategy_weekly_stop_pct);
  }

// ---------------------------------------------------------------------------
// Session state (new-bar gated): day roll, shock filter, breakout window.
// ---------------------------------------------------------------------------

// Recompute the breakout reference from today's closed session bars.
// Returns true when at least N session bars have closed today.
bool HcwBuildBreakoutWindow(const datetime closed_bar_time)
  {
   g_hcw_breakout_high = 0.0;
   g_hcw_breakout_low = 0.0;
   g_hcw_window_ready = false;
   ...
   MqlDateTime utc;
   HcwUtcStruct(closed_bar_time, utc);
   const int today = HcwUtcDayKey(utc);
   ...
   for(int i = copied - 1; i >= 1; --i)
     {
      ...
      if(HcwUtcDayKey(bar_utc) != today)
         continue;
      if(bar_utc.hour < strategy_session_start_hour_utc)
         continue;
      if(counted >= strategy_breakout_window_bars)
         break;
      counted++;
   ...
   if(counted < strategy_breakout_window_bars || hi <= 0.0 || lo <= 0.0 || hi <= lo)
      return false;

   g_hcw_breakout_high = hi;
   g_hcw_breakout_low = lo;
   g_hcw_window_ready = true;
   return true;
  }

// Advance the session state for the newly closed bar (shift 1).
void HcwOnNewClosedBar()
  {
   const datetime closed_time = iTime(_Symbol, PERIOD_H1, 1);
   if(closed_time <= 0)
      return;

   HcwSpreadSample(closed_time);
   HcwBreakersRefresh();

   MqlDateTime utc;
   HcwUtcStruct(closed_time, utc);
   const int day_key = HcwUtcDayKey(utc);
   if(day_key != g_hcw_day_key)
     {
      g_hcw_day_key = day_key;
      g_hcw_shock_blocked = false;
      g_hcw_trade_taken_today = false;
      g_hcw_window_ready = false;
      g_hcw_breakout_high = 0.0;
      g_hcw_breakout_low = 0.0;
     }
   ...
   if(utc.hour == strategy_session_start_hour_utc && !g_hcw_shock_blocked)
     {
      ...
      if(range > 0.0 && atr > 0.0 && range > strategy_shock_atr_mult * atr)
         g_hcw_shock_blocked = true;
     }

   HcwBuildBreakoutWindow(closed_time);
  }
...
bool HcwNewsBlackoutBlocks(const datetime broker_now)
  {
   if(strategy_news_blackout_minutes <= 0)
      return false;

   const datetime bucket = broker_now / 60;
   if(bucket == g_hcw_news_cache_bucket)
   ...
   if(!QM_NewsIsLoaded() &&
      !QM_NewsInit("D:\\QM\\data\\news_calendar",
                   ...
                   qm_news_min_impact))
     {
      g_hcw_news_cache_blocked = true; // fail-closed
      return g_hcw_news_cache_blocked;
     }
   ...
   if(utc_time <= 0)
      utc_time = TimeGMT();
   g_hcw_news_cache_blocked = QM_NewsInWindow(utc_time, _Symbol,
                                              strategy_news_blackout_minutes, 0,
                                              "high");
   return g_hcw_news_cache_blocked;
  }
...
bool HcwNoTradeFilter()
  {
   if((ENUM_TIMEFRAMES)_Period != PERIOD_H1)
      return true;
   if(g_hcw_slot < 0)
      return true;
   if(strategy_breakout_window_bars < 2 || strategy_breakout_window_bars > 4)
      return true;
   if(strategy_ema_period < 15 || strategy_ema_period > 30)
      return true;
   if(strategy_atr_period < 10 || strategy_atr_period > 20)
      return true;
   if(strategy_atr_stop_mult != 1.0)
      return true;
   if(strategy_target_r < 1.5 || strategy_target_r > 2.0)
      return true;
...
bool HcwEntrySignal(QM_EntryRequest &req)
  {
   req.type = QM_BUY;
   req.price = 0.0;
   req.sl = 0.0;
   ...
   if(g_hcw_trade_taken_today)
      return false;

   // Session day must have its full breakout window closed.
   ...
   // Circuit breakers (cached this bar by HcwOnNewClosedBar).
   if(g_hcw_breaker_day_hit || g_hcw_breaker_week_hit)
      return false;

   // Shock filter.
   if(g_hcw_shock_blocked)
      return false;

   // Spread filter.
   if(HcwSpreadFilterBlocks())
      return false;

   // News blackout (fail-closed).
   if(HcwNewsBlackoutBlocks(now))
      return false;

   // Portfolio capacity across this EA's symbol slots.
   if(HcwPortfolioPositionCount() >= strategy_max_positions_total)
      return false;
   ...
   if(close1 <= 0.0 || ema1 <= 0.0 || atr1 <= 0.0)
      return false;

   const double stop_dist = strategy_atr_stop_mult * atr1;
   if(stop_dist <= 0.0)
      return false;
   ...
   if(close1 > g_hcw_breakout_high + buffer && close1 > ema1)
     {
      const double sl = NormalizeDouble(ask - stop_dist, _Digits);
      const double tp = NormalizeDouble(ask + strategy_target_r * stop_dist, _Digits);
      if(sl > 0.0 && tp > ask && (ask - sl) >= point)
        {
         req.type = QM_BUY;
         req.price = 0.0;
         req.sl = sl;
         req.tp = tp;
         req.reason = "hcw_session_breakout_long";
         req.symbol_slot = qm_magic_slot_offset;
         req.expiration_seconds = 0;
         g_hcw_trade_taken_today = true;
         return true;
        }
     }

   if(close1 < g_hcw_breakout_low - buffer && close1 < ema1)
     {
      const double sl = NormalizeDouble(bid + stop_dist, _Digits);
      const double tp = NormalizeDouble(bid - strategy_target_r * stop_dist, _Digits);
      if(sl > bid && tp > 0.0 && (sl - bid) >= point)
        {
         req.type = QM_SELL;
         req.price = 0.0;
         req.sl = sl;
         req.tp = tp;
         req.reason = "hcw_session_breakout_short";
         req.symbol_slot = qm_magic_slot_offset;
         req.expiration_seconds = 0;
         g_hcw_trade_taken_today = true;
         return true;
...
void HcwManageOpenPosition()
  {
   ...
   if(!HcwSelectOurPosition(ticket, ptype, open_price, sl))
     {
      if(g_hcw_active_ticket != 0)
        {
         g_hcw_active_ticket = 0;
         g_hcw_be_done = false;
        }
      return;
     }
   if(ticket != g_hcw_active_ticket)
     {
      g_hcw_active_ticket = ticket;
      g_hcw_be_done = false;
     }
   ...
   if(improves && QM_TM_MoveSL(ticket, be, "hcw_breakeven_one_atr"))
      g_hcw_be_done = true;
  }
