...
#property description "QM5_13079 XBR AUDCAD Return Spread Reversion"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_13079 - Brent/AUDCAD Return-Spread Reversion
// -----------------------------------------------------------------------------
// D1 two-leg energy/FX basket:
//   rspread = log(XBR[t] / XBR[t-L]) + beta_audcad * log(AUDCAD[t] / AUDCAD[t-L])
//   z > entry: short return spread = sell Brent, sell AUDCAD
//   z < -entry: long return spread = buy Brent, buy AUDCAD
...
input double strategy_beta_audcad         = 0.60;
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(CopyClose(g_leg_xbr, PERIOD_D1, 1, history_needed, xbr) != history_needed) // perf-allowed: called only behind the D1 new-bar gate or close-state refresh.
      return false;
   if(CopyClose(g_leg_audcad, PERIOD_D1, 1, history_needed, audcad) != history_needed) // perf-allowed: called only behind the D1 new-bar gate or close-state refresh.
      return false;
   ...
   for(int i = 0; i < z_lookback; ++i)
     {
      ...
      if(xbr[i] <= 0.0 || audcad[i] <= 0.0 || xbr[past_idx] <= 0.0 || audcad[past_idx] <= 0.0)
         return false;

      const double xbr_ret = MathLog(xbr[i] / xbr[past_idx]);
      const double audcad_ret = MathLog(audcad[i] / audcad[past_idx]);
      spreads[i] = xbr_ret + strategy_beta_audcad * audcad_ret;
...
bool Strategy_OpenPair(const int spread_direction)
  {
   if(spread_direction == 0 || Strategy_OpenPairLegCount() > 0)
      return false;
   if(!Strategy_SpreadAllowed(g_leg_xbr) || !Strategy_SpreadAllowed(g_leg_audcad))
      return false;

   const double xbr_weight = 1.0;
   const double audcad_weight = MathMax(0.1, MathAbs(strategy_beta_audcad));
   const double weight_sum = xbr_weight + audcad_weight;
   ...
   const string reason = long_spread ? "QM5_13079_LONG_XBR_AUDCAD_RSPREAD"
                                     : "QM5_13079_SHORT_XBR_AUDCAD_RSPREAD";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_13079_RSPREAD_HOST";
...
int OnInit()
  {
   ...
   string basket_symbols[2] = {g_leg_xbr, g_leg_audcad};
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_13079\",\"ea\":\"xbr-audcad-rspr\"}");
