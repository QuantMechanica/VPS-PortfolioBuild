...
#property description "QM5_9256 MQL5 3-swing slanted trendline breakout"

#include <QM/QM_Common.mqh>

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 9256;
...
input double strategy_take_profit_rr          = 2.2;
input int    strategy_time_exit_bars          = 72;
input int    strategy_min_bars_between_entries = 6;
...
bool Strategy_ReadRates(MqlRates &rates[], const int count)
{
   ...
   const int copied = CopyRates(_Symbol, (ENUM_TIMEFRAMES)_Period, 1, count, rates);
   return (copied == count);
}
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
{
   ...
   // Enforce cooldown bars between entries
   const datetime current_bar_time = iTime(_Symbol, (ENUM_TIMEFRAMES)_Period, 0);
   if(strategy_min_bars_between_entries > 0 && g_last_entry_bar_time > 0)
   {
      const int bars_since_last_entry = (int)((current_bar_time - g_last_entry_bar_time) / PeriodSeconds((ENUM_TIMEFRAMES)_Period));
      if(bars_since_last_entry < strategy_min_bars_between_entries)
         return false;
   }
   ...
   if(signal > 0)
   {
      ...
      g_last_entry_bar_time = current_bar_time;
      return (req.tp > ask);
   }
...
int OnInit()
{
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_9256_mql5-3swing-tl\"}");
