...
slug: xng-mon-trend
type: strategy
strategy_id: BOROWSKI-MOP-XNG-MONTREND-2026_S01
variant_id: BOROWSKI-MOP-XNG-MONTREND-2026_S01
source_id: BOROWSKI-MOP-XNG-MONTREND-2026
status: DRAFT
g0_status: APPROVED
execution_contract_ref: strategy-seeds/cards/approved/QM5_20159_xng-mon-trend_card.md
execution_contract_status: DRAFT
created: 2026-07-25
created_by: Research+Development
last_updated: 2026-07-25
source_authors: "Krzysztof Borowski; Tobias J. Moskowitz, Yao Hua Ooi, Lasse Heje Pedersen"
strategy_mechanic: monday-xng-long-only-when-completed-252d-return-is-positive
...
sources:
  - "[[sources/BOROWSKI-MOP-XNG-MONTREND-2026]]"
concepts:
...
timeframe: D1
timeframes: [D1]
expected_trade_frequency: "Approximately 12-30 completed Monday-session XNG packages/year when the completed 252-D1 return is strictly positive; Q02 must prove or retire the density."
...
q01_status: PASS
q02_status: ENQUEUED
review_focus: "Falsify whether the load-bearing conjunction of XNG Monday premium and a positive completed 252-D1 trend clears density and costs while adding direct natural-gas exposure."
...
`strategy-seeds/sources/BOROWSKI-MOP-XNG-MONTREND-2026/source.md` preserves the
two completely read governed parent lineages.

Borowski supply the positive XNG Monday sample-return direction. Moskowitz, Ooi,
...
- A genuine Monday boundary requires the current D1 bar to be Monday and the
  ...
  no shifted Monday trade.
- Entry uses the first executable quote observed within five minutes of the
  Monday D1 bar open. The Friday-close to Monday-open weekend gap is not captured.
...
research cards and returned `CLEAN` for slug `xng-mon-trend`, strategy ID
`BOROWSKI-MOP-XNG-MONTREND-2026_S01`, and mechanic
`Monday XNG long only when completed 252-D1 return is positive`.

Manual semantic review resolved the closest systems:

- `QM5_12806_xng-rev-weekend` trades fixed Monday/Friday directions and never
  reads the slow XNG trend.
- `QM5_12818_xng-tue-prem` and `QM5_12819_xng-thu-fade` isolate different
  weekdays without a trend state.
- `QM5_20011_xng-thu-tue` is a multi-day Thursday-to-Tuesday carry package.
- `QM5_20018_xng-wed-short` is an unconditional Wednesday short.
- `QM5_20156_xng-wed-trend` and `QM5_20158_xng-tue-trend` use the same
  pre-registered interaction architecture on different weekday effects.
...
- Timeframe: D1.
- Magic slot: 0; allocated magic `201590000`.
...
9. Attach one frozen hard stop `3.0 * ATR(20)` below the executable entry,
   normalized through V5 stop rules. There is no take-profit.
10. Open at most one position for magic `201590000`; no pending order,
...
1. Close the Monday package on the first new D1 bar whose broker weekday is
   not Monday.
2. Close immediately if an unexpected short position exists for the magic.
...
4. Framework Friday close at broker hour 21 remains enabled as a fail-safe,
   although the ordinary Monday-boundary exit should make it unreachable.
...
- One position for magic `201590000` and one consumed decision per broker
  week.
...
XNG Mondays. Moskowitz, Ooi, and Pedersen report time-series momentum across
...
- Fail on any short entry, entry outside a genuine Monday/Friday boundary,
...
|---|---|---|---|---|
| v1 | 2026-07-25 | initial source-backed XNG Monday/trend interaction card | G0 | APPROVED |
...
| Q01 Build Validation | 2026-07-25 | PASS: strict compile 0 errors/0 warnings; targeted build check PASS | `docs/ops/evidence/2026-07-25_qm5_20159_xng_mon_trend_build_q02.md` |
| Q02 Baseline Screening | 2026-07-25 | ENQUEUED: `0e3a27ec` | same evidence |
