...
#property description "QM5_10484 MQL5 FT CCI MA trend-filtered CCI levels"
...
int Strategy_CciMaSignal()
  {
   if(strategy_cci_period <= 0 || strategy_ma_period <= 0)
      return 0;

   const double ma_1 = QM_SMA(_Symbol, strategy_work_tf, strategy_ma_period, 1);
   const double ma_2 = QM_SMA(_Symbol, strategy_work_tf, strategy_ma_period, 2);
   const double cci_1 = QM_CCI(_Symbol, strategy_work_tf, strategy_cci_period, 1);
   const double cci_2 = QM_CCI(_Symbol, strategy_work_tf, strategy_cci_period, 2);
   if(ma_1 <= 0.0 || ma_2 <= 0.0 || cci_1 == EMPTY_VALUE || cci_2 == EMPTY_VALUE)
      return 0;

   if(ma_1 > ma_2)
     {
      if(cci_2 <= -100.0 && cci_1 > -100.0)
         return 1;
      if(cci_2 >= 200.0 && cci_1 < 200.0)
         return -1;
     }
   else if(ma_1 < ma_2)
     {
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   const int signal = Strategy_CciMaSignal();
   if(signal == 0)
      return false;
   ...
   req.sl = sl;
   req.tp = tp;
   req.reason = (signal > 0) ? "FT_CCI_MA_LONG" : "FT_CCI_MA_SHORT";
   return true;
  }
...
void Strategy_ManageOpenPosition()
  {
   // Card specifies fixed ATR SL, 2R TP, opposite-signal exit, and time stop only.
  }
