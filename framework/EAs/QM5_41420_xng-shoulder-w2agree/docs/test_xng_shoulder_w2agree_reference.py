"""Deterministic reference checks for QM5_41420 XNG shoulder agreement."""
...
def agreement_signal(
    ...
    """Mirror strict two-week agreement and its continuation package side."""
...
class XngShoulderTwoWeekAgreementReferenceTest(unittest.TestCase):
    def setUp(self) -> None:
        self.current = date(2026, 10, 19)
        self.newer = self.current - timedelta(days=7)
        self.older = self.current - timedelta(days=14)

    def bars(self, newer_open: float, newer_close: float, older_open: float, older_close: float) -> list[Bar]:
        return package(self.newer, 5, newer_open, newer_close) + package(
            self.older, 5, older_open, older_close
        )

    def test_two_positive_weeks_are_long(self) -> None:
        self.assertEqual(agreement_signal(self.current, self.bars(3, 4, 2, 3))[0], 1)

    def test_two_negative_weeks_are_short(self) -> None:
        self.assertEqual(agreement_signal(self.current, self.bars(4, 3, 3, 2))[0], -1)

    def test_mixed_signs_are_flat(self) -> None:
        self.assertEqual(agreement_signal(self.current, self.bars(3, 4, 3, 2))[0], 0)

    def test_either_exact_zero_is_flat(self) -> None:
        self.assertEqual(agreement_signal(self.current, self.bars(3, 3, 2, 3))[0], 0)
        self.assertEqual(agreement_signal(self.current, self.bars(3, 4, 2, 2))[0], 0)

    def test_reflection_flips_direction(self) -> None:
        up = agreement_signal(self.current, self.bars(3, 4, 2, 3))[0]
        down = agreement_signal(self.current, self.bars(4, 3, 3, 2))[0]
        self.assertEqual(up, -down)

    def test_all_shoulder_anchor_months_are_eligible(self) -> None:
        for value in (date(2026, 4, 6), date(2026, 5, 4), date(2026, 9, 7), date(2026, 10, 5)):
            self.assertTrue(shoulder_week(value))

    def test_nonshoulder_months_are_ineligible(self) -> None:
        for value in (date(2026, 6, 1), date(2026, 8, 3), date(2026, 12, 7)):
            self.assertFalse(shoulder_week(value))

    def test_week_crossing_into_september_uses_monday_anchor(self) -> None:
        self.assertFalse(shoulder_week(date(2026, 8, 31)))

    def test_week_crossing_into_may_uses_monday_anchor(self) -> None:
    ...
    def test_three_to_five_sessions_per_week_are_accepted(self) -> None:
        for newer_count in (3, 4, 5):
            for older_count in (3, 4, 5):
                ...
                self.assertEqual(agreement_signal(self.current, bars)[0], 1)
    ...
    def test_two_and_six_sessions_are_rejected(self) -> None:
        for count in (2, 6):
            ...
            with self.assertRaisesRegex(ValueError, "session count"):
                agreement_signal(self.current, bars)
    ...
    def test_ineligible_week_refuses_before_signal(self) -> None:
        with self.assertRaisesRegex(ValueError, "ineligible month"):
            agreement_signal(date(2026, 11, 2), self.bars(3, 4, 2, 3))
