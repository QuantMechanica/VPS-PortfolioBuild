...
#property description "QM5_41420 XNG Shoulder Two-Week Agreement Continuation"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41420 - XNG Shoulder Two-Week Agreement Continuation
// -----------------------------------------------------------------------------
...
bool Strategy_LoadTwoWeekAgreementSignal(const int current_week_key,
                                         ...
                                         int &direction)
  {
   ...
   // Both adjacent completed weeks must agree strictly. Follow the shared
   // sign; disagreement and exact zero are valid flat states rather than
   // data failures.
   ...
   if(Strategy_WeekAlreadyEntered(g_decision_week_key))
   ...
   else
     {
      ...
      if(!g_signal_valid)
         ...
         g_signal_state = "two_positive_weeks_momentum_long";
      else if(g_signal_direction < 0)
         g_signal_state = "two_negative_weeks_momentum_short";
      else
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41420_XNG_SHOULDER_W2AGREE";
   ...
   req.reason = (g_signal_direction > 0)
      ? "XNG_SHOULDER_W2_AGREE_LONG"
      : "XNG_SHOULDER_W2_AGREE_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(strategy_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41420 ||
      qm_magic_slot_offset != 0)
   ...
   g_attempt_state_key =
      StringFormat("QM5_41420_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41420\",\"ea\":\"xng-shoulder-w2agree\"}");
