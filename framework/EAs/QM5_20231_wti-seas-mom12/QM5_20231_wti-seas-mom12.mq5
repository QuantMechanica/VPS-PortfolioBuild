...
#property description "QM5_20231 WTI Seasonal 12M Momentum Concordance"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_20231 - WTI Seasonal / Twelve-Month Momentum Concordance
// -----------------------------------------------------------------------------
// D1 structural crude-oil sleeve:
//   - first tradable D1 bar of each broker-calendar month
//   - exact cumulative return across twelve completed calendar months
//   - seasonal direction BUY November-May / SELL June-October
//   - enter only when seasonal direction and return-sign state agree
...
int Strategy_SeasonalDirection(const int month_number)
  {
   if(month_number < 1 || month_number > 12)
      return 0;
   if(strategy_winter_first_month <= strategy_winter_last_month)
      return (month_number >= strategy_winter_first_month &&
              month_number <= strategy_winter_last_month) ? 1 : -1;
   return (month_number >= strategy_winter_first_month ||
           month_number <= strategy_winter_last_month) ? 1 : -1;
  }
...
bool Strategy_LoadMomentumSignal(const int current_month_key,
                                 ...
                                 int &trade_direction)
  {
   ...
   momentum_return =
      MathLog(month_closes[0] /
              month_closes[strategy_lookback_months]);
   if(!MathIsValidNumber(momentum_return))
      return false;

   if(momentum_return > 0.0)
      trade_direction = 1;
   else if(momentum_return < 0.0)
      trade_direction = -1;
   return true;
  }
...
void Strategy_CloseExpiredPositions()
  {
   ...
   const int seasonal_direction =
      Strategy_SeasonalDirection(current_month_key % 100);
   ...
   for(int index = PositionsTotal() - 1; index >= 0; --index)
     {
      ...
      if((seasonal_direction > 0 && position_type != POSITION_TYPE_BUY) ||
         (seasonal_direction < 0 && position_type != POSITION_TYPE_SELL))
         should_close = true;
      if(opened <= 0 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   const int seasonal_direction =
      Strategy_SeasonalDirection(month_number);
   if(month_key <= 0 ||
   ...
   if(!Strategy_LoadMomentumSignal(month_key,
                                   momentum_return,
                                   trade_direction) ||
      trade_direction == 0 ||
      trade_direction != seasonal_direction)
      return false;
   ...
   req.reason = StringFormat("WTI_SEAS_MOM12_%s_R%.6f",
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_20231_MONTH_ATTEMPT_%d",
                   QM_FrameworkMagic());
   Strategy_LoadAttemptState();

   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_20231\",\"ea\":\"wti-seas-mom12\"}");
