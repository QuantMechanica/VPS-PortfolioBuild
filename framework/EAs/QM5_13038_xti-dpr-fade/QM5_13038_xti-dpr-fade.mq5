...
#property description "QM5_13038 XTI EIA DPR failed-breakout fade"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_13038 - XTI EIA DPR Failed-Breakout Fade
// -----------------------------------------------------------------------------
// D1 structural WTI sleeve:
//   - uses a fixed mid-month broker-calendar DPR proxy window
//   - fades ATR-sized XTIUSD.DWX Donchian breaches that close back inside the
//     channel with reversal tails and SMA stretch
//   - ATR stop/target, max-hold/mean-reversion exits, no external runtime data
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 13038;
...
input int    strategy_event_start_day    = 12;
input int    strategy_event_end_day      = 16;
input int    strategy_breakout_lookback  = 20;
input int    strategy_trend_period       = 80;
input int    strategy_atr_period         = 20;
input double strategy_min_range_atr      = 1.00;
input double strategy_min_body_atr       = 0.30;
input double strategy_min_tail_atr       = 0.20;
...
bool Strategy_IsDprProxyDay(const datetime t)
  {
   if(t <= 0)
      return false;
   MqlDateTime dt;
...
bool Strategy_LoadDprFadeState(int &direction,
                               double &atr_last,
                               double &sma_last,
                               int &signal_day_key)
  {
   ...
   if(event_time <= 0 || !Strategy_IsDprProxyDay(event_time))
      return false;
   ...
   if(event_body < strategy_min_body_atr * atr_last)
      return false;
   const double upper_tail = event_high - MathMax(event_open, event_close);
   const double lower_tail = MathMin(event_open, event_close) - event_low;
   ...
   const bool failed_up_breakout = (event_high > breakout_high &&
                                    event_close < breakout_high &&
                                    event_close < event_open &&
                                    event_close > sma_last &&
                                    upper_tail >= strategy_min_tail_atr * atr_last);
   const bool failed_down_breakout = (event_low < breakout_low &&
                                      event_close > breakout_low &&
                                      event_close > event_open &&
                                      event_close < sma_last &&
                                      lower_tail >= strategy_min_tail_atr * atr_last);

   if(failed_up_breakout)
      direction = -1;
   else if(failed_down_breakout)
      direction = 1;
   else
      return false;
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(strategy_trend_period <= 1 || strategy_atr_period <= 1)
      return true;
   if(strategy_min_range_atr <= 0.0 || strategy_min_body_atr <= 0.0 || strategy_min_tail_atr < 0.0)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_13038_XTI_DPR_FADE";
   ...
   if(!Strategy_LoadDprFadeState(direction, atr_last, sma_last, signal_day_key))
      return false;
   ...
   req.reason = (direction > 0) ? "XTI_DPR_FADE_LONG" : "XTI_DPR_FADE_SHORT";
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_13038\",\"ea\":\"xti-dpr-fade\"}");
