...
#property description "QM5_13053 XBR XAG Return Spread Reversion"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_13053 - XBR/XAG Return-Spread Reversion
// -----------------------------------------------------------------------------
// D1 two-leg Brent/Silver basket:
//   rspread = log(xbr[t] / xbr[t-L]) - beta * log(XAG[t] / XAG[t-L])
//   z > entry: short return spread = sell oil, buy silver
//   z < -entry: long return spread = buy oil, sell silver
   ...
   for(int i = 0; i < z_lookback; ++i)
     {
      ...
      if(xbr[i] <= 0.0 || xag[i] <= 0.0 || xbr[past_idx] <= 0.0 || xag[past_idx] <= 0.0)
         return false;

      const double xbr_ret = MathLog(xbr[i] / xbr[past_idx]);
      const double xag_ret = MathLog(xag[i] / xag[past_idx]);
      spreads[i] = xbr_ret - strategy_beta * xag_ret;
...
bool Strategy_OpenPair(const int spread_direction)
  {
   if(spread_direction == 0 || Strategy_OpenPairLegCount() > 0)
      return false;
   if(!Strategy_SpreadAllowed(g_leg_xbr) || !Strategy_SpreadAllowed(g_leg_xag))
      return false;

   const double xbr_weight = 1.0;
   const double xag_weight = MathMax(0.1, MathAbs(strategy_beta));
   const double weight_sum = xbr_weight + xag_weight;
   ...
   const string reason = long_spread ? "QM5_13053_LONG_XBR_XAG_RSPREAD"
                                     : "QM5_13053_SHORT_XBR_XAG_RSPREAD";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_13053_RSPREAD_HOST";
...
int OnInit()
  {
   ...
   string basket_symbols[2] = {g_leg_xbr, g_leg_xag};
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_13053\",\"ea\":\"brentsilver-rspr\"}");
