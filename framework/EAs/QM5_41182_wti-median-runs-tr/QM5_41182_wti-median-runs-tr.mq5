...
#property description "QM5_41182 WTI Monthly Median-Runs Persistence Trend"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41182 - WTI Monthly Median-Runs Persistence Trend
// -----------------------------------------------------------------------------
// D1 structural crude-oil sleeve:
//   - reconstruct thirteen consecutive completed broker-month-end closes
//   - assign strict price ranks and omit the unique median rank seven
//   - count chronological runs in the remaining six-low/six-high sequence
//   - continue the newest nonmedian regime only when the run count is <= 7
...
bool Strategy_MedianRunsSignal(const double &closes[],
                               Strategy_SignalMetrics &metrics)
  {
   ...
   int signs[];
   if(ArrayResize(signs, strategy_endpoint_count - 1) != 12)
      return false;
   int sign_count = 0;
   for(int index = 0; index < strategy_endpoint_count; ++index)
     {
      ...
      if(sign < 0)
         ...
         ++metrics.high_count;
      if(StringLen(metrics.sign_path) > 0)
         metrics.sign_path += ",";
      metrics.sign_path += IntegerToString(sign);
     }
                                        ...
                                        int &direction)
  {
   ...
   const bool valid =
      Strategy_LoadMonthlyEndpoints(current_month_key,
                                    ...
                                    closes,
                                    endpoint_times) &&
      Strategy_MedianRunsSignal(closes, metrics);
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
   ...
   else
     {
      ...
      g_signal_valid =
         endpoints_valid &&
         Strategy_MedianRunsSignal(closes, g_signal_metrics);
      if(!g_signal_valid)
         g_signal_state = "median_runs_validation_failed";
      else if(g_signal_metrics.direction == 0 &&
              g_signal_metrics.newest_rank == 7)
         g_signal_state = "newest_is_median_flat";
      else if(g_signal_metrics.direction == 0)
         g_signal_state = "run_count_above_threshold_flat";
      else if(g_signal_metrics.direction > 0)
         g_signal_state = "persistent_high_regime_wti_long";
      else if(g_signal_metrics.direction < 0)
         g_signal_state = "persistent_low_regime_wti_short";
      else
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"endpoint_count\":%d,\"rank_sum\":%d,\"ranks\":\"%s\",\"median_count\":%d,\"omitted_index_zero_based\":%d,\"signs\":\"%s\",\"low_count\":%d,\"high_count\":%d,\"run_count\":%d,\"max_runs\":%d,\"newest_rank\":%d,\"oldest_close\":%.10f,\"newest_close\":%.10f,\"oldest_endpoint\":%I64d,\"newest_endpoint\":%I64d,\"state\":\"%s\"}",
                            ...
                            g_signal_metrics.median_count,
                            g_signal_metrics.omitted_index,
                            g_signal_metrics.sign_path,
                            g_signal_metrics.low_count,
                            g_signal_metrics.high_count,
                            g_signal_metrics.run_count,
                            strategy_max_runs,
                            g_signal_metrics.newest_rank,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41182 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41182_WTI_MEDIAN_RUNS_TREND";
   ...
   req.reason = (g_signal_metrics.direction > 0)
      ? "WTI_MEDIAN_RUNS_HIGH_LONG"
      : "WTI_MEDIAN_RUNS_LOW_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41182 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI median-runs persistence trend holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41182_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41182\",\"ea\":\"wti-median-runs-tr\"}");
