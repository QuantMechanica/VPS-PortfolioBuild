...
#property description "QM5_10451 MQL5 WPR Bollinger ATR Reversal"
...
input double strategy_bb_sl_mult        = 1.0;
...
double Strategy_WilliamsR(const int shift)
  {
   if(strategy_wpr_period <= 0 || shift < 1)
      return EMPTY_VALUE;
...
bool Strategy_NoTradeFilter()
  {
   ...
   const double stop_distance = 0.5 * MathAbs(upper - lower) * strategy_bb_sl_mult;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(strategy_bb_period <= 1 || strategy_atr_period <= 0 ||
      strategy_bb_sl_mult <= 0.0 || strategy_atr_tp_mult <= 0.0)
      return false;
   ...
   if(open1 <= 0.0 || bb_mid <= 0.0 || bb_upper <= bb_lower || atr <= 0.0)
      return false;

   const bool long_signal = (wpr2 <= strategy_wpr_oversold &&
                             wpr1 > strategy_wpr_oversold &&
                             open1 < bb_mid);
   const bool short_signal = (wpr2 >= strategy_wpr_overbought &&
                              wpr1 < strategy_wpr_overbought &&
                              open1 > bb_mid);
   if(!long_signal && !short_signal)
      return false;

   const double stop_distance = 0.5 * MathAbs(bb_upper - bb_lower) * strategy_bb_sl_mult;
   const double target_distance = atr * strategy_atr_tp_mult;
   ...
   if(long_signal)
     {
      ...
      req.tp = Strategy_NormalizePrice(entry + target_distance);
      req.reason = "WPR_BB_ATR_LONG";
   ...
   req.sl = Strategy_NormalizePrice(entry + stop_distance);
   req.tp = Strategy_NormalizePrice(entry - target_distance);
   req.reason = "WPR_BB_ATR_SHORT";
   return (req.sl > req.price && req.tp > 0.0);
  }
...
bool Strategy_ExitSignal()
  {
   // Baseline exits only by fixed SL/TP, framework Friday close, and kill-switch.
   return false;
  }
