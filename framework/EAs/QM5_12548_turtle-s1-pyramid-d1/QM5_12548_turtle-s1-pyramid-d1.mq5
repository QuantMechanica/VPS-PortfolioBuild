...
#property description "QM5_12548 Turtle System 1 — 20-day Donchian breakout, N-unit sizing, pyramiding D1"

#include <QM/QM_Common.mqh>

// ============================================================================
// QM5_12548 — Turtle System 1 (Faith 2007, Appendix A)
// 20-day Donchian channel breakout; ATR/N unit sizing; pyramid up to 4 units
// at 1/2N intervals; 2N trailing stop converging on all units; 10-day channel
// exit; skip rule (skip next signal if last completed trade was profitable).
//
...
input int    strategy_entry_period         = 20;   // Donchian breakout lookback (S1 = 20)
input int    strategy_exit_period          = 10;   // Channel exit lookback (S1 = 10)
...
// Skip rule — persisted via GlobalVariable for EA restart survival
bool     g_skip_next        = false; // true = skip the next entry signal (last trade profitable)
string   g_gvar_skip        = "";    // GlobalVariable key for persistence
...
void CloseAllUnits(const bool from_exit_signal)
  {
   ...
   if(closed > 0 && from_exit_signal)
     {
      g_skip_next = (total_profit > 0.0);
      GlobalVariableSet(g_gvar_skip, g_skip_next ? 1.0 : 0.0);
     }
   g_units = 0;
   g_dir = 0;
   ...
   if(g_dir != 0 && real_count < g_units)
     {
      if(real_count == 0)
        {
         // All units stopped out (losing trade) — do NOT skip next signal
         g_skip_next = false;
         GlobalVariableSet(g_gvar_skip, 0.0);
         g_units = 0;
         g_dir = 0;
         g_last_add_price = 0.0;
         g_current_stop = 0.0;
        ...
        {
         g_units = real_count; // Partial stop-out — rare with converging stops
        }
     }
  }
...
// Detects 20-day Donchian breakout and fills entry request
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   if(g_dir != 0) return false;
   if(g_n <= 0.0) return false;

   // Skip rule: if last trade was profitable, skip this signal (one-skip per profitable trade)
   if(g_skip_next)
     {
      g_skip_next = false;
      GlobalVariableSet(g_gvar_skip, 0.0);
   ...
   if(long_break)
     {
      ...
      req.reason       = "turtle_s1_long_entry";
     }
   else
     {
      ...
      req.tp           = 0.0;
      req.symbol_slot  = qm_magic_slot_offset;  // Unit 0 base slot
      req.reason       = "turtle_s1_short_entry";
     }
   return true;
  }

// Trade Management — called every tick when positions are open
// Handles: Friday close (all units), 10-day channel exit, pyramid adds
void Strategy_ManageOpenPosition()
  {
   ...
   if(qm_friday_close_enabled && QM_FrameworkFridayCloseNow(TimeCurrent()))
     {
      CloseAllUnits(false);  // Friday close is not a skip-rule exit
      return;
     }

   // 10-day channel exit (all units exit simultaneously)
   if(g_dir == 1  && bid <= g_exit_long_level)  { CloseAllUnits(true); return; }
   if(g_dir == -1 && ask >= g_exit_short_level) { CloseAllUnits(true); return; }
...
// Exit Signal — 10-day channel exit and pyramid add are handled in ManageOpenPosition
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", StringFormat("{\"units\":%d,\"dir\":%d,\"skip_next\":%s}",
               g_units, g_dir, g_skip_next ? "true" : "false"));
