...
#property description "QM5_41049 WTI Standard-Wednesday Overnight-Dominant Flow Continuation"
...
//   - enter Thursday only when the streams oppose and the overnight dominates,
...
bool Strategy_LoadWednesdayOvernightDominance(const datetime current_bar_time,
                                    const datetime broker_now,
                                    ...
                                    int &direction)
  {
   ...
   if(!signs_oppose)
      return true;
   if(MathAbs(overnight_flow) <= MathAbs(session_flow))
      return true;

   // Follow the overnight-dominant reconciled Wednesday displacement; magnitude never changes risk.
   if(total_flow > 0.0)
      direction = 1;
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(!QM_InputRequireLong("qm_ea_id", qm_ea_id, 41049) ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "WTI_WED_OVERNIGHT_DOM";
   ...
   int direction = 0;
   if(!Strategy_LoadWednesdayOvernightDominance(g_strategy_d1_bar_time,
                                  broker_now,
                                  overnight_flow,
                                  session_flow,
                                  day_return,
   ...
   if(direction > 0)
     {
      req.type = QM_BUY;
      req.reason = "WTI_WED_OVERNIGHT_DOM_LONG";
     }
   else if(direction < 0)
     {
      req.type = QM_SELL;
      req.reason = "WTI_WED_OVERNIGHT_DOM_SHORT";
     }
   else
      return false;
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI Wednesday overnight-dominance card exits next D1; Friday 21 is fail-safe"))
      return INIT_FAILED;
   ...
   g_attempt_state_key =
      StringFormat("QM5_41049_WED_OVERNIGHT_DOM_THURSDAY_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41049\",\"ea\":\"wti-wed-overnight-dom\"}");
