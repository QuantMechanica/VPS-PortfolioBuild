# QM5_41116_xauxag-mthirdvote-rv - Strategy Spec
...
**Source:** `SCHWEIKERT-CME-XAUXAG-MTHIRDVOTE-RV-2026`
...
month's chronological final log ratio anchors the newest completed month. Two
deterministic floor-third endpoints partition its full relative path:
...
at least two blocks > 0 => SELL XAU, BUY XAG
at least two blocks < 0 => BUY XAU, SELL XAG
otherwise               => FLAT
```

The three blocks contain exactly `a`, `b-a`, and `n-b` adjacent returns from
`P` through `Q[n-1]`; shared ratios are anchors, not duplicated returns. The
month is consumed flat when fewer than two blocks cast the same strict sign,
or on an invalid split, incomplete/nonconsecutive month, asynchronous pair,
or invalid close. Zero abstains. Magnitude and full-month endpoint agreement
never change eligibility, direction, or risk.
...
- Host/traded slot 0: exact `XAUUSD.DWX`, D1, magic `411160000`.
- Companion/traded slot 1: exact `XAGUSD.DWX`, D1, magic `411160001`.
...
The prior is approximately 10-12 completed packages per full post-warm-up
year. Q02 retires below five rather than changing the rule. A strict positive
two-of-three block majority produces SELL XAU / BUY XAG; a strict negative
majority produces BUY XAU / SELL XAG. Zero casts no vote and no majority stays
flat.
...
and the intermarket ratio carrier. The completed-month floor-third partition,
three-block vote, contrarian side, CFD mapping, risk, and lifecycle are
disclosed QM hypotheses. No source return, hedge ratio,
neutrality, CFD equivalence, or correlation result transfers.
