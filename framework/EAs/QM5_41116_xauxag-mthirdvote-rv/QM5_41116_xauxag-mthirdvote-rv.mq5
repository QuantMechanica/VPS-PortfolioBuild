...
#property description "QM5_41116 XAU/XAG Completed-Month Three-Block Vote Reversion"
...
// deterministic floor(n/3) and floor(2*n/3) observation boundaries to split
// the newest month's relative path into three exhaustive cumulative blocks.
// Fade the strict two-of-three block-sign majority with one opposite equal-
// notional package held to the next month. Runtime is native, completed-price-
// only, deterministic, and one-attempt-per-month.
// =============================================================================
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41116 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_MTHIRDVOTE_RV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41116_XAUXAG_MTHIRDVOTE_RV";
...
bool Strategy_LoadMonthlyThirdVote(
      ...
      int &direction)
  {
   ...
   // Q[0]..Q[n-1], Q[a-1] maps to series index n-a and Q[b-1] maps to n-b.
   // Shared ratios are endpoints/anchors only: the three blocks exhaust every
   // adjacent return from the parent-final ratio through Q[n-1] exactly once.
   ...
   if(split_a <= 0 || split_a >= split_b ||
      split_b >= completed_month_sessions ||
      series_index_a <= 0 || series_index_a >= completed_month_sessions ||
       series_index_b <= 0 || series_index_b >= series_index_a)
      return false;
   if(series_index_a >= ArraySize(newest_ratios))
      return false;
   if(series_index_b >= ArraySize(newest_ratios))
      return false;

   const double ratio_a = newest_ratios[series_index_a];
   const double ratio_b = newest_ratios[series_index_b];
   block_1 = ratio_a - parent_final_ratio;
   block_2 = ratio_b - ratio_a;
   block_3 = newest_ratios[0] - ratio_b;
   if(!MathIsValidNumber(ratio_a) || !MathIsValidNumber(ratio_b) ||
      !MathIsValidNumber(block_1) || !MathIsValidNumber(block_2) ||
      !MathIsValidNumber(block_3))
      return false;

   // direction > 0 means BUY XAU / SELL XAG. This is a contrarian package:
   // zero abstains and block magnitude never weights the strict majority.
   const int positive_votes =
      (block_1 > 0.0 ? 1 : 0) +
      (block_2 > 0.0 ? 1 : 0) +
      (block_3 > 0.0 ? 1 : 0);
   const int negative_votes =
      (block_1 < 0.0 ? 1 : 0) +
      (block_2 < 0.0 ? 1 : 0) +
      (block_3 < 0.0 ? 1 : 0);
   if(positive_votes >= 2)
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41116_XAUXAG_MTHIRDVOTE_RV_HOST";
   ...
   double block_3 = 0.0;
   const bool valid =
      Strategy_LoadMonthlyThirdVote(g_signal_month_key,
                                    completed_month_sessions,
                                    parent_month_sessions,
                                    split_a,
                                    split_b,
                                    ...
                                    block_3,
                                    direction);
   string state = "monthly_ratio_three_block_vote_failed";
   if(valid && direction > 0)
      state = "negative_block_majority_fade_long_xau";
   else if(valid && direction < 0)
      state = "positive_block_majority_fade_short_xau";
   else if(valid)
      state = "no_strict_block_majority_flat";

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"completed_month_sessions\":%d,\"parent_month_sessions\":%d,\"split_a\":%d,\"split_b\":%d,\"block_1\":%.12e,\"block_2\":%.12e,\"block_3\":%.12e,\"direction\":%d,\"state\":\"%s\"}",
                            g_signal_month_key,
                            completed_month_sessions,
                            parent_month_sessions,
                            split_a,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG completed-month three-block vote reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41116\",\"ea\":\"xauxag-mthirdvote-rv\"}");
