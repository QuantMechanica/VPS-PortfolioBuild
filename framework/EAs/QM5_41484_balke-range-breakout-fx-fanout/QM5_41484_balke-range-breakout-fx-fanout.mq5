// QM5_41484 balke-range-breakout-fx-fanout — Velocity lineage (Fable, 2026-09-20).
// Byte-for-byte the qualified incumbent QM5_13213 (Balke GMT+3 range breakout, USDJPY H1)
// with a NEW build identity and ten FX magic slots (registry slots 0..9). No strategy
// logic changed; the only differences are qm_ea_id and this header. Authority:
// FABLE-DEC-VELOCITY-INTAKE-20260920 (OWNER-DEC-FABLE-FULL-EXECUTIVE-AUTHORITY-20260917).
#property strict
#property version   "5.0"
#property description "QM5_13213 Balke GMT3-Normalized Range Breakout 03-06/18"
...
// René Balke ForexFactory Range Breakout, exact window per OWNER-verified
// analysis 2026-07-13: range 03:00-06:00, evening flat ~18:00. Ported from
// QM5_9936 (Q04 PASS PF 1.31, 01:00-06:00 window) — same GMT-normalized time
// handling (Strategy_Gmt3Hour: broker_time -> UTC -> +3h), only the window
// hours changed to Balke's exact values and the two 9936 evening hours
// (cancel=13, close=20) collapsed into Balke's single evening resolution.
...
// GMT+3-equivalent frame so the 03:00-06:00 window lands consistently across
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   // Trade Entry: at range_end_hour (default 06:00 GMT+3-equivalent) place
   // both stop orders around the completed range_start_hour-range_end_hour
   // range. Caller guarantees QM_IsNewBar() == true.
...
void Strategy_ManageOpenPosition()
  {
   // Trade Management: at Balke's single evening resolution hour, cancel any
   // stale/opposite stop order (the close of the open position itself is
   // handled by Strategy_ExitSignal at the same hour). Before that, cancel
   // the opposite pending order once one side has triggered.
   const datetime now = TimeCurrent();
   const int hour = Strategy_Gmt3Hour(now);
