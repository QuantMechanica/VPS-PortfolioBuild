...
#property description "QM5_10736 TradingView EMA/WMA Two Target"
...
input int    strategy_tp2_pips           = 40;
input int    strategy_breakeven_pips     = 20;
...
bool Strategy_NoTradeFilter()
  {
   if(strategy_ema_period <= 0 ||
      ...
      strategy_tp2_pips <= strategy_stop_pips ||
      strategy_breakeven_pips <= 0)
      return true;
   ...
   const double stop_price = QM_StopFixedPips(_Symbol, QM_BUY, mid, strategy_stop_pips);
   const double stop_points = MathAbs(mid - stop_price) / point;
   const int broker_stop_points = (int)SymbolInfoInteger(_Symbol, SYMBOL_TRADE_STOPS_LEVEL);
   if(stop_points <= 0.0)
      return true;
   if(broker_stop_points > 0 && stop_points <= (double)broker_stop_points)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.tp = QM_TakeFixedPips(_Symbol, req.type, entry, strategy_tp2_pips);
   req.reason = (signal > 0) ? "ema_wma_cross_long_tp2" : "ema_wma_cross_short_tp2";
...
bool Strategy_ExitSignal()
  {
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      have_long = have_long || (ptype == POSITION_TYPE_BUY);
      have_short = have_short || (ptype == POSITION_TYPE_SELL);
     }
   ...
   const bool bearish_cross = (ema_2 >= wma_2 && ema_1 < wma_1);
   const bool bullish_cross = (ema_2 <= wma_2 && ema_1 > wma_1);
   return (have_long && bearish_cross) || (have_short && bullish_cross);
  }
