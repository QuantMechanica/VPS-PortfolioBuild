# QM5_13057_xti-gbpcad-rspr - Strategy Spec
...
and `GBPCAD.DWX`. On each new D1 host bar it computes
`log(XTI[t] / XTI[t-L]) + beta_gbpcad * log(GBPCAD[t] / GBPCAD[t-L])`,
then standardizes that spread against a rolling lookback window.

`GBPCAD` rises when CAD weakens against GBP, so the plus sign is intentional:
...
EA sells WTI and sells GBPCAD. When the z-score is below the negative entry
threshold, it buys WTI and buys GBPCAD. The package exits when the z-score
...
| `strategy_beta_gbpcad` | 0.60 | 0.40-0.85 | GBPCAD return multiplier and risk-weight proxy |
...
- `GBPCAD.DWX` - inverse-CAD FX leg representing CAD commodity exposure against GBP.
- `QM5_13057_XTI_GBPCAD_RSPREAD_D1` - logical Q02 basket symbol backed by the two traded legs above.

**Explicitly NOT for:**
- `XNGUSD.DWX` - natural gas dynamics are outside the approved card.
- `USDCAD.DWX` - related CAD exposure, but this card specifically uses GBPCAD.
- `CADJPY.DWX`, `CADCHF.DWX`, or `AUDCAD.DWX` - covered by separate oil/CAD baskets.
- `GBPNZD.DWX` - existing GBPCAD/GBPNZD cointegration is not this energy/FX sleeve.
- `XBRUSD.DWX` - Brent is excluded; the card is WTI against GBPCAD.
...
| Regime preference | Mean-revert relative-value dislocations between WTI and inverse-CAD GBPCAD |
...
**Source ID:** `BOC-GBPCAD-OIL-RSPREAD-2026`
**Source type:** government energy / central bank research
**Pointer:** `strategy-seeds/sources/BOC-GBPCAD-OIL-RSPREAD-2026/`
**R1-R4 verdict (Q00):** all PASS / see `artifacts/cards_approved/QM5_13057_xti-gbpcad-rspr.md`
