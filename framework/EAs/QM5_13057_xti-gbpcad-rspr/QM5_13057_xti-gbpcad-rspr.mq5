...
#property description "QM5_13057 XTI GBPCAD Return Spread Reversion"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_13057 - WTI/GBPCAD Return-Spread Reversion
// -----------------------------------------------------------------------------
   ...
   for(int i = 0; i < z_lookback; ++i)
     {
      ...
      if(xti[i] <= 0.0 || gbpcad[i] <= 0.0 || xti[past_idx] <= 0.0 || gbpcad[past_idx] <= 0.0)
         return false;

      const double xti_ret = MathLog(xti[i] / xti[past_idx]);
      const double gbpcad_ret = MathLog(gbpcad[i] / gbpcad[past_idx]);
      spreads[i] = xti_ret + strategy_beta_gbpcad * gbpcad_ret;
...
bool Strategy_OpenPair(const int spread_direction)
  {
   if(spread_direction == 0 || Strategy_OpenPairLegCount() > 0)
      return false;
   if(!Strategy_SpreadAllowed(g_leg_xti) || !Strategy_SpreadAllowed(g_leg_gbpcad))
      return false;

   const double xti_weight = 1.0;
   const double gbpcad_weight = MathMax(0.1, MathAbs(strategy_beta_gbpcad));
   const double weight_sum = xti_weight + gbpcad_weight;
   ...
   const string reason = long_spread ? "QM5_13057_LONG_XTI_GBPCAD_RSPREAD"
                                     : "QM5_13057_SHORT_XTI_GBPCAD_RSPREAD";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_13057_RSPREAD_HOST";
...
int OnInit()
  {
   ...
   string basket_symbols[2] = {g_leg_xti, g_leg_gbpcad};
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_13057\",\"ea\":\"xti-gbpcad-rspr\"}");
