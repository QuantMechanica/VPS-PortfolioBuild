...
#property description "QM5_13133 XTI XNG Pure IVol Spread"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_13133 - XTI/XNG Pure Idiosyncratic-Volatility Spread
// -----------------------------------------------------------------------------
// Monthly source-specified pure-IVol energy basket:
//   - estimate XTI and XNG residual volatility against an equal-weight
//     XTI/XNG/XAU/XAG commodity factor over 252 completed D1 returns
//   - buy lower IVol and sell higher IVol
...
bool Strategy_LoadSignalState(int &pair_direction)
  {
   ...
   const int lookback = MathMax(21, strategy_ivol_lookback_d1);
   ...
   g_cache_ivol_difference = g_cache_xti_ivol - g_cache_xng_ivol;
   if(!MathIsValidNumber(g_cache_ivol_difference))
      return false;
   if(g_cache_ivol_difference < -1.0e-12)
      pair_direction = 1;  // XTI has lower IVol: long XTI, short XNG.
   else if(g_cache_ivol_difference > 1.0e-12)
      pair_direction = -1; // XNG has lower IVol: short XTI, long XNG.
   return true;
  }
...
bool Strategy_OpenPair(const int pair_direction)
  {
   ...
   const string reason = long_xti_short_xng ? "QM5_13133_LONG_XTI_SHORT_XNG_IVOL"
                                            : "QM5_13133_SHORT_XTI_LONG_XNG_IVOL";
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart())
      return true;
   if(strategy_ivol_lookback_d1 < 21 || strategy_ivol_lookback_d1 > 252)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_13133_XTI_XNG_IVOL_HOST";
...
int OnInit()
  {
   ...
   QM_BasketWarmupHistory(basket_symbols,
                          PERIOD_D1,
                          MathMax(320, strategy_ivol_lookback_d1 + strategy_atr_period_d1 + 30));

   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_13133\",\"ea\":\"energy-ivol\"}");
