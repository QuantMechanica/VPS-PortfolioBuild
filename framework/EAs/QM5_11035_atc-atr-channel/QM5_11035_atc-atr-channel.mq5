...
#property description "QM5_11035 ATR channel breakout reversal"
...
input int    strategy_mid_period               = 20;
input int    strategy_atr_period               = 14;
input double strategy_channel_mult             = 1.5;
input double strategy_emergency_sl_atr         = 3.0;
input int    strategy_adx_period               = 14;
input double strategy_adx_min                  = 18.0;  // 0 disables the optional ADX filter.
...
bool Strategy_NoTradeFilter()
  {
   // Time filter: card authorizes a weekend guard only.
   if(strategy_weekend_guard_enabled)
     {
      MqlDateTime dt;
      TimeToStruct(TimeCurrent(), dt);
      if(dt.day_of_week == 5 && dt.hour >= strategy_weekend_guard_hour)
         return true;
     }

   // Spread filter: card rule is spread <= symbol median spread * 2.
   if(strategy_median_spread_points > 0 && strategy_spread_median_mult > 0.0)
     {
      const long spread_points = SymbolInfoInteger(_Symbol, SYMBOL_SPREAD);
      if((double)spread_points > (double)strategy_median_spread_points * strategy_spread_median_mult)
         return true;
     }

   // News filtering is delegated to the framework and Strategy_NewsFilterHook.
   return false;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(entry <= 0.0 || strategy_emergency_sl_atr <= 0.0)
      return false;

   req.sl = QM_StopATR(_Symbol, req.type, entry, strategy_atr_period, strategy_emergency_sl_atr);
   if(req.sl <= 0.0)
      return false;

   req.tp = 0.0;
   req.reason = (cross > 0) ? "ATC_ATR_CHANNEL_LONG" : "ATC_ATR_CHANNEL_SHORT";
   return true;
  }
...
bool Strategy_NewsFilterHook(const datetime broker_time)
  {
   // No card-specific news rule; central framework news mode remains callable for P8.
