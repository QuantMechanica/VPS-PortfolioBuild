...
#property description "QM5_13045 XTI net-import shock fade"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_13045 - XTI Net-Import Shock Fade
// -----------------------------------------------------------------------------
// D1 structural WTI sleeve:
//   - uses Wednesday/Thursday as the weekly EIA net-import WPSR proxy window
//   - fades ATR-sized overextensions after a multi-day same-direction run
//   - ATR stop/target, SMA mean-reversion/time exits, no external runtime data
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 13045;
...
input int    strategy_run_lookback         = 5;
input int    strategy_sma_period           = 50;
input int    strategy_atr_period           = 20;
input double strategy_min_range_atr        = 0.90;
input double strategy_min_body_atr         = 0.25;
input double strategy_min_sma_distance_atr = 0.70;
input double strategy_min_run_atr          = 0.80;
input double strategy_low_close_location   = 0.30;
input double strategy_high_close_location  = 0.70;
input double strategy_atr_sl_mult          = 2.60;
input double strategy_atr_tp_mult          = 2.10;
...
bool Strategy_IsWpsrProxyDay(const datetime t)
  {
   if(t <= 0)
      return false;
   MqlDateTime dt;
...
bool Strategy_LoadNetImportFadeState(int &direction,
                                     double &atr_last,
                                     double &sma_last,
                                     int &signal_month_key)
  {
   ...
   if(signal_time <= 0 || !Strategy_IsWpsrProxyDay(signal_time))
      return false;
   ...
   const int run_shift = 1 + MathMax(2, strategy_run_lookback);
   const double run_start_close = iClose(_Symbol, PERIOD_D1, run_shift); // perf-allowed: compact D1 run state behind new-bar gate.
   if(run_start_close <= 0.0)
      return false;

   const double sma_distance_atr = MathAbs(signal_close - sma_last) / atr_last;
   if(sma_distance_atr < strategy_min_sma_distance_atr)
      return false;

   const double downside_run_atr = (run_start_close - signal_close) / atr_last;
   const double upside_run_atr = (signal_close - run_start_close) / atr_last;

   if(signal_close < signal_open &&
      signal_close < sma_last &&
      close_location <= strategy_low_close_location &&
      downside_run_atr >= strategy_min_run_atr)
      direction = 1;
   else if(signal_close > signal_open &&
           signal_close > sma_last &&
           close_location >= strategy_high_close_location &&
           upside_run_atr >= strategy_min_run_atr)
      direction = -1;
   else
      return false;
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(strategy_report_start_dow < 0 || strategy_report_start_dow > 6 || strategy_report_end_dow < 0 || strategy_report_end_dow > 6)
      return true;
   if(strategy_run_lookback < 2)
      return true;
   if(strategy_sma_period <= 1 || strategy_atr_period <= 1)
      return true;
   if(strategy_min_range_atr <= 0.0 || strategy_min_body_atr <= 0.0 || strategy_min_sma_distance_atr <= 0.0 || strategy_min_run_atr <= 0.0)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_13045_XTI_NETIMP_FADE";
   ...
   if(!Strategy_LoadNetImportFadeState(direction, atr_last, sma_last, signal_month_key))
      return false;
   if(sma_last <= 0.0)
      return false;
   ...
   req.reason = (direction > 0) ? "XTI_NETIMP_FADE_LONG" : "XTI_NETIMP_FADE_SHORT";
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_13045\",\"ea\":\"xti-netimp-fade\"}");
