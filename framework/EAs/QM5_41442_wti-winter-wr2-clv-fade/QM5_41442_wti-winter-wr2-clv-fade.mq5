...
#property description "QM5_41442 WTI Winter WR2 CLV Reversion"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41442 - WTI Winter WR2 Close-Location Reversion
// -----------------------------------------------------------------------------
...
//   - fade an expanding newest week only when its close is in an outer quartile
...
input double strategy_clv_upper            = 0.75;
input double strategy_clv_lower            = 0.25;
...
double   g_newer_week_clv         = 0.0;
bool     g_strict_range_expansion = false;
...
void Strategy_ResetDecisionState()
  {
   ...
   g_newer_week_clv = 0.0;
   g_strict_range_expansion = false;
   g_signal_state = "idle";
  }
...
bool Strategy_LoadWR2CLVSignal(const int current_week_key,
                               const int label_offset,
                               int &direction)
  {
   direction = 0;
   ...
   g_newer_week_clv =
      (g_newer_week_close - g_newer_week_low) / g_week_ranges[0];
   if(!MathIsValidNumber(g_newer_week_clv) ||
      g_newer_week_clv < 0.0 || g_newer_week_clv > 1.0)
      return false;

   g_strict_range_expansion = (g_week_ranges[0] > g_week_ranges[1]);
   if(g_strict_range_expansion && g_newer_week_clv > strategy_clv_upper)
      direction = -1;
   else if(g_strict_range_expansion && g_newer_week_clv < strategy_clv_lower)
      direction = 1;
   return true;
  }
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_WeekAlreadyEntered(g_decision_week_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadWR2CLVSignal(g_decision_week_key,
                                   g_decision_label_offset,
                                   g_signal_direction);
      if(!g_signal_valid)
         g_signal_state = "two_completed_weeks_validation_failed";
       else if(g_signal_direction > 0)
          g_signal_state = "wr2_lower_quartile_reversion_long";
       else if(g_signal_direction < 0)
          g_signal_state = "wr2_upper_quartile_reversion_short";
      else if(!g_strict_range_expansion)
         g_signal_state = "newest_range_not_strictly_wider_flat";
       else if(g_newer_week_clv == strategy_clv_upper ||
               g_newer_week_clv == strategy_clv_lower)
         g_signal_state = "close_location_equality_flat";
      else
          g_signal_state = "close_inside_middle_half_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"week\":%d,\"anchor_month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_week_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"week_bars\":[%d,%d],\"week_ranges\":[%.8f,%.8f],\"new_high\":%.8f,\"new_low\":%.8f,\"new_close\":%.8f,\"close_location\":%.12f,\"strict_range_expansion\":%s,\"state\":\"%s\"}",
                            g_decision_week_key,
                            anchor_month,
                            ...
                            g_newer_week_close,
                            g_newer_week_clv,
                            g_strict_range_expansion ? "true" : "false",
                            g_signal_state));
  }
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(strategy_symbol == "" ||
      ...
      strategy_max_week_bars != 5 ||
       MathAbs(strategy_clv_upper - 0.75) > 1.0e-12 ||
       MathAbs(strategy_clv_lower - 0.25) > 1.0e-12 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41442_WTI_WINTER_WR2_CLV_FADE";
   ...
   req.reason = (g_signal_direction > 0
                 ? "WTI_WINTER_WR2_CLV_FADE_LONG"
                 : "WTI_WINTER_WR2_CLV_FADE_SHORT");
...
int OnInit()
  {
   if(!SymbolSelect(strategy_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41442 ||
      qm_magic_slot_offset != 0)
   ...
   g_attempt_state_key =
      StringFormat("QM5_41442_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41442\",\"ea\":\"wti-winter-wr2-clv-fade\"}");
