# QM5_20208_nzdusd-euraud — Strategy Spec
...
`ln(NZDUSD.DWX) - (-0.286008035) * ln(EURAUD.DWX)`. Score the newest spread
...
| `strategy_beta` | -0.286008035 | fixed | DEV-fitted structural hedge coefficient |
...
- `AUDUSD.DWX` and `EURUSD.DWX` — conversion-history dependencies observed by
  the USD-account tester for EURAUD P/L; both are order-free and have no magic
  slot.
...
| Bar gating | `QM_IsNewBar(NZDUSD.DWX, PERIOD_D1)` consumed once per signal bar |
...
| OOS basket state changes | 19 across 2023–2024 in the frozen scan |
| Typical hold time | multi-day; scan half-life 138.333 D1 bars |
| Expected drawdown profile | high and regime-sensitive; conservative 30% prior |
| Regime preference | NZDUSD/EURAUD fixed-residual mean reversion |
| Win rate target | unknown; Q02 is the first platform-economic judge |

The fixed scan recorded DEV net Sharpe `-0.091704`, OOS net Sharpe `0.474703`,
and OOS return `4.877699%`. The negative DEV score, long half-life, and small
...
`artifacts/cards_approved/QM5_20208_nzdusd-euraud_card.md`
...
`1.0` and `0.286008035`; each leg is independently bounded by its ATR stop.
