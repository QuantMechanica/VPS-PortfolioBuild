...
#property description "QM5_20208 NZDUSD EURAUD Cointegration"
...
//
// Fixed spread:
//   ln(NZDUSD.DWX) - beta * ln(EURAUD.DWX)
//
...
input double strategy_beta              = -0.286008035;
...
bool Strategy_EnsureBasketScope()
  {
   ...
   string allowed[4] = {"NZDUSD.DWX", "EURAUD.DWX", "AUDUSD.DWX", "EURUSD.DWX"};
   for(int i = 0; i < 4; ++i)
...
bool Strategy_RefreshSpreadState()
  {
   ...
   for(int i = 0; i < history_count; ++i)
     {
      if(nzdusd_time[i] <= 0 || nzdusd_time[i] != euraud_time[i])
         return false;
      if(nzdusd[i] <= 0.0 || euraud[i] <= 0.0)
         return false;

      spreads[i] =
         MathLog(nzdusd[i]) - strategy_beta * MathLog(euraud[i]);
...
bool Strategy_PreparePair(const int spread_direction,
                          QM_EntryRequest &host_req)
  {
   ...
   const double nzdusd_weight = 1.0;
   const double euraud_weight = MathAbs(strategy_beta);
   const double weight_sum = nzdusd_weight + euraud_weight;
   if(weight_sum <= 0.0)
      return false;
   ...
   const double euraud_lots =
      Strategy_LotsForLeg(g_leg_euraud, euraud_weight, nzdusd_weight);
   if(nzdusd_lots <= 0.0 || euraud_lots <= 0.0)
      return false;

   const bool long_spread = (spread_direction > 0);
   const QM_OrderType nzdusd_type = long_spread ? QM_BUY : QM_SELL;
   const QM_OrderType euraud_type =
      long_spread
      ? (strategy_beta >= 0.0 ? QM_SELL : QM_BUY)
      : (strategy_beta >= 0.0 ? QM_BUY : QM_SELL);
   const string reason =
      long_spread ? "QM5_20208_LONG_SPREAD_Z_LT_NEG_ENTRY"
                  : "QM5_20208_SHORT_SPREAD_Z_GT_POS_ENTRY";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20208_PAIR_HOST";
...
void OnTick()
  {
   ...
   const bool is_new_signal_bar =
      QM_IsNewBar(g_leg_nzdusd, PERIOD_D1);
