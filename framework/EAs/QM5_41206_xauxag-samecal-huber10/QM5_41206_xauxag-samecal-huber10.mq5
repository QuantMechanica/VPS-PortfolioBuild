...
#property description "QM5_41206 XAU XAG Same-Calendar Huber"
...
//   - reconstruct exact synchronized returns for this month in Y-1..Y-10
//   - estimate paired XAU-minus-XAG direction with fixed-scale Huber location
//   - follow the robust relative-return sign with opposite metal legs
...
double   g_cache_scale = 0.0;
double   g_cache_delta = 0.0;
double   g_cache_huber_location = 0.0;
...
string Strategy_AttemptStateName()
  {
   return "QM5_41206_XAUXAG_SAMECAL_HUBER10_ATTEMPT";
  }
...
bool Strategy_CompletedMonthReturn(const MqlRates &rates[],
                                   ...
                                   datetime &following_time)
  {
   ...
   if(!TimeToStruct(rates[first_index - 1].time, previous_parts) ||
      !TimeToStruct(rates[last_index + 1].time, following_parts))
      return false;
   ...
   month_end_time = rates[last_index].time;
   previous_end_time = rates[first_index - 1].time;
   following_time = rates[last_index + 1].time;
      ...
      for(int index = 0; index < sample_count; ++index)
        {
         ...
         const double residual =
            MathAbs(observations[index] - location);
   ...
   for(int offset = 1; offset <= strategy_history_years; ++offset)
     {
      ...
      if(observation_index < 0 ||
         observation_index >= ArraySize(relative_returns))
         return false;
      ...
      if(!MathIsValidNumber(relative_return))
         return false;
      relative_returns[observation_index] = relative_return;
      ++g_cache_sample_count;
     }

   return Strategy_HuberSignal(relative_returns,
                               g_cache_sample_count,
                               g_cache_median,
                               g_cache_mad,
                               g_cache_scale,
                               g_cache_delta,
                               g_cache_huber_location,
                               pair_direction);
  }
...
bool Strategy_OpenPair(const int pair_direction)
  {
   ...
   const string reason = long_xau_short_xag ? "QM5_41206_LONG_XAU_SHORT_XAG_SAMECAL_HUBER10"
                                            : "QM5_41206_SHORT_XAU_LONG_XAG_SAMECAL_HUBER10";
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart())
      return true;
   if(qm_ea_id != 41206 || qm_magic_slot_offset != 0 || qm_rng_seed != 42)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41206_XAU_XAG_SAMECAL_HUBER10_HOST";
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_41206\",\"ea\":\"xauxag-samecal-huber10\"}");
