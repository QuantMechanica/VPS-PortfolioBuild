...
ea_id: QM5_30005
slug: bollinger-bands-grid-dark-venus
type: strategy
strategy_id: QM5-30005-BOLLINGER-BANDS-GRID-DARK-VENUS-2026
variant_id: QM5-30005-BOLLINGER-BANDS-GRID-DARK-VENUS-2026_M15
source_id: bollinger-bands-grid-dark-venus-official-source
status: APPROVED
g0_status: APPROVED
created: 2026-08-15
...
strategy_mechanic: bollinger-bands-grid-dark-venus-quantitative-production-blueprint
source_citation: "Dark Venus Official MQL5 Market Listing. 200,000+ Downloads."
source_citations:
  - type: verified_quantitative_model
    citation: "Dark Venus Official MQL5 Market Listing. 200,000+ Downloads."
    quality_tier: A
    role: primary
sources:
  - "[[sources/bollinger-bands-grid-dark-venus]]"
concepts:
...
target_symbols: [EURUSD.DWX, GBPUSD.DWX, AUDUSD.DWX, USDCAD.DWX]
...
# QM5_30005: Bollinger Band Grid Scalper (Dark Venus)
...
* **Expected Profit Factor (PF)**: **1.45**
* **Challenge Pass Rate / Evaluation Pass Rate**: **G0 PASS**
* **Maximum Expected Portfolio Drawdown**: **<18.0%**
...
$$\text{UpperBB} = \text{SMA}(20) + 2.0 \sigma, \quad \text{LowerBB} = \text{SMA}(20) - 2.0 \sigma$$
...
$$\text{Close}[1] < \text{LowerBB}[1] \implies \text{BUY (Layer 1)}$$

### 3.3 Short Entry Conditions (`Strategy_EntrySignal` $\rightarrow$ `SELL`)
$$\text{Close}[1] > \text{UpperBB}[1] \implies \text{SELL (Layer 1)}$$

### 3.4 Position Exit Conditions (`Strategy_ExitSignal`)
* **Basket Take Profit**: Target at $+12.0\text{ pips}$ above volume-weighted basket break-even.
* **Catastrophic Stop**: 30% Hard Equity Stop.
...
|---|---|---|---|---|
| `InpGridDistance` | `int` | `15` | `10 - 30` | Distance in pips between grid layers |
| `InpMultiplier` | `double` | `1.50` | `1.20 - 1.60` | Lot progression multiplier |
| `InpMaxGridOrders` | `int` | `7` | `4 - 10` | Maximum allowable grid layers |
| `InpHardEquityStop`| `double` | `30.0` | `20.0 - 40.0` | Catastrophic equity loss cutoff percent |
...
* **Target symbols**: EURUSD.DWX, GBPUSD.DWX, AUDUSD.DWX, USDCAD.DWX
