...
#property description "QM5_41341 XAU/XAG Monthly Bisquare Reversion"
...
// returns and compute their locked 32-step redescending Tukey-bisquare robust
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41341 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_BISQ_RV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41341_XAUXAG_BISQ_RV";
...
bool Strategy_BisquareLocation(const double &returns[],
                               double &median,
                               double &mad,
                               double &cutoff,
                               double &location)
  {
   ...
   cutoff = strategy_bisquare_cutoff * strategy_mad_normalizer * mad;
   if(!MathIsValidNumber(mad) || mad <= 0.0 ||
      !MathIsValidNumber(cutoff) || cutoff <= 0.0)
      return false;
   ...
   for(int step = 0; step < strategy_bisquare_steps; ++step)
     {
      ...
      for(int index = 0; index < strategy_return_count; ++index)
        {
         const double u = (returns[index] - location) / cutoff;
         if(!MathIsValidNumber(u))
            return false;
         double weight = 0.0;
         if(MathAbs(u) < 1.0)
           {
            const double core = 1.0 - u * u;
            weight = core * core;
           }
...
bool Strategy_LoadMonthlyBisquareSignal(
      ...
      int &direction)
  {
   ...
   if(StringLen(return_path) <= 0)
      return false;
   if(!Strategy_BisquareLocation(relative_returns,
                                 median,
                                 mad,
                                 cutoff,
                                 location))
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41341_XAUXAG_BISQ_RV_HOST";
   ...
   string return_path = "";
   const bool valid =
      Strategy_LoadMonthlyBisquareSignal(
         g_signal_month_key,
         month_count,
   ...
   string state = "monthly_bisquare_signal_failed";
   if(valid && direction > 0)
      ...
      state = "nonqualifying_bisquare_flat";

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"month_count\":%d,\"newest_endpoint_time\":%I64d,\"median\":%.12e,\"mad\":%.12e,\"cutoff\":%.12e,\"bisquare_location\":%.12e,\"steps\":%d,\"direction\":%d,\"ratios\":\"%s\",\"returns\":\"%s\",\"state\":\"%s\"}",
                            g_signal_month_key,
                            month_count,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG monthly redescending-bisquare reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41341\",\"ea\":\"xauxag-mbisquare-rv\"}");
