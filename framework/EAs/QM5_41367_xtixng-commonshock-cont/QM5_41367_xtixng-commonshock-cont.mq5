...
#property description "QM5_41367 XTI/XNG Weekly Common-Shock Relative Continuation"
...
// natural gas log returns over that exact interval share a strict sign, buy the
// relative winner and short the loser with an equal-notional opposite-leg package.
// Consume one attempt per week and flatten at the next week boundary.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41367;
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41367 && qm_magic_slot_offset == 0 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XTIXNG_COMMONSHOCK_ATTEMPT_WEEK", qm_ea_id);
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41367_XTIXNG_COMMONSHOCK_CONT";
...
bool Strategy_LoadWeeklyCommonShock(const int current_week,
                                    int &direction)
  {
   ...
   const bool same_positive =
      (g_wti_week_return > 0.0 && g_xng_week_return > 0.0);
   const bool same_negative =
      (g_wti_week_return < 0.0 && g_xng_week_return < 0.0);
   const double relative_return =
      g_wti_week_return - g_xng_week_return;
   if((same_positive || same_negative) &&
      ...
      direction = 1; // WTI outperformed: buy XTI, sell XNG.
           ...
           relative_return < -strategy_signal_epsilon)
      direction = -1; // natural gas outperformed: sell XTI, buy XNG.
   return true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41367_XTIXNG_COMMONSHOCK_CONT_HOST";
   ...
   if(Strategy_OpenOwnedPositionCount() > 0 ||
   ...
   else
     {
      ...
      if(!signal_valid)
         ...
         g_signal_state = "xng_relative_winner_cont_short_xti";
      else if(direction > 0)
         g_signal_state = "wti_relative_winner_cont_long_xti";
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XTI/XNG weekly common-shock package exits at the next broker week"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41367\",\"ea\":\"xtixng-commonshock-cont\"}");
