...
`artifacts/cards_approved/QM5_10241_tv-vwap-retest.md`. See that card's body for
...
`QM5_10241_tv-vwap-retest.mq5`. Framework wiring (risk, magic, news, Friday close)
...
|---|---|---|---|
| `strategy_session_start_hour` | 8 | (see source) | (see strategy logic) |
| `strategy_session_end_hour` | 21 | (see source) | (see strategy logic) |
| `strategy_session_close_flat` | true | (see source) | (see strategy logic) |
| `strategy_allow_shorts` | true | (see source) | (see strategy logic) |
| `strategy_atr_period` | 14 | (see source) | (see strategy logic) |
| `strategy_atr_sl_mult` | 1.0 | (see source) | (see strategy logic) |
| `strategy_atr_tp_mult` | 1.5 | (see source) | (see strategy logic) |
| `strategy_retest_max_bars` | 6 | (see source) | (see strategy logic) |
| `strategy_max_trades_per_day` | 2 | (see source) | (see strategy logic) |
| `strategy_volume_lookback` | 20 | (see source) | (see strategy logic) |
| `strategy_volume_spike_mult` | 1.2 | (see source) | (see strategy logic) |
| `strategy_rejection_wick_frac` | 0.30 | (see source) | (see strategy logic) |
| `strategy_retest_tolerance_atr` | 0.15 | (see source) | (see strategy logic) |
| `strategy_min_vwap_distance_atr` | 0.0 | (see source) | (see strategy logic) |
...
`artifacts/cards_approved/QM5_10241_tv-vwap-retest.md`
