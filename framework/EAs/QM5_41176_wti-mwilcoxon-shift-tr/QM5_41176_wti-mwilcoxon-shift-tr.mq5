...
#property description "QM5_41176 WTI Monthly Mann-Whitney Location-Shift Trend"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41176 - WTI Monthly Mann-Whitney Location-Shift Trend
// -----------------------------------------------------------------------------
...
//   - continue only at inclusive Mann-Whitney U_new boundaries 12 and 24
                                        ...
                                        int &direction)
  {
   ...
   const bool valid =
      Strategy_LoadMonthlyEndpoints(current_month_key,
                                    ...
                                    closes,
                                    endpoint_times) &&
      Strategy_MannWhitneySignal(closes, metrics);
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
   ...
   else
     {
      ...
      if(!g_signal_valid)
         ...
         g_signal_state = "mann_whitney_newer_dominant_wti_long";
      else if(g_signal_metrics.direction < 0)
         g_signal_state = "mann_whitney_newer_trailing_wti_short";
      else
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"endpoint_count\":%d,\"block_size\":%d,\"u_new\":%d,\"u_old\":%d,\"u_complement\":%d,\"newer_rank_sum\":%d,\"older_closes\":\"%s\",\"newer_closes\":\"%s\",\"oldest_close\":%.10f,\"newest_close\":%.10f,\"oldest_endpoint\":%I64d,\"newest_endpoint\":%I64d,\"state\":\"%s\"}",
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41176 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41176_WTI_MANN_WHITNEY_SHIFT_TREND";
   ...
   req.reason = (g_signal_metrics.direction > 0)
      ? "WTI_MANN_WHITNEY_NEWER_DOMINANT_LONG"
      : "WTI_MANN_WHITNEY_NEWER_TRAILING_SHORT";
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI Mann-Whitney location-shift trend holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41176_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41176\",\"ea\":\"wti-mwilcoxon-shift-tr\"}");
