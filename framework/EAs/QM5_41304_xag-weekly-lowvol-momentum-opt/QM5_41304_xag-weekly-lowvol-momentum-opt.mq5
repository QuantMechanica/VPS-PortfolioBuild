...
#property description "QM5_41304 XAGUSD Weekly Low-Volume Momentum Continuation"
...
// =============================================================================
// QM5_41304 - XAGUSD Weekly Low-Volume Momentum Continuation (Residual-Proxy)
// -----------------------------------------------------------------------------
// Structural D1 silver sleeve:
...
//   - only in the low-volume tercile (<= 33rd percentile), continue (trade with)
//     the latest five-bar close return sign
//   - persist the attempted week before fallible gates so there is no retry
//   - ATR hard stop, 10-bar max hold time exit, signal-flip exit
// Runtime uses native MT5 XAG D1 bars only; no external signal feed.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41304;
...
bool Strategy_CopyClosedD1Bars(MqlRates &rates[], const int bars_needed)
  {
   ...
   const int copied = CopyRates(_Symbol, PERIOD_D1, 1, bars_needed, rates); // perf-allowed: one bounded completed-D1 price/tick-volume vector, called only on the framework new-bar weekly-transition path.
...
bool Strategy_LoadWeeklyState(int &direction,
                              ...
                              double &atr_last)
  {
   ...
   // Continuation: follow the sign of the low-volume week return
   if(week_return > 0.0)
      direction = 1;
   ...
   // Low-volume filter: volume must be in the bottom tercile (<= strategy_vol_percentile)
   if(volume_rank > strategy_vol_percentile)
     {
      g_cached_state_reason = "above_volume_percentile";
      return false;
     }
...
void Strategy_PrepareWeeklySignal()
  {
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"week_key\":%d,\"valid\":%s,\"direction\":%d,\"return\":%.10f,\"tick_volume\":%.0f,\"volume_rank\":%.2f,\"reason\":\"%s\"}",
                            g_cached_week_key,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsExpectedHost())
      return true;
   if(qm_ea_id != 41304 || qm_magic_slot_offset != 0)
      return true;
   if(strategy_vol_lookback < 4 || strategy_vol_lookback > 260)
      return true;
   if(strategy_vol_percentile <= 0.0 || strategy_vol_percentile > 100.0)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41304_XAG_LOWVOL_MOM";
   ...
   req.reason = (g_cached_direction > 0)
                ? "XAG_LOW_TICKVOL_WEEK_MOM_LONG"
                : "XAG_LOW_TICKVOL_WEEK_MOM_SHORT";
   return true;
  }
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      // Signal-flip exit: if a new weekly evaluation produces a valid signal opposite
      // the held position direction, close it so reverse position can open atomically
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41304_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41304\",\"ea\":\"xag-weekly-lowvol-momentum-opt\",\"signal\":\"weekly_tick_volume_ranked_momentum_continuation\"}");
...
void OnTick()
  {
   ...
   // News check gates new entry orders only
