...
#property description "QM5_41035 WTI Monthly Information-Flow Divergence"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41035 - WTI Monthly Information-Flow Divergence
// -----------------------------------------------------------------------------
// Direct XTIUSD.DWX D1 structural sleeve:
...
//   - follow the session-flow direction until the next normalized broker month
...
bool Strategy_LoadMonthlyFlowSignal(const int current_month_key,
                                    ...
                                    double &total_flow)
  {
   ...
   // The approved edge trades only strict public/session disagreement and
   // follows the open-to-close session component. The completed-month total
   // is solely a reconciliation identity; its sign must not override the
   // session direction.
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
   ...
   else
     {
      ...
      if(!g_signal_valid)
         ...
         g_signal_state = "opposition_session_long";
      else if(g_signal_direction < 0)
         g_signal_state = "opposition_session_short";
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41035 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41035_WTI_MFLOW_DIV";
...
int OnInit()
  {
   if(!SymbolSelect(g_strategy_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41035 ||
   ...
   g_attempt_state_key =
      StringFormat("QM5_41035_MFLOW_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41035\",\"ea\":\"wti-mflow-div\"}");
