...
#property description "QM5_11673 Thomas Carter H1 Strategy 8 BB/EMA/MACD/RSI"
...
input int    strategy_macd_fast         = 6;
input int    strategy_macd_slow         = 17;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   const double ema_last = QM_EMA(_Symbol, tf, strategy_ema_period, 1);
   const double ema_prev = QM_EMA(_Symbol, tf, strategy_ema_period, 2);
   const double mid_last = QM_BB_Middle(_Symbol, tf, strategy_bb_period, strategy_bb_deviation, 1);
   const double mid_prev = QM_BB_Middle(_Symbol, tf, strategy_bb_period, strategy_bb_deviation, 2);
   ...
   if(ema_last <= 0.0 || ema_prev <= 0.0 || mid_last <= 0.0 || mid_prev <= 0.0 ||
      rsi <= 0.0 || atr <= 0.0)
      return false;

   const double histogram = macd_main - macd_signal;
   const bool long_signal = (ema_prev <= mid_prev && ema_last > mid_last &&
                             histogram > 0.0 && rsi > strategy_rsi_midline);
   const bool short_signal = (ema_prev >= mid_prev && ema_last < mid_last &&
                              histogram < 0.0 && rsi < strategy_rsi_midline);
   ...
   req.reason = long_signal ? "TC_S8_BB_EMA_MACD_RSI_LONG" : "TC_S8_BB_EMA_MACD_RSI_SHORT";
