...
#property description "QM5_9923 Bandy HMA Crossover Trend"

#include <QM/QM_Common.mqh>

// =============================================================================
// QuantMechanica V5 EA: QM5_9923
// Strategy Card: C:/QM/repo/framework/EAs/QM5_9923_bandy-hma-crossover-trend/docs/strategy_card.md
// Source: Howard Bandy, Quantitative Technical Analysis 2015 (9ef19e06-5ca6-5b35-aa06-b8187aa0e016)
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 9923;
...
input int    strategy_hma_fast            = 9;
input int    strategy_hma_slow            = 21;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
{
   ...
   // Long: HMA fast crosses above HMA slow AND close > regime
   if(hma_f1 > hma_s1 && hma_f2 <= hma_s2 && close1 > regime)
   {
      const double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
      if(ask <= 0.0)
         return false;
      ...
      req.sl     = sl;
      req.tp     = 0.0;
      req.reason = "BANDY_HMA_CROSSOVER_BUY";
      return true;
   }

   // Short: HMA fast crosses below HMA slow AND close < regime
   if(hma_f1 < hma_s1 && hma_f2 >= hma_s2 && close1 < regime)
   {
      const double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
      if(bid <= 0.0)
      ...
      req.sl     = sl;
      req.tp     = 0.0;
      req.reason = "BANDY_HMA_CROSSOVER_SELL";
      return true;
   }
...
void Strategy_ManageOpenPosition()
{
   ...
   const double hma_f2 = QM_HMA(_Symbol, PERIOD_D1, strategy_hma_fast, 2, PRICE_CLOSE);
   const double hma_s2 = QM_HMA(_Symbol, PERIOD_D1, strategy_hma_slow, 2, PRICE_CLOSE);
   const bool hma_ok   = (hma_f1 > 0.0 && hma_s1 > 0.0 && hma_f2 > 0.0 && hma_s2 > 0.0);
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
   {
      ...
      // Exit on opposite HMA cross
      if(hma_ok)
      {
         if(pos_type == POSITION_TYPE_BUY && hma_f1 < hma_s1 && hma_f2 >= hma_s2)
         {
            QM_TM_ClosePosition(ticket, QM_EXIT_OPPOSITE_SIGNAL);
            continue;
         }
         else if(pos_type == POSITION_TYPE_SELL && hma_f1 > hma_s1 && hma_f2 <= hma_s2)
         {
            QM_TM_ClosePosition(ticket, QM_EXIT_OPPOSITE_SIGNAL);
            continue;
         }
