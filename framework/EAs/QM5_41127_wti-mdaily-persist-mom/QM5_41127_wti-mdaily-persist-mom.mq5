...
#property description "QM5_41127 WTI Completed-Month Daily-Persistence Momentum"
...
//   - center every return and compute lag-one rho from adjacent products
//   - require J=rho+1/(n-1)>0, then follow the completed-month endpoint
//   - consume the month before fallible gates; never enter late or retry
//   - follow the daily-persistent completed-month direction for one broker month
// Runtime uses MT5-native completed closes, calendar, and framework state only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                      = 41127;
...
double   g_return_mean             = 0.0;
double   g_squared_deviation_sum   = 0.0;
double   g_adjacent_product_sum    = 0.0;
double   g_lag_one_autocorrelation = 0.0;
double   g_bias_adjustment         = 0.0;
double   g_persistence_score       = 0.0;
...
void Strategy_ResetDecisionState()
  {
   ...
   g_return_mean = 0.0;
   g_squared_deviation_sum = 0.0;
   g_adjacent_product_sum = 0.0;
   g_lag_one_autocorrelation = 0.0;
   g_bias_adjustment = 0.0;
   g_persistence_score = 0.0;
   g_signal_state = "idle";
  }
...
bool Strategy_LoadDailyPersistenceSignal(const int current_month_key,
                                  const int label_offset,
                                  int &direction,
                                  int &month_sessions,
                                  ...
                                  double &persistence_score)
  {
   ...
   for(int index = month_sessions - 1; index >= 0; --index)
     {
      ...
      if(!MathIsValidNumber(daily_return))
         return false;
      if(return_index < 0 || return_index >= month_sessions)
         return false;
   ...
   return_mean = net_return / (double)month_sessions;
   if(!MathIsValidNumber(return_mean))
      return false;

   for(int index = 0; index < month_sessions; ++index)
     {
      const double centered = daily_returns[index] - return_mean;
   ...
   // Exact zero variance is a valid consumed flat state. Autocorrelation is
   // undefined, so the score remains zero and no division is attempted.
   if(squared_deviation_sum == 0.0)
      return true;
   if(squared_deviation_sum < 0.0)
      return false;

   for(int index = 1; index < month_sessions; ++index)
     {
      const double current_centered =
         daily_returns[index] - return_mean;
      const double prior_centered =
         daily_returns[index - 1] - return_mean;
   ...
   if(!MathIsValidNumber(lag_one_autocorrelation) ||
      lag_one_autocorrelation < -1.0 - strategy_numerical_tolerance ||
      lag_one_autocorrelation > 1.0 + strategy_numerical_tolerance)
      return false;
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadDailyPersistenceSignal(g_decision_month_key,
                                      ...
                                      g_endpoint_return,
                                      g_return_mean,
                                      g_squared_deviation_sum,
                                      g_adjacent_product_sum,
                                      g_lag_one_autocorrelation,
                                      g_bias_adjustment,
                                      g_persistence_score);
      if(!g_signal_valid)
         g_signal_state = "monthly_persistence_validation_failed";
      else if(g_squared_deviation_sum == 0.0)
         g_signal_state = "zero_variance_flat";
      else if(g_net_return == 0.0)
         g_signal_state = "zero_net_flat";
      else if(g_signal_direction > 0)
         g_signal_state = "daily_persistence_completed_month_long";
      else if(g_signal_direction < 0)
         g_signal_state = "daily_persistence_completed_month_short";
      else
         g_signal_state = "nonpositive_persistence_score_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
                StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"month_sessions\":%d,\"boundary_close\":%.8f,\"final_close\":%.8f,\"net_return\":%.12e,\"endpoint_return\":%.12e,\"return_mean\":%.12e,\"squared_deviation_sum\":%.12e,\"adjacent_product_sum\":%.12e,\"lag_one_autocorrelation\":%.12e,\"bias_adjustment\":%.12e,\"persistence_score\":%.12e,\"threshold\":%.12e,\"state\":\"%s\"}",
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41127 ||
   ...
   if(strategy_entry_grace_minutes != 180 ||
      ...
      strategy_max_month_sessions != 23 ||
       ...
       MathAbs(strategy_persistence_threshold) > 0.000000000001 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41127_WTI_MDAILY_PERSIST_MOM";
   ...
   req.reason = (g_signal_direction > 0)
      ? "WTI_MDAILY_PERSIST_MOM_LONG"
      : "WTI_MDAILY_PERSIST_MOM_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41127 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI completed-month daily-persistence position holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41127_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41127\",\"ea\":\"wti-mdaily-persist-mom\"}");
