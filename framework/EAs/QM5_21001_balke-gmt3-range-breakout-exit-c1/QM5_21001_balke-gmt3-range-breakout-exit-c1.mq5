...
#property description "QM5_21001 Balke GMT3 Range Breakout Exit Challenger C1"
...
// Q15 single-lever contract: OFF is mechanically identical to the parent's
// fixed 18:00 exit; ON reads the frozen DEV-selected exit-hour input.
int Strategy_EffectiveExitHour()
  {
   if(!strategy_opt_enabled)
      return 18;
   return strategy_exit_hour;
  }
...
void Strategy_ManageOpenPosition()
  {
   ...
   const datetime now = TimeCurrent();
   const int hour = Strategy_Gmt3Hour(now);
   if(hour >= Strategy_EffectiveExitHour())
...
bool Strategy_ExitSignal()
  {
   ...
   // state is available.
   const datetime now = TimeCurrent();
   if(Strategy_Gmt3Hour(now) >= Strategy_EffectiveExitHour())
      return true;
