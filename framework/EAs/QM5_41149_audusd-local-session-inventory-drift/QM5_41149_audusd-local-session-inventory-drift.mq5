...
#property description "QM5_41149 AUDUSD Australia/Sydney local-session inventory drift"

#include <QM/QM_Common.mqh>

// =============================================================================
// QuantMechanica V5 EA
// Approved card: QM5_41149_audusd-local-session-inventory-drift
// Source: Breedon & Ranaldo (2013), Intraday Patterns in FX Returns and Order Flow
// =============================================================================

#define STRATEGY_EA_ID             41149
#define STRATEGY_SESSION_OPEN_HOUR 10
#define STRATEGY_SESSION_END_HOUR  16
#define STRATEGY_NEWS_QUERY_COUNT  6
...
datetime Strategy_SydneyDSTStartUTC(const int year)
  {
   const int day = QM_DSTAware_NthWeekdayOfMonth(year, 10, SUNDAY, 1);
   if(day < 1)
      return 0;

   MqlDateTime local_transition;
   ZeroMemory(local_transition);
   local_transition.year = year;
   local_transition.mon = 10;
   local_transition.day = day;
   local_transition.hour = 2;
   return StructToTime(local_transition) - 10 * 3600;
  }

datetime Strategy_SydneyDSTEndUTC(const int year)
  {
   const int day = QM_DSTAware_NthWeekdayOfMonth(year, 4, SUNDAY, 1);
   if(day < 1)
      return 0;
   ...
   ZeroMemory(local_transition);
   local_transition.year = year;
   local_transition.mon = 4;
   local_transition.day = day;
   local_transition.hour = 3;
   return StructToTime(local_transition) - 11 * 3600;
  }

bool Strategy_IsSydneyDSTUTC(const datetime utc_time)
  {
   if(utc_time <= 0)
      return false;
   MqlDateTime utc_parts;
   TimeToStruct(utc_time, utc_parts);
   const datetime dst_start = Strategy_SydneyDSTStartUTC(utc_parts.year);
   const datetime dst_end = Strategy_SydneyDSTEndUTC(utc_parts.year);
   if(dst_start <= 0 || dst_end <= 0)
      return false;
   return (utc_time < dst_end || utc_time >= dst_start);
  }

datetime Strategy_UTCToSydneyLocal(const datetime utc_time)
  {
   if(utc_time <= 0)
      return 0;
   return utc_time + (Strategy_IsSydneyDSTUTC(utc_time) ? 11 : 10) * 3600;
  }

bool Strategy_SameClock(const datetime lhs, const datetime rhs)
  {
   MqlDateTime a;
   MqlDateTime b;
   TimeToStruct(lhs, a);
...
// Test both legal offsets and require one unique round-trip. This fails closed
// for a nonexistent or ambiguous Australia/Sydney wall-clock timestamp.
bool Strategy_SydneyLocalToUTC(const datetime local_time, datetime &utc_time)
  {
   utc_time = 0;
   if(local_time <= 0)
      return false;

   const datetime standard_candidate = local_time - 10 * 3600;
   const datetime daylight_candidate = local_time - 11 * 3600;
   const bool standard_valid =
      Strategy_SameClock(Strategy_UTCToSydneyLocal(standard_candidate), local_time);
   const bool daylight_valid =
      Strategy_SameClock(Strategy_UTCToSydneyLocal(daylight_candidate), local_time);
   if(standard_valid == daylight_valid)
      return false;
   utc_time = standard_valid ? standard_candidate : daylight_candidate;
   return true;
  }
...
bool Strategy_CurrentSydneyTime(datetime &local_time)
  {
   local_time = Strategy_UTCToSydneyLocal(QM_BrokerToUTC(TimeCurrent()));
   return (local_time > 0);
  }
   ...
   for(int index = 0; index < total; ++index)
     {
      ...
      const datetime deal_local =
         Strategy_UTCToSydneyLocal(QM_BrokerToUTC(deal_broker));
...
bool Strategy_IsEntryBar(const datetime local_bar_open)
  {
   MqlDateTime parts;
   TimeToStruct(local_bar_open, parts);
...
bool Strategy_SessionNewsClear(const datetime local_bar_open)
  {
   MqlDateTime anchor;
   TimeToStruct(local_bar_open, anchor);
   anchor.min = 0;
   anchor.sec = 0;

   // Query each trailing one-hour slice at 11:00..16:00 local. The union is
   // exactly the owned [10:00,16:00] interval; Fresh also fails closed when the
   ...
   for(int index = 0; index < STRATEGY_NEWS_QUERY_COUNT; ++index)
     {
      ...
      if(!Strategy_SydneyLocalToUTC(local_anchor, utc_anchor))
         return false;
...
bool Strategy_BuildEntry(const datetime local_bar_open, QM_EntryRequest &req)
  {
   ...
   const double stop =
      QM_StopATRFromValue(_Symbol, QM_SELL, bid, atr, strategy_hard_stop_atr);
   const double stop_points = (stop - bid) / point;
   const long broker_stop_points =
      SymbolInfoInteger(_Symbol, SYMBOL_TRADE_STOPS_LEVEL);
   if(!MathIsValidNumber(stop) || stop <= ask ||
      !MathIsValidNumber(stop_points) || stop_points <= 0.0 ||
      (broker_stop_points > 0 && stop_points < (double)broker_stop_points) ||
      QM_LotsForRiskAtEntry(_Symbol, stop_points, ORDER_TYPE_SELL, bid) <= 0.0)
      return false;
   ...
   req.sl = stop;
   req.tp = 0.0;
   req.reason = "41149_sydney_inventory_sell";
   return true;
  }
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved Sydney intraday card exits at 16:00 local and retains the framework Friday fail-safe"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_%d_SYDNEY_ATTEMPT_%d", qm_ea_id, QM_FrameworkMagic());
   datetime local_now = 0;
   if(!Strategy_CurrentSydneyTime(local_now) ||
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41149\",\"session\":\"Australia/Sydney 10:00-16:00\"}");
...
void OnTick()
  {
   ...
   const datetime local_bar_open =
      Strategy_UTCToSydneyLocal(QM_BrokerToUTC(bar_broker));
   ...
   QM_EntryRequest req;
   ZeroMemory(req);
   if(Strategy_BuildEntry(local_bar_open, req))
     {
      ulong ticket = 0;
