...
type: strategy
strategy_id: WILLIAMS-SCHWEIKERT-XAUXAG-WFLOWAGREEFADE-2026_S01
variant_id: WILLIAMS-SCHWEIKERT-XAUXAG-WFLOWAGREEFADE-2026_S01
source_id: WILLIAMS-SCHWEIKERT-XAUXAG-WFLOWAGREEFADE-2026
ea_id: QM5_41057
slug: xauxag-wflow-agree-fade
status: APPROVED
execution_contract_ref: strategy-seeds/cards/approved/QM5_41057_xauxag-wflow-agree-fade_card.md
execution_contract_status: APPROVED
created: 2026-08-18
created_by: Research+Development
last_updated: 2026-08-18
g0_status: APPROVED
g0_decision: decisions/2026-08-18_qm5_41057_xauxag_weekly_flow_agreement_fade_g0.md
...
strategy_mechanic: exact-prior-monday-friday-synchronized-xau-minus-xag-close-open-versus-open-close-strict-agreement-fade-completed-relative-week-next-monday-paired-friday-flat
sources:
  - "[[sources/WILLIAMS-SCHWEIKERT-XAUXAG-WFLOWAGREEFADE-2026]]"
concepts:
...
strategy_type_flags: [commodity, precious-metals, relative-value, price-flow-decomposition, weekly-flow-agreement-reversion, logical-basket, weekly-entry, friday-close, atr-hard-stop, low-frequency]
markets: [commodities, precious_metals, gold, silver]
timeframes: [D1]
target_symbols: [XAUUSD.DWX, XAGUSD.DWX]
primary_target_symbols: [XAUUSD.DWX, XAGUSD.DWX]
single_symbol_only: false
logical_symbol: QM5_41057_XAU_XAG_WFLOWAGREEFADE_D1
symbol: QM5_41057_XAU_XAG_WFLOWAGREEFADE_D1
host_symbol: XAUUSD.DWX
symbol_slot: 0
magic: 410570000
companion_symbol: XAGUSD.DWX
companion_symbol_slot: 1
companion_magic: 410570001
period: D1
timeframe: D1
expected_trade_frequency: "Approximately 15-30 completed XAU/XAG packages per full post-warm-up year after exact-week, synchronization, and strict relative-flow agreement gates; Q02 must prove at least five/year or retire."
...
review_focus: "Falsify an exact-calendar XAU/XAG weekly relative-flow agreement fade designed to suppress common precious-metal direction: verify every completed close/open endpoint, cross-metal subtraction, strict same-sign admission, completed-week fade sides, basket atomicity, and Friday flattening. Q09 alone may establish realized decorrelation."
modules_used: [no_trade, trade_entry, trade_management, trade_close]
target_modules: [Strategy_NoTradeFilter, Strategy_EntrySignal, Strategy_ManageOpenPosition, Strategy_ExitSignal, Strategy_NewsFilterHook]
hard_rules_at_risk: [exact_synchronized_week, completed_close_open_endpoints, cross_metal_subtraction, strict_flow_agreement, completed_week_fade, flow_reconciliation, monday_decision_clock, weekly_attempt_state, no_current_bar_leakage, no_late_restart_entry, basket_atomicity, aggregate_fixed_risk, notional_mismatch, magic_schema, paired_friday_close, cfd_futures_basis, q02_frequency_floor, portfolio_correlation]
g0_approval_reasoning: "R1 complete OWNER-supplied Tier-A flow decomposition plus peer-reviewed and exchange gold/silver carrier lineage and complete governed weekly endpoint lineage with disclosed conjunction risk; R2 exact sequence, endpoints, subtraction, strict agreement, reconciliation, fade sides, attempt, timing, joint sizing, atomicity, and lifecycle; R3 native synchronized XAU/XAG D1 only; R4 deterministic arithmetic without a banned signal or trained logic; canonical dedup raised only the expected weekly and monthly flow neighbors and manual review established mutually exclusive state, direction, cadence, and lifecycle boundaries."
---

# QM5_41057 XAU/XAG Weekly Relative-Flow Agreement Fade

## Hypothesis

Gold and silver share broad precious-metal drivers but react differently to
monetary, safe-haven, and industrial information. When both the overnight and
session portions of an exact completed week move XAU versus XAG in the same
direction, their agreement identifies a persistent relative displacement. The
candidate tests whether that completed displacement mean-reverts by fading the
XAU-minus-XAG week with opposite legs from the next Monday through Friday.

This is a falsifiable price-flow, relative-value, and calendar translation.
Equal-notional opposite legs suppress some common precious-metal direction but
...
objects and discusses their separate accumulation and interaction. Schweikert
...
intermarket ratio/spread carrier. The governed weekly relative-flow packet
supplies the exact synchronized endpoint map, not the trade direction.

The five-session sums, gold-minus-silver subtraction, strict agreement,
completed-week fade, exact calendar sequence,
synchronized Darwinex CFDs, Monday grace, Friday lifecycle, equal-notional
constraint, fixed-dollar risk, hard stops, spread caps, and attempt ledger are
...
The deterministic pre-allocation checker found no exact identity and only the
expected fuzzy flow-family neighbors. Manual review fixes the load-bearing
boundaries:

- `QM5_41030_xauxag-flowdiv` requires strict component opposition and follows
  the session-relative sign. This card requires strict agreement and fades
  the total. Their admitted signal states are mutually exclusive.
- `QM5_41040_xauxag-wflow-fade` requires strict component opposition plus
  session dominance before fading the completed week. This card admits only
  agreement, so it cannot share an entry state with that system.
- `QM5_41039_xauxag-mflow-div` consumes a complete broker month, follows
  session-relative flow, and holds to the next month. This card uses one exact
  week, a Monday agreement decision, a contrarian total fade, and Friday flat.
- Ratio z-score, OLS, MAD, empirical-tail, failed-break, run-exhaustion,
  quantile-cointegration, and seasonal-surprise systems estimate a relative
  level, center, scale, fitted residual, tail, or long-horizon state. This card
  estimates none of those objects.
- Monthly XAU/XAG momentum and reversal systems use completed monthly returns.
  The weekend basket uses fixed Friday-to-Monday sides. Neither uses the exact
  prior-week information-time state or this Monday-to-Friday lifecycle.
...
- Host/traded slot 0: `XAUUSD.DWX`, D1, magic `410570000`.
- Companion/traded slot 1: `XAGUSD.DWX`, D1, magic `410570001`.
...
The rules below are the complete authorized baseline. No ratio level, fitted center, scale, absolute magnitude threshold,
volatility, month, event, curve, volume, oscillator, range, breakout,
regression, quantile, stationarity, or external-data signal filter is
authorized.

## 4. Entry Rules

1. Evaluate entry only on a new `XAUUSD.DWX` D1 bar while attached to exact
   `XAUUSD.DWX`, D1, EA ID 41057, slot 0.
...
9. Compute both metals' five-session overnight and session sums, the two
   gold-minus-silver relative components, and `week_relative` exactly as
   defined above. Reconcile each metal and the relative total to the frozen
   weekly endpoints within `1e-10`. The current Monday price enters no sum.
10. Require strict component agreement:
    `overnight_relative * session_relative > 0`. If `week_relative > 0`, SELL
    XAU and BUY XAG. If `week_relative < 0`, BUY XAU and SELL XAG. Component
    opposition, exact zero, invalid arithmetic, or failed reconciliation
    consumes the week.
...
- Strict flow agreement, endpoint reconciliation, and completed-week fade
  direction are load-bearing.
...
- Own at most one logical package: one XAU position under magic `410570000`
  and one XAG position under magic `410570001` in opposite directions.
...
No parameter sweep, after-result threshold, flow-sign, fade
side, weekday, component, hedge, or lifecycle change is authorized by this
card.
...
treats their separate accumulation and interaction as potentially informative.
...
week, agreement gate, fade direction, sizing, stops, or lifecycle.

## QM Interpretations

QM fixes the exact prior Monday-through-Friday synchronized endpoints, subtracts
silver flow from gold flow, requires strict component agreement, reconciles all
component totals, and fades the completed relative week. The Monday grace,
durable attempt, equal-notional solve,
...
synchronization, agreement, reconciliation, side, risk, atomicity,
or lifecycle defect. Any change to the carrier, week, component definition,
agreement rule, fade side, sizing, stop, or hold creates a new identity and
...
subtraction, agreement, reconciliation, or fade sides;
late/repeated entry; package-risk or
notional-mismatch breach; orphan survival; wrong Friday/stale lifecycle;
nondeterminism; or registry/risk-mode mismatch.

No weak result may be rescued by admitting opposition, following a component,
adding a magnitude threshold, changing the carrier/clock,
fitting a ratio, or extending the hold.
...
2. all component sign combinations, exact zero, and invalid prices select only
   strict agreement and the correct completed-week fade sides;
3. arithmetic uses all twenty completed close/open return components, excludes
   the current Monday, and reconciles each metal and relative weekly endpoint;
...
|---|---|---|---|---|
| v1 | 2026-08-18 | initial XAU/XAG weekly relative-flow agreement-fade card | G0 | APPROVED |
| v1-build | 2026-08-18 | deterministic synchronized logical-basket implementation and independent mechanic fixtures | Q01 | PASS |
...
|---|---|---|---|
| G0 Research Intake | 2026-08-18 | APPROVED | `decisions/2026-08-18_qm5_41057_xauxag_weekly_flow_agreement_fade_g0.md` |
| Q01 Build Validation | 2026-08-18 | PASS | `D:/QM/reports/framework/21/build_check_20260818_023902.json`; `D:/QM/reports/pipeline/QM5_41057/P1/P1_QM5_41057_result.json` |
| Q02 Baseline Screening | 2026-08-18 | NOT_ENQUEUED_CPU_CEILING | `docs/ops/evidence/2026-08-18_qm5_41057_xauxag_wflow_agree_fade_q01_q02_capacity_stop.md` |
