"""Independent mechanic fixtures for QM5_41057.

The tests cover the exact synchronized six-bar calendar, strict relative-flow
agreement, completed-week fade sides, endpoint reconciliation, Monday grace,
package risk/notional rounding, and weekly lifecycle without invoking MT5 or
...
def flow_agreement_direction(
    overnight_relative: float, session_relative: float
) -> int:
    week_relative = overnight_relative + session_relative
    strict_agreement = (
        (session_relative > 0 and overnight_relative > 0)
        or (session_relative < 0 and overnight_relative < 0)
    )
    if strict_agreement and week_relative > 0:
        return -1
    if strict_agreement and week_relative < 0:
        return 1
    return 0
...
def relative_flow_direction(
    ...
    direction = flow_agreement_direction(
...
class FlowAgreementFadeReferenceTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_positive_agreement_week_is_faded_by_selling_xau(self) -> None:
        xau = component_bars(100.0, 1.01, 1.02)
        xag = component_bars(100.0, 1.0, 1.0)
        ...
        self.assertGreater(session + overnight, 0)

    def test_negative_agreement_week_is_faded_by_buying_xau(self) -> None:
        xau = component_bars(100.0, 0.99, 0.98)
        xag = component_bars(100.0, 1.0, 1.0)
        direction, overnight, session, _, _ = relative_flow_direction(
            100.0, 100.0, xau, xag
        )
        ...
        self.assertLess(session + overnight, 0)

    def test_component_opposition_is_mutually_exclusive_and_flat(self) -> None:
        self.assertEqual(flow_agreement_direction(-0.01, 0.04), 0)
        self.assertEqual(flow_agreement_direction(0.04, -0.01), 0)

    def test_exact_zero_component_remains_flat(self) -> None:
        self.assertEqual(flow_agreement_direction(0.0, 0.05), 0)
        self.assertEqual(flow_agreement_direction(-0.05, 0.0), 0)
        self.assertEqual(flow_agreement_direction(0.0, 0.0), 0)

    def test_opposed_bars_and_unchanged_week_remain_flat(self) -> None:
        opposed = component_bars(100.0, 1.01, 0.99)
        unchanged = component_bars(100.0, 1.0, 1.0)
        self.assertEqual(
            relative_flow_direction(100.0, 100.0, opposed, unchanged)[0],
            0,
        )
        self.assertEqual(
    ...
    def test_each_metal_components_reconcile_to_week_return(self) -> None:
        ...
        overnight = sum(
            math.log(xau[i][0] / (100.0 if i == 0 else xau[i - 1][1]))
            - math.log(xag[i][0] / (80.0 if i == 0 else xag[i - 1][1]))
            for i in range(5)
        )
        session = sum(
            math.log(xau[i][1] / xau[i][0])
            - math.log(xag[i][1] / xag[i][0])
            for i in range(5)
        )
        self.assertAlmostEqual(
            overnight + session,
            math.log(xau[-1][1] / 100.0)
            - math.log(xag[-1][1] / 80.0),
        )
