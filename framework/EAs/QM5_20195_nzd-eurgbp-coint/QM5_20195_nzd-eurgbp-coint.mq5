...
#property description "QM5_20195 NZDUSD EURGBP Cointegration"
...
//
// Card:
//   strategy-seeds/cards/approved/QM5_20195_nzd-eurgbp-coint_card.md
//
// Fixed spread:
//   ln(NZDUSD.DWX) - beta * ln(EURGBP.DWX)
//
...
input double strategy_beta              = -0.101296029;
...
bool Strategy_RefreshSpreadState()
  {
   ...
   for(int i = 0; i < history_count; ++i)
     {
      if(nzdusd_time[i] <= 0 || nzdusd_time[i] != eurgbp_time[i])
         return false;
      if(nzdusd[i] <= 0.0 || eurgbp[i] <= 0.0)
         return false;

      spreads[i] =
         MathLog(nzdusd[i]) - strategy_beta * MathLog(eurgbp[i]);
...
bool Strategy_OpenPair(const int spread_direction)
  {
   ...
   const double weight_sum = nzdusd_weight + eurgbp_weight;
   if(weight_sum <= 0.0)
      return false;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20195_PAIR_HOST";
