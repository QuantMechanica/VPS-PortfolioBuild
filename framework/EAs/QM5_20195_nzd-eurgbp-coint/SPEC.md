# QM5_20195_nzd-eurgbp-coint — Strategy Spec
...
`ln(NZDUSD.DWX) - (-0.101296029) * ln(EURGBP.DWX)`. Score the newest spread
...
| `strategy_beta` | -0.101296029 | fixed | DEV-fitted structural hedge coefficient |
...
| Typical hold time | multi-day; scan half-life 116.313 D1 bars |
| Expected drawdown profile | high and regime-sensitive; conservative 30% prior |
| Regime preference | NZDUSD/EURGBP fixed-residual mean reversion |
| Win rate target | unknown; Q02 is the first platform-economic judge |

The fixed scan recorded DEV net Sharpe `-0.082879`, OOS net Sharpe `0.720866`,
and OOS return `8.085125%`. The negative DEV score, long half-life, and small
...
`artifacts/cards_approved/QM5_20195_nzd-eurgbp-coint_card.md`

Chan supplies the deterministic pair-trading method but makes no performance
claim for NZDUSD/EURGBP.
...
`1.0` and `0.101296029`; each leg is independently bounded by its ATR stop.
If the small EURGBP allocation normalizes below broker minimum volume, the
...
|---|---|---|---|
| v1 | 2026-08-01 | Initial build from approved card | Build task `19bb5e5c-eca4-4cfe-bd92-317fa91add5f` |
