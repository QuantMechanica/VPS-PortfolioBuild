...
#property description "QM5_12867 XBR XAU Return Spread Reversion"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_12867 - XBR/XAU Return-Spread Reversion
// -----------------------------------------------------------------------------
// D1 two-leg Brent/Gold basket:
//   rspread = log(xbr[t] / xbr[t-L]) - beta * log(XAU[t] / XAU[t-L])
   ...
   for(int i = 0; i < z_lookback; ++i)
     {
      ...
      if(xbr[i] <= 0.0 || xau[i] <= 0.0 || xbr[past_idx] <= 0.0 || xau[past_idx] <= 0.0)
         return false;

      const double xbr_ret = MathLog(xbr[i] / xbr[past_idx]);
      const double xau_ret = MathLog(xau[i] / xau[past_idx]);
      spreads[i] = xbr_ret - strategy_beta * xau_ret;
...
bool Strategy_OpenPair(const int spread_direction)
  {
   if(spread_direction == 0 || Strategy_OpenPairLegCount() > 0)
      return false;
   if(!Strategy_SpreadAllowed(g_leg_xbr) || !Strategy_SpreadAllowed(g_leg_xau))
      return false;

   const double xbr_weight = 1.0;
   const double xau_weight = MathMax(0.1, MathAbs(strategy_beta));
   const double weight_sum = xbr_weight + xau_weight;
   ...
   const string reason = long_spread ? "QM5_12867_LONG_XBR_XAU_RSPREAD"
                                     : "QM5_12867_SHORT_XBR_XAU_RSPREAD";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12867_RSPREAD_HOST";
...
int OnInit()
  {
   ...
   string basket_symbols[2] = {g_leg_xbr, g_leg_xau};
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_12867\",\"ea\":\"brentgold-rspread\"}");
