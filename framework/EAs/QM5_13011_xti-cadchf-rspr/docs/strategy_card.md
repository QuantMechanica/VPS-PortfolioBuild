...
type: strategy
strategy_id: BOC-CADCHF-OIL-RSPREAD-2026
source_id: BOC-CADCHF-OIL-RSPREAD-2026
source_citation: "Bank of Canada Staff Analytical Note 2017-1 on the Canadian dollar and commodity prices, with EIA Canada energy-export context."
source_citations:
  ...
  - type: government_energy_context
    ...
    role: supplement
sources:
  - "[[sources/BOC-CADCHF-OIL-RSPREAD-2026]]"
concepts:
...
target_symbols: [XTIUSD.DWX, CADCHF.DWX]
basket_symbols: [XTIUSD.DWX, CADCHF.DWX]
markets: [XTIUSD.DWX, CADCHF.DWX]
primary_target_symbols: [XTIUSD.DWX, CADCHF.DWX]
single_symbol_only: false
logical_symbol: QM5_13011_XTI_CADCHF_RSPREAD_D1
period: D1
timeframes: [D1]
expected_trade_frequency: "D1 XTI/CADCHF return-spread z-score reversion; estimate 8-14 paired packages/year before Q02 validates synchronized history and fills."
...
g0_approval_reasoning: "Mission-directed G0 approval 2026-07-04: R1 PASS Bank of Canada/EIA source packet; R2 PASS deterministic D1 XTI/CADCHF return-spread basket; R3 PASS XTIUSD.DWX and CADCHF.DWX exist in the DWX matrix; R4 PASS no ML/grid/martingale/external runtime data."
---

# XTI/CADCHF D1 Return-Spread Reversion

## Source

- Source: [[sources/BOC-CADCHF-OIL-RSPREAD-2026]]
- Primary citation: Bank of Canada Staff Analytical Note 2017-1, "The Link
  Between the Canadian Dollar and Commodity Prices: Has It Broken?", URL
  https://www.bankofcanada.ca/2017/02/staff-analytical-note-2017-1/.
- Support: EIA Canada Country Analysis Brief for Canadian energy-export
  context, URL https://www.eia.gov/international/analysis/country/CAN.

## Concept

Canadian dollar crosses have a structural commodity channel, but the CAD/oil
relationship can break by regime. This card avoids using CADCHF as a one-way
predictor and trades a two-leg relative-value package:

`return_spread = ln(XTI[t] / XTI[t-L]) - beta_cadchf * ln(CADCHF[t] / CADCHF[t-L])`

When WTI has unusually outperformed CADCHF over the fixed D1 return window, the
basket shorts WTI and buys CADCHF. When WTI has unusually underperformed
CADCHF, it buys WTI and sells CADCHF. The thesis is a temporary return-spread
dislocation between crude and a CAD commodity-FX cross, not an outright crude
forecast.

This is deliberately different from:

- `QM5_12607_wti-cad-confirm`: that EA uses USDCAD only as a confirmation
  filter for an outright WTI position.
- `QM5_12609_wti-cad-spread-mr` and `QM5_12722_wti-cad-brk`: those use
  USDCAD, not CADCHF, and different spread/entry definitions.
- `QM5_13010_xti-cadjpy-rspr`: that uses CADJPY and an oil-exporter /
  oil-importer cross; this card uses CADCHF and pure CAD commodity exposure
  against CHF.
- `QM5_13005_xbr-cad-rspr`: that uses Brent and a different CAD proxy.
- `QM5_13006_xti-nzd-rspread`, `QM5_12840_xti-xng-rspread`, Brent/WTI,
  oil/gold, oil/silver, XAU/XAG, XNG, index, and outright WTI calendar/event
  sleeves: this is a two-leg WTI/CADCHF return-spread basket.
...
- Logical symbol: `QM5_13011_XTI_CADCHF_RSPREAD_D1`.
- Host symbol: `XTIUSD.DWX`.
- Basket legs: `XTIUSD.DWX` and `CADCHF.DWX`.
- Period: D1.
...
- Runtime data: Darwinex MT5 D1 OHLC, spread metadata, ATR, broker time, and
  V5 framework state only. No Bank of Canada, EIA, futures-curve, CFTC,
...
- Copy completed D1 closes for `XTIUSD.DWX` and `CADCHF.DWX`.
...
- Compute `return_spread = xti_ret - strategy_beta_cadchf * cadchf_ret`.
...
The source packet establishes structural lineage for the CAD commodity channel
only. This card imports no source performance number. Q02 and later phases must
validate or reject the `XTIUSD.DWX` / `CADCHF.DWX` basket on Darwinex bars.
...
- risk_class: medium-high because crude volatility, CADCHF liquidity, and
...
- [x] R1 reputable source: single central-bank / government-source packet
  anchored on Bank of Canada commodity-CAD research.
...
- [x] R3 testable: `XTIUSD.DWX` and `CADCHF.DWX` exist in the Darwinex symbol
  matrix.
- [x] R4 compliant: no ML, adaptive PnL fitting, grid, martingale, external
  runtime feed, or more than one position per magic slot.
- [x] Non-duplicate: paired WTI/CADCHF return-spread mean reversion, not
  WTI/USDCAD, WTI/CADJPY, WTI/USDJPY, XTI/NZD, XTI/XNG, Brent/CAD,
  Brent/WTI, metal-ratio, XNG, index, or outright WTI logic.
...
- Slot 0: `XTIUSD.DWX`.
- Slot 1: `CADCHF.DWX`.
- Use the logical basket setfile `QM5_13011_XTI_CADCHF_RSPREAD_D1` for Q02.
...
- trade_entry: D1 standardized XTI/CADCHF return-spread reversion.
...
trade, if Q02 PF is below 1.0 after costs, if synchronized XTI/CADCHF history
...
|---|---|---|---|---|
| v1 | 2026-07-04 | initial XTI/CADCHF return-spread basket build | Q02 | PENDING |
...
| Q01 Build Validation | 2026-07-04 | PENDING | `artifacts/qm5_13011_build_result.json` |
