...
#property description "QM5_13043 XTI residual fuel pressure momentum"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_13043 - XTI Residual Fuel Pressure Momentum
// -----------------------------------------------------------------------------
// D1 structural WTI sleeve:
//   - uses Wednesday/Thursday as the weekly EIA residual-fuel WPSR proxy
//   - requires a short pullback, bullish winter/bunker reaction, and rising SMA
//   - ATR stop/target, SMA/season/time exits, no external runtime data
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 13043;
...
input int    strategy_season_start_month   = 11;
input int    strategy_season_end_month     = 2;
input int    strategy_report_start_dow     = 3;
input int    strategy_report_end_dow       = 4;
input int    strategy_pullback_lookback    = 5;
input double strategy_min_pullback_atr     = 0.25;
...
input double strategy_min_close_location   = 0.64;
...
bool Strategy_IsResidualFuelSeason(const datetime t)
  {
   if(t <= 0)
      return false;
   MqlDateTime dt;
...
bool Strategy_LoadResidualFuelPressureState(double &atr_last, int &signal_day_key)
  {
   atr_last = 0.0;
   signal_day_key = 0;
   ...
   if(signal_time <= 0 || !Strategy_IsReportProxyDay(signal_time))
      return false;
   if(!Strategy_IsResidualFuelSeason(signal_time) || !Strategy_IsResidualFuelSeason(TimeCurrent()))
      return false;
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      if(pos_type != POSITION_TYPE_BUY)
         should_close = true;
      if(!Strategy_IsResidualFuelSeason(now))
         should_close = true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_13043_XTI_RESFUEL_MOM";
   ...
   if(!Strategy_LoadResidualFuelPressureState(atr_last, signal_day_key))
      return false;
   ...
   req.reason = "XTI_RESFUEL_MOM_LONG";
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_13043\",\"ea\":\"xti-resfuel-mom\"}");
