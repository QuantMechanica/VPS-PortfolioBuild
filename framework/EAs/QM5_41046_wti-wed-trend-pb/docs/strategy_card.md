...
type: strategy
strategy_id: EIA-MOP-WTI-WEDTRENDPB-2026_S01
variant_id: EIA-MOP-WTI-WEDTRENDPB-2026_S01
source_id: EIA-MOP-WTI-WEDTRENDPB-2026
ea_id: QM5_41046
slug: wti-wed-trend-pb
status: APPROVED
execution_contract_ref: strategy-seeds/cards/approved/QM5_41046_wti-wed-trend-pb_card.md
execution_contract_status: APPROVED
created: 2026-08-17
created_by: Research+Development
last_updated: 2026-08-17
g0_status: APPROVED
g0_decision: decisions/2026-08-17_wti_wednesday_trend_pullback_g0.md
source_approval: decisions/2026-08-17_wti_wednesday_trend_pullback_source_approval.md
...
strategy_mechanic: standard-wednesday-completed-event-return-opposes-pre-event-252-session-trend-thursday-slow-trend-reentry-next-d1-exit
sources:
  - "[[sources/EIA-MOP-WTI-WEDTRENDPB-2026]]"
concepts:
...
strategy_type_flags: [commodity, energy, crude-oil, scheduled-event-proxy, time-series-momentum, cross-horizon-opposition, pullback-entry, thursday-entry, next-d1-exit, atr-hard-stop, low-frequency, symmetric-long-short]
...
period: D1
timeframe: D1
expected_trade_frequency: "Approximately 18-32 completed WTI positions per full post-warm-up year after exact standard-Wednesday and strict event/slow-trend sign-opposition gates; Q02 must prove at least eight/year or retire."
...
pipeline_phase: Q01
q01_status: PASS
review_focus: "Falsify a WTI standard-Wednesday completed counter-move re-entry stream outside the certified XAU/SP500/NDX/XNG book. Verify exact Monday-Tuesday-Wednesday identity, completed event return, pre-event 252-session trend endpoint, strict sign opposition, slow-trend side, durable Thursday attempt, and first-later-D1 flattening. Q09 alone may establish realized decorrelation."
modules_used: [no_trade, trade_entry, trade_management, trade_close]
target_modules: [Strategy_NoTradeFilter, Strategy_EntrySignal, Strategy_ManageOpenPosition, Strategy_ExitSignal, Strategy_NewsFilterHook]
hard_rules_at_risk: [exact_standard_wednesday_proxy, normalized_energy_label, completed_close_to_close_event_return, pre_event_252_session_trend, no_wednesday_slow_state_leakage, strict_sign_opposition, slow_trend_direction, thursday_decision_clock, thursday_attempt_state, no_current_bar_leakage, no_late_restart_entry, next_d1_exit, risk_mode_dual, cfd_futures_basis, q02_frequency_floor, portfolio_correlation]
g0_approval_reasoning: "R1 combines official EIA event identity with complete-read peer-reviewed WTI trend lineage while declaring the untested conjunction and CFD translation; R2 exact weekdays, label normalization, separate completed endpoints, opposition, slow-trend side, attempt, risk, and lifecycle; R3 registered native XTI D1 with session-label risk explicit; R4 deterministic arithmetic without trained logic, a banned signal, or external feed; canonical dedup found no exact identity and three nonidentical fuzzy matches, while manual family review separated agreement, intraday-flow fade, monthly pullback, pre-event trend, aftershock, and incumbent RSI systems."
---
...
The ordinary Wednesday U.S. petroleum information clock can concentrate a
one-session WTI counter-move. When the completed Wednesday close-to-close move
opposes a separate twelve-month WTI trend that ends before the event session,
the counter-move may partially unwind during the next D1 interval. The
candidate enters Thursday in the slow-trend direction and exits at the first
later D1 boundary.
...
`strategy-seeds/sources/EIA-MOP-WTI-WEDTRENDPB-2026/source.md`, approved before
card extraction in
`decisions/2026-08-17_wti_wednesday_trend_pullback_source_approval.md` at
commit `96f02558c`.
...
establish this WTI-specific weekly counter-move conjunction.

The exact D1 event proxy, uniform energy-label normalization, separate Tuesday
slow-state endpoint, strict sign opposition, slow-trend entry side, Thursday
grace, next-D1 hold, fixed-dollar risk, hard stop, spread cap, and attempt
...
The canonical pre-allocation checker scanned 4,533 EA-registry rows and 625
root-card files. It found no exact identity and surfaced three nonidentical
lexical/source-family fuzzy matches: `wti-dom-trend`, `wti-lr-trend`, and
`xng-lr-trend`. Manual review fixes the load-bearing boundaries:

- `QM5_41045_wti-wed-trend-agree` requires the completed Wednesday return and
  pre-event trend to agree and follows the common sign. This card requires
  opposition and trades the slow-trend sign.
- `QM5_41041_wti-wed-flow-fade` compares Wednesday close-to-open and open-to-
  close components and requires session dominance. This card ignores those
  components and compares the whole event return with a separate 252-session
  state.
- `QM5_20239_wti-pulltrend` uses a completed one-month counter-move, decides
  monthly, and holds to the next month. This card uses one exact Wednesday,
  decides Thursday, and exits next D1.
- `QM5_20154_wti-wed-trend` enters before Wednesday, is long-only, and owns the
  event; this card waits for the completed event bar and is symmetric.
- `QM5_12590_eia-wti-aftershock` uses event-day range expansion rather than a
  separate pre-event twelve-month sign.
- `QM5_20133` and `QM5_20134` trade M30 sequences inside Wednesday.
...
- Host and target: exact `XTIUSD.DWX`, D1, slot 0, magic `410460000`.
...
1. Evaluate only on a new `XTIUSD.DWX` D1 bar while attached to exact
   `XTIUSD.DWX`, D1, EA ID 41046, slot 0.
...
9. A positive slow trend buys WTI after a negative event return. A negative
   slow trend sells WTI after a positive event return. Magnitude never changes
   size.
...
- Exact `XTIUSD.DWX`, D1, EA 41046, and slot 0.
...
- Own at most one `XTIUSD.DWX` position under magic `410460000`.
...
exact weekly counter-move conjunction, entry clock, stop, spread, or lifecycle.
...
strict sign opposition, slow-trend trade side, Thursday grace, durable
attempt, fixed risk, ATR stop, spread ceiling, and next-D1 exit. All are pre-
...
endpoints, current-bar leakage, inclusion of Wednesday in the slow state,
non-opposed signs, wrong slow-trend side, late or repeated entry, wrong
lifecycle, nondeterminism, invalid risk mode, or an unusable standard-Wednesday
...
endpoints, opposition, or side; late or repeated entry; missing stop; wrong
...
agreement; changing the weekday; or extending the hold.
...
2. positive-trend/negative-event and negative-trend/positive-event branches
   trade the correct slow-trend side while agreement, zero, and invalid prices
   remain flat;
...
|---|---|---|---|---|
| v1 | 2026-08-17 | initial standard-Wednesday WTI counter-move/slow-trend re-entry card | G0 | APPROVED |
...
|---|---|---|---|
| G0 Research Intake | 2026-08-17 | APPROVED | `decisions/2026-08-17_wti_wednesday_trend_pullback_g0.md` |
| Q01 Build Validation | 2026-08-17 | PASS | `framework/build/compile/20260817_140351/QM5_41046_wti-wed-trend-pb.compile.log`; `D:/QM/reports/framework/21/build_check_20260817_140351.json`; `D:/QM/reports/pipeline/QM5_41046/P1/P1_QM5_41046_result.json` |
