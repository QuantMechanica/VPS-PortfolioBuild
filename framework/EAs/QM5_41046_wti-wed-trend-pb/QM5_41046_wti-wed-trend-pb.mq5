...
#property description "QM5_41046 WTI Wednesday Counter-Move / Slow-Trend Re-entry"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41046 - WTI Wednesday Counter-Move / Slow-Trend Re-entry
// -----------------------------------------------------------------------------
// Structural D1 crude-oil sleeve:
//   - require exact completed Monday-Tuesday-Wednesday broker sessions
//   - compute the completed Wednesday close-to-close event return
//   - compute a separate 252-session trend ending at Tuesday close
//   - enter Thursday only when the two nonzero returns have opposite signs,
//     rejoining the slow trend after the completed Wednesday counter-move
...
bool Strategy_LoadWednesdayTrendPullback(const datetime current_bar_time,
                                         const datetime broker_now,
                                         double &event_return,
                                         double &slow_trend,
                                         int &direction)
  {
   ...
   // Newest completed bar is Wednesday, followed by Tuesday and Monday.
   // Index lookback+1 is exactly 252 D1 intervals before Tuesday (index 1).
   ...
   // The event return uses Wednesday and Tuesday closes. The slow state ends
   // at Tuesday and therefore cannot contain the Wednesday event move.
   if(bars[0].close <= 0.0 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "WTI_WED_TREND_PULLBACK";
   ...
   int direction = 0;
   if(!Strategy_LoadWednesdayTrendPullback(g_strategy_d1_bar_time,
                                            broker_now,
                                            event_return,
                                            slow_trend,
                                            direction))
   ...
   if(direction > 0)
     {
      req.type = QM_BUY;
      req.reason = "WTI_WED_TREND_PULLBACK_LONG";
     }
   else if(direction < 0)
     {
      req.type = QM_SELL;
      req.reason = "WTI_WED_TREND_PULLBACK_SHORT";
     }
   else
      return false;
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI Wednesday counter-move/slow-trend card exits next D1; Friday 21 is fail-safe"))
      return INIT_FAILED;
   ...
   g_attempt_state_key =
      StringFormat("QM5_41046_WED_TREND_PULLBACK_THURSDAY_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41046\",\"ea\":\"wti-wed-trend-pb\"}");
