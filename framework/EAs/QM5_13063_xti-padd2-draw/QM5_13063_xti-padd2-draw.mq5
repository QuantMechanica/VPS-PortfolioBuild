...
#property description "QM5_13063 XTI Midwest PADD2 stockdraw momentum"
...
bool Strategy_IsPadd2DrawSeason(const datetime t)
  {
   if(t <= 0)
      return false;
   MqlDateTime dt;
...
bool Strategy_LoadPadd2DrawState(double &atr_last,
                                 int &signal_day_key,
                                 int &signal_month_key)
  {
   ...
   if(signal_time <= 0 || !Strategy_IsReportProxyDay(signal_time))
      return false;
   if(!Strategy_IsPadd2DrawSeason(signal_time) || !Strategy_IsPadd2DrawSeason(TimeCurrent()))
      return false;
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      if(pos_type != POSITION_TYPE_BUY)
         should_close = true;
      if(!Strategy_IsPadd2DrawSeason(now))
         should_close = true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_13063_XTI_PADD2_DRAW";
   ...
   if(!Strategy_LoadPadd2DrawState(atr_last, signal_day_key, signal_month_key))
      return false;
   ...
   req.reason = "XTI_PADD2_DRAW_LONG";
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_13063\",\"ea\":\"xti-padd2-draw\"}");
