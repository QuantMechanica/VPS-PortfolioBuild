...
#property description "QM5_10777 TradingView RSI Bollinger Mean Reversion"
...
input int    strategy_rsi_period        = 19;
input double strategy_rsi_long_level    = 20.0;
input double strategy_rsi_short_level   = 72.0;
...
int Strategy_BBMeanRevSignal(const ENUM_TIMEFRAMES tf)
  {
   ...
   const double close_1 = iClose(_Symbol, tf, 1); // perf-allowed: closed-bar source price required for Bollinger breach check.
   if(close_1 <= 0.0 || upper <= 0.0 || lower <= 0.0)
      return 0;
   if(close_1 < lower)
      return +1;
   if(close_1 > upper)
...
bool Strategy_NoTradeFilter()
  {
   if(strategy_rsi_period <= 1 || strategy_bb_period <= 1 || strategy_atr_period <= 1)
      return true;
   if(strategy_bb_deviation <= 0.0 || strategy_atr_sl_mult <= 0.0)
      return true;
   if(strategy_rsi_long_level >= strategy_rsi_short_level)
      return true;
   return false;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   const int bb_signal = Strategy_BBMeanRevSignal(tf);
   const bool long_signal = (rsi < strategy_rsi_long_level && bb_signal > 0);
   const bool short_signal = (rsi > strategy_rsi_short_level && bb_signal < 0);
   ...
   req.reason = long_signal ? "RSI_BB_MR_LONG" : "RSI_BB_MR_SHORT";
   return true;
  }
...
bool Strategy_ExitSignal()
  {
   ...
   const double rsi = QM_RSI(_Symbol, tf, strategy_rsi_period, 1);
   const int bb_signal = Strategy_BBMeanRevSignal(tf);
   const bool long_signal = (rsi > 0.0 && rsi < strategy_rsi_long_level && bb_signal > 0);
   const bool short_signal = (rsi > strategy_rsi_short_level && bb_signal < 0);
      ...
      if(strategy_time_stop_bars > 0)
        {
         ...
         if(opened_at > 0 && (TimeCurrent() - opened_at) >= (strategy_time_stop_bars * seconds_per_bar))
            return true;
        }
     }
   return false;
