"""Independent mechanic fixtures for QM5_41041.
...
component opposition, strict session dominance, return reconciliation,
contrarian direction, Thursday grace/attempt identity, and next-D1 exit.
...
def select_direction(
    ...
    if not signs_oppose or abs(session_flow) <= abs(overnight_flow):
        return 0
    # Fade the completed Wednesday total.
...
class WednesdayFlowFadeReferenceTests(unittest.TestCase):
    ...
    def test_session_dominant_positive_day_is_sold(self) -> None:
        direction, overnight, session, day_return, total = (
            wednesday_direction(100.0, 99.0, 102.0)
        )
    ...
    def test_session_dominant_negative_day_is_bought(self) -> None:
    ...
    def test_overnight_dominance_is_flat(self) -> None:
