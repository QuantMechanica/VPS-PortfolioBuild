...
#property description "QM5_41041 WTI Standard-Wednesday Session-Dominant Flow Fade"
...
//   - enter Thursday only when the streams oppose and the session dominates,
//     fading the reconciled completed Wednesday move
...
bool Strategy_LoadWednesdayFlowFade(const datetime current_bar_time,
                                    const datetime broker_now,
                                    ...
                                    int &direction)
  {
   ...
   if(!signs_oppose)
      return true;
   if(MathAbs(session_flow) <= MathAbs(overnight_flow))
      return true;

   // Fade the completed Wednesday displacement; magnitude never changes risk.
   if(total_flow > 0.0)
      direction = -1;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   int direction = 0;
   if(!Strategy_LoadWednesdayFlowFade(g_strategy_d1_bar_time,
                                  broker_now,
                                  overnight_flow,
                                  session_flow,
                                  day_return,
   ...
   if(direction > 0)
     {
      req.type = QM_BUY;
      req.reason = "WTI_WED_FLOW_FADE_LONG";
     }
   else if(direction < 0)
     {
      req.type = QM_SELL;
      req.reason = "WTI_WED_FLOW_FADE_SHORT";
     }
   else
      return false;
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI Wednesday flow-fade card exits next D1; Friday 21 is fail-safe"))
      return INIT_FAILED;
   ...
   g_attempt_state_key =
      StringFormat("QM5_41041_WED_FLOW_FADE_THURSDAY_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41041\",\"ea\":\"wti-wed-flow-fade\"}");
