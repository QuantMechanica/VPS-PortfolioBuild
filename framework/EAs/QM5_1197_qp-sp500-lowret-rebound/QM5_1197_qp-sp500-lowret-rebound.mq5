...
#property description "QM5_1197 Quantpedia Low-Return Rebound - SP500"

#include <QM/QM_Common.mqh>

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                       = 1197;
...
input int    strategy_min_valid_closes      = 270;
...
input double strategy_gap_risk_kill_mult    = 2.5;
...
bool Strategy_IsSessionBeforeFullHolidayNY(const datetime ny_date)
  {
   datetime cursor = Strategy_DateFloor(ny_date);
   for(int guard = 0; guard < 7; ++guard)
     {
      cursor += 86400;
      const int dow = Strategy_DayOfWeek(cursor);
      if(dow < 1 || dow > 5)
         continue;
      return Strategy_IsUsCashHoliday(cursor);
     }
   return false;
  }
...
bool Strategy_HasLowReturnSignal(int &signal_key)
  {
   signal_key = 0;
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(!Strategy_IsRegularUsCashDayNY(today_ny))
      return true;
   if(Strategy_IsSessionBeforeFullHolidayNY(today_ny))
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QP_SP500_LOWRET_REBOUND";
   ...
   int signal_key = 0;
   if(!Strategy_HasLowReturnSignal(signal_key))
      return false;
...
void Strategy_ManageOpenPosition()
  {
   ...
   if(open_price <= 0.0 || stop_loss <= 0.0 || bid <= 0.0)
      return;

   const double planned_risk = open_price - stop_loss;
   if(planned_risk <= 0.0 || strategy_gap_risk_kill_mult <= 0.0)
      return;

   if(bid <= open_price - strategy_gap_risk_kill_mult * planned_risk)
      QM_TM_ClosePosition(ticket, QM_EXIT_SL_HIT);
  }
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_1197\",\"ea\":\"qp-sp500-lowret-rebound\"}");
