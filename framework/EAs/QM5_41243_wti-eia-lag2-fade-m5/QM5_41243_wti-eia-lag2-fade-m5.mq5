...
#property description "QM5_41243 WTI EIA five-minute lag-2 reaction fade"
...
// -----------------------------------------------------------------------------
// On a standard Wednesday, consume one decision at 10:35 New York. Fade the
// sign of the completed 10:30-10:35 M5 price reaction and flatten at 10:45.
// The price sign is a deliberately bounded proxy, not an inventory-surprise
// classifier; holiday-shifted releases are intentionally not inferred.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                     = 41243;
...
bool Strategy_IsHostChart()
  {
   return (_Symbol == "XTIUSD.DWX" && _Period == PERIOD_M5);
  }
...
void Strategy_LoadDirectionState(const datetime reference_time)
  {
   g_expected_direction_day_key = 0;
   ...
   const int current_day_key = Strategy_NYDayKey(reference_time);
   const double stored = GlobalVariableGet(g_direction_state_key);
   const int signed_day_key = (int)MathRound(stored);
   const int stored_day_key =
      (signed_day_key < 0) ? -signed_day_key : signed_day_key;
   if(current_day_key > 0 && MathIsValidNumber(stored) &&
      stored_day_key >= 19000101 && stored_day_key <= current_day_key)
     {
      g_expected_direction_day_key = stored_day_key;
      g_expected_direction = (signed_day_key > 0) ? 1 : -1;
      return;
     }
...
void Strategy_ClearDirectionState()
  {
   g_expected_direction_day_key = 0;
   g_expected_direction = 0;
   if(g_direction_state_key != "" &&
...
bool Strategy_RecordDirectionState(const int day_key,
                                   const int direction)
  {
   if(day_key <= 0 || (direction != 1 && direction != -1) ||
      g_direction_state_key == "")
      return false;
   const double encoded =
      (direction > 0) ? (double)day_key : -(double)day_key;
   if(GlobalVariableSet(g_direction_state_key, encoded) <= 0)
      return false;
   g_expected_direction_day_key = day_key;
...
void Strategy_PrepareEventSignal()
  {
   ...
   const datetime decision_bar =
      iTime(_Symbol, PERIOD_M5, 0); // perf-allowed: one new-bar event anchor.
   if(decision_bar <= 0 ||
   ...
   if(Strategy_DayAlreadyEntered(day_key))
      return;
   Strategy_ClearDirectionState();

   const datetime now = TimeCurrent();
   const long opening_delay = (long)(now - decision_bar);
   const int now_second = Strategy_NYSecond(now);
   if(opening_delay < 0 ||
      opening_delay >= (long)strategy_entry_grace_seconds ||
      now_second < 0 || now_second >= strategy_entry_grace_seconds)
      return;

   const datetime release_bar =
      iTime(_Symbol, PERIOD_M5, 1); // perf-allowed: exact completed event bar.
   if(release_bar <= 0 || decision_bar - release_bar != 300 ||
      Strategy_NYDayKey(release_bar) != day_key ||
      Strategy_NYHhmm(release_bar) != strategy_release_hhmm_ny)
      return;

   const double release_open =
      iOpen(_Symbol, PERIOD_M5, 1); // perf-allowed: one fixed completed bar.
   const double release_high =
      iHigh(_Symbol, PERIOD_M5, 1); // perf-allowed: one fixed completed bar.
   const double release_low =
      iLow(_Symbol, PERIOD_M5, 1); // perf-allowed: one fixed completed bar.
   const double release_close =
      iClose(_Symbol, PERIOD_M5, 1); // perf-allowed: one fixed completed bar.
   ...
   const int direction = (release_close < release_open) ? 1 : -1;
   if(!Strategy_RecordDirectionState(day_key, direction))
      return;

   const double atr =
      QM_ATR(_Symbol, PERIOD_M5, strategy_atr_period_m5, 1);
   ...
   for(int index = PositionsTotal() - 1; index >= 0; --index)
     {
      ...
      const ENUM_POSITION_TYPE expected_type =
         (g_expected_direction > 0) ? POSITION_TYPE_BUY : POSITION_TYPE_SELL;

      bool should_close =
         (magic_count != 1 || symbol != _Symbol ||
          g_expected_direction_day_key != opened_day_key ||
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   request.type = (g_signal_direction > 0) ? QM_BUY : QM_SELL;
   request.price = 0.0;
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = (g_signal_direction > 0)
                    ? "WTI_EIA_LAG2_FADE_LONG"
                    : "WTI_EIA_LAG2_FADE_SHORT";
   ...
   if(!g_entry_ready || g_signal_day_key <= 0 ||
      ...
      g_signal_day_key != g_expected_direction_day_key ||
      g_signal_direction != g_expected_direction ||
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved card keeps Friday close enabled; WTI event position is flat by 10:45 New York"))
      return INIT_FAILED;

   g_attempt_state_key =
      StringFormat("QM5_41243_WTI_EIA_LAG2_FADE_ATTEMPT_%d",
                   QM_FrameworkMagic());
   g_direction_state_key =
      StringFormat("QM5_41243_WTI_EIA_LAG2_FADE_DIRECTION_%d",
                   QM_FrameworkMagic());
   const datetime reference_time =
      iTime(_Symbol, PERIOD_M5, 0); // perf-allowed: one-time state reference.
   const datetime state_reference =
      (reference_time > 0) ? reference_time : TimeCurrent();
   Strategy_LoadAttemptState(state_reference);
   Strategy_LoadDirectionState(state_reference);

   string warmup_symbols[1];
   warmup_symbols[0] = _Symbol;
   QM_SymbolGuardInit(warmup_symbols);
   QM_BasketWarmupHistory(warmup_symbols, PERIOD_M5, 64);

   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41243\",\"ea\":\"wti-eia-lag2-fade-m5\"}");
