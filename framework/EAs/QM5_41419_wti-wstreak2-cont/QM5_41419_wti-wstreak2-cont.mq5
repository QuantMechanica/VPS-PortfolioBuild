...
#property description "QM5_41419 WTI Fresh Two-Week Sign-Streak Continuation"
...
//   - reconstruct four consecutive completed broker-week ending closes
//   - calculate three adjacent completed-week log returns
//   - require the newest two signs to agree and the predecessor to oppose
...
input int    strategy_history_bars         = 45;
...
bool Strategy_LoadFreshStreakSignal(const int current_week_key,
                                    ...
                                    double &week_end_c4)
  {
   ...
   if(return_r0 > strategy_return_epsilon &&
      return_r1 > strategy_return_epsilon &&
      return_r2 < -strategy_return_epsilon)
      direction = 1;
   else if(return_r0 < -strategy_return_epsilon &&
           return_r1 < -strategy_return_epsilon &&
           return_r2 > strategy_return_epsilon)
      direction = -1;
   return true;
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_WeekAlreadyEntered(g_decision_week_key))
   ...
   else
     {
      ...
      if(!g_signal_valid)
         ...
         g_signal_state = "fresh_positive_two_week_streak_long";
      else if(g_signal_direction < 0)
         g_signal_state = "fresh_negative_two_week_streak_short";
      else if(MathAbs(g_return_r0) <= strategy_return_epsilon ||
              MathAbs(g_return_r1) <= strategy_return_epsilon ||
              MathAbs(g_return_r2) <= strategy_return_epsilon)
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"week\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_week_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"week_bars\":[%d,%d,%d,%d],\"r2\":%.12e,\"r1\":%.12e,\"r0\":%.12e,\"c4\":%.8f,\"c3\":%.8f,\"c2\":%.8f,\"c1\":%.8f,\"state\":\"%s\"}",
                            g_decision_week_key,
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41419_WTI_WSTREAK2_CONT";
   ...
   req.reason = (g_signal_direction > 0)
      ? "WTI_WSTREAK2_CONT_LONG"
      : "WTI_WSTREAK2_CONT_SHORT";
...
int OnInit()
  {
   ...
   if(!SymbolSelect(g_strategy_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41419 ||
      qm_magic_slot_offset != 0)
   ...
   g_attempt_state_key =
      StringFormat("QM5_41419_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41419\",\"ea\":\"wti-wstreak2-cont\"}");
