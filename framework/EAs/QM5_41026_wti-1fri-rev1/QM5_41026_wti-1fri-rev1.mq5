...
#property description "QM5_41026 WTI First-Friday Prior-Month Reversal"
...
//   - decide only on the first genuine Friday of each broker month
//   - buy only after a negative immediately completed calendar-month return
//   - consume the month before fallible gates; never shift, retry, or backfill
//   - flatten through Friday close; later-D1 and four-day exits repair failures
// Runtime uses MT5-native OHLC, calendar, history, and framework state only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                      = 41026;
...
void Strategy_DetectDecisionClock_OnNewBar()
  {
   ...
   if(current_month_key <= 0 ||
      ...
      Strategy_DayOfWeekForTime(normalized_prior) != 4)
      return;
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
   ...
   else
     {
       ...
       if(!g_signal_valid)
          ...
          g_signal_state = "prior_month_negative_long";
       else if(g_signal_direction > 0)
          g_signal_state = "prior_month_positive_flat";
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41026 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41026_WTI_1FRI_MOM1";
...
void Strategy_ManageOpenPosition()
  {
   ...
   // Friday close is the ordinary exit. A later normalized D1 label repairs
   // any Friday-close failure before another entry can be considered.
...
int OnInit()
  {
   if(!SymbolSelect(g_strategy_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41026 ||
   ...
   g_attempt_state_key =
      StringFormat("QM5_41026_1FRI_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41026\",\"ea\":\"wti-1fri-rev1\"}");
