...
#property description "QM5_20240 USDCHF GBPJPY Cointegration"
...
//
// Card:
//   strategy-seeds/cards/approved/QM5_20240_usdchf-gbpjpy_card.md
//
// Fixed spread:
//   ln(USDCHF.DWX) - beta * ln(GBPJPY.DWX)
//
...
input double strategy_beta              = -0.070133022445;
...
bool Strategy_EnsureBasketScope()
  {
   ...
   string allowed[3] = {"USDCHF.DWX", "GBPJPY.DWX", "USDJPY.DWX"};
   for(int i = 0; i < 3; ++i)
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(!QM_SymbolAssertOrLog(g_leg_usdchf) ||
      !QM_SymbolAssertOrLog(g_leg_gbpjpy) ||
   ...
   for(int i = 0; i < history_count; ++i)
     {
      if(usdchf_time[i] <= 0 || usdchf_time[i] != gbpjpy_time[i])
         return false;
      if(usdchf[i] <= 0.0 || gbpjpy[i] <= 0.0)
         return false;

      spreads[i] =
         MathLog(usdchf[i]) - strategy_beta * MathLog(gbpjpy[i]);
...
bool Strategy_PreparePair(const int spread_direction,
                          QM_EntryRequest &host_req)
  {
   ...
   const double weight_sum = usdchf_weight + gbpjpy_weight;
   if(weight_sum <= 0.0)
      return false;
   ...
   const double gbpjpy_lots =
      Strategy_LotsForLeg(g_leg_gbpjpy, gbpjpy_weight, usdchf_weight);
   if(usdchf_lots <= 0.0 || gbpjpy_lots <= 0.0)
      return false;

   const bool long_spread = (spread_direction > 0);
   const QM_OrderType usdchf_type = long_spread ? QM_BUY : QM_SELL;
   const QM_OrderType gbpjpy_type =
      long_spread
      ? (strategy_beta >= 0.0 ? QM_SELL : QM_BUY)
      : (strategy_beta >= 0.0 ? QM_BUY : QM_SELL);
   const string reason =
      long_spread ? "QM5_20240_LONG_SPREAD_Z_LT_NEG_ENTRY"
                  : "QM5_20240_SHORT_SPREAD_Z_GT_POS_ENTRY";
   ...
   g_companion_entry_type = gbpjpy_type;
   g_companion_entry_lots = gbpjpy_lots;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20240_PAIR_HOST";
