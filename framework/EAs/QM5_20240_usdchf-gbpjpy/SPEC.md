# QM5_20240_usdchf-gbpjpy — Strategy Spec
...
`ln(USDCHF.DWX) - (-0.070133022445) * ln(GBPJPY.DWX)`. Score the newest spread
...
| `strategy_beta` | -0.070133022445 | fixed | DEV-fitted structural hedge coefficient |
...
| Typical hold time | multi-day; scan half-life 95.663 D1 bars |
| Expected drawdown profile | high and regime-sensitive; conservative 30% prior |
| Regime preference | USDCHF/GBPJPY fixed-residual mean reversion |
| Win rate target | unknown; Q02 is the first platform-economic judge |

The fixed scan recorded DEV net Sharpe `-0.078836`, OOS net Sharpe `-0.429689`,
and OOS return `-4.294957%`. These are adverse priors, not a promotion claim.
...
Chan supplies the method but makes no USDCHF/GBPJPY performance claim.
...
`1.0` and `0.070133022445`; each leg is bounded by its ATR stop. If the GBPJPY
...
|---|---|---|---|
| v1 | 2026-08-06 | Initial approved-card build | Next-rank-59 non-duplicate fixed-scan FX basket |
