...
#property description "QM5_20300 WTI Self-Relative Low-MAX Regime"
...
// -----------------------------------------------------------------------------
// Source lineage: Hollstein, Prokopczuk & Tharann (2021) define MAX as the
// arithmetic mean of the five largest prior-year daily commodity returns,
// document a negative relation only in their post-financialization subsample,
// use monthly sorts, and include WTI. Full-sample and two-portfolio results are
// null. The self-relative WTI comparison below is a locked QM translation, not
// a source-tested WTI rule. At each genuine broker-month transition:
//   1. load exactly 505 completed D1 closes;
//   2. form two disjoint blocks of exactly 252 simple returns;
//   3. sort each block and average exactly its five largest observations;
//   4. buy lower recent MAX, sell higher recent MAX, and consume a tie flat.
// Runtime uses native XTIUSD.DWX price and broker data only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 20300;
...
input double strategy_max_tolerance           = 1.0e-12;
...
double g_cache_recent_max       = 0.0;
double g_cache_preceding_max    = 0.0;
...
void Strategy_ResetCachedState()
  {
   ...
   g_cache_signal = 0;
   g_cache_recent_max = 0.0;
   g_cache_preceding_max = 0.0;
...
bool Strategy_ComputeMaxBlock(MqlRates &rates[],
...
bool Strategy_LoadSignalState(const datetime decision_bar_time,
                              int &signal)
  {
   ...
   if(decision_bar_time <= 0 ||
      ...
      MathAbs(strategy_max_tolerance - 1.0e-12) > 1.0e-22)
     {
      g_cache_state_reason = "bad_max_contract";
      return false;
     }
   ...
   if(!Strategy_ComputeMaxBlock(rates,
                                0,
                                g_cache_recent_max,
                                g_cache_recent_observations))
     {
      g_cache_state_reason = "invalid_recent_max";
      return false;
     }
   if(!Strategy_ComputeMaxBlock(rates,
                                strategy_prior_block_offset,
                                g_cache_preceding_max,
                                g_cache_preceding_observations))
     {
      g_cache_state_reason = "invalid_preceding_max";
      return false;
     }

   g_cache_max_difference = g_cache_recent_max - g_cache_preceding_max;
   ...
   if(g_cache_recent_observations != strategy_returns_per_block ||
      ...
      !MathIsValidNumber(g_cache_max_difference))
     {
      g_cache_state_reason = "invalid_max_difference";
      return false;
     }

   // Source post-financialization high-minus-low MAX returns are negative:
   // buy the lower recent state and sell the higher recent state.
   if(g_cache_max_difference < -strategy_max_tolerance)
      signal = 1;
   else if(g_cache_max_difference > strategy_max_tolerance)
      signal = -1;
   else
     {
      g_cache_state_reason = "max_tie";
      return true;
     }
...
void Strategy_PrepareMonthlySignal()
  {
   ...
   QM_LogEvent(QM_INFO,
               "MONTHLY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"valid\":%s,\"signal\":%d,\"recent_max\":%.12e,\"preceding_max\":%.12e,\"difference\":%.12e,\"recent_observations\":%d,\"preceding_observations\":%d,\"closes\":%d,\"state\":\"%s\"}",
                             g_cache_month_key,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 20300 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20300_WTI_MAX_REGIME";
   ...
   req.reason = (g_cache_signal > 0) ? "MAX_REGIME_XTI_LONG" : "MAX_REGIME_XTI_SHORT";
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_20300_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_20300\",\"ea\":\"wti-max-regime\",\"stat\":\"two_disjoint_252_return_top5_max_blocks\"}");
