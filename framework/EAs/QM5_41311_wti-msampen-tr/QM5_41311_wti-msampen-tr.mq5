...
#property description "QM5_41311 WTI Monthly Sample Entropy Trend"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41311 - WTI Monthly Sample Entropy Trend
// -----------------------------------------------------------------------------
// D1 structural crude-oil sleeve:
//   - reconstruct sixty-one consecutive completed broker-month-end closes
//   - form sixty adjacent monthly log returns
//   - compute m=2, lag-one SampEn with strict r=0.2*sample-sd matches
//   - follow twelve-month WTI momentum only when SampEn <= 2.5
...
bool Strategy_SampleEntropySignal(const double &closes[],
                                        ...
                                        int &direction)
  {
   ...
   const bool valid =
      Strategy_LoadMonthlyEndpoints(current_month_key,
                                    ...
                                    closes,
                                    endpoint_times) &&
      Strategy_SampleEntropySignal(closes, metrics);
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
   ...
   else
     {
      ...
      g_signal_valid =
         endpoints_valid &&
         Strategy_SampleEntropySignal(closes, g_signal_metrics);
      if(!g_signal_valid)
         g_signal_state = "sample_entropy_validation_failed";
         ...
         g_signal_state = "low_sample_entropy_positive_momentum_wti_long";
      else if(g_signal_metrics.direction < 0)
         g_signal_state = "low_sample_entropy_negative_momentum_wti_short";
      else
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"endpoint_count\":%d,\"return_count\":%d,\"sample_mean\":%.12f,\"sample_sd\":%.12f,\"radius\":%.12f,\"matches_m\":%d,\"matches_m_plus_one\":%d,\"sample_entropy\":%.12f,\"entropy_qualified\":%s,\"momentum_12\":%.12f,\"returns\":\"%s\",\"oldest_close\":%.10f,\"newest_close\":%.10f,\"oldest_endpoint\":%I64d,\"newest_endpoint\":%I64d,\"state\":\"%s\"}",
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41311 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41311_WTI_MSAMPEN";
   ...
   req.reason = (g_signal_metrics.direction > 0)
      ? "WTI_LOW_SAMPEN_MOM_LONG"
      : "WTI_LOW_SAMPEN_MOM_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41311 ||
      qm_magic_slot_offset != 0)
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI monthly low-sample-entropy trend holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41311_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   g_entry_month_state_key =
      StringFormat("QM5_41311_ENTRY_MONTH_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41311\",\"ea\":\"wti-msampen-tr\"}");
