...
#property description "QM5_11873 trix-signal-line-cross — TRIX(14)/signal-line cross (H1 FX)"

#include <QM/QM_Common.mqh>

// =============================================================================
// QuantMechanica V5 EA — QM5_11873 trix-signal-line-cross
// -----------------------------------------------------------------------------
// Source: Unknown author, "Trix Strategy Trading System" (signal-line cross),
//   forexstrategiesresources.com, 2013 (source_id ee502396-b08f-5cc0-a793-
//   ca869b7b75e2; R1 PASS single-source lineage, R2/R3/R4 PASS). Card:
//   artifacts/cards_approved/QM5_11873_trix-signal-line-cross.md
//   (g0_status APPROVED).
//
...
//                  (The card narrates a *100 percent ROC; the scaling factor is
//                  cosmetic — TRIX and its signal are scaled identically, so the
//                  cross bars are unchanged. The proven *1e4 form is kept.)
...
//                  (TRIX[2] >= Signal[2] AND TRIX[1] < Signal[1]).
//   Stop         : 2 x ATR(14)            (card: sl_atr_multiplier 2.0).
//   Take profit  : 4 x ATR(14) = 2 x SL  => 2:1 RR (card: tp 4xATR, RR 2.0).
//   Exit         : SL / TP only; opposite cross also closes (manual flatten).
//
...
// TRIX/signal-line cross entry. Caller guarantees QM_IsNewBar() == true.
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   // ONE trigger EVENT: the TRIX/signal cross. Long and short are mutually
   ...
   if(cross_up)
     {
      ...
      req.sl     = sl;
      req.tp     = tp;
      req.reason = "trix_signal_cross_long";
      return true;
     }
   ...
   req.sl     = sl_s;
   req.tp     = tp_s;
   req.reason = "trix_signal_cross_short";
   return true;
  }
...
// Opposite TRIX/signal cross flattens the open position (manual exit).
