# QM5_12732_edgelab-eurusd-gbpusd-cointegration - Strategy Spec
...
The EA trades the EURUSD.DWX and GBPUSD.DWX D1 cointegration spread. It computes
`S = ln(EURUSD) - beta * ln(GBPUSD)` with beta defaulting to 0.91, then
...
same script ranked EURUSD/GBPUSD as the strongest remaining unbuilt positive
DEV/OOS candidate after the existing 12532/12533/12624/12712/12723/12728/12731 baskets, with
DEV Sharpe 0.65, OOS net Sharpe 0.54, OOS return +2.81%, 20 OOS state changes,
beta 0.91, and 49-day half-life.
...
| strategy_beta | 0.91 | >0 | Hedge coefficient in `ln(EURUSD) - beta * ln(GBPUSD)` |
...
**Designed for:**
- EURUSD.DWX - leg 1 of the fixed EURUSD/GBPUSD spread and the spread numerator.
- GBPUSD.DWX - leg 2 of the fixed EURUSD/GBPUSD spread and the beta-weighted spread denominator.

**Explicitly not for:**
- Other `.DWX` symbols. This card is a fixed two-leg FX-cross basket, not a portable multi-pair strategy.

The EA selects EURUSD.DWX and GBPUSD.DWX only; both legs have direct USD
...
| Regime preference | mean-reverting EUR-vs-GBP USD major-pair relative-value spread |
