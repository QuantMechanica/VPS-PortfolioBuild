...
#property description "QM5_12732 Edge Lab EURUSD GBPUSD Cointegration"
...
input double strategy_beta              = 0.91;
...
bool Strategy_IsHostSymbol()
  {
   return (_Symbol == g_leg_eurusd || _Symbol == g_leg_gbpusd);
  }
...
bool Strategy_EnsureBasketScope()
  {
   ...
   string allowed[2] = {"EURUSD.DWX", "GBPUSD.DWX"};
   for(int i = 0; i < 2; ++i)
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(!Strategy_EnsureBasketScope())
      return false;
   if(!QM_SymbolAssertOrLog(g_leg_eurusd) || !QM_SymbolAssertOrLog(g_leg_gbpusd))
      return false;
   ...
   for(int i = 0; i < lookback; ++i)
     {
      if(eurusd[i] <= 0.0 || gbpusd[i] <= 0.0)
         return false;
      spreads[i] = MathLog(eurusd[i]) - strategy_beta * MathLog(gbpusd[i]);
...
bool Strategy_OpenPair(const int spread_direction)
  {
   ...
   const double gbpusd_weight = MathAbs(strategy_beta);
   const double weight_sum = eurusd_weight + gbpusd_weight;
   if(weight_sum <= 0.0)
      return false;

   const bool long_spread = (spread_direction > 0);
   const QM_OrderType eurusd_type = long_spread ? QM_BUY : QM_SELL;
   const QM_OrderType gbpusd_type = long_spread ? QM_SELL : QM_BUY;
   const string reason = long_spread ? "QM5_12732_LONG_SPREAD_Z_LT_NEG_ENTRY"
                                     : "QM5_12732_SHORT_SPREAD_Z_GT_POS_ENTRY";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12732_PAIR_HOST";
   ...
   if(qm_news_temporal != QM_NEWS_TEMPORAL_OFF || qm_news_compliance != QM_NEWS_COMPLIANCE_NONE)
     {
      if(!QM_NewsAllowsTrade2(g_leg_eurusd, broker_time, qm_news_temporal, qm_news_compliance))
         return true;
      if(!QM_NewsAllowsTrade2(g_leg_gbpusd, broker_time, qm_news_temporal, qm_news_compliance))
         return true;
     }
   else
     {
      if(!QM_NewsAllowsTrade(g_leg_eurusd, broker_time, qm_news_mode_legacy))
         return true;
      if(!QM_NewsAllowsTrade(g_leg_gbpusd, broker_time, qm_news_mode_legacy))
         return true;
     }
   return false;
  }
