...
#property description "QM5_41436 WTI Hurricane-Season NR2 Sign Continuation"
...
//   - follow the newest completed week's own open-to-close direction
...
bool Strategy_LoadNR2SignSignal(const int current_week_key,
                                const int label_offset,
                                int &direction)
  {
   direction = 0;
   ...
   if(g_strict_range_contraction)
     {
      if(g_newer_week_close > g_newer_week_open)
         direction = 1;
      else if(g_newer_week_close < g_newer_week_open)
         direction = -1;
     }
   return true;
  }
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_WeekAlreadyEntered(g_decision_week_key))
      ...
      g_signal_state = "late_hurricane_consumed_flat";
   else if(!Strategy_IsHurricaneMonth(anchor_month))
      g_signal_state = "outside_hurricane_months";
   else
     {
      g_signal_valid =
         Strategy_LoadNR2SignSignal(g_decision_week_key,
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"week\":%d,\"anchor_month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_week_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"week_bars\":[%d,%d],\"week_ranges\":[%.8f,%.8f],\"new_open\":%.8f,\"new_high\":%.8f,\"new_low\":%.8f,\"new_close\":%.8f,\"strict_range_contraction\":%s,\"state\":\"%s\"}",
                            g_decision_week_key,
                            anchor_month,
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41436_WTI_HURR_NR2_SIGN_CONT";
   ...
   req.reason = (g_signal_direction > 0 ?
                 "WTI_HURR_NR2_SIGN_CONT_LONG" :
                 "WTI_HURR_NR2_SIGN_CONT_SHORT");
...
int OnInit()
  {
   if(!SymbolSelect(strategy_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41436 ||
      qm_magic_slot_offset != 0)
   ...
   g_attempt_state_key =
      StringFormat("QM5_41436_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41436\",\"ea\":\"wti-hurr-nr2-sign-cont\"}");
