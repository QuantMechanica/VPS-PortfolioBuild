"""Deterministic reference checks for QM5_41436 WTI hurricane NR2 sign continuation."""
...
def package(anchor: date, count: int, high: float, low: float, final_close: float,
    ...
    rows = [Bar(anchor + timedelta(days=i), first_open if i == 0 else 100.0,
                high, low, (high + low) / 2) for i in range(count)]
    rows[-1] = Bar(rows[-1].session, rows[-1].open, high, low, final_close)
    return list(reversed(rows))


def signal(current_anchor: date, newest_first: list[Bar]) -> tuple[int, tuple[float, float]]:
    ...
    ranges = tuple(max(bar.high for bar in rows) - min(bar.low for bar in rows)
                   for rows in packs)
    newest_open = packs[0][-1].open
    newest_close = packs[0][0].close
    direction = 0
    if ranges[0] < ranges[1]:
        direction = 1 if newest_close > newest_open else -1 if newest_close < newest_open else 0
    return direction, ranges


class WtiHurricaneNR2SignContinuationReferenceTest(unittest.TestCase):
    def setUp(self) -> None:
        self.current = date(2026, 9, 14)
        self.new = self.current - timedelta(days=7)
        self.old = self.current - timedelta(days=14)

    def bars(self, new_range: float = 10, old_range: float = 20,
             new_open: float = 94, new_close: float = 98) -> list[Bar]:
        newest = package(self.new, 5, 100, 100 - new_range, new_close, new_open)
        older = package(self.old, 5, 100, 100 - old_range, 90, 90)
        return newest + older

    def test_contraction_positive_body_is_long(self) -> None:
        self.assertEqual(signal(self.current, self.bars())[0], 1)

    def test_contraction_negative_body_is_short(self) -> None:
        self.assertEqual(signal(self.current, self.bars(new_open=98, new_close=94))[0], -1)

    def test_range_tie_is_flat(self) -> None:
        self.assertEqual(signal(self.current, self.bars(new_range=20, old_range=20))[0], 0)

    def test_expansion_is_flat(self) -> None:
        self.assertEqual(signal(self.current, self.bars(new_range=20, old_range=10))[0], 0)

    def test_zero_body_is_flat(self) -> None:
        self.assertEqual(signal(self.current, self.bars(new_open=96, new_close=96))[0], 0)

    def test_earliest_open_controls_direction(self) -> None:
        rows = self.bars(new_open=94, new_close=98)
        rows[0] = Bar(rows[0].session, 99, rows[0].high, rows[0].low, rows[0].close)
        self.assertEqual(signal(self.current, rows)[0], 1)
    ...
    def test_three_to_five_sessions_are_accepted(self) -> None:
        for count in (3, 4, 5):
            newest = package(self.new, count, 100, 90, 98, 94)
            older = package(self.old, count, 100, 80, 90, 90)
    ...
    def test_two_sessions_are_rejected(self) -> None:
        with self.assertRaisesRegex(ValueError, "session count"):
            signal(self.current, package(self.new, 2, 100, 90, 98, 94) +
                   package(self.old, 5, 100, 80, 90, 90))

    def test_missing_preceding_week_is_rejected(self) -> None:
        with self.assertRaisesRegex(ValueError, "session count"):
            signal(self.current, package(self.new, 5, 100, 90, 98, 94))

    def test_duplicate_session_is_rejected(self) -> None:
        rows = self.bars()
        rows[1] = Bar(rows[0].session, rows[1].open, rows[1].high, rows[1].low, rows[1].close)
        with self.assertRaisesRegex(ValueError, "duplicate session"):
            signal(self.current, rows)
