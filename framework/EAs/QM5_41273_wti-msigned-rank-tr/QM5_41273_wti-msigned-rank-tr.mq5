...
#property description "QM5_41273 WTI Monthly Signed-Rank Trend"
...
//   - derive twelve adjacent monthly log returns
//   - require nonzero returns and pairwise-distinct absolute magnitudes
//   - rank absolute returns 1..12 and sum ranks carried by positive returns
//   - continue only when the centered signed-rank score reaches +/-18
...
input int    strategy_total_rank_sum          = 78;
input int    strategy_score_abs_min           = 18;
input double strategy_zero_epsilon            = 0.000000000001;
...
struct Strategy_SignalMetrics
  {
   ...
   int signed_rank_score;
   int positive_return_count;
   bool zero_return;
   bool absolute_tie;
   string return_path;
   string absolute_rank_path;
   double oldest_close;
   double newest_close;
  };
   ...
   for(int mask = 0; mask < 4096; ++mask)
     {
      ...
      for(int bit = 0; bit < 12; ++bit)
        {
         if((mask & (1 << bit)) != 0)
            positive_rank_sum += bit + 1;
        }
      const int score =
         2 * positive_rank_sum - strategy_total_rank_sum;
      if(score >= strategy_score_abs_min)
         ++long_count;
      else if(score <= -strategy_score_abs_min)
         ++short_count;
     }
   return (long_count == 1062 && short_count == 1062 &&
           long_count + short_count == 2124);
  }

bool Strategy_SignedRankSignal(const double &closes[],
                               Strategy_SignalMetrics &metrics)
  {
   ZeroMemory(metrics);
   metrics.return_path = "";
   metrics.absolute_rank_path = "";
   ...
   if(strategy_endpoint_count != 13 ||
      ...
      strategy_total_rank_sum != 78 ||
      strategy_score_abs_min != 18 ||
      MathAbs(strategy_zero_epsilon - 0.000000000001) >
         0.000000000000000001 ||
      metrics.endpoint_count != strategy_endpoint_count ||
      strategy_endpoint_count != strategy_return_count + 1 ||
   ...
   for(int index = 0; index < strategy_return_count; ++index)
     {
      ...
      if(!MathIsValidNumber(value))
         return false;
      returns[index] = value;
      absolute_returns[index] = MathAbs(value);
      if(!MathIsValidNumber(absolute_returns[index]))
         return false;
      if(StringLen(metrics.return_path) > 0)
         metrics.return_path += ",";
      metrics.return_path += DoubleToString(value, 12);
      if(absolute_returns[index] <= strategy_zero_epsilon)
         metrics.zero_return = true;
     }
   ...
   for(int left = 0; left < strategy_return_count - 1; ++left)
     {
      for(int right = left + 1;
          ...
          ++right)
        {
         if(MathAbs(absolute_returns[left] - absolute_returns[right]) <=
            strategy_zero_epsilon)
           {
            metrics.absolute_tie = true;
            return true;
           }
        }
     }
   ...
   for(int index = 0; index < strategy_return_count; ++index)
     {
      ranks[index] = 1;
      for(int other = 0; other < strategy_return_count; ++other)
        {
         if(absolute_returns[other] < absolute_returns[index])
            ++ranks[index];
        }
      if(ranks[index] < 1 || ranks[index] > strategy_return_count)
         return false;
      for(int prior = 0; prior < index; ++prior)
        {
         if(ranks[prior] == ranks[index])
            return false;
        }
      metrics.total_rank_sum += ranks[index];
      if(returns[index] > strategy_zero_epsilon)
        {
         metrics.positive_rank_sum += ranks[index];
         ++metrics.positive_return_count;
        }
      else if(returns[index] >= -strategy_zero_epsilon)
         return false;
      if(StringLen(metrics.absolute_rank_path) > 0)
         metrics.absolute_rank_path += ",";
      metrics.absolute_rank_path += IntegerToString(ranks[index]);
     }

   if(metrics.total_rank_sum != strategy_total_rank_sum ||
      metrics.positive_rank_sum < 0 ||
      metrics.positive_rank_sum > strategy_total_rank_sum ||
      metrics.positive_return_count < 0 ||
      metrics.positive_return_count > strategy_return_count ||
      StringLen(metrics.absolute_rank_path) <= 0)
      return false;

   metrics.signed_rank_score =
      2 * metrics.positive_rank_sum - metrics.total_rank_sum;
   if(metrics.signed_rank_score < -strategy_total_rank_sum ||
      metrics.signed_rank_score > strategy_total_rank_sum ||
      ((metrics.signed_rank_score + strategy_total_rank_sum) % 2) != 0)
      return false;

   if(metrics.signed_rank_score >= strategy_score_abs_min)
      metrics.direction = 1;
   else if(metrics.signed_rank_score <= -strategy_score_abs_min)
      metrics.direction = -1;
   return true;
  }
                                        ...
                                        int &direction)
  {
   ...
   const bool valid =
      Strategy_LoadMonthlyEndpoints(current_month_key,
                                    ...
                                    closes,
                                    endpoint_times) &&
      Strategy_SignedRankSignal(closes, metrics);
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
   ...
   else
     {
      ...
      g_signal_valid =
         endpoints_valid &&
         Strategy_SignedRankSignal(closes, g_signal_metrics);
      if(!g_signal_valid)
         g_signal_state = "signed_rank_validation_failed";
      else if(g_signal_metrics.direction == 0)
        {
         if(g_signal_metrics.zero_return)
            g_signal_state = "signed_rank_zero_return_flat";
         else if(g_signal_metrics.absolute_tie)
            g_signal_state = "signed_rank_absolute_tie_flat";
         else
            g_signal_state = "signed_rank_score_flat";
        }
      else if(g_signal_metrics.direction > 0)
         g_signal_state = "signed_rank_wti_long";
      else if(g_signal_metrics.direction < 0)
         g_signal_state = "signed_rank_wti_short";
      else
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"endpoint_count\":%d,\"return_count\":%d,\"total_rank_sum\":%d,\"positive_rank_sum\":%d,\"signed_rank_score\":%d,\"positive_return_count\":%d,\"zero_return\":%s,\"absolute_tie\":%s,\"returns\":\"%s\",\"absolute_ranks\":\"%s\",\"oldest_close\":%.10f,\"newest_close\":%.10f,\"oldest_endpoint\":%I64d,\"newest_endpoint\":%I64d,\"state\":\"%s\"}",
                            ...
                            g_signal_metrics.total_rank_sum,
                            g_signal_metrics.positive_rank_sum,
                            g_signal_metrics.signed_rank_score,
                            g_signal_metrics.positive_return_count,
                            g_signal_metrics.zero_return ? "true" : "false",
                            g_signal_metrics.absolute_tie ? "true" : "false",
                            g_signal_metrics.return_path,
                            g_signal_metrics.absolute_rank_path,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41273 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41273_WTI_SIGNED_RANK_TR";
   ...
   req.reason = (g_signal_metrics.direction > 0)
      ? "WTI_SIGNED_RANK_TR_LONG"
      : "WTI_SIGNED_RANK_TR_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41273 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI monthly signed-rank trend holds through Fridays until the next broker month"))
     {
      QM_FrameworkShutdown();
      return INIT_FAILED;
     }
   g_support_contract_valid = Strategy_SignedRankSupportValid();
   ...
   g_attempt_state_key =
      StringFormat("QM5_41273_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41273\",\"ea\":\"wti-msigned-rank-tr\"}");
