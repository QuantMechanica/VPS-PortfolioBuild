...
input double strategy_max_stop_pips      = 80.0;
input double strategy_spread_cap_pips    = 25.0;
input int    strategy_pending_expiry_days = 7;
   ...
   if(!g_pp_active)
     {
      ...
      perm.valid = true;
      perm.reference_bar_time = iTime(_Symbol, QM_PPC_REFERENCE_TF, QM_PPC_CLOSED_SHIFT); // perf-allowed: one closed D1 reference stamp per entry candidate.
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.expiration_seconds = strategy_pending_expiry_days * 86400;

   const int magic = QM_FrameworkMagic();
   if(magic <= 0 || strategy_ma_period < 2 || strategy_tp_r_multiple <= 0.0 ||
      strategy_entry_offset_pips <= 0.0 || strategy_max_stop_pips <= 0.0)
      return false;
   ...
   for(int i = OrdersTotal() - 1; i >= 0; --i)
     {
      ...
      if(order_type == ORDER_TYPE_BUY_STOP || order_type == ORDER_TYPE_SELL_STOP)
         QM_TM_RemovePendingOrder(order_ticket, "new_d1_setup_check");
     }

   MqlRates bars[];
   ArraySetAsSeries(bars, true);
   const int copied = CopyRates(_Symbol, PERIOD_D1, 1, 3, bars); // perf-allowed: fixed three closed D1 bars for outside/inside-bar structure; EntrySignal is framework new-bar gated.
   if(copied != 3)
      return false;

   const MqlRates bar1 = bars[0];
   const MqlRates bar2 = bars[1];
   const MqlRates bar3 = bars[2];
   if(bar1.high <= 0.0 || bar1.low <= 0.0 || bar2.high <= 0.0 || bar2.low <= 0.0 ||
      bar3.high <= 0.0 || bar3.low <= 0.0)
      return false;

   const double ma1 = QM_SMA(_Symbol, PERIOD_D1, strategy_ma_period, 1, PRICE_CLOSE);
   const double ma2 = QM_SMA(_Symbol, PERIOD_D1, strategy_ma_period, 2, PRICE_CLOSE);
   if(ma1 <= 0.0 || ma2 <= 0.0)
      return false;

   const bool bar1_inside = (bar1.high < bar2.high && bar1.low > bar2.low);
   const bool bar2_inside = (bar2.high < bar3.high && bar2.low > bar3.low);
   if(bar1_inside || bar2_inside)
      return false;
   ...
   if(long_setup)
     {
      const double entry = MathMax(bar1.high, bar2.high) + offset;
      double sl = MathMin(bar1.low, bar2.low) - offset;
      if((entry - sl) > max_stop)
         sl = entry - max_stop;
      const double risk = entry - sl;
      if(entry <= ask + min_stop_dist || risk <= min_stop_dist)
         return false;

      req.type = QM_BUY_STOP;
      req.price = NormalizeDouble(entry, _Digits);
      req.sl = NormalizeDouble(sl, _Digits);
      req.tp = NormalizeDouble(entry + (risk * strategy_tp_r_multiple), _Digits);
      req.reason = "QM5_11422_LONG_STOP";
      if(!Pattern_AllowsRequest(req))
         return false;
      return true;
     }
   ...
   if(short_setup)
     {
      const double entry = MathMin(bar1.low, bar2.low) - offset;
      double sl = MathMax(bar1.high, bar2.high) + offset;
      if((sl - entry) > max_stop)
         sl = entry + max_stop;
      const double risk = sl - entry;
      if(entry >= bid - min_stop_dist || risk <= min_stop_dist)
         return false;
      ...
      req.sl = NormalizeDouble(sl, _Digits);
      req.tp = NormalizeDouble(entry - (risk * strategy_tp_r_multiple), _Digits);
      req.reason = "QM5_11422_SHORT_STOP";
      if(!Pattern_AllowsRequest(req))
         return false;
      return true;
     }
...
void Strategy_ManageOpenPosition()
  {
   // P2 default from the card exits by the fixed 2R TP/SL bracket.
  }
...
void OnTick()
  {
   ...
   if(Strategy_NewsFilterHook(broker_now))
      return;
   // FW1 — entry-only news gate. Management and exits above keep running
