# QM5_41401_xng-shoulder-w2fade - Strategy Spec

**EA ID:** QM5_41401  
**Slug:** `xng-shoulder-w2fade`  
**Strategy ID:** `EIA-MOP-XNG-SHOULDER-W2FADE-20260909_S01`  
**Source:** `EIA-MOP-XNG-SHOULDER-W2FADE-20260909`  
**Last revised:** 2026-09-09

## 1. Strategy Logic

At the first tradable `XNGUSD.DWX` D1 bar of a normalized broker week whose
Monday anchor lies in April, May, September, or October, read only the two immediately
completed adjacent three-to-five-session weeks. Compute each
`ln(final_close / first_open)` and trade the inverse of the shared strict sign only when both
...
`414010000`. The symbol is an input; no executable symbol literal is embedded
...
week. Only April-May and September-October anchors may continue. Validate the two
immediately prior adjacent weeks, compute both open-to-close log returns, and
fade their sign only on strict agreement. Require no owned position or
...
late restart, disagreement, exact-zero return, bad history, or failed execution
gate is consumed flat and never retried. Expected cadence is approximately
6-10 completed packages per full year. The expected regime is shoulder-season
two-week exhaustion; expected hold is one normalized week.

## 6. Source Citation

The source-of-record packet is
`strategy-seeds/sources/EIA-MOP-XNG-SHOULDER-W2FADE-20260909/source.md`. It joins
...
two-week shoulder-fade conjunction, continuous-CFD efficacy, and portfolio decorrelation are
not source claims. R1 lineage and R2-R4 status are recorded in
`strategy-seeds/cards/approved/QM5_41401_xng-shoulder-w2fade_card.md`.
...
| entry | shoulder calendar, two completed-week packages, strict agreement, inverse side, durable attempt, spread and frozen ATR stop |
