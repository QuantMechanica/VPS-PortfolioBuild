...
#property description "QM5_41401 XNG Shoulder Two-Week Exhaustion Fade"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41401 - XNG Shoulder Two-Week Exhaustion Fade
// -----------------------------------------------------------------------------
// D1 structural natural-gas sleeve:
//   - decide only in April, May, September, and October
//   - aggregate the two immediately completed 3-to-5-session normalized weeks
//   - fade only strict same-sign two-week return exhaustion
...
input int    strategy_shoulder_month_1     = 4;
input int    strategy_shoulder_month_2     = 5;
input int    strategy_shoulder_month_3     = 9;
input int    strategy_shoulder_month_4     = 10;
...
bool Strategy_IsShoulderMonth(const int month)
  {
   return (month == strategy_shoulder_month_1 ||
           month == strategy_shoulder_month_2 ||
           month == strategy_shoulder_month_3 ||
           month == strategy_shoulder_month_4);
  }
...
bool Strategy_LoadTwoWeekAgreementSignal(const int current_week_key,
                                         ...
                                         int &direction)
  {
   ...
   // sign as the locked exhaustion fade. Disagreement and exact zero are
   // valid flat states rather than data failures.
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   else if(!Strategy_IsShoulderMonth(anchor_month))
      g_signal_state = "outside_shoulder_months";
   else
     {
      g_signal_valid =
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41401_XNG_SHOULDER_W2FADE";
   ...
   req.reason = (g_signal_direction > 0)
      ? "XNG_SHOULDER_W2_FADE_LONG"
      : "XNG_SHOULDER_W2_FADE_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(strategy_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41401 ||
      qm_magic_slot_offset != 0)
   ...
   g_attempt_state_key =
      StringFormat("QM5_41401_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41401\",\"ea\":\"xng-shoulder-w2fade\"}");
