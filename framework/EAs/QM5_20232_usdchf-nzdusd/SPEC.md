# QM5_20232_usdchf-nzdusd — Strategy Spec
...
`ln(USDCHF.DWX) - (-0.270458913) * ln(NZDUSD.DWX)`. Score the newest spread
...
| `strategy_beta` | -0.270458913 | fixed | DEV-fitted structural hedge coefficient |
...
Both symbols are traded and receive registered magic slots. No order-free
conversion symbol is declared.
...
| Bar gating | `QM_IsNewBar(USDCHF.DWX, PERIOD_D1)` once per signal bar |
...
| OOS basket state changes | 16 across 2023–2024 in the frozen scan |
| Typical hold time | multi-day; scan half-life 108.268 D1 bars |
| Expected drawdown profile | high and regime-sensitive; conservative 30% prior |
| Regime preference | USDCHF/NZDUSD fixed-residual mean reversion |
| Win rate target | unknown; Q02 is the first platform-economic judge |

The fixed scan recorded DEV net Sharpe `0.035539`, OOS net Sharpe `-0.387376`,
and OOS return `-3.267369%`. These are adverse priors, not a promotion claim.
...
Chan supplies the method but makes no USDCHF/NZDUSD performance claim.
...
`1.0` and `0.270458913`; each leg is bounded by its ATR stop. If the NZDUSD
allocation falls below broker minimum volume, the whole package is rejected.
...
|---|---|---|---|
| v1 | 2026-08-05 | Initial approved-card build | Next-rank-55 non-duplicate fixed-scan FX basket |
