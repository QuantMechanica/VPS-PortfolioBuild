---
strategy_id: AI-CODEX-FX-COINT66-20260609-USDCHF-NZDUSD
ea_id: QM5_20232
slug: usdchf-nzdusd
type: strategy
status: APPROVED
...
markets: [forex]
timeframes: [D1]
primary_target_symbols: [USDCHF.DWX, NZDUSD.DWX]
target_symbols: [USDCHF.DWX, NZDUSD.DWX]
logical_symbol: QM5_20232_USDCHF_NZDUSD_COINTEGRATION_D1
period: D1
expected_trade_frequency: "Approximately 4 completed two-leg packages per year per traded symbol, inferred from 16 OOS basket state changes across 2023-2024; Q02 must retire the sleeve if realized frequency is below the binding floor."
...
# QM5_20232 USDCHF/NZDUSD D1 Cointegration Basket
...
low-frequency daily implementation. Chan makes no claim for USDCHF/NZDUSD.
...
USDCHF/NZDUSD ranks fifty-fifth of 66 by OOS net Sharpe. Ranks 51 through 54
already have dedicated builds (`QM5_12776`, `QM5_12778`, `QM5_12781`, and
`QM5_12783`), and rank 56 has `QM5_12786`. The deterministic dedup check found
no exact collision across 4,289 registry rows and 405 direct cards. Its single
fuzzy sibling hit was manually resolved as a different exact pair. No basket
manifest declaring exactly the two traded symbols exists.
...
The negative OOS result and approximately 108-bar half-life are adverse
...
`QM5_1156` and `QM5_1257` can select the same instruments inside broad M30/H1
multi-pair universes, but use rolling selection, different formation logic,
different exits, and different logical baskets. This card is dedicated to the
frozen rank-55 D1 beta and residual. Other sibling fixed-scan sleeves use
different instrument pairs, coefficients, magics, and logical identities.

## 2. Concept

Trade temporary deviations in the fixed log-price residual
`ln(USDCHF) - (-0.270458913) * ln(NZDUSD)`. The negative fitted coefficient
...
package retains USD, CHF, NZD, carry, and broad risk-sentiment exposure.
...
`USDCHF.DWX` is the tester host and first traded leg. `NZDUSD.DWX` is the
companion traded leg. Both symbols receive orders and dedicated magic slots;
no conversion-only history is declared.

## 4. Entry Rules

- Load the newest closed D1 price for both traded symbols plus the preceding
  60 time-aligned closed observations.
- Compute `spread = ln(USDCHF) - strategy_beta * ln(NZDUSD)`, with frozen
  `strategy_beta = -0.270458913`.
...
- Require both traded symbols selected and their D1 history warm.
...
Chan supplies the pair-trading method but reports no USDCHF/NZDUSD result. The
...
```yaml
target_modules:
  no_trade: "Warm both traded histories; reject wrong host, timeframe, slot, alignment, or volume."
...
data_requirements: "USDCHF.DWX and NZDUSD.DWX D1 histories"
```
...
|---|---|---|---|
| G0 Research Intake | 2026-08-05 | APPROVED | `decisions/2026-08-05_usdchf_nzdusd_cointegration_g0.md` |
| Q01 Build Validation | 2026-08-05 | PASS | `D:\QM\reports\framework\21\build_check_20260805_221804.json` |
| Q02 Baseline Screening | - | NOT_ENQUEUED | - |

## 16. Lessons Captured

- 2026-08-05: Rank 55 is OOS-negative and below the expected Q02 cadence
...
- 2026-08-05: Same-instrument umbrella implementations are not substitutes for
  the frozen D1 residual because their timeframe, selection, refit, exit, and
  logical-basket contracts differ materially.
- 2026-08-05: Q01 strict compile and build validation passed with zero errors,
  zero failures, and zero warnings; basket and magic-resolver regressions also
  passed.
