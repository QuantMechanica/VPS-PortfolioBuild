...
#property description "QM5_20232 USDCHF NZDUSD Cointegration"
...
//
// Card:
//   strategy-seeds/cards/approved/QM5_20232_usdchf-nzdusd_card.md
//
// Fixed spread:
//   ln(USDCHF.DWX) - beta * ln(NZDUSD.DWX)
//
// A negative beta makes the leg directions sign-aware: a long spread buys
// both pairs and a short spread sells both pairs. The fitted companion weight
// is regression-neutral only; the card preserves that risk caveat.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 20232;
...
input double strategy_beta              = -0.270458913;
...
string   g_leg_host = "USDCHF.DWX";
string   g_leg_companion = "NZDUSD.DWX";
...
int Strategy_SlotForSymbol(const string symbol)
  {
   if(symbol == g_leg_host)
      return 0;
   if(symbol == g_leg_companion)
      return 1;
   return -1;
  }
...
bool Strategy_EnsureBasketScope()
  {
   ...
   string allowed[2] = {"USDCHF.DWX", "NZDUSD.DWX"};
   for(int i = 0; i < 2; ++i)
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(!Strategy_EnsureBasketScope())
      return false;
   if(!QM_SymbolAssertOrLog(g_leg_host) ||
      !QM_SymbolAssertOrLog(g_leg_companion))
      return false;

   double host_close[];
   double companion_close[];
   datetime host_time[];
   datetime companion_time[];
   ArraySetAsSeries(host_close, true);
   ArraySetAsSeries(companion_close, true);
   ArraySetAsSeries(host_time, true);
   ArraySetAsSeries(companion_time, true);

   if(CopyClose(g_leg_host, PERIOD_D1, 1, history_count, host_close) != // perf-allowed: gated D1 refresh.
      history_count)
      return false;
   if(CopyClose(g_leg_companion, PERIOD_D1, 1, history_count, companion_close) != // perf-allowed: gated D1 refresh.
      history_count)
      return false;
   if(CopyTime(g_leg_host, PERIOD_D1, 1, history_count, host_time) != // perf-allowed: gated D1 refresh.
      history_count)
      return false;
   if(CopyTime(g_leg_companion, PERIOD_D1, 1, history_count, companion_time) != // perf-allowed: gated D1 refresh.
      history_count)
      return false;
   ...
   for(int i = 0; i < history_count; ++i)
     {
      if(host_time[i] <= 0 || host_time[i] != companion_time[i])
         return false;
      if(host_close[i] <= 0.0 || companion_close[i] <= 0.0)
         return false;

      spreads[i] =
         MathLog(host_close[i]) -
         strategy_beta * MathLog(companion_close[i]);
...
bool Strategy_PreparePair(const int spread_direction,
                          QM_EntryRequest &host_req)
  {
   ...
   const double companion_weight = MathAbs(strategy_beta);
   const double weight_sum = host_weight + companion_weight;
   if(weight_sum <= 0.0)
      return false;
   ...
   const double host_lots =
      Strategy_LotsForLeg(g_leg_host, host_weight, host_weight);
   const double companion_lots =
      Strategy_LotsForLeg(g_leg_companion,
                          companion_weight,
                          host_weight);
   if(host_lots <= 0.0 || companion_lots <= 0.0)
      return false;

   const bool long_spread = (spread_direction > 0);
   const QM_OrderType host_type = long_spread ? QM_BUY : QM_SELL;
   const QM_OrderType companion_type =
      long_spread
      ? (strategy_beta >= 0.0 ? QM_SELL : QM_BUY)
      : (strategy_beta >= 0.0 ? QM_BUY : QM_SELL);
   const string reason =
      long_spread ? "QM5_20232_LONG_SPREAD_Z_LT_NEG_ENTRY"
                  : "QM5_20232_SHORT_SPREAD_Z_GT_POS_ENTRY";

   const double host_entry =
      QM_OrderTypeIsBuy(host_type)
      ? SymbolInfoDouble(g_leg_host, SYMBOL_ASK)
      : SymbolInfoDouble(g_leg_host, SYMBOL_BID);
   const double host_atr =
      QM_ATR(g_leg_host, PERIOD_D1, strategy_atr_period_d1, 1);
   if(host_entry <= 0.0 || host_atr <= 0.0)
      return false;

   host_req.type = host_type;
   host_req.price = 0.0;
   host_req.sl = QM_StopATRFromValue(g_leg_host,
                                     host_type,
                                     host_entry,
                                     host_atr,
                                     strategy_atr_sl_mult);
   ...
   g_companion_entry_type = companion_type;
   g_companion_entry_lots = companion_lots;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20232_PAIR_HOST";
...
bool Strategy_NewsFilterHook(const datetime broker_time)
  {
   if(qm_news_temporal != QM_NEWS_TEMPORAL_OFF ||
     ...
     {
      if(!QM_NewsAllowsTrade2(g_leg_host,
                              broker_time,
                              qm_news_temporal,
                              qm_news_compliance))
         return true;
      if(!QM_NewsAllowsTrade2(g_leg_companion,
                              broker_time,
                              qm_news_temporal,
                              qm_news_compliance))
         return true;
     ...
     {
      if(!QM_NewsAllowsTrade(g_leg_host,
                             broker_time,
                             qm_news_mode_legacy))
         return true;
      if(!QM_NewsAllowsTrade(g_leg_companion,
                             broker_time,
                             qm_news_mode_legacy))
         return true;
     }
...
int OnInit()
  {
   SymbolSelect(g_leg_host, true);
   SymbolSelect(g_leg_companion, true);
...
void OnTick()
  {
   ...
   const bool is_new_signal_bar =
      QM_IsNewBar(g_leg_host, PERIOD_D1);
      ...
      if(host_opened)
        {
         const bool companion_opened =
            ...
            Strategy_OpenLeg(g_leg_companion,
