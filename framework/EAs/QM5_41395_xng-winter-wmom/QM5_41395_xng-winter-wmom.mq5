...
#property description "QM5_41395 XNG Winter-Demand Weekly Momentum"
...
//   - follow ln(final close / first open), with no magnitude/volatility filter
...
bool Strategy_LoadWeeklyMomentumSignal(const int current_week_key,
                                       ...
                                       double &week_log_return)
  {
   ...
   // Continuation orientation is the identity: positive week -> long,
   // negative week -> short, exact zero -> flat.
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   else
     {
      g_signal_valid =
         Strategy_LoadWeeklyMomentumSignal(g_decision_week_key,
                                           ...
                                           g_week_log_return);
      if(!g_signal_valid)
         g_signal_state = "weekly_return_validation_failed";
      else if(g_signal_direction > 0)
         g_signal_state = "positive_week_momentum_long";
      else if(g_signal_direction < 0)
         g_signal_state = "negative_week_momentum_short";
      else
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"week\":%d,\"anchor_month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_week_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"week_bars\":%d,\"week_open\":%.8f,\"week_close\":%.8f,\"week_log_return\":%.12f,\"state\":\"%s\"}",
                            g_decision_week_key,
                            anchor_month,
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41395_XNG_WINTER_WMOM";
   ...
   req.reason = (g_signal_direction > 0)
      ? "XNG_WINTER_WEEK_MOM_LONG"
      : "XNG_WINTER_WEEK_MOM_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(strategy_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41395 ||
      qm_magic_slot_offset != 0)
   ...
   g_attempt_state_key =
      StringFormat("QM5_41395_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41395\",\"ea\":\"xng-winter-wmom\"}");
