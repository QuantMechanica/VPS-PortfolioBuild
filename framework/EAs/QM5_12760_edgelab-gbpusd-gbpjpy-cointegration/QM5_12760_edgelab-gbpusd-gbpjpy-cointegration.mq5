...
#property description "QM5_12760 Edge Lab GBPUSD GBPJPY Cointegration"
...
bool Strategy_IsHostSymbol()
  {
   return (_Symbol == g_leg_gbpusd || _Symbol == g_leg_gbpjpy);
  }
...
bool Strategy_EnsureBasketScope()
  {
   ...
   string allowed[3] = {"GBPUSD.DWX", "GBPJPY.DWX", "USDJPY.DWX"};
   for(int i = 0; i < 3; ++i)
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(!Strategy_EnsureBasketScope())
      return false;
   if(!QM_SymbolAssertOrLog(g_leg_gbpusd) || !QM_SymbolAssertOrLog(g_leg_gbpjpy))
      return false;
   ...
   for(int i = 0; i < lookback; ++i)
     {
      if(gbpusd[i] <= 0.0 || gbpjpy[i] <= 0.0)
         return false;
      spreads[i] = MathLog(gbpusd[i]) - strategy_beta * MathLog(gbpjpy[i]);
...
bool Strategy_OpenPair(const int spread_direction)
  {
   ...
   const double weight_sum = gbpusd_weight + gbpjpy_weight;
   if(weight_sum <= 0.0)
      return false;
   ...
   const string reason = long_spread ? "QM5_12760_LONG_SPREAD_Z_LT_NEG_ENTRY"
                                     : "QM5_12760_SHORT_SPREAD_Z_GT_POS_ENTRY";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12760_PAIR_HOST";
