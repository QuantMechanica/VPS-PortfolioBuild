"""Deterministic reference checks for QM5_41444 WTI winter WR2 body reversion."""
...
class WtiWinterWR2BodyReversionReferenceTest(unittest.TestCase):
    ...
    def test_range_contraction_is_flat(self) -> None:
        self.assertEqual(signal(self.current, *self.bars(96.0, 94.0, 8.0, 10.0)), 0)

    def test_close_location_is_irrelevant(self) -> None:
        self.assertEqual(signal(self.current, *self.bars(91.0, 92.0)), -1)
        self.assertEqual(signal(self.current, *self.bars(109.0, 108.0)), 1)
    ...
    def test_three_to_five_sessions_are_valid(self) -> None:
        for count in (3, 4, 5):
            ...
            older = package(self.old, count, 95, 100, 90, 95)
