...
#property description "QM5_41240 WTI Same-Calendar Five-Sample Ramsay-E Location"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41240 - WTI Same-Calendar Five-Sample Ramsay-E-Weighted Location
// -----------------------------------------------------------------------------
...
//   - freeze 1.4826*MAD and execute exactly 32 Ramsay-E derivative-weight updates
...
input double strategy_ramsay_a            = 0.3;
input int    strategy_ramsay_iterations   = 32;
...
// -----------------------------------------------------------------------------
// Exact completed-month endpoint and five-sample Ramsay-E helpers.
// -----------------------------------------------------------------------------
...
bool Strategy_RamsayESignal(const double &observations[],
                           ...
                           int &direction)
  {
   ...
   if(strategy_history_years != 5 ||
      ...
      MathAbs(strategy_ramsay_a - 0.3) > 1.0e-12 ||
      ...
      for(int index = 0; index < 5; ++index)
        {
         ...
         // Ramsay-E: psi(u)=u*exp(-a*abs(u)), so w(u)=exp(-a*abs(u)).
         const double absolute_u = MathAbs(u);
         const double exponent = -strategy_ramsay_a * absolute_u;
         if(absolute_u < 0.0 || exponent > 0.0 ||
            !MathIsValidNumber(absolute_u) ||
            !MathIsValidNumber(exponent))
            return false;
...
bool Strategy_LoadRamsayESignal(
   ...
   int &direction)
  {
   ...
   return Strategy_RamsayESignal(observations,
                                sample_count,
                                location_value,
                                direction);
  }
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   // Consume before history, Ramsay-E arithmetic, news, spread, quote,
   ...
   else
     {
      g_signal_valid =
          Strategy_LoadRamsayESignal(
            ...
            g_signal_direction);
      if(!g_signal_valid)
         g_signal_state = "endpoint_sample_or_ramsay_failed";
      else if(g_signal_direction > 0)
         g_signal_state = "fixed_ramsay_long";
      else if(g_signal_direction < 0)
         g_signal_state = "fixed_ramsay_short";
      else
         g_signal_state = "fixed_ramsay_epsilon_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"valid\":%s,\"samples\":%d,\"fixed_ramsay\":%.12g,\"direction\":%d,\"state\":\"%s\"}",
                            g_decision_month_key,
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(!QM_InputRequireLong("qm_ea_id", qm_ea_id, 41240) ||
      ...
      !QM_InputRequireLong("strategy_ramsay_iterations", strategy_ramsay_iterations, 32) ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "WTI_SAMECAL_RAMSAYE5";
   ...
   if(g_signal_direction > 0)
     {
      req.type = QM_BUY;
      req.reason = "WTI_SAMECAL_RAMSAYE5_LONG";
     }
   else
     {
      req.type = QM_SELL;
      req.reason = "WTI_SAMECAL_RAMSAYE5_SHORT";
     }
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI same-calendar five-sample Ramsay-E-location card holds to the next broker month"))
      return INIT_FAILED;
   ...
   g_attempt_state_key =
      StringFormat("QM5_41240_SAMECAL_RAMSAYE5_MONTH_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41240\",\"ea\":\"wti-samecal-ramsaye5\"}");
