...
r2_reasoning: "Fixed month-end calendar, exact 14:00 London trigger, index-return-sign direction rule, ATR stop and fixed flatten time are fully deterministic."
r3_reasoning: "GBPUSD.DWX is a registered DWX carrier with M15 history testable on T1-T10."
...
- Target symbol: `GBPUSD.DWX` (registered DWX carrier).
...
1. Eligible dates are every last local business day, using a Europe/London holiday and DST-aware calendar.
2. At 14:00 London, compute completed month-to-date return of UK100.DWX from its prior-month final close to its latest completed M15 close.
3. If the return is positive, SELL GBPUSD.DWX; if negative, BUY GBPUSD.DWX; exact zero consumes the date flat.
