...
#property description "QM5_1209 Carver Within-Asset Mean Reversion"

#include <QM/QM_Common.mqh>

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 1209;
...
input int    strategy_horizon_d1          = 160;
input int    strategy_ema_span            = 40;
...
#define QM5_1209_SYMBOL_COUNT 9
#define QM5_1209_MAX_GROUP    6
#define QM5_1209_MAX_SPAN     128
#define QM5_1209_BLOCK_COUNT  9

string g_symbols[QM5_1209_SYMBOL_COUNT] =
  {
   "GER40.DWX", "NDX.DWX", "WS30.DWX",
   "EURUSD.DWX", "GBPUSD.DWX", "AUDUSD.DWX", "USDJPY.DWX", "USDCHF.DWX", "USDCAD.DWX"
  };

int g_groups[QM5_1209_SYMBOL_COUNT] =
  {
   0, 0, 0,
   1, 1, 1, 1, 1, 1
  };

int g_block_until_bar[QM5_1209_BLOCK_COUNT];
int g_last_entry_key = 0;
int g_last_exit_key  = 0;

int Strategy_CurrentSymbolIndex()
  {
   for(int i = 0; i < QM5_1209_SYMBOL_COUNT; ++i)
...
bool Strategy_GroupDValues(const int group_id, const int shift, string &members[], const int member_count, double &d_values[])
  {
   if(member_count < 3)
      return false;
   double moves[QM5_1209_MAX_GROUP];
   for(int i = 0; i < member_count; ++i)
     {
      if(Bars(members[i], PERIOD_D1) < strategy_horizon_d1 + strategy_ema_span + strategy_norm_stddev_days + 10)
         return false;
      moves[i] = Strategy_NormalizedMove(members[i], shift);
     }
   double tmp[QM5_1209_MAX_GROUP];
   for(int i = 0; i < member_count; ++i)
      tmp[i] = moves[i];
   const double median_move = Strategy_Median(tmp, member_count);
   for(int i = 0; i < member_count; ++i)
      d_values[i] = moves[i] - median_move;
   return true;
  }
...
bool Strategy_ForecastsForGroup(const int group_id, string &members[], int &member_count, double &forecasts[])
  {
   ...
   const int span = MathMax(2, MathMin(strategy_ema_span, QM5_1209_MAX_SPAN));
   const double alpha = 2.0 / ((double)span + 1.0);

   double ema[QM5_1209_MAX_GROUP];
   for(int i = 0; i < member_count; ++i)
      ema[i] = 0.0;
   ...
   for(int shift = span; shift >= 1; --shift)
     {
      double d_values[QM5_1209_MAX_GROUP];
   ...
   for(int i = 0; i < member_count; ++i)
      forecasts[i] = MathMax(-20.0, MathMin(20.0, raw[i] / scale * 10.0));
   return true;
  }

bool Strategy_IsTopForecast(const string symbol, string &members[], const int member_count, double &forecasts[])
  {
   int own = -1;
   for(int i = 0; i < member_count; ++i)
      if(members[i] == symbol)
   ...
   int rank = 1;
   for(int i = 0; i < member_count; ++i)
      if(i != own && MathAbs(forecasts[i]) > MathAbs(forecasts[own]))
         ++rank;
...
int Strategy_CurrentForecast(double &forecast)
  {
   ...
   string members[QM5_1209_MAX_GROUP];
   double forecasts[QM5_1209_MAX_GROUP];
   int member_count = 0;
   ...
   if(!Strategy_IsTopForecast(_Symbol, members, member_count, forecasts))
      return 0;
...
void Strategy_UpdateStructuralBreak()
  {
   ...
   if(idx < 0 || idx >= QM5_1209_BLOCK_COUNT)
      return;
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(qm_magic_slot_offset != index)
      return true;
   if(strategy_horizon_d1 < 20 || strategy_ema_span < 2 || strategy_norm_stddev_days < 5)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_1209_CARVER_MRINASSET";
   ...
   QM_LogEvent(QM_INFO, "CARVER_MRINASSET_SIGNAL_ON",
...
bool Strategy_ExitSignal()
  {
   ...
   double forecast = 0.0;
   const int desired_direction = Strategy_CurrentForecast(forecast);
   if(desired_direction == 0 || desired_direction != open_direction)
     {
      g_last_exit_key = key;
      return true;
     }
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_1209\",\"strategy\":\"carver-mrinasset\"}");
