...
#property description "QM5_20219 USDJPY NZDUSD Cointegration"
...
//
// Card:
//   strategy-seeds/cards/approved/QM5_20219_usdjpy-nzdusd_card.md
//
// Fixed spread:
//   ln(USDJPY.DWX) - beta * ln(NZDUSD.DWX)
//
// A negative beta makes the leg directions sign-aware: a long spread buys
// both pairs and a short spread sells both pairs. The fitted weights are
// regression-neutral only; the card preserves that risk caveat.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 20219;
...
input double strategy_beta              = -0.782302979;
...
bool Strategy_EnsureBasketScope()
  {
   ...
   string allowed[2] = {"USDJPY.DWX", "NZDUSD.DWX"};
   for(int i = 0; i < 2; ++i)
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(!Strategy_EnsureBasketScope())
      return false;
   if(!QM_SymbolAssertOrLog(g_leg_usdjpy) ||
      !QM_SymbolAssertOrLog(g_leg_nzdusd))
      return false;

   double usdjpy[];
   double nzdusd[];
   datetime usdjpy_time[];
   datetime nzdusd_time[];
   ArraySetAsSeries(usdjpy, true);
   ArraySetAsSeries(nzdusd, true);
   ArraySetAsSeries(usdjpy_time, true);
   ArraySetAsSeries(nzdusd_time, true);

   if(CopyClose(g_leg_usdjpy, PERIOD_D1, 1, history_count, usdjpy) != // perf-allowed: called only by the gated spread refresh.
      history_count)
      return false;
   if(CopyClose(g_leg_nzdusd, PERIOD_D1, 1, history_count, nzdusd) != // perf-allowed: called only by the gated spread refresh.
      history_count)
      return false;
   if(CopyTime(g_leg_usdjpy, PERIOD_D1, 1, history_count, usdjpy_time) != // perf-allowed: called only by the gated spread refresh.
      history_count)
      return false;
   if(CopyTime(g_leg_nzdusd, PERIOD_D1, 1, history_count, nzdusd_time) != // perf-allowed: called only by the gated spread refresh.
      history_count)
      return false;
   ...
   for(int i = 0; i < history_count; ++i)
     {
      if(usdjpy_time[i] <= 0 || usdjpy_time[i] != nzdusd_time[i])
         return false;
      if(usdjpy[i] <= 0.0 || nzdusd[i] <= 0.0)
         return false;

      spreads[i] =
         MathLog(usdjpy[i]) - strategy_beta * MathLog(nzdusd[i]);
...
bool Strategy_PreparePair(const int spread_direction,
                          QM_EntryRequest &host_req)
  {
   ...
   const double nzdusd_weight = MathAbs(strategy_beta);
   const double weight_sum = usdjpy_weight + nzdusd_weight;
   if(weight_sum <= 0.0)
      return false;

   // Preflight both normalized legs before sending either order. A fitted
   // leg weight that falls below broker minimum volume rejects the
   ...
   const double usdjpy_lots =
      Strategy_LotsForLeg(g_leg_usdjpy, usdjpy_weight, usdjpy_weight);
   const double nzdusd_lots =
      Strategy_LotsForLeg(g_leg_nzdusd, nzdusd_weight, usdjpy_weight);
   if(usdjpy_lots <= 0.0 || nzdusd_lots <= 0.0)
      return false;

   const bool long_spread = (spread_direction > 0);
   const QM_OrderType usdjpy_type = long_spread ? QM_BUY : QM_SELL;
   const QM_OrderType nzdusd_type =
      long_spread
      ? (strategy_beta >= 0.0 ? QM_SELL : QM_BUY)
      : (strategy_beta >= 0.0 ? QM_BUY : QM_SELL);
   const string reason =
      long_spread ? "QM5_20219_LONG_SPREAD_Z_LT_NEG_ENTRY"
                  : "QM5_20219_SHORT_SPREAD_Z_GT_POS_ENTRY";

   const double host_entry =
      QM_OrderTypeIsBuy(usdjpy_type)
      ? SymbolInfoDouble(g_leg_usdjpy, SYMBOL_ASK)
      : SymbolInfoDouble(g_leg_usdjpy, SYMBOL_BID);
   const double host_atr =
      QM_ATR(g_leg_usdjpy, PERIOD_D1, strategy_atr_period_d1, 1);
   if(host_entry <= 0.0 || host_atr <= 0.0)
      return false;

   host_req.type = usdjpy_type;
   host_req.price = 0.0;
   host_req.sl = QM_StopATRFromValue(g_leg_usdjpy,
                                     usdjpy_type,
                                     host_entry,
                                     host_atr,
                                     strategy_atr_sl_mult);
   ...
   g_companion_entry_type = nzdusd_type;
   g_companion_entry_lots = nzdusd_lots;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20219_PAIR_HOST";
   ...
   if(qm_news_temporal != QM_NEWS_TEMPORAL_OFF ||
      qm_news_compliance != QM_NEWS_COMPLIANCE_NONE)
     {
      if(!QM_NewsAllowsTrade2(g_leg_usdjpy,
                              ...
                              qm_news_compliance))
         return true;
      if(!QM_NewsAllowsTrade2(g_leg_nzdusd,
                              broker_time,
                              qm_news_temporal,
                              qm_news_compliance))
         return true;
     ...
     {
      if(!QM_NewsAllowsTrade(g_leg_usdjpy,
                             broker_time,
                             qm_news_mode_legacy))
         return true;
      if(!QM_NewsAllowsTrade(g_leg_nzdusd,
                             broker_time,
                             qm_news_mode_legacy))
         return true;
     }
...
void OnTick()
  {
   ...
   const bool is_new_signal_bar =
      QM_IsNewBar(g_leg_usdjpy, PERIOD_D1);
