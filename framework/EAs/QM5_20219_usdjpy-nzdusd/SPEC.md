# QM5_20219_usdjpy-nzdusd — Strategy Spec
...
`ln(USDJPY.DWX) - (-0.782302979) * ln(NZDUSD.DWX)`. Score the newest spread
...
| `strategy_beta` | -0.782302979 | fixed | DEV-fitted structural hedge coefficient |
...
- `NZDUSD.DWX` — fixed companion and second traded leg.

Both traded symbols provide the USD-account conversion paths required by their
own contracts; no order-free conversion symbol or magic slot is declared.
...
| Bar gating | `QM_IsNewBar(USDJPY.DWX, PERIOD_D1)` consumed once per signal bar |
...
| OOS basket state changes | 14 across 2023–2024 in the frozen scan |
| Typical hold time | multi-day; scan half-life 206.281 D1 bars |
| Expected drawdown profile | high and regime-sensitive; conservative 30% prior |
| Regime preference | USDJPY/NZDUSD fixed-residual mean reversion |
| Win rate target | unknown; Q02 is the first platform-economic judge |

The fixed scan recorded DEV net Sharpe `0.121916`, OOS net Sharpe `0.050918`,
and OOS return `0.579866%`. The near-zero OOS score and very long half-life are
...
`1.0` and `0.782302979`; each leg is independently bounded by its ATR stop.
If the NZDUSD allocation normalizes below broker minimum volume, the
...
|---|---|---|---|
| v1 | 2026-08-05 | Initial build from approved card | Rank-40 first-gap fixed-scan FX basket |
