...
#property description "QM5_10747 TradingView VWAP EMA Pullback"
...
bool Strategy_SameBrokerDate(const datetime a, const datetime b)
  {
   MqlDateTime da;
   MqlDateTime db;
   TimeToStruct(a, da);
...
bool Strategy_TouchedEmaZone(const MqlRates &bar, const double fast_ema, const double slow_ema)
  {
   const double zone_low = MathMin(fast_ema, slow_ema);
   const double zone_high = MathMax(fast_ema, slow_ema);
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   const double slow_ema = QM_EMA(_Symbol, tf, strategy_slow_ema_period, 1);
   ...
   if(fast_ema <= 0.0 || slow_ema <= 0.0 || rsi <= 0.0 || adx <= 0.0 || atr <= 0.0)
      return false;
   ...
   if(bar.close > vwap &&
      ...
      plus_di > minus_di)
     {
      ...
      req.reason = "TV_VWAP_EMA_PB_LONG";
   ...
   if(bar.close < vwap &&
      ...
      minus_di > plus_di)
     {
      ...
      req.sl = QM_StopATRFromValue(_Symbol, req.type, entry, atr, strategy_atr_sl_mult);
      req.tp = QM_TakeRR(_Symbol, req.type, entry, req.sl, strategy_reward_risk);
      req.reason = "TV_VWAP_EMA_PB_SHORT";
...
bool Strategy_ExitSignal()
  {
   return !Strategy_IsInsideSession(TimeCurrent());
  }
