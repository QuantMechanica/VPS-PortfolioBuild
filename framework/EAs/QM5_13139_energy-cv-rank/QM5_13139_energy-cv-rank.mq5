...
#property description "QM5_13139 XTI XNG Coefficient-of-Variation Rank"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_13139 - XTI/XNG Coefficient-of-Variation Rank
// -----------------------------------------------------------------------------
// Bimonthly market-neutral energy basket:
//   - reconstruct 37 completed broker month-end closes from native D1 history
//   - calculate 36 monthly log returns per leg
//   - rank sample variance / absolute mean return (coefficient of variation)
//   - long the higher-CV energy leg and short the lower-CV leg
// Runtime is Darwinex-native D1 OHLC only; no external or futures-chain data.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 13139;
...
input int    strategy_cv_window_months         = 36;
...
double   g_cache_xti_cv = 0.0;
double   g_cache_xng_cv = 0.0;
double   g_cache_cv_difference = 0.0;
...
int Strategy_PeriodKeyForTime(const datetime value)
  {
   ...
   if(parts.year <= 0 || parts.mon < 1 || parts.mon > 12)
      return 0;
   return parts.year * 6 + (parts.mon - 1) / 2;
  }
...
bool Strategy_IsRebalanceMonth(const int month_key)
  {
   ...
   return (month >= 1 && month <= 12 && (month % 2) == strategy_rebalance_month_parity);
  }
...
bool Strategy_CoefficientOfVariation(const string symbol,
                                     double &cv_measure,
                                     int &observation_count)
  {
   ...
   const int required_month_closes = strategy_cv_window_months + 1;
   const int count = CopyRates(symbol, PERIOD_D1, 1, strategy_history_bars, rates); // perf-allowed: bounded copy only on a bimonthly D1 rebalance bar.
   ...
   if(ArrayResize(monthly_returns, strategy_cv_window_months) != strategy_cv_window_months)
      return false;

   double return_sum = 0.0;
   for(int i = 0; i < strategy_cv_window_months; ++i)
     {
      ...
      if(newer_close <= 0.0 || older_close <= 0.0)
         return false;
      const double value = MathLog(newer_close / older_close);
      if(!MathIsValidNumber(value))
         return false;
      monthly_returns[observation_count] = value;
      return_sum += value;
      ++observation_count;
     }

   if(observation_count != strategy_cv_window_months || observation_count < 2)
      return false;

   const double mean_return = return_sum / (double)observation_count;
   double squared_sum = 0.0;
   for(int i = 0; i < observation_count; ++i)
     {
      const double centered = monthly_returns[i] - mean_return;
      squared_sum += centered * centered;
     }

   const double sample_variance = squared_sum / (double)(observation_count - 1);
   const double absolute_mean = MathAbs(mean_return);
   if(sample_variance <= 0.0 || absolute_mean <= 1.0e-12 ||
      !MathIsValidNumber(sample_variance) || !MathIsValidNumber(absolute_mean))
      return false;
   cv_measure = sample_variance / absolute_mean;
   return (cv_measure > 0.0 && MathIsValidNumber(cv_measure));
  }
...
bool Strategy_LoadSignalState(const datetime decision_bar_time,
                              int &pair_direction)
  {
   ...
   g_cache_xti_cv = 0.0;
   g_cache_xng_cv = 0.0;
   g_cache_cv_difference = 0.0;
   g_cache_xti_observations = 0;
   g_cache_xng_observations = 0;

   if(decision_bar_time <= 0)
      return false;
   if(!Strategy_CoefficientOfVariation(g_leg_xti,
                                       g_cache_xti_cv,
                                       g_cache_xti_observations))
      return false;
   if(!Strategy_CoefficientOfVariation(g_leg_xng,
                                       g_cache_xng_cv,
                                       g_cache_xng_observations))
      return false;

   g_cache_cv_difference = g_cache_xti_cv - g_cache_xng_cv;
   if(!MathIsValidNumber(g_cache_cv_difference))
      return false;
   if(g_cache_cv_difference > 1.0e-12)
      pair_direction = 1;
   else if(g_cache_cv_difference < -1.0e-12)
      pair_direction = -1;
   return true;
  }
...
void Strategy_AdvanceSignal_OnNewBar()
  {
   ...
   const datetime prior_bar_time = iTime(_Symbol, PERIOD_D1, 1); // perf-allowed: exact first-tradable-bar bimonthly transition check.
...
bool Strategy_OpenPair(const int pair_direction)
  {
   ...
   const string reason = long_xti_short_xng ? "QM5_13139_LONG_XTI_SHORT_XNG_HIGH_CV"
                                            : "QM5_13139_SHORT_XTI_LONG_XNG_HIGH_CV";
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart())
      return true;
   if(strategy_cv_window_months != 36)
      return true;
   if(strategy_history_bars < 900 || strategy_history_bars > 1600)
      return true;
   if(strategy_rebalance_month_parity != 1)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_13139_XTI_XNG_CV_HOST";
   req.symbol_slot = qm_magic_slot_offset;
   req.expiration_seconds = 0;

   if(!g_bimonthly_rebalance_bar || g_cache_period_key <= 0 ||
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_13139\",\"ea\":\"energy-cv-rank\"}");
