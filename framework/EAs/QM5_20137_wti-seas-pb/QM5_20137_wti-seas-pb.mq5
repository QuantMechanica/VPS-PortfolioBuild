...
#property description "QM5_20137 WTI same-calendar seasonal pullback"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_20137 - WTI Same-Calendar Seasonal Pullback
// -----------------------------------------------------------------------------
...
//   2. calculate the sign of WTI's immediately completed broker-month return;
//      and
//   3. trade in the seasonal direction only when the two non-zero signs
//      disagree.
// The package closes and may renew at the next month boundary. A consumed
// month is never retried after sign alignment, restart, rejection, stop, or a
// blocked gate. Runtime uses native MT5 data only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                     = 20137;
...
bool Strategy_LoadPullbackSignal(const datetime decision_bar_time,
                                 ...
                                 int &direction)
  {
   ...
   ArraySetAsSeries(rates, true);
   const int copied =
      CopyRates(_Symbol, PERIOD_D1, 1, strategy_history_bars, rates); // perf-allowed: bounded completed-month reconstruction, called once per broker month.
   if(copied <= 0)
      return false;
   ...
   if(!Strategy_MonthReturn(rates,
                            ...
                            pullback_score))
      return false;
   if(!MathIsValidNumber(pullback_score))
      return false;

   const double threshold =
      MathMax(0.0, strategy_min_abs_return_pct) / 100.0;
   if(pullback_score > threshold)
      direction = 1;
   else if(pullback_score < -threshold)
      direction = -1;
   return true;
  }
...
void Strategy_AdvanceMonthlyState()
  {
   ...
   // checks. Disagreement and all fallible gates remain one-shot monthly.
   if(!Strategy_RecordMonthAttempt(month_key))
      return;

   int seasonal_direction = 0;
   int pullback_direction = 0;
   const bool seasonal_valid =
      Strategy_LoadSeasonalSignal(decision_bar_time,
                                  g_seasonal_score,
                                  g_signal_sample_count,
                                  seasonal_direction);
   const bool pullback_valid =
      Strategy_LoadPullbackSignal(decision_bar_time,
                                  g_pullback_score,
                                  pullback_direction);
   g_signal_valid = seasonal_valid && pullback_valid;
   if(g_signal_valid &&
      seasonal_direction != 0 &&
      pullback_direction != 0 &&
      seasonal_direction == -pullback_direction)
...
bool Strategy_InputsValid()
  {
   if(qm_ea_id != 20137 || qm_magic_slot_offset != 0)
      return false;
   if(strategy_history_years != 10 ||
      strategy_min_history_years != 5 ||
      strategy_history_bars != 3000)
      return false;
   if(MathAbs(strategy_min_abs_return_pct) > 1.0e-12)
      return false;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20137_WTI_SEAS_PB";
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_20137_MONTH_ATTEMPT_%d",
                   QM_FrameworkMagic());
