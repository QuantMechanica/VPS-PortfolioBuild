...
// -----------------------------------------------------------------------------
// Card: Jarrod Goodwin, "Beat the Markets Strategy Guidebook" (~2020),
// Strategy 1 (derived from Larry Williams, 1999). Source:
// D:/QM/strategy_farm/artifacts/cards_approved/
// QM5_11461_goodwin-j-outside-bar-daily-reversion-d1.md
//
// An outside bar (High[1]>High[2] and Low[1]<Low[2]) that closes beyond
// the prior day's extreme is a short-term exhaustion signal: close below
// prior Low -> LONG (bounce expected); close above prior High -> SHORT.
// Entry at market on the next D1 open. P2 simplification: 1-bar hold, exit
// at market regardless of profit; hard 200-pip SL only, no TP. Optional
// no-Friday-setup filter per card baseline.
// -----------------------------------------------------------------------------

bool Strategy_NoTradeFilter()
  {
   if(strategy_hold_bars <= 0 || strategy_sl_pips <= 0)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   const double high_2 = iHigh(_Symbol, PERIOD_CURRENT, 2);
   const double low_2  = iLow(_Symbol, PERIOD_CURRENT, 2);
   if(high_1 <= 0.0 || low_1 <= 0.0 || close_1 <= 0.0 || high_2 <= 0.0 || low_2 <= 0.0)
      return false;

   const bool outside_bar = (high_1 > high_2) && (low_1 < low_2);
   if(!outside_bar)
      return false;
   ...
   const bool go_long  = (close_1 < low_2);
   const bool go_short = (close_1 > high_2);
   if(go_long == go_short)
      return false;
   ...
   req.tp = 0.0;
   req.reason = go_long ? "GOODWINJ_OUTSIDE_BAR_LONG" : "GOODWINJ_OUTSIDE_BAR_SHORT";
   req.symbol_slot = qm_magic_slot_offset;
   req.expiration_seconds = 0;
   return (req.sl > 0.0);
  }
