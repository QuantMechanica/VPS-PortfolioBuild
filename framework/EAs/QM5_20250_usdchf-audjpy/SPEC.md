# QM5_20250_usdchf-audjpy — Strategy Spec
...
`ln(USDCHF.DWX) - (-0.027722525061) * ln(AUDJPY.DWX)`. Score the newest spread
...
| `strategy_beta` | -0.027722525061 | fixed | DEV-fitted structural hedge coefficient |
...
- `AUDUSD.DWX` and `USDJPY.DWX` — conversion-history dependencies observed by
  the USD-account tester for AUDJPY P/L; both are order-free and have no magic
  slot.
...
| Typical hold time | multi-day; scan half-life 97.106 D1 bars |
| Expected drawdown profile | high and regime-sensitive; conservative 30% prior |
| Regime preference | USDCHF/AUDJPY fixed-residual mean reversion |
| Win rate target | unknown; Q02 is the first platform-economic judge |

The fixed scan recorded DEV net Sharpe `-0.046224`, OOS net Sharpe `-0.498678`,
and OOS return `-4.919953%`. The negative OOS score and slow half-life are
...
`1.0` and `0.027722525061`; each leg is independently bounded by its ATR stop.
If the AUDJPY allocation normalizes below broker minimum volume, the
...
|---|---|---|---|
| v1 | 2026-08-06 | Initial build from approved card | Next-rank-61 non-duplicate fixed-scan FX basket |
