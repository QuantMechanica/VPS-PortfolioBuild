...
#property description "QM5_20250 USDCHF AUDJPY Cointegration"
...
//
// Card:
//   strategy-seeds/cards/approved/QM5_20250_usdchf-audjpy_card.md
//
// Fixed spread:
//   ln(USDCHF.DWX) - beta * ln(AUDJPY.DWX)
//
...
input double strategy_beta              = -0.027722525061;
...
bool Strategy_EnsureBasketScope()
  {
   ...
   string allowed[4] = {"USDCHF.DWX", "AUDJPY.DWX", "AUDUSD.DWX", "USDJPY.DWX"};
   for(int i = 0; i < 4; ++i)
...
bool Strategy_RefreshSpreadState()
  {
   ...
   for(int i = 0; i < history_count; ++i)
     {
      if(usdchf_time[i] <= 0 || usdchf_time[i] != audjpy_time[i])
         return false;
      if(usdchf[i] <= 0.0 || audjpy[i] <= 0.0)
         return false;

      spreads[i] =
         MathLog(usdchf[i]) - strategy_beta * MathLog(audjpy[i]);
...
bool Strategy_PreparePair(const int spread_direction,
                          QM_EntryRequest &host_req)
  {
   ...
   const double weight_sum = usdchf_weight + audjpy_weight;
   if(weight_sum <= 0.0)
      return false;
   ...
   const double audjpy_lots =
      Strategy_LotsForLeg(g_leg_audjpy, audjpy_weight, usdchf_weight);
   if(usdchf_lots <= 0.0 || audjpy_lots <= 0.0)
      return false;
   ...
   const string reason =
      long_spread ? "QM5_20250_LONG_SPREAD_Z_LT_NEG_ENTRY"
                  : "QM5_20250_SHORT_SPREAD_Z_GT_POS_ENTRY";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20250_PAIR_HOST";
