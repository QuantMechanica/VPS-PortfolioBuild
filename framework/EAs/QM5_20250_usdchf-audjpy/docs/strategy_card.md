...
# USDCHF/AUDJPY D1 Cointegration Basket
...
low-frequency daily implementation. Chan makes no claim for USDCHF/AUDJPY.
...
Rank 60 USDJPY/EURGBP is built as `QM5_20246`. USDCHF/AUDJPY ranks sixty-first
of 66 by OOS net Sharpe and is the next relationship-level build gap. The
deterministic dedup check found no exact collision across 4,307 registry rows
and 424 direct cards. Three fuzzy matches share one leg only; no tracked
two-leg basket manifest trades this exact pair.

| Pair | DEV net Sharpe | OOS net Sharpe | OOS return | OOS state changes | Fixed DEV beta | Half-life |
|---|---:|---:|---:|---:|---:|---:|
| USDCHF / AUDJPY | -0.046223926559 | -0.498677810996 | -4.919952558569% | 15 | -0.027722525061 | 97.105594983326 D1 bars |
...
`QM5_1156` contains USDCHF but its pair universe excludes AUDJPY. Broad
momentum, carry, trend, and strength baskets that contain both symbols as
unrelated members do not trade this frozen-beta D1 residual or logical
two-leg package. Sibling fixed-scan sleeves use different pairs,
coefficients, magics, and logical identities.

## 2. Concept

Trade temporary deviations in the fixed log-price residual
`ln(USDCHF) - (-0.027722525061) * ln(AUDJPY)`. The negative fitted coefficient
...
package retains USD, CHF, AUD, JPY, carry, and policy-divergence exposure.
...
`USDCHF.DWX` is the tester host and first traded leg. `AUDJPY.DWX` is the
companion traded leg. `AUDUSD.DWX` and `USDJPY.DWX` are warmed only for
...
- Compute `spread = ln(USDCHF) - strategy_beta * ln(AUDJPY)`, with frozen
  `strategy_beta = -0.027722525061`.
...
Chan supplies the pair-trading method but reports no USDCHF/AUDJPY result. The
...
data_requirements: "USDCHF.DWX, AUDJPY.DWX, AUDUSD.DWX, and USDJPY.DWX D1 histories"
```
...
| v2 | 2026-08-06 | initial deterministic V5 basket implementation | Q01 | PASS |
...
- 2026-08-06: Rank 60 is already built as QM5_20246; rank 61 is the first
  relationship-level gap after a semantic umbrella and manifest audit.
- 2026-08-06: Rank 61 is negative in DEV and OOS and slower than the expected
