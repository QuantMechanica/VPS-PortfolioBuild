...
#property description "QM5_20285 WTI Fixed-Step Huber Return Trend"
...
//   3. estimate an even-sample median and raw MAD, then freeze normalized scale;
//   4. run exactly 32 residual-weighted Huber location updates;
//   5. trade the sign of the final bounded-influence location.
// The robust statistic and fixed tuning are transparent QM mechanizations.
// Runtime uses native XTIUSD.DWX price and broker data only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 20285;
...
double g_cache_delta            = 0.0;
double g_cache_huber            = 0.0;
...
bool Strategy_HuberSignal(const double &closes[], int &signal)
  {
   signal = 0;
   const int return_count = strategy_return_months;
   if(return_count != 12 || ArraySize(closes) != return_count + 1)
     {
      g_cache_state_reason = "bad_huber_contract";
      return false;
     }
   ...
   const double delta = strategy_huber_tuning * scale;
   if(mad <= 0.0 || scale <= 0.0 || delta <= 0.0 ||
      !MathIsValidNumber(mad) || !MathIsValidNumber(scale) ||
      !MathIsValidNumber(delta))
     {
      g_cache_state_reason = "nonpositive_or_invalid_huber_scale";
      return false;
     }
   ...
   for(int step = 0; step < strategy_huber_steps; ++step)
     {
      ...
      for(int i = 0; i < return_count; ++i)
        {
         const double residual = MathAbs(monthly_returns[i] - location);
         if(!MathIsValidNumber(residual))
           {
            g_cache_state_reason = "invalid_huber_residual";
            return false;
           }
         double weight = 1.0;
         if(residual > delta)
         ...
         if(weight <= 0.0 || !MathIsValidNumber(weight))
           {
            g_cache_state_reason = "invalid_huber_weight";
            return false;
           }
      ...
      if(total_weight <= 0.0 || !MathIsValidNumber(total_weight) ||
        ...
        {
         g_cache_state_reason = "invalid_huber_weight_sum";
         return false;
        }
      location = weighted_sum / total_weight;
      if(!MathIsValidNumber(location))
        {
         g_cache_state_reason = "invalid_huber_location";
         return false;
        }
     }
   ...
   g_cache_mad = mad;
   g_cache_delta = delta;
   g_cache_huber = location;
   if(location == 0.0)
     {
      g_cache_state_reason = "exact_zero_huber";
      return true;
     }
...
bool Strategy_LoadSignalState(int &signal)
  {
   ...
   return Strategy_HuberSignal(monthly_closes, signal);
  }
...
void Strategy_PrepareMonthlySignal()
  {
   ...
   QM_LogEvent(QM_INFO,
               "MONTHLY_STATE",
               StringFormat("{\"month\":%d,\"valid\":%s,\"signal\":%d,\"median\":%.10f,\"mad\":%.10f,\"delta\":%.10f,\"huber\":%.10f,\"state\":\"%s\"}",
                            g_cache_month_key,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 20285 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20285_WTI_HUBER_MOM";
   ...
   req.reason = (g_cache_signal > 0) ? "HUBER_MOM_XTI_LONG" : "HUBER_MOM_XTI_SHORT";
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_20285_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_20285\",\"ea\":\"wti-huber-mom\"}");
