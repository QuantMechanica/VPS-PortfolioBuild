...
#property description "QM5_21520 XNG weekly quiet-flow momentum"
...
//   - only in the low-volume tail, follow the latest five-bar close return
//   - persist the attempted week before fallible gates so there is no retry
// Runtime uses native MT5 XNG D1 bars only; no external signal feed.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 21520;
...
input double strategy_vol_percentile_cap  = 25.0;
...
bool Strategy_LoadWeeklyState(int &direction,
                              ...
                              double &atr_last)
  {
   ...
   if(volume_rank > strategy_vol_percentile_cap)
     {
      g_cached_state_reason = "above_volume_percentile_cap";
      return false;
     }
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsExpectedHost())
      return true;
   if(qm_ea_id != 21520 || qm_magic_slot_offset != 0)
      return true;
   if(strategy_vol_lookback < 4 || strategy_vol_lookback > 260)
      return true;
   if(strategy_vol_percentile_cap <= 0.0 ||
      strategy_vol_percentile_cap >= 100.0)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_21520_XNG_FLOW_MOM";
   ...
   req.reason = (g_cached_direction > 0)
                ? "XNG_LOW_TICKVOL_WEEK_MOM_LONG"
                : "XNG_LOW_TICKVOL_WEEK_MOM_SHORT";
   return true;
  }
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_21520_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_21520\",\"ea\":\"xng-flow-mom\",\"signal\":\"weekly_quiet_tick_volume_return_continuation\"}");
