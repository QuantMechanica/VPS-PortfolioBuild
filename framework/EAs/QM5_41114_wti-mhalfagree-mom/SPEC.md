# QM5_41114_wti-mhalfagree-mom - Strategy Spec
...
**Source:** `MOP-WTI-MHALFAGREE-MOM-2026`
...
`log(C[n-1]/C[k-1])`. Buy only when both legs are strictly positive and sell
only when both are strictly negative. A zero leg, sign disagreement, invalid
...
follows the agreeing direction for one broker month, using one fixed-risk
...
strict equality handling, direction map, and hold are not parameters.

## 3. Symbol Universe

- Host and traded symbol: exact `XTIUSD.DWX`, D1.
- Symbol slot: 0.
- Magic: `411140000` (governed slot-0 allocation for `XTIUSD.DWX`).
...
- Trigger: strict agreement between the two exhaustive cumulative path halves.
...
- Symmetric direct-WTI monthly structural continuation after two-half path
  confirmation.
...
tests, and explicit WTI membership. Within-month two-half agreement is a
...
alternate split, individual daily-sign vote, return-magnitude threshold,
...
|---|---|---|---|
| v0 | 2026-08-22 | approved build-directory identity | source approval `3e3264609`; EA-ID reservation `97e2d2ddd`; Q00 card `b9b4f0fb0`; governed magic `411140000` |
