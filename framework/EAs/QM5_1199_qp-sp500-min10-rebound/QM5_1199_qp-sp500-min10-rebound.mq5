...
#property description "QM5_1199 Quantpedia SP500 10-Day Minimum Rebound"

#include <QM/QM_Common.mqh>

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                       = 1199;
...
input int    strategy_minimum_lookback_days = 10;
input int    strategy_min_valid_closes      = 60;
...
bool Strategy_HasMinimumReboundSignal(int &signal_key)
  {
   signal_key = 0;

   const int lookback = MathMax(2, strategy_minimum_lookback_days);
   const int min_closes = MathMax(strategy_min_valid_closes, lookback + strategy_atr_period + 5);
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QP_SP500_MIN10_REBOUND";
   ...
   int signal_key = 0;
   if(!Strategy_HasMinimumReboundSignal(signal_key))
      return false;
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_1199\",\"ea\":\"qp-sp500-min10-rebound\"}");
