...
#property description "QM5_41250 WTI Monthly Exact-Permutation Robust Scale Trend"
...
//   - compare old/recent median absolute deviations and enumerate every one
//     of the 924 fixed-size six/six label assignments
//   - follow the recent block mean only after an upper-tail scale expansion
...
input double strategy_scale_epsilon         = 0.000000000001;
input double strategy_compare_tolerance     = 0.00000000000001;
input int    strategy_tail_count_max        = 416;
...
bool Strategy_MadSix(const double &values[],
                     double &median,
                     double &mad)
  {
   median = 0.0;
   mad = 0.0;
   if(!Strategy_MedianSix(values, median))
      return false;

   double deviations[6];
   for(int index = 0; index < 6; ++index)
     {
      deviations[index] = MathAbs(values[index] - median);
      if(!MathIsValidNumber(deviations[index]) || deviations[index] < 0.0)
         return false;
     }
   if(!Strategy_MedianSix(deviations, mad))
      return false;
   return (mad >= 0.0 && MathIsValidNumber(mad));
  }

bool Strategy_PermutationScaleSignal(const double &closes[],
                                     Strategy_SignalMetrics &metrics)
  {
   ...
   if(strategy_month_returns != 12 ||
      ...
      MathAbs(strategy_scale_epsilon - 0.000000000001) >
         0.000000000000000001 ||
      MathAbs(strategy_compare_tolerance - 0.00000000000001) >
   ...
   for(int index = 0; index < strategy_month_returns; ++index)
     {
      ...
      if(!MathIsValidNumber(old_sum) ||
         !MathIsValidNumber(recent_sum))
         return false;
   ...
   metrics.mean_old = old_sum / (double)strategy_block_size;
   metrics.mean_recent = recent_sum / (double)strategy_block_size;
   if(!MathIsValidNumber(metrics.mean_old) ||
      !MathIsValidNumber(metrics.mean_recent) ||
      !Strategy_MadSix(old_values,
                       metrics.median_old,
                       metrics.mad_old) ||
      !Strategy_MadSix(recent_values,
                       metrics.median_recent,
                       metrics.mad_recent) ||
      StringLen(metrics.return_path) <= 0)
      return false;

   metrics.observed_scale_delta = metrics.mad_recent - metrics.mad_old;
   metrics.oldest_close = closes[0];
   metrics.newest_close = closes[metrics.endpoint_count - 1];
   if(!MathIsValidNumber(metrics.observed_scale_delta) ||
      !MathIsValidNumber(metrics.oldest_close) ||
      !MathIsValidNumber(metrics.newest_close))
      return false;

   // A non-expansion is a valid flat decision. Enumeration is required only
   // after the card's strict positive scale gate has passed.
   if(metrics.observed_scale_delta <= strategy_scale_epsilon)
      return true;
         ...
         if((mask & (1 << index)) != 0)
           {
            if(recent_index < 0 || recent_index >= 6)
               return false;
            pseudo_recent[recent_index] = returns[index];
            ++recent_index;
           }
         else
           {
            if(old_index < 0 || old_index >= 6)
               return false;
            pseudo_old[old_index] = returns[index];
            ++old_index;
           }
        }
      if(old_index != 6 || recent_index != 6)
         return false;

      double pseudo_old_median = 0.0;
      double pseudo_old_mad = 0.0;
      double pseudo_recent_median = 0.0;
      double pseudo_recent_mad = 0.0;
      if(!Strategy_MadSix(pseudo_old,
                          pseudo_old_median,
                          pseudo_old_mad) ||
         !Strategy_MadSix(pseudo_recent,
                          pseudo_recent_median,
                          pseudo_recent_mad))
         return false;

      const double perm_delta = pseudo_recent_mad - pseudo_old_mad;
      if(!MathIsValidNumber(perm_delta))
         return false;
      if(perm_delta >= metrics.observed_scale_delta -
                       strategy_compare_tolerance)
         ++metrics.tail_count;
      ++metrics.assignment_count;
     }

   if(metrics.assignment_count != 924 ||
   ...
   if(metrics.tail_count <= strategy_tail_count_max)
     {
      if(metrics.mean_recent > strategy_direction_epsilon)
         metrics.direction = 1;
      else if(metrics.mean_recent < -strategy_direction_epsilon)
         metrics.direction = -1;
     }
   return true;
  }
                                        ...
                                        int &direction)
  {
   ...
   const bool valid =
      Strategy_LoadMonthlyEndpoints(current_month_key,
                                    ...
                                    closes,
                                    endpoint_times) &&
      Strategy_PermutationScaleSignal(closes, metrics);
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
   ...
   else
     {
      ...
      g_signal_valid =
         endpoints_valid &&
         Strategy_PermutationScaleSignal(closes, g_signal_metrics);
      if(!g_signal_valid)
         g_signal_state = "permutation_scale_validation_failed";
      else if(g_signal_metrics.direction == 0)
         g_signal_state = "permutation_scale_unqualified_flat";
      else if(g_signal_metrics.direction > 0)
         g_signal_state = "permutation_scale_expansion_wti_long";
      else if(g_signal_metrics.direction < 0)
         g_signal_state = "permutation_scale_expansion_wti_short";
      else
   ...
   QM_LogEvent(QM_INFO,
               ...
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"endpoint_count\":%d,\"return_count\":%d,\"block_size\":%d,\"assignment_count\":%d,\"tail_count\":%d,\"mean_old\":%.12f,\"mean_recent\":%.12f,\"median_old\":%.12f,\"median_recent\":%.12f,\"mad_old\":%.12f,\"mad_recent\":%.12f,\"observed_scale_delta\":%.12f,\"tail_fraction\":%.12f,\"returns\":\"%s\",\"oldest_close\":%.10f,\"newest_close\":%.10f,\"oldest_endpoint\":%I64d,\"newest_endpoint\":%I64d,\"state\":\"%s\"}",
                            ...
                            g_signal_metrics.mad_old,
                            g_signal_metrics.mad_recent,
                            g_signal_metrics.observed_scale_delta,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41250 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41250_WTI_MPERM_SCALE";
   ...
   req.reason = (g_signal_metrics.direction > 0)
      ? "WTI_MPERM_SCALE_LONG"
      : "WTI_MPERM_SCALE_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41250 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI exact-permutation robust scale trend holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41250_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41250\",\"ea\":\"wti-mperm-scale-tr\"}");
