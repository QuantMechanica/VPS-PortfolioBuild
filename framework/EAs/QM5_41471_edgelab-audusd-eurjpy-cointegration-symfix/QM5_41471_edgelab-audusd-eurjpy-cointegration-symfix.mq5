...
#property description "QM5_41471 Edge Lab AUDUSD EURJPY Cointegration"
...
input double strategy_beta              = 0.279193;
...
bool Strategy_RefreshSpreadState()
  {
   ...
   for(int i = 0; i < lookback; ++i)
     {
      if(audusd[i] <= 0.0 || eurjpy[i] <= 0.0)
         return false;
      spreads[i] = MathLog(audusd[i]) - strategy_beta * MathLog(eurjpy[i]);
...
bool Strategy_OpenPair(const int spread_direction)
  {
   ...
   const double weight_sum = audusd_weight + eurjpy_weight;
   if(weight_sum <= 0.0)
      return false;
   ...
   const string reason = long_spread ? "QM5_41471_LONG_SPREAD_Z_LT_NEG_ENTRY"
                                     : "QM5_41471_SHORT_SPREAD_Z_GT_POS_ENTRY";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41471_PAIR_HOST";
