...
#property description "QM5_41427 WTI Refinery-Maintenance Weekly Close-Location Continuation"
...
//   - trade only February/March/September/October refinery-maintenance anchor weeks
//   - aggregate two consecutive completed broker-week OHLC packages
//   - require negative parent-close to newest-close return
//   - require newest close strictly in its own lower range tercile
//   - consume the week before fallible gates; never enter late or retry
//   - hold one short position until the next normalized broker week
// Runtime uses MT5-native OHLC, calendar, history, and framework state only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                      = 41427;
...
input int    strategy_maintenance_month_1    = 2;
input int    strategy_maintenance_month_2    = 3;
input int    strategy_maintenance_month_3    = 9;
input int    strategy_maintenance_month_4    = 10;
input int    strategy_required_weeks         = 2;
input int    strategy_min_week_bars          = 3;
input int    strategy_max_week_bars          = 5;
input double strategy_return_epsilon         = 0.0;
input double strategy_clv_lower              = 0.3333333333333333;
...
int      g_new_week_bars          = 0;
int      g_parent_week_bars       = 0;
double   g_new_open               = 0.0;
double   g_new_high               = 0.0;
double   g_new_low                = 0.0;
double   g_new_close              = 0.0;
double   g_parent_open            = 0.0;
double   g_parent_high            = 0.0;
double   g_parent_low             = 0.0;
double   g_parent_close           = 0.0;
double   g_weekly_return          = 0.0;
...
bool Strategy_IsMaintenanceMonth(const int month)
  {
   return (month == strategy_maintenance_month_1 ||
           month == strategy_maintenance_month_2 ||
           month == strategy_maintenance_month_3 ||
           month == strategy_maintenance_month_4);
  }
...
bool Strategy_LoadCloseLocationSignal(const int current_week_key,
                                          const int label_offset,
                                          int &direction,
                                          int &new_week_bars,
                                          int &parent_week_bars,
                                          double &new_open,
                                          double &new_high,
                                          double &new_low,
                                          double &new_close,
                                          ...
                                          double &close_location)
  {
   ...
   int found_weeks = 0;
   bool parent_boundary_seen = false;
   ...
   for(int index = 0; index < copied; ++index)
     {
      ...
      if(found_weeks == 0 || week_key != week_keys[found_weeks - 1])
        {
         if(found_weeks >= 2)
           {
            if(Strategy_NextWeekKey(week_key) != week_keys[1])
               return false;
      ...
      // Series order is newest to oldest. Overwrite until the oldest session
      // in the bucket so this becomes the completed week's first-session open.
   ...
   if(found_weeks != 2 || !parent_boundary_seen)
      return false;
   for(int bucket = 0; bucket < 2; ++bucket)
     {
      if(week_counts[bucket] < strategy_min_week_bars ||
         ...
         return false;
      if(week_opens[bucket] <= 0.0 ||
         week_highs[bucket] <= 0.0 ||
         week_lows[bucket] <= 0.0 ||
   ...
   new_week_bars = week_counts[0];
   parent_week_bars = week_counts[1];
   new_open = week_opens[0];
   new_high = week_highs[0];
   new_low = week_lows[0];
   new_close = week_closes[0];
   parent_open = week_opens[1];
   parent_high = week_highs[1];
   parent_low = week_lows[1];
   parent_close = week_closes[1];

   const double new_range = new_high - new_low;
   if(new_range <= 0.0 || !MathIsValidNumber(new_range))
      return false;
   weekly_return = MathLog(new_close / parent_close);
   if(!MathIsValidNumber(weekly_return))
      return false;
   close_location = (new_close - new_low) / new_range;
   if(!MathIsValidNumber(close_location) ||
      close_location < 0.0 || close_location > 1.0)
      return false;

   if(weekly_return < -strategy_return_epsilon &&
      close_location < strategy_clv_lower)
      direction = -1;
   return true;
  }
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   else if(!Strategy_IsMaintenanceMonth(anchor_month))
      g_signal_state = "outside_refinery_maintenance_months";
   else
     {
      g_signal_valid =
         Strategy_LoadCloseLocationSignal(g_decision_week_key,
      ...
      if(!g_signal_valid)
         ...
         g_signal_state = "negative_return_lower_tercile_short";
      else if(g_weekly_return >= -strategy_return_epsilon)
         g_signal_state = "nonnegative_return_flat";
      else if(g_close_location == strategy_clv_lower)
         g_signal_state = "close_location_equality_flat";
      else
         g_signal_state = "negative_return_not_lower_tercile_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"week\":%d,\"anchor_month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_week_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"new_week_bars\":%d,\"parent_week_bars\":%d,\"new_open\":%.8f,\"new_high\":%.8f,\"new_low\":%.8f,\"new_close\":%.8f,\"parent_open\":%.8f,\"parent_high\":%.8f,\"parent_low\":%.8f,\"parent_close\":%.8f,\"weekly_return\":%.12f,\"close_location\":%.12f,\"state\":\"%s\"}",
                            g_decision_week_key,
                            anchor_month,
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(strategy_symbol == "" ||
      ...
      strategy_maintenance_month_1 != 2 ||
      strategy_maintenance_month_2 != 3 ||
      strategy_maintenance_month_3 != 9 ||
      strategy_maintenance_month_4 != 10 ||
      strategy_required_weeks != 2 ||
      strategy_min_week_bars != 3 ||
      strategy_max_week_bars != 5 ||
      MathAbs(strategy_return_epsilon) > 1.0e-15 ||
      MathAbs(strategy_clv_lower - 0.3333333333333333) > 1.0e-12 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41427_WTI_REFMAINT_WCLV_CONT";
   ...
   req.type = QM_SELL;
   req.reason = "WTI_REFMAINT_WCLV_CONT_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(strategy_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41427 ||
      qm_magic_slot_offset != 0)
   ...
   g_attempt_state_key =
      StringFormat("QM5_41427_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41427\",\"ea\":\"wti-refmaint-wclv-cont\"}");
