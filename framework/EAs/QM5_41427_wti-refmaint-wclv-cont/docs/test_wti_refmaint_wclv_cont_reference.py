"""Deterministic checks for QM5_41427 refinery-maintenance weekly continuation."""
...
class WtiRefineryMaintenanceCloseLocationReferenceTest(unittest.TestCase):
    ...
    def bars(self, newest_close: float = 78.0, parent_close: float = 80.0) -> list[Bar]:
        newest = package(self.new_anchor, [81.0, 80.5, 80.0, 79.0, newest_close], 76.0, 84.0)
        parent = package(self.parent_anchor, [82.0, 81.5, 81.0, 80.5, parent_close], 75.0, 82.0)
        return newest + parent

    def test_negative_return_and_lower_tercile_is_short(self) -> None:
    ...
    def test_nonnegative_return_is_flat(self) -> None:
    ...
    def test_exact_close_location_boundary_is_flat(self) -> None:
        ...
        direction, _, close_location = signal(
            self.current, self.bars(newest_close=boundary, parent_close=81.0)
        )
    ...
    def test_three_to_five_sessions_are_accepted(self) -> None:
        for count in (3, 4, 5):
            newest = package(self.new_anchor, [80.0] * (count - 1) + [78.0], 76.0, 84.0)
