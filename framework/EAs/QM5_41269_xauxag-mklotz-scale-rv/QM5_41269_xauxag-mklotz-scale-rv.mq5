...
#property description "QM5_41269 XAU/XAG Monthly Centered Klotz Scale Reversion"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_41269 - XAU/XAG Monthly Centered Klotz Scale Reversion
// -----------------------------------------------------------------------------
// At the first synchronized D1 boundary of a broker month, reconstruct the
// latest exactly matched XAU/XAG close pair in each of thirteen consecutive
// completed broker months. Split the twelve adjacent gold-minus-silver log
// returns into fixed old/recent blocks of six and subtract each block's mean.
// Strict pooled centered-residual ranks receive the frozen N=12 squared-normal
// Klotz scores. An inclusive exact 924-assignment upper-half state fades the
// raw block-mean shift with an opposite equal-notional package to next month.
// Runtime is native, completed-price-only, deterministic, and one consumed
// attempt per month. T1 and the inclusive tail are integrity diagnostics and
// activity gates only; neither is a p-value and neither changes risk.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41269;
...
input double strategy_klotz_expected           = 3.9642160041063397;
input double strategy_klotz_denominator        = 1.2716448806860048;
input int    strategy_assignment_count         = 924;
input int    strategy_tail_count_max           = 494;
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41269 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
            strategy_xag_symbol == "XAGUSD.DWX" &&
            strategy_endpoint_count == 13 &&
            strategy_return_count == 12 &&
            strategy_block_size == 6 &&
            MathAbs(strategy_relative_epsilon - 1.0e-12) <= 1.0e-18 &&
            MathAbs(strategy_klotz_expected -
                    3.9642160041063397) <= 1.0e-12 &&
            MathAbs(strategy_klotz_denominator -
                    1.2716448806860048) <= 1.0e-12 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_MKUIPER_RV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }
...
bool Strategy_PreparePackage(const int direction,
                             ...
                             double &xag_stop)
  {
   ...
   QM_LogEvent(QM_INFO,
               "PACKAGE_PLAN",
               StringFormat("{\"direction\":%d,\"xau_atr\":%.12e,\"xag_atr\":%.12e,\"xau_stop_distance\":%.12e,\"xag_stop_distance\":%.12e,\"raw_xau_lots\":%.8f,\"raw_xag_lots\":%.8f,\"xau_lots\":%.8f,\"xag_lots\":%.8f,\"xau_notional\":%.12e,\"xag_notional\":%.12e,\"normalized_stop_risk\":%.12e,\"notional_mismatch_fraction\":%.12e}",
                            direction,
                            xau_atr,
                            xag_atr,
                            xau_actual_stop_distance,
                            xag_actual_stop_distance,
                            raw_xau_lots,
                            raw_xag_lots,
                            xau_lots,
                            xag_lots,
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41269_XAUXAG_KLOTZ_RV";
...
bool Strategy_KlotzScore(const int rank, double &score)
  {
   score = 0.0;
   switch(rank)
     {
      case 1:  score = 2.0336952456315065; break;
      case 2:  score = 1.0405555206952889; break;
      case 3:  score = 0.54216113018145117; break;
      case 4:  score = 0.25240799405049096; break;
      case 5:  score = 0.086072547360949524; break;
      case 6:  score = 0.0093235661866525334; break;
      case 7:  score = 0.0093235661866525334; break;
      case 8:  score = 0.086072547360949524; break;
      case 9:  score = 0.25240799405049096; break;
      case 10: score = 0.54216113018145117; break;
      case 11: score = 1.0405555206952889; break;
      case 12: score = 2.0336952456315065; break;
      default: return false;
     }
   return MathIsValidNumber(score) && score >= 0.0;
  }

bool Strategy_KlotzSignal(
      const double &relative_returns[],
      double &old_mean,
      double &recent_mean,
      double &recent_score,
      double &standardized_t1,
      ...
      int &direction)
  {
   ...
   if(ArraySize(relative_returns) != strategy_return_count ||
      strategy_return_count != 12 || strategy_block_size != 6 ||
      MathAbs(strategy_relative_epsilon - 1.0e-12) > 1.0e-18 ||
      MathAbs(strategy_klotz_expected -
              3.9642160041063397) > 1.0e-12 ||
      MathAbs(strategy_klotz_denominator -
      ...
      strategy_assignment_count != 924 ||
      strategy_tail_count_max != 494 ||
      strategy_klotz_denominator <= 0.0)
      return false;

   double residuals[12];
   double ranked_residuals[12];
   int ranked_labels[12];
   double old_sum = 0.0;
   double recent_sum = 0.0;
   for(int index = 0; index < strategy_block_size; ++index)
     {
      const double old_value = relative_returns[index];
      const double recent_value =
         relative_returns[index + strategy_block_size];
      if(!MathIsValidNumber(old_value) ||
         !MathIsValidNumber(recent_value))
         return false;
      old_sum += old_value;
      recent_sum += recent_value;
     }
   old_mean = old_sum / (double)strategy_block_size;
   recent_mean = recent_sum / (double)strategy_block_size;
   if(!MathIsValidNumber(old_mean) || !MathIsValidNumber(recent_mean))
      return false;
   ...
   for(int index = 0; index < strategy_return_count; ++index)
     {
      residuals[index] =
         ...
         ((index < strategy_block_size) ? old_mean : recent_mean);
      if(!MathIsValidNumber(residuals[index]))
         return false;
      ranked_residuals[index] = residuals[index];
      ranked_labels[index] = (index < strategy_block_size) ? 0 : 1;
      if(StringLen(residual_path) > 0)
         residual_path += ",";
      residual_path +=
         StringFormat("%s:%.12e",
                      (ranked_labels[index] == 0) ? "O" : "R",
                      residuals[index]);
     }

   for(int left = 0; left < strategy_return_count; ++left)
     {
      for(int right = left + 1; right < strategy_return_count; ++right)
        {
         const double tie_tolerance =
            strategy_relative_epsilon *
            MathMax(1.0,
                    MathMax(MathAbs(residuals[left]),
                            MathAbs(residuals[right])));
         if(!MathIsValidNumber(tie_tolerance))
            return false;
         if(MathAbs(residuals[left] - residuals[right]) <= tie_tolerance)
           {
            tie_detected = true;
            return true; // A valid tied state consumes the month flat.
           }
        }
     }

   for(int index = 1; index < strategy_return_count; ++index)
     {
      const double residual = ranked_residuals[index];
      const int label = ranked_labels[index];
      int cursor = index - 1;
      while(cursor >= 0 && ranked_residuals[cursor] > residual)
        {
         ranked_residuals[cursor + 1] = ranked_residuals[cursor];
         ranked_labels[cursor + 1] = ranked_labels[cursor];
         --cursor;
        }
      ranked_residuals[cursor + 1] = residual;
      ranked_labels[cursor + 1] = label;
     }
   ...
   for(int index = 0; index < strategy_return_count; ++index)
     {
      double score = 0.0;
      if(!Strategy_KlotzScore(index + 1, score))
         return false;
      total_score += score;
      if(ranked_labels[index] == 1)
         recent_score += score;
      if(StringLen(rank_path) > 0)
         rank_path += ",";
      rank_path +=
         StringFormat("%d:%s:%.12e:%.12e",
                      index + 1,
                      (ranked_labels[index] == 0) ? "O" : "R",
                      ranked_residuals[index],
                      score);
     }
   const double total_tolerance =
      strategy_relative_epsilon * MathMax(1.0, MathAbs(total_score));
   if(!MathIsValidNumber(total_score) ||
      !MathIsValidNumber(recent_score) ||
      !MathIsValidNumber(total_tolerance) ||
      MathAbs(total_score - 7.928432008212679) > total_tolerance)
      return false;

   standardized_t1 =
      (recent_score - strategy_klotz_expected) /
      strategy_klotz_denominator;
   if(!MathIsValidNumber(standardized_t1))
      return false;

   int choice[6] = {0, 1, 2, 3, 4, 5};
   while(true)
     {
      double permutation_score = 0.0;
      for(int item = 0; item < strategy_block_size; ++item)
        {
         double score = 0.0;
         if(!Strategy_KlotzScore(choice[item] + 1, score))
            return false;
         permutation_score += score;
        }
      if(!MathIsValidNumber(permutation_score))
         return false;
      const double tail_tolerance =
         strategy_relative_epsilon * MathMax(1.0, MathAbs(recent_score));
      if(!MathIsValidNumber(tail_tolerance))
         return false;
      if(permutation_score + tail_tolerance >= recent_score)
         ++tail_count;
      ++assignment_count;

      int pivot = strategy_block_size - 1;
      while(pivot >= 0 &&
            choice[pivot] == strategy_return_count -
                             strategy_block_size + pivot)
         --pivot;
      if(pivot < 0)
         break;
      ++choice[pivot];
      for(int item = pivot + 1; item < strategy_block_size; ++item)
         choice[item] = choice[item - 1] + 1;
     }
   if(assignment_count != strategy_assignment_count ||
      tail_count < 0 || tail_count > strategy_assignment_count)
      return false;

   const double score_tolerance =
      strategy_relative_epsilon *
      MathMax(1.0, MathAbs(strategy_klotz_expected));
   if(!MathIsValidNumber(score_tolerance) ||
      recent_score + score_tolerance < strategy_klotz_expected ||
      tail_count > strategy_tail_count_max)
      return true;

   const double mean_delta = recent_mean - old_mean;
   const double location_tolerance =
      strategy_relative_epsilon *
      MathMax(1.0, MathMax(MathAbs(old_mean), MathAbs(recent_mean)));
   if(!MathIsValidNumber(mean_delta) ||
      !MathIsValidNumber(location_tolerance))
      return false;
   if(mean_delta > location_tolerance)
      direction = -1; // SELL XAU / BUY XAG.
   else if(mean_delta < -location_tolerance)
      direction = 1;  // BUY XAU / SELL XAG.
   return true;
  }

bool Strategy_LoadMonthlyKlotzSignal(
      ...
      int &direction)
  {
   ...
   const int xag_copied =
      CopyRates(g_leg_xag, // perf-allowed: one bounded thirteen-month scan behind a consumed monthly attempt.
      ...
      while(xau_index < xau_copied && xag_index < xag_copied)
        {
         ...
         if(month_count < 0 ||
            ...
            month_count >= ArraySize(newest_first_times))
            return false;
   ...
   for(int index = 0; index < strategy_endpoint_count; ++index)
     {
      ...
      if(!MathIsValidNumber(chronological_ratios[index]) ||
         ...
         return false;
      if(StringLen(endpoint_path) > 0)
         endpoint_path += ",";
      endpoint_path +=
         StringFormat("%d@%I64d",
                      Strategy_MonthKey(chronological_times[index]),
                      (long)chronological_times[index]);
      if(StringLen(ratio_path) > 0)
         ratio_path += ",";
      ratio_path += DoubleToString(chronological_ratios[index], 12);
     }

   for(int index = 0; index < ArraySize(relative_returns); ++index)
     {
      const double value =
         chronological_ratios[index + 1] - chronological_ratios[index];
      if(!MathIsValidNumber(value))
         return false;
      relative_returns[index] = value;
      if(StringLen(return_path) > 0)
         return_path += ",";
      return_path += DoubleToString(value, 12);
     }
   if(StringLen(return_path) <= 0)
      return false;

   return Strategy_KlotzSignal(relative_returns,
                               old_mean,
                               recent_mean,
                               recent_score,
                               standardized_t1,
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41269_XAUXAG_KLOTZ_RV_HOST";
   ...
   string rank_path = "";
   const bool valid =
      Strategy_LoadMonthlyKlotzSignal(
         g_signal_month_key,
         month_count,
   ...
   string state = "monthly_klotz_signal_failed";
   if(valid && direction > 0)
      state = "qualified_klotz_mean_fade_long_xau";
   else if(valid && direction < 0)
      state = "qualified_klotz_mean_fade_short_xau";
   else if(valid && tie_detected)
      state = "centered_residual_tie_flat";
   else if(valid &&
           recent_score + score_tolerance < strategy_klotz_expected)
      state = "klotz_score_below_expected_flat";
   else if(valid && tail_count > strategy_tail_count_max)
      state = "klotz_tail_above_cap_flat";
   else if(valid && MathAbs(mean_delta) <= location_tolerance)
      state = "neutral_raw_mean_shift_flat";
   else if(valid)
      state = "nonqualifying_klotz_flat";

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"month_count\":%d,\"newest_endpoint_time\":%I64d,\"old_mean\":%.12e,\"recent_mean\":%.12e,\"recent_score\":%.12e,\"expected\":%.12e,\"denominator\":%.12e,\"t1\":%.12e,\"assignments\":%d,\"tail_count\":%d,\"tie_detected\":%s,\"direction\":%d,\"endpoints\":\"%s\",\"ratios\":\"%s\",\"returns\":\"%s\",\"residuals\":\"%s\",\"ranks\":\"%s\",\"state\":\"%s\"}",
                            g_signal_month_key,
                            month_count,
                            ...
                            recent_mean,
                            recent_score,
                            strategy_klotz_expected,
                            strategy_klotz_denominator,
                            standardized_t1,
                            assignment_count,
                            tail_count,
                            tie_detected ? "true" : "false",
                            direction,
                            endpoint_path,
                            ratio_path,
                            return_path,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG monthly centered Klotz scale reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41269\",\"ea\":\"xauxag-mklotz-scale-rv\"}");
