...
#property description "QM5_20245 WTI Variance-Ratio RSM"
...
//   - return-sign direction from the newest 12 returns at threshold 0.40
//   - persistence follows RSM; anti-persistence reverses RSM
...
input int    strategy_rsm_lookback_months    = 12;
...
bool Strategy_LoadMemoryRsmSignal(const int current_month_key,
                                  ...
                                  int &trade_direction)
  {
   ...
   if(strategy_vr_window_months != 32 ||
      strategy_vr_q != 2 ||
      strategy_rsm_lookback_months != 12 ||
      needed_closes != 33)
      return false;
   ...
   // month_closes[] is reverse chronological. Build the thirty-two returns
   // in chronological order so lag-one products match the approved formula.
   ...
   const int rsm_start =
      strategy_vr_window_months - strategy_rsm_lookback_months;
   for(int index = rsm_start;
       index < strategy_vr_window_months;
       ++index)
     {
      if(monthly_returns[index] >= 0.0)
         ++positive_count;
     }

   positive_probability =
      (double)positive_count / (double)strategy_rsm_lookback_months;
   if(!MathIsValidNumber(positive_probability) ||
      positive_probability < 0.0 ||
      positive_probability > 1.0)
      return false;

   // A statistically insignificant memory state validly consumes the month
   // flat. Significant persistence follows RSM; anti-persistence reverses it.
   if(MathAbs(z_value) <= strategy_significance_z)
      return true;

   const int sign_direction =
      (positive_probability >= strategy_positive_threshold) ? 1 : -1;
   const int memory_direction = (z_value > 0.0) ? 1 : -1;
   trade_direction = sign_direction * memory_direction;
   return true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(!Strategy_LoadMemoryRsmSignal(month_key,
                                    variance_ratio,
                                    z_value,
                                    positive_probability,
                                    positive_count,
   ...
   req.reason = StringFormat("WTI_VR_RSM_%s_P%02d_OF_%02d_Z%.4f_VR%.4f",
                             trade_direction > 0 ? "LONG" : "SHORT",
                             positive_count,
                             strategy_rsm_lookback_months,
                             z_value,
                             variance_ratio);
   return true;
  }
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_20245_MONTH_ATTEMPT_%d",
                   QM_FrameworkMagic());
   Strategy_LoadAttemptState();

   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_20245\",\"ea\":\"wti-vr-rsm\"}");
