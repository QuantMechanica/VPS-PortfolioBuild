...
#property description "QM5_10303 Narang Price Trend Continuation"
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   const double close_price = QM_SMA(_Symbol, strategy_signal_tf, 1, 1, PRICE_CLOSE);
   ...
   if(close_price <= 0.0 || ema_fast <= 0.0 || ema_slow <= 0.0 || adx <= 0.0)
      return false;
   if(adx < strategy_adx_threshold)
      return false;

   int direction = 0;
   if(close_price > ema_slow && ema_fast > ema_slow)
      direction = 1;
   if(close_price < ema_slow && ema_fast < ema_slow)
      direction = -1;
   if(direction == 0)
   ...
   req.reason = (direction > 0) ? "NARANG_TREND_LONG" : "NARANG_TREND_SHORT";
...
bool Strategy_ExitSignal()
  {
   ...
   const double ema_fast = QM_EMA(_Symbol, strategy_signal_tf, strategy_fast_ema_period, 1);
   const double ema_slow = QM_EMA(_Symbol, strategy_signal_tf, strategy_slow_ema_period, 1);
   if(close_price <= 0.0 || ema_fast <= 0.0 || ema_slow <= 0.0)
      return false;
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      if(position_type == POSITION_TYPE_BUY &&
         (close_price < ema_fast || ema_fast < ema_slow))
         return true;
