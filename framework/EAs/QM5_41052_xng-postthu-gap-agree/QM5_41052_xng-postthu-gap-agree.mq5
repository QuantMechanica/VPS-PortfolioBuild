...
#property description "QM5_41052 XNG Post-Thursday Gap-Agreement Friday Continuation"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41052 - XNG Standard-Thursday Post-Event-Gap Continuation
// -----------------------------------------------------------------------------
...
//   - enter Friday only when both nonzero components agree in sign
...
bool Strategy_LoadPostThursdayGapAgreement(const datetime current_bar_time,
                                            const datetime broker_now,
                                            ...
                                            int &direction)
  {
   ...
   const bool signs_agree =
      ((event_session_flow > 0.0 && post_event_gap > 0.0) ||
       (event_session_flow < 0.0 && post_event_gap < 0.0));
   if(!signs_agree)
      return true;

   // Follow the cross-boundary common sign; magnitude never changes risk.
   if(total_flow > 0.0)
      direction = 1;
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(!QM_InputRequireLong("qm_ea_id", qm_ea_id, 41052) ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "XNG_POSTTHU_GAP_AGREE";
   ...
   int direction = 0;
   if(!Strategy_LoadPostThursdayGapAgreement(g_strategy_d1_bar_time,
                                              broker_now,
                                              event_session_flow,
                                              post_event_gap,
                                              confirmed_path,
   ...
   if(direction > 0)
     {
      req.type = QM_BUY;
      req.reason = "XNG_POSTTHU_GAP_AGREE_LONG";
     }
   else if(direction < 0)
     {
      req.type = QM_SELL;
      req.reason = "XNG_POSTTHU_GAP_AGREE_SHORT";
     }
   else
      return false;
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XNG post-Thursday gap-agreement card exits Friday 21; later D1 is repair"))
      return INIT_FAILED;
   ...
   g_attempt_state_key =
      StringFormat("QM5_41052_POSTTHU_GAP_AGREE_FRIDAY_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41052\",\"ea\":\"xng-postthu-gap-agree\"}");
