...
#property description "QM5_41120 XAU/XAG Completed-Month Fixed-Open Residence Reversion"
...
// Freeze the first chronological ratio as an anchor, require at least three
// quarters of all later ratios and the final ratio strictly on the same side,
// then fade that residence state with one opposite equal-notional package held
// to the next month. Runtime is native, completed-price-only, deterministic,
// and one-attempt-per-month.
// =============================================================================
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41120 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_MSEQDOM_RV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41120_XAUXAG_MOPEN_RESIDENCE_RV";
...
bool Strategy_LoadMonthlyOpenResidence(
      const int current_month_key,
      int &completed_month_sessions,
      int &later_observations,
      int &above_anchor,
      int &below_anchor,
      int &required_residence,
      double &anchor,
      double &final_ratio,
      int &direction)
  {
   ...
   // The first older synchronized pair proves the completed-month package was
   ...
   // explicitly so the first completed-month ratio is the immutable anchor.
   ...
   later_observations = completed_month_sessions - 1;
   if(later_observations < 1)
      return false;

   anchor = chronological_ratios[0];
   final_ratio = chronological_ratios[completed_month_sessions - 1];
   if(!MathIsValidNumber(anchor) || !MathIsValidNumber(final_ratio))
      return false;

   for(int i = 1; i < completed_month_sessions; ++i)
     {
      const double ratio = chronological_ratios[i];
      if(!MathIsValidNumber(ratio))
         return false;
      if(ratio > anchor)
         ++above_anchor;
   ...
   required_residence =
      ...
      strategy_residence_denominator;
   if(required_residence < 1 || required_residence > later_observations ||
      above_anchor < 0 || below_anchor < 0 ||
      above_anchor + below_anchor > later_observations)
      return false;

   // direction > 0 means BUY XAU / SELL XAG. Strict anchor ties count toward
   // neither side, while the final ratio must independently confirm the same
   // qualifying side. Residence surplus and distance never affect sizing.
   if(above_anchor >= required_residence && final_ratio > anchor)
      direction = -1;
   else if(below_anchor >= required_residence && final_ratio < anchor)
      direction = 1;
   return true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41120_XAUXAG_MOPEN_RESIDENCE_RV_HOST";
   ...
   int later_observations = 0;
   int above_anchor = 0;
   int below_anchor = 0;
   int required_residence = 0;
   int direction = 0;
   double anchor = 0.0;
   double final_ratio = 0.0;
   const bool valid =
      Strategy_LoadMonthlyOpenResidence(g_signal_month_key,
                                        completed_month_sessions,
                                        later_observations,
                                        above_anchor,
                                        below_anchor,
                                        ...
                                        final_ratio,
                                        direction);
   string state = "monthly_open_residence_failed";
   if(valid && direction > 0)
      state = "lower_open_residence_fade_long_xau";
   else if(valid && direction < 0)
      state = "upper_open_residence_fade_short_xau";
   else if(valid)
      state = "open_residence_flat";

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"completed_month_sessions\":%d,\"later_observations\":%d,\"above_anchor\":%d,\"below_anchor\":%d,\"required_residence\":%d,\"anchor\":%.12e,\"final_ratio\":%.12e,\"direction\":%d,\"state\":\"%s\"}",
                            g_signal_month_key,
                            completed_month_sessions,
                            later_observations,
                            above_anchor,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG completed-month fixed-open residence reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41120\",\"ea\":\"xauxag-mopen-residence-rv\"}");
