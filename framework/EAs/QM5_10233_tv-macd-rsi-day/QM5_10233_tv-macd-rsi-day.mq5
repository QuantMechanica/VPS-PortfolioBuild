...
bool Strategy_IsEntryWindowNY(const datetime broker_time)
  {
   const int hhmm = Strategy_HHMM(Strategy_BrokerToNewYork(broker_time));
   return (hhmm >= strategy_entry_start_hhmm_ny && hhmm <= strategy_entry_end_hhmm_ny);
  }

bool Strategy_IsEODFlatNY(const datetime broker_time)
  {
   return (Strategy_HHMM(Strategy_BrokerToNewYork(broker_time)) >= strategy_eod_flat_hhmm_ny);
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(Strategy_SelectOpenPosition(existing_ticket))
      return false;
   ...
   if(Bars(_Symbol, entry_tf) < strategy_volume_sma_period + strategy_macd_slow + strategy_macd_signal + 5 ||
      Bars(_Symbol, confirm_tf) < strategy_ema_slow + 5)
      return false;

   const double close_1 = iClose(_Symbol, entry_tf, 1);
   if(close_1 <= 0.0)
      return false;
   ...
   if(ema_fast_1 <= 0.0 || ema_slow_1 <= 0.0 || confirm_fast_1 <= 0.0 || confirm_slow_1 <= 0.0)
      return false;
   ...
   const long volume_1 = iVolume(_Symbol, entry_tf, 1);
...
bool Strategy_ExitSignal()
  {
   ...
   const ENUM_TIMEFRAMES entry_tf = (strategy_entry_tf == PERIOD_CURRENT) ? (ENUM_TIMEFRAMES)_Period : strategy_entry_tf;
   const ENUM_TIMEFRAMES confirm_tf = (strategy_confirm_tf == PERIOD_CURRENT) ? entry_tf : strategy_confirm_tf;
   if(Bars(_Symbol, entry_tf) < strategy_macd_slow + strategy_macd_signal + 5 ||
      Bars(_Symbol, confirm_tf) < strategy_ema_slow + 5)
      return false;
   ...
   if(close_1 <= 0.0 || ema_fast_1 <= 0.0 || ema_slow_1 <= 0.0 ||
      confirm_fast_1 <= 0.0 || confirm_slow_1 <= 0.0 || rsi_1 <= 0.0)
      return false;

   const bool bearish_opposite = (ema_fast_1 < ema_slow_1 &&
                                  close_1 < ema_fast_1 &&
                                  confirm_fast_1 < confirm_slow_1 &&
                                  macd_main_2 >= macd_sig_2 &&
                                  macd_main_1 < macd_sig_1 &&
                                  rsi_1 >= strategy_short_rsi_min &&
                                  rsi_1 <= strategy_short_rsi_max);
   const bool bullish_opposite = (ema_fast_1 > ema_slow_1 &&
                                  close_1 > ema_fast_1 &&
                                  confirm_fast_1 > confirm_slow_1 &&
                                  macd_main_2 <= macd_sig_2 &&
                                  macd_main_1 > macd_sig_1 &&
   ...
   if(pos_type == POSITION_TYPE_BUY && bearish_opposite)
      return true;
   if(pos_type == POSITION_TYPE_SELL && bullish_opposite)
      return true;
