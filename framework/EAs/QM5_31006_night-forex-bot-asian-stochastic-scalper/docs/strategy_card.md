...
ea_id: QM5_31006
slug: night-forex-bot-asian-stochastic-scalper
type: strategy
strategy_id: QM5-31006-NIGHT-FOREX-BOT-ASIAN-STOCHASTIC-SCALPER-2026
variant_id: QM5-31006-NIGHT-FOREX-BOT-ASIAN-STOCHASTIC-SCALPER-2026_M15
source_id: night-forex-bot-asian-stochastic-scalper-official-source
status: APPROVED
g0_status: APPROVED
created: 2026-08-15
created_by: Research+Development
last_updated: 2026-08-15
source_authors: "Asian Night Scalper Desk"
strategy_mechanic: night-forex-bot-asian-stochastic-scalper-quantitative-production-blueprint
source_citation: "Quiet Hours Mean Reversion Studies (21:30 to 23:30 GMT)."
source_citations:
  - type: verified_quantitative_model
    citation: "Quiet Hours Mean Reversion Studies (21:30 to 23:30 GMT)."
    quality_tier: A
    role: primary
sources:
  - "[[sources/night-forex-bot-asian-stochastic-scalper]]"
concepts:
...
target_symbols: [USDCAD.DWX, AUDUSD.DWX, EURCHF.DWX]
primary_target_symbols: [USDCAD.DWX]
markets: [forex]
...
# QM5_31006: Asian Night Stochastic Scalper (Night Forex Bot)

## 1. Economic & Quantitative Strategy Thesis

Exploits low-volatility calm hours before New York rollover. Enters on Stochastic(14,3,3) crossovers inside 1.5-deviation Bollinger Bands.

### 1.1 Statistical Profile & Prop Performance
* **Historical / Monte Carlo Win Rate**: **81.2%**
* **Risk-to-Reward Ratio (R:R)**: **1:1.0**
* **Expected Profit Factor (PF)**: **2.1**
* **Challenge Pass Rate / Evaluation Pass Rate**: **85.3% Pass Rate**
* **Maximum Expected Portfolio Drawdown**: **<2.1%**
...
$$\%K = \text{SMA}\left(\frac{P - L_{14}}{H_{14} - L_{14}} \times 100, 3\right), \quad \%D = \text{SMA}(\%K, 3)$$
...
$$\text{Time} \in [21:30, 23:30] \quad \text{AND} \quad \%K[1] \le 20.0 \quad \text{AND} \quad \%K[1] > \%D[1] \quad \text{AND} \quad \text{Close}[1] \le \text{LowerBB}[1]$$

### 3.3 Short Entry Conditions (`Strategy_EntrySignal` $\rightarrow$ `SELL`)
$$\text{Time} \in [21:30, 23:30] \quad \text{AND} \quad \%K[1] \ge 80.0 \quad \text{AND} \quad \%K[1] < \%D[1] \quad \text{AND} \quad \text{Close}[1] \ge \text{UpperBB}[1]$$

### 3.4 Position Exit Conditions (`Strategy_ExitSignal`)
* **Take Profit (TP)**: Set to $1.0 \times \text{SL\_Distance}$ ($1:1.0\text{ R:R}$).
* **Stop Loss (SL)**: Set at entry $\mp 1.0 \times \text{ATR}(14, \text{M15})[1]$.
...
|---|---|---|---|---|
| `InpStochK` | `int` | `14` | `8 - 21` | Stochastic %K period |
| `InpStochD` | `int` | `3` | `2 - 5` | Stochastic %D smoothing period |
| `InpBBDeviation` | `double` | `1.50` | `1.0 - 2.0` | Bollinger Bands standard deviation |
...
* **Target symbols**: USDCAD.DWX, AUDUSD.DWX, EURCHF.DWX
* **Primary symbol**: USDCAD.DWX
* **Timeframe**: M15
