...
#property description "QM5_21518 WTI Brent-Confirmed Twelve-Month Trend"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_21518 - WTI Brent-Confirmed Twelve-Month Trend
// -----------------------------------------------------------------------------
// Source lineage: Moskowitz, Ooi & Pedersen (2012) own-return trend plus
// CME/ICE/EIA evidence for the linked but distinct WTI and Brent benchmarks.
// At the first processed D1 bar of each new broker month:
//   1. intersect completed XTI/BRENT D1 history at exact timestamps;
//   2. derive thirteen consecutive synchronized month-end closes;
//   3. calculate independent twelve-month WTI and Brent log returns;
//   4. trade WTI only when both strict return signs agree.
// The confirmation rule, CFD mapping, ATR stop, and fixed-dollar sizing are
// transparent QM mechanizations. XBRUSD.DWX remains read-only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 21518;
...
const string g_state_symbol    = "XBRUSD.DWX";
...
double g_cache_brent_return      = 0.0;
...
bool Strategy_LoadSynchronizedHistory(const datetime decision_bar_time,
                                      ...
                                      double &brent_closes[])
  {
   ...
   MqlRates xti_rates[];
   MqlRates brent_rates[];
   ArraySetAsSeries(xti_rates, false);
   ArraySetAsSeries(brent_rates, false);
   ...
   const int xti_count = CopyRates(g_strategy_symbol, // perf-allowed: bounded completed-D1 read only on a genuine monthly decision bar.
                                   ...
                                   xti_rates);
   // perf-allowed: read-only BRENT history is copied on the same monthly path.
   const int brent_count = CopyRates(g_state_symbol, // perf-allowed: bounded read-only BRENT history only on the same monthly decision path.
                                   PERIOD_D1,
                                   1,
                                   strategy_history_bars_d1,
                                   brent_rates);
   ...
   if(xti_count < required_common ||
      brent_count < required_common ||
      !Strategy_ValidateRates(xti_rates, xti_count) ||
      !Strategy_ValidateRates(brent_rates, brent_count))
      return false;

   const int capacity = MathMin(xti_count, brent_count);
   if(ArrayResize(common_times, capacity) != capacity ||
      ArrayResize(xti_closes, capacity) != capacity ||
      ArrayResize(brent_closes, capacity) != capacity)
      return false;

   int i = 0;
   int j = 0;
   int common_count = 0;
   while(i < xti_count && j < brent_count)
     {
      const datetime xti_time = xti_rates[i].time;
      const datetime brent_time = brent_rates[j].time;
      if(xti_time == brent_time)
        {
         if(common_count > 0 &&
         ...
         brent_closes[common_count] = brent_rates[j].close;
         ++common_count;
         ++i;
         ++j;
        }
      else if(xti_time < brent_time)
         ++i;
      else
         ++j;
     }
   ...
   ArrayResize(brent_closes, common_count);
...
bool Strategy_BenchmarkTrends(const datetime &common_times[],
                              ...
                              double &brent_return)
  {
   ...
   if(strategy_trend_months != 12 ||
      ...
      common_count != ArraySize(brent_closes) ||
      common_count <= 0)
      return false;

   double wti_month_end_closes[];
   double brent_month_end_closes[];
   int month_keys[];
   if(ArrayResize(wti_month_end_closes, common_count) != common_count ||
      ArrayResize(brent_month_end_closes, common_count) != common_count ||
   ...
   for(int i = 0; i < common_count; ++i)
     {
      if(common_times[i] <= 0 ||
         ...
         !MathIsValidNumber(brent_closes[i]))
         return false;
      ...
      if(month_count <= 0 || month_keys[month_count - 1] != month_key)
         {
          ...
          brent_month_end_closes[month_count] = brent_closes[i];
          ++month_count;
         }
      else
        {
         wti_month_end_closes[month_count - 1] = xti_closes[i];
         brent_month_end_closes[month_count - 1] = brent_closes[i];
        }
     }
   ...
   const int expected_brent_previous_month =
      QM_CalendarPeriodKey(PERIOD_MN1, g_state_symbol, 1);
   if(expected_previous_month <= 0 ||
      expected_brent_previous_month != expected_previous_month ||
   ...
   const double brent_endpoint_ratio =
      brent_month_end_closes[month_count - 1] / brent_month_end_closes[start];
   if(wti_endpoint_ratio <= 0.0 || brent_endpoint_ratio <= 0.0 ||
      !MathIsValidNumber(wti_endpoint_ratio) ||
      !MathIsValidNumber(brent_endpoint_ratio))
      return false;
   wti_return = MathLog(wti_endpoint_ratio);
   brent_return = MathLog(brent_endpoint_ratio);
   if(!MathIsValidNumber(wti_return) || !MathIsValidNumber(brent_return))
      return false;

   double wti_chained_return = 0.0;
   double brent_chained_return = 0.0;
   for(int i = start; i < month_count - 1; ++i)
     {
      const double wti_monthly_return =
         MathLog(wti_month_end_closes[i + 1] / wti_month_end_closes[i]);
      const double brent_monthly_return =
         MathLog(brent_month_end_closes[i + 1] / brent_month_end_closes[i]);
      if(!MathIsValidNumber(wti_monthly_return) ||
         !MathIsValidNumber(brent_monthly_return))
         return false;
      wti_chained_return += wti_monthly_return;
      brent_chained_return += brent_monthly_return;
     }
   return (MathIsValidNumber(wti_chained_return) &&
           MathIsValidNumber(brent_chained_return) &&
           MathAbs(wti_return - wti_chained_return) <= strategy_return_tolerance &&
           MathAbs(brent_return - brent_chained_return) <= strategy_return_tolerance);
  }
...
bool Strategy_LoadSignalState(const datetime decision_bar_time, int &signal)
  {
   ...
   g_cache_brent_return = brent_return;

   if(wti_return == 0.0 || brent_return == 0.0)
     {
      g_cache_state_reason = "exact_zero_benchmark_return";
      return true;
     }
   if((wti_return > 0.0) != (brent_return > 0.0))
     {
      g_cache_state_reason = "benchmark_sign_disagreement";
      return true;
     }

   signal = (wti_return > 0.0) ? 1 : -1;
...
void Strategy_PrepareMonthlySignal()
  {
   ...
   QM_LogEvent(QM_INFO,
               "MONTHLY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"valid\":%s,\"signal\":%d,\"wti_return_12m\":%.12e,\"brent_return_12m\":%.12e,\"common_closes\":%d,\"state\":\"%s\"}",
                            g_cache_month_key,
                            (long)g_decision_bar_time,
                            g_cache_signal_valid ? "true" : "false",
                            g_cache_signal,
                            g_cache_wti_return,
                            g_cache_brent_return,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 21518 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_21518_WTI_BRENT_CFM_TREND";
   ...
   req.reason = (g_cache_signal > 0) ?
                "BRENT_CFM_TREND_XTI_LONG" : "BRENT_CFM_TREND_XTI_SHORT";
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_21518_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_21518\",\"ea\":\"wti-brent-cfm\",\"brent\":\"read_only\"}");
