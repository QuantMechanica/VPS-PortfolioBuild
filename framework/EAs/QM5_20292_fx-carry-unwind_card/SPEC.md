# QM5_20292_fx-carry-unwind_card - Strategy Spec

- **EA ID:** QM5_20292
- **Slug:** fx-carry-unwind
- **Governed build label:** `QM5_20292_fx-carry-unwind_card`
...
The three ratio thresholds, selected-leg count, and hold limit accept only the
card's enumerated P3 values. Every structural parameter remains locked. The
generated Q02 setfiles use the baseline column exactly.
...
R1 lineage and R2-R4 PASS are recorded in the mirrored
`docs/strategy_card.md` from
`D:/QM/strategy_farm/artifacts/cards_approved/QM5_20292_fx-carry-unwind_card.md` and
...
|---|---|---:|---:|
| Backtest Q01-Q14 | `RISK_FIXED` | 1000 USD | `RISK_PERCENT=0` |
| Live, only after separate authorization | `RISK_PERCENT` | manifest-authorized value | `RISK_FIXED=0` |

Only backtest setfiles are built here, with `RISK_FIXED=1000`,
`RISK_PERCENT=0`, and `PORTFOLIO_WEIGHT=1`. At the baseline's two-leg setting,
each leg receives half of the fixed package stop-risk budget; the one-leg P3
variant receives the full package budget. There is one position maximum per
...
| v2 | 2026-08-24 | Burn-window rebuild; current corset, bounded buffers, declared P3 axes | `build-QM5_20292_fx-carry-unwind_card` |
