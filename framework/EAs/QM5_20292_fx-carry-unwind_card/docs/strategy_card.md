...
q01_status: PASS_SMOKE_DEFERRED
q02_status: ENQUEUED
expected_pf: 1.3
...
logical_symbol: QM5_20292_FX_CARRY_UNWIND_D1
review_focus: "Falsify a weekly, price-only stress-gated reversal of the two strongest broker-native positive-carry crosses. The dynamic swap rank, seven-major realized-volatility breadth, and atomic two-leg lifecycle jointly distinguish it from ordinary carry, fixed AUDJPY risk-off, USD rebound, momentum, and cointegration sleeves."
g0_approval_reasoning: "R1 PASS: Lien (2015), Wiley 3rd ed., Chapter 18 pp.153-160, tier-A OWNER-approved SRC04 and complete bounded extract; R2 PASS: deterministic weekly latch, broker-swap-mode carry-efficiency rank, opposite-side atomic two-leg package, closed bars, hard ATR stops, stress hysteresis and time exit; R3 PASS: all 13 required .DWX symbols are registered, including native AUDCHF.DWX and AUDJPY.DWX targets, while symbols with absent or incomparable swap metadata fail closed; R4 PASS: fixed arithmetic and ranking only, with no ML, banned indicator, external runtime feed, grid, martingale, scale-in, or pyramiding."
---
...
- [x] R1 review evidence: named long-standing FX author, Wiley book, precise
  chapter/page boundary, OWNER-approved SRC04 intake, and complete local
  bounded text reviewed.
- [x] R2 review evidence: fixed weekly clock, fixed realized-volatility state,
  deterministic swap conversion/rank, opposite-side mapping, hard stops,
  hysteresis exit, and time exit.
- [x] R3 review evidence: all required symbols are registered `.DWX` FX instruments;
  runtime uses MT5 OHLC, swap, contract, spread, quote, and position metadata
  only.
- [x] R4 review evidence: arithmetic and fixed ranks only; no trained model, banned
  indicator, external runtime feed, grid, martingale, scale-in, or pyramiding.
- [x] Low-frequency review evidence: at most one attempted package per broker week and entry
  only in the high-volatility regime.
- [x] Governance review: the 2026-08-12 OWNER diversity/funnel mission authorizes
  one non-duplicate structural low-frequency edge when approved diverse backlog and
  diverse infrastructure repairs are exhausted; deterministic allocation produced
  `QM5_20292` before build.
...
| v2 | 2026-08-12 | metadata reconciliation, duplicate review, and diversity-mission authorization | G0 | APPROVED |
| v3 | 2026-08-12 | deterministic build and strict compile; worker-bound smoke deferred by the standard recorder | Q02 | ENQUEUED |

## Pipeline Phase Status

| Phase | Date | Verdict | Evidence path |
|---|---|---|---|
| G0 Research Intake | 2026-08-12 | APPROVED | `decisions/2026-08-12_qm5_20292_fx_carry_unwind_g0.md` |
| Q01 Build Validation | 2026-08-12 | PASS_SMOKE_DEFERRED | `docs/ops/evidence/2026-08-12_qm5_20292_fx_carry_unwind_q01.md` |
| Q02 Baseline Screening | 2026-08-12 | ENQUEUED | `docs/ops/evidence/2026-08-12_qm5_20292_fx_carry_unwind_q01.md` |
...
- 2026-08-12: The prior approval commit changed only `g0_status`; R1-R4,
  status, checklists, and phase evidence were reconciled before deterministic
  allocation and build. Missing or zero custom-symbol swap remains an explicit
  per-symbol ineligibility, never a momentum or static-rate fallback.
- 2026-08-12: Q01 static validation and strict compilation passed with zero
  compiler errors/warnings. The sole direct smoke attempt never reached MT5
  because custom-history admission requires a worker-bound, privatized work
  item. The standard build recorder preserved that setup error, converted
  smoke to a worker-owned P2 deferral, and atomically enqueued one logical Q02
  row; no retry or manual dispatch was made.
