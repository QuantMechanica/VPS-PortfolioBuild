...
#define QM5_20292_MAX_PACKAGE_LEGS 2
...
bool Strategy_RealizedVolRatio(const string symbol,
                               ...
                               double &out_ratio)
  {
   ...
   if(copied < required_closes || rates_size < required_closes)
      return false;
   ...
   if(ArrayResize(returns, return_count) != return_count)
      return false;
   const int returns_size = ArraySize(returns);
   if(returns_size != return_count)
      return false;

   for(int i = 0; i < return_count; ++i)
     {
      if(i < 0 || i >= returns_size || i >= rates_size ||
         i + 1 < 0 || i + 1 >= rates_size)
         return false;
   ...
   if(ArrayResize(rolling_vols, window_count) != window_count)
      return false;
   const int rolling_size = ArraySize(rolling_vols);
   if(rolling_size != window_count)
      return false;
   ...
   for(int start = 0; start < window_count; ++start)
     {
      if(start > 0)
        {
         const int leaving_index = start - 1;
         const int entering_index = start + strategy_rv_window_d1 - 1;
         if(leaving_index < 0 || leaving_index >= ArraySize(returns))
            return false;
         if(entering_index < 0 || entering_index >= ArraySize(returns))
            return false;
         const double leaving = returns[leaving_index];
         const double entering = returns[entering_index];
   ...
   if(ArrayResize(baseline_values,
      ...
      strategy_rv_baseline_observations)
      return false;
   const int baseline_size = ArraySize(baseline_values);
   if(baseline_size != strategy_rv_baseline_observations)
      return false;
   for(int i = 0; i < strategy_rv_baseline_observations; ++i)
     {
      if(i < 0 || i >= baseline_size || i + 1 < 0 ||
         i + 1 >= rolling_size)
         return false;
...
bool Strategy_PackageCompositionValid()
  {
   ...
   if(total != strategy_selected_legs)
      return false;
   int distinct = 0;
   for(int i = 0; i < QM5_20292_TARGET_COUNT; ++i)
     {
   ...
   return (distinct == strategy_selected_legs);
  }
...
bool Strategy_SpreadAllowed(const string symbol)
  {
   ...
   if(copied < strategy_spread_history_d1 ||
      rates_size < strategy_spread_history_d1)
      return false;
   double spreads[];
   if(ArrayResize(spreads, strategy_spread_history_d1) !=
      strategy_spread_history_d1)
      return false;
   const int spreads_size = ArraySize(spreads);
   if(spreads_size != strategy_spread_history_d1)
      return false;
   ...
   for(int i = 0; i < strategy_spread_history_d1; ++i)
     {
      ...
      if(rates[i].spread > 0)
        {
         if(positive_count < 0 || positive_count >= ArraySize(spreads))
            return false;
...
int Strategy_RankCarryCandidates(StrategyCarryCandidate &candidates[])
  {
   int count = 0;
   const int capacity = ArraySize(candidates);
   if(capacity < QM5_20292_TARGET_COUNT)
      return 0;
   for(int i = 0; i < QM5_20292_TARGET_COUNT; ++i)
     {
      StrategyCarryCandidate candidate;
...
bool Strategy_PrepareLeg(const StrategyCarryCandidate &candidate,
                         QM_BasketOrderRequest &request)
  {
   ...
   if(strategy_selected_legs < 1 ||
      strategy_selected_legs > QM5_20292_MAX_PACKAGE_LEGS)
      return false;
   const double risk_weight = 1.0 / (double)strategy_selected_legs;
...
bool Strategy_OpenPreparedLeg(const QM_BasketOrderRequest &basket_request,
                              ulong &out_ticket)
  {
   ...
   QM_RiskMode leg_risk_mode = QM_RISK_MODE_UNSET;
   double leg_risk_value = 0.0;
   if(RISK_FIXED > 0.0 && RISK_PERCENT <= 0.0)
     {
      leg_risk_mode = QM_RISK_MODE_FIXED;
      leg_risk_value = RISK_FIXED / (double)strategy_selected_legs;
     }
   else if(RISK_PERCENT > 0.0 && RISK_FIXED <= 0.0)
     {
      leg_risk_mode = QM_RISK_MODE_PERCENT;
      leg_risk_value = RISK_PERCENT / (double)strategy_selected_legs;
     }
   if(leg_risk_mode == QM_RISK_MODE_UNSET || leg_risk_value <= 0.0)
      return false;
...
bool Strategy_OpenCarryUnwindPackage()
  {
   ...
   QM_BasketOrderRequest requests[QM5_20292_MAX_PACKAGE_LEGS];
   for(int i = 0; i < strategy_selected_legs; ++i)
   ...
   for(int i = 0; i < strategy_selected_legs; ++i)
     {
      ulong ticket = 0;
      if(Strategy_OpenPreparedLeg(requests[i], ticket))
         continue;
...
bool Strategy_IsAllowedEntryRatio()
  {
   return (MathAbs(strategy_stress_entry_ratio - 1.25) <= 1.0e-12 ||
           MathAbs(strategy_stress_entry_ratio - 1.50) <= 1.0e-12 ||
           MathAbs(strategy_stress_entry_ratio - 1.75) <= 1.0e-12);
  }

bool Strategy_IsAllowedExitRatio()
  {
   return (MathAbs(strategy_stress_exit_ratio - 1.00) <= 1.0e-12 ||
           MathAbs(strategy_stress_exit_ratio - 1.10) <= 1.0e-12 ||
           MathAbs(strategy_stress_exit_ratio - 1.20) <= 1.0e-12);
  }

bool Strategy_IsAllowedHoldBars()
  {
   return (strategy_max_hold_d1_bars == 3 ||
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(strategy_rv_window_d1 != 21 ||
      ...
      !Strategy_IsAllowedEntryRatio() ||
      !Strategy_IsAllowedExitRatio() ||
      strategy_selected_legs < 1 ||
      strategy_selected_legs > QM5_20292_MAX_PACKAGE_LEGS ||
      strategy_atr_period_d1 != 20 ||
      MathAbs(strategy_atr_sl_mult - 2.5) > 1.0e-12 ||
      !Strategy_IsAllowedHoldBars() ||
...
void Strategy_ManageOpenPosition()
  {
   ...
   if(open_legs <= 0)
      return;
   if(open_legs != strategy_selected_legs ||
