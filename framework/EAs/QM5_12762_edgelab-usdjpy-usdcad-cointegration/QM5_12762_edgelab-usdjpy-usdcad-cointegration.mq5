...
#property description "QM5_12762 Edge Lab USDJPY USDCAD Cointegration"
...
input double strategy_beta              = 0.196;
...
bool Strategy_IsHostSymbol()
  {
   return (_Symbol == g_leg_usdjpy || _Symbol == g_leg_usdcad);
  }
...
bool Strategy_EnsureBasketScope()
  {
   ...
   string allowed[2] = {"USDJPY.DWX", "USDCAD.DWX"};
   for(int i = 0; i < 2; ++i)
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(!Strategy_EnsureBasketScope())
      return false;
   if(!QM_SymbolAssertOrLog(g_leg_usdjpy) || !QM_SymbolAssertOrLog(g_leg_usdcad))
      return false;
   ...
   for(int i = 0; i < lookback; ++i)
     {
      if(usdjpy[i] <= 0.0 || usdcad[i] <= 0.0)
         return false;
      spreads[i] = MathLog(usdjpy[i]) - strategy_beta * MathLog(usdcad[i]);
...
bool Strategy_OpenPair(const int spread_direction)
  {
   ...
   const double weight_sum = usdjpy_weight + usdcad_weight;
   if(weight_sum <= 0.0)
      return false;
   ...
   const string reason = long_spread ? "QM5_12762_LONG_SPREAD_Z_LT_NEG_ENTRY"
                                     : "QM5_12762_SHORT_SPREAD_Z_GT_POS_ENTRY";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12762_PAIR_HOST";
