# QM5_12762_edgelab-usdjpy-usdcad-cointegration - Strategy Spec
...
The EA trades the USDJPY.DWX and USDCAD.DWX D1 cointegration spread. It computes
`S = ln(USDJPY) - beta * ln(USDCAD)` with beta defaulting to 0.196, then
...
same script ranked USDJPY/USDCAD as the strongest remaining unbuilt
...
state changes, beta 0.196, and 381-day half-life.
...
| strategy_beta | 0.196 | >0 | Hedge coefficient in `ln(USDJPY) - beta * ln(USDCAD)` |
...
**Designed for:**
- USDJPY.DWX - leg 1 of the fixed USDJPY/USDCAD spread and the spread numerator.
- USDCAD.DWX - leg 2 of the fixed USDJPY/USDCAD spread and the beta-weighted spread denominator.

**Explicitly not for:**
- Other `.DWX` symbols. This card is a fixed two-leg FX-cross basket, not a portable multi-pair strategy.

The EA selects USDJPY.DWX and USDCAD.DWX only; both USD-base legs are available
...
|---|---:|---:|---:|---:|---:|---:|
| USDJPY~USDCAD | -0.12 | 0.10 | +1.20% | 15 | 0.196 | 381d |
