...
#property description "QM5_41068 WTI Completed-Week Acceleration Momentum"
...
//   - trade only strict same-sign magnitude acceleration and follow that sign
...
bool Strategy_LoadAccelerationSignal(const int current_week_key,
                                    const int label_offset,
                                    int &direction,
                                    double &new_return,
                                    double &old_return,
                                    ...
                                    double &week_end_old)
  {
   ...
   if(old_return > 0.0 && new_return > 0.0 &&
      MathAbs(new_return) > MathAbs(old_return))
      direction = 1;
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_WeekAlreadyEntered(g_decision_week_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadAccelerationSignal(g_decision_week_key,
      ...
      if(!g_signal_valid)
         ...
         g_signal_state = "positive_acceleration_long";
      else if(g_signal_direction < 0)
         g_signal_state = "negative_acceleration_short";
      else if(g_new_return == 0.0 || g_old_return == 0.0)
         g_signal_state = "exact_zero_flat";
      else if((g_old_return < 0.0 && g_new_return > 0.0) ||
              (g_old_return > 0.0 && g_new_return < 0.0))
         g_signal_state = "sign_opposition_flat";
      else
         g_signal_state = "nonaccelerating_flat";
     }
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41068 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41068_WTI_WACCEL_MOM";
...
int OnInit()
  {
   if(!SymbolSelect(g_strategy_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41068 ||
   ...
   g_attempt_state_key =
      StringFormat("QM5_41068_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41068\",\"ea\":\"wti-waccel-mom\"}");
