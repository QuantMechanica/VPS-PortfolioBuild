"""Deterministic reference checks for QM5_41068 WTI completed-week acceleration."""
...
def acceleration_signal(
    ...
    """Mirror three completed week ends and strict same-sign magnitude acceleration."""
...
class WeekAccelerationReferenceTest(unittest.TestCase):
    def test_positive_acceleration_is_long(self) -> None:
        ...
        valid, direction, new_value, old_value, endpoints = acceleration_signal(
        ...
        self.assertEqual(endpoints, (121.0, 110.0, 105.0))

    def test_negative_acceleration_is_short(self) -> None:
        current, now, bars = sample(81.0, 90.0, 95.0)
        offset = label_offset(current, now)
        assert offset is not None
        valid, direction, new_value, old_value, _ = acceleration_signal(
            20260824, bars, offset
        )
        self.assertTrue(valid)
        self.assertEqual(direction, -1)
    ...
    def test_opposed_zero_equal_and_decelerating_states_are_flat(self) -> None:
        current, now, bars = sample(110.0, 100.0, 105.0)
        offset = label_offset(current, now)
        assert offset is not None
        self.assertEqual(acceleration_signal(20260824, bars, offset)[1], 0)

        current, now, bars = sample(100.0, 100.0, 105.0)
        offset = label_offset(current, now)
        assert offset is not None
        valid, direction, new_value, _, _ = acceleration_signal(
            20260824, bars, offset
        )
        self.assertTrue(valid)
        ...
        current, now, bars = sample(105.0, 100.0, 90.0)
        offset = label_offset(current, now)
        assert offset is not None
        self.assertEqual(acceleration_signal(20260824, bars, offset)[1], 0)

    def test_uniform_prior_date_label_normalization_and_grace(self) -> None:
        current, now, bars = sample(121.0, 110.0, 105.0, True)
        decision, late, count, offset = decision_clock(current, now, bars)
        self.assertTrue(decision)
        self.assertFalse(late)
        self.assertEqual(count, 0)
        ...
        assert offset is not None
        self.assertEqual(acceleration_signal(20260824, bars, offset)[1], 1)
    ...
    def test_nonconsecutive_or_current_week_endpoint_rejected(self) -> None:
        ...
        self.assertFalse(acceleration_signal(20260824, bars, offset)[0])
        _, _, bars = sample(121.0, 110.0, 105.0)
        bars.insert(0, Bar(datetime(2026, 8, 24, tzinfo=UTC), 111.0))
        self.assertFalse(acceleration_signal(20260824, bars, offset)[0])
