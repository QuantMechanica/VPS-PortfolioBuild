...
#property description "QM5_41039 XAU/XAG Monthly Relative-Flow Divergence"
...
// -----------------------------------------------------------------------------
// At the first synchronized D1 boundary of a broker month, decompose every
// completed session of the immediately prior month into close-to-open and
// open-to-close log flows for both metals. Subtract silver from gold, require
// strict opposition between the two relative components, follow the relative
// session sign with opposite equal-notional legs, and hold to the next month.
// Runtime uses synchronized MT5-native OHLC/calendar/history only; no external
// data, trained logic, current-bar signal endpoint, or retry surface exists.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41039;
...
const double STRATEGY_NOTIONAL_RATIO = 1.0;
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41039 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_MFLOWDIV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41039_XAUXAG_MFLOWDIV";
...
bool Strategy_LoadMonthlyRelativeFlow(
      const int current_month_key,
      int &prior_month_bar_count,
      double &xau_overnight,
      double &xau_session,
      double &xag_overnight,
      double &xag_session,
      double &overnight_relative,
      double &session_relative,
      double &xau_month_return,
      double &xag_month_return,
      int &direction)
  {
   ...
   xau_month_return = 0.0;
   xag_month_return = 0.0;
   direction = 0;
   ...
   if(current_month_key <= 0 ||
      ...
      strategy_history_bars < strategy_max_prior_month_bars + 1)
      return false;
   ...
   const int xau_copied =
      CopyRates(g_leg_xau, // perf-allowed: bounded synchronized completed-month scan behind one monthly attempt.
                PERIOD_D1, 1, strategy_history_bars, xau_bars);
   const int xag_copied =
      CopyRates(g_leg_xag, // perf-allowed: bounded synchronized completed-month scan behind one monthly attempt.
                PERIOD_D1, 1, strategy_history_bars, xag_bars);
   const int available = MathMin(xau_copied, xag_copied);
   if(available < strategy_min_prior_month_bars + 1)
      return false;

   if(xau_bars[0].time <= 0 ||
      xau_bars[0].time != xag_bars[0].time)
      return false;
   const int prior_month_key = Strategy_MonthKey(xau_bars[0].time);
   if(prior_month_key <= 0 ||
      Strategy_MonthKey(xag_bars[0].time) != prior_month_key ||
      Strategy_NextMonthKey(prior_month_key) != current_month_key)
      return false;

   while(prior_month_bar_count < available &&
         Strategy_MonthKey(xau_bars[prior_month_bar_count].time) ==
            prior_month_key)
     {
      const int index = prior_month_bar_count;
      const MqlRates xau_bar = xau_bars[index];
      const MqlRates xag_bar = xag_bars[index];
      if(xau_bar.time <= 0 || xau_bar.time != xag_bar.time ||
         Strategy_MonthKey(xag_bar.time) != prior_month_key ||
         xau_bar.open <= 0.0 || xau_bar.close <= 0.0 ||
         xag_bar.open <= 0.0 || xag_bar.close <= 0.0 ||
         !MathIsValidNumber(xau_bar.open) ||
         !MathIsValidNumber(xau_bar.close) ||
         !MathIsValidNumber(xag_bar.open) ||
   ...
   if(prior_month_bar_count < strategy_min_prior_month_bars ||
      ...
      prior_month_bar_count >= available)
      return false;

   const MqlRates xau_anchor = xau_bars[prior_month_bar_count];
   const MqlRates xag_anchor = xag_bars[prior_month_bar_count];
   const int anchor_month_key = Strategy_MonthKey(xau_anchor.time);
   if(xau_anchor.time <= 0 || xau_anchor.time != xag_anchor.time ||
      xau_anchor.close <= 0.0 || xag_anchor.close <= 0.0 ||
      !MathIsValidNumber(xau_anchor.close) ||
      !MathIsValidNumber(xag_anchor.close) ||
      xau_bars[prior_month_bar_count - 1].time <= xau_anchor.time ||
      xag_bars[prior_month_bar_count - 1].time <= xag_anchor.time ||
      anchor_month_key <= 0 ||
      Strategy_MonthKey(xag_anchor.time) != anchor_month_key ||
      Strategy_NextMonthKey(anchor_month_key) != prior_month_key)
      return false;

   for(int index = prior_month_bar_count - 1; index >= 0; --index)
     {
      const double xau_prior_close = xau_bars[index + 1].close;
      const double xag_prior_close = xag_bars[index + 1].close;
      const double xau_open = xau_bars[index].open;
      const double xau_close = xau_bars[index].close;
      const double xag_open = xag_bars[index].open;
      const double xag_close = xag_bars[index].close;
      if(xau_prior_close <= 0.0 || xag_prior_close <= 0.0 ||
         xau_open <= 0.0 || xau_close <= 0.0 ||
         xag_open <= 0.0 || xag_close <= 0.0)
         return false;

      const double xau_open_flow = MathLog(xau_open / xau_prior_close);
      const double xau_session_flow = MathLog(xau_close / xau_open);
      const double xag_open_flow = MathLog(xag_open / xag_prior_close);
      const double xag_session_flow = MathLog(xag_close / xag_open);
   ...
   xau_month_return = MathLog(xau_bars[0].close / xau_anchor.close);
   xag_month_return = MathLog(xag_bars[0].close / xag_anchor.close);
   ...
   const double relative_total = overnight_relative + session_relative;
   const double relative_month_return =
      xau_month_return - xag_month_return;
   if(!MathIsValidNumber(xau_month_return) ||
      !MathIsValidNumber(xag_month_return) ||
      !MathIsValidNumber(xau_total) ||
      !MathIsValidNumber(xag_total) ||
      !MathIsValidNumber(overnight_relative) ||
      !MathIsValidNumber(session_relative) ||
      !MathIsValidNumber(relative_total) ||
      !MathIsValidNumber(relative_month_return) ||
      MathAbs(xau_total - xau_month_return) >
         strategy_reconcile_tolerance ||
      MathAbs(xag_total - xag_month_return) >
         strategy_reconcile_tolerance ||
      MathAbs(relative_total - relative_month_return) >
         strategy_reconcile_tolerance)
      return false;

   if(session_relative > 0.0 && overnight_relative < 0.0)
      direction = 1;
   else if(session_relative < 0.0 && overnight_relative > 0.0)
      direction = -1;
   return true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41039_XAUXAG_MFLOWDIV_HOST";
   ...
   double xau_month_return = 0.0;
   double xag_month_return = 0.0;
   const bool valid =
      Strategy_LoadMonthlyRelativeFlow(g_signal_month_key,
                                       prior_month_bars,
                                       xau_overnight,
                                       xau_session,
                                       xag_overnight,
                                       ...
                                       xag_month_return,
                                       direction);
   string state = "endpoint_or_reconciliation_failed";
   if(valid && direction > 0)
      state = "relative_opposition_long_xau";
   else if(valid && direction < 0)
      state = "relative_opposition_short_xau";
   else if(valid && (overnight_relative == 0.0 ||
                     session_relative == 0.0))
      state = "exact_zero_flat";
   else if(valid)
      state = "relative_component_agreement_flat";

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"prior_month_bars\":%d,\"xau_overnight\":%.12e,\"xau_session\":%.12e,\"xag_overnight\":%.12e,\"xag_session\":%.12e,\"overnight_relative\":%.12e,\"session_relative\":%.12e,\"xau_month_return\":%.12e,\"xag_month_return\":%.12e,\"direction\":%d,\"state\":\"%s\"}",
                            g_signal_month_key,
                            prior_month_bars,
                            xau_overnight,
                            xau_session,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG monthly flow-divergence package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41039\",\"ea\":\"xauxag-mflow-div\"}");
