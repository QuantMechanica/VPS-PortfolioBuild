...
slug: xng-wed-trend
type: strategy
strategy_id: BOROWSKI-MOP-XNG-WEDTREND-2026_S01
variant_id: BOROWSKI-MOP-XNG-WEDTREND-2026_S01
source_id: BOROWSKI-MOP-XNG-WEDTREND-2026
status: DRAFT
g0_status: APPROVED
execution_contract_ref: strategy-seeds/cards/approved/QM5_20156_xng-wed-trend_card.md
execution_contract_status: DRAFT
created: 2026-07-25
created_by: Research+Development
last_updated: 2026-07-25
source_authors: "Krzysztof Borowski; Tobias J. Moskowitz, Yao Hua Ooi, Lasse Heje Pedersen"
strategy_mechanic: wednesday-xng-short-only-when-completed-252d-return-is-negative
...
sources:
  - "[[sources/BOROWSKI-MOP-XNG-WEDTREND-2026]]"
concepts:
...
timeframe: D1
timeframes: [D1]
expected_trade_frequency: "Approximately 12-30 completed Wednesday-session XNG packages/year when the completed 252-D1 return is strictly negative; Q02 must prove or retire the density."
...
review_focus: "Falsify whether the load-bearing conjunction of XNG Wednesday weakness and a negative completed 252-D1 trend clears density and costs while adding direct natural-gas exposure."
...
`strategy-seeds/sources/BOROWSKI-MOP-XNG-WEDTREND-2026/source.md` preserves the
two completely read governed parent lineages.

Borowski supply the weak XNG Wednesday direction. Moskowitz, Ooi,
...
Monday-close-to-Wednesday-open attachment, an ATR stop, or QM portfolio
...
- A genuine Wednesday boundary requires the current D1 bar to be Wednesday and the
  immediately prior completed D1 bar to be Monday. A Wednesday holiday produces
  no shifted Wednesday trade.
- Entry uses the first executable quote observed within five minutes of the
  Wednesday D1 bar open. The Monday-close to Wednesday-open return is not captured.
...
research cards and returned `CLEAN` for slug `xng-wed-trend`, strategy ID
`BOROWSKI-MOP-XNG-WEDTREND-2026_S01`, and mechanic
`Wednesday XNG short only when completed 252-D1 return is negative`.

Manual semantic review resolved the closest systems:

- `QM5_12596_xng-mon-fade` is an unconditional Wednesday short and never reads
  the slow XNG trend.
- `QM5_12603_xng-tsmom12m` is a year-round symmetric monthly trend package
  without a weekday gate.
- `QM5_12750` and `QM5_12779` condition Wednesday trades on the observed opening
  gap and target gap fill; this card never reads or targets that gap.
- `QM5_20016_xti-xng-mon-rv` is a two-leg fixed-direction XTI/XNG Wednesday
  basket; this card is a single-symbol, trend-conditioned XNG package.
- `QM5_20029_xng-monfri-daily` rotates an unconditional Wednesday short and
  Friday long; this card has no Friday entry.
- `QM5_20141_xng-sumtrend` is a July-November weekly seasonal short rather
  than a Wednesday-session effect.
- `QM5_20145_xng-fri-trend` buys positive-trend Fridays; this card shorts
  negative-trend Wednesdays.
- `QM5_12567_cum-rsi2-commodity` is a two-day oscillator pullback.

The genuine Wednesday boundary and negative completed 252-D1 sign are jointly
...
- Timeframe: D1.
- Magic slot: 0; allocated magic `201560000`.
- Decision clock: first observed tick within five minutes of a genuine Wednesday
  D1 bar immediately following a Monday D1 bar.
- Direction: short-only, and only while completed 252-D1 log return is
  strictly negative.
- Ordinary lifecycle: close on the first new non-Wednesday D1 bar.
...
2. Evaluate entry only when the current D1 bar is broker-calendar Wednesday and
   the immediately prior completed D1 bar is Monday.
3. Require the first observed tick to occur within five minutes of the
   current Wednesday D1 bar open. A late attachment consumes no attempt and does
   not enter.
4. Derive a stable Wednesday-anchored broker week key. Persist that key as
...
10. Open at most one position for magic `201560000`; no pending order,
    same-week retry, second entry, or scale-in is authorized.

## 5. Exit Rules

1. Close the Wednesday package on the first new D1 bar whose broker weekday is
   not Wednesday.
...
4. Framework Friday close at broker hour 21 remains enabled as a fail-safe,
   although the ordinary Wednesday-boundary exit should make it unreachable.
...
- Fail closed for a non-Wednesday current D1 bar, a prior bar that is not Monday,
...
- One position for magic `201560000` and one consumed decision per broker
  week.
...
| `strategy_entry_grace_minutes` | 5 | [5] | maximum Wednesday-bar attachment delay |
...
There is no baseline parameter sweep. The Wednesday gate and negative 252-D1 sign
...
- Forced session flatten: first non-Wednesday D1 bar.

## Exit Precedence

1. Framework kill switch and the server-side hard stop.
2. First non-Wednesday D1 boundary or unexpected-long cleanup.
...
XNG Wednesdays. Moskowitz, Ooi, and Pedersen report time-series momentum across
...
- Fail on any long entry, entry outside a genuine Wednesday/Monday boundary,
...
- no_trade: exact host/D1/EA/slot, locked-input, genuine-Wednesday, grace,
  history, arithmetic, trend, spread, quote, stop, consumed-week, and
  owned-position guards.
- trade_entry: genuine Wednesday boundary, negative completed 252-D1 state, one
...
|---|---|---|---|---|
| v1 | 2026-07-25 | initial source-backed XNG Wednesday/trend interaction card | G0 | APPROVED |
...
| Q01 Build Validation | 2026-07-25 | PASS: strict compile 0 errors/0 warnings; targeted build check PASS | `docs/ops/evidence/2026-07-25_qm5_20156_xng_tue_trend_build_q02.md` |
