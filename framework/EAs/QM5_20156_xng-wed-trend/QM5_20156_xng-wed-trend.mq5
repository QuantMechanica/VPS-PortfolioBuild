...
#property description "QM5_20156 XNG Wednesday Weakness Negative-Trend Agreement"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_20156 - XNG Wednesday Weakness / Slow-Trend Agreement
// -----------------------------------------------------------------------------
// D1 structural crude-oil sleeve:
//   - first observed tick of a genuine broker-calendar Wednesday D1 bar
//   - SELL only when the completed 252-D1 XNG log return is negative
...
bool Strategy_LoadMomentum(double &momentum,
                           int &direction)
  {
   ...
   const int copied =
      CopyClose(_Symbol, // perf-allowed: bounded Wednesday D1 momentum sample.
                PERIOD_D1,
                1,
                required,
                closes);
...
void Strategy_CloseExpiredPositions()
  {
   const datetime current_bar =
      iTime(_Symbol, PERIOD_D1, 0); // perf-allowed: Wednesday D1 lifecycle gate.
   if(current_bar <= 0)
      return;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "XNG_WED_TREND";
   req.symbol_slot = qm_magic_slot_offset;
   req.expiration_seconds = 0;

   const datetime current_bar =
      iTime(_Symbol, PERIOD_D1, 0); // perf-allowed: Wednesday D1 entry calendar.
   if(!Strategy_IsGenuineWednesdayBoundary(current_bar) ||
   ...
   req.reason = "XNG_WED_NEG252_SHORT";
   return true;
  }
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_20156_WEEK_ATTEMPT_%d",
                   QM_FrameworkMagic());
   Strategy_LoadAttemptState(TimeCurrent());

   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_20156\",\"ea\":\"xng-wed-trend\"}");
