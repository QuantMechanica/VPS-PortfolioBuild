...
#property description "QM5_41205 XNG Ten-Year Same-Calendar Huber Seasonality"
...
// -----------------------------------------------------------------------------
// Structural D1 natural-gas sleeve:
...
//   - require all ten exact years, with no skip or substitution
//   - compute the even median and raw even MAD across those ten returns
//   - freeze scale=1.4826*MAD and delta=1.5*scale
//   - run exactly 32 bounded Huber reweighted-location updates
//   - follow only the final location sign beyond the inclusive epsilon band
...
double   g_signal_median          = 0.0;
double   g_signal_mad             = 0.0;
double   g_signal_scale           = 0.0;
double   g_signal_delta           = 0.0;
double   g_signal_huber_location  = 0.0;
...
// -----------------------------------------------------------------------------
// Exact completed-month endpoint and fixed-scale Huber helpers.
// -----------------------------------------------------------------------------
...
void Strategy_SortAscending(double &values[], const int count)
  {
   if(count < 1 || ArraySize(values) < count)
      return;

   for(int outer = 1; outer < count; ++outer)
     {
      if(outer >= ArraySize(values))
         return;
      const double current = values[outer];
      int inner = outer - 1;
      while(inner >= 0 && values[inner] > current)
        {
         if(inner + 1 >= ArraySize(values))
            return;
         values[inner + 1] = values[inner];
         --inner;
        }
      if(inner + 1 < 0 || inner + 1 >= ArraySize(values))
         return;
      values[inner + 1] = current;
     }
  }

bool Strategy_HuberSignal(const double &observations[],
                          ...
                          int &direction)
  {
   ...
   if(strategy_history_years != 10 ||
      ...
      MathAbs(strategy_mad_normalizer - 1.4826) > 1.0e-12 ||
      MathAbs(strategy_huber_tuning - 1.5) > 1.0e-12 ||
      strategy_huber_steps != 32 ||
      sample_count != strategy_required_observations ||
      ArraySize(observations) < sample_count)
      return false;

   double sorted_values[];
   if(ArrayResize(sorted_values, sample_count) != sample_count ||
      ArraySize(sorted_values) < sample_count)
      return false;
   for(int index = 0; index < sample_count; ++index)
     {
      if(index < 0 || index >= ArraySize(observations) ||
         index >= ArraySize(sorted_values) ||
         !MathIsValidNumber(observations[index]))
         return false;
      sorted_values[index] = observations[index];
     }

   Strategy_SortAscending(sorted_values, sample_count);
   for(int index = 1; index < sample_count; ++index)
     {
      if(index >= ArraySize(sorted_values) ||
         sorted_values[index] < sorted_values[index - 1])
         return false;
     }

   if(ArraySize(sorted_values) < 6)
      return false;
   median_value = (sorted_values[4] + sorted_values[5]) / 2.0;
   if(!MathIsValidNumber(median_value))
      return false;

   double absolute_deviations[];
   if(ArrayResize(absolute_deviations, sample_count) != sample_count ||
      ArraySize(absolute_deviations) < sample_count)
      return false;
   for(int index = 0; index < sample_count; ++index)
     {
      if(index < 0 || index >= ArraySize(observations) ||
         index >= ArraySize(absolute_deviations))
         return false;
      absolute_deviations[index] =
         MathAbs(observations[index] - median_value);
      if(!MathIsValidNumber(absolute_deviations[index]))
         return false;
     }

   Strategy_SortAscending(absolute_deviations, sample_count);
   for(int index = 1; index < sample_count; ++index)
     {
      if(index >= ArraySize(absolute_deviations) ||
         absolute_deviations[index] < absolute_deviations[index - 1])
         return false;
     }

   if(ArraySize(absolute_deviations) < 6)
      return false;
   mad_value =
   ...
   if(!MathIsValidNumber(mad_value) || mad_value <= 0.0 ||
      !MathIsValidNumber(scale_value) || scale_value <= 0.0 ||
      !MathIsValidNumber(delta_value) || delta_value <= 0.0)
      return false;

   double location = median_value;
   for(int step = 0; step < strategy_huber_steps; ++step)
     {
      ...
      for(int index = 0; index < sample_count; ++index)
        {
         if(index < 0 || index >= ArraySize(observations))
            return false;
         const double residual = MathAbs(observations[index] - location);
         ...
         if(!MathIsValidNumber(weight) || weight <= 0.0 || weight > 1.0)
            return false;
         weighted_sum += weight * observations[index];
         weight_sum += weight;
         if(!MathIsValidNumber(weighted_sum) ||
            !MathIsValidNumber(weight_sum))
            return false;
        }
      ...
      if(!MathIsValidNumber(location))
         return false;
     }

   huber_value = location;
   if(huber_value > strategy_signal_epsilon)
      direction = 1;
   else if(huber_value < -strategy_signal_epsilon)
      direction = -1;
   return true;
  }

bool Strategy_LoadHuberSignal(const datetime decision_bar_time,
                              ...
                              int &direction)
  {
   ...
   for(int offset = 1; offset <= strategy_history_years; ++offset)
     {
      const int observation_index = offset - 1;
      if(observation_index < 0 ||
         observation_index >= ArraySize(observations))
         return false;
      ...
      if(!Strategy_CompletedMonthReturn(rates,
                                        ...
                                        sample_return))
         return false;
      observations[observation_index] = sample_return;
      ++sample_count;
     }

   if(sample_count != strategy_required_observations)
      return false;

   return Strategy_HuberSignal(observations,
                               sample_count,
                               median_value,
                               mad_value,
                               scale_value,
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   // Consume before history, Huber arithmetic, news, spread, quote, ATR,
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadHuberSignal(g_decision_bar_time,
                                  ...
                                  g_signal_scale,
                                  g_signal_delta,
                                  g_signal_huber_location,
                                  g_signal_direction);
      if(!g_signal_valid)
         g_signal_state = "endpoint_exact_year_sample_median_mad_or_huber_failed";
      else if(g_signal_direction > 0)
         g_signal_state = "huber_long";
      else if(g_signal_direction < 0)
         g_signal_state = "huber_short";
      else
         g_signal_state = "huber_tie_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"valid\":%s,\"samples\":%d,\"median\":%.12g,\"mad\":%.12g,\"scale\":%.12g,\"delta\":%.12g,\"huber_location\":%.12g,\"direction\":%d,\"state\":\"%s\"}",
                            g_decision_month_key,
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(!QM_InputRequireLong("qm_ea_id", qm_ea_id, 41205) ||
      ...
      !QM_InputRequireLong("strategy_required_observations", strategy_required_observations, 10) ||
      !QM_InputRequireDouble("strategy_mad_normalizer", strategy_mad_normalizer, 1.4826, 1.0e-12) ||
      !QM_InputRequireDouble("strategy_huber_tuning", strategy_huber_tuning, 1.5, 1.0e-12) ||
      !QM_InputRequireLong("strategy_huber_steps", strategy_huber_steps, 32) ||
      !QM_InputRequireDouble("strategy_signal_epsilon", strategy_signal_epsilon, 0.000000000001, 1.0e-20) ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "XNG_SAMECAL_HUBER10";
   ...
   if(g_signal_direction > 0)
     {
      req.type = QM_BUY;
      req.reason = "XNG_SAMECAL_HUBER10_LONG";
     }
   else
     {
      req.type = QM_SELL;
      req.reason = "XNG_SAMECAL_HUBER10_SHORT";
     }
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41205 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XNG ten-year same-calendar fixed-scale Huber card holds to the next broker month"))
      return INIT_FAILED;
   ...
   g_attempt_state_key =
      StringFormat("QM5_41205_SAMECAL_HUBER10_MONTH_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41205\",\"ea\":\"xng-samecal-huber10\"}");
