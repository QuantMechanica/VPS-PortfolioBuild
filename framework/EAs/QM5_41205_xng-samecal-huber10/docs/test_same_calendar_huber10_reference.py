"""Independent deterministic fixtures for QM5_41205.

The suite covers normalized D1 month labels, strict completed-month endpoints,
the all-or-nothing Y-1..Y-10 sample, even median/MAD, frozen-scale Huber
weights, exactly 32 updates, direction separation from nearby estimators,
durable monthly attempts, lifecycle repair, and static card/build conformance.
...
class SameCalendarHuberTenReferenceTests(unittest.TestCase):
    ...
    def test_static_build_contract_matches_approved_card(self) -> None:
        source_path = EA_DIR / "QM5_41205_xng-samecal-huber10.mq5"
        source = source_path.read_text(encoding="utf-8")
        setfile = (
            EA_DIR
            / "sets"
            / "QM5_41205_xng-samecal-huber10_XNGUSD.DWX_D1_backtest.set"
        ...
        self.assertLess(
            ...
            prepare.index("Strategy_LoadHuberSignal"),
        )
        loader = source[
            source.index("bool Strategy_LoadHuberSignal") :
            source.index("void Strategy_PrepareDecisionSignal()")
        ].lower()
        ...
        approved = (
            ...
            / "cards"
            / "approved"
            / "QM5_41205_xng-samecal-huber10_card.md"
        )
        ...
        self.assertIn(
            "41205,xng-samecal-huber10,0,XNGUSD.DWX,412050000", magic_rows
        )
        resolver = (
            REPO_ROOT / "framework" / "include" / "QM" / "QM_MagicResolver.mqh"
        ).read_text(encoding="utf-8")
        self.assertIn("412050000", resolver)
