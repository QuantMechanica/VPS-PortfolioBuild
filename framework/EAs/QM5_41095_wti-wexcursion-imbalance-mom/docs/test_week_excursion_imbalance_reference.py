"""Deterministic reference checks for QM5_41095 weekly excursion imbalance."""
...
def excursion_imbalance_signal(
    ...
    """Mirror bounded weekly OHLC, strict 2:1 excursions, and settlement sign."""
...
def sample(
    completed: tuple[float, float, float, float] = (100.0, 130.0, 90.0, 115.0),
    count: int = 5,
...
class WeekExcursionImbalanceReferenceTest(unittest.TestCase):
    ...
    def test_strict_long_and_short_paths(self) -> None:
        _, _, bars = sample()
        valid, direction, count, values, imbalanced, settlement_agrees = self.signal(bars)
        self.assertTrue(valid)
        self.assertTrue(imbalanced)
        self.assertTrue(settlement_agrees)
        self.assertEqual(direction, 1)
        self.assertEqual(count, 5)
        self.assertEqual(values, (100.0, 130.0, 90.0, 115.0, 30.0, 10.0))

        _, _, bars = sample(completed=(110.0, 120.0, 80.0, 95.0))
    ...
    def test_three_four_and_five_session_packages_are_accepted(self) -> None:
        for count in (3, 4, 5):
            ...
            valid, direction, actual_count, _, imbalanced, settlement_agrees = self.signal(bars)
            self.assertTrue(valid)
            self.assertTrue(imbalanced)
            self.assertTrue(settlement_agrees)
            self.assertEqual(direction, 1)
    ...
    def test_ratio_equality_and_subthreshold_are_flat(self) -> None:
        ...
        for aggregate, expected_up, expected_down in cases:
            ...
            valid, direction, _, values, imbalanced, settlement_agrees = self.signal(bars)
            self.assertTrue(valid)
            self.assertFalse(imbalanced)
            self.assertFalse(settlement_agrees)
            self.assertEqual(direction, 0)
            self.assertEqual(values[-2:], (expected_up, expected_down))

    def test_close_equality_and_both_settlement_disagreements_are_flat(self) -> None:
        for aggregate in (
            (100.0, 130.0, 90.0, 100.0),
            (100.0, 130.0, 90.0, 95.0),
            (110.0, 120.0, 80.0, 115.0),
        ):
            _, _, bars = sample(completed=aggregate)
            valid, direction, _, _, imbalanced, settlement_agrees = self.signal(bars)
            self.assertTrue(valid)
            self.assertTrue(imbalanced)
            self.assertFalse(settlement_agrees)
            self.assertEqual(direction, 0)
    ...
    def test_static_build_contract_is_fixed_risk_and_completed_data_only(self) -> None:
        source = (EA_DIR / "QM5_41095_wti-wexcursion-imbalance-mom.mq5").read_text(
            encoding="utf-8"
        )
        setfile = (
            EA_DIR
            / "sets"
            / "QM5_41095_wti-wexcursion-imbalance-mom_XTIUSD.DWX_D1_backtest.set"
