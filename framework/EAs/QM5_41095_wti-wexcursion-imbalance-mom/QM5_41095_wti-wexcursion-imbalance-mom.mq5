...
#property description "QM5_41095 WTI Weekly Excursion-Imbalance Momentum"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41095 - WTI Weekly Excursion-Imbalance Momentum
// -----------------------------------------------------------------------------
...
//   - require the final close to agree with the dominant excursion direction
//   - consume the week before fallible gates; never enter late or retry
//   - hold the completed excursion-imbalance signal for one broker week
// Runtime uses MT5-native OHLC, calendar, history, and framework state only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                      = 41095;
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_WeekAlreadyEntered(g_decision_week_key))
   ...
   else
     {
      ...
      if(!g_signal_valid)
         ...
         g_signal_state = "up_excursion_dominant_settlement_long";
       else if(g_signal_direction < 0)
          g_signal_state = "down_excursion_dominant_settlement_short";
          ...
          g_signal_state = "excursion_settlement_disagreement_flat";
       else
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
                StringFormat("{\"week\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_week_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"week_bars\":%d,\"week_open\":%.8f,\"week_high\":%.8f,\"week_low\":%.8f,\"week_close\":%.8f,\"up_excursion\":%.8f,\"down_excursion\":%.8f,\"excursion_imbalanced\":%s,\"settlement_agrees\":%s,\"state\":\"%s\"}",
                            g_decision_week_key,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41095 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41095_WTI_WEXCURSION_IMBALANCE_MOM";
   ...
   req.reason = (g_signal_direction > 0)
       ? "WTI_WEXCURSION_MOM_LONG"
       : "WTI_WEXCURSION_MOM_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
       !Strategy_IsHostChart() || qm_ea_id != 41095 ||
   ...
   g_attempt_state_key =
      StringFormat("QM5_41095_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
                "{\"card\":\"QM5_41095\",\"ea\":\"wti-wexcursion-imbalance-mom\"}");
