...
#property description "QM5_20275 Gold Silver Fresh Run Fade"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_20275 - Gold/Silver Fresh Relative-Return Run Fade
// -----------------------------------------------------------------------------
// A two-leg D1 relative-value package. Five newest same-sign XAU/XAG log-ratio
// returns, preceded by a run break, define one fresh exhaustion event. The
// package trades the inverse relative direction and exits on the first
// completed counter-return or the fixed stale guard. Runtime data is
// Darwinex-native only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 20275;
...
input int    strategy_run_length_d1        = 5;
...
double   g_relative_returns[6];
double   g_latest_relative_return = 0.0;
int      g_signal_xau_direction = 0;
...
string Strategy_AttemptStateName()
  {
   return "QM5_20275_GSR_RUNFADE_D1_ATTEMPT";
  }
   ...
   if(GlobalVariableCheck(state_name))
     {
      ...
      if(!MathIsValidNumber(stored_value))
         return false;
      const datetime stored_bar = (datetime)MathRound(stored_value);
...
bool Strategy_LoadRatioState()
  {
   g_latest_relative_return = 0.0;
   g_signal_xau_direction = 0;
   g_signal_diagnostic = "UNSET";
   ArrayInitialize(g_relative_returns, 0.0);

   const int needed_bars = strategy_run_length_d1 + 2;
   if(strategy_run_length_d1 != 5 || needed_bars != 7)
     {
      g_signal_diagnostic = "RUN_WINDOW_INVALID";
      return false;
     }
   ...
   for(int index = 0; index < 6; ++index)
     {
      g_relative_returns[index] = ratios[index] - ratios[index + 1];
      if(!MathIsValidNumber(g_relative_returns[index]))
        {
         g_signal_diagnostic = "INVALID_RELATIVE_RETURN";
         return false;
        }
     }
   ...
   bool upper_run = true;
   bool lower_run = true;
   for(int index = 0; index < strategy_run_length_d1; ++index)
     {
      if(g_relative_returns[index] <= 0.0)
         upper_run = false;
      if(g_relative_returns[index] >= 0.0)
         lower_run = false;
     }

   if(upper_run && g_relative_returns[5] <= 0.0)
      g_signal_xau_direction = -1;
   else if(lower_run && g_relative_returns[5] >= 0.0)
      g_signal_xau_direction = 1;

   g_latest_relative_return = g_relative_returns[0];
...
bool Strategy_OpenPair(const int xau_direction)
  {
   ...
   const string reason = long_xau_short_xag
                         ? "QM5_20275_LONG_XAU_SHORT_XAG_RUNFADE"
                         : "QM5_20275_SHORT_XAU_LONG_XAG_RUNFADE";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20275_GSR_RUNFADE_HOST";
   ...
   QM_LogEvent(QM_INFO,
               "ENTRY_ATTEMPT",
               StringFormat("{\"reason\":\"GSR_RUNFADE\",\"decision_bar\":%I64d,\"d1\":%.10f,\"d2\":%.10f,\"d3\":%.10f,\"d4\":%.10f,\"d5\":%.10f,\"d6\":%.10f,\"xau_direction\":%d}",
                            (long)decision_bar_time,
                            g_relative_returns[0],
                            g_relative_returns[1],
                            g_relative_returns[2],
                            g_relative_returns[3],
                            g_relative_returns[4],
                            g_relative_returns[5],
                            g_signal_xau_direction));

   if(!Strategy_NewsAllowsEntry(TimeCurrent()))
      return false;
   if(!Strategy_OpenPair(g_signal_xau_direction))
     {
      QM_LogEvent(QM_WARN,
                  "ENTRY_REJECTED",
                  "{\"result\":\"BASKET_OPEN_FAILED\",\"reason\":\"GSR_RUNFADE\"}");
     }
   return false;
  }
   ...
   if(g_new_d1_bar)
     {
      ...
      const bool counter_return =
         (xau_direction < 0 && g_latest_relative_return <= 0.0) ||
         (xau_direction > 0 && g_latest_relative_return >= 0.0);
      if(counter_return)
        {
         QM_LogEvent(QM_INFO,
                     "STRATEGY_EXIT",
                     StringFormat("{\"reason\":\"GSR_FIRST_COUNTER_RETURN\",\"latest_relative_return\":%.10f,\"xau_direction\":%d}",
                                  g_latest_relative_return,
                                  xau_direction));
         Strategy_ClosePair(QM_EXIT_STRATEGY);
         return;
        }
...
int OnInit()
  {
   ...
   QM_BasketWarmupHistory(basket_symbols, PERIOD_D1, 100);

   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_20275\",\"ea\":\"gsr-runfade\"}");
