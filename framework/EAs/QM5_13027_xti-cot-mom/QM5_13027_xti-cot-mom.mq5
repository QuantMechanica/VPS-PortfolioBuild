...
#property description "QM5_13027 CFTC COT Friday WTI positioning momentum"

#include <QM/QM_Common.mqh>

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 13027;
...
input double strategy_min_signal_return_pct    = 1.10;
input double strategy_min_atr_return_mult      = 0.55;
input double strategy_max_signal_return_pct    = 9.0;
...
input double strategy_profit_close_atr_mult    = 1.40;
input double strategy_adverse_close_atr_mult   = 1.00;
...
bool Strategy_LoadMomentumSignal(QM_OrderType &entry_type, double &signal_return_pct)
  {
   entry_type = QM_BUY;
         ...
         if(atr_last > 0.0)
           {
            ...
            const double favorable = MathMax(0.0, strategy_profit_close_atr_mult) * atr_last;
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(strategy_max_hold_days <= 0)
      return true;
   if(strategy_profit_close_atr_mult < 0.0)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_13027_XTI_COT_FRI_MOM";
   ...
   if(!Strategy_LoadMomentumSignal(entry_type, signal_return_pct))
      return false;
   ...
   req.reason = (entry_type == QM_BUY) ? "XTI_COT_FRI_MOM_LONG" : "XTI_COT_FRI_MOM_SHORT";
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_13027\",\"ea\":\"xti-cot-mom\"}");
