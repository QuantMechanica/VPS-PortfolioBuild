...
#property description "QM5_41191 WTI Same-Calendar Signed-Rank Seasonality"
...
//   - reject epsilon-zero observations and absolute-return ties
//   - trade the centered sum of strict positive absolute-return ranks
...
int      g_signal_positive_ranks  = 0;
int      g_signal_total_ranks     = 0;
int      g_signal_score           = 0;
...
void Strategy_ResetDecisionState()
  {
   ...
   g_signal_positive_ranks = 0;
   g_signal_total_ranks = 0;
   g_signal_score = 0;
   g_signal_state = "idle";
  }
...
void Strategy_CloseExpiredPositions()
  {
   ...
   const datetime now = TimeCurrent();
   const int current_month_key =
      Strategy_CurrentNormalizedMonthKey();
...
// -----------------------------------------------------------------------------
// Exact completed-month endpoint and signed-rank helpers.
// -----------------------------------------------------------------------------
...
bool Strategy_SignedRankSignal(const double &observations[],
                               ...
                               int &direction)
  {
   ...
   for(int index = 0; index < sample_count; ++index)
     {
      if(!MathIsValidNumber(observations[index]) ||
         MathAbs(observations[index]) <= strategy_signal_epsilon)
         return false;

      for(int other = index + 1; other < sample_count; ++other)
        {
         if(!MathIsValidNumber(observations[other]) ||
            MathAbs(MathAbs(observations[index]) -
                    MathAbs(observations[other])) <=
   ...
   for(int index = 0; index < sample_count; ++index)
     {
      const double absolute_value = MathAbs(observations[index]);
      int rank = 1;
      for(int other = 0; other < sample_count; ++other)
        {
         if(MathAbs(observations[other]) < absolute_value)
            ++rank;
        }

      if(rank < 1 || rank > sample_count || rank_seen[rank])
         return false;
      rank_seen[rank] = true;
      total_rank_sum += rank;
   ...
   const int expected_total = sample_count * (sample_count + 1) / 2;
   for(int rank = 1; rank <= sample_count; ++rank)
     {
      if(!rank_seen[rank])
         return false;
     }
   if(total_rank_sum != expected_total ||
      positive_rank_sum < 0 || positive_rank_sum > expected_total)
      return false;

   centered_score = 2 * positive_rank_sum - expected_total;
   if(centered_score < -expected_total ||
      centered_score > expected_total ||
      (centered_score + expected_total) % 2 != 0)
      return false;

   if(centered_score > 0)
      direction = 1;
   else if(centered_score < 0)
      direction = -1;
   return true;
  }

bool Strategy_LoadSignedRankSignal(const datetime decision_bar_time,
                                   ...
                                   int &direction)
  {
   ...
   return Strategy_SignedRankSignal(observations,
                                    sample_count,
                                    positive_rank_sum,
                                    total_rank_sum,
                                    centered_score,
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   // Consume before history, ranks, news, spread, quote, ATR, sizing, or order
   // gates. A restart or fallible gate can never create a later retry.
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadSignedRankSignal(g_decision_bar_time,
                                       g_decision_month_key,
                                       g_decision_label_offset,
                                       g_signal_sample_count,
                                       g_signal_positive_ranks,
                                       g_signal_total_ranks,
                                       g_signal_score,
                                       g_signal_direction);
      if(!g_signal_valid)
         g_signal_state = "endpoint_sample_zero_tie_or_rank_failed";
      else if(g_signal_direction > 0)
         g_signal_state = "signed_rank_long";
      else if(g_signal_direction < 0)
         g_signal_state = "signed_rank_short";
      else
         g_signal_state = "signed_rank_tie_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"valid\":%s,\"samples\":%d,\"v_plus\":%d,\"rank_total\":%d,\"score\":%d,\"direction\":%d,\"state\":\"%s\"}",
                            g_decision_month_key,
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(!QM_InputRequireLong("qm_ea_id", qm_ea_id, 41191) ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(g_signal_direction > 0)
     {
      req.type = QM_BUY;
      req.reason = "WTI_SAMECAL_SRANK_LONG";
     }
   else
     {
      req.type = QM_SELL;
      req.reason = "WTI_SAMECAL_SRANK_SHORT";
     }
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41191 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI signed-rank same-calendar card holds to the next broker month"))
      return INIT_FAILED;
   ...
   g_attempt_state_key =
      StringFormat("QM5_41191_SAMECAL_SRANK_MONTH_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41191\",\"ea\":\"wti-samecal-srank\"}");
