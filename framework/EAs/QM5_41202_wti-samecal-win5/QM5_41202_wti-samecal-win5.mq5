...
#property description "QM5_41202 WTI Five-Year Same-Calendar Winsorized Seasonality"
...
//   - reconstruct the upcoming calendar month's completed return in Y-1..Y-5
//   - require all five exact years, with no skip or substitution
//   - sort five returns, cap each extreme at its nearest retained boundary
//   - average the five retained terms with exact weights 2,1,2 and divisor 5
...
input int    strategy_required_observations = 5;
...
double   g_signal_raw_min         = 0.0;
double   g_signal_boundary_low    = 0.0;
double   g_signal_median          = 0.0;
double   g_signal_boundary_high   = 0.0;
double   g_signal_raw_max         = 0.0;
double   g_signal_winsor_mean     = 0.0;
...
void Strategy_ResetDecisionState()
  {
   ...
   g_signal_raw_min = 0.0;
   g_signal_boundary_low = 0.0;
   g_signal_median = 0.0;
   g_signal_boundary_high = 0.0;
   g_signal_raw_max = 0.0;
   g_signal_winsor_mean = 0.0;
   g_signal_state = "idle";
  }
...
// -----------------------------------------------------------------------------
// Exact completed-month endpoint and Winsorized helpers.
// -----------------------------------------------------------------------------
...
bool Strategy_WinsorizedSignal(const double &observations[],
                               ...
                               int &direction)
  {
   ...
   for(int index = 0; index < sample_count; ++index)
     {
      if(!MathIsValidNumber(observations[index]))
         return false;
   ...
   boundary_low = sorted_values[1];
   median_value = sorted_values[2];
   boundary_high = sorted_values[3];
   raw_max = sorted_values[4];
   if(!MathIsValidNumber(raw_min) ||
      !MathIsValidNumber(boundary_low) ||
      !MathIsValidNumber(median_value) ||
      !MathIsValidNumber(boundary_high) ||
      !MathIsValidNumber(raw_max) ||
      raw_min > boundary_low || boundary_low > median_value ||
      median_value > boundary_high || boundary_high > raw_max)
      return false;

   double winsorized[];
   if(ArrayResize(winsorized, sample_count) != sample_count)
      return false;
   for(int index = 0; index < sample_count; ++index)
      winsorized[index] = sorted_values[index];
   ...
   for(int index = 0; index < sample_count; ++index)
     {
      if(!MathIsValidNumber(winsorized[index]))
         return false;
      sum += winsorized[index];
     }

   winsor_value = sum / (double)sample_count;
   const double locked_formula =
      (2.0 * boundary_low + median_value + 2.0 * boundary_high) / 5.0;
   if(!MathIsValidNumber(sum) ||
      !MathIsValidNumber(winsor_value) ||
      !MathIsValidNumber(locked_formula) ||
      MathAbs(winsor_value - locked_formula) > 1.0e-15)
      return false;

   if(winsor_value > strategy_signal_epsilon)
      direction = 1;
   else if(winsor_value < -strategy_signal_epsilon)
      direction = -1;
   return true;
  }

bool Strategy_LoadWinsorizedSignal(const datetime decision_bar_time,
                                   ...
                                   int &direction)
  {
   ...
   for(int offset = 1; offset <= strategy_history_years; ++offset)
     {
      ...
      if(!Strategy_CompletedMonthReturn(rates,
                                        ...
                                        sample_return))
         return false;
      observations[offset - 1] = sample_return;
      ++sample_count;
     }
   ...
   return Strategy_WinsorizedSignal(observations,
                                    sample_count,
                                    raw_min,
                                    boundary_low,
                                    median_value,
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   // Consume before history, Winsorized arithmetic, news, spread, quote, ATR,
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadWinsorizedSignal(g_decision_bar_time,
                                       g_decision_month_key,
                                       g_decision_label_offset,
                                       g_signal_sample_count,
                                       g_signal_raw_min,
                                       g_signal_boundary_low,
                                       g_signal_median,
                                       g_signal_boundary_high,
                                       g_signal_raw_max,
                                       g_signal_winsor_mean,
                                       g_signal_direction);
      if(!g_signal_valid)
         g_signal_state = "endpoint_exact_year_sample_sort_cap_or_mean_failed";
      else if(g_signal_direction > 0)
         g_signal_state = "winsorized_long";
      else if(g_signal_direction < 0)
         g_signal_state = "winsorized_short";
      else
         g_signal_state = "winsorized_tie_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"valid\":%s,\"samples\":%d,\"raw_min\":%.12g,\"boundary_low\":%.12g,\"median\":%.12g,\"boundary_high\":%.12g,\"raw_max\":%.12g,\"winsor_mean\":%.12g,\"direction\":%d,\"state\":\"%s\"}",
                            g_decision_month_key,
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(!QM_InputRequireLong("qm_ea_id", qm_ea_id, 41202) ||
      ...
      !QM_InputRequireLong("strategy_required_observations", strategy_required_observations, 5) ||
      !QM_InputRequireLong("strategy_winsor_tail_count", strategy_winsor_tail_count, 1) ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(g_signal_direction > 0)
     {
      req.type = QM_BUY;
      req.reason = "WTI_SAMECAL_WIN5_LONG";
     }
   else
     {
      req.type = QM_SELL;
      req.reason = "WTI_SAMECAL_WIN5_SHORT";
     }
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41202 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI five-year same-calendar Winsorized card holds to the next broker month"))
      return INIT_FAILED;
   ...
   g_attempt_state_key =
      StringFormat("QM5_41202_SAMECAL_WIN5_MONTH_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41202\",\"ea\":\"wti-samecal-win5\"}");
