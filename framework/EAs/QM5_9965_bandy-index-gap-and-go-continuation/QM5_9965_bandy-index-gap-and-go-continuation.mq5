...
#property description "QM5_9965 Bandy Index Gap-and-Go Continuation"

#include <QM/QM_Common.mqh>

// =============================================================================
// QuantMechanica V5 EA: QM5_9965 bandy-index-gap-and-go-continuation
// -----------------------------------------------------------------------------
// Source: Howard B. Bandy, "Quantitative Technical Analysis", Blue Owl Press,
// 2015, ISBN 978-0-9791037-7-1. Card:
// artifacts/cards_approved/QM5_9965_bandy-index-gap-and-go-continuation.md
//
// Entry (evaluated at next D1 open after gap bar closes):
//   gap = open[1] - close[2]
//   Significance: |gap| >= 0.5 * ATR(14)[2]
//   Long entry:  gap > 0 AND close[1] > open[1] AND close[1] > SMA(200)[1]
//   Short entry: gap < 0 AND close[1] < open[1] AND close[1] < SMA(200)[1]
//   Anti-cluster: 2 D1 bars between same-direction entries
// Exit:
//   Opposite-direction bar close (BUY: close[1] < open[1], SELL: close[1] > open[1])
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   // Long: up-gap (gap > 0) + bullish close on gap bar (bar1.close > bar1.open) + above regime SMA
   const bool long_signal = (gap > 0.0 && bar1.close > bar1.open && bar1.close > regime_sma);

   // Short: down-gap (gap < 0) + bearish close on gap bar (bar1.close < bar1.open) + below regime SMA
   ...
   if(long_signal && !short_signal)
     {
      ...
      req.sl     = sl;
      req.tp     = 0.0;
      req.reason = "BANDY_INDEX_GAP_AND_GO_LONG";
   ...
   if(short_signal && !long_signal)
     {
      ...
      req.sl     = sl;
      req.tp     = 0.0;
      req.reason = "BANDY_INDEX_GAP_AND_GO_SHORT";
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      // Opposite-direction bar close on completed bar 1
      MqlRates bar1;
      if(!QM_ReadBar(_Symbol, PERIOD_D1, 1, bar1))
         continue;
      ...
      if(pos_type == POSITION_TYPE_BUY)
        {
         // Long exit: first D1 close where close < open
         if(bar1.close < bar1.open)
            return true;
        }
      else if(pos_type == POSITION_TYPE_SELL)
        {
         // Short exit: first D1 close where close > open
         if(bar1.close > bar1.open)
            return true;
        }
     }
