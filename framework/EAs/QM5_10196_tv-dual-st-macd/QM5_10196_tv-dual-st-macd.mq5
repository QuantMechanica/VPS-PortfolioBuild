...
#property description "QM5_10196 TradingView Dual Supertrend MACD"
...
input int    strategy_st1_atr_period    = 10;
input double strategy_st1_factor        = 3.0;
input int    strategy_st2_atr_period    = 20;
input double strategy_st2_factor        = 5.0;
input int    strategy_macd_fast         = 12;
input int    strategy_macd_slow         = 26;
input int    strategy_macd_signal       = 9;
input int    strategy_stop_atr_period   = 14;
input double strategy_stop_atr_mult     = 2.0;
input int    strategy_supertrend_bars   = 220;
...
int QM10196_CurrentPositionDirection()
  {
   const int magic = QM_FrameworkMagic();
   if(magic <= 0)
      return 0;
...
int QM10196_ClosedBarSignal()
  {
   ...
   const double macd_main = QM_MACD_Main(_Symbol,
                                         ...
                                         1);
   const double macd_sig = QM_MACD_Signal(_Symbol,
                                          (ENUM_TIMEFRAMES)_Period,
                                          strategy_macd_fast,
                                          strategy_macd_slow,
                                          strategy_macd_signal,
                                          1);
   const double hist = macd_main - macd_sig;

   if(st1 > 0 && st2 > 0 && hist > 0.0)
      return 1;
   if(st1 < 0 && st2 < 0 && hist < 0.0)
      return -1;
   return 0;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.type = (g_qm10196_cached_signal > 0) ? QM_BUY : QM_SELL;
   req.price = 0.0;
   const double entry = QM_EntryMarketPrice(req.type);
   req.sl = QM_StopATR(_Symbol,
                       req.type,
                       entry,
                       strategy_stop_atr_period,
                       strategy_stop_atr_mult);
   if(req.sl <= 0.0)
      return false;

   req.reason = (g_qm10196_cached_signal > 0)
                ? "dual_supertrend_macd_long"
                : "dual_supertrend_macd_short";
   return true;
  }
