...
#property description "QM5_11827 Carter M5 S17 EMA38 MACD Stoch PSAR SD"
...
input int    strategy_slow_ema_period     = 8;
...
input double strategy_stoch_long_ceiling  = 80.0;
...
input int    strategy_stddev_avg_bars     = 20;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(strategy_fast_ema_period <= 0 ||
      ...
      strategy_stddev_period <= 0 ||
      strategy_stddev_avg_bars <= 0)
      return false;

   const double ema_fast_1 = QM_EMA(_Symbol, PERIOD_M5, strategy_fast_ema_period, 1);
   const double ema_slow_1 = QM_EMA(_Symbol, PERIOD_M5, strategy_slow_ema_period, 1);
   const double ema_fast_2 = QM_EMA(_Symbol, PERIOD_M5, strategy_fast_ema_period, 2);
   const double ema_slow_2 = QM_EMA(_Symbol, PERIOD_M5, strategy_slow_ema_period, 2);
   if(ema_fast_1 <= 0.0 || ema_slow_1 <= 0.0 || ema_fast_2 <= 0.0 || ema_slow_2 <= 0.0)
      return false;

   const double macd_main = QM_MACD_Main(_Symbol, PERIOD_M5, strategy_macd_fast, strategy_macd_slow, strategy_macd_signal, 1);
   const double macd_sig = QM_MACD_Signal(_Symbol, PERIOD_M5, strategy_macd_fast, strategy_macd_slow, strategy_macd_signal, 1);
   ...
   const bool long_cross = (ema_fast_1 > ema_slow_1 && ema_fast_2 <= ema_slow_2);
   const bool short_cross = (ema_fast_1 < ema_slow_1 && ema_fast_2 >= ema_slow_2);
   ...
   if(long_cross && macd_hist > 0.0 && stoch_k > stoch_d && stoch_k < strategy_stoch_long_ceiling)
     {
      ...
      const double tp = QM_TakeATR(_Symbol, QM_BUY, entry, strategy_atr_period, strategy_atr_tp_mult);
      if(sl <= 0.0 || tp <= 0.0)
         return false;
      req.type = QM_BUY;
      req.price = 0.0;
      req.sl = sl;
      req.tp = tp;
      req.reason = "EMA3_8_MACD_STOCH_PSAR_SD_LONG";
      return true;
     }
   ...
   if(short_cross && macd_hist < 0.0 && stoch_k < stoch_d && stoch_k > strategy_stoch_short_floor)
     {
      ...
      const double tp = QM_TakeATR(_Symbol, QM_SELL, entry, strategy_atr_period, strategy_atr_tp_mult);
      if(sl <= 0.0 || tp <= 0.0)
         return false;
      req.type = QM_SELL;
      req.price = 0.0;
      req.sl = sl;
      req.tp = tp;
      req.reason = "EMA3_8_MACD_STOCH_PSAR_SD_SHORT";
      return true;
     }
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      if(pos_type == POSITION_TYPE_BUY && bid > 0.0 && psar < bid && (current_sl <= 0.0 || psar > current_sl))
         QM_TM_MoveSL(ticket, psar, "PSAR_TRAIL_PROFIT");
      else if(pos_type == POSITION_TYPE_SELL && ask > 0.0 && psar > ask && (current_sl <= 0.0 || psar < current_sl))
         QM_TM_MoveSL(ticket, psar, "PSAR_TRAIL_PROFIT");
     }
  }
