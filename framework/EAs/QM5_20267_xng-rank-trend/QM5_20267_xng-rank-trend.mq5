...
#property description "QM5_20267 XNG Pairwise Rank Trend"
...
//   2. compare all 78 older/newer endpoint pairs and reject exact ties;
//   3. trade the Mann-Kendall score sign only when abs(S) is at least 28;
//   4. consume the month flat when the rank path is weak or unavailable.
// The ordinal path-quality gate is a transparent QM mechanization, not an
// author rule. Runtime uses native XNGUSD.DWX price and broker data only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 20267;
...
int    g_cache_rank_score       = 0;
int    g_cache_pair_count       = 0;
double g_cache_kendall_tau      = 0.0;
...
bool Strategy_LoadMonthlyCloses(double &closes[])
  {
   ...
   if(strategy_rank_points != 13 || strategy_history_bars_d1 != 800)
      return false;
   ...
   const int required_closes = strategy_rank_points;
...
void Strategy_ResetCachedState()
  {
   ...
   g_cache_signal = 0;
   g_cache_rank_score = 0;
   g_cache_pair_count = 0;
   g_cache_kendall_tau = 0.0;
   g_cache_state_reason = "not_evaluated";
  }

bool Strategy_RankSignal(const double &closes[], int &signal)
  {
   signal = 0;
   if(ArraySize(closes) != 13 || strategy_rank_points != 13)
     {
   ...
   int score = 0;
   int pair_count = 0;
   for(int i = 0; i < strategy_rank_points; ++i)
     {
      ...
      for(int j = i + 1; j < strategy_rank_points; ++j)
        {
         ...
         if(closes[j] == closes[i])
           {
            g_cache_state_reason = "tied_endpoint";
            return false;
           }
   ...
   if(pair_count != 78 || score < -78 || score > 78 || (score % 2) != 0)
     {
      g_cache_state_reason = "invalid_pair_score";
      return false;
     }

   const double tau = (double)score / (double)pair_count;
   if(!MathIsValidNumber(tau) || tau < -1.0 || tau > 1.0)
     {
      g_cache_state_reason = "invalid_rank_state";
      return false;
     }

   g_cache_rank_score = score;
   g_cache_pair_count = pair_count;
   g_cache_kendall_tau = tau;
   if(MathAbs(score) < strategy_min_abs_score)
     {
      g_cache_state_reason = "weak_rank_trend";
      return true;
     }
...
bool Strategy_LoadSignalState(int &signal)
  {
   ...
   return Strategy_RankSignal(monthly_closes, signal);
  }
...
void Strategy_PrepareMonthlySignal()
  {
   ...
   QM_LogEvent(QM_INFO,
               "MONTHLY_STATE",
               StringFormat("{\"month\":%d,\"valid\":%s,\"signal\":%d,\"rank_score\":%d,\"pairs\":%d,\"tau\":%.8f,\"state\":\"%s\"}",
                            g_cache_month_key,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 20267 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20267_XNG_RANK_TREND";
   ...
   req.reason = (g_cache_signal > 0) ? "RANK_TREND_XNG_LONG" : "RANK_TREND_XNG_SHORT";
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_20267_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_20267\",\"ea\":\"xng-rank-trend\"}");
