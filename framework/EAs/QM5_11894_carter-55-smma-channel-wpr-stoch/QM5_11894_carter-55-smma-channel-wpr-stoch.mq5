...
#property description "QM5_11894 Carter 55-SMMA Channel WPR Stoch"
...
input double strategy_wpr_upper_level   = -25.0;
input double strategy_wpr_lower_level   = -75.0;
input int    strategy_wpr_cross_lookback = 3;
...
input int    strategy_structure_buffer_pips = 2;
input double strategy_target_rr         = 2.0;
input int    strategy_timeout_bars      = 120;
...
bool Strategy_NoTradeFilter()
  {
   // Card declares no strategy-specific time/session/spread filter.
   return false;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   const double stoch_d_1 = QM_Stoch_D(_Symbol, PERIOD_H1, strategy_stoch_k, strategy_stoch_d, strategy_stoch_slowing, 1);
...
void Strategy_ManageOpenPosition()
  {
   // Card declares no trailing, break-even, partial close, or scale-in logic.
  }
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      if(strategy_timeout_bars > 0 && seconds_per_bar > 0 && opened > 0)
        {
      ...
      if(close_1 <= 0.0 || smma_high_1 <= 0.0 || smma_low_1 <= 0.0)
         continue;
      if(pos_type == POSITION_TYPE_BUY && close_1 < smma_high_1)
         return true;
      if(pos_type == POSITION_TYPE_SELL && close_1 > smma_low_1)
         return true;
     }
