...
`artifacts/cards_approved/QM5_10118_tv-rsi-trend-cont.md`. See that card's body for
...
`QM5_10118_tv-rsi-trend-cont.mq5`. Framework wiring (risk, magic, news, Friday close)
...
| `strategy_rsi_min` | 50.0 | (see source) | (see strategy logic) |
...
| `strategy_stoch_max` | 80.0 | (see source) | (see strategy logic) |
| `strategy_ema_trend_period` | 200 | (see source) | (see strategy logic) |
| `strategy_ema_exit_period` | 20 | (see source) | (see strategy logic) |
| `strategy_atr_period` | 14 | (see source) | (see strategy logic) |
| `strategy_atr_sl_mult` | 1.75 | (see source) | (see strategy logic) |
| `strategy_profit_atr_mult` | 2.25 | (see source) | (see strategy logic) |
...
| Cadence note | "Source reports 111 BTC trades over ~19 months on 2H; ported filtered long-only estimate 45-80 trades/year/symbol." |
...
`artifacts/cards_approved/QM5_10118_tv-rsi-trend-cont.md`
