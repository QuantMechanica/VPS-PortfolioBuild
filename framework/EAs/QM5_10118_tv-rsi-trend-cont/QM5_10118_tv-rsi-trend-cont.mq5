...
input double strategy_rsi_min           = 50.0;
...
input double strategy_stoch_max         = 80.0;
input int    strategy_ema_trend_period  = 200;
input int    strategy_ema_exit_period   = 20;
input int    strategy_atr_period        = 14;
input double strategy_atr_sl_mult       = 1.75;
input double strategy_profit_atr_mult   = 2.25;
...
bool Strategy_NoTradeFilter()
  {
   if(strategy_atr_period <= 0 || strategy_atr_sl_mult <= 0.0 || strategy_max_spread_frac < 0.0)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(strategy_rsi_period <= 0 || strategy_macd_fast <= 0 || strategy_macd_slow <= 0 ||
      strategy_macd_signal <= 0 || strategy_stoch_k_period <= 0 ||
      strategy_stoch_d_period <= 0 || strategy_stoch_slowing <= 0 ||
      strategy_ema_trend_period <= 0 || strategy_atr_period <= 0 ||
   ...
   const double macd_main = QM_MACD_Main(_Symbol, tf, strategy_macd_fast, strategy_macd_slow,
                                         strategy_macd_signal, 1, PRICE_CLOSE);
   const double macd_signal = QM_MACD_Signal(_Symbol, tf, strategy_macd_fast, strategy_macd_slow,
                                             strategy_macd_signal, 1, PRICE_CLOSE);
   ...
   if(rsi <= strategy_rsi_min)
      return false;
   if(macd_main <= macd_signal)
      return false;
   if(stoch_k > strategy_stoch_max || stoch_d > strategy_stoch_max)
      return false;
   if(low <= ema200)
      return false;
   ...
   req.sl = sl;
   req.reason = "TV_RSI_TREND_CONT_LONG";
   return true;
  }
...
void Strategy_ManageOpenPosition()
  {
   if(strategy_atr_period <= 0 || strategy_ema_exit_period <= 0 || strategy_profit_atr_mult <= 0.0)
      return;
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      if(close >= entry + strategy_profit_atr_mult * atr)
        {
         const string key = StringFormat("QM5_10118_TRAIL_%I64u", ticket);
