"""Deterministic reference checks for QM5_41460 WTI summer NR2 down-week continuation."""
...
class WtiSummerNR2DownWeekContinuationReferenceTest(unittest.TestCase):
    current = date(2026, 7, 13)
    new = current - timedelta(days=7)
    old = current - timedelta(days=14)

    def bars(self, close: float = 98.0, new_range: float = 8.0,
    ...
    def test_contraction_negative_week_is_short(self) -> None:
    ...
    def test_three_to_five_sessions_are_valid(self) -> None:
        for count in (3, 4, 5):
            newest = package(self.new, count, 100, 104, 96, 98)
