# QM5_41023_wti-mends-mom - Strategy Spec

**EA ID:** QM5_41023  
**Slug:** `wti-mends-mom`  
**Strategy ID:** `MOP-WTI-MENDS-MOM-2026_S01`  
**Source:** `MOP-WTI-MENDS-MOM-2026`  
**Author:** Codex  
...
reconstructs the immediately completed month. It measures the return from the
preceding month-end through the fifth chronological prior-month close and the
return across the prior month's final five close-to-close intervals. It enters
only when both non-overlapping segment signs strictly agree: long for two
positive returns and short for two negative returns. Disagreement or exact
...
|---|---:|---|
| `strategy_opening_sessions` | 5 | prior-month opening segment |
| `strategy_closing_intervals` | 5 | prior-month final intervals |
| `strategy_min_prior_month_bars` | 15 | enforces segment separation |
...
| `strategy_history_bars` | 90 | bounded two-month endpoint scan |
...
- Formation: the completed prior month's first and final five-session
  segments, with no shared return interval.
- Hold: first five completed D1 bars of the entry broker month.

## 5. Expected Behaviour

- Approximately 5-8 positions per full post-warm-up year after strict sign
  agreement.
...
`strategy-seeds/sources/MOP-WTI-MENDS-MOM-2026/source.md`.
