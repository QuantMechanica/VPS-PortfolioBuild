...
#property description "QM5_1232 Carver cost-conditioned fast momentum"

#include <QM/QM_Common.mqh>

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 1232;
...
input double strategy_max_annual_cost_sr = 0.13;
...
#define QM5_1232_SYMBOL_COUNT 12
#define QM5_1232_VARIANT_COUNT 6
...
int g_fast[QM5_1232_VARIANT_COUNT] = {2, 4, 8, 16, 32, 64};
int g_slow[QM5_1232_VARIANT_COUNT] = {8, 16, 32, 64, 128, 256};
double g_turnover[QM5_1232_VARIANT_COUNT] = {25.0, 18.0, 12.0, 8.0, 5.0, 3.0};
...
int Strategy_CurrentSymbolIndex()
  {
   for(int i = 0; i < QM5_1232_SYMBOL_COUNT; ++i)
...
bool Strategy_EwmacAtShift(const int fast, const int slow, const int signal_shift, double &out_forecast)
  {
   out_forecast = 0.0;
   const int warmup = slow + 30;
   const int min_bars = signal_shift + warmup + strategy_atr_period + 2;
   ...
   const double sigma = Strategy_CloseChangeStdDev(signal_shift, strategy_atr_period);
   if(sigma <= 0.0)
      return false;

   const double raw = (fast_ema - slow_ema) / sigma;
   out_forecast = Strategy_Clamp(strategy_forecast_scalar * raw,
...
bool Strategy_ForecastAtShift(const int signal_shift, double &out_forecast)
  {
   out_forecast = 0.0;
   const double atr = QM_ATR(_Symbol, PERIOD_D1, strategy_atr_period, signal_shift);
   const double median_spread_points = Strategy_MedianSpreadPoints(strategy_spread_median_days);
   if(atr <= 0.0 || median_spread_points <= 0.0)
      return false;

   const double cost_per_trade_sr = MathMax(0.0001, (median_spread_points * _Point) / atr);
   const double max_forecast_turnover = strategy_max_annual_cost_sr / cost_per_trade_sr;

   double sum = 0.0;
   int included = 0;
   for(int i = 0; i < QM5_1232_VARIANT_COUNT; ++i)
     {
      if(g_turnover[i] > max_forecast_turnover)
         continue;

      double forecast = 0.0;
      if(!Strategy_EwmacAtShift(g_fast[i], g_slow[i], signal_shift, forecast))
         continue;
   ...
   out_forecast = Strategy_Clamp(sum / (double)included,
                                 -MathAbs(strategy_forecast_cap),
                                 MathAbs(strategy_forecast_cap));
   return true;
  }
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(Strategy_SlotForCurrentSymbol() != qm_magic_slot_offset)
      return true;
   if(strategy_atr_period < 2 || strategy_spread_median_days < 2)
      return true;
   if(strategy_max_annual_cost_sr <= 0.0 || strategy_entry_forecast <= 0.0)
      return true;
   if(strategy_forecast_scalar <= 0.0 || strategy_forecast_cap <= 0.0 || strategy_stop_atr_mult <= 0.0)
      return true;
   return false;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "CARVER_FASTMOM_COST";
   ...
   req.reason = long_signal ? "FASTMOM_COST_LONG" : "FASTMOM_COST_SHORT";
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_1232\",\"ea\":\"carver-fastmom-cost\"}");
