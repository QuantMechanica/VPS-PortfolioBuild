...
#property description "QM5_1401 Harmonic Shark XABCD H4"

#include <QM/QM_Common.mqh>

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                     = 1401;
...
input int    strategy_min_od_bars              = 25;
input int    strategy_max_od_bars              = 80;
...
input double strategy_ox_d_min                = 0.886;
input double strategy_ox_d_max                = 1.130;
input int    strategy_atr_period               = 14;
input double strategy_sl_atr_mult              = 0.5;
input double strategy_sl_cap_atr_mult          = 2.5;
input double strategy_tp1_cd_retracement       = 0.382;
input double strategy_tp2_cd_retracement       = 0.618;
input double strategy_tp1_close_fraction       = 0.50;
input bool   strategy_macro_bias_enabled       = true;
input int    strategy_macro_fast_sma           = 50;
input int    strategy_macro_slow_sma           = 200;
...
bool Strategy_RatioInRange(const double value, const double lo, const double hi)
  {
   return (value >= (lo - strategy_fib_tolerance_pct) && value <= (hi + strategy_fib_tolerance_pct));
  }

bool Strategy_MacroOK(const bool bullish)
  {
   if(!strategy_macro_bias_enabled)
      return true;

   const double fast = QM_SMA(_Symbol, PERIOD_H4, strategy_macro_fast_sma, 1, PRICE_CLOSE);
   const double slow = QM_SMA(_Symbol, PERIOD_H4, strategy_macro_slow_sma, 1, PRICE_CLOSE);
   if(fast <= 0.0 || slow <= 0.0)
      return false;

   return bullish ? (fast > slow) : (fast < slow);
  }
...
bool Strategy_BuildEntry(const bool bullish, const double c_price,
                         const double d_price, const double atr, QM_EntryRequest &req)
  {
   ...
   req.reason = bullish ? "HARMONIC_SHARK_BULLISH" : "HARMONIC_SHARK_BEARISH";
...
bool Strategy_CheckBullish(const StrategyPivot &o, const StrategyPivot &x,
                           const StrategyPivot &a, const StrategyPivot &b,
                           const StrategyPivot &c, const MqlRates &d_bar,
                           QM_EntryRequest &req)
  {
   if(o.kind != +1 || x.kind != -1 || a.kind != +1 || b.kind != -1 || c.kind != +1)
      return false;
   if(o.shift < strategy_min_od_bars || o.shift > strategy_max_od_bars)
      return false;
   // Invalidation guard: C must not exceed A
   if(c.price > a.price)
      return false;
   if(x.price >= o.price || a.price <= x.price || b.price >= a.price || c.price <= b.price)
      return false;

   const double ox = MathAbs(o.price - x.price);
   const double xa = MathAbs(a.price - x.price);
   const double ab = MathAbs(a.price - b.price);
   const double bc = MathAbs(c.price - b.price);
   if(ox <= 0.0 || xa <= 0.0 || ab <= 0.0 || bc <= 0.0)
      return false;
   ...
   const double d_0886 = o.price - strategy_ox_d_min * ox;
   const double d_1130 = o.price - strategy_ox_d_max * ox;
   const double zone_low = d_1130 - strategy_fib_tolerance_pct * ox;
   const double zone_high = d_0886 + strategy_fib_tolerance_pct * ox;

   const bool low_touched = (d_bar.low >= zone_low && d_bar.low <= zone_high);
   const bool bullish_bar = (d_bar.close > d_bar.open);
   const bool close_inside_ox = (d_bar.close >= x.price && d_bar.close <= o.price);
   if(!low_touched || !bullish_bar || !close_inside_ox)
      return false;
   if(!Strategy_MacroOK(true))
      return false;

   const double atr = QM_ATR(_Symbol, PERIOD_H4, strategy_atr_period, 1);
   return Strategy_BuildEntry(true, c.price, d_bar.low, atr, req);
  }

bool Strategy_CheckBearish(const StrategyPivot &o, const StrategyPivot &x,
                           const StrategyPivot &a, const StrategyPivot &b,
                           const StrategyPivot &c, const MqlRates &d_bar,
                           QM_EntryRequest &req)
  {
   ...
   if(o.shift < strategy_min_od_bars || o.shift > strategy_max_od_bars)
      return false;
   // Invalidation guard: C must not exceed A downwards
   if(c.price < a.price)
      return false;
   if(x.price <= o.price || a.price >= x.price || b.price <= a.price || c.price >= b.price)
      return false;

   const double ox = MathAbs(x.price - o.price);
   const double xa = MathAbs(x.price - a.price);
   const double ab = MathAbs(b.price - a.price);
   const double bc = MathAbs(b.price - c.price);
   if(ox <= 0.0 || xa <= 0.0 || ab <= 0.0 || bc <= 0.0)
      return false;
   ...
   const double d_0886 = o.price + strategy_ox_d_min * ox;
   const double d_1130 = o.price + strategy_ox_d_max * ox;
   const double zone_low = d_0886 - strategy_fib_tolerance_pct * ox;
   const double zone_high = d_1130 + strategy_fib_tolerance_pct * ox;

   const bool high_touched = (d_bar.high >= zone_low && d_bar.high <= zone_high);
   const bool bearish_bar = (d_bar.close < d_bar.open);
   const bool close_inside_ox = (d_bar.close >= o.price && d_bar.close <= x.price);
   if(!high_touched || !bearish_bar || !close_inside_ox)
      return false;
   if(!Strategy_MacroOK(false))
      return false;

   const double atr = QM_ATR(_Symbol, PERIOD_H4, strategy_atr_period, 1);
   return Strategy_BuildEntry(false, c.price, d_bar.high, atr, req);
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   const int bars_needed = (strategy_scan_bars > strategy_max_od_bars + 8) ? strategy_scan_bars : strategy_max_od_bars + 8;
   MqlRates rates[];
   ArraySetAsSeries(rates, true);
   const int copied = CopyRates(_Symbol, PERIOD_H4, 1, bars_needed, rates);
   if(copied < strategy_min_od_bars + 6)
      return false;
   ...
   for(int i = pivot_count - 5; i >= 0; --i)
     {
      if(Strategy_CheckBullish(pivots[i], pivots[i + 1], pivots[i + 2], pivots[i + 3], pivots[i + 4], rates[0], req))
        {
         g_reuse_until_time = TimeCurrent() + strategy_reuse_guard_bars * PeriodSeconds(PERIOD_H4);
         return true;
        }
      if(Strategy_CheckBearish(pivots[i], pivots[i + 1], pivots[i + 2], pivots[i + 3], pivots[i + 4], rates[0], req))
        {
         g_reuse_until_time = TimeCurrent() + strategy_reuse_guard_bars * PeriodSeconds(PERIOD_H4);
         return true;
        }
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_1401\",\"ea\":\"harmonic_shark_xabcd_h4\"}");
