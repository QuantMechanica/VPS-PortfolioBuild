...
#property description "QM5_41449 XAU/XAG Weekly WR2 Body Momentum"
...
// closes. Trade only when the newest ratio-close range is strictly wider than
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41449 && qm_magic_slot_offset == 0 &&
            g_leg_xau != "" && g_leg_xag != "" &&
            g_leg_xau != g_leg_xag &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_WR2_BODY_MOM_ATTEMPT_WEEK", qm_ea_id);
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41449_XAUXAG_WR2_BODY_MOM";
...
bool Strategy_LoadWr2Body(const int current_week,
                         int &direction)
  {
   direction = 0;
   g_newest_week_body = 0.0;
   g_range_expanded = false;
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41449_XAUXAG_WR2_BODY_MOM_HOST";
   ...
   if(Strategy_OpenOwnedPositionCount() > 0 ||
     ...
     {
      signal_valid = Strategy_LoadWr2Body(g_signal_week_key, direction);
      if(!signal_valid)
      ...
      else if(!g_range_expanded)
         g_signal_state = "range_not_expanded_flat";
      else
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG weekly ratio range-expansion body continuation package exits at the next broker week"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41449\",\"ea\":\"xauxag-wr2-body-mom\"}");
