"""Deterministic reference checks for QM5_41449 XAU/XAG weekly WR2 body momentum."""
...
MQ5 = EA_DIR / "QM5_41449_xauxag-wr2-body-mom.mq5"
SETFILE = (
    EA_DIR
    / "sets"
    / "QM5_41449_xauxag-wr2-body-mom_QM5_41449_XAU_XAG_WR2_BODY_MOM_D1_D1_backtest.set"
)
...
def wr2_body_signal(prior_range: float, newest_range: float, body: float) -> int:
...
class Wr2BodyMomentumReferenceTest(unittest.TestCase):
    def test_ratio_range_and_both_continuation_sides(self) -> None:
        prior_width, _ = range_and_body([4.60, 4.61, 4.59, 4.60])
        ...
        self.assertEqual(wr2_body_signal(prior_width, positive_width, positive_body), 1)
        self.assertEqual(wr2_body_signal(prior_width, negative_width, negative_body), -1)
    ...
    def test_range_ties_contraction_body_ties_and_invalid_are_flat(self) -> None:
        for prior, newest, body in (
            ...
            with self.subTest(prior=prior, newest=newest, body=body):
                self.assertEqual(wr2_body_signal(prior, newest, body), 0)
    ...
    def test_static_artifacts_lock_signal_risk_and_unpinned_framework(self) -> None:
        ...
        self.assertIn("qm_ea_id                    = 41449", source)
        ...
        self.assertRegex(resolver, re.compile(r"414490000.*414490001", re.DOTALL))
