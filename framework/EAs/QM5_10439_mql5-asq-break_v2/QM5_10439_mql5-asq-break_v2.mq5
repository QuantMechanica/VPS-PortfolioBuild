...
#property description "QM5_10439 MQL5 ASQ Seven-Condition Breakout _v2"

#include <QM/QM_Common.mqh>

// v2: source unchanged. Root cause: Q02 ONINIT_FAILED on GBPUSD.DWX
...
double Strategy_HighestPriorHigh(const int lookback)
  {
   ...
   for(int shift = 2; shift <= lookback + 1; ++shift)
     {
      const double high = iHigh(_Symbol, PERIOD_M5, shift); // perf-allowed: structural/indicator read; called from QM_IsNewBar-gated context
      if(high <= 0.0)
         return 0.0;
...
double Strategy_LowestPriorLow(const int lookback)
  {
   ...
   for(int shift = 2; shift <= lookback + 1; ++shift)
     {
      const double low = iLow(_Symbol, PERIOD_M5, shift); // perf-allowed: structural/indicator read; called from QM_IsNewBar-gated context
      if(low <= 0.0)
         return 0.0;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(strategy_breakout_lookback <= 0 || strategy_atr_period <= 0)
      return false;
   const double close1 = iClose(_Symbol, PERIOD_M5, 1); // perf-allowed: structural/indicator read; called from QM_IsNewBar-gated context
   const double close2 = iClose(_Symbol, PERIOD_M5, 2); // perf-allowed: structural/indicator read; called from QM_IsNewBar-gated context
   const double atr = QM_ATR(_Symbol, PERIOD_M5, strategy_atr_period, 1);
