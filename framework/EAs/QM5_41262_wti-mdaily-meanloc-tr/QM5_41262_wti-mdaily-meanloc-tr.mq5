...
#property description "QM5_41262 WTI Completed-Month Daily Mean-Location Trend"
...
//   - require one older bar to prove the completed-month boundary
//   - average every completed-month daily close without rounding
//   - follow the strict sign of final_close / mean_close - 1
//   - consume the month before fallible gates; never enter late or retry
//   - hold the completed-month path-location direction for one broker month
// Runtime uses MT5-native completed closes, calendar, and framework state only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                      = 41262;
...
double   g_mean_close              = 0.0;
double   g_location               = 0.0;
...
bool Strategy_LoadMeanLocationSignal(const int current_month_key,
                                     const int label_offset,
                                     int &direction,
                                     int &month_sessions,
   ...
   for(int index = 0; index < month_sessions; ++index)
     {
      ...
      if(current_close <= 0.0 || !MathIsValidNumber(current_close))
         return false;
      close_sum += current_close;
      if(!MathIsValidNumber(close_sum))
         return false;
     }

   final_close = month_closes[0];
   mean_close = close_sum / (double)month_sessions;
   if(final_close <= 0.0 || !MathIsValidNumber(final_close) ||
      mean_close <= 0.0 || !MathIsValidNumber(mean_close))
      return false;
   location = final_close / mean_close - 1.0;
   if(!MathIsValidNumber(location))
      return false;

   if(location > strategy_direction_epsilon)
      direction = 1;
   else if(location < -strategy_direction_epsilon)
      direction = -1;
   return true;
  }
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadMeanLocationSignal(g_decision_month_key,
                                         ...
                                         g_location);
      if(!g_signal_valid)
         g_signal_state = "monthly_daily_mean_location_validation_failed";
      else if(g_signal_direction == 0)
         g_signal_state = "neutral_mean_location_flat";
      else if(g_signal_direction > 0)
         g_signal_state = "completed_month_mean_location_long";
      else if(g_signal_direction < 0)
         g_signal_state = "completed_month_mean_location_short";
      else
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"month_sessions\":%d,\"boundary_close\":%.8f,\"final_close\":%.8f,\"mean_close\":%.8f,\"location\":%.12e,\"state\":\"%s\"}",
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41262 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41262_WTI_MDAILY_MEANLOC_TR";
   ...
   req.reason = (g_signal_direction > 0)
      ? "WTI_MDAILY_MEANLOC_TR_LONG"
      : "WTI_MDAILY_MEANLOC_TR_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41262 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI completed-month daily mean-location position holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41262_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41262\",\"ea\":\"wti-mdaily-meanloc-tr\"}");
