...
#property description "QM5_13064 XTI West Coast PADD5 stockdraw squeeze"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_13064 - XTI West Coast PADD 5 Stockdraw Squeeze
// -----------------------------------------------------------------------------
// D1 structural WTI sleeve:
//   - uses Thursday/Friday as the post-WPSR PADD 5 crude-stock proxy window
//   - requires May-October season, monthly cap, compressed D1 context,
//     bullish ATR-sized breakout, and fast-over-slow SMA trend confirmation
//   - ATR stop/target, SMA/season/time exits, no external runtime data
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 13064;
...
input int    strategy_season_start_month    = 5;
input int    strategy_season_end_month      = 10;
input int    strategy_report_start_dow      = 4;
input int    strategy_report_end_dow        = 5;
input int    strategy_context_lookback      = 12;
input int    strategy_compression_lookback  = 7;
input double strategy_max_compression_atr   = 1.65;
input double strategy_max_open_extension_atr = 0.30;
...
input double strategy_breakout_buffer_atr   = 0.03;
input double strategy_atr_sl_mult           = 2.70;
input double strategy_atr_tp_mult           = 2.45;
...
bool Strategy_IsPadd5SqueezeSeason(const datetime t)
  {
   if(t <= 0)
      return false;
   MqlDateTime dt;
...
bool Strategy_RangeExcludingSignal(const int lookback,
                                   double &highest_high,
                                   double &lowest_low)
  {
...
bool Strategy_LoadPadd5SqueezeState(double &atr_last,
                                    int &signal_day_key,
                                    int &signal_month_key)
  {
   ...
   if(signal_time <= 0 || !Strategy_IsReportProxyDay(signal_time))
      return false;
   if(!Strategy_IsPadd5SqueezeSeason(signal_time) || !Strategy_IsPadd5SqueezeSeason(TimeCurrent()))
      return false;
   ...
   if(!Strategy_RangeExcludingSignal(strategy_context_lookback, context_high, context_low))
      return false;

   double compression_high = 0.0;
   double compression_low = 0.0;
   if(!Strategy_RangeExcludingSignal(strategy_compression_lookback, compression_high, compression_low))
      return false;
   const double compression_range = compression_high - compression_low;
   if(compression_range <= 0.0 || compression_range > strategy_max_compression_atr * atr_last)
      return false;

   const double breakout_buffer = strategy_breakout_buffer_atr * atr_last;
   if(signal_close <= context_high + breakout_buffer)
      return false;
   if(signal_open > context_high + strategy_max_open_extension_atr * atr_last)
      return false;
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      if(pos_type != POSITION_TYPE_BUY)
         should_close = true;
      if(!Strategy_IsPadd5SqueezeSeason(now))
         should_close = true;
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(strategy_report_start_dow < 0 || strategy_report_start_dow > 6 || strategy_report_end_dow < 0 || strategy_report_end_dow > 6)
      return true;
   if(strategy_context_lookback < 5 || strategy_compression_lookback < 3)
      return true;
   if(strategy_sma_period <= 1 || strategy_slow_sma_period <= strategy_sma_period || strategy_sma_slope_shift <= 0 || strategy_atr_period <= 1)
      return true;
   if(strategy_max_compression_atr <= 0.0 || strategy_max_open_extension_atr < 0.0)
      return true;
   ...
   if(strategy_min_close_location <= 0.0 || strategy_min_close_location >= 1.0)
      return true;
   if(strategy_breakout_buffer_atr < 0.0)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_13064_XTI_PADD5_SQZ";
   ...
   if(!Strategy_LoadPadd5SqueezeState(atr_last, signal_day_key, signal_month_key))
      return false;
   ...
   req.reason = "XTI_PADD5_SQUEEZE_LONG";
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_13064\",\"ea\":\"xti-padd5-sqz\"}");
