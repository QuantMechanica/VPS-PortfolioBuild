...
#property description "QM5_20183 GBPUSD USDCHF Cointegration"
...
//
// Fixed spread:
//   ln(GBPUSD.DWX) - beta * ln(USDCHF.DWX)
//
...
input double strategy_beta              = -0.651268047;
...
bool Strategy_IsHostSymbol()
  {
   return (_Symbol == g_leg_gbpusd || _Symbol == g_leg_usdchf);
  }
...
bool Strategy_EnsureBasketScope()
  {
   ...
   string allowed[2] = {"GBPUSD.DWX", "USDCHF.DWX"};
   for(int i = 0; i < 2; ++i)
...
bool Strategy_RefreshSpreadState()
  {
   ...
   for(int i = 0; i < history_count; ++i)
     {
      if(gbpusd_time[i] <= 0 || gbpusd_time[i] != usdchf_time[i])
         return false;
      if(gbpusd[i] <= 0.0 || usdchf[i] <= 0.0)
         return false;

      spreads[i] =
         MathLog(gbpusd[i]) - strategy_beta * MathLog(usdchf[i]);
...
bool Strategy_OpenPair(const int spread_direction)
  {
   ...
   const double weight_sum = gbpusd_weight + usdchf_weight;
   if(weight_sum <= 0.0)
      return false;
   ...
   const string reason =
      long_spread ? "QM5_20183_LONG_SPREAD_Z_LT_NEG_ENTRY"
                  : "QM5_20183_SHORT_SPREAD_Z_GT_POS_ENTRY";

   const bool gbpusd_ok =
      Strategy_OpenLeg(g_leg_gbpusd,
                       gbpusd_type,
                       gbpusd_weight,
                       weight_sum,
                       reason);
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20183_PAIR_HOST";
