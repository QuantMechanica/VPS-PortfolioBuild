...
#property description "QM5_1138 Quantpedia Low-Volatility Next-Day Edge - SP500"

#include <QM/QM_Common.mqh>

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                       = 1138;
...
input int    strategy_vol_period_days       = 21;
input int    strategy_rank_lookback_days    = 250;
input int    strategy_lowest_count          = 25;
input int    strategy_min_valid_closes      = 280;
...
input int    strategy_safety_exit_minute_ny = 0;
...
double Strategy_RealizedVolatility(const int start_shift)
  {
   const int period = MathMax(2, strategy_vol_period_days);
   double sum = 0.0;
   double sum_sq = 0.0;
   int samples = 0;
   ...
   double variance = (sum_sq / (double)samples) - mean * mean;
   if(variance < 0.0)
      variance = 0.0;
   return MathSqrt(variance);
  }

bool Strategy_LowVolSignalFromCompletedD1()
  {
   const int vol_period = MathMax(2, strategy_vol_period_days);
   const int rank_lookback = MathMax(1, strategy_rank_lookback_days);
   const int required_bars = MathMax(strategy_min_valid_closes, vol_period + rank_lookback + 2);
   if(Bars(_Symbol, PERIOD_D1) < required_bars)
      return false;

   const double signal_vol = Strategy_RealizedVolatility(1);
   if(signal_vol <= 0.0)
      return false;

   int lower_count = 0;
   int valid_obs = 0;
   for(int i = 0; i < rank_lookback; ++i)
     {
      const double obs_vol = Strategy_RealizedVolatility(2 + i);
      if(obs_vol <= 0.0)
         return false;
      ++valid_obs;
      if(obs_vol < signal_vol)
         ++lower_count;
     }

   if(valid_obs < rank_lookback)
      return false;

   return (lower_count < MathMax(1, strategy_lowest_count));
  }

int Strategy_TradingDaysElapsed(const datetime start_date_ny, const datetime current_date_ny)
  {
   datetime cursor = Strategy_DateFloor(start_date_ny);
   const datetime current = Strategy_DateFloor(current_date_ny);
   int elapsed = 0;

   while(cursor < current && elapsed < 20)
     {
      cursor += 86400;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QP_SP500_LOWVOL_NEXTDAY_LONG";
   ...
   const datetime today_ny = Strategy_MakeDate(ny.year, ny.mon, ny.day);
   const int date_key = Strategy_DateKey(today_ny);
   ...
   if(!Strategy_LowVolSignalFromCompletedD1())
      return false;
...
bool Strategy_ExitSignal()
  {
   ...
   const datetime open_broker = (datetime)PositionGetInteger(POSITION_TIME);
   const datetime open_ny_date = Strategy_DateFloor(Strategy_BrokerToNewYork(open_broker));
   const int target_elapsed = MathMax(0, strategy_holding_days - 1);
   if(Strategy_TradingDaysElapsed(open_ny_date, today_ny) < target_elapsed)
      return false;
   ...
   if(ny.hour > strategy_safety_exit_hour_ny || (ny.hour == strategy_safety_exit_hour_ny && ny.min >= strategy_safety_exit_minute_ny))
     {
      g_last_exit_date_key = date_key;
      return true;
     }
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_1138\",\"ea\":\"qp-sp500-lowvol-nextday\"}");
