...
#property description "QM5_20203 EURUSD AUDJPY Cointegration"
...
//
// Card:
//   strategy-seeds/cards/approved/QM5_20203_eurusd-audjpy_card.md
//
// Fixed spread:
//   ln(EURUSD.DWX) - beta * ln(AUDJPY.DWX)
//
...
input double strategy_beta              = -0.160071209;
...
bool Strategy_EnsureBasketScope()
  {
   ...
   string allowed[4] = {"EURUSD.DWX", "AUDJPY.DWX", "AUDUSD.DWX", "USDJPY.DWX"};
   for(int i = 0; i < 4; ++i)
...
bool Strategy_RefreshSpreadState()
  {
   ...
   for(int i = 0; i < history_count; ++i)
     {
      if(eurusd_time[i] <= 0 || eurusd_time[i] != audjpy_time[i])
         return false;
      if(eurusd[i] <= 0.0 || audjpy[i] <= 0.0)
         return false;

      spreads[i] =
         MathLog(eurusd[i]) - strategy_beta * MathLog(audjpy[i]);
...
bool Strategy_OpenPair(const int spread_direction)
  {
   ...
   const double weight_sum = eurusd_weight + audjpy_weight;
   if(weight_sum <= 0.0)
      return false;
   ...
   const double audjpy_lots =
      Strategy_LotsForLeg(g_leg_audjpy, audjpy_weight, weight_sum);
   if(eurusd_lots <= 0.0 || audjpy_lots <= 0.0)
      return false;
   ...
   const string reason =
      long_spread ? "QM5_20203_LONG_SPREAD_Z_LT_NEG_ENTRY"
                  : "QM5_20203_SHORT_SPREAD_Z_GT_POS_ENTRY";
   ...
   const bool eurusd_ok =
      Strategy_OpenLeg(g_leg_eurusd,
                       ...
                       eurusd_lots,
                       reason);
   const bool audjpy_ok =
      Strategy_OpenLeg(g_leg_audjpy,
                       audjpy_type,
                       audjpy_lots,
                       reason);
   if(eurusd_ok && audjpy_ok)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20203_PAIR_HOST";
