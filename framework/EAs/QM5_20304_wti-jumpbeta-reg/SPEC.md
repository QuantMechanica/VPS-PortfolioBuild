# QM5_20304_wti-jumpbeta-reg - Strategy Spec

**EA ID:** QM5_20304
**Slug:** `wti-jumpbeta-reg`
**Source:** `HOLLSTEIN-WTI-JUMPBETA-REG-2026`
...
inclusive two-sigma realized-jump residual, and regress WTI return on an
intercept, common-energy return, and the jump residual. Buy WTI when the
recent jump-beta coefficient is lower than its preceding coefficient; sell
when it is higher. XNG is read-only.
...
| Drawdown profile | Sparse fixed-risk WTI jump-regime losses with gap, roll, conditioning, and persistent-state exposure |
...
The primary source defines option-derived aggregate jump beta and uses
monthly low-minus-high commodity exposure. The EA's realized two-CFD factor
...
two disjoint WTI jump-beta histories, owns one magic, and makes XNG read-only.
`QM5_20303` estimates a rolling smooth-volatility-change coefficient rather
than an extreme-day jump coefficient. WTI tail, moment, trend, calendar,
event, breakout, reversal, robust-location, and variance-ratio builds use
other state objects. `QM5_12567` is a short-horizon long-only XNG oscillator
pullback.
...
pooled/equal weights, wrong standard-deviation denominator, wrong jump factor,
too few jump rows, singular OLS acceptance, inverted direction, any XNG
order, repeated attempt, missing stop, hold beyond forty days, risk mismatch,
...
| v2 | 2026-08-13 | Implemented locked common-energy jump-beta regime | Strict compile, deployment, reference, and P1 checks pass |
...
inclusive two-sigma realized-jump factor, three-column OLS, restart-safe
monthly attempt state, one-position lifecycle, frozen hard stop, and fixed-risk
contract. Strict compile passed with zero errors and zero warnings; the target
build check passed with zero failures and zero warnings; seven independent
formula, direction, support, block-boundary, count, chronology, and freshness
tests passed; all T1-T10 research-factory binary hashes match; and P1 artifact
validation found the compiled `.ex5`. XNG remains a read-only factor input.
