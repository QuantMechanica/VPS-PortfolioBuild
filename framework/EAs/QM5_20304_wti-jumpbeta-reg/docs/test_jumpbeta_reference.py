"""Independent reference checks for the QM5_20304 locked jump-beta contract."""
...
def block_design(
    ...
    if len(xti_returns) != 2 * RETURNS_PER_BLOCK:
        raise ValueError("exactly 504 WTI returns are required")
    ...
    return xti, market, jump, xti_weight, xng_weight, jump_days
...
def synthetic_block(jump_loading: float, phase: float) -> tuple[list[float], list[float]]:
    xti: list[float] = []
    xng: list[float] = []
