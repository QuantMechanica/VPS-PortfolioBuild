"""Deterministic reference checks for QM5_41455 XAU/XAG weekly WR2 body reversion."""
...
MQ5 = EA_DIR / "QM5_41455_xauxag-wr2-body-rv.mq5"
SETFILE = (
    EA_DIR
    / "sets"
    / "QM5_41455_xauxag-wr2-body-rv_QM5_41455_XAU_XAG_WR2_BODY_RV_D1_D1_backtest.set"
)
...
def wr2_body_reversion_signal(prior_range: float, newest_range: float, body: float) -> int:
    if any(not math.isfinite(value) for value in (prior_range, newest_range, body)):
        return 0
    if newest_range <= 0 or newest_range <= prior_range:
        return 0
    if body > 1e-10:
        return -1
    if body < -1e-10:
...
class Wr2BodyReversionReferenceTest(unittest.TestCase):
    def test_ratio_range_and_both_reversion_sides(self) -> None:
        prior_width, _ = range_and_body([4.58, 4.60, 4.57, 4.59])
        positive_width, positive_body = range_and_body([4.58, 4.60, 4.57, 4.61])
        negative_width, negative_body = range_and_body([4.61, 4.59, 4.62, 4.57])
        self.assertEqual(
            wr2_body_reversion_signal(prior_width, positive_width, positive_body), -1
        )
        self.assertEqual(
            wr2_body_reversion_signal(prior_width, negative_width, negative_body), 1
        )

    def test_range_ties_contraction_body_ties_and_invalid_are_flat(self) -> None:
        for prior, newest, body in (
            ...
            with self.subTest(prior=prior, newest=newest, body=body):
                self.assertEqual(wr2_body_reversion_signal(prior, newest, body), 0)
    ...
    def test_static_artifacts_lock_signal_risk_and_unpinned_framework(self) -> None:
        ...
        self.assertIn("qm_ea_id                    = 41455", source)
        self.assertIn("Strategy_LoadWr2Body", source)
        ...
        self.assertRegex(resolver, re.compile(r"414550000.*414550001", re.DOTALL))
