...
#property description "QM5_41411 WTI Three-Week Alternation Continuation"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41411 - WTI Three-Week Alternation Continuation
// -----------------------------------------------------------------------------
// D1 structural crude-oil sleeve:
//   - aggregate the three immediately completed 3-to-5-session normalized weeks
//   - require strict oldest-to-newest sign alternation
//   - follow the newest completed-week sign for one week
...
input int    strategy_history_bars         = 35;
input int    strategy_required_weeks       = 3;
...
int      g_week_bars[3]           = {0, 0, 0};
double   g_week_opens[3]          = {0.0, 0.0, 0.0};
double   g_week_closes[3]         = {0.0, 0.0, 0.0};
double   g_week_log_returns[3]    = {0.0, 0.0, 0.0};
...
void Strategy_ResetDecisionState()
  {
   ...
   for(int index = 0; index < 3; ++index)
     {
      g_week_bars[index] = 0;
      g_week_opens[index] = 0.0;
      g_week_closes[index] = 0.0;
      g_week_log_returns[index] = 0.0;
     }
   g_signal_state = "idle";
  }
...
bool Strategy_LoadThreeWeekAlternationSignal(const int current_week_key,
                                             const int label_offset,
                                             int &direction)
  {
   direction = 0;
   ...
   const int copied =
      CopyRates(_Symbol, // perf-allowed: bounded completed-week scan behind the sole QM_IsNewBar branch.
                ...
                strategy_history_bars,
                bars);
   if(copied < 3 * strategy_min_week_bars + 1)
      return false;

   int week_keys[3] = {0, 0, 0};
   int week_bars[3] = {0, 0, 0};
   double week_opens[3] = {0.0, 0.0, 0.0};
   double week_closes[3] = {0.0, 0.0, 0.0};
   int week_index = 0;
   ...
   for(int index = 0; index < copied; ++index)
     {
      ...
      if(week_keys[week_index] == 0)
        ...
        {
         if(Strategy_NextWeekKey(week_key) != week_keys[week_index])
            return false;
         if(week_index >= 2)
           {
   ...
   if(!older_boundary_seen || week_index != 2)
      return false;
   for(int package_index = 0; package_index < 3; ++package_index)
     {
   ...
   for(int package_index = 0; package_index < 3; ++package_index)
     {
      g_week_bars[package_index] = week_bars[package_index];
      g_week_opens[package_index] = week_opens[package_index];
      g_week_closes[package_index] = week_closes[package_index];
      g_week_log_returns[package_index] =
         MathLog(g_week_closes[package_index] /
                 g_week_opens[package_index]);
      if(!MathIsValidNumber(g_week_log_returns[package_index]))
         return false;
     }

   // Arrays are newest-to-oldest. Exact alternation is symmetric under
   // reversal; follow the newest completed-week sign.
   if(g_week_log_returns[0] > strategy_return_epsilon &&
      g_week_log_returns[1] < -strategy_return_epsilon &&
      g_week_log_returns[2] > strategy_return_epsilon)
      direction = 1;
   else if(g_week_log_returns[0] < -strategy_return_epsilon &&
           g_week_log_returns[1] > strategy_return_epsilon &&
           g_week_log_returns[2] < -strategy_return_epsilon)
      direction = -1;
   return true;
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   // Consume before OHLC validation, signal, news,
   ...
   if(Strategy_WeekAlreadyEntered(g_decision_week_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadThreeWeekAlternationSignal(g_decision_week_key,
                                                  g_decision_label_offset,
                                                  g_signal_direction);
      if(!g_signal_valid)
         g_signal_state = "three_week_validation_failed";
      else if(g_signal_direction > 0)
         g_signal_state = "positive_negative_positive_continuation_long";
      else if(g_signal_direction < 0)
         g_signal_state = "negative_positive_negative_continuation_short";
      else
         g_signal_state = "weekly_signs_not_strictly_alternating_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"week\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_week_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"newest_week_bars\":%d,\"middle_week_bars\":%d,\"oldest_week_bars\":%d,\"newest_week_return\":%.12f,\"middle_week_return\":%.12f,\"oldest_week_return\":%.12f,\"state\":\"%s\"}",
                            g_decision_week_key,
                            ...
                            g_week_bars[1],
                            g_week_bars[2],
                            g_week_log_returns[0],
                            g_week_log_returns[1],
                            g_week_log_returns[2],
                            g_signal_state));
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41411_WTI_WALT3_CONT";
...
int OnInit()
  {
   if(!SymbolSelect(strategy_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41411 ||
      qm_magic_slot_offset != 0)
   ...
   g_attempt_state_key =
      StringFormat("QM5_41411_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41411\",\"ea\":\"wti-walt3-cont\"}");
