...
#property description "QM5_41364 XTI/XNG Weekly Close-Location Divergence Reversion"
...
// week. When one energy leg closes strictly in its own upper range tercile and the
// other strictly in its lower range tercile, fade the high-location leg with
// an equal-notional opposite-leg package.
// Consume one attempt per week and flatten at the next week boundary.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41364;
...
input double strategy_clv_lower               = 0.333333333333;
input double strategy_clv_upper               = 0.666666666667;
...
double   g_xti_clv = 0.0;
double   g_xng_clv = 0.0;
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41364 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
            strategy_xng_symbol == "XNGUSD.DWX" &&
            strategy_history_bars_d1 == 30 &&
            strategy_min_week_sessions == 3 &&
            strategy_max_week_sessions == 5 &&
            MathAbs(strategy_clv_lower - 0.333333333333) <= 1.0e-12 &&
            MathAbs(strategy_clv_upper - 0.666666666667) <= 1.0e-12 &&
            strategy_clv_lower > 0.0 &&
            strategy_clv_lower < strategy_clv_upper &&
            strategy_clv_upper < 1.0 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XTIXNG_WCLVDIV_ATTEMPT_WEEK", qm_ea_id);
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41364_XTIXNG_WCLVDIV_RV";
...
bool Strategy_LoadWeeklyCloseLocationDivergence(const int current_week,
                                                int &direction)
  {
   direction = 0;
   g_xti_week_high = 0.0;
   ...
   if(completed_week <= 0 ||
      ...
      g_xti_week_high <= 0.0 || g_xti_week_low <= 0.0 ||
      g_xti_week_close <= 0.0 || g_xng_week_high <= 0.0 ||
      g_xng_week_low <= 0.0 || g_xng_week_close <= 0.0)
      return false;
   ...
   g_xti_clv = (g_xti_week_close - g_xti_week_low) / xti_range;
   g_xng_clv = (g_xng_week_close - g_xng_week_low) / xng_range;
   if(!MathIsValidNumber(g_xti_clv) || !MathIsValidNumber(g_xng_clv) ||
      g_xti_clv < 0.0 || g_xti_clv > 1.0 ||
      g_xng_clv < 0.0 || g_xng_clv > 1.0)
      return false;

   if(g_xti_clv > strategy_clv_upper &&
      g_xng_clv < strategy_clv_lower)
      direction = -1;
   else if(g_xti_clv < strategy_clv_lower &&
           g_xng_clv > strategy_clv_upper)
      direction = 1;
   return true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41364_XTIXNG_WCLVDIV_RV_HOST";
   ...
   if(Strategy_OpenOwnedPositionCount() > 0 ||
     ...
     {
      signal_valid =
         Strategy_LoadWeeklyCloseLocationDivergence(g_signal_week_key,
                                                    direction);
      if(!signal_valid)
         g_signal_state = "weekly_ohlc_validation_failed";
      else if(direction < 0)
         g_signal_state = "xti_upper_xng_lower_fade_short_xti";
      else if(direction > 0)
         g_signal_state = "xti_lower_xng_upper_fade_long_xti";
      else
         g_signal_state = "boundary_or_interior_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"week\":%d,\"decision_bar\":%I64d,\"late\":%s,\"valid\":%s,\"direction\":%d,\"xti_clv\":%.12f,\"xng_clv\":%.12f,\"xti_high\":%.10f,\"xti_low\":%.10f,\"xti_close\":%.10f,\"xng_high\":%.10f,\"xng_low\":%.10f,\"xng_close\":%.10f,\"sessions\":%d,\"state\":\"%s\"}",
                            g_signal_week_key,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XTI/XNG weekly close-location divergence package exits at the next broker week"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41364\",\"ea\":\"xtixng-wclv-div-rv\"}");
