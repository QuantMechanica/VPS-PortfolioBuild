...
#property description "QM5_1084 Chan XLE Basket Z-Score Arbitrage _v2"

#include <QM/QM_Common.mqh>

// =============================================================================
// QuantMechanica Strategy Card: QM5_1084_v2
// Logic: Mean-reverting basket arbitrage based on Z-Score of spread.
// Symbols: SP500, NDX, WS30, GDAXI.
// =============================================================================
...
bool Strategy_ComputeZ(double &z)
  {
   ...
   for(int i = 0; i < STRATEGY_SYMBOL_COUNT; ++i)
     {
      if(Bars(g_symbols[i], PERIOD_D1) < n + 5)
         return false;
     }
   ...
   if(valid_bars < n * 0.9) return false;

   const double mean = sum / (double)valid_bars;
   const double variance = (sumsq / (double)valid_bars) - mean * mean;
   if(variance <= 0.0)
      return false;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.reason = (g_cached_spread_direction > 0) ? "BASKET_LONG_TARGET" : "BASKET_SHORT_TARGET";
...
int OnInit()
  {
   // Resolve slot dynamically to avoid INIT_FAILED on secondary symbols
   const int dynamic_slot = Strategy_CurrentSlot();
   ...
   // Initialize symbol guard for basket symbols
   QM_SymbolGuardInit(g_symbols);

   QM_LogEvent(QM_INFO, "INIT_OK", StringFormat("{\"dynamic_slot\":%d}", dynamic_slot));
