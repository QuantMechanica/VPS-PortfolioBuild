...
#property description "QM5_41414 XAU/XAG Weekly Relative-Vote Flip Continuation"
...
// -----------------------------------------------------------------------------
// Reconstruct five synchronized week-end XAU/XAG close pairs for the five
// immediately preceding completed broker weeks. Form four adjacent relative
// returns and the overlapping older/newer three-week sign votes. Trade only a
// strict majority flip and follow the new relative winner with an equal-
// notional opposite-leg package. Consume one attempt per week and flatten at
// the next week boundary.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41414;
...
double   g_relative_return[4]; // chronological: oldest interval to newest.
int      g_old_vote = 0;
int      g_new_vote = 0;
bool     g_all_relative_strict = false;
double   g_xau_week_close[5]; // newest through oldest completed week.
double   g_xag_week_close[5];
int      g_week_key[5];
int      g_week_sessions[5];
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41414_XAUXAG_WRELVOTE_FLIP_CONT";
...
bool Strategy_LoadRelativeVoteFlip(const int current_week,
                                   int &direction)
  {
   direction = 0;
   g_old_vote = 0;
   ...
   for(int interval = 0; interval < 4; ++interval)
     {
      const int older_slot = 4 - interval;
   ...
   for(int interval = 0; interval < 3; ++interval)
      g_old_vote += (g_relative_return[interval] > 0.0) ? 1 : -1;
   for(int interval = 1; interval < 4; ++interval)
      g_new_vote += (g_relative_return[interval] > 0.0) ? 1 : -1;
   if(g_old_vote < 0 && g_new_vote > 0)
      direction = 1; // New WTI majority: buy XAU, sell XAG.
   else if(g_old_vote > 0 && g_new_vote < 0)
      direction = -1; // New XAG majority: sell XAU, buy XAG.
   return true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41414_XAUXAG_WRELVOTE_FLIP_CONT_HOST";
   ...
   if(Strategy_OpenOwnedPositionCount() > 0 ||
     ...
     {
      signal_valid =
         Strategy_LoadRelativeVoteFlip(g_signal_week_key, direction);
      if(!signal_valid)
         g_signal_state = "endpoint_validation_failed";
      else if(direction > 0)
         g_signal_state = "new_wti_majority_followed";
      else if(direction < 0)
         g_signal_state = "new_xag_majority_followed";
      else if(!g_all_relative_strict)
         g_signal_state = "relative_tie_flat";
      else
         g_signal_state = "no_strict_vote_flip_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"week\":%d,\"decision_bar\":%I64d,\"late\":%s,\"valid\":%s,\"direction\":%d,\"d0\":%.12e,\"d1\":%.12e,\"d2\":%.12e,\"d3\":%.12e,\"old_vote\":%d,\"new_vote\":%d,\"oldest_week\":%d,\"newest_week\":%d,\"oldest_sessions\":%d,\"newest_sessions\":%d,\"state\":\"%s\"}",
                            g_signal_week_key,
...
int OnInit()
  {
   g_leg_xau = strategy_host_symbol;
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG weekly relative-vote flip continuation package exits at the next broker week"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41414\",\"ea\":\"xauxag-wrelvote-flip-cont\"}");
