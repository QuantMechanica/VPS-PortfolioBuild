"""Deterministic reference checks for QM5_41107 monthly WTI inside body."""
...
def inside_body_signal(
    ...
    """Mirror two-month OHLC aggregation and the strict inside-body signal."""
    ...
    direction = 0
    inside = new_high < parent_high and new_low > parent_low
    if inside and new_close > new_open:
        direction = 1
    elif inside and new_close < new_open:
        direction = -1
    return (
        True,
        direction,
...
def sample(
    new: tuple[float, float, float, float] = (100.0, 108.0, 92.0, 107.0),
...
class MonthInsideBodyReferenceTest(unittest.TestCase):
    def signal(self, bars: list[Bar], offset: timedelta = timedelta(0)):
        return inside_body_signal(202608, bars, offset)
    ...
    def test_strict_long_and_short_paths(self) -> None:
        ...
        self.assertEqual(values, (108.0, 92.0, 100.0, 107.0, 110.0, 90.0))

        _, _, bars = sample(new=(107.0, 108.0, 92.0, 93.0))
    ...
    def test_equal_noninside_and_inside_doji_states_are_flat(self) -> None:
        cases = [
            ((100.0, 110.0, 92.0, 107.0), (100.0, 110.0, 90.0, 105.0)),
            ((100.0, 108.0, 90.0, 107.0), (100.0, 110.0, 90.0, 105.0)),
            ((100.0, 112.0, 88.0, 107.0), (100.0, 110.0, 90.0, 105.0)),
            ((100.0, 112.0, 92.0, 107.0), (100.0, 110.0, 90.0, 105.0)),
            ((100.0, 108.0, 88.0, 107.0), (100.0, 110.0, 90.0, 105.0)),
            ((100.0, 108.0, 92.0, 100.0), (100.0, 110.0, 90.0, 105.0)),
        ]
    ...
    def test_newest_open_close_determine_side_parent_body_does_not(self) -> None:
        _, _, bars_a = sample(
            new=(100.0, 108.0, 92.0, 107.0),
            parent=(91.0, 110.0, 90.0, 109.0),
        )
        _, _, bars_b = sample(
            new=(107.0, 108.0, 92.0, 93.0),
    ...
    def test_static_build_contract_is_fixed_risk_and_completed_data_only(self) -> None:
        source = (EA_DIR / "QM5_41107_wti-minside-body-mom.mq5").read_text(
            encoding="utf-8"
        )
        setfile = (
            EA_DIR
            / "sets"
            / "QM5_41107_wti-minside-body-mom_XTIUSD.DWX_D1_backtest.set"
        ...
        self.assertLess(
            ...
            source.index("Strategy_LoadInsideBodySignal(g_decision_month_key"),
        )
        for marker in (
            "qm_ea_id=41107",
            "RISK_FIXED=1000",
        ...
        approved_card = REPO_ROOT / "strategy-seeds" / "cards" / "approved" / "QM5_41107_wti-minside-body-mom_card.md"
