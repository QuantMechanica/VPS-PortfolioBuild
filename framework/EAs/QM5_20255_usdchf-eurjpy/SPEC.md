# QM5_20255_usdchf-eurjpy — Strategy Spec
...
`ln(USDCHF.DWX) - (-0.075286902527) * ln(EURJPY.DWX)`. Score the newest spread
...
| `strategy_beta` | -0.075286902527 | fixed | DEV-fitted structural hedge coefficient |
...
- `USDJPY.DWX` — conversion-history dependency observed by the USD-account
  tester for EURJPY P/L; it is order-free and has no magic slot.
...
| Typical hold time | multi-day; scan half-life 97.412 D1 bars |
| Expected drawdown profile | high and regime-sensitive; conservative 30% prior |
| Regime preference | USDCHF/EURJPY fixed-residual mean reversion |
| Win rate target | unknown; Q02 is the first platform-economic judge |

The fixed scan recorded DEV net Sharpe `-0.045662`, OOS net Sharpe `-0.547994`,
and OOS return `-5.473567%`. The negative scores and slow half-life are
...
`1.0` and `0.075286902527`; each leg is independently bounded by its ATR stop.
If the EURJPY allocation normalizes below broker minimum volume, the whole
...
|---|---|---|---|
| v1 | 2026-08-06 | Initial build from approved card | Next-rank-64 non-duplicate fixed-scan FX basket |
