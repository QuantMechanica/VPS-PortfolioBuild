...
#property description "QM5_20255 USDCHF EURJPY Cointegration"
...
//
// Card:
//   strategy-seeds/cards/approved/QM5_20255_usdchf-eurjpy_card.md
//
// Fixed spread:
//   ln(USDCHF.DWX) - beta * ln(EURJPY.DWX)
//
...
input double strategy_beta              = -0.075286902527;
...
string   g_leg_usdchf = "USDCHF.DWX";
...
bool Strategy_EnsureBasketScope()
  {
   ...
   string allowed[3] = {"USDCHF.DWX", "EURJPY.DWX", "USDJPY.DWX"};
   for(int i = 0; i < 3; ++i)
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(!Strategy_EnsureBasketScope())
      return false;
   if(!QM_SymbolAssertOrLog(g_leg_usdchf) ||
      !QM_SymbolAssertOrLog(g_leg_eurjpy) ||
      !QM_SymbolAssertOrLog(g_conversion_usdjpy))
      return false;

   double usdchf[];
   double eurjpy[];
   datetime usdchf_time[];
   datetime eurjpy_time[];
   ArraySetAsSeries(usdchf, true);
   ArraySetAsSeries(eurjpy, true);
   ArraySetAsSeries(usdchf_time, true);
   ArraySetAsSeries(eurjpy_time, true);

   if(CopyClose(g_leg_usdchf, PERIOD_D1, 1, history_count, usdchf) != // perf-allowed: called only by the gated spread refresh.
      history_count)
      return false;
   if(CopyClose(g_leg_eurjpy, PERIOD_D1, 1, history_count, eurjpy) != // perf-allowed: called only by the gated spread refresh.
      history_count)
      return false;
   if(CopyTime(g_leg_usdchf, PERIOD_D1, 1, history_count, usdchf_time) != // perf-allowed: called only by the gated spread refresh.
      history_count)
      return false;
   if(CopyTime(g_leg_eurjpy, PERIOD_D1, 1, history_count, eurjpy_time) != // perf-allowed: called only by the gated spread refresh.
      history_count)
      return false;
   ...
   for(int i = 0; i < history_count; ++i)
     {
      if(usdchf_time[i] <= 0 || usdchf_time[i] != eurjpy_time[i])
         return false;
      if(usdchf[i] <= 0.0 || eurjpy[i] <= 0.0)
         return false;

      spreads[i] =
         MathLog(usdchf[i]) - strategy_beta * MathLog(eurjpy[i]);
...
bool Strategy_PreparePair(const int spread_direction,
                          QM_EntryRequest &host_req)
  {
   ...
   const double usdchf_weight = 1.0;
   const double eurjpy_weight = MathAbs(strategy_beta);
   const double weight_sum = usdchf_weight + eurjpy_weight;
   if(weight_sum <= 0.0)
      return false;
   ...
   const double usdchf_lots =
      Strategy_LotsForLeg(g_leg_usdchf, usdchf_weight, usdchf_weight);
   const double eurjpy_lots =
      Strategy_LotsForLeg(g_leg_eurjpy, eurjpy_weight, usdchf_weight);
   if(usdchf_lots <= 0.0 || eurjpy_lots <= 0.0)
      return false;

   const bool long_spread = (spread_direction > 0);
   const QM_OrderType usdchf_type = long_spread ? QM_BUY : QM_SELL;
   const QM_OrderType eurjpy_type =
      long_spread
      ? (strategy_beta >= 0.0 ? QM_SELL : QM_BUY)
      : (strategy_beta >= 0.0 ? QM_BUY : QM_SELL);
   const string reason =
      long_spread ? "QM5_20255_LONG_SPREAD_Z_LT_NEG_ENTRY"
                  : "QM5_20255_SHORT_SPREAD_Z_GT_POS_ENTRY";

   const double host_entry =
      QM_OrderTypeIsBuy(usdchf_type)
      ? SymbolInfoDouble(g_leg_usdchf, SYMBOL_ASK)
      : SymbolInfoDouble(g_leg_usdchf, SYMBOL_BID);
   const double host_atr =
      QM_ATR(g_leg_usdchf, PERIOD_D1, strategy_atr_period_d1, 1);
   if(host_entry <= 0.0 || host_atr <= 0.0)
      return false;

   host_req.type = usdchf_type;
   host_req.price = 0.0;
   host_req.sl = QM_StopATRFromValue(g_leg_usdchf,
                                     usdchf_type,
                                     host_entry,
                                     host_atr,
                                     strategy_atr_sl_mult);
   ...
   g_companion_entry_type = eurjpy_type;
   g_companion_entry_lots = eurjpy_lots;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20255_PAIR_HOST";
...
bool Strategy_NewsFilterHook(const datetime broker_time)
  {
   if(qm_news_temporal != QM_NEWS_TEMPORAL_OFF ||
     ...
     {
      if(!QM_NewsAllowsTrade2(g_leg_usdchf,
                              broker_time,
                              qm_news_temporal,
                              qm_news_compliance))
         return true;
      if(!QM_NewsAllowsTrade2(g_leg_eurjpy,
                              broker_time,
                              qm_news_temporal,
                              qm_news_compliance))
         return true;
     ...
     {
      if(!QM_NewsAllowsTrade(g_leg_usdchf,
                             broker_time,
                             qm_news_mode_legacy))
         return true;
      if(!QM_NewsAllowsTrade(g_leg_eurjpy,
                             broker_time,
                             qm_news_mode_legacy))
         return true;
     }
...
int OnInit()
  {
   SymbolSelect(g_leg_usdchf, true);
...
void OnTick()
  {
   ...
   const bool is_new_signal_bar =
      QM_IsNewBar(g_leg_usdchf, PERIOD_D1);
