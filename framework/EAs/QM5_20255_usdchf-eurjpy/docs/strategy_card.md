---
strategy_id: AI-CODEX-FX-COINT66-20260609-USDCHF-EURJPY
ea_id: 20255
slug: usdchf-eurjpy
type: strategy
status: APPROVED
...
markets: [forex]
timeframes: [D1]
primary_target_symbols: [USDCHF.DWX, EURJPY.DWX]
target_symbols: [USDCHF.DWX, EURJPY.DWX]
logical_symbol: QM5_20255_USDCHF_EURJPY_COINTEGRATION_D1
period: D1
...
# USDCHF/EURJPY D1 Cointegration Basket
...
low-frequency daily implementation. Chan makes no claim for USDCHF/EURJPY.
...
Rank 63 USDCHF/EURAUD is already implemented as `QM5_20252`.
USDCHF/EURJPY ranks sixty-fourth of 66 by OOS net Sharpe and is the next
relationship-level build gap. The deterministic dedup check found no exact
collision across 4,312 registry rows and 429 direct cards; no tracked pair
preset or dedicated two-leg basket manifest trades this exact pair.

| Pair | DEV net Sharpe | OOS net Sharpe | OOS return | OOS state changes | Fixed DEV beta | Half-life |
|---|---:|---:|---:|---:|---:|---:|
| USDCHF / EURJPY | -0.045661686086 | -0.547994298753 | -5.473566746133% | 15 | -0.075286902527 | 97.411859950023 D1 bars |

The negative DEV and OOS results, long half-life, and low inferred cadence are
adverse evidence. This card authorizes a one-shot low-frequency falsification
test, not a certified edge. Terminal economic, cadence, or history failure
retires the exact sleeve without a filter, beta refit, or parameter rescue.

## Non-Duplicate Boundary

`QM5_20252` implements rank 63 USDCHF/EURAUD, not rank 64. Six broad basket
manifests contain USDCHF and EURJPY only as unrelated universe members. The
dynamic cointegration umbrellas `QM5_1156` and `QM5_1257` omit EURJPY from
their candidate universes. Sibling fixed-scan sleeves use different pairs,
coefficients, magics, and logical identities.

## 2. Concept

Trade temporary deviations in the fixed log-price residual
`ln(USDCHF) - (-0.075286902527) * ln(EURJPY)`. The negative fitted coefficient
...
package retains USD, CHF, EUR, JPY, carry, and policy-divergence exposure.

## Hypothesis

The frozen scan found negative DEV and OOS performance. This card tests
whether large fixed-residual deviations survive native tick costs and swap in
the canonical D1 implementation. Economic, cadence, or history failure
retires this exact sleeve.
...
`USDCHF.DWX` is the tester host and first traded leg. `EURJPY.DWX` is the
companion traded leg. `USDJPY.DWX` is warmed only for USD-account conversion;
it receives neither an order nor a magic slot.

## 4. Entry Rules

- Load the newest closed D1 price for both traded symbols plus the preceding
  60 time-aligned closed observations.
- Compute `spread = ln(USDCHF) - strategy_beta * ln(EURJPY)`, with frozen
  `strategy_beta = -0.075286902527`.
...
- Permit entry only on host `USDCHF.DWX`, D1 or the supported H1 tester
  wrapper, and magic slot 0.
- Require both traded symbols and the conversion history selected and warm.
...
Chan supplies the pair-trading method but reports no USDCHF/EURJPY result.
The pair table is in-house evidence net only of the scan's approximate 0.8
bp/leg cost assumption. Swap was unmodeled; the deterministic pipeline is the
judge.
...
data_requirements: "USDCHF.DWX, EURJPY.DWX, and USDJPY.DWX D1 histories"
```
...
| v2 | 2026-08-06 | first deterministic V5 basket build | Q01 | PASS |

## 15. Pipeline Phase Status

| Phase | Date | Verdict | Evidence path |
|---|---|---|---|
| G0 Research Intake | 2026-08-06 | APPROVED | `decisions/2026-08-06_usdchf_eurjpy_cointegration_g0.md` |
| Q01 Build Validation | 2026-08-06 | PASS | `D:\QM\reports\framework\21\build_check_20260806_221156.json`; `D:\QM\reports\compile\20260806_221113\summary.csv` |
| Q02 Baseline Screening | 2026-08-06 | PENDING | `D:\QM\reports\state\claude_sweep_enqueue_2026-06-10.json`; work item `72ca17ca-f9df-40d5-806d-1d815ee4ea08` |

## 16. Lessons Captured

- 2026-08-06: Rank 63 is already represented by the dedicated QM5_20252
  sleeve; rank 64 is the first relationship-level gap after a semantic audit.
- 2026-08-06: Rank 64 is DEV- and OOS-negative and slower than the expected
...
- 2026-08-06: Strict Q01 build validation passed with zero compile errors,
  zero warnings, registered two-leg magics, and source-bound `RISK_FIXED`
  backtest setfiles.
