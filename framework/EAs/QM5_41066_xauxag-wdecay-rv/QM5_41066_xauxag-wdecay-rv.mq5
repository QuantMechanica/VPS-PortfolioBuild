...
#property description "QM5_41066 XAU/XAG Weekly Deceleration Reversion"
...
// returns have the same strict sign but the newest absolute move is strictly
// smaller, fade that waning direction with an equal-notional opposite-leg
// package. Consume one attempt per week and flatten at the next week boundary.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41066;
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41066 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_WDECAY_ATTEMPT_WEEK", qm_ea_id);
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41066_XAUXAG_WDECAY_RV";
...
bool Strategy_LoadWeeklyDeceleration(const int current_week,
                                     int &direction)
  {
   direction = 0;
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41066_XAUXAG_WDECAY_RV_HOST";
   ...
   if(Strategy_OpenOwnedPositionCount() > 0 ||
     ...
     {
      signal_valid =
         Strategy_LoadWeeklyDeceleration(g_signal_week_key, direction);
      if(!signal_valid)
         g_signal_state = "endpoint_validation_failed";
      else if(direction < 0)
         g_signal_state = "positive_deceleration_short_xau";
      else if(direction > 0)
         g_signal_state = "negative_deceleration_long_xau";
         ...
         g_signal_state = "no_strict_decay_flat";
     }
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG weekly deceleration package exits at the next broker week"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41066\",\"ea\":\"xauxag-wdecay-rv\"}");
