"""Deterministic reference checks for QM5_41066 XAU/XAG weekly decay."""
...
def weekly_decay_signal(
    current_week_key: int,
    completed_newest_first: list[PairBar],
) -> tuple[bool, int, float, float, tuple[float, float, float]]:
    """Mirror three synchronized week ends and strict same-sign decay."""
...
class WeeklyDecelerationReferenceTest(unittest.TestCase):
    def test_positive_strict_decay_sells_xau(self) -> None:
        bars = pair_bars((5.15, 5.10, 5.00))
        valid, direction, new_value, old_value, _ = weekly_decay_signal(20260824, bars)
        self.assertTrue(valid)
        self.assertEqual(direction, -1)
        self.assertAlmostEqual(new_value, 0.05)
        self.assertAlmostEqual(old_value, 0.10)

    def test_negative_strict_decay_buys_xau(self) -> None:
        bars = pair_bars((4.85, 4.90, 5.00))
        valid, direction, new_value, old_value, _ = weekly_decay_signal(20260824, bars)
        self.assertTrue(valid)
        self.assertEqual(direction, 1)
        self.assertAlmostEqual(new_value, -0.05)
    ...
    def test_equality_acceleration_mixed_sign_and_zero_are_flat(self) -> None:
        cases = (
            (5.20, 5.10, 5.00),
            (5.25, 5.10, 5.00),
            (5.05, 5.10, 5.00),
        ...
        for spreads in cases:
            with self.subTest(spreads=spreads):
                valid, direction, *_ = weekly_decay_signal(20260824, pair_bars(spreads))
                self.assertTrue(valid)
    ...
    def test_endpoint_validation_rejects_bad_order_gap_and_price(self) -> None:
        ...
        self.assertFalse(weekly_decay_signal(20260824, bars)[0])
        bars = pair_bars((5.15, 5.10, 5.00))
