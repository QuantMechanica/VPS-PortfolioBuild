...
bool Strategy_SymbolReturn(const string symbol, double &out_return)
  {
   out_return = 0.0;
   if(strategy_lookback_d1_bars <= 0 || strategy_skip_d1_bars <= 0)
      return false;

   SymbolSelect(symbol, true);
   const double recent_close = iClose(symbol, PERIOD_D1, strategy_skip_d1_bars);
   const double lookback_close = iClose(symbol, PERIOD_D1, strategy_lookback_d1_bars);
