...
#property description "QM5_41178 XTI/XNG Monthly Mann-Whitney Location-Shift Reversion"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_41178 - XTI/XNG Monthly Mann-Whitney Location-Shift Reversion
// -----------------------------------------------------------------------------
// At the first synchronized D1 boundary of a broker month, reconstruct the
// latest exactly matched XTI/XNG close pair in each of twelve consecutive
// completed broker months. Compare the six newer oil-minus-gas log ratios
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41178 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XTIXNG_MWILCOXON_RV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41178_XTIXNG_MWILCOXON_RV";
...
bool Strategy_LoadMonthlyMannWhitney(
      ...
      int &direction)
  {
   ...
   const int xti_copied =
      CopyRates(g_leg_xti, // perf-allowed: one bounded twelve-month scan behind a consumed monthly attempt.
                PERIOD_D1, 1, strategy_history_bars_d1, xti_bars);
   const int xng_copied =
      CopyRates(g_leg_xng, // perf-allowed: one bounded twelve-month scan behind a consumed monthly attempt.
   ...
   // direction > 0 means BUY XTI / SELL XNG. Fade the fixed-block ratio
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41178_XTIXNG_MWILCOXON_RV_HOST";
   ...
   if(valid && direction > 0)
      state = "lower_newer_ratio_location_fade_long_xti";
   else if(valid && direction < 0)
      state = "higher_newer_ratio_location_fade_short_xti";
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XTI/XNG fixed-block Mann-Whitney location-shift reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41178\",\"ea\":\"xtixng-mwilcoxon-rv\"}");
