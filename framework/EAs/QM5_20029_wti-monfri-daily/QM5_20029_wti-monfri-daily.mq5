...
#property description "QM5_20029 WTI Monday-Friday Daily Rotation"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_20029 - WTI Monday-Friday Daily Rotation
// -----------------------------------------------------------------------------
// Gorska and Krawiec (2015) WTI weekday carrier:
//   - SELL Monday and BUY Friday at the opening of a broker D1 bar
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_20029_WTI_MONFRI_ATTEMPT_%d",
                       QM_FrameworkMagic());
  }
...
bool Strategy_PrimeLateWeekdayStateAttach()
  {
   MqlRates current_bar;
   ZeroMemory(current_bar);
   if(!QM_ReadBar(_Symbol, PERIOD_D1, 0, current_bar))
      return true;
   const int attach_weekday = Strategy_Weekday(current_bar.time);
   if(attach_weekday != strategy_long_weekday && attach_weekday != strategy_short_weekday)
      return true;
   ...
   // persist the missed weekday-state decision when attaching after the only
...
bool Strategy_ConsumeWeekdayStateDecision()
  {
   g_entry_decision_ready = false;
   if(!g_strategy_new_d1_bar || g_strategy_d1_bar_time <= 0)
      return false;
   const int decision_weekday = Strategy_Weekday(g_strategy_d1_bar_time);
   if(decision_weekday != strategy_long_weekday && decision_weekday != strategy_short_weekday)
      return false;
   ...
   // rejected weekday-state signal is terminal for that broker day.
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsWtiD1())
      return true;
   if(qm_ea_id != 20029 || qm_magic_slot_offset != 0)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ZeroMemory(req);
   const int entry_weekday = Strategy_Weekday(g_strategy_d1_bar_time);
   req.type = (entry_weekday == strategy_long_weekday ? QM_BUY : QM_SELL);
   req.price = 0.0;
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = (req.type == QM_BUY ? "WTI_FRI_DAILY_LONG" : "WTI_MON_DAILY_SHORT");
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         QM_FRIDAY_CLOSE_CARD_RULE,
                        "Approved Monday/Friday daily rotation retains Friday flattening"))
      return INIT_FAILED;

   Strategy_LoadAttemptState(TimeCurrent());
   if(!Strategy_PrimeLateWeekdayStateAttach())
      return INIT_FAILED;

   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_20029\",\"ea\":\"wti-monfri-daily\"}");
...
void OnTick()
  {
   ...
   if(!Strategy_ConsumeWeekdayStateDecision())
      return;
