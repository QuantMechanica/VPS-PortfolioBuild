...
#property description "QM5_41014 XTI XNG Thursday-session relative-value basket"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_41014 - XTI/XNG Thursday-session relative-value basket
// -----------------------------------------------------------------------------
// At the first eligible broker-Thursday D1 boundary, buy WTI and sell Natural
// Gas. Both legs target equal absolute USD notionals and share one framework
// risk budget. The complete package closes at broker Thursday hour 21.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41014;
...
input int    strategy_entry_dow              = 4;
...
bool Strategy_CompletedPredecessorIsWednesday()
  {
   ...
   return (Strategy_DayOfWeek(completed_bar + (datetime)86400) == 3);
  }
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41014 && qm_magic_slot_offset == 0 &&
           strategy_xng_symbol == "XNGUSD.DWX" &&
           strategy_entry_dow == 4 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XTIXNG_THU_ATTEMPT_WEEK", qm_ea_id);
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41014_XTIXNG_THU_RV";
...
bool Strategy_DecisionClockReady(int &week_key)
  {
   ...
   if(!g_is_new_bar || g_current_host_bar <= 0 ||
      ...
      !Strategy_CompletedPredecessorIsWednesday())
      return false;
...
bool Strategy_EntryWindowReady(const int week_key)
  {
   ...
   // use its broker date for the card's Thursday decision and week key.
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41014_XTIXNG_THU_RV_HOST";
...
int OnInit()
  {
   ...
   g_pair_entry_time = Strategy_CurrentPairEntryTime();
   // Any Thursday attachment consumes the week flat. The shared new-bar
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41014\",\"ea\":\"xtixng-thu-rv\"}");
