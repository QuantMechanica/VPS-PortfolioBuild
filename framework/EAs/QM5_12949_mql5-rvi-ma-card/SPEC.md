# QM5_12949_mql5-rvi-ma-card — Strategy Spec

**EA ID:** QM5_12949
**Slug:** `mql5-rvi-ma-card`
...
`artifacts/cards_approved/QM5_12949_mql5-rvi-ma-card.md`. See that card's body for
...
`QM5_12949_mql5-rvi-ma-card.mq5`. Framework wiring (risk, magic, news, Friday close)
...
|---|---|---|---|
| `strategy_rvi_period` | 10 | (see source) | (see strategy logic) |
| `strategy_ma_period` | 100 | (see source) | (see strategy logic) |
| `strategy_atr_period` | 14 | (see source) | (see strategy logic) |
| `strategy_atr_slow_period` | 100 | (see source) | (see strategy logic) |
| `strategy_atr_min_ratio` | 0.5 | (see source) | (see strategy logic) |
| `strategy_atr_sl_mult` | 1.6 | (see source) | (see strategy logic) |
| `strategy_tp_r_mult` | 2.0 | (see source) | (see strategy logic) |
| `strategy_max_bars_in_trade` | 48 | (see source) | (see strategy logic) |
...
| Cadence note | "Medium frequency; RVI main/signal cross filtered by MA side, roughly 55-110 trades per year per symbol" |
...
**Pointer:** `strategy-seeds/sources/ba57d97a-0ee0-5a87-aa6d-fb5a37f08bdb/`
**R1–R4 verdict (Q00):** all PASS — see
`artifacts/cards_approved/QM5_12949_mql5-rvi-ma-card.md`
