...
input double strategy_mid_line           = 50.0;
input int    strategy_sma_period         = 200;
input int    strategy_atr_period         = 14;
input int    strategy_range_atr_period   = 50;
input double strategy_range_sanity_mult  = 0.5;
...
// Bermaui kernel: raw Stochastic %K -> WilderMA pass -> HMA pass.
double GetBermauiStoch(int shift)
{
   double k_raw[60];
   for(int i = 0; i < 60; i++)
   {
      k_raw[i] = QM_Stoch_K(_Symbol, PERIOD_H4, strategy_stoch_k_period, strategy_stoch_d_period,
                             strategy_stoch_slowing, shift + i); // perf-allowed: range scan
   }

   double k_smooth1[30];
   for(int j = 0; j < 30; j++)
   {
      double wma = k_raw[j + 25];
      for(int k = j + 24; k >= j; k--)
      {
         wma = (wma * (strategy_wilder_period - 1.0) + k_raw[k]) / (double)strategy_wilder_period;
      }
      k_smooth1[j] = wma;
   }
   ...
   for(int n = 0; n < 5; n++)
   {
      ...
      {
         wma_half += k_smooth1[n + i] * (half_period - i);
      ...
      {
         wma_full += k_smooth1[n + i] * (strategy_hma_period - i);
...
bool IsMidCrossUp()
{
   return (GetBermauiStoch(2) < strategy_mid_line && GetBermauiStoch(1) >= strategy_mid_line);
}
...
{
   if(Bars(_Symbol, PERIOD_H4) < strategy_sma_period + 60) // perf-allowed: O(1) bar count for warmup
      return true;
   ...
   if(atr1 < strategy_range_sanity_mult * atr_range)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
{
   ...
   if(strategy_stoch_k_period < 2 || strategy_wilder_period < 2 || strategy_hma_period < 2)
      return false;
   ...
   const double stoch1 = GetBermauiStoch(1);
   const double stoch2 = GetBermauiStoch(2);
   const double delta = stoch1 - stoch2;

   if(IsMidCrossUp() && delta > 0.0 && close1_d1 > sma200_d1)
   {
      side = QM_BUY;
      reason = "BERMAUI_STOCH_LONG";
   }
   else if(IsMidCrossDown() && delta < 0.0 && close1_d1 < sma200_d1)
   {
      side = QM_SELL;
...
void Strategy_ManageOpenPosition()
{
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
   {
      ...
      // 1. Move SL to break-even-plus-spread at +1.0 * ATR
      ...
      // 2. Close 50% at +2.0 * ATR (profit-target)
      if(!g_tp1_taken)
      {
         bool hit_tp1 = false;
...
bool Strategy_ExitSignal()
{
   ...
   if(Strategy_SelectOurPosition(position_type, ticket))
   {
      ...
      if(position_type == POSITION_TYPE_BUY && IsMidCrossDown() && delta < 0.0)
         return true;
      if(position_type == POSITION_TYPE_SELL && IsMidCrossUp() && delta > 0.0)
         return true;
   }
