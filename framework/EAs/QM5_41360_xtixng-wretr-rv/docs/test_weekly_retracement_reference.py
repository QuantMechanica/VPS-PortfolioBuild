"""Reference checks for QM5_41360 XTI/XNG weekly partial-retracement continuation."""
...
def weekly_retracement_signal(
    ...
    """Mirror strict sign opposition and strictly smaller newest retracement."""
...
class WeeklyRetracementReferenceTest(unittest.TestCase):
    def test_smaller_negative_retracement_after_positive_impulse_sells_xti(self) -> None:
        bars = pair_bars((5.20, 5.30, 5.00))
    ...
    def test_smaller_positive_retracement_after_negative_impulse_buys_xti(self) -> None:
        bars = pair_bars((4.80, 4.70, 5.00))
        valid, direction, new_value, old_value, spreads = weekly_retracement_signal(20260824, bars)
        self.assertTrue(valid)
        self.assertEqual(direction, 1)
        self.assertAlmostEqual(new_value, 0.10)
    ...
    def test_equality_non_smaller_same_sign_and_zero_are_flat(self) -> None:
        cases = (
            (5.00, 5.10, 5.00),
            (4.80, 5.10, 5.00),
            (5.20, 5.10, 5.00),
