...
#property description "QM5_41392 XNG Shoulder-Season Weekly Reversal"
...
//   - fade ln(final close / first open), with no magnitude/volatility filter
...
input int    strategy_spring_month_1       = 4;
input int    strategy_spring_month_2       = 5;
input int    strategy_autumn_month_1       = 9;
input int    strategy_autumn_month_2       = 10;
...
bool Strategy_IsShoulderMonth(const int month)
  {
   return (month == strategy_spring_month_1 ||
           month == strategy_spring_month_2 ||
           month == strategy_autumn_month_1 ||
           month == strategy_autumn_month_2);
  }
...
bool Strategy_LoadWeeklyReversalSignal(const int current_week_key,
                                       const int label_offset,
                                       int &direction,
                                       int &week_bars,
                                       double &week_open,
                                       ...
                                       double &week_log_return)
  {
   ...
   // Contrarian orientation is the identity: positive week -> short,
   // negative week -> long, exact zero -> flat.
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_WeekAlreadyEntered(g_decision_week_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadWeeklyReversalSignal(g_decision_week_key,
      ...
      if(!g_signal_valid)
         ...
         g_signal_state = "negative_week_reversal_long";
      else if(g_signal_direction < 0)
         g_signal_state = "positive_week_reversal_short";
      else
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41392_XNG_SHOULDER_WREV";
   ...
   req.reason = (g_signal_direction > 0)
      ? "XNG_SHOULDER_WEEK_REV_LONG"
      : "XNG_SHOULDER_WEEK_REV_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(strategy_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41392 ||
      qm_magic_slot_offset != 0)
   ...
   g_attempt_state_key =
      StringFormat("QM5_41392_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41392\",\"ea\":\"xng-shoulder-wrev\"}");
