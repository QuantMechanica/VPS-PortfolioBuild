...
#property description "QM5_41199 WTI Five-Year Same-Calendar Trimmed-Mean Seasonality"
...
//   - sort five returns, drop the minimum and maximum, average the middle three
...
input int    strategy_trim_each_tail       = 1;
input int    strategy_retained_observations = 3;
...
double   g_signal_deleted_min     = 0.0;
double   g_signal_deleted_max     = 0.0;
double   g_signal_retained_sum    = 0.0;
double   g_signal_trimmed_mean    = 0.0;
...
void Strategy_ResetDecisionState()
  {
   ...
   g_signal_deleted_min = 0.0;
   g_signal_deleted_max = 0.0;
   g_signal_retained_sum = 0.0;
   g_signal_trimmed_mean = 0.0;
   g_signal_state = "idle";
  }
...
// -----------------------------------------------------------------------------
// Exact completed-month endpoint and trimmed-mean helpers.
// -----------------------------------------------------------------------------
...
bool Strategy_TrimmedMeanSignal(const double &observations[],
                                ...
                                int &direction)
  {
   ...
   if(strategy_history_years != 5 ||
      ...
      strategy_trim_each_tail != 1 ||
      strategy_retained_observations != 3 ||
      sample_count != strategy_required_observations ||
      ArraySize(observations) < sample_count)
      return false;

   double sorted[];
   if(ArrayResize(sorted, sample_count) != sample_count ||
      ArrayCopy(sorted, observations, 0, 0, sample_count) != sample_count)
      return false;
   ...
   Strategy_SortAscending(sorted, sample_count);
   ...
   deleted_min = sorted[0];
   deleted_max = sorted[4];
   retained_sum = sorted[1] + sorted[2] + sorted[3];
   trimmed_mean = retained_sum / 3.0;
   if(!MathIsValidNumber(deleted_min) ||
      !MathIsValidNumber(deleted_max) ||
      !MathIsValidNumber(retained_sum) ||
      !MathIsValidNumber(trimmed_mean))
      return false;

   if(trimmed_mean > strategy_signal_epsilon)
      direction = 1;
   else if(trimmed_mean < -strategy_signal_epsilon)
      direction = -1;
   return true;
  }

bool Strategy_LoadTrimmedMeanSignal(const datetime decision_bar_time,
                                    ...
                                    int &direction)
  {
   ...
   return Strategy_TrimmedMeanSignal(observations,
                                     sample_count,
                                     deleted_min,
                                     deleted_max,
                                     retained_sum,
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   // Consume before history, trim arithmetic, news, spread, quote, ATR,
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadTrimmedMeanSignal(g_decision_bar_time,
                                        g_decision_month_key,
                                        g_decision_label_offset,
                                        g_signal_sample_count,
                                        g_signal_deleted_min,
                                        g_signal_deleted_max,
                                        g_signal_retained_sum,
                                        g_signal_trimmed_mean,
                                        g_signal_direction);
      if(!g_signal_valid)
         g_signal_state = "endpoint_exact_year_sample_sort_or_trim_failed";
      else if(g_signal_direction > 0)
         g_signal_state = "trimmed_mean_long";
      else if(g_signal_direction < 0)
         g_signal_state = "trimmed_mean_short";
      else
         g_signal_state = "trimmed_mean_tie_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"valid\":%s,\"samples\":%d,\"deleted_min\":%.12g,\"deleted_max\":%.12g,\"retained_sum\":%.12g,\"trimmed_mean\":%.12g,\"direction\":%d,\"state\":\"%s\"}",
                            g_decision_month_key,
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(!QM_InputRequireLong("qm_ea_id", qm_ea_id, 41199) ||
      ...
      !QM_InputRequireLong("strategy_trim_each_tail", strategy_trim_each_tail, 1) ||
      !QM_InputRequireLong("strategy_retained_observations", strategy_retained_observations, 3) ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(g_signal_direction > 0)
     {
      req.type = QM_BUY;
      req.reason = "WTI_SAMECAL_TRIM5_LONG";
     }
   else
     {
      req.type = QM_SELL;
      req.reason = "WTI_SAMECAL_TRIM5_SHORT";
     }
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41199 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI five-year same-calendar trimmed-mean card holds to the next broker month"))
      return INIT_FAILED;
   ...
   g_attempt_state_key =
      StringFormat("QM5_41199_SAMECAL_TRIM5_MONTH_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41199\",\"ea\":\"wti-samecal-trim5\"}");
