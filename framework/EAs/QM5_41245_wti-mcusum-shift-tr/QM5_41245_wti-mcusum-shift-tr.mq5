...
#property description "QM5_41245 WTI Monthly Centered-CUSUM Return Shift Trend"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41245 - WTI Monthly Centered-CUSUM Return Shift Trend
// -----------------------------------------------------------------------------
// D1 structural crude-oil sleeve:
//   - reconstruct thirteen consecutive completed broker-month-end closes
//   - derive twelve adjacent log returns and mean-center their partial sums
//   - follow the post-segment mean only for one unique central maximum split
...
bool Strategy_CusumSignal(const double &closes[],
                          Strategy_SignalMetrics &metrics)
  {
   ...
   if(ArrayResize(returns, strategy_month_returns) !=
      ...
      ArrayResize(cusums, strategy_month_returns - 1) !=
         strategy_month_returns - 1)
      return false;
   ...
   double running = 0.0;
   metrics.max_abs_cusum = 0.0;
   for(int index = 0; index < strategy_month_returns - 1; ++index)
     {
      running += returns[index];
      const int split = index + 1;
      const double value =
         running - (double)split * metrics.mean_return;
      if(!MathIsValidNumber(running) || !MathIsValidNumber(value))
         return false;
      cusums[index] = value;
      const double magnitude = MathAbs(value);
      if(magnitude > metrics.max_abs_cusum)
         metrics.max_abs_cusum = magnitude;
      if(StringLen(metrics.cusum_path) > 0)
         metrics.cusum_path += ",";
      metrics.cusum_path += DoubleToString(value, 12);
     }
   if(!MathIsValidNumber(metrics.max_abs_cusum) ||
      StringLen(metrics.cusum_path) <= 0)
      return false;

   metrics.change_index = 0;
   metrics.maxima_count = 0;
   for(int index = 0; index < ArraySize(cusums); ++index)
     {
      const double distance =
         MathAbs(MathAbs(cusums[index]) - metrics.max_abs_cusum);
      if(!MathIsValidNumber(distance))
         return false;
      if(distance <= strategy_tie_epsilon)
        {
         ++metrics.maxima_count;
         if(metrics.change_index == 0)
           {
            metrics.change_index = index + 1;
            metrics.selected_cusum = cusums[index];
           }
        }
     }
   if(metrics.maxima_count <= 0 || metrics.change_index <= 0)
      return false;

   metrics.oldest_close = closes[0];
   metrics.newest_close = closes[metrics.endpoint_count - 1];
   metrics.direction = 0;
   metrics.post_mean = 0.0;

   const bool qualifies =
      metrics.max_abs_cusum > strategy_tie_epsilon &&
      metrics.maxima_count == 1 &&
      metrics.change_index >= strategy_min_split &&
      metrics.change_index <= strategy_max_split;
   if(!qualifies)
      return true;

   double post_sum = 0.0;
   for(int index = metrics.change_index;
       index < strategy_month_returns;
       ++index)
     {
      post_sum += returns[index];
      if(!MathIsValidNumber(post_sum))
         return false;
     }
   const int post_count =
      strategy_month_returns - metrics.change_index;
   if(post_count <= 0)
      return false;
   metrics.post_mean = post_sum / (double)post_count;
   if(!MathIsValidNumber(metrics.post_mean))
      return false;
   if(metrics.post_mean > strategy_tie_epsilon)
      metrics.direction = 1;
   else if(metrics.post_mean < -strategy_tie_epsilon)
      metrics.direction = -1;
   return true;
  }
                                        ...
                                        int &direction)
  {
   ...
   const bool valid =
      Strategy_LoadMonthlyEndpoints(current_month_key,
                                    ...
                                    closes,
                                    endpoint_times) &&
      Strategy_CusumSignal(closes, metrics);
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41245 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41245_WTI_MCUSUM_SHIFT";
   ...
   req.reason = (g_signal_metrics.direction > 0)
      ? "WTI_MCUSUM_UPSHIFT_LONG"
      : "WTI_MCUSUM_DOWNSHIFT_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41245 ||
   ...
   g_attempt_state_key =
      StringFormat("QM5_41245_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41245\",\"ea\":\"wti-mcusum-shift-tr\"}");
