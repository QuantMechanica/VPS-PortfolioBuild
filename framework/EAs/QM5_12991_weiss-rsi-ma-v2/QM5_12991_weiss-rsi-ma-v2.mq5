...
#property description "QM5_12991 Weissman RSI Moving Average Filter Reversion v2"
...
input double          strategy_long_exit_rsi  = 55.0;
input double          strategy_short_exit_rsi = 45.0;
input double          strategy_stop_pct       = 1.0;
input int             strategy_max_hold_bars  = 90;
input int             strategy_max_spread_pts = 0;
input bool            strategy_use_atr_trail  = false;
...
// Trade Management: optional challenger ATR trail. Disabled by default so the
// v2 default keeps the original initial stop semantics.
void Strategy_ManageOpenPosition()
  {
   if(!strategy_use_atr_trail ||
      strategy_trail_atr_period <= 0 ||
      ...
      if(rsi_1 > 0.0 && rsi_2 > 0.0)
        {
         if(ptype == POSITION_TYPE_BUY &&
            rsi_2 <= strategy_long_exit_rsi &&
            rsi_1 > strategy_long_exit_rsi)
            return true;
         if(ptype == POSITION_TYPE_SELL &&
            rsi_2 >= strategy_short_exit_rsi &&
            rsi_1 < strategy_short_exit_rsi)
            return true;
        }
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_12991_weiss-rsi-ma-v2\"}");
