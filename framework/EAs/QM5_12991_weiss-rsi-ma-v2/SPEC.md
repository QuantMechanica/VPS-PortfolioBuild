# QM5_12991_weiss-rsi-ma-v2 - Strategy Spec

**EA ID:** QM5_12991
**Slug:** weiss-rsi-ma-v2
**Source:** D2C_13SLEEVE_EXIT_SURGERY_AUDIT_2026-07-03 challenger of QM5_11165
...
This EA trades H1 mean reversion only in the direction of a long moving-average filter. A long entry is opened at the next bar when the last completed close is above SMA(200) and RSI(9) crosses below 25; a short entry is opened when the last completed close is below SMA(200) and RSI(9) crosses above 75. V2 keeps entries, initial stop, and news behavior unchanged; long positions close when RSI(9) crosses above 55, short positions close when RSI(9) crosses below 45, and both sides also close after 90 H1 bars if no RSI exit or stop has fired. Each trade uses a 1.0% fixed price stop from entry and no fixed take-profit; an input-switchable ATR trail is present but disabled by default.
...
| strategy_long_exit_rsi | 55.0 | 0-100 | Long exit threshold crossed upward. |
| strategy_short_exit_rsi | 45.0 | 0-100 | Short exit threshold crossed downward. |
| strategy_stop_pct | 1.0 | >0 | Percent price distance for the initial stop. |
| strategy_max_hold_bars | 90 | 1+ | Maximum holding period in strategy timeframe bars. |
| strategy_max_spread_pts | 0 | 0+ | Optional spread cap in points; 0 leaves the cap disabled. |
| strategy_use_atr_trail | false | true/false | Optional challenger ATR trail; default OFF. |
| strategy_trail_atr_period | 14 | 1+ | ATR period for optional trail. |
| strategy_trail_atr_mult | 2.0 | >0 | ATR multiple for optional trail. |
