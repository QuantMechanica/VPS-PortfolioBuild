...
#property description "QM5_41181 XAU/XAG Thirteen-Month Pairwise-Rank Reversion"
...
// completed broker months. Compare all 78 older/newer gold-minus-silver log-
// ratio pairs, sum their strict signs, and fade only inclusive abs(S)>=14
// with an opposite, equal-notional package held to the next month.
// Runtime is native, completed-price-only, deterministic, and one attempt per
// month.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41181;
...
input int    strategy_score_threshold         = 14;
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41181 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_MKENDALL_RV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41181_XAUXAG_MKENDALL_RV";
...
bool Strategy_LoadMonthlyPairScore(
      ...
      int &direction)
  {
   ...
   // Give every older/newer ratio pair exactly one sign vote. Exact ties
   // consume the month flat; they are never deleted or average-ranked.
   for(int older = 0; older < strategy_endpoint_count - 1; ++older)
     {
      for(int newer = older + 1; newer < strategy_endpoint_count; ++newer)
        {
         ++pair_count;
         if(chronological_ratios[newer] ==
   ...
   if(pair_count !=
         strategy_endpoint_count * (strategy_endpoint_count - 1) / 2 ||
      pair_count != 78)
      return false;
   if(tie_count > 0)
      return true;

   const int absolute_score =
      (pair_score < 0 ? -pair_score : pair_score);
   if(pair_score < -pair_count || pair_score > pair_count ||
      (pair_score % 2) != 0 ||
      ((pair_count - absolute_score) % 2) != 0)
      return false;

   // direction > 0 means BUY XAU / SELL XAG. Fade the qualified ordinal
   // ratio trend; score magnitude above the boundary never scales risk.
   if(pair_score >= strategy_score_threshold)
      direction = -1;
   else if(pair_score <= -strategy_score_threshold)
      direction = 1;
   return true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41181_XAUXAG_MKENDALL_RV_HOST";
   ...
   string state = "monthly_pair_score_failed";
   if(valid && direction > 0)
      state = "negative_pair_trend_fade_long_xau";
   else if(valid && direction < 0)
      state = "positive_pair_trend_fade_short_xau";
   else if(valid && tie_count > 0)
      state = "ratio_tie_flat";
   else if(valid)
      state = "pair_score_interior_flat";

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"month_count\":%d,\"newest_endpoint_time\":%I64d,\"endpoint_displacement\":%.12e,\"pair_count\":%d,\"pair_score\":%d,\"tie_count\":%d,\"direction\":%d,\"state\":\"%s\"}",
                            g_signal_month_key,
                            month_count,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG thirteen-month all-pairs ratio-rank reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41181\",\"ea\":\"xauxag-mkendall-rv\"}");
