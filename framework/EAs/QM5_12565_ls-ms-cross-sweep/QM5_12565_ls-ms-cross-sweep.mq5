...
#property description "QM5_12565 ls-ms-cross-sweep — port of QM5_10692_tv-ls-ms"
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(entry <= 0.0 || point <= 0.0 || sl <= 0.0 || tp <= 0.0)
      return false;
   if(go_long && sl >= entry)
      return false;
   if(!go_long && sl <= entry)
...
void Strategy_ManageOpenPosition()
  {
   // Card specifies fixed ATR SL/TP and a separate bar-count time exit.
  }
