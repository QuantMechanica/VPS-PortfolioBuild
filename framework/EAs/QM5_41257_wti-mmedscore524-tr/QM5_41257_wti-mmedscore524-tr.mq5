...
#property description "QM5_41257 WTI Exact Non-Neutral Median-Score Trend"
...
//   - count recent returns above the strict pooled grand-median boundary
//   - enumerate every one of the 924 fixed-size six/six label assignments
//   - continue any non-neutral 4+/2- recent upper-half majority
...
input int    strategy_tail_count_max        = 524;
input int    strategy_recent_high_long_min  = 4;
input int    strategy_recent_high_short_max = 2;
...
struct Strategy_SignalMetrics
  {
   ...
   int tail_count;
   int recent_high_count;
   int neutral_distance;
   bool pooled_tie;
...
bool Strategy_RecentHighCountForMask(const int mask,
                                     int &recent_high_count)
  {
   recent_high_count = 0;
   if(mask < 0 || mask >= 4096 ||
   ...
   for(int rank = strategy_block_size;
       rank < strategy_month_returns;
       ++rank)
     {
      if((mask & (1 << rank)) != 0)
         ++recent_high_count;
     }
   return (recent_high_count >= 0 &&
           recent_high_count <= strategy_block_size);
  }

bool Strategy_MedianScoreSignal(const double &closes[],
                                Strategy_SignalMetrics &metrics)
  {
   ...
   for(int left = 0; left < strategy_month_returns - 1; ++left)
     {
      for(int right = left + 1;
          right < strategy_month_returns;
          ++right)
        {
   ...
   int observed_mask = 0;
   for(int rank = 0; rank < strategy_month_returns; ++rank)
     {
   ...
   if(Strategy_Popcount12(observed_mask) != strategy_block_size ||
      !Strategy_RecentHighCountForMask(observed_mask,
                                       metrics.recent_high_count) ||
      StringLen(metrics.sorted_membership_path) <= 0)
      return false;
   metrics.neutral_distance =
      MathAbs(metrics.recent_high_count - strategy_block_size / 2);
   if(metrics.neutral_distance < 0 || metrics.neutral_distance > 3)
      return false;
   ...
   for(int mask = 0; mask < 4096; ++mask)
     {
      if(Strategy_Popcount12(mask) != strategy_block_size)
         continue;
      int perm_recent_high = 0;
      if(!Strategy_RecentHighCountForMask(mask, perm_recent_high))
         return false;
      const int perm_distance =
         MathAbs(perm_recent_high - strategy_block_size / 2);
      if(perm_distance >= metrics.neutral_distance)
         ++metrics.tail_count;
   ...
   if(metrics.assignment_count != strategy_assignment_count ||
      ...
      return false;
   int expected_tail_count = 924;
   if(metrics.neutral_distance == 3)
      expected_tail_count = 2;
   else if(metrics.neutral_distance == 2)
      expected_tail_count = 74;
   else if(metrics.neutral_distance == 1)
      expected_tail_count = 524;
   ...
   if(tail_qualified != count_qualified)
      return false;

   if(metrics.recent_high_count >= strategy_recent_high_long_min)
      metrics.direction = 1;
   else if(metrics.recent_high_count <= strategy_recent_high_short_max)
      metrics.direction = -1;
   return true;
  }
                                        ...
                                        int &direction)
  {
   ...
   const bool valid =
      Strategy_LoadMonthlyEndpoints(current_month_key,
                                    ...
                                    closes,
                                    endpoint_times) &&
      Strategy_MedianScoreSignal(closes, metrics);
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
   ...
   else
     {
      ...
      g_signal_valid =
         endpoints_valid &&
         Strategy_MedianScoreSignal(closes, g_signal_metrics);
      if(!g_signal_valid)
         g_signal_state = "median_score_validation_failed";
      else if(g_signal_metrics.direction == 0)
         g_signal_state = g_signal_metrics.pooled_tie
            ? "median_score_pooled_tie_flat"
            : "median_score_neutral_flat";
      else if(g_signal_metrics.direction > 0)
         g_signal_state = "median_score_shift_wti_long";
      else if(g_signal_metrics.direction < 0)
         g_signal_state = "median_score_shift_wti_short";
      else
   ...
   QM_LogEvent(QM_INFO,
               ...
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"endpoint_count\":%d,\"return_count\":%d,\"block_size\":%d,\"assignment_count\":%d,\"tail_count\":%d,\"recent_high_count\":%d,\"neutral_distance\":%d,\"pooled_tie\":%s,\"tail_fraction\":%.12f,\"returns\":\"%s\",\"sorted_membership\":\"%s\",\"oldest_close\":%.10f,\"newest_close\":%.10f,\"oldest_endpoint\":%I64d,\"newest_endpoint\":%I64d,\"state\":\"%s\"}",
                            ...
                            g_signal_metrics.recent_high_count,
                            g_signal_metrics.neutral_distance,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41257 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41257_WTI_MMEDSCORE524";
   ...
   req.reason = (g_signal_metrics.direction > 0)
      ? "WTI_MMEDSCORE524_LONG"
      : "WTI_MMEDSCORE524_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41257 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI non-neutral median-score trend holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41257_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41257\",\"ea\":\"wti-mmedscore524-tr\"}");
