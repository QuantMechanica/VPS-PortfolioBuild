...
#property description "QM5_21522 WTI Low-Downside-Beta Trend"
...
// Hollstein, Prokopczuk & Tharann (2021) downside-beta definition and null.
...
//   3. estimate WTI downside beta in two disjoint 252-return blocks;
//   4. admit the twelve-month WTI trend only when recent beta is lower.
// SP500.DWX is read-only and never receives a magic or order request.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 21522;
...
input int    strategy_beta_returns_per_block    = 252;
input int    strategy_beta_recent_block_offset  = 252;
input int    strategy_beta_common_closes        = 505;
input int    strategy_beta_history_bars_d1      = 900;
...
double   g_cache_beta_preceding         = 0.0;
double   g_cache_beta_recent            = 0.0;
double   g_cache_market_mean_preceding  = 0.0;
double   g_cache_market_mean_recent     = 0.0;
int      g_cache_down_days_preceding    = 0;
int      g_cache_down_days_recent       = 0;
...
bool Strategy_LoadSynchronizedBetaHistory(const datetime decision_bar_time,
                                          ...
                                          double &sp500_closes[])
  {
   ...
   if(decision_bar_time <= 0 ||
      strategy_beta_returns_per_block != 252 ||
      strategy_beta_recent_block_offset != 252 ||
      strategy_beta_common_closes != 505 ||
      strategy_beta_history_bars_d1 != 900 ||
      strategy_beta_common_closes !=
         2 * strategy_beta_returns_per_block + 1)
      return false;
   ...
   const int sp500_count = CopyRates(g_factor_symbol, // perf-allowed
                                     ...
                                     strategy_beta_history_bars_d1,
                                     sp500_rates);
   if(xti_count < strategy_beta_common_closes ||
      sp500_count < strategy_beta_common_closes ||
      !Strategy_ValidateRates(xti_rates, xti_count) ||
   ...
   if(common_count < strategy_beta_common_closes)
      return false;
   const int selected_start = common_count - strategy_beta_common_closes;
   if(ArrayResize(common_times, strategy_beta_common_closes) !=
         strategy_beta_common_closes ||
      ArrayResize(xti_closes, strategy_beta_common_closes) !=
         strategy_beta_common_closes ||
      ArrayResize(sp500_closes, strategy_beta_common_closes) !=
         strategy_beta_common_closes)
      return false;

   for(int k = 0; k < strategy_beta_common_closes; ++k)
     {
   ...
   const datetime newest_common =
      common_times[strategy_beta_common_closes - 1];
...
bool Strategy_DownsideBetaBlock(const double &asset_returns[],
                                const double &market_returns[],
                                const int offset,
                                const int length,
                                double &downside_beta,
                                double &market_mean_return,
                                int &downside_days)
  {
   downside_beta = 0.0;
   market_mean_return = 0.0;
   downside_days = 0;
   const int total_returns = ArraySize(asset_returns);
   if(total_returns != ArraySize(market_returns) ||
      total_returns != strategy_beta_common_closes - 1 ||
      length != strategy_beta_returns_per_block ||
      offset < 0 || offset + length > total_returns)
      return false;
   ...
   for(int i = offset; i < offset + length; ++i)
     {
      if(!MathIsValidNumber(asset_returns[i]) ||
         ...
         return false;
      market_mean_return += market_returns[i];
     }
   market_mean_return /= (double)length;
   if(!MathIsValidNumber(market_mean_return))
      return false;
   ...
   for(int i = offset; i < offset + length; ++i)
     {
      if(market_returns[i] >= market_mean_return)
         continue;
      const double market_delta =
         market_returns[i] - market_downside_mean;
      covariance +=
         (asset_returns[i] - asset_mean) * market_delta;
      market_variance += market_delta * market_delta;
     }

   if(market_variance <= strategy_variance_epsilon ||
      !MathIsValidNumber(market_variance) ||
      !MathIsValidNumber(covariance))
      return false;

   // Demeaning selected rows is intercept OLS. The common n or n-1 divisor
   // cancels in covariance / variance, so no population-only shortcut occurs.
   downside_beta = covariance / market_variance;
   return MathIsValidNumber(downside_beta);
  }

bool Strategy_LoadBetaState(const datetime decision_bar_time)
  {
   datetime common_times[];
   double xti_closes[];
   double sp500_closes[];
   if(!Strategy_LoadSynchronizedBetaHistory(decision_bar_time,
                                            common_times,
                                            xti_closes,
                                            sp500_closes))
      return false;
   g_cache_common_closes = ArraySize(common_times);
   if(g_cache_common_closes != strategy_beta_common_closes)
      return false;

   const int return_count = strategy_beta_common_closes - 1;
   double xti_returns[];
   ...
   if(!Strategy_DownsideBetaBlock(xti_returns,
                                  sp500_returns,
                                  0,
                                  strategy_beta_returns_per_block,
                                  g_cache_beta_preceding,
                                  g_cache_market_mean_preceding,
                                  g_cache_down_days_preceding))
      return false;
   if(!Strategy_DownsideBetaBlock(xti_returns,
                                  sp500_returns,
                                  strategy_beta_recent_block_offset,
                                  strategy_beta_returns_per_block,
                                  g_cache_beta_recent,
                                  g_cache_market_mean_recent,
                                  g_cache_down_days_recent))
      return false;
   return true;
  }
...
void Strategy_ResetCachedState()
  {
   ...
   g_cache_beta_preceding = 0.0;
   g_cache_beta_recent = 0.0;
   g_cache_market_mean_preceding = 0.0;
   g_cache_market_mean_recent = 0.0;
   g_cache_down_days_preceding = 0;
   g_cache_down_days_recent = 0;
...
bool Strategy_LoadSignalState(const datetime decision_bar_time, int &signal)
  {
   ...
   if(!Strategy_LoadBetaState(decision_bar_time))
     {
      g_cache_state_reason = "invalid_downside_beta_state";
      return false;
     }

   if(!(g_cache_beta_recent <
        g_cache_beta_preceding - strategy_beta_tolerance))
     {
      g_cache_state_reason = "falling_beta_gate_blocked";
      return true;
     }
...
void Strategy_PrepareMonthlySignal()
  {
   ...
   QM_LogEvent(QM_INFO,
               "MONTHLY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"valid\":%s,\"signal\":%d,\"return_12m\":%.12e,\"beta_preceding\":%.12e,\"beta_recent\":%.12e,\"mean_preceding\":%.12e,\"mean_recent\":%.12e,\"down_preceding\":%d,\"down_recent\":%d,\"common_closes\":%d,\"state\":\"%s\"}",
                            g_cache_month_key,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 21522 ||
   ...
   if(strategy_trend_months != 12 ||
      ...
      strategy_beta_common_closes != 505 ||
      strategy_beta_history_bars_d1 != 900 ||
      strategy_min_down_days != 100)
      return true;
   if(MathAbs(strategy_beta_tolerance - 1.0e-12) > 1.0e-18 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_21522_WTI_LOWDB_TREND";
   ...
   req.reason = (g_cache_signal > 0) ?
                "LOWDB_TREND_XTI_LONG" : "LOWDB_TREND_XTI_SHORT";
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_21522_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_21522\",\"ea\":\"wti-lowdb-trend\",\"sp500\":\"read_only\"}");
