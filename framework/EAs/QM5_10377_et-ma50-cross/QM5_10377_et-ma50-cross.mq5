...
#property description "QM5_10377 Elite Trader MA50 Cross"
...
input bool   strategy_symmetric_shorts  = true;
input bool   strategy_use_d1_sma200     = false;
...
bool Strategy_HasOpenPosition(ENUM_POSITION_TYPE &pos_type)
  {
...
bool Strategy_D1TrendAllows(const QM_OrderType side)
  {
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   string reason = "";
   if(Strategy_CrossAboveMA())
     {
      side = QM_BUY;
      reason = "MA50_CROSS_LONG";
     }
   else if(strategy_symmetric_shorts && Strategy_CrossBelowMA())
     {
      side = QM_SELL;
      reason = "MA50_CROSS_SHORT";
     }
   else
      return false;

   if(!Strategy_D1TrendAllows(side))
      return false;
...
bool Strategy_ExitSignal()
  {
   ...
   if(pos_type == POSITION_TYPE_BUY)
      return Strategy_CrossBelowMA();
   if(pos_type == POSITION_TYPE_SELL)
      return Strategy_CrossAboveMA();
