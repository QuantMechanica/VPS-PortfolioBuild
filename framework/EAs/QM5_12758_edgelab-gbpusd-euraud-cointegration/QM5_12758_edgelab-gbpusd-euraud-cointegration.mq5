...
#property description "QM5_12758 Edge Lab GBPUSD EURAUD Cointegration"
...
input double strategy_beta              = 0.03;
...
bool Strategy_IsHostSymbol()
  {
   return (_Symbol == g_leg_gbpusd || _Symbol == g_leg_euraud);
  }
...
bool Strategy_EnsureBasketScope()
  {
   ...
   // accounting; GBPUSD itself already settles in USD.
   string allowed[3] = {"GBPUSD.DWX", "EURAUD.DWX", "AUDUSD.DWX"};
   for(int i = 0; i < 3; ++i)
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(!Strategy_EnsureBasketScope())
      return false;
   if(!QM_SymbolAssertOrLog(g_leg_gbpusd) || !QM_SymbolAssertOrLog(g_leg_euraud))
      return false;
   ...
   for(int i = 0; i < lookback; ++i)
     {
      if(gbpusd[i] <= 0.0 || euraud[i] <= 0.0)
         return false;
      spreads[i] = MathLog(gbpusd[i]) - strategy_beta * MathLog(euraud[i]);
...
bool Strategy_OpenPair(const int spread_direction)
  {
   ...
   const double weight_sum = gbpusd_weight + euraud_weight;
   if(weight_sum <= 0.0)
      return false;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12758_PAIR_HOST";
