...
#property description "QM5_20262 XNG Linear-Trend Quality"
...
// -----------------------------------------------------------------------------
// Source lineage: Moskowitz, Ooi & Pedersen (2012) monthly XNG own-price trend.
// At each broker-month transition:
//   1. derive thirteen consecutive completed XNG month-end closes from D1 data;
//   2. fit log(close) on the fixed chronological index 0..12;
//   3. trade the slope sign only when R-squared is at least 0.50;
//   4. consume the month flat when the path is weak, degenerate, or unavailable.
// The OLS path-quality gate is a transparent QM mechanization, not an author
// rule. Runtime uses native XNGUSD.DWX price and broker data only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 20262;
...
input int    strategy_regression_points   = 13;
input double strategy_min_r_squared       = 0.50;
input double strategy_slope_epsilon       = 1.0e-10;
...
double g_cache_slope            = 0.0;
double g_cache_r_squared        = 0.0;
double g_cache_sxx              = 0.0;
double g_cache_syy              = 0.0;
...
bool Strategy_LoadMonthlyCloses(double &closes[])
  {
   ...
   if(strategy_regression_points != 13 || strategy_history_bars_d1 != 800)
      return false;
   ...
   const int required_closes = strategy_regression_points;
...
void Strategy_ResetCachedState()
  {
   ...
   g_cache_signal = 0;
   g_cache_slope = 0.0;
   g_cache_r_squared = 0.0;
   g_cache_sxx = 0.0;
   g_cache_syy = 0.0;
   g_cache_state_reason = "not_evaluated";
  }

bool Strategy_RegressionSignal(const double &closes[], int &signal)
  {
   signal = 0;
   if(ArraySize(closes) != 13 || strategy_regression_points != 13)
     {
   ...
   double log_prices[];
   ArrayResize(log_prices, strategy_regression_points);
   double y_sum = 0.0;
   for(int i = 0; i < strategy_regression_points; ++i)
     {
      ...
      if(!MathIsValidNumber(log_prices[i]))
        {
         g_cache_state_reason = "bad_log";
         return false;
        }
      y_sum += log_prices[i];
     }

   const double x_bar = 6.0;
   const double y_bar = y_sum / (double)strategy_regression_points;
   double sxx = 0.0;
   double sxy = 0.0;
   double syy = 0.0;
   for(int i = 0; i < strategy_regression_points; ++i)
     {
      const double dx = (double)i - x_bar;
      const double dy = log_prices[i] - y_bar;
      sxx += dx * dx;
      sxy += dx * dy;
      syy += dy * dy;
     }
   ...
   g_cache_syy = syy;
   if(sxx <= 0.0 || syy <= 0.0 ||
      !MathIsValidNumber(sxx) || !MathIsValidNumber(sxy) ||
      !MathIsValidNumber(syy))
     {
      g_cache_state_reason = "degenerate_regression";
      return false;
     }

   const double slope = sxy / sxx;
   double r_squared = (sxy * sxy) / (sxx * syy);
   if(!MathIsValidNumber(slope) || !MathIsValidNumber(r_squared) ||
      r_squared < -1.0e-12 || r_squared > 1.0 + 1.0e-12)
     {
      g_cache_state_reason = "invalid_regression";
      return false;
     }

   r_squared = MathMax(0.0, MathMin(1.0, r_squared));
   g_cache_slope = slope;
   g_cache_r_squared = r_squared;
   if(MathAbs(slope) <= strategy_slope_epsilon)
     {
      g_cache_state_reason = "flat_slope";
      return true;
     }
   if(r_squared < strategy_min_r_squared)
     {
      g_cache_state_reason = "weak_fit";
      return true;
     }

   signal = (slope > 0.0) ? 1 : -1;
...
bool Strategy_LoadSignalState(int &signal)
  {
   ...
   return Strategy_RegressionSignal(monthly_closes, signal);
  }
...
void Strategy_PrepareMonthlySignal()
  {
   ...
   QM_LogEvent(QM_INFO,
               "MONTHLY_STATE",
               StringFormat("{\"month\":%d,\"valid\":%s,\"signal\":%d,\"slope\":%.12f,\"r2\":%.8f,\"sxx\":%.8f,\"syy\":%.12f,\"state\":\"%s\"}",
                            g_cache_month_key,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 20262 ||
   ...
   if(qm_stress_reject_probability != 0.0)
      return true;
   if(strategy_regression_points != 13 ||
      MathAbs(strategy_min_r_squared - 0.50) > 1.0e-12 ||
      MathAbs(strategy_slope_epsilon - 1.0e-10) > 1.0e-20)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20262_XNG_LR_TREND";
   ...
   req.reason = (g_cache_signal > 0) ? "LR_TREND_XNG_LONG" : "LR_TREND_XNG_SHORT";
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_20262_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_20262\",\"ea\":\"xng-lr-trend\"}");
