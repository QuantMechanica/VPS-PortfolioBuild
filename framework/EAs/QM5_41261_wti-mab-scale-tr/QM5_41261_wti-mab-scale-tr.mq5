...
#property description "QM5_41261 WTI Ansari-Bradley Scale-State Trend"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41261 - WTI Ansari-Bradley Symmetric-Rank Scale-State Trend
// -----------------------------------------------------------------------------
...
//   - sum symmetric end-rank scores carried by the actual recent block
//   - enumerate every one of the 924 fixed-size six/six label assignments
//   - require the locked inclusive lower-tail state at score 21 / count 522
...
input int    strategy_score_max              = 21;
input int    strategy_tail_count_max         = 522;
...
int Strategy_SymmetricScoreForRank(const int rank_index)
  {
   if(rank_index < 0 || rank_index >= strategy_return_count)
      return -1;
   const int left_score = rank_index + 1;
   const int right_score = strategy_return_count - rank_index;
   return (left_score < right_score) ? left_score : right_score;
  }

bool Strategy_SymmetricScoreForMask(const int mask,
                                    int &symmetric_score)
  {
   symmetric_score = 0;
   if(mask < 0 || mask >= 4096 ||
   ...
   for(int rank = 0; rank < strategy_return_count; ++rank)
     {
      const int rank_score = Strategy_SymmetricScoreForRank(rank);
      if(rank_score <= 0)
         return false;
      if((mask & (1 << rank)) != 0)
         symmetric_score += rank_score;
     }
   return (symmetric_score >= 12 && symmetric_score <= 30);
  }

int Strategy_ExpectedLowerTailCount(const int symmetric_score)
  {
   switch(symmetric_score)
     {
      case 12: return 1;
      case 13: return 5;
      case 14: return 14;
...
bool Strategy_AnsariBradleySignal(const double &closes[],
                                  Strategy_SignalMetrics &metrics)
  {
   ...
   if(Strategy_Popcount12(observed_mask) != strategy_block_size ||
      !Strategy_SymmetricScoreForMask(observed_mask,
                                      metrics.symmetric_score) ||
   ...
   for(int mask = 0; mask < 4096; ++mask)
     {
      if(Strategy_Popcount12(mask) != strategy_block_size)
         continue;
      int perm_score = 0;
      if(!Strategy_SymmetricScoreForMask(mask, perm_score))
         return false;
      if(perm_score <= metrics.symmetric_score)
         ++metrics.tail_count;
   ...
   if(metrics.assignment_count != strategy_assignment_count ||
      ...
      return false;
   const int expected_tail_count =
      Strategy_ExpectedLowerTailCount(metrics.symmetric_score);
   if(metrics.tail_count != expected_tail_count)
      return false;
                                        ...
                                        int &direction)
  {
   ...
   const bool valid =
      Strategy_LoadMonthlyEndpoints(current_month_key,
                                    ...
                                    closes,
                                    endpoint_times) &&
      Strategy_AnsariBradleySignal(closes, metrics);
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
   ...
   else
     {
      ...
      g_signal_valid =
         endpoints_valid &&
         Strategy_AnsariBradleySignal(closes, g_signal_metrics);
      if(!g_signal_valid)
         g_signal_state = "ansari_bradley_validation_failed";
      else if(g_signal_metrics.direction == 0)
        {
         if(g_signal_metrics.pooled_tie)
            g_signal_state = "ansari_bradley_pooled_tie_flat";
         else if(g_signal_metrics.symmetric_score > strategy_score_max)
            g_signal_state = "ansari_bradley_score_tail_flat";
         else
            g_signal_state = "ansari_bradley_neutral_return_flat";
        }
      else if(g_signal_metrics.direction > 0)
         g_signal_state = "ansari_bradley_wti_long";
      else if(g_signal_metrics.direction < 0)
         g_signal_state = "ansari_bradley_wti_short";
      else
   ...
   QM_LogEvent(QM_INFO,
               ...
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"endpoint_count\":%d,\"return_count\":%d,\"block_size\":%d,\"assignment_count\":%d,\"tail_count\":%d,\"symmetric_score\":%d,\"recent_return\":%.12f,\"pooled_tie\":%s,\"tail_fraction\":%.12f,\"returns\":\"%s\",\"sorted_membership\":\"%s\",\"oldest_close\":%.10f,\"newest_close\":%.10f,\"oldest_endpoint\":%I64d,\"newest_endpoint\":%I64d,\"state\":\"%s\"}",
                            ...
                            g_signal_metrics.symmetric_score,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41261 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41261_WTI_MAB_SCALE_TR";
   ...
   req.reason = (g_signal_metrics.direction > 0)
      ? "WTI_MAB_SCALE_TR_LONG"
      : "WTI_MAB_SCALE_TR_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41261 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI Ansari-Bradley scale-state trend holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41261_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41261\",\"ea\":\"wti-mab-scale-tr\"}");
