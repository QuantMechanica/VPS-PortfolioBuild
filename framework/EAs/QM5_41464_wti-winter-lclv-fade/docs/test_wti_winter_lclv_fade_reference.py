"""Deterministic reference checks for QM5_41464 WTI winter lower-CLV reversion."""
...
class WtiWinterLowerClvReversionReferenceTest(unittest.TestCase):
    ...
    def test_lower_tercile_is_long(self) -> None:
        direction, clv = signal(self.current, self.bars(close=95.0))
    ...
    def test_middle_and_higher_closes_are_flat(self) -> None:
    ...
    def test_negative_body_can_also_signal(self) -> None:
        self.assertEqual(signal(self.current, self.bars(open_=109.0, close=95.0))[0], 1)
    ...
    def test_three_to_five_sessions_are_valid(self) -> None:
        for count in (3, 4, 5):
            self.assertEqual(signal(self.current, self.bars(close=95.0, count=count))[0], 1)
