# QM5_41464_wti-winter-lclv-fade - Strategy Spec

**EA ID:** QM5_41464
**Strategy ID:** `BURAKOV-CRABEL-YANG-WTI-WINTER-LCLV-FADE-20260913_S01`

## 1. Strategy Logic

At the first tradable `XTIUSD.DWX` D1 bar of each November-May normalized
broker week, aggregate exactly the immediately completed three-to-five-session
week. Buy only when its final close lies strictly below one third of its full
high-low range. Equality or any higher close is flat. Candle-body direction and
range rank are intentionally absent. Consume the decision week before fallible
gates and never retry.

Hold at most one long through the decision week. Use one frozen
...
`414640000`. Runtime uses native D1 OHLC/timestamps, quotes, spread, ATR,
...
Expected cadence is eight to twelve positions per full post-warm-up year. Q02
...
It combines peer-reviewed WTI November-May seasonality, academic
commodity reversal, and governed reputable completed-week/close-location
lineage. The exact weekly rule is an untested QM translation.

The rule differs from `QM5_12567` (XNG oscillator pullback),
`QM5_41442`/`QM5_41456` (two-week range-state symmetric fades), and
`QM5_20209`/`QM5_20218` (calendar-month return-sign rules). This candidate
uses one completed week, no range comparison, no body predicate, and a strict
lower-tercile close.
...
| entry | November-May calendar, one completed week, lower-tercile CLV, attempt, spread, ATR stop |
| management | long-only integrity, next-week closure, stale repair |
| close | framework reason mapping and broker hard stop |

## 9. Safety Boundary

Gaps, slippage, roll/basis, financing, winter-season instability, weekly-
horizon translation, and overlap with WTI candidates can dominate. No manual
...
| v1-q01 | 2026-09-13 | PACER audit and 10 reference tests PASS; strict compile/build PASS; Q02 not enqueued because CPU reached 100.0% against the 97.0% ceiling |
