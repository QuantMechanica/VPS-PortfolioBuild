...
#property description "QM5_11206 Freqtrade ASDTS MACD Trend State"
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(strategy_macd_fast <= 0 || strategy_macd_slow <= strategy_macd_fast ||
      ...
      strategy_warmup_bars < strategy_macd_slow + strategy_macd_signal)
      return true;

   const ENUM_TIMEFRAMES tf = (ENUM_TIMEFRAMES)_Period;
   const double macd_warm = QM_MACD_Main(_Symbol, tf,
                                         strategy_macd_fast,
                                         strategy_macd_slow,
                                         strategy_macd_signal,
                                         strategy_warmup_bars);
   const double signal_warm = QM_MACD_Signal(_Symbol, tf,
                                             strategy_macd_fast,
                                             strategy_macd_slow,
                                             strategy_macd_signal,
                                             strategy_warmup_bars);
   if(macd_warm == 0.0 && signal_warm == 0.0)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "FT_ASDTS_MACD_LONG";
   req.symbol_slot = qm_magic_slot_offset;
   req.expiration_seconds = 0;

   const ENUM_TIMEFRAMES tf = (ENUM_TIMEFRAMES)_Period;
   const double macd = QM_MACD_Main(_Symbol, tf,
                                    strategy_macd_fast,
                                    strategy_macd_slow,
                                    strategy_macd_signal,
                                    1);
   const double signal = QM_MACD_Signal(_Symbol, tf,
                                        strategy_macd_fast,
                                        strategy_macd_slow,
                                        strategy_macd_signal,
                                        1);
   if(macd <= 0.0 || macd <= signal)
      return false;
