...
#property description "QM5_13029 GBPCAD GBPNZD Cointegration"
...
string   g_leg_gbpcad = "GBPCAD.DWX";
...
bool Strategy_IsHostSymbol()
  {
   return (_Symbol == g_leg_gbpcad || _Symbol == g_leg_gbpnzd);
  }
...
bool Strategy_EnsureBasketScope()
  {
   ...
   string allowed[4] = {"GBPCAD.DWX", "GBPNZD.DWX", "USDCAD.DWX", "NZDUSD.DWX"};
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(!Strategy_EnsureBasketScope())
      return false;
   if(!QM_SymbolAssertOrLog(g_leg_gbpcad) || !QM_SymbolAssertOrLog(g_leg_gbpnzd))
      return false;

   double gbpcad[];
   double gbpnzd[];
   ArraySetAsSeries(gbpcad, true);
   ArraySetAsSeries(gbpnzd, true);
   if(CopyClose(g_leg_gbpcad, PERIOD_D1, 1, lookback, gbpcad) != lookback) // perf-allowed: Strategy_EntrySignal is called only after the framework QM_IsNewBar gate.
      return false;
   ...
   for(int i = 0; i < lookback; ++i)
     {
      if(gbpcad[i] <= 0.0 || gbpnzd[i] <= 0.0)
         return false;
      spreads[i] = MathLog(gbpcad[i]) - strategy_beta * MathLog(gbpnzd[i]);
...
bool Strategy_OpenPair(const int spread_direction)
  {
   ...
   const double gbpcad_weight = 1.0;
   const double gbpnzd_weight = MathAbs(strategy_beta);
   const double weight_sum = gbpcad_weight + gbpnzd_weight;
   if(weight_sum <= 0.0)
      return false;

   const bool long_spread = (spread_direction > 0);
   const QM_OrderType gbpcad_type = long_spread ? QM_BUY : QM_SELL;
   const QM_OrderType gbpnzd_type = long_spread
                                    ? (strategy_beta >= 0.0 ? QM_SELL : QM_BUY)
                                    : (strategy_beta >= 0.0 ? QM_BUY : QM_SELL);
   const string reason = long_spread ? "QM5_13029_LONG_SPREAD_Z_LT_NEG_ENTRY"
                                     : "QM5_13029_SHORT_SPREAD_Z_GT_POS_ENTRY";

   bool gbpcad_ok = Strategy_OpenLeg(g_leg_gbpcad, gbpcad_type, gbpcad_weight, weight_sum, reason);
   bool gbpnzd_ok = Strategy_OpenLeg(g_leg_gbpnzd, gbpnzd_type, gbpnzd_weight, weight_sum, reason);
   if(gbpcad_ok && gbpnzd_ok)
     {
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_13029_PAIR_HOST";
...
bool Strategy_NewsFilterHook(const datetime broker_time)
  {
   ...
   if(qm_news_temporal != QM_NEWS_TEMPORAL_OFF || qm_news_compliance != QM_NEWS_COMPLIANCE_NONE)
     {
      if(!QM_NewsAllowsTrade2(g_leg_gbpcad, broker_time, qm_news_temporal, qm_news_compliance))
         return true;
     ...
     {
      if(!QM_NewsAllowsTrade(g_leg_gbpcad, broker_time, qm_news_mode_legacy))
         return true;
...
int OnInit()
  {
   SymbolSelect(g_leg_gbpcad, true);
