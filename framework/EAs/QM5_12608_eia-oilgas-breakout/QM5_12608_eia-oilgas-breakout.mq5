...
#property description "QM5_12608 EIA Oil Gas Ratio Breakout"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_12608 - EIA Oil/Gas Ratio Breakout
// -----------------------------------------------------------------------------
// D1 two-leg energy basket:
//   spread = ln(XTIUSD.DWX) - beta * ln(XNGUSD.DWX)
//   upside channel break: buy ratio  = buy XTI, sell XNG
//   downside channel break: sell ratio = sell XTI, buy XNG
// Runtime uses MT5 OHLC only; no external EIA data, futures curve, API, or ML.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 12608;
...
input int    strategy_exit_lookback_d1    = 20;
input double strategy_beta                = 1.0;
input double strategy_breakout_buffer_sd  = 0.10;
...
bool Strategy_LoadSpreadSeries(const int count_raw, double &spreads[])
  {
   ...
   if(CopyClose(g_leg_xti, PERIOD_D1, 1, count, xti) != count) // perf-allowed: called only behind D1 new-bar or close-state guards.
      return false;
   if(CopyClose(g_leg_xng, PERIOD_D1, 1, count, xng) != count) // perf-allowed: called only behind D1 new-bar or close-state guards.
      return false;
   ...
   for(int i = 0; i < count; ++i)
     {
      if(xti[i] <= 0.0 || xng[i] <= 0.0)
         return false;
      spreads[i] = MathLog(xti[i]) - strategy_beta * MathLog(xng[i]);
...
bool Strategy_LoadSignalState(int &direction)
  {
   ...
   const int channel_lookback = MathMax(20, strategy_channel_lookback_d1);
   const int exit_lookback = MathMax(5, strategy_exit_lookback_d1);
   const int count = MathMax(channel_lookback + 1, exit_lookback);

   double spreads[];
   if(!Strategy_LoadSpreadSeries(count, spreads))
      return false;

   g_current_spread = spreads[0];

   double prior_high = spreads[1];
   double prior_low = spreads[1];
   double sum = 0.0;
   for(int i = 1; i <= channel_lookback; ++i)
     {
      prior_high = MathMax(prior_high, spreads[i]);
      prior_low = MathMin(prior_low, spreads[i]);
      sum += spreads[i];
     }
   const double mean = sum / (double)channel_lookback;
   double var_sum = 0.0;
   for(int i = 1; i <= channel_lookback; ++i)
     {
   ...
   const double sd = MathSqrt(var_sum / (double)MathMax(1, channel_lookback - 1));
   if(sd <= 0.0 || !MathIsValidNumber(sd))
      return false;

   double exit_sum = 0.0;
   for(int i = 0; i < exit_lookback; ++i)
      exit_sum += spreads[i];
   g_exit_midline = exit_sum / (double)exit_lookback;

   const double buffer = MathMax(0.0, strategy_breakout_buffer_sd) * sd;
   if(g_current_spread > prior_high + buffer)
      direction = 1;
   else if(g_current_spread < prior_low - buffer)
      direction = -1;
...
bool Strategy_OpenPair(const int ratio_direction)
  {
   ...
   const string reason = long_ratio ? "QM5_12608_LONG_XTI_XNG_BREAKOUT"
                                    : "QM5_12608_SHORT_XTI_XNG_BREAKOUT";
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart())
      return true;
   if(strategy_channel_lookback_d1 < 20 || strategy_exit_lookback_d1 < 5)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12608_RATIO_HOST";
...
bool Strategy_ExitSignal()
  {
   ...
   if(pair_direction > 0 && g_current_spread < g_exit_midline)
      Strategy_ClosePair(QM_EXIT_STRATEGY);
   else if(pair_direction < 0 && g_current_spread > g_exit_midline)
      Strategy_ClosePair(QM_EXIT_STRATEGY);
   return false;
  }
...
int OnInit()
  {
   ...
   QM_BasketWarmupHistory(basket_symbols, PERIOD_D1, MathMax(180, strategy_channel_lookback_d1 + strategy_exit_lookback_d1 + strategy_atr_period_d1 + 10));

   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_12608\",\"ea\":\"eia-oilgas-breakout\"}");
