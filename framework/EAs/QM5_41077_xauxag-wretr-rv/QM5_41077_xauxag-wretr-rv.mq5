...
#property description "QM5_41077 XAU/XAG Weekly Partial-Retracement Continuation"
...
// -----------------------------------------------------------------------------
// Reconstruct three synchronized completed broker-week-end XAU/XAG close pairs
// and form two adjacent changes in ln(XAU)-ln(XAG). When the newest relative
// return has the strict opposite sign and a strictly smaller absolute magnitude,
// follow that partial retracement toward the pre-impulse anchor with an equal-
// notional opposite-leg package. Consume one attempt per week and flatten at
// the next week boundary.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41077;
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41077 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_WRETR_ATTEMPT_WEEK", qm_ea_id);
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41077_XAUXAG_WRETR_RV";
...
bool Strategy_LoadWeeklyRetracement(const int current_week,
                                    int &direction)
  {
   direction = 0;
   ...
   if(g_old_relative_return > 0.0 &&
      g_new_relative_return < 0.0 && strictly_smaller)
      direction = -1;
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41077_XAUXAG_WRETR_RV_HOST";
   ...
   if(Strategy_OpenOwnedPositionCount() > 0 ||
     ...
     {
      signal_valid =
         Strategy_LoadWeeklyRetracement(g_signal_week_key, direction);
      if(!signal_valid)
         g_signal_state = "endpoint_validation_failed";
      else if(direction < 0)
         g_signal_state = "positive_impulse_partial_retracement_short_xau";
      else if(direction > 0)
         g_signal_state = "negative_impulse_partial_retracement_long_xau";
         ...
         g_signal_state = "no_strict_smaller_retracement_flat";
     }
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG weekly partial-retracement package exits at the next broker week"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41077\",\"ea\":\"xauxag-wretr-rv\"}");
