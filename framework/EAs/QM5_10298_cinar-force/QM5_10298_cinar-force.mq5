...
#property description "QM5_10298 Cinar Force Index Zero-Line"
...
bool Strategy_ForceIndex(const int period, double &out_fi)
  {
   out_fi = 0.0;
   if(period <= 1)
      return false;

   const int bars_needed = MathMax(period * 4 + 2, period + 2);
   MqlRates rates[];
   ArraySetAsSeries(rates, true);
   const int copied = CopyRates(_Symbol, strategy_signal_tf, 1, bars_needed, rates); // perf-allowed: bespoke closed-bar Force Index OHLCV window; caller is QM_IsNewBar-gated.
   if(copied < period + 2)
      return false;
   ...
   for(int i = copied - 2; i >= 0; --i)
     {
      ...
      if(close_now <= 0.0 || close_prev <= 0.0 || volume <= 0.0)
         continue;

      const double raw_force = (close_now - close_prev) * volume;
      if(!ema_ready)
        {
         ema = raw_force;
         ema_ready = true;
         ...
         ema = alpha * raw_force + (1.0 - alpha) * ema;
     }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   // Trade Entry: card requires D1 Force Index(13) zero-line direction.
   if(_Period != strategy_signal_tf)
      return false;
   if(strategy_force_period <= 1 || strategy_atr_period <= 0 || strategy_atr_sl_mult <= 0.0)
      return false;

   double force_index = 0.0;
   if(!Strategy_ForceIndex(strategy_force_period, force_index))
      return false;
   if(force_index == 0.0)
      return false;

   const QM_OrderType desired_side = (force_index > 0.0) ? QM_BUY : QM_SELL;
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      // Trade Close: stop-and-reverse on Force Index zero-line sign flip.
   ...
   req.sl = sl;
   req.tp = 0.0;
   req.reason = (desired_side == QM_BUY) ? "CINAR_FORCE_LONG" : "CINAR_FORCE_SHORT";
   req.symbol_slot = qm_magic_slot_offset;
   req.expiration_seconds = 0;
   return true;
  }
