"""Deterministic reference checks for QM5_41370 leader-persistence reversion."""
...
MQ5 = EA_DIR / "QM5_41370_xtixng-cs-leadpersist-rv.mq5"
SETFILE = (
    EA_DIR
    / "sets"
    / "QM5_41370_xtixng-cs-leadpersist-rv_QM5_41370_XTI_XNG_CS_LEADPERSIST_RV_D1_D1_backtest.set"
)
...
def leader_persistence_signal(
    ...
    if older_same and newer_same and d0 > epsilon and d1 > epsilon:
        direction = -1  # WTI led both weeks: sell XTI, buy XNG.
    elif older_same and newer_same and d0 < -epsilon and d1 < -epsilon:
        direction = 1  # Natural gas led both weeks: buy XTI, sell XNG.
...
class LeaderPersistenceReferenceTest(unittest.TestCase):
    def test_positive_common_shocks_keep_wti_leader_and_fade_wti(self) -> None:
        valid, direction, returns, counts = leader_persistence_signal(
            20260824, completed_bars((100.0, 50.0), (110.0, 52.0), (121.0, 54.0))
        )
    ...
    def test_positive_common_shocks_keep_xng_leader_and_fade_xng(self) -> None:
        valid, direction, returns, _ = leader_persistence_signal(
            20260824, completed_bars((100.0, 50.0), (104.0, 56.0), (108.0, 63.0))
        )
        self.assertTrue(valid)
        self.assertEqual(direction, 1)
    ...
    def test_switch_zero_and_mixed_sign_weeks_are_flat(self) -> None:
        ...
        for case in cases:
            with self.subTest(case=case):
                valid, direction, *_ = leader_persistence_signal(
    ...
    def test_static_artifacts_lock_identity_signal_and_fixed_risk(self) -> None:
        ...
        self.assertIn("qm_ea_id                    = 41370", source)
        ...
        self.assertNotIn("QM5_41369", source)
        ...
        self.assertEqual(manifest["logical_symbol"], "QM5_41370_XTI_XNG_CS_LEADPERSIST_RV_D1")
        ...
        self.assertRegex(resolver, re.compile(r"413700000.*413700001", re.DOTALL))
