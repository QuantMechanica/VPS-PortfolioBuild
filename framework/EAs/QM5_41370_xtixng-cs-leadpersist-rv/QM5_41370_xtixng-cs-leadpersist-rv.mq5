...
#property description "QM5_41370 XTI/XNG Common-Shock Leader-Persistence Reversion"
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41370 && qm_magic_slot_offset == 0 &&
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41370_XTIXNG_CS_LEADPERSIST_RV";
...
bool Strategy_LoadLeaderPersistence(const int current_week,
                                    int &direction)
  {
   ...
   if(older_same_sign && newer_same_sign &&
      ...
      direction = -1; // WTI remained the winner: sell XTI, buy XNG.
   else if(older_same_sign && newer_same_sign &&
           g_older_relative_return < -strategy_signal_epsilon &&
           g_newer_relative_return < -strategy_signal_epsilon)
      direction = 1; // natural gas remained the winner: buy XTI, sell XNG.
   return true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41370_XTIXNG_CS_LEADPERSIST_RV_HOST";
   ...
   if(Strategy_OpenOwnedPositionCount() > 0 ||
   ...
   else
     {
      ...
      if(!signal_valid)
         ...
         g_signal_state = "xng_relative_leader_persisted_fade_long_xti";
      else if(direction < 0)
         g_signal_state = "wti_relative_leader_persisted_fade_short_xti";
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41370\",\"ea\":\"xtixng-cs-leadpersist-rv\"}");
