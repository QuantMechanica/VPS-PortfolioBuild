...
#property description "QM5_12473 GitHub London Breakout"
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(strategy_threshold_minutes <= 0 || strategy_open_minutes <= 0 ||
      strategy_risky_stop_price <= 0.0)
      return false;
   ...
   const int day_key = now_dt.year * 1000 + now_dt.day_of_year;
   static int session_day_key = -1;
   static bool range_ready = false;
   static bool trade_taken = false;
   static double session_upper = 0.0;
   static double session_lower = 0.0;
   ...
   if(!range_ready)
     {
      ...
      MqlRates rates[];
      const int copied = CopyRates(_Symbol, PERIOD_M1, threshold_start, threshold_end, rates); // perf-allowed: closed-bar session range cache.
      if(copied <= 0)
         return false;
      ...
      if(bars_seen < MathMin(strategy_threshold_minutes, 30) || upper <= 0.0 || lower <= 0.0 || upper <= lower)
         return false;
   ...
   const double stop_distance = 0.5 * strategy_risky_stop_price;
   if(ask <= 0.0 || bid <= 0.0 || stop_distance <= 0.0)
      return false;

   if(ask > session_upper && (ask - session_upper) <= strategy_risky_stop_price)
     {
      req.type = QM_BUY;
      req.price = 0.0;
      req.sl = NormalizeDouble(ask - stop_distance, _Digits);
      req.tp = NormalizeDouble(ask + stop_distance, _Digits);
      req.reason = "GH_LONDON_BREAK_LONG";
      trade_taken = true;
      return true;
     }

   if(bid < session_lower && (session_lower - bid) <= strategy_risky_stop_price)
     {
      req.type = QM_SELL;
      req.price = 0.0;
      req.sl = NormalizeDouble(bid + stop_distance, _Digits);
      req.tp = NormalizeDouble(bid - stop_distance, _Digits);
      req.reason = "GH_LONDON_BREAK_SHORT";
      trade_taken = true;
      return true;
     }
