...
`artifacts/cards_approved/QM5_10250_tv-bb-scalp.md`. See that card's body for
...
`QM5_10250_tv-bb-scalp.mq5`. Framework wiring (risk, magic, news, Friday close)
...
| `strategy_ema_exit_period` | 8 | (see source) | (see strategy logic) |
| `strategy_ema_fast_period` | 12 | (see source) | (see strategy logic) |
| `strategy_ema_slow_period` | 26 | (see source) | (see strategy logic) |
| `strategy_use_ema200_filter` | true | (see source) | (see strategy logic) |
| `strategy_ema_trend_period` | 200 | (see source) | (see strategy logic) |
| `strategy_volume_sma_period` | 20 | (see source) | (see strategy logic) |
| `strategy_atr_period` | 14 | (see source) | (see strategy logic) |
| `strategy_fx_atr_sl_mult` | 1.2 | (see source) | (see strategy logic) |
| `strategy_index_sl_percent` | 0.35 | (see source) | (see strategy logic) |
...
| `strategy_spread_avg_bars` | 50 | (see source) | (see strategy logic) |
| `strategy_spread_avg_mult` | 1.5 | (see source) | (see strategy logic) |
...
`artifacts/cards_approved/QM5_10250_tv-bb-scalp.md`
