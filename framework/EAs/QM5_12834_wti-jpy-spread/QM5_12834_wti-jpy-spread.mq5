...
#property description "QM5_12834 WTI USDJPY Spread Mean Reversion"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_12834 - WTI/USDJPY oil-yen spread mean reversion
// -----------------------------------------------------------------------------
// D1 two-leg basket:
//   spread = ln(XTIUSD.DWX) - beta * ln(USDJPY.DWX)
//   rich spread: sell XTI, buy USDJPY
//   cheap spread: buy XTI, sell USDJPY
// Runtime uses MT5 OHLC only; no macro feed, futures curve, API, or ML.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                     = 12834;
...
input int    strategy_usdjpy_max_spread_pts = 60;
...
bool Strategy_SpreadAllowed(const string symbol)
  {
   ...
   if(symbol == g_leg_usdjpy && strategy_usdjpy_max_spread_pts > 0)
      return (spread_points <= strategy_usdjpy_max_spread_pts);
   return true;
  }
...
bool Strategy_EntryTimeReady(const datetime broker_time)
  {
   ...
   if(!Strategy_SymbolTradeReady(g_leg_xti, broker_time))
      return false;
   if(!Strategy_SymbolTradeReady(g_leg_usdjpy, broker_time))
      return false;
   return true;
  }
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(CopyClose(g_leg_xti, PERIOD_D1, 1, lookback, xti) != lookback) // perf-allowed: called only behind D1 new-bar or close-state refresh.
      return false;
   if(CopyClose(g_leg_usdjpy, PERIOD_D1, 1, lookback, usdjpy) != lookback) // perf-allowed: called only behind D1 new-bar or close-state refresh.
      return false;
   ...
   for(int i = 0; i < lookback; ++i)
     {
      if(xti[i] <= 0.0 || usdjpy[i] <= 0.0)
         return false;
      spreads[i] = MathLog(xti[i]) - strategy_beta * MathLog(usdjpy[i]);
...
bool Strategy_OpenPair(const int spread_direction)
  {
   if(spread_direction == 0 || Strategy_OpenPairLegCount() > 0)
      return false;
   if(!Strategy_SpreadAllowed(g_leg_xti) || !Strategy_SpreadAllowed(g_leg_usdjpy))
      return false;

   const double xti_weight = 1.0;
   const double usdjpy_weight = MathMax(0.1, MathAbs(strategy_beta));
   const double weight_sum = xti_weight + usdjpy_weight;
   const bool long_spread = (spread_direction > 0);
   const QM_OrderType xti_type = long_spread ? QM_BUY : QM_SELL;
   const QM_OrderType usdjpy_type = long_spread ? QM_SELL : QM_BUY;
   const string reason = long_spread ? "QM5_12834_LONG_XTI_USDJPY_SPREAD"
                                     : "QM5_12834_SHORT_XTI_USDJPY_SPREAD";
   ...
   bool usdjpy_ok = Strategy_OpenBasketLeg(g_leg_usdjpy, usdjpy_type, usdjpy_weight, weight_sum, reason);
   if(usdjpy_ok)
     {
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12834_SPREAD_HOST";
