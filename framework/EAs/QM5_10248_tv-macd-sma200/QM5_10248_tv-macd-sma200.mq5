...
#property description "QM5_10248 MACD Momentum SMA200 Filter"
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   const double macd_signal = QM_MACD_Signal(_Symbol,
                                             ...
                                             strategy_macd_signal,
                                             1);
   const double hist = macd_main - macd_signal;
   const double sma = QM_SMA(_Symbol, strategy_signal_tf, strategy_sma_period, 1);
   const double close = QM_SMA(_Symbol, strategy_signal_tf, 1, 1, PRICE_CLOSE);
   if(close <= 0.0 || sma <= 0.0)
      return false;

   int direction = 0;
   if(hist > 0.0 && macd_main > 0.0 && macd_main > macd_signal && close > sma)
      direction = 1;
   if(hist < 0.0 && macd_main < 0.0 && macd_main < macd_signal && close < sma)
      direction = -1;
   if(direction == 0)
