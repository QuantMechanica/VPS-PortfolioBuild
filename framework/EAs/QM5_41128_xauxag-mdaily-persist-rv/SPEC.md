# QM5_41128_xauxag-mdaily-persist-rv - Strategy Spec

**EA ID:** QM5_41128

**Slug:** `xauxag-mdaily-persist-rv`

**Strategy ID:** `SCHWEIKERT-MEHLITZ-CME-XAUXAG-MDAILY-PERSIST-RV-2026_S01`

**Source:** `SCHWEIKERT-MEHLITZ-CME-XAUXAG-MDAILY-PERSIST-RV-2026`
...
`N` be their signed sum, `mu=N/n`, `S` the sum of squared centered returns,
`A` the sum of adjacent centered-return products, `rho=A/S`, and
`J=rho+1/(n-1)`. Require endpoint identity, finite arithmetic, `S>0`, bounded
`rho`, and strict `J>0`. Fade positive `N` with SELL XAU / BUY XAG and fade
negative `N` with BUY XAU / SELL XAG. Every other valid or malformed state
...
- Host/traded slot 0: exact `XAUUSD.DWX`, D1, magic `411280000`.
- Companion/traded slot 1: exact `XAGUSD.DWX`, D1, magic `411280001`.
...
- Trigger: lag-one centered-return score `rho+1/(n-1)>0`.
...
and Julia S. Mehlitz and Benjamin R. Auer (2024), *The European Journal of
Finance* 30(8), 773-802, DOI `10.1080/1351847X.2023.2220118`.
...
lineage, commodity-return memory concept, monthly clock, and auditable
statistic. The within-month daily ratio horizon, fixed correction, fade
direction, CFD mapping, execution, and risk are disclosed QM hypotheses; no
...
signal price, fitted threshold, signal-strength sizing, external feed, retry,
...
- trade_entry: month attempt, exact synchronized package, centered-return
  persistence statistic, endpoint identity, spread/quote/ATR/stop checks,
  equal-notional sizing, and atomic two-order submission.
...
|---|---|---|---|
| v0 | 2026-08-23 | approved source build | source approval `8889d7f32`; card `711b92417`; governed magics `411280000`/`411280001` |
