...
#property description "QM5_41128 XAU/XAG Completed-Month Daily-Persistence Reversion"
...
// Center every gold-minus-silver relative return, compute lag-one dependence
// rho and the fixed short-sample score J=rho+1/(n-1), and require strict J>0.
// Fade the completed-month endpoint with one opposite, equal-notional package
// held to the next month. Runtime is native, completed-price-only,
// deterministic, and one-attempt-per-month.
// =============================================================================
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41128 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
            ...
            strategy_sample_bias_adjustment &&
            MathAbs(strategy_persistence_threshold) <= 1.0e-12 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_MDAILY_PERSIST_RV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41128_XAUXAG_MDAILY_PERSIST_RV";
...
bool Strategy_LoadMonthlyDailyPersistence(
      ...
      double &adjacent_product_sum,
      double &lag_one_autocorrelation,
      double &bias_adjustment,
      ...
      int &direction)
  {
   ...
   const int xau_copied =
      CopyRates(g_leg_xau, // perf-allowed: one bounded completed-month scan behind a consumed monthly attempt.
                PERIOD_D1, 1, strategy_history_bars_d1, xau_bars);
   const int xag_copied =
      CopyRates(g_leg_xag, // perf-allowed: one bounded completed-month scan behind a consumed monthly attempt.
   ...
   // every gold-minus-silver relative return ending inside it appears once.
   ...
   for(int i = 1; i <= completed_month_sessions; ++i)
     {
      ...
      if(!MathIsValidNumber(net_displacement))
         return false;
     }
   ...
   if(!MathIsValidNumber(endpoint_displacement) ||
      MathAbs(net_displacement - endpoint_displacement) >
   ...
   for(int i = 0; i < return_count; ++i)
     {
      ...
      if(!MathIsValidNumber(squared_deviation_sum))
         return false;
     }

   // Zero variance is a valid consumed flat state: rho is undefined, so do
   // not apply the bias adjustment or divide by zero.
   ...
   for(int i = 1; i < return_count; ++i)
     {
      const double current_centered = daily_returns[i] - return_mean;
      const double prior_centered = daily_returns[i - 1] - return_mean;
      const double product = current_centered * prior_centered;
      if(!MathIsValidNumber(product))
         return false;
      adjacent_product_sum += product;
      if(!MathIsValidNumber(adjacent_product_sum))
         return false;
     }

   lag_one_autocorrelation =
      adjacent_product_sum / squared_deviation_sum;
   if(!MathIsValidNumber(lag_one_autocorrelation) ||
      lag_one_autocorrelation <
         -1.0 - strategy_numerical_tolerance ||
      lag_one_autocorrelation >
         1.0 + strategy_numerical_tolerance)
      return false;
   if(lag_one_autocorrelation < -1.0)
      lag_one_autocorrelation = -1.0;
   if(lag_one_autocorrelation > 1.0)
      lag_one_autocorrelation = 1.0;

   bias_adjustment = strategy_sample_bias_adjustment
      ? 1.0 / (double)(return_count - 1)
      : 0.0;
   persistence_score = lag_one_autocorrelation + bias_adjustment;
   if(!MathIsValidNumber(bias_adjustment) ||
      !MathIsValidNumber(persistence_score))
      return false;

   // direction > 0 means BUY XAU / SELL XAG. Strictly positive persistence
   // qualifies, and the completed-month relative displacement is faded.
   if(persistence_score > strategy_persistence_threshold)
     {
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41128_XAUXAG_MDAILY_PERSIST_RV_HOST";
   ...
   double lag_one_autocorrelation = 0.0;
   double bias_adjustment = 0.0;
   double persistence_score = 0.0;
   const bool valid =
      Strategy_LoadMonthlyDailyPersistence(
         g_signal_month_key,
         completed_month_sessions,
         return_count,
         net_displacement,
         ...
         persistence_score,
         direction);
   string state = "monthly_daily_persistence_failed";
   if(valid && direction > 0)
      state = "persistent_negative_net_fade_long_xau";
   else if(valid && direction < 0)
      state = "persistent_positive_net_fade_short_xau";
   else if(valid && return_count > 0 &&
           squared_deviation_sum == 0.0)
      state = "zero_variance_flat";
   else if(valid && net_displacement == 0.0)
      state = "net_zero_flat";
   else if(valid)
      state = "nonpositive_persistence_score_flat";

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"completed_month_sessions\":%d,\"return_count\":%d,\"net_displacement\":%.12e,\"endpoint_displacement\":%.12e,\"return_mean\":%.12e,\"squared_deviation_sum\":%.12e,\"adjacent_product_sum\":%.12e,\"lag_one_autocorrelation\":%.12e,\"bias_adjustment\":%.12e,\"persistence_score\":%.12e,\"threshold\":%.12e,\"direction\":%d,\"state\":\"%s\"}",
                            g_signal_month_key,
                            completed_month_sessions,
                            return_count,
                            net_displacement,
                            ...
                            bias_adjustment,
                            persistence_score,
                            strategy_persistence_threshold,
                            direction,
                            state));
   if(!valid || direction == 0)
      return false;
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG completed-month daily-persistence reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41128\",\"ea\":\"xauxag-mdaily-persist-rv\"}");
