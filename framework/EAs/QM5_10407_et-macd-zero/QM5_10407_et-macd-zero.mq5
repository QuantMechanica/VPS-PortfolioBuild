...
#property description "QM5_10407 Elite Trader MACD Zero-Line Reversal"
...
input double strategy_time_stop_min_r    = 0.25;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(strategy_session_end_hhmm >= 0 && now_hhmm >= strategy_session_end_hhmm)
      return false;
   ...
   req.tp = 0.0;
   req.reason = cross_up ? "ET_MACD_ZERO_LONG" : "ET_MACD_ZERO_SHORT";
   return true;
  }
      ...
      if(strategy_time_stop_bars > 0 && strategy_time_stop_min_r > 0.0)
        {
         ...
         if(opened_at > 0 && bar_seconds > 0 &&
            TimeCurrent() - opened_at >= strategy_time_stop_bars * bar_seconds)
           {
            ...
            const double initial_r = MathAbs(open_price - sl);
            const double open_profit = (ptype == POSITION_TYPE_BUY)
                                       ? (bid - open_price)
                                       : (open_price - ask);
            if(initial_r > 0.0 && open_profit < strategy_time_stop_min_r * initial_r)
               return true;
           }
        }
     }
