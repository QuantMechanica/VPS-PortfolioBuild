...
input int             strategy_cmo_length     = 14;
...
bool Strategy_NoTradeFilter()
  {
   // Card declares no extra strategy-specific time, spread, or regime filter.
   return false;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(strategy_cmo_length <= 1 ||
      strategy_signal_bar < 1 ||
   ...
   for(int sample = 0; sample < 2; ++sample)
     {
      const int shift = strategy_signal_bar + sample;
      double sum_up = 0.0;
      double sum_down = 0.0;
      ...
      const double cmo = (denom > 0.0) ? (100.0 * (sum_up - sum_down) / denom) : 0.0;
      if(sample == 0)
         cmo_last = cmo;
      else
         cmo_prev = cmo;
   ...
   const bool bullish_flip = (cmo_prev < 0.0 && cmo_last > 0.0);
   const bool bearish_flip = (cmo_prev > 0.0 && cmo_last < 0.0);
   ...
   req.sl = sl;
   req.tp = 0.0;
   req.reason = bullish_flip ? "CMO_CLOUD_BULLISH_COLOR_CHANGE"
                             : "CMO_CLOUD_BEARISH_COLOR_CHANGE";
   return true;
  }
...
bool Strategy_ExitSignal()
  {
   ...
   if(strategy_cmo_length <= 1 || strategy_signal_bar < 1)
      return false;
