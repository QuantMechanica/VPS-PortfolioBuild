...
#property description "QM5_41073 WTI Completed-Week Outside-Settlement Momentum"
...
//   - require a strict higher high and lower low in the newer week
//   - require settlement beyond the matching parent extreme
//   - require matching first-open-to-last-close sign and outer-quartile close
//   - consume the week before fallible gates; never enter late or retry
//   - follow the completed outside-settlement direction for one week
// Runtime uses MT5-native OHLC, calendar, history, and framework state only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                      = 41073;
...
input double strategy_close_quartile        = 0.75;
...
bool Strategy_LoadOutsideSettlementSignal(const int current_week_key,
                                          const int label_offset,
                                          int &direction,
                                          bool &outside_week,
                                          int &new_week_bars,
                                          ...
                                          double &close_location)
  {
   ...
   outside_week = (new_high > parent_high && new_low < parent_low);
   ...
   const double lower_quartile = 1.0 - strategy_close_quartile;
   if(outside_week &&
      new_close > new_open &&
      new_close > parent_high &&
      close_location > strategy_close_quartile)
      direction = 1;
   else if(outside_week &&
           new_close < new_open &&
           new_close < parent_low &&
           close_location < lower_quartile)
      direction = -1;
   return true;
  }
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_WeekAlreadyEntered(g_decision_week_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadOutsideSettlementSignal(g_decision_week_key,
      ...
      if(!g_signal_valid)
         ...
         g_signal_state = "outside_upper_settlement_long";
      else if(g_signal_direction < 0)
         g_signal_state = "outside_lower_settlement_short";
      else if(!g_outside_week)
         g_signal_state = "not_outside_flat";
      else if(g_new_close == g_new_open)
         g_signal_state = "own_week_zero_flat";
      else if(g_new_close == g_parent_high ||
              g_new_close == g_parent_low)
         g_signal_state = "parent_boundary_equality_flat";
      else if(g_new_close < g_parent_high &&
              g_new_close > g_parent_low)
         g_signal_state = "settlement_inside_parent_flat";
      else if(g_close_location == strategy_close_quartile ||
              g_close_location == 1.0 - strategy_close_quartile)
         g_signal_state = "quartile_equality_flat";
      else
         g_signal_state = "directional_confluence_failed_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"week\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_week_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"outside\":%s,\"new_week_bars\":%d,\"parent_week_bars\":%d,\"new_open\":%.8f,\"new_high\":%.8f,\"new_low\":%.8f,\"new_close\":%.8f,\"parent_open\":%.8f,\"parent_high\":%.8f,\"parent_low\":%.8f,\"parent_close\":%.8f,\"close_location\":%.12f,\"state\":\"%s\"}",
                            g_decision_week_key,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41073 ||
   ...
   if(strategy_entry_grace_minutes != 180 ||
      ...
      MathAbs(strategy_close_quartile - 0.75) > 1.0e-12 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41073_WTI_WOUTSIDE_SETTLE";
   ...
   req.reason = (g_signal_direction > 0)
      ? "WTI_WOUTSIDE_SETTLE_LONG"
      : "WTI_WOUTSIDE_SETTLE_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_strategy_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41073 ||
   ...
   g_attempt_state_key =
      StringFormat("QM5_41073_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41073\",\"ea\":\"wti-woutside-settle\"}");
