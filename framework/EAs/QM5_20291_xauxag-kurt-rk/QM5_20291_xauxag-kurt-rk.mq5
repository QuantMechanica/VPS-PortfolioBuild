...
#property description "QM5_20291 XAU XAG Historical Kurtosis Rank"
...
//   - calculate each leg's Pearson historical kurtosis from exactly 252
//     completed simple D1 returns using the source-defined denominators
//   - long the higher-kurtosis precious-metal leg and short the lower leg
// Runtime is Darwinex-native D1 OHLC only; no external or futures-chain data.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 20291;
...
input double strategy_variance_floor          = 1.0e-16;
...
double   g_cache_xau_kurtosis = 0.0;
double   g_cache_xag_kurtosis = 0.0;
double   g_cache_kurtosis_difference = 0.0;
...
bool Strategy_HistoricalKurtosis(const string symbol,
                                 const datetime decision_bar_time,
                                 double &kurtosis_measure,
                                 int &observation_count)
  {
   kurtosis_measure = 0.0;
   observation_count = 0;
   ...
   if(observation_count != strategy_lookback_d1 || observation_count < 2)
      return false;

   const double mean_return = return_sum / (double)observation_count;
   if(!MathIsValidNumber(mean_return))
      return false;
   ...
   for(int i = 0; i < observation_count; ++i)
     {
      ...
      fourth_sum += centered_sq * centered_sq;
     }

   const double sample_variance =
      squared_sum / (double)(observation_count - 1);
   const double fourth_central_moment =
      fourth_sum / (double)observation_count;
   if(sample_variance <= strategy_variance_floor ||
      fourth_central_moment <= 0.0 ||
      !MathIsValidNumber(sample_variance) ||
      !MathIsValidNumber(fourth_central_moment))
      return false;

   const double variance_squared = sample_variance * sample_variance;
   if(variance_squared <= 0.0 || !MathIsValidNumber(variance_squared))
      return false;
   kurtosis_measure = fourth_central_moment / variance_squared;
...
bool Strategy_LoadSignalState(const datetime decision_bar_time,
  ...
  {
   pair_direction = 0;
   g_cache_xau_kurtosis = 0.0;
   g_cache_xag_kurtosis = 0.0;
   g_cache_kurtosis_difference = 0.0;
   g_cache_xau_observations = 0;
   g_cache_xag_observations = 0;

   if(!Strategy_HistoricalKurtosis(g_leg_xau,
                                   decision_bar_time,
                                   g_cache_xau_kurtosis,
                                   g_cache_xau_observations))
      return false;
   if(!Strategy_HistoricalKurtosis(g_leg_xag,
                                   decision_bar_time,
                                   g_cache_xag_kurtosis,
                                   g_cache_xag_observations))
      return false;

   g_cache_kurtosis_difference =
      g_cache_xau_kurtosis - g_cache_xag_kurtosis;
   if(!MathIsValidNumber(g_cache_kurtosis_difference))
      return false;
   if(g_cache_kurtosis_difference > strategy_rank_tolerance)
      pair_direction = 1;
   else if(g_cache_kurtosis_difference < -strategy_rank_tolerance)
      pair_direction = -1;
   return true;
  }
...
bool Strategy_OpenPair(const int pair_direction)
  {
   ...
   const string reason = long_xau_short_xag ? "QM5_20291_LONG_XAU_SHORT_XAG_HIGHKURT"
                                            : "QM5_20291_SHORT_XAU_LONG_XAG_HIGHKURT";
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart())
      return true;
   if(qm_ea_id != 20291 || qm_magic_slot_offset != 0 || qm_rng_seed != 42)
      return true;
   ...
   if(strategy_lookback_d1 != 252 || strategy_history_bars != 320 ||
      ...
      MathAbs(strategy_variance_floor - 1.0e-16) > 1.0e-24 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20291_XAU_XAG_HKURT_HOST";
...
int OnInit()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 20291 ||
   ...
   g_attempt_state_key =
      StringFormat("QM5_20291_MONTH_ATTEMPT_%d",
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_20291\",\"ea\":\"xauxag-kurt-rk\"}");
