...
#property description "QM5_20279 WTI Exponential-Recency Return Momentum"
...
//   3. give the newest return age zero and each older return one more month;
//   4. apply 2^(-age/3.0), normalize the twelve weights, and trade the sign.
// The fixed exponential-recency statistic is a transparent QM mechanization,
// not an author rule. Runtime uses native XTIUSD.DWX broker data only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 20279;
...
input double strategy_half_life_months    = 3.0;
...
void Strategy_ResetCachedState()
  {
   ...
   g_cache_signal = 0;
   g_cache_expw_mean = 0.0;
   g_cache_state_reason = "not_evaluated";
  }

bool Strategy_ExponentialWeightedMeanSignal(const double &closes[], int &signal)
  {
   signal = 0;
   const int return_count = strategy_return_months;
   if(return_count != 12 ||
      MathAbs(strategy_half_life_months - 3.0) > 1.0e-12 ||
      ArraySize(closes) != return_count + 1)
     {
      g_cache_state_reason = "bad_expw_contract";
      return false;
     }
   ...
   for(int i = 0; i < return_count; ++i)
     {
      ...
      const int age_months = return_count - 1 - i;
      if(age_months != 11 - i || age_months < 0 || age_months > 11)
        {
         g_cache_state_reason = "invalid_return_age";
         return false;
        }
      const double exponent = -(double)age_months / strategy_half_life_months;
      const double weight = MathPow(2.0, exponent);
      if(weight <= 0.0 || weight > 1.0 || !MathIsValidNumber(weight))
        {
         g_cache_state_reason = "invalid_exponential_weight";
         return false;
        }
      if((age_months == 0 && MathAbs(weight - 1.0) > 1.0e-12) ||
         (age_months == 3 && MathAbs(weight - 0.5) > 1.0e-12) ||
         (age_months == 6 && MathAbs(weight - 0.25) > 1.0e-12) ||
         (age_months == 9 && MathAbs(weight - 0.125) > 1.0e-12))
        {
         g_cache_state_reason = "half_life_anchor_failed";
         return false;
        }

      const double weighted_return = weight * monthly_return;
      if(!MathIsValidNumber(weighted_return))
        {
         g_cache_state_reason = "invalid_weighted_return";
         return false;
        }
      weighted_sum += weighted_return;
      weight_total += weight;
      if(!MathIsValidNumber(weighted_sum) ||
         !MathIsValidNumber(weight_total))
        {
         g_cache_state_reason = "invalid_weighted_state";
         return false;
        }
     }

   if(weight_total <= 0.0 || !MathIsValidNumber(weight_total))
     {
      g_cache_state_reason = "invalid_weight_total";
      return false;
     }

   const double expw_mean = weighted_sum / weight_total;
   if(!MathIsValidNumber(expw_mean))
     {
      g_cache_state_reason = "invalid_expw_mean";
      return false;
     }

   g_cache_expw_mean = expw_mean;
   if(expw_mean == 0.0)
     {
      g_cache_state_reason = "exact_zero_expw_mean";
      return true;
     }

   signal = (expw_mean > 0.0) ? 1 : -1;
...
bool Strategy_LoadSignalState(int &signal)
  {
   ...
   return Strategy_ExponentialWeightedMeanSignal(monthly_closes, signal);
  }
...
void Strategy_PrepareMonthlySignal()
  {
   ...
   QM_LogEvent(QM_INFO,
               "MONTHLY_STATE",
               StringFormat("{\"month\":%d,\"valid\":%s,\"signal\":%d,\"expw_mean\":%.10f,\"half_life\":%.1f,\"state\":\"%s\"}",
                            g_cache_month_key,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 20279 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20279_WTI_EXPW_MOM";
   ...
   req.reason = (g_cache_signal > 0) ? "EXPW_MOM_XTI_LONG" : "EXPW_MOM_XTI_SHORT";
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_20279_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_20279\",\"ea\":\"wti-expw-mom\"}");
