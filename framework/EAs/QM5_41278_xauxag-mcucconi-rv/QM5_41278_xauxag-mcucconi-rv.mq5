...
#property description "QM5_41278 XAU/XAG Monthly Exact Cucconi Reversion"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_41278 - XAU/XAG Monthly Exact Cucconi Reversion
// -----------------------------------------------------------------------------
...
// changes into fixed old/recent blocks of six, reject relative pooled ties,
// combine the recent sample's squared pooled ranks and squared contrary-ranks
// with the classical Cucconi correlation correction, and enumerate every
// C(12,6)=924 label assignment for an inclusive exact upper tail. A qualified
// tail fades the recent block's pooled rank displacement with an opposite,
// equal-notional package held to the next month. Runtime is deterministic,
// completed-price-only, and one consumed attempt per month.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41278;
...
input int    strategy_tail_count_max          = 480;
input double strategy_relative_epsilon        = 0.000000000001;
input double strategy_rank_square_expectation = 325.0;
input double strategy_rank_square_sd          = 83.3966426182733;
input double strategy_rank_component_rho      = -0.8953271028037383;
input int    strategy_neutral_rank_sum        = 39;
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41278 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
            ...
            MathAbs(strategy_rank_square_expectation - 325.0) <= 1.0e-12 &&
            MathAbs(strategy_rank_square_sd - 83.3966426182733) <= 1.0e-12 &&
            MathAbs(strategy_rank_component_rho + 0.8953271028037383) <= 1.0e-12 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_MCUCCONI_RV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41278_XAUXAG_MCUCCONI_RV";
...
bool Strategy_CucconiFromLabels(const int &labels[],
                                 double &statistic,
                                 int &recent_rank_sum)
  {
   statistic = 0.0;
   ...
   int old_total = 0;
   int recent_total = 0;
   int rank_square_sum = 0;
   int contrary_rank_square_sum = 0;
   for(int index = 0; index < ArraySize(labels); ++index)
     {
      if(labels[index] == 0)
      ...
      else if(labels[index] == 1)
        {
         ...
         const int contrary_rank = strategy_return_count + 1 - rank;
         recent_rank_sum += rank;
         rank_square_sum += rank * rank;
         contrary_rank_square_sum += contrary_rank * contrary_rank;
        }
      else
         return false;
     }
   if(old_total != strategy_block_size ||
      recent_total != strategy_block_size ||
      recent_rank_sum < 21 || recent_rank_sum > 57)
      return false;

   if(strategy_rank_square_sd <= 0.0 ||
      !MathIsValidNumber(strategy_rank_square_sd) ||
      !MathIsValidNumber(strategy_rank_component_rho) ||
      MathAbs(strategy_rank_component_rho) >= 1.0)
      return false;

   const double u =
      ((double)rank_square_sum - strategy_rank_square_expectation) /
      strategy_rank_square_sd;
   const double v =
      ((double)contrary_rank_square_sum - strategy_rank_square_expectation) /
      strategy_rank_square_sd;
   const double denominator =
      2.0 * (1.0 - strategy_rank_component_rho *
                   strategy_rank_component_rho);
   if(!MathIsValidNumber(u) || !MathIsValidNumber(v) ||
      !MathIsValidNumber(denominator) || denominator <= 0.0)
      return false;
   statistic =
      (u * u + v * v -
       2.0 * strategy_rank_component_rho * u * v) / denominator;
   if(!MathIsValidNumber(statistic) ||
      statistic < -strategy_relative_epsilon)
      return false;
   statistic = MathMax(0.0, statistic);
   return true;
  }
...
bool Strategy_LoadMonthlyCucconiSignal(
      ...
      int &direction)
  {
   ...
   for(int index = 0; index < strategy_return_count; ++index)
     {
      ...
      pooled_labels[index] = (index < strategy_block_size) ? 0 : 1;
     }
   ...
   for(int index = 0; index < strategy_return_count; ++index)
     {
      if(pooled_labels[index] == 1)
         label_path += "R";
      else
         label_path += "O";
     }
   if(!Strategy_CucconiFromLabels(pooled_labels,
                                  observed_c,
                                  recent_rank_sum))
      return false;

   const double tail_epsilon =
      strategy_relative_epsilon * MathMax(1.0, MathAbs(observed_c));
   int permutation_labels[12];
   for(int mask = 0; mask < (1 << strategy_return_count); ++mask)
     {
      if(Strategy_PopCount12(mask) != strategy_block_size)
         continue;
      for(int index = 0; index < strategy_return_count; ++index)
         permutation_labels[index] =
            ((mask & (1 << index)) != 0) ? 1 : 0;
      double permutation_c = 0.0;
      int permutation_rank_sum = 0;
      if(!Strategy_CucconiFromLabels(permutation_labels,
                                     permutation_c,
                                     permutation_rank_sum))
         return false;
      ++assignment_count;
      if(permutation_c + tail_epsilon >= observed_c)
         ++tail_count;
     }
   ...
   const bool qualifies = (tail_count <= strategy_tail_count_max);
   if(!qualifies || recent_rank_sum == strategy_neutral_rank_sum)
      return true;

   // direction > 0 means BUY XAU / SELL XAG. A qualified recent block above
   // the pooled-rank neutral sum is faded short XAU; below neutral is faded
   // long XAU. Neither statistic nor displacement magnitude scales risk.
   direction = (recent_rank_sum > strategy_neutral_rank_sum) ? -1 : 1;
   return true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41278_XAUXAG_MCUCCONI_RV_HOST";
   ...
   string label_path = "";
   const bool valid =
      Strategy_LoadMonthlyCucconiSignal(
         g_signal_month_key,
         month_count,
   ...
   string state = "monthly_cucconi_signal_failed";
   if(valid && direction > 0)
      state = "qualified_recent_low_rank_fade_long_xau";
   else if(valid && direction < 0)
      state = "qualified_recent_high_rank_fade_short_xau";
   else if(valid && tie_detected)
      state = "relative_pooled_change_tie_flat";
   else if(valid && tail_count > strategy_tail_count_max)
      state = "exact_upper_tail_above_locked_cutoff_flat";
   else if(valid && recent_rank_sum == strategy_neutral_rank_sum)
      state = "neutral_recent_rank_sum_flat";
   else if(valid)
      state = "nonqualifying_exact_cucconi_flat";

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"month_count\":%d,\"newest_endpoint_time\":%I64d,\"observed_c\":%.12e,\"assignment_count\":%d,\"tail_count\":%d,\"recent_rank_sum\":%d,\"tie_detected\":%s,\"direction\":%d,\"ratios\":\"%s\",\"returns\":\"%s\",\"pooled_labels\":\"%s\",\"state\":\"%s\"}",
                            g_signal_month_key,
                            month_count,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG monthly exact Cucconi reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41278\",\"ea\":\"xauxag-mcucconi-rv\"}");
