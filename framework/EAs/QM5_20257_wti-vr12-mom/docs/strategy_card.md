...
slug: wti-vr12-mom
status: APPROVED
execution_contract_ref: strategy-seeds/cards/approved/QM5_20257_wti-vr12-mom_card.md
...
sources:
  - "[[sources/MEHLITZ-AUER-WTI-R12Q13-2026]]"
...
review_focus: "Falsify the published WTI R12-q13 memory state as direct crude-oil exposure distinct from the certified XAU/SP500/NDX/XNG book; only Q09 may establish realized decorrelation."
modules_used: [no_trade, trade_entry, trade_management, trade_close]
target_modules: [Strategy_NoTradeFilter, Strategy_EntrySignal, Strategy_ManageOpenPosition, Strategy_ExitSignal, Strategy_NewsFilterHook]
hard_rules_at_risk: [completed_month_reconstruction, twelve_month_return, q13_variance_ratio, heteroskedastic_robust_variance, fixed_significance_threshold, memory_direction_matrix, restart_attempt_state, risk_mode, friday_close_exception, cfd_futures_basis, q02_frequency_floor, portfolio_correlation]
g0_approval_reasoning: "APPROVED under decisions/2026-08-07_qm5_20257_wti_vr12_mom_g0.md: tier-A peer-reviewed complete-read source with explicit WTI and source-declared R12-q13 membership; locked 33 month ends, 32 returns, twelve-month ranking return, q13 lag/VR/robust weights, two-sided 10% threshold, continuation/reversal matrix, persisted monthly attempt, ATR stop, rollover, and stale exit; registered XTIUSD.DWX D1 route; deterministic native arithmetic only. Dedup scanned 4,314 registry rows and 431 cards, found no exact collision, and the expected R1-q2/R3-q4/R6-q7 source siblings and XAU/XAG mechanic neighbor were manually resolved."
---
...
WTI's completed twelve-month direction may persist only when the latest 32
monthly returns exhibit statistically significant memory across the matching
twelve-lag horizon. In a significant anti-persistent state, the same twelve-month
move may instead reverse. Applying the source-declared `R12-q13` state to WTI
...
`strategy-seeds/sources/MEHLITZ-AUER-WTI-R12Q13-2026/source.md`. Its completely
...
twelve-month ranking return to `q=13`, estimates the robust variance-ratio state
...
The deterministic pre-allocation checker found no exact collision and four
expected fuzzy source/mechanic neighbors. Manual review fixes the boundary:
...
- `QM5_20256_wti-vr6-mom` is `R6-q7`: six-month return sign, six lags,
  weights `12/7,10/7,8/7,6/7,4/7,2/7`, and six robust terms.
- `QM5_20257` is `R12-q13`: twelve-month return sign, twelve lags, weights
  `24/13,22/13,20/13,18/13,16/13,14/13,12/13,10/13,8/13,6/13,4/13,2/13`,
  and their twelve squared robust weights.
- `QM5_12603_wti-tsmom12m` follows the twelve-month sign without a memory
...
- `QM5_20245_wti-vr-rsm` combines `q=2` memory with twelve binary monthly
  signs, not the cumulative source-matched `R12-q13` pair.
- `QM5_20249_xauxag-vr-spread` is a two-leg metals basket driven by relative
  XAU/XAG return and `q=2` leg-memory states, not direct WTI `R12-q13`.
...
the WTI `R12-q13` specification. Verdict:
...
- Timeframe: D1.
- Magic slot: 0; intended magic `202570000` after deterministic allocation.
...
- Ranking state: cumulative latest twelve completed monthly log returns.
...
`d[t]=r[t]-mean(r)` and `SSE=sum(d[t]^2)`. For `k=1..12`:

```text
rho[k]   = sum(d[t] * d[t-k], t=k..31) / SSE
delta[k] = sum(d[t]^2 * d[t-k]^2, t=k..31) / SSE^2
w[k]     = 2 * (13-k) / 13
VR(13)    = 1 + sum(w[k] * rho[k], k=1..12)
theta(13) = sum(w[k]^2 * delta[k], k=1..12)
z_vr      = (VR(13)-1) / sqrt(theta(13))
R12       = sum(r[t], t=20..31)
```

Require `abs(z_vr)>1.64485362695147`. Set the trade direction to
`sign(R12)*sign(z_vr)`: persistence continues the twelve-month direction and
anti-persistence reverses it. Zero `R12`, an insignificant state, or invalid
arithmetic is flat.
...
1. Require exact EA ID `20257`, `XTIUSD.DWX` D1, slot 0, registered magic, and
...
4. Form exactly 32 chronological monthly log returns. Compute the mean,
   centered deviations, `SSE`, lag-one through lag-twelve autocorrelations, the
   fixed `q=13` weight vector, and the twelve robust variance terms.
...
explicitly include WTI, and declare the `R12-q13` configuration. They do not
...
sparsity, twelve-lag estimator noise, single-energy concentration, CFD roll and
...
- Retire on zero trades or fewer than five completed packages per full
  post-warm-up year.
- Fail on missing or nonconsecutive endpoints, wrong R12 orientation, wrong
...
- trade_entry: month-end reconstruction, `R12-q13` statistic, persisted attempt,
...
|---|---|---|---|---|
| v1 | 2026-08-07 | initial source-bounded WTI R12-q13 card | G0 | APPROVED |
...
|---|---|---|---|
| G0 Research Intake | 2026-08-07 | APPROVED | `decisions/2026-08-07_qm5_20257_wti_vr12_mom_g0.md` |
| Q01 Build Validation | 2026-08-07 | PASS | strict compile `D:/QM/reports/compile/20260807_000049/summary.csv`; build check `D:/QM/reports/framework/21/build_check_20260806_235959.json` |
| Q02 Baseline Screening | 2026-08-07 | NOT_ENQUEUED_CPU_CEILING | `docs/ops/evidence/2026-08-07_qm5_20257_wti_vr12_mom_q01_cpu_stop.md` |
