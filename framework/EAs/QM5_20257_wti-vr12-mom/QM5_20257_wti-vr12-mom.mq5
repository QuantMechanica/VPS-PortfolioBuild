...
#property description "QM5_20257 WTI R12-q13 Memory-Enhanced Momentum"
...
// -----------------------------------------------------------------------------
// Source baseline: Mehlitz & Auer (2024), R12-q13 twelve-month memory-enhanced momentum.
// At each broker-month transition:
//   1. derive 32 completed monthly log returns from D1 month-end closes;
//   2. calculate the q=13 Lo-MacKinlay heteroskedasticity-robust VR test over lags 1-12;
//   3. continue the latest completed twelve-month return when persistence is significant;
...
input int    strategy_rank_months         = 12;
input int    strategy_vr_q                = 13;
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 20257 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20257_WTI_VR12_MOM";
   ...
   req.reason = (g_cache_signal > 0) ? "VR12_MEMORY_XTI_LONG" : "VR12_MEMORY_XTI_SHORT";
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_20257_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_20257\",\"ea\":\"wti-vr12-mom\"}");
