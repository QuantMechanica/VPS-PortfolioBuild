...
#property description "QM5_12751 Edge Lab EURUSD EURAUD Cointegration"
...
input double strategy_beta              = 0.34;
...
bool Strategy_IsHostSymbol()
  {
   return (_Symbol == g_leg_eurusd || _Symbol == g_leg_euraud);
  }
...
bool Strategy_EnsureBasketScope()
  {
   ...
   // EURAUD needs AUD conversion history for USD-denominated tester
   // accounting; EURUSD itself already settles in USD.
   string allowed[3] = {"EURUSD.DWX", "EURAUD.DWX", "AUDUSD.DWX"};
   for(int i = 0; i < 3; ++i)
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(!Strategy_EnsureBasketScope())
      return false;
   if(!QM_SymbolAssertOrLog(g_leg_eurusd) || !QM_SymbolAssertOrLog(g_leg_euraud))
      return false;
   ...
   if(CopyClose(g_leg_eurusd, PERIOD_D1, 1, lookback, eurusd) != lookback) // perf-allowed: Strategy_EntrySignal is called only after the framework QM_IsNewBar gate.
      return false;
   if(CopyClose(g_leg_euraud, PERIOD_D1, 1, lookback, euraud) != lookback) // perf-allowed: Strategy_EntrySignal is called only after the framework QM_IsNewBar gate.
      return false;
   ...
   for(int i = 0; i < lookback; ++i)
     {
      if(eurusd[i] <= 0.0 || euraud[i] <= 0.0)
         return false;
      spreads[i] = MathLog(eurusd[i]) - strategy_beta * MathLog(euraud[i]);
...
bool Strategy_OpenPair(const int spread_direction)
  {
   ...
   const double weight_sum = eurusd_weight + euraud_weight;
   if(weight_sum <= 0.0)
      return false;

   const bool long_spread = (spread_direction > 0);
   const QM_OrderType eurusd_type = long_spread ? QM_BUY : QM_SELL;
   const QM_OrderType euraud_type = long_spread ? QM_SELL : QM_BUY;
   const string reason = long_spread ? "QM5_12751_LONG_SPREAD_Z_LT_NEG_ENTRY"
                                     : "QM5_12751_SHORT_SPREAD_Z_GT_POS_ENTRY";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12751_PAIR_HOST";
   ...
   if(qm_news_temporal != QM_NEWS_TEMPORAL_OFF || qm_news_compliance != QM_NEWS_COMPLIANCE_NONE)
     {
      if(!QM_NewsAllowsTrade2(g_leg_eurusd, broker_time, qm_news_temporal, qm_news_compliance))
         return true;
      if(!QM_NewsAllowsTrade2(g_leg_euraud, broker_time, qm_news_temporal, qm_news_compliance))
         return true;
     }
   else
     {
