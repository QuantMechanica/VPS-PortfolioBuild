...
#property description "QM5_12786 Edge Lab USDCHF EURGBP Cointegration"
...
input double strategy_beta              = 0.138258947;
...
bool Strategy_IsHostSymbol()
  {
   return (_Symbol == g_leg_usdchf || _Symbol == g_leg_eurgbp);
  }
...
bool Strategy_EnsureBasketScope()
  {
   ...
   // EURGBP needs EURUSD/GBPUSD conversion history for USD-denominated
   // tester accounting. USDCHF supplies its own CHF conversion path.
   string allowed[4] = {"USDCHF.DWX", "EURGBP.DWX", "EURUSD.DWX", "GBPUSD.DWX"};
   for(int i = 0; i < 4; ++i)
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(!Strategy_EnsureBasketScope())
      return false;
   if(!QM_SymbolAssertOrLog(g_leg_usdchf) || !QM_SymbolAssertOrLog(g_leg_eurgbp))
      return false;
   ...
   if(CopyClose(g_leg_usdchf, PERIOD_D1, 1, lookback, usdchf) != lookback) // perf-allowed: Strategy_EntrySignal is called only after the framework QM_IsNewBar gate.
      return false;
   if(CopyClose(g_leg_eurgbp, PERIOD_D1, 1, lookback, eurgbp) != lookback) // perf-allowed: Strategy_EntrySignal is called only after the framework QM_IsNewBar gate.
      return false;
   ...
   for(int i = 0; i < lookback; ++i)
     {
      if(usdchf[i] <= 0.0 || eurgbp[i] <= 0.0)
         return false;
      spreads[i] = MathLog(usdchf[i]) - strategy_beta * MathLog(eurgbp[i]);
...
bool Strategy_OpenPair(const int spread_direction)
  {
   ...
   const double weight_sum = usdchf_weight + eurgbp_weight;
   if(weight_sum <= 0.0)
      return false;

   const bool long_spread = (spread_direction > 0);
   const QM_OrderType usdchf_type = long_spread ? QM_BUY : QM_SELL;
   const QM_OrderType eurgbp_type = long_spread ? QM_SELL : QM_BUY;
   const string reason = long_spread ? "QM5_12786_LONG_SPREAD_Z_LT_NEG_ENTRY"
                                     : "QM5_12786_SHORT_SPREAD_Z_GT_POS_ENTRY";

   bool usdchf_ok = Strategy_OpenLeg(g_leg_usdchf, usdchf_type, usdchf_weight, weight_sum, reason);
   bool eurgbp_ok = Strategy_OpenLeg(g_leg_eurgbp, eurgbp_type, eurgbp_weight, weight_sum, reason);
   if(usdchf_ok && eurgbp_ok)
     {
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12786_PAIR_HOST";
   ...
   if(qm_news_temporal != QM_NEWS_TEMPORAL_OFF || qm_news_compliance != QM_NEWS_COMPLIANCE_NONE)
     {
      if(!QM_NewsAllowsTrade2(g_leg_usdchf, broker_time, qm_news_temporal, qm_news_compliance))
         return true;
      if(!QM_NewsAllowsTrade2(g_leg_eurgbp, broker_time, qm_news_temporal, qm_news_compliance))
         return true;
     }
   else
     {
