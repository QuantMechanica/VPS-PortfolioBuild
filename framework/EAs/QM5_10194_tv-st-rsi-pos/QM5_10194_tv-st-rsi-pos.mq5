...
input int    strategy_supertrend_lookback   = 120;
input int    strategy_rsi_period            = 14;
input double strategy_rsi_bull_threshold    = 50.0;
...
bool Strategy_SupertrendBullish(const int target_shift)
  {
   const ENUM_TIMEFRAMES tf = (ENUM_TIMEFRAMES)_Period;
   const int period = MathMax(1, strategy_supertrend_atr_period);
   const int lookback = MathMax(period + 5, strategy_supertrend_lookback);
   ...
   for(int shift = lookback; shift >= target_shift; --shift)
     {
      ...
      const double basic_upper = median + strategy_supertrend_mult * atr;
      const double basic_lower = median - strategy_supertrend_mult * atr;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   const ENUM_TIMEFRAMES tf = (ENUM_TIMEFRAMES)_Period;
   const double rsi = QM_RSI(_Symbol, tf, MathMax(1, strategy_rsi_period), 1, PRICE_CLOSE);
   if(rsi <= strategy_rsi_bull_threshold)
      return false;

   if(strategy_use_adx_filter)
     {
      const double adx = QM_ADX(_Symbol, tf, MathMax(1, strategy_adx_period), 1);
      if(adx <= strategy_adx_threshold)
         return false;
     }

   if(!Strategy_SupertrendBullish(1))
      return false;
   ...
   req.price = ask;
   req.sl = QM_StopATR(_Symbol, QM_BUY, ask, MathMax(1, strategy_stop_atr_period), strategy_stop_atr_mult);
   req.reason = "TV_ST_RSI_POS_LONG";
...
bool Strategy_ExitSignal()
  {
   ...
   return !Strategy_SupertrendBullish(1);
  }
