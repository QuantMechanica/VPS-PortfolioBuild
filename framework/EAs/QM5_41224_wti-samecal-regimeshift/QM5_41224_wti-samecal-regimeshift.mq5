...
#property description "QM5_41224 WTI Same-Calendar Chronological Regime Shift"
...
//   - require all ten exact prior years with no substitution or compression
//   - average lags Y-1..Y-5 and Y-6..Y-10 in chronological blocks
//   - trade only when the block means have strict opposite signs
//   - follow the recent five-year block beyond the +/-1e-12 boundaries
...
input int    strategy_block_years         = 5;
...
double   g_signal_recent_sum      = 0.0;
double   g_signal_older_sum       = 0.0;
double   g_signal_recent_mean     = 0.0;
double   g_signal_older_mean      = 0.0;
...
void Strategy_ResetDecisionState()
  {
   ...
   g_signal_recent_sum = 0.0;
   g_signal_older_sum = 0.0;
   g_signal_recent_mean = 0.0;
   g_signal_older_mean = 0.0;
   g_signal_state = "idle";
  }
...
// -----------------------------------------------------------------------------
// Exact completed-month endpoint and chronological regime-shift helpers.
// -----------------------------------------------------------------------------
...
bool Strategy_RegimeShiftSignal(const double &observations[],
                                const int sample_count,
                                double &recent_sum_value,
                                double &older_sum_value,
                                double &recent_mean_value,
                                double &older_mean_value,
                                int &direction)
  {
   recent_sum_value = 0.0;
   older_sum_value = 0.0;
   recent_mean_value = 0.0;
   older_mean_value = 0.0;
   direction = 0;

   if(strategy_history_years != 10 ||
      strategy_block_years != 5 ||
      strategy_block_years * 2 != strategy_history_years ||
   ...
   for(int index = 0; index < sample_count; ++index)
     {
      if(index < 0 || index >= ArraySize(observations))
         return false;
      const double sample_return = observations[index];
      if(!MathIsValidNumber(sample_return))
         return false;

      if(index < strategy_block_years)
         recent_sum_value += sample_return;
      else
         older_sum_value += sample_return;
      if(!MathIsValidNumber(recent_sum_value) ||
         !MathIsValidNumber(older_sum_value))
         return false;
     }
   ...
   if(!MathIsValidNumber(recent_mean_value) ||
      !MathIsValidNumber(older_mean_value))
      return false;

   if(recent_mean_value > strategy_signal_epsilon &&
      older_mean_value < -strategy_signal_epsilon)
      direction = 1;
   else if(recent_mean_value < -strategy_signal_epsilon &&
           older_mean_value > strategy_signal_epsilon)
      direction = -1;
   return true;
  }

bool Strategy_LoadRegimeShiftSignal(
   ...
   int &direction)
  {
   ...
   for(int offset = 1; offset <= strategy_history_years; ++offset)
     {
      ...
      if(sample_index < 0 || sample_index >= ArraySize(observations))
         return false;
   ...
   return Strategy_RegimeShiftSignal(observations,
                                     sample_count,
                                     recent_sum_value,
                                     older_sum_value,
                                     recent_mean_value,
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   // Consume before history, block arithmetic, news, spread, quote, ATR,
   ...
   else
     {
      g_signal_valid =
         Strategy_LoadRegimeShiftSignal(
            ...
            g_signal_recent_sum,
            g_signal_older_sum,
            g_signal_recent_mean,
            g_signal_older_mean,
            g_signal_direction);
      if(!g_signal_valid)
         g_signal_state = "endpoint_or_complete_blocks_failed";
      else if(g_signal_direction > 0)
         g_signal_state = "regime_shift_recent_long";
      else if(g_signal_direction < 0)
         g_signal_state = "regime_shift_recent_short";
      else
         g_signal_state = "blocks_not_strictly_opposed_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"valid\":%s,\"samples\":%d,\"recent_sum\":%.12g,\"older_sum\":%.12g,\"recent_mean\":%.12g,\"older_mean\":%.12g,\"direction\":%d,\"state\":\"%s\"}",
                            g_decision_month_key,
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(!QM_InputRequireLong("qm_ea_id", qm_ea_id, 41224) ||
      ...
      !QM_InputRequireLong("strategy_block_years", strategy_block_years, 5) ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "WTI_SAMECAL_REGIMESHIFT";
   ...
   if(g_signal_direction > 0)
     {
      req.type = QM_BUY;
      req.reason = "WTI_SAMECAL_REGIMESHIFT_LONG";
     }
   else
     {
      req.type = QM_SELL;
      req.reason = "WTI_SAMECAL_REGIMESHIFT_SHORT";
     }
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41224 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI same-calendar chronological regime-shift card holds to the next broker month"))
      return INIT_FAILED;
   ...
   g_attempt_state_key =
      StringFormat("QM5_41224_SAMECAL_REGIMESHIFT_MONTH_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41224\",\"ea\":\"wti-samecal-regimeshift\"}");
