...
#property description "QM5_20278 WTI Linear-Recency Return Momentum"
...
//   3. weight the oldest through newest returns by the integers 1 through 12;
//   4. divide by the exact total 78 and trade the resulting sign.
// The fixed linear-recency statistic is a transparent QM mechanization, not an
// author rule. Runtime uses native XTIUSD.DWX price and broker data only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 20278;
...
void Strategy_ResetCachedState()
  {
   ...
   g_cache_signal = 0;
   g_cache_linw_mean = 0.0;
   g_cache_state_reason = "not_evaluated";
  }

bool Strategy_LinearWeightedMeanSignal(const double &closes[], int &signal)
  {
   signal = 0;
   ...
   if(return_count != 12 ||
     ...
     {
      g_cache_state_reason = "bad_linw_contract";
      return false;
     }

   double weighted_sum = 0.0;
   int observed_weight_total = 0;
   for(int i = 0; i < return_count; ++i)
     {
      ...
      const int weight = strategy_weight_start + i * strategy_weight_step;
      if(weight != i + 1 || weight <= 0)
        {
         g_cache_state_reason = "invalid_linear_weight";
         return false;
        }
      const double weighted_return = (double)weight * monthly_return;
      ...
      observed_weight_total += weight;
      if(!MathIsValidNumber(weighted_sum))
        {
         g_cache_state_reason = "invalid_weighted_sum";
         return false;
        }
     }

   if(observed_weight_total != strategy_weight_total ||
      observed_weight_total != 78)
     {
      g_cache_state_reason = "invalid_weight_total";
      return false;
     }

   const double linw_mean = weighted_sum / (double)observed_weight_total;
   if(!MathIsValidNumber(linw_mean))
     {
      g_cache_state_reason = "invalid_linw_mean";
      return false;
     }

   g_cache_linw_mean = linw_mean;
   if(linw_mean == 0.0)
     {
      g_cache_state_reason = "exact_zero_linw_mean";
      return true;
     }

   signal = (linw_mean > 0.0) ? 1 : -1;
...
bool Strategy_LoadSignalState(int &signal)
  {
   ...
   return Strategy_LinearWeightedMeanSignal(monthly_closes, signal);
  }
...
void Strategy_PrepareMonthlySignal()
  {
   ...
   QM_LogEvent(QM_INFO,
               "MONTHLY_STATE",
               StringFormat("{\"month\":%d,\"valid\":%s,\"signal\":%d,\"linw_mean\":%.10f,\"weight_total\":%d,\"state\":\"%s\"}",
                            g_cache_month_key,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 20278 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20278_WTI_LINW_MOM";
   ...
   req.reason = (g_cache_signal > 0) ? "LINW_MOM_XTI_LONG" : "LINW_MOM_XTI_SHORT";
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_20278_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_20278\",\"ea\":\"wti-linw-mom\"}");
