...
#property description "QM5_20190 WTI Brent Same-Calendar Seasonality"
...
// -----------------------------------------------------------------------------
// Monthly crude-benchmark relative-value basket:
//   - reconstruct synchronized returns for this calendar month in prior years
//   - average each leg over exactly ten prior years with five paired samples
//   - long the higher seasonal crude benchmark and short the lower one
...
double   g_cache_xbr_same_month_return = 0.0;
...
string Strategy_AttemptStateName()
  {
   return "QM5_20190_OILBENCH_CAL_ATTEMPT";
  }
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      else if(symbol == g_leg_xbr)
        {
         xbr_direction = direction;
         ++xbr_count;
        }
     }
   if(!stops_valid || xti_count != 1 || xbr_count != 1 ||
      xti_direction != -xbr_direction)
      return false;
...
bool Strategy_LoadSignalState(const datetime decision_bar_time,
                              int &pair_direction)
  {
   ...
   g_cache_xbr_same_month_return = 0.0;
   ...
   ArraySetAsSeries(xbr_rates, true);
   const int xti_count =
      CopyRates(g_leg_xti, PERIOD_D1, 1, strategy_history_bars, xti_rates); // perf-allowed: bounded copy only on the consumed monthly D1 decision.
   const int xbr_count =
      CopyRates(g_leg_xbr, PERIOD_D1, 1, strategy_history_bars, xbr_rates); // perf-allowed: bounded copy only on the consumed monthly D1 decision.
   if(xti_count <= 0 || xbr_count <= 0)
      return false;
   ...
   for(int offset = 1; offset <= strategy_history_years; ++offset)
     {
      ...
      double xbr_return = 0.0;
      datetime xti_month_time = 0;
      datetime xti_previous_time = 0;
      datetime xbr_month_time = 0;
      datetime xbr_previous_time = 0;

      if(!Strategy_MonthReturn(xti_rates,
                               xti_count,
                               sample_year,
                               decision_parts.mon,
                               xti_return,
                               xti_month_time,
                               xti_previous_time) ||
         !Strategy_MonthReturn(xbr_rates,
                               xbr_count,
                               sample_year,
                               decision_parts.mon,
                               xbr_return,
      ...
      if(xti_month_time != xbr_month_time ||
         xti_previous_time != xbr_previous_time)
         continue;
   ...
   g_cache_xti_same_month_return =
      xti_sum / (double)g_cache_sample_count;
   g_cache_xbr_same_month_return =
      xbr_sum / (double)g_cache_sample_count;
   g_cache_relative_score =
      g_cache_xti_same_month_return -
      g_cache_xbr_same_month_return;

   if(!MathIsValidNumber(g_cache_xti_same_month_return) ||
      !MathIsValidNumber(g_cache_xbr_same_month_return) ||
...
bool Strategy_IsPairMagic(const long magic)
  {
   ...
   const int xbr_magic = QM_MagicChecked(qm_ea_id, 1, g_leg_xbr);
   return (magic == xti_magic || magic == xbr_magic);
  }
...
bool Strategy_OpenPair(const int pair_direction)
  {
   ...
   const bool long_xti_short_xbr = (pair_direction > 0);
   const QM_OrderType xti_type = long_xti_short_xbr ? QM_BUY : QM_SELL;
   const QM_OrderType xbr_type = long_xti_short_xbr ? QM_SELL : QM_BUY;
   const string reason = long_xti_short_xbr ? "QM5_20190_LONG_WTI_SHORT_BRENT_SAMECAL"
                                            : "QM5_20190_SHORT_WTI_LONG_BRENT_SAMECAL";
   const double weight_sum = 2.0;

   QM_BasketOrderRequest xti_req;
   QM_BasketOrderRequest xbr_req;
   if(!Strategy_PrepareLeg(g_leg_xti,
                           xti_type,
                           1.0,
                           weight_sum,
                           reason,
                           xti_req) ||
      !Strategy_PrepareLeg(g_leg_xbr,
                           xbr_type,
                           1.0,
                           weight_sum,
                           reason,
   ...
   if(!Strategy_SubmitLeg(xti_req))
      return false;
   if(Strategy_SubmitLeg(xbr_req) &&
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart())
      return true;
   if(qm_ea_id != 20190 || qm_magic_slot_offset != 0 || qm_rng_seed != 42)
      return true;
   ...
   if(strategy_atr_period_d1 != 20 ||
      ...
      return true;
   if(strategy_xti_max_spread_pts != 1000 ||
      strategy_xbr_max_spread_pts != 1500 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20190_WTI_BRENT_CAL_HOST";
   ...
   if(qm_news_temporal != QM_NEWS_TEMPORAL_OFF ||
      qm_news_compliance != QM_NEWS_COMPLIANCE_NONE)
     {
      if(!QM_NewsAllowsTrade2(g_leg_xti,
                              ...
                              qm_news_compliance))
         return false;
      if(!QM_NewsAllowsTrade2(g_leg_xbr,
                              broker_time,
                              qm_news_temporal,
                              qm_news_compliance))
         return false;
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_20190\",\"ea\":\"oilbench-cal\"}");
