...
The EA tracks Asia, London, and New York sessions using broker timestamps converted to UTC. After a completed child session is fully contained inside its parent session range, the parent high and low become the trade map. A long entry fires when a closed bar sweeps below the parent low and closes back above that low; a short entry fires when a closed bar sweeps above the parent high and closes back below that high. The target is the opposite parent-session level, the stop is the sweep extreme plus the configured ATR buffer, and open trades are closed at the next configured session end if neither stop nor target is reached.
...
|---|---:|---|---|
| `strategy_asia_start_hour_utc` | 0 | 0-23 | UTC hour when the Asia session starts after broker-time conversion. |
| `strategy_asia_end_hour_utc` | 8 | 0-24 | UTC hour when the Asia session ends after broker-time conversion. |
| `strategy_london_start_hour_utc` | 8 | 0-23 | UTC hour when the London session starts after broker-time conversion. |
| `strategy_london_end_hour_utc` | 16 | 0-24 | UTC hour when the London session ends after broker-time conversion. |
| `strategy_newyork_start_hour_utc` | 13 | 0-23 | UTC hour when the New York session starts after broker-time conversion. |
| `strategy_newyork_end_hour_utc` | 21 | 0-24 | UTC hour when the New York session ends after broker-time conversion. |
...
|---|---|---|---|
| v1 | 2026-06-18 | Initial build from card | 0f6d41e1-0b96-47d3-96c2-57c09f5607b3 |
