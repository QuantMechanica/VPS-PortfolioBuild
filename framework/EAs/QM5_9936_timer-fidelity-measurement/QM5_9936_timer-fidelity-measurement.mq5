...
// Measurement-only control. This variant moves only open-position management
// and Strategy_ExitSignal evaluation from OnTick to OnTimer.
input int    measurement_timer_interval_ms  = 1000;
...
int OnInit()
  {
   ...
   if(!QM_PropInit(qm_ea_id))
      return INIT_FAILED;
   if(measurement_timer_interval_ms <= 0 ||
      !EventSetMillisecondTimer(measurement_timer_interval_ms))
      return INIT_FAILED;
