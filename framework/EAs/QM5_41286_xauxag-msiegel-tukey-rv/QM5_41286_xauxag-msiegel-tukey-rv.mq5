...
#property description "QM5_41286 XAU/XAG Monthly Siegel-Tukey Reversion"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_41286 - XAU/XAG Monthly Siegel-Tukey Tail-Occupancy Reversion
// -----------------------------------------------------------------------------
// At the first synchronized D1 boundary of a broker month, reconstruct the
// latest exactly matched XAU/XAG close pair in each of seventeen consecutive
// completed broker months. Split the sixteen adjacent gold-minus-silver log
// changes into fixed old/recent blocks of eight, reject relative pooled ties,
// map ascending pooled observations to the locked Siegel-Tukey score path,
// and enumerate every C(16,8)=12,870 recent-label assignment for an inclusive
// exact lower tail. A qualifying low recent-block scale score fades the recent
// cumulative ratio move with an opposed, equal-notional package held to the
// next month. Runtime is deterministic, completed-price-only, and one consumed
// attempt per month.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41286;
...
input int    strategy_endpoint_count          = 17;
input int    strategy_return_count            = 16;
input int    strategy_block_size              = 8;
input int    strategy_assignment_count        = 12870;
input int    strategy_score_max               = 68;
input int    strategy_tail_count_max          = 6698;
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41286 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_ST_RV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41286_XAUXAG_ST_RV";
...
int Strategy_PopCount16(const int mask)
  {
   if(mask < 0 || mask >= (1 << 16))
      return -1;
   int count = 0;
...
int Strategy_SiegelTukeyScoreForRank(const int rank_index)
  {
   // Ascending pooled observation ranks use the source-defined alternating
   // extremes path. The explicit lattice keeps runtime and fixtures identical.
   switch(rank_index)
     {
      case 0:  return 1;
      case 1:  return 4;
...
bool Strategy_SiegelTukeyScorePathValid()
  {
   if(strategy_return_count != 16 || strategy_block_size != 8)
      return false;
   int total = 0;
   for(int rank = 0; rank < strategy_return_count; ++rank)
     {
      const int score = Strategy_SiegelTukeyScoreForRank(rank);
      if(score < 1 || score > strategy_return_count)
         return false;
      for(int prior = 0; prior < rank; ++prior)
         if(Strategy_SiegelTukeyScoreForRank(prior) == score)
            return false;
      total += score;
     }
   return (total == 136);
...
bool Strategy_SiegelTukeyFromLabels(const int &labels[],
                                    int &score)
  {
   score = 0;
   if(ArraySize(labels) != strategy_return_count ||
      !Strategy_SiegelTukeyScorePathValid())
      return false;
   ...
   for(int index = 0; index < ArraySize(labels); ++index)
     {
      if(labels[index] == 0)
        ...
        {
         ++recent_total;
         const int rank_score = Strategy_SiegelTukeyScoreForRank(index);
         if(rank_score <= 0)
            return false;
         score += rank_score;
        }
...
bool Strategy_LoadMonthlySiegelTukeySignal(
      ...
      int &direction)
  {
   ...
   MqlRates xau_bars[];
   MqlRates xag_bars[];
   double newest_first_ratios[17];
   datetime newest_first_times[17];
   double chronological_ratios[17];
   datetime chronological_times[17];
   double relative_returns[16];
   double pooled_values[16];
   int pooled_labels[16];
   ArraySetAsSeries(xau_bars, true);
   ArraySetAsSeries(xag_bars, true);

   const int xau_copied =
      CopyRates(g_leg_xau, // perf-allowed: one bounded seventeen-month scan behind a consumed monthly attempt.
                PERIOD_D1, 1, strategy_history_bars_d1, xau_bars);
   const int xag_copied =
      CopyRates(g_leg_xag, // perf-allowed: one bounded seventeen-month scan behind a consumed monthly attempt.
   ...
   if(StringLen(return_path) <= 0)
      return false;
   recent_move =
      chronological_ratios[strategy_return_count] -
      chronological_ratios[strategy_block_size];
   if(!MathIsValidNumber(recent_move))
      return false;
   ...
   if(!Strategy_SiegelTukeyFromLabels(pooled_labels, observed_score))
      return false;

   int permutation_labels[16];
   for(int mask = 0; mask < (1 << strategy_return_count); ++mask)
     {
      if(Strategy_PopCount16(mask) != strategy_block_size)
         continue;
      for(int index = 0; index < strategy_return_count; ++index)
         permutation_labels[index] =
            ((mask & (1 << index)) != 0) ? 1 : 0;
      int permutation_score = 0;
      if(!Strategy_SiegelTukeyFromLabels(permutation_labels,
                                         permutation_score))
         return false;
      ++assignment_count;
      if(permutation_score <= observed_score)
         ++tail_count;
     }
   ...
   const bool qualifies_by_score = (observed_score <= strategy_score_max);
   const bool qualifies_by_tail = (tail_count <= strategy_tail_count_max);
   if(qualifies_by_score != qualifies_by_tail)
      return false;
   if(!qualifies_by_score || recent_move == 0.0)
      return true;

   // direction > 0 means BUY XAU / SELL XAG. The recent eight-change
   // cumulative ratio move is faded; score magnitude never scales risk.
   direction = (recent_move > 0.0) ? -1 : 1;
   return true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41286_XAUXAG_ST_RV_HOST";
   ...
   string label_path = "";
   const bool valid =
      Strategy_LoadMonthlySiegelTukeySignal(
         g_signal_month_key,
         month_count,
   ...
   string state = "monthly_siegel_tukey_signal_failed";
   if(valid && direction > 0)
      state = "qualified_negative_recent_move_fade_long_xau";
   else if(valid && direction < 0)
      state = "qualified_positive_recent_move_fade_short_xau";
   else if(valid && tie_detected)
      state = "relative_pooled_change_tie_flat";
   else if(valid && observed_score > strategy_score_max)
      state = "siegel_tukey_lower_tail_not_qualified_flat";
   else if(valid && recent_move == 0.0)
      state = "zero_recent_cumulative_ratio_move_flat";
   else if(valid)
      state = "nonqualifying_exact_siegel_tukey_flat";

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"month_count\":%d,\"newest_endpoint_time\":%I64d,\"siegel_tukey_score\":%d,\"assignment_count\":%d,\"tail_count\":%d,\"recent_move\":%.12e,\"tie_detected\":%s,\"direction\":%d,\"ratios\":\"%s\",\"returns\":\"%s\",\"pooled_labels\":\"%s\",\"state\":\"%s\"}",
                            g_signal_month_key,
                            month_count,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG monthly Siegel-Tukey reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41286\",\"ea\":\"xauxag-msiegel-tukey-rv\"}");
