...
#property description "QM5_41353 WTI Monthly ADF-PP Agreement Trend"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41353 - WTI Monthly ADF-Phillips-Perron Agreement-Gated Trend
// -----------------------------------------------------------------------------
...
//   - require ADF t >= -2.594 and PP Z-tau >= -2.594
...
input int    strategy_pp_regression_observations = 59;
input int    strategy_pp_residual_dof       = 57;
input int    strategy_pp_bartlett_lags      = 11;
...
struct Strategy_SignalMetrics
  {
   ...
   double pp_rho;
   double pp_sse;
   double pp_residual_variance;
   double pp_regression_sigma;
   double pp_gamma0;
   double pp_long_run_variance;
   double pp_se_rho;
   double pp_raw_tau;
   double pp_z_tau;
   double momentum_12;
...
bool Strategy_PPCore(const double &levels[],
                     ...
                     string &covariance_path)
  {
   ...
   if(level_count != 60 || strategy_level_count != 60 ||
      ...
      strategy_pp_regression_observations != 59 ||
      strategy_pp_residual_dof != 57 ||
      strategy_pp_bartlett_lags != 11 ||
      strategy_energy_floor != 0.000000000000000001)
      return false;
   ...
   for(int index = 1; index < level_count; ++index)
     {
      ...
      if(!MathIsValidNumber(mean_lhs) || !MathIsValidNumber(mean_rhs))
         return false;
     }
   ...
   for(int index = 1; index < level_count; ++index)
     {
      ...
      if(!MathIsValidNumber(xx) || !MathIsValidNumber(xy) || xx < 0.0)
         return false;
      sxx += xx;
      sxy += xy;
   ...
   for(int row = 0; row < observation_count; ++row)
     {
      const double residual = levels[row + 1] - alpha - rho * levels[row];
      const double residual_square = residual * residual;
      if(!MathIsValidNumber(residual) ||
   ...
   residual_variance = sse / (double)strategy_pp_residual_dof;
   ...
   if(!MathIsValidNumber(residual_variance) ||
      ...
      !MathIsValidNumber(gamma0) || !MathIsValidNumber(se_rho) ||
      residual_variance <= strategy_energy_floor ||
      regression_sigma <= strategy_energy_floor ||
      gamma0 <= strategy_energy_floor || se_rho <= strategy_energy_floor)
      return false;

   long_run_variance = gamma0;
   for(int lag = 1; lag <= strategy_pp_bartlett_lags; ++lag)
     {
      ...
      const double autocovariance = cross_sum / (double)observation_count;
      const double bartlett_weight =
         1.0 - (double)lag / (double)(strategy_pp_bartlett_lags + 1);
      ...
      if(StringLen(covariance_path) > 0)
         covariance_path += ",";
      covariance_path += StringFormat("g%d=%.16f,w%d=%.16f",
                                      lag, autocovariance,
                                      lag, bartlett_weight);
     }
   ...
   const double variance_scale = MathSqrt(gamma0 / long_run_variance);
   const double correction =
      0.5 * ((long_run_variance - gamma0) / long_run_sigma) *
      ((double)observation_count * se_rho / regression_sigma);
   pp_z_tau = variance_scale * raw_tau - correction;
   if(!MathIsValidNumber(raw_tau) || !MathIsValidNumber(long_run_sigma) ||
      !MathIsValidNumber(variance_scale) || !MathIsValidNumber(correction) ||
...
bool Strategy_AgreementSignal(const double &closes[],
                        Strategy_SignalMetrics &metrics)
  {
   ...
   if(!Strategy_PPCore(levels,
                       ...
                       metrics.pp_rho,
                       metrics.pp_sse,
                       metrics.pp_residual_variance,
                       metrics.pp_regression_sigma,
                       metrics.pp_gamma0,
                       metrics.pp_long_run_variance,
                       metrics.pp_se_rho,
                       metrics.pp_raw_tau,
                       metrics.pp_z_tau,
                       metrics.covariance_path))
      return false;
   metrics.pp_qualified = (metrics.pp_z_tau >= strategy_pp_z_tau_min);
   metrics.persistence_qualified =
      (metrics.adf_qualified && metrics.pp_qualified);
...
bool Strategy_AgreementReferenceSelfTest()
  {
   ...
   for(int index = 0; index < 60; ++index)
     {
      ...
      const double adf_only_level =
         4.0 + 0.020 * MathSin(0.11 * (double)index) +
         0.010 * MathCos(1.91 * (double)index);
      const double pp_only_level =
         4.0 - 0.002 * (double)index +
         0.018 * MathSin(1.19 * (double)index) +
         0.009 * MathCos(1.91 * (double)index);
      up_closes[index] = MathExp(up_level);
      down_closes[index] = MathExp(down_level);
      adf_only_closes[index] = MathExp(adf_only_level);
      pp_only_closes[index] = MathExp(pp_only_level);
   ...
   if(!Strategy_AgreementSignal(adf_only_closes, metrics) ||
      ...
      MathAbs(metrics.adf_t - (-2.2291816679882848)) >
         0.000000001 ||
      MathAbs(metrics.pp_z_tau - (-3.571010451512681)) >
         0.000000001)
      return false;

   if(!Strategy_AgreementSignal(pp_only_closes, metrics) ||
      metrics.adf_qualified || !metrics.pp_qualified ||
      metrics.persistence_qualified || metrics.direction != 0 ||
      MathAbs(metrics.adf_t - (-2.638439000868299)) >
         0.000000001 ||
      MathAbs(metrics.pp_z_tau - (-1.9387075563749554)) >
         0.000000001)
      return false;

   if(!Strategy_AgreementSignal(mean_reverting_closes, metrics) ||
      metrics.adf_qualified || metrics.pp_qualified ||
      metrics.persistence_qualified || metrics.direction != 0 ||
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   else
     {
      ...
      if(!g_signal_valid)
         g_signal_state = "agreement_validation_failed";
              ...
              !g_signal_metrics.pp_qualified)
         g_signal_state = "both_gates_reject_consumed_flat";
      else if(!g_signal_metrics.adf_qualified)
         g_signal_state = "adf_rejects_pp_only_consumed_flat";
      else if(!g_signal_metrics.pp_qualified)
         g_signal_state = "pp_rejects_adf_only_consumed_flat";
   ...
   QM_LogEvent(QM_INFO,
               ...
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"endpoint_count\":%d,\"level_count\":%d,\"mean_y\":%.16f,\"mean_z\":%.16f,\"mean_w\":%.16f,\"szz\":%.16f,\"sww\":%.16f,\"szw\":%.16f,\"szy\":%.16f,\"swy\":%.16f,\"determinant\":%.16f,\"alpha\":%.16f,\"gamma\":%.16f,\"phi\":%.16f,\"sse\":%.16f,\"residual_variance\":%.16f,\"se_gamma\":%.16f,\"adf_t\":%.12f,\"adf_qualified\":%s,\"pp_z_tau\":%.12f,\"pp_qualified\":%s,\"persistence_qualified\":%s,\"momentum_12\":%.12f,\"levels\":\"%s\",\"regression\":\"%s\",\"covariance\":\"%s\",\"oldest_close\":%.10f,\"newest_close\":%.10f,\"oldest_endpoint\":%I64d,\"newest_endpoint\":%I64d,\"state\":\"%s\"}",
                            ...
                            g_signal_metrics.pp_qualified ? "true" : "false",
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41353 ||
      qm_magic_slot_offset != 0 ||
      QM_FrameworkMagic() != 413530000 ||
      qm_rng_seed != 42)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41353_WTI_ADF_PP_AGREE";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41353 ||
      qm_magic_slot_offset != 0)
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI monthly ADF-PP agreement trend holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41353_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   g_entry_month_state_key =
      StringFormat("QM5_41353_ENTRY_MONTH_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41353\",\"ea\":\"wti-adf-pp-agree-tr\"}");
