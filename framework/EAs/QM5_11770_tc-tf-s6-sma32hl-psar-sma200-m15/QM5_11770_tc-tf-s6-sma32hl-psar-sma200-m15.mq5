...
#property description "QM5_11770 tc-tf-s6-sma32hl-psar-sma200-m15 — SMA(32) High/Low band breakout + PSAR + SMA100/200 trend (M15)"

#include <QM/QM_Common.mqh>

// =============================================================================
// QuantMechanica V5 EA — QM5_11770 tc-tf-s6-sma32hl-psar-sma200-m15
// -----------------------------------------------------------------------------
// Source: Thomas Carter, "Forex Trend Following Strategies: 20 Trend Following
//         Systems", System #6, self-published 2014 (R1 FAIL — anonymous retail
//         author; mechanical rule R2 PASS).
// Card: artifacts/cards_approved/QM5_11770_tc-tf-s6-sma32hl-psar-sma200-m15.md
//       (g0_status APPROVED).
//
// Mechanics (closed-bar reads at shift 1; TF = M15):
//   Band       : SMA(32) on HIGH = upper channel, SMA(32) on LOW = lower channel.
//   Trend STATE: macro SMA(100) + SMA(200) and price relative to BOTH, plus a
//                bullish/bearish closed bar. These are STATES (currently-true
...
//   Stop       : PSAR dot at the trigger bar, capped at sl_cap_pips, with an
//                ATR(14)-below-SMA(32,Low) floor per the card SL note.
//   Take profit: fixed tp_pips (card M15 default: 10 pips).
//   Spread     : skip only a genuinely wide spread (fail-open on .DWX 0 spread).
//
// Note: the card frontmatter lists SMA(100) AND SMA(200); the body says
//   "confirmed by long-term SMA(100/200)". Literal reading -> price must be above
//   (long) / below (short) BOTH MAs. Slug names "sma200" but the rule keeps both
//   filters (the 100 is the nearer macro line, the 200 the structural one).
//
...
input int    strategy_atr_period        = 14;     // ATR period for the SL floor
input double strategy_tp_pips           = 10.0;   // fixed take-profit (M15 default per card)
input double strategy_sl_cap_pips       = 20.0;   // P2 cap on the PSAR-derived stop distance
input double strategy_spread_cap_pips   = 12.0;   // spread cap (only blocks genuinely wide spread)
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   // --- ATR(14) at the trigger bar for the SL floor (card SL = SMA32Low - ATR) ---
   ...
   if(long_cross && long_bull && long_trend && long_psar)
     {
      ...
      // Stop = PSAR dot, with the card floor (SMA32Low - ATR) as a fallback,
      // capped at sl_cap_pips below entry.
      const double atr_floor = lower1 - atr1;       // card: SMA32Low[0] - ATR(14)
      double sl = sar1;
      if(sl >= entry)                                // SAR not below price — use the ATR floor
         sl = atr_floor;
      if(sl >= entry)                                // floor still invalid — fall back to the cap
         sl = entry - cap_dist;
      else if(cap_dist > 0.0 && (entry - sl) > cap_dist)
         sl = entry - cap_dist;                      // clamp an over-wide stop to the cap
      ...
      req.reason = "carter_s6_sma32band_psar_long";
      return true;
     }
   ...
   if(short_cross && short_bear && short_trend && short_psar)
     {
      ...
      const double atr_floor = upper1 + atr1;       // mirror: SMA32High + ATR(14)
      double sl = sar1;
      if(sl <= entry)                                // SAR not above price — use the ATR floor
         sl = atr_floor;
      if(sl <= entry)                                // floor still invalid — fall back to the cap
         sl = entry + cap_dist;
      ...
      req.sl     = QM_StopRulesNormalizePrice(_Symbol, sl);
      req.tp     = QM_StopRulesNormalizePrice(_Symbol, tp);
      req.reason = "carter_s6_sma32band_psar_short";
      return true;
     }
...
// Fixed PSAR/ATR-stop + fixed-TP strategy — no active management.
