...
#property description "QM5_41119 XAU/XAG Completed-Month Close-Quartile Reversion"
...
// Rank the final close by strict lower count, use fixed ceil(n/4) outer rank
// sets, and fade only a unique lower/upper close quartile with one opposite
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41119 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_MCLOSE_QUARTILE_RV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41119_XAUXAG_MCLOSE_QUARTILE_RV";
...
bool Strategy_LoadMonthlyCloseQuartile(
      ...
      int &direction)
  {
         ...
         Strategy_MonthKey(xau_bars[index].time) == completed_month_key)
     {
      if(completed_month_sessions >= strategy_max_month_sessions ||
         ...
         !Strategy_LogRatio(xau_bars[index].close,
                            ...
                            ratios[completed_month_sessions]))
         return false;
   ...
   // CopyRates series order is newest to oldest. The final completed-month
   // close is ratios[0]. Count strictly lower ratios to obtain its zero-based
   // chronological-set rank; any exact tie consumes the month flat.
   ...
   for(int i = 1; i < completed_month_sessions; ++i)
     {
      ...
      if(ratios[i] == newest_ratio)
         newest_tied = true;
      else if(ratios[i] < newest_ratio)
         ++newest_rank;
     }

   if(!MathIsValidNumber(newest_ratio) || newest_rank < 0 ||
      newest_rank >= completed_month_sessions)
      return false;

   tail_count = (completed_month_sessions + 3) / 4;
   if(tail_count <= 0 || 2 * tail_count >= completed_month_sessions)
      return false;
   if(newest_tied)
      return true;

   // direction > 0 means BUY XAU / SELL XAG. Fade the fixed outer rank sets;
   // every interior unique rank remains consumed flat.
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41119_XAUXAG_MCLOSE_QUARTILE_RV_HOST";
   ...
   bool newest_tied = false;
   const bool valid =
      Strategy_LoadMonthlyCloseQuartile(g_signal_month_key,
                                        completed_month_sessions,
                                        tail_count,
                                        newest_rank,
                                        newest_ratio,
                                        newest_tied,
                                        direction);
   string state = "monthly_ratio_close_quartile_failed";
   if(valid && direction > 0)
      state = "lower_close_quartile_fade_long_xau";
   else if(valid && direction < 0)
      state = "upper_close_quartile_fade_short_xau";
   else if(valid && newest_tied)
      state = "newest_close_tie_flat";
   else if(valid)
      state = "interior_close_rank_flat";

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"completed_month_sessions\":%d,\"tail_count\":%d,\"newest_rank\":%d,\"newest_ratio\":%.12e,\"newest_tied\":%s,\"direction\":%d,\"state\":\"%s\"}",
                            g_signal_month_key,
                            completed_month_sessions,
                            tail_count,
                            newest_rank,
                            newest_ratio,
                            newest_tied ? "true" : "false",
                            direction,
                            state));
   if(!valid || direction == 0)
      return false;
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG completed-month close-quartile reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41119\",\"ea\":\"xauxag-mclose-quartile-rv\"}");
