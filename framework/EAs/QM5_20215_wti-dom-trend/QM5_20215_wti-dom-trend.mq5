...
#property description "QM5_20215 WTI Exact Day-of-Month Slow-Trend Agreement"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_20215 - WTI Exact Day-of-Month / Slow-Trend Agreement
// -----------------------------------------------------------------------------
// Structural D1 crude-oil sleeve:
//   - exact broker day 1: BUY only with positive completed 252-D1 return
//   - exact broker day 26: SELL only with negative completed 252-D1 return
...
bool Strategy_LoadMomentum(double &momentum,
                           int &direction)
  {
   ...
   const int copied =
      CopyClose(_Symbol, // perf-allowed: bounded completed D1 trend sample.
                PERIOD_D1,
                1,
                required,
                closes);
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(signal_day == strategy_long_day &&
     ...
     {
      req.type = QM_BUY;
      req.reason = "WTI_DOM1_POS252_LONG";
     }
   else if(signal_day == strategy_short_day &&
           direction == -1)
     {
      req.type = QM_SELL;
      req.reason = "WTI_DOM26_NEG252_SHORT";
     }
   else
      return false;
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI exact-date trend card requires Friday 21 flattening"))
      return INIT_FAILED;

   g_attempt_state_key =
      StringFormat("QM5_20215_DATE_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_20215\",\"ea\":\"wti-dom-trend\"}");
