...
#property description "QM5_41246 XAU/XAG Thirteen-Month Turning-Point Reversion"
...
// completed broker months. Count strict local peaks and troughs in the
// chronological gold-minus-silver log-ratio path. When TP<=7, fade the
// oldest-to-newest displacement with an opposite, equal-notional package held
// to the next month. Runtime is native, completed-price-only, deterministic,
// and one consumed attempt per month.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41246;
...
input int    strategy_max_turning_points       = 7;
input double strategy_ratio_tie_epsilon         = 0.000000000001;
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41246 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
            strategy_xag_symbol == "XAGUSD.DWX" &&
            strategy_endpoint_count == 13 &&
            strategy_max_turning_points == 7 &&
            MathAbs(strategy_ratio_tie_epsilon - 1.0e-12) <= 1.0e-18 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_MTURNPOINT_RV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41246_XAUXAG_MTURNPOINT_RV";
...
bool Strategy_LoadMonthlyTurningPointSignal(
      ...
      int &direction)
  {
   ...
   // The card requires every endpoint pair to be separated beyond epsilon.
   // Ties consume the month flat and never enter the local-extrema count.
   for(int older = 0; older < strategy_endpoint_count - 1; ++older)
     {
      for(int newer = older + 1; newer < strategy_endpoint_count; ++newer)
        {
         const double distance =
            MathAbs(chronological_ratios[newer] -
                    chronological_ratios[older]);
         if(!MathIsValidNumber(distance))
            return false;
         if(distance <= strategy_ratio_tie_epsilon)
            ++tie_count;
        }
     }
   if(tie_count > 0)
   ...
   for(int index = 1; index < strategy_endpoint_count - 1; ++index)
     {
      ++interior_count;
      const bool peak =
         chronological_ratios[index - 1] <
            chronological_ratios[index] &&
         chronological_ratios[index] >
            chronological_ratios[index + 1];
      const bool trough =
         chronological_ratios[index - 1] >
   ...
   if(interior_count != strategy_endpoint_count - 2 ||
      interior_count != 11 ||
      turning_point_count < 0 ||
      turning_point_count > interior_count)
      return false;

   const bool persistent =
      turning_point_count <= strategy_max_turning_points &&
      3 * turning_point_count < 22;
   if(!persistent)
      return true;

   // direction > 0 means BUY XAU / SELL XAG. Fade the qualified persistent
   // log-ratio displacement; count and displacement magnitude never scale risk.
   if(endpoint_displacement > strategy_ratio_tie_epsilon)
      direction = -1;
   else if(endpoint_displacement < -strategy_ratio_tie_epsilon)
      direction = 1;
   return true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41246_XAUXAG_MTURNPOINT_RV_HOST";
   ...
   string state = "monthly_turning_point_signal_failed";
   if(valid && direction > 0)
      state = "persistent_ratio_down_fade_long_xau";
   else if(valid && direction < 0)
      state = "persistent_ratio_up_fade_short_xau";
   else if(valid && tie_count > 0)
      state = "ratio_tie_flat";
   else if(valid &&
           turning_point_count > strategy_max_turning_points)
      state = "turning_point_count_not_persistent_flat";
   else if(valid)
      state = "zero_endpoint_displacement_flat";

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"month_count\":%d,\"newest_endpoint_time\":%I64d,\"endpoint_displacement\":%.12e,\"interior_count\":%d,\"turning_point_count\":%d,\"tie_count\":%d,\"direction\":%d,\"ratios\":\"%s\",\"state\":\"%s\"}",
                            g_signal_month_key,
                            month_count,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG thirteen-month turning-point persistence reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41246\",\"ea\":\"xauxag-mturnpoint-rv\"}");
