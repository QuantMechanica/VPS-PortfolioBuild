...
#property description "QM5_41135 XAU/XAG Completed-Month Daily Interquartile-Mean Reversion"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_41135 - XAU/XAG Completed-Month Daily Interquartile-Mean Reversion
// -----------------------------------------------------------------------------
...
// Sort every gold-minus-silver relative return, remove floor(n/4) observations
// from each tail, and fade the strict sign of the retained central-band mean.
// The opposite, equal-notional package is held to the next month. Runtime is
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41135 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_MDAILY_IQRMEAN_RV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41135_XAUXAG_MDAILY_IQRMEAN_RV";
...
bool Strategy_LoadMonthlyDailyInterquartileMean(
      ...
      int &direction)
  {
   ...
   if(ArrayResize(series_ratios, strategy_max_month_sessions) !=
      ...
      strategy_max_month_sessions ||
      ArrayResize(sorted_returns, strategy_max_month_sessions) !=
      strategy_max_month_sessions)
      return false;
   ...
   if(ArrayResize(sorted_returns, return_count) != return_count ||
   ...
   retained_count = return_count - 2 * trim_each_tail;
   const int retained_begin = trim_each_tail;
   const int retained_end = return_count - trim_each_tail; // exclusive
   if(trim_each_tail < 4 || trim_each_tail > 5 ||
      retained_count < strategy_min_retained_returns ||
      retained_count > 13 ||
      retained_begin < 0 || retained_end > return_count ||
      retained_begin >= retained_end ||
      retained_end - retained_begin != retained_count ||
      2 * trim_each_tail + retained_count != return_count)
      return false;

   for(int i = retained_begin; i < retained_end; ++i)
     {
      if(i < 0 || i >= ArraySize(sorted_returns) ||
         !MathIsValidNumber(sorted_returns[i]))
         return false;
      central_sum += sorted_returns[i];
      if(!MathIsValidNumber(central_sum))
         return false;
     }
   central_mean = central_sum / (double)retained_count;
   if(!MathIsValidNumber(central_mean))
      return false;

   // direction > 0 means BUY XAU / SELL XAG. Fade only the strict sign of the
   // retained central band. The raw endpoint is an identity diagnostic only.
   if(central_mean < 0.0)
      direction = 1;
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41135_XAUXAG_MDAILY_IQRMEAN_RV_HOST";
   ...
   int return_count = 0;
   int trim_each_tail = 0;
   int retained_count = 0;
   int direction = 0;
   double raw_displacement = 0.0;
   double endpoint_displacement = 0.0;
   double central_sum = 0.0;
   double central_mean = 0.0;
   const bool valid =
      Strategy_LoadMonthlyDailyInterquartileMean(
         g_signal_month_key,
         completed_month_sessions,
         return_count,
         raw_displacement,
         ...
         central_mean,
         direction);
   string state = "monthly_daily_iqrmean_failed";
   if(valid && direction > 0)
      state = "negative_central_mean_fade_long_xau";
   else if(valid && direction < 0)
      state = "positive_central_mean_fade_short_xau";
   else if(valid && central_mean == 0.0)
      state = "zero_central_mean_flat";
   else if(valid)
      state = "central_mean_sign_flat";

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"completed_month_sessions\":%d,\"return_count\":%d,\"raw_displacement\":%.12e,\"endpoint_displacement\":%.12e,\"trim_each_tail\":%d,\"retained_count\":%d,\"central_sum\":%.12e,\"central_mean\":%.12e,\"direction\":%d,\"state\":\"%s\"}",
                            g_signal_month_key,
                            completed_month_sessions,
                            return_count,
                            raw_displacement,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG completed-month daily interquartile-mean reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41135\",\"ea\":\"xauxag-mdaily-iqrmean-rv\"}");
