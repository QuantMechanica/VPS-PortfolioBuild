...
#property description "QM5_41083 XAU/XAG Weekly Leg-Divergence Reversion"
...
// silver log returns over that exact interval have strictly opposite signs,
// fade the relative winner with an equal-notional opposite-leg package.
// Consume one attempt per week and flatten at the next week boundary.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41083;
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41083 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_WLEGDIV_ATTEMPT_WEEK", qm_ea_id);
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41083_XAUXAG_WLEGDIV_RV";
...
bool Strategy_LoadWeeklyLegDivergence(const int current_week,
                                      int &direction)
  {
   direction = 0;
   ...
   if(newer_week <= 0 || older_week <= 0 ||
      ...
      g_newer_week_sessions < strategy_min_week_sessions ||
      g_newer_week_sessions > strategy_max_week_sessions ||
      g_older_week_sessions < strategy_min_week_sessions ||
      g_older_week_sessions > strategy_max_week_sessions ||
   ...
   if(g_gold_week_return > 0.0 && g_silver_week_return < 0.0)
      direction = -1;
   else if(g_gold_week_return < 0.0 && g_silver_week_return > 0.0)
      direction = 1;
   return true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41083_XAUXAG_WLEGDIV_RV_HOST";
   ...
   if(Strategy_OpenOwnedPositionCount() > 0 ||
     ...
     {
      signal_valid =
         Strategy_LoadWeeklyLegDivergence(g_signal_week_key, direction);
      if(!signal_valid)
         g_signal_state = "endpoint_validation_failed";
      else if(direction < 0)
         g_signal_state = "gold_up_silver_down_fade_short_xau";
      else if(direction > 0)
         g_signal_state = "gold_down_silver_up_fade_long_xau";
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG weekly leg-divergence package exits at the next broker week"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41083\",\"ea\":\"xauxag-wlegdiv-rv\"}");
