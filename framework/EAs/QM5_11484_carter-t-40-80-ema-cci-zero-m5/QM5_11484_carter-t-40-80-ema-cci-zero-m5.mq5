...
#property description "QM5_11484 carter-t-40-80-ema-cci-zero-m5 — 40/80 EMA trend + CCI zero-cross (M5)"

#include <QM/QM_Common.mqh>

// =============================================================================
// QuantMechanica V5 EA — QM5_11484 carter-t-40-80-ema-cci-zero-m5
// -----------------------------------------------------------------------------
// Source: Thomas Carter, "20 Forex Trading Strategies (5 Minute Time Frame)",
//         System #10 (2014). Card:
//         artifacts/cards_approved/QM5_11484_carter-t-40-80-ema-cci-zero-m5.md
//         (g0_status APPROVED).
//
// Mechanics (closed-bar reads at shift 1; M5):
//   Trend STATE  : EMA(fast) vs EMA(slow) — fast>slow = up bias, fast<slow = down.
//   Trigger EVENT: CCI(period) crosses its zero line in the trend direction.
...
//   No-Friday-entry: card filter — block new entries on the broker Friday session.
...
input int    strategy_sl_pips           = 12;     // fixed stop-loss in pips (P3 sweep 10/12/15)
input int    strategy_tp_pips           = 12;     // fixed take-profit in pips (P3 sweep 10/12/15)
input bool   strategy_no_friday_entry   = true;   // skip new entries on broker Friday
...
// Entry: EMA(40)/EMA(80) trend state + CCI zero-line cross in the trend
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   // --- No-Friday-entry filter (card): skip new entries on broker Friday ---
   ...
   // cci@1 = last closed bar, cci@2 = prior closed bar.
   ...
   // Fresh upward zero-cross confirming an up-trend.
   const bool long_trigger  = (trend_up   && cci_cur > 0.0 && cci_prev <= 0.0);
   // Fresh downward zero-cross confirming a down-trend.
