# QM5_20256_wti-vr6-mom — Strategy Spec
...
- Timeframe: D1
- Magic: 202560000
- Canonical card: strategy-seeds/cards/approved/QM5_20256_wti-vr6-mom_card.md
...
monthly log returns. It computes the source-declared Mehlitz-Auer R6-q7 state:

- R6 is the sum of the latest six completed monthly log returns.
- VR(7) is `1 + 12/7*rho(1) + 10/7*rho(2) + 8/7*rho(3) +
  6/7*rho(4) + 4/7*rho(5) + 2/7*rho(6)`.
- The heteroskedastic robust variance uses squared weights `144/49`, `100/49`,
  `64/49`, `36/49`, `16/49`, and `4/49`.
- The state is actionable only when `abs(z)` exceeds `1.64485362695147`.
- Significant persistence follows R6; significant anti-persistence reverses R6.
...
three robust terms. QM5_20256 implements the separately source-declared R6-q7
member: six-month rank, lags one through six, and six differently weighted
robust terms. QM5_20059 is plain six-month WTI momentum without a memory
significance gate, anti-persistence reversal, or flat state.
...
| strategy_rank_months | 6 | R6 ranking return |
| strategy_vr_q | 7 | q7 order |
...
- Trade entry: one source R6-q7 direction per broker month through the V5
...
is `strategy-seeds/sources/MEHLITZ-AUER-WTI-R6Q7-2026/source.md`.

## Kill Criteria

Retire below five completed trades per full post-warm-up year, on wrong or
nonconsecutive month ends, wrong R6/q7 arithmetic, a missing significance
