...
#property description "QM5_41076 XAU/XAG Weekly Acceleration-Overshoot Reversion"
...
// -----------------------------------------------------------------------------
// Reconstruct three synchronized completed broker-week-end XAU/XAG close
// pairs and form two adjacent changes in ln(XAU)-ln(XAG). When both relative
// returns have the same strict sign and the newest absolute move is strictly
// larger, fade that accelerating direction with an equal-notional opposite-leg
// package. Consume one attempt per week and flatten at the next week boundary.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41076;
...
double   g_old_relative_return = 0.0;
double   g_new_relative_return = 0.0;
double   g_spread_old = 0.0;
double   g_spread_middle = 0.0;
double   g_spread_new = 0.0;
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41076 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_WACCEL_ATTEMPT_WEEK", qm_ea_id);
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41076_XAUXAG_WACCEL_RV";
...
bool Strategy_LoadWeeklyAcceleration(const int current_week,
                                     int &direction)
  {
   direction = 0;
   g_old_relative_return = 0.0;
   g_new_relative_return = 0.0;
   g_spread_old = 0.0;
   g_spread_middle = 0.0;
   g_spread_new = 0.0;
   ...
   double spreads[3] = {0.0, 0.0, 0.0};
   int found = 0;
   for(int index = 0;
       index < strategy_history_bars_d1 && found < 3;
       ++index)
     {
   ...
   g_spread_new = spreads[0];
   g_spread_middle = spreads[1];
   g_spread_old = spreads[2];
   g_new_relative_return = g_spread_new - g_spread_middle;
   g_old_relative_return = g_spread_middle - g_spread_old;
   if(!MathIsValidNumber(g_new_relative_return) ||
      !MathIsValidNumber(g_old_relative_return))
      return false;

   const bool strictly_accelerating =
      (MathAbs(g_new_relative_return) > MathAbs(g_old_relative_return));
   if(g_old_relative_return > 0.0 &&
      g_new_relative_return > 0.0 && strictly_accelerating)
      direction = -1;
   else if(g_old_relative_return < 0.0 &&
           g_new_relative_return < 0.0 && strictly_accelerating)
      direction = 1;
   return true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41076_XAUXAG_WACCEL_RV_HOST";
   ...
   if(Strategy_OpenOwnedPositionCount() > 0 ||
     ...
     {
      signal_valid =
         Strategy_LoadWeeklyAcceleration(g_signal_week_key, direction);
      if(!signal_valid)
         g_signal_state = "endpoint_validation_failed";
      else if(direction < 0)
         g_signal_state = "positive_acceleration_short_xau";
      else if(direction > 0)
         g_signal_state = "negative_acceleration_long_xau";
      else if(g_new_relative_return == 0.0 ||
              g_old_relative_return == 0.0)
         ...
         g_signal_state = "no_strict_acceleration_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"week\":%d,\"decision_bar\":%I64d,\"late\":%s,\"valid\":%s,\"direction\":%d,\"old_return\":%.12e,\"new_return\":%.12e,\"spread_old\":%.12e,\"spread_middle\":%.12e,\"spread_new\":%.12e,\"state\":\"%s\"}",
                            g_signal_week_key,
                            (long)g_current_host_bar,
                            g_late_decision ? "true" : "false",
                            signal_valid ? "true" : "false",
                            direction,
                            g_old_relative_return,
                            g_new_relative_return,
                            g_spread_old,
                            g_spread_middle,
                            g_spread_new,
                            g_signal_state));
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG weekly acceleration overshoot package exits at the next broker week"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41076\",\"ea\":\"xauxag-waccel-rv\"}");
