"""Reference checks for QM5_41076 XAU/XAG weekly acceleration overshoot."""
...
def weekly_acceleration_signal(
    ...
    """Mirror three synchronized week ends and strict same-sign acceleration."""
...
class WeeklyAccelerationReferenceTest(unittest.TestCase):
    def test_positive_strict_acceleration_sells_xau(self) -> None:
        bars = pair_bars((5.20, 5.05, 5.00))
    ...
    def test_negative_strict_acceleration_buys_xau(self) -> None:
        bars = pair_bars((4.80, 4.95, 5.00))
        valid, direction, new_value, old_value, _ = weekly_acceleration_signal(20260824, bars)
        self.assertTrue(valid)
        self.assertEqual(direction, 1)
        self.assertAlmostEqual(new_value, -0.15)
    ...
    def test_equality_nonacceleration_mixed_sign_and_zero_are_flat(self) -> None:
        cases = (
            (5.20, 5.10, 5.00),
            (5.15, 5.10, 5.00),
            (5.05, 5.10, 5.00),
