...
#property description "QM5_41378 WTI Half-Year Weekly Momentum"
...
//   - validate 26 consecutive completed 2-to-5-session broker weeks
//   - exclude t-4..t-1; compute log(final close[t-5] / first open[t-26])
//   - follow the exact CMOM26,5 formation return's strict sign
...
input int    strategy_history_bars          = 160;
input int    strategy_total_weeks           = 26;
input int    strategy_skip_recent_weeks     = 4;
input int    strategy_min_week_bars         = 2;
...
int      g_formation_bars         = 0;
double   g_formation_open         = 0.0;
double   g_formation_close        = 0.0;
double   g_formation_log_return   = 0.0;
...
void Strategy_ResetDecisionState()
  {
   ...
   g_formation_bars = 0;
   g_formation_open = 0.0;
   g_formation_close = 0.0;
   g_formation_log_return = 0.0;
   g_signal_state = "idle";
  }
...
bool Strategy_LoadWeeklyMomentumSignal(const int current_week_key,
                                       ...
                                       int &direction,
                                       int &formation_bars,
                                       double &formation_open,
                                       double &formation_close,
                                       double &formation_log_return)
  {
   direction = 0;
   formation_bars = 0;
   formation_open = 0.0;
   formation_close = 0.0;
   formation_log_return = 0.0;
   if(current_week_key <= 0 ||
   ...
   ArraySetAsSeries(bars, true);
   const int copied =
      CopyRates(_Symbol, // perf-allowed: bounded CMOM26,5 scan behind the sole QM_IsNewBar branch.
                PERIOD_D1,
                1,
                strategy_history_bars,
                bars);
   if(copied < strategy_total_weeks * strategy_min_week_bars + 1)
      return false;

   int week_keys[26];
   int week_counts[26];
   double week_opens[26];
   double week_closes[26];
      ...
      if(week_key != active_week_key)
        {
         if(active_week_key == 0)
           {
            if(Strategy_NextWeekKey(week_key) != current_week_key)
               return false;
            week_index = 0; // t-1: newest excluded chronology week.
           }
         else
           {
            if(Strategy_NextWeekKey(week_key) != active_week_key)
               return false;
            if(week_index == strategy_total_weeks - 1)
              {
         ...
         if(week_index < 0 || week_index >= strategy_total_weeks)
            return false;
   ...
   if(!older_boundary_seen || week_index != strategy_total_weeks - 1)
      return false;
   for(int index = 0; index < strategy_total_weeks; ++index)
     {
   ...
   // CMOM26,5: t-4..t-1 (indices 3..0) are chronology-only. Formation
   // endpoints are first open t-26 (index 25) and final close t-5 (index 4).
   formation_open = week_opens[strategy_total_weeks - 1];
   formation_close = week_closes[strategy_skip_recent_weeks];
   for(int index = strategy_skip_recent_weeks;
       index < strategy_total_weeks;
       ++index)
      formation_bars += week_counts[index];
   if(formation_open <= 0.0 || formation_close <= 0.0 ||
      !MathIsValidNumber(formation_open) ||
      !MathIsValidNumber(formation_close))
      return false;

   formation_log_return = MathLog(formation_close / formation_open);
   if(!MathIsValidNumber(formation_log_return))
      return false;
   if(formation_log_return > strategy_return_epsilon)
      direction = 1;
   else if(formation_log_return < -strategy_return_epsilon)
      direction = -1;
   return true;
  }
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   else
     {
      g_signal_valid =
         Strategy_LoadWeeklyMomentumSignal(g_decision_week_key,
                                           ...
                                           g_formation_open,
                                           g_formation_close,
                                           g_formation_log_return);
      if(!g_signal_valid)
         g_signal_state = "cmom265_validation_failed";
      else if(g_signal_direction > 0)
         g_signal_state = "positive_cmom265_long";
      else if(g_signal_direction < 0)
         g_signal_state = "negative_cmom265_short";
      else
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"week\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_week_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"formation_bars\":%d,\"formation_open_t26\":%.8f,\"formation_close_t5\":%.8f,\"formation_log_return\":%.12f,\"state\":\"%s\"}",
                            g_decision_week_key,
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41378_WTI_WMOM265";
...
int OnInit()
  {
   if(!SymbolSelect(strategy_symbol, true) ||
       !Strategy_IsHostChart() || qm_ea_id != 41378 ||
      qm_magic_slot_offset != 0)
   ...
   g_attempt_state_key =
      StringFormat("QM5_41378_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
