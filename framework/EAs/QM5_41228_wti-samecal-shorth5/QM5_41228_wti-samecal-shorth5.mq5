...
#property description "QM5_41228 WTI Same-Calendar Shortest-Half Midmean"
...
//   - sort the five returns and compare all three adjacent 3-value spans
//   - select the earliest narrowest window and average its three values
//   - follow only when that shortest-half midmean is beyond +/-1e-12
...
input int    strategy_window_size         = 3;
input int    strategy_window_count        = 3;
...
// -----------------------------------------------------------------------------
// Exact completed-month endpoint and shortest-half-midmean helpers.
// -----------------------------------------------------------------------------
...
bool Strategy_ShortestHalfMidmeanSignal(const double &observations[],
                                        ...
                                        int &direction)
  {
   ...
   if(ArraySize(sorted_values) != sample_count)
      return false;
   for(int index = 0; index < sample_count; ++index)
     {
   ...
   double spans[];
   if(ArrayResize(spans, strategy_window_count) != strategy_window_count)
      return false;
   for(int index = 0; index < strategy_window_count; ++index)
     {
      const int end_index = index + strategy_window_size - 1;
      if(end_index < index || end_index >= sample_count)
         return false;
      spans[index] = sorted_values[end_index] - sorted_values[index];
      if(!MathIsValidNumber(spans[index]) || spans[index] < 0.0)
         return false;
     }
   ...
   location_value =
      (sorted_values[best_index] +
       sorted_values[best_index + 1] +
       sorted_values[best_index + 2]) / 3.0;
...
bool Strategy_LoadShortestHalfMidmeanSignal(
   ...
   int &direction)
  {
   ...
   // Exact year membership is load-bearing; the location helper sorts values.
   ...
   return Strategy_ShortestHalfMidmeanSignal(observations,
                                              sample_count,
                                              location_value,
                                              direction);
  }
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   // Consume before history, shortest-half arithmetic, news, spread, quote,
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
     ...
     {
      g_signal_valid =
          Strategy_LoadShortestHalfMidmeanSignal(
            g_decision_bar_time,
            ...
            g_signal_direction);
      if(!g_signal_valid)
         g_signal_state = "endpoint_sample_or_shorth_failed";
      else if(g_signal_direction > 0)
         g_signal_state = "shortest_half_midmean_long";
      else if(g_signal_direction < 0)
         g_signal_state = "shortest_half_midmean_short";
      else
         g_signal_state = "shortest_half_midmean_epsilon_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"valid\":%s,\"samples\":%d,\"shortest_half_midmean\":%.12g,\"direction\":%d,\"state\":\"%s\"}",
                            g_decision_month_key,
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(!QM_InputRequireLong("qm_ea_id", qm_ea_id, 41228) ||
      ...
      !QM_InputRequireLong("strategy_window_size", strategy_window_size, 3) ||
      !QM_InputRequireLong("strategy_window_count", strategy_window_count, 3) ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "WTI_SAMECAL_SHORTH5";
   ...
   if(g_signal_direction > 0)
     {
      req.type = QM_BUY;
      req.reason = "WTI_SAMECAL_SHORTH5_LONG";
     }
   else
     {
      req.type = QM_SELL;
      req.reason = "WTI_SAMECAL_SHORTH5_SHORT";
     }
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41228 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI same-calendar shortest-half-midmean card holds to the next broker month"))
      return INIT_FAILED;
   ...
   g_attempt_state_key =
      StringFormat("QM5_41228_SAMECAL_SHORTH5_MONTH_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41228\",\"ea\":\"wti-samecal-shorth5\"}");
