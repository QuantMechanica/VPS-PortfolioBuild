"""Independent deterministic fixtures for QM5_41228.

The suite covers normalized D1 labels, completed WTI month endpoints, exact
Y-5..Y-1 membership, shortest-half-midmean arithmetic, strict earliest-window
ties, sign boundaries, durable attempts, lifecycle, quotes, and static
...
def shortest_half_midmean_signal(
    ...
    spans = tuple(
        sorted_values[index + 2] - sorted_values[index]
    ...
    for index in range(1, 3):
        if spans[index] < spans[best_index]:
            best_index = index
    selected = sorted_values[best_index : best_index + 3]
    location = sum(selected) / 3.0
    ...
    if location > EPSILON:
        ...
        direction = 0
    return ShorthResult(
        sorted_values, spans, best_index, location, direction
    )
...
class SameCalendarShorthReferenceTests(unittest.TestCase):
    ...
    def test_shortest_three_window_and_mean_are_exact(self) -> None:
        values = [-0.20, -0.19, 0.001, 0.20, 0.21]
        result = shortest_half_midmean_signal(values)
        self.assertIsNotNone(result)
        assert result is not None
        self.assertEqual(result.sorted_values, tuple(values))
        for observed, wanted in zip(result.spans, (0.201, 0.390, 0.209)):
            self.assertAlmostEqual(observed, wanted)
        self.assertEqual(result.best_index, 0)
        self.assertAlmostEqual(result.location, -0.12966666666666668)
        self.assertEqual(result.direction, -1)

    def test_disagreement_fixture_opposes_mean_median_trim_and_winsor(self) -> None:
        values = [-0.20, -0.19, 0.001, 0.20, 0.21]
        result = shortest_half_midmean_signal(values)
        raw_mean = sum(values) / 5.0
        median = sorted(values)[2]
        trimmed = sum(sorted(values)[1:4]) / 3.0
        winsorized = [-0.19, -0.19, 0.001, 0.20, 0.20]
        winsor_mean = sum(winsorized) / 5.0
        self.assertEqual(result.direction if result else 0, -1)
        self.assertGreater(raw_mean, 0.0)
        self.assertGreater(median, 0.0)
        self.assertGreater(trimmed, 0.0)
        self.assertGreater(winsor_mean, 0.0)

    def test_exact_span_tie_selects_earliest_window(self) -> None:
        values = [-0.03125, -0.015625, 0.0, 0.015625, 0.03125]
        result = shortest_half_midmean_signal(values)
        self.assertIsNotNone(result)
        assert result is not None
        self.assertEqual(result.spans, (0.03125, 0.03125, 0.03125))
        self.assertEqual(result.best_index, 0)
        self.assertEqual(result.location, -0.015625)
        self.assertEqual(result.direction, -1)
        self.assertEqual(sum(values), 0.0)
        self.assertEqual(sorted(values)[2], 0.0)

    def test_sort_makes_year_order_irrelevant_after_exact_membership(self) -> None:
        values = [-0.20, -0.19, 0.001, 0.20, 0.21]
        forward = shortest_half_midmean_signal(values)
        reverse = shortest_half_midmean_signal(list(reversed(values)))
        self.assertEqual(forward, reverse)

    def test_strict_epsilon_and_invalid_states_fail_closed(self) -> None:
        at_positive = shortest_half_midmean_signal([EPSILON] * 5)
        at_negative = shortest_half_midmean_signal([-EPSILON] * 5)
        beyond = shortest_half_midmean_signal([EPSILON + 1.0e-15] * 5)
        ...
        self.assertIsNone(shortest_half_midmean_signal([0.01] * 4))
        self.assertIsNone(
            shortest_half_midmean_signal(
                [0.01, 0.01, math.nan, 0.01, 0.01]
            )
        )
    ...
    def test_static_build_contract_matches_approved_card(self) -> None:
        source_path = EA_DIR / "QM5_41228_wti-samecal-shorth5.mq5"
        source = source_path.read_text(encoding="utf-8")
        set_path = (
            EA_DIR
            / "sets"
            / "QM5_41228_wti-samecal-shorth5_XTIUSD.DWX_D1_backtest.set"
        )
        ...
        for marker in (
            ...
            "Strategy_ShortestHalfMidmeanSignal",
        ...
        self.assertLess(
            ...
            prepare.index("Strategy_LoadShortestHalfMidmeanSignal"),
        )
        ...
        approved = (
            ...
            / "cards"
            / "approved"
            / "QM5_41228_wti-samecal-shorth5_card.md"
        )
        ...
        self.assertIn(
            "41228,wti-samecal-shorth5,0,XTIUSD.DWX,412280000",
            magic_rows,
        )
        resolver = (
            REPO_ROOT
        ...
        self.assertIn("412280000", resolver)
