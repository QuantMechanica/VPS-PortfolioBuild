...
#property description "QM5_12978 Edge Lab GBPUSD USDCAD Cointegration"
...
input double strategy_beta              = -1.140460285727;
...
bool Strategy_IsHostSymbol()
  {
   return (_Symbol == g_leg_gbpusd || _Symbol == g_leg_usdcad);
  }
...
bool Strategy_EnsureBasketScope()
  {
   ...
   string allowed[2] = {"GBPUSD.DWX", "USDCAD.DWX"};
   for(int i = 0; i < 2; ++i)
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(!Strategy_EnsureBasketScope())
      return false;
   if(!QM_SymbolAssertOrLog(g_leg_gbpusd) || !QM_SymbolAssertOrLog(g_leg_usdcad))
      return false;
...
bool Strategy_OpenPair(const int spread_direction)
  {
   ...
   const double weight_sum = gbpusd_weight + usdcad_weight;
   if(weight_sum <= 0.0)
      return false;

   const bool long_spread = (spread_direction > 0);
   const QM_OrderType gbpusd_type = long_spread ? QM_BUY : QM_SELL;
   const QM_OrderType usdcad_type = long_spread
                                    ? (strategy_beta >= 0.0 ? QM_SELL : QM_BUY)
                                    : (strategy_beta >= 0.0 ? QM_BUY : QM_SELL);
   const string reason = long_spread ? "QM5_12978_LONG_SPREAD_Z_LT_NEG_ENTRY"
                                     : "QM5_12978_SHORT_SPREAD_Z_GT_POS_ENTRY";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12978_PAIR_HOST";
