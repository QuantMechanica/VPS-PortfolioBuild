"""Deterministic reference checks for QM5_41448 XAU/XAG weekly WR2 CLV reversion."""
...
MQ5 = EA_DIR / "QM5_41448_xauxag-wr2-clv-rv.mq5"
SETFILE = (
    EA_DIR
    / "sets"
    / "QM5_41448_xauxag-wr2-clv-rv_QM5_41448_XAU_XAG_WR2_CLV_RV_D1_D1_backtest.set"
)
...
def wr2_clv_signal(prior_range: float, newest_range: float, clv: float) -> int:
    if any(not math.isfinite(value) for value in (prior_range, newest_range, clv)):
        return 0
    if prior_range <= 0 or newest_range <= prior_range or not 0 <= clv <= 1:
        return 0
    if clv > 0.75:
        return -1
    if clv < 0.25:
...
class Wr2ClvReversionReferenceTest(unittest.TestCase):
    def test_ratio_range_and_both_contrarian_sides(self) -> None:
        prior_width, _ = range_and_clv([4.60, 4.61, 4.59, 4.60])
        upper_width, upper_clv = range_and_clv([4.58, 4.61, 4.57, 4.62])
        lower_width, lower_clv = range_and_clv([4.62, 4.59, 4.63, 4.57])
        self.assertEqual(wr2_clv_signal(prior_width, upper_width, upper_clv), -1)
        self.assertEqual(wr2_clv_signal(prior_width, lower_width, lower_clv), 1)

    def test_ties_contraction_boundaries_and_interior_are_flat(self) -> None:
        for prior, newest, clv in (
            ...
            with self.subTest(prior=prior, newest=newest, clv=clv):
                self.assertEqual(wr2_clv_signal(prior, newest, clv), 0)
    ...
    def test_static_artifacts_lock_signal_risk_and_unpinned_framework(self) -> None:
        ...
        self.assertIn("qm_ea_id                    = 41448", source)
        self.assertIn("Strategy_LoadWr2Clv", source)
        ...
        self.assertRegex(resolver, re.compile(r"414480000.*414480001", re.DOTALL))
