---
strategy_id: FMR-MOMTS-2010_XAU_XAG_S03
...
g0_approval_reasoning: "OWNER commodity-sleeve mission authorizes the build. R1 PASS peer-reviewed JBF paper and complete accepted manuscript; R2 PASS locked prior-one-completed-month return rank, monthly paired hold, equal fixed risk, ATR stops, restart guard, and orphan cleanup; R3 PASS native registered XAU/XAG D1 data; R4 PASS no ML, banned indicator, external feed, grid, martingale, or pyramiding. Exact dedup CLEAN: existing XAU/XAG builds use ratio reversion, ratio breakout, return-spread reversion, conditional quantiles, or twelve-month momentum."
---

# XAU/XAG Monthly 1-Month Cross-Sectional Momentum
...
over the prior one completed broker month and shorts the weaker metal.
...
each of the two consecutive completed broker months ending at `t-1`:

```text
r_i = latest_completed_month_close_i / prior_month_close_i - 1
```

- `r_XAU > r_XAG`: BUY XAU, SELL XAG.
- `r_XAU < r_XAG`: SELL XAU, BUY XAG.
...
- Logical basket: `QM5_20057_XAU_XAG_XMOM1_D1`.
...
- Require exact XAU D1 host, magic slot 0, and two consecutive completed months.
- Calculate exactly one simple monthly return per leg.
...
|---|---:|---|---|
| `strategy_return_window_months` | 1 | locked | completed monthly returns |
...
momentum, not a precious-metals carrier. `QM5_20050` ranks twelve completed
monthly returns; no existing XAU/XAG build ranks only the immediately completed
month and holds that winner-minus-loser package for one month.

## Framework Alignment

- no_trade: host/window/history/spread/ATR/lot/magic/package/attempt guards.
- trade_entry: monthly one-return rank, paired orders, equal risk, hard stops.
