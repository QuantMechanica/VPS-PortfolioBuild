...
#property description "QM5_11428 connors-double7s-sma200-d1 — Connors Double 7's, SMA(200) trend + 7-day close extreme (D1)"

#include <QM/QM_Common.mqh>

// =============================================================================
// QuantMechanica V5 EA — QM5_11428 connors-double7s-sma200-d1
// -----------------------------------------------------------------------------
// Source: Larry Connors & Cesar Alvarez, "Short-Term Trading Strategies That
// Work" (2009), "Double 7's Strategy". Card:
// artifacts/cards_approved/QM5_11428_connors-double7s-sma200-d1.md (APPROVED).
// Sibling QM5_11366 implements the same strategy (different ea_id) — same idiom,
// built independently to THIS card. qm_ea_id = 11428.
//
   ...
   for(int s = 2; s <= lookback; ++s)
     {
      ...
      if(cs <= 0.0)
         return false;
      if(c1 > cs)
         return false; // a window close is lower -> not the N-day low
     }
   return true;
  }
