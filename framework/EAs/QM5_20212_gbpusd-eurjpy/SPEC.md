# QM5_20212_gbpusd-eurjpy — Strategy Spec
...
`ln(GBPUSD.DWX) - (-0.080732288) * ln(EURJPY.DWX)`. Score the newest spread
...
| `strategy_beta` | -0.080732288 | fixed | DEV-fitted structural hedge coefficient |
...
- `GBPUSD.DWX` — fixed tester host and first traded leg.
- `EURJPY.DWX` — fixed companion and second traded leg.
- `EURUSD.DWX` and `USDJPY.DWX` — conversion-history dependencies observed by
  the USD-account tester for EURJPY P/L; both are order-free and have no magic
  slot.
...
| Bar gating | `QM_IsNewBar(GBPUSD.DWX, PERIOD_D1)` consumed once per signal bar |
...
| Metric | Expected |
|---|---|
| Trades / year / symbol | about 4 completed package entries per leg |
| OOS basket state changes | 17 across 2023–2024 in the frozen scan |
| Typical hold time | multi-day; scan half-life 103.753 D1 bars |
| Expected drawdown profile | high and regime-sensitive; conservative 30% prior |
| Regime preference | GBPUSD/EURJPY fixed-residual mean reversion |
| Win rate target | unknown; Q02 is the first platform-economic judge |

The fixed scan recorded DEV net Sharpe `-0.215933`, OOS net Sharpe `0.221108`,
and OOS return `2.097281%`. The negative DEV score, long half-life, and small
absolute companion weight are adverse priors, not a promotion claim.
...
`strategy-seeds/cards/approved/QM5_20212_gbpusd-eurjpy_card.md`

Chan supplies the deterministic pair-trading method but makes no performance
claim for GBPUSD/EURJPY.
...
`1.0` and `0.038239845`; each leg is independently bounded by its ATR stop.
If the EURJPY allocation normalizes below broker minimum volume, the
whole package is rejected before any order is sent. ENV-to-mode validation
remains enforced by `QM_FrameworkInit`.
...
|---|---|---|---|
| v1 | 2026-08-04 | Initial build from approved card | Next-ranked non-duplicate fixed-scan FX basket |
