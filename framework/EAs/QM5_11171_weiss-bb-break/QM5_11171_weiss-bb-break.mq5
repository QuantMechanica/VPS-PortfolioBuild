...
#property description "QM5_11171 Weissman Bollinger breakout to mean exit"
...
bool Strategy_NoTradeFilter()
  {
   // Card has no strategy-specific time or spread gate; framework news/calendar gates apply.
   return false;
  }
...
void Strategy_ManageOpenPosition()
  {
   // Card specifies no trailing, break-even, partial close, or target.
  }
...
bool Strategy_ExitSignal()
  {
   ...
   const double middle = QM_BB_Middle(_Symbol, PERIOD_CURRENT,
                                      strategy_bb_period,
                                      strategy_bb_deviation,
                                      1);
   if(middle <= 0.0)
      ...
      if(position_type == POSITION_TYPE_BUY)
        {
         ...
         if(bid > 0.0 && bid <= middle)
            return true;
        }
      else if(position_type == POSITION_TYPE_SELL)
        {
         const double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
         if(ask > 0.0 && ask >= middle)
            return true;
        }
     }
