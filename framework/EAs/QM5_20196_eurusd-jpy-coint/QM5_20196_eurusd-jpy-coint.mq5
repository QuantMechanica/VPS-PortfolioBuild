...
#property description "QM5_20196 EURUSD USDJPY Cointegration"
...
//
// Card:
//   strategy-seeds/cards/approved/QM5_20196_eurusd-jpy-coint_card.md
//
// Fixed spread:
//   ln(EURUSD.DWX) - beta * ln(USDJPY.DWX)
//
...
input double strategy_beta              = -0.505485905;
...
bool Strategy_IsHostSymbol()
  {
   return (_Symbol == g_leg_eurusd || _Symbol == g_leg_usdjpy);
  }
...
bool Strategy_EnsureBasketScope()
  {
   ...
   string allowed[2] = {"EURUSD.DWX", "USDJPY.DWX"};
   for(int i = 0; i < 2; ++i)
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(!QM_SymbolAssertOrLog(g_leg_eurusd) ||
      !QM_SymbolAssertOrLog(g_leg_usdjpy))
      return false;
   ...
   for(int i = 0; i < history_count; ++i)
     {
      if(eurusd_time[i] <= 0 || eurusd_time[i] != usdjpy_time[i])
         return false;
      if(eurusd[i] <= 0.0 || usdjpy[i] <= 0.0)
         return false;

      spreads[i] =
         MathLog(eurusd[i]) - strategy_beta * MathLog(usdjpy[i]);
...
bool Strategy_OpenPair(const int spread_direction)
  {
   ...
   const double usdjpy_weight = MathAbs(strategy_beta);
   const double weight_sum = eurusd_weight + usdjpy_weight;
   if(weight_sum <= 0.0)
      return false;

   // Preflight both normalized legs before sending either order. A companion
   // leg below broker minimum volume rejects the complete package instead of
   // leaving a directional orphan.
   const double eurusd_lots =
      Strategy_LotsForLeg(g_leg_eurusd, eurusd_weight, weight_sum);
   const double usdjpy_lots =
      Strategy_LotsForLeg(g_leg_usdjpy, usdjpy_weight, weight_sum);
   if(eurusd_lots <= 0.0 || usdjpy_lots <= 0.0)
      return false;

   const bool long_spread = (spread_direction > 0);
   const QM_OrderType eurusd_type = long_spread ? QM_BUY : QM_SELL;
   const QM_OrderType usdjpy_type =
      long_spread
      ? (strategy_beta >= 0.0 ? QM_SELL : QM_BUY)
      : (strategy_beta >= 0.0 ? QM_BUY : QM_SELL);
   const string reason =
      long_spread ? "QM5_20196_LONG_SPREAD_Z_LT_NEG_ENTRY"
                  : "QM5_20196_SHORT_SPREAD_Z_GT_POS_ENTRY";

   const bool eurusd_ok =
      Strategy_OpenLeg(g_leg_eurusd,
                       eurusd_type,
                       eurusd_lots,
                       reason);
   const bool usdjpy_ok =
      Strategy_OpenLeg(g_leg_usdjpy,
                       usdjpy_type,
                       usdjpy_lots,
                       reason);
   if(eurusd_ok && usdjpy_ok)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20196_PAIR_HOST";
