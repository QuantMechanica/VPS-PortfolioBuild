# QM5_20196_eurusd-jpy-coint — Strategy Spec
...
`ln(EURUSD.DWX) - (-0.505485905) * ln(USDJPY.DWX)`. Score the newest spread
...
| `strategy_beta` | -0.505485905 | fixed | DEV-fitted structural hedge coefficient |
...
- `USDJPY.DWX` — fixed companion and second traded leg from the same row.
...
| Basket state changes | approximately 10 per year |
| Typical hold time | multi-day; scan half-life 87.404 D1 bars |
...
The fixed scan recorded DEV net Sharpe `0.661899`, OOS net Sharpe `0.686450`,
OOS return `4.994192%`, and 20 OOS state changes. These are weak frontier
priors, not a promotion claim.
...
`artifacts/cards_approved/QM5_20196_eurusd-jpy-coint_card.md`

Chan supplies the deterministic pair-trading method but makes no performance
claim for EURUSD/USDJPY.
...
| Live | not authorized | no live setfile or deployment artifact exists |
...
`1.0` and `0.505485905`; each leg is independently bounded by its ATR stop.
