...
#property description "QM5_41047 XNG Thursday Counter-Move / Slow-Trend Re-entry"
...
//   - enter Friday only when the two nonzero returns have opposite signs,
//     rejoining the slow trend after the completed Thursday counter-move
...
bool Strategy_LoadThursdayTrendPullback(const datetime current_bar_time,
                                        const datetime broker_now,
                                        double &event_return,
                                        double &slow_trend,
                                        int &direction)
  {
   ...
   // Strict multiplication implements the card's nonzero opposition gate.
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "XNG_THU_TREND_PULLBACK";
   ...
   int direction = 0;
   if(!Strategy_LoadThursdayTrendPullback(g_strategy_d1_bar_time,
                                          broker_now,
                                          event_return,
                                          slow_trend,
                                          direction))
   ...
   if(direction > 0)
     {
      req.type = QM_BUY;
      req.reason = "XNG_THU_TREND_PULLBACK_LONG";
     }
   else if(direction < 0)
     {
      req.type = QM_SELL;
      req.reason = "XNG_THU_TREND_PULLBACK_SHORT";
     }
   else
      return false;
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XNG Thursday counter-move/slow-trend card holds Friday to the next D1 boundary"))
      return INIT_FAILED;
   ...
   g_attempt_state_key =
      StringFormat("QM5_41047_THU_TREND_PULLBACK_FRIDAY_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41047\",\"ea\":\"xng-thu-trend-pb\"}");
