...
const int STRATEGY_PAIR_COUNT = 4;
string g_pair_a[4] = {"EURUSD.DWX", "AUDUSD.DWX", "USDCHF.DWX", "USDJPY.DWX"};
string g_pair_b[4] = {"GBPUSD.DWX", "NZDUSD.DWX", "USDCAD.DWX", "EURUSD.DWX"};
int    g_pair_slot_a[4] = {0, 2, 4, 6};
int    g_pair_slot_b[4] = {1, 3, 5, 0};
...
bool Strategy_ReadLogPrices(const string sym_a,
                            ...
                            double &log_b)
  {
   const double close_a = iClose(sym_a, PERIOD_H4, shift);
   const double close_b = iClose(sym_b, PERIOD_H4, shift);
...
bool Strategy_PairStats(double &z, double &corr)
  {
   ...
   const int pair_index = Strategy_PairIndex();
   if(pair_index < 0 || strategy_formation_bars < 60 || strategy_zscore_bars < 20)
      return false;
   ...
   const int n_form = strategy_formation_bars;
   const int n_z = strategy_zscore_bars;
   const int need_bars = MathMax(n_form, n_z) + 3;
   if(Bars(sym_a, PERIOD_H4) < need_bars || Bars(sym_b, PERIOD_H4) < need_bars)
      return false;
   ...
   double sum_xx = 0.0;
   double sum_xy = 0.0;
   for(int shift = 1; shift <= n_form; ++shift)
     {
      double y = 0.0;
      double x = 0.0;
   ...
   const double denom = (double)n_form * sum_xx - sum_x * sum_x;
   if(MathAbs(denom) <= DBL_EPSILON)
      return false;
   const double beta = ((double)n_form * sum_xy - sum_x * sum_y) / denom;
   if(!MathIsValidNumber(beta))
      return false;

   double spread_sum = 0.0;
   double spread_sumsq = 0.0;
   for(int shift = 1; shift <= n_z; ++shift)
     {
      double y = 0.0;
      double x = 0.0;
      if(!Strategy_ReadLogPrices(sym_a, sym_b, shift, y, x))
   ...
   const double mean = spread_sum / (double)n_z;
   const double variance = (spread_sumsq / (double)n_z) - mean * mean;
   if(variance <= 0.0)
      return false;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_10025_RW_FX_BROAD_PAIR";
   ...
   req.sl = (req.type == QM_BUY) ? entry - atr * strategy_atr_sl_mult
                                 : entry + atr * strategy_atr_sl_mult;
   req.reason = (spread_direction > 0) ? "QM5_10025_LONG_SPREAD_Z_LT_NEG2" : "QM5_10025_SHORT_SPREAD_Z_GT_POS2";
   return true;
  }
...
bool Strategy_ExitSignal()
  {
   ...
   if(MathAbs(z) <= strategy_exit_z)
      return true;
   if(MathAbs(z) >= strategy_spread_stop_z)
      return true;
   if(corr < 0.50)
      return true;
