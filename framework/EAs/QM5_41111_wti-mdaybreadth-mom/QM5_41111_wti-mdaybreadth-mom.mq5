...
#property description "QM5_41111 WTI Completed-Month Daily-Sign Breadth Momentum"
...
//   - anchor every newest-month daily close return at the parent final close
//   - require a strict daily-sign majority and same-sign endpoint return
//   - consume the month before fallible gates; never enter late or retry
//   - follow the breadth-confirmed completed-month direction for one month
// Runtime uses MT5-native completed closes, calendar, history, and framework state only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                      = 41111;
...
bool Strategy_LoadDailyBreadthSignal(const int current_month_key,
                                      const int label_offset,
                                      int &direction,
                                      int &new_month_bars,
                                      ...
                                      double &net_return)
  {
   ...
   if(up_count + down_count + flat_count != new_month_bars)
      return false;
   newest_final_close = previous_close;
   net_return = newest_final_close / parent_final_close - 1.0;
   if(!MathIsValidNumber(net_return))
      return false;

   if(2 * up_count > new_month_bars && net_return > 0.0)
      direction = 1;
   else if(2 * down_count > new_month_bars && net_return < 0.0)
      direction = -1;
   return true;
  }
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   // Consume before close validation, sign counting, news, spread, quote,
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadDailyBreadthSignal(g_decision_month_key,
      ...
      if(!g_signal_valid)
         ...
         g_signal_state = "daily_majority_endpoint_agreement_long";
      else if(g_signal_direction < 0)
         g_signal_state = "daily_majority_endpoint_agreement_short";
      else if(g_newest_final_close == g_parent_final_close)
         g_signal_state = "endpoint_equality_flat";
      else if(2 * g_up_count <= g_new_month_bars &&
              2 * g_down_count <= g_new_month_bars)
         g_signal_state = "no_strict_daily_majority_flat";
      else
         g_signal_state = "breadth_endpoint_disagreement_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"new_month_bars\":%d,\"parent_month_bars\":%d,\"up_count\":%d,\"down_count\":%d,\"flat_count\":%d,\"parent_final_close\":%.8f,\"newest_final_close\":%.8f,\"net_return\":%.12e,\"state\":\"%s\"}",
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41111 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41111_WTI_MDAYBREADTH_MOM";
   ...
   req.reason = (g_signal_direction > 0)
      ? "WTI_MDAYBREADTH_MOM_LONG"
      : "WTI_MDAYBREADTH_MOM_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41111 ||
   ...
   g_attempt_state_key =
      StringFormat("QM5_41111_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
                "{\"card\":\"QM5_41111\",\"ea\":\"wti-mdaybreadth-mom\"}");
