...
#property description "QM5_20019 XAU XAG weekend relative-value basket"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_20019 - XAU/XAG weekend relative-value basket
// -----------------------------------------------------------------------------
...
bool Strategy_IsHostChart()
  {
   return (_Symbol == g_leg_xau && _Period == PERIOD_H1 &&
           qm_magic_slot_offset == 0);
  }

bool Strategy_InputsValid()
  {
   return (qm_ea_id == 20019 && qm_magic_slot_offset == 0 &&
           strategy_xag_symbol == "XAGUSD.DWX" &&
           strategy_entry_dow == 5 && strategy_entry_hour_broker == 21 &&
...
bool Strategy_MondayBoundaryReached()
  {
   datetime entry_time = g_pair_entry_time;
   if(entry_time <= 0)
      entry_time = Strategy_CurrentPairEntryTime();
   return (g_is_new_bar && g_current_host_bar > entry_time &&
           Strategy_DayOfWeek(g_current_host_bar) == 1);
  }

string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_WKEND_ATTEMPT_DAY", qm_ea_id);
  }
...
bool Strategy_PreparePackage(double &xau_lots,
                             ...
                             double &xag_stop)
  {
   ...
   const double lot_ratio_xau_to_xag =
      strategy_notional_ratio * xag_notional_per_lot / xau_notional_per_lot;
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_20019_XAUXAG_WKEND";
...
bool Strategy_EntryWindowReady()
  {
   if(!g_is_new_bar || g_current_host_bar <= 0 ||
      ...
      Strategy_Hour(g_current_host_bar) != strategy_entry_hour_broker ||
   ...
   if(opening_delay < 0 ||
      ...
      return false;
   const datetime xag_bar = iTime(g_leg_xag, PERIOD_H1, 0); // perf-allowed: synchronized weekend boundary.
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_20019_XAUXAG_WKEND_HOST";
...
void Strategy_ManageOpenPosition()
  {
   ...
   if(Strategy_MondayBoundaryReached())
     {
      Strategy_CloseAllOwned(QM_EXIT_STRATEGY);
      return;
     }
...
int OnInit()
  {
   ...
   QM_BasketWarmupHistory(basket_symbols, PERIOD_H1, 80);
   QM_BasketWarmupHistory(basket_symbols, PERIOD_D1, 40);
   g_current_host_bar = iTime(g_leg_xau, PERIOD_H1, 0); // perf-allowed: restart state anchor.
   Strategy_LoadAttemptState(g_current_host_bar);
   g_pair_entry_time = Strategy_CurrentPairEntryTime();
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_20019\",\"ea\":\"xauxag-wkend\"}");
   ...
   if(g_is_new_bar)
     {
      ...
      g_current_host_bar = iTime(g_leg_xau, PERIOD_H1, 0); // perf-allowed: new-bar lifecycle and entry anchor.
     }
