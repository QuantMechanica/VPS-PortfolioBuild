# QM5_41213_xauxag-samecal-signscore - Strategy Spec

**EA ID:** QM5_41213

Slug: `xauxag-samecal-signscore`

Strategy ID: `KELOHARJU-PAPAILIAS-RCORE-XAUXAG-SAMECAL-SIGNSCORE-2026_S01`

Source: `KELOHARJU-PAPAILIAS-RCORE-XAUXAG-SAMECAL-SIGNSCORE-2026`
...
Map each relative return to one when nonnegative and zero when negative. For
count `x`, paired sample `n`, and fixed null `p0=0.5`, compute the signed
score `z=(x-n*p0)/sqrt(n*p0*(1-p0))=(2*x-n)/sqrt(n)` without continuity
correction. A score strictly above `1.0+1e-10` buys XAU and sells XAG; a
score strictly below `-1.0-1e-10` reverses both legs. Invalid state and the
inclusive confidence band consume the month flat. An opened package closes
at the next month.
...
| `strategy_null_probability` | 0.5 | fixed Bernoulli sign null |
...
- Host/traded slot 0: `XAUUSD.DWX`, magic `412130000`.
- Companion/traded slot 1: `XAGUSD.DWX`, magic `412130001`.
...
- Approximately four to seven two-leg packages/year after warm-up; Q02 retires
...
hold. Papailias, Liu, and Thomakos (2021), “Return Signal Momentum,” *Journal
of Banking & Finance* 124, 106063, supplies the binary return-sign
representation and equal weighting. Commit-pinned R Core `stats::prop.test`
source supplies the null-half expected counts and uncorrected Pearson-score
arithmetic only.
...
- No-Trade: exact host/slot/input, normalized label convention, synchronized
  endpoint, sample, finite sign score, spread, quote, stop, lot, package, and
  consumed-month guards.
- Entry: strict null-half sign-score side, opposite legs, shared fixed risk, and frozen
  stops.
