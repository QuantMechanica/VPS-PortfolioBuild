...
#property description "QM5_41213 XAU XAG Same-Calendar Relative Sign-Score"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_41213 - XAU/XAG Same-Calendar Relative Bernoulli Sign-Score
// -----------------------------------------------------------------------------
// Monthly precious-metals relative-value basket:
//   - reconstruct up to ten synchronized prior-year returns for this month
//   - map XAU-minus-XAG returns to signs and score against Bernoulli p0=0.5
...
int      g_cache_nonnegative_count = 0;
double   g_cache_score_denominator = 0.0;
double   g_cache_sign_score = 0.0;
...
string Strategy_AttemptStateName()
  {
   return "QM5_41213_XAUXAG_SAMECAL_SIGNSCORE_ATTEMPT";
  }
...
bool Strategy_SignScoreSignal(const double &observations[],
                              const int sample_count,
                              int &nonnegative_count,
                              ...
                              int &direction)
  {
   ...
   if(nonnegative_count < 0 || nonnegative_count > sample_count)
      return false;

   const double null_variance =
      (double)sample_count * strategy_null_probability *
      (1.0 - strategy_null_probability);
   if(!MathIsValidNumber(null_variance) || null_variance <= 0.0)
      return false;

   denominator_value = MathSqrt(null_variance);
   if(!MathIsValidNumber(denominator_value) || denominator_value <= 0.0)
      return false;
...
bool Strategy_LoadSignalState(const int decision_month_key,
                               int &pair_direction)
  {
   ...
   g_cache_nonnegative_count = 0;
   g_cache_score_denominator = 0.0;
   g_cache_sign_score = 0.0;
   ...
   return Strategy_SignScoreSignal(relative_returns,
                                   g_cache_sample_count,
                                   g_cache_nonnegative_count,
                                   g_cache_score_denominator,
                                   g_cache_sign_score,
                                   pair_direction);
  }
...
bool Strategy_OpenPair(const int pair_direction)
  {
   ...
   const string reason = long_xau_short_xag ? "QM5_41213_LONG_XAU_SHORT_XAG_SAMECAL_SIGNSCORE"
                                            : "QM5_41213_SHORT_XAU_LONG_XAG_SAMECAL_SIGNSCORE";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41213_XAU_XAG_SAMECAL_SIGNSCORE_HOST";
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_41213\",\"ea\":\"xauxag-samecal-signscore\"}");
