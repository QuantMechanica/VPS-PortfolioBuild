...
#property description "QM5_41216 Grimes Nested Pullback v2"
...
input bool   strategy_use_atr_trail_after_be       = false;
input double strategy_atr_trail_trigger_r          = 1.5;
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      if(strategy_use_atr_trail_after_be)
        {
         if(gained >= strategy_atr_trail_trigger_r * original_r &&
            strategy_atr_trail_mult > 0.0)
            QM_TM_TrailATR(ticket, strategy_h4_atr_period, strategy_atr_trail_mult);
         continue;
        }
...
void OnTick()
  {
   ...
   // News blackout gates entries only; open-position management and exits
   // remain reachable during restricted windows.
