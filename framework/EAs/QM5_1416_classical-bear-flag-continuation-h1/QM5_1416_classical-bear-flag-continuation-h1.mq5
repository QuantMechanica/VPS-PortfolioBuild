...
bool Strategy_MacroBias()
{
   ...
   const bool sma_falling = (sma_vals[0] <= sma_vals[1]);
   const bool price_below = (h1_rates[0].close < sma_vals[0]);
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
{
   ...
   // Breakdown candidate bar at shift 1
   ...
   {
      const int flag_start_shift = breakout_shift + 1; // start of flag (most recent bar before breakdown)
      ...
      for(int n_pole = strategy_pole_min_bars; n_pole <= strategy_pole_max_bars; ++n_pole)
      {
         ...
         // Phase 3: Breakdown Trigger at shift 1
         ...
         // Breakdown condition: close <= lower_TL(t_now) - 0.4 * ATR
         if(rates[breakout_shift].close > lower_line_now - strategy_breakout_buffer_atr * atr)
            continue;
...
bool Strategy_ExitSignal()
{
   ...
   // 1. Pattern-failure hard exit: if H1 close falls/rises back inside flag channel within first 6 bars after entry
   if(bars_open >= 1 && bars_open <= strategy_failure_exit_bars && g_active_lower_intercept > 0.0)
   {
      MqlRates rates[];
      ArraySetAsSeries(rates, true);
