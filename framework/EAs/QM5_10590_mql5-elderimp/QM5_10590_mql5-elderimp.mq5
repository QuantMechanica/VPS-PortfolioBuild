...
#property description "QM5_10590 MQL5 Elder Impulse Color Change"
...
int Strategy_ElderImpulseColor(const int shift)
  {
   ...
   const double ema_now = QM_EMA(_Symbol, strategy_signal_tf, strategy_ema_period, shift);
   const double ema_prev = QM_EMA(_Symbol, strategy_signal_tf, strategy_ema_period, shift + 1);
   const double macd_now = QM_MACD_Main(_Symbol, strategy_signal_tf,
                                        strategy_macd_fast,
                                        strategy_macd_slow,
                                        strategy_macd_signal,
                                        shift);
   const double macd_sig_now = QM_MACD_Signal(_Symbol, strategy_signal_tf,
                                              strategy_macd_fast,
                                              strategy_macd_slow,
                                              strategy_macd_signal,
                                              shift);
   const double macd_prev = QM_MACD_Main(_Symbol, strategy_signal_tf,
                                         ...
                                         strategy_macd_signal,
                                         shift + 1);
   const double macd_sig_prev = QM_MACD_Signal(_Symbol, strategy_signal_tf,
                                               strategy_macd_fast,
                                               strategy_macd_slow,
                                               strategy_macd_signal,
                                               shift + 1);
   ...
   const double hist_now = macd_now - macd_sig_now;
   const double hist_prev = macd_prev - macd_sig_prev;

   if(ema_now > ema_prev && hist_now > hist_prev)
      return 1;
   if(ema_now < ema_prev && hist_now < hist_prev)
      return -1;
...
int Strategy_ElderImpulseChangeSignal()
  {
   const int latest = Strategy_ElderImpulseColor(1);
   const int prior = Strategy_ElderImpulseColor(2);
   if(latest == 1 && prior != 1)
      return 1;
   if(latest == -1 && prior != -1)
      return -1;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   const int signal = Strategy_ElderImpulseChangeSignal();
   if(signal == 0)
      return false;
   ...
   req.reason = (signal > 0) ? "ELDER_IMPULSE_GREEN_CHANGE" : "ELDER_IMPULSE_RED_CHANGE";
   req.symbol_slot = qm_magic_slot_offset;
...
bool Strategy_ExitSignal()
  {
   // Opposite Elder Impulse color changes are handled inside Strategy_EntrySignal
   // under the framework closed-bar gate so this hook stays O(1) per tick.
   return false;
  }
