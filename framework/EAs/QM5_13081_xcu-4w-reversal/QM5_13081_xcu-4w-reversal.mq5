...
#property description "QM5_13081 XCU 4-Week Commodity Reversal"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_13081 - XCU 4-Week Commodity Reversal
// -----------------------------------------------------------------------------
// D1 structural copper sleeve:
//   - evaluates one weekly overreaction setup on XCUUSD.DWX
//   - fades fixed 20-D1-bar copper return extremes
//   - exits by max-hold guard or ATR stop
// Runtime uses MT5 OHLC only; no futures curve, API, CSV, or ML.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 13081;
...
input double strategy_min_abs_return_pct   = 4.0;
...
bool Strategy_IsXcuD1()
  {
   return (_Symbol == "XCUUSD.DWX" && _Period == PERIOD_D1);
  }
...
bool Strategy_LoadReversalState(double &close_last,
                                ...
                                int &signal_week_key)
  {
   const datetime current_bar_time = iTime(_Symbol, PERIOD_D1, 0); // perf-allowed: D1 weekly calendar gate behind framework new-bar.
   ...
   close_lookback = iClose(_Symbol, PERIOD_D1, 1 + strategy_lookback_days); // perf-allowed: fixed D1 lookback close.
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_13081_XCU_4W_REVERSAL";
   ...
   req.reason = (reversal_direction > 0) ? "XCU_4W_REVERSAL_LONG" : "XCU_4W_REVERSAL_SHORT";
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_13081\",\"ea\":\"xcu-4w-reversal\"}");
