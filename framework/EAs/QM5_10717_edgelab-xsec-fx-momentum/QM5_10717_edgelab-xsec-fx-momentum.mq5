...
#property description "QM5_10717 Edge Lab cross-sectional FX momentum basket"
...
input int    strategy_vol_rank_days      = 252;
input double strategy_vol_top_decile     = 0.90;
...
string PairRoot(const string symbol)
  {
...
string BaseCurrency(const string symbol)
  {
   return StringSubstr(PairRoot(symbol), 0, 3);
  }

string QuoteCurrency(const string symbol)
  {
   return StringSubstr(PairRoot(symbol), 3, 3);
  }
...
double PairReturn(const string symbol, const int lookback_days)
  {
   const int bars = MathMax(2, lookback_days);
   const double now_close = iClose(symbol, PERIOD_D1, 1);
   const double then_close = iClose(symbol, PERIOD_D1, bars + 1);
   if(now_close <= 0.0 || then_close <= 0.0)
      return 0.0;
   return (now_close / then_close) - 1.0;
  }

bool CurrencyStrength(const string ccy, double &score)
  {
   score = 0.0;
   int samples = 0;
   for(int i = 0; i < FX8_SYMBOL_COUNT; ++i)
     {
      ...
      if(base != ccy && quote != ccy)
         continue;
      const double r = PairReturn(g_symbols[i], strategy_lookback_days);
      score += (base == ccy) ? r : -r;
      samples++;
     }
   if(samples < 4)
...
bool MomentumCrashGuardRed()
  {
   ...
   const int window = MathMax(60, strategy_vol_rank_days);
   ...
   if(samples < 40)
      return true;
   const double percentile = (double)below_or_equal / (double)samples;
   return (percentile >= strategy_vol_top_decile);
  }
...
bool RankCurrencies(CurrencyRank &ranks[])
  {
   ...
   for(int i = 0; i < FX8_CCY_COUNT; ++i)
     {
      ranks[i].ccy = g_ccy[i];
      if(!CurrencyStrength(g_ccy[i], ranks[i].score))
         return false;
     }
...
bool OpenCurrencyPair(const string long_ccy, const string short_ccy)
  {
   ...
   req.tp = 0.0;
   req.lots = 0.0;
   req.reason = "QM5_10717_weekly_xsec_momentum";
   req.symbol_slot = idx;
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"ea\":\"QM5_10717\",\"logical_symbol\":\"FX8_BASKET_D1\"}");
...
void OnTick()
  {
   ...
   if(!QM_NewsAllowsTrade(_Symbol, TimeCurrent(), qm_news_mode))
      return;
   if(!IsWeeklyRebalanceBar())
      return;

   CloseBasketPositions();
   if(MomentumCrashGuardRed())
      return;
