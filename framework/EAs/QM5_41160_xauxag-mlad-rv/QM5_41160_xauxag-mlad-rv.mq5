...
#property description "QM5_41160 XAU/XAG Thirteen-Month LAD Ratio Reversion"
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41160 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_MLAD_RV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41160_XAUXAG_MLAD_RV";
...
bool Strategy_LoadMonthlyLad(
      ...
      int &direction)
  {
   ...
   // direction > 0 means BUY XAU / SELL XAG. Fade only the strict LAD
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41160_XAUXAG_MLAD_RV_HOST";
   ...
   if(valid && direction > 0)
      state = "negative_lad_fade_long_xau";
   else if(valid && direction < 0)
      state = "positive_lad_fade_short_xau";
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG thirteen-month LAD ratio reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41160\",\"ea\":\"xauxag-mlad-rv\"}");
