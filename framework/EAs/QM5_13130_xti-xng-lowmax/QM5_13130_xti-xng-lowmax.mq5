...
#property description "QM5_13130 XTI XNG Post-Financialization Low-MAX Rank"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_13130 - XTI/XNG Post-Financialization Low-MAX Rank
// -----------------------------------------------------------------------------
// Monthly market-neutral energy basket:
//   - calculate each leg's MAX measure as the mean of its five largest simple
//     daily returns over the latest 252 completed D1 observations
//   - long the lower-MAX energy leg and short the higher-MAX leg
// Runtime is Darwinex-native D1 OHLC only; no external or futures-chain data.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 13130;
...
double   g_cache_xti_max = 0.0;
double   g_cache_xng_max = 0.0;
...
bool Strategy_LoadSignalState(const datetime decision_bar_time,
                              int &pair_direction)
  {
   ...
   if(decision_bar_time <= 0)
      return false;
   if(!Strategy_MaxMeasure(g_leg_xti,
                           g_cache_xti_max,
                           g_cache_xti_observations))
      return false;
   if(!Strategy_MaxMeasure(g_leg_xng,
                           g_cache_xng_max,
                           g_cache_xng_observations))
      return false;

   g_cache_max_difference = g_cache_xti_max - g_cache_xng_max;
   if(!MathIsValidNumber(g_cache_max_difference))
      return false;
   if(g_cache_max_difference < -1.0e-12)
      pair_direction = 1;
   else if(g_cache_max_difference > 1.0e-12)
      pair_direction = -1;
   return true;
  }
...
bool Strategy_OpenPair(const int pair_direction)
  {
   ...
   const string reason = long_xti_short_xng ? "QM5_13130_LONG_XTI_SHORT_XNG_LOW_MAX"
                                            : "QM5_13130_SHORT_XTI_LONG_XNG_LOW_MAX";
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart())
      return true;
   if(strategy_lookback_d1 != 252 || strategy_top_return_count != 5)
      return true;
   if(strategy_history_bars < strategy_lookback_d1 + 1 ||
      strategy_history_bars > 1000 ||
      strategy_top_return_count <= 0 ||
      strategy_top_return_count > strategy_lookback_d1)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_13130_XTI_XNG_LOWMAX_HOST";
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_13130\",\"ea\":\"xti-xng-lowmax\"}");
