# QM5_13130_xti-xng-lowmax - Strategy Spec

**EA ID:** QM5_13130
**Slug:** `xti-xng-lowmax`
**Strategy ID:** `HOLLSTEIN-MAX-2021_XTI_XNG_S01`
...
XNG, computes exactly 252 simple daily returns, and calculates each leg's MAX
measure as the arithmetic mean of its five largest returns.

It buys the lower-MAX leg and shorts the higher-MAX leg. The fixed package
risk is split equally, each leg receives a frozen `ATR(20) * 3.5` hard stop,
and the package closes at the next month transition, after 35 days, or
...
| `strategy_top_return_count` | 5 | locked | largest returns averaged into MAX |
...
The 252-return window, top-five arithmetic mean, low-versus-high direction,
monthly renewal, equal half-risk carrier, and no same-month re-entry are locked.

## 3. Symbol Universe

- Logical symbol: `QM5_13130_XTI_XNG_LOWMAX_D1`.
- Host/traded slot 0: `XTIUSD.DWX`, D1, magic `131300000`.
- Traded slot 1: `XNGUSD.DWX`, D1, magic `131300001`.
...
- The carrier is opposite-side and equal-risk, not guaranteed beta or dollar
  neutral.
- XNG gaps, legging, weak source robustness, and the narrow rank make risk high.
...
The paper ranks 26 collateralized commodity futures. Its full-sample MAX spread
and full-sample two-portfolio split are null; only the December 2000-December
2015 subperiod supports a negative high-minus-low relation. The EA narrows the
test to two continuous CFDs and uses 252 simple returns instead of daily futures
excess returns over calendar months. Q02 must reject any failure of that modern
effect; no source performance number is imported.
...
- **Entry:** prior-252-return top-five MAX calculation and rank, paired orders,
