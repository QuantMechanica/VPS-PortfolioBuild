...
#property description "QM5_41314 WTI Monthly Jarque-Bera Trend"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41314 - WTI Monthly Jarque-Bera Trend
// -----------------------------------------------------------------------------
// D1 structural crude-oil sleeve:
//   - reconstruct forty-nine consecutive completed broker-month-end closes
//   - form forty-eight adjacent monthly log returns and biased moments
//   - combine squared skewness and squared Fisher excess kurtosis
//   - qualify the finite-sample Jarque-Bera statistic at or above 1.04
...
input double strategy_variance_floor       = 0.000000000000000001;
input double strategy_jb_boundary          = 1.04;
...
bool Strategy_JarqueBeraCore(const double &values[],
                             double &return_mean,
                             double &moment_2,
                             double &moment_3,
                             double &moment_4,
   ...
   for(int index = 0; index < value_count; ++index)
     {
      ...
      const double cube = square * centered;
      const double fourth = square * square;
      if(!MathIsValidNumber(centered) ||
         !MathIsValidNumber(square) || square < 0.0 ||
         !MathIsValidNumber(cube) ||
         !MathIsValidNumber(fourth) || fourth < 0.0)
         return false;
      moment_2 += square;
      moment_3 += cube;
      moment_4 += fourth;
      if(!MathIsValidNumber(moment_2) ||
         !MathIsValidNumber(moment_3) ||
         !MathIsValidNumber(moment_4))
         return false;
     }

   const double count = (double)value_count;
   moment_2 /= count;
   moment_3 /= count;
   moment_4 /= count;
   if(!MathIsValidNumber(moment_2) ||
      !MathIsValidNumber(moment_3) ||
      !MathIsValidNumber(moment_4) ||
      moment_2 <= strategy_variance_floor ||
      moment_4 < 0.0)
      return false;

   const double skew_denominator = moment_2 * MathSqrt(moment_2);
   const double kurtosis_denominator = moment_2 * moment_2;
   if(skew_denominator <= 0.0 || kurtosis_denominator <= 0.0 ||
      !MathIsValidNumber(skew_denominator) ||
      !MathIsValidNumber(kurtosis_denominator))
      return false;

   skewness = moment_3 / skew_denominator;
   excess_kurtosis = moment_4 / kurtosis_denominator - 3.0;
   const double skew_component = skewness * skewness;
   const double excess_component =
      excess_kurtosis * excess_kurtosis / 4.0;
   jarque_bera = count / 6.0 *
      (skew_component + excess_component);
   return (MathIsValidNumber(skewness) &&
           MathIsValidNumber(excess_kurtosis) &&
           MathIsValidNumber(skew_component) && skew_component >= 0.0 &&
           MathIsValidNumber(excess_component) && excess_component >= 0.0 &&
           MathIsValidNumber(jarque_bera) && jarque_bera >= 0.0);
  }

bool Strategy_JarqueBeraReferenceSelfTest()
  {
   // Pinned SciPy TestJarqueBera R-compatible fixture.
   double fixture[5] =
     {
   ...
   double constant_path[5] = {0.01, 0.01, 0.01, 0.01, 0.01};
   ...
   if(!Strategy_JarqueBeraCore(fixture,
                              return_mean,
                              moment_2,
                              moment_3,
                              moment_4,
                              ...
                              excess_kurtosis,
                              jarque_bera) ||
      MathAbs(return_mean - (-0.3758955135266857)) > 0.000000000001 ||
      MathAbs(moment_2 - 1.0997847585017662) > 0.000000000001 ||
      MathAbs(moment_3 - (-0.29313419144364505)) > 0.000000000001 ||
      MathAbs(moment_4 - 2.700398537026828) > 0.000000000001 ||
      MathAbs(skewness - (-0.2541586721782321)) > 0.000000000001 ||
      MathAbs(excess_kurtosis - (-0.767392030299042)) > 0.000000000001 ||
      MathAbs(jarque_bera - 0.17651605223752) > 0.000000000001)
      return false;

   if(Strategy_JarqueBeraCore(constant_path,
                              return_mean,
                              moment_2,
                              moment_3,
                              moment_4,
...
bool Strategy_JarqueBeraSignal(const double &closes[],
                               Strategy_SignalMetrics &metrics)
  {
   ...
   metrics.direction = 0;
   metrics.shape_qualified = false;

   if(strategy_month_returns != 48 ||
      strategy_variance_floor != 0.000000000000000001 ||
      MathAbs(strategy_jb_boundary - 1.04) >
         0.000000000000001 ||
   ...
   if(!Strategy_JarqueBeraCore(returns,
                              metrics.return_mean,
                              metrics.moment_2,
                              metrics.moment_3,
                              metrics.moment_4,
                                        ...
                                        int &direction)
  {
   ...
   const bool valid =
      Strategy_LoadMonthlyEndpoints(current_month_key,
                                    ...
                                    closes,
                                    endpoint_times) &&
      Strategy_JarqueBeraSignal(closes, metrics);
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
   ...
   else
     {
      ...
      g_signal_valid =
         endpoints_valid &&
         Strategy_JarqueBeraSignal(closes, g_signal_metrics);
      if(!g_signal_valid)
         g_signal_state = "jarque_bera_validation_failed";
      else if(!g_signal_metrics.shape_qualified)
         g_signal_state = "jarque_bera_below_boundary_consumed_flat";
      else if(g_signal_metrics.direction == 0)
         g_signal_state = "qualified_neutral_momentum_consumed_flat";
      else if(g_signal_metrics.direction > 0)
         g_signal_state = "high_shape_positive_momentum_wti_long";
      else if(g_signal_metrics.direction < 0)
         g_signal_state = "high_shape_negative_momentum_wti_short";
      else
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"endpoint_count\":%d,\"return_count\":%d,\"return_mean\":%.12f,\"m2\":%.16f,\"m3\":%.16f,\"m4\":%.16f,\"skewness\":%.12f,\"excess_kurtosis\":%.12f,\"jarque_bera\":%.12f,\"shape_qualified\":%s,\"momentum_12\":%.12f,\"returns\":\"%s\",\"oldest_close\":%.10f,\"newest_close\":%.10f,\"oldest_endpoint\":%I64d,\"newest_endpoint\":%I64d,\"state\":\"%s\"}",
                            ...
                            g_signal_metrics.moment_2,
                            g_signal_metrics.moment_3,
                            g_signal_metrics.moment_4,
                            g_signal_metrics.skewness,
                            g_signal_metrics.excess_kurtosis,
                            g_signal_metrics.jarque_bera,
                            g_signal_metrics.shape_qualified ? "true" : "false",
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41314 ||
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41314 ||
      qm_magic_slot_offset != 0)
      return INIT_PARAMETERS_INCORRECT;
   if(!Strategy_JarqueBeraReferenceSelfTest())
      return INIT_FAILED;
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI monthly Jarque-Bera trend holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41314_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   g_entry_month_state_key =
      StringFormat("QM5_41314_ENTRY_MONTH_%d", QM_FrameworkMagic());
