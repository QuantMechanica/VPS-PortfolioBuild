...
#property description "QM5_13010 XTI CADJPY Return Spread Reversion"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_13010 - WTI/CADJPY Return-Spread Reversion
// -----------------------------------------------------------------------------
// D1 two-leg energy/FX basket:
//   rspread = log(XTI[t] / XTI[t-L]) - beta_cadjpy * log(CADJPY[t] / CADJPY[t-L])
//   z > entry: short return spread = sell WTI, buy CADJPY
//   z < -entry: long return spread = buy WTI, sell CADJPY
   ...
   for(int i = 0; i < z_lookback; ++i)
     {
      ...
      if(xti[i] <= 0.0 || cadjpy[i] <= 0.0 || xti[past_idx] <= 0.0 || cadjpy[past_idx] <= 0.0)
         return false;

      const double xti_ret = MathLog(xti[i] / xti[past_idx]);
      const double cadjpy_ret = MathLog(cadjpy[i] / cadjpy[past_idx]);
      spreads[i] = xti_ret - strategy_beta_cadjpy * cadjpy_ret;
...
bool Strategy_OpenPair(const int spread_direction)
  {
   if(spread_direction == 0 || Strategy_OpenPairLegCount() > 0)
      return false;
   if(!Strategy_SpreadAllowed(g_leg_xti) || !Strategy_SpreadAllowed(g_leg_cadjpy))
      return false;

   const double xti_weight = 1.0;
   const double cadjpy_weight = MathMax(0.1, MathAbs(strategy_beta_cadjpy));
   const double weight_sum = xti_weight + cadjpy_weight;
   ...
   const string reason = long_spread ? "QM5_13010_LONG_XTI_CADJPY_RSPREAD"
                                     : "QM5_13010_SHORT_XTI_CADJPY_RSPREAD";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_13010_RSPREAD_HOST";
...
int OnInit()
  {
   ...
   string basket_symbols[2] = {g_leg_xti, g_leg_cadjpy};
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_13010\",\"ea\":\"xti-cadjpy-rspr\"}");
