...
#property description "QM5_1111 Quantpedia FX Momentum 12M"

#include <QM/QM_Common.mqh>

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 1111;
...
bool Strategy_SymbolMomentumScore(const string symbol, double &out_score)
  {
   out_score = 0.0;
   if(strategy_lookback_d1_bars <= 0 || strategy_min_d1_bars <= strategy_lookback_d1_bars)
      return false;
   ...
   const double lookback_close = iClose(symbol, PERIOD_D1, strategy_lookback_d1_bars + 1);
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(Strategy_CurrentSymbolIndex() < 0)
      return true;
   if(strategy_bucket_size <= 0 || strategy_atr_period <= 0 || strategy_atr_sl_mult <= 0.0)
      return true;
   return false;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.reason = (direction > 0) ? "QP_FX_MOM_12M_TOP3_LONG" : "QP_FX_MOM_12M_BOTTOM3_SHORT";
   if(req.sl <= 0.0)
      return false;
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_1111\",\"ea\":\"qp-fx-momentum-12m\"}");
