...
#property description "QM5_11768 connors-cumrsi2-200sma-d1 — Connors Cumulative RSI(2) + SMA200 trend filter (D1)"

#include <QM/QM_Common.mqh>

// =============================================================================
// QuantMechanica V5 EA — QM5_11768 connors-cumrsi2-200sma-d1
// -----------------------------------------------------------------------------
// Source: Larry Connors & Cesar Alvarez, "Short-Term Trading Strategies That
//         Work" (2009), "Cumulative RSI Strategy".
//         source_id ef7afc8e-406f-5f05-90d5-8258a7fb7123.
// Card: artifacts/cards_approved/QM5_11768_connors-cumrsi2-200sma-d1.md
//       (g0_status APPROVED). Sibling realization: QM5_11498.
//
// Mechanics (D1, all reads on CLOSED bars at shift >= 1):
//   Trend STATE   : close@1 > SMA(200) -> macro uptrend (mirror for downtrend).
//                   SMA200 is the trend STATE, NOT a per-bar trigger.
//   Cumulative RSI: cumRSI = sum of RSI(rsi_period=2) over the last
//                   cum_window=2 closed bars (card: "sum the last 2 days'
//                   RSI(2) readings"):
//                     cumRSI@1 = RSI@1 + RSI@2 ; cumRSI@2 = RSI@2 + RSI@3.
//   Entry EVENT   : the cumulative RSI crosses INTO the extreme between the
//                   prior closed bar and the last closed bar — a single trigger
//                   EVENT (card NOTE: "the cumulative-RSI threshold crossing =
//                   single trigger EVENT"):
//                     LONG  -> cumRSI@1 < entry_cum_long  AND cumRSI@2 >= entry_cum_long.
//                     SHORT -> cumRSI@1 > entry_cum_short AND cumRSI@2 <= entry_cum_short.
//                   Modelling it as a CROSS (not a bare level test) avoids the
//                   two-cross-same-bar trap and stops it re-firing on every bar
//                   the cumulative sum stays extreme. Card thresholds:
//                     long  cumRSI < 45  (RSI2@1 + RSI2@2 < 45)
//                     short cumRSI > 155 (= 200 - 45, symmetric on the 2-day sum)
//   Stop          : entry -/+ atr_stop_mult * ATR(14)  (card: 2 x ATR(14)).
//                   P2 cap: skip the trade if the ATR stop distance exceeds
//                   max_stop_pips.
//   Take profit   : card hard cap = 4 x ATR(14). No discretionary TP; the
//                   RSI(2) revert is the primary exit, the 4xATR is a cap.
//   Exit          : single-bar RSI(2) reverts past the exit level
//                   (LONG rsi@1 > exit_long=65, SHORT rsi@1 < exit_short=35), OR
//                   the macro trend breaks, OR max-hold bars elapsed. The
//                   protective ATR stop / 4xATR TP are the disaster brakes.
...
input int    strategy_atr_period           = 14;    // ATR for the protective stop / TP cap
input double strategy_entry_cum_long       = 45.0;  // cumRSI < this -> oversold (long)
input double strategy_entry_cum_short      = 155.0; // cumRSI > this -> overbought (short); = 200 - 45
input double strategy_exit_rsi_long        = 65.0;  // RSI(2) > this -> exit long (Connors)
input double strategy_exit_rsi_short       = 35.0;  // RSI(2) < this -> exit short (= 100 - 65)
input double strategy_atr_stop_mult        = 2.0;   // stop distance = mult * ATR(14)
input double strategy_atr_tp_mult          = 4.0;   // card hard-cap TP = mult * ATR(14)
...
input bool   strategy_no_friday_entry      = true;  // no Friday entries
   ...
   if(long_trend && long_event)
     {
      ...
      req.sl     = sl;
      req.tp     = tp;    // 4xATR hard cap; RSI(2) revert is the primary exit
...
// No active trade management beyond the fixed ATR protective stop / 4xATR TP.
// The rule exits (RSI revert / trend break / max-hold) live in Strategy_ExitSignal.
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"ea\":\"QM5_11768_connors-cumrsi2-200sma-d1\"}");
