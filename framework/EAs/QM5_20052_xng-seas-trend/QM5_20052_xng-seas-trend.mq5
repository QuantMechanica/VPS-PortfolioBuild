...
#property description "QM5_20052 XNG Seasonal-Window Trend"
...
//   - eligible only in the source volatility seasons (May-Sep, Nov-Jan)
//   - direction = sign of prior 6-month log return
//   - monthly package exits at rebalance, season end, or stale-position guard
// Runtime uses MT5 OHLC/broker calendar only; no curve, inventory, API, or ML.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 20052;
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      bool should_close = monthly_rebalance || !Strategy_IsSourceVolMonth(now);
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20052_XNG_SEAS_TREND";
   ...
   if(!Strategy_IsMonthlyRebalanceBar())
      return false;
   if(!Strategy_IsSourceVolMonth(TimeCurrent()))
      return false;
   ...
   req.reason = (direction > 0) ? "XNG_SEAS_TREND_LONG" : "XNG_SEAS_TREND_SHORT";
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_20052\",\"ea\":\"xng-seas-trend\"}");
