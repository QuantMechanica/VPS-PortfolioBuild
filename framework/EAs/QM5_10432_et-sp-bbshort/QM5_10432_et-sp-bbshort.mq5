...
#property description "QM5_10432 Elite Trader S&P Bollinger Short"
...
input int             strategy_min_history_bars = 230;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   const double upper_1 = QM_BB_Upper(_Symbol, strategy_timeframe,
                                      strategy_bb_period, strategy_bb_deviation, 1);
   const double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   const double point = SymbolInfoDouble(_Symbol, SYMBOL_POINT);
   if(close_1 <= 0.0 || upper_1 <= 0.0 || bid <= 0.0 || point <= 0.0)
      return false;
   ...
   req.reason = "ET_SP_BBSHORT_CLOSE_ABOVE_UPPER";
...
bool Strategy_ExitSignal()
  {
   if(strategy_bb_period <= 1 || strategy_bb_deviation <= 0.0 || strategy_hold_bars <= 0)
      return false;
   ...
   const double close_1 = iClose(_Symbol, strategy_timeframe, 1);
   const double middle_1 = QM_BB_Middle(_Symbol, strategy_timeframe,
                                        strategy_bb_period, strategy_bb_deviation, 1);
   if(close_1 <= 0.0 || middle_1 <= 0.0)
      return false;
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      if(open_shift >= strategy_hold_bars)
         return true;
     }
