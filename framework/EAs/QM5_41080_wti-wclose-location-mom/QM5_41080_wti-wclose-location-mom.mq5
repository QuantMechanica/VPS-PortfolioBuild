...
#property description "QM5_41080 WTI Completed-Week Close-Location Momentum"
...
//   - compute the parent-close to newest-close strict return sign
//   - confirm that sign with the newest close in its own outer range fifth
//   - consume the week before fallible gates; never enter late or retry
//   - follow the confirmed completed-week direction for one broker week
// Runtime uses MT5-native OHLC, calendar, history, and framework state only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                      = 41080;
...
input double strategy_clv_upper             = 0.80;
input double strategy_clv_lower             = 0.20;
   ...
   if(Strategy_WeekAlreadyEntered(g_decision_week_key))
   ...
   else
     {
      ...
      if(!g_signal_valid)
         ...
         g_signal_state = "positive_return_upper_fifth_long";
      else if(g_signal_direction < 0)
         g_signal_state = "negative_return_lower_fifth_short";
      else if(g_weekly_return == 0.0)
         g_signal_state = "return_zero_flat";
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41080 ||
   ...
   if(strategy_entry_grace_minutes != 180 ||
      ...
      MathAbs(strategy_clv_upper - 0.80) > 1.0e-12 ||
      MathAbs(strategy_clv_lower - 0.20) > 1.0e-12 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41080_WTI_WCLOSE_LOCATION_MOM";
   ...
   req.reason = (g_signal_direction > 0)
      ? "WTI_WCLOSE_LOCATION_MOM_LONG"
      : "WTI_WCLOSE_LOCATION_MOM_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_strategy_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41080 ||
   ...
   g_attempt_state_key =
      StringFormat("QM5_41080_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41080\",\"ea\":\"wti-wclose-location-mom\"}");
