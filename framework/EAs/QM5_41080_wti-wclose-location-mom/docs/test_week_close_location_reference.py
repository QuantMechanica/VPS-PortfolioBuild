"""Deterministic reference checks for QM5_41080 weekly close location."""
...
def close_location_signal(
    ...
    upper: float = 0.80,
    lower: float = 0.20,
) -> tuple[bool, int, tuple[int, int], tuple[float, ...], float, float]:
    """Mirror the bounded two-week OHLC aggregation and strict signal."""

    empty = (False, 0, (0, 0), (0.0,) * 8, 0.0, 0.0)
    ...
    _, _, _, parent_close = weekly[1]
    new_range = new_high - new_low
    if new_range <= 0.0 or not math.isfinite(new_range):
        return empty
    weekly_return = math.log(new_close / parent_close)
    clv = (new_close - new_low) / new_range
    if (
        not math.isfinite(weekly_return)
        or not math.isfinite(clv)
        or not 0.0 <= clv <= 1.0
    ):
        return empty

    direction = 0
    if weekly_return > 0.0 and clv > upper:
        direction = 1
    elif weekly_return < 0.0 and clv < lower:
        direction = -1
...
def sample(
    new: tuple[float, float, float, float] = (100.0, 120.0, 90.0, 118.0),
...
class WeekCloseLocationReferenceTest(unittest.TestCase):
    def signal(self, bars: list[Bar], offset: timedelta = timedelta(0)):
        return close_location_signal(20260824, bars, offset)

    def test_strict_long_and_short_paths(self) -> None:
        _, _, bars = sample()
        valid, direction, counts, values, weekly_return, clv = self.signal(bars)
        self.assertTrue(valid)
        self.assertEqual(direction, 1)
        self.assertEqual(counts, (5, 5))
        self.assertEqual(values, (100.0, 120.0, 90.0, 118.0, 95.0, 110.0, 90.0, 100.0))
        self.assertGreater(weekly_return, 0.0)
        self.assertGreater(clv, 0.80)

        _, _, bars = sample(new=(100.0, 110.0, 80.0, 82.0))
        valid, direction, _, _, weekly_return, clv = self.signal(bars)
        self.assertTrue(valid)
        self.assertEqual(direction, -1)
        self.assertLess(weekly_return, 0.0)
        self.assertLess(clv, 0.20)
    ...
    def test_three_four_and_five_session_weeks_are_accepted(self) -> None:
        for new_count in (3, 4, 5):
            for parent_count in (3, 4, 5):
                ...
                valid, direction, counts, _, _, _ = self.signal(bars)
                self.assertTrue(valid)
                self.assertEqual(direction, 1)
    ...
    def test_strict_equalities_zero_and_disagreement_are_flat(self) -> None:
        cases = [
            ((100.0, 120.0, 80.0, 112.0), (95.0, 110.0, 90.0, 100.0)),
            ((100.0, 120.0, 80.0, 88.0), (95.0, 110.0, 90.0, 100.0)),
            ((100.0, 120.0, 80.0, 100.0), (95.0, 110.0, 90.0, 100.0)),
            ((100.0, 120.0, 90.0, 118.0), (95.0, 125.0, 90.0, 120.0)),
            ((100.0, 110.0, 80.0, 82.0), (75.0, 90.0, 70.0, 80.0)),
        ]
    ...
    def test_parent_close_is_the_only_parent_signal_value(self) -> None:
        _, _, bars_a = sample(parent=(95.0, 110.0, 90.0, 100.0))
        _, _, bars_b = sample(parent=(70.0, 150.0, 60.0, 100.0))
        result_a = self.signal(bars_a)
        result_b = self.signal(bars_b)
    ...
    def test_malformed_zero_range_nonconsecutive_and_current_week_rejected(self) -> None:
        ...
        _, _, bars = sample()
        bars.insert(0, Bar(datetime(2026, 8, 24, tzinfo=UTC), 118.0, 119.0, 117.0, 118.5))
        self.assertFalse(self.signal(bars)[0])
    ...
    def test_static_build_contract_is_fixed_risk_and_completed_data_only(self) -> None:
        source = (EA_DIR / "QM5_41080_wti-wclose-location-mom.mq5").read_text(encoding="utf-8")
        setfile = (EA_DIR / "sets" / "QM5_41080_wti-wclose-location-mom_XTIUSD.DWX_D1_backtest.set").read_text(encoding="utf-8")
        for marker in (
        ...
        self.assertLess(source.index("Strategy_RecordWeekAttempt(g_decision_week_key)"), source.index("Strategy_LoadCloseLocationSignal(g_decision_week_key"))
        for marker in (
            "qm_ea_id=41080",
            "RISK_FIXED=1000",
            "RISK_PERCENT=0",
            ...
            "strategy_clv_upper=0.80",
            "strategy_clv_lower=0.20",
        ):
