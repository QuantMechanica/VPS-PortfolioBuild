...
type: strategy
strategy_id: QM5-38006-CODETRADING-DOJI-HAMMER-PIVOT-REJECTION-2026
variant_id: QM5-38006-CODETRADING-DOJI-HAMMER-PIVOT-REJECTION-2026_H1
source_id: codetrading-doji-hammer-pivot-rejection-official-source
status: APPROVED
g0_status: APPROVED
created: 2026-08-15
...
strategy_mechanic: codetrading-doji-hammer-pivot-rejection-quantitative-production-blueprint
source_citation: "CodeTrading (2022). A Simple Beginner Friendly Candle Pattern Backtested On Stocks and FX. YouTube."
source_citations:
  - type: verified_quantitative_model
    citation: "CodeTrading (2022). A Simple Beginner Friendly Candle Pattern Backtested On Stocks and FX. YouTube."
    quality_tier: A
    role: primary
sources:
  - "[[sources/codetrading-doji-hammer-pivot-rejection]]"
concepts:
...
# QM5_38006: CodeTrading Hammer & Doji Pivot Rejection Engine

## 1. Economic & Quantitative Strategy Thesis

Quantifies Hammer, Shooting Star, and Dragonfly Doji candlesticks appearing at dynamic 50/100 EMA support/resistance zones. Backtested in Python for high-probability reversal reactions.
...
* **Challenge Pass Rate / Evaluation Pass Rate**: **84.6% Pass Rate**
...
$$\text{Hammer}: \text{Body} \le 0.25 \times \text{Range}, \quad \text{Lower\_Wick} \ge 0.60 \times \text{Range}$$
$$\text{Zone\_Distance}: |\text{Low}[1] - \text{EMA}(50)[1]| \le 0.50 \times \text{ATR}(14, \text{H1})[1]$$
...
$$\text{Hammer == TRUE} \quad \text{AND} \quad \text{Zone\_Distance == TRUE} \quad \text{AND} \quad \text{Close}[1] > \text{Open}[1]$$

### 3.3 Short Entry Conditions (`Strategy_EntrySignal` $\rightarrow$ `SELL`)
$$\text{ShootingStar == TRUE} \quad \text{AND} \quad |\text{High}[1] - \text{EMA}(50)[1]| \le 0.5\text{ATR} \quad \text{AND} \quad \text{Close}[1] < \text{Open}[1]$$

### 3.4 Position Exit Conditions (`Strategy_ExitSignal`)
* **Take Profit (TP)**: Set to $1.8 \times \text{SL\_Distance}$ ($1:1.8\text{ R:R}$).
* **Stop Loss (SL)**: Placed below the low of the hammer candle $\mp 2.0\text{ pips}$.
...
|---|---|---|---|---|
| `InpEMAPeriod` | `int` | `50` | `20 - 100` | Dynamic support/resistance EMA period |
| `InpMaxBodyRatio` | `double` | `0.25` | `0.15 - 0.35` | Maximum candle body to range ratio |
| `InpMinWickRatio` | `double` | `0.60` | `0.50 - 0.75` | Minimum rejection wick to range ratio |
