# QM5_20298_wti-vov-regime — Strategy Spec

**EA ID:** QM5_20298  
**Slug:** `wti-vov-regime`  
**Source:** `HOLLSTEIN-WTI-VOV-REGIME-2026`  
...
attempt and load exactly 543 completed `XTIUSD.DWX` D1 closes. Calculate two
...
| Drawdown profile | Sparse fixed-risk WTI uncertainty-regime losses with gap, roll, and persistent-state exposure |
| State | Long when recent VoV is lower; short when recent VoV is higher |

WTI exposure and the uncertainty-of-risk state are diversification hypotheses.
Q09 alone owns any realized portfolio-overlap conclusion.
...
commodity relation, renews monthly, and includes WTI. The EA substitutes a
nested realized-volatility proxy and an own-history comparison. Those are
locked QM hypotheses, not source-tested WTI rules. See the bounded packet and
approved card.
...
This EA compares two disjoint WTI history blocks, owns one magic, and has no
shared risk or orphan leg. `QM5_13046` uses raw realized-volatility level to
gate a stretch fade; `QM5_20295` uses Pearson kurtosis; `QM5_12567` is a
short-horizon long-only RSI pullback. Trend, calendar, event, variance-ratio,
breakout, and ordinary reversal builds use other state objects.
...
PASS. The registered one-slot EA implements the exact nested estimator,
...
six independent denominator, direction, support, invariance, chronology,
count, and freshness tests passed; and P1 artifact validation found the
compiled `.ex5`.
