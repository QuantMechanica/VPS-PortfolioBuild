...
#property description "QM5_20064 WTI 2M Time Series Momentum"

#include <QM/QM_Common.mqh>

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 20064;
...
input int    strategy_momentum_lookback_d1 = 42;
...
bool Strategy_IsHost()
  {
   return (_Symbol == "XTIUSD.DWX" && _Period == PERIOD_D1 &&
           qm_ea_id == 20064 && qm_magic_slot_offset == 0);
  }
...
bool Strategy_IsMonthlyRebalanceBar()
  {
   ...
   return (current_key > 0 && prior_key > 0 && current_key != prior_key);
  }
...
bool Strategy_LoadMomentum(double &momentum, int &direction)
  {
   ...
   const int copied = CopyClose(_Symbol, PERIOD_D1, 1, lookback + 1, closes); // perf-allowed: bounded monthly decision sample.
   if(copied < lookback + 1 || closes[0] <= 0.0 || closes[lookback] <= 0.0)
      return false;
   momentum = MathLog(closes[0] / closes[lookback]);
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      if(monthly_rebalance || (opened > 0 && now - opened >= hold_seconds))
...
bool Strategy_NoTradeFilter()
  {
   return (!Strategy_IsHost() || strategy_momentum_lookback_d1 != 42 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20064_WTI_TSMOM2M";
   ...
   if(month_key <= 0 || month_key == g_last_entry_month_key ||
      ...
      return false;
   if(SymbolInfoInteger(_Symbol, SYMBOL_SPREAD) > strategy_max_spread_points)
      return false;

   double momentum = 0.0;
   int direction = 0;
   if(!Strategy_LoadMomentum(momentum, direction) || direction == 0)
      return false;
   ...
   if(entry_price <= 0.0)
      return false;
   req.sl = QM_StopATRFromValue(_Symbol, req.type, entry_price, atr_last,
                                strategy_atr_sl_mult);
   if(req.sl <= 0.0)
      return false;
   req.reason = (direction > 0) ? "WTI_TSMOM2M_LONG" : "WTI_TSMOM2M_SHORT";
