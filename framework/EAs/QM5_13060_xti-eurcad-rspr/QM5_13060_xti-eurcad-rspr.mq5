...
#property description "QM5_13060 XTI EURCAD Return Spread Reversion"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_13060 - WTI/EURCAD Return-Spread Reversion
// -----------------------------------------------------------------------------
   ...
   for(int i = 0; i < z_lookback; ++i)
     {
      ...
      if(xti[i] <= 0.0 || eurcad[i] <= 0.0 || xti[past_idx] <= 0.0 || eurcad[past_idx] <= 0.0)
         return false;

      const double xti_ret = MathLog(xti[i] / xti[past_idx]);
      const double eurcad_ret = MathLog(eurcad[i] / eurcad[past_idx]);
      spreads[i] = xti_ret + strategy_beta_eurcad * eurcad_ret;
...
bool Strategy_OpenPair(const int spread_direction)
  {
   if(spread_direction == 0 || Strategy_OpenPairLegCount() > 0)
      return false;
   if(!Strategy_SpreadAllowed(g_leg_xti) || !Strategy_SpreadAllowed(g_leg_eurcad))
      return false;

   const double xti_weight = 1.0;
   const double eurcad_weight = MathMax(0.1, MathAbs(strategy_beta_eurcad));
   const double weight_sum = xti_weight + eurcad_weight;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_13060_RSPREAD_HOST";
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_13060\",\"ea\":\"xti-eurcad-rspr\"}");
