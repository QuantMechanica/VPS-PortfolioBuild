...
# Approved Build-Time Card

The complete approved card of record is
`strategy-seeds/cards/xti-levbrk_card.md`. It defines a once-weekly-capped,
short-only `XTIUSD.DWX` H4 continuation below a large completed negative D1
impulse, with `RISK_FIXED=1000` backtest risk.

## Hypothesis

WTI exhibits a standard return/volatility leverage effect. A large completed
negative D1 candle is used only as a short-lived volatility regime; a later
completed H4 close below its low confirms downside continuation.
...
See the complete card of record. Expected density is 6-14 trades/year.

## Author Claims

The paper establishes a standard crude-oil leverage effect, not short-breakout
returns. No source performance claim is imported.
