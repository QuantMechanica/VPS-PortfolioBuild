...
#property description "QM5_41134 WTI Completed-Month Daily-Return Interquartile-Mean Momentum"
...
//   - sort every within-month daily return without rounding
//   - remove floor(n/4) observations from each tail
//   - follow the arithmetic mean of the exact retained central band
...
int      g_trim_each_tail          = 0;
int      g_retained_count          = 0;
double   g_retained_low            = 0.0;
double   g_retained_high           = 0.0;
double   g_central_sum             = 0.0;
double   g_central_mean            = 0.0;
...
bool Strategy_LoadDailyIqrMeanSignal(const int current_month_key,
                                     const int label_offset,
                                     int &direction,
                                     int &month_sessions,
                                     ...
                                     double &central_mean)
  {
   ...
   for(int index = 0; index < copied; ++index)
     {
      ...
      const int month_key = Strategy_MonthKeyForTime(normalized_label);
      const int date_key = Strategy_DateKeyForTime(normalized_label);
      MqlDateTime label_parts;
      ZeroMemory(label_parts);
      if(month_key <= 0 || date_key <= 0 ||
         month_key == current_month_key ||
         !TimeToStruct(normalized_label, label_parts) ||
         label_parts.day_of_week < 1 ||
         label_parts.day_of_week > 5)
         return false;
   ...
   for(int index = month_sessions - 1; index >= 0; --index)
     {
      ...
      if(!MathIsValidNumber(daily_return))
         return false;
      daily_returns[return_count] = daily_return;
      raw_sum += daily_return;
      if(!MathIsValidNumber(raw_sum))
         return false;
      ++return_count;
      previous_close = current_close;
     }
   if(return_count != month_sessions ||
      ArrayResize(daily_returns, return_count) != return_count)
      return false;
   ...
   const int first_retained = trim_each_tail;
   const int last_retained = return_count - trim_each_tail - 1;
   ...
   retained_high = daily_returns[last_retained];
   if(!MathIsValidNumber(retained_low) ||
      !MathIsValidNumber(retained_high) ||
      retained_low > retained_high)
      return false;

   int observed_retained = 0;
   for(int index = first_retained; index <= last_retained; ++index)
     {
      if(index < 0 || index >= ArraySize(daily_returns) ||
         !MathIsValidNumber(daily_returns[index]))
         return false;
      central_sum += daily_returns[index];
      if(!MathIsValidNumber(central_sum))
         return false;
      ++observed_retained;
     }
   if(observed_retained != retained_count)
      return false;
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadDailyIqrMeanSignal(g_decision_month_key,
                                          ...
                                          g_central_mean);
      if(!g_signal_valid)
         g_signal_state = "monthly_daily_iqrmean_validation_failed";
      else if(g_central_mean == 0.0)
         g_signal_state = "zero_daily_iqrmean_flat";
      else if(g_signal_direction > 0)
         g_signal_state = "daily_iqrmean_completed_month_long";
      else if(g_signal_direction < 0)
         g_signal_state = "daily_iqrmean_completed_month_short";
      else
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"month_sessions\":%d,\"trim_each_tail\":%d,\"retained_count\":%d,\"retained_low\":%.12e,\"retained_high\":%.12e,\"central_sum\":%.12e,\"central_mean\":%.12e,\"boundary_close\":%.8f,\"final_close\":%.8f,\"raw_sum\":%.12e,\"endpoint_return\":%.12e,\"raw_direction\":%d,\"endpoint_agrees\":%s,\"state\":\"%s\"}",
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41134 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41134_WTI_MDAILY_IQRMEAN_MOM";
   ...
   req.reason = (g_signal_direction > 0)
      ? "WTI_MDAILY_IQRMEAN_MOM_LONG"
      : "WTI_MDAILY_IQRMEAN_MOM_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41134 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI completed-month daily-return-interquartile-mean position holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41134_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41134\",\"ea\":\"wti-mdaily-iqrmean-mom\"}");
