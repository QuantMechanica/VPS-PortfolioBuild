...
#property description "QM5_41456 WTI Winter NR2 CLV Reversion"
...
//   - fade a contracting newest week only when its close is in an outer quartile
...
bool Strategy_LoadNR2CLVSignal(const int current_week_key,
                               const int label_offset,
                               int &direction)
  {
   direction = 0;
   ...
   if(g_strict_range_contraction && g_newer_week_clv > strategy_clv_upper)
      direction = -1;
   else if(g_strict_range_contraction && g_newer_week_clv < strategy_clv_lower)
      direction = 1;
   return true;
  }
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_WeekAlreadyEntered(g_decision_week_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadNR2CLVSignal(g_decision_week_key,
                                   g_decision_label_offset,
                                   g_signal_direction);
      if(!g_signal_valid)
         g_signal_state = "two_completed_weeks_validation_failed";
       else if(g_signal_direction > 0)
          g_signal_state = "nr2_lower_quartile_reversion_long";
       else if(g_signal_direction < 0)
          g_signal_state = "nr2_upper_quartile_reversion_short";
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"week\":%d,\"anchor_month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_week_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"week_bars\":[%d,%d],\"week_ranges\":[%.8f,%.8f],\"new_high\":%.8f,\"new_low\":%.8f,\"new_close\":%.8f,\"close_location\":%.12f,\"strict_range_contraction\":%s,\"state\":\"%s\"}",
                            g_decision_week_key,
                            anchor_month,
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41456_WTI_WINTER_NR2_CLV_FADE";
   ...
   req.reason = (g_signal_direction > 0
                 ? "WTI_WINTER_NR2_CLV_FADE_LONG"
                 : "WTI_WINTER_NR2_CLV_FADE_SHORT");
...
int OnInit()
  {
   if(!SymbolSelect(strategy_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41456 ||
      qm_magic_slot_offset != 0)
   ...
   g_attempt_state_key =
      StringFormat("QM5_41456_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41456\",\"ea\":\"wti-winter-nr2-clv-fade\"}");
