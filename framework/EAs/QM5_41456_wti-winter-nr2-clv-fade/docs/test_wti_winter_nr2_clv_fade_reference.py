"""Deterministic reference checks for QM5_41456 WTI winter NR2/CLV reversion."""
...
class WtiWinterNR2CLVFadeReferenceTest(unittest.TestCase):
    current = date(2026, 12, 14)
    new = current - timedelta(days=7)
    old = current - timedelta(days=14)

    def bars(self, close_location: float = 0.8, new_range: float = 8.0,
    ...
    def test_contraction_upper_quartile_is_short(self) -> None:
        self.assertEqual(signal(self.current, *self.bars(0.8))[0], -1)

    def test_contraction_lower_quartile_is_long(self) -> None:
        self.assertEqual(signal(self.current, *self.bars(0.2))[0], 1)
    ...
    def test_range_expansion_is_flat(self) -> None:
        self.assertEqual(signal(self.current, *self.bars(0.8, 20, 10))[0], 0)

    def test_upper_clv_equality_is_flat(self) -> None:
        self.assertEqual(signal(self.current, *self.bars(0.75)), (0, 0.75))

    def test_lower_clv_equality_is_flat(self) -> None:
        self.assertEqual(signal(self.current, *self.bars(0.25)), (0, 0.25))
    ...
    def test_three_to_five_sessions_are_valid(self) -> None:
        for count in (3, 4, 5):
            newest = package(self.new, count, 100, 90, 98)
            older = package(self.old, count, 110, 90, 100)
