...
#property description "QM5_41125 XAU/XAG Completed-Month Mean-to-RMS Coherence Reversion"
...
// Sum every relative return ending in that month into signed displacement N
// and squared path Q. When abs(N)/sqrt(n*Q) is at least 0.16, fade N with one
// opposite, equal-notional package held to the next month. Exact-zero returns
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41125 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
            ...
            MathAbs(strategy_coherence_threshold - 0.16) <= 1.0e-12 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_MRMS_COH_RV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41125_XAUXAG_MRMS_COH_RV";
...
bool Strategy_LoadMonthlyRmsCoherence(
      ...
      int &direction)
  {
   ...
   // The older synchronized pair is both the completeness proof and the left
   // endpoint for the first relative return ending inside the signal month.
   ...
   // CopyRates series order is newest to oldest. Place the older boundary at
   // index zero, then reverse the complete month so every return ending in the
   // month contributes exactly once.
   ...
   for(int i = 1; i <= completed_month_sessions; ++i)
     {
      ...
      // Exact-zero constituent returns are valid. Accumulate N and Q from the
      // same bounded loop without demeaning or sample-variance correction.
      net_displacement += relative_return;
      squared_path += relative_return * relative_return;
      if(!MathIsValidNumber(net_displacement) ||
         !MathIsValidNumber(squared_path))
         return false;
     }
   ...
   if(squared_path == 0.0)
      return (net_displacement == 0.0 && endpoint_displacement == 0.0);
   if(squared_path < 0.0)
      return false;
   ...
   if(denominator <= 0.0 || !MathIsValidNumber(denominator))
      return false;
   coherence = MathAbs(net_displacement) / denominator;
   if(!MathIsValidNumber(coherence) ||
      coherence < 0.0 ||
      coherence > 1.0 + strategy_numerical_tolerance)
      return false;
   if(coherence > 1.0)
      coherence = 1.0;

   // direction > 0 means BUY XAU / SELL XAG. Threshold equality qualifies;
   // the completed-month signed relative displacement is always faded.
   if(coherence >= strategy_coherence_threshold)
     {
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41125_XAUXAG_MRMS_COH_RV_HOST";
   ...
   double squared_path = 0.0;
   double endpoint_displacement = 0.0;
   double coherence = 0.0;
   const bool valid =
      Strategy_LoadMonthlyRmsCoherence(g_signal_month_key,
                                       completed_month_sessions,
                                       return_count,
                                       net_displacement,
                                       squared_path,
                                       ...
                                       coherence,
                                       direction);
   string state = "monthly_rms_coherence_failed";
   if(valid && direction > 0)
      state = "coherent_negative_net_fade_long_xau";
   else if(valid && direction < 0)
      state = "coherent_positive_net_fade_short_xau";
   else if(valid && return_count > 0 && squared_path == 0.0)
      state = "zero_squared_path_flat";
   else if(valid && net_displacement == 0.0)
      state = "net_zero_flat";
   else if(valid)
      state = "below_coherence_threshold_flat";

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"completed_month_sessions\":%d,\"return_count\":%d,\"net_displacement\":%.12e,\"squared_path\":%.12e,\"endpoint_displacement\":%.12e,\"coherence\":%.12e,\"threshold\":%.12e,\"direction\":%d,\"state\":\"%s\"}",
                            g_signal_month_key,
                            completed_month_sessions,
                            return_count,
                            net_displacement,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG completed-month mean-to-RMS coherence reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41125\",\"ea\":\"xauxag-mrms-coherence-rv\"}");
