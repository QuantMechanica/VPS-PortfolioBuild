...
#property description "QM5_10179 TradingView EMA Channel Risk Reward"
...
input double          strategy_max_spread_frac = 0.15;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(strategy_ema_period <= 1 || strategy_atr_period <= 0 ||
      strategy_min_stop_atr <= 0.0 || strategy_max_stop_atr <= 0.0 ||
   ...
   const double ema_low_2 = QM_EMA(_Symbol, strategy_signal_tf, strategy_ema_period, 2, PRICE_LOW);
   const double atr = QM_ATR(_Symbol, strategy_signal_tf, strategy_atr_period, 1);
   const double point = SymbolInfoDouble(_Symbol, SYMBOL_POINT);
   const double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
   const double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);

   if(close_1 <= 0.0 || close_2 <= 0.0 || prev_low <= 0.0 || prev_high <= 0.0 ||
      ema_high_1 <= 0.0 || ema_high_2 <= 0.0 || ema_low_1 <= 0.0 || ema_low_2 <= 0.0 ||
      atr <= 0.0 || point <= 0.0 || ask <= 0.0 || bid <= 0.0)
      return false;

   const bool long_cross = (close_2 <= ema_high_2 && close_1 > ema_high_1);
   const bool short_cross = (close_2 >= ema_low_2 && close_1 < ema_low_1);
   if(!long_cross && !short_cross)
      return false;

   const double min_stop_distance = strategy_min_stop_atr * atr;
   const double max_stop_distance = strategy_max_stop_atr * atr;
   double entry = 0.0;
   double sl = 0.0;
   ...
   if(long_cross)
     {
      side = QM_BUY;
      entry = ask;
      double stop_distance = entry - prev_low;
   ...
   else
     {
      side = QM_SELL;
      entry = bid;
      double stop_distance = prev_high - entry;
      if(stop_distance > max_stop_distance)
         return false;
      if(stop_distance < min_stop_distance)
         sl = entry + min_stop_distance;
   ...
   if(stop_distance <= 0.0 || spread < 0.0)
      return false;
   if(spread > strategy_max_spread_frac * stop_distance)
      return false;

   req.type = side;
   req.price = 0.0;
   req.sl = sl;
   req.tp = QM_TakeRR(_Symbol, side, entry, sl, strategy_take_profit_rr);
   req.reason = long_cross ? "EMA_CHANNEL_LONG_CROSS" : "EMA_CHANNEL_SHORT_CROSS";
