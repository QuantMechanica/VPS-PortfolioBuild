...
#property description "QM5_1126 Moskowitz TSMOM 12M"

#include <QM/QM_Common.mqh>

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 1126;
...
input double strategy_signal_threshold_pct  = 0.5;
input int    strategy_atr_period            = 14;
input double strategy_atr_sl_mult           = 3.0;
input int    strategy_spread_days           = 20;
input double strategy_spread_mult           = 3.0;

#define QM5_1126_SYMBOL_COUNT 9

string g_symbols[QM5_1126_SYMBOL_COUNT] =
  {
   "GDAXI.DWX", "NDX.DWX", "UK100.DWX", "WS30.DWX", "EURUSD.DWX",
   "GBPUSD.DWX", "USDJPY.DWX", "AUDUSD.DWX", "XAUUSD.DWX"
  };

int g_slots[QM5_1126_SYMBOL_COUNT] = {0, 1, 2, 3, 4, 5, 6, 7, 8};
...
int Strategy_CurrentSymbolIndex()
  {
   for(int i = 0; i < QM5_1126_SYMBOL_COUNT; ++i)
...
int Strategy_TsmomDirection()
  {
   if(strategy_recent_offset_d1_bars <= 0 || strategy_lookback_d1_bars <= strategy_recent_offset_d1_bars)
      return 0;
   if(Bars(_Symbol, PERIOD_D1) < MathMax(strategy_min_d1_bars, strategy_lookback_d1_bars + 5))
      return 0;

   const double recent_close = iClose(_Symbol, PERIOD_D1, strategy_recent_offset_d1_bars);
   const double lookback_close = iClose(_Symbol, PERIOD_D1, strategy_lookback_d1_bars);
   if(recent_close <= 0.0 || lookback_close <= 0.0)
      return 0;

   const double r12 = (recent_close / lookback_close) - 1.0;
   const double threshold = MathAbs(strategy_signal_threshold_pct) / 100.0;
   if(r12 > threshold)
      return 1;
   if(r12 < -threshold)
      return -1;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   const int rebalance_key = Strategy_RebalanceKey(iTime(_Symbol, PERIOD_D1, 0));
   ...
   req.tp = 0.0;
   req.symbol_slot = Strategy_CurrentSymbolSlot();
   req.reason = (direction > 0) ? "MOP_TSMOM_12M_LONG" : "MOP_TSMOM_12M_SHORT";
   if(req.sl <= 0.0)
      return false;
...
bool Strategy_ExitSignal()
  {
   ...
   const int desired_direction = Strategy_TsmomDirection();
   const int magic = QM_FrameworkMagic();
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      const ulong ticket = PositionGetTicket(i);
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_1126\",\"ea\":\"moskowitz-tsmom-12m\"}");
