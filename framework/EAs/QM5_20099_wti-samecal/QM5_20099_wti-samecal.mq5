...
#property description "QM5_20099 WTI same-calendar-month seasonal sign"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_20099 - WTI Same-Calendar-Month Seasonal Sign
// -----------------------------------------------------------------------------
// At each broker-month boundary, average WTI's completed return for the same
// calendar month across prior years. Trade the sign for one month:
//   positive historical average -> BUY XTIUSD.DWX
//   negative historical average -> SELL XTIUSD.DWX
...
bool Strategy_LoadSeasonalSignal(const datetime decision_bar_time,
                                 ...
                                 int &direction)
  {
   ...
   const int copied = CopyRates(_Symbol, PERIOD_D1, 1, strategy_history_bars, rates); // perf-allowed: bespoke bounded same-calendar return window, called only on the monthly boundary.
   if(copied <= 0)
      return false;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20099_WTI_SAMECAL";
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_20099_MONTH_ATTEMPT_%d",
                   QM_FrameworkMagic());
