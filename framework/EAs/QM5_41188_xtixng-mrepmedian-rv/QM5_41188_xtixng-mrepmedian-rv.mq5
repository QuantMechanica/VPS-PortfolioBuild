...
#property description "QM5_41188 XTI/XNG Monthly Repeated-Median Ratio Reversion"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_41188 - XTI/XNG Monthly Repeated-Median Ratio Reversion
// -----------------------------------------------------------------------------
...
// completed broker months. For every oil-minus-gas log-ratio pivot, take the
// median of its twelve forward slopes; then take the median of all thirteen
// pivot medians and fade its strict sign.
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XTIXNG_MREPMEDIAN_RV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41188_XTIXNG_MREPMEDIAN_RV";
...
bool Strategy_LoadMonthlyRepeatedMedian(
      ...
      int &direction)
  {
   ...
   // direction > 0 means BUY XTI / SELL XNG. Fade only the strict repeated-
   // median ratio-slope sign; endpoint displacement remains diagnostic only.
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41188_XTIXNG_MREPMEDIAN_RV_HOST";
   ...
   const bool valid =
      Strategy_LoadMonthlyRepeatedMedian(
         g_signal_month_key,
         month_count,
   ...
   string state = "monthly_repeated_median_failed";
   if(valid && direction > 0)
      state = "negative_repeated_median_fade_long_xti";
   else if(valid && direction < 0)
      state = "positive_repeated_median_fade_short_xti";
   else if(valid && repeated_median == 0.0)
      state = "zero_repeated_median_flat";
   else if(valid)
      state = "repeated_median_sign_flat";

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"month_count\":%d,\"newest_endpoint_time\":%I64d,\"endpoint_displacement\":%.12e,\"grouped_slope_count\":%d,\"pivot_median_count\":%d,\"inner_median_min\":%.12e,\"inner_median_max\":%.12e,\"repeated_median\":%.12e,\"direction\":%d,\"state\":\"%s\"}",
                            g_signal_month_key,
                            month_count,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XTI/XNG thirteen-month repeated-median ratio-reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41188\",\"ea\":\"xtixng-mrepmedian-rv\"}");
