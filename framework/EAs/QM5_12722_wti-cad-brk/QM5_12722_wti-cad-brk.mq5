...
#property description "QM5_12722 WTI CAD Petro-Currency Breakout"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_12722 - WTI/CAD petro-currency channel breakout
// -----------------------------------------------------------------------------
// D1 two-leg basket:
//   spread = ln(XTIUSD.DWX) - beta * ln(USDCAD.DWX)
//   break above entry channel: buy WTI, sell USDCAD
//   break below entry channel: sell WTI, buy USDCAD
// Runtime uses MT5 OHLC only; no macro feed, futures curve, API, or ML.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 12722;
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(CopyClose(g_leg_xti, PERIOD_D1, 1, lookback, xti) != lookback) // perf-allowed: called only behind the D1 new-bar gate or close-state refresh.
      return false;
   if(CopyClose(g_leg_usdcad, PERIOD_D1, 1, lookback, usdcad) != lookback) // perf-allowed: called only behind the D1 new-bar gate or close-state refresh.
      return false;
   ...
   for(int i = 0; i < lookback; ++i)
     {
      if(xti[i] <= 0.0 || usdcad[i] <= 0.0)
         return false;
      spreads[i] = MathLog(xti[i]) - strategy_beta * MathLog(usdcad[i]);
...
bool Strategy_OpenPair(const int spread_direction)
  {
   if(spread_direction == 0 || Strategy_OpenPairLegCount() > 0)
      return false;
   if(!Strategy_SpreadAllowed(g_leg_xti) || !Strategy_SpreadAllowed(g_leg_usdcad))
      return false;

   const double xti_weight = 1.0;
   const double usdcad_weight = 1.0;
   const double weight_sum = xti_weight + usdcad_weight;
   ...
   const string reason = long_spread ? "QM5_12722_LONG_XTI_SHORT_USDCAD"
                                     : "QM5_12722_SHORT_XTI_LONG_USDCAD";
