...
#property description "QM5_41374 XTI/XNG Weekly Efficiency-Divergence Reversion"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_41374 - XTI/XNG Weekly Body/Range Efficiency-Divergence Reversion
// -----------------------------------------------------------------------------
// Reconstruct synchronized XTI/XNG OHLC for the immediately completed broker
// week. Compute each leg's absolute weekly body divided by its weekly range.
// When exactly one leg is above the upper efficiency tercile and the other is
// below the lower tercile, fade the high-efficiency leg's body direction with
// an equal-notional opposite-leg package.
// Consume one attempt per week and flatten at the next week boundary.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41374;
...
input int    strategy_min_week_sessions       = 3;
input int    strategy_max_week_sessions       = 5;
input double strategy_efficiency_lower        = 0.333333333333;
input double strategy_efficiency_upper        = 0.666666666667;
...
double   g_xti_week_open = 0.0;
double   g_xti_week_high = 0.0;
double   g_xti_week_low = 0.0;
double   g_xti_week_close = 0.0;
double   g_xng_week_open = 0.0;
double   g_xng_week_high = 0.0;
double   g_xng_week_low = 0.0;
double   g_xng_week_close = 0.0;
double   g_xti_efficiency = 0.0;
double   g_xng_efficiency = 0.0;
int      g_week_sessions = 0;
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41374 && qm_magic_slot_offset == 0 &&
            strategy_xti_symbol != "" && strategy_xng_symbol != "" &&
            QM_MagicSymbolCanonical(strategy_xti_symbol) != QM_MagicSymbolCanonical(strategy_xng_symbol) &&
            QM_MagicSymbolCanonical(strategy_xti_symbol) == QM_MagicSymbolCanonical(_Symbol) &&
            strategy_history_bars_d1 == 30 &&
            strategy_min_week_sessions == 3 &&
            strategy_max_week_sessions == 5 &&
            MathAbs(strategy_efficiency_lower - 0.333333333333) <= 1.0e-12 &&
            MathAbs(strategy_efficiency_upper - 0.666666666667) <= 1.0e-12 &&
            strategy_efficiency_lower > 0.0 &&
            strategy_efficiency_lower < strategy_efficiency_upper &&
            strategy_efficiency_upper < 1.0 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XTIXNG_WEFFDIV_ATTEMPT_WEEK", qm_ea_id);
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41374_XTIXNG_WEFFDIV_RV";
...
bool Strategy_LoadWeeklyEfficiencyDivergence(const int current_week,
                                              int &direction)
  {
   direction = 0;
   g_xti_week_open = 0.0;
   g_xti_week_high = 0.0;
   g_xti_week_low = 0.0;
   g_xti_week_close = 0.0;
   g_xng_week_open = 0.0;
   g_xng_week_high = 0.0;
   g_xng_week_low = 0.0;
   g_xng_week_close = 0.0;
   g_xti_efficiency = 0.0;
   g_xng_efficiency = 0.0;
   g_week_sessions = 0;
   ...
   const int xti_copied =
      CopyRates(g_leg_xti, // perf-allowed: decision-only bounded completed-week OHLC scan.
                PERIOD_D1, 1, strategy_history_bars_d1, xti_bars);
   const int xng_copied =
      CopyRates(g_leg_xng, // perf-allowed: decision-only bounded completed-week OHLC scan.
   ...
   for(int index = 0; index < strategy_history_bars_d1; ++index)
     {
      ...
      else if(week_key != completed_week)
         break;

      if(xti_bars[index].open <= 0.0 || xti_bars[index].high <= 0.0 ||
         xti_bars[index].low <= 0.0 || xti_bars[index].close <= 0.0 ||
         xng_bars[index].open <= 0.0 || xng_bars[index].high <= 0.0 ||
         xng_bars[index].low <= 0.0 || xng_bars[index].close <= 0.0 ||
         !MathIsValidNumber(xti_bars[index].open) ||
         !MathIsValidNumber(xti_bars[index].high) ||
         !MathIsValidNumber(xti_bars[index].low) ||
         !MathIsValidNumber(xti_bars[index].close) ||
         !MathIsValidNumber(xng_bars[index].open) ||
         !MathIsValidNumber(xng_bars[index].high) ||
         !MathIsValidNumber(xng_bars[index].low) ||
         !MathIsValidNumber(xng_bars[index].close) ||
         xti_bars[index].high < xti_bars[index].open ||
         xti_bars[index].high < xti_bars[index].close ||
         xti_bars[index].low > xti_bars[index].open ||
         xti_bars[index].low > xti_bars[index].close ||
         xng_bars[index].high < xng_bars[index].open ||
         xng_bars[index].high < xng_bars[index].close ||
         xng_bars[index].low > xng_bars[index].open ||
         xng_bars[index].low > xng_bars[index].close)
         return false;

      if(g_week_sessions == 0)
        {
         g_xti_week_high = xti_bars[index].high;
         g_xti_week_low = xti_bars[index].low;
         g_xti_week_close = xti_bars[index].close;
         g_xng_week_high = xng_bars[index].high;
         g_xng_week_low = xng_bars[index].low;
         g_xng_week_close = xng_bars[index].close;
        }
      else
        {
         g_xti_week_high = MathMax(g_xti_week_high, xti_bars[index].high);
         g_xti_week_low = MathMin(g_xti_week_low, xti_bars[index].low);
         g_xng_week_high = MathMax(g_xng_week_high, xng_bars[index].high);
         g_xng_week_low = MathMin(g_xng_week_low, xng_bars[index].low);
        }
      // Arrays are newest-first, so the final assignment is the chronological
      // open of the immediately completed week.
      g_xti_week_open = xti_bars[index].open;
      g_xng_week_open = xng_bars[index].open;
      ++g_week_sessions;
     }

   if(completed_week <= 0 ||
      Strategy_NextWeekKey(completed_week) != current_week ||
      g_week_sessions < strategy_min_week_sessions ||
      g_week_sessions > strategy_max_week_sessions ||
      g_xti_week_open <= 0.0 || g_xti_week_high <= 0.0 ||
      g_xti_week_low <= 0.0 ||
      g_xti_week_close <= 0.0 || g_xng_week_high <= 0.0 ||
      g_xng_week_open <= 0.0 || g_xng_week_low <= 0.0 ||
      g_xng_week_close <= 0.0)
      return false;

   const double xti_range = g_xti_week_high - g_xti_week_low;
   const double xng_range = g_xng_week_high - g_xng_week_low;
   if(xti_range <= 0.0 || xng_range <= 0.0 ||
      !MathIsValidNumber(xti_range) || !MathIsValidNumber(xng_range))
      return false;

   const double xti_body = g_xti_week_close - g_xti_week_open;
   const double xng_body = g_xng_week_close - g_xng_week_open;
   if(!MathIsValidNumber(xti_body) || !MathIsValidNumber(xng_body) ||
      MathAbs(xti_body) <= 1.0e-12 || MathAbs(xng_body) <= 1.0e-12)
      return false;

   g_xti_efficiency = MathAbs(xti_body) / xti_range;
   g_xng_efficiency = MathAbs(xng_body) / xng_range;
   if(!MathIsValidNumber(g_xti_efficiency) ||
      !MathIsValidNumber(g_xng_efficiency) ||
      g_xti_efficiency <= 0.0 || g_xti_efficiency > 1.0 ||
      g_xng_efficiency <= 0.0 || g_xng_efficiency > 1.0)
      return false;

   if(g_xti_efficiency > strategy_efficiency_upper &&
      g_xng_efficiency < strategy_efficiency_lower)
      direction = (xti_body > 0.0) ? -1 : 1;
   else if(g_xng_efficiency > strategy_efficiency_upper &&
           g_xti_efficiency < strategy_efficiency_lower)
      direction = (xng_body > 0.0) ? 1 : -1;
   return true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41374_XTIXNG_WEFFDIV_RV_HOST";
   ...
   if(Strategy_OpenOwnedPositionCount() > 0 ||
     ...
     {
      signal_valid =
         Strategy_LoadWeeklyEfficiencyDivergence(g_signal_week_key,
                                                    direction);
      if(!signal_valid)
         g_signal_state = "weekly_ohlc_validation_failed";
       else if(direction < 0)
          g_signal_state = "efficiency_divergence_fade_short_xti";
       else if(direction > 0)
          g_signal_state = "efficiency_divergence_fade_long_xti";
       else
          g_signal_state = "efficiency_boundary_or_interior_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
                StringFormat("{\"week\":%d,\"decision_bar\":%I64d,\"late\":%s,\"valid\":%s,\"direction\":%d,\"xti_efficiency\":%.12f,\"xng_efficiency\":%.12f,\"xti_open\":%.10f,\"xti_high\":%.10f,\"xti_low\":%.10f,\"xti_close\":%.10f,\"xng_open\":%.10f,\"xng_high\":%.10f,\"xng_low\":%.10f,\"xng_close\":%.10f,\"sessions\":%d,\"state\":\"%s\"}",
                            g_signal_week_key,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XTI/XNG weekly efficiency-divergence package exits at the next broker week"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41374\",\"ea\":\"xtixng-weffdiv-rv\"}");
