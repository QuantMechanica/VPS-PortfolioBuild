...
#property description "QM5_41192 XTI/XNG Completed-Month Daily Hodges-Lehmann Reversion"
...
// immediately completed 17-to-23-session calendar month plus one older pair.
// Form every inclusive self/cross-pair average of the oil-minus-gas daily
// relative returns, take their exact odd/even pseudomedian, and fade its strict
// sign. The opposite, equal-notional package is held to the next month. Runtime
// is native, completed-price-only, deterministic, and one-attempt-per-month.
// =============================================================================
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41192 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
            ...
            MathAbs(strategy_numerical_tolerance - 1.0e-10) <= 1.0e-18 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XTIXNG_MDAILY_HL_RV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41192_XTIXNG_MDAILY_HL_RV";
...
bool Strategy_SynchronizedPairValid(MqlRates &xti_bars[],
                                    MqlRates &xng_bars[],
                                    const int index)
  {
   if(index < 0 || index >= ArraySize(xti_bars) ||
      index >= ArraySize(xng_bars) ||
      xti_bars[index].time <= 0 ||
      xti_bars[index].time != xng_bars[index].time ||
      xti_bars[index].close <= 0.0 || xng_bars[index].close <= 0.0 ||
      !MathIsValidNumber(xti_bars[index].close) ||
      !MathIsValidNumber(xng_bars[index].close))
      return false;
   if(index > 0 &&
      (xti_bars[index - 1].time <= xti_bars[index].time ||
       xng_bars[index - 1].time <= xng_bars[index].time))
      return false;
   return true;
  }

bool Strategy_LoadMonthlyDailyPseudomedian(
      ...
      int &pair_count,
      int &median_left_index,
      int &median_right_index,
      double &pseudomedian,
      int &direction)
  {
   ...
   MqlRates xti_bars[];
   MqlRates xng_bars[];
   double series_ratios[];
   double chronological_ratios[];
   double daily_returns[];
   double pairwise_averages[];
   ArraySetAsSeries(xti_bars, true);
   ArraySetAsSeries(xng_bars, true);
   if(ArrayResize(series_ratios, strategy_max_month_sessions) !=
      strategy_max_month_sessions ||
      ArrayResize(chronological_ratios,
                  strategy_max_month_sessions + 1) !=
       strategy_max_month_sessions + 1 ||
       ArrayResize(daily_returns, strategy_max_month_sessions) !=
       strategy_max_month_sessions ||
       ArrayResize(pairwise_averages, strategy_max_pair_count) !=
       strategy_max_pair_count)
      return false;

   const int xti_copied =
      CopyRates(g_leg_xti, // perf-allowed: one bounded completed-month scan behind a consumed monthly attempt.
                PERIOD_D1, 1, strategy_history_bars_d1, xti_bars);
   const int xng_copied =
      CopyRates(g_leg_xng, // perf-allowed: one bounded completed-month scan behind a consumed monthly attempt.
                PERIOD_D1, 1, strategy_history_bars_d1, xng_bars);
   if(xti_copied != strategy_history_bars_d1 ||
      xng_copied != strategy_history_bars_d1 ||
      !Strategy_SynchronizedPairValid(xti_bars, xng_bars, 0))
      return false;

   const int completed_month_key = Strategy_MonthKey(xti_bars[0].time);
   if(completed_month_key <= 0 ||
      Strategy_MonthKey(xng_bars[0].time) != completed_month_key ||
      Strategy_NextMonthKey(completed_month_key) != current_month_key)
      return false;

   int index = 0;
   while(index < strategy_history_bars_d1 &&
         Strategy_MonthKey(xti_bars[index].time) == completed_month_key)
     {
      ...
      if(completed_month_sessions >= strategy_max_month_sessions ||
         !Strategy_SynchronizedPairValid(xti_bars, xng_bars, index) ||
         Strategy_MonthKey(xng_bars[index].time) != completed_month_key ||
         !Strategy_LogRatio(xti_bars[index].close,
                            xng_bars[index].close,
                            series_ratios[completed_month_sessions]))
         return false;
   ...
   if(completed_month_sessions < strategy_min_month_sessions ||
      ...
      index >= strategy_history_bars_d1 ||
      !Strategy_SynchronizedPairValid(xti_bars, xng_bars, index))
      return false;

   // The adjacent older synchronized pair proves the completed month was not
   // truncated and supplies the left endpoint for the first month-ending
   // relative return.
   const int older_month_key = Strategy_MonthKey(xti_bars[index].time);
   if(older_month_key <= 0 ||
      Strategy_MonthKey(xng_bars[index].time) != older_month_key ||
   ...
   // CopyRates series order is newest to oldest. Reverse the complete month so
   // every oil-minus-gas relative return ending inside it appears once.
   for(int i = 0; i < completed_month_sessions; ++i)
     {
      const double ratio =
         series_ratios[completed_month_sessions - 1 - i];
   ...
   for(int i = 1; i <= completed_month_sessions; ++i)
     {
      ...
      raw_displacement += relative_return;
      if(!MathIsValidNumber(raw_displacement))
         return false;
     }
   ...
   const double identity_scale =
      MathMax(1.0, MathAbs(endpoint_displacement));
   ...
   const int minimum_pair_count =
      strategy_min_month_sessions * (strategy_min_month_sessions + 1) / 2;
   if(pair_count < minimum_pair_count ||
      pair_count > strategy_max_pair_count ||
      pair_count < return_count)
      return false;
      ...
      for(int j = i; j < return_count; ++j)
        {
         ...
         const double pair_average =
            0.5 * (daily_returns[i] + daily_returns[j]);
         if(!MathIsValidNumber(pair_average))
            return false;
         if(i == j)
           {
            const double self_scale =
               MathMax(1.0, MathAbs(daily_returns[i]));
            if(MathAbs(pair_average - daily_returns[i]) >
               strategy_numerical_tolerance * self_scale)
               return false;
           }
   ...
   if(pair_index != pair_count ||
      ArrayResize(pairwise_averages, pair_count) != pair_count ||
   ...
   if(median_left_index == median_right_index)
      ...
      pseudomedian = 0.5 *
         (pairwise_averages[median_left_index] +
          pairwise_averages[median_right_index]);
   if(!MathIsValidNumber(pseudomedian))
      return false;

   // direction > 0 means BUY XTI / SELL XNG. Fade only the strict sign of the
   // pairwise pseudomedian. The raw endpoint is an identity diagnostic only.
   if(pseudomedian < 0.0)
      direction = 1;
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41192_XTIXNG_MDAILY_HL_RV_HOST";
   ...
   int return_count = 0;
   int pair_count = 0;
   int median_left_index = -1;
   int median_right_index = -1;
   int direction = 0;
   double raw_displacement = 0.0;
   double endpoint_displacement = 0.0;
   double pseudomedian = 0.0;
   const bool valid =
      Strategy_LoadMonthlyDailyPseudomedian(
         g_signal_month_key,
         completed_month_sessions,
         return_count,
         raw_displacement,
         ...
         pseudomedian,
         direction);
   string state = "monthly_daily_pseudomedian_failed";
   if(valid && direction > 0)
      state = "negative_pseudomedian_fade_long_xti";
   else if(valid && direction < 0)
      state = "positive_pseudomedian_fade_short_xti";
   else if(valid && pseudomedian == 0.0)
      state = "zero_pseudomedian_flat";
   else if(valid)
      state = "pseudomedian_sign_flat";

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"completed_month_sessions\":%d,\"return_count\":%d,\"raw_displacement\":%.12e,\"endpoint_displacement\":%.12e,\"pair_count\":%d,\"median_left_index\":%d,\"median_right_index\":%d,\"pseudomedian\":%.12e,\"direction\":%d,\"state\":\"%s\"}",
                            g_signal_month_key,
                            completed_month_sessions,
                            return_count,
                            raw_displacement,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XTI/XNG completed-month daily Hodges-Lehmann reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41192\",\"ea\":\"xtixng-mdaily-hl-rv\"}");
