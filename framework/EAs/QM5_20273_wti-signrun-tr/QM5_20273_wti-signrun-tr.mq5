...
#property description "QM5_20273 WTI Dominant Sign-Run Trend"
...
//   3. find the longest positive and negative same-sign runs, resetting on zero;
//   4. trade only the unique direction whose run is at least four months.
// The longest-run statistic is a transparent QM mechanization, not an author
// rule. Runtime uses native XTIUSD.DWX price and broker data only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 20273;
...
input int    strategy_min_run_months      = 4;
...
int    g_cache_longest_positive = 0;
int    g_cache_longest_negative = 0;
int    g_cache_zero_count       = 0;
...
void Strategy_ResetCachedState()
  {
   ...
   g_cache_signal = 0;
   g_cache_longest_positive = 0;
   g_cache_longest_negative = 0;
   g_cache_zero_count = 0;
   g_cache_state_reason = "not_evaluated";
  }

bool Strategy_DominantRunSignal(const double &closes[], int &signal)
  {
   signal = 0;
   ...
   if(point_count != 13 ||
      interval_count != 12 ||
      strategy_min_run_months != 4 ||
      ArraySize(closes) != point_count)
     {
      g_cache_state_reason = "bad_sign_run_contract";
      return false;
     }

   int current_positive = 0;
   int current_negative = 0;
   int longest_positive = 0;
   int longest_negative = 0;
   int zero_count = 0;
   ...
   for(int i = 0; i < interval_count; ++i)
     {
      ...
      if(monthly_return > 0.0)
        {
         ++current_positive;
         current_negative = 0;
         if(current_positive > longest_positive)
            longest_positive = current_positive;
        }
      else if(monthly_return < 0.0)
        {
         ++current_negative;
         current_positive = 0;
         if(current_negative > longest_negative)
            longest_negative = current_negative;
        }
      else
        {
         current_positive = 0;
      ...
      if(current_positive < 0 || current_negative < 0 ||
         longest_positive < current_positive ||
         longest_negative < current_negative)
        {
         g_cache_state_reason = "invalid_run_state";
         return false;
        }
     }

   if(longest_positive < 0 || longest_negative < 0 ||
      longest_positive > interval_count ||
      longest_negative > interval_count ||
      zero_count < 0 || zero_count > interval_count)
     {
      g_cache_state_reason = "invalid_run_summary";
      return false;
     }

   g_cache_longest_positive = longest_positive;
   g_cache_longest_negative = longest_negative;
   g_cache_zero_count = zero_count;

   if(longest_positive >= strategy_min_run_months &&
      longest_positive > longest_negative)
     {
      signal = 1;
      g_cache_state_reason = "qualified_long";
     }
   else if(longest_negative >= strategy_min_run_months &&
           longest_negative > longest_positive)
     {
      signal = -1;
      g_cache_state_reason = "qualified_short";
     }
   else if(longest_positive == longest_negative &&
           longest_positive >= strategy_min_run_months)
      g_cache_state_reason = "equal_longest_runs";
   else
      g_cache_state_reason = "no_unique_four_month_run";
   return true;
  }
...
bool Strategy_LoadSignalState(int &signal)
  {
   ...
   return Strategy_DominantRunSignal(monthly_closes, signal);
  }
...
void Strategy_PrepareMonthlySignal()
  {
   ...
   QM_LogEvent(QM_INFO,
               "MONTHLY_STATE",
               StringFormat("{\"month\":%d,\"valid\":%s,\"signal\":%d,\"longest_positive\":%d,\"longest_negative\":%d,\"zeros\":%d,\"state\":\"%s\"}",
                            g_cache_month_key,
                            g_cache_signal_valid ? "true" : "false",
                            g_cache_signal,
                            g_cache_longest_positive,
                            g_cache_longest_negative,
                            g_cache_zero_count,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 20273 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20273_WTI_DOMINANT_SIGN_RUN";
   ...
   req.reason = (g_cache_signal > 0) ? "DOMINANT_SIGN_RUN_XTI_LONG" : "DOMINANT_SIGN_RUN_XTI_SHORT";
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_20273_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_20273\",\"ea\":\"wti-signrun-tr\"}");
