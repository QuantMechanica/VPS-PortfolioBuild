...
#property description "QM5_41313 WTI Monthly Ljung-Box Trend"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41313 - WTI Monthly Ljung-Box-Portmanteau Trend
// -----------------------------------------------------------------------------
...
//   - aggregate ordinary autocorrelation at lags one through six
//   - qualify the finite-sample Ljung-Box Q6 statistic at or above 5.35
...
input int    strategy_ljung_box_lags       = 6;
input double strategy_variance_floor       = 0.000000000000000001;
input double strategy_q_boundary           = 5.35;
   ...
   for(int index = 0; index < value_count; ++index)
     {
      ...
      if(!MathIsValidNumber(centered[index]))
         return false;
      const double square = centered[index] * centered[index];
      if(!MathIsValidNumber(square) || square < 0.0)
         return false;
      variance_sum += square;
      if(!MathIsValidNumber(variance_sum))
         return false;
     }
   if(variance_sum <= strategy_variance_floor)
      return false;

   double weighted_squared_sum = 0.0;
   for(int lag = 1; lag <= strategy_ljung_box_lags; ++lag)
     {
      ...
      const double autocorrelation = numerator / variance_sum;
      if(!MathIsValidNumber(autocorrelation))
         return false;
      const double lag_denominator = (double)(value_count - lag);
      const double weighted_square =
         autocorrelation * autocorrelation / lag_denominator;
      if(lag_denominator <= 0.0 ||
         !MathIsValidNumber(weighted_square) ||
         weighted_square < 0.0)
         return false;
      weighted_squared_sum += weighted_square;
      if(!MathIsValidNumber(weighted_squared_sum))
         return false;
      if(StringLen(autocorrelation_path) > 0)
         autocorrelation_path += ",";
      autocorrelation_path += DoubleToString(autocorrelation, 16);
     }

   ljung_box_q6 =
      (double)value_count * (double)(value_count + 2) *
      weighted_squared_sum;
   return (MathIsValidNumber(ljung_box_q6) && ljung_box_q6 >= 0.0 &&
           StringLen(autocorrelation_path) > 0);
  }

bool Strategy_LjungBoxReferenceSelfTest()
  {
   double alternating_path[48];
   double constant_path[48];
   for(int index = 0; index < 48; ++index)
     {
      alternating_path[index] =
         ((index % 2) == 0) ? 0.01 : -0.01;
   ...
   double ljung_box_q6 = 0.0;
   string autocorrelation_path = "";
   if(!Strategy_LjungBoxCore(alternating_path,
                            return_mean,
                            variance_sum,
                            ljung_box_q6,
                            autocorrelation_path) ||
      MathAbs(return_mean) > 0.000000000000001 ||
      MathAbs(variance_sum - 0.0048) > 0.000000000001 ||
      MathAbs(ljung_box_q6 - 278.125) > 0.000000001 ||
      StringLen(autocorrelation_path) <= 0)
      return false;

   if(Strategy_LjungBoxCore(constant_path,
                            return_mean,
                            variance_sum,
                            ljung_box_q6,
                            autocorrelation_path))
...
bool Strategy_LjungBoxSignal(const double &closes[],
                             Strategy_SignalMetrics &metrics)
  {
   ...
   metrics.direction = 0;
   metrics.portmanteau_qualified = false;

   if(strategy_month_returns != 48 ||
      strategy_ljung_box_lags != 6 ||
      strategy_variance_floor != 0.000000000000000001 ||
      MathAbs(strategy_q_boundary - 5.35) >
         0.000000000000001 ||
   ...
   if(!Strategy_LjungBoxCore(returns,
                            metrics.return_mean,
                            metrics.variance_sum,
                            metrics.ljung_box_q6,
                            metrics.autocorrelation_path))
      return false;
   metrics.portmanteau_qualified =
   ...
   if(!metrics.portmanteau_qualified)
      return true;
                                        ...
                                        int &direction)
  {
   ...
   const bool valid =
      Strategy_LoadMonthlyEndpoints(current_month_key,
                                    ...
                                    closes,
                                    endpoint_times) &&
      Strategy_LjungBoxSignal(closes, metrics);
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
   ...
   else
     {
      ...
      g_signal_valid =
         endpoints_valid &&
         Strategy_LjungBoxSignal(closes, g_signal_metrics);
      if(!g_signal_valid)
         g_signal_state = "ljung_box_validation_failed";
      else if(!g_signal_metrics.portmanteau_qualified)
         g_signal_state = "ljung_box_below_boundary_consumed_flat";
      else if(g_signal_metrics.direction == 0)
         g_signal_state = "qualified_neutral_momentum_consumed_flat";
      else if(g_signal_metrics.direction > 0)
         g_signal_state = "high_portmanteau_positive_momentum_wti_long";
      else if(g_signal_metrics.direction < 0)
         g_signal_state = "high_portmanteau_negative_momentum_wti_short";
      else
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"endpoint_count\":%d,\"return_count\":%d,\"return_mean\":%.12f,\"variance_sum\":%.16f,\"ljung_box_q6\":%.12f,\"portmanteau_qualified\":%s,\"momentum_12\":%.12f,\"returns\":\"%s\",\"autocorrelations\":\"%s\",\"oldest_close\":%.10f,\"newest_close\":%.10f,\"oldest_endpoint\":%I64d,\"newest_endpoint\":%I64d,\"state\":\"%s\"}",
                            ...
                            g_signal_metrics.variance_sum,
                            g_signal_metrics.ljung_box_q6,
                            g_signal_metrics.portmanteau_qualified ? "true" : "false",
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41313 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41313_WTI_LJUNGBOX";
   ...
   req.reason = (g_signal_metrics.direction > 0)
      ? "WTI_LJUNGBOX_MOM_LONG"
      : "WTI_LJUNGBOX_MOM_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41313 ||
      qm_magic_slot_offset != 0)
      return INIT_PARAMETERS_INCORRECT;
   if(!Strategy_LjungBoxReferenceSelfTest())
      return INIT_FAILED;
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI monthly Ljung-Box trend holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41313_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   g_entry_month_state_key =
      StringFormat("QM5_41313_ENTRY_MONTH_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41313\",\"ea\":\"wti-ljungbox-tr\"}");
