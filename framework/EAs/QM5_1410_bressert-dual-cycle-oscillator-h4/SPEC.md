# QM5_1410_bressert-dual-cycle-oscillator-h4 — Strategy Spec

**EA ID:** QM5_1410
**Slug:** `bressert-dual-cycle-oscillator-h4`
...
`artifacts/cards_approved/QM5_1410_bressert-dual-cycle-oscillator-h4.md`. See that card's body for
...
`QM5_1410_bressert-dual-cycle-oscillator-h4.mq5`. Framework wiring (risk, magic, news, Friday close)
is inherited from `QM_Common.mqh` and is not redocumented here.

The card's news rule is an entry-only, high-impact blackout of exactly
480 minutes before and after an event. Open-position management and exits stay
reachable during that window. TP1 completion is reconstructed from durable
position deal history after restart. The 4 ATR gain cap is evaluated as
current favorable price gain minus the position's reconstructed adverse price
excursion; it is not installed as a static broker TP.
...
| `strategy_dss_short_n` | 8 | (see source) | (see strategy logic) |
| `strategy_dss_short_m` | 3 | (see source) | (see strategy logic) |
| `strategy_dss_long_n` | 21 | (see source) | (see strategy logic) |
| `strategy_dss_long_m` | 7 | (see source) | (see strategy logic) |
| `strategy_timing_band_oversold` | 30.0 | (see source) | (see strategy logic) |
| `strategy_timing_band_overbought` | 70.0 | (see source) | (see strategy logic) |
| `strategy_timing_band_mid` | 50.0 | (see source) | (see strategy logic) |
| `strategy_sl_atr_mult` | 1.50 | (see source) | (see strategy logic) |
| `strategy_tp1_atr_mult` | 1.50 | (see source) | (see strategy logic) |
| `strategy_tp1_close_fraction` | 0.50 | (see source) | (see strategy logic) |
| `strategy_tp_cap_atr_mult` | 4.00 | (see source) | (see strategy logic) |
...
| `strategy_reuse_guard_bars` | 6 | (see source) | (see strategy logic) |
...
`artifacts/cards_approved/QM5_1410_bressert-dual-cycle-oscillator-h4.md`
...
| v2 | 2026-08-24 | Review rework | Exact ±2 H4 news entry window, restart-idempotent TP1, MAE-adjusted 4 ATR cap, direct Q08 MAE hook, pooled indicators, and bounded buffers. |
