...
#property description "QM5_13201 DAX Convex Opening-Range Breakout"

#include <QM/QM_Common.mqh>

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 13201;
...
input int    strategy_range_start_hour_berlin = 8;
input int    strategy_range_bars        = 1;
input int    strategy_trigger_hour_berlin = 9;
input int    strategy_pending_end_hour_berlin = 10;
input int    strategy_exit_hour_berlin  = 18;

int  g_trade_day_key = 0;
bool g_orders_placed_today = false;
bool g_position_seen_today = false;

datetime Strategy_LastSundayUTC(const int year, const int month)
  {
   MqlDateTime parts;
   ZeroMemory(parts);
   parts.year = year;
   ...
   parts.day = 31;
   parts.hour = 1;
   datetime boundary = StructToTime(parts);
   if(boundary <= 0 || !TimeToStruct(boundary, parts))
      return 0;
   return boundary - parts.day_of_week * 86400;
  }

bool Strategy_IsEUDSTUTC(const datetime utc_time)
  {
   MqlDateTime parts;
   if(utc_time <= 0 || !TimeToStruct(utc_time, parts))
      return false;
   const datetime start_utc = Strategy_LastSundayUTC(parts.year, 3);
   const datetime end_utc = Strategy_LastSundayUTC(parts.year, 10);
   return (start_utc > 0 && end_utc > 0 &&
           utc_time >= start_utc && utc_time < end_utc);
  }

datetime Strategy_BrokerToBerlin(const datetime broker_time)
  {
   const datetime utc_time = QM_BrokerToUTC(broker_time);
   return utc_time + (Strategy_IsEUDSTUTC(utc_time) ? 2 : 1) * 3600;
  }

bool Strategy_BerlinParts(const datetime broker_time, MqlDateTime &parts)
  {
   if(broker_time <= 0)
      return false;
   return TimeToStruct(Strategy_BrokerToBerlin(broker_time), parts);
  }

int Strategy_BerlinDayKey(const datetime broker_time)
  {
   MqlDateTime parts;
   if(!Strategy_BerlinParts(broker_time, parts))
      return 0;
   return parts.year * 10000 + parts.mon * 100 + parts.day;
  }

int Strategy_BerlinHour(const datetime broker_time)
  {
   MqlDateTime parts;
   if(!Strategy_BerlinParts(broker_time, parts))
      return -1;
...
bool Strategy_IsWeekdayBerlin(const datetime broker_time)
  {
   MqlDateTime parts;
   if(!Strategy_BerlinParts(broker_time, parts))
      return false;
...
void Strategy_ResetDay(const datetime broker_time)
  {
   const int day_key = Strategy_BerlinDayKey(broker_time);
...
bool Strategy_HadSetupToday(const datetime broker_now)
  {
   const int day_key = Strategy_BerlinDayKey(broker_now);
   if(day_key <= 0)
      return true;
   ...
   for(int index = HistoryOrdersTotal() - 1; index >= 0; --index)
     {
      ...
      if(Strategy_BerlinDayKey(setup_time) == day_key)
         return true;
     }
   return false;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   if(strategy_range_bars != 1 || strategy_atr_period != 14 ||
   ...
   const datetime range_time = iTime(_Symbol, PERIOD_H1, 1);
   if(current_bar_time <= 0 || range_time <= 0)
      return false;

   Strategy_ResetDay(current_bar_time);
   const int day_key = Strategy_BerlinDayKey(current_bar_time);
   if(day_key <= 0 || !Strategy_IsWeekdayBerlin(current_bar_time))
      return false;
   if(Strategy_BerlinHour(current_bar_time) != strategy_trigger_hour_berlin ||
      Strategy_BerlinHour(range_time) != strategy_range_start_hour_berlin)
      return false;
   if(Strategy_BerlinDayKey(range_time) != day_key)
      return false;
   ...
   const double range_high = iHigh(_Symbol, PERIOD_H1, 1);
   const double range_low = iLow(_Symbol, PERIOD_H1, 1);
   ...
   // A gap through one threshold at 09:00 is a real stop fill at the first
   ...
   if(ask >= buy_price && bid > sell_price)
     {
      if(!Strategy_BuildRequest(QM_BUY, ask, range_low, 0,
                                "DAX_CONVEX_ORB_GAP_BUY", request))
         return false;
      ulong ticket = 0;
   ...
   if(bid <= sell_price && ask < buy_price)
     {
      if(!Strategy_BuildRequest(QM_SELL, bid, range_high, 0,
                                "DAX_CONVEX_ORB_GAP_SELL", request))
         return false;
      ulong ticket = 0;
      QM_TM_OpenPosition(request, ticket);
      return false;
   ...
   if(!buy_ok || !sell_ok)
      Strategy_CancelOurPendingOrders("dax_convex_orb_incomplete_pair");
   return false;
  }
...
void Strategy_ManageOpenPosition()
  {
   ...
   const int berlin_hour = Strategy_BerlinHour(broker_now);
   if(has_position || berlin_hour < strategy_trigger_hour_berlin ||
      berlin_hour >= strategy_pending_end_hour_berlin)
      Strategy_CancelOurPendingOrders(has_position ? "dax_convex_orb_oco" :
                                                     "dax_convex_orb_expired");
  }
...
bool Strategy_ExitSignal()
  {
   ...
   const datetime broker_now = TimeCurrent();
   const int current_day_key = Strategy_BerlinDayKey(broker_now);
   const int position_day_key = Strategy_BerlinDayKey(
      (datetime)PositionGetInteger(POSITION_TIME));
   if(current_day_key <= 0 || position_day_key <= 0 ||
      current_day_key != position_day_key)
      return true;
   return (Strategy_BerlinHour(broker_now) >= strategy_exit_hour_berlin);
  }
...
int OnInit()
  {
   if(_Symbol != "GDAXI.DWX" || _Period != PERIOD_H1)
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_13201_dax-convex-orb\"}");
...
void OnTradeTransaction(const MqlTradeTransaction &trans,
                        ...
                        const MqlTradeResult &result)
  {
   ...
   if(entry == DEAL_ENTRY_IN || entry == DEAL_ENTRY_INOUT)
     {
      g_position_seen_today = true;
      Strategy_CancelOurPendingOrders("dax_convex_orb_oco_fill");
     }
  }
