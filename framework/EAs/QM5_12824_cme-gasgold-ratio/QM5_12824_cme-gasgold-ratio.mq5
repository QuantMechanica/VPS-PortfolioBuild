...
#property description "QM5_12824 CME Natural Gas Gold Ratio Reversion"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_12824 - CME Natural Gas/Gold Ratio Reversion
// -----------------------------------------------------------------------------
// D1 two-leg commodity basket:
//   spread = ln(XNGUSD.DWX) - beta * ln(XAUUSD.DWX)
//   z > entry: short ratio = sell XNG, buy gold
//   z < -entry: long ratio = buy XNG, sell gold
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(CopyClose(g_leg_xng, PERIOD_D1, 1, lookback, xng) != lookback) // perf-allowed: called only behind the D1 new-bar gate or close-state refresh.
      return false;
   ...
   for(int i = 0; i < lookback; ++i)
     {
      if(xng[i] <= 0.0 || xau[i] <= 0.0)
         return false;
      spreads[i] = MathLog(xng[i]) - strategy_beta * MathLog(xau[i]);
...
bool Strategy_OpenPair(const int ratio_direction)
  {
   ...
   const string reason = long_ratio ? "QM5_12824_LONG_XNG_XAU_RATIO"
                                    : "QM5_12824_SHORT_XNG_XAU_RATIO";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12824_RATIO_HOST";
...
int OnInit()
  {
   ...
   QM_BasketWarmupHistory(basket_symbols, PERIOD_D1, MathMax(160, strategy_z_lookback_d1 + strategy_atr_period_d1 + 10));

   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_12824\",\"ea\":\"cme-gasgold-ratio\"}");
