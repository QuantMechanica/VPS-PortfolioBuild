...
#property description "QM5_41259 WTI Exact-Permutation Wasserstein-1 Shift Trend"
...
//   - sort each block and measure equal-weight Wasserstein-1 distance
...
input double strategy_wasserstein_epsilon   = 0.000000000001;
...
bool Strategy_SortSix(const double &values[], double &ordered[])
  {
   if(ArraySize(values) != 6 || ArraySize(ordered) != 6)
      return false;
...
bool Strategy_MedianSix(const double &values[], double &median)
  {
   median = 0.0;
   double ordered[6];
   if(!Strategy_SortSix(values, ordered))
      return false;
...
bool Strategy_WassersteinForMask(const double &values[],
                                  ...
                                  double &wasserstein)
  {
   ...
   double ordered_old[6];
   double ordered_recent[6];
   if(!Strategy_SortSix(old_values, ordered_old) ||
      !Strategy_SortSix(recent_values, ordered_recent))
      return false;

   double paired_difference_sum = 0.0;
   for(int rank = 0; rank < 6; ++rank)
     {
      const double paired_difference =
         MathAbs(ordered_old[rank] - ordered_recent[rank]);
      if(!MathIsValidNumber(paired_difference))
         return false;
      paired_difference_sum += paired_difference;
     }

   // Equal empirical weights in one dimension reduce W1 to monotone
   // order-statistic pairing. The locked divisor is the six quantile pairs.
   if(strategy_block_size != 6 ||
      !MathIsValidNumber(paired_difference_sum))
      return false;
   wasserstein = paired_difference_sum / (double)strategy_block_size;
   return (wasserstein >= 0.0 && MathIsValidNumber(wasserstein));
  }

bool Strategy_WassersteinSignal(const double &closes[],
                                Strategy_SignalMetrics &metrics)
  {
   ...
   if(strategy_month_returns != 12 ||
      ...
      MathAbs(strategy_wasserstein_epsilon - 0.000000000001) >
   ...
   if(Strategy_Popcount12(observed_mask) != strategy_block_size ||
      !Strategy_WassersteinForMask(returns,
                                   observed_mask,
                                   metrics.observed_wasserstein))
      return false;

   metrics.comparison_epsilon =
      strategy_wasserstein_epsilon *
      MathMax(1.0, MathAbs(metrics.observed_wasserstein));
   ...
   for(int mask = 0; mask < 4096; ++mask)
     {
      if(Strategy_Popcount12(mask) != strategy_block_size)
         continue;
      double perm_wasserstein = 0.0;
      if(!Strategy_WassersteinForMask(returns,
                                      mask,
                                      perm_wasserstein))
         return false;
      if(perm_wasserstein + metrics.comparison_epsilon >=
         metrics.observed_wasserstein)
         ++metrics.tail_count;
                                        ...
                                        int &direction)
  {
   ...
   const bool valid =
      Strategy_LoadMonthlyEndpoints(current_month_key,
                                    ...
                                    closes,
                                    endpoint_times) &&
      Strategy_WassersteinSignal(closes, metrics);
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
   ...
   else
     {
      ...
      g_signal_valid =
         endpoints_valid &&
         Strategy_WassersteinSignal(closes, g_signal_metrics);
      if(!g_signal_valid)
         g_signal_state = "wasserstein_validation_failed";
      else if(g_signal_metrics.direction == 0)
         g_signal_state = "wasserstein_unqualified_or_neutral_flat";
      else if(g_signal_metrics.direction > 0)
         g_signal_state = "wasserstein_shift_wti_long";
      else if(g_signal_metrics.direction < 0)
         g_signal_state = "wasserstein_shift_wti_short";
      else
   ...
   QM_LogEvent(QM_INFO,
               ...
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"endpoint_count\":%d,\"return_count\":%d,\"block_size\":%d,\"assignment_count\":%d,\"tail_count\":%d,\"observed_wasserstein\":%.12f,\"comparison_epsilon\":%.16f,\"median_old\":%.12f,\"median_recent\":%.12f,\"median_delta\":%.12f,\"tail_fraction\":%.12f,\"returns\":\"%s\",\"oldest_close\":%.10f,\"newest_close\":%.10f,\"oldest_endpoint\":%I64d,\"newest_endpoint\":%I64d,\"state\":\"%s\"}",
                            ...
                            g_signal_metrics.observed_wasserstein,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41259 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41259_WTI_MWASSER_SHIFT";
   ...
   req.reason = (g_signal_metrics.direction > 0)
      ? "WTI_MWASSER_SHIFT_LONG"
      : "WTI_MWASSER_SHIFT_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41259 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         QM_FRIDAY_CLOSE_DISABLED,
          "Approved WTI Wasserstein-1 distribution-shift trend holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
       StringFormat("QM5_41259_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
                "{\"card\":\"QM5_41259\",\"ea\":\"wti-mwasser-shift-tr\"}");
