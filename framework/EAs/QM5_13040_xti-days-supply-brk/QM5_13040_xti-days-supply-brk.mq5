...
#property description "QM5_13040 XTI days-of-supply tight-cover breakout"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_13040 - XTI Days-of-Supply Tight-Cover Breakout
// -----------------------------------------------------------------------------
// D1 structural WTI sleeve:
//   - official EIA crude days-of-supply lineage, price-only runtime
//   - Wednesday/Thursday WPSR proxy bar, monthly capped
//   - Donchian breakout plus upper 126-D1 close-channel position
//   - ATR stop/target, SMA/time exits, no external runtime data
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 13040;
...
input double strategy_min_pullback_atr      = 0.40;
input double strategy_min_anchor_position   = 0.70;
...
bool Strategy_LoadSignalState(double &atr_last, int &signal_day_key, int &signal_month_key)
  {
   ...
   const int breakout_lookback = MathMax(10, strategy_breakout_lookback);
   ...
   const int history_needed = MathMax(anchor_lookback, MathMax(breakout_lookback + 1, pullback_lookback + 1));
   ...
   for(int i = 1; i <= breakout_lookback; ++i)
     {
      if(rates[i].high <= 0.0)
         return false;
      if(i == 1 || rates[i].high > prior_high)
   ...
   if(anchor_position < strategy_min_anchor_position)
      return false;

   double pullback_low_close = rates[1].close;
   for(int i = 1; i <= pullback_lookback; ++i)
     {
      if(rates[i].close <= 0.0)
         return false;
      if(rates[i].close < pullback_low_close)
         pullback_low_close = rates[i].close;
     }
   if(signal_close - pullback_low_close < strategy_min_pullback_atr * atr_last)
      return false;
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(strategy_report_start_dow < 0 || strategy_report_start_dow > 6 || strategy_report_end_dow < 0 || strategy_report_end_dow > 6)
      return true;
   if(strategy_breakout_lookback < 10 || strategy_anchor_lookback < 40 || strategy_pullback_lookback < 2)
      return true;
   if(strategy_min_pullback_atr <= 0.0 || strategy_min_anchor_position <= 0.0 || strategy_min_anchor_position >= 1.0)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_13040_XTI_DAYS_SUPPLY_BRK";
   ...
   req.reason = "XTI_DAYS_SUPPLY_BRK_LONG";
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_13040\",\"ea\":\"xti-days-supply-brk\"}");
