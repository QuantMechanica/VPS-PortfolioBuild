"""Deterministic reference checks for QM5_41070 WTI completed-week deceleration."""
...
def deceleration_signal(
    current_week_key: int,
    completed_newest_first: list[Bar],
    offset: timedelta,
) -> tuple[bool, int, float, float, tuple[float, float, float]]:
    """Mirror three completed week ends and strict same-sign magnitude deceleration."""
    ...
    if (
        old_return > 0.0
        and new_return > 0.0
        and abs(new_return) < abs(old_return)
    ):
        direction = 1
    elif (
        old_return < 0.0
...
class WeekDecelerationReferenceTest(unittest.TestCase):
    def test_positive_deceleration_is_long(self) -> None:
        current, now, bars = sample(126.0, 120.0, 100.0)
        decision, late, count, offset = decision_clock(current, now, bars)
        self.assertTrue(decision)
        self.assertFalse(late)
        self.assertEqual(count, 0)
        assert offset is not None
        valid, direction, new_value, old_value, endpoints = deceleration_signal(
        ...
        self.assertEqual(endpoints, (126.0, 120.0, 100.0))

    def test_negative_deceleration_is_short(self) -> None:
        current, now, bars = sample(95.0, 100.0, 120.0)
        offset = label_offset(current, now)
        assert offset is not None
        valid, direction, new_value, old_value, _ = deceleration_signal(
            20260824, bars, offset
        )
        self.assertTrue(valid)
        self.assertEqual(direction, -1)
    ...
    def test_opposed_zero_equal_and_accelerating_states_are_flat(self) -> None:
        current, now, bars = sample(110.0, 100.0, 105.0)
        offset = label_offset(current, now)
        assert offset is not None
        self.assertEqual(deceleration_signal(20260824, bars, offset)[1], 0)

        current, now, bars = sample(100.0, 100.0, 105.0)
        offset = label_offset(current, now)
        assert offset is not None
        valid, direction, new_value, _, _ = deceleration_signal(
            20260824, bars, offset
        )
        self.assertTrue(valid)
        ...
        current, now, bars = sample(400.0, 200.0, 100.0)
        offset = label_offset(current, now)
        assert offset is not None
        self.assertEqual(deceleration_signal(20260824, bars, offset)[1], 0)

        current, now, bars = sample(121.0, 110.0, 105.0)
        offset = label_offset(current, now)
        assert offset is not None
        self.assertEqual(deceleration_signal(20260824, bars, offset)[1], 0)

    def test_uniform_prior_date_label_normalization_and_grace(self) -> None:
        current, now, bars = sample(126.0, 120.0, 100.0, True)
        decision, late, count, offset = decision_clock(current, now, bars)
        self.assertTrue(decision)
        self.assertFalse(late)
        self.assertEqual(count, 0)
        ...
        assert offset is not None
        self.assertEqual(deceleration_signal(20260824, bars, offset)[1], 1)
    ...
    def test_nonconsecutive_or_current_week_endpoint_rejected(self) -> None:
        ...
        self.assertFalse(deceleration_signal(20260824, bars, offset)[0])
        _, _, bars = sample(126.0, 120.0, 100.0)
        bars.insert(0, Bar(datetime(2026, 8, 24, tzinfo=UTC), 111.0))
        self.assertFalse(deceleration_signal(20260824, bars, offset)[0])
