# QM5_12863_oilgold-rspread - Strategy Spec
...
This EA implements a low-frequency market-neutral oil/gold basket on
`XTIUSD.DWX` and `XAUUSD.DWX`. On each new D1 host bar it computes a rolling
return spread:

`log(XTI[t] / XTI[t-L]) - beta * log(XAU[t] / XAU[t-L])`

The current return spread is standardized against recent D1 history. A high
positive z-score means WTI has outperformed gold over the fixed return window,
so the basket sells WTI and buys gold. A high negative z-score buys WTI and
...
absolute XTI/XAU log price ratio, or `QM5_12605_cme-oilgold-brk`, which follows a
ratio-level breakout. This EA trades temporary D1 relative-return dislocation
between WTI and gold.
...
- Logical basket symbol: `QM5_12863_XTI_XAU_RSPREAD_D1`.
...
that lineage to define the XTI/XAU pair and tests a fixed-window return-spread
