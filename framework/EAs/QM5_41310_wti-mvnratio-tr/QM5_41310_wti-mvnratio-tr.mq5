...
#property description "QM5_41310 WTI Monthly Raw von Neumann Ratio Trend"
...
//   - reconstruct twenty consecutive completed monthly log returns
//   - qualify low successive variation with raw von Neumann eta below two
//   - follow the newest twelve-month return only when the path qualifies
...
input double strategy_eta_boundary         = 2.0;
...
input int    strategy_history_bars_d1      = 1000;
...
struct Strategy_SignalMetrics
  {
   ...
   double mean_return;
   double variance_sum;
   double successive_difference_sum;
   double eta;
   double momentum_12;
   double oldest_close;
   double newest_close;
...
bool Strategy_LoadMonthlyEndpoints(const int current_month_key,
                                   ...
                                   datetime &endpoint_times[])
  {
   ...
   const int required_endpoints = strategy_month_returns + 1;
   if(current_month_key <= 0 ||
      strategy_month_returns != 20 ||
      required_endpoints != 21 ||
   ...
   ArraySetAsSeries(bars, true);
   const int copied =
      CopyRates(_Symbol, // perf-allowed: one bounded twenty-one-endpoint scan behind a consumed monthly attempt or monthly restart validation.
                PERIOD_D1,
                1,
                strategy_history_bars_d1,
                bars);
   ...
   if(ArrayResize(reverse_closes, required_endpoints) !=
         required_endpoints ||
      ArrayResize(reverse_times, required_endpoints) !=
         required_endpoints ||
      ArrayResize(reverse_month_keys, required_endpoints) !=
         required_endpoints)
      return false;
   ...
   for(int index = 0;
       index < copied && endpoint_count < required_endpoints;
       ++index)
     {
   ...
   if(endpoint_count != required_endpoints || endpoint_count != 21)
      return false;
...
bool Strategy_RawVonNeumannSignal(const double &closes[],
                                  Strategy_SignalMetrics &metrics)
  {
   ...
   if(ArrayResize(monthly_returns, strategy_month_returns) !=
   ...
   for(int index = 0; index < metrics.endpoint_count; ++index)
     {
      if(closes[index] <= 0.0 || !MathIsValidNumber(closes[index]))
         return false;
      if(index >= strategy_month_returns)
         continue;
      const double close_ratio = closes[index + 1] / closes[index];
      if(close_ratio <= 0.0 || !MathIsValidNumber(close_ratio))
         return false;
      monthly_returns[index] = MathLog(close_ratio);
      if(!MathIsValidNumber(monthly_returns[index]))
         return false;
      return_sum += monthly_returns[index];
     }
   if(!MathIsValidNumber(return_sum))
      return false;

   metrics.mean_return = return_sum / (double)strategy_month_returns;
   ...
   for(int index = 0; index < strategy_month_returns; ++index)
     {
      const double centered = monthly_returns[index] - metrics.mean_return;
      metrics.variance_sum += centered * centered;
      if(index >= momentum_start)
         metrics.momentum_12 += monthly_returns[index];
      if(index > 0)
        {
         const double delta =
            monthly_returns[index] - monthly_returns[index - 1];
         metrics.successive_difference_sum += delta * delta;
        }
     }

   if(!MathIsValidNumber(metrics.variance_sum) ||
      !MathIsValidNumber(metrics.successive_difference_sum) ||
      !MathIsValidNumber(metrics.momentum_12) ||
      metrics.variance_sum <= strategy_variance_floor ||
      metrics.successive_difference_sum < 0.0)
      return false;

   metrics.eta =
      metrics.successive_difference_sum / metrics.variance_sum;
   if(!MathIsValidNumber(metrics.eta) || metrics.eta < 0.0)
      return false;
                                        ...
                                        int &direction)
  {
   ...
   const bool valid =
      Strategy_LoadMonthlyEndpoints(current_month_key,
                                    ...
                                    closes,
                                    endpoint_times) &&
      Strategy_RawVonNeumannSignal(closes, metrics);
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
   ...
   else
     {
      ...
      g_signal_valid =
         endpoints_valid &&
         Strategy_RawVonNeumannSignal(closes, g_signal_metrics);
      if(!g_signal_valid)
         g_signal_state = "raw_von_neumann_validation_failed";
      else if(g_signal_metrics.direction == 0)
         g_signal_state = "raw_von_neumann_nonqualifying_or_tie_flat";
      else if(g_signal_metrics.direction > 0)
         g_signal_state = "raw_von_neumann_wti_long";
      else if(g_signal_metrics.direction < 0)
         g_signal_state = "raw_von_neumann_wti_short";
      else
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"endpoint_count\":%d,\"return_count\":%d,\"mean_return\":%.12f,\"variance_sum\":%.12f,\"successive_difference_sum\":%.12f,\"eta\":%.12f,\"momentum_12\":%.12f,\"oldest_close\":%.10f,\"newest_close\":%.10f,\"oldest_endpoint\":%I64d,\"newest_endpoint\":%I64d,\"state\":\"%s\"}",
                            ...
                            g_signal_metrics.successive_difference_sum,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41310 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41310_WTI_MVNRATIO_TREND";
   ...
   req.reason = (g_signal_metrics.direction > 0)
      ? "WTI_MVNRATIO_TREND_LONG"
      : "WTI_MVNRATIO_TREND_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41310 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI raw von Neumann monthly trend holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41310_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41310\",\"ea\":\"wti-mvnratio-tr\"}");
