...
#property description "QM5_41372 XTI/XNG Weekly Relative-Vote Flip Continuation"
...
// -----------------------------------------------------------------------------
// Reconstruct five synchronized week-end XTI/XNG close pairs for the five
...
double   g_xti_week_close[5]; // newest through oldest completed week.
double   g_xng_week_close[5];
int      g_week_key[5];
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41372 && qm_magic_slot_offset == 0 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XTIXNG_WRELVOTE_FLIP_ATTEMPT_WEEK", qm_ea_id);
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41372_XTIXNG_WRELVOTE_FLIP_CONT";
...
bool Strategy_LoadRelativeVoteFlip(const int current_week,
                                   int &direction)
  {
   ...
   for(int interval = 0; interval < 4; ++interval)
     {
      ...
      const double xti_return =
         MathLog(g_xti_week_close[newer_slot] /
                 g_xti_week_close[older_slot]);
      const double xng_return =
         MathLog(g_xng_week_close[newer_slot] /
                 g_xng_week_close[older_slot]);
      g_relative_return[interval] = xti_return - xng_return;
   ...
   if(g_old_vote < 0 && g_new_vote > 0)
      direction = 1; // New WTI majority: buy XTI, sell XNG.
   else if(g_old_vote > 0 && g_new_vote < 0)
      direction = -1; // New XNG majority: sell XTI, buy XNG.
   return true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41372_XTIXNG_WRELVOTE_FLIP_CONT_HOST";
   ...
   if(Strategy_OpenOwnedPositionCount() > 0 ||
   ...
   else
     {
      ...
      if(!signal_valid)
         ...
         g_signal_state = "new_xng_majority_followed";
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XTI/XNG weekly relative-vote flip continuation package exits at the next broker week"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41372\",\"ea\":\"xtixng-wrelvote-flip-cont\"}");
