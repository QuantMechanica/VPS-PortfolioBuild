...
#property description "QM5_10217 TradingView price-volume breakout"
...
input int    strategy_trend_ma_period   = 50;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   for(int shift = 2; shift < 2 + strategy_breakout_lookback; ++shift)
     {
      ...
      prior_max_volume = MathMax(prior_max_volume, v);
     }

   int signal = 0;
   if(close1 > prior_high && volume1 > prior_max_volume && close1 > trend_ma)
      signal = 1;
   else if(close1 < prior_low && volume1 > prior_max_volume && close1 < trend_ma)
      signal = -1;
   if(signal == 0)
      return false;
   ...
   const double structure_distance = (signal > 0) ? MathAbs(entry - low1)
                                                  : MathAbs(high1 - entry);
   double risk_distance = MathMax(structure_distance, strategy_atr_sl_mult * atr);
   risk_distance = MathMin(risk_distance, strategy_atr_cap_mult * atr);
   if(risk_distance <= 0.0)
      return false;

   req.type = side;
   req.price = 0.0;
   req.sl = (signal > 0) ? entry - risk_distance : entry + risk_distance;
   req.tp = QM_TakeRR(_Symbol, side, entry, req.sl, strategy_tp_r_mult);
   req.reason = (signal > 0) ? "TV_PRICE_VOL_BRK_LONG" : "TV_PRICE_VOL_BRK_SHORT";
   req.symbol_slot = qm_magic_slot_offset;
   req.expiration_seconds = 0;
...
void Strategy_ManageOpenPosition()
  {
   // Card specifies fixed protective bracket exits only.
  }
...
bool Strategy_ExitSignal()
  {
   // Opposite-signal closes are handled inside the closed-bar entry hook before
   // any replacement entry is requested.
   return false;
  }
