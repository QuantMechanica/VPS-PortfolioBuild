...
#property description "QM5_41032 WTI Weekly Public/Professional Flow Divergence"
...
//   - enter next Monday only when the completed-flow signs oppose, following session flow
...
bool Strategy_LoadFlowDivergence(const datetime current_bar_time,
                                ...
                                int &direction)
  {
   ...
   if(session_flow > 0.0 && overnight_flow < 0.0)
      direction = 1;
   else if(session_flow < 0.0 && overnight_flow > 0.0)
      direction = -1;
   return true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   int direction = 0;
   if(!Strategy_LoadFlowDivergence(g_strategy_d1_bar_time,
                              broker_now,
                              overnight_flow,
                              session_flow,
                              direction))
   ...
   if(direction > 0)
     {
      req.type = QM_BUY;
      req.reason = "WTI_FLOW_DIV_LONG";
     }
   else if(direction < 0)
     {
      req.type = QM_SELL;
      req.reason = "WTI_FLOW_DIV_SHORT";
     }
   else
      return false;
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI weekly flow-divergence card uses Friday 21 as ordinary exit"))
      return INIT_FAILED;
   ...
   g_attempt_state_key =
      StringFormat("QM5_41032_FLOW_DIV_WEEK_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41032\",\"ea\":\"wti-flow-div\"}");
