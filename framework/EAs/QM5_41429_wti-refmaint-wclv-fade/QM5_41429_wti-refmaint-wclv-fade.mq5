...
#property description "QM5_41429 WTI Refinery-Maintenance Weekly Close-Location Reversion"
...
//   - require positive parent-close to newest-close return
//   - require newest close strictly in its own upper range tercile
...
input double strategy_clv_upper              = 0.6666666666666667;
...
bool Strategy_LoadCloseLocationSignal(const int current_week_key,
                                          ...
                                          double &close_location)
  {
   ...
   if(weekly_return > strategy_return_epsilon &&
      close_location > strategy_clv_upper)
      direction = -1;
   return true;
  }
   ...
   if(Strategy_WeekAlreadyEntered(g_decision_week_key))
   ...
   else
     {
      ...
      if(!g_signal_valid)
         ...
         g_signal_state = "positive_return_upper_tercile_short";
      else if(g_weekly_return <= strategy_return_epsilon)
         g_signal_state = "nonpositive_return_flat";
      else if(g_close_location == strategy_clv_upper)
         g_signal_state = "close_location_equality_flat";
      else
         g_signal_state = "positive_return_not_upper_tercile_flat";
     }
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(strategy_symbol == "" ||
      ...
      MathAbs(strategy_clv_upper - 0.6666666666666667) > 1.0e-12 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41429_WTI_REFMAINT_WCLV_FADE";
   ...
   req.type = QM_SELL;
   req.reason = "WTI_REFMAINT_WCLV_FADE_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(strategy_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41429 ||
      qm_magic_slot_offset != 0)
   ...
   g_attempt_state_key =
      StringFormat("QM5_41429_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41429\",\"ea\":\"wti-refmaint-wclv-fade\"}");
