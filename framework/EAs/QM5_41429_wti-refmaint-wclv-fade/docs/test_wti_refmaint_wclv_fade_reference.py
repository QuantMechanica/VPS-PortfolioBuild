"""Deterministic checks for QM5_41429 refinery-maintenance weekly reversion."""
...
def signal(current_anchor: date, newest_first: list[Bar]) -> tuple[int, float, float]:
    ...
    direction = -int(weekly_return > 0.0 and close_location > CLV_UPPER)
...
class WtiRefineryMaintenanceCloseLocationReferenceTest(unittest.TestCase):
    def setUp(self) -> None:
        self.current = date(2026, 3, 16)
        self.new_anchor = self.current - timedelta(days=7)
        self.parent_anchor = self.current - timedelta(days=14)

    def bars(self, newest_close: float = 82.0, parent_close: float = 80.0) -> list[Bar]:
        newest = package(self.new_anchor, [79.0, 79.5, 80.0, 81.0, newest_close], 76.0, 84.0)
        parent = package(self.parent_anchor, [82.0, 81.5, 81.0, 80.5, parent_close], 75.0, 82.0)
        return newest + parent

    def test_positive_return_and_upper_tercile_is_short(self) -> None:
        direction, weekly_return, close_location = signal(self.current, self.bars())
        self.assertEqual(direction, -1)
        self.assertGreater(weekly_return, 0.0)
        self.assertGreater(close_location, CLV_UPPER)

    def test_positive_return_below_upper_tercile_is_flat(self) -> None:
        self.assertEqual(signal(self.current, self.bars(newest_close=81.0, parent_close=80.0))[0], 0)

    def test_nonpositive_return_is_flat(self) -> None:
        self.assertEqual(signal(self.current, self.bars(newest_close=79.0, parent_close=80.0))[0], 0)
