...
type: strategy
strategy_id: WILLIAMS-MOP-XNG-MFLOWDIV-2026_S01
variant_id: WILLIAMS-MOP-XNG-MFLOWDIV-2026_S01
source_id: WILLIAMS-MOP-XNG-MFLOWDIV-2026
ea_id: QM5_41037
slug: xng-mflow-div
status: APPROVED
execution_contract_ref: strategy-seeds/cards/approved/QM5_41037_xng-mflow-div_card.md
execution_contract_status: APPROVED
created: 2026-08-17
created_by: Research+Development
last_updated: 2026-08-17
g0_status: APPROVED
g0_decision: decisions/2026-08-17_xng_monthly_flow_divergence_g0.md
...
source_citations:
  ...
  - type: peer_reviewed_trading_paper
    ...
    quality_tier: A
    role: explicit_natural_gas_carrier_and_one_month_formation_hold_scope
strategy_mechanic: exact-immediately-completed-broker-month-xng-close-open-and-open-close-log-flow-strict-sign-opposition-follow-session-entry-first-new-month-hold-to-next-month
sources:
  - "[[sources/WILLIAMS-MOP-XNG-MFLOWDIV-2026]]"
concepts:
  - "[[concepts/price-flow-decomposition]]"
  - "[[concepts/monthly-flow-divergence]]"
  - "[[concepts/monthly-return-reconciliation]]"
indicators:
  - "[[indicators/completed-log-return]]"
  - "[[indicators/broker-calendar]]"
  - "[[indicators/atr-risk-stop]]"
strategy_type_flags: [commodity, energy, natural-gas, price-flow-decomposition, flow-divergence, monthly-entry, monthly-hold, atr-hard-stop, low-frequency, symmetric-long-short]
...
hard_rules_at_risk: [immediately_completed_broker_month, normalized_energy_label, completed_close_open_endpoints, strict_flow_sign_opposition, session_following_direction, flow_reconciliation, no_current_bar_leakage, monthly_decision_clock, monthly_attempt_state, no_late_restart_entry, risk_mode_dual, friday_close_disabled, cfd_futures_basis, q02_frequency_floor, portfolio_correlation]
g0_approval_reasoning: "R1 complete OWNER-supplied Tier-A flow-decomposition extraction plus complete-read peer-reviewed XNG one-month carrier lineage with disclosed conjunction risk; R2 exact month, endpoints, opposition, reconciliation, session-following direction, timing, retry, risk, and lifecycle; R3 native registered XNG D1 only with the inferred energy-session offset disclosed; R4 deterministic arithmetic without banned signal or trained logic; canonical dedup found no exact identity and manual review separated the WTI carrier port plus XNG monthly-return, weekly-volume, and incumbent RSI families."
---

# QM5_41037 XNG Monthly Public/Session Flow Divergence

## Hypothesis

A completed XNG month whose close-to-open public flow opposes its open-to-close
session flow contains an observable disagreement between overnight repricing
and the liquid session. Following the completed session-flow sign through the
next broker month may isolate professional information while rejecting months
when both channels already agree.

This is a falsifiable price-flow and calendar translation. The sources do not
test the exact monthly opposition gate, session-following direction,
continuous CFD, fixed entry clock, ATR stop, or QM portfolio.

## Source Traceability And Claim Boundary

The sole governed composite packet is
`strategy-seeds/sources/WILLIAMS-MOP-XNG-MFLOWDIV-2026/source.md`, approved
before card extraction in
`decisions/2026-08-17_xng_monthly_flow_divergence_source_approval.md` at commit
`29856a0d7`.
...
they do not validate this component-opposition gate or session direction.

The exact completed-month selection, component log sums, strict opposition,
session-following direction, telescoping reconciliation, normalized label
...
- Neither source defines monthly component opposition, session-following
  direction, a continuous-CFD label policy, an ATR stop, a spread ceiling, an
  attempt ledger, or a fixed cash risk budget.

## QM Interpretations

- The strict opposition-only eligibility state and direction from completed
  session flow are pre-result QM mechanizations, not source findings.
- `XNGUSD.DWX` is a continuous-CFD carrier rather than a rolled NYMEX futures
  ...
  The session-offset registry marks XNG's value as inferred from the measured
  XTI energy sibling, so Q02 owns this explicit labeling risk.
...
The deterministic pre-allocation checker scanned 4,524 EA-registry rows and
620 root cards. It found no exact match and raised the expected WTI monthly-
flow family. Manual review fixes the load-bearing boundaries:

- `QM5_41035_wti-mflow-div` has the same information-clock mechanic on WTI.
  This card's exact XNG carrier is a distinct economic identity expressly
  permitted by the OWNER mission as a second XNG edge; no WTI magic, position,
  result, or source-performance claim transfers.
- `QM5_20204_xng-tsmom1m` follows every nonzero completed-month total. This
  card rejects agreement months and follows session flow rather than the
...
- `QM5_20054_xng-1m-contr` fades every nonzero completed-month total. This
  card is flat on agreement and never uses total-return direction.
- `QM5_21504_xng-flowrev` and `QM5_21520_xng-flow-mom` use five-close weekly
  returns gated by upper or lower native tick-volume ranks. This card uses no
  volume, decomposes every completed prior-month open/close interval, and
  renews only at month boundaries.
...
- Timeframe: D1; magic slot 0; planned magic `410370000`.
...
- Signal: opposite strict signs for completed prior-month close-to-open and
  open-to-close log-return sums; direction equals session-flow sign.
...
session_flow > 0 and overnight_flow < 0 => BUY
session_flow < 0 and overnight_flow > 0 => SELL
otherwise                                => flat
```
...
11. BUY only for positive session flow and negative overnight flow. SELL only
    for negative session flow and positive overnight flow. Agreement, exact
    zero, invalid arithmetic, failed reconciliation, or a missing endpoint
    consumes the month flat. Magnitude and total-flow sign never alter the
    direction or risk budget.
...
- Fail closed on invalid label offset, date/month identity, history, OHLC,
  timestamps, component opposition, session direction, reconciliation,
...
- trade_entry: monthly flow decomposition, strict opposition,
  reconciliation, session-following symmetric direction, ATR hard stop,
  slot 0.
...
- direction different from the completed session-flow sign;
...
2. Run deterministic reference cases covering zero and `+1` label offsets,
   ...
   component arithmetic, opposition/agreement/zero, reconciliation, session
   direction including overnight-dominant cases, attempt state, risk, and
   rollover.
