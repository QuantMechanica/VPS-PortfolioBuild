...
#property description "QM5_12756 Edge Lab USDCHF USDCAD Cointegration"
...
input double strategy_beta              = 0.55;
...
bool Strategy_IsHostSymbol()
  {
   return (_Symbol == g_leg_usdchf || _Symbol == g_leg_usdcad);
  }
...
bool Strategy_EnsureBasketScope()
  {
   ...
   // Both legs are USD-base majors; select both legs explicitly so the host run
   // can warm and trade the complete basket.
   string allowed[2] = {"USDCHF.DWX", "USDCAD.DWX"};
   for(int i = 0; i < 2; ++i)
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(!Strategy_EnsureBasketScope())
      return false;
   if(!QM_SymbolAssertOrLog(g_leg_usdchf) || !QM_SymbolAssertOrLog(g_leg_usdcad))
      return false;
   ...
   for(int i = 0; i < lookback; ++i)
     {
      if(usdchf[i] <= 0.0 || usdcad[i] <= 0.0)
         return false;
      spreads[i] = MathLog(usdchf[i]) - strategy_beta * MathLog(usdcad[i]);
...
bool Strategy_OpenPair(const int spread_direction)
  {
   ...
   const double usdcad_weight = MathAbs(strategy_beta);
   const double weight_sum = usdchf_weight + usdcad_weight;
   if(weight_sum <= 0.0)
      return false;

   const bool long_spread = (spread_direction > 0);
   const QM_OrderType usdchf_type = long_spread ? QM_BUY : QM_SELL;
   const QM_OrderType usdcad_type = long_spread ? QM_SELL : QM_BUY;
   const string reason = long_spread ? "QM5_12756_LONG_SPREAD_Z_LT_NEG_ENTRY"
                                     : "QM5_12756_SHORT_SPREAD_Z_GT_POS_ENTRY";

   bool usdchf_ok = Strategy_OpenLeg(g_leg_usdchf, usdchf_type, usdchf_weight, weight_sum, reason);
   bool usdcad_ok = Strategy_OpenLeg(g_leg_usdcad, usdcad_type, usdcad_weight, weight_sum, reason);
   if(usdchf_ok && usdcad_ok)
     {
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12756_PAIR_HOST";
