...
int Strategy_Direction()
  {
   ...
   if(fast_1 > slow_1 && slow_1 > slow_2)
      return 1;
   if(slow_1 < fast_1 && slow_1 < slow_2)
      return -1;
   return 0;
  }

bool Strategy_FindPosition(ENUM_POSITION_TYPE &position_type, ulong &ticket)
  {
...
double Strategy_CatastrophicStop(const QM_OrderType side, const double entry_price)
  {
   if(entry_price <= 0.0 || strategy_atr_period <= 0 || strategy_atr_sl_mult <= 0.0)
      return 0.0;
   ...
   return QM_StopATRFromValue(_Symbol, side, entry_price, stop_distance, 1.0);
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.reason = (direction > 0) ? "WEISS_ICHI2_LONG" : "WEISS_ICHI2_SHORT";
   return true;
  }
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      const double stop = Strategy_CatastrophicStop(side, entry_price);
      if(stop > 0.0)
...
bool Strategy_ExitSignal()
  {
   ...
   ulong ticket = 0;
   if(!Strategy_FindPosition(position_type, ticket))
      return false;
