...
#property description "QM5_41048 XNG Thursday Event / Slow-Trend Agreement"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41048 - XNG Thursday Event / Slow-Trend Agreement
// -----------------------------------------------------------------------------
// Structural D1 natural-gas sleeve:
//   - require exact completed Tuesday-Wednesday-Thursday broker sessions
//   - compute the completed Thursday close-to-close event return
//   - compute a separate 252-session trend ending at Wednesday close
//   - enter Friday only when the two nonzero returns have the same sign,
//     continuing completed Thursday/slow-trend agreement
//   - consume one exact-Friday attempt before every fallible entry gate
//   - hold through the weekend and flatten at the first later D1 boundary
...
input int    strategy_trend_lookback_d1    = 252;
...
bool Strategy_PrimeLateSignalAttach()
  {
   ...
   // Consume the initialization edge and persist the missed Friday decision.
...
bool Strategy_LoadThursdayTrendAgreement(const datetime current_bar_time,
                                         const datetime broker_now,
                                         double &event_return,
                                         double &slow_trend,
                                         int &direction)
  {
   ...
   if(!Strategy_IsFriday(broker_now) ||
      ...
      strategy_trend_lookback_d1 != 252)
      return false;
   ...
   // Newest completed bar is Thursday, followed by Wednesday and Tuesday.
   // Index lookback+1 is exactly 252 D1 intervals before Wednesday (index 1).
   const int required_bars = strategy_trend_lookback_d1 + 2;
   MqlRates bars[];
   ArraySetAsSeries(bars, true);
   const int copied = CopyRates(_Symbol, // perf-allowed: new-D1-bar gated by OnTick
                                PERIOD_D1,
                                1,
                                required_bars,
                                bars);
   ...
   // Apply the current bar's one uniform energy-label offset to the three
   // event-clock sessions. Exact dates and bounded gaps reject holiday shifts
   // or substitutions while tolerating one-hour broker DST transitions.
   ...
   const int slow_index = strategy_trend_lookback_d1 + 1;
   if(slow_index >= copied ||
      bars[slow_index].time <= 0 ||
      bars[slow_index].time >= bars[1].time)
      return false;

   // The event return uses Thursday and Wednesday closes. The slow state ends
   // at Wednesday and therefore cannot contain the Thursday event move.
   if(bars[0].close <= 0.0 ||
      !MathIsValidNumber(bars[0].close) ||
      bars[1].close <= 0.0 ||
      !MathIsValidNumber(bars[1].close) ||
      bars[slow_index].close <= 0.0 ||
      !MathIsValidNumber(bars[slow_index].close))
      return false;

   event_return =
      MathLog(bars[0].close / bars[1].close);
   slow_trend =
      MathLog(bars[1].close / bars[slow_index].close);
   if(!MathIsValidNumber(event_return) ||
      !MathIsValidNumber(slow_trend))
      return false;

   // Strict multiplication implements the card's nonzero agreement gate.
   if(event_return * slow_trend <= 0.0)
      return true;

   if(slow_trend > 0.0)
      direction = 1;
   else if(slow_trend < 0.0)
      direction = -1;
   return true;
  }
   ...
   for(int index = PositionsTotal() - 1; index >= 0; --index)
     {
      ...
      // later than the Friday entry date (normally Monday open).
      if(!should_close &&
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "XNG_THU_TREND_AGREEMENT";
   ...
   double event_return = 0.0;
   double slow_trend = 0.0;
   int direction = 0;
   if(!Strategy_LoadThursdayTrendAgreement(g_strategy_d1_bar_time,
                                           broker_now,
                                           event_return,
                                           slow_trend,
                                           direction))
   ...
   if(direction > 0)
     {
      req.type = QM_BUY;
      req.reason = "XNG_THU_TREND_AGREEMENT_LONG";
     }
   else if(direction < 0)
     {
      req.type = QM_SELL;
      req.reason = "XNG_THU_TREND_AGREEMENT_SHORT";
     }
   else
      return false;
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"date\":%d,\"event_return\":%.12e,\"slow_trend\":%.12e,\"direction\":%d}",
                            date_key,
                            event_return,
                            slow_trend,
                            direction));
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XNG Thursday event/slow-trend agreement card holds Friday to the next D1 boundary"))
      return INIT_FAILED;
   ...
   g_attempt_state_key =
      StringFormat("QM5_41048_THU_TREND_AGREEMENT_FRIDAY_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41048\",\"ea\":\"xng-thu-trend-agree\"}");
