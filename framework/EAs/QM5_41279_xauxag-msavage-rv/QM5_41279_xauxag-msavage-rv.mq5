...
#property description "QM5_41279 XAU/XAG Monthly Exact Savage-Score Reversion"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_41279 - XAU/XAG Monthly Exact Centered-Savage-Score Reversion
// -----------------------------------------------------------------------------
...
// map pooled ranks to the locked centered Savage harmonic scores, and enumerate
...
input int    strategy_score_denominator       = 27720;
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41279 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_MSAVAGE_RV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41279_XAUXAG_MSAVAGE_RV";
...
bool Strategy_SavageScoreNumeratorForRank(const int rank,
                                           int &numerator)
  {
   numerator = 0;
   // Exact centered Savage scores a(r)=sum[j=1..r]1/(12-j+1)-1,
   // represented on their least common denominator 27720. Keeping the
   // lattice integral prevents platform-dependent tail-boundary drift.
   switch(rank)
     {
      case 1:  numerator = -25410; break;
      case 2:  numerator = -22890; break;
      case 3:  numerator = -20118; break;
      case 4:  numerator = -17038; break;
      case 5:  numerator = -13573; break;
      case 6:  numerator =  -9613; break;
      case 7:  numerator =  -4993; break;
      case 8:  numerator =    551; break;
      case 9:  numerator =   7481; break;
      case 10: numerator =  16721; break;
      case 11: numerator =  30581; break;
      case 12: numerator =  58301; break;
      default: return false;
     }
   return true;
  }

bool Strategy_SavageScoreInvariant()
  {
   if(strategy_score_denominator != 27720)
      return false;
   int total = 0;
   for(int rank = 1; rank <= 12; ++rank)
     {
      int numerator = 0;
      if(!Strategy_SavageScoreNumeratorForRank(rank, numerator))
         return false;
      total += numerator;
     }
   return (total == 0);
...
bool Strategy_SavageFromLabels(const int &labels[],
                                double &score,
                                int &score_numerator)
  {
   score = 0.0;
...
bool Strategy_LoadMonthlySavageSignal(
      ...
      int &direction)
  {
   ...
   if(!Strategy_SavageFromLabels(pooled_labels,
                                 observed_score,
                                 observed_score_numerator))
      return false;

   const double tail_epsilon = strategy_relative_epsilon;
   ...
   for(int mask = 0; mask < (1 << strategy_return_count); ++mask)
     {
      ...
      int permutation_score_numerator = 0;
      if(!Strategy_SavageFromLabels(permutation_labels,
                                    permutation_score,
                                    permutation_score_numerator))
         return false;
      ++assignment_count;
      if(MathAbs(permutation_score) + tail_epsilon >=
         MathAbs(observed_score))
         ++tail_count;
     }
   ...
   if(!qualifies || MathAbs(observed_score) <= strategy_relative_epsilon)
      return true;

   // direction > 0 means BUY XAU / SELL XAG. A positive recent centered
   // Savage score is faded short XAU; a negative score is faded long XAU.
   // Neither score sign nor magnitude scales the fixed package risk.
   direction = (observed_score > 0.0) ? -1 : 1;
   return true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41279_XAUXAG_MSAVAGE_RV_HOST";
   ...
   int tail_count = 0;
   int observed_score_numerator = 0;
   int direction = 0;
   ...
   string label_path = "";
   const bool valid =
      Strategy_LoadMonthlySavageSignal(
         g_signal_month_key,
         month_count,
   ...
   string state = "monthly_savage_signal_failed";
   if(valid && direction > 0)
      state = "qualified_negative_savage_score_fade_long_xau";
   else if(valid && direction < 0)
      state = "qualified_positive_savage_score_fade_short_xau";
   ...
   else if(valid && MathAbs(observed_score) <= strategy_relative_epsilon)
      state = "zero_savage_score_flat";
   else if(valid)
      state = "nonqualifying_exact_savage_flat";

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"month_count\":%d,\"newest_endpoint_time\":%I64d,\"observed_score\":%.12e,\"observed_score_numerator\":%d,\"assignment_count\":%d,\"tail_count\":%d,\"tie_detected\":%s,\"direction\":%d,\"ratios\":\"%s\",\"returns\":\"%s\",\"pooled_labels\":\"%s\",\"state\":\"%s\"}",
                            g_signal_month_key,
                            month_count,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG monthly exact centered-Savage-score reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41279\",\"ea\":\"xauxag-msavage-rv\"}");
