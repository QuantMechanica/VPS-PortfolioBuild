...
#property description "QM5_41237 WTI Same-Calendar Five-Sample Cauchy Location"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41237 - WTI Same-Calendar Five-Sample Cauchy-Weighted Location
// -----------------------------------------------------------------------------
...
//   - freeze 1.4826*MAD and execute exactly 32 Cauchy derivative-weight updates
...
input int    strategy_cauchy_iterations   = 32;
...
// -----------------------------------------------------------------------------
// Exact completed-month endpoint and five-sample-Cauchy helpers.
// -----------------------------------------------------------------------------
...
bool Strategy_CauchySignal(const double &observations[],
...
bool Strategy_LoadCauchySignal(
   ...
   int &direction)
  {
   ...
   return Strategy_CauchySignal(observations,
                                sample_count,
                                location_value,
                                direction);
  }
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   // Consume before history, Cauchy arithmetic, news, spread, quote,
   ...
   else
     {
      g_signal_valid =
          Strategy_LoadCauchySignal(
            ...
            g_signal_direction);
      if(!g_signal_valid)
         g_signal_state = "endpoint_sample_or_cauchy_failed";
      else if(g_signal_direction > 0)
         g_signal_state = "fixed_cauchy_long";
      else if(g_signal_direction < 0)
         g_signal_state = "fixed_cauchy_short";
      else
         g_signal_state = "fixed_cauchy_epsilon_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"valid\":%s,\"samples\":%d,\"fixed_cauchy\":%.12g,\"direction\":%d,\"state\":\"%s\"}",
                            g_decision_month_key,
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(!QM_InputRequireLong("qm_ea_id", qm_ea_id, 41237) ||
      ...
      !QM_InputRequireLong("strategy_cauchy_iterations", strategy_cauchy_iterations, 32) ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "WTI_SAMECAL_CAUCHY5";
   ...
   if(g_signal_direction > 0)
     {
      req.type = QM_BUY;
      req.reason = "WTI_SAMECAL_CAUCHY5_LONG";
     }
   else
     {
      req.type = QM_SELL;
      req.reason = "WTI_SAMECAL_CAUCHY5_SHORT";
     }
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41237 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI same-calendar five-sample Cauchy-location card holds to the next broker month"))
      return INIT_FAILED;
   ...
   g_attempt_state_key =
      StringFormat("QM5_41237_SAMECAL_CAUCHY5_MONTH_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41237\",\"ea\":\"wti-samecal-cauchy5\"}");
