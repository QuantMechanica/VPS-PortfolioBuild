# QM5_41124_wti-mrms-coherence-mom - Strategy Spec
...
**Source:** `MOP-WTI-MRMS-COHERENCE-MOM-2026`
...
on every session in the completed month. Let `N` be their signed sum, `Q` their
sum of squares, and `C=abs(N)/sqrt(n*Q)`. Require endpoint identity, finite
arithmetic, positive squared path, and bounded `C`. Buy only when `C>=0.16`
and `N>0`; sell only when `C>=0.16` and `N<0`. Every other valid or malformed
state consumes the month flat.

The position follows the coherent direction for one broker month, using one
...
- Symmetric direct-WTI monthly structural continuation after path-coherence
  qualification.
...
formation/hold, and explicit WTI membership. Within-month mean-to-RMS
coherence is a disclosed QM hypothesis; no source or sibling result transfers
...
|---|---|---|---|
| v0 | 2026-08-23 | approved source build | source approval `04f9f9b01`; card `4e1500023`; governed magic `411240000` |
