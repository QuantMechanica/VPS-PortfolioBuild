...
#property description "QM5_41124 WTI Completed-Month Mean-to-RMS Coherence Momentum"
...
double   g_squared_path            = 0.0;
double   g_endpoint_return         = 0.0;
double   g_coherence               = 0.0;
...
bool Strategy_LoadCoherenceSignal(const int current_month_key,
                                  const int label_offset,
                                  int &direction,
                                  int &month_sessions,
                                  ...
                                  double &coherence)
  {
   ...
   for(int index = month_sessions - 1; index >= 0; --index)
     {
      ...
      squared_path += daily_return * daily_return;
   ...
   const double denominator =
      MathSqrt((double)month_sessions * squared_path);
   if(denominator <= 0.0 || !MathIsValidNumber(denominator))
      return false;
   coherence = MathAbs(net_return) / denominator;
   if(!MathIsValidNumber(coherence) || coherence < 0.0 ||
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadCoherenceSignal(g_decision_month_key,
      ...
      else if(g_squared_path == 0.0)
         g_signal_state = "zero_squared_path_flat";
      else if(g_net_return == 0.0)
         g_signal_state = "zero_net_flat";
      else if(g_signal_direction > 0)
         g_signal_state = "coherent_completed_month_long";
      else if(g_signal_direction < 0)
         g_signal_state = "coherent_completed_month_short";
      else
         g_signal_state = "below_coherence_threshold_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"month_sessions\":%d,\"boundary_close\":%.8f,\"final_close\":%.8f,\"net_return\":%.12e,\"squared_path\":%.12e,\"endpoint_return\":%.12e,\"coherence\":%.12e,\"threshold\":%.12e,\"state\":\"%s\"}",
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41124 ||
   ...
   if(strategy_entry_grace_minutes != 180 ||
      ...
      MathAbs(strategy_coherence_threshold - 0.16) > 0.000000000001 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41124_WTI_MRMS_COHERENCE_MOM";
   ...
   req.reason = (g_signal_direction > 0)
      ? "WTI_MRMS_COHERENCE_MOM_LONG"
      : "WTI_MRMS_COHERENCE_MOM_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41124 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI completed-month RMS-coherence position holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41124_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41124\",\"ea\":\"wti-mrms-coherence-mom\"}");
