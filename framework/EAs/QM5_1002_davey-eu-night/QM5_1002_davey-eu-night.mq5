...
input int    ea_id_input              = 1002;
input int    magic_slot_offset_input  = 0;
...
input group "News"
input QM_NewsMode news_mode_input     = QM_NEWS_PAUSE;

input group "Friday Close"
input bool   friday_close_enabled_input     = true;
input int    friday_close_hour_broker_input = 21;
...
int FrameworkMagic()
  {
   ...
   return QM_Magic(ea_id_input, magic_slot_offset_input);
  }
...
bool Strategy_EntrySignal(const datetime now_time, QM_EntryRequest &req)
  {
   ZeroMemory(req);
   req.symbol_slot = magic_slot_offset_input;
...
int OnInit()
  {
   if(!QM_FrameworkInit(ea_id_input,
                        magic_slot_offset_input,
                        RISK_PERCENT,
                        RISK_FIXED,
                        PORTFOLIO_WEIGHT,
                        news_mode_input,
                        friday_close_enabled_input,
                        friday_close_hour_broker_input))
      return INIT_FAILED;
...
void OnTick()
  {
   if(!QM_KillSwitchCheck())
      return;
   if(!QM_NewsAllowsTrade(_Symbol, TimeCurrent(), news_mode_input))
      return;
