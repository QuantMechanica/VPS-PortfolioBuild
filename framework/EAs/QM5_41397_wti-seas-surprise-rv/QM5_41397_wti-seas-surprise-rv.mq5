...
#property description "QM5_41397 WTI Monthly Standardized Seasonal-Surprise Reversion"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41397 - WTI Monthly Standardized Seasonal-Surprise Reversion
// -----------------------------------------------------------------------------
// Structural low-frequency natural-gas sleeve:
...
//   - standardize by the n-1 sample deviation and fade only |z| > 0.50+1e-10
...
double   g_signal_surprise_z      = 0.0;
...
// -----------------------------------------------------------------------------
// Exact completed-month endpoint and standardized seasonal-surprise helpers.
// -----------------------------------------------------------------------------
...
bool Strategy_SeasonalSurpriseSignal(const double realized_return,
                                     ...
                                     int &direction)
  {
   ...
   surprise_z = (realized_return - seasonal_mean) / seasonal_sd;
   if(!MathIsValidNumber(surprise_z))
      return false;
...
bool Strategy_LoadSeasonalSurpriseSignal(const datetime decision_bar_time,
                                         ...
                                         int &direction)
  {
   ...
   return Strategy_SeasonalSurpriseSignal(realized_return,
                                          observations,
                                          sample_count,
                                          seasonal_mean,
                                          seasonal_sd,
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   // Consume before history, surprise arithmetic, news, spread, quote, ATR,
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadSeasonalSurpriseSignal(g_decision_bar_time,
      ...
      if(!g_signal_valid)
         ...
         g_signal_state = "seasonal_surprise_buy";
      else if(g_signal_direction < 0)
         g_signal_state = "seasonal_surprise_sell";
      else
         g_signal_state = "seasonal_surprise_band_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"valid\":%s,\"samples\":%d,\"realized_return\":%.12g,\"seasonal_mean\":%.12g,\"seasonal_sd\":%.12g,\"surprise_z\":%.12g,\"direction\":%d,\"state\":\"%s\"}",
                            g_decision_month_key,
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(!QM_InputRequireLong("qm_ea_id", qm_ea_id, 41397) ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "WTI_SEAS_SURPRISE_RV";
   ...
   if(g_signal_direction > 0)
     {
      req.type = QM_BUY;
      req.reason = "WTI_SEAS_SURPRISE_BUY";
     }
   else
     {
      req.type = QM_SELL;
      req.reason = "WTI_SEAS_SURPRISE_SELL";
     }
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI monthly standardized seasonal-surprise card holds to the next broker month"))
      return INIT_FAILED;
   ...
   g_attempt_state_key =
      StringFormat("QM5_41397_SEAS_SURPRISE_MONTH_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41397\",\"ea\":\"wti-seas-surprise-rv\"}");
...
void OnTick()
  {
   ...
   // gate. Framework news settings remain configurable and are never pinned.
   const datetime broker_now = TimeCurrent();
