...
bool HmrEntrySignal(QM_EntryRequest &req)
  {
   ...
   // Entry window: UTC hour within [start + N, end] inclusive, and strictly
   // after the opening-range bars (a range bar cannot fail against its own
   // reference).
   if(utc.hour < strategy_session_start_hour_utc + strategy_opening_range_bars ||
