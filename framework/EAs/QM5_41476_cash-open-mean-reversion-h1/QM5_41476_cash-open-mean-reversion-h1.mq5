...
#property description "QM5_41476 cash-open-mean-reversion-h1 — session-flat cash-open index mean reversion (H1)"
...
// =============================================================================
// QuantMechanica V5 EA — QM5_41476 cash-open-mean-reversion-h1
// -----------------------------------------------------------------------------
// Card: artifacts/cards_approved/QM5_41476_cash-open-mean-reversion-h1.md
//   (QM-RESEARCH-2026-0006 H-MR; preregistration 8129b0fc...d2d).
// Strategy logic: QM5_41476_CashOpenMR.mqh (the five Strategy_* hooks
...
input int    strategy_opening_range_bars     = 3;      // 2..4 (card range)
input int    strategy_ema_period             = 20;     // 15..30
input double strategy_breakout_buffer_atr    = 0.025;  // 0.0..0.05 x ATR pierce buffer
input int    strategy_atr_period             = 14;     // 10..20
input double strategy_atr_stop_mult          = 1.0;    // 0.5..1.5 x ATR beyond failed extreme
input double strategy_min_target_r           = 0.5;    // 0.4..0.8 min reward:risk eligibility
...
input double strategy_shock_atr_mult         = 3.0;    // 2.5..4.0 x ATR (cash-open bar is wide)
...
#include "QM5_41476_CashOpenMR.mqh"

CQMChartPanelCompare g_hmr_panel;

// EA identity comes from the registered source filename, not renderer copy.
string QM41476_ConsoleStrategyName()
  {
...
void QM41476_RefreshChartPanel()
  {
   if(!g_hmr_panel.Ready())
      return;

   QM_ConsoleSnapshot snapshot;
   snapshot.Reset();
   snapshot.strategy_name = QM41476_ConsoleStrategyName();
   ...
   snapshot.reason = "Failed-breakout reversion evaluated on the next H1 bar";

   const datetime now = TimeCurrent();
   const datetime bar = iTime(_Symbol, PERIOD_H1, 0); // perf-allowed: 5s timer-gated panel countdown reads the forming H1 bar time only (structural, no indicator math)
   ...
   QM_ConsoleAddGate(snapshot, "news", "News blackout",
                     !news_enabled ? "Disabled" : (g_hmr_news_cache_blocked ? "Blackout window" : "Clear"),
                     !news_enabled ? QM_GATE_OFF : (g_hmr_news_cache_blocked ? QM_GATE_BLOCK : QM_GATE_PASS));
   ...
   const bool in_entry_window = (utc.hour >= strategy_session_start_hour_utc + strategy_opening_range_bars &&
   ...
   QM_ConsoleAddGate(snapshot, "shock", "Shock filter",
                     g_hmr_shock_blocked ? "First bar shock" : "Clear",
                     g_hmr_shock_blocked ? QM_GATE_BLOCK : QM_GATE_PASS);
   QM_ConsoleAddGate(snapshot, "breaker_day", "Daily breaker",
                     g_hmr_breaker_day_hit ? "Hit (-1.0%)" : "Armed",
                     g_hmr_breaker_day_hit ? QM_GATE_BLOCK : QM_GATE_PASS);
   QM_ConsoleAddGate(snapshot, "breaker_week", "Weekly breaker",
                     g_hmr_breaker_week_hit ? "Hit (-2.0%)" : "Armed",
                     g_hmr_breaker_week_hit ? QM_GATE_BLOCK : QM_GATE_PASS);
   ...
   const int spread_days = ArraySize(g_hmr_daily_median);
   QM_ConsoleAddGate(snapshot, "spread", "Spread vs 20d median",
                     !quote ? "No valid quote"
                            : (spread_days < strategy_spread_min_days
                                  ...
                                  : (g_hmr_spread_median > 0.0
                                        ? "Median " + QM_PanelFormatNumber(g_hmr_spread_median, 1) + " pts"
                                        : "No median")),
   ...
   QM_ConsoleAddLine(snapshot.risk, "Risk basis",
                     ...
                     QM_PanelFormatNumber(strategy_atr_stop_mult, 2) + " | min RR " +
                     QM_PanelFormatNumber(strategy_min_target_r, 2));

   const int family = HmrPortfolioPositionCount();
   ...
   // Opening reference range of the current session (with midpoint target).
   if(g_hmr_window_ready)
     {
      MqlDateTime utc_bar;
      HmrUtcStruct(now, utc_bar);
      snapshot.range_start = now - utc_bar.hour * 3600 - utc_bar.min * 60 - utc_bar.sec +
                             strategy_session_start_hour_utc * 3600;
      snapshot.range_end = snapshot.range_start + strategy_opening_range_bars * PeriodSeconds(PERIOD_H1);
      snapshot.range_high = g_hmr_or_high;
      snapshot.range_low = g_hmr_or_low;
      const int digits = (int)SymbolInfoInteger(_Symbol, SYMBOL_DIGITS);
      snapshot.range_high_text = QM_PanelFormatNumber(snapshot.range_high, digits);
      snapshot.range_low_text = QM_PanelFormatNumber(snapshot.range_low, digits) + " | mid " +
                                QM_PanelFormatNumber(g_hmr_or_mid, digits);
   ...
   else if(g_hmr_trade_taken_today)
     {
      snapshot.state = QM_CONSOLE_WAITING_SETUP;
      snapshot.reason = "Entry taken today; waiting for the next session";
     }
   else if(g_hmr_breaker_day_hit || g_hmr_breaker_week_hit)
     {
      snapshot.state = QM_CONSOLE_BLOCKED;
      snapshot.reason = g_hmr_breaker_day_hit ? "Daily breaker hit — no entries today"
                                              : "Weekly breaker hit — no entries this week";
     }

   g_hmr_panel.Populate(snapshot);
   g_hmr_panel.Refresh(snapshot);
  }
...
bool Strategy_NoTradeFilter()
  {
   return HmrNoTradeFilter();
  }

bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   return HmrEntrySignal(req);
  }

void Strategy_ManageOpenPosition()
  {
   HmrManageOpenPosition();
  }

bool Strategy_ExitSignal()
  {
   return HmrExitSignal();
  }

// The card's high-impact blackout is enforced in the new-bar entry path
// (HmrNewsBlackoutBlocks) so it can never suppress exits or the flatten.
...
int OnInit()
  {
   ...
   g_hmr_slot = QM41476_ResolveSlot();
   if(g_hmr_slot != qm_magic_slot_offset)
     {
      QM_LogEvent(QM_ERROR, "INIT_SLOT_MISMATCH",
                  StringFormat("{\"chart\":\"%s\",\"slot\":%d,\"offset\":%d}",
                               QM_LoggerEscapeJson(_Symbol), g_hmr_slot, qm_magic_slot_offset));
      return INIT_FAILED;
     }
   ...
   if(MQLInfoInteger(MQL_TESTER) == 0 && MQLInfoInteger(MQL_OPTIMIZATION) == 0)
     {
      ...
      if(g_hmr_panel.Initialize(ChartID(),
                                qm_ea_id,
                                QM41476_ConsoleStrategyName(),
                                QM_FrameworkMagic(),
                                qm_panel_build_hash,
                                qm_show_chart_panel,
                                ...
                                qm_design_version, qm_apply_chart_scheme))
        {
         if(EventSetTimer(5))
            ...
            g_hmr_panel.Shutdown();
        }
     }

   QM_LogEvent(QM_INFO, "INIT_OK", "{\"ea\":\"QM5_41476_cash-open-mean-reversion-h1\"}");
...
void OnTradeTransaction(const MqlTradeTransaction &trans,
                        ...
                        const MqlTradeResult &result)
  {
   ...
   g_hmr_panel.InvalidatePerformance();
  }

void OnChartEvent(const int id, const long &lparam, const double &dparam, const string &sparam)
  {
   g_hmr_panel.OnChartEvent(id, sparam);
   // An explicit display retry may be the first successful renderer startup.
   if(!g_qm_fw_timer_active && g_hmr_panel.Ready())
     {
      if(EventSetTimer(5))
         g_qm_fw_timer_active = true;
      else
