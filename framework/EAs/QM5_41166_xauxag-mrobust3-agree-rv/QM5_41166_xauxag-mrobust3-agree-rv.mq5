...
#property description "QM5_41166 XAU/XAG Thirteen-Month Robust-Three Consensus Reversion"
...
// completed broker months. Compute exact pooled Theil-Sen, profiled finite
// LAD, and nested Siegel repeated-median slopes over that one ratio path. Fade
// only one unanimous strict sign with an opposite, equal-notional package
// held to the next month.
// Runtime is native, completed-price-only, deterministic, and one attempt per
// month.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41166;
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41166 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
            ...
            MathAbs(strategy_loss_tie_tolerance - 1.0e-12) <= 1.0e-18 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_MROBUST3_AGREE_RV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41166_XAUXAG_MROBUST3_AGREE_RV";
...
bool Strategy_LoadMonthlyRobustConsensus(
      ...
      double &theilsen,
      int &lad_candidate_count,
      double &lad_min_loss,
      int &lad_minimizer_count,
      double &lad_slope,
      int &grouped_slope_count,
      int &pivot_median_count,
      double &inner_median_min,
      double &inner_median_max,
      double &repeated_median,
      int &direction)
  {
   month_count = 0;
   ...
   pair_slope_count = 0;
   theilsen = 0.0;
   lad_candidate_count = 0;
   lad_min_loss = 0.0;
   lad_minimizer_count = 0;
   lad_slope = 0.0;
   grouped_slope_count = 0;
   pivot_median_count = 0;
   inner_median_min = 0.0;
   inner_median_max = 0.0;
   repeated_median = 0.0;
   direction = 0;
   ...
   if(expected_pair_count != 78 ||
      ArrayResize(newest_first_ratios, strategy_month_end_count) !=
      ...
      ArrayResize(chronological_times, strategy_month_end_count) !=
         strategy_month_end_count ||
      ArrayResize(pair_slopes, expected_pair_count) != expected_pair_count ||
      ArrayResize(ordered_pair_slopes, expected_pair_count) !=
         expected_pair_count ||
      ArrayResize(candidate_losses, expected_pair_count) !=
         expected_pair_count ||
      ArrayResize(pivot_medians, strategy_month_end_count) !=
         strategy_month_end_count)
      return false;
   ...
   for(int lower = 0; lower < strategy_month_end_count - 1; ++lower)
     {
      for(int upper = lower + 1;
          upper < strategy_month_end_count;
          ++upper)
        {
         if(pair_slope_count < 0 || pair_slope_count >= expected_pair_count)
            return false;
         ...
         if(!MathIsValidNumber(slope))
            return false;
         pair_slopes[pair_slope_count] = slope;
         ordered_pair_slopes[pair_slope_count] = slope;
         ++pair_slope_count;
        }
     }
   if(pair_slope_count != expected_pair_count ||
      !ArraySort(ordered_pair_slopes))
      return false;
   ...
   const int theilsen_low_index = pair_slope_count / 2 - 1;
   ...
   for(int candidate = 0; candidate < pair_slope_count; ++candidate)
     {
      ...
      for(int index = 0; index < strategy_month_end_count; ++index)
        {
         const double term =
            MathAbs(chronological_ratios[index] -
      ...
      if(!have_lad_minimum || loss < lad_min_loss)
        {
         lad_min_loss = loss;
         have_lad_minimum = true;
        }
     }

   if(!have_lad_minimum || lad_candidate_count != expected_pair_count ||
      !MathIsValidNumber(lad_min_loss) || lad_min_loss < 0.0)
      return false;

   double lad_minimizers[];
   if(ArrayResize(lad_minimizers, expected_pair_count) !=
      expected_pair_count)
      return false;
   for(int candidate = 0; candidate < lad_candidate_count; ++candidate)
     {
      const double loss_delta =
         MathAbs(candidate_losses[candidate] - lad_min_loss);
      if(!MathIsValidNumber(loss_delta))
         return false;
      if(loss_delta <= strategy_loss_tie_tolerance)
        {
         if(lad_minimizer_count < 0 ||
            lad_minimizer_count >= expected_pair_count)
            return false;
         lad_minimizers[lad_minimizer_count] = pair_slopes[candidate];
         ++lad_minimizer_count;
        }
     }
   if(lad_minimizer_count <= 0 ||
      ArrayResize(lad_minimizers, lad_minimizer_count) !=
         lad_minimizer_count ||
      !ArraySort(lad_minimizers))
      return false;
   ...
   if((lad_minimizer_count % 2) == 1)
      ...
      lad_slope =
         lad_minimizers[lad_center - 1] / 2.0 +
         lad_minimizers[lad_center] / 2.0;
   if(!MathIsValidNumber(lad_slope))
      return false;

   const int slopes_per_pivot = strategy_month_end_count - 1;
   if(slopes_per_pivot != 12)
      return false;
   for(int pivot = 0; pivot < strategy_month_end_count; ++pivot)
     {
      double pivot_slopes[];
      if(ArrayResize(pivot_slopes, slopes_per_pivot) != slopes_per_pivot)
         return false;
      int slope_index = 0;
      for(int other = 0; other < strategy_month_end_count; ++other)
        {
         if(other == pivot)
            continue;
         ...
         if(!MathIsValidNumber(slope))
            return false;
         pivot_slopes[slope_index] = slope;
         ++slope_index;
         ++grouped_slope_count;
        }
      if(slope_index != slopes_per_pivot || !ArraySort(pivot_slopes))
         return false;
      for(int index = 0; index < slopes_per_pivot; ++index)
        {
         if(!MathIsValidNumber(pivot_slopes[index]) ||
            (index > 0 && pivot_slopes[index] < pivot_slopes[index - 1]))
            return false;
        }

      const int center_low_index = slopes_per_pivot / 2 - 1;
      ...
      if(!MathIsValidNumber(inner_median))
         return false;
      pivot_medians[pivot] = inner_median;
      if(pivot == 0 || inner_median < inner_median_min)
         inner_median_min = inner_median;
      if(pivot == 0 || inner_median > inner_median_max)
         inner_median_max = inner_median;
      ++pivot_median_count;
     }

   if(grouped_slope_count != 156 ||
      pivot_median_count != strategy_month_end_count ||
   ...
   // direction > 0 means BUY XAU / SELL XAG. Fade only one unanimous strict
   // sign; every zero or estimator disagreement consumes the month flat.
   if(theilsen < 0.0 && lad_slope < 0.0 && repeated_median < 0.0)
      direction = 1;
   else if(theilsen > 0.0 && lad_slope > 0.0 &&
           repeated_median > 0.0)
      direction = -1;
   return true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41166_XAUXAG_MROBUST3_AGREE_RV_HOST";
   ...
   int month_count = 0;
   int pair_slope_count = 0;
   int lad_candidate_count = 0;
   int lad_minimizer_count = 0;
   int grouped_slope_count = 0;
   int pivot_median_count = 0;
   int direction = 0;
   datetime newest_endpoint_time = 0;
   double endpoint_displacement = 0.0;
   double theilsen = 0.0;
   double lad_min_loss = 0.0;
   double lad_slope = 0.0;
   double inner_median_min = 0.0;
   double inner_median_max = 0.0;
   double repeated_median = 0.0;
   const bool valid =
   ...
   string state = "monthly_robust_consensus_failed";
   if(valid && direction > 0)
      state = "unanimous_negative_fade_long_xau";
   else if(valid && direction < 0)
      state = "unanimous_positive_fade_short_xau";
   else if(valid &&
           (theilsen == 0.0 || lad_slope == 0.0 ||
            repeated_median == 0.0))
      state = "zero_estimator_flat";
   else if(valid)
      state = "estimator_disagreement_flat";

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"month_count\":%d,\"newest_endpoint_time\":%I64d,\"endpoint_displacement\":%.12e,\"pair_slope_count\":%d,\"theilsen\":%.12e,\"lad_candidate_count\":%d,\"lad_min_loss\":%.12e,\"lad_minimizer_count\":%d,\"lad_slope\":%.12e,\"grouped_slope_count\":%d,\"pivot_median_count\":%d,\"inner_median_min\":%.12e,\"inner_median_max\":%.12e,\"repeated_median\":%.12e,\"direction\":%d,\"state\":\"%s\"}",
                            g_signal_month_key,
                            month_count,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG thirteen-month robust-three consensus reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41166\",\"ea\":\"xauxag-mrobust3-agree-rv\"}");
