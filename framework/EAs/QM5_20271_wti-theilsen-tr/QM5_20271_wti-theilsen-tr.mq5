...
#property description "QM5_20271 WTI Theil-Sen Robust Trend"
...
//   2. form all 78 forward pairwise log-price slopes using denominator j-i;
//   3. sort the slopes and average exact zero-based indexes 38 and 39;
//   4. trade the robust central slope sign, consuming zero/invalid months flat.
// The Theil-Sen path estimator is a transparent QM mechanization, not an
// author rule. Runtime uses native XTIUSD.DWX price and broker data only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 20271;
...
input int    strategy_slope_points       = 13;
...
double g_cache_theilsen_slope  = 0.0;
double g_cache_center_low      = 0.0;
double g_cache_center_high     = 0.0;
...
bool Strategy_LoadMonthlyCloses(double &closes[])
  {
   ...
   if(strategy_slope_points != 13 || strategy_history_bars_d1 != 800)
      return false;
   ...
   const int required_closes = strategy_slope_points;
...
void Strategy_ResetCachedState()
  {
   ...
   g_cache_signal = 0;
   g_cache_theilsen_slope = 0.0;
   g_cache_center_low = 0.0;
   g_cache_center_high = 0.0;
   g_cache_state_reason = "not_evaluated";
  }

bool Strategy_TheilSenSignal(const double &closes[], int &signal)
  {
   signal = 0;
   const int point_count = strategy_slope_points;
   const int expected_slope_count = point_count * (point_count - 1) / 2;
   if(point_count != 13 ||
      expected_slope_count != 78 ||
      ArraySize(closes) != point_count)
     {
      g_cache_state_reason = "bad_theilsen_contract";
      return false;
     }

   double log_prices[];
   ArrayResize(log_prices, point_count);
   for(int i = 0; i < point_count; ++i)
     {
      ...
      log_prices[i] = MathLog(closes[i]);
      if(!MathIsValidNumber(log_prices[i]))
        {
         g_cache_state_reason = "invalid_log_price";
         return false;
        }
     }

   double sorted_slopes[];
   ArrayResize(sorted_slopes, expected_slope_count);
   int slope_index = 0;
   for(int i = 0; i < point_count - 1; ++i)
     {
      for(int j = i + 1; j < point_count; ++j)
        {
         const int month_distance = j - i;
         if(month_distance <= 0 || slope_index >= expected_slope_count)
           {
            g_cache_state_reason = "invalid_pair_enumeration";
            return false;
           }
         const double slope =
            (log_prices[j] - log_prices[i]) / (double)month_distance;
         if(!MathIsValidNumber(slope))
           {
            g_cache_state_reason = "invalid_pairwise_slope";
            return false;
           }
         sorted_slopes[slope_index] = slope;
         ++slope_index;
        }
     }

   if(slope_index != expected_slope_count)
     {
      g_cache_state_reason = "wrong_pair_count";
      return false;
     }

   ArraySort(sorted_slopes);
   for(int i = 0; i < expected_slope_count; ++i)
     {
      if(!MathIsValidNumber(sorted_slopes[i]) ||
         (i > 0 && sorted_slopes[i] < sorted_slopes[i - 1]))
        {
         g_cache_state_reason = "invalid_sorted_slope_state";
         return false;
        }
     }

   const int center_low_index = expected_slope_count / 2 - 1;
   const int center_high_index = expected_slope_count / 2;
   if(center_low_index != 38 || center_high_index != 39)
     {
      g_cache_state_reason = "wrong_median_indexes";
      return false;
     }
   const double center_low = sorted_slopes[center_low_index];
   const double center_high = sorted_slopes[center_high_index];
   const double theilsen_slope = center_low / 2.0 + center_high / 2.0;
   if(!MathIsValidNumber(center_low) ||
      !MathIsValidNumber(center_high) ||
      center_low > center_high ||
      !MathIsValidNumber(theilsen_slope))
     {
      g_cache_state_reason = "invalid_theilsen_state";
      return false;
     }

   g_cache_center_low = center_low;
   g_cache_center_high = center_high;
   g_cache_theilsen_slope = theilsen_slope;
   if(theilsen_slope == 0.0)
     {
      g_cache_state_reason = "exact_zero_theilsen_slope";
      return true;
     }

   signal = (theilsen_slope > 0.0) ? 1 : -1;
...
bool Strategy_LoadSignalState(int &signal)
  {
   ...
   return Strategy_TheilSenSignal(monthly_closes, signal);
  }
...
void Strategy_PrepareMonthlySignal()
  {
   ...
   QM_LogEvent(QM_INFO,
               "MONTHLY_STATE",
               StringFormat("{\"month\":%d,\"valid\":%s,\"signal\":%d,\"theilsen_slope\":%.10f,\"center_low\":%.10f,\"center_high\":%.10f,\"state\":\"%s\"}",
                            g_cache_month_key,
                            g_cache_signal_valid ? "true" : "false",
                            g_cache_signal,
                            g_cache_theilsen_slope,
                            g_cache_center_low,
                            g_cache_center_high,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 20271 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20271_WTI_THEILSEN_TREND";
   ...
   req.reason = (g_cache_signal > 0) ? "THEILSEN_TREND_XTI_LONG" : "THEILSEN_TREND_XTI_SHORT";
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_20271_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_20271\",\"ea\":\"wti-theilsen-tr\"}");
