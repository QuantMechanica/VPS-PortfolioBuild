...
#property description "QM5_41168 XAU/XAG Fourteen-Month Cox-Stuart Paired-Sign Reversion"
...
// latest exactly matched XAU/XAG close pair in each of fourteen consecutive
// completed broker months. Compare the two seven-month halves with exactly
// seven fixed Cox-Stuart pairs and fade only a strict five-of-seven sign
// direction using an opposite, equal-notional package held to the next month.
// Runtime is native, completed-price-only, deterministic, and one attempt per
// month.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41168;
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41168 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_MCOXSTUART_RV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41168_XAUXAG_MCOXSTUART_RV";
...
bool Strategy_LoadMonthlyPairedSign(
      ...
      int &direction)
  {
   ...
   const int xau_copied =
      CopyRates(g_leg_xau, // perf-allowed: one bounded fourteen-month scan behind a consumed monthly attempt.
                PERIOD_D1, 1, strategy_history_bars_d1, xau_bars);
   const int xag_copied =
      CopyRates(g_leg_xag, // perf-allowed: one bounded fourteen-month scan behind a consumed monthly attempt.
   ...
   for(int pair = 0; pair < strategy_pair_count; ++pair)
     {
      ...
      if(older < 0 || newer <= older ||
         ...
         older >= ArraySize(chronological_ratios) ||
      ...
      if(pair > 0)
         pair_signs += ",";
      pair_signs += IntegerToString(sign);
      ++pair_count;
     }

   if(pair_count != strategy_pair_count || pair_count != 7 ||
      positive_count + negative_count + tie_count != pair_count)
      return false;

   // direction > 0 means BUY XAU / SELL XAG. Fade only a strict five-of-seven
   // paired-sign direction; any exact tie or a 4/3 split is flat.
   if(tie_count > 0)
      return true;
   if(positive_count >= strategy_signs_required)
      direction = -1;
   else if(negative_count >= strategy_signs_required)
      direction = 1;
   return true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41168_XAUXAG_MCOXSTUART_RV_HOST";
   ...
   string pair_signs = "";
   const bool valid =
      Strategy_LoadMonthlyPairedSign(
         g_signal_month_key,
         month_count,
   ...
   string state = "monthly_paired_sign_failed";
   if(valid && direction > 0)
      state = "five_negative_fade_long_xau";
   else if(valid && direction < 0)
      state = "five_positive_fade_short_xau";
   else if(valid && tie_count > 0)
      state = "paired_sign_tie_flat";
   else if(valid)
      state = "four_three_split_flat";

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"month_count\":%d,\"newest_endpoint_time\":%I64d,\"endpoint_displacement\":%.12e,\"pair_count\":%d,\"pair_signs\":\"%s\",\"positive_count\":%d,\"negative_count\":%d,\"tie_count\":%d,\"direction\":%d,\"state\":\"%s\"}",
                            g_signal_month_key,
                            month_count,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG fourteen-month Cox-Stuart paired-sign reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41168\",\"ea\":\"xauxag-mcoxstuart-rv\"}");
