...
#property description "QM5_41174 XAU/XAG Thirteen-Month Spearman Ratio-Rank Reversion"
...
// completed broker months. Strict-rank the gold-minus-silver log ratios,
// measure their Spearman association with calendar order through the exact
// integer displacement score, and fade only abs(T)>=104 with an opposite,
// equal-notional package held to the next month.
// Runtime is native, completed-price-only, deterministic, and one attempt per
// month.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41174;
...
input int    strategy_score_threshold         = 104;
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41174 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_MSPEARMAN_RV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41174_XAUXAG_MSPEARMAN_RV";
...
bool Strategy_LoadMonthlySpearman(
      ...
      int &direction)
  {
   ...
   for(int index = 0; index < strategy_endpoint_count; ++index)
     {
      const int delta = ranks[index] - (index + 1);
      rank_displacement_sum += delta * delta;
      if(index > 0)
         rank_csv += ",";
      rank_csv += IntegerToString(ranks[index]);
     }
   rank_score = 364 - rank_displacement_sum;
   if(rank_displacement_sum < 0 || rank_displacement_sum > 728 ||
      rank_score < -364 || rank_score > 364 ||
      (rank_displacement_sum % 2) != 0 || (rank_score % 2) != 0 ||
      rank_score != 364 - rank_displacement_sum)
      return false;

   // direction > 0 means BUY XAU / SELL XAG. Fade the qualified ratio-rank
   // trend; score magnitude above the inclusive boundary never scales risk.
   if(rank_score >= strategy_score_threshold)
      direction = -1;
   else if(rank_score <= -strategy_score_threshold)
      direction = 1;
   return true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41174_XAUXAG_MSPEARMAN_RV_HOST";
   ...
   string state = "monthly_spearman_failed";
   if(valid && direction > 0)
      state = "negative_rank_trend_fade_long_xau";
   else if(valid && direction < 0)
      state = "positive_rank_trend_fade_short_xau";
   else if(valid && tie_count > 0)
      state = "ratio_rank_tie_flat";
   else if(valid)
      state = "rank_score_interior_flat";

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"month_count\":%d,\"newest_endpoint_time\":%I64d,\"endpoint_displacement\":%.12e,\"rank_displacement_sum\":%d,\"rank_score\":%d,\"rank_csv\":\"%s\",\"tie_count\":%d,\"direction\":%d,\"state\":\"%s\"}",
                            g_signal_month_key,
                            month_count,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG thirteen-month Spearman ratio-rank reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41174\",\"ea\":\"xauxag-mspearman-rv\"}");
