# QM5_41154_eurjpy-carry-unwind-crisis-momentum - Strategy Spec

**EA ID:** QM5_41154

**Slug:** `eurjpy-carry-unwind-crisis-momentum`
...
At each new `EURJPY.DWX` D1 bar, the EA reads the immediately completed D1
...
`EURJPY.DWX`; the other three symbols are signal data only.
...
- Host/execution symbol: exact `EURJPY.DWX`, D1, slot 0, magic `411540000`.
- Signal-only symbols: `AUDJPY.DWX`, `NZDJPY.DWX`, and `CADJPY.DWX` plus the
  host series.
...
The entry decision runs once behind `QM_IsNewBar(EURJPY.DWX, PERIOD_D1)`.
...
|---|---|---|---|
| v1 | 2026-08-29 | Initial build from OWNER-approved G0 card | `baf614cf-9fd2-462f-ac4a-f8f56a018916` |
