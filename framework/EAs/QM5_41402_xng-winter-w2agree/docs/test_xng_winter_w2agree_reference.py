"""Deterministic reference checks for QM5_41402 XNG winter agreement."""
...
def agreement_signal(
    ...
    """Mirror two adjacent completed weeks and strict sign agreement."""
...
class XngWinterTwoWeekAgreementReferenceTest(unittest.TestCase):
    def setUp(self) -> None:
        self.current = date(2026, 1, 19)
    ...
    def test_all_winter_anchor_months_are_eligible(self) -> None:
        for value in (
            date(2026, 11, 2),
            date(2026, 12, 7),
            date(2026, 1, 5),
    ...
    def test_ineligible_week_refuses_before_signal(self) -> None:
        with self.assertRaisesRegex(ValueError, "ineligible month"):
            agreement_signal(date(2026, 7, 6), self.bars(3, 4, 2, 3))
