...
#property description "QM5_41260 XAU/XAG Monthly Exact Anderson-Darling Reversion"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_41260 - XAU/XAG Monthly Exact Anderson-Darling Reversion
// -----------------------------------------------------------------------------
...
// two-sample Anderson-Darling statistic, and enumerate all C(12,6)=924 label
// assignments for an inclusive exact upper tail. A qualified tail fades the
// recent block's pooled rank displacement with an opposite, equal-notional
// package held to the next month. Runtime is native, completed-price-only,
// deterministic, and one consumed attempt per month.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41260;
...
input int    strategy_tail_numerator          = 1;
input int    strategy_tail_denominator        = 2;
input int    strategy_tail_count_max          = 452;
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41260 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_MAD2_RV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41260_XAUXAG_MAD2_RV";
...
bool Strategy_AndersonDarlingFromLabels(const int &labels[],
                                        double &statistic)
  {
   statistic = 0.0;
   ...
   int recent_seen = 0;
   double sum = 0.0;
   for(int index = 0; index < strategy_return_count - 1; ++index)
     {
      if(labels[index] == 0)
         ++old_seen;
      else
         ++recent_seen;
      const int pooled_rank = index + 1;
      const double old_delta =
         (double)(strategy_return_count * old_seen -
                  strategy_block_size * pooled_rank);
      const double recent_delta =
         (double)(strategy_return_count * recent_seen -
                  strategy_block_size * pooled_rank);
      const double denominator =
         (double)(pooled_rank * (strategy_return_count - pooled_rank));
      if(denominator <= 0.0)
         return false;
      const double term =
         ((old_delta * old_delta / (double)strategy_block_size) +
          (recent_delta * recent_delta / (double)strategy_block_size)) /
         denominator;
      if(!MathIsValidNumber(term))
         return false;
      sum += term;
      if(!MathIsValidNumber(sum))
         return false;
     }
   statistic = sum / (double)strategy_return_count;
   return MathIsValidNumber(statistic);
  }
...
bool Strategy_LoadMonthlyAd2Signal(
      ...
      int &direction)
  {
   ...
   if(recent_rank_sum < 21 || recent_rank_sum > 57 ||
      !Strategy_AndersonDarlingFromLabels(pooled_labels, observed_a2))
      return false;

   const double tail_epsilon =
      strategy_stat_epsilon * MathMax(1.0, MathAbs(observed_a2));
   ...
   for(int mask = 0; mask < (1 << strategy_return_count); ++mask)
     {
      ...
      for(int index = 0; index < strategy_return_count; ++index)
         permutation_labels[index] =
            ((mask & (1 << index)) != 0) ? 1 : 0;
      double permutation_a2 = 0.0;
      if(!Strategy_AndersonDarlingFromLabels(permutation_labels,
                                             permutation_a2))
         return false;
      ++assignment_count;
      if(permutation_a2 + tail_epsilon >= observed_a2)
         ++tail_count;
     }
   ...
   const bool qualifies =
      ...
      tail_count * strategy_tail_denominator <=
         assignment_count * strategy_tail_numerator;
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41260_XAUXAG_MAD2_RV_HOST";
   ...
   double observed_a2 = 0.0;
   string ratio_path = "";
   string return_path = "";
   string label_path = "";
   const bool valid =
      Strategy_LoadMonthlyAd2Signal(
         g_signal_month_key,
         month_count,
   ...
   string state = "monthly_ad2_signal_failed";
   if(valid && direction > 0)
      ...
      state = "exact_upper_tail_above_locked_cutoff_flat";
           ...
           tail_count * strategy_tail_denominator >
              assignment_count * strategy_tail_numerator)
      state = "exact_upper_tail_above_half_flat";
   else if(valid && recent_rank_sum == strategy_neutral_rank_sum)
      state = "neutral_recent_rank_sum_flat";
   else if(valid)
      state = "nonqualifying_exact_ad2_flat";

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"month_count\":%d,\"newest_endpoint_time\":%I64d,\"observed_a2\":%.12e,\"assignment_count\":%d,\"tail_count\":%d,\"recent_rank_sum\":%d,\"direction\":%d,\"ratios\":\"%s\",\"returns\":\"%s\",\"pooled_labels\":\"%s\",\"state\":\"%s\"}",
                            g_signal_month_key,
                            month_count,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG monthly exact Anderson-Darling reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41260\",\"ea\":\"xauxag-mad2-rv\"}");
