...
#property description "QM5_41058 XNG Weekly Flow-Agreement Continuation"
...
//   - require the exact prior Monday-Friday broker week and preceding Friday
//   - sum five completed close-to-open and open-to-close log-return streams
//   - enter on the next Monday only when both completed-flow signs agree
...
bool Strategy_LoadFlowAgreement(const datetime current_bar_time,
                                ...
                                int &direction)
  {
   ...
   const int copied = CopyRates(_Symbol,          // perf-allowed: exact
                                PERIOD_D1,         // six completed bars:
                                1,                 // prior Mon-Fri plus
                                6,                 // preceding Friday.
                                bars);
   if(copied != 6)
      return false;

   // Newest first: prior Friday, Thursday, Wednesday, Tuesday, Monday,
   // preceding Friday. Weekday plus bounded gaps rejects holiday shifts and
   // missing data while tolerating one-hour broker DST transitions.
   datetime session_times[6];
   ...
   for(int index = 0; index < 6; ++index)
     {
      ...
      if(bars[index].time <= 0 ||
         ...
         (index < 5 &&
          (bars[index].open <= 0.0 ||
           !MathIsValidNumber(bars[index].open))) ||
   ...
   if(!Strategy_GapHoursAllowed(session_times[4],
                                session_times[5],
                                68,
                                76))
      return false;
   ...
   for(int index = 0; index < 5; ++index)
     {
      const double overnight_return =
         MathLog(bars[index].open / bars[index + 1].close);
      const double intraday_return =
         MathLog(bars[index].close / bars[index].open);
   ...
   const double weekly_return =
      MathLog(bars[0].close / bars[5].close);
   const double component_total =
      overnight_flow + session_flow;
   if(!MathIsValidNumber(weekly_return) ||
      !MathIsValidNumber(component_total) ||
      MathAbs(component_total - weekly_return) >
         strategy_reconcile_tolerance)
      return false;

   if(overnight_flow > 0.0 && session_flow > 0.0)
      direction = 1;
   else if(overnight_flow < 0.0 && session_flow < 0.0)
      direction = -1;
   return true;
  }
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(strategy_max_hold_days != 8 ||
      ...
      MathAbs(strategy_reconcile_tolerance - 1.0e-10) > 1.0e-15)
      return true;
   return false;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   int direction = 0;
   if(!Strategy_LoadFlowAgreement(g_strategy_d1_bar_time,
                              broker_now,
                              overnight_flow,
                              session_flow,
                              direction))
   ...
   if(direction > 0)
     {
      req.type = QM_BUY;
      req.reason = "XNG_FLOW_AGREE_LONG";
     }
   else if(direction < 0)
     {
      req.type = QM_SELL;
      req.reason = "XNG_FLOW_AGREE_SHORT";
     }
   else
      return false;
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"date\":%d,\"overnight_flow\":%.12e,\"session_flow\":%.12e,\"direction\":%d}",
                            date_key,
                            overnight_flow,
                            session_flow,
                            direction));
...
int OnInit()
  {
   if(!SymbolSelect("XNGUSD.DWX", true) ||
      !Strategy_IsXngD1() ||
      qm_ea_id != 41058 ||
      qm_magic_slot_offset != 0)
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XNG weekly flow-agreement card uses Friday 21 as ordinary exit"))
      return INIT_FAILED;
   ...
   g_attempt_state_key =
      StringFormat("QM5_41058_FLOW_WEEK_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41058\",\"ea\":\"xng-wflow-agree\"}");
