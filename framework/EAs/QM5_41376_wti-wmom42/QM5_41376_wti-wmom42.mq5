...
#property description "QM5_41376 WTI Skipped-Week Three-Week Momentum"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41376 - WTI Skipped-Week Three-Week Momentum
// -----------------------------------------------------------------------------
// D1 structural crude-oil sleeve:
//   - validate four consecutive completed 3-to-5-session broker weeks
//   - exclude t-1; compute log(final close[t-2] / first open[t-4])
//   - follow the exact CMOM4,2 formation return's strict sign
...
input int    strategy_formation_weeks       = 3;
input int    strategy_skip_recent_weeks     = 1;
...
bool Strategy_LoadWeeklyMomentumSignal(const int current_week_key,
                                       ...
                                       double &week_log_return)
  {
   ...
   ArraySetAsSeries(bars, true);
   const int copied =
      CopyRates(_Symbol, // perf-allowed: bounded CMOM4,2 scan behind the sole QM_IsNewBar branch.
                PERIOD_D1,
                1,
                strategy_history_bars,
                bars);
         ...
         if(active_week_key == 0)
           {
            if(Strategy_NextWeekKey(week_key) != current_week_key)
               return false;
            week_index = 0; // t-1: chronology-only skipped week.
           }
         else
           {
   ...
   // CMOM4,2: t-1 (index 0) is deliberately excluded. The formation endpoints
   // are the first open of t-4 (index 3) and final close of t-2 (index 1).
   week_open = week_opens[3];
   week_close = week_closes[1];
   week_bars = week_counts[1] + week_counts[2] + week_counts[3];
   if(week_open <= 0.0 || week_close <= 0.0 ||
      !MathIsValidNumber(week_open) ||
      !MathIsValidNumber(week_close))
      return false;

   week_log_return = MathLog(week_close / week_open);
   if(!MathIsValidNumber(week_log_return))
      return false;
   if(week_log_return > strategy_return_epsilon)
      direction = 1;
   else if(week_log_return < -strategy_return_epsilon)
      direction = -1;
   return true;
  }
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   else
     {
      g_signal_valid =
         Strategy_LoadWeeklyMomentumSignal(g_decision_week_key,
                                           ...
                                           g_week_log_return);
      if(!g_signal_valid)
         g_signal_state = "cmom42_validation_failed";
      else if(g_signal_direction > 0)
         g_signal_state = "positive_cmom42_long";
      else if(g_signal_direction < 0)
         g_signal_state = "negative_cmom42_short";
      else
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"week\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_week_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"formation_bars\":%d,\"formation_open_t4\":%.8f,\"formation_close_t2\":%.8f,\"formation_log_return\":%.12f,\"state\":\"%s\"}",
                            g_decision_week_key,
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41376_WTI_WMOM42";
...
int OnInit()
  {
   if(!SymbolSelect(strategy_symbol, true) ||
       !Strategy_IsHostChart() || qm_ea_id != 41376 ||
      qm_magic_slot_offset != 0)
   ...
   g_attempt_state_key =
      StringFormat("QM5_41376_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
