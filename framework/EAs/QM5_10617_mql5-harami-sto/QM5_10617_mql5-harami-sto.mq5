...
#property description "QM5_10617 MQL5 Harami + Stochastic reversal"
...
input double strategy_stoch_entry_oversold      = 30.0;
input double strategy_stoch_entry_overbought    = 70.0;
input double strategy_stoch_exit_low            = 20.0;
input double strategy_stoch_exit_high           = 80.0;
...
void Strategy_ManageOpenPosition()
  {
   // Card baseline uses fixed initial SL/TP only; no trailing, BE, or partial close.
  }
...
bool Strategy_ExitSignal()
  {
   ...
   const double stoch_last = QM_Stoch_D(_Symbol, strategy_work_tf,
                                        ...
                                        strategy_stoch_slowing,
                                        1);
   const double stoch_prev = QM_Stoch_D(_Symbol, strategy_work_tf,
                                        strategy_stoch_k_period,
                                        strategy_stoch_d_period,
                                        strategy_stoch_slowing,
                                        2);
   if(stoch_last <= 0.0 || stoch_prev <= 0.0)
      return false;

   if(position_type == POSITION_TYPE_BUY)
     {
      if(stoch_prev >= strategy_stoch_exit_high && stoch_last < strategy_stoch_exit_high)
         return true;
      if(stoch_prev >= strategy_stoch_exit_low && stoch_last < strategy_stoch_exit_low)
         return true;
     }
