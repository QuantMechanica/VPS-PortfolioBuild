...
#property description "QM5_20251 WTI same-calendar / return-sign concordance"
...
// -----------------------------------------------------------------------------
// At each broker-month boundary:
//   1. estimate WTI's historical return sign for the same calendar month over
//      up to ten prior years (minimum five);
//   2. classify the prior twelve completed monthly return signs with the
//      source-fixed 0.40 positive-sign probability threshold; and
//   3. trade in the shared direction only when the two non-zero signs agree.
// The package closes and may renew at the next month boundary. A consumed
// month is never retried after sign disagreement, restart, rejection, stop,
// or a blocked gate. Runtime uses native MT5 data only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                     = 20251;
...
double g_seasonal_score = 0.0;
double g_positive_probability = 0.0;
...
void Strategy_LoadAttemptState(const datetime reference_time)
  {
   ...
   // Future markers can remain after later tester runs. Do not let them
...
bool Strategy_LoadSeasonalSignal(const datetime decision_bar_time,
                                 ...
                                 int &direction)
  {
   ...
   const int copied = CopyRates(_Symbol, PERIOD_D1, 1, strategy_history_bars, rates); // perf-allowed: bounded same-calendar sample, called once per broker month.
   if(copied <= 0)
      return false;
...
bool Strategy_LoadReturnSignSignal(const int current_month_key,
                                   ...
                                   int &direction)
  {
   ...
   if(current_month_key <= 0 || needed_closes != 13)
      return false;

   MqlRates bars[];
   ArraySetAsSeries(bars, true);
   const int copied =
      CopyRates(_Symbol, // perf-allowed: bounded monthly endpoint sample, called once per broker month.
                PERIOD_D1,
                1,
   ...
   for(int index = 0; index < strategy_lookback_months; ++index)
     {
      ...
      const double monthly_return =
         MathLog(month_closes[index] / month_closes[index + 1]);
...
void Strategy_LogMonthlyAttempt(const int month_key,
                                ...
                                const bool return_sign_valid,
                                const int return_sign_direction)
  {
   string outcome = "agreement_signal";
   if(!seasonal_valid)
      outcome = "seasonal_history_invalid";
   else if(!return_sign_valid)
      outcome = "return_sign_history_invalid";
   else if(seasonal_direction == 0)
      outcome = "seasonal_zero";
   else if(return_sign_direction == 0)
      outcome = "return_sign_zero";
   else if(seasonal_direction != return_sign_direction)
   ...
   QM_LogEvent(QM_INFO,
               ...
               StringFormat("{\"month_key\":%d,"
                            ...
                            "\"return_sign_valid\":%d,"
                            "\"positive_count\":%d,"
                            "\"positive_probability\":%.10f,"
                            "\"return_sign_direction\":%d,"
                            "\"outcome\":\"%s\"}",
                            month_key,
                            ...
                            g_seasonal_score,
                            seasonal_direction,
                            return_sign_valid ? 1 : 0,
                            g_positive_count,
                            g_positive_probability,
                            return_sign_direction,
                            outcome));
  }
...
void Strategy_AdvanceMonthlyState()
  {
   ...
   g_seasonal_score = 0.0;
   g_positive_probability = 0.0;
   g_positive_count = 0;
   ...
   // Consume before history, signal, spread, ATR, price, news, or order
   // checks. Sign disagreement and all fallible gates remain one-shot monthly.
   if(!Strategy_RecordMonthAttempt(month_key))
      return;

   int seasonal_direction = 0;
   int return_sign_direction = 0;
   const bool seasonal_valid =
      Strategy_LoadSeasonalSignal(decision_bar_time,
                                  ...
                                  seasonal_direction);
   const bool return_sign_valid =
      Strategy_LoadReturnSignSignal(month_key,
                                    g_positive_probability,
                                    g_positive_count,
                                    return_sign_direction);
   g_signal_valid = seasonal_valid && return_sign_valid;
   ...
   if(g_signal_valid &&
      ...
      return_sign_direction != 0 &&
      seasonal_direction == return_sign_direction)
      g_signal_direction = seasonal_direction;
  }
...
bool Strategy_InputsValid()
  {
   if(qm_ea_id != 20251 || qm_magic_slot_offset != 0)
      return false;
...
bool Strategy_NoTradeFilter()
  {
   return (!Strategy_IsWtiD1() || !Strategy_InputsValid());
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20251_WTI_CAL_RSM";
   ...
   req.reason =
      (g_signal_direction > 0) ?
      StringFormat("WTI_CAL_RSM_LONG_P%02d_OF_%02d",
                   g_positive_count,
                   strategy_lookback_months) :
      StringFormat("WTI_CAL_RSM_SHORT_P%02d_OF_%02d",
                   g_positive_count,
                   strategy_lookback_months);
   return true;
  }
...
int OnInit()
  {
   if(!Strategy_IsWtiD1() || !Strategy_InputsValid())
   ...
   g_attempt_state_key =
      StringFormat("QM5_20251_MONTH_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               StringFormat("{\"card\":\"QM5_20251\","
                            "\"ea\":\"wti-cal-rsm\","
                            "\"history_years\":%d,"
                            "\"min_history_years\":%d,"
                            "\"lookback_months\":%d,"
                            "\"positive_threshold\":%.4f,"
                            "\"history_bars\":%d,"
                            "\"atr_period\":%d,"
                            "\"atr_sl_mult\":%.4f,"
                            "\"max_hold_days\":%d,"
...
void OnTick()
  {
   ...
   // Monthly and stale exits always precede entry-only news and spread gates.
