"""Deterministic reference checks for QM5_41082 WTI weekly run-break dominance."""
...
def run_break_signal(
    current_week_key: int,
    completed_newest_first: list[Bar],
    offset: timedelta,
) -> tuple[bool, int, tuple[float, float, float], tuple[float, float, float, float]]:
    """Mirror four week ends and strict newest-over-combined-older dominance."""
    ...
    direction = 0
    combined_older = abs(oldest) + abs(middle)
    if (
        oldest > 0.0
        and middle > 0.0
        and newest < 0.0
        and abs(newest) > combined_older
    ):
        direction = -1
    elif (
        oldest < 0.0
...
class WeekRunBreakReferenceTest(unittest.TestCase):
    def test_positive_newest_net_sign_is_long(self) -> None:
        current, now, bars = sample(110.0, 90.0, 95.0, 100.0)
        decision, late, count, offset = decision_clock(current, now, bars)
        self.assertTrue(decision)
        self.assertFalse(late)
        self.assertEqual(count, 0)
        assert offset is not None
        valid, direction, returns, endpoints = run_break_signal(
        ...
        self.assertGreater(abs(newest), abs(oldest) + abs(middle))
        self.assertGreater(oldest + middle + newest, 0.0)
        self.assertEqual(endpoints, (110.0, 90.0, 95.0, 100.0))

    def test_negative_newest_net_sign_is_short(self) -> None:
        current, now, bars = sample(90.0, 110.0, 105.0, 100.0)
        offset = label_offset(current, now)
        assert offset is not None
        valid, direction, returns, _ = run_break_signal(
            20260824, bars, offset
        )
        oldest, middle, newest = returns
        self.assertTrue(valid)
    ...
    def test_wrong_path_zero_equal_and_nondominant_states_are_flat(self) -> None:
        cases = [
            sample(110.0, 105.0, 100.0, 95.0),
            sample(90.0, 100.0, 95.0, 100.0),
            sample(100.0, 100.0, 95.0, 100.0),
            sample(1.0, math.exp(2.0), math.e, 1.0),
            sample(108.0, 110.0, 105.0, 100.0),
        ]
        for current, now, bars in cases:
            offset = label_offset(current, now)
            assert offset is not None
            valid, direction, _, _ = run_break_signal(
                20260824, bars, offset
            )
            self.assertTrue(valid)
    ...
    def test_uniform_prior_date_label_normalization_and_grace(self) -> None:
        current, now, bars = sample(110.0, 90.0, 95.0, 100.0, True)
        decision, late, count, offset = decision_clock(current, now, bars)
        self.assertTrue(decision)
        self.assertFalse(late)
        self.assertEqual(count, 0)
        ...
        assert offset is not None
        self.assertEqual(run_break_signal(20260824, bars, offset)[1], 1)
    ...
    def test_nonconsecutive_or_current_week_endpoint_rejected(self) -> None:
        ...
        self.assertFalse(run_break_signal(20260824, bars, offset)[0])
        _, _, bars = sample(110.0, 90.0, 95.0, 100.0)
        bars.insert(0, Bar(datetime(2026, 8, 24, tzinfo=UTC), 111.0))
        self.assertFalse(run_break_signal(20260824, bars, offset)[0])
    ...
    def test_bad_endpoint_reverse_order_and_short_history_rejected(self) -> None:
        ...
        self.assertFalse(run_break_signal(20260824, bars, offset)[0])
        _, _, bars = sample(110.0, 90.0, 95.0, 100.0)
        self.assertFalse(run_break_signal(20260824, bars[:-1], offset)[0])
