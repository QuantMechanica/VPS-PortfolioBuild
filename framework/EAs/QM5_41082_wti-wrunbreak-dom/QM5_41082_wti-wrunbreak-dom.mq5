...
#property description "QM5_41082 WTI Completed-Week Run-Break Dominance"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41082 - WTI Completed-Week Run-Break Dominance
// -----------------------------------------------------------------------------
...
//   - require two older same-sign weeks and one opposed newest week
//   - prove the newest move exceeds both older moves combined
//   - consume the week before fallible gates; never enter late or retry
//   - follow the newest and cumulative-three-week sign for one week
// Runtime uses MT5-native OHLC, calendar, history, and framework state only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                      = 41082;
...
double   g_newest_return          = 0.0;
double   g_middle_return         = 0.0;
...
bool Strategy_LoadRunBreakSignal(const int current_week_key,
                                    const int label_offset,
                                    int &direction,
                                    double &newest_return,
                                    double &middle_return,
                                    ...
                                    double &week_end_c4)
  {
   ...
   newest_return = MathLog(week_end_c1 / week_end_c2);
   middle_return = MathLog(week_end_c2 / week_end_c3);
   oldest_return = MathLog(week_end_c3 / week_end_c4);
   if(!MathIsValidNumber(newest_return) ||
      !MathIsValidNumber(middle_return) ||
      !MathIsValidNumber(oldest_return))
      return false;

   const double combined_older_magnitude =
      MathAbs(oldest_return) + MathAbs(middle_return);
   if(oldest_return > 0.0 && middle_return > 0.0 &&
      newest_return < 0.0 &&
      MathAbs(newest_return) > combined_older_magnitude)
      direction = -1;
   else if(oldest_return < 0.0 && middle_return < 0.0 &&
           newest_return > 0.0 &&
           MathAbs(newest_return) > combined_older_magnitude)
      direction = 1;
   return true;
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_WeekAlreadyEntered(g_decision_week_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadRunBreakSignal(g_decision_week_key,
      ...
      if(!g_signal_valid)
         ...
         g_signal_state = "positive_newest_net_long";
      else if(g_signal_direction < 0)
         g_signal_state = "negative_newest_net_short";
      else if(g_newest_return == 0.0 || g_middle_return == 0.0 ||
              g_oldest_return == 0.0)
         g_signal_state = "exact_zero_flat";
      else if((g_oldest_return > 0.0 && g_middle_return > 0.0 &&
               g_newest_return < 0.0) ||
              (g_oldest_return < 0.0 && g_middle_return < 0.0 &&
               g_newest_return > 0.0))
         g_signal_state = "newest_not_combined_dominant_flat";
      else
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"week\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_week_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"oldest_return\":%.12e,\"middle_return\":%.12e,\"newest_return\":%.12e,\"c4\":%.8f,\"c3\":%.8f,\"c2\":%.8f,\"c1\":%.8f,\"state\":\"%s\"}",
                            g_decision_week_key,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41082 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41082_WTI_WRUNBREAK_DOM";
   ...
   req.reason = (g_signal_direction > 0)
      ? "WTI_WRUNBREAK_DOM_LONG"
      : "WTI_WRUNBREAK_DOM_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_strategy_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41082 ||
   ...
   g_attempt_state_key =
      StringFormat("QM5_41082_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41082\",\"ea\":\"wti-wrunbreak-dom\"}");
