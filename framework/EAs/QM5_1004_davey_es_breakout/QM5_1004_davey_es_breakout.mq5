...
input int    framework_ea_id          = 1004;
input int    framework_magic_slot     = 0;
...
input group "News"
input QM_NewsMode framework_news_mode = QM_NEWS_OFF;

input group "Friday Close"
input bool   framework_friday_close_enabled     = true;
input int    framework_friday_close_hour_broker = 21;
...
void PlaceEntry(const StrategySignal signal)
  {
   ...
   QM_EntryRequest req;
   ZeroMemory(req);
   req.symbol_slot = framework_magic_slot;
...
int OnInit()
  {
   if(!QM_FrameworkInit(framework_ea_id,
                        framework_magic_slot,
                        RISK_PERCENT,
                        RISK_FIXED,
                        PORTFOLIO_WEIGHT,
                        framework_news_mode,
                        framework_friday_close_enabled,
                        framework_friday_close_hour_broker))
      return INIT_FAILED;
...
void OnTick()
  {
   if(!QM_KillSwitchCheck())
      return;
   if(!QM_NewsAllowsTrade(_Symbol, TimeCurrent(), framework_news_mode))
      return;
