...
#property description "QM5_11030 atc-zigzag-pend — ZigZag confirmed-swing pending breakout (FX, H1)"

#include <QM/QM_Common.mqh>

// =============================================================================
// QuantMechanica V5 EA — QM5_11030 atc-zigzag-pend
// -----------------------------------------------------------------------------
// Source: Evgeny Gnidko, Interview (ATC 2012), MQL5 Articles 607 (ice_rain_ATC2012).
...
//            side fills (position exists), the opposite pending order is cancelled.
//            One active position per symbol/magic.
//   Filter : swing range (lastHigh-lastLow) >= min_range_atr * ATR(atr_period).
//   Stop   : SL = sl_atr_mult * ATR from the stop entry price.
//   Take   : TP = tp_atr_mult * ATR from the stop entry price (TP >> SL).
//   Trail  : after price moves trail_start_atr*ATR in favour, ATR trailing stop.
//   Spread : fail-OPEN on .DWX (0 modeled spread); block only a genuinely wide one.
//
// This is a structural pending-order strategy, so OnTick below is bespoke (it
// manages two pending orders + a cached confirmed-pivot state) rather than the
...
input double strategy_tp_atr_mult       = 3.0;   // target distance = mult * ATR (TP >> SL)
input double strategy_trail_start_atr   = 1.0;   // arm trailing after +this*ATR in favour
input double strategy_trail_atr_mult    = 2.0;   // ATR trailing-stop distance once armed
input int    strategy_pending_expiry_h  = 72;    // pending-order GTC expiry, in hours (0 = GTC)
...
// position per magic exists at a time.
