...
#property description "QM5_41131 WTI Completed-Month Daily Tail-Trim Momentum"
...
//   - sort all daily log returns and delete exactly one minimum and maximum
//   - follow the sign of the retained inner sum; raw endpoint is diagnostic only
...
double   g_min_return              = 0.0;
double   g_max_return              = 0.0;
double   g_inner_sum               = 0.0;
...
bool Strategy_LoadDailyTailTrimSignal(const int current_month_key,
                                       const int label_offset,
                                       int &direction,
                                       int &month_sessions,
                                       ...
                                       int &retained_count)
  {
   ...
   if(!boundary_seen || month_sessions < strategy_min_month_sessions ||
      month_sessions > strategy_max_month_sessions ||
      month_sessions <= 2 * strategy_trim_each_tail ||
   ...
   if(ArrayResize(sorted_returns, month_sessions) != month_sessions)
      return false;
   for(int index = 0; index < month_sessions; ++index)
      sorted_returns[index] = daily_returns[index];
   ArraySort(sorted_returns);

   for(int index = 0; index < month_sessions; ++index)
     {
      if(!MathIsValidNumber(sorted_returns[index]))
         return false;
      if(index > 0 && sorted_returns[index] < sorted_returns[index - 1])
         return false;
     }

   min_return = sorted_returns[0];
   max_return = sorted_returns[month_sessions - 1];
   retained_count =
      month_sessions - 2 * strategy_trim_each_tail;
   if(retained_count < 1)
      return false;
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadDailyTailTrimSignal(g_decision_month_key,
                                          ...
                                          g_retained_count);
      if(!g_signal_valid)
         g_signal_state = "monthly_tail_trim_validation_failed";
      else if(g_inner_sum == 0.0)
         g_signal_state = "zero_inner_sum_flat";
      else if(g_signal_direction > 0)
         g_signal_state = "daily_tail_trim_completed_month_long";
      else if(g_signal_direction < 0)
         g_signal_state = "daily_tail_trim_completed_month_short";
      else
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"month_sessions\":%d,\"retained_count\":%d,\"boundary_close\":%.8f,\"final_close\":%.8f,\"raw_sum\":%.12e,\"endpoint_return\":%.12e,\"min_return\":%.12e,\"max_return\":%.12e,\"inner_sum\":%.12e,\"raw_direction\":%d,\"endpoint_agrees\":%s,\"state\":\"%s\"}",
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41131 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41131_WTI_MDAILY_TAILTRIM_MOM";
   ...
   req.reason = (g_signal_direction > 0)
      ? "WTI_MDAILY_TAILTRIM_MOM_LONG"
      : "WTI_MDAILY_TAILTRIM_MOM_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41131 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI completed-month daily tail-trim position holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41131_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41131\",\"ea\":\"wti-mdaily-tailtrim-mom\"}");
