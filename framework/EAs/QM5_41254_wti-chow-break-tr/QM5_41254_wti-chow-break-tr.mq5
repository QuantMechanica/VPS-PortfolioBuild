...
#property description "QM5_41254 WTI Scanned Two-Regression Structural-Break Trend"
...
//   - reconstruct 252 consecutive completed D1 closes before the new month
//   - scan pooled versus two-segment OLS RSS over log-price splits k=63..189
//   - follow the selected newest segment's slope after an inclusive F>=3 gate
...
input int    strategy_observation_count       = 252;
input int    strategy_split_min                = 63;
input int    strategy_split_max                = 189;
input double strategy_score_threshold          = 3.0;
input double strategy_rss_epsilon              = 0.0000000000000001;
input double strategy_improvement_tolerance    = 0.000000000001;
input double strategy_slope_epsilon            = 0.000000000001;
...
struct Strategy_SignalMetrics
  {
   ...
   int selected_split;
   double pooled_intercept;
   double pooled_slope;
   double pooled_rss;
   double left_intercept;
   double left_slope;
   double left_rss;
   double recent_intercept;
   double recent_slope;
   double recent_rss;
   double split_rss;
   double improvement;
...
bool Strategy_LoadCompletedCloses(const int current_month_key,
                                  const int label_offset,
                                  double &closes[],
                                  datetime &endpoint_times[])
  {
   ...
   const int endpoint_target = strategy_observation_count;
   if(current_month_key <= 0 ||
      strategy_observation_count != 252 || endpoint_target != 252 ||
   ...
   ArraySetAsSeries(bars, true);
   const int copied =
      CopyRates(_Symbol, // perf-allowed: one bounded 252-close scan behind a consumed monthly attempt or monthly restart validation.
                PERIOD_D1,
                1,
                strategy_history_bars,
                bars);
   ...
   for(int index = 0;
       ...
       ++index)
     {
      ...
      if(endpoint_count < 0 ||
         ...
         endpoint_count >= ArraySize(reverse_times))
         return false;
   ...
   if(endpoint_count != endpoint_target || endpoint_count != 252)
      return false;
   ...
   for(int index = 0; index < endpoint_count; ++index)
     {
      ...
      if(reverse_index < 0 ||
         ...
         reverse_index >= ArraySize(reverse_times))
         return false;
      ...
      if(closes[index] <= 0.0 ||
         ...
         (index > 0 && endpoint_times[index] <= endpoint_times[index - 1]))
         return false;
     }

   const int newest_month_key =
      Strategy_MonthKeyForTime(endpoint_times[endpoint_count - 1]);
   if(Strategy_NextMonthKey(newest_month_key) != current_month_key)
      return false;
   return true;
  }

bool Strategy_OlsFit(const double &values[],
                     const int start,
                     const int count,
                     double &intercept,
                     double &slope,
                     double &rss)
  {
   ...
   if(start < 0 || count < 2 || end <= start || end > value_count)
      return false;
   ...
   for(int index = start; index < end; ++index)
     {
      ...
      sum_xx += x * x;
      sum_xy += x * y;
      if(!MathIsValidNumber(sum_x) ||
         !MathIsValidNumber(sum_y) ||
         !MathIsValidNumber(sum_xx) ||
         !MathIsValidNumber(sum_xy))
         return false;
     }

   const double sample_count = (double)count;
   const double denominator =
      sample_count * sum_xx - sum_x * sum_x;
   if(!MathIsValidNumber(denominator) ||
      denominator <= strategy_rss_epsilon)
      return false;

   slope =
      (sample_count * sum_xy - sum_x * sum_y) / denominator;
   intercept = (sum_y - slope * sum_x) / sample_count;
   if(!MathIsValidNumber(intercept) || !MathIsValidNumber(slope))
      return false;

   for(int index = start; index < end; ++index)
     {
      const double fitted = intercept + slope * (double)index;
      const double residual = values[index] - fitted;
      const double square = residual * residual;
      if(!MathIsValidNumber(fitted) ||
         !MathIsValidNumber(residual) ||
         square < 0.0 || !MathIsValidNumber(square))
         return false;
      rss += square;
      if(!MathIsValidNumber(rss))
         return false;
     }
   return (rss >= 0.0);
  }

bool Strategy_ScannedRegressionBreakSignal(
        const double &closes[],
        Strategy_SignalMetrics &metrics)
  {
   ZeroMemory(metrics);
   metrics.observation_count = ArraySize(closes);

   if(strategy_observation_count != 252 ||
      strategy_split_min != 63 ||
      strategy_split_max != 189 ||
      MathAbs(strategy_score_threshold - 3.0) >
         0.000000000000001 ||
      MathAbs(strategy_rss_epsilon -
              0.0000000000000001) >
         0.00000000000000000001 ||
      MathAbs(strategy_improvement_tolerance -
              0.000000000001) >
         0.000000000000000001 ||
      MathAbs(strategy_slope_epsilon -
              0.000000000001) >
         0.000000000000000001 ||
      metrics.observation_count != strategy_observation_count)
      return false;

   double log_prices[];
   if(ArrayResize(log_prices, strategy_observation_count) !=
         strategy_observation_count)
      return false;
   for(int index = 0; index < strategy_observation_count; ++index)
     {
   ...
   metrics.newest_close =
      closes[strategy_observation_count - 1];
   metrics.direction = 0;
   metrics.selected_split = 0;
   metrics.score = 0.0;

   if(!Strategy_OlsFit(log_prices,
                       0,
                       strategy_observation_count,
                       metrics.pooled_intercept,
                       metrics.pooled_slope,
                       metrics.pooled_rss) ||
      metrics.pooled_rss <= strategy_rss_epsilon)
      return false;

   double best_score = -1.0;
   const double residual_dof =
      (double)(strategy_observation_count - 4);
   if(!MathIsValidNumber(residual_dof) ||
      residual_dof != 248.0)
      return false;

   for(int split = strategy_split_min;
       split <= strategy_split_max;
       ++split)
     {
      double left_intercept = 0.0;
      double left_slope = 0.0;
      double left_rss = 0.0;
      double recent_intercept = 0.0;
      double recent_slope = 0.0;
      double recent_rss = 0.0;
      if(!Strategy_OlsFit(log_prices,
                          0,
                          split,
                          left_intercept,
                          left_slope,
                          left_rss) ||
         !Strategy_OlsFit(log_prices,
                          split,
                          strategy_observation_count - split,
                          recent_intercept,
                          recent_slope,
                          recent_rss))
         return false;

      const double split_rss = left_rss + recent_rss;
      if(!MathIsValidNumber(split_rss) ||
         split_rss <= strategy_rss_epsilon)
         return false;

      double improvement = metrics.pooled_rss - split_rss;
      const double tolerance_scale =
         MathMax(1.0, MathAbs(metrics.pooled_rss));
      const double negative_limit =
         -strategy_improvement_tolerance * tolerance_scale;
      if(!MathIsValidNumber(improvement) ||
         !MathIsValidNumber(tolerance_scale) ||
         !MathIsValidNumber(negative_limit) ||
         improvement < negative_limit)
         return false;
      if(improvement < 0.0)
         improvement = 0.0;
      ...
      const double score =
         (improvement / 2.0) /
         (split_rss / residual_dof);
      if(!MathIsValidNumber(score) || score < 0.0)
         return false;

      // An exact tie deliberately replaces the older split: latest tie wins.
      if(score > best_score || score == best_score)
        {
         best_score = score;
         metrics.selected_split = split;
         metrics.left_intercept = left_intercept;
         metrics.left_slope = left_slope;
         metrics.left_rss = left_rss;
         metrics.recent_intercept = recent_intercept;
         metrics.recent_slope = recent_slope;
         metrics.recent_rss = recent_rss;
         metrics.split_rss = split_rss;
         metrics.improvement = improvement;
        }
     }

   if(metrics.selected_split < strategy_split_min ||
      metrics.selected_split > strategy_split_max ||
      best_score < 0.0 || !MathIsValidNumber(best_score))
      return false;
   metrics.score = best_score;
   if(metrics.score < strategy_score_threshold)
      return true;

   if(metrics.recent_slope > strategy_slope_epsilon)
      metrics.direction = 1;
   else if(metrics.recent_slope < -strategy_slope_epsilon)
      metrics.direction = -1;
   return true;
  }
...
bool Strategy_ExpectedDirectionForMonth(const int current_month_key,
                                        ...
                                        int &direction)
  {
   ...
   Strategy_SignalMetrics metrics;
   const bool valid =
      Strategy_LoadCompletedCloses(current_month_key,
                                   label_offset,
                                   closes,
                                   endpoint_times) &&
      Strategy_ScannedRegressionBreakSignal(closes, metrics);
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
   ...
   else
     {
      ...
      const bool endpoints_valid =
         Strategy_LoadCompletedCloses(g_decision_month_key,
                                      g_decision_label_offset,
                                      closes,
                                      endpoint_times);
      if(endpoints_valid)
        {
         g_signal_metrics.observation_count = ArraySize(closes);
         if(ArraySize(endpoint_times) ==
               g_signal_metrics.observation_count &&
            g_signal_metrics.observation_count ==
               strategy_observation_count)
           {
            g_oldest_endpoint_time = endpoint_times[0];
            g_newest_endpoint_time =
               endpoint_times[g_signal_metrics.observation_count - 1];
           }
        }

      g_signal_valid =
         endpoints_valid &&
         Strategy_ScannedRegressionBreakSignal(closes,
                                               g_signal_metrics);
      if(!g_signal_valid)
         g_signal_state = "regression_break_arithmetic_validation_failed";
      else if(g_signal_metrics.direction == 0)
         g_signal_state = "regression_break_below_threshold_or_zero_slope_flat";
      else if(g_signal_metrics.direction > 0)
         g_signal_state = "regression_break_positive_recent_slope_wti_long";
      else if(g_signal_metrics.direction < 0)
         g_signal_state = "regression_break_negative_recent_slope_wti_short";
      else
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"observation_count\":%d,\"selected_split\":%d,\"pooled_intercept\":%.12f,\"pooled_slope\":%.12f,\"pooled_rss\":%.12f,\"left_intercept\":%.12f,\"left_slope\":%.12f,\"left_rss\":%.12f,\"recent_intercept\":%.12f,\"recent_slope\":%.12f,\"recent_rss\":%.12f,\"split_rss\":%.12f,\"improvement\":%.12f,\"score\":%.12f,\"oldest_close\":%.10f,\"newest_close\":%.10f,\"oldest_endpoint\":%I64d,\"newest_endpoint\":%I64d,\"state\":\"%s\"}",
                            ...
                            g_signal_metrics.observation_count,
                            g_signal_metrics.selected_split,
                            g_signal_metrics.pooled_intercept,
                            g_signal_metrics.pooled_slope,
                            g_signal_metrics.pooled_rss,
                            g_signal_metrics.left_intercept,
                            g_signal_metrics.left_slope,
                            g_signal_metrics.left_rss,
                            g_signal_metrics.recent_intercept,
                            g_signal_metrics.recent_slope,
                            g_signal_metrics.recent_rss,
                            g_signal_metrics.split_rss,
                            g_signal_metrics.improvement,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41254 ||
      qm_magic_slot_offset != 0 || qm_rng_seed != 42 ||
      QM_FrameworkMagic() != 412540000)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41254_WTI_CHOW_BREAK";
   ...
   req.reason = (g_signal_metrics.direction > 0)
      ? "WTI_BREAK_RECENT_SLOPE_LONG"
      : "WTI_BREAK_RECENT_SLOPE_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41254 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI scanned two-regression structural-break trend holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41254_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41254\",\"ea\":\"wti-chow-break-tr\"}");
