"""Deterministic reference checks for QM5_41430 WTI hurricane-season continuation."""
...
def positive_week_signal(current_anchor: date, newest_first: list[Bar]) -> tuple[int, float]:
    """Mirror the immediately completed positive-week, long-only contract."""
...
class WtiHurricaneSeasonPositiveWeekReferenceTest(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_negative_week_is_flat_not_short(self) -> None:
    ...
    def test_two_and_six_sessions_are_rejected(self) -> None:
        for count in (2, 6):
            with self.assertRaisesRegex(ValueError, "session count"):
                positive_week_signal(self.current, package(self.prior, count, 80, 76))

    def test_missing_immediately_prior_week_is_rejected(self) -> None:
        skipped = self.current - timedelta(days=14)
        with self.assertRaisesRegex(ValueError, "session count"):
            positive_week_signal(self.current, package(skipped, 5, 80, 76))
    ...
    def test_ineligible_week_refuses_before_signal(self) -> None:
        with self.assertRaisesRegex(ValueError, "ineligible month"):
            positive_week_signal(date(2026, 11, 2), package(self.prior, 5, 80, 76))
