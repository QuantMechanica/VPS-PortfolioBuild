...
#property description "QM5_41430 WTI Hurricane-Season Positive-Week Continuation"
...
//   - buy only after a strictly positive weekly open-to-close return
...
bool Strategy_LoadPositiveWeekSignal(const int current_week_key,
                                     ...
                                     int &direction)
  {
   ...
   // The hurricane-season sleeve is deliberately asymmetric: a strictly positive
   // completed week may continue long; nonpositive returns consume flat.
   ...
   if(Strategy_WeekAlreadyEntered(g_decision_week_key))
   ...
   else
     {
      ...
      if(!g_signal_valid)
         ...
         g_signal_state = "positive_week_continuation_long";
      else
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41430_WTI_HURR_POSWEEK_CONT";
   ...
   req.type = QM_BUY;
   req.reason = "WTI_HURR_POSWEEK_CONT_LONG";
...
int OnInit()
  {
   if(!SymbolSelect(strategy_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41430 ||
      qm_magic_slot_offset != 0)
   ...
   g_attempt_state_key =
      StringFormat("QM5_41430_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41430\",\"ea\":\"wti-hurr-posweek-cont\"}");
