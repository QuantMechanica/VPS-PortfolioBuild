...
period: D1
expected_trade_frequency: "Monthly WTI six-month time-series-momentum package; estimate 7-12 entries/year when the trailing six-month return clears the neutral band."
...
g0_approval_reasoning: "R1 PASS published JFE/AQR time-series-momentum source; R2 PASS deterministic monthly WTI six-month return-sign rule with ATR stop and time exits; R3 PASS XTIUSD.DWX available; R4 PASS no ML/grid/martingale/external data."
expected_pf: 1.08
...
CFD using a faster six-month trend horizon: once per month, trade
`XTIUSD.DWX` in the direction of its prior six-month D1 log return and flatten
the package at the next monthly rebalance or stale-position guard.

This is deliberately different from:

- `QM5_12603_wti-tsmom12m`: this card uses a six-month horizon, not the pure
  12-month return-sign package.
- `QM5_12616_tsmom-9m-commodity-xtiusd`: this card does not use a 9-month
  primary signal or a separate 3-month confirmation filter; it tests the
  faster six-month trend sleeve directly.
- `QM5_12594_yang-wti-reversal` and `QM5_12621_comm-reversal-4wk-xtiusd`:
  this follows intermediate WTI trend rather than fading return extremes.
- WTI calendar/event sleeves: no weekday, month-of-year, WPSR, hurricane,
  refinery, OPEC, or expiry-window trigger.
- XNG natural-gas sleeves: this is WTI oil exposure, not natural gas.
...
- Symbol: XTIUSD.DWX.
- Period: D1.
- Expected trade frequency: approximately 7-12 entries/year.
...
- expected_pf: 1.08
- expected_dd_pct: 18
- expected_trade_frequency: approximately 7-12 entries/year.
...
- [x] R2 mechanical: fixed monthly rebalance, six-month return-sign direction,
...
- [x] Non-duplicate: six-month WTI trend horizon, not 12-month WTI TSMOM, not
  9-month/3-month confirmed TSMOM, not WTI reversal, not WTI calendar/event
  logic, not XNG, not ratio-basket, and not RSI commodity pullback.

## Framework Alignment

- no_trade: D1 and XTIUSD.DWX guard, parameter guard, spread cap, and monthly
  rebalance gate.
- trade_entry: monthly six-month D1 return-sign long/short package.
...
|---|---|---|---|
| v1 | 2026-06-28 | initial structural WTI six-month TSMOM build | G0 | APPROVED |
