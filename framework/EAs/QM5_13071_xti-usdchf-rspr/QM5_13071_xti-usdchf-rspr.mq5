...
#property description "QM5_13071 XTI USDCHF Return Spread Reversion"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_13071 - WTI/USDCHF Return-Spread Reversion
// -----------------------------------------------------------------------------
// D1 two-leg energy/FX basket:
//   rspread = log(XTI[t] / XTI[t-L]) + beta_usdchf * log(USDCHF[t] / USDCHF[t-L])
//   z > entry: short return spread = sell WTI, sell USDCHF
//   z < -entry: long return spread = buy WTI, buy USDCHF
...
input double strategy_beta_usdchf         = 0.75;
...
input int    strategy_usdchf_max_spread_pts = 80;
...
bool Strategy_SpreadAllowed(const string symbol)
  {
   ...
   if(symbol == g_leg_usdchf && strategy_usdchf_max_spread_pts > 0)
      return (spread_points <= strategy_usdchf_max_spread_pts);
   return true;
  }
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(CopyClose(g_leg_xti, PERIOD_D1, 1, history_needed, xti) != history_needed) // perf-allowed: called only behind the D1 new-bar gate or close-state refresh.
      return false;
   if(CopyClose(g_leg_usdchf, PERIOD_D1, 1, history_needed, usdchf) != history_needed) // perf-allowed: called only behind the D1 new-bar gate or close-state refresh.
      return false;
   ...
   for(int i = 0; i < z_lookback; ++i)
     {
      ...
      if(xti[i] <= 0.0 || usdchf[i] <= 0.0 || xti[past_idx] <= 0.0 || usdchf[past_idx] <= 0.0)
         return false;

      const double xti_ret = MathLog(xti[i] / xti[past_idx]);
      const double usdchf_ret = MathLog(usdchf[i] / usdchf[past_idx]);
      spreads[i] = xti_ret + strategy_beta_usdchf * usdchf_ret;
...
bool Strategy_OpenPair(const int spread_direction)
  {
   if(spread_direction == 0 || Strategy_OpenPairLegCount() > 0)
      return false;
   if(!Strategy_SpreadAllowed(g_leg_xti) || !Strategy_SpreadAllowed(g_leg_usdchf))
      return false;

   const double xti_weight = 1.0;
   const double usdchf_weight = MathMax(0.1, MathAbs(strategy_beta_usdchf));
   const double weight_sum = xti_weight + usdchf_weight;
   ...
   const string reason = long_spread ? "QM5_13071_LONG_XTI_USDCHF_RSPREAD"
                                     : "QM5_13071_SHORT_XTI_USDCHF_RSPREAD";

   bool xti_ok = Strategy_OpenLeg(g_leg_xti, xti_type, xti_weight, weight_sum, reason);
   bool usdchf_ok = Strategy_OpenLeg(g_leg_usdchf, usdchf_type, usdchf_weight, weight_sum, reason);
   if(xti_ok && usdchf_ok)
     {
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart())
      return true;
   if(strategy_return_lookback_d1 < 5 || strategy_z_lookback_d1 < 40 || strategy_beta_usdchf <= 0.0)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_13071_RSPREAD_HOST";
   ...
   if(qm_news_temporal != QM_NEWS_TEMPORAL_OFF || qm_news_compliance != QM_NEWS_COMPLIANCE_NONE)
     {
      if(!QM_NewsAllowsTrade2(g_leg_xti, broker_time, qm_news_temporal, qm_news_compliance))
         return true;
      if(!QM_NewsAllowsTrade2(g_leg_usdchf, broker_time, qm_news_temporal, qm_news_compliance))
         return true;
     }
   else
     {
      if(!QM_NewsAllowsTrade(g_leg_xti, broker_time, qm_news_mode_legacy))
         return true;
      if(!QM_NewsAllowsTrade(g_leg_usdchf, broker_time, qm_news_mode_legacy))
         return true;
     }
   return false;
  }
...
int OnInit()
  {
   ...
   string basket_symbols[2] = {g_leg_xti, g_leg_usdchf};
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_13071\",\"ea\":\"xti-usdchf-rspr\"}");
