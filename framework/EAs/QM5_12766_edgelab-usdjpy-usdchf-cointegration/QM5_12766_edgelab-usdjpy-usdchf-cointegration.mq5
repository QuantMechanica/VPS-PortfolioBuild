...
#property description "QM5_12766 Edge Lab USDJPY USDCHF Cointegration"
...
input double strategy_beta              = 0.435197;
...
bool Strategy_IsHostSymbol()
  {
   return (_Symbol == g_leg_usdjpy || _Symbol == g_leg_usdchf);
  }
...
bool Strategy_EnsureBasketScope()
  {
   ...
   string allowed[2] = {"USDJPY.DWX", "USDCHF.DWX"};
   for(int i = 0; i < 2; ++i)
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(!Strategy_EnsureBasketScope())
      return false;
   if(!QM_SymbolAssertOrLog(g_leg_usdjpy) || !QM_SymbolAssertOrLog(g_leg_usdchf))
      return false;
   ...
   for(int i = 0; i < lookback; ++i)
     {
      if(usdjpy[i] <= 0.0 || usdchf[i] <= 0.0)
         return false;
      spreads[i] = MathLog(usdjpy[i]) - strategy_beta * MathLog(usdchf[i]);
...
bool Strategy_OpenPair(const int spread_direction)
  {
   ...
   const double weight_sum = usdjpy_weight + usdchf_weight;
   if(weight_sum <= 0.0)
      return false;

   const bool long_spread = (spread_direction > 0);
   const QM_OrderType usdjpy_type = long_spread ? QM_BUY : QM_SELL;
   const QM_OrderType usdchf_type = long_spread ? QM_SELL : QM_BUY;
   const string reason = long_spread ? "QM5_12766_LONG_SPREAD_Z_LT_NEG_ENTRY"
                                     : "QM5_12766_SHORT_SPREAD_Z_GT_POS_ENTRY";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12766_PAIR_HOST";
