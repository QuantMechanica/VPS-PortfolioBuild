...
#property description "QM5_41233 WTI Same-Calendar Five-Sample Gastwirth"
...
//   - reconstruct the upcoming calendar month's completed return in Y-5..Y-1
//   - require all five exact years without substitution
//   - compute GSL-linear quantiles at 1/3, 1/2, and 2/3
//   - aggregate them with the fixed Gastwirth weights 0.3/0.4/0.3
//   - verify the exact five-sample 0.2/0.6/0.2 order-statistic invariant
//   - follow only when the robust location is beyond +/-1e-12
...
input int    strategy_history_years       = 5;
input double strategy_gastwirth_lower_weight  = 0.3;
input double strategy_gastwirth_median_weight = 0.4;
input double strategy_gastwirth_upper_weight  = 0.3;
...
// -----------------------------------------------------------------------------
// Exact completed-month endpoint and five-sample Gastwirth helpers.
// -----------------------------------------------------------------------------
...
bool Strategy_GSLLinearQuantile5(const double &sorted_values[],
                                 const double fraction,
                                 double &quantile)
  {
   quantile = 0.0;
   if(ArraySize(sorted_values) < 5 ||
      !MathIsValidNumber(fraction) ||
   ...
   if(!MathIsValidNumber(h) || !MathIsValidNumber(delta) ||
      lower_index < 0 || lower_index > 4 ||
      delta < 0.0 || delta >= 1.0)
      return false;

   if(lower_index == 4)
      quantile = sorted_values[4];
   else
      quantile = (1.0 - delta) * sorted_values[lower_index] +
                 delta * sorted_values[lower_index + 1];
   return MathIsValidNumber(quantile);
  }

bool Strategy_GastwirthSignal(const double &observations[],
                              ...
                              int &direction)
  {
   ...
   if(strategy_history_years != 5 ||
      MathAbs(strategy_gastwirth_lower_weight - 0.3) > 1.0e-12 ||
      MathAbs(strategy_gastwirth_median_weight - 0.4) > 1.0e-12 ||
      MathAbs(strategy_gastwirth_upper_weight - 0.3) > 1.0e-12 ||
      MathAbs(strategy_signal_epsilon - 1.0e-12) > 1.0e-16 ||
      sample_count != strategy_history_years ||
      sample_count != 5 ||
      ArraySize(observations) < 5)
      return false;
   ...
   for(int index = 0; index < 5; ++index)
     {
      if(!MathIsValidNumber(sorted_values[index]) ||
         (index > 0 && sorted_values[index] < sorted_values[index - 1]))
         return false;
     }

   double q_one_third = 0.0;
   double q_one_half = 0.0;
   double q_two_thirds = 0.0;
   if(!Strategy_GSLLinearQuantile5(sorted_values,
                                   1.0 / 3.0,
                                   q_one_third) ||
   ...
   const double simplified_invariant =
      0.2 * sorted_values[1] +
      0.6 * sorted_values[2] +
      0.2 * sorted_values[3];
   if(!MathIsValidNumber(location_value) ||
      !MathIsValidNumber(simplified_invariant) ||
      MathAbs(location_value - simplified_invariant) > 1.0e-12)
      return false;

   if(location_value > strategy_signal_epsilon)
      direction = 1;
   else if(location_value < -strategy_signal_epsilon)
      direction = -1;
   return true;
  }

bool Strategy_LoadGastwirthSignal(
   ...
   int &direction)
  {
   ...
   // Exact year membership is load-bearing. Robust arithmetic sorts copies;
   // the original return vector remains in chronological year order.
   ...
   return Strategy_GastwirthSignal(observations,
                                   sample_count,
                                   location_value,
                                   direction);
  }
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   // Consume before history, Gastwirth arithmetic, news, spread, quote,
   // ATR, sizing, or order gates. A restart can never create a later retry.
   ...
   else
     {
      g_signal_valid =
          Strategy_LoadGastwirthSignal(
            ...
            g_signal_direction);
      if(!g_signal_valid)
         g_signal_state = "endpoint_sample_or_gastwirth_failed";
      else if(g_signal_direction > 0)
         g_signal_state = "fixed_gastwirth_long";
      else if(g_signal_direction < 0)
         g_signal_state = "fixed_gastwirth_short";
      else
         g_signal_state = "fixed_gastwirth_epsilon_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"valid\":%s,\"samples\":%d,\"fixed_gastwirth\":%.12g,\"direction\":%d,\"state\":\"%s\"}",
                            g_decision_month_key,
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(!QM_InputRequireLong("qm_ea_id", qm_ea_id, 41233) ||
      ...
      !QM_InputRequireDouble("strategy_gastwirth_lower_weight", strategy_gastwirth_lower_weight, 0.3, 1.0e-12) ||
      !QM_InputRequireDouble("strategy_gastwirth_median_weight", strategy_gastwirth_median_weight, 0.4, 1.0e-12) ||
      !QM_InputRequireDouble("strategy_gastwirth_upper_weight", strategy_gastwirth_upper_weight, 0.3, 1.0e-12) ||
      !QM_InputRequireDouble("strategy_signal_epsilon", strategy_signal_epsilon, 0.000000000001, 1.0e-16) ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(g_signal_direction > 0)
     {
      req.type = QM_BUY;
      req.reason = "WTI_SAMECAL_GAST5_LONG";
     }
   else
     {
      req.type = QM_SELL;
      req.reason = "WTI_SAMECAL_GAST5_SHORT";
     }
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41233 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI same-calendar five-sample Gastwirth card holds to the next broker month"))
      return INIT_FAILED;
   ...
   g_attempt_state_key =
      StringFormat("QM5_41233_SAMECAL_GAST5_MONTH_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41233\",\"ea\":\"wti-samecal-gast5\"}");
