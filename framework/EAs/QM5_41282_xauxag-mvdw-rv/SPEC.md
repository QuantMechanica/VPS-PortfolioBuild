# QM5_41282_xauxag-mvdw-rv - Strategy Spec
...
**Source:** `AI-CODEX-XAUXAG-MVDW-RV-20260902`
...
ascending. For each recent rank `r`, use the Van der Waerden normal score
`a(r)=Phi^-1(r/13)`. To prevent platform-dependent tail-boundary drift, the
scores are frozen as nearest-rounded signed integer numerators on denominator
`1e15`:
...
two-sided tail when `abs(S_perm_numerator)>=abs(S_observed_numerator)`. The
state qualifies when `tail_count<=462`. Positive `S` sells XAU and buys XAG;
negative `S` buys XAU and sells XAG; exact zero is flat. Ties, score-invariant
failure, invalid
...
| `strategy_relative_epsilon` | 1e-12 | locked | raw-change relative tie tolerance |
| `strategy_score_denominator` | 1000000000000000 | locked | frozen Van der Waerden-score lattice denominator |
...
- `XAUUSD.DWX` - exact D1 host, traded slot 0, governed magic `412820000`.
- `XAGUSD.DWX` - exact D1 companion, traded slot 1, governed magic
  `412820001`.
- `QM5_41282_XAU_XAG_VDW_RV_D1` - logical tester symbol hosted on XAU.
...
`strategy-seeds/sources/AI-CODEX-XAUXAG-MVDW-RV-20260902/source.md`.
Its approval is
`decisions/2026-09-02_xauxag_monthly_van_der_waerden_reversion_source_approval.md`;
G0 is
`decisions/2026-09-02_qm5_41282_xauxag_monthly_van_der_waerden_reversion_g0.md`.

Schweikert (2018) and CME Group support only the state-dependent carrier and
opposed-leg structure. NIST/SEMATECH and SAS/STAT provide the Van der Waerden
normal-quantile linear-rank score and exact two-sample test identity; van der Waerden supplies
...
This EA uses signed normal-quantile scores on raw pooled ranks and the exact
absolute tail of its signed integer sum. It is not Savage's harmonic
exponential-order statistic or a linear Wilcoxon rank sum. Locked fixtures
prove Van der Waerden-only, Wilcoxon-only, and Savage-only decisions:
`RRROOOORORRO`, `RRROROOOOORR`, and `RRROOOOOORRR`, respectively.
...
- `no_trade`: exact host, period, identity, slots, magics, risk, news, Friday,
  stress, strategy locks, clock, history, Van der Waerden arithmetic, and package state.
...
|---|---|---|---|
| v1 | 2026-09-02 | Initial build from approved card | OWNER commodity portfolio mission; governed magics `412820000` and `412820001` |
