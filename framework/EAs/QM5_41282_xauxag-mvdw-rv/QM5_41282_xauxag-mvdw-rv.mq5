...
#property description "QM5_41282 XAU/XAG Monthly Exact Van der Waerden Reversion"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_41282 - XAU/XAG Monthly Exact Van der Waerden Reversion
// -----------------------------------------------------------------------------
...
// map pooled ranks to locked signed normal-quantile scores, and enumerate
// every C(12,6)=924 label assignment for an inclusive exact two-sided absolute
// score tail. A qualified tail fades the recent block's signed score with an opposite,
...
input int    strategy_tail_count_max          = 462;
input double strategy_relative_epsilon        = 0.000000000001;
input long   strategy_score_denominator       = 1000000000000000;
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41282 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_MVDW_RV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41282_XAUXAG_MVDW_RV";
...
bool Strategy_VanDerWaerdenScoreNumeratorForRank(const int rank,
                                                 long &numerator)
  {
   numerator = 0;
   // Van der Waerden normal scores a(r)=Phi^-1(r/13), frozen as signed
   // integers after nearest rounding at 15 decimal places. The integral
   // lattice makes every exact-tail comparison platform independent.
   switch(rank)
     {
      case 1:  numerator = -1426076872272847; break;
      case 2:  numerator = -1020076232786202; break;
      case 3:  numerator =  -736315917376130; break;
      case 4:  numerator =  -502402223373355; break;
      case 5:  numerator =  -293381232121193; break;
      case 6:  numerator =   -96558615289639; break;
      case 7:  numerator =    96558615289639; break;
      case 8:  numerator =   293381232121193; break;
      case 9:  numerator =   502402223373355; break;
      case 10: numerator =   736315917376130; break;
      case 11: numerator =  1020076232786202; break;
      case 12: numerator =  1426076872272847; break;
      default: return false;
     }
   return true;
  }

long Strategy_AbsoluteScoreNumerator(const long value)
  {
   return (value < 0) ? -value : value;
  }

bool Strategy_VanDerWaerdenScoreInvariant()
  {
   if(strategy_score_denominator != 1000000000000000)
      return false;
   long total = 0;
   for(int rank = 1; rank <= 12; ++rank)
     {
      long numerator = 0;
      if(!Strategy_VanDerWaerdenScoreNumeratorForRank(rank, numerator))
         return false;
      total += numerator;
     }
   return (total == 0);
...
bool Strategy_VanDerWaerdenFromLabels(const int &labels[],
                                      double &score,
                                      long &score_numerator)
  {
   score = 0.0;
   score_numerator = 0;
   ...
   if(old_total != strategy_block_size ||
      ...
      !Strategy_VanDerWaerdenScoreInvariant())
      return false;
   if(strategy_score_denominator <= 0)
      return false;
   score = (double)score_numerator / (double)strategy_score_denominator;
...
bool Strategy_LoadMonthlyVanDerWaerdenSignal(
      ...
      int &direction)
  {
   ...
   if(!Strategy_VanDerWaerdenFromLabels(pooled_labels,
                                 observed_score,
                                 observed_score_numerator))
      return false;
   ...
   for(int mask = 0; mask < (1 << strategy_return_count); ++mask)
     {
      ...
      long permutation_score_numerator = 0;
      if(!Strategy_VanDerWaerdenFromLabels(permutation_labels,
                                    permutation_score,
                                    permutation_score_numerator))
         return false;
      ++assignment_count;
      if(Strategy_AbsoluteScoreNumerator(permutation_score_numerator) >=
   ...
   if(!qualifies || observed_score_numerator == 0)
      return true;

   // direction > 0 means BUY XAU / SELL XAG. A positive recent centered
   // Van der Waerden score is faded short XAU; a negative score is faded long XAU.
   // Neither score sign nor magnitude scales the fixed package risk.
   direction = (observed_score_numerator > 0) ? -1 : 1;
   return true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41282_XAUXAG_MVDW_RV_HOST";
   ...
   int tail_count = 0;
   long observed_score_numerator = 0;
   int direction = 0;
   ...
   string label_path = "";
   const bool valid =
      Strategy_LoadMonthlyVanDerWaerdenSignal(
         g_signal_month_key,
         month_count,
   ...
   string state = "monthly_vdw_signal_failed";
   if(valid && direction > 0)
      state = "qualified_negative_vdw_score_fade_long_xau";
   else if(valid && direction < 0)
      state = "qualified_positive_vdw_score_fade_short_xau";
   ...
   else if(valid && observed_score_numerator == 0)
      state = "zero_vdw_score_flat";
   else if(valid)
      state = "nonqualifying_exact_vdw_flat";

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"month_count\":%d,\"newest_endpoint_time\":%I64d,\"observed_score\":%.12e,\"observed_score_numerator\":%I64d,\"assignment_count\":%d,\"tail_count\":%d,\"tie_detected\":%s,\"direction\":%d,\"ratios\":\"%s\",\"returns\":\"%s\",\"pooled_labels\":\"%s\",\"state\":\"%s\"}",
                            g_signal_month_key,
                            month_count,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG monthly exact Van der Waerden reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41282\",\"ea\":\"xauxag-mvdw-rv\"}");
