...
#property description "QM5_11179 FT001 EMA Heikin-Ashi"
...
input int    strategy_ha_warmup_bars    = 120;
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(QM_EMA(_Symbol, tf, strategy_ema_slow, 2) <= 0.0)
      return true;
   ...
   if(strategy_max_spread_atr_pct > 0.0)
     {
      ...
      const double atr = QM_ATR(_Symbol, tf, strategy_ema_slow, 1);
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(strategy_stoploss_pct <= 0.0 || strategy_roi_0_pct <= 0.0)
      return false;
   ...
   if(ema_fast_1 <= 0.0 || ema_fast_2 <= 0.0 || ema_mid_1 <= 0.0 || ema_mid_2 <= 0.0)
      return false;

   const int warmup = MathMax(strategy_ha_warmup_bars, strategy_ema_slow + 2);
   MqlRates rates[];
   ArraySetAsSeries(rates, true);
   const int copied = CopyRates(_Symbol, tf, 1, warmup, rates);
   if(copied < strategy_ema_slow)
      return false;

   double ha_open_prev = 0.0;
   double ha_close_prev = 0.0;
   double ha_open_1 = 0.0;
   double ha_close_1 = 0.0;
   for(int bar = copied - 1; bar >= 0; --bar)
     {
      const double ha_close = (rates[bar].open + rates[bar].high + rates[bar].low + rates[bar].close) / 4.0;
      const double ha_open = (bar == copied - 1)
                             ? (rates[bar].open + rates[bar].close) / 2.0
                             : (ha_open_prev + ha_close_prev) / 2.0;
   ...
   if(!(ema_fast_1 > ema_mid_1 && ema_fast_2 <= ema_mid_2))
      return false;
   if(!(ha_close_1 > ema_fast_1 && ha_open_1 < ha_close_1))
      return false;
   ...
   req.reason = "ft001_ema_ha_long";
   req.symbol_slot = qm_magic_slot_offset;
   req.expiration_seconds = 0;
   if(req.sl <= 0.0 || req.tp <= 0.0 || req.sl >= entry || req.tp <= entry)
      return false;

   // perf-allowed: Heikin-Ashi needs raw OHLC; CopyRates is bounded and called only after the framework closed-bar gate.
   return true;
  }
...
void Strategy_ManageOpenPosition()
  {
   ...
   // perf-allowed: Heikin-Ashi needs raw OHLC; CopyRates is bounded and runs once per closed bar while a position is open.
   if(ema_mid_1 > ema_slow_1 && ema_mid_2 <= ema_slow_2 &&
      ha_close_1 < ema_fast_1 && ha_open_1 > ha_close_1)
      QM_TM_ClosePosition(active_ticket, QM_EXIT_OPPOSITE_SIGNAL);
  }
