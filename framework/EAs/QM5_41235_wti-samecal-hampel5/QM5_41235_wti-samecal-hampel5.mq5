...
#property description "QM5_41235 WTI Same-Calendar Five-Sample Hampel"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41235 - WTI Same-Calendar Five-Sample Redescending Hampel
// -----------------------------------------------------------------------------
...
//   - compute the odd median and raw MAD from sorted copies
//   - freeze the normalized MAD scale and execute exactly 32 Hampel updates
//   - use the fixed Hampel a=2, b=4, c=8 redescending weight boundaries
//   - follow only when the final robust location is beyond +/-1e-12
...
// -----------------------------------------------------------------------------
// Exact completed-month endpoint and five-sample-Hampel helpers.
// -----------------------------------------------------------------------------
...
bool Strategy_HampelSignal(const double &observations[],
                           ...
                           int &direction)
  {
   ...
   for(int index = 0; index < 5; ++index)
     {
      deviations[index] = MathAbs(observations[index] - median);
      if(!MathIsValidNumber(deviations[index]))
         return false;
     }
   ArraySort(deviations);
   if(ArraySize(deviations) != 5)
      return false;

   const double raw_mad = deviations[2];
   const double scale = strategy_mad_normalizer * raw_mad;
   if(raw_mad <= 0.0 || scale <= 0.0 ||
      !MathIsValidNumber(raw_mad) || !MathIsValidNumber(scale))
      return false;
...
bool Strategy_LoadHampelSignal(
   ...
   int &direction)
  {
   ...
   return Strategy_HampelSignal(observations,
                                sample_count,
                                location_value,
                                direction);
  }
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   // Consume before history, Hampel arithmetic, news, spread, quote,
   ...
   else
     {
      g_signal_valid =
          Strategy_LoadHampelSignal(
            ...
            g_signal_direction);
      if(!g_signal_valid)
         g_signal_state = "endpoint_sample_or_hampel_failed";
      else if(g_signal_direction > 0)
         g_signal_state = "fixed_hampel_long";
      else if(g_signal_direction < 0)
         g_signal_state = "fixed_hampel_short";
      else
         g_signal_state = "fixed_hampel_epsilon_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"valid\":%s,\"samples\":%d,\"fixed_hampel\":%.12g,\"direction\":%d,\"state\":\"%s\"}",
                            g_decision_month_key,
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(!QM_InputRequireLong("qm_ea_id", qm_ea_id, 41235) ||
      ...
      !QM_InputRequireDouble("strategy_hampel_a", strategy_hampel_a, 2.0, 1.0e-12) ||
      !QM_InputRequireDouble("strategy_hampel_b", strategy_hampel_b, 4.0, 1.0e-12) ||
      !QM_InputRequireDouble("strategy_hampel_c", strategy_hampel_c, 8.0, 1.0e-12) ||
      !QM_InputRequireLong("strategy_hampel_steps", strategy_hampel_steps, 32) ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "WTI_SAMECAL_HAMPEL5";
   ...
   if(g_signal_direction > 0)
     {
      req.type = QM_BUY;
      req.reason = "WTI_SAMECAL_HAMPEL5_LONG";
     }
   else
     {
      req.type = QM_SELL;
      req.reason = "WTI_SAMECAL_HAMPEL5_SHORT";
     }
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41235 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI same-calendar five-sample Hampel card holds to the next broker month"))
      return INIT_FAILED;
   ...
   g_attempt_state_key =
      StringFormat("QM5_41235_SAMECAL_HAMPEL5_MONTH_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41235\",\"ea\":\"wti-samecal-hampel5\"}");
