...
#property description "QM5_10093 GitHub SafeScalper EMA RSI Breakout"
...
input double strategy_breakout_atr_buffer        = 0.50;
input double strategy_ema_sep_atr_fraction       = 0.10;
input double strategy_buy_rsi_min                = 40.0;
input double strategy_buy_rsi_max                = 65.0;
input double strategy_sell_rsi_min               = 35.0;
input double strategy_sell_rsi_max               = 60.0;
input int    strategy_stop_points                = 150;
input int    strategy_take_points                = 200;
input bool   strategy_breakeven_enabled          = true;
input int    strategy_breakeven_trigger_points   = 100;
input int    strategy_breakeven_buffer_points    = 10;
...
int Strategy_PointsAsFixedPips(const int points)
  {
...
bool Strategy_BuildRequest(const QM_OrderType side, QM_EntryRequest &req)
  {
   ...
   const int stop_pips = Strategy_PointsAsFixedPips(strategy_stop_points);
   const int take_pips = Strategy_PointsAsFixedPips(strategy_take_points);
   ...
   req.reason = QM_OrderTypeIsBuy(side) ? "GH_SSP_BREAKOUT_LONG" : "GH_SSP_BREAKOUT_SHORT";
   return true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   const double min_sep = atr * strategy_ema_sep_atr_fraction;
   const bool long_signal =
      ema_fast > ema_slow &&
      MathAbs(ema_fast - ema_slow) >= min_sep &&
      close1 > ema_fast && close1 > ema_slow &&
      close1 > breakout_high + atr * strategy_breakout_atr_buffer &&
      rsi >= strategy_buy_rsi_min && rsi <= strategy_buy_rsi_max &&
      close1 > close2;
   ...
   const bool short_signal =
      ...
      MathAbs(ema_fast - ema_slow) >= min_sep &&
      close1 < ema_fast && close1 < ema_slow &&
      close1 < breakout_low - atr * strategy_breakout_atr_buffer &&
      rsi >= strategy_sell_rsi_min && rsi <= strategy_sell_rsi_max &&
      close1 < close2;

   if(long_signal == short_signal)
      return false;

   return Strategy_BuildRequest(long_signal ? QM_BUY : QM_SELL, req);
  }
...
void Strategy_ManageOpenPosition()
  {
   ...
   const int magic = QM_FrameworkMagic();
   const int trigger_pips = Strategy_PointsAsFixedPips(strategy_breakeven_trigger_points);
   const int buffer_pips = Strategy_PointsAsFixedPips(strategy_breakeven_buffer_points);
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      QM_TM_MoveToBreakEven(ticket, trigger_pips, buffer_pips);
     }
  }
