# QM5_13006_xti-nzd-rspread - Strategy Spec
...
as a two-leg basket on `XTIUSD.DWX` and `NZDUSD.DWX`. It computes the D1 log
spread:

`ln(XTIUSD) - beta_nzd * ln(NZDUSD)`

If the spread is rich by z-score it sells WTI and buys NZDUSD. If the spread is
cheap it buys WTI and sells NZDUSD. The package exits on spread z-score
reversion, max-hold expiry, broken-package repair, Friday close, or per-leg ATR
stops.

This is not a duplicate of `QM5_12837_wti-audnzd-mr`: that EA uses a three-leg
AUD/NZD hedge basket, while this one removes AUD and tests a pure WTI/NZD
residual spread. It is also distinct from WTI/AUDUSD breakout, WTI/CAD,
WTI/EURUSD, WTI/JPY, WTI/CHF, XTI/XNG, energy/metal, XAU/XAG, XNG RSI, and WTI
event/calendar builds.
...
| `strategy_beta_nzd` | 0.5 | 0.3-0.7 | NZDUSD coefficient in the log spread |
...
- `NZDUSD.DWX` - NZD commodity-FX leg, magic slot 1.
- Logical basket symbol: `QM5_13006_XTI_NZD_RSPREAD_D1`.
...
- Expected spread packages/year: about 5-10, default card estimate 7.
- Typical hold: days to several weeks.
- Regime preference: temporary WTI versus NZDUSD commodity-FX dislocations.
...
June 2017. RBNZ official material provides central-bank supplement for
