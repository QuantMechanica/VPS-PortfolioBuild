...
#property description "QM5_41384 WTI Two-Month Trend / Two-Month Hold"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41384 - WTI Two-Month Trend / Two-Month Hold
// -----------------------------------------------------------------------------
...
//   1. derive three consecutive completed WTI month-end closes from D1;
//   2. calculate the exact two-month return ln(C[2] / C[0]);
...
// The exact two-month endpoint pair remains independent of the two-month clock.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41384;
...
input int    strategy_return_months            = 2;
...
double g_cache_two_month_return    = 0.0;
...
void Strategy_ResetCachedState()
  {
   ...
   g_cache_signal = 0;
   g_cache_two_month_return = 0.0;
   g_cache_state_reason = "not_evaluated";
  }

bool Strategy_TwoMonthReturnSignal(const double &closes[],
                                   int &signal)
  {
   signal = 0;
   if(strategy_return_months != 2 ||
      ArraySize(closes) != 3)
     {
      g_cache_state_reason = "bad_two_month_contract";
      return false;
     }
   ...
   const double endpoint_ratio = closes[2] / closes[0];
   ...
   const double two_month_return = MathLog(endpoint_ratio);
   if(!MathIsValidNumber(two_month_return))
     {
      g_cache_state_reason = "invalid_two_month_return";
      return false;
     }
   ...
   if(!MathIsValidNumber(chained_return) ||
      MathAbs(two_month_return - chained_return) > 1.0e-10)
     {
      g_cache_state_reason = "endpoint_path_identity_failed";
      return false;
     }

   g_cache_two_month_return = two_month_return;
   if(two_month_return == 0.0)
     {
      g_cache_state_reason = "exact_zero_two_month_return";
      return true;
     }

   signal = (two_month_return > 0.0) ? 1 : -1;
...
bool Strategy_LoadSignalState(int &signal)
  {
   ...
   return Strategy_TwoMonthReturnSignal(monthly_closes,
                                        signal);
  }
...
void Strategy_PrepareBimonthlySignal()
  {
   ...
   QM_LogEvent(QM_INFO,
               "MONTHLY_STATE",
               StringFormat("{\"month\":%d,\"valid\":%s,\"signal\":%d,\"return_2m\":%.10f,\"state\":\"%s\"}",
                            g_cache_month_key,
                            g_cache_signal_valid ? "true" : "false",
                            g_cache_signal,
                            g_cache_two_month_return,
                            g_cache_state_reason));
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41384_WTI_TSMOM2_H2";
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41384_BIMONTH_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41384\",\"ea\":\"wti-tsmom2-h2\"}");
