...
#property description "QM5_12860 WTI Brent Return Shock Fade"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_12860 - WTI/Brent Return-Shock Fade
// -----------------------------------------------------------------------------
// D1 two-leg crude benchmark basket:
//   return_spread = log_return(Brent, N) - beta * log_return(WTI, N)
//   z > entry: short Brent return shock = sell Brent, buy WTI
//   z < -entry: long Brent recovery package = buy Brent, sell WTI
...
double Strategy_ReturnSpreadAt(const double &xti[], const double &xbr[], const int idx, const int return_lookback)
  {
   const double xti_now = xti[idx];
   const double xti_then = xti[idx + return_lookback];
   const double xbr_now = xbr[idx];
   const double xbr_then = xbr[idx + return_lookback];
   if(xti_now <= 0.0 || xti_then <= 0.0 || xbr_now <= 0.0 || xbr_then <= 0.0)
      return EMPTY_VALUE;

   const double xti_ret = MathLog(xti_now / xti_then);
   const double xbr_ret = MathLog(xbr_now / xbr_then);
   const double value = xbr_ret - strategy_beta * xti_ret;
   if(!MathIsValidNumber(value))
      return EMPTY_VALUE;
   return value;
  }

bool Strategy_RefreshReturnState()
  {
   g_state_ready = false;
   const int ret_lookback = MathMax(1, strategy_return_lookback_d1);
   const int z_lookback = MathMax(30, strategy_z_lookback_d1);
   const int history_needed = z_lookback + ret_lookback + 1;
   ...
   for(int i = 0; i < z_lookback; ++i)
     {
      spreads[i] = Strategy_ReturnSpreadAt(xti, xbr, i, ret_lookback);
      if(spreads[i] == EMPTY_VALUE)
         return false;
      sum += spreads[i];
     }

   g_return_mean = sum / (double)z_lookback;
   double var_sum = 0.0;
   for(int i = 0; i < z_lookback; ++i)
     {
      const double d = spreads[i] - g_return_mean;
      var_sum += d * d;
     }

   g_return_sd = MathSqrt(var_sum / (double)MathMax(1, z_lookback - 1));
   if(g_return_sd <= 0.0 || !MathIsValidNumber(g_return_sd))
      return false;

   g_return_z = (spreads[0] - g_return_mean) / g_return_sd;
   g_state_ready = MathIsValidNumber(g_return_z);
   if(g_state_ready)
...
bool Strategy_OpenPair(const int spread_direction)
  {
   if(spread_direction == 0 || Strategy_OpenPairLegCount() > 0)
      return false;
   if(!Strategy_SpreadAllowed(g_leg_xti) || !Strategy_SpreadAllowed(g_leg_xbr))
      return false;

   const double xbr_weight = 1.0;
   const double xti_weight = MathMax(0.1, MathAbs(strategy_beta));
   const double weight_sum = xbr_weight + xti_weight;
   const bool long_spread = (spread_direction > 0);
   const QM_OrderType xbr_type = long_spread ? QM_BUY : QM_SELL;
   const QM_OrderType xti_type = long_spread ? QM_SELL : QM_BUY;
   const string reason = long_spread ? "QM5_12860_LONG_BRENT_WTI_RSHOCK"
                                     : "QM5_12860_SHORT_BRENT_WTI_RSHOCK";
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart())
      return true;
   if(strategy_return_lookback_d1 < 1)
      return true;
   if(strategy_z_lookback_d1 < 30)
      return true;
   if(strategy_beta <= 0.0)
      return true;
   if(strategy_entry_z <= 0.0 || strategy_exit_z < 0.0 || strategy_entry_z <= strategy_exit_z)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12860_RSHOCK_HOST";
   req.symbol_slot = qm_magic_slot_offset;
   req.expiration_seconds = 0;

   if(!Strategy_RefreshReturnState())
      return false;
   if(Strategy_OpenPairLegCount() > 0)
      return false;

   if(g_return_z > strategy_entry_z)
      Strategy_OpenPair(-1);
   else if(g_return_z < -strategy_entry_z)
      Strategy_OpenPair(1);
...
bool Strategy_ExitSignal()
  {
   ...
   const datetime current_d1_bar = iTime(_Symbol, PERIOD_D1, 0); // perf-allowed: cheap D1 timestamp guard before optional return-state refresh.
   if(current_d1_bar > 0 && current_d1_bar != g_last_state_bar)
      Strategy_RefreshReturnState();
   if(g_state_ready && MathAbs(g_return_z) < strategy_exit_z)
      Strategy_ClosePair(QM_EXIT_STRATEGY);
...
int OnInit()
  {
   ...
   string basket_symbols[2] = {g_leg_xti, g_leg_xbr};
   QM_SymbolGuardInit(basket_symbols);
   QM_BasketWarmupHistory(basket_symbols,
                          PERIOD_D1,
                          MathMax(260, strategy_z_lookback_d1 + strategy_return_lookback_d1 + strategy_atr_period_d1 + 20));

   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_12860\",\"ea\":\"wti-brent-rshock\"}");
