...
#property description "QM5_11617 robo-psar001-ao-ema5-m30 — PSAR + AO + EMA5 trend strategy"
...
input double strategy_sar_step          = 0.01;
input double strategy_sar_max           = 0.10;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(close1 <= 0.0 || sar1 <= 0.0 || ema1 <= 0.0 || atr1 <= 0.0)
      return false;

   const double ao_fast = QM_SMA(_Symbol, _Period, strategy_ao_fast_period, 1, PRICE_MEDIAN);
   const double ao_slow = QM_SMA(_Symbol, _Period, strategy_ao_slow_period, 1, PRICE_MEDIAN);
   if(ao_fast <= 0.0 || ao_slow <= 0.0)
      return false;
   const double ao = ao_fast - ao_slow;

   QM_OrderType side;
   if(sar1 < close1 && ao > 0.0 && close1 > ema1)
      side = QM_BUY;
   else if(sar1 > close1 && ao < 0.0 && close1 < ema1)
      side = QM_SELL;
   else
      return false;
   ...
   const double sl = QM_StopRulesNormalizePrice(_Symbol, sar1);
...
void Strategy_ManageOpenPosition()
  {
   ...
   if(point <= 0.0)
      return;
   const double target_sl = QM_TM_NormalizePrice(_Symbol, sar1);
   if(target_sl <= 0.0)
      return;
