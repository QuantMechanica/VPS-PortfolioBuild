...
#property description "QM5_1115 Quantpedia Lunch Effect - SP500"

#include <QM/QM_Common.mqh>

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                       = 1115;
...
input int    strategy_short_entry_hour_ny   = 11;
input int    strategy_flip_hour_ny          = 12;
input int    strategy_long_exit_hour_ny     = 14;
input int    strategy_safety_exit_min_ny    = 5;

datetime g_last_spread_h1_bar = 0;
double   g_cached_median_spread = 0.0;
int      g_last_short_date_key = 0;
int      g_last_long_date_key = 0;
...
bool Strategy_IsRegularUsCashDay(const datetime broker_time)
  {
   ...
   if(Strategy_IsUsCashHoliday(ny_time))
      return false;
   if(Strategy_IsEarlyCloseDay(ny_time))
      return false;
   return true;
  }
...
bool Strategy_GetOurPosition(ulong &ticket, long &position_type)
  {
   ticket = 0;
   position_type = -1;
...
bool Strategy_NyBarTime(const datetime broker_bar_time, MqlDateTime &ny_dt)
  {
   if(broker_bar_time <= 0)
      return false;
   TimeToStruct(Strategy_BrokerToNewYork(broker_bar_time), ny_dt);
   return true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   const datetime bar_time = iTime(_Symbol, (ENUM_TIMEFRAMES)_Period, 0);
   MqlDateTime ny;
   if(!Strategy_NyBarTime(bar_time, ny))
      return false;
   ...
   const bool has_position = Strategy_GetOurPosition(ticket, position_type);

   if(ny.hour == strategy_short_entry_hour_ny && ny.min == 0)
     {
      if(has_position || g_last_short_date_key == date_key)
         return false;
      ...
      req.type = QM_SELL;
      req.price = bid;
      req.sl = QM_StopATRFromValue(_Symbol, QM_SELL, bid, atr, strategy_atr_sl_mult);
      if(req.sl <= bid)
         return false;
      req.reason = "QP_LUNCH_SP500_SHORT_1100_NY";
      g_last_short_date_key = date_key;
      return true;
     }

   if(ny.hour == strategy_flip_hour_ny && ny.min == 0)
     {
      if(has_position || g_last_long_date_key == date_key)
         return false;
      ...
      if(req.sl <= 0.0 || req.sl >= ask)
         return false;
      req.reason = "QP_LUNCH_SP500_LONG_1200_NY";
      g_last_long_date_key = date_key;
      return true;
     }
...
void Strategy_ManageOpenPosition()
  {
   // Card specifies a fixed intraday ATR hard stop only.
  }
...
bool Strategy_ExitSignal()
  {
   ulong ticket = 0;
   long position_type = -1;
   if(!Strategy_GetOurPosition(ticket, position_type))
      return false;

   MqlDateTime ny;
   if(!Strategy_NyBarTime(TimeCurrent(), ny))
      return false;

   const int date_key = ny.year * 10000 + ny.mon * 100 + ny.day;
   if(position_type == POSITION_TYPE_SELL && ny.hour >= strategy_flip_hour_ny)
      return true;

   if(position_type == POSITION_TYPE_BUY)
     {
      if(ny.hour > strategy_long_exit_hour_ny || (ny.hour == strategy_long_exit_hour_ny && ny.min >= 0))
        {
   ...
   if(ny.hour == strategy_long_exit_hour_ny && ny.min >= strategy_safety_exit_min_ny)
      return true;
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_1115\",\"ea\":\"qp-lunch-sp500\"}");
