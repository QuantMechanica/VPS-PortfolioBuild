...
#property description "QM5_11783 rsi14-8020-counter-trend-m15 — RSI(14) 80/20 counter-trend fade (M15)"

#include <QM/QM_Common.mqh>

// =============================================================================
// QuantMechanica V5 EA — QM5_11783 rsi14-8020-counter-trend-m15
// -----------------------------------------------------------------------------
// Source: 'Scalp using the RSI oscillator', in *My Top Three Scalping Trading
//   Strategies*, ~2019 (PDF 481460064-Top3ScalpingStrategies.pdf, pp.6-7).
// Card: artifacts/cards_approved/QM5_11783_rsi14-8020-counter-trend-m15.md
//   (g0_status APPROVED).
//
// Mechanics (counter-trend mean-reversion, closed-bar reads at shift 1; M15):
//   Concept     : Fade RSI(14) extremes at the 80/20 levels.
...
// the extreme level). Long and short can never fire on the same bar because the
// upper-fade and lower-fade conditions are mutually exclusive. The "was at the
// extreme" condition is read from the immediately-prior closed bar (shift 2),
// the cross-back from the trigger bar (shift 1) — a single fresh event per bar.
//
// .DWX invariants honoured:
//   - Spread guard fails OPEN on zero modeled spread (only a genuinely wide
//     spread blocks).
//   - No swap gate. RISK_FIXED in tester. Pip-correct SL/TP via QM_StopFixedPips
//     / QM_TakeRR (no raw points).
