...
#property description "QM5_12782 Katz seasonal crossover XTI D1"
...
bool Strategy_NoTradeFilter()
  {
   // XTI-specific port of the Katz Ch.8 seasonal model.
   if(!Strategy_IsXtiD1())
      return true;
   ...
   if(bullish_cross && stoch_k < strategy_stoch_long_max)
     {
      ...
      req.reason = "KATZ_XTI_SEASONAL_LONG_STOP";
   ...
   if(bearish_cross && stoch_k > strategy_stoch_short_min)
     {
      ...
      req.tp = NormalizeDouble(entry - atr * strategy_tp_atr_mult, digits);
      req.expiration_seconds = expiry_seconds;
      req.reason = "KATZ_XTI_SEASONAL_SHORT_STOP";
      return (req.sl > 0.0 && req.tp > 0.0);
     }
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_12782\",\"ea\":\"katz-seas-xti\"}");
