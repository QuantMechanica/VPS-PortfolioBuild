...
#property description "QM5_41020 WTI Fixed Week-Closing Segment Momentum"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41020 - WTI Fixed Week-Closing Segment Momentum
// -----------------------------------------------------------------------------
// Structural D1 crude-oil sleeve:
//   - require exact prior-Tuesday through prior-Friday plus current Monday
//   - follow log(prior-Friday close / prior-Tuesday close) on Monday
//   - consume one exact-Monday attempt before every fallible entry gate
//   - close at the first Wednesday D1 boundary; Friday close is a fail-safe
//   - repair malformed or out-of-lifecycle carry with one frozen ATR hard stop
// Native MT5 calendar/OHLC/history only; no external runtime data.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                     = 41020;
...
bool Strategy_LoadClosingSegment(const datetime current_bar_time,
                                 ...
                                 int &direction)
  {
   ...
   const int copied = CopyRates(_Symbol,          // perf-allowed: exact
                                PERIOD_D1,         // four-bar completed
                                1,                 // weekly sequence behind
                                4,                 // one QM_IsNewBar edge.
                                bars);
   if(copied != 4)
      return false;

   // Newest first: prior Friday, Thursday, Wednesday, Tuesday. Day-of-week
   // plus bounded timestamp gaps rejects holiday shifts and stale/missing
   // data while tolerating the broker's one-hour DST transitions.
   const datetime friday_time = bars[0].time + label_offset;
   const datetime thursday_time = bars[1].time + label_offset;
   const datetime wednesday_time = bars[2].time + label_offset;
   const datetime tuesday_time = bars[3].time + label_offset;
   if(Strategy_DayOfWeek(friday_time) != 5 ||
      Strategy_DayOfWeek(thursday_time) != 4 ||
      Strategy_DayOfWeek(wednesday_time) != 3 ||
      Strategy_DayOfWeek(tuesday_time) != 2)
      return false;
   if(!Strategy_GapHoursAllowed(current_session_time, friday_time, 68, 76) ||
      !Strategy_GapHoursAllowed(friday_time, thursday_time, 20, 28) ||
      !Strategy_GapHoursAllowed(thursday_time, wednesday_time, 20, 28) ||
      !Strategy_GapHoursAllowed(wednesday_time, tuesday_time, 20, 28))
      return false;
   ...
   closing_return = MathLog(bars[0].close / bars[3].close);
   if(!MathIsValidNumber(closing_return))
      return false;
   if(closing_return > 0.0)
      direction = 1;
   else if(closing_return < 0.0)
      direction = -1;
   return true;
  }
   ...
   for(int index = PositionsTotal() - 1; index >= 0; --index)
     {
      ...
      // Wednesday is the ordinary lifecycle exit. Retry throughout Wednesday,
      // Thursday, and Friday until the position is flat; Friday close remains
      // an independent framework fail-safe.
      if(!should_close &&
         now > opened &&
         (current_day == 3 || current_day == 4 || current_day == 5))
         should_close = true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   double closing_return = 0.0;
   int direction = 0;
   if(!Strategy_LoadClosingSegment(g_strategy_d1_bar_time,
                                   broker_now,
                                   closing_return,
                                   direction))
      return false;
   ...
   if(direction > 0)
     {
      req.type = QM_BUY;
      req.reason = "WTI_WEEK_CLOSE_MOM_LONG";
     }
   else if(direction < 0)
     {
      req.type = QM_SELL;
      req.reason = "WTI_WEEK_CLOSE_MOM_SHORT";
     }
   else
      return false;
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"date\":%d,\"closing_return\":%.12e,\"direction\":%d}",
                            date_key,
                            closing_return,
                            direction));
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI week-closing card keeps Friday 21 as a fail-safe"))
      return INIT_FAILED;
   ...
   g_attempt_state_key =
      StringFormat("QM5_41020_WEEK_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41020\",\"ea\":\"wti-wclose-mom\"}");
...
void OnTick()
  {
   ...
   // EntrySignal deliberately consumes the exact Monday before this entry-only
