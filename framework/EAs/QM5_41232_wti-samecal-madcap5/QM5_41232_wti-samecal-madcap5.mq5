...
#property description "QM5_41232 WTI Same-Calendar Five-Sample MAD Cap"
...
//   - freeze inclusive bounds at median +/- three raw MADs
//   - retain all five clipped observations with equal weight and divisor five
//   - follow only when the capped location is beyond +/-1e-12
...
// -----------------------------------------------------------------------------
// Exact completed-month endpoint and five-sample MAD-cap helpers.
// -----------------------------------------------------------------------------
...
bool Strategy_MADCapSignal(const double &observations[],
                           ...
                           int &direction)
  {
   ...
   if(raw_mad <= 0.0 || !MathIsValidNumber(raw_mad))
      return false;
   ...
   if(!MathIsValidNumber(lower_bound) ||
      !MathIsValidNumber(upper_bound) ||
...
bool Strategy_LoadMADCapSignal(
   ...
   int &direction)
  {
   ...
   return Strategy_MADCapSignal(observations,
                                sample_count,
                                location_value,
                                direction);
  }
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   // Consume before history, MAD-cap arithmetic, news, spread, quote,
   ...
   else
     {
      g_signal_valid =
          Strategy_LoadMADCapSignal(
            ...
            g_signal_direction);
      if(!g_signal_valid)
         g_signal_state = "endpoint_sample_or_madcap_failed";
      else if(g_signal_direction > 0)
         g_signal_state = "fixed_madcap_long";
      else if(g_signal_direction < 0)
         g_signal_state = "fixed_madcap_short";
      else
         g_signal_state = "fixed_madcap_epsilon_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"valid\":%s,\"samples\":%d,\"fixed_madcap\":%.12g,\"direction\":%d,\"state\":\"%s\"}",
                            g_decision_month_key,
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(!QM_InputRequireLong("qm_ea_id", qm_ea_id, 41232) ||
      ...
      !QM_InputRequireDouble("strategy_mad_cap_mult", strategy_mad_cap_mult, 3.0, 1.0e-12) ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "WTI_SAMECAL_MADCAP5";
   ...
   if(g_signal_direction > 0)
     {
      req.type = QM_BUY;
      req.reason = "WTI_SAMECAL_MADCAP5_LONG";
     }
   else
     {
      req.type = QM_SELL;
      req.reason = "WTI_SAMECAL_MADCAP5_SHORT";
     }
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41232 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI same-calendar five-sample MAD-cap card holds to the next broker month"))
      return INIT_FAILED;
   ...
   g_attempt_state_key =
      StringFormat("QM5_41232_SAMECAL_MADCAP5_MONTH_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41232\",\"ea\":\"wti-samecal-madcap5\"}");
