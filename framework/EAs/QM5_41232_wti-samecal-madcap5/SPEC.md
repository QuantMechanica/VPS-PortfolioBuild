...
All five observations are mandatory. Each return uses the prior calendar
month's final completed close and the target month's final completed close,
with a following-month D1 bar confirming completion.

Keep the returns in chronological order and sort only copies. The locked
location is:
...
lower  = median - 3 * MAD
upper  = median + 3 * MAD
c[i]   = min(upper, max(lower, r[i]))
location = sum(c[0..4]) / 5
```

Buy only when `location > +1e-12`, sell only when
`location < -1e-12`, and consume flat inside the inclusive epsilon band or
on any invalid state. MAD must be strictly positive. Persist the month attempt
before every fallible history or entry gate. An accepted position holds to the
next broker month behind one frozen hard stop, subject to malformed-state and
40-day repair.
...
| `strategy_mad_cap_mult` | 3.0 | symmetric raw-MAD cap multiplier |
...
- Slot 0, deterministic magic `412320000`.
...
positions per year because only invalid history, nonpositive MAD, and an
epsilon tie stay flat. Q02 retires on zero trades, below five completed
positions in a full scored year, or nonpositive governed economics. It does
not tune any rule.

The locked disagreement fixtures prevent substitution by existing estimators:

- `[-.20,-.05,+.01,+.03,+.19]` clips to
  `[-.17,-.05,+.01,+.03,+.19]`, yielding `+.002` and BUY; raw mean,
  middle-three trim, endpoint Winsor, midhinge, shortest-three,
  inclusive-pair pseudomedian, and the fixed bisquare sibling SELL, while the
  trimean is flat.
- `[-.15,-.03,0,+.03,+.04]` clips to
  `[-.09,-.03,0,+.03,+.04]`, yielding `-.01` and SELL; median, trim,
  Winsor, trimean, midhinge, and pseudomedian siblings are flat, while
  shortest-three and fixed bisquare siblings buy.

Sign reflection reverses both mappings. The existing twelve-adjacent-month
MAD-cap EA uses a different information object and cannot satisfy exact
matching-calendar membership.
...
direction, and monthly renewal. The governed `MOP-WTI-MADCAP-2026` packet
fixes the raw-MAD center, symmetric three-MAD cap, inclusive clipping, and
equal-weight retention convention.
...
| exact sample, median/MAD, frozen cap, clipped mean | `Strategy_LoadMADCapSignal` and `Strategy_MADCapSignal` |
...
exact years, median/MAD indexes, frozen bounds, inclusive clipping, retention
of all five values, exact divisor, epsilon, both disagreement fixtures,
