...
#property description "QM5_41451 XAU/XAG Weekly NR2 CLV Momentum"
...
// closes. Trade only when the newest ratio-close range is strictly narrower
// than the prior range and its final close is in a strict outer quartile. Follow that
// relative settlement extreme through an equal-notional opposite-leg package,
// consume one attempt per week, and flatten at the next week boundary.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41451;
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41451 && qm_magic_slot_offset == 0 &&
            g_leg_xau != "" && g_leg_xag != "" &&
            g_leg_xau != g_leg_xag &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_NR2_CLV_MOM_ATTEMPT_WEEK", qm_ea_id);
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41451_XAUXAG_NR2_CLV_MOM";
...
bool Strategy_LoadNr2Clv(const int current_week,
                         int &direction)
  {
   direction = 0;
   g_newest_week_clv = 0.0;
   ...
   g_newest_week_clv =
      (g_week_ratio_final[0] - g_week_ratio_min[0]) /
      g_week_ratio_range[0];
   if(!MathIsValidNumber(g_newest_week_clv) ||
      g_newest_week_clv < 0.0 ||
      g_newest_week_clv > 1.0)
      return false;

   if(g_newest_week_clv > strategy_clv_upper)
      direction = 1;  // High ratio settlement: buy XAU, sell XAG.
   else if(g_newest_week_clv < strategy_clv_lower)
      direction = -1; // Low ratio settlement: sell XAU, buy XAG.
   return true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41451_XAUXAG_NR2_CLV_MOM_HOST";
   ...
   if(Strategy_OpenOwnedPositionCount() > 0 ||
     ...
     {
      signal_valid = Strategy_LoadNr2Clv(g_signal_week_key, direction);
      if(!signal_valid)
         g_signal_state = "ratio_week_validation_failed";
      else if(direction > 0)
         g_signal_state = "high_ratio_settlement_followed";
      else if(direction < 0)
         g_signal_state = "low_ratio_settlement_followed";
      else if(!g_range_contracted)
         g_signal_state = "range_not_contracted_flat";
      else
         g_signal_state = "ratio_clv_interior_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"week\":%d,\"decision_bar\":%I64d,\"late\":%s,\"valid\":%s,\"direction\":%d,\"prior_range\":%.12e,\"newest_range\":%.12e,\"newest_clv\":%.9f,\"oldest_week\":%d,\"newest_week\":%d,\"oldest_sessions\":%d,\"newest_sessions\":%d,\"state\":\"%s\"}",
                            g_signal_week_key,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG weekly ratio range-contraction CLV continuation package exits at the next broker week"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41451\",\"ea\":\"xauxag-nr2-clv-mom\"}");
