"""Independent mechanic fixtures for QM5_41059.

The suite covers native and prior-day energy-label normalization, the
three-hour decision grace, exact completed calendar-month endpoints, fixed
Y-1..Y-10 sampling, the five-sample floor, the non-negative binary map, the
inclusive source-defined q=0.40 boundary, the load-bearing disagreement with
mean and median, monthly renewal, and the 35-day survivor guard. It does not
...
def is_month_boundary(
    ...
    if current is None or current.date() != broker_now.date():
        return False, 0
    if broker_now - current > dt.timedelta(minutes=grace_minutes):
        return False, 0
...
def hit_rate_direction(
    ...
    if not all(math.isfinite(value) for value in observations):
        return None, 0, 0
    positives = sum(value >= 0.0 for value in observations)
    frequency = positives / len(observations)
    direction = 1 if frequency >= threshold else -1
    return frequency, positives, direction
...
class SameCalendarHitRateReferenceTests(unittest.TestCase):
    def test_native_month_boundary_inside_grace(self) -> None:
        boundary, offset = is_month_boundary(
            dt.datetime(2026, 8, 3),
            dt.datetime(2026, 7, 31),
            dt.datetime(2026, 8, 3, 3),
        )
    ...
    def test_prior_day_energy_labels_normalize_uniformly(self) -> None:
        boundary, offset = is_month_boundary(
            ...
            dt.datetime(2026, 8, 1, 2),
        )
        self.assertTrue(boundary)
        self.assertEqual(offset, 1)

    def test_after_three_hour_grace_is_rejected(self) -> None:
        boundary, _ = is_month_boundary(
            dt.datetime(2026, 8, 3),
            dt.datetime(2026, 7, 31),
            dt.datetime(2026, 8, 3, 3, 0, 1),
        )
    ...
    def test_q40_boundary_is_inclusive_and_below_is_short(self) -> None:
        at_boundary = [-0.04, -0.03, -0.02, 0.001, 0.001]
        below = [-0.04, -0.03, -0.02, -0.01, 0.001]
        self.assertEqual(hit_rate_direction(at_boundary), (0.4, 2, 1))
        self.assertEqual(hit_rate_direction(below), (0.2, 1, -1))

    def test_zero_is_non_negative_per_source_binary_map(self) -> None:
        frequency, positives, direction = hit_rate_direction(
            [-0.04, -0.03, -0.02, 0.0, 0.001]
        )
        self.assertEqual(frequency, 0.4)
        self.assertEqual(positives, 2)
        self.assertEqual(direction, 1)

    def test_q40_can_disagree_with_negative_mean_and_median(self) -> None:
        observations = [-0.04, -0.03, -0.02, 0.001, 0.001]
        frequency, _, direction = hit_rate_direction(observations)
        self.assertEqual(frequency, 0.4)
        self.assertEqual(direction, 1)
    ...
    def test_five_sample_floor_and_ten_year_cap(self) -> None:
        self.assertEqual(hit_rate_direction([0.01] * 4), (None, 0, 0))
        self.assertEqual(hit_rate_direction([0.01] * 5)[2], 1)
        self.assertEqual(hit_rate_direction([-0.01] * 10)[2], -1)
        self.assertEqual(hit_rate_direction([0.01] * 11), (None, 0, 0))
