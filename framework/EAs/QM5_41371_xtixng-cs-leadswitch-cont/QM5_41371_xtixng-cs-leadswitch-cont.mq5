...
#property description "QM5_41371 XTI/XNG Common-Shock Leader-Switch Continuation"
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41371 && qm_magic_slot_offset == 0 &&
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41371_XTIXNG_CS_LEADSWITCH_CONT";
...
bool Strategy_LoadLeaderSwitch(const int current_week,
                                    int &direction)
  {
   ...
   if(older_same_sign && newer_same_sign &&
      ...
      direction = 1; // WTI became the winner: buy XTI, sell XNG.
   else if(older_same_sign && newer_same_sign &&
           g_older_relative_return > strategy_signal_epsilon &&
           g_newer_relative_return < -strategy_signal_epsilon)
      direction = -1; // natural gas became the winner: sell XTI, buy XNG.
   return true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41371_XTIXNG_CS_LEADSWITCH_CONT_HOST";
   ...
   if(Strategy_OpenOwnedPositionCount() > 0 ||
   ...
   else
     {
      ...
      if(!signal_valid)
         ...
         g_signal_state = "wti_newest_winner_followed_long_xti";
      else if(direction > 0)
         g_signal_state = "xng_newest_winner_followed_short_xti";
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XTI/XNG common-shock leader-switch continuation package exits at the next broker week"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41371\",\"ea\":\"xtixng-cs-leadswitch-cont\"}");
