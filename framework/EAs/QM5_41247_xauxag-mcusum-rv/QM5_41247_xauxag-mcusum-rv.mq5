...
#property description "QM5_41247 XAU/XAG Monthly Centered-CUSUM Reversion"
...
// completed broker months. Center the twelve adjacent gold-minus-silver log
// returns, select one unique maximum absolute cumulative deviation in the
// fixed central split band, and fade the post-split relative-return mean with
// an opposite, equal-notional package held to the next month. Runtime is
// native, completed-price-only, deterministic, and one consumed attempt per
// month.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41247;
...
input int    strategy_min_split               = 4;
input int    strategy_max_split               = 8;
input double strategy_tie_epsilon             = 0.000000000001;
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_MCUSUM_RV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41247_XAUXAG_MCUSUM_RV";
...
bool Strategy_LoadMonthlyCusumSignal(
      const int current_month_key,
      int &month_count,
      datetime &newest_endpoint_time,
      double &mean_return,
      double &max_abs_cusum,
      int &selected_split,
      double &selected_cusum,
      int &maxima_count,
      double &post_mean,
      string &ratio_path,
      string &return_path,
      ...
      int &direction)
  {
   ...
   mean_return = return_sum / (double)ArraySize(relative_returns);
   if(!MathIsValidNumber(mean_return) || StringLen(return_path) <= 0)
      return false;

   double running = 0.0;
   for(int index = 0; index < ArraySize(cusums); ++index)
     {
      running += relative_returns[index];
      const int split = index + 1;
      const double value = running - (double)split * mean_return;
      if(!MathIsValidNumber(running) || !MathIsValidNumber(value))
         return false;
      cusums[index] = value;
      const double magnitude = MathAbs(value);
      if(magnitude > max_abs_cusum)
         max_abs_cusum = magnitude;
      if(StringLen(cusum_path) > 0)
         cusum_path += ",";
      cusum_path += DoubleToString(value, 12);
     }
   if(!MathIsValidNumber(max_abs_cusum) || StringLen(cusum_path) <= 0)
      return false;

   for(int index = 0; index < ArraySize(cusums); ++index)
     {
      const double distance =
         MathAbs(MathAbs(cusums[index]) - max_abs_cusum);
      if(!MathIsValidNumber(distance))
         return false;
      if(distance <= strategy_tie_epsilon)
        {
         ++maxima_count;
         if(selected_split == 0)
           {
            selected_split = index + 1;
            selected_cusum = cusums[index];
           }
        }
     }
   if(maxima_count <= 0 || selected_split <= 0)
      return false;

   const bool qualifies =
      max_abs_cusum > strategy_tie_epsilon &&
      maxima_count == 1 &&
      selected_split >= strategy_min_split &&
      selected_split <= strategy_max_split;
   if(!qualifies)
      return true;

   double post_sum = 0.0;
   for(int index = selected_split;
       index < ArraySize(relative_returns);
       ++index)
     {
      post_sum += relative_returns[index];
      if(!MathIsValidNumber(post_sum))
         return false;
     }
   const int post_count = ArraySize(relative_returns) - selected_split;
   if(post_count <= 0)
      return false;
   post_mean = post_sum / (double)post_count;
   if(!MathIsValidNumber(post_mean))
      return false;

   // direction > 0 means BUY XAU / SELL XAG. Fade the qualified post-split
   // relative-return mean; neither CUSUM nor mean magnitude scales risk.
   if(post_mean > strategy_tie_epsilon)
      direction = -1;
   else if(post_mean < -strategy_tie_epsilon)
      direction = 1;
   return true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41247_XAUXAG_MCUSUM_RV_HOST";
   ...
   double max_abs_cusum = 0.0;
   double selected_cusum = 0.0;
   double post_mean = 0.0;
   string ratio_path = "";
   string return_path = "";
   string cusum_path = "";
   const bool valid =
      Strategy_LoadMonthlyCusumSignal(
         g_signal_month_key,
         month_count,
   ...
   string state = "monthly_cusum_signal_failed";
   if(valid && direction > 0)
      state = "negative_post_mean_fade_long_xau";
   else if(valid && direction < 0)
      state = "positive_post_mean_fade_short_xau";
   else if(valid && max_abs_cusum <= strategy_tie_epsilon)
      state = "zero_cusum_path_flat";
   else if(valid && maxima_count != 1)
      state = "tied_cusum_maximum_flat";
   else if(valid &&
           (selected_split < strategy_min_split ||
            selected_split > strategy_max_split))
      state = "edge_cusum_split_flat";
   else if(valid)
      state = "zero_post_mean_flat";

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"month_count\":%d,\"newest_endpoint_time\":%I64d,\"mean_return\":%.12e,\"max_abs_cusum\":%.12e,\"selected_split\":%d,\"selected_cusum\":%.12e,\"maxima_count\":%d,\"post_mean\":%.12e,\"direction\":%d,\"ratios\":\"%s\",\"returns\":\"%s\",\"cusums\":\"%s\",\"state\":\"%s\"}",
                            g_signal_month_key,
                            month_count,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG monthly centered-CUSUM reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41247\",\"ea\":\"xauxag-mcusum-rv\"}");
