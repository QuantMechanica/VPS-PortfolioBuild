"""Independent deterministic fixtures for QM5_41230.

The suite covers normalized D1 labels, completed WTI month endpoints, exact
Y-5..Y-1 membership, fixed five-sample midhinge arithmetic, sign boundaries,
durable attempts, lifecycle, quotes, and static card/build conformance. It
does not invoke MT5.
"""
...
def midhinge_signal(observations: list[float]) -> MidhingeResult | None:
    if len(observations) != 5 or any(
        not math.isfinite(value) for value in observations
    ):
        return None
    sorted_values = tuple(sorted(observations))
    lower_hinge = sorted_values[1]
    upper_hinge = sorted_values[3]
    location = (lower_hinge + upper_hinge) / 2.0
...
class SameCalendarMidhingeReferenceTests(unittest.TestCase):
    ...
    def test_fixed_hinges_and_divisor_are_exact(self) -> None:
        result = midhinge_signal([-6.0, -4.0, -3.0, 5.0, 5.5])
        self.assertIsNotNone(result)
        assert result is not None
        self.assertEqual(result.lower_hinge, -4.0)
        self.assertEqual(result.upper_hinge, 5.0)
        self.assertEqual(result.location, 0.5)
        self.assertEqual(result.direction, 1)

    def test_disagreement_fixture_opposes_peer_estimators(self) -> None:
        values = [-6.0, -4.0, -3.0, 5.0, 5.5]
        result = midhinge_signal(values)
        raw_mean = sum(values) / 5.0
        trimmed_mean = sum(sorted(values)[1:4]) / 3.0
        winsor_mean = sum([-4.0, -4.0, -3.0, 5.0, 5.0]) / 5.0
        trimean = (-4.0 + 2.0 * -3.0 + 5.0) / 4.0
        shortest_three_mean = sum([-6.0, -4.0, -3.0]) / 3.0
        self.assertEqual(result.direction if result else 0, 1)
        self.assertLess(raw_mean, 0.0)
        self.assertLess(trimmed_mean, 0.0)
        self.assertLess(winsor_mean, 0.0)
        self.assertLess(trimean, 0.0)
        self.assertLess(shortest_three_mean, 0.0)

    def test_second_fixture_opposes_raw_mean_median_and_trimean(self) -> None:
        values = [-6.0, -5.0, 2.0, 3.0, 12.0]
        result = midhinge_signal(values)
        self.assertIsNotNone(result)
        assert result is not None
        self.assertEqual(result.location, -1.0)
        self.assertEqual(result.direction, -1)
        self.assertGreater(sum(values) / 5.0, 0.0)
        self.assertGreater(sorted(values)[2], 0.0)
        self.assertGreater((-5.0 + 2.0 * 2.0 + 3.0) / 4.0, 0.0)

    def test_median_and_extremes_do_not_enter_location(self) -> None:
        left = midhinge_signal([-100.0, -4.0, -3.0, 5.0, 100.0])
        right = midhinge_signal([-6.0, -4.0, 4.9, 5.0, 5.5])
        self.assertIsNotNone(left)
        self.assertIsNotNone(right)
        self.assertEqual(left.location if left else 9.0, 0.5)
        self.assertEqual(right.location if right else 9.0, 0.5)

    def test_sort_makes_year_order_irrelevant_after_membership(self) -> None:
        values = [-6.0, -5.0, 2.0, 3.0, 12.0]
        self.assertEqual(midhinge_signal(values), midhinge_signal(values[::-1]))

    def test_strict_epsilon_and_invalid_states_fail_closed(self) -> None:
        at_positive = midhinge_signal([EPSILON] * 5)
        at_negative = midhinge_signal([-EPSILON] * 5)
        beyond = midhinge_signal([EPSILON + 1.0e-15] * 5)
        ...
        self.assertIsNone(midhinge_signal([0.01] * 4))
        self.assertIsNone(midhinge_signal([0.01, 0.01, math.nan, 0.01, 0.01]))
    ...
    def test_static_build_contract_matches_approved_card(self) -> None:
        source_path = EA_DIR / "QM5_41230_wti-samecal-midhinge5.mq5"
        source = source_path.read_text(encoding="utf-8")
        set_path = (
            EA_DIR
            / "sets"
            / "QM5_41230_wti-samecal-midhinge5_XTIUSD.DWX_D1_backtest.set"
        )
        ...
        self.assertLess(
            ...
            prepare.index("Strategy_LoadMidhingeSignal"),
        )
        ...
        approved = (
            ...
            / "cards"
            / "approved"
            / "QM5_41230_wti-samecal-midhinge5_card.md"
        )
        ...
        self.assertIn(
            "41230,wti-samecal-midhinge5,0,XTIUSD.DWX,412300000",
            magic_rows,
        )
        resolver = (
            REPO_ROOT
        ...
        self.assertIn("412300000", resolver)
