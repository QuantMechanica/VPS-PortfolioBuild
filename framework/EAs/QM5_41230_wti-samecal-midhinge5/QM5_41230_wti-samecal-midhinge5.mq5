...
#property description "QM5_41230 WTI Same-Calendar Five-Sample Fixed Midhinge"
...
//   - sort the five returns and read fixed order statistics 1 and 3
//   - average only the lower and upper hinges; median and extremes are excluded
//   - follow only when the divisor-two fixed midhinge is beyond +/-1e-12
...
input int    strategy_lower_hinge_index   = 1;
input int    strategy_upper_hinge_index   = 3;
input int    strategy_midhinge_divisor     = 2;
...
// -----------------------------------------------------------------------------
// Exact completed-month endpoint and five-sample-midhinge helpers.
// -----------------------------------------------------------------------------
...
bool Strategy_MidhingeSignal(const double &observations[],
                             ...
                             int &direction)
  {
   ...
   if(strategy_history_years != 5 ||
      strategy_lower_hinge_index != 1 ||
      strategy_upper_hinge_index != 3 ||
      strategy_midhinge_divisor != 2 ||
   ...
   const double lower_hinge = sorted_values[1];
   const double upper_hinge = sorted_values[3];
   if(!MathIsValidNumber(lower_hinge) ||
      !MathIsValidNumber(upper_hinge) ||
      lower_hinge > sorted_values[2] ||
      sorted_values[2] > upper_hinge)
      return false;

   location_value = (lower_hinge + upper_hinge) / 2.0;
...
bool Strategy_LoadMidhingeSignal(
   ...
   int &direction)
  {
   ...
   return Strategy_MidhingeSignal(observations,
                                              sample_count,
                                              location_value,
                                              direction);
  }
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   // Consume before history, midhinge arithmetic, news, spread, quote,
   ...
   else
     {
      g_signal_valid =
          Strategy_LoadMidhingeSignal(
            ...
            g_signal_direction);
      if(!g_signal_valid)
         g_signal_state = "endpoint_sample_or_midhinge_failed";
      else if(g_signal_direction > 0)
         g_signal_state = "fixed_midhinge_long";
      else if(g_signal_direction < 0)
         g_signal_state = "fixed_midhinge_short";
      else
         g_signal_state = "fixed_midhinge_epsilon_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"valid\":%s,\"samples\":%d,\"fixed_midhinge\":%.12g,\"direction\":%d,\"state\":\"%s\"}",
                            g_decision_month_key,
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(!QM_InputRequireLong("qm_ea_id", qm_ea_id, 41230) ||
      ...
      !QM_InputRequireLong("strategy_lower_hinge_index", strategy_lower_hinge_index, 1) ||
      !QM_InputRequireLong("strategy_upper_hinge_index", strategy_upper_hinge_index, 3) ||
      !QM_InputRequireLong("strategy_midhinge_divisor", strategy_midhinge_divisor, 2) ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "WTI_SAMECAL_MIDHINGE5";
   ...
   if(g_signal_direction > 0)
     {
      req.type = QM_BUY;
      req.reason = "WTI_SAMECAL_MIDHINGE5_LONG";
     }
   else
     {
      req.type = QM_SELL;
      req.reason = "WTI_SAMECAL_MIDHINGE5_SHORT";
     }
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41230 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI same-calendar five-sample midhinge card holds to the next broker month"))
      return INIT_FAILED;
   ...
   g_attempt_state_key =
      StringFormat("QM5_41230_SAMECAL_MIDHINGE5_MONTH_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41230\",\"ea\":\"wti-samecal-midhinge5\"}");
