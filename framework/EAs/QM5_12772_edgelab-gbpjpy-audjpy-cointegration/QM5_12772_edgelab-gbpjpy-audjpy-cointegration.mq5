...
#property description "QM5_12772 Edge Lab GBPJPY AUDJPY Cointegration"
...
input double strategy_beta              = 0.725275;
...
bool Strategy_IsHostSymbol()
  {
   return (_Symbol == g_leg_gbpjpy || _Symbol == g_leg_audjpy);
  }
...
bool Strategy_EnsureBasketScope()
  {
   ...
   // Both traded legs settle in JPY, so MT5 also needs USDJPY conversion
   // history for USD-denominated tester accounting.
   string allowed[3] = {"GBPJPY.DWX", "AUDJPY.DWX", "USDJPY.DWX"};
   for(int i = 0; i < 3; ++i)
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(!Strategy_EnsureBasketScope())
      return false;
   if(!QM_SymbolAssertOrLog(g_leg_gbpjpy) || !QM_SymbolAssertOrLog(g_leg_audjpy))
      return false;
   ...
   for(int i = 0; i < lookback; ++i)
     {
      if(gbpjpy[i] <= 0.0 || audjpy[i] <= 0.0)
         return false;
      spreads[i] = MathLog(gbpjpy[i]) - strategy_beta * MathLog(audjpy[i]);
...
bool Strategy_OpenPair(const int spread_direction)
  {
   ...
   const double weight_sum = gbpjpy_weight + audjpy_weight;
   if(weight_sum <= 0.0)
      return false;
   ...
   const string reason = long_spread ? "QM5_12772_LONG_SPREAD_Z_LT_NEG_ENTRY"
                                     : "QM5_12772_SHORT_SPREAD_Z_GT_POS_ENTRY";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12772_PAIR_HOST";
