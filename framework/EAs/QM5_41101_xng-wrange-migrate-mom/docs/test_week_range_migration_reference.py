"""Deterministic reference checks for QM5_41101 weekly XNG range migration."""
...
def range_migration_signal(
    ...
    _, new_high, new_low, _ = weekly[0]
    _, parent_high, parent_low, _ = weekly[1]
...
class WeekRangeMigrationReferenceTest(unittest.TestCase):
    def signal(self, bars: list[Bar], offset: timedelta = timedelta(0)):
        return range_migration_signal(20260824, bars, offset)
    ...
    def test_strict_long_and_short_paths(self) -> None:
        ...
        self.assertEqual(values, (100.0, 120.0, 95.0, 118.0, 95.0, 110.0, 90.0, 100.0))
    ...
    def test_static_build_contract_is_fixed_risk_and_completed_data_only(self) -> None:
        source = (EA_DIR / "QM5_41101_xng-wrange-migrate-mom.mq5").read_text(encoding="utf-8")
        setfile = (EA_DIR / "sets" / "QM5_41101_xng-wrange-migrate-mom_XNGUSD.DWX_D1_backtest.set").read_text(encoding="utf-8")
        for marker in (
        ...
        self.assertLess(source.index("Strategy_RecordWeekAttempt(g_decision_week_key)"), source.index("Strategy_LoadRangeMigrationSignal(g_decision_week_key"))
        for marker in (
            "qm_ea_id=41101",
            "RISK_FIXED=1000",
            "RISK_PERCENT=0",
