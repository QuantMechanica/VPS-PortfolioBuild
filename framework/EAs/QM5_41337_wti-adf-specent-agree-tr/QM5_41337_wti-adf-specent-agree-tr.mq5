...
#property description "QM5_41337 WTI Monthly ADF-Spectral-Entropy Agreement Trend"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41337 - WTI Monthly ADF-Spectral-Entropy Agreement-Gated Trend
// -----------------------------------------------------------------------------
...
//   - require ADF t >= -2.594 and normalized spectral entropy <= 0.88
...
input int    strategy_spectral_return_count = 48;
...
struct Strategy_SignalMetrics
  {
   ...
   double se_gamma;
   double adf_t;
   int spectral_return_count;
   double spectral_return_mean;
   double spectral_total_power;
   double spectral_probability_sum;
...
bool Strategy_SpectralEntropyCore(const double &values[],
                                  ...
                                  string &power_path)
  {
   ...
   if(value_count != 48 || strategy_spectral_return_count != 48 ||
   ...
   for(int bin = 1; bin <= strategy_dft_bins; ++bin)
     {
      ...
      for(int index = 0; index < value_count; ++index)
        {
         const double angle =
            two_pi * (double)bin * (double)index / (double)value_count;
   ...
   for(int bin_index = 0; bin_index < strategy_dft_bins; ++bin_index)
     {
   ...
   if(!MathIsValidNumber(probability_sum) ||
      MathAbs(probability_sum - 1.0) > strategy_probability_tolerance ||
      !MathIsValidNumber(entropy_numerator))
      return false;

   const double entropy_denominator = MathLog((double)strategy_dft_bins);
...
bool Strategy_AgreementSignal(const double &closes[],
                              Strategy_SignalMetrics &metrics)
  {
   ...
   metrics.spectral_return_count = strategy_spectral_return_count;
   metrics.direction = 0;
   ...
   double returns[48];
   for(int index = 0; index < strategy_spectral_return_count; ++index)
     {
   ...
   if(!Strategy_SpectralEntropyCore(
         returns,
         metrics.spectral_return_mean,
         metrics.spectral_total_power,
         metrics.spectral_probability_sum,
...
bool Strategy_AgreementReferenceSelfTest()
  {
   ...
   string power_path = "";
   if(!Strategy_SpectralEntropyCore(
         paired_bin, return_mean, total_power, probability_sum,
         spectral_entropy, power_path) ||
      ...
      return false;
   if(!Strategy_SpectralEntropyCore(
         nyquist_bin, return_mean, total_power, probability_sum,
         spectral_entropy, power_path) ||
      MathAbs(total_power - 0.2304) > 0.0000000001 ||
      MathAbs(spectral_entropy) > 0.0000000001)
      return false;
   if(Strategy_SpectralEntropyCore(
         constant_returns, return_mean, total_power, probability_sum,
         spectral_entropy, power_path))
      return false;

   double up_closes[60];
   double down_closes[60];
   double spectral_only_closes[60];
   double high_entropy_closes[60];
   double constant_closes[60];
   double high_entropy_level = 4.5;
   for(int index = 0; index < 60; ++index)
     {
      ...
      const double spectral_only_level =
      ...
      spectral_only_closes[index] = MathExp(spectral_only_level);
      high_entropy_closes[index] = MathExp(high_entropy_level);
      ...
      if(index < 59)
        {
         ...
         high_entropy_level += next_return;
        }
     }
   ...
   if(!Strategy_AgreementSignal(up_closes, metrics) ||
      ...
      MathAbs(metrics.adf_t - (-0.28754973622603336)) >
         0.000000001 ||
      MathAbs(metrics.spectral_entropy - 0.6275447573250452) >
         0.000000001 ||
   ...
   if(!Strategy_AgreementSignal(down_closes, metrics) ||
      ...
      MathAbs(metrics.adf_t - (-0.34439061991466297)) >
         0.000000001 ||
      MathAbs(metrics.spectral_entropy - 0.46533690501576275) >
         0.000000001 ||
      MathAbs(metrics.momentum_12 - (-0.15505319427565833)) >
         0.000000001)
      return false;

   if(!Strategy_AgreementSignal(spectral_only_closes, metrics) ||
      metrics.adf_qualified || !metrics.entropy_qualified ||
      metrics.persistence_qualified || metrics.direction != 0 ||
      MathAbs(metrics.adf_t - (-20.092593377208576)) >
         0.000000001 ||
      MathAbs(metrics.spectral_entropy - 0.054309307685209675) >
         0.000000001)
      return false;

   if(!Strategy_AgreementSignal(high_entropy_closes, metrics) ||
      !metrics.adf_qualified || metrics.entropy_qualified ||
      metrics.persistence_qualified || metrics.direction != 0 ||
      MathAbs(metrics.adf_t - 0.4245704987551788) >
         0.000000001 ||
      MathAbs(metrics.spectral_entropy - 0.986608277678501) >
         0.000000001 ||
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   else
     {
      ...
      if(!g_signal_valid)
         g_signal_state = "agreement_validation_failed";
              ...
              !g_signal_metrics.entropy_qualified)
         g_signal_state = "both_gates_reject_consumed_flat";
      else if(!g_signal_metrics.adf_qualified)
         g_signal_state = "adf_rejects_spectral_only_consumed_flat";
      else if(!g_signal_metrics.entropy_qualified)
         g_signal_state = "entropy_rejects_adf_only_consumed_flat";
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"endpoint_count\":%d,\"level_count\":%d,\"spectral_return_count\":%d,\"mean_y\":%.16f,\"mean_z\":%.16f,\"mean_w\":%.16f,\"szz\":%.16f,\"sww\":%.16f,\"szw\":%.16f,\"szy\":%.16f,\"swy\":%.16f,\"determinant\":%.16f,\"alpha\":%.16f,\"gamma\":%.16f,\"phi\":%.16f,\"sse\":%.16f,\"residual_variance\":%.16f,\"se_gamma\":%.16f,\"adf_t\":%.12f,\"adf_qualified\":%s,\"spectral_return_mean\":%.12f,\"spectral_total_power\":%.16f,\"spectral_probability_sum\":%.12f,\"spectral_entropy\":%.12f,\"entropy_qualified\":%s,\"persistence_qualified\":%s,\"momentum_12\":%.12f,\"levels\":\"%s\",\"returns\":\"%s\",\"regression\":\"%s\",\"powers\":\"%s\",\"oldest_close\":%.10f,\"newest_close\":%.10f,\"oldest_endpoint\":%I64d,\"newest_endpoint\":%I64d,\"state\":\"%s\"}",
                            ...
                            g_signal_metrics.spectral_return_count,
                            ...
                            g_signal_metrics.spectral_return_mean,
                            g_signal_metrics.spectral_total_power,
                            g_signal_metrics.spectral_probability_sum,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41337 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41337_WTI_AGREE";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41337 ||
      qm_magic_slot_offset != 0)
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI monthly ADF-Spectral-Entropy agreement trend holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41337_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   g_entry_month_state_key =
      StringFormat("QM5_41337_ENTRY_MONTH_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41337\",\"ea\":\"wti-adf-specent-agree-tr\"}");
