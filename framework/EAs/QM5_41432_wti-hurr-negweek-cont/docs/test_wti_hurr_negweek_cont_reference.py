"""Deterministic reference checks for QM5_41432 WTI hurricane-season continuation."""
...
def hurricane_week(anchor: date) -> bool:
    return anchor.month in ELIGIBLE_MONTHS


def negative_week_signal(current_anchor: date, newest_first: list[Bar]) -> tuple[int, float]:
    """Mirror the immediately completed negative-week, short-only contract."""
    if not hurricane_week(current_anchor):
...
class WtiHurricaneSeasonNegativeWeekReferenceTest(unittest.TestCase):
    def setUp(self) -> None:
        self.current = date(2026, 9, 14)
        self.prior = self.current - timedelta(days=7)

    def test_negative_week_is_short(self) -> None:
        self.assertEqual(negative_week_signal(self.current, package(self.prior, 5, 80, 76))[0], -1)

    def test_positive_week_is_flat_not_long(self) -> None:
        self.assertEqual(negative_week_signal(self.current, package(self.prior, 5, 76, 80))[0], 0)

    def test_exact_zero_is_flat(self) -> None:
        direction, weekly_return = negative_week_signal(
            self.current, package(self.prior, 5, 78, 78)
        )
        self.assertEqual(direction, 0)
        self.assertEqual(weekly_return, 0.0)

    def test_all_hurricane_anchor_months_are_eligible(self) -> None:
        for value in (
            date(2026, 8, 3),
            date(2026, 9, 7),
            date(2026, 10, 5),
        ):
            self.assertTrue(hurricane_week(value))

    def test_nonhurricane_months_are_ineligible(self) -> None:
        for value in (date(2026, 7, 6), date(2026, 11, 2), date(2026, 4, 6)):
            self.assertFalse(hurricane_week(value))

    def test_week_crossing_into_august_uses_monday_anchor(self) -> None:
        self.assertFalse(hurricane_week(date(2026, 7, 27)))

    def test_week_crossing_into_november_uses_monday_anchor(self) -> None:
        self.assertTrue(hurricane_week(date(2026, 10, 26)))

    def test_three_to_five_sessions_are_accepted(self) -> None:
        for count in (3, 4, 5):
            self.assertEqual(
                negative_week_signal(self.current, package(self.prior, count, 80, 76))[0],
                -1,
            )
