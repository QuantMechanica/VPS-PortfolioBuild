...
#property description "QM5_41164 XAU/XAG Thirteen-Month Repeated-Median Ratio Reversion"
...
// completed broker months. Give each ratio endpoint a group of twelve
// forward-oriented slopes, take the even median inside each pivot group, then
// take the odd median across all thirteen pivots. Fade the strict repeated-
// median sign with an opposite, equal-notional package held to the next month.
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41164 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_MREPMEDIAN_RV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41164_XAUXAG_MREPMEDIAN_RV";
...
bool Strategy_LoadMonthlyRepeatedMedian(
      ...
      int &direction)
  {
   ...
   // direction > 0 means BUY XAU / SELL XAG. Fade only the strict repeated-
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41164_XAUXAG_MREPMEDIAN_RV_HOST";
   ...
   if(valid && direction > 0)
      state = "negative_repeated_median_fade_long_xau";
   else if(valid && direction < 0)
      state = "positive_repeated_median_fade_short_xau";
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG thirteen-month repeated-median ratio reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41164\",\"ea\":\"xauxag-mrepmedian-rv\"}");
