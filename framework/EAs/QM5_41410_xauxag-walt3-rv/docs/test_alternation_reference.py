"""Deterministic reference checks for QM5_41410 weekly alternation fade."""
...
MQ5 = EA_DIR / "QM5_41410_xauxag-walt3-rv.mq5"
SETFILE = (
    EA_DIR
    / "sets"
    / "QM5_41410_xauxag-walt3-rv_QM5_41410_XAU_XAG_WALT3_RV_D1_D1_backtest.set"
)
...
class AlternationReferenceTest(unittest.TestCase):
    def test_positive_negative_positive_fades_xau(self) -> None:
        self.assertEqual(alternation_signal([0.03, -0.02, 0.01]), -1)

    def test_negative_positive_negative_fades_xag(self) -> None:
        self.assertEqual(alternation_signal([-0.03, 0.02, -0.01]), 1)
    ...
    def test_static_artifacts_lock_identity_signal_and_fixed_risk(self) -> None:
        ...
        self.assertIn("qm_ea_id                    = 41410", source)
        ...
        self.assertRegex(resolver, re.compile(r"414100000.*414100001", re.DOTALL))
