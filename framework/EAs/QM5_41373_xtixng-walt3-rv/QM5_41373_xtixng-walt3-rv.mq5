...
#property description "QM5_41373 XTI/XNG Weekly Alternation Reversion"
...
// -----------------------------------------------------------------------------
// Reconstruct four synchronized week-end XTI/XNG close pairs for the four
...
double   g_xti_week_close[4]; // newest through oldest completed week.
double   g_xng_week_close[4];
int      g_week_key[4];
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41373 && qm_magic_slot_offset == 0 &&
            g_leg_xti != "" && g_leg_xng != "" &&
            g_leg_xti != g_leg_xng &&
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41373_XTIXNG_WALT3_RV";
...
bool Strategy_LoadAlternation(const int current_week,
                              int &direction)
  {
   ...
   if(g_relative_return[0] > 0.0 &&
      ...
      direction = -1; // Newest WTI winner: sell XTI, buy XNG.
   else if(g_relative_return[0] < 0.0 &&
           g_relative_return[1] > 0.0 &&
           g_relative_return[2] < 0.0)
      direction = 1; // Newest XNG winner: buy XTI, sell XNG.
   return true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41373_XTIXNG_WALT3_RV_HOST";
   ...
   if(Strategy_OpenOwnedPositionCount() > 0 ||
   ...
   else
     {
      ...
      if(!signal_valid)
         ...
         g_signal_state = "newest_xng_winner_faded";
      else if(direction < 0)
         g_signal_state = "newest_xti_winner_faded";
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XTI/XNG weekly strict alternation reversion package exits at the next broker week"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41373\",\"ea\":\"xtixng-walt3-rv\"}");
