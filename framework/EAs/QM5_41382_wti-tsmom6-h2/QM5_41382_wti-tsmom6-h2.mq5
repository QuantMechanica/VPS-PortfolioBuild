...
#property description "QM5_41382 WTI Six-Month Trend / Two-Month Hold"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41382 - WTI Six-Month Trend / Two-Month Hold
// -----------------------------------------------------------------------------
...
//   1. derive seven consecutive completed WTI month-end closes from D1;
//   2. calculate the exact six-month return ln(C[6] / C[0]);
...
// The exact six-month endpoint pair remains independent of the two-month clock.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41382;
...
input int    strategy_return_months            = 6;
...
double g_cache_six_month_return   = 0.0;
...
void Strategy_ResetCachedState()
  {
   ...
   g_cache_signal = 0;
   g_cache_six_month_return = 0.0;
   g_cache_state_reason = "not_evaluated";
  }

bool Strategy_SixMonthReturnSignal(const double &closes[],
                                   int &signal)
  {
   signal = 0;
   if(strategy_return_months != 6 ||
      ArraySize(closes) != 7)
     {
      g_cache_state_reason = "bad_six_month_contract";
      return false;
     }
   ...
   const double endpoint_ratio = closes[6] / closes[0];
   ...
   const double six_month_return = MathLog(endpoint_ratio);
   if(!MathIsValidNumber(six_month_return))
     {
      g_cache_state_reason = "invalid_six_month_return";
      return false;
     }
   ...
   if(!MathIsValidNumber(chained_return) ||
      MathAbs(six_month_return - chained_return) > 1.0e-10)
     {
      g_cache_state_reason = "endpoint_path_identity_failed";
      return false;
     }

   g_cache_six_month_return = six_month_return;
   if(six_month_return == 0.0)
     {
      g_cache_state_reason = "exact_zero_six_month_return";
      return true;
     }

   signal = (six_month_return > 0.0) ? 1 : -1;
...
bool Strategy_LoadSignalState(int &signal)
  {
   ...
   return Strategy_SixMonthReturnSignal(monthly_closes,
                                        signal);
  }
...
void Strategy_PrepareBimonthlySignal()
  {
   ...
   QM_LogEvent(QM_INFO,
               "MONTHLY_STATE",
               StringFormat("{\"month\":%d,\"valid\":%s,\"signal\":%d,\"return_6m\":%.10f,\"state\":\"%s\"}",
                            g_cache_month_key,
                            g_cache_signal_valid ? "true" : "false",
                            g_cache_signal,
                            g_cache_six_month_return,
                            g_cache_state_reason));
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41382_WTI_TSMOM6_H2";
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41382_BIMONTH_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41382\",\"ea\":\"wti-tsmom6-h2\"}");
