...
#property description "QM5_41033 WTI Weekly Opposed-Flow Dominance"
...
//   - enter next Monday only when the completed-flow signs oppose, following the reconciled total-flow sign
...
bool Strategy_LoadFlowDominance(const datetime current_bar_time,
                                 ...
                                 int &direction)
  {
   ...
   if(!MathIsValidNumber(overnight_flow) ||
      ...
      !MathIsValidNumber(week_return) ||
      !MathIsValidNumber(total_flow))
      return false;

   if(MathAbs(total_flow - week_return) >
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(!QM_InputRequireLong("qm_ea_id", qm_ea_id, 41033) ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   int direction = 0;
   if(!Strategy_LoadFlowDominance(g_strategy_d1_bar_time,
                                  broker_now,
                                  overnight_flow,
                                  session_flow,
                                  week_return,
   ...
   if(direction > 0)
     {
      req.type = QM_BUY;
      req.reason = "WTI_FLOW_DOM_LONG";
     }
   else if(direction < 0)
     {
      req.type = QM_SELL;
      req.reason = "WTI_FLOW_DOM_SHORT";
     }
   else
      return false;
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"date\":%d,\"overnight_flow\":%.12e,\"session_flow\":%.12e,\"week_return\":%.12e,\"total_flow\":%.12e,\"direction\":%d}",
                            date_key,
                            overnight_flow,
                            session_flow,
                            week_return,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI weekly opposed-flow dominance card uses Friday 21 as ordinary exit"))
      return INIT_FAILED;
   ...
   g_attempt_state_key =
      StringFormat("QM5_41033_FLOW_DOM_WEEK_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41033\",\"ea\":\"wti-flow-dom\"}");
