...
#property description "QM5_41025 WTI Exact-Day Prior-Month Momentum"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41025 - WTI Exact-Day / Prior-Month Momentum Agreement
// -----------------------------------------------------------------------------
// D1 structural crude-oil sleeve:
//   - buy exact broker day 8 only after a positive completed calendar month
//   - sell exact broker day 26 only after a negative completed calendar month
//   - consume the date before fallible gates; never shift, retry, or backfill
...
int      g_decision_date_key       = 0;
int      g_decision_day            = 0;
   ...
   for(int index = PositionsTotal() - 1; index >= 0; --index)
     {
      ...
      const datetime opened =
         (datetime)PositionGetInteger(POSITION_TIME);
      if(Strategy_DateKeyForTime(opened) == date_key)
         return true;
     }

   const datetime now = TimeCurrent();
   const datetime history_start = now - (long)4 * 86400;
   ...
   for(int index = deal_count - 1; index >= 0; --index)
     {
      ...
      if(Strategy_DateKeyForTime(deal_time) == date_key)
         return true;
     }
   return false;
  }
...
void Strategy_LoadAttemptState(const datetime reference_time)
  {
   ...
   const int current_date_key = Strategy_DateKeyForTime(reference_time);
   const double stored = GlobalVariableGet(g_attempt_state_key);
   const int stored_date_key = (int)MathRound(stored);
   if(current_date_key > 0 && MathIsValidNumber(stored) &&
      stored_date_key >= 19000101 && stored_date_key <= current_date_key)
     {
...
void Strategy_DetectDecisionClock_OnNewBar()
  {
   ...
   if(current_month_key <= 0 ||
      ...
      (decision_day != strategy_long_day &&
       decision_day != strategy_short_day))
      return;

   g_decision_bar = true;
   g_decision_month_key = current_month_key;
   g_decision_date_key = current_date_key;
   g_decision_day = decision_day;
...
void Strategy_PrepareDecisionSignal()
  {
   if(!g_decision_bar || g_decision_month_key <= 0 ||
      g_decision_date_key <= 0 || g_decision_day <= 0 ||
      g_decision_bar_time <= 0)
      return;

   if(g_decision_date_key == g_last_attempt_date_key)
     {
      g_signal_state = "date_already_consumed";
      return;
     }

   // Consume before endpoint validation, signal, news, spread, quote, ATR,
   // sizing, or order gates. The exact broker date is the prerequisite.
   if(!Strategy_RecordDateAttempt(g_decision_date_key))
     {
      g_signal_state = "attempt_persist_failed";
      return;
     }

   if(Strategy_DateAlreadyEntered(g_decision_date_key))
   ...
   else
     {
       ...
       if(!g_signal_valid)
       ...
       else if(g_decision_day == strategy_long_day &&
               g_signal_direction > 0)
          g_signal_state = "day8_positive_long";
       else if(g_decision_day == strategy_short_day &&
               g_signal_direction < 0)
          g_signal_state = "day26_negative_short";
         ...
         {
          g_signal_direction = 0;
          g_signal_state = "date_sign_disagreement_flat";
         }
      }

   QM_LogEvent(QM_INFO,
               "MONTHLY_STATE",
               StringFormat("{\"date\":%d,\"day\":%d,\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"prior_month_return\":%.12e,\"state\":\"%s\"}",
                            g_decision_date_key,
                            g_decision_day,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41025 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41025_WTI_DOM_MOM1";
   ...
   if(!g_decision_bar || g_late_decision ||
      g_decision_month_key <= 0 ||
      g_decision_date_key <= 0 ||
      g_decision_date_key != g_last_attempt_date_key ||
...
int OnInit()
  {
   if(!SymbolSelect(g_strategy_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41025 ||
   ...
   g_attempt_state_key =
      StringFormat("QM5_41025_DOM_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41025\",\"ea\":\"wti-dom-mom1\"}");
