...
#property description "QM5_41172 WTI Monthly Pettitt Central Change-Point Trend"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41172 - WTI Monthly Pettitt Central Change-Point Trend
// -----------------------------------------------------------------------------
// D1 structural crude-oil sleeve:
//   - reconstruct thirteen consecutive completed broker-month-end closes
//   - rank the closes and scan twelve signed cumulative rank sums
//   - follow the post-shift direction only for one central maximum split
...
bool Strategy_PettittSignal(const double &closes[],
   ...
   for(int index = 0; index < strategy_endpoint_count - 1; ++index)
     {
      ...
      const int signed_u =
         ...
         change_index * (strategy_endpoint_count + 1);
      const int absolute_u = (int)MathAbs(signed_u);
      if((signed_u % 2) != 0 || absolute_u > 42)
         return false;

      if(StringLen(metrics.u_path) > 0)
         metrics.u_path += ",";
      metrics.u_path += IntegerToString(signed_u);

      if(absolute_u > metrics.u_star)
        {
         metrics.u_star = absolute_u;
         metrics.change_index = change_index;
         metrics.signed_u = signed_u;
         metrics.maxima_count = 1;
        }
      else if(absolute_u == metrics.u_star)
        {
         ++metrics.maxima_count;
        }
     }
   if(metrics.u_star <= 0 || metrics.u_star > 42 ||
      metrics.maxima_count <= 0 || StringLen(metrics.u_path) <= 0)
      return false;

   metrics.oldest_close = closes[0];
   metrics.newest_close = closes[strategy_endpoint_count - 1];
   metrics.direction = 0;
   if(metrics.maxima_count == 1 &&
      metrics.change_index >= strategy_min_change_index &&
      metrics.change_index <= strategy_max_change_index)
     {
      if(metrics.signed_u < 0)
         metrics.direction = 1;
      else if(metrics.signed_u > 0)
         metrics.direction = -1;
      else
         return false;
     }
                                        ...
                                        int &direction)
  {
   ...
   const bool valid =
      Strategy_LoadMonthlyEndpoints(current_month_key,
                                    ...
                                    closes,
                                    endpoint_times) &&
      Strategy_PettittSignal(closes, metrics);
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
   ...
   else
     {
      ...
      g_signal_valid =
         endpoints_valid &&
         Strategy_PettittSignal(closes, g_signal_metrics);
      if(!g_signal_valid)
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41172 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41172_WTI_PETTITT_SHIFT";
   ...
   req.reason = (g_signal_metrics.direction > 0)
      ? "WTI_PETTITT_UPSHIFT_LONG"
      : "WTI_PETTITT_DOWNSHIFT_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41172 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI Pettitt central change-point trend holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41172_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41172\",\"ea\":\"wti-mpettitt-shift-tr\"}");
