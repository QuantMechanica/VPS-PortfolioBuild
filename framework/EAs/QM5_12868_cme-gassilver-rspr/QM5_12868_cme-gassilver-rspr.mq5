...
#property description "QM5_12868 XNG XAG Return Spread Reversion"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_12868 - XNG/XAG Return-Spread Reversion
// -----------------------------------------------------------------------------
// D1 two-leg natural gas/silver basket:
//   rspread = log(XNG[t] / XNG[t-L]) - beta * log(XAG[t] / XAG[t-L])
//   z > entry: short return spread = sell natural gas, buy silver
//   z < -entry: long return spread = buy natural gas, sell silver
   ...
   for(int i = 0; i < z_lookback; ++i)
     {
      ...
      if(xng[i] <= 0.0 || xag[i] <= 0.0 || xng[past_idx] <= 0.0 || xag[past_idx] <= 0.0)
         return false;

      const double xng_ret = MathLog(xng[i] / xng[past_idx]);
      const double xag_ret = MathLog(xag[i] / xag[past_idx]);
      spreads[i] = xng_ret - strategy_beta * xag_ret;
...
bool Strategy_OpenPair(const int spread_direction)
  {
   ...
   const string reason = long_spread ? "QM5_12868_LONG_XNG_XAG_RSPREAD"
                                     : "QM5_12868_SHORT_XNG_XAG_RSPREAD";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12868_RSPREAD_HOST";
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_12868\",\"ea\":\"cme-gassilver-rspr\"}");
