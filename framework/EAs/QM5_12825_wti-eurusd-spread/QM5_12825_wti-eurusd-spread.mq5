...
#property description "QM5_12825 WTI EURUSD Spread Mean Reversion"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_12825 - WTI/EURUSD oil-dollar spread mean reversion
// -----------------------------------------------------------------------------
// D1 two-leg basket:
//   spread = ln(XTIUSD.DWX) - beta * ln(EURUSD.DWX)
//   rich spread: sell XTI, buy EURUSD
//   cheap spread: buy XTI, sell EURUSD
// Runtime uses MT5 OHLC only; no macro feed, futures curve, API, or ML.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                     = 12825;
...
bool Strategy_SpreadAllowed(const string symbol)
  {
   ...
   if(symbol == g_leg_eurusd && strategy_eurusd_max_spread_pts > 0)
      return (spread_points <= strategy_eurusd_max_spread_pts);
   return true;
  }
...
bool Strategy_EntryTimeReady(const datetime broker_time)
  {
   ...
   if(!Strategy_SymbolTradeReady(g_leg_xti, broker_time))
      return false;
   if(!Strategy_SymbolTradeReady(g_leg_eurusd, broker_time))
      return false;
   return true;
  }
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(CopyClose(g_leg_xti, PERIOD_D1, 1, lookback, xti) != lookback) // perf-allowed: called only behind D1 new-bar or close-state refresh.
      return false;
   if(CopyClose(g_leg_eurusd, PERIOD_D1, 1, lookback, eurusd) != lookback) // perf-allowed: called only behind D1 new-bar or close-state refresh.
      return false;
   ...
   for(int i = 0; i < lookback; ++i)
     {
      if(xti[i] <= 0.0 || eurusd[i] <= 0.0)
         return false;
      spreads[i] = MathLog(xti[i]) - strategy_beta * MathLog(eurusd[i]);
...
bool Strategy_OpenPair(const int spread_direction)
  {
   if(spread_direction == 0 || Strategy_OpenPairLegCount() > 0)
      return false;
   if(!Strategy_SpreadAllowed(g_leg_xti) || !Strategy_SpreadAllowed(g_leg_eurusd))
      return false;

   const double xti_weight = 1.0;
   const double eurusd_weight = MathMax(0.1, MathAbs(strategy_beta));
   const double weight_sum = xti_weight + eurusd_weight;
   const bool long_spread = (spread_direction > 0);
   const QM_OrderType xti_type = long_spread ? QM_BUY : QM_SELL;
   const QM_OrderType eurusd_type = long_spread ? QM_SELL : QM_BUY;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12825_SPREAD_HOST";
