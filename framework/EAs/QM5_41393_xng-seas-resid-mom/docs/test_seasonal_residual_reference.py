"""Independent deterministic fixtures for QM5_41393.
...
class SeasonalResidualReferenceTests(unittest.TestCase):
    ...
    def test_static_build_contract_matches_approved_card(self) -> None:
        source = (EA_DIR / "QM5_41393_xng-seas-resid-mom.mq5").read_text(encoding="utf-8")
        set_path = EA_DIR / "sets" / "QM5_41393_xng-seas-resid-mom_XNGUSD.DWX_D1_backtest.set"
        ...
        self.assertRegex(
            setfile,
            r"(?m)^; build_hash:\s+(?:PENDING_BUILD_CHECK|[0-9a-f]{64})$",
        )
        self.assertEqual([set_path], list((EA_DIR / "sets").glob("*.set")))
        self.assertEqual([], list((EA_DIR / "sets").glob("*live*")))

        approved = REPO_ROOT / "strategy-seeds" / "cards" / "approved" / "QM5_41393_xng-seas-resid-mom_card.md"
        ...
        self.assertIn("41393,xng-seas-resid-mom,0,XNGUSD.DWX,413930000", magic_rows)
        resolver = (REPO_ROOT / "framework" / "include" / "QM" / "QM_MagicResolver.mqh").read_text(encoding="utf-8")
        self.assertIn("413930000", resolver)
