...
#property description "QM5_41472 EURGBP AUDJPY Cointegration"
...
input double strategy_beta              = -0.12202869296345396;
...
bool Strategy_OpenPair(const int spread_direction)
  {
   ...
   const double weight_sum = eurgbp_weight + audjpy_weight;
   if(weight_sum <= 0.0)
      return false;

   const bool long_spread = (spread_direction > 0);
   const QM_OrderType eurgbp_type = long_spread ? QM_BUY : QM_SELL;
   const QM_OrderType audjpy_type = long_spread
                                    ? (strategy_beta >= 0.0 ? QM_SELL : QM_BUY)
                                    : (strategy_beta >= 0.0 ? QM_BUY : QM_SELL);
   const string reason = long_spread ? "QM5_41472_LONG_SPREAD_Z_LT_NEG_ENTRY"
                                     : "QM5_41472_SHORT_SPREAD_Z_GT_POS_ENTRY";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41472_PAIR_HOST";
