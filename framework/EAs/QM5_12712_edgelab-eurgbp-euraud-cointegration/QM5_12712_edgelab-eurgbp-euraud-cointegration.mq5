...
#property description "QM5_12712 Edge Lab EURGBP EURAUD Cointegration"
...
input double strategy_beta              = 0.31;
...
bool Strategy_IsHostSymbol()
  {
   return (_Symbol == g_leg_eurgbp || _Symbol == g_leg_euraud);
  }
...
bool Strategy_EnsureBasketScope()
  {
   ...
   string allowed[5] = {"EURGBP.DWX", "EURAUD.DWX", "EURUSD.DWX", "GBPUSD.DWX", "AUDUSD.DWX"};
   for(int i = 0; i < 5; ++i)
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(!Strategy_EnsureBasketScope())
      return false;
   if(!QM_SymbolAssertOrLog(g_leg_eurgbp) || !QM_SymbolAssertOrLog(g_leg_euraud))
      return false;
   ...
   for(int i = 0; i < lookback; ++i)
     {
      if(eurgbp[i] <= 0.0 || euraud[i] <= 0.0)
         return false;
      spreads[i] = MathLog(eurgbp[i]) - strategy_beta * MathLog(euraud[i]);
...
bool Strategy_OpenPair(const int spread_direction)
  {
   ...
   const double eurgbp_weight = 1.0;
   const double euraud_weight = MathAbs(strategy_beta);
   const double weight_sum = eurgbp_weight + euraud_weight;
   if(weight_sum <= 0.0)
      return false;

   const bool long_spread = (spread_direction > 0);
   const QM_OrderType eurgbp_type = long_spread ? QM_BUY : QM_SELL;
   const QM_OrderType euraud_type = long_spread ? QM_SELL : QM_BUY;
   const string reason = long_spread ? "QM5_12712_LONG_SPREAD_Z_LT_NEG_ENTRY"
                                     : "QM5_12712_SHORT_SPREAD_Z_GT_POS_ENTRY";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12712_PAIR_HOST";
