...
#property description "QM5_41315 WTI Monthly ARCH-LM Trend"
...
//   - demean sixty adjacent monthly log returns and normalize their squares
//   - regress current normalized squares on an intercept and exact lags 1..6
//   - qualify ARCH_LM=54*centered-R2 at or above the pre-data 4.73 boundary
...
input int    strategy_month_returns        = 60;
input int    strategy_arch_lags            = 6;
input int    strategy_regression_rows      = 54;
input double strategy_energy_floor         = 0.000000000000000001;
input double strategy_sst_floor            = 0.000000000000000001;
input double strategy_arch_lm_boundary     = 4.73;
...
struct Strategy_SignalMetrics
  {
   ...
   double regression_y_mean;
   double centered_sst;
   double residual_sse;
   double centered_r_squared;
   double arch_lm;
   string coefficients;
   double momentum_12;
   string return_path;
...
bool Strategy_LoadMonthlyEndpoints(const int current_month_key,
                                   ...
                                   datetime &endpoint_times[])
  {
   ...
   if(current_month_key <= 0 ||
      strategy_month_returns != 60 || endpoint_target != 61 ||
...
bool Strategy_ARCHLMCore(const double &values[],
                         double &return_mean,
                         double &residual_energy,
                         double &regression_y_mean,
                         double &centered_sst,
                         ...
                         double &coefficients[])
  {
   return_mean = 0.0;
   residual_energy = 0.0;
   regression_y_mean = 0.0;
   centered_sst = 0.0;
   residual_sse = 0.0;
   centered_r_squared = 0.0;
   arch_lm = 0.0;
   ArrayResize(coefficients, 0);

   const int value_count = ArraySize(values);
   if(value_count != 60 || strategy_month_returns != 60 ||
      strategy_arch_lags != 6 || strategy_regression_rows != 54 ||
   ...
   double normalized_squares[60];
   for(int index = 0; index < value_count; ++index)
     {
      const double residual = values[index] - return_mean;
      const double square = residual * residual;
      if(!MathIsValidNumber(residual) ||
         !MathIsValidNumber(square) || square < 0.0)
         return false;
      normalized_squares[index] = square;
      residual_energy += square;
      if(!MathIsValidNumber(residual_energy))
         return false;
     }
   residual_energy /= (double)value_count;
   if(!MathIsValidNumber(residual_energy) ||
      residual_energy <= strategy_energy_floor)
      return false;
   for(int index = 0; index < value_count; ++index)
     {
      normalized_squares[index] /= residual_energy;
      if(!MathIsValidNumber(normalized_squares[index]) ||
         normalized_squares[index] < 0.0)
         return false;
     }
   ...
   for(int current = strategy_arch_lags;
       ...
       ++current)
     {
      const double y = normalized_squares[current];
      double x[7];
      x[0] = 1.0;
      for(int lag = 1; lag <= strategy_arch_lags; ++lag)
      ...
      for(int row = 0; row < 7; ++row)
        {
         ...
         if(!MathIsValidNumber(normal[row][7]))
            return false;
        }
      ++rows_built;
     }
   if(rows_built != strategy_regression_rows || rows_built != 54)
      return false;
   regression_y_mean /= (double)rows_built;
   if(!MathIsValidNumber(regression_y_mean))
      return false;

   // Deterministic partial-pivot Gauss-Jordan solve of the fixed 7x7 normal
   // equations. Normalized squared residuals keep the scale bounded.
   for(int pivot_col = 0; pivot_col < 7; ++pivot_col)
     {
      ...
      if(!MathIsValidNumber(pivot_abs) || pivot_abs <= 0.000000000001)
         return false;
      ...
      if(!MathIsValidNumber(divisor) ||
         MathAbs(divisor) <= 0.000000000001)
         return false;
      for(int col = pivot_col; col < 8; ++col)
        {
         normal[pivot_col][col] /= divisor;
         if(!MathIsValidNumber(normal[pivot_col][col]))
            return false;
        }
      for(int row = 0; row < 7; ++row)
        {
   ...
   if(ArrayResize(coefficients, 7) != 7)
      return false;
   for(int index = 0; index < 7; ++index)
     {
      coefficients[index] = normal[index][7];
      if(!MathIsValidNumber(coefficients[index]))
         return false;
     }
   ...
   for(int current = strategy_arch_lags;
       ...
       ++current)
     {
      ...
      for(int lag = 1; lag <= strategy_arch_lags; ++lag)
         fitted += coefficients[lag] *
                   normalized_squares[current - lag];
      const double centered_y = y - regression_y_mean;
      const double error = y - fitted;
      centered_sst += centered_y * centered_y;
      residual_sse += error * error;
      if(!MathIsValidNumber(fitted) ||
         !MathIsValidNumber(centered_sst) || centered_sst < 0.0 ||
         !MathIsValidNumber(residual_sse) || residual_sse < 0.0)
         return false;
     }
   if(centered_sst <= strategy_sst_floor)
      return false;

   centered_r_squared = 1.0 - residual_sse / centered_sst;
   if(!MathIsValidNumber(centered_r_squared) ||
      centered_r_squared < -0.0000000001 ||
      centered_r_squared > 1.0000000001)
      return false;
   centered_r_squared =
      MathMax(0.0, MathMin(1.0, centered_r_squared));
   arch_lm = (double)rows_built * centered_r_squared;
   return (MathIsValidNumber(arch_lm) && arch_lm >= 0.0 &&
           arch_lm <= (double)rows_built);
  }
   ...
   for(int index = 0; index < 60; ++index)
     {
      fixture[index] =
         ...
         0.007 * MathCos(0.17 * (double)index) +
         0.00008 * (double)index +
         0.003 * MathSin(0.09 * (double)(index * index));
      constant_path[index] = 0.01;
     }

   double return_mean = 0.0;
   double residual_energy = 0.0;
   double regression_y_mean = 0.0;
   double centered_sst = 0.0;
   double residual_sse = 0.0;
   double centered_r_squared = 0.0;
   double arch_lm = 0.0;
   double coefficients[];
   if(!Strategy_ARCHLMCore(fixture,
                           return_mean,
                           residual_energy,
                           regression_y_mean,
                           centered_sst,
                           ...
                           arch_lm,
                           coefficients) ||
      ArraySize(coefficients) != 7 ||
      MathAbs(return_mean - 0.001933307718746096) > 0.000000000001 ||
      MathAbs(residual_energy - 0.00010520479526597679) >
         0.000000000001 ||
      MathAbs(regression_y_mean - 0.8869341773646903) >
         0.000000001 ||
      MathAbs(centered_sst - 47.837224142288406) > 0.0000001 ||
      MathAbs(residual_sse - 16.520477503965697) > 0.0000001 ||
      MathAbs(centered_r_squared - 0.6546522545951513) >
         0.000000001 ||
      MathAbs(arch_lm - 35.35122174813817) > 0.0000001 ||
      MathAbs(coefficients[0] - 0.4208990758024) > 0.0000001 ||
      MathAbs(coefficients[1] - 0.8215833877861458) > 0.0000001 ||
      MathAbs(coefficients[6] - (-0.039301153660391525)) >
         0.0000001)
      return false;

   if(Strategy_ARCHLMCore(constant_path,
                          return_mean,
                          residual_energy,
                          regression_y_mean,
                          centered_sst,
...
bool Strategy_ARCHLMSignal(const double &closes[],
                           Strategy_SignalMetrics &metrics)
  {
   ...
   metrics.direction = 0;
   metrics.arch_qualified = false;

   if(strategy_month_returns != 60 || strategy_arch_lags != 6 ||
      strategy_regression_rows != 54 ||
      strategy_energy_floor != 0.000000000000000001 ||
      strategy_sst_floor != 0.000000000000000001 ||
      MathAbs(strategy_arch_lm_boundary - 4.73) >
         0.000000000000001 ||
   ...
   for(int index = 0; index < strategy_month_returns; ++index)
     {
      if(index < 0 || index + 1 >= metrics.endpoint_count || index >= 60)
         return false;
   ...
   double coefficients[];
   if(!Strategy_ARCHLMCore(returns,
                           metrics.return_mean,
                           metrics.residual_energy,
                           metrics.regression_y_mean,
                           metrics.centered_sst,
                           metrics.residual_sse,
                           metrics.centered_r_squared,
                           metrics.arch_lm,
                           coefficients) ||
      ArraySize(coefficients) != 7)
      return false;
   for(int index = 0; index < ArraySize(coefficients); ++index)
     {
      if(index > 0)
         metrics.coefficients += ",";
      metrics.coefficients += DoubleToString(coefficients[index], 12);
     }
   metrics.arch_qualified =
                                        ...
                                        int &direction)
  {
   ...
   const bool valid =
      Strategy_LoadMonthlyEndpoints(current_month_key,
                                    ...
                                    closes,
                                    endpoint_times) &&
      Strategy_ARCHLMSignal(closes, metrics);
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
   ...
   else
     {
      ...
      g_signal_valid =
         endpoints_valid &&
         Strategy_ARCHLMSignal(closes, g_signal_metrics);
      if(!g_signal_valid)
         g_signal_state = "arch_lm_validation_failed";
      else if(!g_signal_metrics.arch_qualified)
         g_signal_state = "arch_lm_below_boundary_consumed_flat";
      else if(g_signal_metrics.direction == 0)
         g_signal_state = "qualified_neutral_momentum_consumed_flat";
      else if(g_signal_metrics.direction > 0)
         g_signal_state = "arch_cluster_positive_momentum_wti_long";
      else if(g_signal_metrics.direction < 0)
         g_signal_state = "arch_cluster_negative_momentum_wti_short";
      else
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"endpoint_count\":%d,\"return_count\":%d,\"return_mean\":%.12f,\"residual_energy\":%.16f,\"regression_y_mean\":%.12f,\"centered_sst\":%.12f,\"residual_sse\":%.12f,\"centered_r_squared\":%.12f,\"arch_lm\":%.12f,\"arch_qualified\":%s,\"coefficients\":\"%s\",\"momentum_12\":%.12f,\"returns\":\"%s\",\"oldest_close\":%.10f,\"newest_close\":%.10f,\"oldest_endpoint\":%I64d,\"newest_endpoint\":%I64d,\"state\":\"%s\"}",
                            ...
                            g_signal_metrics.regression_y_mean,
                            g_signal_metrics.centered_sst,
                            g_signal_metrics.residual_sse,
                            g_signal_metrics.centered_r_squared,
                            g_signal_metrics.arch_lm,
                            g_signal_metrics.arch_qualified ? "true" : "false",
                            g_signal_metrics.coefficients,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41315 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41315_WTI_ARCHLM";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41315 ||
      qm_magic_slot_offset != 0)
      return INIT_PARAMETERS_INCORRECT;
   if(!Strategy_ARCHLMReferenceSelfTest())
      return INIT_FAILED;
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI monthly ARCH-LM trend holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41315_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   g_entry_month_state_key =
      StringFormat("QM5_41315_ENTRY_MONTH_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41315\",\"ea\":\"wti-archlm-tr\"}");
