...
#property description "QM5_12993 XNG XAU Return Spread Reversion"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_12993 - XNG/XAU Return-Spread Reversion
// -----------------------------------------------------------------------------
// D1 two-leg natural-gas/gold basket:
//   rspread = log(XNG[t] / XNG[t-L]) - beta * log(XAU[t] / XAU[t-L])
//   z > entry: short return spread = sell natural gas, buy gold
//   z < -entry: long return spread = buy natural gas, sell gold
// Runtime uses MT5 OHLC only; no external CME feed, weather data, or ML.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 12993;
   ...
   for(int i = 0; i < z_lookback; ++i)
     {
      ...
      if(xng[i] <= 0.0 || xau[i] <= 0.0 || xng[past_idx] <= 0.0 || xau[past_idx] <= 0.0)
         return false;

      const double xng_ret = MathLog(xng[i] / xng[past_idx]);
      const double xau_ret = MathLog(xau[i] / xau[past_idx]);
      spreads[i] = xng_ret - strategy_beta * xau_ret;
...
bool Strategy_OpenPair(const int spread_direction)
  {
   if(spread_direction == 0 || Strategy_OpenPairLegCount() > 0)
      return false;
   if(!Strategy_SpreadAllowed(g_leg_xng) || !Strategy_SpreadAllowed(g_leg_xau))
      return false;

   const double xng_weight = 1.0;
   const double xau_weight = MathMax(0.1, MathAbs(strategy_beta));
   const double weight_sum = xng_weight + xau_weight;
   ...
   const string reason = long_spread ? "QM5_12993_LONG_XNG_XAU_RSPREAD"
                                     : "QM5_12993_SHORT_XNG_XAU_RSPREAD";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12993_RSPREAD_HOST";
...
int OnInit()
  {
   ...
   string basket_symbols[2] = {g_leg_xng, g_leg_xau};
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_12993\",\"ea\":\"xnggold-rspread\"}");
