...
#property description "QM5_41271 WTI Siegel-Tukey Alternating-Extremes Scale Trend"
...
//   - reconstruct seventeen consecutive completed broker-month-end closes
//   - derive sixteen adjacent log returns in fixed older/recent blocks of eight
//   - assign consecutive ranks by alternating between the pooled extremes
//   - enumerate all 12,870 fixed-size eight/eight label assignments
//   - require the locked inclusive lower-tail state at score 68 / count 6,698
//   - continue the sign of the actual recent eight-return cumulative move
...
struct Strategy_SignalMetrics
  {
   ...
   int tail_count;
   int siegel_tukey_score;
   bool pooled_tie;
   double tail_fraction;
   double recent_return;
   string return_path;
   string sorted_membership_path;
   double oldest_close;
   double newest_close;
  };
...
bool Strategy_LoadMonthlyEndpoints(const int current_month_key,
                                   ...
                                   datetime &endpoint_times[])
  {
   ...
   if(current_month_key <= 0 ||
      strategy_endpoint_count != 17 || strategy_return_count != 16 ||
   ...
   ArraySetAsSeries(bars, true);
   const int copied =
      CopyRates(_Symbol, // perf-allowed: one bounded seventeen-month scan behind a consumed monthly attempt or monthly restart validation.
                PERIOD_D1,
                1,
                strategy_history_bars_d1,
                bars);
   ...
   if(endpoint_count != endpoint_target || endpoint_count != 17)
      return false;
...
bool Strategy_SiegelTukeyScoreForMask(const int mask,
                                      int &siegel_tukey_score)
  {
   siegel_tukey_score = 0;
   if(mask < 0 || mask >= 65536 ||
      Strategy_Popcount16(mask) != strategy_block_size)
      return false;

   for(int rank = 0; rank < strategy_return_count; ++rank)
     {
      const int rank_score = Strategy_SiegelTukeyScoreForRank(rank);
      if(rank_score <= 0)
         return false;
      if((mask & (1 << rank)) != 0)
         siegel_tukey_score += rank_score;
     }
   return (siegel_tukey_score >= 36 && siegel_tukey_score <= 100);
  }

bool Strategy_SiegelTukeySignal(const double &closes[],
                                Strategy_SignalMetrics &metrics)
  {
   ZeroMemory(metrics);
   metrics.return_path = "";
   metrics.sorted_membership_path = "";
   metrics.endpoint_count = ArraySize(closes);
   metrics.return_count = strategy_return_count;
   metrics.block_size = strategy_block_size;
   metrics.direction = 0;

   if(strategy_endpoint_count != 17 ||
   ...
   double ordered_returns[16];
   int ordered_labels[16];
   for(int index = 0; index < strategy_return_count; ++index)
     {
      ordered_returns[index] = returns[index];
      ordered_labels[index] =
         (index < strategy_block_size) ? 0 : 1;
     }
   ...
   for(int left = 0; left < strategy_return_count - 1; ++left)
     {
      for(int right = left + 1;
          ...
          ++right)
        {
         if(ordered_returns[right] < ordered_returns[left])
           {
            const double swap_return = ordered_returns[left];
            ordered_returns[left] = ordered_returns[right];
            ordered_returns[right] = swap_return;
            const int swap_label = ordered_labels[left];
            ordered_labels[left] = ordered_labels[right];
            ordered_labels[right] = swap_label;
           }
        }
     }
   ...
   for(int rank = 0; rank < strategy_return_count; ++rank)
     {
      if(rank > 0 && ordered_returns[rank] == ordered_returns[rank - 1])
        {
         metrics.pooled_tie = true;
         return true;
        }
      if(ordered_labels[rank] == 1)
         observed_mask |= (1 << rank);
      else if(ordered_labels[rank] != 0)
         return false;
      if(StringLen(metrics.sorted_membership_path) > 0)
         metrics.sorted_membership_path += ",";
      metrics.sorted_membership_path +=
         (ordered_labels[rank] == 1) ? "R" : "O";
     }

   if(Strategy_Popcount16(observed_mask) != strategy_block_size ||
      !Strategy_SiegelTukeyScoreForMask(observed_mask,
                                        metrics.siegel_tukey_score) ||
      StringLen(metrics.sorted_membership_path) <= 0)
      return false;
   ...
   for(int mask = 0; mask < 65536; ++mask)
     {
      if(Strategy_Popcount16(mask) != strategy_block_size)
         continue;
      int perm_score = 0;
      if(!Strategy_SiegelTukeyScoreForMask(mask, perm_score))
         return false;
      if(perm_score <= metrics.siegel_tukey_score)
         ++metrics.tail_count;
      ++metrics.assignment_count;
     }

   if(metrics.assignment_count != strategy_assignment_count ||
      metrics.tail_count < 1 ||
      metrics.tail_count > strategy_assignment_count)
      return false;
   metrics.tail_fraction =
      (double)metrics.tail_count / (double)metrics.assignment_count;
   if(!MathIsValidNumber(metrics.tail_fraction) ||
      metrics.tail_fraction <= 0.0 || metrics.tail_fraction > 1.0)
      return false;

   const bool score_qualified =
      (metrics.siegel_tukey_score <= strategy_score_max);
   const bool tail_qualified =
                                        ...
                                        int &direction)
  {
   ...
   const bool valid =
      Strategy_LoadMonthlyEndpoints(current_month_key,
                                    ...
                                    closes,
                                    endpoint_times) &&
      Strategy_SiegelTukeySignal(closes, metrics);
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
   ...
   else
     {
      ...
      g_signal_valid =
         endpoints_valid &&
         Strategy_SiegelTukeySignal(closes, g_signal_metrics);
      if(!g_signal_valid)
         g_signal_state = "siegel_tukey_validation_failed";
      else if(g_signal_metrics.direction == 0)
        {
         if(g_signal_metrics.pooled_tie)
            g_signal_state = "siegel_tukey_pooled_tie_flat";
         else if(g_signal_metrics.siegel_tukey_score > strategy_score_max)
            g_signal_state = "siegel_tukey_score_tail_flat";
         else
            g_signal_state = "siegel_tukey_neutral_return_flat";
        }
      else if(g_signal_metrics.direction > 0)
         g_signal_state = "siegel_tukey_wti_long";
      else if(g_signal_metrics.direction < 0)
         g_signal_state = "siegel_tukey_wti_short";
      else
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"endpoint_count\":%d,\"return_count\":%d,\"block_size\":%d,\"assignment_count\":%d,\"tail_count\":%d,\"siegel_tukey_score\":%d,\"recent_return\":%.12f,\"pooled_tie\":%s,\"tail_fraction\":%.12f,\"returns\":\"%s\",\"sorted_membership\":\"%s\",\"oldest_close\":%.10f,\"newest_close\":%.10f,\"oldest_endpoint\":%I64d,\"newest_endpoint\":%I64d,\"state\":\"%s\"}",
                            ...
                            g_signal_metrics.assignment_count,
                            g_signal_metrics.tail_count,
                            g_signal_metrics.siegel_tukey_score,
                            g_signal_metrics.recent_return,
                            g_signal_metrics.pooled_tie ? "true" : "false",
                            g_signal_metrics.tail_fraction,
                            g_signal_metrics.return_path,
                            g_signal_metrics.sorted_membership_path,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41271 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41271_WTI_ST_SCALE_TR";
   ...
   req.reason = (g_signal_metrics.direction > 0)
      ? "WTI_ST_SCALE_TR_LONG"
      : "WTI_ST_SCALE_TR_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41271 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI Siegel-Tukey alternating-extremes scale trend holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41271_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41271\",\"ea\":\"wti-msiegel-tukey-scale-tr\"}");
