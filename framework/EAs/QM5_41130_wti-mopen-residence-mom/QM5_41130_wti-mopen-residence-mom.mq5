...
#property description "QM5_41130 WTI Completed-Month Fixed-Open Residence Momentum"
...
//   - compare every month close with one immutable older boundary close
//   - require at least ceil(3*n/4) strict observations on one side
//   - verify the chronological log-return sum equals the endpoint return
//   - consume the month before fallible gates; never enter late or retry
//   - follow the residence-confirmed completed-month direction for one month
// Runtime uses MT5-native completed closes, calendar, and framework state only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                      = 41130;
...
bool Strategy_LoadOpenResidenceSignal(const int current_month_key,
                                      const int label_offset,
                                      int &direction,
                                      int &month_sessions,
                                      int &above_count,
                                      ...
                                      double &endpoint_return)
  {
   ...
   if(required_residence <= 0 || required_residence > month_sessions)
      return false;
   ...
   for(int index = month_sessions - 1; index >= 0; --index)
     {
      ...
      const double close_ratio = current_close / previous_close;
      if(close_ratio <= 0.0 || !MathIsValidNumber(close_ratio))
         return false;
      const double daily_return = MathLog(close_ratio);
   ...
   if(above_count + below_count + tie_count != month_sessions)
      return false;

   final_close = previous_close;
   const double endpoint_ratio = final_close / boundary_close;
   if(endpoint_ratio <= 0.0 || !MathIsValidNumber(endpoint_ratio))
      return false;
   endpoint_return = MathLog(endpoint_ratio);
   if(!MathIsValidNumber(endpoint_return) ||
      MathAbs(net_return - endpoint_return) > strategy_numerical_tolerance)
      return false;

   if(above_count >= required_residence && endpoint_return > 0.0)
      direction = 1;
   else if(below_count >= required_residence && endpoint_return < 0.0)
      direction = -1;
   return true;
  }
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadOpenResidenceSignal(g_decision_month_key,
                                          ...
                                          g_below_count,
                                          g_tie_count,
                                          g_required_residence,
                                          g_boundary_close,
                                          g_final_close,
                                          g_net_return,
                                          g_endpoint_return);
      if(!g_signal_valid)
         g_signal_state = "monthly_open_residence_validation_failed";
      else if(g_endpoint_return == 0.0)
         g_signal_state = "endpoint_equality_flat";
      else if(g_signal_direction > 0)
         g_signal_state = "fixed_open_residence_completed_month_long";
      else if(g_signal_direction < 0)
         g_signal_state = "fixed_open_residence_completed_month_short";
      else if(g_above_count < g_required_residence &&
              g_below_count < g_required_residence)
         g_signal_state = "insufficient_fixed_open_residence_flat";
      else
         g_signal_state = "residence_endpoint_disagreement_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
                StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"month_sessions\":%d,\"above\":%d,\"below\":%d,\"ties\":%d,\"required\":%d,\"boundary_close\":%.8f,\"final_close\":%.8f,\"log_return_sum\":%.12e,\"endpoint_return\":%.12e,\"state\":\"%s\"}",
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41130 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41130_WTI_MOPEN_RESIDENCE_MOM";
   ...
   req.reason = (g_signal_direction > 0)
      ? "WTI_MOPEN_RESIDENCE_MOM_LONG"
      : "WTI_MOPEN_RESIDENCE_MOM_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41130 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI completed-month fixed-open-residence position holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41130_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41130\",\"ea\":\"wti-mopen-residence-mom\"}");
