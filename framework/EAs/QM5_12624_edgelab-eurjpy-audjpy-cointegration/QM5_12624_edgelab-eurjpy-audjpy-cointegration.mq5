...
#property description "QM5_12624 Edge Lab EURJPY AUDJPY Cointegration"
...
input double strategy_beta              = 0.60;
...
bool Strategy_IsHostSymbol()
  {
   return (_Symbol == g_leg_eurjpy || _Symbol == g_leg_audjpy);
  }
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(!Strategy_EnsureBasketScope())
      return false;
   if(!QM_SymbolAssertOrLog(g_leg_eurjpy) || !QM_SymbolAssertOrLog(g_leg_audjpy))
      return false;
   ...
   if(CopyClose(g_leg_eurjpy, PERIOD_D1, 1, lookback, eur) != lookback) // perf-allowed: Strategy_EntrySignal is called only after the framework QM_IsNewBar gate.
      return false;
   if(CopyClose(g_leg_audjpy, PERIOD_D1, 1, lookback, aud) != lookback) // perf-allowed: Strategy_EntrySignal is called only after the framework QM_IsNewBar gate.
      return false;
   ...
   for(int i = 0; i < lookback; ++i)
     {
      if(eur[i] <= 0.0 || aud[i] <= 0.0)
         return false;
      spreads[i] = MathLog(eur[i]) - strategy_beta * MathLog(aud[i]);
...
bool Strategy_OpenPair(const int spread_direction)
  {
   ...
   const double aud_weight = MathAbs(strategy_beta);
   const double weight_sum = eur_weight + aud_weight;
   if(weight_sum <= 0.0)
      return false;
   ...
   const string reason = long_spread ? "QM5_12624_LONG_SPREAD_Z_LT_NEG_ENTRY"
                                     : "QM5_12624_SHORT_SPREAD_Z_GT_POS_ENTRY";

   bool eur_ok = Strategy_OpenLeg(g_leg_eurjpy, eur_type, eur_weight, weight_sum, reason);
   bool aud_ok = Strategy_OpenLeg(g_leg_audjpy, aud_type, aud_weight, weight_sum, reason);
   if(eur_ok && aud_ok)
     {
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12624_PAIR_HOST";
