...
#property description "QM5_20259 XNG Multi-Horizon Momentum Vote"
...
// -----------------------------------------------------------------------------
// Source lineage: Moskowitz, Ooi & Pedersen (2012) own-return-sign momentum.
// At each broker-month transition:
//   1. derive thirteen consecutive completed month-end closes from bounded D1 history;
//   2. calculate XNG's completed one-, three-, and twelve-month log-return signs;
//   3. buy when at least two signs are positive and sell when at least two are negative;
//   4. consume the month flat when any component is zero or the history is invalid.
// Runtime uses native XNGUSD.DWX price/broker data only. No external feed.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 20259;
...
double g_cache_fast_return      = 0.0;
double g_cache_medium_return    = 0.0;
double g_cache_slow_return      = 0.0;
...
bool Strategy_LoadMonthlyCloses(double &closes[])
  {
   ...
   if(strategy_fast_months != 1 ||
      ...
      strategy_slow_months != 12 ||
      strategy_required_votes != 2 ||
      strategy_history_bars_d1 < 400)
      return false;
   ...
   const int required_closes = strategy_slow_months + 1;
...
bool Strategy_StrictReturnSign(const double newest_close,
                               ...
                               int &return_sign)
  {
   return_value = 0.0;
   return_sign = 0;
   if(newest_close <= 0.0 || prior_close <= 0.0)
      return false;

   return_value = MathLog(newest_close / prior_close);
   if(!MathIsValidNumber(return_value) || return_value == 0.0)
      return false;

   return_sign = (return_value > 0.0) ? 1 : -1;
   return true;
  }

bool Strategy_MomentumVoteSignal(const double &closes[],
                                 int &signal)
  {
   signal = 0;
   g_cache_fast_return = 0.0;
   g_cache_medium_return = 0.0;
   g_cache_slow_return = 0.0;
   g_cache_vote_sum = 0;

   const int count = ArraySize(closes);
   if(count != 13 ||
      strategy_fast_months != 1 ||
      strategy_medium_months != 3 ||
      strategy_slow_months != 12 ||
      strategy_required_votes != 2)
      return false;

   const int newest_index = count - 1;
   int fast_sign = 0;
   int medium_sign = 0;
   int slow_sign = 0;
   if(!Strategy_StrictReturnSign(closes[newest_index],
                                 closes[newest_index - strategy_fast_months],
                                 g_cache_fast_return,
                                 fast_sign))
      return false;
   if(!Strategy_StrictReturnSign(closes[newest_index],
                                 closes[newest_index - strategy_medium_months],
                                 g_cache_medium_return,
                                 medium_sign))
      return false;
   if(!Strategy_StrictReturnSign(closes[newest_index],
                                 closes[newest_index - strategy_slow_months],
                                 g_cache_slow_return,
                                 slow_sign))
      return false;
   ...
   if(positive_votes >= strategy_required_votes)
      signal = 1;
   else if(negative_votes >= strategy_required_votes)
      signal = -1;

   return (signal != 0 &&
           ((signal > 0 && g_cache_vote_sum > 0) ||
            (signal < 0 && g_cache_vote_sum < 0)));
  }
...
bool Strategy_LoadSignalState(int &signal)
  {
   ...
   return Strategy_MomentumVoteSignal(monthly_closes, signal);
  }
...
void Strategy_AdvanceSignal_OnNewBar()
  {
   ...
   QM_LogEvent(QM_INFO,
               "MONTHLY_STATE",
               StringFormat("{\"month\":%d,\"valid\":%s,\"signal\":%d,\"vote\":%d,\"r1\":%.10f,\"r3\":%.10f,\"r12\":%.10f}",
                            g_cache_month_key,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 20259 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20259_XNG_MOM_VOTE";
   ...
   req.reason = (g_cache_signal > 0) ? "MOM_VOTE_XNG_LONG" : "MOM_VOTE_XNG_SHORT";
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_20259_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_20259\",\"ea\":\"xng-mom-vote\"}");
