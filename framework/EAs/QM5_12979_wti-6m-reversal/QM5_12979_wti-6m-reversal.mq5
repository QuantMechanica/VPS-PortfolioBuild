...
#property description "QM5_12979 WTI 6M Overextension Fade"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_12979 - WTI 6M Overextension Fade
// -----------------------------------------------------------------------------
// D1 structural WTI sleeve:
//   - evaluates one monthly overextension setup on XTIUSD.DWX
//   - fades fixed 120-D1-bar WTI return extremes
//   - requires price to be stretched from SMA(20) by an ATR multiple
//   - exits by 6M-return zero-cross, max-hold guard, or ATR stop
// Runtime uses MT5 OHLC only; no futures chain, EIA feed, CFTC feed, API, CSV,
// or ML.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 12979;
...
input double strategy_fade_threshold_pct   = 15.0;
input double strategy_stretch_atr_mult     = 1.25;
...
bool Strategy_LoadReversalState(double &return_pct,
                                double &atr_last,
                                double &sma_last,
                                double &stretch_atr,
                                ...
                                const bool require_monthly_gate)
  {
   ...
   return_pct = 100.0 * ((close_last / close_lookback) - 1.0);
   stretch_atr = (close_last - sma_last) / atr_last;
   if(!MathIsValidNumber(return_pct) || !MathIsValidNumber(stretch_atr))
      return false;
...
void Strategy_CloseOpenPositionsIfNeeded()
  {
   ...
   const bool state_ready = Strategy_LoadReversalState(return_pct,
                                                       atr_last,
                                                       sma_last,
                                                       stretch_atr,
                                                       signal_month_key,
      ...
      if(state_ready)
        {
         ...
         if(position_type == POSITION_TYPE_BUY && return_pct >= 0.0)
            should_close = true;
         if(position_type == POSITION_TYPE_SELL && return_pct <= 0.0)
            should_close = true;
        }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12979_WTI_6M_REVERSAL";
   ...
   double stretch_atr = 0.0;
   int signal_month_key = -1;
   if(!Strategy_LoadReversalState(return_pct,
                                  atr_last,
                                  sma_last,
                                  stretch_atr,
                                  signal_month_key,
   ...
   if(return_pct <= -min_return && stretch_atr <= -min_stretch)
      reversal_direction = 1;
   else if(return_pct >= min_return && stretch_atr >= min_stretch)
      reversal_direction = -1;
   else
      return false;
   ...
   req.reason = (reversal_direction > 0) ? "WTI_6M_REVERSAL_LONG" : "WTI_6M_REVERSAL_SHORT";
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_12979\",\"ea\":\"wti-6m-reversal\"}");
