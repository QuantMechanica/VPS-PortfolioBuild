"""Independent formula fixtures for QM5_41018.
...
def completed_label_maps_to_tuesday(completed_label: dt.datetime) -> bool:
    # Factory energy D1 labels use the prior calendar date.
    return (completed_label + dt.timedelta(days=1)).weekday() == 1


def normal_exit_due(value: dt.datetime, exit_hour: int = 21) -> bool:
    return value.weekday() == 2 and value.hour >= exit_hour
...
class WednesdayRelativeValueReferenceTests(unittest.TestCase):
    ...
    def test_prior_day_energy_label_grace_uses_modulo_one_day(self) -> None:
        labelled = dt.datetime(2026, 8, 11, 0, 0)  # Tuesday label
        self.assertTrue(
            within_entry_grace(dt.datetime(2026, 8, 12, 0, 5), labelled, 5)
        )
        self.assertFalse(
            within_entry_grace(dt.datetime(2026, 8, 12, 0, 5, 1), labelled, 5)
        )

    def test_native_same_day_label_grace_is_also_supported(self) -> None:
        labelled = dt.datetime(2026, 8, 12, 0, 0)
        self.assertTrue(
            within_entry_grace(dt.datetime(2026, 8, 12, 0, 4, 59), labelled, 5)
        )

    def test_completed_monday_label_maps_to_tuesday_session(self) -> None:
        self.assertTrue(
            completed_label_maps_to_tuesday(dt.datetime(2026, 8, 10, 0, 0))
        )
        self.assertFalse(
            completed_label_maps_to_tuesday(dt.datetime(2026, 8, 9, 0, 0))
        )

    def test_normal_exit_is_wednesday_21_only(self) -> None:
        self.assertFalse(normal_exit_due(dt.datetime(2026, 8, 12, 20, 59, 59)))
        self.assertTrue(normal_exit_due(dt.datetime(2026, 8, 12, 21, 0, 0)))
        self.assertFalse(normal_exit_due(dt.datetime(2026, 8, 13, 21, 0, 0)))
    ...
    def test_joint_solver_caps_one_package_risk_and_notional_error(self) -> None:
        ...
        self.assertLessEqual(error, 10.0)

    def test_package_directions_are_not_interchangeable(self) -> None:
        expected = {"XTIUSD.DWX": "BUY", "XNGUSD.DWX": "SELL"}
        self.assertEqual(expected["XTIUSD.DWX"], "BUY")
        self.assertEqual(expected["XNGUSD.DWX"], "SELL")
