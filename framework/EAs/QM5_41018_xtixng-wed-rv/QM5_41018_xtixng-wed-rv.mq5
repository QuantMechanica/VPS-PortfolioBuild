...
#property description "QM5_41018 XTI XNG Wednesday-session relative-value basket"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_41018 - XTI/XNG Wednesday-session relative-value basket
// -----------------------------------------------------------------------------
// At the first eligible broker-Wednesday D1 boundary, buy WTI and sell Natural
// Gas. Both legs target equal absolute USD notionals and share one framework
// risk budget. The complete package closes at broker Wednesday hour 21.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41018;
...
input double strategy_max_notional_error_pct  = 10.0;
input int    strategy_max_hold_days           = 3;
input int    strategy_xti_max_spread_pts      = 2500;
...
bool Strategy_CompletedPredecessorIsTuesday()
  {
   ...
   return (Strategy_DayOfWeek(completed_bar + (datetime)86400) == 2);
  }
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41018 && qm_magic_slot_offset == 0 &&
           ...
           MathAbs(strategy_max_notional_error_pct - 10.0) <= 1.0e-12 &&
           strategy_max_hold_days == 3 &&
           strategy_xti_max_spread_pts == 2500 &&
...
bool Strategy_SymbolReady(const string symbol)
  {
   ...
   if(trade_mode == SYMBOL_TRADE_MODE_DISABLED ||
      ...
      return false;
   if(symbol == g_leg_xti && trade_mode == SYMBOL_TRADE_MODE_SHORTONLY)
      return false;
   if(symbol == g_leg_xng && trade_mode == SYMBOL_TRADE_MODE_LONGONLY)
      return false;
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XTIXNG_WED_ATTEMPT_WEEK", qm_ea_id);
  }
...
bool Strategy_PreparePackage(double &xti_lots,
                             ...
                             double &xng_stop)
  {
   ...
   const double xti_entry = SymbolInfoDouble(g_leg_xti, SYMBOL_ASK);
   const double xng_entry = SymbolInfoDouble(g_leg_xng, SYMBOL_BID);
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41018_XTIXNG_WED_RV";
...
bool Strategy_OpenPair()
  {
   ...
   if(!Strategy_PreparePackage(xti_lots, xng_lots, xti_stop, xng_stop))
      return false;
   if(!Strategy_OpenLeg(g_leg_xti, QM_BUY, xti_lots, xti_stop))
      return false;
   if(Strategy_OpenLeg(g_leg_xng, QM_SELL, xng_lots, xng_stop) &&
...
bool Strategy_DecisionClockReady(int &week_key)
  {
   ...
   if(!g_is_new_bar || g_current_host_bar <= 0 ||
      ...
      !Strategy_CompletedPredecessorIsTuesday())
      return false;
...
bool Strategy_EntryWindowReady(const int week_key)
  {
   ...
   // use its broker date for the card's Wednesday decision and week key.
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41018_XTIXNG_WED_RV_HOST";
...
int OnInit()
  {
   ...
   g_pair_entry_time = Strategy_CurrentPairEntryTime();
   // Any Wednesday attachment consumes the week flat. The shared new-bar
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41018\",\"ea\":\"xtixng-wed-rv\"}");
