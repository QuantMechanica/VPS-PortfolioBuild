...
#property description "QM5_41043 XNG Standard-Thursday Strict Flow-Agreement Continuation"
...
//   - enter Friday only when both nonzero streams have the same strict sign,
//     following the reconciled completed Thursday move
...
bool Strategy_LoadThursdayFlowAgreement(const datetime current_bar_time,
                                         const datetime broker_now,
                                         ...
                                         int &direction)
  {
   ...
   // Follow the completed Thursday displacement; magnitude never changes risk.
   if(total_flow > 0.0)
      direction = 1;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   int direction = 0;
   if(!Strategy_LoadThursdayFlowAgreement(g_strategy_d1_bar_time,
                                           broker_now,
                                           overnight_flow,
                                           session_flow,
                                           day_return,
   ...
   if(direction > 0)
     {
      req.type = QM_BUY;
      req.reason = "XNG_THU_FLOW_AGREE_LONG";
     }
   else if(direction < 0)
     {
      req.type = QM_SELL;
      req.reason = "XNG_THU_FLOW_AGREE_SHORT";
     }
   else
      return false;
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XNG Thursday flow-agreement card holds through Friday to the next D1 boundary"))
      return INIT_FAILED;
   ...
   g_attempt_state_key =
      StringFormat("QM5_41043_THU_FLOW_AGREE_FRIDAY_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41043\",\"ea\":\"xng-thu-flow-agree\"}");
