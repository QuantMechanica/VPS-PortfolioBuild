...
#property description "QM5_20189 XAU XAG Calendar Momentum Agreement"
...
//   - reconstruct the exact immediately completed relative monthly return
//   - trade only when the seasonal and one-month relative signs agree
//   - long XAU/short XAG on positive agreement; reverse on negative agreement
...
double   g_cache_xau_same_month_return = 0.0;
double   g_cache_xag_same_month_return = 0.0;
double   g_cache_seasonal_score = 0.0;
double   g_cache_xau_momentum_return = 0.0;
double   g_cache_xag_momentum_return = 0.0;
double   g_cache_momentum_score = 0.0;
...
string Strategy_AttemptStateName()
  {
   return "QM5_20189_XAUXAG_CALMOM1_ATTEMPT";
  }
...
bool Strategy_LoadSignalState(const datetime decision_bar_time,
  ...
  {
   pair_direction = 0;
   g_cache_xau_same_month_return = 0.0;
   g_cache_xag_same_month_return = 0.0;
   g_cache_seasonal_score = 0.0;
   g_cache_xau_momentum_return = 0.0;
   g_cache_xag_momentum_return = 0.0;
   g_cache_momentum_score = 0.0;
   ...
   g_cache_xau_same_month_return =
      xau_sum / (double)g_cache_sample_count;
   g_cache_xag_same_month_return =
      xag_sum / (double)g_cache_sample_count;
   g_cache_seasonal_score =
      g_cache_xau_same_month_return -
      g_cache_xag_same_month_return;

   if(!MathIsValidNumber(g_cache_xau_same_month_return) ||
      !MathIsValidNumber(g_cache_xag_same_month_return) ||
      !MathIsValidNumber(g_cache_seasonal_score))
      return false;

   int momentum_year = 0;
   int momentum_month = 0;
   Strategy_PreviousMonth(decision_parts.year,
                          decision_parts.mon,
                          momentum_year,
                          momentum_month);
   datetime xau_momentum_end = 0;
   datetime xau_momentum_start = 0;
   datetime xag_momentum_end = 0;
   datetime xag_momentum_start = 0;
   if(!Strategy_MonthReturn(xau_rates,
                            xau_count,
                            momentum_year,
                            momentum_month,
                            g_cache_xau_momentum_return,
                            xau_momentum_end,
                            xau_momentum_start) ||
      !Strategy_MonthReturn(xag_rates,
                            xag_count,
                            momentum_year,
                            momentum_month,
                            g_cache_xag_momentum_return,
                            xag_momentum_end,
                            xag_momentum_start))
      return false;
   if(xau_momentum_end != xag_momentum_end ||
      xau_momentum_start != xag_momentum_start)
      return false;

   g_cache_momentum_score =
      g_cache_xau_momentum_return -
      g_cache_xag_momentum_return;
   if(!MathIsValidNumber(g_cache_xau_momentum_return) ||
      !MathIsValidNumber(g_cache_xag_momentum_return) ||
      !MathIsValidNumber(g_cache_momentum_score))
      return false;

   if(g_cache_seasonal_score > strategy_signal_epsilon &&
      g_cache_momentum_score > strategy_signal_epsilon)
      pair_direction = 1;
   else if(g_cache_seasonal_score < -strategy_signal_epsilon &&
           g_cache_momentum_score < -strategy_signal_epsilon)
      pair_direction = -1;
   return true;
  }
...
bool Strategy_OpenPair(const int pair_direction)
  {
   ...
   const string reason = long_xau_short_xag ? "QM5_20189_LONG_XAU_SHORT_XAG_CALMOM1"
                                            : "QM5_20189_SHORT_XAU_LONG_XAG_CALMOM1";
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart())
      return true;
   if(qm_ea_id != 20189 || qm_magic_slot_offset != 0 || qm_rng_seed != 42)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20189_XAU_XAG_CALMOM1_HOST";
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_20189\",\"ea\":\"xauxag-calmom1\"}");
