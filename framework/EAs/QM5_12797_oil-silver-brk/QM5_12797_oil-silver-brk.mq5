...
#property description "QM5_12797 Oil Silver Ratio Breakout"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_12797 - Oil/Silver Ratio Breakout
// -----------------------------------------------------------------------------
// D1 two-leg commodity basket:
//   spread = ln(XTIUSD.DWX) - beta * ln(XAGUSD.DWX)
//   z > entry: long ratio = buy WTI, sell silver
//   z < -entry: short ratio = sell WTI, buy silver
// The EA runs from the XTIUSD.DWX host chart and trades both registered legs
// through QM_BasketOrder. Runtime uses MT5 OHLC only; no external source data.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 12797;
...
input int    strategy_z_lookback_d1       = 180;
...
datetime Strategy_PairOpenTime()
  {
   datetime opened = 0;
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      if(pos_time <= 0)
         continue;
      if(opened == 0 || pos_time < opened)
         opened = pos_time;
     }
   return opened;
  }

bool Strategy_MaxHoldExpired()
  {
   if(strategy_max_hold_days <= 0)
      return false;

   datetime opened = Strategy_PairOpenTime();
   if(opened <= 0)
      opened = g_pair_entry_time;
   if(opened <= 0)
      return false;

   const long max_seconds = (long)strategy_max_hold_days * 86400;
   return ((long)(TimeCurrent() - opened) >= max_seconds);
  }
...
bool Strategy_RefreshSpreadState()
  {
   ...
   for(int i = 0; i < lookback; ++i)
     {
      if(xti[i] <= 0.0 || xag[i] <= 0.0)
         return false;
      spreads[i] = MathLog(xti[i]) - strategy_beta * MathLog(xag[i]);
...
bool Strategy_OpenPair(const int ratio_direction)
  {
   if(ratio_direction == 0 || Strategy_OpenPairLegCount() > 0)
      return false;
   if(!Strategy_SpreadAllowed(g_leg_xti) || !Strategy_SpreadAllowed(g_leg_xag))
      return false;

   const double xti_weight = 1.0;
   const double xag_weight = MathMax(0.1, MathAbs(strategy_beta));
   const double weight_sum = xti_weight + xag_weight;
   ...
   const string reason = long_ratio ? "QM5_12797_LONG_RATIO_BREAKOUT"
                                    : "QM5_12797_SHORT_RATIO_BREAKOUT";
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart())
      return true;
   if(strategy_z_lookback_d1 < 30 || strategy_beta <= 0.0 || strategy_entry_z <= 0.0)
      return true;
   if(strategy_exit_z < 0.0 || strategy_exit_z >= strategy_entry_z)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12797_RATIO_HOST";
...
bool Strategy_ExitSignal()
  {
   ...
   const int ratio_direction = Strategy_CurrentPairDirection();
   bool exit_now = Strategy_MaxHoldExpired();
   if(g_state_ready && ratio_direction > 0 && g_spread_z < strategy_exit_z)
      exit_now = true;
   if(g_state_ready && ratio_direction < 0 && g_spread_z > -strategy_exit_z)
      exit_now = true;
   if(ratio_direction == 0)
      exit_now = true;
...
int OnInit()
  {
   ...
   string basket_symbols[2] = {g_leg_xti, g_leg_xag};
   QM_SymbolGuardInit(basket_symbols);
   QM_BasketWarmupHistory(basket_symbols, PERIOD_D1, MathMax(220, strategy_z_lookback_d1 + strategy_atr_period_d1 + 10));

   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_12797\",\"ea\":\"oil-silver-brk\"}");
