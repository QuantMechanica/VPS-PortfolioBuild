...
#property description "QM5_12730 WTI March Calendar Premium"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_12730 - WTI March Calendar Premium
// -----------------------------------------------------------------------------
// D1 structural month-of-year sleeve:
//   - long XTIUSD.DWX only during broker-calendar March D1 bars
...
int Strategy_OpenedDayKey(const datetime t)
  {
   MqlDateTime dt;
   TimeToStruct(t, dt);
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      const int opened_day_key = Strategy_OpenedDayKey(opened);
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12730_WTI_MAR_PREM";
   ...
   req.reason = "WTI_MARCH_PREMIUM_LONG";
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_12730\",\"ea\":\"wti-mar-prem\"}");
...
// 2026-07-02 canonical OnTick order (audit finding — binding):
// kill-switch → Friday-close → NoTradeFilter → ManageOpenPosition → ExitSignal
// → news gate → IsNewBar → EntrySignal
void OnTick()
  {
   ...
   // Position management and time-exits run through news windows.
   ...
   // News gate sits here: only blocks new entries, not management above.
