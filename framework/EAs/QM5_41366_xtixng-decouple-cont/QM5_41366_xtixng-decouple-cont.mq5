...
#property description "QM5_41366 XTI/XNG Weekly Decoupling-Shock Continuation"
...
// -----------------------------------------------------------------------------
// Reconstruct the synchronized week-end XTI/XNG close pairs for the two
// immediately preceding completed broker weeks. When the individual WTI and
// natural gas log returns over that exact interval have strict opposite signs,
// buy the winner and sell the loser in an equal-notional opposite-leg package.
// Consume one attempt per week and flatten at the next week boundary.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41366;
...
double   g_wti_week_return = 0.0;
double   g_xng_week_return = 0.0;
double   g_xti_older_close = 0.0;
double   g_xti_newer_close = 0.0;
double   g_xng_older_close = 0.0;
double   g_xng_newer_close = 0.0;
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41366 && qm_magic_slot_offset == 0 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XTIXNG_DECOUPLE_ATTEMPT_WEEK", qm_ea_id);
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41366_XTIXNG_DECOUPLE_CONT";
...
bool Strategy_LoadWeeklyDecoupling(const int current_week,
                                    int &direction)
  {
   direction = 0;
   g_wti_week_return = 0.0;
   g_xng_week_return = 0.0;
   g_xti_older_close = 0.0;
   g_xti_newer_close = 0.0;
   g_xng_older_close = 0.0;
   g_xng_newer_close = 0.0;
      ...
      if(week_key == newer_week)
        {
         if(g_newer_week_sessions == 0)
           {
            g_xti_newer_close = xti_bars[index].close;
            g_xng_newer_close = xng_bars[index].close;
           }
      ...
      else if(week_key == older_week)
        {
         if(g_older_week_sessions == 0)
           {
            g_xti_older_close = xti_bars[index].close;
            g_xng_older_close = xng_bars[index].close;
           }
   ...
   if(newer_week <= 0 || older_week <= 0 ||
      ...
      g_xti_older_close <= 0.0 || g_xti_newer_close <= 0.0 ||
      g_xng_older_close <= 0.0 || g_xng_newer_close <= 0.0)
      return false;

   g_wti_week_return = MathLog(g_xti_newer_close / g_xti_older_close);
   g_xng_week_return = MathLog(g_xng_newer_close / g_xng_older_close);
   if(!MathIsValidNumber(g_wti_week_return) ||
      !MathIsValidNumber(g_xng_week_return))
      return false;

   if(g_wti_week_return > strategy_signal_epsilon &&
      g_xng_week_return < -strategy_signal_epsilon)
      direction = 1; // WTI won and XNG lost: buy XTI, sell XNG.
   else if(g_wti_week_return < -strategy_signal_epsilon &&
           g_xng_week_return > strategy_signal_epsilon)
      direction = -1; // XNG won and WTI lost: sell XTI, buy XNG.
   return true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41366_XTIXNG_DECOUPLE_CONT_HOST";
   ...
   if(Strategy_OpenOwnedPositionCount() > 0 ||
     ...
     {
      signal_valid =
         Strategy_LoadWeeklyDecoupling(g_signal_week_key, direction);
      if(!signal_valid)
         g_signal_state = "endpoint_validation_failed";
      else if(direction < 0)
         g_signal_state = "wti_positive_xng_negative_buy_winner_sell_loser";
      else if(direction > 0)
         g_signal_state = "wti_negative_xng_positive_sell_loser_buy_winner";
      else if(g_wti_week_return == 0.0 ||
              g_xng_week_return == 0.0)
         g_signal_state = "exact_zero_flat";
      else if((g_wti_week_return > 0.0) ==
              (g_xng_week_return > 0.0))
         g_signal_state = "same_sign_flat";
      else
         g_signal_state = "opposite_sign_inside_deadband_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"week\":%d,\"decision_bar\":%I64d,\"late\":%s,\"valid\":%s,\"direction\":%d,\"wti_return\":%.12e,\"xng_return\":%.12e,\"xti_older\":%.10f,\"xti_newer\":%.10f,\"xng_older\":%.10f,\"xng_newer\":%.10f,\"older_sessions\":%d,\"newer_sessions\":%d,\"state\":\"%s\"}",
                            g_signal_week_key,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XTI/XNG weekly decoupling package exits at the next broker week"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41366\",\"ea\":\"xtixng-decouple-cont\"}");
