"""Reference checks for QM5_41366 XTI/XNG weekly decoupling continuation."""
...
MQ5 = EA_DIR / "QM5_41366_xtixng-decouple-cont.mq5"
SETFILE = (
    EA_DIR
    / "sets"
    / "QM5_41366_xtixng-decouple-cont_QM5_41366_XTI_XNG_DECOUPLE_CONT_D1_D1_backtest.set"
)
...
def weekly_decoupling_signal(
    ...
    direction = 0
    if wti_return > epsilon and xng_return < -epsilon:
        direction = 1  # WTI won and XNG lost: buy XTI, sell XNG.
    elif wti_return < -epsilon and xng_return > epsilon:
...
class WeeklyDecouplingReferenceTest(unittest.TestCase):
    def test_wti_up_xng_down_buys_winner_and_sells_loser(self) -> None:
        valid, direction, wti, xng, counts = weekly_decoupling_signal(
            20260824, completed_bars((100.0, 50.0), (110.0, 45.0))
        )
        self.assertTrue(valid)
        self.assertEqual(direction, 1)
        self.assertGreater(wti, 0.0)
        self.assertLess(xng, 0.0)
        self.assertEqual(counts, (5, 5))

    def test_wti_down_xng_up_sells_loser_and_buys_winner(self) -> None:
        valid, direction, wti, xng, _ = weekly_decoupling_signal(
            20260824, completed_bars((100.0, 50.0), (90.0, 55.0))
        )
        self.assertTrue(valid)
        self.assertEqual(direction, -1)
        self.assertLess(wti, 0.0)
        self.assertGreater(xng, 0.0)

    def test_same_sign_zero_and_inside_deadband_are_flat(self) -> None:
        for newer in (
            (100.0, 55.0),
            (110.0, 50.0),
            (110.0, 55.0),
            (90.0, 45.0),
            (100.0 * math.exp(0.5e-10), 45.0),
            (90.0, 50.0 * math.exp(-0.5e-10)),
        ):
            with self.subTest(newer=newer):
                valid, direction, *_ = weekly_decoupling_signal(
    ...
    def test_asynchrony_and_nonconsecutive_weeks_fail_closed(self) -> None:
        ...
        self.assertFalse(weekly_decoupling_signal(20260824, bars)[0])

        missing_week = make_week(
            datetime(2026, 8, 17, tzinfo=UTC), 110.0, 45.0
        ) + make_week(datetime(2026, 8, 3, tzinfo=UTC), 100.0, 50.0)
        self.assertFalse(weekly_decoupling_signal(20260824, missing_week)[0])
    ...
    def test_uniform_timestamp_shift_preserves_label(self) -> None:
        ...
        native = weekly_decoupling_signal(20260824, bars)
        shifted_result = weekly_decoupling_signal(20260824, shifted)
    ...
    def test_static_artifacts_lock_identity_and_safety(self) -> None:
        ...
        self.assertIn("qm_ea_id                    = 41366", source)
        self.assertIn("Strategy_LoadWeeklyDecoupling", source)
        self.assertIn("g_wti_week_return > strategy_signal_epsilon", source)
        self.assertIn("g_xng_week_return < -strategy_signal_epsilon", source)
        self.assertIn("g_wti_week_return < -strategy_signal_epsilon", source)
        self.assertIn("g_xng_week_return > strategy_signal_epsilon", source)
        ...
        self.assertRegex(resolver, re.compile(r"413660000.*413660001", re.DOTALL))
