...
#property description "QM5_12848 WTI Brent Spread Breakout"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_12848 - WTI/Brent Spread Breakout
// -----------------------------------------------------------------------------
// D1 two-leg crude benchmark basket:
//   spread = log(Brent) - beta * log(WTI)
//   spread > prior channel high: buy Brent, sell WTI
//   spread < prior channel low:  sell Brent, buy WTI
...
bool Strategy_RefreshSpreadState()
  {
   ...
   const int max_lookback = MathMax(entry_lookback, exit_lookback);
   const int history_needed = max_lookback + 1;
   ...
   double spreads[];
   ArrayResize(spreads, history_needed);
   for(int i = 0; i < history_needed; ++i)
     {
      if(xti[i] <= 0.0 || xbr[i] <= 0.0)
         return false;
      spreads[i] = MathLog(xbr[i]) - strategy_beta * MathLog(xti[i]);
      if(!MathIsValidNumber(spreads[i]))
         return false;
     }

   g_spread_current = spreads[0];
...
bool Strategy_OpenPair(const int spread_direction)
  {
   ...
   const string reason = long_spread ? "QM5_12848_LONG_BRENT_WTI_BREAKOUT"
                                     : "QM5_12848_SHORT_BRENT_WTI_BREAKOUT";
...
int Strategy_CurrentSpreadDirection()
  {
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      const ulong ticket = PositionGetTicket(i);
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12848_BREAKOUT_HOST";
   ...
   if(g_spread_current > g_entry_high)
      Strategy_OpenPair(1);
   else if(g_spread_current < g_entry_low)
      Strategy_OpenPair(-1);
...
bool Strategy_ExitSignal()
  {
   ...
   const int spread_direction = Strategy_CurrentSpreadDirection();
   if(g_state_ready && spread_direction > 0 && g_spread_current < g_exit_low)
      Strategy_ClosePair(QM_EXIT_STRATEGY);
   else if(g_state_ready && spread_direction < 0 && g_spread_current > g_exit_high)
      Strategy_ClosePair(QM_EXIT_STRATEGY);
...
int OnInit()
  {
   ...
   QM_BasketWarmupHistory(basket_symbols,
                          PERIOD_D1,
                          MathMax(260, strategy_entry_lookback_d1 + strategy_atr_period_d1 + 20));

   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_12848\",\"ea\":\"wti-brent-brk\"}");
