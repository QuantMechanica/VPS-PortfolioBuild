...
#property description "QM5_12111 (v2) bressert-double-stochastic-h1"
...
{
   // Stage 1: raw Stoch %K via QM_Stoch_K (framework helper; no CopyBuffer)
   const int raw_count = dss_period * 2 + dss_slowing + 2;
   double raw_k_buf[];
   ArraySetAsSeries(raw_k_buf, true);
   ArrayResize(raw_k_buf, raw_count);
   for(int idx = 0; idx < raw_count; idx++)
     {
      raw_k_buf[idx] = QM_Stoch_K(_Symbol, PERIOD_H1, raw_stoch_k, raw_stoch_d, raw_stoch_slowing, shift + 1 + idx);
     }
   const int ck = raw_count;
...
{
   return QM_EMA(_Symbol, PERIOD_H1, period, shift + 1); // uses QM_* framework helper; no raw handle/CopyBuffer
}
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   const double close = iClose(_Symbol, PERIOD_H1, 1); // perf-allowed: EMA trend filter bar read
