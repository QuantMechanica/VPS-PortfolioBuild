...
#property description "QM5_41150 GBPUSD Europe/London local-session inventory drift"
...
// Approved card: QM5_41150_gbpusd-local-session-inventory-drift
// Source: Breedon & Ranaldo (2013), Intraday Patterns in FX Returns and Order Flow
// =============================================================================

#define STRATEGY_EA_ID             41150
#define STRATEGY_SESSION_OPEN_HOUR 7
...
datetime Strategy_UTCToLondonLocal(const datetime utc_time)
  {
   if(utc_time <= 0)
      return 0;
   return QM_LondonCalendarUTCToLondonLocal(utc_time);
  }

bool Strategy_LondonLocalToUTC(const datetime local_time, datetime &utc_time)
  {
   utc_time = 0;
   if(local_time <= 0)
      return false;
   ...
   if(!TimeToStruct(local_time, parts))
      return false;
   const int date_key = parts.year * 10000 + parts.mon * 100 + parts.day;
   if(!QM_LondonCalendarLondonLocalToUTC(date_key, parts.hour, parts.min, utc_time))
      return false;
   return (utc_time > 0 && Strategy_UTCToLondonLocal(utc_time) == local_time);
  }
...
bool Strategy_CurrentLondonTime(datetime &local_time)
  {
   local_time = Strategy_UTCToLondonLocal(QM_BrokerToUTC(TimeCurrent()));
   return (local_time > 0);
  }
   ...
   for(int index = 0; index < total; ++index)
     {
      ...
      const datetime deal_local =
         Strategy_UTCToLondonLocal(QM_BrokerToUTC(deal_broker));
...
bool Strategy_SessionNewsClear(const datetime local_bar_open)
  {
   ...
   // Query each trailing one-hour slice at 08:00..16:00 local. The union is
   // exactly the owned [07:00,16:00] interval. Fresh fails closed when the
   ...
   for(int index = 0; index < STRATEGY_NEWS_QUERY_COUNT; ++index)
     {
      ...
      if(!Strategy_LondonLocalToUTC(local_anchor, utc_anchor))
         return false;
...
bool Strategy_BuildEntry(const datetime local_bar_open, QM_EntryRequest &req)
  {
   ...
   req.sl = stop;
   req.tp = 0.0;
   req.reason = "41150_london_inventory_sell";
   return true;
  }
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved London intraday card exits at 16:00 local and retains the framework Friday fail-safe"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_%d_LONDON_ATTEMPT_%d", qm_ea_id, QM_FrameworkMagic());
   datetime local_now = 0;
   if(!Strategy_CurrentLondonTime(local_now) ||
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41150\",\"session\":\"Europe/London 07:00-16:00\"}");
...
void OnTick()
  {
   ...
   const datetime local_bar_open =
      Strategy_UTCToLondonLocal(QM_BrokerToUTC(bar_broker));
