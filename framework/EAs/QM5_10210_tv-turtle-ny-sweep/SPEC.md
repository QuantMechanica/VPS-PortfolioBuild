...
`artifacts/cards_approved/QM5_10210_tv-turtle-ny-sweep.md`. See that card's body for
...
`QM5_10210_tv-turtle-ny-sweep.mq5`. Framework wiring (risk, magic, news, Friday close)
...
|---|---|---|---|
| `strategy_timeframe_minutes` | 5 | (see source) | (see strategy logic) |
| `strategy_preopen_start_hhmm_ny` | 0 | (see source) | (see strategy logic) |
| `strategy_ny_open_hhmm` | 930 | (see source) | (see strategy logic) |
| `strategy_ny_flat_hhmm` | 1600 | (see source) | (see strategy logic) |
| `strategy_atr_period` | 14 | (see source) | (see strategy logic) |
| `strategy_stop_atr_buffer` | 0.25 | (see source) | (see strategy logic) |
| `strategy_expansion_atr_mult` | 1.20 | (see source) | (see strategy logic) |
| `strategy_expansion_body_ratio` | 0.60 | (see source) | (see strategy logic) |
| `strategy_retrace_body_fraction` | 0.50 | (see source) | (see strategy logic) |
| `strategy_max_scan_bars` | 240 | (see source) | (see strategy logic) |
| `strategy_pending_expiry_minutes` | 120 | (see source) | (see strategy logic) |
| `strategy_max_spread_atr_fraction` | 0.20 | (see source) | (see strategy logic) |
...
`artifacts/cards_approved/QM5_10210_tv-turtle-ny-sweep.md`
