# QM5_20252_usdchf-euraud — Strategy Spec
...
`ln(USDCHF.DWX) - (-0.013891609131) * ln(EURAUD.DWX)`. Score the newest spread
...
| `strategy_beta` | -0.013891609131 | fixed | DEV-fitted structural hedge coefficient |
...
- `USDCHF.DWX` — fixed tester host and first traded leg.
- `EURAUD.DWX` — fixed companion and second traded leg.
- `AUDUSD.DWX` — conversion-history dependency observed by the USD-account
  tester for EURAUD P/L; it is order-free and has no magic slot.
...
| Bar gating | `QM_IsNewBar(USDCHF.DWX, PERIOD_D1)` consumed once per signal bar |
...
| OOS basket state changes | 15 across 2023–2024 in the frozen scan |
| Typical hold time | multi-day; scan half-life 97.342 D1 bars |
| Expected drawdown profile | high and regime-sensitive; conservative 30% prior |
| Regime preference | USDCHF/EURAUD fixed-residual mean reversion |
| Win rate target | unknown; Q02 is the first platform-economic judge |

The fixed scan recorded DEV net Sharpe `0.035741`, OOS net Sharpe `-0.511802`,
and OOS return `-5.026004%`. The negative OOS score and slow half-life are
...
`1.0` and `0.013891609131`; each leg is independently bounded by its ATR stop.
If the EURAUD allocation normalizes below broker minimum volume, the whole
package is rejected before any order is sent. ENV-to-mode validation remains
enforced by `QM_FrameworkInit`.
...
|---|---|---|---|
| v1 | 2026-08-06 | Initial build from approved card | Next-rank-63 non-duplicate fixed-scan FX basket |
