---
strategy_id: AI-CODEX-FX-COINT66-20260609-USDCHF-EURAUD
ea_id: 20252
slug: usdchf-euraud
type: strategy
status: APPROVED
...
markets: [forex]
timeframes: [D1]
primary_target_symbols: [USDCHF.DWX, EURAUD.DWX]
target_symbols: [USDCHF.DWX, EURAUD.DWX]
logical_symbol: QM5_20252_USDCHF_EURAUD_COINTEGRATION_D1
period: D1
...
# USDCHF/EURAUD D1 Cointegration Basket
...
low-frequency daily implementation. Chan makes no claim for USDCHF/EURAUD.
...
Rank 62 USDJPY/AUDUSD is already implemented as explicit slots in QM5_1156
and QM5_1257. USDCHF/EURAUD ranks sixty-third of 66 by OOS net Sharpe and is
the next relationship-level build gap. The deterministic dedup check found no
exact collision across 4,309 registry rows and 426 direct cards; no tracked
pair preset or dedicated two-leg basket manifest trades this exact pair.
...
`QM5_1156` and `QM5_1257` implement rank 62 USDJPY/AUDUSD, not rank 63.
Broad momentum, carry, trend, and strength baskets that contain USDCHF and
EURAUD as unrelated universe members do not trade this frozen-beta D1
...
`ln(USDCHF) - (-0.013891609131) * ln(EURAUD)`. The negative fitted coefficient
...
package retains USD, CHF, EUR, AUD, carry, and policy-divergence exposure.

## Hypothesis

The frozen scan found slightly positive DEV but negative OOS performance. This
card tests whether large fixed-residual deviations survive native tick costs
and swap in the canonical D1 implementation. Economic, cadence, or history
...
logical_symbol: QM5_20252_USDCHF_EURAUD_COINTEGRATION_D1
tester_currency: USD
conversion_only_symbols: [AUDUSD.DWX]
```

`USDCHF.DWX` is the tester host and first traded leg. `EURAUD.DWX` is the
companion traded leg. `AUDUSD.DWX` is warmed only for USD-account conversion;
...
- Compute `spread = ln(USDCHF) - strategy_beta * ln(EURAUD)`, with frozen
  `strategy_beta = -0.013891609131`.
...
Chan supplies the pair-trading method but reports no USDCHF/EURAUD result.
...
data_requirements: "USDCHF.DWX, EURAUD.DWX, and AUDUSD.DWX D1 histories"
```
...
|---|---|---|---|
| G0 Research Intake | 2026-08-06 | APPROVED | `decisions/2026-08-06_usdchf_euraud_cointegration_g0.md` |
| Q01 Build Validation | 2026-08-06 | PASS | `D:\QM\reports\framework\21\build_check_20260806_190426.json`; `D:\QM\reports\compile\20260806_190427\summary.csv` |
| Q02 Baseline Screening | - | NOT_ENQUEUED | - |

## 16. Lessons Captured

- 2026-08-06: Rank 62 is already represented by two explicit umbrella pair
  slots; rank 63 is the first relationship-level gap after a semantic audit.
- 2026-08-06: Rank 63 is OOS-negative and slower than the expected Q02 cadence
