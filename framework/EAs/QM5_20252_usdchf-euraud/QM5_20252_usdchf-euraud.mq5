...
#property description "QM5_20252 USDCHF EURAUD Cointegration"
...
//
// Card:
//   strategy-seeds/cards/approved/QM5_20252_usdchf-euraud_card.md
//
// Fixed spread:
//   ln(USDCHF.DWX) - beta * ln(EURAUD.DWX)
//
...
input double strategy_beta              = -0.013891609131;
...
bool Strategy_EnsureBasketScope()
  {
   ...
   string allowed[3] = {"USDCHF.DWX", "EURAUD.DWX", "AUDUSD.DWX"};
   for(int i = 0; i < 3; ++i)
...
bool Strategy_RefreshSpreadState()
  {
   ...
   for(int i = 0; i < history_count; ++i)
     {
      if(usdchf_time[i] <= 0 || usdchf_time[i] != euraud_time[i])
         return false;
      if(usdchf[i] <= 0.0 || euraud[i] <= 0.0)
         return false;

      spreads[i] =
         MathLog(usdchf[i]) - strategy_beta * MathLog(euraud[i]);
...
bool Strategy_PreparePair(const int spread_direction,
                          QM_EntryRequest &host_req)
  {
   ...
   const double weight_sum = usdchf_weight + euraud_weight;
   if(weight_sum <= 0.0)
      return false;
   ...
   const double euraud_lots =
      Strategy_LotsForLeg(g_leg_euraud, euraud_weight, usdchf_weight);
   if(usdchf_lots <= 0.0 || euraud_lots <= 0.0)
      return false;
   ...
   const string reason =
      long_spread ? "QM5_20252_LONG_SPREAD_Z_LT_NEG_ENTRY"
                  : "QM5_20252_SHORT_SPREAD_Z_GT_POS_ENTRY";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20252_PAIR_HOST";
