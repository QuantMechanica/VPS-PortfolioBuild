...
#property description "QM5_41400 WTI Monthly Fixed-Recency Sign Trend"
...
//   - reduce each nonzero return to its strict sign
//   - weight signs by fixed chronological ages 1..12, oldest to newest
//   - continue only when the centered fixed-recency score reaches +/-18
...
input int    strategy_weight_start            = 1;
input int    strategy_weight_step             = 1;
input int    strategy_weight_total            = 78;
...
bool Strategy_RecencySignSupportValid()
  {
   if(strategy_return_count != 12 ||
      strategy_weight_start != 1 || strategy_weight_step != 1 ||
      strategy_weight_total != 78 || strategy_score_abs_min != 18)
      return false;
   ...
   for(int mask = 0; mask < 4096; ++mask)
     {
      ...
      for(int bit = 0; bit < 12; ++bit)
        {
         if((mask & (1 << bit)) != 0)
            positive_weight += strategy_weight_start +
                               bit * strategy_weight_step;
        }
      const int score = 2 * positive_weight - strategy_weight_total;
...
bool Strategy_RecencySignSignal(const double &closes[],
                                Strategy_SignalMetrics &metrics)
  {
   ...
   if(strategy_endpoint_count != 13 || strategy_return_count != 12 ||
      strategy_weight_start != 1 || strategy_weight_step != 1 ||
      strategy_weight_total != 78 || strategy_score_abs_min != 18 ||
   ...
   for(int index = 0; index < strategy_return_count; ++index)
     {
      ...
      if(MathAbs(value) <= strategy_zero_epsilon)
        {
      ...
      const int weight =
         strategy_weight_start + index * strategy_weight_step;
      if(weight != index + 1 || weight < 1 || weight > 12)
         return false;
      metrics.total_weight += weight;
      if(value > strategy_zero_epsilon)
        {
         metrics.positive_weight += weight;
         ++metrics.positive_return_count;
        }
      else if(value >= -strategy_zero_epsilon)
         return false;
      if(StringLen(metrics.weight_path) > 0)
         metrics.weight_path += ",";
      metrics.weight_path += IntegerToString(weight);
     }

   if(StringLen(metrics.return_path) <= 0 || metrics.zero_return)
      return true;
   if(metrics.total_weight != strategy_weight_total ||
      metrics.positive_weight < 0 ||
      metrics.positive_weight > strategy_weight_total ||
      metrics.positive_return_count < 0 ||
      metrics.positive_return_count > strategy_return_count ||
      StringLen(metrics.weight_path) <= 0)
      return false;

   metrics.sign_score =
      2 * metrics.positive_weight - metrics.total_weight;
   if(metrics.sign_score < -strategy_weight_total ||
      metrics.sign_score > strategy_weight_total ||
      ((metrics.sign_score + strategy_weight_total) % 2) != 0)
      return false;

   if(metrics.sign_score >= strategy_score_abs_min)
      metrics.direction = 1;
   else if(metrics.sign_score <= -strategy_score_abs_min)
      metrics.direction = -1;
   return true;
  }
                                        ...
                                        int &direction)
  {
   ...
   const bool valid =
      Strategy_LoadMonthlyEndpoints(current_month_key,
                                    ...
                                    closes,
                                    endpoint_times) &&
      Strategy_RecencySignSignal(closes, metrics);
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
   ...
   else
     {
      ...
      g_signal_valid =
         endpoints_valid &&
         Strategy_RecencySignSignal(closes, g_signal_metrics);
      if(!g_signal_valid)
         g_signal_state = "recency_sign_validation_failed";
      else if(g_signal_metrics.direction == 0)
        {
         if(g_signal_metrics.zero_return)
            g_signal_state = "recency_sign_zero_return_flat";
         else
            g_signal_state = "recency_sign_score_flat";
        }
      else if(g_signal_metrics.direction > 0)
         g_signal_state = "recency_sign_wti_long";
      else if(g_signal_metrics.direction < 0)
         g_signal_state = "recency_sign_wti_short";
      else
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"endpoint_count\":%d,\"return_count\":%d,\"total_weight\":%d,\"positive_weight\":%d,\"sign_score\":%d,\"positive_return_count\":%d,\"zero_return\":%s,\"returns\":\"%s\",\"weights\":\"%s\",\"oldest_close\":%.10f,\"newest_close\":%.10f,\"oldest_endpoint\":%I64d,\"newest_endpoint\":%I64d,\"state\":\"%s\"}",
                            ...
                            g_signal_metrics.total_weight,
                            g_signal_metrics.positive_weight,
                            g_signal_metrics.sign_score,
                            g_signal_metrics.positive_return_count,
                            g_signal_metrics.zero_return ? "true" : "false",
                            g_signal_metrics.return_path,
                            g_signal_metrics.weight_path,
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(strategy_endpoint_count != 13 ||
      ...
      strategy_weight_start != 1 ||
      strategy_weight_step != 1 ||
      strategy_weight_total != 78 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41400_WTI_MRECENCY_SIGN_TR";
   ...
   req.reason = (g_signal_metrics.direction > 0)
      ? "WTI_MRECENCY_SIGN_TR_LONG"
      : "WTI_MRECENCY_SIGN_TR_SHORT";
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI monthly fixed-recency sign trend holds through Fridays until the next broker month"))
     {
      QM_FrameworkShutdown();
      return INIT_FAILED;
     }
   g_support_contract_valid = Strategy_RecencySignSupportValid();
   ...
   g_attempt_state_key =
      StringFormat("QM5_41400_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41400\",\"ea\":\"wti-mrecency-sign-tr\"}");
