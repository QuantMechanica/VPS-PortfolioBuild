...
#property description "QM5_41110 XAU/XAG Monthly Outside-Range Residence Reversion"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_41110 - XAU/XAG Monthly Outside-Range Residence Reversion
// -----------------------------------------------------------------------------
// At the first synchronized D1 boundary of a broker month, reconstruct the
// parent completed month's daily-close XAU/XAG log-ratio range. Fade only when
// the newest completed month has at least five closes beyond exactly one
// parent boundary, no opposite breach, and its final close is still outside.
// Runtime uses synchronized MT5-native close/calendar/history only; no
// external data, fitted center, current-bar signal endpoint, or retry exists.
// =============================================================================
...
input int    strategy_min_outside_sessions    = 5;
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41110 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_MOUTSIDE_RES_RV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41110_XAUXAG_MOUTSIDE_RES_RV";
...
bool Strategy_LoadMonthlyOutsideResidence(
      const int current_month_key,
      int &new_month_sessions,
      int &parent_month_sessions,
      int &above_count,
      int &below_count,
      double &newest_final,
      double &parent_range_low,
      double &parent_range_high,
      int &direction)
  {
   new_month_sessions = 0;
   parent_month_sessions = 0;
   above_count = 0;
   below_count = 0;
   newest_final = 0.0;
   parent_range_low = 0.0;
   parent_range_high = 0.0;
   direction = 0;
   ...
   if(current_month_key <= 0 ||
      ...
      strategy_max_month_sessions != 23 ||
      strategy_min_outside_sessions != 5 ||
      strategy_min_outside_sessions > strategy_min_month_sessions ||
      strategy_min_month_sessions > strategy_max_month_sessions ||
      strategy_history_bars_d1 < 2 * strategy_max_month_sessions)
      return false;
   ...
   const int xau_copied =
      CopyRates(g_leg_xau, // perf-allowed: bounded synchronized two-month scan behind one monthly attempt.
                PERIOD_D1, 1, strategy_history_bars_d1, xau_bars);
   const int xag_copied =
      CopyRates(g_leg_xag, // perf-allowed: bounded synchronized two-month scan behind one monthly attempt.
                PERIOD_D1, 1, strategy_history_bars_d1, xag_bars);
   const int available = MathMin(xau_copied, xag_copied);
   if(available < 2 * strategy_min_month_sessions)
      return false;

   if(xau_bars[0].time <= 0 || xau_bars[0].time != xag_bars[0].time)
      return false;
   const int newest_month_key = Strategy_MonthKey(xau_bars[0].time);
   if(newest_month_key <= 0 ||
      Strategy_MonthKey(xag_bars[0].time) != newest_month_key ||
      Strategy_NextMonthKey(newest_month_key) != current_month_key)
      return false;

   int index = 0;
   while(index < available &&
         Strategy_MonthKey(xau_bars[index].time) == newest_month_key)
     {
      if(new_month_sessions >= strategy_max_month_sessions)
         return false;
      const MqlRates xau_bar = xau_bars[index];
      const MqlRates xag_bar = xag_bars[index];
      if(xau_bar.time <= 0 || xau_bar.time != xag_bar.time ||
         Strategy_MonthKey(xag_bar.time) != newest_month_key ||
   ...
   if(new_month_sessions < strategy_min_month_sessions ||
      new_month_sessions > strategy_max_month_sessions ||
      index >= available)
      return false;
   ...
   if(parent_month_key <= 0 ||
      ...
      Strategy_NextMonthKey(parent_month_key) != newest_month_key)
      return false;
   ...
   while(index < available &&
         Strategy_MonthKey(xau_bars[index].time) == parent_month_key)
     {
      ...
      const MqlRates xag_bar = xag_bars[index];
      if(xau_bar.time <= 0 || xau_bar.time != xag_bar.time ||
         Strategy_MonthKey(xag_bar.time) != parent_month_key ||
         xau_bar.close <= 0.0 || xag_bar.close <= 0.0 ||
         !MathIsValidNumber(xau_bar.close) ||
         !MathIsValidNumber(xag_bar.close) ||
         xau_bars[index - 1].time <= xau_bar.time ||
      ...
      if(parent_month_sessions == 0)
        {
         parent_range_low = ratio;
         parent_range_high = ratio;
        }
      else
        {
         parent_range_low = MathMin(parent_range_low, ratio);
         parent_range_high = MathMax(parent_range_high, ratio);
        }
   ...
   if(parent_month_sessions < strategy_min_month_sessions ||
      ...
      index >= available ||
      !MathIsValidNumber(newest_final) ||
      !MathIsValidNumber(parent_range_low) ||
      !MathIsValidNumber(parent_range_high) ||
      parent_range_high <= parent_range_low)
      return false;

   // Seeing the synchronized boundary into the next older calendar month
   // proves that the parent package was not truncated by missing history.
   ...
   // direction > 0 means BUY XAU / SELL XAG. Fade persistent one-sided
   // residence outside the parent month's observed ratio range.
   if(above_count >= strategy_min_outside_sessions &&
      below_count == 0 && newest_final > parent_range_high)
      direction = -1;
   else if(below_count >= strategy_min_outside_sessions &&
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41110_XAUXAG_MOUTSIDE_RES_RV_HOST";
   ...
   int new_month_sessions = 0;
   int parent_month_sessions = 0;
   int above_count = 0;
   int below_count = 0;
   int direction = 0;
   double newest_final = 0.0;
   double parent_range_low = 0.0;
   double parent_range_high = 0.0;
   const bool valid =
      Strategy_LoadMonthlyOutsideResidence(g_signal_month_key,
                                           new_month_sessions,
                                           parent_month_sessions,
                                           above_count,
                                           below_count,
                                           ...
                                           parent_range_high,
                                           direction);
   string state = "monthly_outside_residence_failed";
   if(valid && direction > 0)
      state = "lower_residence_long_xau";
   else if(valid && direction < 0)
      state = "upper_residence_short_xau";
   else if(valid)
      state = "outside_residence_conditions_flat";

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"new_month_sessions\":%d,\"parent_month_sessions\":%d,\"above_count\":%d,\"below_count\":%d,\"newest_final\":%.12e,\"parent_range_low\":%.12e,\"parent_range_high\":%.12e,\"direction\":%d,\"state\":\"%s\"}",
                            g_signal_month_key,
                            new_month_sessions,
                            parent_month_sessions,
                            above_count,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG monthly outside-range residence-reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41110\",\"ea\":\"xauxag-moutside-res-rv\"}");
