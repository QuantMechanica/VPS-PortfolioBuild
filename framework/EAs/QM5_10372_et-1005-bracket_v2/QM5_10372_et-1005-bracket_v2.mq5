...
#property description "QM5_10372_v2 Elite Trader 10:05 High-Low Bracket"
...
bool Strategy_RangeQualityOK()
  {
   ...
   if(range_width < 4.0 * spread)
      return false;
   return (strategy_max_range_atr_mult <= 0.0 || range_width <= strategy_max_range_atr_mult * atr);
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(strategy_entry_offset_ticks < 0 ||
      ...
      return false;
   const datetime bar_time = iTime(_Symbol, _Period, 1); // perf-allowed: one read per QM_IsNewBar gate; bracket needs closed bar time
   if(bar_time <= 0)
      return false;
   ...
   if(!g_range_ready && Strategy_InWindow(bar_min, open_min, bracket_min))
     {
      const double high = iHigh(_Symbol, _Period, 1); // perf-allowed: one bar read per QM_IsNewBar gate
      const double low = iLow(_Symbol, _Period, 1); // perf-allowed
   ...
   const double close_last = iClose(_Symbol, _Period, 1); // perf-allowed: one bar read per QM_IsNewBar gate
   const double close_prev = iClose(_Symbol, _Period, 2); // perf-allowed
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_10372_et-1005-bracket_v2\"}");
