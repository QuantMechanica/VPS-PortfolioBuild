...
#property description "QM5_20253 WTI R3-q4 Memory-Enhanced Momentum"
...
// -----------------------------------------------------------------------------
// Source baseline: Mehlitz & Auer (2024), R3-q4 three-month memory-enhanced momentum.
// At each broker-month transition:
//   1. derive 32 completed monthly log returns from D1 month-end closes;
//   2. calculate the q=4 Lo-MacKinlay heteroskedasticity-robust VR test over lags 1-3;
//   3. continue the latest completed three-month return when persistence is significant;
...
input int    strategy_rank_months         = 3;
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 20253)
      return true;
   ...
   if(MathAbs(strategy_significance_z - 1.64485362695147) > 1.0e-12)
      return true;
   if(strategy_history_bars_d1 != 900 &&
      strategy_history_bars_d1 != 1200 &&
      strategy_history_bars_d1 != 1600)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20253_WTI_VR3_MOM";
   ...
   req.reason = (g_cache_signal > 0) ? "VR3_MEMORY_XTI_LONG" : "VR3_MEMORY_XTI_SHORT";
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_20253_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_20253\",\"ea\":\"wti-vr3-mom\"}");
