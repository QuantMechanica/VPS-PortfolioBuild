...
#property description "QM5_9248 MQL5 ATR Exhaustion Reversal"
...
input double strategy_atr_min_mult      = 2.0;
input double strategy_atr_max_mult      = 4.0;
input int    strategy_signal_bars       = 4;
input double strategy_stop_atr_mult     = 0.4;
input double strategy_take_rr           = 1.8;
input int    strategy_sma_exit_period   = 10;
input int    strategy_max_hold_bars     = 18;
input double strategy_adverse_r_exit    = 1.2;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(strategy_atr_period <= 0 || strategy_atr_median_bars < 3 || strategy_signal_bars != 4)
      return false;
   ...
   for(int shift = 1; shift <= strategy_atr_median_bars; ++shift)
     {
      const double atr_sample = QM_ATR(_Symbol, PERIOD_CURRENT, strategy_atr_period, shift);
      if(atr_sample <= 0.0)
         return false;
      atr_values[atr_count] = atr_sample;
      atr_count++;
     }
   if(atr_count <= 0)
      return false;
   ...
   if((atr_count % 2) == 1)
      median_atr = atr_values[atr_count / 2];
   else
      median_atr = (atr_values[(atr_count / 2) - 1] + atr_values[atr_count / 2]) * 0.5;
   if(median_atr <= 0.0)
      return false;

   const double atr_now = QM_ATR(_Symbol, PERIOD_CURRENT, strategy_atr_period, 1);
   if(atr_now < strategy_atr_min_mult * median_atr)
      return false;
   ...
   const double stop_buffer = strategy_stop_atr_mult * atr_now;
   if(stop_buffer <= 0.0 || strategy_take_rr <= 0.0)
      return false;
   ...
   if(long_signal)
     {
      ...
      req.sl = sl_price;
      req.tp = tp_price;
      req.reason = "ATR_EXHAUSTION_REVERSAL_LONG";
      return true;
     }
   ...
   req.sl = sl_price;
   req.tp = tp_price;
   req.reason = "ATR_EXHAUSTION_REVERSAL_SHORT";
   return true;
  }
...
void Strategy_ManageOpenPosition()
  {
   if(strategy_adverse_r_exit <= 0.0)
      return;
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      const double initial_r = MathAbs(open_price - sl_price);
      if(initial_r <= 0.0)
         continue;
      ...
      const double adverse_move = is_buy ? (open_price - market_price)
                                         : (market_price - open_price);
      if(adverse_move >= strategy_adverse_r_exit * initial_r)
...
bool Strategy_ExitSignal()
  {
   if(strategy_sma_exit_period <= 0 || strategy_max_hold_bars <= 0)
      return false;
