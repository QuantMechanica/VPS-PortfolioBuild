...
#property description "QM5_13144 XTI XNG 11-to-10-Month Microscopic Momentum"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_13144 - XTI/XNG 11-to-10-Month Microscopic Momentum
// -----------------------------------------------------------------------------
// Monthly opposite-side energy package:
//   - isolate each leg's return between the t-11 and t-10 month boundaries
...
input int    strategy_far_boundary_months     = 11;
input int    strategy_near_boundary_months    = 10;
...
double   g_cache_xti_micro_return = 0.0;
double   g_cache_xng_micro_return = 0.0;
...
bool Strategy_MicroscopicReturn(const string symbol,
                                ...
                                double &micro_return)
  {
   ...
   MqlDateTime far_dt;
   MqlDateTime near_dt;
   Strategy_ShiftMonths(decision_dt, strategy_far_boundary_months, far_dt);
   Strategy_ShiftMonths(decision_dt, strategy_near_boundary_months, near_dt);
   const datetime far_boundary = StructToTime(far_dt);
   const datetime near_boundary = StructToTime(near_dt);
   if(far_boundary <= 0 || near_boundary <= 0 || near_boundary <= far_boundary)
      return false;
   ...
   double far_close = 0.0;
   double near_close = 0.0;
   datetime far_close_time = 0;
   datetime near_close_time = 0;
   for(int i = 0; i < count; ++i)
     {
      ...
      if(close_price <= 0.0 || !MathIsValidNumber(close_price))
         continue;
      if(near_close_time == 0 && bar_time < near_boundary)
        {
         near_close_time = bar_time;
         near_close = close_price;
        }
      if(far_close_time == 0 && bar_time < far_boundary)
        {
         far_close_time = bar_time;
         far_close = close_price;
        }
      if(near_close_time > 0 && far_close_time > 0)
         break;
     }

   if(near_close_time <= 0 || far_close_time <= 0 ||
      near_close <= 0.0 || far_close <= 0.0 ||
      near_close_time <= far_close_time)
      return false;

   const long max_gap_seconds = (long)strategy_max_boundary_gap_days * 86400;
   const long near_gap = (long)(near_boundary - near_close_time);
   const long far_gap = (long)(far_boundary - far_close_time);
   if(near_gap <= 0 || far_gap <= 0 ||
      near_gap > max_gap_seconds || far_gap > max_gap_seconds)
      return false;

   micro_return = MathLog(near_close / far_close);
   return MathIsValidNumber(micro_return);
  }
...
bool Strategy_LoadSignalState(const datetime decision_bar_time,
  ...
  {
   pair_direction = 0;
   g_cache_xti_micro_return = 0.0;
   g_cache_xng_micro_return = 0.0;
   g_cache_return_difference = 0.0;

   if(!Strategy_MicroscopicReturn(g_leg_xti,
                                  decision_bar_time,
                                  g_cache_xti_micro_return))
      return false;
   if(!Strategy_MicroscopicReturn(g_leg_xng,
                                  decision_bar_time,
                                  g_cache_xng_micro_return))
      return false;

   g_cache_return_difference = g_cache_xti_micro_return - g_cache_xng_micro_return;
   if(!MathIsValidNumber(g_cache_return_difference))
      return false;
   if(g_cache_return_difference > 1.0e-12)
      pair_direction = 1;
   else if(g_cache_return_difference < -1.0e-12)
      pair_direction = -1;
   return true;
  }
...
bool Strategy_OpenPair(const int pair_direction)
  {
   ...
   const string reason = long_xti_short_xng ? "QM5_13144_LONG_XTI_SHORT_XNG_MICRO11"
                                            : "QM5_13144_SHORT_XTI_LONG_XNG_MICRO11";
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart())
      return true;
   if(strategy_far_boundary_months != 11 ||
      strategy_near_boundary_months != 10 ||
      strategy_far_boundary_months <= strategy_near_boundary_months)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_13144_XTI_XNG_MICRO11_HOST";
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_13144\",\"ea\":\"energy-micro11\"}");
