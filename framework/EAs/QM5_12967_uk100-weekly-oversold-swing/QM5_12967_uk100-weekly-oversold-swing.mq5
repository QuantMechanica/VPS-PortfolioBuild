...
#property description "QM5_12967 UK100 weekly oversold swing"

#include <QM/QM_Common.mqh>

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 12967;
...
input int    strategy_entry_lookback_low  = 10;
input int    strategy_sma_exit_period     = 10;
input int    strategy_time_stop_days      = 15;
input int    strategy_atr_period          = 14;
input double strategy_atr_sl_mult         = 2.5;

bool Strategy_IsTarget()
  {
   return (_Symbol == "UK100.DWX" && _Period == PERIOD_D1 && qm_magic_slot_offset == 0);
  }
...
int Strategy_D1BarsHeld(const datetime opened)
  {
   if(opened <= 0)
      return 0;
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsTarget())
      return true;
   if(strategy_sma_regime_period <= 1 || strategy_entry_lookback_low <= 1)
      return true;
   if(strategy_sma_exit_period <= 1 || strategy_time_stop_days <= 0)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "UK100_10D_LOW_SWING";
   ...
   if(close1 <= 0.0 || sma200 <= 0.0 || lowest <= 0.0 || point <= 0.0)
      return false;
   if(close1 <= sma200)
      return false;
   if(close1 > lowest + point * 0.5)
      return false;
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      if(Strategy_D1BarsHeld(opened) >= strategy_time_stop_days)
         return true;
     }
   return false;
  }
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_12967\",\"ea\":\"uk100-weekly-oversold-swing\"}");
