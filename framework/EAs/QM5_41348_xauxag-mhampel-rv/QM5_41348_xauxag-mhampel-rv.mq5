...
#property description "QM5_41348 XAU/XAG Monthly Hampel Reversion"
...
// returns and compute their locked 32-step redescending Hampel robust
// location. Fade its sign with an opposed, equal-target-notional package held
// to the next month. Runtime is deterministic, completed-price-only, and one
// consumed attempt per month.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41348;
...
input double strategy_hampel_a                = 2.0;
input double strategy_hampel_b                = 4.0;
input double strategy_hampel_c                = 8.0;
input int    strategy_hampel_steps            = 32;
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41348 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
            ...
            MathAbs(strategy_hampel_a - 2.0) <= 1.0e-12 &&
            MathAbs(strategy_hampel_b - 4.0) <= 1.0e-12 &&
            MathAbs(strategy_hampel_c - 8.0) <= 1.0e-12 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_HAMPEL_RV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41348_XAUXAG_HAMPEL_RV";
...
bool Strategy_Sort12(double &values[])
  {
...
bool Strategy_HampelLocation(const double &returns[],
                               double &median,
                               double &mad,
                               double &scale,
                               double &location)
  {
   ...
   scale = 0.0;
   location = 0.0;
   if(ArraySize(returns) != 12 || strategy_return_count != 12 ||
      strategy_hampel_steps != 32 ||
      MathAbs(strategy_mad_normalizer - 1.4826) > 1.0e-12 ||
      MathAbs(strategy_hampel_a - 2.0) > 1.0e-12 ||
      MathAbs(strategy_hampel_b - 4.0) > 1.0e-12 ||
   ...
   for(int index = 0; index < strategy_return_count; ++index)
     {
      if(!MathIsValidNumber(returns[index]))
         return false;
      sorted[index] = returns[index];
     }
   if(!Strategy_Sort12(sorted))
      return false;
   median = 0.5 * (sorted[5] + sorted[6]);
   if(!MathIsValidNumber(median))
      return false;

   for(int index = 0; index < strategy_return_count; ++index)
     {
      deviations[index] = MathAbs(returns[index] - median);
      if(!MathIsValidNumber(deviations[index]))
         return false;
     }
   if(!Strategy_Sort12(deviations))
      return false;
   mad = 0.5 * (deviations[5] + deviations[6]);
   scale = strategy_mad_normalizer * mad;
   if(!MathIsValidNumber(mad) || mad <= 0.0 ||
      !MathIsValidNumber(scale) || scale <= 0.0)
      return false;
      ...
      for(int index = 0; index < strategy_return_count; ++index)
        {
         const double normalized =
            MathAbs((returns[index] - location) / scale);
         ...
         if(normalized <= strategy_hampel_a)
            ...
            weight = strategy_hampel_a / normalized;
         else if(normalized < strategy_hampel_c)
            weight =
               strategy_hampel_a * (strategy_hampel_c - normalized) /
               ((strategy_hampel_c - strategy_hampel_b) * normalized);
         if(!MathIsValidNumber(weight) || weight < 0.0)
            return false;
         weighted_sum += weight * returns[index];
         weight_sum += weight;
        }
      if(!MathIsValidNumber(weighted_sum) ||
         !MathIsValidNumber(weight_sum) || weight_sum <= 0.0)
         return false;
...
bool Strategy_LoadMonthlyHampelSignal(
      ...
      int &direction)
  {
   ...
   if(StringLen(return_path) <= 0)
      return false;
   if(!Strategy_HampelLocation(relative_returns,
                                 median,
                                 mad,
                                 scale,
                                 location))
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41348_XAUXAG_HAMPEL_RV_HOST";
   ...
   string return_path = "";
   const bool valid =
      Strategy_LoadMonthlyHampelSignal(
         g_signal_month_key,
         month_count,
   ...
   string state = "monthly_hampel_signal_failed";
   if(valid && direction > 0)
      state = "negative_robust_location_fade_long_xau";
   else if(valid && direction < 0)
      state = "positive_robust_location_fade_short_xau";
   else if(valid && MathAbs(location) <= strategy_signal_epsilon)
      state = "robust_location_inside_epsilon_flat";
   else if(valid)
      state = "nonqualifying_hampel_flat";

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"month_count\":%d,\"newest_endpoint_time\":%I64d,\"median\":%.12e,\"mad\":%.12e,\"scale\":%.12e,\"hampel_location\":%.12e,\"steps\":%d,\"direction\":%d,\"ratios\":\"%s\",\"returns\":\"%s\",\"state\":\"%s\"}",
                            g_signal_month_key,
                            month_count,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG monthly redescending-hampel reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41348\",\"ea\":\"xauxag-mhampel-rv\"}");
