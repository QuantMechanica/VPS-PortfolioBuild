...
// =============================================================================
// QM5_20025 - WTI February-October Daily Rotation
// -----------------------------------------------------------------------------
// Li et al. (2022) WTI weekday carrier:
//   - BUY only at the opening of a broker month-state D1 bar
...
input int    strategy_long_month          = 2;
input int    strategy_short_month         = 10;
...
int Strategy_MonthOfYear(const datetime value)
  {
   MqlDateTime parts;
   ZeroMemory(parts);
   if(value <= 0 || !TimeToStruct(value, parts))
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_20025_WTI_FEBOCT_ATTEMPT_%d",
                       QM_FrameworkMagic());
  }
...
bool Strategy_PrimeLateMonthStateAttach()
  {
   ...
   if(!QM_ReadBar(_Symbol, PERIOD_D1, 0, current_bar))
      return true;
   const int attach_month = Strategy_MonthOfYear(current_bar.time);
   if(attach_month != strategy_long_month && attach_month != strategy_short_month)
      return true;
   ...
   // persist the missed month-state decision when attaching after the only
...
bool Strategy_ConsumeMonthStateDecision()
  {
   ...
   if(!g_strategy_new_d1_bar || g_strategy_d1_bar_time <= 0)
      return false;
   const int decision_month = Strategy_MonthOfYear(g_strategy_d1_bar_time);
   if(decision_month != strategy_long_month && decision_month != strategy_short_month)
      return false;
   ...
   // rejected month-state signal is terminal for that broker day.
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsWtiD1())
      return true;
   if(qm_ea_id != 20025 || qm_magic_slot_offset != 0)
      return true;
   if(!qm_friday_close_enabled ||
      qm_friday_close_hour_broker != 21)
      return true;
   if(strategy_long_month != 2 || strategy_short_month != 10)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ZeroMemory(req);
   const int entry_month = Strategy_MonthOfYear(g_strategy_d1_bar_time);
   req.type = (entry_month == strategy_long_month ? QM_BUY : QM_SELL);
   req.price = 0.0;
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = (req.type == QM_BUY ? "WTI_FEB_DAILY_LONG" : "WTI_OCT_DAILY_SHORT");
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved February/October daily rotation retains Friday flattening"))
      return INIT_FAILED;
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_20025\",\"ea\":\"wti-feboct-daily\"}");
