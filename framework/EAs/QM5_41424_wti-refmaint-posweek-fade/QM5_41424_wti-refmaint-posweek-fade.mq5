...
#property description "QM5_41424 WTI Refinery-Maintenance Positive-Week Reversion"
...
//   - decide only in February, March, September, and October
//   - aggregate the immediately completed 3-to-5-session normalized week
//   - short only after a strictly positive weekly open-to-close return
...
bool Strategy_LoadPositiveWeekSignal(const int current_week_key,
                                     const int label_offset,
                                     int &direction)
  {
   direction = 0;
      ...
      if(prior_week_key == 0)
        {
         if(Strategy_NextWeekKey(week_key) != current_week_key)
            return false;
         prior_week_key = week_key;
        }
      else if(week_key != prior_week_key)
        {
         if(Strategy_NextWeekKey(week_key) != prior_week_key)
            return false;
   ...
   g_newer_week_bars = week_bars;
   g_newer_week_open = week_open;
   g_newer_week_close = week_close;
   g_newer_week_log_return =
      MathLog(g_newer_week_close / g_newer_week_open);
   if(!MathIsValidNumber(g_newer_week_log_return))
      return false;

   // The maintenance sleeve is deliberately asymmetric: a strictly positive
   // completed week is faded short; nonpositive returns consume flat.
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_WeekAlreadyEntered(g_decision_week_key))
      ...
      g_signal_state = "outside_maintenance_months";
   else
     {
      g_signal_valid =
         Strategy_LoadPositiveWeekSignal(g_decision_week_key,
                                         g_decision_label_offset,
                                         g_signal_direction);
      if(!g_signal_valid)
         g_signal_state = "completed_week_validation_failed";
      else if(g_signal_direction < 0)
         g_signal_state = "positive_week_reversion_short";
      else
         g_signal_state = "nonpositive_week_consumed_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"week\":%d,\"anchor_month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_week_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"prior_week_bars\":%d,\"prior_week_return\":%.12f,\"state\":\"%s\"}",
                            g_decision_week_key,
                            anchor_month,
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41424_WTI_REFMAINT_POSWEEK_FADE";
   ...
   req.type = QM_SELL;
   req.reason = "WTI_REFMAINT_POSWEEK_FADE_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(strategy_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41424 ||
      qm_magic_slot_offset != 0)
   ...
   g_attempt_state_key =
      StringFormat("QM5_41424_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41424\",\"ea\":\"wti-refmaint-posweek-fade\"}");
