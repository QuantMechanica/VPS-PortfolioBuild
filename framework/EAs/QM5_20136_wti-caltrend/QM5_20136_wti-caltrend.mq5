...
#property description "QM5_20136 WTI same-calendar and 63-D1 trend agreement"
...
//   2. calculate the sign of WTI's completed 63-D1 log return; and
//   3. trade only when the two non-zero signs agree.
// The package closes and may renew at the next month boundary. A consumed
// month is never retried after disagreement, restart, rejection, stop, or a
// blocked gate. Runtime uses native MT5 data only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                     = 20136;
...
bool Strategy_LoadMomentumSignal(double &momentum_score,
                                 int &direction)
  {
   momentum_score = 0.0;
   direction = 0;
   ...
   const int copied = CopyClose(_Symbol, PERIOD_D1, 1, strategy_momentum_lookback_d1 + 1, closes); // perf-allowed: bounded sample called only from the monthly boundary.
   ...
   momentum_score = MathLog(close_recent / close_past);
...
void Strategy_AdvanceMonthlyState()
  {
   ...
   const bool momentum_valid =
      Strategy_LoadMomentumSignal(g_momentum_score,
                                  momentum_direction);
...
bool Strategy_InputsValid()
  {
   if(qm_ea_id != 20136 || qm_magic_slot_offset != 0)
      return false;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20136_WTI_CALTREND";
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_20136_MONTH_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_20136\",\"ea\":\"wti-caltrend\"}");
