...
#property description "QM5_12981 Brent Feb-Sep Seasonal Premium"
...
input int    strategy_end_month          = 9;
...
bool Strategy_InSourceWindow(const datetime t)
  {
   const int mon = Strategy_Month(t);
   return (mon >= strategy_start_month && mon <= strategy_end_month);
  }

bool Strategy_IsFirstTradableBarOfMonth(const datetime current_d1_bar)
  {
   const datetime prior_d1_bar = iTime(_Symbol, PERIOD_D1, 1); // perf-allowed: D1 calendar gate behind new-bar gate.
   if(current_d1_bar <= 0 || prior_d1_bar <= 0)
      return false;
   return (Strategy_MonthKey(current_d1_bar) != Strategy_MonthKey(prior_d1_bar));
  }
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      bool should_close = !Strategy_InSourceWindow(current_d1_bar > 0 ? current_d1_bar : now);
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12981_BRENT_FEBSEP_PREM";
   ...
   if(current_d1_bar <= 0)
      return false;
   if(!Strategy_InSourceWindow(current_d1_bar))
      return false;
   if(!Strategy_IsFirstTradableBarOfMonth(current_d1_bar))
      return false;

   const int month_key = Strategy_MonthKey(current_d1_bar);
   ...
   req.reason = "BRENT_FEBSEP_MONTH_START_LONG";
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_12981\",\"ea\":\"brent-febsep-prem\"}");
