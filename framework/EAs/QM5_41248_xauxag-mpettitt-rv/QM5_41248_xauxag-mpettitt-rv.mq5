...
#property description "QM5_41248 XAU/XAG Monthly Pettitt Ratio Change-Point Reversion"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_41248 - XAU/XAG Monthly Pettitt Ratio Change-Point Reversion
// -----------------------------------------------------------------------------
...
// completed broker months. Strict-rank the gold-minus-silver log ratios, compute
// every Pettitt cumulative rank sum, and fade one unique maximizing split
// only when it lies in the locked central K=4..9 band. The opposite,
// equal-notional package is held to the next month.
// Runtime is native, completed-price-only, deterministic, and one attempt per
// month.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41248;
...
input int    strategy_min_change_index        = 4;
input int    strategy_max_change_index        = 9;
...
int      g_pair_direction = 0;
string   g_pair_direction_state_key = "";
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41248 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
...
bool Strategy_PairCompositionValid()
  {
   if(g_pair_direction != 1 && g_pair_direction != -1)
      return false;
   int owned_count = 0;
   int xau_count = 0;
   int xag_count = 0;
   ...
   const ENUM_POSITION_TYPE expected_xau_type =
      (g_pair_direction > 0) ? POSITION_TYPE_BUY : POSITION_TYPE_SELL;
   const ENUM_POSITION_TYPE expected_xag_type =
      (g_pair_direction > 0) ? POSITION_TYPE_SELL : POSITION_TYPE_BUY;
   return (owned_count == 2 && xau_count == 1 && xag_count == 1 &&
           xau_type == expected_xau_type && xag_type == expected_xag_type);
  }
...
void Strategy_CloseAllOwned(const QM_ExitReason reason)
  {
   ...
   if(Strategy_OpenOwnedPositionCount() == 0)
     {
      g_pair_direction = 0;
      if(g_pair_direction_state_key != "")
         GlobalVariableDel(g_pair_direction_state_key);
     }
  }
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_MPETTITT_RV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }

string Strategy_PairDirectionStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_MPETTITT_RV_PAIR_DIRECTION_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }

void Strategy_LoadPairDirectionState()
  {
   g_pair_direction_state_key = Strategy_PairDirectionStateKey();
   g_pair_direction = 0;
   if(!GlobalVariableCheck(g_pair_direction_state_key))
      return;
   const double stored = GlobalVariableGet(g_pair_direction_state_key);
   const int stored_direction = (int)MathRound(stored);
   if(MathIsValidNumber(stored) &&
      (stored_direction == 1 || stored_direction == -1))
      g_pair_direction = stored_direction;
   else
      GlobalVariableDel(g_pair_direction_state_key);
  }

bool Strategy_RecordPairDirectionState(const int direction)
  {
   if(direction != 1 && direction != -1)
      return false;
   if(g_pair_direction_state_key == "")
      g_pair_direction_state_key = Strategy_PairDirectionStateKey();
   // Keep the in-process state fail-closed even if terminal persistence fails.
   g_pair_direction = direction;
   if(GlobalVariableSet(g_pair_direction_state_key, (double)direction) > 0)
      return true;
   g_pair_direction = 0;
   return false;
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41248_XAUXAG_MPETTITT_RV";
...
bool Strategy_OpenPair(const int direction)
  {
   ...
   if(!Strategy_PreparePackage(direction,
                               ...
                               xau_stop, xag_stop))
      return false;
   if(!Strategy_RecordPairDirectionState(direction))
      return false;
   const QM_OrderType xau_type = (direction > 0) ? QM_BUY : QM_SELL;
   const QM_OrderType xag_type = (direction > 0) ? QM_SELL : QM_BUY;
...
bool Strategy_LoadMonthlyPettitt(
      ...
      int &direction)
  {
   ...
   // observation: the card requires a strict permutation of 1..13.
   ...
   int ranks[];
   if(ArrayResize(ranks, strategy_endpoint_count) != strategy_endpoint_count)
      return false;
   bool seen_ranks[14];
   ArrayInitialize(seen_ranks, false);
   ...
   for(int index = 0; index < strategy_endpoint_count; ++index)
     {
      ...
      for(int other = 0; other < strategy_endpoint_count; ++other)
        {
         if(chronological_ratios[other] < chronological_ratios[index])
            ++rank;
        }
      if(rank < 1 || rank > strategy_endpoint_count || seen_ranks[rank])
         return false;
   ...
   for(int index = 0; index < strategy_endpoint_count; ++index)
     {
      if(index > 0)
         rank_csv += ",";
      rank_csv += IntegerToString(ranks[index]);
     }

   int cumulative_rank_sum = 0;
   int unique_change_index = 0;
   int unique_signed_rank_sum = 0;
   for(int k = 1; k < strategy_endpoint_count; ++k)
     {
      cumulative_rank_sum += ranks[k - 1];
      const int rank_sum =
         2 * cumulative_rank_sum - (strategy_endpoint_count + 1) * k;
      const int absolute_rank_sum = MathAbs(rank_sum);
      if((rank_sum % 2) != 0 || absolute_rank_sum > 42)
         return false;
      if(k > 1)
         rank_sum_csv += ",";
      rank_sum_csv += IntegerToString(rank_sum);

      if(absolute_rank_sum > rank_sum_max)
        {
         rank_sum_max = absolute_rank_sum;
         maximum_count = 1;
         unique_change_index = k;
         unique_signed_rank_sum = rank_sum;
        }
      else if(absolute_rank_sum == rank_sum_max)
        {
         ++maximum_count;
        }
     }

   if(rank_sum_max <= 0 || rank_sum_max > 42 ||
      (rank_sum_max % 2) != 0 || maximum_count <= 0)
      return false;

   if(maximum_count == 1)
     {
      change_index = unique_change_index;
      signed_rank_sum = unique_signed_rank_sum;
     }

   // direction > 0 means BUY XAU / SELL XAG. Negative U[K] means the later
   // ratio regime is higher, so fade it short. Positive U[K] means the later
   // ratio regime is lower, so fade it long. Magnitude never scales risk.
   if(maximum_count == 1 &&
      change_index >= strategy_min_change_index &&
      change_index <= strategy_max_change_index)
     {
      if(signed_rank_sum < 0)
         direction = -1;
      else if(signed_rank_sum > 0)
         direction = 1;
     }
   return true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41248_XAUXAG_MPETTITT_RV_HOST";
   ...
   string state = "monthly_pettitt_failed";
   if(valid && direction > 0)
      state = "lower_late_ratio_fade_long_xau";
   else if(valid && direction < 0)
      state = "higher_late_ratio_fade_short_xau";
   else if(valid && tie_count > 0)
      state = "ratio_rank_tie_flat";
   else if(valid && maximum_count != 1)
      state = "pettitt_maximum_tied_flat";
   else if(valid)
      state = "pettitt_edge_split_flat";

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"month_count\":%d,\"newest_endpoint_time\":%I64d,\"endpoint_displacement\":%.12e,\"rank_sum_max\":%d,\"maximum_count\":%d,\"change_index\":%d,\"signed_rank_sum\":%d,\"rank_csv\":\"%s\",\"rank_sum_csv\":\"%s\",\"tie_count\":%d,\"direction\":%d,\"state\":\"%s\"}",
                            g_signal_month_key,
                            month_count,
...
void Strategy_ManageOpenPosition()
  {
   ...
   if(open_positions <= 0)
     {
      if(g_pair_direction != 0 ||
         (g_pair_direction_state_key != "" &&
          GlobalVariableCheck(g_pair_direction_state_key)))
        {
         g_pair_direction = 0;
         GlobalVariableDel(g_pair_direction_state_key);
        }
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG thirteen-month Pettitt ratio change-point reversion package holds through Fridays until next broker month"))
     {
   ...
   Strategy_LoadPairDirectionState();
   g_pair_entry_time = Strategy_CurrentPairEntryTime();
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41248\",\"ea\":\"xauxag-mpettitt-rv\"}");
