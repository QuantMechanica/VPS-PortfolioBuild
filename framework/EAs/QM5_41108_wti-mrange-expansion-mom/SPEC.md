# QM5_41108_wti-mrange-expansion-mom - Strategy Spec
...
**Source:** `MOP-WTI-MRANGE-EXPANSION-MOM-2026`
...
Require the newest completed month's aggregate high-low range to be strictly
wider than its parent's aggregate range. Buy when that expanded month closes
above its own first open; sell when it closes below its own first open. Equal
or narrower ranges, equal open/close, incomplete history, or malformed states
consume the month flat. The position follows the expanded monthly body for
one broker month, using one fixed-risk budget and a frozen ATR hard stop.
...
- Symbol slot: 0.
- Magic: `411080000` (governed slot-0 allocation for `XTIUSD.DWX`).
...
- Formation: the two immediately completed consecutive broker-calendar month
  OHLC packages.
- Trigger: strict newest-range-greater-than-parent comparison plus the newest
  completed month's own body sign.
...
- Symmetric direct-WTI monthly structural continuation after range expansion.
...
tests, and explicit WTI membership. Strict monthly range expansion and the
expanded-month body state are disclosed QM hypotheses; no paper or sibling
...
current-month breakout, minimum expansion ratio, external feed, retry,
...
|---|---|---|---|
| v0 | 2026-08-22 | approved build-directory identity | source approval `de681718f`; EA-ID reservation `2de3e2cc9`; Q00 card `9612dd539`; governed magic `411080000` |
