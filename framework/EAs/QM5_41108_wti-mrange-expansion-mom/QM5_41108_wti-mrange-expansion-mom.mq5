...
#property description "QM5_41108 WTI Completed-Month Range-Expansion Momentum"
...
//   - aggregate two consecutive completed broker-calendar months
//   - require the newest full range strictly wider than its parent range
//   - follow the expanded month's own open-to-close body direction
//   - consume the month before fallible gates; never enter late or retry
//   - follow the completed expanded-month body for one broker month
// Runtime uses MT5-native OHLC, calendar, history, and framework state only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                      = 41108;
...
bool Strategy_LoadRangeExpansionSignal(const int current_month_key,
                                   const int label_offset,
                                   int &direction,
                                   int &new_month_bars,
                                   ...
                                   double &parent_low)
  {
   ...
   for(int index = 0; index < copied; ++index)
     {
      ...
      // CopyRates is a series (newest to oldest), so the last assigned open
      // is the chronologically first session open for this month.
      month_opens[bucket] = bars[index].open;
      ++month_counts[bucket];
   ...
   for(int bucket = 0; bucket < 2; ++bucket)
     {
      ...
      if(month_highs[bucket] <= 0.0 ||
         month_lows[bucket] <= 0.0 ||
         month_opens[bucket] <= 0.0 ||
         month_closes[bucket] <= 0.0 ||
         aggregate_range <= 0.0 ||
         !MathIsValidNumber(month_highs[bucket]) ||
         !MathIsValidNumber(month_lows[bucket]) ||
         !MathIsValidNumber(month_opens[bucket]) ||
   ...
   new_open = month_opens[0];
   new_close = month_closes[0];
   parent_high = month_highs[1];
   parent_low = month_lows[1];
   ...
   const bool expanded = (new_range > parent_range);
   if(expanded && new_close > new_open)
      direction = 1;
   else if(expanded && new_close < new_open)
      direction = -1;
   return true;
  }
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadRangeExpansionSignal(g_decision_month_key,
                                           ...
                                           g_parent_high,
                                           g_parent_low);
      const double new_range = g_new_high - g_new_low;
      const double parent_range = g_parent_high - g_parent_low;
      if(!g_signal_valid)
         g_signal_state = "monthly_ohlc_validation_failed";
      else if(g_signal_direction > 0)
         g_signal_state = "range_expansion_long";
      else if(g_signal_direction < 0)
         g_signal_state = "range_expansion_short";
      else if(new_range == parent_range)
         g_signal_state = "range_equality_flat";
      else if(new_range > parent_range)
         g_signal_state = "expanded_body_equality_flat";
      else
         g_signal_state = "not_expanded_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"new_month_bars\":%d,\"parent_month_bars\":%d,\"new_high\":%.8f,\"new_low\":%.8f,\"new_open\":%.8f,\"new_close\":%.8f,\"parent_high\":%.8f,\"parent_low\":%.8f,\"state\":\"%s\"}",
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41108 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41108_WTI_MRANGE_EXPANSION_MOM";
   ...
   req.reason = (g_signal_direction > 0)
      ? "WTI_MRANGE_EXPANSION_MOM_LONG"
      : "WTI_MRANGE_EXPANSION_MOM_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41108 ||
   ...
   g_attempt_state_key =
      StringFormat("QM5_41108_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41108\",\"ea\":\"wti-mrange-expansion-mom\"}");
