...
#property description "QM5_41355 XAU/XAG Monthly Fixed-Trim Reversion"
...
// returns, sort them, delete exactly two observations from each tail, and
// average the middle eight. Fade that sign with an opposed, equal-target-
// notional package held
// to the next month. Runtime is deterministic, completed-price-only, and one
// consumed attempt per month.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41355;
...
input int    strategy_retained_count           = 8;
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_MTRIM2_RV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41355_XAUXAG_MTRIM2_RV";
...
bool Strategy_TrimmedMean(const double &returns[],
                          double &location)
  {
   location = 0.0;
   if(ArraySize(returns) != 12 || strategy_return_count != 12 ||
      strategy_trim_each_tail != 2 || strategy_retained_count != 8)
      return false;
   ...
   const int first = strategy_trim_each_tail;
   const int last_exclusive = strategy_return_count - strategy_trim_each_tail;
   if(first != 2 || last_exclusive != 10 ||
      last_exclusive - first != strategy_retained_count)
      return false;
   for(int index = first; index < last_exclusive; ++index)
      retained_sum += sorted[index];
   location = retained_sum / (double)strategy_retained_count;
   return MathIsValidNumber(location);
  }

bool Strategy_LoadMonthlyTrimmedSignal(
      ...
      int &direction)
  {
   ...
   if(StringLen(return_path) <= 0)
      return false;
   if(!Strategy_TrimmedMean(relative_returns, location))
      return false;
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41355_XAUXAG_MTRIM2_RV_HOST";
   ...
   string return_path = "";
   const bool valid =
      Strategy_LoadMonthlyTrimmedSignal(
         g_signal_month_key,
         month_count,
   ...
   string state = "monthly_trimmed_signal_failed";
   if(valid && direction > 0)
      state = "negative_trimmed_mean_fade_long_xau";
   else if(valid && direction < 0)
      state = "positive_trimmed_mean_fade_short_xau";
   else if(valid && MathAbs(location) <= strategy_signal_epsilon)
      state = "trimmed_mean_inside_epsilon_flat";
   else if(valid)
      state = "nonqualifying_trimmed_mean_flat";

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"month_count\":%d,\"newest_endpoint_time\":%I64d,\"trim_each_tail\":%d,\"retained_count\":%d,\"trimmed_mean\":%.12e,\"direction\":%d,\"ratios\":\"%s\",\"returns\":\"%s\",\"state\":\"%s\"}",
                            g_signal_month_key,
                            month_count,
                            (long)newest_endpoint_time,
                            strategy_trim_each_tail,
                            strategy_retained_count,
                            location,
                            direction,
                            ratio_path,
                            return_path,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG monthly fixed-trim reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41355\",\"ea\":\"xauxag-mtrim2-rv\"}");
