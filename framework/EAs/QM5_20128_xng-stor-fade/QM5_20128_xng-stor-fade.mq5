...
#property description "QM5_20128 EIA XNG storage-release M30 failed-break fade"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_20128 - EIA XNG Storage-Release M30 Failed-Break Fade
// -----------------------------------------------------------------------------
// Standard Thursdays only:
//   - require the 10:30-11:00 New York WNGSR bar to break the prior-hour range
//   - wait for the 11:00-11:30 bar to reclaim that range and impulse midpoint
//   - fade toward the release open and flatten in the same New York session
// The New York date is consumed before fallible history, signal, news, spread,
// geometry, price, or order checks. Runtime uses native MT5 OHLC only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                     = 20128;
...
input int    strategy_confirmation_hhmm_ny     = 1100;
...
input double strategy_reclaim_mid_fraction     = 0.50;
input int    strategy_atr_period               = 20;
input double strategy_stop_buffer_atr          = 0.25;
input double strategy_min_reward_risk          = 0.50;
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 20128 &&
           qm_magic_slot_offset == 0 &&
           strategy_release_hhmm_ny == 1030 &&
           strategy_confirmation_hhmm_ny == 1100 &&
           ...
           MathAbs(strategy_reclaim_mid_fraction - 0.50) <= 1.0e-12 &&
           strategy_atr_period == 20 &&
           MathAbs(strategy_stop_buffer_atr - 0.25) <= 1.0e-12 &&
           MathAbs(strategy_min_reward_risk - 0.50) <= 1.0e-12 &&
...
void Strategy_AdvanceEventState()
  {
   ...
   if(!Strategy_BarMatches(1, day_key, strategy_confirmation_hhmm_ny) ||
   ...
   const double release_open =
      iOpen(_Symbol, PERIOD_M30, 2); // perf-allowed: bounded event bar on a genuine new bar.
   const double release_high =
      iHigh(_Symbol, PERIOD_M30, 2); // perf-allowed: bounded event bar on a genuine new bar.
   const double release_low =
      iLow(_Symbol, PERIOD_M30, 2); // perf-allowed: bounded event bar on a genuine new bar.
   const double release_close =
      iClose(_Symbol, PERIOD_M30, 2); // perf-allowed: bounded event bar on a genuine new bar.
   const double reclaim_open =
      iOpen(_Symbol, PERIOD_M30, 1); // perf-allowed: bounded reclaim bar on a genuine new bar.
   const double reclaim_high =
      iHigh(_Symbol, PERIOD_M30, 1); // perf-allowed: bounded reclaim bar on a genuine new bar.
   const double reclaim_low =
      iLow(_Symbol, PERIOD_M30, 1); // perf-allowed: bounded reclaim bar on a genuine new bar.
   const double reclaim_close =
      iClose(_Symbol, PERIOD_M30, 1); // perf-allowed: bounded reclaim bar on a genuine new bar.
   ...
   const double pre_high_second =
      iHigh(_Symbol, PERIOD_M30, 4); // perf-allowed: fixed two-bar source range on a genuine new bar.
   ...
   const double pre_low_second =
      iLow(_Symbol, PERIOD_M30, 4); // perf-allowed: fixed two-bar source range on a genuine new bar.
   ...
   const double release_midpoint =
      release_low + strategy_reclaim_mid_fraction * release_range;
   const bool reclaim_inside =
      (reclaim_close > pre_low && reclaim_close < pre_high);
   const bool bullish_impulse =
      (release_close > release_open && release_close > pre_high);
   const bool bearish_impulse =
      (release_close < release_open && release_close < pre_low);

   if(bullish_impulse &&
      reclaim_close < reclaim_open &&
      reclaim_inside &&
      reclaim_close < release_midpoint)
     {
   ...
   g_signal_target_price = release_open;
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_20128_XNG_STORAGE_FADE_M30";
   ...
   request.sl = QM_StopRulesNormalizePrice(_Symbol, g_signal_stop_price);
   request.tp = QM_StopRulesNormalizePrice(_Symbol, g_signal_target_price);
   ...
   request.reason =
      (request.type == QM_BUY) ?
      "XNG_WNGSR_M30_FAILED_BREAK_LONG" :
      "XNG_WNGSR_M30_FAILED_BREAK_SHORT";
   return true;
  }
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_20128_XNG_STOR_FADE_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_20128\",\"ea\":\"xng-stor-fade\"}");
