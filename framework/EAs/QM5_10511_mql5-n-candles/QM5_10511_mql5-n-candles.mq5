...
#property description "QM5_10511 MQL5 N Consecutive Candles Trend"
...
input int    strategy_n_candles         = 3;
input int    strategy_atr_period        = 14;
input double strategy_atr_sl_mult       = 1.5;
input double strategy_tp_rr             = 1.25;
...
int Strategy_StreakSignal()
  {
   if(strategy_n_candles <= 0)
      return 0;

   bool all_bullish = true;
   bool all_bearish = true;
   for(int shift = 1; shift <= strategy_n_candles; ++shift)
     {
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   if(strategy_n_candles <= 0 || strategy_atr_period <= 0 ||
   ...
   const int signal = Strategy_StreakSignal();
   if(signal == 0)
      return false;
   ...
   req.sl = sl;
   req.tp = tp;
   req.reason = (signal > 0) ? "N_CANDLES_BULLISH_LONG" : "N_CANDLES_BEARISH_SHORT";
...
void Strategy_ManageOpenPosition()
  {
   // Card baseline disables trailing, break-even, and partial-close management.
  }
