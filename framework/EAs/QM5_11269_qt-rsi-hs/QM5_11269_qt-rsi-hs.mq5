...
input int    strategy_pattern_horizon   = 25;
input double strategy_delta_rsi         = 0.2;
input double strategy_head_ratio        = 1.1;
input double strategy_shoulder_ratio    = 1.1;
input int    strategy_atr_period        = 14;
input double strategy_atr_sl_mult       = 2.0;
input double strategy_head_atr_buffer   = 0.25;
input double strategy_exit_rsi_change   = 4.0;
input int    strategy_exit_bars         = 5;
...
bool Strategy_NoTradeFilter()
  {
   if(strategy_rsi_period <= 0 ||
      strategy_pattern_horizon < 8 ||
      strategy_pattern_horizon > 60 ||
      strategy_delta_rsi <= 0.0 ||
      strategy_head_ratio <= 1.0 ||
      strategy_shoulder_ratio <= 1.0 ||
      strategy_atr_period <= 0 ||
      strategy_atr_sl_mult <= 0.0 ||
      strategy_head_atr_buffer < 0.0 ||
      strategy_exit_rsi_change <= 0.0 ||
      strategy_exit_bars <= 0)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   const int horizon = strategy_pattern_horizon;
   MqlRates rates[];
   ...
   const double atr_stop = QM_StopATR(_Symbol, QM_SELL, bid, strategy_atr_period, strategy_atr_sl_mult);
   const double head_stop = NormalizeDouble(rates[head_shift].high + atr * strategy_head_atr_buffer, _Digits);
   if(atr_stop <= bid)
      return false;
...
bool Strategy_ExitSignal()
  {
   ...
   if(rsi_now == EMPTY_VALUE || !MathIsValidNumber(rsi_now))
      return false;
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      const int rsi_pos = StringFind(comment, "rsi=");
      if(rsi_pos >= 0)
         entry_rsi = StringToDouble(StringSubstr(comment, rsi_pos + 4));

      if(entry_rsi > 0.0 && rsi_now - entry_rsi > strategy_exit_rsi_change)
         return true;
