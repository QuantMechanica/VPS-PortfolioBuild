...
#property description "QM5_1402 Harmonic Cypher XABCD H4"

#include <QM/QM_Common.mqh>

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                     = 1402;
...
input int    strategy_min_xd_bars              = 20;
input int    strategy_max_xd_bars              = 60;
input int    strategy_scan_bars                = 200;
input double strategy_fib_tolerance_pct        = 0.03;
input double strategy_ab_xa_min                = 0.382;
input double strategy_ab_xa_max                = 0.618;
input double strategy_bc_ab_min                = 1.272;
input double strategy_bc_ab_max                = 1.414;
input double strategy_bc_xa_min                = 1.130;
input double strategy_bc_xa_max                = 1.414;
input double strategy_cd_xc_retrace            = 0.786;
...
bool Strategy_MacroOK(const bool bullish, const double current_close)
  {
   if(!strategy_macro_bias_enabled)
      return true;

   const double sma = QM_SMA(_Symbol, PERIOD_H4, strategy_macro_sma_period, 1, PRICE_CLOSE);
   if(sma <= 0.0)
      return false;

   return bullish ? (current_close > sma) : (current_close < sma);
  }
...
bool Strategy_BuildEntry(const bool bullish, const double x_price, const double c_price,
                         const double d_price, const double atr, QM_EntryRequest &req)
  {
   ...
   double sl = bullish ? (x_price - strategy_sl_atr_mult * atr)
                       : (x_price + strategy_sl_atr_mult * atr);
   ...
   req.reason = bullish ? "HARMONIC_CYPHER_BULLISH" : "HARMONIC_CYPHER_BEARISH";
...
bool Strategy_CheckBullish(const StrategyPivot &x, const StrategyPivot &a,
                           ...
                           const MqlRates &d_bar, QM_EntryRequest &req)
  {
   ...
   if(x.shift < strategy_min_xd_bars || x.shift > strategy_max_xd_bars)
      return false;
   // Geometry: X-low < A-high, B-low > X-low and < A-high, C-high > A-high, D-low > X-low (invalidation guard)
   if(!(x.price < a.price && b.price > x.price && b.price < a.price && c.price > a.price))
      return false;
   if(d_bar.low <= x.price)
      return false;

   const double xa = MathAbs(a.price - x.price);
   const double ab = MathAbs(a.price - b.price);
   const double bc = MathAbs(c.price - b.price);
   const double xc = MathAbs(c.price - x.price);
   ...
   if(!Strategy_RatioInRange(bc / ab, strategy_bc_ab_min, strategy_bc_ab_max))
      return false;
   if(!Strategy_RatioInRange(bc / xa, strategy_bc_xa_min, strategy_bc_xa_max))
      return false;

   const double d_target = c.price - strategy_cd_xc_retrace * xc;
   const double zone_low = d_target - strategy_fib_tolerance_pct * xc;
   const double zone_high = d_target + strategy_fib_tolerance_pct * xc;
   const bool low_touched = (d_bar.low >= zone_low && d_bar.low <= zone_high);
   const bool bullish_bar = (d_bar.close > d_bar.open);
   const bool close_inside_cx = (d_bar.close >= x.price && d_bar.close <= c.price);
   if(!low_touched || !bullish_bar || !close_inside_cx)
      return false;
   if(!Strategy_MacroOK(true, d_bar.close))
      return false;

   const double atr = QM_ATR(_Symbol, PERIOD_H4, strategy_atr_period, 1);
   return Strategy_BuildEntry(true, x.price, c.price, d_bar.low, atr, req);
  }
...
bool Strategy_CheckBearish(const StrategyPivot &x, const StrategyPivot &a,
                           ...
                           const MqlRates &d_bar, QM_EntryRequest &req)
  {
   ...
   if(x.shift < strategy_min_xd_bars || x.shift > strategy_max_xd_bars)
      return false;
   // Geometry: X-high > A-low, B-high < X-high and > A-low, C-low < A-low, D-high < X-high (invalidation guard)
   if(!(x.price > a.price && b.price < x.price && b.price > a.price && c.price < a.price))
      return false;
   if(d_bar.high >= x.price)
      return false;
   ...
   const bool close_inside_cx = (d_bar.close >= c.price && d_bar.close <= x.price);
   if(!high_touched || !bearish_bar || !close_inside_cx)
      return false;
   if(!Strategy_MacroOK(false, d_bar.close))
      return false;

   const double atr = QM_ATR(_Symbol, PERIOD_H4, strategy_atr_period, 1);
   return Strategy_BuildEntry(false, x.price, c.price, d_bar.high, atr, req);
  }
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_1402\",\"ea\":\"harmonic_cypher_xabcd_h4\"}");
