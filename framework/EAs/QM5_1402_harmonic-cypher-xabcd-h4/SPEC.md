# QM5_1402_harmonic-cypher-xabcd-h4 - Strategy Spec

**EA ID:** QM5_1402
**Slug:** `harmonic-cypher-xabcd-h4`
**Source:** `6e967762-b26d-59a3-b076-35c17f2e7c36` (see `D:/QM/strategy_farm/artifacts/cards_approved/QM5_1402_harmonic-cypher-xabcd-h4.md`)
...
The EA scans closed H4 bars for a Williams-fractal XABCD Cypher pattern. A bullish setup requires alternating low-high-low-high-low pivots (X-low, A-high, B-low, C-high, D-low) where B retraces 0.382-0.618 of XA, C extends 1.272-1.414 of AB (and 1.13-1.414 of XA), D must not exceed X (invalidation guard), and D touches the 0.786 retracement zone of XC. A bullish rejection bar confirms the D completion. A bearish setup mirrors the same ratios and rejection rule. The EA enters at market on the next H4 bar, places a structural stop beyond X capped at 2.5 ATR, exits half at the 38.2% CD retracement (moving the remaining runner to break-even), and targets the 61.8% CD retracement for full exit.
...
| `strategy_min_xd_bars` | 20 | 1-100 | Minimum bars from X pivot to D rejection bar. |
| `strategy_max_xd_bars` | 60 | 20-200 | Maximum bars from X pivot to D rejection bar. |
...
| `strategy_ab_xa_min` | 0.382 | 0.2-0.8 | Minimum AB/XA retracement. |
| `strategy_ab_xa_max` | 0.618 | 0.3-0.9 | Maximum AB/XA retracement. |
| `strategy_bc_ab_min` | 1.272 | 1.0-2.0 | Minimum BC/AB extension. |
| `strategy_bc_ab_max` | 1.414 | 1.1-2.5 | Maximum BC/AB extension. |
| `strategy_bc_xa_min` | 1.130 | 1.0-2.0 | Minimum BC/XA extension cross-check. |
| `strategy_bc_xa_max` | 1.414 | 1.1-2.5 | Maximum BC/XA extension cross-check. |
| `strategy_cd_xc_retrace` | 0.786 | 0.5-1.0 | D-completion retracement of XC leg. |
| `strategy_atr_period` | 14 | 2-100 | ATR period for stop placement. |
| `strategy_sl_atr_mult` | 0.5 | 0.1-5.0 | ATR buffer beyond X structure. |
...
| `strategy_macro_bias_enabled` | true | true/false | Enable H4 SMA(200) trend continuation filter. |
| `strategy_macro_sma_period` | 200 | 10-500 | Macro SMA period on H4. |
...
|---|---|
| Base timeframe | `H4` |
| Multi-timeframe refs | `H4` SMA(200) macro-bias filter |
...
| Regime preference | Trend-continuation pullback following extended C-leg. |
...
**Pointer:** `D:/QM/strategy_farm/artifacts/cards_approved/QM5_1402_harmonic-cypher-xabcd-h4.md`
**R1-R4 verdict (Q00):** all R1-R4 PASS per `D:/QM/strategy_farm/artifacts/cards_approved/QM5_1402_harmonic-cypher-xabcd-h4.md`
