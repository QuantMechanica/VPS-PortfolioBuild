...
#property description "QM5_20217 WTI Weekend Opening-Gap Momentum"
...
//   - BUY above Friday high plus 0.10 * lagged 90-D1 return volatility
//   - SELL below Friday low minus the symmetric volatility buffer
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(direction == 1)
     {
      req.type = QM_BUY;
      req.reason = "WTI_WKEND_GAP_UP_LONG";
     }
   else if(direction == -1)
     {
      req.type = QM_SELL;
      req.reason = "WTI_WKEND_GAP_DOWN_SHORT";
     }
   else
      return false;
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI weekend gap momentum card requires Friday 21 flattening"))
      return INIT_FAILED;

   g_attempt_state_key =
      StringFormat("QM5_20217_MONDAY_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_20217\",\"ea\":\"wti-wkend-mom\"}");
