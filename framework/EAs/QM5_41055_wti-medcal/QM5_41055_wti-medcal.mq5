...
#property description "QM5_41055 WTI Median Same-Calendar Seasonality"
...
//   - require five observations and trade the bounded sample median's sign
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(!QM_InputRequireLong("qm_ea_id", qm_ea_id, 41055) ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(g_signal_direction > 0)
     {
      req.type = QM_BUY;
      req.reason = "WTI_MEDCAL_LONG";
     }
   else
     {
      req.type = QM_SELL;
      req.reason = "WTI_MEDCAL_SHORT";
     }
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41055 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI median same-calendar card holds to the next broker month"))
      return INIT_FAILED;
   ...
   g_attempt_state_key =
      StringFormat("QM5_41055_MEDCAL_MONTH_ATTEMPT_%d",
                   QM_FrameworkMagic());
