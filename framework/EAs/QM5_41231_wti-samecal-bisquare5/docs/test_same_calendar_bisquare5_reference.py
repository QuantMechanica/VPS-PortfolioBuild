"""Independent deterministic fixtures for QM5_41231.
...
bisquare updates, sign boundaries, durable attempts, lifecycle, quotes, and
...
def bisquare_signal(observations: list[float]) -> BisquareResult | None:
    ...
    if not all(
        math.isfinite(value) and value > 0.0
        for value in (raw_mad, scale, cutoff)
    ):
        return None

    location = median
    for _ in range(BISQUARE_STEPS):
        weighted_sum = 0.0
        weight_sum = 0.0
...
class SameCalendarBisquareReferenceTests(unittest.TestCase):
    ...
    def test_median_mad_scale_and_updates_are_exact(self) -> None:
        values = [-0.06, -0.01, -0.005, 0.005, 0.02]
        result = bisquare_signal(values)
        self.assertIsNotNone(result)
        assert result is not None
        self.assertEqual(result.sorted_values, tuple(sorted(values)))
        self.assertEqual(result.median, -0.005)
        self.assertEqual(result.raw_mad, 0.01)
        self.assertAlmostEqual(result.scale, 0.014826)
        self.assertAlmostEqual(result.cutoff, 0.06945981)
        self.assertAlmostEqual(result.location, 0.00124940938226171, places=15)
    ...
    def test_disagreement_fixture_opposes_peer_estimators(self) -> None:
        ...
        winsor_mean = sum([-0.01, -0.01, -0.005, 0.005, 0.005]) / 5.0
        trimean = (-0.01 + 2.0 * -0.005 + 0.005) / 4.0
        midhinge = (-0.01 + 0.005) / 2.0
        shortest_three_mean = sum([-0.01, -0.005, 0.005]) / 3.0
        inclusive_pairs = sorted(
            (left + right) / 2.0
            for index, left in enumerate(sorted(values))
            for right in sorted(values)[index:]
        )
        ...
        self.assertLess(midhinge, 0.0)
        self.assertLess(shortest_three_mean, 0.0)
        self.assertLess(inclusive_pairs[7], 0.0)

    def test_sign_reflection_is_exactly_symmetric(self) -> None:
        values = [-0.06, -0.01, -0.005, 0.005, 0.02]
        positive = bisquare_signal(values)
        negative = bisquare_signal([-value for value in values])
        self.assertIsNotNone(positive)
        self.assertIsNotNone(negative)
        assert positive is not None and negative is not None
        self.assertAlmostEqual(negative.location, -positive.location, places=15)
    ...
    def test_strict_support_gives_remote_tail_zero_influence(self) -> None:
        first = bisquare_signal([0.0, 1.0, 2.0, 3.0, 100.0])
        farther = bisquare_signal([0.0, 1.0, 2.0, 3.0, 1000.0])
    ...
    def test_sort_makes_year_order_irrelevant_after_membership(self) -> None:
        values = [-0.06, -0.01, -0.005, 0.005, 0.02]
        forward = bisquare_signal(values)
        reverse = bisquare_signal(values[::-1])
    ...
    def test_strict_epsilon_and_invalid_states_fail_closed(self) -> None:
        centered = bisquare_signal([-2.0, -1.0, 0.0, 1.0, 2.0])
        positive = bisquare_signal(
        ...
        self.assertIsNone(bisquare_signal([0.01] * 4))
        self.assertIsNone(bisquare_signal([0.01] * 5))
        self.assertIsNone(bisquare_signal([0.01, 0.01, math.nan, 0.01, 0.01]))
    ...
    def test_static_build_contract_matches_approved_card(self) -> None:
        source_path = EA_DIR / "QM5_41231_wti-samecal-bisquare5.mq5"
        source = source_path.read_text(encoding="utf-8")
        set_path = (
            EA_DIR
            / "sets"
            / "QM5_41231_wti-samecal-bisquare5_XTIUSD.DWX_D1_backtest.set"
        )
        ...
        for marker in (
            ...
            "strategy_bisquare_cutoff     = 4.685;",
        ...
        self.assertLess(
            ...
            prepare.index("Strategy_LoadBisquareSignal"),
        )
        ...
        approved = (
            ...
            / "cards"
            / "approved"
            / "QM5_41231_wti-samecal-bisquare5_card.md"
        )
        ...
        self.assertIn(
            "41231,wti-samecal-bisquare5,0,XTIUSD.DWX,412310000",
            magic_rows,
        )
        resolver = (
            REPO_ROOT
        ...
        self.assertIn("412310000", resolver)
