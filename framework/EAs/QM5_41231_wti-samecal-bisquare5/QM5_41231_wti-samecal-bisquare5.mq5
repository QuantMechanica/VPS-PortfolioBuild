...
#property description "QM5_41231 WTI Same-Calendar Five-Sample Bisquare"
...
//   - freeze the normalized cutoff and execute exactly 32 bisquare updates
//   - give observations at or beyond strict support exactly zero influence
...
// -----------------------------------------------------------------------------
// Exact completed-month endpoint and five-sample-bisquare helpers.
// -----------------------------------------------------------------------------
...
bool Strategy_BisquareSignal(const double &observations[],
                             ...
                             int &direction)
  {
   ...
   if(raw_mad <= 0.0 || scale <= 0.0 || cutoff <= 0.0 ||
   ...
   for(int step = 0; step < strategy_bisquare_steps; ++step)
     {
      ...
      for(int index = 0; index < 5; ++index)
        {
         const double normalized = (observations[index] - mu) / cutoff;
         if(!MathIsValidNumber(normalized))
            return false;

         double weight = 0.0;
         if(MathAbs(normalized) < 1.0)
           {
...
bool Strategy_LoadBisquareSignal(
   ...
   int &direction)
  {
   ...
   return Strategy_BisquareSignal(observations,
                                  sample_count,
                                  location_value,
                                  direction);
  }
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   // Consume before history, bisquare arithmetic, news, spread, quote,
   ...
   else
     {
      g_signal_valid =
          Strategy_LoadBisquareSignal(
            ...
            g_signal_direction);
      if(!g_signal_valid)
         g_signal_state = "endpoint_sample_or_bisquare_failed";
      else if(g_signal_direction > 0)
         g_signal_state = "fixed_bisquare_long";
      else if(g_signal_direction < 0)
         g_signal_state = "fixed_bisquare_short";
      else
         g_signal_state = "fixed_bisquare_epsilon_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"valid\":%s,\"samples\":%d,\"fixed_bisquare\":%.12g,\"direction\":%d,\"state\":\"%s\"}",
                            g_decision_month_key,
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(!QM_InputRequireLong("qm_ea_id", qm_ea_id, 41231) ||
      ...
      !QM_InputRequireDouble("strategy_bisquare_cutoff", strategy_bisquare_cutoff, 4.685, 1.0e-12) ||
      !QM_InputRequireLong("strategy_bisquare_steps", strategy_bisquare_steps, 32) ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "WTI_SAMECAL_BISQUARE5";
   ...
   if(g_signal_direction > 0)
     {
      req.type = QM_BUY;
      req.reason = "WTI_SAMECAL_BISQUARE5_LONG";
     }
   else
     {
      req.type = QM_SELL;
      req.reason = "WTI_SAMECAL_BISQUARE5_SHORT";
     }
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41231 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI same-calendar five-sample bisquare card holds to the next broker month"))
      return INIT_FAILED;
   ...
   g_attempt_state_key =
      StringFormat("QM5_41231_SAMECAL_BISQUARE5_MONTH_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41231\",\"ea\":\"wti-samecal-bisquare5\"}");
