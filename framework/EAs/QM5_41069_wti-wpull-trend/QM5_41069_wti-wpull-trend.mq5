...
#property description "QM5_41069 WTI Completed-Week Pullback Trend Re-entry"
...
//   - reconstruct three consecutive completed broker-week-end closes
//   - calculate two adjacent, non-overlapping completed-week log returns
//   - require strict sign opposition and a strictly smaller newest countermove
//   - follow the older completed-week sign for one broker week
//   - consume the week before fallible gates; never enter late or retry
//   - close at the next broker week, with a fixed-risk ATR hard stop
// Runtime uses MT5-native OHLC, calendar, history, and framework state only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                      = 41069;
...
double   g_new_return             = 0.0;
double   g_old_return             = 0.0;
double   g_week_end_new          = 0.0;
double   g_week_end_middle       = 0.0;
double   g_week_end_old          = 0.0;
...
bool Strategy_LoadPullbackSignal(const int current_week_key,
                                 const int label_offset,
                                 int &direction,
                                 double &new_return,
                                 double &old_return,
                                 double &week_end_new,
                                 double &week_end_middle,
                                 double &week_end_old)
  {
   direction = 0;
   new_return = 0.0;
   ...
   int found = 0;
   for(int index = 0; index < copied && found < 3; ++index)
     {
   ...
   week_end_new = week_closes[0];
   week_end_middle = week_closes[1];
   week_end_old = week_closes[2];
   new_return = MathLog(week_end_new / week_end_middle);
   old_return = MathLog(week_end_middle / week_end_old);
   if(!MathIsValidNumber(new_return) ||
      !MathIsValidNumber(old_return))
      return false;

   // For positive prices and opposed signs, comparing C1 with C3 is exactly
   // equivalent to comparing the two absolute log returns. It also keeps the
   // card's equal-magnitude case deterministically flat instead of exposing
   // equality to independent MathLog rounding.
   if(old_return > 0.0 && new_return < 0.0 &&
      week_end_new > week_end_old)
      direction = 1;
   else if(old_return < 0.0 && new_return > 0.0 &&
           week_end_new < week_end_old)
      direction = -1;
   return true;
  }
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_WeekAlreadyEntered(g_decision_week_key))
     ...
     {
      g_signal_valid =
          Strategy_LoadPullbackSignal(g_decision_week_key,
      ...
      if(!g_signal_valid)
         g_signal_state = "endpoint_validation_failed";
          ...
          g_signal_state = "positive_trend_smaller_negative_pullback_long";
       else if(g_signal_direction < 0)
          g_signal_state = "negative_trend_smaller_positive_pullback_short";
       else if(g_new_return == 0.0 || g_old_return == 0.0)
          g_signal_state = "exact_zero_flat";
       else if((g_new_return > 0.0 && g_old_return > 0.0) ||
               (g_new_return < 0.0 && g_old_return < 0.0))
          g_signal_state = "same_sign_flat";
       else
          g_signal_state = "equal_or_larger_countermove_flat";
     }

   QM_LogEvent(QM_INFO,
                "STRATEGY_STATE",
               StringFormat("{\"week\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_week_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"old_return\":%.12e,\"new_return\":%.12e,\"c_old\":%.8f,\"c_middle\":%.8f,\"c_new\":%.8f,\"state\":\"%s\"}",
                            g_decision_week_key,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41069 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41069_WTI_WPULL_TREND";
   ...
   req.reason = (g_signal_direction > 0)
       ? "WTI_WPULL_TREND_LONG"
       : "WTI_WPULL_TREND_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_strategy_symbol, true) ||
       !Strategy_IsHostChart() || qm_ea_id != 41069 ||
   ...
   g_attempt_state_key =
       StringFormat("QM5_41069_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41069\",\"ea\":\"wti-wpull-trend\"}");
