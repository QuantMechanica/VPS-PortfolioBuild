...
#property description "QM5_12977 WTI Propane Heating-Season Draw Displacement"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_12977 - WTI Propane Heating-Season Draw Displacement
// -----------------------------------------------------------------------------
// D1 structural WTI sleeve:
//   - long only during the Oct-Mar propane heating-season draw window
//   - requires rising SMA trend plus ATR-normalized upside displacement
//   - exits on draw-window end, trend/channel failure, max hold, Friday close,
//     or ATR hard stop
// Runtime uses MT5 OHLC only; no EIA, propane, weather, API, CSV, or futures feed.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 12977;
...
bool Strategy_InDrawWindow(const datetime t)
  {
   MqlDateTime dt;
   TimeToStruct(t, dt);
...
bool Strategy_LoadClosedState(double &open_last,
                              ...
                              datetime &closed_time)
  {
   ...
   open_last = iOpen(_Symbol, PERIOD_D1, 1);    // perf-allowed: D1 signal body check.
   high_last = iHigh(_Symbol, PERIOD_D1, 1);    // perf-allowed: D1 close-location check.
   low_last = iLow(_Symbol, PERIOD_D1, 1);      // perf-allowed: D1 close-location check.
   close_last = iClose(_Symbol, PERIOD_D1, 1);  // perf-allowed: D1 displacement close.
   close_prev = iClose(_Symbol, PERIOD_D1, 2);  // perf-allowed: D1 close-to-close displacement.
...
void Strategy_CloseOpenPositionsIfNeeded()
  {
   ...
   if(!Strategy_LoadClosedState(open_last, high_last, low_last, close_last,
                                close_prev, exit_low, trend_sma,
                                trend_sma_prior, atr, closed_time))
      return;

   const bool in_window = Strategy_InDrawWindow(closed_time);
   const int magic = QM_FrameworkMagic();
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(strategy_trend_period <= 1 || strategy_sma_slope_shift <= 0)
      return true;
   if(strategy_exit_channel <= 1 || strategy_atr_period <= 0)
      return true;
   if(strategy_min_return_atr <= 0.0 || strategy_min_body_atr <= 0.0)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12977_WTI_PROPANE_DRAW";
   ...
   if(!Strategy_LoadClosedState(open_last, high_last, low_last, close_last,
                                ...
                                trend_sma_prior, atr, closed_time))
      return false;
   if(!Strategy_InDrawWindow(closed_time))
      return false;
   if(close_last <= trend_sma || trend_sma <= trend_sma_prior)
      return false;

   const double return_atr = (close_last - close_prev) / atr;
   const double body_atr = (close_last - open_last) / atr;
   const double range = high_last - low_last;
   if(range <= 0.0)
      return false;
   const double close_location = (close_last - low_last) / range;

   if(return_atr < strategy_min_return_atr)
      return false;
   if(body_atr < strategy_min_body_atr)
      return false;
   ...
   req.reason = "WTI_PROPANE_DRAW_DISPLACEMENT_LONG";
   return true;
  }
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_12977\",\"ea\":\"wti-propane-draw\"}");
