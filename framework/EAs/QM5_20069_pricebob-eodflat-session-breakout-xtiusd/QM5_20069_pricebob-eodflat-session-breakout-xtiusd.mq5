...
#property description "QM5_20069 PriceBob EOD-flat session breakout on XTIUSD"
...
input int    strategy_reference_hour_broker          = 13;
input int    strategy_reference_minute_broker        = 30;
input int    strategy_session_close_hour_broker      = 21;
input int    strategy_session_close_minute_broker    = 0;
...
bool Strategy_NoTradeFilter()
  {
   // The session, range and spread rules gate only new entries. Keeping them
   // in Strategy_EntrySignal lets the forced-flat exit keep running every tick.
   // The central framework news gate remains callable for P8 and entry-only.
   return false;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   // The approved card defines an M15 reference bar and M15 close trigger.
   // This check gates entries only, so an accidental chart mismatch cannot
   // suspend forced session-close handling for an existing position.
   if((ENUM_TIMEFRAMES)_Period != PERIOD_M15)
      return false;

   if(strategy_reference_hour_broker < 0 ||
      strategy_reference_hour_broker > 23 ||
      strategy_reference_minute_broker < 0 ||
      strategy_reference_minute_broker > 59 ||
      strategy_session_close_hour_broker < 0 ||
      strategy_session_close_hour_broker > 23 ||
      strategy_session_close_minute_broker < 0 ||
      strategy_session_close_minute_broker > 59 ||
   ...
   // Hook-local state advances once per closed M15 bar under the framework's
   // single QM_IsNewBar() gate. It never rescans a history window.
   ...
   MqlDateTime closed_bar_dt;
   ZeroMemory(closed_bar_dt);
   TimeToStruct(closed_bar.time, closed_bar_dt);

   // Capture exactly the card-stated 13:30-13:45 broker-time M15 bar after
   // it closes. The ATR gate is evaluated once and latched for this session.
   if(closed_bar_dt.hour == strategy_reference_hour_broker &&
      closed_bar_dt.min == strategy_reference_minute_broker)
     {
      reference_seen = true;
   ...
   const int now_minutes = broker_now.hour * 60 + broker_now.min;
   const int close_minutes =
      strategy_session_close_hour_broker * 60 +
      strategy_session_close_minute_broker;
   ...
   // positive spread wider than 20% of the reference range blocks entry.
   if(ask > bid &&
   ...
   if(closed_bar.close > reference_high)
     {
      ...
      if(sl_price <= 0.0 || sl_price >= entry_price || tp_price <= entry_price)
         return false;
...
bool Strategy_ExitSignal()
  {
   if(strategy_session_close_hour_broker < 0 ||
      strategy_session_close_hour_broker > 23 ||
      strategy_session_close_minute_broker < 0 ||
      strategy_session_close_minute_broker > 59)
      return false;
...
bool Strategy_NewsFilterHook(const datetime broker_time)
  {
   // The card uses the standing high-impact calendar convention. The central
   // framework entry gate supplies it; this hook stays callable for P8.
   return false;
  }
