...
`artifacts/cards_approved/QM5_10197_tv-ssl-wavetrend.md`. See that card's body for
...
`QM5_10197_tv-ssl-wavetrend.mq5`. Framework wiring (risk, magic, news, Friday close)
...
|---|---|---|---|
| `strategy_ssl_period` | 10 | (see source) | (see strategy logic) |
| `strategy_baseline_ema` | 60 | (see source) | (see strategy logic) |
| `strategy_wt_channel_len` | 10 | (see source) | (see strategy logic) |
| `strategy_wt_average_len` | 21 | (see source) | (see strategy logic) |
| `strategy_wt_signal_len` | 4 | (see source) | (see strategy logic) |
| `strategy_keltner_ema` | 20 | (see source) | (see strategy logic) |
...
| `strategy_rr` | 2.0 | (see source) | (see strategy logic) |
| `strategy_max_candle_atr` | 1.5 | (see source) | (see strategy logic) |
| `strategy_ema_sr_period` | 200 | (see source) | (see strategy logic) |
