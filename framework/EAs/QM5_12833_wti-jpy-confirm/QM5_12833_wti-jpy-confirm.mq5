...
#property description "QM5_12833 WTI JPY Oil-Importer Confirmation Trend"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_12833 - WTI JPY Oil-Importer Confirmation Trend
// -----------------------------------------------------------------------------
// D1 structural WTI sleeve:
//   - host/traded symbol: XTIUSD.DWX
//   - read-only confirmation symbol: USDJPY.DWX as Japan oil-importer FX proxy
//   - weekly entry gate
//   - direction requires WTI momentum and same-direction USDJPY movement
// Runtime uses MT5 OHLC/broker calendar only; no macro feed, API, CSV, or ML.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 12833;
...
input string strategy_jpy_proxy_symbol        = "USDJPY.DWX";
input int    strategy_oil_lookback_d1         = 63;
input int    strategy_jpy_lookback_d1         = 63;
input double strategy_min_oil_return_pct      = 3.0;
input double strategy_min_jpy_proxy_return_pct = 1.0;
...
bool Strategy_LoadSignal(int &direction)
  {
   ...
   double jpy_proxy_return = 0.0;
   if(!Strategy_LoadLogReturn(_Symbol, strategy_oil_lookback_d1, oil_return))
      return false;
   if(!Strategy_LoadLogReturn(strategy_jpy_proxy_symbol, strategy_jpy_lookback_d1, jpy_proxy_return))
      return false;
   ...
   const double jpy_threshold = MathMax(0.0, strategy_min_jpy_proxy_return_pct) / 100.0;

   if(oil_return > oil_threshold && jpy_proxy_return > jpy_threshold && oil_close > trend_sma)
      direction = 1;
   else if(oil_return < -oil_threshold && jpy_proxy_return < -jpy_threshold && oil_close < trend_sma)
      direction = -1;
   else
      direction = 0;
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(qm_magic_slot_offset != 0)
      return true;
   if(strategy_jpy_proxy_symbol != "USDJPY.DWX")
      return true;
   if(strategy_oil_lookback_d1 < 10 || strategy_jpy_lookback_d1 < 10)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12833_WTI_JPY_CONFIRM";
   ...
   req.reason = (direction > 0) ? "WTI_JPY_CONFIRM_LONG" : "WTI_JPY_CONFIRM_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(strategy_jpy_proxy_symbol, true))
     {
      PrintFormat("QM5_12833 failed to select confirmation symbol %s", strategy_jpy_proxy_symbol);
      return INIT_FAILED;
     }
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_12833\",\"ea\":\"wti-jpy-confirm\"}");
