"""Deterministic reference checks for QM5_41412 WTI weekly alternation fade."""
...
def alternation_signal(
    ...
    """Mirror three adjacent weeks and fade the newest sign on alternation."""
...
class WtiWeeklyAlternationFadeReferenceTest(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_positive_negative_positive_sells(self) -> None:
        self.assertEqual(
            alternation_signal(self.current, self.bars((3, 4), (4, 3), (2, 3)))[0],
            -1,
        )

    def test_negative_positive_negative_buys(self) -> None:
        self.assertEqual(
            alternation_signal(self.current, self.bars((4, 3), (3, 4), (3, 2)))[0],
            1,
        )

    def test_newest_sign_is_faded_not_followed(self) -> None:
