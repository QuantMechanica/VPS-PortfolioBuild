...
#property description "QM5_10823 TradingView SuperTrend ATR Trailing Stop"
...
input double strategy_initial_stop_atr_mult   = 2.0;
input double strategy_tighten_threshold_pct   = 1.0;
input int    strategy_tighten_mode            = 0;    // 0=percent move, 1=ATR move
input double strategy_tighten_atr_mult        = 1.0;
input int    strategy_position_mode           = 2;    // 0=long only, 1=short only, 2=both
...
bool RefreshSuperTrendCache()
  {
   ...
   MqlRates rates[];
   const int copied = CopyRates(_Symbol, (ENUM_TIMEFRAMES)_Period, 1, lookback, rates); // perf-allowed: bespoke SuperTrend OHLC math runs from Strategy_EntrySignal after framework QM_IsNewBar gate.
...
double StrategyFavorableMoveThreshold()
  {
   if(strategy_tighten_mode == 1)
     {
      const double atr = QM_ATR(_Symbol, (ENUM_TIMEFRAMES)_Period, strategy_atr_period, 1);
      return (atr > 0.0 && strategy_tighten_atr_mult > 0.0) ? atr * strategy_tighten_atr_mult : 0.0;
     }
   const double price = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   return (price > 0.0 && strategy_tighten_threshold_pct > 0.0) ? price * strategy_tighten_threshold_pct / 100.0 : 0.0;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   const bool allow_long = (strategy_position_mode == 0 || strategy_position_mode == 2);
   const bool allow_short = (strategy_position_mode == 1 || strategy_position_mode == 2);
   const bool long_signal = (allow_long && g_st_bull && !g_st_prev_bull);
   const bool short_signal = (allow_short && !g_st_bull && g_st_prev_bull);
   ...
   req.sl = QM_StopATR(_Symbol, req.type, entry_price, strategy_atr_period, strategy_initial_stop_atr_mult);
   if(req.sl <= 0.0)
      return false;
   if(long_signal && req.sl >= entry_price)
      return false;
   if(short_signal && req.sl <= entry_price)
      return false;

   req.tp = 0.0;
   req.reason = long_signal ? "supertrend_flip_bullish" : "supertrend_flip_bearish";
   return true;
  }
...
void Strategy_ManageOpenPosition()
  {
   ...
   const int magic = QM_FrameworkMagic();
   const double threshold = StrategyFavorableMoveThreshold();
   if(threshold <= 0.0)
      return;
      ...
      if(type == POSITION_TYPE_BUY)
        {
         ...
         if(!g_st_bull || (bid > 0.0 && bid <= g_st_line))
            return true;
        }
      else if(type == POSITION_TYPE_SELL)
        {
         const double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
         if(g_st_bull || (ask > 0.0 && ask >= g_st_line))
            return true;
        }
     }
   return false;
