...
#property description "QM5_41028 WTI Month-Boundary Gap Fade"
...
//   - decide only on the first genuine session of each broker month
//   - fade the prior-final-close/current-first-open boundary gap sign
...
input int    strategy_entry_session_ordinal   = 1;
...
double   g_boundary_gap_return     = 0.0;
...
void Strategy_DetectDecisionClock_OnNewBar()
  {
   ...
   if(!QM_ReadBar(_Symbol, PERIOD_D1, 0, current_bar) ||
      !QM_ReadBar(_Symbol, PERIOD_D1, 1, prior_month_bar) ||
      current_bar.time <= 0 || prior_month_bar.time <= 0 ||
      prior_month_bar.time >= current_bar.time)
      return;
   ...
   if(strategy_entry_session_ordinal != 1 ||
      ...
      normalized_before <= 0 ||
      normalized_before >= normalized_current ||
      Strategy_DateKeyForTime(normalized_current) ==
...
bool Strategy_LoadBoundaryGapSignal(const int current_month_key,
                                    const int label_offset,
                                    int &direction,
                                    double &boundary_gap_return)
  {
   direction = 0;
   boundary_gap_return = 0.0;
   ...
   const int before_month_key =
      Strategy_MonthKeyForTime(normalized_before);
   if(normalized_current <= 0 || normalized_before <= 0 ||
      normalized_before >= normalized_current ||
      Strategy_MonthKeyForTime(normalized_current) != current_month_key ||
      before_month_key <= 0 || before_month_key == current_month_key ||
      Strategy_NextMonthKey(before_month_key) != current_month_key ||
      current_bar.open <= 0.0 || prior_month_bar.close <= 0.0 ||
      !MathIsValidNumber(current_bar.open) ||
      !MathIsValidNumber(prior_month_bar.close))
      return false;

   boundary_gap_return =
      MathLog(current_bar.open / prior_month_bar.close);
   if(!MathIsValidNumber(boundary_gap_return))
      return false;

   if(boundary_gap_return < 0.0)
      direction = 1;
   else if(boundary_gap_return > 0.0)
      direction = -1;
   return true;
  }
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
     ...
     {
       g_signal_valid =
          Strategy_LoadBoundaryGapSignal(g_decision_month_key,
                                         g_decision_label_offset,
                                         g_signal_direction,
                                         g_boundary_gap_return);
       if(!g_signal_valid)
          g_signal_state = "endpoint_validation_failed";
       else if(g_signal_direction > 0)
          g_signal_state = "negative_boundary_gap_long";
       else if(g_signal_direction < 0)
          g_signal_state = "positive_boundary_gap_short";
       else if(g_boundary_gap_return == 0.0)
   ...
   QM_LogEvent(QM_INFO,
               "MONTHLY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"boundary_gap_return\":%.12e,\"state\":\"%s\"}",
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41028 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41028_WTI_MGAP_FADE";
...
int OnInit()
  {
   if(!SymbolSelect(g_strategy_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41028 ||
   ...
   g_attempt_state_key =
      StringFormat("QM5_41028_MGAP_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41028\",\"ea\":\"wti-mgap-fade\"}");
