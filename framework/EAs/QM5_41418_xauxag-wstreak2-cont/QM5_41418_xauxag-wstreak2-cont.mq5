...
#property description "QM5_41418 XAU/XAG Fresh Two-Week Streak Continuation"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_41418 - XAU/XAG Fresh Two-Week Streak Continuation
// -----------------------------------------------------------------------------
// Reconstruct four synchronized week-end XAU/XAG close pairs for the four
// immediately preceding completed broker weeks. Form three adjacent relative
// returns and trade only strict -,+,+ or +,-,-: the newest two returns form a
// fresh same-sign streak after an opposite predecessor. Follow the streak with
// an equal-notional opposite-leg package, consume one attempt per week, and
// flatten at the next week boundary.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41418;
...
double   g_relative_return[3]; // chronological: oldest interval to newest.
bool     g_all_relative_strict = false;
double   g_xau_week_close[4]; // newest through oldest completed week.
double   g_xag_week_close[4];
int      g_week_key[4];
int      g_week_sessions[4];
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41418 && qm_magic_slot_offset == 0 &&
            g_leg_xau != "" && g_leg_xag != "" &&
            g_leg_xau != g_leg_xag &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_WSTREAK2_CONT_ATTEMPT_WEEK", qm_ea_id);
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41418_XAUXAG_WSTREAK2_CONT";
...
bool Strategy_LoadFreshTwoWeekStreak(const int current_week,
                                    int &direction)
  {
   direction = 0;
   g_all_relative_strict = false;
   for(int index = 0; index < 3; ++index)
      g_relative_return[index] = 0.0;
   for(int slot = 0; slot < 4; ++slot)
     {
      g_xau_week_close[slot] = 0.0;
      g_xag_week_close[slot] = 0.0;
      g_week_key[slot] = 0;
      ...
      if(slot >= 0 && slot < 4)
        {
         if(g_week_sessions[slot] == 0)
           {
            g_xau_week_close[slot] = xau_bars[index].close;
            g_xag_week_close[slot] = xag_bars[index].close;
           }
   ...
   for(int interval = 0; interval < 3; ++interval)
     {
      const int older_slot = 3 - interval;
      const int newer_slot = older_slot - 1;
      const double xau_return =
         MathLog(g_xau_week_close[newer_slot] /
                 g_xau_week_close[older_slot]);
      const double xag_return =
         MathLog(g_xag_week_close[newer_slot] /
                 g_xag_week_close[older_slot]);
      g_relative_return[interval] = xau_return - xag_return;
      if(!MathIsValidNumber(xau_return) ||
         !MathIsValidNumber(xag_return) ||
         !MathIsValidNumber(g_relative_return[interval]))
         return false;
      if(MathAbs(g_relative_return[interval]) <= strategy_signal_epsilon)
         return true; // A tied interval is valid history but consumes flat.
     }

   g_all_relative_strict = true;
   // Array order is chronological oldest interval through newest interval.
   // The oldest sign must oppose the newest two, making the streak fresh.
   if(g_relative_return[0] < 0.0 &&
      g_relative_return[1] > 0.0 &&
      g_relative_return[2] > 0.0)
      direction = 1; // Fresh positive ratio streak: buy XAU, sell XAG.
   else if(g_relative_return[0] > 0.0 &&
           g_relative_return[1] < 0.0 &&
           g_relative_return[2] < 0.0)
      direction = -1; // Fresh negative ratio streak: sell XAU, buy XAG.
   return true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41418_XAUXAG_WSTREAK2_CONT_HOST";
   ...
   g_all_relative_strict = false;
   for(int interval = 0; interval < 3; ++interval)
      g_relative_return[interval] = 0.0;
   ...
   if(Strategy_OpenOwnedPositionCount() > 0 ||
     ...
     {
      signal_valid = Strategy_LoadFreshTwoWeekStreak(g_signal_week_key,
                                                     direction);
      if(!signal_valid)
         g_signal_state = "endpoint_validation_failed";
      else if(direction > 0)
         g_signal_state = "newest_xau_winner_followed";
      else if(direction < 0)
         g_signal_state = "newest_xag_winner_followed";
      else if(!g_all_relative_strict)
         g_signal_state = "relative_tie_flat";
      else
         g_signal_state = "no_fresh_two_week_streak_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"week\":%d,\"decision_bar\":%I64d,\"late\":%s,\"valid\":%s,\"direction\":%d,\"d0\":%.12e,\"d1\":%.12e,\"d2\":%.12e,\"oldest_week\":%d,\"newest_week\":%d,\"oldest_sessions\":%d,\"newest_sessions\":%d,\"state\":\"%s\"}",
                            g_signal_week_key,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG fresh two-week streak continuation package exits at the next broker week"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41418\",\"ea\":\"xauxag-wstreak2-cont\"}");
