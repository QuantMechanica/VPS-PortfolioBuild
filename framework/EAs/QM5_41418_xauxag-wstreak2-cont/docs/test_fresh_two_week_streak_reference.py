"""Deterministic reference checks for QM5_41418 weekly fresh two-week streak continuation."""
...
MQ5 = EA_DIR / "QM5_41418_xauxag-wstreak2-cont.mq5"
SETFILE = (
    EA_DIR
    / "sets"
    / "QM5_41418_xauxag-wstreak2-cont_QM5_41418_XAU_XAG_WSTREAK2_CONT_D1_D1_backtest.set"
)
...
class FreshTwoWeekStreakReferenceTest(unittest.TestCase):
    def test_negative_positive_positive_follows_xau(self) -> None:
        self.assertEqual(fresh_two_week_streak_signal([-0.03, 0.02, 0.01]), 1)

    def test_positive_negative_negative_follows_xag(self) -> None:
        self.assertEqual(fresh_two_week_streak_signal([0.03, -0.02, -0.01]), -1)
    ...
    def test_static_artifacts_lock_identity_signal_and_fixed_risk(self) -> None:
        ...
        self.assertIn("qm_ea_id                    = 41418", source)
        ...
        self.assertRegex(resolver, re.compile(r"414180000.*414180001", re.DOTALL))
