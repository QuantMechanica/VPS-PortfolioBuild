---
strategy_id: AI-CODEX-FX-COINT66-20260609-EURUSD-EURJPY
ea_id: QM5_20224
slug: eurusd-eurjpy
type: strategy
status: APPROVED
...
markets: [forex]
timeframes: [D1]
primary_target_symbols: [EURUSD.DWX, EURJPY.DWX]
target_symbols: [EURUSD.DWX, EURJPY.DWX]
logical_symbol: QM5_20224_EURUSD_EURJPY_COINTEGRATION_D1
period: D1
...
# QM5_20224 EURUSD/EURJPY D1 Cointegration Basket
...
low-frequency daily implementation. Chan makes no claim for EURUSD/EURJPY.
...
EURUSD/EURJPY ranks forty-sixth of 66 by OOS net Sharpe. Rank 44 is already
represented by `QM5_20223`, and rank 45 by `QM5_12768`. The deterministic
research dedup check returned CLEAN across 4,281 registry rows and 397 cards;
exact-pair searches, registry searches, and an unordered traded-symbol
manifest reconciliation found no dedicated fixed-beta EURUSD/EURJPY D1 card,
EA, allocation, or logical basket.
...
138-bar half-life also implies long holds with material swap and regime risk.
...
`ln(EURUSD) - (-0.236324029) * ln(EURJPY)`. The negative fitted coefficient
...
The package retains USD, EUR, JPY, carry, and broad risk-sentiment exposure.
...
`EURUSD.DWX` is the tester host and first traded leg. `EURJPY.DWX` is the
...
- Compute `spread = ln(EURUSD) - strategy_beta * ln(EURJPY)`, with frozen
  `strategy_beta = -0.236324029`.
...
Chan supplies the pair-trading method but reports no EURUSD/EURJPY result.
...
data_requirements: "EURUSD.DWX, EURJPY.DWX, and USDJPY.DWX D1 histories"
```
...
- 2026-08-05: Rank 46 is OOS-negative and below the expected Q02 cadence
