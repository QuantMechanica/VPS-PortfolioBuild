...
#property description "QM5_20224 EURUSD EURJPY Cointegration"
...
//
// Fixed spread:
//   ln(EURUSD.DWX) - beta * ln(EURJPY.DWX)
//
...
input double strategy_beta              = -0.236324029;
...
bool Strategy_EnsureBasketScope()
  {
   ...
   string allowed[3] = {"EURUSD.DWX", "EURJPY.DWX", "USDJPY.DWX"};
   for(int i = 0; i < 3; ++i)
...
bool Strategy_RefreshSpreadState()
  {
   ...
   for(int i = 0; i < history_count; ++i)
     {
      if(eurusd_time[i] <= 0 || eurusd_time[i] != eurjpy_time[i])
         return false;
      if(eurusd[i] <= 0.0 || eurjpy[i] <= 0.0)
         return false;

      spreads[i] =
         MathLog(eurusd[i]) - strategy_beta * MathLog(eurjpy[i]);
...
bool Strategy_PreparePair(const int spread_direction,
                          QM_EntryRequest &host_req)
  {
   ...
   const double weight_sum = eurusd_weight + eurjpy_weight;
   if(weight_sum <= 0.0)
      return false;
   ...
   const double eurjpy_lots =
      Strategy_LotsForLeg(g_leg_eurjpy, eurjpy_weight, eurusd_weight);
   if(eurusd_lots <= 0.0 || eurjpy_lots <= 0.0)
      return false;
   ...
   const string reason =
      long_spread ? "QM5_20224_LONG_SPREAD_Z_LT_NEG_ENTRY"
                  : "QM5_20224_SHORT_SPREAD_Z_GT_POS_ENTRY";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20224_PAIR_HOST";
