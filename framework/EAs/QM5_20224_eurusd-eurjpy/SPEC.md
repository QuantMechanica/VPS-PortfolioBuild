# QM5_20224_eurusd-eurjpy — Strategy Spec
...
`ln(EURUSD.DWX) - (-0.236324029) * ln(EURJPY.DWX)`. Score the newest spread
...
| `strategy_beta` | -0.236324029 | fixed | DEV-fitted structural hedge coefficient |
...
| Typical hold time | multi-day; scan half-life 137.788 D1 bars |
| Expected drawdown profile | high and regime-sensitive; conservative 30% prior |
| Regime preference | EURUSD/EURJPY fixed-residual mean reversion |
| Win rate target | unknown; Q02 is the first platform-economic judge |

The fixed scan recorded DEV net Sharpe `0.473267`, OOS net Sharpe `-0.118543`,
and OOS return `-1.026394%`. The negative OOS score and slow half-life are
...
`1.0` and `0.236324029`; each leg is independently bounded by its ATR stop.
...
|---|---|---|---|
| v1 | 2026-08-05 | Initial build from approved card | Next-rank-46 non-duplicate fixed-scan FX basket |
