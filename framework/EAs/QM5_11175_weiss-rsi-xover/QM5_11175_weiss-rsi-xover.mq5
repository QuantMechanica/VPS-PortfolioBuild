...
#property description "QM5_11175 Weissman RSI Threshold Crossover"
...
input double strategy_rsi_lower          = 25.0;
input double strategy_rsi_upper          = 75.0;
input double strategy_stop_pct           = 1.0;
input double strategy_take_profit_pct    = 3.0;
...
bool Strategy_NoTradeFilter()
  {
   if(strategy_rsi_period <= 1 ||
      strategy_rsi_lower <= 0.0 ||
      strategy_rsi_upper >= 100.0 ||
      strategy_rsi_lower >= strategy_rsi_upper ||
   ...
   if(rsi_prev > strategy_rsi_upper && rsi_last <= strategy_rsi_upper)
     {
      ...
      req.sl = sl;
      req.tp = tp;
      req.reason = "QM5_11175_RSI_SHORT";
      return true;
     }
...
void Strategy_ManageOpenPosition()
  {
   // Card specifies fixed 1% stop and 3% target only; no trailing or partial close.
  }

// Trade Close
bool Strategy_ExitSignal()
  {
   // Card specifies no discretionary indicator exit; positions close via SL/TP or framework Friday close.
   return false;
  }
