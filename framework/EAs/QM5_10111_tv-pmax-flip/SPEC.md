...
`artifacts/cards_approved/QM5_10111_tv-pmax-flip.md`. See that card's body for
...
`QM5_10111_tv-pmax-flip.mq5`. Framework wiring (risk, magic, news, Friday close)
...
| `strategy_atr_mult` | 3.0 | (see source) | (see strategy logic) |
| `strategy_ma_period` | 10 | (see source) | (see strategy logic) |
| `strategy_filter_atr_period` | 14 | (see source) | (see strategy logic) |
| `strategy_min_stop_atr` | 0.5 | (see source) | (see strategy logic) |
| `strategy_max_stop_atr` | 4.0 | (see source) | (see strategy logic) |
| `strategy_max_spread_frac` | 0.10 | (see source) | (see strategy logic) |
| `strategy_use_protective_tp` | true | (see source) | (see strategy logic) |
| `strategy_tp_atr_mult` | 4.0 | (see source) | (see strategy logic) |
| `strategy_pmax_warmup_bars` | 150 | (see source) | (see strategy logic) |
...
| Cadence note | "H1 trailing-stop reversal signals are less frequent than bar-by-bar momentum; conservative estimate 25-55 trades/year/symbol." |
...
`artifacts/cards_approved/QM5_10111_tv-pmax-flip.md`
