"""Independent deterministic fixtures for QM5_41239.

The suite covers normalized D1 labels, completed WTI month endpoints, exact
Y-5..Y-1 membership, odd median/MAD arithmetic, frozen-scale Soft-L1 updates,
...
def soft_l1_weight(signed_residual: float) -> float | None:
    if not math.isfinite(signed_residual):
        return None
    squared = signed_residual * signed_residual
    if not math.isfinite(squared) or squared < 0.0:
        return None
    radicand = 1.0 + squared
    if not math.isfinite(radicand) or radicand < 1.0:
        return None
    root = math.sqrt(radicand)
    if not math.isfinite(root) or root < 1.0:
        return None
    weight = 1.0 / root
...
def soft_l1_signal(observations: list[float]) -> SoftL1Result | None:
    ...
    location = median
    trace = [location]
    for _ in range(SOFT_L1_ITERATIONS):
        weighted_sum = 0.0
        weight_sum = 0.0
        for value in observations:
            weight = soft_l1_weight((value - location) / scale)
            if weight is None:
                return None
...
class SameCalendarSoftL1ReferenceTests(unittest.TestCase):
    ...
    def test_locked_fixture_median_mad_scale_and_32_updates(self) -> None:
        values = [-0.120, -0.075, -0.020, 0.115, 0.120]
        result = soft_l1_signal(values)
        self.assertIsNotNone(result)
        assert result is not None
        self.assertEqual(result.sorted_values, tuple(values))
        self.assertEqual(result.median, -0.020)
        self.assertAlmostEqual(result.raw_mad, 0.100)
        self.assertAlmostEqual(result.scale, 0.14826)
        self.assertEqual(len(result.trace), SOFT_L1_ITERATIONS + 1)
        self.assertNotEqual(result.trace[1], result.trace[-1])
        self.assertAlmostEqual(result.location, 0.001324252685, places=12)
        self.assertEqual(result.direction, 1)

    def test_soft_l1_weights_are_signed_symmetric_positive_and_decaying(self) -> None:
        self.assertEqual(soft_l1_weight(0.0), 1.0)
        self.assertAlmostEqual(soft_l1_weight(1.0) or 0.0, 1.0 / math.sqrt(2.0))
        self.assertAlmostEqual(soft_l1_weight(-1.0) or 0.0, 1.0 / math.sqrt(2.0))
        self.assertAlmostEqual(soft_l1_weight(2.0) or 0.0, 1.0 / math.sqrt(5.0))
        self.assertGreater(soft_l1_weight(1000.0) or 0.0, 0.0)
        self.assertLess(soft_l1_weight(1000.0) or 1.0, soft_l1_weight(100.0) or 0.0)
        self.assertIsNone(soft_l1_weight(math.nan))
        self.assertIsNone(soft_l1_weight(1.0e308))
    ...
    def test_disagreement_fixture_opposes_named_peers(self) -> None:
        ...
        self.assertAlmostEqual(result.location if result else 0.0,
                               0.001324252685,
                               places=12)
        self.assertAlmostEqual(cauchy, -0.004100768370, places=12)
        self.assertAlmostEqual(arctangent, -0.004348219120, places=12)
        self.assertEqual(median, -0.020)
        self.assertLess(cauchy, 0.0)
        self.assertLess(arctangent, 0.0)
        self.assertLess(median, 0.0)

    def test_second_fixture_opposes_raw_mean(self) -> None:
        values = [-0.120, -0.115, -0.110, 0.155, 0.200]
        result = soft_l1_signal(values)
        self.assertIsNotNone(result)
        assert result is not None
        self.assertAlmostEqual(result.location, -0.100961055448, places=12)
        self.assertEqual(result.direction, -1)
        self.assertAlmostEqual(sum(values) / 5.0, 0.002, places=12)

    def test_sign_reflection_and_permutation_are_exactly_symmetric(self) -> None:
        values = [-0.120, -0.075, -0.020, 0.115, 0.120]
        positive = soft_l1_signal(values)
        reflected = soft_l1_signal([-value for value in values])
        reversed_order = soft_l1_signal(values[::-1])
    ...
    def test_strict_epsilon_and_invalid_states_fail_closed(self) -> None:
        centered = soft_l1_signal([-2.0, -1.0, 0.0, 1.0, 2.0])
        positive = soft_l1_signal(
        ...
        self.assertIsNone(soft_l1_signal([0.01] * 4))
        self.assertIsNone(soft_l1_signal([0.01] * 5))
        self.assertIsNone(soft_l1_signal([0.01, 0.01, math.nan, 0.02, 0.03]))
    ...
    def test_static_build_contract_matches_approved_card(self) -> None:
        source_path = EA_DIR / "QM5_41239_wti-samecal-softl1-5.mq5"
        source = source_path.read_text(encoding="utf-8")
        set_path = (
            EA_DIR
            / "sets"
            / "QM5_41239_wti-samecal-softl1-5_XTIUSD.DWX_D1_backtest.set"
        )
        ...
        self.assertLess(
            ...
            prepare.index("Strategy_LoadSoftL1Signal"),
        )
        ...
        approved = (
            ...
            / "cards"
            / "approved"
            / "QM5_41239_wti-samecal-softl1-5_card.md"
        )
        ...
        self.assertIn(
            "41239,wti-samecal-softl1-5,0,XTIUSD.DWX,412390000",
            magic_rows,
        )
        resolver = (
            REPO_ROOT
        ...
        self.assertIn("412390000", resolver)
