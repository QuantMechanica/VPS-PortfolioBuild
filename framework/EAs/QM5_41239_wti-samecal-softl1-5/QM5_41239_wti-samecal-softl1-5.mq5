...
#property description "QM5_41239 WTI Same-Calendar Five-Sample Soft-L1 Location"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41239 - WTI Same-Calendar Five-Sample Soft-L1-Weighted Location
// -----------------------------------------------------------------------------
...
//   - freeze 1.4826*MAD and execute exactly 32 soft-L1 derivative-weight updates
...
input int    strategy_soft_l1_iterations   = 32;
...
// -----------------------------------------------------------------------------
// Exact completed-month endpoint and five-sample Soft-L1 helpers.
// -----------------------------------------------------------------------------
...
bool Strategy_SoftL1Signal(const double &observations[],
                           ...
                           int &direction)
  {
   ...
   if(strategy_history_years != 5 ||
      ...
      strategy_soft_l1_iterations != 32 ||
   ...
   double mu = median;
   for(int step = 0; step < strategy_soft_l1_iterations; ++step)
     {
      ...
      for(int index = 0; index < 5; ++index)
        {
         ...
         // rho(z)=2*(sqrt(1+z)-1), z=u^2, so rho'(z)=1/sqrt(1+z).
         const double radicand = 1.0 + squared_u;
         if(radicand < 1.0 || !MathIsValidNumber(radicand))
            return false;
         const double root = MathSqrt(radicand);
         if(root < 1.0 || !MathIsValidNumber(root))
            return false;
...
bool Strategy_LoadSoftL1Signal(
   ...
   int &direction)
  {
   ...
   return Strategy_SoftL1Signal(observations,
                                sample_count,
                                location_value,
                                direction);
  }
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   // Consume before history, soft-L1 arithmetic, news, spread, quote,
   ...
   else
     {
      g_signal_valid =
          Strategy_LoadSoftL1Signal(
            ...
            g_signal_direction);
      if(!g_signal_valid)
         g_signal_state = "endpoint_sample_or_soft_l1_failed";
      else if(g_signal_direction > 0)
         g_signal_state = "fixed_soft_l1_long";
      else if(g_signal_direction < 0)
         g_signal_state = "fixed_soft_l1_short";
      else
         g_signal_state = "fixed_soft_l1_epsilon_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"valid\":%s,\"samples\":%d,\"fixed_soft_l1\":%.12g,\"direction\":%d,\"state\":\"%s\"}",
                            g_decision_month_key,
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(!QM_InputRequireLong("qm_ea_id", qm_ea_id, 41239) ||
      ...
      !QM_InputRequireLong("strategy_soft_l1_iterations", strategy_soft_l1_iterations, 32) ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "WTI_SAMECAL_SOFTL1_5";
   ...
   if(g_signal_direction > 0)
     {
      req.type = QM_BUY;
      req.reason = "WTI_SAMECAL_SOFTL1_5_LONG";
     }
   else
     {
      req.type = QM_SELL;
      req.reason = "WTI_SAMECAL_SOFTL1_5_SHORT";
     }
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41239 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI same-calendar five-sample soft-L1-location card holds to the next broker month"))
      return INIT_FAILED;
   ...
   g_attempt_state_key =
      StringFormat("QM5_41239_SAMECAL_SOFTL1_5_MONTH_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41239\",\"ea\":\"wti-samecal-softl1-5\"}");
