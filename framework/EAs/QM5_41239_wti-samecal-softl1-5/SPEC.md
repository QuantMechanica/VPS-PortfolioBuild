# QM5_41239_wti-samecal-softl1-5 - Strategy Spec
...
- source approval:
  `decisions/2026-08-31_wti_same_calendar_soft_l1_5_source_approval.md`
- approved card:
  `strategy-seeds/cards/approved/QM5_41239_wti-samecal-softl1-5_card.md`
- G0 decision:
  `decisions/2026-08-31_qm5_41239_wti_same_calendar_soft_l1_5_g0.md`
- host and traded symbol: exact `XTIUSD.DWX`, D1, slot 0
- deterministic magic: `412390000`
...
`[-0.120,-0.075,-0.020,+0.115,+0.120]` make the locked soft-L1 statistic buy
at approximately `+0.001324252685`, while the otherwise matched Cauchy and
arctangent locations sell at approximately `-0.004100768370` and
`-0.004348219120`; the median is also negative. Sign reflection reverses the
mapping. The square-root-tail weight `1/sqrt(1+u^2)` therefore commits to a
different signal path from Cauchy's `1/(1+u^2)` and arctangent's
`1/(1+u^4)` rather than renaming another same-calendar rule. A second locked
fixture makes soft-L1 sell while raw mean buys.
...
fixes the soft-L1 loss `rho(z)=2*(sqrt(1+z)-1)` and its scale convention. QM
fixes the derivative weight, raw-MAD multiplier, median start, frozen scale,
and exact 32-update convention before testing.
...
| exact sample, median/MAD, fixed-step Soft-L1 | `Strategy_LoadSoftL1Signal` and `Strategy_SoftL1Signal` |
...
exact years, median/MAD indexes, frozen scale, square-root weights, exact update
...
|---|---|---|---|
| v1 | 2026-08-31 | G0-approved WTI same-calendar Soft-L1 build | Q01 pending |
