...
#property description "QM5_20206 XAU XAG Momentum IVol Double Screen"
...
// -----------------------------------------------------------------------------
// Monthly source-specified momentum-IVol precious-metals basket:
//   - align 64 completed D1 closes for XTI, XNG, XAU, and XAG
//   - regress 63 XAU and XAG returns on their equal-weight commodity factor
//   - buy the momentum winner only when it is also the lower-IVol metal
...
double   g_cache_xau_momentum = 0.0;
double   g_cache_xag_momentum = 0.0;
...
string Strategy_AttemptStateName()
  {
   return "QM5_20206_XAUXAG_MOMIVOL_ATTEMPT";
  }
...
bool Strategy_ResidualStd(const double &asset_returns[],
  ...
  {
   residual_std = 0.0;
   if(count != strategy_signal_lookback_d1 || count <= 2 ||
...
bool Strategy_LoadAlignedCloses(double &xti_close[],
  ...
  {
   const int close_count = strategy_signal_lookback_d1 + 1;
...
bool Strategy_LoadSignalState(int &pair_direction)
  {
   ...
   g_cache_xau_momentum = 0.0;
   g_cache_xag_momentum = 0.0;
   ...
   const int lookback = strategy_signal_lookback_d1;
   double xau_returns[];
   double xag_returns[];
   ...
   g_cache_xau_momentum = MathLog(xau_close[0] / xau_close[lookback]);
   g_cache_xag_momentum = MathLog(xag_close[0] / xag_close[lookback]);
   g_cache_momentum_difference =
      g_cache_xau_momentum - g_cache_xag_momentum;
   if(!MathIsValidNumber(g_cache_ivol_difference) ||
      !MathIsValidNumber(g_cache_xau_momentum) ||
      !MathIsValidNumber(g_cache_xag_momentum) ||
      !MathIsValidNumber(g_cache_momentum_difference))
      return false;
   if(g_cache_momentum_difference > 1.0e-12 &&
      g_cache_ivol_difference < -1.0e-12)
      pair_direction = 1;  // XAU momentum winner and lower IVol.
   else if(g_cache_momentum_difference < -1.0e-12 &&
           g_cache_ivol_difference > 1.0e-12)
      pair_direction = -1; // XAG momentum winner and lower IVol.
   return true;
  }
...
bool Strategy_OpenPair(const int pair_direction)
  {
   ...
   const string reason = long_xau_short_xag
                         ? "QM5_20206_LONG_XAU_SHORT_XAG_MOMIVOL"
                         : "QM5_20206_SHORT_XAU_LONG_XAG_MOMIVOL";
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart())
      return true;
   if(qm_ea_id != 20206 || qm_magic_slot_offset != 0 || qm_rng_seed != 42)
      return true;
   ...
   if(qm_friday_close_enabled || qm_friday_close_hour_broker != 21 ||
      ...
      return true;
   if(strategy_signal_lookback_d1 != 63 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20206_XAU_XAG_MOMIVOL_HOST";
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_20206\",\"ea\":\"xauxag-momivol\"}");
