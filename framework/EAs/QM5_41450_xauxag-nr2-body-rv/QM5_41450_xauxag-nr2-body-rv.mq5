...
#property description "QM5_41450 XAU/XAG Weekly NR2 Body Reversion"
...
// Fade that body through an equal-notional opposite-leg package, consume
// one attempt per week, and flatten at the next week boundary.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41450;
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41450 && qm_magic_slot_offset == 0 &&
            g_leg_xau != "" && g_leg_xag != "" &&
            g_leg_xau != g_leg_xag &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_NR2_BODY_RV_ATTEMPT_WEEK", qm_ea_id);
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41450_XAUXAG_NR2_BODY_RV";
...
bool Strategy_LoadNr2Body(const int current_week,
                         int &direction)
  {
   ...
   if(g_newest_week_body > strategy_body_epsilon)
      direction = -1; // Positive ratio body: sell XAU, buy XAG.
   else if(g_newest_week_body < -strategy_body_epsilon)
      direction = 1;  // Negative ratio body: buy XAU, sell XAG.
   return true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41450_XAUXAG_NR2_BODY_RV_HOST";
   ...
   if(Strategy_OpenOwnedPositionCount() > 0 ||
   ...
   else
     {
      ...
      if(!signal_valid)
         ...
         g_signal_state = "negative_ratio_body_faded";
      else if(direction < 0)
         g_signal_state = "positive_ratio_body_faded";
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG weekly ratio range-contraction body reversion package exits at the next broker week"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41450\",\"ea\":\"xauxag-nr2-body-rv\"}");
