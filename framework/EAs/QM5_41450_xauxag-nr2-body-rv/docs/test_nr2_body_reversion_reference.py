"""Deterministic reference checks for QM5_41450 XAU/XAG weekly NR2 body reversion."""
...
MQ5 = EA_DIR / "QM5_41450_xauxag-nr2-body-rv.mq5"
SETFILE = (
    EA_DIR
    / "sets"
    / "QM5_41450_xauxag-nr2-body-rv_QM5_41450_XAU_XAG_NR2_BODY_RV_D1_D1_backtest.set"
)
...
def nr2_body_reversion_signal(prior_range: float, newest_range: float, body: float) -> int:
...
class Nr2BodyReversionReferenceTest(unittest.TestCase):
    def test_ratio_range_and_both_reversion_sides(self) -> None:
        prior_width, _ = range_and_body([4.55, 4.64, 4.53, 4.61])
        ...
        self.assertEqual(
            nr2_body_reversion_signal(prior_width, positive_width, positive_body), -1
        )
        self.assertEqual(
            nr2_body_reversion_signal(prior_width, negative_width, negative_body), 1
        )
    ...
    def test_range_ties_expansion_body_ties_and_invalid_are_flat(self) -> None:
        for prior, newest, body in (
            ...
            with self.subTest(prior=prior, newest=newest, body=body):
                self.assertEqual(nr2_body_reversion_signal(prior, newest, body), 0)
    ...
    def test_static_artifacts_lock_signal_risk_and_unpinned_framework(self) -> None:
        ...
        self.assertIn("qm_ea_id                    = 41450", source)
        ...
        self.assertRegex(resolver, re.compile(r"414500000.*414500001", re.DOTALL))
