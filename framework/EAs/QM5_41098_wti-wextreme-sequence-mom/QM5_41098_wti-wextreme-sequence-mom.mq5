...
#property description "QM5_41098 WTI Weekly Extreme-Sequence Momentum"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41098 - WTI Weekly Extreme-Sequence Momentum
// -----------------------------------------------------------------------------
// D1 structural crude-oil sleeve:
//   - aggregate the immediately completed three-to-five-session broker week
//   - require one unique session for each aggregate weekly extreme
//   - require extreme chronology and the final settlement direction to agree
//   - consume the week before fallible gates; never enter late or retry
//   - hold the completed extreme-sequence signal for one broker week
...
int      g_high_series_index      = -1;
int      g_low_series_index       = -1;
int      g_extreme_sequence       = 0;
bool     g_extremes_unique        = false;
bool     g_settlement_agrees      = false;
...
void Strategy_ResetDecisionState()
  {
   ...
   g_low_occurrences = 0;
   g_high_series_index = -1;
   g_low_series_index = -1;
   g_extreme_sequence = 0;
   g_extremes_unique = false;
   g_settlement_agrees = false;
   g_signal_state = "idle";
  }
...
bool Strategy_LoadExtremeSequenceSignal(const int current_week_key,
                                        const int label_offset,
                                        int &direction,
                                        int &week_bars,
                                        double &week_open,
                                        ...
                                        int &low_occurrences,
                                        int &high_series_index,
                                        int &low_series_index,
                                        int &extreme_sequence,
                                        bool &extremes_unique,
                                        bool &settlement_agrees)
  {
   direction = 0;
   week_bars = 0;
   week_open = 0.0;
   ...
   low_series_index = -1;
   extreme_sequence = 0;
   extremes_unique = false;
   settlement_agrees = false;
   if(current_week_key <= 0 ||
   ...
   for(int index = 0; index < copied; ++index)
     {
      ...
      if(week_bars == 0)
        ...
        {
         if(bars[index].high > week_high)
           {
            week_high = bars[index].high;
            high_occurrences = 1;
            high_series_index = week_bars;
           }
         else if(bars[index].high == week_high)
            ++high_occurrences;

         if(bars[index].low < week_low)
           {
            week_low = bars[index].low;
            low_occurrences = 1;
            low_series_index = week_bars;
           }
         else if(bars[index].low == week_low)
            ++low_occurrences;
        }
   ...
   // CopyRates uses newest-to-oldest series order. A larger series index is
   // therefore earlier in chronological time.
   if(low_series_index > high_series_index)
      extreme_sequence = 1;   // unique low occurred before unique high
   else if(high_series_index > low_series_index)
      extreme_sequence = -1;  // unique high occurred before unique low

   settlement_agrees =
      ((extreme_sequence > 0 && week_close > week_open) ||
       (extreme_sequence < 0 && week_close < week_open));
   if(settlement_agrees)
      direction = extreme_sequence;
   return true;
  }
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_WeekAlreadyEntered(g_decision_week_key))
   ...
   else
     {
      ...
      if(!g_signal_valid)
         ...
         g_signal_state = "unique_low_before_high_settlement_long";
       else if(g_signal_direction < 0)
          g_signal_state = "unique_high_before_low_settlement_short";
       else if(!g_extremes_unique)
          g_signal_state = "repeated_extreme_ambiguous_flat";
       else if(g_week_close == g_week_open)
          g_signal_state = "settlement_equality_flat";
       else if(g_extreme_sequence == 0)
          g_signal_state = "same_session_extremes_ambiguous_flat";
       else if(!g_settlement_agrees)
          g_signal_state = "extreme_sequence_settlement_disagreement_flat";
       else
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
                StringFormat("{\"week\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_week_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"week_bars\":%d,\"week_open\":%.8f,\"week_high\":%.8f,\"week_low\":%.8f,\"week_close\":%.8f,\"high_occurrences\":%d,\"low_occurrences\":%d,\"high_series_index\":%d,\"low_series_index\":%d,\"extreme_sequence\":%d,\"extremes_unique\":%s,\"settlement_agrees\":%s,\"state\":\"%s\"}",
                            g_decision_week_key,
                            ...
                            g_signal_direction,
                             ...
                             g_low_series_index,
                             g_extreme_sequence,
                             g_extremes_unique ? "true" : "false",
                             g_settlement_agrees ? "true" : "false",
                            g_signal_state));
  }
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41098 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41098_WTI_WEXTREME_SEQUENCE_MOM";
   ...
   req.reason = (g_signal_direction > 0)
       ? "WTI_WEXTREME_SEQUENCE_LONG"
       : "WTI_WEXTREME_SEQUENCE_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
       !Strategy_IsHostChart() || qm_ea_id != 41098 ||
   ...
   g_attempt_state_key =
      StringFormat("QM5_41098_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
                "{\"card\":\"QM5_41098\",\"ea\":\"wti-wextreme-sequence-mom\"}");
