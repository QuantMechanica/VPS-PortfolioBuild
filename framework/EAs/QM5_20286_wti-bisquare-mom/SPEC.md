# QM5_20286_wti-bisquare-mom — Strategy Spec

**EA ID:** QM5_20286
**Slug:** `wti-bisquare-mom`
**Source:** `MOP-WTI-BISQUARE-2026` (see
`strategy-seeds/sources/MOP-WTI-BISQUARE-2026/`)
...
freeze `cutoff = 4.685 * 1.4826 * MAD`, and run exactly 32 redescending
bisquare reweighted-mean updates from the median. Observations at or beyond the
cutoff receive zero influence. Buy for a positive final location and sell for
a negative one. Exact zero or invalid state consumes the month flat. Every
...
| `strategy_bisquare_cutoff` | `4.685` | `[4.685]` | Normalized redescending cutoff |
| `strategy_mad_normalizer` | `1.4826` | `[1.4826]` | Raw-MAD scale normalization |
| `strategy_bisquare_steps` | `32` | `[32]` | Exact re-centering updates |
...
| Regime preference | Persistent broad crude-oil direction after redescending tail rejection |
| Win rate target | Low to medium; expectancy must come from slow trend packages |

The WTI carrier and redescending statistic are diversification hypotheses
only. Q09 owns any realized portfolio-overlap conclusion.

## 6. Source Citation

**Source ID:** `MOP-WTI-BISQUARE-2026`
**Source type:** peer-reviewed trading paper with bounded QM statistical
mechanization
**Pointer:** `strategy-seeds/sources/MOP-WTI-BISQUARE-2026/source.md`
**R1–R4 verdict (G0):** all PASS; see
`strategy-seeds/cards/approved/QM5_20286_wti-bisquare-mom_card.md`
...
family and includes WTI. The bisquare estimator is a locked QM hypothesis; the
...
mu[0] = m
u[i,j] = (r[i] - mu[j]) / cutoff
w[i,j] = (1-u[i,j]^2)^2 if abs(u[i,j]) < 1 else 0
mu[j+1] = sum(w[i,j]*r[i]) / sum(w[i,j]), j=0..31
```

The scale and cutoff freeze before iteration. All 32 updates execute. Buy for
`mu[32] > 0`, sell for `mu[32] < 0`, and consume exact zero, nonpositive
weight sum, or invalid state flat.

## 9. Non-Duplicate Boundary

`QM5_20285` performs fixed-step Huber reweighting and retains positive tail
influence. This EA uses a redescending score with exact zero influence at and
beyond the cutoff. `QM5_20282`, `QM5_20283`, `QM5_20277`, `QM5_20270`, and
`QM5_20276` use one-shot cap, quartile trimean, Winsor, trim, and pseudomedian
functionals. The strict support boundary, squared weights, zero-tail
influence, frozen cutoff, and update count are jointly load-bearing.
...
orientation, wrong median/MAD/scale, mutable cutoff, wrong support or weights,
nonzero tail influence, other-than-32 updates, zero-weight fallback,
wrong-side entry, repeated attempt, missing hard stop, hold beyond forty days,
...
row for `QM5_20286 / XTIUSD.DWX`. The binding path-anchored capacity sample
found five exact T1-T10 tester processes against the ceiling of seven. The
bounded apply created work item `db894631-d726-4ff2-98c3-ef8ab043d0ff` for
Q02. Immediate readback found attempt 0 pending and unclaimed with no verdict.
No dispatch tick, terminal launch, or manual backtest was issued by this
mission. This is a handoff, not a Q02 result.
