...
#property description "QM5_20286 WTI Redescending Bisquare Return Trend"
...
//   2. form twelve adjacent chronological monthly log returns;
//   3. estimate an even-sample median and raw MAD, then freeze a cutoff;
//   4. run exactly 32 redescending bisquare location updates;
//   5. trade the sign of the final zero-tail-influence location.
// The robust statistic and fixed cutoff are transparent QM mechanizations.
// Runtime uses native XTIUSD.DWX price and broker data only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 20286;
...
input double strategy_bisquare_cutoff      = 4.685;
input double strategy_mad_normalizer      = 1.4826;
input int    strategy_bisquare_steps       = 32;
...
double g_cache_median           = 0.0;
double g_cache_mad              = 0.0;
double g_cache_cutoff            = 0.0;
double g_cache_bisquare          = 0.0;
...
void Strategy_ResetCachedState()
  {
   ...
   g_cache_mad = 0.0;
   g_cache_cutoff = 0.0;
   g_cache_bisquare = 0.0;
   g_cache_state_reason = "not_evaluated";
  }

bool Strategy_BisquareSignal(const double &closes[], int &signal)
  {
   signal = 0;
   const int return_count = strategy_return_months;
   if(return_count != 12 || ArraySize(closes) != return_count + 1)
     {
      g_cache_state_reason = "bad_bisquare_contract";
      return false;
     }
   ...
   if(!MathIsValidNumber(median))
     {
      g_cache_state_reason = "invalid_median";
      return false;
     }
   ...
   for(int i = 0; i < return_count; ++i)
     {
      sorted_deviations[i] = MathAbs(monthly_returns[i] - median);
      if(!MathIsValidNumber(sorted_deviations[i]))
        {
         g_cache_state_reason = "invalid_deviation";
         return false;
        }
     }
   ArraySort(sorted_deviations);
   for(int i = 1; i < return_count; ++i)
      if(sorted_deviations[i] < sorted_deviations[i - 1])
        {
   ...
   const double scale = strategy_mad_normalizer * mad;
   const double cutoff = strategy_bisquare_cutoff * scale;
   if(mad <= 0.0 || scale <= 0.0 || cutoff <= 0.0 ||
      !MathIsValidNumber(mad) || !MathIsValidNumber(scale) ||
      !MathIsValidNumber(cutoff))
     {
      g_cache_state_reason = "nonpositive_or_invalid_bisquare_scale";
      return false;
     }

   double location = median;
   for(int step = 0; step < strategy_bisquare_steps; ++step)
     {
      ...
      for(int i = 0; i < return_count; ++i)
        {
         const double normalized =
            (monthly_returns[i] - location) / cutoff;
         const double absolute_normalized = MathAbs(normalized);
         if(!MathIsValidNumber(normalized) ||
            !MathIsValidNumber(absolute_normalized))
           {
            g_cache_state_reason = "invalid_bisquare_normalized_residual";
            return false;
           }

         double weight = 0.0;
         if(absolute_normalized < 1.0)
           {
            const double core = 1.0 - normalized * normalized;
            weight = core * core;
           }
         if(weight < 0.0 || !MathIsValidNumber(weight))
           {
            g_cache_state_reason = "invalid_bisquare_weight";
            return false;
           }
         weighted_sum += weight * monthly_returns[i];
         total_weight += weight;
        }

      if(total_weight <= 0.0 || !MathIsValidNumber(total_weight) ||
         !MathIsValidNumber(weighted_sum))
        {
         g_cache_state_reason = "invalid_bisquare_weight_sum";
         return false;
        }
      location = weighted_sum / total_weight;
      if(!MathIsValidNumber(location))
        {
         g_cache_state_reason = "invalid_bisquare_location";
         return false;
        }
     }

   g_cache_median = median;
   g_cache_mad = mad;
   g_cache_cutoff = cutoff;
   g_cache_bisquare = location;
   if(location == 0.0)
     {
      g_cache_state_reason = "exact_zero_bisquare";
      return true;
     }

   signal = (location > 0.0) ? 1 : -1;
...
bool Strategy_LoadSignalState(int &signal)
  {
   ...
   return Strategy_BisquareSignal(monthly_closes, signal);
  }
...
void Strategy_PrepareMonthlySignal()
  {
   ...
   QM_LogEvent(QM_INFO,
               "MONTHLY_STATE",
               StringFormat("{\"month\":%d,\"valid\":%s,\"signal\":%d,\"median\":%.10f,\"mad\":%.10f,\"cutoff\":%.10f,\"bisquare\":%.10f,\"state\":\"%s\"}",
                            g_cache_month_key,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 20286 ||
   ...
   if(strategy_return_months != 12 ||
      MathAbs(strategy_bisquare_cutoff - 4.685) > 1.0e-12 ||
      MathAbs(strategy_mad_normalizer - 1.4826) > 1.0e-12 ||
      strategy_bisquare_steps != 32)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20286_WTI_BISQUARE_MOM";
   ...
   req.reason = (g_cache_signal > 0) ? "BISQUARE_MOM_XTI_LONG" : "BISQUARE_MOM_XTI_SHORT";
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_20286_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_20286\",\"ea\":\"wti-bisquare-mom\"}");
