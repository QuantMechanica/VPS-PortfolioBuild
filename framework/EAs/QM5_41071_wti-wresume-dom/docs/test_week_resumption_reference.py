"""Deterministic reference checks for QM5_41071 WTI weekly resumption."""
...
def resumption_signal(
    current_week_key: int,
    completed_newest_first: list[Bar],
    offset: timedelta,
) -> tuple[bool, int, tuple[float, float, float], tuple[float, float, float, float]]:
    """Mirror four week ends and the strict resume/counter/resume rule."""
    ...
    resume = math.log(closes[0] / closes[1])
    counter = math.log(closes[1] / closes[2])
    oldest = math.log(closes[2] / closes[3])
    direction = 0
    if oldest > 0.0 and counter < 0.0 and resume > 0.0 and abs(resume) > abs(counter):
        direction = 1
    elif oldest < 0.0 and counter > 0.0 and resume < 0.0 and abs(resume) > abs(counter):
        direction = -1
    return True, direction, (oldest, counter, resume), tuple(closes)
...
def sample(
    ...
    now = current
    bars = [
        Bar(datetime(2026, 8, 21, tzinfo=UTC), c1),
        Bar(datetime(2026, 8, 20, tzinfo=UTC), c1 - 0.5),
        Bar(datetime(2026, 8, 14, tzinfo=UTC), c2),
        Bar(datetime(2026, 8, 13, tzinfo=UTC), c2 + 0.5),
        Bar(datetime(2026, 8, 7, tzinfo=UTC), c3),
        Bar(datetime(2026, 7, 31, tzinfo=UTC), c4),
    ]
...
class WeekResumptionReferenceTest(unittest.TestCase):
    def test_positive_resumption_is_long(self) -> None:
        current, now, bars = sample(125.0, 114.0, 120.0, 100.0)
        decision, late, count, offset = decision_clock(current, now, bars)
        self.assertTrue(decision)
        self.assertFalse(late)
        self.assertEqual(count, 0)
        assert offset is not None
        valid, direction, returns, endpoints = resumption_signal(20260824, bars, offset)
        oldest, counter, resume = returns
        self.assertTrue(valid)
        self.assertEqual(direction, 1)
        self.assertGreater(oldest, 0.0)
        self.assertLess(counter, 0.0)
        self.assertGreater(resume, 0.0)
        self.assertGreater(abs(resume), abs(counter))
        self.assertEqual(endpoints, (125.0, 114.0, 120.0, 100.0))

    def test_negative_resumption_is_short(self) -> None:
        current, now, bars = sample(95.0, 106.0, 100.0, 120.0)
        offset = label_offset(current, now)
        assert offset is not None
        valid, direction, returns, _ = resumption_signal(20260824, bars, offset)
        oldest, counter, resume = returns
        self.assertTrue(valid)
        self.assertEqual(direction, -1)
        self.assertLess(oldest, 0.0)
        self.assertGreater(counter, 0.0)
        self.assertLess(resume, 0.0)
        self.assertGreater(abs(resume), abs(counter))

    def test_wrong_path_zero_equal_and_nondominant_states_are_flat(self) -> None:
        cases = [
            sample(105.0, 114.0, 120.0, 100.0),
            sample(132.0, 120.0, 110.0, 100.0),
            sample(100.0, 100.0, 110.0, 90.0),
            sample(math.e, 1.0, math.e, 1.0),
            sample(115.0, 110.0, 120.0, 100.0),
        ]
        for current, now, bars in cases:
            offset = label_offset(current, now)
            assert offset is not None
            valid, direction, _, _ = resumption_signal(20260824, bars, offset)
            self.assertTrue(valid)
            self.assertEqual(direction, 0)

    def test_uniform_prior_date_label_normalization_and_grace(self) -> None:
        current, now, bars = sample(125.0, 114.0, 120.0, 100.0, True)
        decision, late, count, offset = decision_clock(current, now, bars)
        self.assertTrue(decision)
        self.assertFalse(late)
        self.assertEqual(count, 0)
        ...
        assert offset is not None
        self.assertEqual(resumption_signal(20260824, bars, offset)[1], 1)
    ...
    def test_nonconsecutive_or_current_week_endpoint_rejected(self) -> None:
        ...
        offset = label_offset(current, now)
        assert offset is not None
        bars[-1] = Bar(datetime(2026, 7, 24, tzinfo=UTC), 100.0)
        self.assertFalse(resumption_signal(20260824, bars, offset)[0])
        _, _, bars = sample(125.0, 114.0, 120.0, 100.0)
        bars.insert(0, Bar(datetime(2026, 8, 24, tzinfo=UTC), 111.0))
        self.assertFalse(resumption_signal(20260824, bars, offset)[0])

    def test_bad_endpoint_reverse_order_and_short_history_rejected(self) -> None:
        current, now, bars = sample(125.0, 114.0, 120.0, 100.0)
        offset = label_offset(current, now)
        assert offset is not None
        bars[4] = Bar(bars[4].opened, 0.0)
        self.assertFalse(resumption_signal(20260824, bars, offset)[0])
        _, _, bars = sample(125.0, 114.0, 120.0, 100.0)
        bars[1] = Bar(bars[0].opened + timedelta(days=1), bars[1].close)
        self.assertFalse(resumption_signal(20260824, bars, offset)[0])
        _, _, bars = sample(125.0, 114.0, 120.0, 100.0)
        self.assertFalse(resumption_signal(20260824, bars[:-1], offset)[0])
