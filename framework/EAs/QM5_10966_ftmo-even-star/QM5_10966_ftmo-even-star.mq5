...
input int    strategy_resistance_lookback_bars = 60;
input int    strategy_entry_window_bars       = 3;
input int    strategy_max_hold_bars           = 20;
input int    strategy_slope_negative_max_bars = 10;
...
input double strategy_resistance_atr_mult     = 0.35;
...
double CandleHigh3(const MqlRates &a, const MqlRates &b, const MqlRates &c)
  {
   return MathMax(a.high, MathMax(b.high, c.high));
  }

bool LoadStrategyBars(MqlRates &rates[])
  {
   const int resistance_bars_needed = strategy_resistance_lookback_bars + strategy_entry_window_bars + 8;
   const int sma_bars_needed = strategy_sma_slow_period + strategy_entry_window_bars + 12;
   const int bars_needed = (resistance_bars_needed > sma_bars_needed) ? resistance_bars_needed : sma_bars_needed;
...
int CountHigherSwingProgression(const MqlRates &rates[],
...
double HighestPriorSwingHigh(const MqlRates &rates[], const int start_shift, const int lookback_bars)
  {
...
double NearestPriorSwingLowBelow(const MqlRates &rates[],
                                 const int start_shift,
                                 const int lookback_bars,
                                 const double entry)
  {
   ...
   for(int shift = start_shift + 2; shift <= last_shift - 2; ++shift)
     {
      if(!IsSwingLow(rates, shift))
         continue;
      if(rates[shift].low < entry && rates[shift].low > nearest)
         nearest = rates[shift].low;
     }
   return (nearest > -DBL_MAX) ? nearest : 0.0;
  }

bool PatternNearRoundNumber(const double pattern_high)
  {
   const double point = SymbolInfoDouble(_Symbol, SYMBOL_POINT);
   if(point <= 0.0 || strategy_round_step_points <= 0 || pattern_high <= 0.0)
      return false;
   ...
   const double nearest = MathRound(pattern_high / step) * step;
   const double tolerance = pattern_high * (strategy_round_tolerance_pct / 100.0);
   return (MathAbs(pattern_high - nearest) <= tolerance);
  }

bool SmaSlopeAlreadyNegativeTooLong(const int start_shift)
  {
   int negative_count = 0;
   for(int shift = start_shift; shift < start_shift + strategy_slope_negative_max_bars + 1; ++shift)
     {
   ...
   return (negative_count > strategy_slope_negative_max_bars);
  }

bool IsEveningStarSetup(const MqlRates &rates[], const int bars_after_c3, double &pattern_high, double &candle3_low)
  {
   ...
   if(CountHigherSwingProgression(rates, c3, strategy_swing_lookback_bars, true) < 2)
      return false;
   if(CountHigherSwingProgression(rates, c3, strategy_swing_lookback_bars, false) < 2)
      return false;

   if(SmaSlopeAlreadyNegativeTooLong(c3))
      return false;
   ...
   if(c2_body > strategy_candle2_body_ratio * c1_body)
      return false;
   if(rates[c2].high < rates[c1].high || rates[c2].high < rates[c3].high)
      return false;
   ...
   pattern_high = CandleHigh3(rates[c1], rates[c2], rates[c3]);
   candle3_low = rates[c3].low;

   const double resistance_high = HighestPriorSwingHigh(rates, c3 + 1, strategy_resistance_lookback_bars);
   const bool near_swing_resistance = (resistance_high > 0.0 && MathAbs(pattern_high - resistance_high) <= strategy_resistance_atr_mult * atr);
   return (near_swing_resistance || PatternNearRoundNumber(pattern_high));
  }
   ...
   for(int bars_after_c3 = 1; bars_after_c3 <= strategy_entry_window_bars; ++bars_after_c3)
     {
      ...
      double candle3_low = 0.0;
      if(!IsEveningStarSetup(rates, bars_after_c3, pattern_high, candle3_low))
         continue;

      if(rates[1].low >= candle3_low || rates[1].close >= candle3_low)
         continue;

      const double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
      const double point = SymbolInfoDouble(_Symbol, SYMBOL_POINT);
      const double atr = QM_ATR(_Symbol, PERIOD_H4, strategy_atr_period, 1);
      if(bid <= 0.0 || point <= 0.0 || atr <= 0.0)
         return false;
      ...
      if(swing_tp > 0.0)
        {
         const double swing_rr = (bid - swing_tp) / stop_distance;
      ...
      req.reason = "ftmo_evening_star_h4_breakout";
      req.symbol_slot = qm_magic_slot_offset;
      req.expiration_seconds = 0;
      return true;
     }
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      if(initial_risk <= 0.0 || ask > open_price - initial_risk)
         continue;
