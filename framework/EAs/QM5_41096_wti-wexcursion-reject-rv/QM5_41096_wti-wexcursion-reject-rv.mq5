...
#property description "QM5_41096 WTI Weekly Excursion-Rejection Reversal"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41096 - WTI Weekly Excursion-Rejection Reversal
// -----------------------------------------------------------------------------
// D1 structural crude-oil sleeve:
//   - aggregate the immediately completed broker-week OHLC package
//   - compare high-open with open-low using a strict two-to-one inequality
//   - require the final close to reject the dominant excursion direction
//   - consume the week before fallible gates; never enter late or retry
//   - fade the completed failed auction for one broker week
// Runtime uses MT5-native OHLC, calendar, history, and framework state only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                      = 41096;
...
double   g_up_excursion           = 0.0;
double   g_down_excursion         = 0.0;
bool     g_excursion_imbalanced   = false;
bool     g_settlement_rejects     = false;
...
void Strategy_ResetDecisionState()
  {
   ...
   g_week_close = 0.0;
   g_up_excursion = 0.0;
   g_down_excursion = 0.0;
   g_excursion_imbalanced = false;
   g_settlement_rejects = false;
   g_signal_state = "idle";
  }
...
bool Strategy_LoadExcursionImbalanceSignal(const int current_week_key,
                                           const int label_offset,
                                           int &direction,
                                           int &week_bars,
                                           double &week_open,
                                           ...
                                           double &week_close,
                                           double &up_excursion,
                                           double &down_excursion,
                                           bool &excursion_imbalanced,
                                           bool &settlement_rejects)
  {
   direction = 0;
   week_bars = 0;
   week_open = 0.0;
   ...
   up_excursion = 0.0;
   down_excursion = 0.0;
   excursion_imbalanced = false;
   settlement_rejects = false;
   if(current_week_key <= 0 ||
   ...
   if(up_excursion < 0.0 || down_excursion < 0.0 ||
      !MathIsValidNumber(up_excursion) ||
      !MathIsValidNumber(down_excursion))
      return false;

   const double scaled_down =
      strategy_excursion_multiplier * down_excursion;
   const double scaled_up =
      strategy_excursion_multiplier * up_excursion;
   if(!MathIsValidNumber(scaled_down) ||
      !MathIsValidNumber(scaled_up))
      return false;

   const bool bullish_excursion = (up_excursion > scaled_down);
   const bool bearish_excursion = (down_excursion > scaled_up);
   excursion_imbalanced = (bullish_excursion || bearish_excursion);
   settlement_rejects =
      ((bullish_excursion && week_close < week_open) ||
       (bearish_excursion && week_close > week_open));
   if(bullish_excursion && week_close < week_open)
      direction = -1;
   else if(bearish_excursion && week_close > week_open)
      direction = 1;
   return true;
  }
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_WeekAlreadyEntered(g_decision_week_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadExcursionImbalanceSignal(g_decision_week_key,
                                                ...
                                                g_up_excursion,
                                                g_down_excursion,
                                                g_excursion_imbalanced,
                                                g_settlement_rejects);
      if(!g_signal_valid)
         g_signal_state = "weekly_ohlc_validation_failed";
      else if(g_signal_direction > 0)
         g_signal_state = "down_excursion_dominant_rejected_long";
       else if(g_signal_direction < 0)
          g_signal_state = "up_excursion_dominant_rejected_short";
       else if(g_week_close == g_week_open)
          g_signal_state = "settlement_equality_flat";
       else if(!g_excursion_imbalanced)
          g_signal_state = "excursion_threshold_not_strictly_exceeded_flat";
       else if(!g_settlement_rejects)
          g_signal_state = "excursion_settlement_agreement_flat";
       else
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
                StringFormat("{\"week\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_week_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"week_bars\":%d,\"week_open\":%.8f,\"week_high\":%.8f,\"week_low\":%.8f,\"week_close\":%.8f,\"up_excursion\":%.8f,\"down_excursion\":%.8f,\"excursion_imbalanced\":%s,\"settlement_rejects\":%s,\"state\":\"%s\"}",
                            g_decision_week_key,
                            ...
                            g_signal_direction,
                             ...
                             g_up_excursion,
                             g_down_excursion,
                             g_excursion_imbalanced ? "true" : "false",
                             g_settlement_rejects ? "true" : "false",
                            g_signal_state));
  }
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41096 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41096_WTI_WEXCURSION_REJECT_RV";
   ...
   req.reason = (g_signal_direction > 0)
       ? "WTI_WEXCURSION_REJECT_LONG"
       : "WTI_WEXCURSION_REJECT_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
       !Strategy_IsHostChart() || qm_ea_id != 41096 ||
   ...
   g_attempt_state_key =
      StringFormat("QM5_41096_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
                "{\"card\":\"QM5_41096\",\"ea\":\"wti-wexcursion-reject-rv\"}");
