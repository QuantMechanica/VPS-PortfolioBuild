# QM5_20197_eurjpy-eurgbp — Strategy Spec
...
`ln(EURJPY.DWX) - (-0.679904414) * ln(EURGBP.DWX)`. Score the newest spread
...
| `strategy_beta` | -0.679904414 | fixed | DEV-fitted structural hedge coefficient |
...
| Typical hold time | multi-day; scan half-life 83.425 D1 bars |
| Expected drawdown profile | high and regime-sensitive; conservative 30% prior |
| Regime preference | EURJPY/EURGBP fixed-residual mean reversion |
| Win rate target | unknown; Q02 is the first platform-economic judge |

The fixed scan recorded DEV net Sharpe `0.717918`, OOS net Sharpe `0.646491`,
and OOS return `8.050213%`. The sub-0.8 OOS score and slow half-life are
...
`artifacts/cards_approved/QM5_20197_eurjpy-eurgbp_card.md`

Chan supplies the deterministic pair-trading method but makes no performance
claim for EURJPY/EURGBP.
...
`1.0` and `0.679904414`; each leg is independently bounded by its ATR stop.
