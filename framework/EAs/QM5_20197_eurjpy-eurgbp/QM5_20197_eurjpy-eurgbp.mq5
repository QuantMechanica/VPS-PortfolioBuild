...
#property description "QM5_20197 EURJPY EURGBP Cointegration"
...
//
// Card:
//   strategy-seeds/cards/approved/QM5_20197_eurjpy-eurgbp_card.md
//
// Fixed spread:
//   ln(EURJPY.DWX) - beta * ln(EURGBP.DWX)
//
...
input double strategy_beta              = -0.679904414;
...
bool Strategy_EnsureBasketScope()
  {
   ...
   string allowed[5] = {"EURJPY.DWX", "EURGBP.DWX", "USDJPY.DWX", "GBPUSD.DWX", "EURUSD.DWX"};
   for(int i = 0; i < 5; ++i)
...
bool Strategy_RefreshSpreadState()
  {
   ...
   for(int i = 0; i < history_count; ++i)
     {
      if(eurjpy_time[i] <= 0 || eurjpy_time[i] != eurgbp_time[i])
         return false;
      if(eurjpy[i] <= 0.0 || eurgbp[i] <= 0.0)
         return false;

      spreads[i] =
         MathLog(eurjpy[i]) - strategy_beta * MathLog(eurgbp[i]);
...
bool Strategy_OpenPair(const int spread_direction)
  {
   ...
   const double weight_sum = eurjpy_weight + eurgbp_weight;
   if(weight_sum <= 0.0)
      return false;
   ...
   if(eurjpy_lots <= 0.0 || eurgbp_lots <= 0.0)
      return false;
   ...
   const string reason =
      long_spread ? "QM5_20197_LONG_SPREAD_Z_LT_NEG_ENTRY"
                  : "QM5_20197_SHORT_SPREAD_Z_GT_POS_ENTRY";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20197_PAIR_HOST";
