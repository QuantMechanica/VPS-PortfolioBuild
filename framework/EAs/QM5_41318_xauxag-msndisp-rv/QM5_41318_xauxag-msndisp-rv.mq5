...
#property description "QM5_41318 XAU/XAG Monthly Sn-Core Reversion"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_41318 - XAU/XAG Completed-Month Sn-Core Dispersion Reversion
// -----------------------------------------------------------------------------
// At the first synchronized executable D1 boundary of a broker month, take
// the final seventeen exactly timestamp-matched XAU/XAG closes from the
// immediately completed month. Form sixteen gold-minus-silver log-ratio
// changes and their raw Rousseeuw-Croux Sn core: the eighth one-based value
// from each fifteen-distance leave-one-out array, followed by the eighth
// one-based value of those sixteen inner values. Fade an inclusive three-core
// net displacement with an opposed, equal-target-notional package. Runtime is
// deterministic, completed-price-only, and one consumed attempt per month.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41318;
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41318 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
            ...
            MathAbs(strategy_sn_core_floor - 1.0e-12) <= 1.0e-18 &&
            MathAbs(strategy_net_core_multiplier - 3.0) <= 1.0e-12 &&
            MathAbs(strategy_endpoint_tolerance - 1.0e-10) <= 1.0e-14 &&
...
bool Strategy_ForeignExposureExists()
  {
   for(int index = PositionsTotal() - 1; index >= 0; --index)
     {
      const ulong ticket = PositionGetTicket(index);
      ...
      if(symbol != g_leg_xau && symbol != g_leg_xag)
         continue;
      if(!Strategy_IsOwnedMagic(PositionGetInteger(POSITION_MAGIC)))
         return true;
     }
   ...
   for(int index = PositionsTotal() - 1; index >= 0; --index)
     {
      ...
      if(stop <= 0.0 || volume <= 0.0 || opened <= 0.0 || opened_at <= 0 ||
         ...
         !MathIsValidNumber(volume) || !MathIsValidNumber(opened) ||
         target != 0.0 ||
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_SN_RV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41318_XAUXAG_SN_RV";
...
bool Strategy_LoadMonthlySnSignal(
      const int current_month_key,
      int &session_count,
      datetime &newest_pair_time,
      int &return_count,
      ...
      int &direction)
  {
   ...
   MqlRates xau_bars[];
   MqlRates xag_bars[];
   double newest_first_ratios[23];
   datetime newest_first_times[23];
   ...
   const int xau_copied =
      CopyRates(g_leg_xau, // perf-allowed: one bounded completed-month scan behind a consumed monthly attempt or restart validation.
                PERIOD_D1, 1, strategy_history_bars_d1, xau_bars);
   const int xag_copied =
      CopyRates(g_leg_xag, // perf-allowed: one bounded completed-month scan behind a consumed monthly attempt or restart validation.
   ...
   // Restart validation can occur after completed current-month sessions.
   // Skip those observations independently; they never enter the signal.
   while(xau_index < xau_copied &&
         Strategy_MonthKey(xau_bars[xau_index].time) == current_month_key)
      ++xau_index;
   while(xag_index < xag_copied &&
         Strategy_MonthKey(xag_bars[xag_index].time) == current_month_key)
      ++xag_index;
   if(xau_index >= xau_copied || xag_index >= xag_copied ||
      Strategy_MonthKey(xau_bars[xau_index].time) != prior_month_key ||
      Strategy_MonthKey(xag_bars[xag_index].time) != prior_month_key)
      return false;

   // Every immediately completed-month session must be paired exactly.
   // A missing or shifted timestamp is a setup mismatch, never an invitation
   // to silently shorten the formation sample.
   while(xau_index < xau_copied && xag_index < xag_copied)
     {
      const int xau_month = Strategy_MonthKey(xau_bars[xau_index].time);
      const int xag_month = Strategy_MonthKey(xag_bars[xag_index].time);
      const bool xau_in_prior = (xau_month == prior_month_key);
      const bool xag_in_prior = (xag_month == prior_month_key);
      if(!xau_in_prior && !xag_in_prior)
         break;
      if(!xau_in_prior || !xag_in_prior ||
         xau_bars[xau_index].time != xag_bars[xag_index].time ||
         xau_bars[xau_index].time >= g_current_host_bar ||
         session_count < 0 ||
      ...
      if(!Strategy_LogRatio(xau_bars[xau_index].close,
                            ...
                            ratio))
         return false;
      newest_first_ratios[session_count] = ratio;
      newest_first_times[session_count] = xau_bars[xau_index].time;
      if(session_count == 0)
         newest_pair_time = xau_bars[xau_index].time;
      ++session_count;
      ++xau_index;
      ++xag_index;
     }
   ...
   if(xau_index >= xau_copied || xag_index >= xag_copied ||
      Strategy_MonthKey(xau_bars[xau_index].time) != older_month_key ||
      Strategy_MonthKey(xag_bars[xag_index].time) != older_month_key ||
   ...
   // The arrays are newest-first. Reverse exactly the final seventeen pairs.
   ...
   if(return_count != strategy_return_count)
      return false;

   endpoint_return =
      chronological_ratios[strategy_return_count] -
      chronological_ratios[0];
   endpoint_error = MathAbs(net_return - endpoint_return);
   ...
   double distances[15];
   for(int subject = 0; subject < strategy_return_count; ++subject)
     {
      int local_count = 0;
      for(int peer = 0; peer < strategy_return_count; ++peer)
        {
         if(peer == subject)
            continue;
         if(local_count < 0 || local_count >= strategy_inner_distance_count)
            return false;
   ...
   if(directed_distance_count != 240 ||
      inner_count != strategy_outer_count)
      return false;
   ArraySort(inner_medians);
   for(int index = 0; index < strategy_outer_count; ++index)
      if(inner_medians[index] < 0.0 ||
         !MathIsValidNumber(inner_medians[index]) ||
         (index > 0 && inner_medians[index] < inner_medians[index - 1]))
         return false;

   // Raw Sn core only: deliberately omit both the 1.1926 consistency
   // multiplier and every finite-sample correction.
   sn_core = inner_medians[strategy_outer_lomed_one_based - 1];
   if(!MathIsValidNumber(sn_core))
      return false;
   if(sn_core <= strategy_sn_core_floor)
      return true; // Valid month, consumed flat at the locked core floor.

   threshold = strategy_net_core_multiplier * sn_core;
   if(threshold <= 0.0 || !MathIsValidNumber(threshold))
      return false;

   // direction > 0 means BUY XAU / SELL XAG. Fade the relative displacement.
   // Both three-core comparisons are deliberately inclusive.
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41318_XAUXAG_SN_RV_HOST";
   ...
   int session_count = 0;
   int return_count = 0;
   int directed_distance_count = 0;
   int inner_count = 0;
   int direction = 0;
   datetime newest_pair_time = 0;
   double net_return = 0.0;
   double endpoint_return = 0.0;
   ...
   string state = "monthly_sn_signal_failed";
   if(valid && direction > 0)
      state = "three_sn_negative_net_fade_long_xau";
   else if(valid && direction < 0)
      state = "three_sn_positive_net_fade_short_xau";
   else if(valid && sn_core <= strategy_sn_core_floor)
      state = "sn_core_floor_flat";
   else if(valid)
      state = "three_sn_core_boundary_flat";

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"completed_month\":%d,\"session_count\":%d,\"newest_pair_time\":%I64d,\"return_count\":%d,\"directed_distance_count\":%d,\"inner_count\":%d,\"inner_index_zero_based\":%d,\"outer_index_zero_based\":%d,\"net_return\":%.12e,\"endpoint_return\":%.12e,\"endpoint_error\":%.12e,\"sn_core\":%.12e,\"threshold\":%.12e,\"direction\":%d,\"ratios\":\"%s\",\"returns\":\"%s\",\"state\":\"%s\"}",
                            g_signal_month_key,
                            Strategy_PreviousMonthKey(g_signal_month_key),
                            session_count,
                            (long)newest_pair_time,
                            return_count,
                            directed_distance_count,
                            inner_count,
                            strategy_inner_median_one_based - 1,
                            strategy_outer_lomed_one_based - 1,
                            net_return,
                            endpoint_return,
                            endpoint_error,
                            sn_core,
...
bool Strategy_RefreshExpectedDirection()
  {
   ...
   if(!valid ||
      session_count < strategy_month_sessions_min ||
      session_count > strategy_month_sessions_max ||
      return_count != strategy_return_count ||
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG completed-month Sn-core reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41318\",\"ea\":\"xauxag-msndisp-rv\"}");
