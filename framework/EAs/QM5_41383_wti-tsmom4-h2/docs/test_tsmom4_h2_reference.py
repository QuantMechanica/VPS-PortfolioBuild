#!/usr/bin/env python3
"""Independent vectors for QM5_41383's locked statistic and clock."""
...
def consecutive(month_keys: list[int]) -> bool:
    return len(month_keys) == 5 and all(
        next_month(month_keys[i]) == month_keys[i + 1] for i in range(4)
    )
...
def signal(closes: list[float]) -> tuple[int | None, float | None]:
    ...
    chained_return = sum(math.log(closes[i + 1] / closes[i]) for i in range(4))
...
def main() -> None:
    months = [202610, 202611, 202612, 202701, 202702]
    assert consecutive(months)
    assert not consecutive([202610, 202611, 202701, 202702, 202703])

    rising = [100.0, 101.0, 99.0, 103.0, 110.0]
    falling = [100.0, 99.0, 101.0, 96.0, 90.0]
    flat = [100.0, 105.0, 95.0, 106.0, 100.0]
    ...
    irregular = [100.0, 92.0, 108.0, 91.0, 104.0]
    direction, endpoint = signal(irregular)
    assert direction == 1 and endpoint is not None
    assert abs(endpoint - sum(math.log(irregular[i + 1] / irregular[i]) for i in range(4))) <= 1.0e-12
