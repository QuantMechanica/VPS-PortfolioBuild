...
#property description "QM5_41404 WTI Winter Two-Week Agreement Momentum"
...
//   - follow only strict same-sign weekly return agreement
...
bool Strategy_LoadTwoWeekAgreementSignal(const int current_week_key,
                                         const int label_offset,
                                         int &direction)
  {
   direction = 0;
   ...
   // Both adjacent completed weeks must agree strictly. Trade their common
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_WeekAlreadyEntered(g_decision_week_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadTwoWeekAgreementSignal(g_decision_week_key,
                                             g_decision_label_offset,
                                             g_signal_direction);
      if(!g_signal_valid)
         g_signal_state = "two_week_validation_failed";
      else if(g_signal_direction > 0)
         g_signal_state = "two_positive_weeks_continuation_long";
      else if(g_signal_direction < 0)
         g_signal_state = "two_negative_weeks_continuation_short";
      else
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41404_WTI_WINTER_W2AGREE";
   ...
   req.reason = (g_signal_direction > 0)
      ? "WTI_WINTER_W2_AGREE_LONG"
      : "WTI_WINTER_W2_AGREE_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(strategy_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41404 ||
      qm_magic_slot_offset != 0)
   ...
   g_attempt_state_key =
      StringFormat("QM5_41404_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41404\",\"ea\":\"wti-winter-w2agree\"}");
