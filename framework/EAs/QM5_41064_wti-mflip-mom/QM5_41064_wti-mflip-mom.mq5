...
#property description "QM5_41064 WTI Fresh Monthly Return-Sign Handoff Momentum"
...
//   - reconstruct three consecutive completed broker-month-end closes
//   - calculate two adjacent, non-overlapping completed-month log returns
//   - trade only a strict sign handoff, following the newest monthly sign
//   - consume the month before fallible gates; never enter late or retry
//   - close at the next broker month, with a fixed-risk ATR hard stop
// Runtime uses MT5-native OHLC, calendar, history, and framework state only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                      = 41064;
...
double   g_month_end_new          = 0.0;
double   g_month_end_middle       = 0.0;
double   g_month_end_old          = 0.0;
...
bool Strategy_LoadSignHandoffSignal(const int current_month_key,
                                    const int label_offset,
                                    int &direction,
                                    double &new_return,
                                    double &old_return,
                                    double &month_end_new,
                                    double &month_end_middle,
                                    double &month_end_old)
  {
   direction = 0;
   new_return = 0.0;
   old_return = 0.0;
   month_end_new = 0.0;
   month_end_middle = 0.0;
   month_end_old = 0.0;
   if(current_month_key <= 0 ||
   ...
   int month_keys[3] = {0, 0, 0};
   double month_closes[3] = {0.0, 0.0, 0.0};
   int found = 0;
   for(int index = 0; index < copied && found < 3; ++index)
     {
      ...
      if(found > 0 && month_key == month_keys[found - 1])
         continue;
      ...
      else if(Strategy_NextMonthKey(month_key) != month_keys[found - 1])
         return false;
   ...
   month_end_new = month_closes[0];
   month_end_middle = month_closes[1];
   month_end_old = month_closes[2];
   new_return = MathLog(month_end_new / month_end_middle);
   old_return = MathLog(month_end_middle / month_end_old);
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadSignHandoffSignal(g_decision_month_key,
      ...
      if(!g_signal_valid)
         ...
         g_signal_state = "negative_to_positive_long";
      else if(g_signal_direction < 0)
         g_signal_state = "positive_to_negative_short";
   ...
   QM_LogEvent(QM_INFO,
               "MONTHLY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"old_return\":%.12e,\"new_return\":%.12e,\"c_old\":%.8f,\"c_middle\":%.8f,\"c_new\":%.8f,\"state\":\"%s\"}",
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41064 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41064_WTI_MFLIP_MOM";
...
int OnInit()
  {
   if(!SymbolSelect(g_strategy_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41064 ||
   ...
   g_attempt_state_key =
      StringFormat("QM5_41064_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41064\",\"ea\":\"wti-mflip-mom\"}");
