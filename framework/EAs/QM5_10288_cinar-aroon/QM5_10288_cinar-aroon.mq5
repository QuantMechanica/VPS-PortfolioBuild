...
#property description "QM5_10288 Cinar Aroon Direction Flip"
...
bool Strategy_NoTradeFilter()
  {
   // Card specifies no time, spread, or additional regime filter.
   return false;
  }
...
int Strategy_CalculateAroonDirection()
  {
   ...
   const int low_shift = iLowest(_Symbol,
                                 ...
                                 strategy_aroon_period,
                                 strategy_signal_shift);
   if(high_shift < strategy_signal_shift || low_shift < strategy_signal_shift)
      return 0;

   const int periods_since_high = high_shift - strategy_signal_shift;
   const int periods_since_low = low_shift - strategy_signal_shift;
   const double aroon_up = ((double)(strategy_aroon_period - periods_since_high) /
                            (double)strategy_aroon_period) * 100.0;
   const double aroon_down = ((double)(strategy_aroon_period - periods_since_low) /
                              (double)strategy_aroon_period) * 100.0;
...
bool Strategy_HasPosition(const int direction)
  {
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      if(direction > 0 && ptype == POSITION_TYPE_BUY)
         return true;
      if(direction < 0 && ptype == POSITION_TYPE_SELL)
         return true;
     }
   return false;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.reason = (direction > 0) ? "CINAR_AROON_LONG" : "CINAR_AROON_SHORT";
   return true;
  }
