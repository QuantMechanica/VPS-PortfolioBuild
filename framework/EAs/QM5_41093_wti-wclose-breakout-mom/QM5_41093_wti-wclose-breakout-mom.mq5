...
#property description "QM5_41093 WTI Weekly Closing Breakout Momentum"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41093 - WTI Weekly Closing Breakout Momentum
// -----------------------------------------------------------------------------
// D1 structural crude-oil sleeve:
//   - aggregate two consecutive completed broker-week OHLC packages
//   - compare only the newest completed week's final close with the parent range
//   - buy above the parent high, sell below the parent low, otherwise stay flat
//   - consume the week before fallible gates; never enter late or retry
//   - hold the completed closing-breakout signal for one broker week
// Runtime uses MT5-native OHLC, calendar, history, and framework state only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                      = 41093;
...
bool Strategy_LoadClosingBreakoutSignal(const int current_week_key,
                                        const int label_offset,
                                        int &direction,
                                        int &new_week_bars,
   ...
   for(int index = 0; index < copied; ++index)
     {
      ...
      // Series order is newest-to-oldest. The first bar in a bucket supplies
      // the final close; overwriting open on every bar leaves the first open.
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_WeekAlreadyEntered(g_decision_week_key))
     ...
     {
      g_signal_valid =
          Strategy_LoadClosingBreakoutSignal(g_decision_week_key,
      ...
      if(!g_signal_valid)
         g_signal_state = "weekly_ohlc_validation_failed";
          ...
          g_signal_state = "strict_parent_high_close_breakout_long";
      else if(g_signal_direction < 0)
          g_signal_state = "strict_parent_low_close_breakout_short";
       else if(!g_closing_breakout)
          g_signal_state = "parent_range_equality_or_inside_flat";
       else
          g_signal_state = "closing_breakout_direction_invalid";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
                StringFormat("{\"week\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_week_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"new_week_bars\":%d,\"parent_week_bars\":%d,\"new_open\":%.8f,\"new_high\":%.8f,\"new_low\":%.8f,\"new_close\":%.8f,\"parent_open\":%.8f,\"parent_high\":%.8f,\"parent_low\":%.8f,\"parent_close\":%.8f,\"closing_breakout\":%s,\"state\":\"%s\"}",
                            g_decision_week_key,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41093 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41093_WTI_WCLOSE_BREAKOUT_MOM";
   ...
   req.reason = (g_signal_direction > 0)
       ? "WTI_WCLOSE_BREAKOUT_MOM_LONG"
       : "WTI_WCLOSE_BREAKOUT_MOM_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
       !Strategy_IsHostChart() || qm_ea_id != 41093 ||
   ...
   g_attempt_state_key =
       StringFormat("QM5_41093_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
                "{\"card\":\"QM5_41093\",\"ea\":\"wti-wclose-breakout-mom\"}");
