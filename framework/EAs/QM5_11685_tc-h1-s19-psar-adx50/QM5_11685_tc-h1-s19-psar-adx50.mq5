...
#property description "QM5_11685 tc-h1-s19-psar-adx50 — PSAR(0.02,0.2) flip + ADX(50) directional (H1)"

#include <QM/QM_Common.mqh>

// =============================================================================
// QuantMechanica V5 EA — QM5_11685 tc-h1-s19-psar-adx50
// -----------------------------------------------------------------------------
// Source: Thomas Carter, "20 Forex Trading Strategies Collection (1 Hour Time
//         Frame)", Strategy #19, self-published 2014.
// Card: artifacts/cards_approved/QM5_11685_tc-h1-s19-psar-adx50.md
//       (g0_status APPROVED). source_id 6b5ab225-a2d3-54b1-ac8b-2b000a205468.
//
...
//   Stop           : fixed sl_pips (50p default — card "50 pips").
//   Take profit    : fixed tp_pips (50p default, 1:1) via QM_TakeRR.
//   Defensive exit : card "Exit = close when PSAR flips to the opposite side OR
//                    when ADX directional lines cross". Both are implemented as
//                    single-event closed-bar checks in Strategy_ExitSignal.
...
// Defensive exit per card: close when the PSAR flips to the opposite side OR
// when the ADX +DI/-DI directional lines cross against the open position.
// Each is a single event (shift 2 -> shift 1), so the exit fires once.
bool Strategy_ExitSignal()
  {
   ...
   if(sar_now > 0.0 && sar_prev > 0.0)
     {
      ...
      if(low1 > 0.0 && high1 > 0.0 && low2 > 0.0 && high2 > 0.0)
        {
         // SAR flips ABOVE price (bullish->bearish dot) closes a long.
         const bool flip_to_above = (sar_prev < low2  && sar_now > high1);
         // SAR flips BELOW price (bearish->bullish dot) closes a short.
         const bool flip_to_below = (sar_prev > high2 && sar_now < low1);
   ...
   // --- DI-cross exit (card: "OR when ADX directional lines cross") ---
