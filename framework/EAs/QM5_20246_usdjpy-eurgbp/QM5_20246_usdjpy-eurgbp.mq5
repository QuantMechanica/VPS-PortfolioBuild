...
#property description "QM5_20246 USDJPY EURGBP Cointegration"
...
//
// Fixed spread:
//   ln(USDJPY.DWX) - beta * ln(EURGBP.DWX)
//
...
input double strategy_beta              = -1.281773609960;
...
bool Strategy_EnsureBasketScope()
  {
   ...
   string allowed[4] = {"USDJPY.DWX", "EURGBP.DWX", "GBPUSD.DWX", "EURUSD.DWX"};
   for(int i = 0; i < 4; ++i)
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(CopyClose(g_leg_usdjpy, PERIOD_D1, 1, history_count, usdjpy) != // perf-allowed: called only by the gated spread refresh.
      history_count)
      return false;
   if(CopyClose(g_leg_eurgbp, PERIOD_D1, 1, history_count, eurgbp) != // perf-allowed: called only by the gated spread refresh.
      history_count)
      return false;
   if(CopyTime(g_leg_usdjpy, PERIOD_D1, 1, history_count, usdjpy_time) != // perf-allowed: called only by the gated spread refresh.
      history_count)
      return false;
   if(CopyTime(g_leg_eurgbp, PERIOD_D1, 1, history_count, eurgbp_time) != // perf-allowed: called only by the gated spread refresh.
      history_count)
      return false;
   ...
   for(int i = 0; i < history_count; ++i)
     {
      if(usdjpy_time[i] <= 0 || usdjpy_time[i] != eurgbp_time[i])
         return false;
      if(usdjpy[i] <= 0.0 || eurgbp[i] <= 0.0)
         return false;

      spreads[i] =
         MathLog(usdjpy[i]) - strategy_beta * MathLog(eurgbp[i]);
...
bool Strategy_PreparePair(const int spread_direction,
                          QM_EntryRequest &host_req)
  {
   ...
   const double weight_sum = usdjpy_weight + eurgbp_weight;
   if(weight_sum <= 0.0)
      return false;

   // Preflight both normalized legs before sending either order. A fitted
   // leg weight that falls below broker minimum volume rejects the complete
   // package instead of leaving a directional orphan.
   ...
   const double eurgbp_lots =
      Strategy_LotsForLeg(g_leg_eurgbp, eurgbp_weight, usdjpy_weight);
   if(usdjpy_lots <= 0.0 || eurgbp_lots <= 0.0)
      return false;

   const bool long_spread = (spread_direction > 0);
   const QM_OrderType usdjpy_type = long_spread ? QM_BUY : QM_SELL;
   const QM_OrderType eurgbp_type =
      long_spread
      ? (strategy_beta >= 0.0 ? QM_SELL : QM_BUY)
      : (strategy_beta >= 0.0 ? QM_BUY : QM_SELL);
   const string reason =
      long_spread ? "QM5_20246_LONG_SPREAD_Z_LT_NEG_ENTRY"
                  : "QM5_20246_SHORT_SPREAD_Z_GT_POS_ENTRY";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20246_PAIR_HOST";
   ...
   if(qm_news_temporal != QM_NEWS_TEMPORAL_OFF ||
   ...
   else
     {
      if(!QM_NewsAllowsTrade(g_leg_usdjpy,
                             ...
                             qm_news_mode_legacy))
         return true;
      if(!QM_NewsAllowsTrade(g_leg_eurgbp,
                             broker_time,
                             qm_news_mode_legacy))
         return true;
     }
