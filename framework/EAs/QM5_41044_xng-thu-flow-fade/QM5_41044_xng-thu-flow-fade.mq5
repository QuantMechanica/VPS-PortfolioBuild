...
#property description "QM5_41044 XNG Standard-Thursday Session-Dominant Flow Fade"
...
//   - split Thursday into close-to-open and open-to-close log-return streams
//   - enter Friday only when both nonzero streams strictly oppose and the
//     session stream strictly dominates, fading the completed Thursday move
//   - consume one exact-Friday attempt before every fallible entry gate
//   - flatten at the first later D1 boundary; Friday close remains disabled
...
bool Strategy_LoadThursdayFlowFade(const datetime current_bar_time,
                                   const datetime broker_now,
                                   ...
                                   int &direction)
  {
   ...
   // Only the authorized completed Thursday open/close and Wednesday close
   // are price endpoints. Tuesday proves exact sequence identity only.
   if(bars[0].open <= 0.0 ||
   ...
   // The card requires strict nonzero opposition and strict session
   // dominance. Agreement, exact zero, and equal/overnight dominance are flat.
   const bool signs_oppose =
      ((overnight_flow > 0.0 && session_flow < 0.0) ||
       (overnight_flow < 0.0 && session_flow > 0.0));
   if(!signs_oppose ||
      MathAbs(session_flow) <= MathAbs(overnight_flow))
      return true;

   // Fade the completed Thursday displacement; magnitude never changes risk.
   if(total_flow > 0.0)
      direction = -1;
   ...
   for(int index = PositionsTotal() - 1; index >= 0; --index)
     {
      ...
      // later than the entry date (normally Monday open).
      if(!should_close &&
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   int direction = 0;
   if(!Strategy_LoadThursdayFlowFade(g_strategy_d1_bar_time,
                                     broker_now,
                                     overnight_flow,
                                     session_flow,
                                     day_return,
   ...
   if(direction > 0)
     {
      req.type = QM_BUY;
      req.reason = "XNG_THU_FLOW_FADE_LONG";
     }
   else if(direction < 0)
     {
      req.type = QM_SELL;
      req.reason = "XNG_THU_FLOW_FADE_SHORT";
     }
   else
      return false;
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XNG Thursday flow-fade card holds through Friday to the next D1 boundary"))
      return INIT_FAILED;
   ...
   g_attempt_state_key =
      StringFormat("QM5_41044_THU_FLOW_FADE_FRIDAY_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41044\",\"ea\":\"xng-thu-flow-fade\"}");
