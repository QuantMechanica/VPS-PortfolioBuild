"""Independent mechanic fixtures for QM5_41044.
...
contrarian direction, Friday grace/attempt identity, and next-D1 exit.
...
def select_direction(
    ...
    if not signs_oppose or abs(session_flow) <= abs(overnight_flow):
        return 0
    # Fade the completed Thursday total.
...
class ThursdayFlowFadeReferenceTests(unittest.TestCase):
    ...
    def test_session_dominant_positive_day_is_sold(self) -> None:
        direction, overnight, session, day_return, total = (
            thursday_direction(100.0, 99.0, 102.0)
        )
