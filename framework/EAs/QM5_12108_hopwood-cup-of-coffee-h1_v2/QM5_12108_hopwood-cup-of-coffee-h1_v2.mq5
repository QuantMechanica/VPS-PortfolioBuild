...
#property description "QM5_12108_hopwood-cup-of-coffee-h1_v2 — v2 rebuild: fixed qm_ea_id 12108→12108"
...
{
   const int handle = iStochastic(_Symbol, PERIOD_H1, stoch_k, stoch_d, stoch_slowing, MODE_SMA, STO_CLOSECLOSE);
   if(handle == INVALID_HANDLE) { out_k = 50.0; out_d = 50.0; return; }
   double k_buf[], d_buf[];
   ArraySetAsSeries(k_buf, true);
   ArraySetAsSeries(d_buf, true);
   const int ck = CopyBuffer(handle, 0, shift + 1, 2, k_buf);
   const int cd = CopyBuffer(handle, 1, shift + 1, 2, d_buf);
   IndicatorRelease(handle);
   out_k = (ck >= 2) ? k_buf[0] : 50.0;
   out_d = (cd >= 2) ? d_buf[0] : 50.0;
}
