"""Deterministic reference checks for QM5_41416 relative-vote flip."""
...
MQ5 = EA_DIR / "QM5_41416_xauxag-wrelvote-flip-fade.mq5"
SETFILE = (
    EA_DIR
    / "sets"
    / "QM5_41416_xauxag-wrelvote-flip-fade_QM5_41416_XAU_XAG_WRELVOTE_FLIP_FADE_D1_D1_backtest.set"
)
...
class RelativeVoteFlipReferenceTest(unittest.TestCase):
    def test_negative_to_positive_majority_fades_xau(self) -> None:
        self.assertEqual(vote_flip_signal([-0.03, 0.02, -0.01, 0.04]), 1)

    def test_positive_to_negative_majority_fades_xag(self) -> None:
        self.assertEqual(vote_flip_signal([0.03, -0.02, 0.01, -0.04]), -1)
    ...
    def test_static_artifacts_lock_identity_signal_and_fixed_risk(self) -> None:
        ...
        self.assertIn("qm_ea_id                    = 41416", source)
        ...
        self.assertRegex(resolver, re.compile(r"414160000.*414160001", re.DOTALL))
