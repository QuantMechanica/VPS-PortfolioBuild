...
#property description "QM5_41416 XAU/XAG Weekly Relative-Vote Flip Reversion"
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41416 && qm_magic_slot_offset == 0 &&
            g_leg_xau != "" && g_leg_xag != "" &&
            g_leg_xau != g_leg_xag &&
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41416_XAUXAG_WRELVOTE_FLIP_FADE";
...
bool Strategy_LoadRelativeVoteFlip(const int current_week,
                                   int &direction)
  {
   ...
   if(g_old_vote < 0 && g_new_vote > 0)
      direction = -1; // New XAU majority: sell XAU, buy XAG.
   else if(g_old_vote > 0 && g_new_vote < 0)
      direction = 1; // New XAG majority: buy XAU, sell XAG.
   return true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41416_XAUXAG_WRELVOTE_FLIP_FADE_HOST";
   ...
   if(Strategy_OpenOwnedPositionCount() > 0 ||
   ...
   else
     {
      ...
      if(!signal_valid)
         ...
         g_signal_state = "new_xau_majority_faded";
      else if(direction < 0)
         g_signal_state = "new_xag_majority_faded";
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG weekly relative-vote flip reversion package exits at the next broker week"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41416\",\"ea\":\"xauxag-wrelvote-flip-fade\"}");
