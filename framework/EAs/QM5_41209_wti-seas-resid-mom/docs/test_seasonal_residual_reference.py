"""Independent deterministic fixtures for QM5_41209.
...
arithmetic, n-1 scaling, strict z boundaries, continuation direction, durable
...
def quote_spread_allows(bid: float, ask: float, point: float) -> bool:
    ...
    return math.isfinite(spread_points) and 0.0 <= spread_points <= 1500.0
...
class SeasonalResidualReferenceTests(unittest.TestCase):
    ...
    def test_five_through_ten_use_arithmetic_mean_and_n_minus_one(self) -> None:
        ...
        for count in range(5, 11):
            ...
            result = seasonal_residual(-0.02, observations)
    ...
    def test_strict_boundaries_and_continuation_sides(self) -> None:
        observations = [-2.0, -1.0, 0.0, 1.0, 2.0]
        sample_sd = statistics.stdev(observations)
        boundary = ENTRY_Z + TOLERANCE
        equal_high = seasonal_residual(boundary * sample_sd, observations)
        equal_low = seasonal_residual(-boundary * sample_sd, observations)
        above = seasonal_residual((boundary + 1e-8) * sample_sd, observations)
        below = seasonal_residual(-(boundary + 1e-8) * sample_sd, observations)
    ...
    def test_seasonal_adjustment_differs_from_raw_month_momentum(self) -> None:
        observations = [-0.14, -0.12, -0.11, -0.10, -0.09, -0.08]
        realized = -0.02
        result = seasonal_residual(realized, observations)
        self.assertIsNotNone(result)
        self.assertEqual(result.direction if result else 9, 1)
        raw_one_month_momentum = -1
        self.assertNotEqual(result.direction if result else 0, raw_one_month_momentum)

    def test_nonfinite_short_and_zero_scale_fail_closed(self) -> None:
        self.assertIsNone(seasonal_residual(0.1, [0.0] * 5))
        self.assertIsNone(seasonal_residual(math.nan, [-2, -1, 0, 1, 2]))
        self.assertIsNone(seasonal_residual(0.1, [-2, -1, 0, 1]))
        self.assertIsNone(seasonal_residual(0.1, [-2, -1, 0, 1, math.inf]))

    def test_zero_spread_reachable_crossed_and_excessive_rejected(self) -> None:
        self.assertTrue(quote_spread_allows(2.5, 2.5, 0.001))
        self.assertTrue(quote_spread_allows(2.5, 4.0, 0.001))
        self.assertFalse(quote_spread_allows(2.5, 4.001, 0.001))
    ...
    def test_static_build_contract_matches_approved_card(self) -> None:
        source = (EA_DIR / "QM5_41209_wti-seas-resid-mom.mq5").read_text(encoding="utf-8")
        set_path = EA_DIR / "sets" / "QM5_41209_wti-seas-resid-mom_XTIUSD.DWX_D1_backtest.set"
        ...
        self.assertLess(
            ...
            prepare.index("Strategy_LoadSeasonalResidualSignal"),
        )
        loader = source[source.index("bool Strategy_LoadSeasonalResidualSignal") : source.index("void Strategy_PrepareDecisionSignal()")].lower()
        ...
        approved = REPO_ROOT / "strategy-seeds" / "cards" / "approved" / "QM5_41209_wti-seas-resid-mom_card.md"
        ...
        self.assertIn("41209,wti-seas-resid-mom,0,XTIUSD.DWX,412090000", magic_rows)
        resolver = (REPO_ROOT / "framework" / "include" / "QM" / "QM_MagicResolver.mqh").read_text(encoding="utf-8")
        self.assertIn("412090000", resolver)
