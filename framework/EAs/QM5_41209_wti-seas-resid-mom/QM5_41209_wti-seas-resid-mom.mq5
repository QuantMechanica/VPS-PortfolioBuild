...
#property description "QM5_41209 WTI Monthly Standardized Seasonal-Residual Momentum"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41209 - WTI Monthly Standardized Seasonal-Residual Momentum
// -----------------------------------------------------------------------------
// Structural low-frequency WTI crude-oil sleeve:
//   - decide only at a genuine normalized broker-month boundary
//   - reconstruct the just-completed WTI calendar-month log return
//   - estimate that calendar month's mean from up to ten earlier exact years
//   - skip missing earlier years without substitution and require at least five
//   - standardize by the n-1 sample deviation and follow only |z| > 0.50+1e-10
...
double   g_signal_realized_return = 0.0;
double   g_signal_seasonal_mean   = 0.0;
double   g_signal_seasonal_sd     = 0.0;
double   g_signal_residual_z      = 0.0;
...
void Strategy_ResetDecisionState()
  {
   ...
   g_signal_realized_return = 0.0;
   g_signal_seasonal_mean = 0.0;
   g_signal_seasonal_sd = 0.0;
   g_signal_residual_z = 0.0;
   g_signal_state = "idle";
  }
...
// -----------------------------------------------------------------------------
// Exact completed-month endpoint and standardized seasonal-residual helpers.
// -----------------------------------------------------------------------------
...
bool Strategy_CompletedMonthReturn(const MqlRates &rates[],
                                   ...
                                   const int label_offset,
                                   const int terminal_following_month_key,
                                   double &month_return)
  {
   month_return = 0.0;
   ...
   if(previous_month_key != Strategy_PreviousMonthKey(target_month_key) ||
      rates[first_index - 1].time >= rates[first_index].time)
      return false;
      ...
      const int following_month_key =
         Strategy_MonthKeyForTime(
            Strategy_NormalizedLabel(rates[last_index + 1].time,
                                     label_offset));
      if(following_month_key != Strategy_NextMonthKey(target_month_key) ||
         rates[last_index].time >= rates[last_index + 1].time)
         return false;
     }
   else if(terminal_following_month_key !=
...
bool Strategy_SeasonalResidualSignal(const double realized_return,
                                     const double &observations[],
                                     const int sample_count,
                                     double &seasonal_mean,
                                     double &seasonal_sd,
                                     double &residual_z,
                                     int &direction)
  {
   ...
   if(strategy_history_years != 10 ||
      ...
      MathAbs(strategy_entry_z - 0.50) > 1.0e-12 ||
      MathAbs(strategy_signal_tolerance - 1.0e-10) > 1.0e-20 ||
      sample_count < strategy_min_observations ||
      sample_count > strategy_history_years ||
      ArraySize(observations) < sample_count ||
      !MathIsValidNumber(realized_return))
      return false;
   ...
   seasonal_mean = sum / (double)sample_count;
   if(!MathIsValidNumber(seasonal_mean))
      return false;
   ...
   for(int index = 0; index < sample_count; ++index)
     {
      if(index >= ArraySize(observations))
         return false;
      const double deviation = observations[index] - seasonal_mean;
   ...
   if(!MathIsValidNumber(sample_variance) || sample_variance <= 0.0)
      return false;

   seasonal_sd = MathSqrt(sample_variance);
   if(!MathIsValidNumber(seasonal_sd) || seasonal_sd <= 0.0)
      return false;

   residual_z = (realized_return - seasonal_mean) / seasonal_sd;
   if(!MathIsValidNumber(residual_z))
      return false;

   const double boundary = strategy_entry_z + strategy_signal_tolerance;
   if(residual_z > boundary)
      direction = 1;
   else if(residual_z < -boundary)
      direction = -1;
   return true;
  }

bool Strategy_LoadSeasonalResidualSignal(const datetime decision_bar_time,
                                         ...
                                         int &direction)
  {
   ...
   const int realized_month_key =
      Strategy_PreviousMonthKey(decision_month_key);
   const int realized_year = realized_month_key / 100;
   const int realized_month = realized_month_key % 100;
   if(realized_year < 1900 || realized_month < 1 || realized_month > 12 ||
      realized_year - strategy_history_years < 1900)
      return false;
   ...
   for(int offset = 1; offset <= strategy_history_years; ++offset)
     {
      const int sample_month_key =
         (realized_year - offset) * 100 + realized_month;
   ...
   return Strategy_SeasonalResidualSignal(realized_return,
                                          observations,
                                          sample_count,
                                          seasonal_mean,
                                          seasonal_sd,
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   // Consume before history, residual arithmetic, news, spread, quote, ATR,
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadSeasonalResidualSignal(g_decision_bar_time,
                                              g_decision_month_key,
                                              g_decision_label_offset,
                                              g_signal_sample_count,
                                              g_signal_realized_return,
                                              g_signal_seasonal_mean,
                                              g_signal_seasonal_sd,
                                              g_signal_residual_z,
                                              g_signal_direction);
      if(!g_signal_valid)
         g_signal_state = "endpoint_sample_mean_sd_or_z_failed";
      else if(g_signal_direction > 0)
         g_signal_state = "seasonal_residual_buy";
      else if(g_signal_direction < 0)
         g_signal_state = "seasonal_residual_sell";
      else
         g_signal_state = "seasonal_residual_band_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"valid\":%s,\"samples\":%d,\"realized_return\":%.12g,\"seasonal_mean\":%.12g,\"seasonal_sd\":%.12g,\"residual_z\":%.12g,\"direction\":%d,\"state\":\"%s\"}",
                            g_decision_month_key,
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(!QM_InputRequireLong("qm_ea_id", qm_ea_id, 41209) ||
      ...
      !QM_InputRequireDouble("strategy_entry_z", strategy_entry_z, 0.50, 1.0e-12) ||
      !QM_InputRequireDouble("strategy_signal_tolerance", strategy_signal_tolerance, 0.0000000001, 1.0e-20) ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(g_signal_direction > 0)
     {
      req.type = QM_BUY;
      req.reason = "WTI_SEAS_RESID_MOM_BUY";
     }
   else
     {
      req.type = QM_SELL;
      req.reason = "WTI_SEAS_RESID_MOM_SELL";
     }
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41209 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI monthly standardized seasonal-residual momentum card holds to the next broker month"))
      return INIT_FAILED;
   ...
   g_attempt_state_key =
      StringFormat("QM5_41209_SEAS_RESID_MONTH_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41209\",\"ea\":\"wti-seas-resid-mom\"}");
