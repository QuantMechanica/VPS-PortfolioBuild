...
- Time-stop: full close once
  `bars_since_entry >= time_stop_mult * g_position_cd_bars`, regardless of P&L.

**One documented simplification vs. the source card:**
...
At the D-touch bar the projected D has formed, so the EA measures the CD leg as
`C_shift - 2`, requires `abs(CD/AB - 1) <= time_symmetry_tolerance`, and stores
the admitted CD bar count for the card's `time_stop_mult * CD-bars` exit.
...
- `time_symmetry_tolerance` (0.20) is evaluated at the D-touch bar and remains
  the bounded handle for the P3 CD-leg time-symmetry sweep.
...
| `time_symmetry_tolerance` | 0.20 | 0.1-0.3 | Maximum absolute deviation of CD-bars/AB-bars from 1.0 |
...
> The §1 fractal-pivot simplification changes which swings qualify while
> retaining the card's price equality, time symmetry, and CD-relative stop.
...
| v1.1 | 2026-08-16 | Codex recovery review | wired time-symmetry input and CD-relative time-stop; host-slot audit PASS |
