...
input double time_symmetry_tolerance    = 0.20;   // reserved for P3 CD-leg time-symmetry sweep (inert pre-D, see SPEC §1)
...
input double time_stop_mult             = 1.5;    // time-stop = mult*10 bars flat cap (stand-in for 1.5x CD-bars, see SPEC §1)
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   int c_bars = 0, ab_bars = 0;
   if(FindABC(_Symbol, true, A, B, C, c_bars, ab_bars) && d1_regime_ok)
     {
   ...
   int c_bars2 = 0, ab_bars2 = 0;
   if(FindABC(_Symbol, false, A2, B2, C2p, c_bars2, ab_bars2) && d1_regime_ok)
     {
...
void Strategy_ManageOpenPosition()
  {
   ...
   const double atr14_1 = QM_ATR(_Symbol, PERIOD_CURRENT, atr_period, 1);
   const int    time_stop_cap = (int)MathRound(time_stop_mult * 10.0); // flat stand-in for 1.5x CD-bars
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      if(g_position_D <= 0.0 || g_position_C <= 0.0)
         continue;
