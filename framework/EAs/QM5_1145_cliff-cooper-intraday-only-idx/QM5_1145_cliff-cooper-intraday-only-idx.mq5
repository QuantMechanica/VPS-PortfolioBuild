...
#property description "QM5_1145 Cliff Cooper Intraday Only Index"

#include <QM/QM_Common.mqh>

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                       = 1145;
...
input bool   strategy_skip_friday_entries   = false;
input bool   strategy_use_gap_filter        = true;
input double strategy_max_overnight_gap_pct = 1.0;
input bool   strategy_trade_short_side      = true;
...
bool Strategy_OvernightGapAllowsEntry()
  {
   if(!strategy_use_gap_filter)
      return true;

   const double previous_close = iClose(_Symbol, PERIOD_D1, 1);
   const double today_open = iOpen(_Symbol, PERIOD_D1, 0);
   if(previous_close <= 0.0 || today_open <= 0.0)
      return false;

   const double gap_pct = MathAbs(today_open - previous_close) / previous_close * 100.0;
   return (gap_pct <= strategy_max_overnight_gap_pct);
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   req.type = strategy_trade_short_side ? QM_SELL : QM_BUY;
   req.price = 0.0;
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = strategy_trade_short_side ? "CLIFF_COOPER_INTRADAY_SHORT" : "CLIFF_COOPER_INTRADAY_LONG";
   ...
   const int entry_minute = open_minute + MathMax(0, strategy_entry_offset_minutes);
   ...
   if(g_last_entry_date_key == date_key)
      return false;
   if(!Strategy_OvernightGapAllowsEntry())
      return false;

   const QM_OrderType order_type = strategy_trade_short_side ? QM_SELL : QM_BUY;
   const double entry = QM_EntryMarketPrice(order_type);
   const double atr = QM_ATR(_Symbol, PERIOD_M30, strategy_atr_period, 1);
   if(entry <= 0.0 || atr <= 0.0)
      return false;

   if(strategy_trade_short_side)
      req.sl = NormalizeDouble(entry + strategy_atr_sl_mult * atr, _Digits);
   else
      req.sl = NormalizeDouble(entry - strategy_atr_sl_mult * atr, _Digits);
   if(req.sl <= 0.0)
      return false;
   if(strategy_trade_short_side && req.sl <= entry)
      return false;
   if(!strategy_trade_short_side && req.sl >= entry)
      return false;
...
void Strategy_ManageOpenPosition()
  {
   // Card default: intraday hold only; no trailing, pyramiding, or partial close.
  }
...
bool Strategy_ExitSignal()
  {
   ...
   const int exit_minute = close_minute - MathMax(0, strategy_exit_offset_minutes);
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"ea\":\"QM5_1145\",\"card\":\"cliff-cooper-intraday-only-idx\"}");
