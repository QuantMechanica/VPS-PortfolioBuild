...
status: APPROVED
execution_contract_ref: strategy-seeds/cards/approved/QM5_20270_wti-trimmean-mom_card.md
...
source_citations:
  - type: peer_reviewed_paper
    ...
    location: "DOI https://doi.org/10.1016/j.jfineco.2011.11.003; complete-paper evidence strategy-seeds/sources/MOP-TSMOM-2012/source.md; bounded extraction strategy-seeds/sources/MOP-WTI-TRIMMEAN-2026/source.md"
    quality_tier: A
...
sources:
  - "[[sources/MOP-WTI-TRIMMEAN-2026]]"
concepts:
  - "[[concepts/time-series-momentum]]"
  - "[[concepts/robust-location]]"
  - "[[concepts/crude-oil-structural-trend]]"
indicators:
  - "[[indicators/monthly-return-trimmed-mean]]"
...
q01_status: PASS
q02_status: ENQUEUED
review_focus: "Falsify a direct WTI monthly robust central-return direction whose fixed tail deletion differs from cumulative momentum, binary sign breadth, multi-horizon votes, rank trend, OLS trend, and the two-center median carrier; Q09 alone may establish realized book decorrelation."
modules_used: [no_trade, trade_entry, trade_management, trade_close]
target_modules: [Strategy_NoTradeFilter, Strategy_EntrySignal, Strategy_ManageOpenPosition, Strategy_ExitSignal, Strategy_NewsFilterHook]
hard_rules_at_risk: [completed_month_reconstruction, disjoint_return_intervals, chronological_return_orientation, ascending_sort, exact_tail_deletion, middle_eight_divisor, monthly_attempt_state, risk_mode_dual, friday_close_disabled, cfd_futures_basis, q02_frequency_floor, portfolio_correlation]
g0_approval_reasoning: "APPROVED under decisions/2026-08-10_qm5_20270_wti_trimmean_mom_g0.md: R1 complete-read peer-reviewed WTI source; R2 fixed twelve disjoint monthly returns, exact two-per-tail trim, middle-eight mean, and lifecycle; R3 registered XTI D1 route; R4 deterministic native arithmetic. No exact or fuzzy dedup identity; shared-source cumulative, sign-count, rank, regression, and median mechanics were manually resolved."
---
...
twelve-month endpoint return can be dominated by one oil shock. Deleting the
two largest and two smallest of twelve disjoint monthly returns and averaging
the middle eight estimates a broad central direction while retaining more
distribution information than a two-center median.
...
The source does not use a monthly-return trimmed mean. The robust estimator,
exact deleted and retained indexes, Darwinex continuous CFD, broker-month
...
The canonical checker scanned 4,327 EA-registry rows and 443 cards. It found no
...
- `QM5_20269` averages only sorted indexes 5 and 6, whereas this card deletes
  exactly two observations per tail and equally weights indexes 2 through 9.

None sorts twelve non-overlapping WTI monthly returns, removes indexes 0, 1,
10, and 11, and trades the sign of the remaining eight-return mean. Endpoint
count, disjoint intervals, log-return orientation, sort, deletion indexes,
retained indexes, divisor, symmetric mapping, consumed attempt, and renewal
...
- Timeframe: D1; magic slot 0; intended magic `202700000`.
...
BUY when `trimmed_mean > 0`; SELL when `trimmed_mean < 0`. An exact-zero,
malformed, nonconsecutive, unavailable, or nonfinite state consumes the month
flat. Trimmed-mean magnitude does not scale risk.

## Rules

These are the complete authorized baseline. There is no signal-parameter
sweep and no fallback to a cumulative return, binary sign count, two-center
...
1. Require exact EA ID `20270`, `XTIUSD.DWX` D1, magic slot 0, and every
...
6. Keep endpoints oldest to newest; require positive finite closes and
   ...
   returns and sort a copy ascending.
7. Delete sorted indexes `0`, `1`, `10`, and `11`; average exactly indexes
   `2..9` with divisor eight. Buy for a positive result and sell for a negative
   result. Exact zero or invalid arithmetic stays flat for the consumed month.
...
- Reject a consumed attempt, owned exposure, same-month entry history,
  ...
  or nonfinite close, invalid log return, exact-zero trimmed mean, excessive
...
| `strategy_trim_each_tail` | 2 | [2] | sorted observations deleted per tail |
...
sort, exact tail deletion, retained indexes, divisor, direction, entry clock,
risk, stop, hold, and no-retry policy are locked. Changing any of them requires
...
WTI in their commodity universe. They do not claim that this trimmed-mean rule
...
correlation with XNG or risk assets can dominate the premise. Tail deletion
can suppress an economically decisive oil shock and the retained observations
remain dependent. It is a descriptive formation statistic, not a confidence
guarantee.
...
- Fail on wrong endpoint order, nonconsecutive months, current-month leakage,
  overlapping return intervals, incorrect sort, wrong deletion indexes,
  wrong divisor, wrong-side entry, repeated monthly attempt, hold beyond forty
...
- Retire on nonpositive governed economics or later portfolio-correlation
  rejection.
- Do not rescue failure by changing lookback, trim count, retained indexes,
...
- [x] R2: PASS. Fixed endpoints, disjoint returns, exact sort and trim,
...
- [x] Dedup: no exact or fuzzy identity; shared-source cumulative, sign-count,
  rank, regression, and median systems were manually resolved.
...
- trade_entry: month-attempt persistence, endpoint reconstruction, monthly-
  return sorting and middle-eight mean, spread/quote/ATR/stop checks, and one
  fixed-risk order.
...
|---|---|---|---|---|
| v1 | 2026-08-10 | initial source-bounded WTI trimmed-mean card | G0 | APPROVED |
...
|---|---|---|---|
| G0 Research Intake | 2026-08-10 | APPROVED | `decisions/2026-08-10_qm5_20270_wti_trimmean_mom_g0.md` |
| Q01 Build Validation | 2026-08-10 | PASS | `D:/QM/reports/compile/20260810_093222/summary.csv`; `D:/QM/reports/framework/21/build_check_20260810_093222.json`; `D:/QM/reports/pipeline/QM5_20270/P1/P1_QM5_20270_result.json` |
| Q02 Baseline Screening | 2026-08-10 | ENQUEUED; pending, attempt 0, unclaimed | work item `7922d63b-dbb4-4269-bc4c-6fcaf7a760c1`; `docs/ops/evidence/2026-08-10_qm5_20270_wti_trimmean_mom_q01_q02_enqueue.md` |
