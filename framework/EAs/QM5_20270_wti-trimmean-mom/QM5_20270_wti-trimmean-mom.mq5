...
#property description "QM5_20270 WTI Trimmed-Mean Momentum"
...
//   3. sort the returns, delete two observations from each tail, and average
//      exact zero-based indexes 2 through 9;
//   4. trade the middle-eight mean sign, consuming an exact-zero/invalid month flat.
// The robust return order statistic is a transparent QM mechanization, not an
// author rule. Runtime uses native XTIUSD.DWX price and broker data only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 20270;
...
double g_cache_trimmed_mean    = 0.0;
double g_cache_retained_low     = 0.0;
double g_cache_retained_high    = 0.0;
...
void Strategy_ResetCachedState()
  {
   ...
   g_cache_signal = 0;
   g_cache_trimmed_mean = 0.0;
   g_cache_retained_low = 0.0;
   g_cache_retained_high = 0.0;
   g_cache_state_reason = "not_evaluated";
  }

bool Strategy_TrimmedMeanSignal(const double &closes[], int &signal)
  {
   signal = 0;
   const int return_count = strategy_return_months;
   const int trim_each_tail = strategy_trim_each_tail;
   const int retained_count = return_count - 2 * trim_each_tail;
   if(return_count != 12 ||
      trim_each_tail != 2 ||
      retained_count != 8 ||
      ArraySize(closes) != return_count + 1)
     {
      g_cache_state_reason = "bad_trim_contract";
      return false;
     }
   ...
   for(int i = 0; i < return_count; ++i)
     {
      ...
      if(i >= trim_each_tail && i < return_count - trim_each_tail)
         trimmed_sum += sorted_returns[i];
     }

   const double retained_low = sorted_returns[trim_each_tail];
   const double retained_high = sorted_returns[return_count - trim_each_tail - 1];
   const double trimmed_mean = trimmed_sum / (double)retained_count;
   if(!MathIsValidNumber(trimmed_sum) ||
      !MathIsValidNumber(retained_low) ||
      !MathIsValidNumber(retained_high) ||
      retained_low > retained_high ||
      !MathIsValidNumber(trimmed_mean))
     {
      g_cache_state_reason = "invalid_trimmed_mean_state";
      return false;
     }

   g_cache_retained_low = retained_low;
   g_cache_retained_high = retained_high;
   g_cache_trimmed_mean = trimmed_mean;
   if(trimmed_mean == 0.0)
     {
      g_cache_state_reason = "exact_zero_trimmed_mean";
      return true;
     }

   signal = (trimmed_mean > 0.0) ? 1 : -1;
...
bool Strategy_LoadSignalState(int &signal)
  {
   ...
   return Strategy_TrimmedMeanSignal(monthly_closes, signal);
  }
...
void Strategy_PrepareMonthlySignal()
  {
   ...
   QM_LogEvent(QM_INFO,
               "MONTHLY_STATE",
               StringFormat("{\"month\":%d,\"valid\":%s,\"signal\":%d,\"trimmed_mean\":%.10f,\"retained_low\":%.10f,\"retained_high\":%.10f,\"state\":\"%s\"}",
                            g_cache_month_key,
                            g_cache_signal_valid ? "true" : "false",
                            g_cache_signal,
                            g_cache_trimmed_mean,
                            g_cache_retained_low,
                            g_cache_retained_high,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 20270 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20270_WTI_TRIMMEAN_MOM";
   ...
   req.reason = (g_cache_signal > 0) ? "TRIMMEAN_MOM_XTI_LONG" : "TRIMMEAN_MOM_XTI_SHORT";
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_20270_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_20270\",\"ea\":\"wti-trimmean-mom\"}");
