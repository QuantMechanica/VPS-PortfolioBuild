...
#property description "QM5_10488 MQL5 Basic CCI RSI dual oscillator - v2 recompile: QM_MagicResolver refresh"
...
input ENUM_TIMEFRAMES strategy_work_tf   = PERIOD_M15;
input int    strategy_rsi_period         = 14;
input double strategy_rsi_level_up       = 55.0;
input double strategy_rsi_level_down     = 45.0;
input int    strategy_cci_period         = 14;
input double strategy_cci_level_up       = 100.0;
input double strategy_cci_level_down     = -100.0;
...
int Strategy_DualOscillatorSignal()
  {
   if(strategy_rsi_period <= 0 || strategy_cci_period <= 0)
      return 0;

   const double rsi = QM_RSI(_Symbol, strategy_work_tf, strategy_rsi_period, 1);
   const double cci = QM_CCI(_Symbol, strategy_work_tf, strategy_cci_period, 1);
   if(rsi == EMPTY_VALUE || cci == EMPTY_VALUE)
      return 0;

   if(rsi > strategy_rsi_level_up && cci > strategy_cci_level_up)
      return 1;
   if(rsi < strategy_rsi_level_down && cci < strategy_cci_level_down)
      return -1;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   const int signal = Strategy_DualOscillatorSignal();
   if(signal == 0)
      return false;
   ...
   req.sl = sl;
   req.tp = tp;
   req.reason = (signal > 0) ? "CCI_RSI_LONG" : "CCI_RSI_SHORT";
   return true;
  }
...
void Strategy_ManageOpenPosition()
  {
   // Card disables source trailing for P2; SL/TP, opposite signal, and time stop handle exits.
  }
