...
#property description "QM5_41132 WTI Completed-Month Weekday-Median Momentum"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41132 - WTI Completed-Month Weekday-Median Momentum
// -----------------------------------------------------------------------------
...
//   - partition returns by normalized Monday-through-Friday ending weekday
//   - average within weekday and follow the median of the five bucket means
//   - keep the raw month endpoint diagnostic only; it never gates direction
//   - consume the month before fallible gates; never enter late or retry
//   - hold the robust completed-month direction for one broker month
// Runtime uses MT5-native completed closes, calendar, and framework state only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                      = 41132;
...
input int    strategy_weekday_buckets      = 5;
input int    strategy_min_bucket_observations = 3;
input int    strategy_max_bucket_observations = 5;
...
double   g_raw_sum                 = 0.0;
double   g_endpoint_return         = 0.0;
int      g_weekday_counts[5];
double   g_weekday_means[5];
double   g_weekday_median          = 0.0;
...
void Strategy_ResetDecisionState()
  {
   ...
   g_weekday_median = 0.0;
   for(int bucket = 0; bucket < 5; ++bucket)
     {
      g_weekday_counts[bucket] = 0;
      g_weekday_means[bucket] = 0.0;
     }
   g_signal_state = "idle";
  }
...
bool Strategy_LoadWeekdayMedianSignal(const int current_month_key,
                                      const int label_offset,
                                      int &direction,
                                      int &month_sessions,
                                      ...
                                      double &weekday_median)
  {
   ...
   if(current_month_key <= 0 ||
      ...
      strategy_weekday_buckets != 5 ||
      strategy_min_bucket_observations != 3 ||
      strategy_max_bucket_observations != 5)
      return false;
   ...
   if(ArrayResize(month_closes,
      ...
      strategy_max_month_sessions)
      return false;
   datetime month_labels[];
   if(ArrayResize(month_labels,
                  strategy_max_month_sessions) !=
      strategy_max_month_sessions)
      return false;
      ...
      if(month_key == completed_month_key)
        {
         ...
         month_labels[month_sessions] = normalized;
   ...
   int weekday_counts[5];
   double weekday_sums[5];
   for(int bucket = 0; bucket < 5; ++bucket)
     {
      weekday_counts[bucket] = 0;
      weekday_sums[bucket] = 0.0;
     }
   ...
   for(int index = month_sessions - 1; index >= 0; --index)
     {
      ...
      MqlDateTime ending_parts;
      ZeroMemory(ending_parts);
      if(!TimeToStruct(month_labels[index], ending_parts) ||
         ending_parts.day_of_week < 1 ||
         ending_parts.day_of_week > 5)
         return false;
      const int bucket = ending_parts.day_of_week - 1;

      const double daily_return =
         MathLog(current_close) - MathLog(previous_close);
      if(!MathIsValidNumber(daily_return))
         return false;
      ++weekday_counts[bucket];
      if(weekday_counts[bucket] >
         strategy_max_bucket_observations)
         return false;
      weekday_sums[bucket] += daily_return;
      raw_sum += daily_return;
      if(!MathIsValidNumber(weekday_sums[bucket]) ||
         !MathIsValidNumber(raw_sum))
         return false;
      ++return_count;
      previous_close = current_close;
     }
   if(return_count != month_sessions)
      return false;
   ...
   if(!MathIsValidNumber(endpoint_return) ||
      MathAbs(raw_sum - endpoint_return) >
         strategy_numerical_tolerance)
      return false;

   double sorted_means[5];
   for(int bucket = 0; bucket < 5; ++bucket)
     {
      if(weekday_counts[bucket] <
            strategy_min_bucket_observations ||
         weekday_counts[bucket] >
            strategy_max_bucket_observations)
         return false;
      const double bucket_mean =
         weekday_sums[bucket] / (double)weekday_counts[bucket];
      if(!MathIsValidNumber(bucket_mean))
         return false;
      g_weekday_counts[bucket] = weekday_counts[bucket];
      g_weekday_means[bucket] = bucket_mean;
      sorted_means[bucket] = bucket_mean;
     }

   ArraySort(sorted_means);
   for(int index = 0; index < 5; ++index)
     {
      if(!MathIsValidNumber(sorted_means[index]))
         return false;
      if(index > 0 && sorted_means[index] < sorted_means[index - 1])
         return false;
     }

   weekday_median = sorted_means[2];
   if(weekday_median > 0.0)
      direction = 1;
   else if(weekday_median < 0.0)
      direction = -1;
   return true;
  }
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadWeekdayMedianSignal(g_decision_month_key,
                                          ...
                                          g_weekday_median);
      if(!g_signal_valid)
         g_signal_state = "monthly_weekday_median_validation_failed";
      else if(g_weekday_median == 0.0)
         g_signal_state = "zero_weekday_median_flat";
      else if(g_signal_direction > 0)
         g_signal_state = "weekday_median_completed_month_long";
      else if(g_signal_direction < 0)
         g_signal_state = "weekday_median_completed_month_short";
      else
   ...
   if(g_raw_sum > 0.0)
      raw_direction = 1;
   else if(g_raw_sum < 0.0)
      raw_direction = -1;
   const bool endpoint_agrees =
      (g_signal_direction != 0 &&
       raw_direction == g_signal_direction);

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"month_sessions\":%d,\"weekday_counts\":[%d,%d,%d,%d,%d],\"weekday_means\":[%.12e,%.12e,%.12e,%.12e,%.12e],\"weekday_median\":%.12e,\"boundary_close\":%.8f,\"final_close\":%.8f,\"raw_sum\":%.12e,\"endpoint_return\":%.12e,\"raw_direction\":%d,\"endpoint_agrees\":%s,\"state\":\"%s\"}",
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41132 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41132_WTI_MWEEKDAY_MED_MOM";
   ...
   req.reason = (g_signal_direction > 0)
      ? "WTI_MWEEKDAY_MED_MOM_LONG"
      : "WTI_MWEEKDAY_MED_MOM_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41132 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI completed-month weekday-median position holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41132_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41132\",\"ea\":\"wti-mweekday-med-mom\"}");
