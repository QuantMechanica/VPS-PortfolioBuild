...
#property description "QM5_41421 WTI Refinery-Maintenance Negative-Week Continuation"
...
//   - short only after a strictly negative weekly open-to-close return
...
bool Strategy_LoadNegativeWeekSignal(const int current_week_key,
                                     const int label_offset,
                                     int &direction)
  {
   direction = 0;
   ...
   // The maintenance sleeve is deliberately asymmetric: a strictly negative
   // completed week may continue short; nonnegative returns consume flat.
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_WeekAlreadyEntered(g_decision_week_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadNegativeWeekSignal(g_decision_week_key,
                                         g_decision_label_offset,
                                         g_signal_direction);
      if(!g_signal_valid)
         g_signal_state = "completed_week_validation_failed";
      else if(g_signal_direction < 0)
         g_signal_state = "negative_week_continuation_short";
      else
         g_signal_state = "nonnegative_week_consumed_flat";
     }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41421_WTI_REFMAINT_NEGWEEK_CONT";
   ...
   req.type = QM_SELL;
   req.reason = "WTI_REFMAINT_NEGWEEK_CONT_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(strategy_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41421 ||
      qm_magic_slot_offset != 0)
   ...
   g_attempt_state_key =
      StringFormat("QM5_41421_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41421\",\"ea\":\"wti-refmaint-negweek-cont\"}");
