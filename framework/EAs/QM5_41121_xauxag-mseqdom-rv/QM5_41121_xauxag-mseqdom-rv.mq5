...
#property description "QM5_41121 XAU/XAG Completed-Month Sequence-Dominance Reversion"
...
// Classify every chronological nonzero relative-return transition as a
// same-sign sequence or opposite-sign reversal. When sequences are at least
// reversals, fade the month's net ratio displacement with one opposite,
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41121 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41121_XAUXAG_MSEQDOM_RV";
...
bool Strategy_LoadMonthlySequenceDominance(
      ...
      int &direction)
  {
   ...
   // explicitly before forming adjacent relative returns so every sequence
   // and reversal is classified in chronological order exactly once.
   ...
   for(int i = 1; i < completed_month_sessions; ++i)
     {
      ...
      if(!MathIsValidNumber(relative_return) || relative_return == 0.0)
         return false;

      const int current_sign = (relative_return > 0.0) ? 1 : -1;
      if(i > 1)
        {
         ++transition_count;
   ...
   if(transition_count != completed_month_sessions - 2 ||
   ...
   // direction > 0 means BUY XAU / SELL XAG. The fixed inclusive sequence
   // majority only qualifies the state; completed-month net sign alone sets
   // the inverse side. Equality, when arithmetically possible, qualifies.
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41121_XAUXAG_MSEQDOM_RV_HOST";
   ...
   const bool valid =
      Strategy_LoadMonthlySequenceDominance(g_signal_month_key,
                                            completed_month_sessions,
                                            return_count,
                                            transition_count,
                                            sequences,
                                            ...
                                            net_displacement,
                                            direction);
   string state = "monthly_sequence_dominance_failed";
   if(valid && direction > 0)
      state = "sequence_dominant_negative_net_fade_long_xau";
   else if(valid && direction < 0)
      state = "sequence_dominant_positive_net_fade_short_xau";
   else if(valid && net_displacement == 0.0)
      state = "net_zero_flat";
   else if(valid)
      state = "reversal_dominant_flat";

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"completed_month_sessions\":%d,\"return_count\":%d,\"transition_count\":%d,\"sequences\":%d,\"reversals\":%d,\"net_displacement\":%.12e,\"direction\":%d,\"state\":\"%s\"}",
                            g_signal_month_key,
                            completed_month_sessions,
                            return_count,
                            transition_count,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG completed-month sequence-dominance reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41121\",\"ea\":\"xauxag-mseqdom-rv\"}");
