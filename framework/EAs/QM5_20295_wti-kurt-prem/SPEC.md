...
completed WTI D1 closes and compute 252 chronological simple returns. Calculate
...
Designed only for registered `XTIUSD.DWX`, D1, magic slot 0. It is not a port
to XNG, XAU, XAG, XBR, or an unregistered crude proxy. The carrier is outright
WTI, not a two-leg rank or ratio basket.
...
| Drawdown profile | Sparse fixed-risk WTI fourth-moment losses with gap and regime-persistence exposure |
| State | Long above Pearson kurtosis 3; short below 3 |

The crude carrier and fourth-moment state are diversification hypotheses only.
...
full-sample cross-sectional relation, uses monthly sorts, and includes WTI.
The two-way result and regression slope are insignificant, and the later
subperiod reverses sign insignificantly. The absolute benchmark-three rule is
a locked QM hypothesis, not a source-tested WTI rule. See the bounded source
...
`QM5_13131` and `QM5_20291` are two-leg cross-sectional kurtosis ranks; this
EA has one WTI state, one magic, one position, no relative rank, and no orphan
state. `QM5_20290` uses third-moment skewness around zero. `QM5_13130` and
`QM5_20294` use only the five largest returns. `QM5_12567` is a short-horizon
long-only oscillator pullback. Other WTI trend, reversal, calendar, event,
