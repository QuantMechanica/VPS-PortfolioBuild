...
#property description "QM5_20244 WTI Trend Sign Concordance"
...
//   - cumulative twelve-month log-return direction
//   - twelve completed monthly return signs, non-negative share threshold 0.40
//   - enter only when cumulative trend and return-sign state agree
...
bool Strategy_LoadConcordanceSignal(const int current_month_key,
                                    ...
                                    int &trade_direction)
  {
   ...
   if(!MathIsValidNumber(cumulative_return))
      return false;

   const int trend_direction =
      (cumulative_return > 0.0) ? 1 :
      ((cumulative_return < 0.0) ? -1 : 0);
   const int sign_direction =
      (positive_probability >= strategy_positive_threshold) ? 1 : -1;

   // A valid disagreement or exact-zero trend consumes the month flat.
   if(trend_direction == 0 || trend_direction != sign_direction)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(!Strategy_LoadConcordanceSignal(month_key,
                                      cumulative_return,
                                      positive_probability,
                                      positive_count,
                                      trade_direction) ||
   ...
   req.reason = StringFormat("WTI_TREND_SIGN_%s_P%02d_OF_%02d_R%.4f",
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_20244_MONTH_ATTEMPT_%d",
                   QM_FrameworkMagic());
   Strategy_LoadAttemptState();

   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_20244\",\"ea\":\"wti-trend-sign\"}");
