...
#property description "QM5_10219 TradingView Market Open Impulse"
...
input int    strategy_open_hhmm         = 1630;
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(!has_position && _Period != PERIOD_M5)
      return true;

   const long spread_points = SymbolInfoInteger(_Symbol, SYMBOL_SPREAD);
   if(!has_position && strategy_max_spread_points > 0 && spread_points > strategy_max_spread_points)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(range <= atr * strategy_impulse_atr_mult)
      return false;

   const double midpoint = (high1 + low1) * 0.5;
   const bool long_signal = (close1 > midpoint && close1 > open1);
   const bool short_signal = (close1 < midpoint && close1 < open1);
   ...
   req.price = 0.0;
   req.sl = NormalizeDouble(long_signal ? low1 : high1, _Digits);
   if((long_signal && req.sl >= entry) || (short_signal && req.sl <= entry))
      return false;
   ...
   req.reason = long_signal ? "TV_OPEN_IMPULSE_LONG" : "TV_OPEN_IMPULSE_SHORT";
   return true;
  }
...
void Strategy_ManageOpenPosition()
  {
   // Baseline card: breakeven is optional and disabled for first P2 pass.
  }
...
bool Strategy_ExitSignal()
  {
   // Baseline exits via fixed candle-extreme SL, 3R TP, and framework Friday close.
   return false;
  }
