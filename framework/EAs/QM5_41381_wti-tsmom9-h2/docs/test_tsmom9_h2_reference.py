#!/usr/bin/env python3
"""Independent vectors for QM5_41381's locked statistic and clock."""
...
def consecutive(month_keys: list[int]) -> bool:
    return len(month_keys) == 10 and all(
        next_month(month_keys[i]) == month_keys[i + 1] for i in range(9)
    )
...
def signal(closes: list[float]) -> tuple[int | None, float | None]:
    ...
    chained_return = sum(math.log(closes[i + 1] / closes[i]) for i in range(9))
...
def main() -> None:
    months = [202605, 202606, 202607, 202608, 202609,
              202610, 202611, 202612, 202701, 202702]
    assert consecutive(months)
    assert not consecutive(months[:5] + [202612] + months[6:])

    rising = [100.0, 101.0, 99.0, 103.0, 102.0,
              106.0, 104.0, 108.0, 107.0, 110.0]
    falling = [100.0, 99.0, 101.0, 96.0, 98.0,
               94.0, 95.0, 92.0, 93.0, 90.0]
    flat = [100.0, 105.0, 95.0, 106.0, 94.0,
            107.0, 93.0, 108.0, 92.0, 100.0]
