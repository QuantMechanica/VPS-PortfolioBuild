...
#property description "QM5_13076 XTI NZDCAD Return Spread Reversion"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_13076 - WTI/NZDCAD Return-Spread Reversion
// -----------------------------------------------------------------------------
// D1 two-leg energy/FX basket:
//   rspread = log(XTI[t] / XTI[t-L]) + beta_nzdcad * log(NZDCAD[t] / NZDCAD[t-L])
//   z > entry: short return spread = sell WTI, sell NZDCAD
//   z < -entry: long return spread = buy WTI, buy NZDCAD
...
input double strategy_beta_nzdcad         = 0.55;
...
input int    strategy_nzdcad_max_spread_pts = 140;
   ...
   for(int i = 0; i < z_lookback; ++i)
     {
      ...
      if(xti[i] <= 0.0 || nzdcad[i] <= 0.0 || xti[past_idx] <= 0.0 || nzdcad[past_idx] <= 0.0)
         return false;

      const double xti_ret = MathLog(xti[i] / xti[past_idx]);
      const double nzdcad_ret = MathLog(nzdcad[i] / nzdcad[past_idx]);
      spreads[i] = xti_ret + strategy_beta_nzdcad * nzdcad_ret;
...
bool Strategy_OpenPair(const int spread_direction)
  {
   if(spread_direction == 0 || Strategy_OpenPairLegCount() > 0)
      return false;
   if(!Strategy_SpreadAllowed(g_leg_xti) || !Strategy_SpreadAllowed(g_leg_nzdcad))
      return false;

   const double xti_weight = 1.0;
   const double nzdcad_weight = MathMax(0.1, MathAbs(strategy_beta_nzdcad));
   const double weight_sum = xti_weight + nzdcad_weight;
   ...
   const string reason = long_spread ? "QM5_13076_LONG_XTI_NZDCAD_RSPREAD"
                                     : "QM5_13076_SHORT_XTI_NZDCAD_RSPREAD";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_13076_RSPREAD_HOST";
...
int OnInit()
  {
   ...
   string basket_symbols[2] = {g_leg_xti, g_leg_nzdcad};
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_13076\",\"ea\":\"xti-nzdcad-rspr\"}");
