...
`artifacts/cards_approved/QM5_10205_tv-chop-dmi-psar.md`. See that card's body for
...
`QM5_10205_tv-chop-dmi-psar.mq5`. Framework wiring (risk, magic, news, Friday close)
...
|---|---|---|---|
| `strategy_timeframe` | PERIOD_H1 | (see source) | (see strategy logic) |
| `strategy_chop_period` | 14 | (see source) | (see strategy logic) |
| `strategy_chop_smoothing` | 4 | (see source) | (see strategy logic) |
| `strategy_chop_long_level` | 61.8 | (see source) | (see strategy logic) |
| `strategy_chop_short_level` | 38.2 | (see source) | (see strategy logic) |
| `strategy_dmi_period` | 14 | (see source) | (see strategy logic) |
| `strategy_adx_key_level` | 25.0 | (see source) | (see strategy logic) |
| `strategy_psar_start` | 0.015 | (see source) | (see strategy logic) |
| `strategy_psar_increment` | 0.001 | (see source) | (see strategy logic) |
...
| `strategy_max_spread_stop_fraction` | 0.15 | (see source) | (see strategy logic) |
| `strategy_follow_trend` | true | (see source) | (see strategy logic) |
| `strategy_enable_psar_exit` | true | (see source) | (see strategy logic) |
| `strategy_enable_dmi_exit` | true | (see source) | (see strategy logic) |
| `strategy_psar_warmup_bars` | 80 | (see source) | (see strategy logic) |
...
`artifacts/cards_approved/QM5_10205_tv-chop-dmi-psar.md`
