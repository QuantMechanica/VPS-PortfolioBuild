...
#property description "QM5_10117 TradingView 2MA PSAR ATR Trend"
...
bool Strategy_NoTradeFilter()
  {
   // No Trade Filter: framework handles time/news; card adds a spread cap.
   ...
   if(strategy_max_spread_stop_pct > 0.0)
     {
      ...
      if(stop_distance <= 0.0)
         return true;
      if(spread_price > strategy_max_spread_stop_pct * stop_distance)
         return true;
     }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   // Trade Entry: EMA trend state, close-vs-fast-EMA, and PSAR confirmation.
   req.type = QM_BUY;
   req.price = 0.0;
   req.sl = 0.0;
   req.tp = 0.0;
   ...
   const double psar = QM_SAR(_Symbol, strategy_timeframe, strategy_psar_step, strategy_psar_maximum, 1);
   if(fast_ema <= 0.0 || slow_ema <= 0.0 || psar <= 0.0)
      return false;

   const int price_vs_fast = QM_Sig_Price_Above_MA(_Symbol, strategy_timeframe, strategy_fast_ema_period, 0.0, 1);
   ...
   double entry = 0.0;
   if(strategy_longs_enabled && fast_ema > slow_ema && price_vs_fast > 0 && psar < bid)
     {
      side = QM_BUY;
      entry = ask;
     }
   else if(strategy_shorts_enabled && fast_ema < slow_ema && price_vs_fast < 0 && psar > ask)
     {
      side = QM_SELL;
      entry = bid;
     }
   ...
   req.sl = sl;
   req.tp = 0.0;
   req.reason = (side == QM_BUY) ? "EMA_PSAR_ATR_LONG" : "EMA_PSAR_ATR_SHORT";
   req.symbol_slot = qm_magic_slot_offset;
   req.expiration_seconds = 0;
   return true;
  }
...
bool Strategy_ExitSignal()
  {
   ...
   const double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
   const double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   if(fast_ema <= 0.0 || slow_ema <= 0.0 || psar <= 0.0 || ask <= 0.0 || bid <= 0.0)
      return false;

   if(position_type == POSITION_TYPE_BUY && (fast_ema < slow_ema || psar > bid))
      return true;
   if(position_type == POSITION_TYPE_SELL && (fast_ema > slow_ema || psar < ask))
      return true;
   return false;
  }
