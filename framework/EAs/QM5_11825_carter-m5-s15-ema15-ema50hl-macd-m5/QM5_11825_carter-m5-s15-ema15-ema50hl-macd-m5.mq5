...
#property description "QM5_11825 carter-m5-s15-ema15-ema50hl-macd-m5 — EMA15 + EMA50 H/L channel + MACD state, EMA15-reclaim trigger (M5)"

#include <QM/QM_Common.mqh>

// =============================================================================
// QuantMechanica V5 EA — QM5_11825 carter-m5-s15-ema15-ema50hl-macd-m5
// -----------------------------------------------------------------------------
// Source: Thomas Carter, "20 Forex Trading Strategies (5 Minute Time Frame)"
//         (367145560), Strategy #15. Card:
//         artifacts/cards_approved/QM5_11825_carter-m5-s15-ema15-ema50hl-macd-m5.md
//         (g0_status APPROVED).
//
// Mechanics (closed-bar reads at shift 1; M5):
//   Channel STATE : EMA(50) of bar High / Low forms a price channel.
//       upper = EMA(channel_period, PRICE_HIGH)
//       lower = EMA(channel_period, PRICE_LOW)
//   Trend  STATE : fast EMA(15) on close vs the channel —
//       long  trend : ema_fast > upper   (EMA15 close above the high channel)
//       short trend : ema_fast < lower   (EMA15 close below the low channel)
//   MACD   STATE : MACD(15,70,1). With signal period 1, the signal line equals
//       the main line, so the card's "histogram" is the MACD MAIN value itself.
//       long  state : macd_main > 0      (bullish "green arrow" substitute)
//       short state : macd_main < 0      (bearish substitute)
//   Trigger EVENT (the ONE fresh event — avoids the two-cross-same-bar trap):
//       Price reclaims EMA(15) after a pullback. Using closed-bar closes:
//       long  : close[2] <= ema_fast[2] AND close[1] > ema_fast[1]
//       short : close[2] >= ema_fast[2] AND close[1] < ema_fast[1]
//   The channel breakout + MACD sign are STATEs that must hold ON the trigger
//   bar; the EMA15-reclaim is the single per-bar EVENT. They are never required
//   to be two simultaneous fresh crosses.
//   Stop  : 2 * ATR(14) from entry (long below / short above; card: below the
//           EMA50-LOW channel for longs / above EMA50-HIGH for shorts —
//           realized via the symmetric ATR stop the factory specifies).
//   Take  : symmetric distance via RR = tp_atr_mult / sl_atr_mult (card: 1:1).
//   Exit  : SL / TP only (card: "no discretionary manual exit"). A defensive
...
input int    strategy_macd_signal        = 1;      // MACD signal period (card: 1 -> hist = main)
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   // --- Trend STATE: fast EMA(15) on close vs the channel (shift 1 and 2) ---
   const double ema_fast1 = QM_EMA(_Symbol, _Period, strategy_ema_fast_period, 1, PRICE_CLOSE);
   const double ema_fast2 = QM_EMA(_Symbol, _Period, strategy_ema_fast_period, 2, PRICE_CLOSE);
   if(ema_fast1 <= 0.0 || ema_fast2 <= 0.0)
      return false;

   const bool long_trend  = (ema_fast1 > upper);
   const bool short_trend = (ema_fast1 < lower);
   if(!long_trend && !short_trend)
      return false; // EMA15 inside the channel — no breakout state

   // --- MACD STATE: MACD(15,70,1) main value sign (signal=1 -> hist = main) ---
   ...
   // --- Trigger EVENT: price reclaims EMA15 after a pullback (one fresh event) ---
   const double close1 = iClose(_Symbol, _Period, 1); // perf-allowed: single closed-bar close
   const double close2 = iClose(_Symbol, _Period, 2); // perf-allowed: single closed-bar close
   if(close1 <= 0.0 || close2 <= 0.0)
      return false;

   const bool reclaim_up   = (close2 <= ema_fast2 && close1 > ema_fast1);
   const bool reclaim_down = (close2 >= ema_fast2 && close1 < ema_fast1);

   // --- Compose: trend STATE + MACD STATE + matching EMA15-reclaim EVENT ---
   QM_OrderType side;
   if(long_trend && macd_bull && reclaim_up)
      side = QM_BUY;
   else if(short_trend && macd_bear && reclaim_down)
      side = QM_SELL;
   else
      return false;
   ...
   req.sl     = sl;
   req.tp     = tp;
   req.reason = (side == QM_BUY) ? "carter_s15_ema_chan_macd_long"
                                 : "carter_s15_ema_chan_macd_short";
   return true;
  }
