...
#property description "QM5_13301 Balke Minute-Precision GMT3 Range Breakout"
...
// René Balke Range Breakout, minute-precision variant of QM5_13213
// (framework/EAs/QM5_13213_balke-gmt3-range-breakout). QM5_13213 only exposes
// hour-granular window inputs (range 03:00-06:00, exit 18:00). Balke's actual
// live-gold setup is 03:05-06:05 / close+delete 18:55 — a 5-minute offset from
// our hour-only port — per OWNER-verified source survey
// docs/research/balke_windows_survey_2026-07-15.md ("a change of an hour or
// even half an hour can make the difference between profit and getting
// wrecked" — Balke). This EA adds *_minute inputs alongside the existing
// *_hour inputs so the window can land on any 5-minute boundary, and re-bases
// the range-build / entry-fire / evening-cancel logic from PERIOD_H1 to
// PERIOD_M5 bars so the closed-bar gate actually lands on :05/:55 boundaries
// instead of only ever ticking at the top of the hour. ATR sizing and the
// 2-bar swing trail stay on PERIOD_H1 (unchanged from QM5_13213) — those are
// volatility/structure filters, not the time-window logic this variant exists
// to fix. Same GMT3-normalized time base as QM5_13213 (broker_time -> UTC ->
// +3h) — NOT the earlier broken QM5_1142 raw-broker-clock approach.
...
// M5-bar lookback buffer for the same-day range scan. 432 M5 bars = 36h,
// proportional to QM5_13213's 36 H1-bar buffer.
input int    strategy_range_scan_bars       = 432;
...
// Broker time is GMT+2 (winter) / GMT+3 (summer, US DST) — QM_BrokerToUTC
// normalizes both onto true UTC, then +3h re-projects into a fixed
// GMT+3-equivalent frame so the range/exit windows land consistently across
// the DST boundary instead of drifting by an hour on raw broker-clock reads.
...
int Strategy_Gmt3MinuteOfDay(const datetime broker_time)
  {
   const datetime utc = QM_BrokerToUTC(broker_time);
   const datetime gmt3 = utc + 3 * 3600;
   MqlDateTime dt;
...
int Strategy_RangeStartMinutes() { return strategy_range_start_hour * 60 + strategy_range_start_minute; }
int Strategy_RangeEndMinutes()   { return strategy_range_end_hour   * 60 + strategy_range_end_minute; }
int Strategy_ExitMinutes()       { return strategy_exit_hour        * 60 + strategy_exit_minute; }
...
bool Strategy_BuildRangeForToday(const int day_key, double &range_high, double &range_low)
  {
   ...
   const int range_start_minutes = Strategy_RangeStartMinutes();
   const int range_end_minutes   = Strategy_RangeEndMinutes();

   MqlRates rates[];
   ArraySetAsSeries(rates, true);
   const int copied = CopyRates(_Symbol, PERIOD_M5, 1, strategy_range_scan_bars, rates); // perf-allowed: closed-bar session range scan, bounded by input default 432.
   if(copied <= 0)
      return false;
   ...
   for(int i = 0; i < copied; ++i)
     {
      ...
      if(Strategy_Gmt3DayKey(bar_time) != day_key)
         continue;
      const int minute_of_day = Strategy_Gmt3MinuteOfDay(bar_time);
      if(minute_of_day < range_start_minutes || minute_of_day >= range_end_minutes)
         continue;
   ...
   const int expected_bars = (range_end_minutes - range_start_minutes) / 5;
   return (bars_in_range >= expected_bars && range_high > range_low && range_low > 0.0);
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   // Trade Entry: at range_end_hour:range_end_minute (default 06:00
   // GMT+3-equivalent) place both stop orders around the completed
   // range_start-range_end range. Caller guarantees QM_IsNewBar() == true —
   // the EA is deployed on an M5 chart so this closed-bar gate lands on the
   // exact 5-minute boundary the window inputs target.
   ...
   const int minute_of_day = Strategy_Gmt3MinuteOfDay(now);
   if(minute_of_day != Strategy_RangeEndMinutes())
      return false;
   ...
   Strategy_PopulateEntry(req, QM_SELL_STOP, range_low, range_high, "BALKE_RANGE_SELL_STOP");
...
void Strategy_ManageOpenPosition()
  {
   // Trade Management: at Balke's single evening resolution minute, cancel
   // any stale/opposite stop order (the close of the open position itself is
   // handled by Strategy_ExitSignal at the same minute). Before that, cancel
   // the opposite pending order once one side has triggered. Runs every tick
   // (not bar-gated) so the exit/cancel minute fires at real-time precision
   // regardless of chart period.
   const datetime now = TimeCurrent();
   const int minute_of_day = Strategy_Gmt3MinuteOfDay(now);
   if(minute_of_day >= Strategy_ExitMinutes())
...
bool Strategy_ExitSignal()
  {
   // Trade Close: Balke's single evening flat time (default 18:00
   // GMT+3-equivalent), or range opposite-side touch if the original range
   // state is available. Runs every tick — real-time minute precision.
   const datetime now = TimeCurrent();
   if(Strategy_Gmt3MinuteOfDay(now) >= Strategy_ExitMinutes())
      return true;
