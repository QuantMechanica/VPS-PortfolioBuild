...
#property description "QM5_12950 MQL5 Accumulation Distribution Price Confirmation"

#include <QM/QM_Common.mqh>

// =============================================================================
// QuantMechanica V5 EA: QM5_12950 - MQL5 Accumulation Distribution Price Confirmation
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 12950;
...
input int    strategy_max_bars_in_trade = 36;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
{
   ...
   const double ad1    = QM_AD(_Symbol, (ENUM_TIMEFRAMES)_Period, 1, VOLUME_TICK);
   const double ad2    = QM_AD(_Symbol, (ENUM_TIMEFRAMES)_Period, 2, VOLUME_TICK);

   if(close1 <= 0.0 || ema100 <= 0.0 || high1 <= 0.0 || high2 <= 0.0 || low1 <= 0.0 || low2 <= 0.0)
      return false;

   const bool buy_signal  = (close1 > ema100 && high1 > high2 && ad1 > ad2);
   const bool sell_signal = (close1 < ema100 && low1 < low2 && ad1 < ad2);
   ...
   if(buy_signal)
   {
      ...
      req.sl = sl;
      req.tp = tp;
      req.reason = "ad_price_continuation_long";
      return true;
   }
   ...
   if(sell_signal)
   {
      ...
      req.sl = sl;
      req.tp = tp;
      req.reason = "ad_price_continuation_short";
      return true;
   }
...
bool Strategy_ExitSignal()
{
   ...
   const double ad1    = QM_AD(_Symbol, (ENUM_TIMEFRAMES)_Period, 1, VOLUME_TICK);
   const double ad2    = QM_AD(_Symbol, (ENUM_TIMEFRAMES)_Period, 2, VOLUME_TICK);
   const double ad3    = QM_AD(_Symbol, (ENUM_TIMEFRAMES)_Period, 3, VOLUME_TICK);

   if(close1 <= 0.0 || ema100 <= 0.0)
      return false;
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
   {
      ...
      {
         if((ad1 < ad2 && ad2 < ad3) || close1 < ema100)
            return true;
      }
