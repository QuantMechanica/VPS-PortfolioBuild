# QM5_20294_xauxag-max-rk - Strategy Spec
...
metal's MAX characteristic from exactly 252 completed simple D1 returns. MAX
is the arithmetic mean of the five largest returns. Buy the lower-MAX metal,
short the higher-MAX metal, split one fixed-risk package equally, and close
and rerank at the next broker-month transition. A tie or invalid input
consumes the month without a trade.

This is a pure upper-order-statistic cross-sectional rank. It uses no price
ratio, OLS residual, spread z-score, kurtosis, skewness, expected shortfall,
trend, oscillator, calendar direction, trained output, or adaptive threshold.
...
| `strategy_top_return_count` | 5 | largest returns included in MAX |
...
- Host/slot 0: `XAUUSD.DWX`, intended magic `202940000`.
- Traded slot 1: `XAGUSD.DWX`, intended magic `202940001`.
...
defined MAX is lower than XAG's; otherwise the sides reverse. A tie or invalid
state remains flat. Q02 retires the candidate below five completed packages
per full post-warm-up year, and Q09 alone may establish realized correlation
to the certified book.
...
2150017, DOI `10.1142/S2010139221500178`. The full-sample MAX result and the
directly relevant two-portfolio split are insignificant; the locked negative
direction is post-financialization-subsample-only. No efficacy transfers to
this two-CFD carrier.
