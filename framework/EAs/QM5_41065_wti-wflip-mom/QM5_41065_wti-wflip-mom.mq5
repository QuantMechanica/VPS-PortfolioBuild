...
#property description "QM5_41065 WTI Fresh Weekly Return-Sign Handoff Momentum"
...
//   - trade only a strict sign handoff, following the newest weekly sign
...
bool Strategy_LoadSignHandoffSignal(const int current_week_key,
                                    const int label_offset,
                                    int &direction,
                                    double &new_return,
                                    double &old_return,
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_WeekAlreadyEntered(g_decision_week_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadSignHandoffSignal(g_decision_week_key,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41065 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41065_WTI_WFLIP_MOM";
...
int OnInit()
  {
   if(!SymbolSelect(g_strategy_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41065 ||
   ...
   g_attempt_state_key =
      StringFormat("QM5_41065_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41065\",\"ea\":\"wti-wflip-mom\"}");
