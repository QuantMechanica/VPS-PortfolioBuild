...
#property description "QM5_11126 TradingMarkets RSI2 Limit Pullback"
...
input double strategy_rsi_entry         = 5.0;
input double strategy_rsi_exit          = 65.0;
input int    strategy_sma_period        = 200;
input double strategy_limit_discount_pct = 2.0;
input int    strategy_atr_period        = 14;
input double strategy_atr_sl_mult       = 2.5;
input double strategy_stop_floor_pct    = 4.0;
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(!has_open_position && strategy_max_spread_points > 0)
     {
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(strategy_rsi_period <= 0 ||
      ...
      strategy_limit_discount_pct <= 0.0 ||
      strategy_atr_period <= 0 ||
      strategy_atr_sl_mult <= 0.0 ||
      strategy_stop_floor_pct <= 0.0 ||
   ...
   if(close_last <= 0.0 || sma_last <= 0.0 || rsi_last <= 0.0 || atr_last <= 0.0)
      return false;

   if(close_last <= sma_last || rsi_last >= strategy_rsi_entry)
      return false;

   const double limit_price = NormalizeDouble(close_last * (1.0 - strategy_limit_discount_pct / 100.0), _Digits);
   if(limit_price <= 0.0)
      return false;
   ...
   const double stop_distance = MathMax(atr_last * strategy_atr_sl_mult,
                                        limit_price * strategy_stop_floor_pct / 100.0);
   if(stop_distance <= 0.0)
      return false;

   const double stop_price = NormalizeDouble(limit_price - stop_distance, _Digits);
   if(stop_price <= 0.0 || stop_price >= limit_price)
      return false;
...
bool Strategy_ExitSignal()
  {
   if(strategy_rsi_period <= 0 || strategy_max_hold_bars <= 0)
      return false;
