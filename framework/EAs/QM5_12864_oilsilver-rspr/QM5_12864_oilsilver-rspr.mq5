...
#property description "QM5_12864 XTI XAG Return Spread Reversion"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_12864 - XTI/XAG Return-Spread Reversion
// -----------------------------------------------------------------------------
// D1 two-leg oil/silver basket:
//   rspread = log(XTI[t] / XTI[t-L]) - beta * log(XAG[t] / XAG[t-L])
   ...
   for(int i = 0; i < z_lookback; ++i)
     {
      ...
      if(xti[i] <= 0.0 || xag[i] <= 0.0 || xti[past_idx] <= 0.0 || xag[past_idx] <= 0.0)
         return false;

      const double xti_ret = MathLog(xti[i] / xti[past_idx]);
      const double xag_ret = MathLog(xag[i] / xag[past_idx]);
      spreads[i] = xti_ret - strategy_beta * xag_ret;
...
bool Strategy_OpenPair(const int spread_direction)
  {
   ...
   const string reason = long_spread ? "QM5_12864_LONG_XTI_XAG_RSPREAD"
                                     : "QM5_12864_SHORT_XTI_XAG_RSPREAD";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12864_RSPREAD_HOST";
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_12864\",\"ea\":\"oilsilver-rspr\"}");
