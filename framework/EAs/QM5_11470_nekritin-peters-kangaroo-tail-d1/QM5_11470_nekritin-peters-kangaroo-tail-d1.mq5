...
#property description "QM5_11470 Nekritin/Peters Kangaroo Tail D1"

#include <QM/QM_Common.mqh>

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 11470;
...
input int    strategy_sr_lookback_bars    = 60;
...
// Trade Entry: D1 Kangaroo Tail stop entry on the bar after the pin bar.
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(strategy_tail_ratio <= 0.0 || strategy_room_bars < 1 ||
      strategy_entry_offset_pips < 0 || strategy_stop_offset_pips < 0 ||
      strategy_max_stop_pips <= 0 || strategy_fractal_wing < 1)
      return false;
   ...
   const double open1 = iOpen(_Symbol, PERIOD_D1, 1); // perf-allowed: D1 pin-bar OHLC, called only after QM_IsNewBar gate.
   const double high1 = iHigh(_Symbol, PERIOD_D1, 1); // perf-allowed: D1 pin-bar OHLC, called only after QM_IsNewBar gate.
   const double low1 = iLow(_Symbol, PERIOD_D1, 1); // perf-allowed: D1 pin-bar OHLC, called only after QM_IsNewBar gate.
   const double close1 = iClose(_Symbol, PERIOD_D1, 1); // perf-allowed: D1 pin-bar OHLC, called only after QM_IsNewBar gate.
   if(open1 <= 0.0 || high1 <= 0.0 || low1 <= 0.0 || close1 <= 0.0 || high1 <= low1)
      return false;

   const double body_raw = MathAbs(close1 - open1);
   const double body = MathMax(body_raw, pip);
   const double candle_mid = (high1 + low1) / 2.0;
   ...
   if(strategy_trend_filter)
     {
      if(strategy_trend_bars < 1)
         return false;
     }

   bool bullish = (lower_shadow >= strategy_tail_ratio * body &&
                   MathMin(open1, close1) > candle_mid &&
                   low1 < prior_low);
   bool bearish = (upper_shadow >= strategy_tail_ratio * body &&
                   MathMax(open1, close1) < candle_mid &&
                   high1 > prior_high);

   if(strategy_trend_filter && bullish)
     {
      double trend_low = DBL_MAX;
      for(int shift = 2; shift < 2 + strategy_trend_bars; ++shift)
        {
         const double low = iLow(_Symbol, PERIOD_D1, shift); // perf-allowed: optional bounded trendy-tail scan.
         if(low <= 0.0)
            return false;
         trend_low = MathMin(trend_low, low);
        }
      bullish = (low1 < trend_low);
     }
   if(strategy_trend_filter && bearish)
     {
      double trend_high = -DBL_MAX;
      for(int shift = 2; shift < 2 + strategy_trend_bars; ++shift)
        {
         const double high = iHigh(_Symbol, PERIOD_D1, shift); // perf-allowed: optional bounded trendy-tail scan.
         if(high <= 0.0)
            return false;
         trend_high = MathMax(trend_high, high);
        }
      bearish = (high1 > trend_high);
     }
   ...
   if(bullish)
     {
      ...
      req.sl = sl;
      req.tp = tp;
      req.reason = "kangaroo_tail_bullish_d1";
      return true;
     }
   ...
   for(int shift = 2 + wing; shift <= lookback; ++shift)
     {
      ...
      if(candidate <= 0.0 || candidate >= entry)
         continue;
      bool is_fractal = true;
      for(int j = 1; j <= wing; ++j)
        {
   ...
   req.sl = sl;
   req.tp = tp;
   req.reason = "kangaroo_tail_bearish_d1";
   return true;
  }
...
void OnTick()
  {
   // Q08 evidence lifecycle: no guard may skip floating-MAE sampling.
   ...
   // News blackout gates entries only. Management and exits above must remain
