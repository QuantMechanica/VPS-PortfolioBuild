...
#property description "QM5_41027 WTI Month-Opening Session Reversal"
...
//   - decide only on the second genuine session of each broker month
//   - fade the completed first session's exact open-to-close return sign
//   - consume the month before fallible gates; never shift, retry, or backfill
//   - flatten at the next normalized D1 boundary; Friday close is a fail-safe
// Runtime uses MT5-native OHLC, calendar, history, and framework state only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                      = 41027;
...
input int    strategy_entry_session_ordinal   = 2;
input int    strategy_formation_sessions      = 1;
...
double   g_first_session_return    = 0.0;
...
void Strategy_DetectDecisionClock_OnNewBar()
  {
   ...
   MqlRates first_session_bar;
   MqlRates prior_month_bar;
   if(!QM_ReadBar(_Symbol, PERIOD_D1, 0, current_bar) ||
      !QM_ReadBar(_Symbol, PERIOD_D1, 1, first_session_bar) ||
      !QM_ReadBar(_Symbol, PERIOD_D1, 2, prior_month_bar) ||
      current_bar.time <= 0 || first_session_bar.time <= 0 ||
      prior_month_bar.time <= 0 ||
      prior_month_bar.time >= first_session_bar.time ||
      first_session_bar.time >= current_bar.time)
      return;
   ...
   const datetime normalized_first =
      Strategy_NormalizedLabel(first_session_bar.time, label_offset);
   const datetime normalized_before =
      Strategy_NormalizedLabel(prior_month_bar.time, label_offset);
   const int current_month_key =
      Strategy_MonthKeyForTime(normalized_current);
   const int before_month_key =
      Strategy_MonthKeyForTime(normalized_before);
   if(strategy_entry_session_ordinal != 2 ||
      strategy_formation_sessions != 1 ||
      current_month_key <= 0 || before_month_key <= 0 ||
      ...
      normalized_before <= 0 || normalized_first <= 0 ||
      normalized_before >= normalized_first ||
      normalized_first >= normalized_current ||
      Strategy_DateKeyForTime(normalized_current) ==
         Strategy_DateKeyForTime(normalized_first) ||
      Strategy_DateKeyForTime(normalized_first) ==
         Strategy_DateKeyForTime(normalized_before) ||
      Strategy_MonthKeyForTime(normalized_first) != current_month_key ||
      before_month_key == current_month_key ||
      Strategy_NextMonthKey(before_month_key) != current_month_key)
      return;
...
bool Strategy_LoadFirstSessionSignal(const int current_month_key,
                                     const int label_offset,
                                     int &direction,
                                     double &first_session_return)
  {
   direction = 0;
   first_session_return = 0.0;
   if(current_month_key <= 0 || strategy_formation_sessions != 1 ||
      (label_offset != 0 && label_offset != 86400))
      return false;

   MqlRates first_session_bar;
   MqlRates prior_month_bar;
   if(!QM_ReadBar(_Symbol, PERIOD_D1, 1, first_session_bar) ||
      !QM_ReadBar(_Symbol, PERIOD_D1, 2, prior_month_bar) ||
      first_session_bar.time <= 0 || prior_month_bar.time <= 0 ||
      prior_month_bar.time >= first_session_bar.time)
      return false;
   ...
   const int before_month_key =
      Strategy_MonthKeyForTime(normalized_before);
   if(normalized_first <= 0 || normalized_before <= 0 ||
      normalized_before >= normalized_first ||
      Strategy_MonthKeyForTime(normalized_first) != current_month_key ||
      before_month_key <= 0 || before_month_key == current_month_key ||
      Strategy_NextMonthKey(before_month_key) != current_month_key ||
      first_session_bar.open <= 0.0 || first_session_bar.close <= 0.0 ||
      !MathIsValidNumber(first_session_bar.open) ||
      !MathIsValidNumber(first_session_bar.close))
      return false;

   first_session_return =
      MathLog(first_session_bar.close / first_session_bar.open);
   if(!MathIsValidNumber(first_session_return))
      return false;

   if(first_session_return < 0.0)
      direction = 1;
   else if(first_session_return > 0.0)
      direction = -1;
   return true;
  }
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
     ...
     {
       g_signal_valid =
          Strategy_LoadFirstSessionSignal(g_decision_month_key,
                                          g_decision_label_offset,
                                          g_signal_direction,
                                          g_first_session_return);
       if(!g_signal_valid)
          g_signal_state = "endpoint_validation_failed";
       else if(g_signal_direction > 0)
          g_signal_state = "first_session_negative_long";
       else if(g_signal_direction < 0)
          g_signal_state = "first_session_positive_short";
       else if(g_first_session_return == 0.0)
   ...
   QM_LogEvent(QM_INFO,
               "MONTHLY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"first_session_return\":%.12e,\"state\":\"%s\"}",
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41027 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41027_WTI_MOPEN_REV1";
...
void Strategy_ManageOpenPosition()
  {
   ...
   // The first later normalized D1 label is the ordinary exit. Re-evaluate
   // every tick so a transient close failure is retried until flat.
...
int OnInit()
  {
   if(!SymbolSelect(g_strategy_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41027 ||
   ...
   g_attempt_state_key =
      StringFormat("QM5_41027_MOPEN_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41027\",\"ea\":\"wti-mopen-rev1\"}");
