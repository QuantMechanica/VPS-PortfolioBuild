...
#property description "QM5_41340 WTI/XNG Twelve-Month Sign-Divergence Trend"
...
// At the first processed D1 bar of each new broker month, intersect completed
// XTI/XNG history, reconstruct thirteen synchronized month ends, and require
// their exact twelve-month log-return signs to disagree. Trade only WTI in its
// own sign. XNGUSD.DWX is a read-only state veto and is never ordered.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41340;
...
input double strategy_sign_epsilon          = 1.0e-12;
input double strategy_chain_tolerance       = 1.0e-10;
...
bool Strategy_TwelveMonthTrend(const string symbol,
                               const datetime &common_times[],
                               const double &closes[],
                               double &trend_return)
  {
   trend_return = 0.0;
      ...
      if(month_count <= 0 || month_keys[month_count - 1] != month_key)
        {
         ...
         month_end_closes[month_count] = closes[i];
         ++month_count;
        }
      else
         month_end_closes[month_count - 1] = closes[i];
     }
   ...
   const int expected_previous_month =
      QM_CalendarPeriodKey(PERIOD_MN1, symbol, 1);
   ...
   return (MathIsValidNumber(chained_return) &&
           MathAbs(trend_return - chained_return) <= strategy_chain_tolerance);
  }
...
bool Strategy_LoadSignalState(const datetime decision_bar_time, int &signal)
  {
   ...
   if(!Strategy_TwelveMonthTrend(g_strategy_symbol,
                                 common_times,
                                 xti_closes,
                                 wti_return))
     {
      g_cache_state_reason = "invalid_wti_twelve_month_trend";
      return false;
     }
   g_cache_wti_return = wti_return;

   double xng_return = 0.0;
   if(!Strategy_TwelveMonthTrend(g_state_symbol,
                                 common_times,
                                 xng_closes,
                                 xng_return))
     {
      g_cache_state_reason = "invalid_xng_twelve_month_trend";
      return false;
     }
   g_cache_xng_return = xng_return;

   const bool wti_positive = (wti_return > strategy_sign_epsilon);
   const bool wti_negative = (wti_return < -strategy_sign_epsilon);
   const bool xng_positive = (xng_return > strategy_sign_epsilon);
   const bool xng_negative = (xng_return < -strategy_sign_epsilon);
   if((!wti_positive && !wti_negative) ||
      (!xng_positive && !xng_negative))
     {
      g_cache_state_reason = "annual_sign_tie";
      return true;
     }
   if((wti_positive && xng_positive) ||
      (wti_negative && xng_negative))
     {
      g_cache_state_reason = "annual_signs_agree";
      return true;
     }

   signal = wti_positive ? 1 : -1;
   g_cache_state_reason =
      (signal > 0) ? "qualified_wti_long_xng_down" :
                     "qualified_wti_short_xng_up";
   return true;
  }
...
void Strategy_DetectMonthlyRebalance_OnNewBar()
  {
   ...
   if(!QM_ReadBar(_Symbol, PERIOD_D1, 0, decision_bar) ||
      decision_bar.time <= 0)
      return;
   const long elapsed = (long)(TimeCurrent() - decision_bar.time);
   const long grace = (long)strategy_entry_grace_minutes * 60;
...
void Strategy_PrepareMonthlySignal()
  {
   ...
   QM_LogEvent(QM_INFO,
               "MONTHLY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"valid\":%s,\"signal\":%d,\"wti_12m\":%.12e,\"xng_12m\":%.12e,\"common_closes\":%d,\"state\":\"%s\"}",
                            g_cache_month_key,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41340 ||
   ...
   if(strategy_trend_months != 12 ||
      strategy_history_bars_d1 != 500 ||
      MathAbs(strategy_sign_epsilon - 1.0e-12) > 1.0e-18 ||
      MathAbs(strategy_chain_tolerance - 1.0e-10) > 1.0e-16 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41340_WTI_XNG_DIVTREND";
   ...
   req.reason = (g_cache_signal > 0) ?
                "XTI_UP_XNG_DOWN_LONG" : "XTI_DOWN_XNG_UP_SHORT";
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41340_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41340\",\"ea\":\"wti-xng-divtrend\",\"xng\":\"read_only\"}");
