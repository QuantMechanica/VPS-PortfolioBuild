...
#property description "QM5_41365 XTI/XNG Weekly Decoupling-Shock Reversion"
...
// buy the loser and sell the winner in an equal-notional opposite-leg package.
// Consume one attempt per week and flatten at the next week boundary.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41365;
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41365 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41365_XTIXNG_DECOUPLE_RV";
...
bool Strategy_LoadWeeklyDecoupling(const int current_week,
                                    int &direction)
  {
   ...
   if(g_wti_week_return > strategy_signal_epsilon &&
      ...
      direction = -1; // WTI won and XNG lost: sell XTI, buy XNG.
   else if(g_wti_week_return < -strategy_signal_epsilon &&
           g_xng_week_return > strategy_signal_epsilon)
      direction = 1; // XNG won and WTI lost: buy XTI, sell XNG.
   return true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41365_XTIXNG_DECOUPLE_RV_HOST";
   ...
   if(Strategy_OpenOwnedPositionCount() > 0 ||
   ...
   else
     {
      ...
      if(!signal_valid)
         ...
         g_signal_state = "wti_positive_xng_negative_sell_winner_buy_loser";
      else if(direction > 0)
         g_signal_state = "wti_negative_xng_positive_buy_loser_sell_winner";
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41365\",\"ea\":\"xtixng-decouple-rv\"}");
