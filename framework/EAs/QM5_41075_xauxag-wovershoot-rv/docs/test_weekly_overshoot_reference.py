"""Deterministic reference checks for QM5_41075 XAU/XAG weekly reversal overshoot."""
...
def weekly_overshoot_signal(
    current_week_key: int,
    completed_newest_first: list[PairBar],
) -> tuple[bool, int, float, float, tuple[float, float, float]]:
    """Mirror strict sign opposition and newest absolute dominance."""
...
class WeeklyOvershootReferenceTest(unittest.TestCase):
    def test_positive_reversal_overshoot_sells_xau(self) -> None:
        bars = pair_bars((5.20, 5.05, 5.10))
        valid, direction, new_value, old_value, _ = weekly_overshoot_signal(20260824, bars)
        self.assertTrue(valid)
        self.assertEqual(direction, -1)
        self.assertAlmostEqual(new_value, 0.15)
        self.assertAlmostEqual(old_value, -0.05)

    def test_negative_reversal_overshoot_buys_xau(self) -> None:
        bars = pair_bars((4.80, 4.95, 4.90))
        valid, direction, new_value, old_value, _ = weekly_overshoot_signal(20260824, bars)
        self.assertTrue(valid)
        self.assertEqual(direction, 1)
        self.assertAlmostEqual(new_value, -0.15)
    ...
    def test_equality_nondominance_same_sign_and_zero_are_flat(self) -> None:
        cases = (
            (5.20, 5.10, 5.20),
            (5.15, 5.10, 5.20),
            (5.20, 5.10, 5.00),
        ...
        for spreads in cases:
            with self.subTest(spreads=spreads):
                valid, direction, *_ = weekly_overshoot_signal(20260824, pair_bars(spreads))
                self.assertTrue(valid)
    ...
    def test_endpoint_validation_rejects_bad_order_gap_and_price(self) -> None:
        ...
        self.assertFalse(weekly_overshoot_signal(20260824, bars)[0])
        bars = pair_bars((5.20, 5.05, 5.10))
