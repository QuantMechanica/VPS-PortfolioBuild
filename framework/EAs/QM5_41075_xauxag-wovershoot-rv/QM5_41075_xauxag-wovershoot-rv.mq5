...
#property description "QM5_41075 XAU/XAG Weekly Reversal-Overshoot Reversion"
...
// pairs and form two adjacent changes in ln(XAU)-ln(XAG). When the returns
// have strict opposite signs and the newest absolute move is strictly larger,
// fade that newest reversal with an equal-notional opposite-leg package.
// Consume one attempt per week and flatten at the next week boundary.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41075;
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41075 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_WOVERSHOOT_ATTEMPT_WEEK", qm_ea_id);
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41075_XAUXAG_WOVERSHOOT_RV";
...
bool Strategy_LoadWeeklyOvershoot(const int current_week,
                                  int &direction)
  {
   direction = 0;
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41075_XAUXAG_WOVERSHOOT_RV_HOST";
   ...
   if(Strategy_OpenOwnedPositionCount() > 0 ||
     ...
     {
      signal_valid =
         Strategy_LoadWeeklyOvershoot(g_signal_week_key, direction);
      if(!signal_valid)
         g_signal_state = "endpoint_validation_failed";
      else if(direction < 0)
         g_signal_state = "positive_reversal_overshoot_short_xau";
      else if(direction > 0)
         g_signal_state = "negative_reversal_overshoot_long_xau";
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG weekly reversal-overshoot package exits at the next broker week"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41075\",\"ea\":\"xauxag-wovershoot-rv\"}");
