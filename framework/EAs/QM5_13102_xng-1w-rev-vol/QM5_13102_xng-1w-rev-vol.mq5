...
#property description "QM5_13102 XNG one-week high-volatility reversal"
...
//   - direction fades the prior 5 closed D1 return
//   - current 20-D1 realized volatility must rank above a configured percentile
//   - one entry per broker week; ATR stop, time exit, neutral-return exit
// Runtime uses MT5 OHLC/broker calendar only; no EIA, CFTC, curve, API, CSV,
// inventory, external feed, or ML.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 13102;
...
input int    strategy_reversal_lookback_days = 5;
input double strategy_min_week_return_pct    = 2.00;
input int    strategy_vol_window_d1          = 20;
input int    strategy_vol_rank_lookback_d1   = 120;
input double strategy_min_vol_pctile         = 65.0;
input int    strategy_atr_period             = 20;
input double strategy_atr_sl_mult            = 2.25;
input int    strategy_hold_days              = 5;
input double strategy_exit_neutral_pct       = 0.25;
...
bool Strategy_CopyClosedD1Closes(double &closes[], const int bars_needed)
  {
   ...
   const int copied = CopyClose(_Symbol, PERIOD_D1, 1, bars_needed, closes); // perf-allowed: bounded D1 vector behind QM_IsNewBar for structural weekly reversal and volatility rank.
...
bool Strategy_LoadReturnOnly(double &return_pct)
  {
   return_pct = 0.0;
   const int lookback = MathMax(2, strategy_reversal_lookback_days);
   double closes[];
...
bool Strategy_LoadSignalState(int &direction,
                              ...
                              int &week_key)
  {
   ...
   if(!Strategy_VolPercentile(closes, current_vol, vol_pctile))
      return false;
   if(vol_pctile < strategy_min_vol_pctile)
      return false;
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      if(pos_type != POSITION_TYPE_BUY && pos_type != POSITION_TYPE_SELL)
         should_close = true;
      if(has_return && pos_type == POSITION_TYPE_BUY && return_pct >= -strategy_exit_neutral_pct)
         should_close = true;
      if(has_return && pos_type == POSITION_TYPE_SELL && return_pct <= strategy_exit_neutral_pct)
         should_close = true;
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(qm_magic_slot_offset != 0)
      return true;
   if(strategy_reversal_lookback_days < 2 || strategy_reversal_lookback_days > 30)
      return true;
   ...
   if(strategy_vol_rank_lookback_d1 < 40 || strategy_vol_rank_lookback_d1 > 260)
      return true;
   if(strategy_min_vol_pctile <= 0.0 || strategy_min_vol_pctile >= 100.0)
      return true;
   if(strategy_atr_period <= 1 || strategy_atr_sl_mult <= 0.0)
      return true;
   if(strategy_hold_days <= 0)
      return true;
   if(strategy_exit_neutral_pct < 0.0)
      return true;
   return false;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_13102_XNG_1W_REV_VOL";
   ...
   req.tp = 0.0;
   req.reason = (direction > 0) ? "XNG_1W_HIGHVOL_REV_LONG" : "XNG_1W_HIGHVOL_REV_SHORT";
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_13102\",\"ea\":\"xng-1w-rev-vol\"}");
...
void OnTick()
  {
   ...
   // This strategy's management exits depend on completed D1 returns. Keep the
   // bounded CopyClose work behind the single framework new-bar gate. Friday
   // close remains per-tick above, and news clearance below gates entries only.
   if(!QM_IsNewBar())
      return;
