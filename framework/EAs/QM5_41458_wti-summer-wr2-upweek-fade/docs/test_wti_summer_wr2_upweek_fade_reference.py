"""Deterministic reference checks for QM5_41458 WTI summer WR2 up-week fade."""
...
class WtiSummerWR2UpWeekFadeReferenceTest(unittest.TestCase):
    current = date(2026, 7, 13)
    new = current - timedelta(days=7)
    old = current - timedelta(days=14)

    def bars(self, close: float = 102.0, new_range: float = 12.0,
    ...
    def test_expansion_positive_week_is_short(self) -> None:
    ...
    def test_three_to_five_sessions_are_valid(self) -> None:
        for count in (3, 4, 5):
            newest = package(self.new, count, 100, 106, 94, 102)
