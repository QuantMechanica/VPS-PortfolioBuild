...
bool Strategy_BollingerTouch(const bool long_side, double &swing_price)
  {
   ...
   for(int shift = 1; shift <= strategy_touch_lookback_bars; ++shift)
     {
      ...
      const double upper = QM_BB_Upper(_Symbol, PERIOD_CURRENT, strategy_bb_period,
                                       strategy_bb_deviation, shift, PRICE_CLOSE);
      const double low = iLow(_Symbol, PERIOD_CURRENT, shift);
      const double high = iHigh(_Symbol, PERIOD_CURRENT, shift);
