...
#property description "QM5_20228 USDCAD GBPJPY Cointegration"
...
//
// Card:
//   strategy-seeds/cards/approved/QM5_20228_usdcad-gbpjpy_card.md
//
// Fixed spread:
//   ln(USDCAD.DWX) - beta * ln(GBPJPY.DWX)
//
...
input double strategy_beta              = -0.231842927;
...
bool Strategy_EnsureBasketScope()
  {
   ...
   string allowed[3] = {"USDCAD.DWX", "GBPJPY.DWX", "USDJPY.DWX"};
   for(int i = 0; i < 3; ++i)
...
bool Strategy_RefreshSpreadState()
  {
   ...
   for(int i = 0; i < history_count; ++i)
     {
      if(usdcad_time[i] <= 0 || usdcad_time[i] != gbpjpy_time[i])
         return false;
      if(usdcad[i] <= 0.0 || gbpjpy[i] <= 0.0)
         return false;

      spreads[i] =
         MathLog(usdcad[i]) - strategy_beta * MathLog(gbpjpy[i]);
...
bool Strategy_PreparePair(const int spread_direction,
                          QM_EntryRequest &host_req)
  {
   ...
   const double weight_sum = usdcad_weight + gbpjpy_weight;
   if(weight_sum <= 0.0)
      return false;
   ...
   const double gbpjpy_lots =
      Strategy_LotsForLeg(g_leg_gbpjpy, gbpjpy_weight, usdcad_weight);
   if(usdcad_lots <= 0.0 || gbpjpy_lots <= 0.0)
      return false;
   ...
   const string reason =
      long_spread ? "QM5_20228_LONG_SPREAD_Z_LT_NEG_ENTRY"
                  : "QM5_20228_SHORT_SPREAD_Z_GT_POS_ENTRY";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20228_PAIR_HOST";
