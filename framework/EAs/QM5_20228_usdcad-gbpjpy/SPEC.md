# QM5_20228_usdcad-gbpjpy — Strategy Spec
...
`ln(USDCAD.DWX) - (-0.231842927) * ln(GBPJPY.DWX)`. Score the newest spread
...
| `strategy_beta` | -0.231842927 | fixed | DEV-fitted structural hedge coefficient |
...
- `GBPJPY.DWX` — fixed companion and second traded leg.
- `USDJPY.DWX` — conversion-history dependency for GBPJPY P/L; order-free
  and without a magic slot.
...
| Bar gating | `QM_IsNewBar(USDCAD.DWX, PERIOD_D1)` once per signal bar |
...
| Typical hold time | multi-day; scan half-life 65.507 D1 bars |
| Expected drawdown profile | high and regime-sensitive; conservative 30% prior |
| Regime preference | USDCAD/GBPJPY fixed-residual mean reversion |
| Win rate target | unknown; Q02 is the first platform-economic judge |

The fixed scan recorded DEV net Sharpe `0.011529`, OOS net Sharpe `-0.194853`,
and OOS return `-1.353675%`. These are adverse priors, not a promotion claim.
...
Chan supplies the method but makes no USDCAD/GBPJPY performance claim.
...
`1.0` and `0.231842927`; each leg is bounded by its ATR stop. If the GBPJPY
...
|---|---|---|---|
| v1 | 2026-08-05 | Initial approved-card build | Next-rank-50 non-duplicate fixed-scan FX basket |
