...
#property description "QM5_11697 Paul Langer BB20 M5 scalp with D1 trend"
...
input int    strategy_pending_expire_bars       = 1;
input int    strategy_session_start_hour_broker = 8;
input int    strategy_session_end_hour_broker   = 18;
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(strategy_max_spread_pips > 0 && ask > bid)
     {
   ...
   else if(strategy_session_start_hour_broker > strategy_session_end_hour_broker)
     {
      if(dt.hour < strategy_session_start_hour_broker && dt.hour >= strategy_session_end_hour_broker)
         return true;
     }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   const bool bullish_trend = (d1_close > d1_sma);
   const bool bearish_trend = (d1_close < d1_sma);
   ...
   if(bullish_trend && low1 <= lower && close1 > open1)
     {
      const double entry = QM_StopRulesNormalizePrice(_Symbol, high1 + offset);
      if(entry <= ask)
         return false;
      ...
      req.reason = "PL_BB20_M5_LONG_STOP";
      req.expiration_seconds = expiry_seconds;
      return (req.sl > 0.0 && req.sl < entry && req.tp > entry);
     }

   if(bearish_trend && high1 >= upper && close1 < open1)
     {
      const double entry = QM_StopRulesNormalizePrice(_Symbol, low1 - offset);
      if(entry >= bid)
         return false;
      ...
      req.sl = QM_StopATRFromValue(_Symbol, req.type, entry, atr, strategy_atr_sl_mult);
      req.tp = QM_TakeFixedPips(_Symbol, req.type, entry, strategy_take_profit_pips);
      req.reason = "PL_BB20_M5_SHORT_STOP";
      req.expiration_seconds = expiry_seconds;
...
bool Strategy_ExitSignal()
  {
   // Card exits are protective SL, fixed TP, pending-order expiry, break-even,
   // and framework Friday close. No discretionary signal close is specified.
   return false;
  }
