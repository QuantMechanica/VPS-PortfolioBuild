...
#property description "QM5_10161 TradingView Multi-Timeframe MACD Confirm"
...
input ENUM_TIMEFRAMES strategy_confirm_tf = PERIOD_H4;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   const double sig_prev = QM_MACD_Signal(_Symbol, strategy_signal_tf,
                                          ...
                                          strategy_macd_signal,
                                          2);
   const double htf_macd = QM_MACD_Main(_Symbol, strategy_confirm_tf,
                                        strategy_macd_fast,
                                        strategy_macd_slow,
                                        strategy_macd_signal,
                                        1);
   const double htf_sig = QM_MACD_Signal(_Symbol, strategy_confirm_tf,
                                         strategy_macd_fast,
                                         strategy_macd_slow,
                                         strategy_macd_signal,
                                         1);
   ...
   if(!((bullish_cross && htf_bullish) || (bearish_cross && htf_bearish)))
      return false;
   ...
   req.reason = bullish_cross ? "TV_MTF_MACD_LONG" : "TV_MTF_MACD_SHORT";
   return true;
  }
...
void Strategy_ManageOpenPosition()
  {
   // Baseline card has no break-even, partial, or trailing management.
  }
...
bool Strategy_ExitSignal()
  {
   ...
   if(have_buy && macd_prev >= sig_prev && macd_now < sig_now)
      return true;
   if(!have_buy && macd_prev <= sig_prev && macd_now > sig_now)
      return true;
