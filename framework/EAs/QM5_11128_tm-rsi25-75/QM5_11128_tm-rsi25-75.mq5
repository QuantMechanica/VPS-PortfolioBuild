...
#property description "QM5_11128 TradingMarkets RSI 25/75 ETF pullback"
...
input double strategy_long_entry_rsi    = 25.0;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(close_1 <= 0.0 || sma_1 <= 0.0 || rsi_1 <= 0.0)
      return false;
   ...
   if(close_1 > sma_1 && rsi_1 < strategy_long_entry_rsi)
     {
      req.type = QM_BUY;
      req.price = 0.0;
      req.sl = QM_StopATR(_Symbol, req.type, ask, strategy_atr_period, strategy_atr_sl_mult);
      req.reason = "RSI25_75_LONG_PULLBACK";
      return (req.sl > 0.0);
     }

   if(strategy_enable_shorts && close_1 < sma_1 && rsi_1 > strategy_exit_rsi)
     {
      req.type = QM_SELL;
      req.price = 0.0;
      req.sl = QM_StopATR(_Symbol, req.type, bid, strategy_atr_period, strategy_atr_sl_mult);
      req.reason = "RSI25_75_SHORT_PULLBACK";
      return (req.sl > 0.0);
     }
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      if(pos_type == POSITION_TYPE_BUY && rsi_1 > strategy_exit_rsi)
         return true;
      if(pos_type == POSITION_TYPE_SELL && rsi_1 < strategy_long_entry_rsi)
         return true;
