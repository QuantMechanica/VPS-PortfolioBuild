...
#property description "QM5_41285 XAU/XAG Monthly Exact Jonckheere-Terpstra Reversion"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_41285 - XAU/XAG Monthly Exact Jonckheere-Terpstra Reversion
// -----------------------------------------------------------------------------
...
// changes into three fixed chronological blocks of four, reject relative
// pooled ties, and compute the classic Jonckheere-Terpstra ordered-win count
// across all 48 cross-block comparisons. Enumerate every
// C(12,4)*C(8,4)=34,650 labeled strict-rank allocation for an inclusive exact
// two-sided distance-from-24 tail. A qualified high/low score is faded with an
// opposite, equal-notional package held to the next month. Runtime is
// deterministic, completed-price-only, and one consumed attempt per month.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41285;
...
input int    strategy_group_count              = 3;
input int    strategy_group_size               = 4;
input int    strategy_comparison_count         = 48;
input int    strategy_assignment_count         = 34650;
input int    strategy_tail_count_max           = 18034;
input int    strategy_score_center             = 24;
input int    strategy_lower_score_max          = 19;
input int    strategy_upper_score_min          = 29;
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41285 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_MJT_RV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41285_XAUXAG_MJT_RV";
...
int Strategy_PopCount12(const int mask)
  {
   if(mask < 0 || mask >= (1 << 12))
      return -1;
   int count = 0;
...
bool Strategy_JonckheereTerpstraInputInvariant()
  {
   return (strategy_return_count == 12 &&
           strategy_group_count == 3 &&
           strategy_group_size == 4 &&
           strategy_return_count ==
              strategy_group_count * strategy_group_size &&
           strategy_comparison_count == 48 &&
           strategy_score_center * 2 == strategy_comparison_count &&
           strategy_assignment_count == 34650 &&
           strategy_tail_count_max == 18034 &&
           strategy_lower_score_max == 19 &&
           strategy_upper_score_min == 29 &&
           strategy_score_center - strategy_lower_score_max ==
              strategy_upper_score_min - strategy_score_center);
  }

bool Strategy_JonckheereTerpstraScoreFromLabels(const int &labels[],
                                                int &score)
  {
   score = 0;
   if(ArraySize(labels) != strategy_return_count ||
      !Strategy_JonckheereTerpstraInputInvariant())
      return false;

   int group_totals[3] = {0, 0, 0};
   for(int index = 0; index < ArraySize(labels); ++index)
     {
      const int label = labels[index];
      if(label < 0 || label >= strategy_group_count)
         return false;
      ++group_totals[label];
     }
   for(int group = 0; group < strategy_group_count; ++group)
      if(group_totals[group] != strategy_group_size)
         return false;

   // labels[] is ordered by strictly ascending pooled return. Each lower-rank
   // observation from an earlier chronological group that precedes a
   // higher-rank observation from a later group contributes one ordered win.
   for(int lower_rank = 0; lower_rank < strategy_return_count; ++lower_rank)
      for(int higher_rank = lower_rank + 1;
          higher_rank < strategy_return_count;
          ++higher_rank)
         if(labels[lower_rank] < labels[higher_rank])
            ++score;
   return (score >= 0 && score <= strategy_comparison_count);
  }

bool Strategy_SortPooledReturns(double &values[], int &labels[])
  {
   if(ArraySize(values) != strategy_return_count ||
      ArraySize(labels) != strategy_return_count)
      return false;
      ...
      while(cursor >= 0 && values[cursor] > value)
        {
         ...
         labels[cursor + 1] = labels[cursor];
         --cursor;
        }
      values[cursor + 1] = value;
      labels[cursor + 1] = label;
     }
   for(int index = 1; index < strategy_return_count; ++index)
      if(!MathIsValidNumber(values[index - 1]) ||
         !MathIsValidNumber(values[index]) ||
         values[index - 1] >= values[index])
         return false; // Relative ties were rejected before this strict-order check.
   return MathIsValidNumber(values[0]);
  }

bool Strategy_LoadMonthlyJonckheereTerpstraSignal(
      ...
      int &direction)
  {
   ...
   double pooled_values[12];
   int pooled_labels[12];
      ...
      for(int right = left + 1; right < strategy_return_count; ++right)
        {
         const double tie_tolerance =
            ...
            MathMax(1.0,
                    MathMax(MathAbs(relative_returns[left]),
                            MathAbs(relative_returns[right])));
         if(!MathIsValidNumber(tie_tolerance))
            return false;
         if(MathAbs(relative_returns[left] - relative_returns[right]) <=
            tie_tolerance)
           {
            tie_detected = true;
            return true; // A valid relative pooled tie consumes the month flat.
           }
        }
     }
   ...
   for(int index = 0; index < strategy_return_count; ++index)
     {
      pooled_values[index] = relative_returns[index];
      pooled_labels[index] = index / strategy_group_size;
     }
   if(!Strategy_SortPooledReturns(pooled_values, pooled_labels))
      return false;

   for(int index = 0; index < strategy_return_count; ++index)
     {
      label_path += IntegerToString(pooled_labels[index]);
     }

   if(!Strategy_JonckheereTerpstraScoreFromLabels(pooled_labels,
                                                  observed_score))
      return false;
   score_deviation =
      Strategy_AbsInt(observed_score - strategy_score_center);

   int four_rank_masks[495];
   int four_rank_mask_count = 0;
   for(int mask = 0; mask < (1 << strategy_return_count); ++mask)
     {
      if(Strategy_PopCount12(mask) != strategy_group_size)
         continue;
      if(four_rank_mask_count >= ArraySize(four_rank_masks))
         return false;
      four_rank_masks[four_rank_mask_count] = mask;
      ++four_rank_mask_count;
     }
   if(four_rank_mask_count != ArraySize(four_rank_masks))
      return false;

   int permutation_labels[12];
   for(int group_zero_index = 0;
       group_zero_index < four_rank_mask_count;
       ++group_zero_index)
     {
      const int group_zero_mask = four_rank_masks[group_zero_index];
      for(int group_one_index = 0;
          group_one_index < four_rank_mask_count;
          ++group_one_index)
        {
         const int group_one_mask = four_rank_masks[group_one_index];
         if((group_zero_mask & group_one_mask) != 0)
            continue;
         for(int index = 0; index < strategy_return_count; ++index)
           {
            if((group_zero_mask & (1 << index)) != 0)
               permutation_labels[index] = 0;
            else if((group_one_mask & (1 << index)) != 0)
               permutation_labels[index] = 1;
            else
               permutation_labels[index] = 2;
           }
         int permutation_score = 0;
         if(!Strategy_JonckheereTerpstraScoreFromLabels(
               permutation_labels,
               permutation_score))
            return false;
         ++assignment_count;
         if(Strategy_AbsInt(permutation_score - strategy_score_center) >=
            score_deviation)
            ++tail_count;
        }
     }
   if(assignment_count != strategy_assignment_count ||
      tail_count <= 0 || tail_count > assignment_count)
      return false;

   const bool qualifies_by_tail =
      (tail_count <= strategy_tail_count_max);
   const bool qualifies_by_score =
      (observed_score <= strategy_lower_score_max ||
       observed_score >= strategy_upper_score_min);
   if(qualifies_by_tail != qualifies_by_score)
      return false;
   if(!qualifies_by_tail)
      return true;

   // direction > 0 means BUY XAU / SELL XAG. An unusually low ordered-win
   // count is faded long XAU; an unusually high count is faded short XAU.
   // Neither score sign nor magnitude scales the fixed package risk.
   direction =
      (observed_score >= strategy_upper_score_min) ? -1 : 1;
   return true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41285_XAUXAG_JT_RV_HOST";
   ...
   string label_path = "";
   const bool valid =
      Strategy_LoadMonthlyJonckheereTerpstraSignal(
         g_signal_month_key,
         month_count,
   ...
   string state = "monthly_jt_signal_failed";
   if(valid && direction > 0)
      state = "qualified_low_j_fade_long_xau";
   else if(valid && direction < 0)
      state = "qualified_high_j_fade_short_xau";
   else if(valid && tie_detected)
      state = "relative_pooled_change_tie_flat";
   else if(valid && tail_count > strategy_tail_count_max)
      state = "exact_tail_above_locked_cutoff_flat";
   else if(valid && observed_score == strategy_score_center)
      state = "center_j_score_flat";
   else if(valid)
      state = "nonqualifying_exact_jt_flat";

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"month_count\":%d,\"newest_endpoint_time\":%I64d,\"observed_j\":%d,\"score_deviation\":%d,\"assignment_count\":%d,\"tail_count\":%d,\"tie_detected\":%s,\"direction\":%d,\"ratios\":\"%s\",\"returns\":\"%s\",\"pooled_group_labels\":\"%s\",\"state\":\"%s\"}",
                            g_signal_month_key,
                            month_count,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG monthly exact Jonckheere-Terpstra reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41285\",\"ea\":\"xauxag-mjt-rv\"}");
