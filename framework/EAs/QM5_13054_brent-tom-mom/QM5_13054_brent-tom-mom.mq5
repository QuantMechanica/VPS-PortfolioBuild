...
#property description "QM5_13054 Brent Turn-Of-Month Momentum"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_13054 - Brent Turn-Of-Month Momentum
// -----------------------------------------------------------------------------
...
int Strategy_DayFromDayKey(const int day_key)
  {
   return day_key % 100;
  }
...
int Strategy_CurrentDayKey()
  {
   int day_key = QM_CalendarPeriodKey(PERIOD_D1, _Symbol, 0);
   if(day_key > 0)
      return day_key;
...
bool Strategy_IsTomWindowKey(const int day_key, int &cycle_key)
  {
   cycle_key = 0;
   if(day_key <= 0)
      return false;

   const int year = Strategy_YearFromDayKey(day_key);
   const int month = Strategy_MonthFromDayKey(day_key);
   const int day = Strategy_DayFromDayKey(day_key);
   ...
   if(pre_days > 0 && day >= days_in_month - pre_days + 1)
     {
      cycle_key = Strategy_MonthKey(year, month);
      return true;
     }

   if(post_days > 0 && day <= post_days)
     {
      cycle_key = Strategy_PreviousMonthKey(year, month);
      return true;
     }
...
bool Strategy_LoadMomentumState(double &return_pct,
                                ...
                                const bool require_tom_window)
  {
   ...
   const int day_key = Strategy_CurrentDayKey();
   const bool in_tom = Strategy_IsTomWindowKey(day_key, cycle_key);
...
void Strategy_CloseOpenPositionsIfNeeded()
  {
   ...
   const bool in_tom = Strategy_IsTomWindowKey(Strategy_CurrentDayKey(), current_cycle_key);
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_13054_BRENT_TOM_MOM";
   ...
   req.reason = (direction > 0) ? "BRENT_TOM_MOM_LONG" : "BRENT_TOM_MOM_SHORT";
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_13054\",\"ea\":\"brent-tom-mom\"}");
...
void OnTick()
  {
   ...
   // News blackout gates NEW entries only; management above keeps enforcing
   // strategy time exits through news windows.
