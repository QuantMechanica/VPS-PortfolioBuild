# QM5_20220_usdcad-audjpy — Strategy Spec
...
`ln(USDCAD.DWX) - (-0.186232670) * ln(AUDJPY.DWX)`. Score the newest spread
...
| `strategy_beta` | -0.186232670 | fixed | DEV-fitted structural hedge coefficient |
...
- `USDCAD.DWX` — fixed tester host and first traded leg.
...
| Typical hold time | multi-day; scan half-life 73.380 D1 bars |
| Expected drawdown profile | high and regime-sensitive; conservative 30% prior |
| Regime preference | USDCAD/AUDJPY fixed-residual mean reversion |
| Win rate target | unknown; Q02 is the first platform-economic judge |

The fixed scan recorded DEV net Sharpe `0.285291`, OOS net Sharpe `-0.060279`,
and OOS return `-0.374459%`. The negative OOS score and slow half-life are
...
`strategy-seeds/cards/approved/QM5_20220_usdcad-audjpy_card.md`

Chan supplies the deterministic pair-trading method but makes no performance
claim for USDCAD/AUDJPY.
...
`1.0` and `0.186232670`; each leg is independently bounded by its ATR stop.
...
|---|---|---|---|
| v1 | 2026-08-05 | Initial build from approved card | Next-rank-42 non-duplicate fixed-scan FX basket |
