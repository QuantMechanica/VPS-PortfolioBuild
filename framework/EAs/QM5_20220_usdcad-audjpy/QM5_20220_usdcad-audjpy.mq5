...
#property description "QM5_20220 USDCAD AUDJPY Cointegration"
...
//
// Fixed spread:
//   ln(USDCAD.DWX) - beta * ln(AUDJPY.DWX)
//
...
input double strategy_beta              = -0.186232670;
...
bool Strategy_EnsureBasketScope()
  {
   ...
   string allowed[4] = {"USDCAD.DWX", "AUDJPY.DWX", "AUDUSD.DWX", "USDJPY.DWX"};
   for(int i = 0; i < 4; ++i)
...
bool Strategy_RefreshSpreadState()
  {
   ...
   for(int i = 0; i < history_count; ++i)
     {
      if(usdcad_time[i] <= 0 || usdcad_time[i] != audjpy_time[i])
         return false;
      if(usdcad[i] <= 0.0 || audjpy[i] <= 0.0)
         return false;

      spreads[i] =
         MathLog(usdcad[i]) - strategy_beta * MathLog(audjpy[i]);
...
bool Strategy_PreparePair(const int spread_direction,
                          QM_EntryRequest &host_req)
  {
   ...
   const double weight_sum = usdcad_weight + audjpy_weight;
   if(weight_sum <= 0.0)
      return false;
   ...
   const double audjpy_lots =
      Strategy_LotsForLeg(g_leg_audjpy, audjpy_weight, usdcad_weight);
   if(usdcad_lots <= 0.0 || audjpy_lots <= 0.0)
      return false;
   ...
   const string reason =
      long_spread ? "QM5_20220_LONG_SPREAD_Z_LT_NEG_ENTRY"
                  : "QM5_20220_SHORT_SPREAD_Z_GT_POS_ENTRY";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20220_PAIR_HOST";
