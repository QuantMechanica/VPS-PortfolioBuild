...
#property description "QM5_12749 Edge Lab NZDUSD AUDJPY Cointegration"
...
input double strategy_beta              = 0.07;
...
bool Strategy_IsHostSymbol()
  {
   return (_Symbol == g_leg_nzdusd || _Symbol == g_leg_audjpy);
  }
...
bool Strategy_EnsureBasketScope()
  {
   ...
   // AUDJPY needs AUD and JPY conversion history for USD-denominated tester
   // accounting; NZDUSD itself already settles in USD.
   string allowed[4] = {"NZDUSD.DWX", "AUDJPY.DWX", "AUDUSD.DWX", "USDJPY.DWX"};
   for(int i = 0; i < 4; ++i)
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(!Strategy_EnsureBasketScope())
      return false;
   if(!QM_SymbolAssertOrLog(g_leg_nzdusd) || !QM_SymbolAssertOrLog(g_leg_audjpy))
      return false;
   ...
   if(CopyClose(g_leg_nzdusd, PERIOD_D1, 1, lookback, nzdusd) != lookback) // perf-allowed: Strategy_EntrySignal is called only after the framework QM_IsNewBar gate.
      return false;
   if(CopyClose(g_leg_audjpy, PERIOD_D1, 1, lookback, audjpy) != lookback) // perf-allowed: Strategy_EntrySignal is called only after the framework QM_IsNewBar gate.
      return false;
   ...
   for(int i = 0; i < lookback; ++i)
     {
      if(nzdusd[i] <= 0.0 || audjpy[i] <= 0.0)
         return false;
      spreads[i] = MathLog(nzdusd[i]) - strategy_beta * MathLog(audjpy[i]);
...
bool Strategy_OpenPair(const int spread_direction)
  {
   ...
   const double weight_sum = nzdusd_weight + audjpy_weight;
   if(weight_sum <= 0.0)
      return false;
   ...
   const string reason = long_spread ? "QM5_12749_LONG_SPREAD_Z_LT_NEG_ENTRY"
                                     : "QM5_12749_SHORT_SPREAD_Z_GT_POS_ENTRY";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12749_PAIR_HOST";
