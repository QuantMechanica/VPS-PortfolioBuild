...
#property description "QM5_41356 XAU/XAG Monthly Fixed-Tail Winsorized Reversion"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_41356 - XAU/XAG Monthly Fixed-Tail Winsorized Reversion
// -----------------------------------------------------------------------------
...
// returns, sort them, replace the two lowest by sorted index 2 and the two
// highest by sorted index 9, and average all twelve capped observations. Fade
// that sign with an opposed, equal-target-notional package held to the next
// month. Runtime is deterministic, completed-price-only, and one consumed
// attempt per month.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41356;
...
input int    strategy_winsor_each_tail        = 2;
input int    strategy_winsor_divisor          = 12;
...
bool Strategy_InputsValid()
  {
   return (Strategy_DoubleInputsFinite() &&
            ...
            g_leg_xau != g_leg_xag &&
            qm_ea_id == 41356 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
            strategy_xag_symbol == "XAGUSD.DWX" &&
            strategy_endpoint_count == 13 &&
            strategy_return_count == 12 &&
            strategy_winsor_each_tail == 2 &&
            strategy_winsor_divisor == 12 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_MWINSOR2_RV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41356_XAUXAG_MWINSOR2_RV";
...
bool Strategy_WinsorizedMean(const double &returns[],
                             double &location)
  {
   location = 0.0;
   if(ArraySize(returns) != 12 || strategy_return_count != 12 ||
      strategy_winsor_each_tail != 2 || strategy_winsor_divisor != 12)
      return false;
   ...
   if(!Strategy_Sort12(sorted))
      return false;
   if(strategy_winsor_each_tail != 2 ||
      strategy_winsor_divisor != strategy_return_count)
      return false;

   double capped_sum = 0.0;
   for(int index = 0; index < strategy_return_count; ++index)
     {
      double capped = sorted[index];
      if(index < strategy_winsor_each_tail)
         capped = sorted[strategy_winsor_each_tail];
      else if(index >= strategy_return_count - strategy_winsor_each_tail)
         capped = sorted[strategy_return_count - strategy_winsor_each_tail - 1];
      if(!MathIsValidNumber(capped))
         return false;
      capped_sum += capped;
     }
   location = capped_sum / (double)strategy_winsor_divisor;
   return MathIsValidNumber(location);
  }

bool Strategy_LoadMonthlyWinsorizedSignal(
      ...
      int &direction)
  {
   ...
   if(StringLen(return_path) <= 0)
      return false;
   if(!Strategy_WinsorizedMean(relative_returns, location))
      return false;
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41356_XAUXAG_MWINSOR2_RV_HOST";
   ...
   string return_path = "";
   const bool valid =
      Strategy_LoadMonthlyWinsorizedSignal(
         g_signal_month_key,
         month_count,
   ...
   string state = "monthly_winsorized_signal_failed";
   if(valid && direction > 0)
      state = "negative_winsorized_mean_fade_long_xau";
   else if(valid && direction < 0)
      state = "positive_winsorized_mean_fade_short_xau";
   else if(valid && MathAbs(location) <= strategy_signal_epsilon)
      state = "winsorized_mean_inside_epsilon_flat";
   else if(valid)
      state = "nonqualifying_winsorized_mean_flat";

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"month_count\":%d,\"newest_endpoint_time\":%I64d,\"winsor_each_tail\":%d,\"winsor_divisor\":%d,\"winsorized_mean\":%.12e,\"direction\":%d,\"ratios\":\"%s\",\"returns\":\"%s\",\"state\":\"%s\"}",
                            g_signal_month_key,
                            month_count,
                            (long)newest_endpoint_time,
                            strategy_winsor_each_tail,
                            strategy_winsor_divisor,
                            location,
                            direction,
                            ratio_path,
                            return_path,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG monthly fixed-tail Winsor reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41356\",\"ea\":\"xauxag-mwinsor2-rv\"}");
