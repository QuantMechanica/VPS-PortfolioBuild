...
#property description "QM5_13041 XTI loose-supply breakdown fade"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_13041 - XTI Loose Days-of-Supply Breakdown Fade
// -----------------------------------------------------------------------------
// D1 structural WTI sleeve:
//   - official EIA crude days-of-supply lineage, price-only runtime
//   - Wednesday/Thursday WPSR proxy bar, monthly capped
//   - Donchian breakdown plus lower 126-D1 close-channel position
//   - ATR stop/target, SMA/time exits, no external runtime data
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 13041;
...
input int    strategy_breakdown_lookback    = 55;
input int    strategy_anchor_lookback       = 126;
input int    strategy_rebound_lookback      = 5;
input double strategy_min_rebound_atr       = 0.40;
input double strategy_max_anchor_position   = 0.30;
...
bool Strategy_LoadSignalState(double &atr_last, int &signal_day_key, int &signal_month_key)
  {
   atr_last = 0.0;
   signal_day_key = 0;
   signal_month_key = 0;

   const int breakdown_lookback = MathMax(10, strategy_breakdown_lookback);
   const int anchor_lookback = MathMax(40, strategy_anchor_lookback);
   const int rebound_lookback = MathMax(2, strategy_rebound_lookback);
   const int history_needed = MathMax(anchor_lookback, MathMax(breakdown_lookback + 1, rebound_lookback + 1));

   MqlRates rates[];
   ArraySetAsSeries(rates, true);
   if(CopyRates(_Symbol, PERIOD_D1, 1, history_needed, rates) != history_needed) // perf-allowed: compact D1 signal state behind the framework new-bar gate.
      return false;
   ...
   const double signal_open = rates[0].open;
   ...
   const double directional_close_location = (signal_high - signal_close) / signal_range;
   if(signal_range < strategy_min_range_atr * atr_last)
      return false;
   if(directional_close_location < strategy_min_close_location)
      return false;
   if(signal_close >= sma_last || sma_last >= sma_past)
      return false;

   double prior_low = 0.0;
   for(int i = 1; i <= breakdown_lookback; ++i)
     {
      if(rates[i].low <= 0.0)
         return false;
      if(i == 1 || rates[i].low < prior_low)
         prior_low = rates[i].low;
     }
   if(signal_close >= prior_low)
      return false;

   double anchor_low_close = 0.0;
   double anchor_high_close = 0.0;
   for(int i = 0; i < anchor_lookback; ++i)
     {
      const double c = rates[i].close;
      if(c <= 0.0)
         return false;
      if(i == 0 || c < anchor_low_close)
         anchor_low_close = c;
      if(i == 0 || c > anchor_high_close)
         anchor_high_close = c;
     }
   if(anchor_high_close <= anchor_low_close)
      return false;
   const double anchor_position = (signal_close - anchor_low_close) / (anchor_high_close - anchor_low_close);
   if(anchor_position > strategy_max_anchor_position)
      return false;

   double rebound_high_close = rates[1].close;
   for(int i = 1; i <= rebound_lookback; ++i)
     {
      if(rates[i].close <= 0.0)
         return false;
      if(rates[i].close > rebound_high_close)
         rebound_high_close = rates[i].close;
     }
   if(rebound_high_close - signal_close < strategy_min_rebound_atr * atr_last)
      return false;
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(strategy_report_start_dow < 0 || strategy_report_start_dow > 6 || strategy_report_end_dow < 0 || strategy_report_end_dow > 6)
      return true;
   if(strategy_breakdown_lookback < 10 || strategy_anchor_lookback < 40 || strategy_rebound_lookback < 2)
      return true;
   if(strategy_min_rebound_atr <= 0.0 || strategy_max_anchor_position <= 0.0 || strategy_max_anchor_position >= 1.0)
      return true;
   if(strategy_sma_period <= 1 || strategy_sma_slope_shift <= 0 || strategy_atr_period <= 1)
      return true;
   if(strategy_min_range_atr <= 0.0 || strategy_min_close_location <= 0.0 || strategy_min_close_location >= 1.0)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_13041_XTI_LOOSE_SUPPLY_FADE";
   ...
   if(!Strategy_LoadSignalState(atr_last, signal_day_key, signal_month_key))
      return false;
   ...
   req.reason = "XTI_LOOSE_SUPPLY_FADE_SHORT";
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_13041\",\"ea\":\"xti-loose-supply-fade\"}");
