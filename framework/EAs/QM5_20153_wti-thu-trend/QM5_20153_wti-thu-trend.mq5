...
#property description "QM5_20153 WTI Thursday Premium Positive-Trend Agreement"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_20153 - WTI Thursday Premium / Slow-Trend Agreement
// -----------------------------------------------------------------------------
// D1 structural crude-oil sleeve:
//   - first observed tick of a genuine broker-calendar Thursday D1 bar
//   - BUY only when the completed 252-D1 WTI log return is positive
...
bool Strategy_IsThursdayBar(const datetime value)
  {
   if(value <= 0)
      return false;
   MqlDateTime parts;
...
bool Strategy_IsGenuineThursdayBoundary(const datetime current_bar)
  {
   if(!Strategy_IsThursdayBar(current_bar))
      return false;
...
bool Strategy_LoadMomentum(double &momentum,
                           int &direction)
  {
   ...
   const int copied =
      CopyClose(_Symbol, // perf-allowed: bounded Thursday D1 momentum sample.
                PERIOD_D1,
                1,
                required,
                closes);
...
void Strategy_CloseExpiredPositions()
  {
   const datetime current_bar =
      iTime(_Symbol, PERIOD_D1, 0); // perf-allowed: Thursday D1 lifecycle gate.
   if(current_bar <= 0)
      return;

   const bool current_bar_is_thursday =
      Strategy_IsThursdayBar(current_bar);
   const datetime now = TimeCurrent();
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "WTI_THU_TREND";
   req.symbol_slot = qm_magic_slot_offset;
   req.expiration_seconds = 0;

   const datetime current_bar =
      iTime(_Symbol, PERIOD_D1, 0); // perf-allowed: Thursday D1 entry calendar.
   if(!Strategy_IsGenuineThursdayBoundary(current_bar) ||
   ...
   req.reason = "WTI_THU_POS252_LONG";
   return true;
  }
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_20153_WEEK_ATTEMPT_%d",
                   QM_FrameworkMagic());
   Strategy_LoadAttemptState(TimeCurrent());

   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_20153\",\"ea\":\"wti-thu-trend\"}");
