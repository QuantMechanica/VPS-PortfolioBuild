...
#property description "QM5_12763 WTI Refinery Utilization Squeeze Breakout"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_12763 - WTI Refinery Utilization Squeeze Breakout
// -----------------------------------------------------------------------------
// D1 structural WTI sleeve:
//   - long only during the May-July pre-summer refinery-utilization ramp
//   - requires ATR compression, rising slow D1 trend, and upside range breakout
//   - exits on ramp-window end, trend/range failure, max hold, Friday close, or SL
// Runtime uses MT5 OHLC only; no EIA, refinery, outage, API, CSV, or futures feed.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 12763;
...
input int    strategy_entry_channel       = 25;
input int    strategy_exit_channel        = 12;
input int    strategy_atr_fast_period     = 10;
input int    strategy_atr_slow_period     = 30;
input double strategy_compression_ratio   = 0.80;
...
bool Strategy_Channel(const int lookback, double &highest_high, double &lowest_low)
  {
   ...
   for(int shift = 2; shift < lookback + 2; ++shift)
     {
      const double high = iHigh(_Symbol, PERIOD_D1, shift); // perf-allowed: D1 refinery squeeze channel on closed bars.
      const double low = iLow(_Symbol, PERIOD_D1, shift);   // perf-allowed: D1 refinery squeeze channel on closed bars.
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(strategy_entry_channel <= 1 || strategy_exit_channel <= 1)
      return true;
   if(strategy_atr_fast_period <= 0 || strategy_atr_slow_period <= 0)
      return true;
   if(strategy_atr_fast_period >= strategy_atr_slow_period)
      return true;
   if(strategy_compression_ratio <= 0.0 || strategy_atr_sl_mult <= 0.0)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12763_WTI_REF_SQZ_BRK";
   ...
   if(close_last <= trend_sma || trend_sma <= trend_sma_prior)
      return false;
   if((atr_fast / atr_slow) > strategy_compression_ratio)
      return false;
   ...
   req.sl = QM_StopATR(_Symbol, req.type, entry_price, strategy_atr_slow_period, strategy_atr_sl_mult);
   if(req.sl <= 0.0)
      return false;
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_12763\",\"ea\":\"wti-ref-sqz-brk\"}");
