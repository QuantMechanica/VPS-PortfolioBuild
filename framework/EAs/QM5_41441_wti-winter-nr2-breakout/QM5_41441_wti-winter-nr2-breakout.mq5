...
#property description "QM5_41441 WTI Winter NR2 Close Breakout"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41441 - WTI Winter NR2 Close Breakout
// -----------------------------------------------------------------------------
// D1 structural WTI sleeve:
//   - decide only in November through May during the WTI winter interval
//   - aggregate two consecutive completed 3-to-5-session normalized weeks
//   - require the newest full range to contract versus the preceding week
//   - freeze the newest completed week's high-low range as the breakout box
//   - enter on the first current-week completed D1 close outside that box
//   - consume only a valid breakout before fallible gates; never retry it
//   - hold only through the entry week, with a frozen ATR hard stop
// Runtime uses configured-symbol MT5 OHLC/calendar/state only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                      = 41441;
...
input int    strategy_history_bars         = 30;
input int    strategy_winter_month_1       = 11;
input int    strategy_winter_month_2       = 12;
input int    strategy_winter_month_3       = 1;
input int    strategy_winter_month_4       = 2;
input int    strategy_winter_month_5       = 3;
input int    strategy_winter_month_6       = 4;
input int    strategy_winter_month_7       = 5;
input int    strategy_required_weeks       = 2;
...
int      g_week_bars[2];
double   g_week_ranges[2];
double   g_newer_week_high        = 0.0;
double   g_newer_week_low         = 0.0;
double   g_breakout_close         = 0.0;
bool     g_strict_range_contraction = false;
string   g_signal_state           = "idle";

// -----------------------------------------------------------------------------
// Calendar and identity helpers.
// -----------------------------------------------------------------------------
...
int Strategy_WeekAnchorMonth(const int week_key)
  {
   if(week_key < 19000101)
      return 0;
   return (week_key / 100) % 100;
  }

bool Strategy_IsWinterMonth(const int month)
  {
   return (month == strategy_winter_month_1 ||
           month == strategy_winter_month_2 ||
           month == strategy_winter_month_3 ||
           month == strategy_winter_month_4 ||
           month == strategy_winter_month_5 ||
           month == strategy_winter_month_6 ||
           month == strategy_winter_month_7);
  }
...
// -----------------------------------------------------------------------------
// Owned-position and attempt-ledger helpers.
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// Completed-week signal construction.
// -----------------------------------------------------------------------------
...
void Strategy_ResetDecisionState()
  {
   ...
   for(int index = 0; index < 2; ++index)
     {
      g_week_bars[index] = 0;
      g_week_ranges[index] = 0.0;
     }
   g_newer_week_high = 0.0;
   g_newer_week_low = 0.0;
   g_breakout_close = 0.0;
   g_strict_range_contraction = false;
   g_signal_state = "idle";
  }
...
void Strategy_DetectDecisionClock_OnNewBar()
  {
   ...
   // The breakout trigger is a completed D1 close in the current week.
   // Monday's opening bar has no such close yet and cannot signal.
   if(current_week_count < 1 ||
      current_week_count > strategy_max_week_bars ||
      current_week_count >= copied)
      return;
...
bool Strategy_LoadNR2BreakoutSignal(const int current_week_key,
                                    const int label_offset,
                                    int &direction)
  {
   direction = 0;
   for(int bucket = 0; bucket < 2; ++bucket)
     {
      g_week_bars[bucket] = 0;
      g_week_ranges[bucket] = 0.0;
     }
   g_newer_week_high = 0.0;
   g_newer_week_low = 0.0;
   g_breakout_close = 0.0;
   ...
   ArraySetAsSeries(bars, true);
   const int copied =
      CopyRates(_Symbol, // perf-allowed: bounded two-week OHLC scan behind the sole QM_IsNewBar branch.
                PERIOD_D1,
                1,
                strategy_history_bars,
                bars);
   if(copied < strategy_min_week_bars * 2 + 2)
      return false;

   int week_keys[2] = {0, 0};
   int week_counts[2] = {0, 0};
   double week_opens[2] = {0.0, 0.0};
   double week_highs[2] = {0.0, 0.0};
   double week_lows[2] = {0.0, 0.0};
   double week_closes[2] = {0.0, 0.0};
   int last_session_date_keys[2] = {0, 0};
   int found_weeks = 0;
   bool oldest_boundary_seen = false;
   int current_week_count = 0;
   int last_current_session_date_key = 0;
      ...
      if(week_key == current_week_key)
        {
         ...
         if(session_date_key <= 0 ||
            (last_current_session_date_key > 0 &&
             session_date_key >= last_current_session_date_key))
            return false;
         last_current_session_date_key = session_date_key;
         if(current_week_count == 0)
            g_breakout_close = bars[index].close;
         ++current_week_count;
         if(current_week_count > strategy_max_week_bars)
            return false;
         continue;
        }

      if(found_weeks == 0 || week_key != week_keys[found_weeks - 1])
        {
         if(found_weeks >= 2)
           {
         ...
         else if(Strategy_NextWeekKey(week_key) != week_keys[found_weeks - 1])
            return false;
         week_keys[found_weeks] = week_key;
         ++found_weeks;
        }

      const int bucket = found_weeks - 1;
      if(bucket < 0 || bucket > 1)
         return false;
      const int session_date_key = Strategy_DateKeyForTime(normalized);
      if(session_date_key <= 0 ||
         (last_session_date_keys[bucket] > 0 &&
          session_date_key >= last_session_date_keys[bucket]))
         return false;
      last_session_date_keys[bucket] = session_date_key;

      if(week_counts[bucket] == 0)
        {
         week_highs[bucket] = bars[index].high;
         week_lows[bucket] = bars[index].low;
         week_closes[bucket] = bars[index].close;
        }
      else
        {
         week_highs[bucket] = MathMax(week_highs[bucket], bars[index].high);
         week_lows[bucket] = MathMin(week_lows[bucket], bars[index].low);
        }
      // Bars are newest-first, so the last accepted open is chronological first.
      week_opens[bucket] = bars[index].open;
      ++week_counts[bucket];
      if(week_counts[bucket] > strategy_max_week_bars)
         return false;
     }

   if(current_week_count < 1 ||
      current_week_count != g_current_week_bar_count ||
      g_breakout_close <= 0.0 ||
      !MathIsValidNumber(g_breakout_close) ||
      found_weeks != 2 || !oldest_boundary_seen)
      return false;
   for(int bucket = 0; bucket < 2; ++bucket)
     {
      if(week_counts[bucket] < strategy_min_week_bars ||
         week_counts[bucket] > strategy_max_week_bars ||
         week_opens[bucket] <= 0.0 ||
         week_highs[bucket] <= 0.0 || week_lows[bucket] <= 0.0 ||
         week_closes[bucket] <= 0.0 ||
         week_highs[bucket] < week_opens[bucket] ||
         week_lows[bucket] > week_opens[bucket] ||
         week_highs[bucket] < week_lows[bucket] ||
         week_highs[bucket] < week_closes[bucket] ||
         week_lows[bucket] > week_closes[bucket] ||
         !MathIsValidNumber(week_opens[bucket]) ||
         !MathIsValidNumber(week_highs[bucket]) ||
         !MathIsValidNumber(week_lows[bucket]) ||
         !MathIsValidNumber(week_closes[bucket]))
         return false;

      const double full_range = week_highs[bucket] - week_lows[bucket];
      if(full_range <= 0.0 || !MathIsValidNumber(full_range))
         return false;
      g_week_bars[bucket] = week_counts[bucket];
      g_week_ranges[bucket] = full_range;
     }

   g_newer_week_high = week_highs[0];
   g_newer_week_low = week_lows[0];
   g_strict_range_contraction = (g_week_ranges[0] < g_week_ranges[1]);
   if(g_strict_range_contraction)
     {
      if(g_breakout_close > g_newer_week_high)
         direction = 1;
      else if(g_breakout_close < g_newer_week_low)
         direction = -1;
     }
   return true;
  }
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   const int anchor_month =
      Strategy_WeekAnchorMonth(g_decision_week_key);
   if(Strategy_WeekAlreadyEntered(g_decision_week_key))
      g_signal_state = "entry_deal_already_exists";
   else if(g_late_decision)
      g_signal_state = "late_new_bar_flat";
   else if(!Strategy_IsWinterMonth(anchor_month))
      g_signal_state = "outside_winter_months";
   else
     {
      g_signal_valid =
         Strategy_LoadNR2BreakoutSignal(g_decision_week_key,
                                        g_decision_label_offset,
                                        g_signal_direction);
      if(!g_signal_valid)
         g_signal_state = "nr2_breakout_state_validation_failed";
      else if(g_signal_direction > 0)
         g_signal_state = "nr2_completed_close_breakout_long";
      else if(g_signal_direction < 0)
         g_signal_state = "nr2_completed_close_breakout_short";
      else if(!g_strict_range_contraction)
         g_signal_state = "newest_range_not_strictly_narrower_flat";
      else
         g_signal_state = "no_current_week_completed_close_breakout";

      // A non-breakout remains observational.  The first fully valid
      // completed-close breakout consumes the week before fallible gates.
      if(g_signal_valid && g_signal_direction != 0 &&
         !Strategy_RecordWeekAttempt(g_decision_week_key))
        {
         g_signal_valid = false;
         g_signal_direction = 0;
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"week\":%d,\"anchor_month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_week_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"week_bars\":[%d,%d],\"week_ranges\":[%.8f,%.8f],\"box_high\":%.8f,\"box_low\":%.8f,\"breakout_close\":%.8f,\"strict_range_contraction\":%s,\"state\":\"%s\"}",
                            g_decision_week_key,
                            anchor_month,
                            ...
                            g_newer_week_low,
                            g_breakout_close,
                            g_strict_range_contraction ? "true" : "false",
                            g_signal_state));
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41441_WTI_WINTER_NR2_BREAKOUT";
   ...
   req.reason = (g_signal_direction > 0 ?
                 "WTI_WINTER_NR2_BREAKOUT_LONG" :
                 "WTI_WINTER_NR2_BREAKOUT_SHORT");
...
int OnInit()
  {
   if(!SymbolSelect(strategy_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41441 ||
      qm_magic_slot_offset != 0)
   ...
   g_attempt_state_key =
      StringFormat("QM5_41441_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41441\",\"ea\":\"wti-winter-nr2-breakout\"}");
