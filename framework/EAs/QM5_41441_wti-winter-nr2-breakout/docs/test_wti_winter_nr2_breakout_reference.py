"""Deterministic reference checks for QM5_41441 WTI winter NR2 close breakout."""
...
class WtiWinterNR2CloseBreakoutReferenceTest(unittest.TestCase):
    def setUp(self) -> None:
        self.current = date(2026, 12, 14)
    ...
    def test_ineligible_month_refuses(self) -> None:
        with self.assertRaisesRegex(ValueError, "ineligible month"):
            signal(date(2026, 6, 1), self.current_close(101), self.history())

    def test_all_seven_winter_anchor_months_are_eligible(self) -> None:
        for current in (date(2026, 11, 9), date(2026, 12, 14), date(2027, 1, 11),
                        date(2027, 2, 8), date(2027, 3, 8), date(2027, 4, 12),
                        date(2027, 5, 10)):
            newer = current - timedelta(days=7)
            older = current - timedelta(days=14)
            history = package(newer, 5, 100, 90) + package(older, 5, 100, 80)
            self.assertEqual(signal(current, [Bar(current, 95, 101, 89, 101)], history)[0], 1)
