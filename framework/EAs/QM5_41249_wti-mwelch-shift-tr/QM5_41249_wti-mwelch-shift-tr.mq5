...
#property description "QM5_41249 WTI Monthly Welch Mean-Shift Trend"
...
//   - follow a standardized unequal-variance mean shift only when aligned
//     with the sign of the recent block mean
...
bool Strategy_WelchSignal(const double &closes[],
                           Strategy_SignalMetrics &metrics)
  {
   ...
   double old_ss = 0.0;
   double recent_ss = 0.0;
   for(int index = 0; index < strategy_month_returns; ++index)
     {
      const double center = (index < strategy_block_size)
         ? metrics.mean_old
         : metrics.mean_recent;
      const double difference = returns[index] - center;
      ...
      if(index < strategy_block_size)
         ...
         recent_ss += square;
      if(!MathIsValidNumber(old_ss) ||
         !MathIsValidNumber(recent_ss))
         return false;
     }

   const int variance_denominator = strategy_block_size - 1;
   if(variance_denominator != 5)
      return false;
   metrics.var_old = old_ss / (double)variance_denominator;
   metrics.var_recent = recent_ss / (double)variance_denominator;
   metrics.se2 = metrics.var_old / (double)strategy_block_size +
                 metrics.var_recent / (double)strategy_block_size;
   metrics.oldest_close = closes[0];
   metrics.newest_close = closes[metrics.endpoint_count - 1];
   if(!MathIsValidNumber(metrics.var_old) ||
      !MathIsValidNumber(metrics.var_recent) ||
      !MathIsValidNumber(metrics.se2) ||
      metrics.var_old < 0.0 || metrics.var_recent < 0.0 ||
      metrics.se2 < 0.0)
      return false;

   // A degenerate or effectively zero Welch denominator is a valid flat
   // decision, not permission to use a pooled or fallback variance.
                                        ...
                                        int &direction)
  {
   ...
   const bool valid =
      Strategy_LoadMonthlyEndpoints(current_month_key,
                                    ...
                                    closes,
                                    endpoint_times) &&
      Strategy_WelchSignal(closes, metrics);
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
   ...
   else
     {
      ...
      g_signal_valid =
         endpoints_valid &&
         Strategy_WelchSignal(closes, g_signal_metrics);
      if(!g_signal_valid)
         g_signal_state = "welch_arithmetic_validation_failed";
      else if(g_signal_metrics.direction == 0)
         g_signal_state = "welch_unqualified_or_degenerate_flat";
      else if(g_signal_metrics.direction > 0)
         g_signal_state = "welch_positive_shift_wti_long";
      else if(g_signal_metrics.direction < 0)
         g_signal_state = "welch_negative_shift_wti_short";
      else
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"endpoint_count\":%d,\"return_count\":%d,\"block_size\":%d,\"mean_old\":%.12f,\"mean_recent\":%.12f,\"var_old\":%.12f,\"var_recent\":%.12f,\"se2\":%.12f,\"standard_error\":%.12f,\"score\":%.12f,\"returns\":\"%s\",\"oldest_close\":%.10f,\"newest_close\":%.10f,\"oldest_endpoint\":%I64d,\"newest_endpoint\":%I64d,\"state\":\"%s\"}",
                            ...
                            g_signal_metrics.var_old,
                            g_signal_metrics.var_recent,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41249 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41249_WTI_MWELCH_SHIFT";
   ...
   req.reason = (g_signal_metrics.direction > 0)
      ? "WTI_MWELCH_UPSHIFT_LONG"
      : "WTI_MWELCH_DOWNSHIFT_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41249 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI fixed-block Welch mean-shift trend holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41249_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41249\",\"ea\":\"wti-mwelch-shift-tr\"}");
