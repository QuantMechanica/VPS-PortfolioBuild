...
#property description "QM5_12858 Baker Hughes rig-count Friday WTI momentum"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_12858 - Baker Hughes Rig-Count Friday Momentum
// -----------------------------------------------------------------------------
// D1 structural WTI sleeve:
//   - host/traded symbol: XTIUSD.DWX
//   - first new-week bar entry after large final-workday displacement
//   - short time stop plus ATR hard stop
// Runtime uses MT5 OHLC/broker calendar only; no Baker Hughes/EIA feed.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 12858;
...
input double strategy_min_signal_return_pct  = 0.80;
input double strategy_min_atr_return_mult    = 0.45;
input double strategy_max_signal_return_pct  = 8.0;
input double strategy_close_location_min     = 0.60;
input int    strategy_signal_min_dow         = 4;
input int    strategy_atr_period             = 20;
input double strategy_atr_sl_mult            = 2.50;
input int    strategy_max_hold_days          = 3;
input double strategy_adverse_close_atr_mult = 0.75;
...
bool Strategy_LoadSignal(QM_OrderType &signal_type, double &signal_return_pct)
  {
   signal_type = QM_BUY;
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      if(prior_close > 0.0 && atr_last > 0.0 && strategy_adverse_close_atr_mult > 0.0)
        {
         const double entry_price = PositionGetDouble(POSITION_PRICE_OPEN);
         const ENUM_POSITION_TYPE pos_type = (ENUM_POSITION_TYPE)PositionGetInteger(POSITION_TYPE);
         const double adverse = strategy_adverse_close_atr_mult * atr_last;
         if(pos_type == POSITION_TYPE_BUY && prior_close <= entry_price - adverse)
            should_close = true;
         if(pos_type == POSITION_TYPE_SELL && prior_close >= entry_price + adverse)
            should_close = true;
        }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12858_RIGCOUNT_FRI_MOM";
   ...
   if(!Strategy_LoadSignal(signal_type, signal_return_pct))
      return false;

   const double entry_price = QM_EntryMarketPrice(signal_type);
   if(entry_price <= 0.0)
      return false;
   ...
   req.reason = (signal_type == QM_BUY) ? "RIGCOUNT_FRI_MOM_LONG" : "RIGCOUNT_FRI_MOM_SHORT";
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_12858\",\"ea\":\"rigcount-fri-mom\"}");
