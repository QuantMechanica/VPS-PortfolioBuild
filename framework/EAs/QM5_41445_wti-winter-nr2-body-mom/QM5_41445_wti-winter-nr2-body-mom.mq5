...
#property description "QM5_41445 WTI Winter NR2 Body Momentum"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41445 - WTI Winter NR2 Body Momentum
// -----------------------------------------------------------------------------
// D1 structural crude-oil sleeve:
//   - decide only in November through May during the WTI winter season
//   - aggregate two consecutive completed 3-to-5-session normalized weeks
//   - follow a contracting newest week's own open-to-close body direction
//   - consume the week before fallible gates; never enter late or retry
//   - hold only through the signal week, with a frozen ATR hard stop
// Runtime uses configured-symbol MT5 OHLC/calendar/state only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                      = 41445;
...
double   g_newer_week_open        = 0.0;
double   g_newer_week_high        = 0.0;
double   g_newer_week_low         = 0.0;
double   g_newer_week_close       = 0.0;
double   g_newer_week_body        = 0.0;
...
void Strategy_ResetDecisionState()
  {
   ...
   g_newer_week_open = 0.0;
   g_newer_week_high = 0.0;
   g_newer_week_low = 0.0;
   g_newer_week_close = 0.0;
   g_newer_week_body = 0.0;
...
bool Strategy_LoadNR2BodySignal(const int current_week_key,
                               const int label_offset,
                               int &direction)
  {
   direction = 0;
   ...
   const int copied =
      CopyRates(_Symbol, // perf-allowed: bounded two-week OHLC scan behind the sole QM_IsNewBar branch.
                ...
                strategy_history_bars,
                bars);
   if(copied < strategy_min_week_bars * 2 + 1)
      return false;
   ...
   if(found_weeks != 2 || !oldest_boundary_seen)
      return false;
   for(int bucket = 0; bucket < 2; ++bucket)
     {
   ...
   g_newer_week_open = week_opens[0];
   g_newer_week_high = week_highs[0];
   g_newer_week_low = week_lows[0];
   g_newer_week_close = week_closes[0];
   g_newer_week_body = g_newer_week_close - g_newer_week_open;
   if(!MathIsValidNumber(g_newer_week_body))
      return false;

   g_strict_range_contraction = (g_week_ranges[0] < g_week_ranges[1]);
   if(g_strict_range_contraction && g_newer_week_body > 0.0)
      direction = 1;
   else if(g_strict_range_contraction && g_newer_week_body < 0.0)
      direction = -1;
   return true;
  }
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   // Consume before calendar eligibility, OHLC validation, signal, news,
   // spread, quote, ATR, sizing, or order gates.
   ...
   if(Strategy_WeekAlreadyEntered(g_decision_week_key))
      ...
      g_signal_state = "late_winter_consumed_flat";
   else if(!Strategy_IsWinterMonth(anchor_month))
      g_signal_state = "outside_winter_months";
   else
     {
      g_signal_valid =
         Strategy_LoadNR2BodySignal(g_decision_week_key,
                                   g_decision_label_offset,
                                   g_signal_direction);
      if(!g_signal_valid)
         g_signal_state = "two_completed_weeks_validation_failed";
      else if(g_signal_direction > 0)
         g_signal_state = "nr2_positive_body_continuation_long";
      else if(g_signal_direction < 0)
         g_signal_state = "nr2_negative_body_continuation_short";
      else if(!g_strict_range_contraction)
         g_signal_state = "newest_range_not_strictly_narrower_flat";
      else
         g_signal_state = "newest_week_zero_body_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"week\":%d,\"anchor_month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_week_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"week_bars\":[%d,%d],\"week_ranges\":[%.8f,%.8f],\"new_open\":%.8f,\"new_high\":%.8f,\"new_low\":%.8f,\"new_close\":%.8f,\"new_body\":%.8f,\"strict_range_contraction\":%s,\"state\":\"%s\"}",
                            g_decision_week_key,
                            anchor_month,
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41445_WTI_WINTER_NR2_BODY_MOM";
   ...
   req.reason = (g_signal_direction > 0
                 ? "WTI_WINTER_NR2_BODY_MOM_LONG"
                 : "WTI_WINTER_NR2_BODY_MOM_SHORT");
...
int OnInit()
  {
   if(!SymbolSelect(strategy_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41445 ||
      qm_magic_slot_offset != 0)
   ...
   g_attempt_state_key =
      StringFormat("QM5_41445_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41445\",\"ea\":\"wti-winter-nr2-body-mom\"}");
