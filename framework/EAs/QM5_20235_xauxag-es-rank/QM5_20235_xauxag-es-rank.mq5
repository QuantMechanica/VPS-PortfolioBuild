...
#property description "QM5_20235 XAU XAG Expected-Shortfall Rank"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_20235 - XAU/XAG Monthly Expected-Shortfall Rank
// -----------------------------------------------------------------------------
// Monthly structural precious-metal basket:
...
double   g_cache_xau_es = 0.0;
double   g_cache_xag_es = 0.0;
...
bool Strategy_ExpectedShortfallPair(const int decision_month_key,
                                    double &xau_es,
                                    double &xag_es,
                                    int &observation_count)
  {
   ...
   const int requested =
      MathMax(strategy_history_bars, strategy_min_daily_observations + 40);
   const int xau_count =
      CopyRates(g_leg_xau, PERIOD_D1, 1, requested, xau_rates); // perf-allowed: bounded copy only on a monthly D1 rebalance bar.
   const int xag_count =
      CopyRates(g_leg_xag, PERIOD_D1, 1, requested, xag_rates); // perf-allowed: bounded copy only on a monthly D1 rebalance bar.
   if(xau_count < strategy_min_daily_observations + 1 ||
      xag_count < strategy_min_daily_observations + 1)
      return false;
   ...
   for(int month_index = 0; month_index < strategy_es_window_months; ++month_index)
     {
      expected_month_keys[month_index] =
         Strategy_PreviousMonthKey(decision_month_key, month_index + 1);
      month_observations[month_index] = 0;
      if(expected_month_keys[month_index] <= 0)
         return false;
     }

   const int capacity = (xau_count < xag_count ? xau_count : xag_count);
   double xau_returns[];
   double xag_returns[];
   if(ArrayResize(xau_returns, capacity) != capacity ||
      ArrayResize(xag_returns, capacity) != capacity)
      return false;
   ...
   // A prior common close can precede the formation window so the first return
   // ending inside its first expected month remains observable.
   for(int xau_index = xau_count - 1; xau_index >= 0; --xau_index)
     {
      ...
      if(previous_xau_close > 0.0 && previous_xag_close > 0.0)
        {
         const int bar_month_key = Strategy_MonthKeyForTime(bar_time);
         if(bar_month_key <= 0)
            return false;
         ...
         if(expected_index >= 0)
           {
            ...
            xau_returns[observation_count] = xau_return;
            xag_returns[observation_count] = xag_return;
            ++observation_count;
   ...
   if(ArrayResize(xau_returns, observation_count) != observation_count ||
      ArrayResize(xag_returns, observation_count) != observation_count)
      return false;
   ArraySort(xau_returns);
   ArraySort(xag_returns);

   int tail_count =
      (int)MathCeil((double)observation_count * strategy_tail_probability);
   tail_count = MathMax(1, MathMin(observation_count, tail_count));
   double xau_tail_sum = 0.0;
   double xag_tail_sum = 0.0;
   for(int index = 0; index < tail_count; ++index)
     {
      xau_tail_sum += xau_returns[index];
      xag_tail_sum += xag_returns[index];
     }

   xau_es = xau_tail_sum / (double)tail_count;
   xag_es = xag_tail_sum / (double)tail_count;
   return (MathIsValidNumber(xau_es) && MathIsValidNumber(xag_es));
  }
bool Strategy_LoadSignalState(const datetime decision_bar_time,
                              int &pair_direction)
  {
   ...
   if(!Strategy_ExpectedShortfallPair(decision_month_key,
                                      g_cache_xau_es,
                                      g_cache_xag_es,
   ...
   g_cache_es_difference = g_cache_xau_es - g_cache_xag_es;
...
bool Strategy_OpenPair(const int pair_direction)
  {
   ...
   const string reason = long_xau_short_xag ? "QM5_20235_LONG_XAU_SHORT_XAG_HIGH_ES"
                                            : "QM5_20235_SHORT_XAU_LONG_XAG_HIGH_ES";
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart())
      return true;
   if(qm_ea_id != 20235 || qm_magic_slot_offset != 0)
      return true;
   if(strategy_es_window_months != 12)
      return true;
   if(MathAbs(strategy_tail_probability - 0.05) > 1.0e-12)
      return true;
   if(strategy_history_bars != 400)
      return true;
   if(strategy_min_daily_observations != 220)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20235_XAU_XAG_ES_HOST";
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_20235_MONTH_ATTEMPT_%d",
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_20235\",\"ea\":\"xauxag-es-rank\"}");
