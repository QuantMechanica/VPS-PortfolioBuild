...
#property description "QM5_41223 WTI Same-Calendar Four-Year Exponential-Weight Seasonality"
...
//   - retain each finite return with its exact, uncompressed prior-year age
//   - weight lag k by 2^(-(k-1)/4), normalize, and trade the weighted sign
//   - follow only when the mean is strictly beyond +/-1e-12
...
input double strategy_half_life_years     = 4.0;
...
double   g_signal_weight_sum      = 0.0;
double   g_signal_weighted_sum    = 0.0;
double   g_signal_weighted_mean   = 0.0;
...
void Strategy_ResetDecisionState()
  {
   ...
   g_signal_weight_sum = 0.0;
   g_signal_weighted_sum = 0.0;
   g_signal_weighted_mean = 0.0;
   g_signal_state = "idle";
  }
...
// -----------------------------------------------------------------------------
// Exact completed-month endpoint and exponential year-weight helpers.
// -----------------------------------------------------------------------------
...
bool Strategy_ExponentialYearWeightSignal(const double &observations[],
                                          const int &ages[],
                                          const int sample_count,
                                          double &weight_sum_value,
                                          double &weighted_sum_value,
                                          double &weighted_mean_value,
                                          int &direction)
  {
   weight_sum_value = 0.0;
   weighted_sum_value = 0.0;
   weighted_mean_value = 0.0;
   direction = 0;

   if(strategy_history_years != 10 ||
      strategy_min_observations != 5 ||
      MathAbs(strategy_half_life_years - 4.0) > 1.0e-12 ||
      ...
      ArraySize(ages) < sample_count)
      return false;

   for(int index = 0; index < sample_count; ++index)
     {
      if(index >= ArraySize(observations) || index >= ArraySize(ages) ||
         !MathIsValidNumber(observations[index]) ||
         ages[index] < 0 || ages[index] >= strategy_history_years ||
         (index > 0 && ages[index] <= ages[index - 1]))
         return false;

      const double exponent =
         -((double)ages[index]) / strategy_half_life_years;
      const double weight = MathPow(2.0, exponent);
      if(!MathIsValidNumber(exponent) ||
         !MathIsValidNumber(weight) || weight <= 0.0)
         return false;

      weight_sum_value += weight;
      weighted_sum_value += weight * observations[index];
      if(!MathIsValidNumber(weight_sum_value) ||
         !MathIsValidNumber(weighted_sum_value))
         return false;
     }

   if(weight_sum_value <= 0.0 ||
      !MathIsValidNumber(weight_sum_value) ||
      !MathIsValidNumber(weighted_sum_value))
      return false;

   weighted_mean_value = weighted_sum_value / weight_sum_value;
   if(!MathIsValidNumber(weighted_mean_value))
      return false;

   if(weighted_mean_value > strategy_signal_epsilon)
      direction = 1;
   else if(weighted_mean_value < -strategy_signal_epsilon)
      direction = -1;
   return true;
  }

bool Strategy_LoadExponentialYearWeightSignal(
   ...
   int &direction)
  {
   ...
   if(ArrayResize(observations, strategy_history_years) !=
      ...
      ArrayResize(ages, strategy_history_years) != strategy_history_years ||
      ArraySize(ages) < strategy_history_years)
      return false;
   ...
   return Strategy_ExponentialYearWeightSignal(observations,
                                                ages,
                                                sample_count,
                                                weight_sum_value,
                                                weighted_sum_value,
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   // Consume before history, year-weight arithmetic, news, spread, quote, ATR,
   ...
   else
     {
      g_signal_valid =
         Strategy_LoadExponentialYearWeightSignal(
            ...
            g_signal_weight_sum,
            g_signal_weighted_sum,
            g_signal_weighted_mean,
            g_signal_direction);
      if(!g_signal_valid)
         g_signal_state = "endpoint_sample_or_year_weight_failed";
      else if(g_signal_direction > 0)
         g_signal_state = "year_weight_long";
      else if(g_signal_direction < 0)
         g_signal_state = "year_weight_short";
      else
         g_signal_state = "year_weight_epsilon_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"valid\":%s,\"samples\":%d,\"weight_sum\":%.12g,\"weighted_sum\":%.12g,\"weighted_mean\":%.12g,\"direction\":%d,\"state\":\"%s\"}",
                            g_decision_month_key,
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(!QM_InputRequireLong("qm_ea_id", qm_ea_id, 41223) ||
      ...
      !QM_InputRequireDouble("strategy_half_life_years", strategy_half_life_years, 4.0, 1.0e-12) ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(g_signal_direction > 0)
     {
      req.type = QM_BUY;
      req.reason = "WTI_SAMECAL_EXPW4_LONG";
     }
   else
     {
      req.type = QM_SELL;
      req.reason = "WTI_SAMECAL_EXPW4_SHORT";
     }
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41223 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI same-calendar four-year exponential-weight card holds to the next broker month"))
      return INIT_FAILED;
   ...
   g_attempt_state_key =
      StringFormat("QM5_41223_SAMECAL_EXPW4_MONTH_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41223\",\"ea\":\"wti-samecal-expw4\"}");
