...
#property description "QM5_10603 MQL5 MovingAverage_FN direction flip v2"
...
bool Strategy_NoTradeFilter()
  {
   if(strategy_signal_tf != (ENUM_TIMEFRAMES)_Period ||
      strategy_filter_period < 4 ||
      strategy_smooth_period < 2 ||
...
bool Strategy_MAFNSignal(int &signal)
  {
   ...
   // Called only from Strategy_EntrySignal after the framework new-bar gate.
   const int copied = CopyRates(_Symbol, strategy_signal_tf, 1, strategy_calc_bars, rates);
   if(copied < strategy_filter_period + strategy_smooth_period + 3)
      return false;
   ...
   for(int i = 0; i < copied; ++i)
     {
      ...
      if(i == 0)
        ...
        {
         filter = filter + alpha_filter * (price - filter);
         smooth = smooth + alpha_smooth * (filter - smooth);
        }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   int signal = 0;
   if(!Strategy_MAFNSignal(signal) || signal == 0)
      return false;
   ...
   if(Strategy_ReadOurPosition(pos_type, pos_ticket))
     {
      ...
      if(!QM_TM_ClosePosition(pos_ticket, QM_EXIT_OPPOSITE_SIGNAL))
         return false;
     }
   ...
   if(point <= 0.0)
      return false;
   if(QM_LotsForRisk(_Symbol, MathAbs(entry - sl) / point) <= 0.0)
      return false;
