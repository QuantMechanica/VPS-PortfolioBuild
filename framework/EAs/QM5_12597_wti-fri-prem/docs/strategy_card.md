...
period: D1
expected_trade_frequency: "Weekly D1 WTI Friday-calendar sleeve; estimate 45-52 trades/year after broker holidays and framework filters."
...
g0_approval_reasoning: "R1 PASS academic WTI calendar-effects source; R2 PASS deterministic Friday D1 long/time-flat rule with ATR stop; R3 PASS XTIUSD.DWX available; R4 PASS no ML/grid/martingale/external data."
expected_pf: 1.1
expected_dd_pct: 16.0
---

# WTI Friday Calendar Premium
...
Academic WTI calendar-effects research reports an asymmetry across weekdays:
early-week crude-oil returns are weak on average, while Friday is the strongest
positive weekday in the studied WTI sample. This card isolates the Friday side
as a low-frequency XTIUSD.DWX sleeve: long only on the broker-calendar Friday D1
bar, with deterministic time exits and ATR risk.

This is deliberately different from:

- `QM5_12596_wti-mon-fade`: this card is long Friday premium, not short Monday
  weakness.
...
- `QM5_12591` and `QM5_12593`: not hurricane-season or refinery-maintenance
  logic.
...
- Current broker-calendar D1 bar must be Friday.
...
- Framework Friday close remains enabled and is the primary same-day flattening
  path before the weekend.
...
The source is used for structural lineage around weekday seasonality in WTI
returns, including a positive Friday average return in the studied sample. No
...
- [x] R2 mechanical: fixed broker-calendar weekday, single D1 long entry, ATR
  stop, Friday/next-bar time exit.
...
- [x] R4 compliant: no ML, no adaptive PnL fitting, no grid, no martingale, one
  position per magic.
- [x] Non-duplicate: long Friday premium is not the existing Monday short,
  monthly WTI seasonality, WPSR, refinery, hurricane, reversal, or RSI commodity
  sleeve.
...
- trade_management: Friday-close/first non-Friday D1 bar and max-hold stale
  position exits.
...
|---|---|---|---|---|
| v1 | 2026-06-27 | initial structural WTI Friday calendar-premium build | G0 | APPROVED |
