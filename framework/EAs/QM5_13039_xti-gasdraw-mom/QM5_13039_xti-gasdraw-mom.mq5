...
#property description "QM5_13039 XTI gasoline inventory pressure momentum"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_13039 - XTI Gasoline Draw Pressure Momentum
// -----------------------------------------------------------------------------
// D1 structural WTI sleeve:
//   - uses Wednesday/Thursday as the weekly EIA gasoline-stock WPSR proxy window
//   - requires a short pullback, bullish release-window reaction, and rising SMA
//   - ATR stop/target, SMA/season/time exits, no external runtime data
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 13039;
...
input int    strategy_season_end_month     = 8;
...
bool Strategy_IsDrivingSeason(const datetime t)
  {
   if(t <= 0)
      return false;
   MqlDateTime dt;
...
bool Strategy_LoadGasPressureState(double &atr_last, int &signal_day_key)
  {
   atr_last = 0.0;
   signal_day_key = 0;
   ...
   if(signal_time <= 0 || !Strategy_IsReportProxyDay(signal_time))
      return false;
   if(!Strategy_IsDrivingSeason(signal_time) || !Strategy_IsDrivingSeason(TimeCurrent()))
      return false;
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      if(pos_type != POSITION_TYPE_BUY)
         should_close = true;
      if(!Strategy_IsDrivingSeason(now))
         should_close = true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_13039_XTI_GASDRAW_MOM";
   ...
   if(!Strategy_LoadGasPressureState(atr_last, signal_day_key))
      return false;
   ...
   req.reason = "XTI_GASDRAW_MOM_LONG";
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_13039\",\"ea\":\"xti-gasdraw-mom\"}");
