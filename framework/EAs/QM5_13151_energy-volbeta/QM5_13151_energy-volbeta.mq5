...
#property description "QM5_13151 XTI XNG Smooth-Volatility Beta Rank"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_13151 - XTI/XNG Monthly Smooth-Volatility Beta Rank
// -----------------------------------------------------------------------------
// Price-native falsification of a commodity aggregate-volatility anomaly:
//   - form an inverse-volatility XTI/XNG benchmark from completed D1 returns
//   - derive changes in its 20-day realized volatility on non-jump days
//   - regress each leg on the benchmark return and smooth-volatility innovation
//   - buy higher smooth-volatility beta, short lower beta for one broker month
// This does not reproduce the source's option-derived equity volatility factor.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 13151;
...
double   g_cache_xti_vol_beta = 0.0;
double   g_cache_xng_vol_beta = 0.0;
double   g_cache_vol_beta_difference = 0.0;
...
bool Strategy_LoadSynchronizedReturns(const datetime decision_bar_time,
                                      ...
                                      double &market_stddev)
  {
   ...
   if(decision_bar_time <= 0 || strategy_lookback_d1 < 30 ||
      strategy_rv_window_d1 < 5)
      return false;

   const int total_returns = strategy_lookback_d1 + strategy_rv_window_d1;
   ...
   const long endpoint_gap =
      (long)(decision_bar_time - xti_rates[0].time);
   if(endpoint_gap < 0 ||
   ...
   if(ArrayResize(xti_returns, total_returns) != total_returns ||
      ...
      ArrayResize(market_returns, total_returns) != total_returns)
      return false;
   ...
   const int first_sample = strategy_rv_window_d1;
   double xti_sum = 0.0;
   double xng_sum = 0.0;
   for(int i = 0; i < strategy_lookback_d1; ++i)
     {
      const int sample = first_sample + i;
      xti_sum += xti_returns[sample];
      xng_sum += xng_returns[sample];
     }
   ...
   const double xti_stddev =
      MathSqrt(xti_ss / (double)(strategy_lookback_d1 - 1));
   const double xng_stddev =
      MathSqrt(xng_ss / (double)(strategy_lookback_d1 - 1));
   ...
   if(inverse_vol_sum <= 0.0 ||
      !MathIsValidNumber(inverse_vol_sum))
      return false;
   g_cache_xti_benchmark_weight =
      (1.0 / xti_stddev) / inverse_vol_sum;
   g_cache_xng_benchmark_weight =
      (1.0 / xng_stddev) / inverse_vol_sum;

   for(int sample = 0; sample < total_returns; ++sample)
     {
      market_returns[sample] =
         g_cache_xti_benchmark_weight * xti_returns[sample] +
   ...
   for(int i = 0; i < strategy_lookback_d1; ++i)
      market_sum += market_returns[first_sample + i];
   ...
   for(int i = 0; i < strategy_lookback_d1; ++i)
     {
      const double centered =
         market_returns[first_sample + i] - market_mean;
      market_ss += centered * centered;
     }
   market_stddev =
      MathSqrt(market_ss / (double)(strategy_lookback_d1 - 1));
   return (market_stddev > 1.0e-12 &&
           MathIsValidNumber(market_stddev));
  }

bool Strategy_RollingStd(const double &values[],
                         const int end_index,
                         const int window,
                         double &stddev)
  {
...
bool Strategy_SmoothVolBeta(const double &asset_returns[],
                            ...
                            int &smooth_days)
  {
   ...
   const int total_returns =
      strategy_lookback_d1 + strategy_rv_window_d1;
   if(ArraySize(asset_returns) != total_returns ||
      ArraySize(market_returns) != total_returns ||
      market_stddev <= 0.0)
      return false;
   ...
   for(int sample = 0; sample < strategy_lookback_d1; ++sample)
     {
      const int return_index = strategy_rv_window_d1 + sample;
      double rv_current = 0.0;
      double rv_previous = 0.0;
      if(!Strategy_RollingStd(market_returns,
                              return_index,
                              strategy_rv_window_d1,
                              rv_current) ||
         !Strategy_RollingStd(market_returns,
                              return_index - 1,
                              strategy_rv_window_d1,
                              rv_previous))
         return false;

      const double market_innovation =
         market_returns[return_index] - market_mean;
      const bool jump_day =
         (MathAbs(market_innovation) >= jump_threshold);
      const double smooth_vol_innovation =
      ...
      double x[3];
      x[0] = 1.0;
      x[1] = market_returns[return_index];
      x[2] = smooth_vol_innovation;
      const double y = asset_returns[return_index];
      if(!MathIsValidNumber(y) ||
         !MathIsValidNumber(smooth_vol_innovation))
         return false;
   ...
   if(smooth_days < strategy_min_smooth_days)
      return false;
   ...
   for(int pivot_col = 0; pivot_col < 3; ++pivot_col)
     {
      ...
      if(pivot_abs <= 1.0e-16 ||
         !MathIsValidNumber(pivot_abs))
         return false;
      ...
      if(MathAbs(divisor) <= 1.0e-16 ||
         !MathIsValidNumber(divisor))
         return false;
      for(int col = pivot_col; col < 4; ++col)
      ...
      for(int row = 0; row < 3; ++row)
        {
         ...
         for(int col = pivot_col; col < 4; ++col)
            normal[row][col] -=
               factor * normal[pivot_col][col];
        }
     }

   vol_beta = normal[2][3];
   return MathIsValidNumber(vol_beta);
  }
...
bool Strategy_LoadSignalState(const datetime decision_bar_time,
  ...
  {
   pair_direction = 0;
   g_cache_xti_vol_beta = 0.0;
   g_cache_xng_vol_beta = 0.0;
   g_cache_vol_beta_difference = 0.0;
   g_cache_market_mean = 0.0;
   g_cache_market_stddev = 0.0;
   g_cache_smooth_days = 0;
   ...
   int xti_smooth_days = 0;
   int xng_smooth_days = 0;
   if(!Strategy_SmoothVolBeta(xti_returns,
                              market_returns,
                              g_cache_market_mean,
                              g_cache_market_stddev,
                              g_cache_xti_vol_beta,
                              xti_smooth_days))
      return false;
   if(!Strategy_SmoothVolBeta(xng_returns,
                              market_returns,
                              g_cache_market_mean,
                              g_cache_market_stddev,
                              g_cache_xng_vol_beta,
                              xng_smooth_days))
      return false;
   if(xti_smooth_days != xng_smooth_days)
      return false;
   g_cache_smooth_days = xti_smooth_days;

   g_cache_vol_beta_difference =
      g_cache_xti_vol_beta - g_cache_xng_vol_beta;
   if(!MathIsValidNumber(g_cache_vol_beta_difference))
      return false;

   // The source's high-minus-low smooth aggregate-volatility beta spread is
   // positive, so the higher-beta leg is held long.
   if(g_cache_vol_beta_difference > 1.0e-12)
      pair_direction = 1;
   else if(g_cache_vol_beta_difference < -1.0e-12)
      pair_direction = -1;
   return true;
  }
...
bool Strategy_OpenPair(const int pair_direction)
  {
   ...
   const string reason = long_xti_short_xng ? "QM5_13151_LONG_XTI_SHORT_XNG_HIGH_VBETA"
                                            : "QM5_13151_SHORT_XTI_LONG_XNG_HIGH_VBETA";
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart())
      return true;
   if(strategy_lookback_d1 != 252 || strategy_rv_window_d1 != 20 ||
      MathAbs(strategy_jump_exclusion_z - 2.0) > 1.0e-12 ||
      strategy_min_smooth_days != 200)
      return true;
   if(strategy_history_bars < 330 || strategy_history_bars > 450)
      return true;
   if(strategy_history_bars < strategy_lookback_d1 + strategy_rv_window_d1 + 1)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_13151_XTI_XNG_VBETA_HOST";
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_13151\",\"ea\":\"energy-volbeta\"}");
