...
#property description "QM5_10169 RSI Divergence With EMA Trend Conversion"
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      if(ptype == POSITION_TYPE_BUY && ema_fast > ema_slow)
         QM_TM_TrailATR(ticket, strategy_atr_period, strategy_trail_atr_mult);
      if(ptype == POSITION_TYPE_SELL && ema_fast < ema_slow)
         QM_TM_TrailATR(ticket, strategy_atr_period, strategy_trail_atr_mult);
     }
  }
...
bool Strategy_ExitSignal()
  {
   ...
   const double ema_fast = QM_EMA(_Symbol, PERIOD_D1, strategy_ema_fast_period, 1);
   const double ema_slow = QM_EMA(_Symbol, PERIOD_D1, strategy_ema_slow_period, 1);
   if(rsi_now <= 0.0 || ema_fast <= 0.0 || ema_slow <= 0.0)
      return false;
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      const int marker = StringFind(comment, "RSI=");
      const double entry_rsi = (marker >= 0) ? StringToDouble(StringSubstr(comment, marker + 4)) : 0.0;
      if(entry_rsi <= 0.0)
         continue;

      const ENUM_POSITION_TYPE ptype = (ENUM_POSITION_TYPE)PositionGetInteger(POSITION_TYPE);
      if(ptype == POSITION_TYPE_BUY &&
         rsi_now < strategy_centerline &&
         rsi_now < entry_rsi &&
         ema_fast <= ema_slow)
         return true;
...
void OnTick()
  {
   ...
   // management path per the 2026-07-02 OnTick-ordering audit finding (see
   // QM5_12821 commit dc418a720): position management and exits must keep
   // running through news windows.
