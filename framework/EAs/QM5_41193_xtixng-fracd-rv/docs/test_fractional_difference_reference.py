...
EA_PATH = EA_DIR / "QM5_41193_xtixng-fracd-rv.mq5"
CARD_PATH = (
    REPO
    / "strategy-seeds"
    / "cards"
    / "approved"
    / "QM5_41193_xtixng-fracd-rv_card.md"
)
...
class FractionalDifferenceReferenceTests(unittest.TestCase):
    ...
    def test_latest_output_is_held_out(self) -> None:
        ...
        self.assertEqual(direction_pos, -1)  # SELL XTI / BUY XNG
        self.assertEqual(direction_neg, 1)  # BUY XTI / SELL XNG
    ...
    def test_manifest_card_and_registry_contract(self) -> None:
        ...
        self.assertEqual(manifest["logical_symbol"], "QM5_41193_XTI_XNG_FRACD_RV_D1")
        ...
        self.assertEqual(
            [row for row in magic_rows if row.startswith("41193,")],
            [
                "41193,xtixng-fracd-rv,0,XTIUSD.DWX,411930000,2026-08-29,Codex governed allocator,active",
                "41193,xtixng-fracd-rv,1,XNGUSD.DWX,411930001,2026-08-29,Codex governed allocator,active",
            ],
        )
    ...
    def test_three_presets_are_backtest_only_fixed_risk(self) -> None:
        ...
        logical = [path for path in setfiles if "QM5_41193_XTI_XNG_FRACD_RV_D1" in path.name]
