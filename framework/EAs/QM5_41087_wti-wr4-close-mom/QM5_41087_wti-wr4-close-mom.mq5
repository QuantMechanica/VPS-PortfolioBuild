...
#property description "QM5_41087 WTI Weekly WR4 Close Momentum"
...
//   - aggregate four consecutive completed broker-week OHLC packages
//   - require the newest completed week to be strict widest-range-of-four
//   - follow its own body only when the close confirms in the outer quartile
//   - consume the week before fallible gates; never enter late or retry
//   - hold the confirmed structural repricing direction for one broker week
// Runtime uses MT5-native OHLC, calendar, history, and framework state only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                      = 41087;
...
input int    strategy_history_bars          = 50;
input int    strategy_required_weeks        = 4;
...
bool     g_widest_range_four      = false;
...
bool Strategy_LoadWR4Signal(const int current_week_key,
                            const int label_offset,
                            int &direction)
  {
   direction = 0;
   ...
   const int copied =
      CopyRates(_Symbol, // perf-allowed: bounded completed-week OHLC scan behind the sole QM_IsNewBar branch.
                ...
                strategy_history_bars,
                bars);
   if(copied < strategy_min_week_bars * 4 + 1)
      return false;

   int week_keys[4] = {0, 0, 0, 0};
   int week_counts[4] = {0, 0, 0, 0};
   double week_opens[4] = {0.0, 0.0, 0.0, 0.0};
   double week_highs[4] = {0.0, 0.0, 0.0, 0.0};
   double week_lows[4] = {0.0, 0.0, 0.0, 0.0};
   ...
   if(found_weeks != 4 || !oldest_boundary_seen)
      return false;
   for(int bucket = 0; bucket < 4; ++bucket)
     {
      ...
      if(week_opens[bucket] <= 0.0 ||
         ...
         week_highs[bucket] <
            MathMax(week_opens[bucket], week_closes[bucket]) ||
         week_lows[bucket] >
            MathMin(week_opens[bucket], week_closes[bucket]) ||
   ...
   g_new_open = week_opens[0];
   g_new_high = week_highs[0];
   g_new_low = week_lows[0];
   g_new_close = week_closes[0];
   g_week_body = MathLog(g_new_close / g_new_open);
   if(!MathIsValidNumber(g_week_body))
      return false;
   g_close_location =
      (g_new_close - g_new_low) / g_week_ranges[0];
   if(!MathIsValidNumber(g_close_location) ||
      g_close_location < 0.0 || g_close_location > 1.0)
      return false;

   g_widest_range_four =
      (g_week_ranges[0] > g_week_ranges[1] &&
       g_week_ranges[0] > g_week_ranges[2] &&
       g_week_ranges[0] > g_week_ranges[3]);

   if(g_widest_range_four &&
      g_week_body > 0.0 &&
      g_close_location > strategy_clv_upper)
      direction = 1;
   else if(g_widest_range_four &&
           g_week_body < 0.0 &&
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_WeekAlreadyEntered(g_decision_week_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadWR4Signal(g_decision_week_key,
                                g_decision_label_offset,
                                g_signal_direction);
      if(!g_signal_valid)
         g_signal_state = "weekly_ohlc_validation_failed";
      else if(g_signal_direction > 0)
         g_signal_state = "strict_wr4_positive_body_upper_quartile_long";
      else if(g_signal_direction < 0)
         g_signal_state = "strict_wr4_negative_body_lower_quartile_short";
      else if(!g_widest_range_four)
         g_signal_state = "newest_not_strict_widest_of_four_flat";
      else if(g_week_body == 0.0)
         g_signal_state = "body_zero_flat";
      else if(g_close_location == strategy_clv_upper ||
              g_close_location == strategy_clv_lower)
         g_signal_state = "close_location_equality_flat";
      else
         g_signal_state = "body_location_disagreement_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"week\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_week_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"week_bars\":[%d,%d,%d,%d],\"week_ranges\":[%.8f,%.8f,%.8f,%.8f],\"new_open\":%.8f,\"new_high\":%.8f,\"new_low\":%.8f,\"new_close\":%.8f,\"week_body\":%.12f,\"close_location\":%.12f,\"strict_wr4\":%s,\"state\":\"%s\"}",
                            g_decision_week_key,
                            ...
                            g_week_body,
                            g_close_location,
                            g_widest_range_four ? "true" : "false",
                            g_signal_state));
  }
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41087 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41087_WTI_WR4_CLOSE_MOM";
   ...
   req.reason = (g_signal_direction > 0)
      ? "WTI_WR4_CLOSE_MOM_LONG"
      : "WTI_WR4_CLOSE_MOM_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41087 ||
   ...
   g_attempt_state_key =
      StringFormat("QM5_41087_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41087\",\"ea\":\"wti-wr4-close-mom\"}");
