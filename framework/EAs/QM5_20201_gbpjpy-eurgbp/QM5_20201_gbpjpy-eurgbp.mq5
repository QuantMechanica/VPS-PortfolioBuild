...
#property description "QM5_20201 GBPJPY EURGBP Cointegration"
...
//
// Card:
//   strategy-seeds/cards/approved/QM5_20201_gbpjpy-eurgbp_card.md
//
// Fixed spread:
//   ln(GBPJPY.DWX) - beta * ln(EURGBP.DWX)
//
...
input double strategy_beta              = -1.681438352;
...
bool Strategy_EnsureBasketScope()
  {
   ...
   string allowed[5] = {"GBPJPY.DWX", "EURGBP.DWX", "USDJPY.DWX", "GBPUSD.DWX", "EURUSD.DWX"};
   for(int i = 0; i < 5; ++i)
...
bool Strategy_RefreshSpreadState()
  {
   ...
   for(int i = 0; i < history_count; ++i)
     {
      if(gbpjpy_time[i] <= 0 || gbpjpy_time[i] != eurgbp_time[i])
         return false;
      if(gbpjpy[i] <= 0.0 || eurgbp[i] <= 0.0)
         return false;

      spreads[i] =
         MathLog(gbpjpy[i]) - strategy_beta * MathLog(eurgbp[i]);
...
bool Strategy_OpenPair(const int spread_direction)
  {
   ...
   const double weight_sum = gbpjpy_weight + eurgbp_weight;
   if(weight_sum <= 0.0)
      return false;
   ...
   const double gbpjpy_lots =
      Strategy_LotsForLeg(g_leg_gbpjpy, gbpjpy_weight, weight_sum);
   const double eurgbp_lots =
      Strategy_LotsForLeg(g_leg_eurgbp, eurgbp_weight, weight_sum);
   if(gbpjpy_lots <= 0.0 || eurgbp_lots <= 0.0)
      return false;
   ...
   const string reason =
      long_spread ? "QM5_20201_LONG_SPREAD_Z_LT_NEG_ENTRY"
                  : "QM5_20201_SHORT_SPREAD_Z_GT_POS_ENTRY";

   const bool gbpjpy_ok =
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20201_PAIR_HOST";
