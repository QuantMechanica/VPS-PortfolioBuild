# QM5_20201_gbpjpy-eurgbp — Strategy Spec
...
`ln(GBPJPY.DWX) - (-1.681438352) * ln(EURGBP.DWX)`. Score the newest spread
...
| `strategy_beta` | -1.681438352 | fixed | DEV-fitted structural hedge coefficient |
...
- `USDJPY.DWX`, `GBPUSD.DWX`, and `EURUSD.DWX` — conversion-history
  dependencies for JPY/GBP P/L and EUR margin under the USD-account tester;
...
| OOS basket state changes | 20 across 2023–2024 in the frozen scan |
| Typical hold time | multi-day; scan half-life 82.785 D1 bars |
| Expected drawdown profile | high and regime-sensitive; conservative 30% prior |
| Regime preference | GBPJPY/EURGBP fixed-residual mean reversion |
| Win rate target | unknown; Q02 is the first platform-economic judge |

The fixed scan recorded DEV net Sharpe `0.676373`, OOS net Sharpe `0.619982`,
and OOS return `7.697720%`. The sub-0.8 OOS score and slow half-life are
...
`artifacts/cards_approved/QM5_20201_gbpjpy-eurgbp_card.md`

Chan supplies the deterministic pair-trading method but makes no performance
claim for GBPJPY/EURGBP.
...
`1.0` and `1.681438352`; each leg is independently bounded by its ATR stop.
