...
#property description "QM5_41363 XTI/XNG Weekly Relative-Flow Divergence"
...
// Monday-Friday week. Split both energy legs into close-to-open and open-to-close
// log-return flows, subtract XNG from XTI, and trade only when the two
// relative components disagree. Follow the session-relative component with
// opposite equal-notional legs and flatten the package Friday at broker 21.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41363;
...
MqlRates g_flow_xti_bars[];
MqlRates g_flow_xng_bars[];
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41363 && qm_magic_slot_offset == 0 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XTIXNG_FLOWDIV_ATTEMPT_DATE", qm_ea_id);
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41363_XTIXNG_FLOWDIV";
...
bool Strategy_RefreshFlowWindow()
  {
   ArrayResize(g_flow_xti_bars, 6);
   ArrayResize(g_flow_xng_bars, 6);
   ArraySetAsSeries(g_flow_xti_bars, true);
   ArraySetAsSeries(g_flow_xng_bars, true);
   const int xti_copied =
      CopyRates(g_leg_xti, PERIOD_D1, 1, 6, g_flow_xti_bars);
   const int xng_copied =
      CopyRates(g_leg_xng, PERIOD_D1, 1, 6, g_flow_xng_bars);
   g_flow_window_valid = (xti_copied == 6 && xng_copied == 6);
...
bool Strategy_UpdateFlowWindow()
  {
   ...
   MqlRates xti_closed[];
   MqlRates xng_closed[];
   ArrayResize(xti_closed, 1);
   ArrayResize(xng_closed, 1);
   ArraySetAsSeries(xti_closed, true);
   ArraySetAsSeries(xng_closed, true);
   if(CopyRates(g_leg_xti, PERIOD_D1, 1, 1, xti_closed) != 1 ||
      CopyRates(g_leg_xng, PERIOD_D1, 1, 1, xng_closed) != 1 ||
      xti_closed[0].time != g_flow_anchor_host_bar ||
      xng_closed[0].time != g_flow_anchor_host_bar)
      return Strategy_RefreshFlowWindow();

   for(int index = 5; index > 0; --index)
     {
      g_flow_xti_bars[index] = g_flow_xti_bars[index - 1];
      g_flow_xng_bars[index] = g_flow_xng_bars[index - 1];
     }
   g_flow_xti_bars[0] = xti_closed[0];
   g_flow_xng_bars[0] = xng_closed[0];
...
bool Strategy_LoadRelativeFlow(double &overnight_relative,
                               ...
                               int &direction)
  {
   ...
   const datetime xng_current =
      iTime(g_leg_xng, PERIOD_D1, 0); // perf-allowed: entry-only sync gate.
   if(xng_current != g_current_host_bar ||
      !g_flow_window_valid ||
      g_flow_anchor_host_bar != g_current_host_bar)
      return false;
   ...
   for(int index = 0; index < 6; ++index)
     {
      if(g_flow_xti_bars[index].time <= 0 ||
         g_flow_xti_bars[index].time != g_flow_xng_bars[index].time ||
         Strategy_DayOfWeek(g_flow_xti_bars[index].time) != expected_days[index] ||
         Strategy_DayKey(g_flow_xti_bars[index].time) !=
            Strategy_DayKey(broker_now -
                            (datetime)(expected_offsets[index] * 86400)))
         return false;
      if(g_flow_xti_bars[index].close <= 0.0 || g_flow_xng_bars[index].close <= 0.0 ||
         !MathIsValidNumber(g_flow_xti_bars[index].close) ||
         !MathIsValidNumber(g_flow_xng_bars[index].close) ||
         (index < 5 &&
          (g_flow_xti_bars[index].open <= 0.0 || g_flow_xng_bars[index].open <= 0.0 ||
           !MathIsValidNumber(g_flow_xti_bars[index].open) ||
           !MathIsValidNumber(g_flow_xng_bars[index].open))))
         return false;
     }

   if(!Strategy_GapHoursAllowed(g_current_host_bar,
                                g_flow_xti_bars[0].time, 68, 76))
      return false;
   for(int index = 0; index < 4; ++index)
     {
      if(!Strategy_GapHoursAllowed(g_flow_xti_bars[index].time,
                                   g_flow_xti_bars[index + 1].time, 20, 28))
         return false;
     }
   if(!Strategy_GapHoursAllowed(g_flow_xti_bars[4].time,
                                g_flow_xti_bars[5].time, 68, 76))
      return false;

   for(int index = 0; index < 5; ++index)
     {
      const double xti_overnight =
         MathLog(g_flow_xti_bars[index].open / g_flow_xti_bars[index + 1].close);
      const double xng_overnight =
         MathLog(g_flow_xng_bars[index].open / g_flow_xng_bars[index + 1].close);
      const double xti_session =
         MathLog(g_flow_xti_bars[index].close / g_flow_xti_bars[index].open);
      const double xng_session =
         MathLog(g_flow_xng_bars[index].close / g_flow_xng_bars[index].open);
      if(!MathIsValidNumber(xti_overnight) ||
         !MathIsValidNumber(xng_overnight) ||
         !MathIsValidNumber(xti_session) ||
         !MathIsValidNumber(xng_session))
         return false;
      overnight_relative += xti_overnight - xng_overnight;
      session_relative += xti_session - xng_session;
     }

   if(!MathIsValidNumber(overnight_relative) ||
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41363_XTIXNG_FLOWDIV_HOST";
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"date\":%d,\"overnight_relative\":%.12e,\"session_relative\":%.12e,\"direction\":%d}",
                            g_signal_date_key,
                            overnight_relative,
                            session_relative,
                            direction));
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XTI/XNG weekly flow-divergence package uses paired Friday 21 exit"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41363\",\"ea\":\"xtixng-flowdiv\"}");
