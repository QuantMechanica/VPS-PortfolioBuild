...
#property description "QM5_13131 XTI XNG Historical-Kurtosis Rank"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_13131 - XTI/XNG Historical-Kurtosis Rank
// -----------------------------------------------------------------------------
// Monthly market-neutral energy basket:
//   - calculate each leg's Pearson historical kurtosis from the latest 252
//     completed simple D1 returns using the source's exact denominators
//   - long the higher-kurtosis energy leg and short the lower-kurtosis leg
// Runtime is Darwinex-native D1 OHLC only; no external or futures-chain data.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 13131;
...
double   g_cache_xti_kurtosis = 0.0;
double   g_cache_xng_kurtosis = 0.0;
...
bool Strategy_KurtosisMeasure(const string symbol,
                              ...
                              int &observation_count)
  {
   ...
   const int count = CopyRates(symbol, PERIOD_D1, 1, requested, rates); // perf-allowed: bounded copy only on the first D1 bar of a broker month.
   ...
   if(sample_variance <= 0.0 || !MathIsValidNumber(sample_variance))
      return false;
   const double fourth_central_moment = fourth_sum / (double)observation_count;
   const double variance_squared = sample_variance * sample_variance;
   if(fourth_central_moment < 0.0 || variance_squared <= 0.0 ||
      !MathIsValidNumber(fourth_central_moment) ||
      !MathIsValidNumber(variance_squared))
      return false;
...
bool Strategy_LoadSignalState(const datetime decision_bar_time,
                              int &pair_direction)
  {
   ...
   if(!Strategy_KurtosisMeasure(g_leg_xti,
      ...
      return false;
   if(!Strategy_KurtosisMeasure(g_leg_xng,
                                g_cache_xng_kurtosis,
                                g_cache_xng_observations))
      return false;

   g_cache_kurtosis_difference = g_cache_xti_kurtosis - g_cache_xng_kurtosis;
   if(!MathIsValidNumber(g_cache_kurtosis_difference))
      return false;
   if(g_cache_kurtosis_difference > 1.0e-12)
      pair_direction = 1;
   else if(g_cache_kurtosis_difference < -1.0e-12)
      pair_direction = -1;
   return true;
  }
...
bool Strategy_OpenPair(const int pair_direction)
  {
   ...
   const string reason = long_xti_short_xng ? "QM5_13131_LONG_XTI_SHORT_XNG_HIGH_KURT"
                                            : "QM5_13131_SHORT_XTI_LONG_XNG_HIGH_KURT";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_13131_XTI_XNG_HKURT_HOST";
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_13131\",\"ea\":\"energy-kurt-rank\"}");
