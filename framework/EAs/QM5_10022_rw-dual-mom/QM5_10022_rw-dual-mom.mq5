...
int Strategy_MonthOf(const datetime t)
  {
   MqlDateTime dt;
   TimeToStruct(t, dt);
   return dt.mon;
...
double Strategy_Return6M(const string symbol)
  {
...
bool Strategy_SymbolSelected()
  {
   ...
   int ranks[4] = {0, 1, 2, 3};
   for(int i = 0; i < STRATEGY_UNIVERSE_SIZE; ++i)
      returns[i] = Strategy_Return6M(g_strategy_symbols[i]);
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_10022\",\"ea\":\"rw_dual_mom\"}");
