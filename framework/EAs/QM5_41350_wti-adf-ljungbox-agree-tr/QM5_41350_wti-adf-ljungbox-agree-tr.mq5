...
#property description "QM5_41350 WTI Monthly ADF-Ljung-Box Agreement Trend"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41350 - WTI Monthly ADF-Ljung-Box Agreement-Gated Trend
// -----------------------------------------------------------------------------
...
//   - require ADF t >= -2.594 and six-lag Ljung-Box Q >= 5.35
...
input int    strategy_ljungbox_return_count = 48;
input int    strategy_ljungbox_lags         = 6;
input double strategy_ljungbox_q_min        = 5.35;
...
struct Strategy_SignalMetrics
  {
   ...
   double se_gamma;
   double adf_t;
   int ljungbox_return_count;
   double ljungbox_return_mean;
   double ljungbox_denominator;
   double ljungbox_q;
   double momentum_12;
   string level_path;
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   else
     {
      ...
      if(!g_signal_valid)
         g_signal_state = "agreement_validation_failed";
              ...
              !g_signal_metrics.ljungbox_qualified)
         g_signal_state = "both_gates_reject_consumed_flat";
      else if(!g_signal_metrics.adf_qualified)
         g_signal_state = "adf_rejects_ljungbox_only_consumed_flat";
      else if(!g_signal_metrics.ljungbox_qualified)
         g_signal_state = "ljungbox_rejects_adf_only_consumed_flat";
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"endpoint_count\":%d,\"level_count\":%d,\"ljungbox_return_count\":%d,\"mean_y\":%.16f,\"mean_z\":%.16f,\"mean_w\":%.16f,\"szz\":%.16f,\"sww\":%.16f,\"szw\":%.16f,\"szy\":%.16f,\"swy\":%.16f,\"determinant\":%.16f,\"alpha\":%.16f,\"gamma\":%.16f,\"phi\":%.16f,\"sse\":%.16f,\"residual_variance\":%.16f,\"se_gamma\":%.16f,\"adf_t\":%.12f,\"adf_qualified\":%s,\"ljungbox_return_mean\":%.12f,\"ljungbox_denominator\":%.16f,\"ljungbox_q\":%.12f,\"ljungbox_qualified\":%s,\"persistence_qualified\":%s,\"momentum_12\":%.12f,\"levels\":\"%s\",\"returns\":\"%s\",\"regression\":\"%s\",\"autocorrelations\":\"%s\",\"oldest_close\":%.10f,\"newest_close\":%.10f,\"oldest_endpoint\":%I64d,\"newest_endpoint\":%I64d,\"state\":\"%s\"}",
                            ...
                            g_signal_metrics.ljungbox_return_count,
                            ...
                            g_signal_metrics.ljungbox_return_mean,
                            g_signal_metrics.ljungbox_denominator,
                            g_signal_metrics.ljungbox_q,
                            g_signal_metrics.ljungbox_qualified ? "true" : "false",
                            ...
                            g_signal_metrics.autocorrelation_path,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41350 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41350_WTI_AGREE";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41350 ||
      qm_magic_slot_offset != 0)
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI monthly ADF-Ljung-Box agreement trend holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41350_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   g_entry_month_state_key =
      StringFormat("QM5_41350_ENTRY_MONTH_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41350\",\"ea\":\"wti-adf-ljungbox-agree-tr\"}");
