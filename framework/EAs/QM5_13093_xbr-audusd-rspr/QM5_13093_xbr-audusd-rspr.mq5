...
#property description "QM5_13093 XBR AUDUSD Return Spread Reversion"
...
//   rspread = log(XBR[t] / XBR[t-L]) - beta_audusd * log(AUDUSD[t] / AUDUSD[t-L])
//   z > entry: short return spread = sell Brent, buy AUDUSD
//   z < -entry: long return spread = buy Brent, sell AUDUSD
   ...
   for(int i = 0; i < z_lookback; ++i)
     {
      ...
      if(xbr[i] <= 0.0 || audusd[i] <= 0.0 || xbr[past_idx] <= 0.0 || audusd[past_idx] <= 0.0)
         return false;

      const double xbr_ret = MathLog(xbr[i] / xbr[past_idx]);
      const double audusd_ret = MathLog(audusd[i] / audusd[past_idx]);
      spreads[i] = xbr_ret - strategy_beta_audusd * audusd_ret;
...
bool Strategy_OpenPair(const int spread_direction)
  {
   if(spread_direction == 0 || Strategy_OpenPairLegCount() > 0)
      return false;
   if(!Strategy_SpreadAllowed(g_leg_xbr) || !Strategy_SpreadAllowed(g_leg_audusd))
      return false;

   const double xbr_weight = 1.0;
   const double audusd_weight = MathMax(0.1, MathAbs(strategy_beta_audusd));
   const double weight_sum = xbr_weight + audusd_weight;
   ...
   const string reason = long_spread ? "QM5_13093_LONG_XBR_AUDUSD_RSPREAD"
                                     : "QM5_13093_SHORT_XBR_AUDUSD_RSPREAD";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_13093_RSPREAD_HOST";
...
int OnInit()
  {
   ...
   string basket_symbols[2] = {g_leg_xbr, g_leg_audusd};
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_13093\",\"ea\":\"xbr-audusd-rspr\"}");
