...
#property description "QM5_20239 WTI Pre-Pullback Trend"
...
//   - exact 12-month trend ending before the newest completed month
//   - newest completed month must move opposite that older trend
//   - enter in the older trend direction for one monthly package
...
input int    strategy_pullback_months       = 1;
...
bool Strategy_LoadSignal(const int current_month_key,
                         double &older_trend_return,
                         double &pullback_return,
                         int &trade_direction)
  {
   older_trend_return = 0.0;
   pullback_return = 0.0;
   trade_direction = 0;

   const int needed_closes =
      strategy_trend_months + strategy_pullback_months + 1;
   if(strategy_trend_months != 12 ||
      strategy_pullback_months != 1 ||
      needed_closes != 14 ||
      current_month_key <= 0)
      return false;
   ...
   pullback_return = MathLog(month_closes[0] / month_closes[1]);
   older_trend_return =
      MathLog(month_closes[1] /
              month_closes[strategy_trend_months + 1]);
   if(!MathIsValidNumber(pullback_return) ||
      !MathIsValidNumber(older_trend_return))
      return false;

   if(older_trend_return > 0.0 && pullback_return < 0.0)
      trade_direction = 1;
   else if(older_trend_return < 0.0 && pullback_return > 0.0)
      trade_direction = -1;
   return true;
  }
...
void Strategy_CloseExpiredPositions()
  {
   ...
   double older_trend_return = 0.0;
   double pullback_return = 0.0;
   int expected_direction = 0;
   const bool signal_valid =
      Strategy_LoadSignal(current_month_key,
                          older_trend_return,
                          pullback_return,
                          expected_direction);
   ...
   for(int index = PositionsTotal() - 1; index >= 0; --index)
     {
      ...
      if(signal_valid && expected_direction != 0)
        {
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsWtiD1())
      return true;
   if(qm_ea_id != 20239 || qm_magic_slot_offset != 0)
      return true;
   if(strategy_trend_months != 12 ||
      strategy_pullback_months != 1 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.reason = StringFormat("WTI_PULLTREND_%s_T%.6f_P%.6f",
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_20239_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   Strategy_LoadAttemptState();

   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_20239\",\"ea\":\"wti-pulltrend\"}");
