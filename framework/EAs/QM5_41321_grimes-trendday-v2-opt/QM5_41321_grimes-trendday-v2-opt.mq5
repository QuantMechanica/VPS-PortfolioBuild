...
#property description "QM5_41321 Grimes Compressed Trend-Day Breakout v2 - DL-089 opt sibling"
// Strategy Card: QM5_41321 (grimes-trendday-v2-opt), G0 APPROVED 2026-09-03.
// Source: Adam H. Grimes, Finding trend days in index futures, 2015-09-16.
//
// EXIT SURGERY v2 — parent QM5_10943
// Surgical delta: add strategy_min_session_hold_h = 4.0.
// Guard: do NOT place pending order if seconds remaining to session close
// (strategy_session_close_hour_broker : strategy_session_close_minute_broker)
// is less than strategy_min_session_hold_h * 3600.
//
// Evidence: EXIT_SURGERY_SCAN_2026-07-04.md §3.2; 120 trades, WR gradient
// 12% (<1h) -> 77% (4-12h). TIME_MGMT x39 in <1h bucket = pending orders
// placed late, filled, then closed at 22:45 within <1h. Blocking entries
// after 18:45 (22:45 - 4h) removes the late-entry noise; retains 17:30-18:45
// window where 77% WR winners are generated.
// All other logic and parameters IDENTICAL to parent QM5_10943.
...
// v2 surgical addition: minimum session time remaining before pending placement.
// Do not submit a pending order if fewer than this many hours remain before
// session close (strategy_session_close_hour_broker:strategy_session_close_minute_broker).
// Evidence: EXIT_SURGERY_SCAN_2026-07-04.md §3.2 — TIME_MGMT x39 in <1h bucket
// = late-entry pending orders filled and closed at 22:45 within <1h.
// Default 4.0h blocks entries after 18:45 for a 22:45 close.
input double strategy_min_session_hold_h         = 4.0;
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(strategy_atr_period_d1 <= 0 ||
      ...
      strategy_min_session_hold_h <= 0.0)
      return true;
   return false;
  }
...
// v2: adds session-lifetime guard before pending placement.
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   // v2 surgical guard: do not place pending if fewer than strategy_min_session_hold_h
   // hours remain before session close. This eliminates TIME_MGMT x39 kills in the
   // <1h hold bucket (EXIT_SURGERY_SCAN_2026-07-04.md §3.2).
   if(seconds_to_close < (int)(strategy_min_session_hold_h * 3600.0))
      return false;
   ...
   if(last_close > g_opening_range_high && last_close > g_prior_d1_high)
     {
      ...
      req.reason = "grimes_trendday_v2_long";
      req.symbol_slot = qm_magic_slot_offset;
   ...
   if(last_close < g_opening_range_low && last_close < g_prior_d1_low)
     {
      ...
      req.sl = stop;
      req.tp = NormalizeDouble(entry_ref - strategy_target_r_mult * risk, _Digits);
      req.reason = "grimes_trendday_v2_short";
      req.symbol_slot = qm_magic_slot_offset;
      req.expiration_seconds = expiry;
      g_trade_submitted_today = true;
      return true;
...
void Strategy_ManageOpenPosition()
  {
   ...
   if(valid && improves)
      QM_TM_MoveSL(ticket, normalized_trail_stop, "grimes_trendday_v2_prior3_trail");
  }
...
// News Filter Hook. No custom event logic beyond the central framework news filter.
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"ea\":\"QM5_41321_grimes-trendday-v2-opt\",\"surgery\":\"min_session_hold_4h\"}");
