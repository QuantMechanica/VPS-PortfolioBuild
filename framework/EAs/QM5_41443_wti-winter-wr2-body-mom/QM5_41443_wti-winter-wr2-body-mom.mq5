...
#property description "QM5_41443 WTI Winter WR2 Body Momentum"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41443 - WTI Winter WR2 Body Momentum
// -----------------------------------------------------------------------------
...
//   - follow an expanding newest week's own open-to-close body direction
...
bool Strategy_LoadWR2BodySignal(const int current_week_key,
                               const int label_offset,
                               int &direction)
  {
   direction = 0;
   ...
   if(g_strict_range_expansion && g_newer_week_body > 0.0)
      direction = 1;
   else if(g_strict_range_expansion && g_newer_week_body < 0.0)
      direction = -1;
   return true;
  }
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_WeekAlreadyEntered(g_decision_week_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadWR2BodySignal(g_decision_week_key,
                                   g_decision_label_offset,
                                   g_signal_direction);
      if(!g_signal_valid)
         g_signal_state = "two_completed_weeks_validation_failed";
      else if(g_signal_direction > 0)
         g_signal_state = "wr2_positive_body_continuation_long";
      else if(g_signal_direction < 0)
         g_signal_state = "wr2_negative_body_continuation_short";
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"week\":%d,\"anchor_month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_week_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"week_bars\":[%d,%d],\"week_ranges\":[%.8f,%.8f],\"new_open\":%.8f,\"new_high\":%.8f,\"new_low\":%.8f,\"new_close\":%.8f,\"new_body\":%.8f,\"strict_range_expansion\":%s,\"state\":\"%s\"}",
                            g_decision_week_key,
                            anchor_month,
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41443_WTI_WINTER_WR2_BODY_MOM";
   ...
   req.reason = (g_signal_direction > 0
                 ? "WTI_WINTER_WR2_BODY_MOM_LONG"
                 : "WTI_WINTER_WR2_BODY_MOM_SHORT");
...
int OnInit()
  {
   if(!SymbolSelect(strategy_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41443 ||
      qm_magic_slot_offset != 0)
   ...
   g_attempt_state_key =
      StringFormat("QM5_41443_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41443\",\"ea\":\"wti-winter-wr2-body-mom\"}");
