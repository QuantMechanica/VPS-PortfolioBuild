...
#property description "QM5_41091 WTI Weekly Inside-Body Momentum"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41091 - WTI Weekly Inside-Body Momentum
// -----------------------------------------------------------------------------
// D1 structural crude-oil sleeve:
//   - aggregate two consecutive completed broker-week OHLC packages
//   - require the newest high/low interval to sit strictly inside its parent
//   - follow the contained week's strict first-open to final-close direction
//   - consume the week before fallible gates; never enter late or retry
//   - hold the completed compression/body signal for one broker week
// Runtime uses MT5-native OHLC, calendar, history, and framework state only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                      = 41091;
...
bool Strategy_LoadInsideBodySignal(const int current_week_key,
                                  const int label_offset,
                                  int &direction,
                                  int &new_week_bars,
                                  ...
                                  bool &strict_inside)
  {
   ...
   strict_inside = (new_high < parent_high && new_low > parent_low);
   if(!strict_inside)
      return true;

   const double new_body = new_close - new_open;
   if(!MathIsValidNumber(new_body))
      return false;
   if(new_body > 0.0)
      direction = 1;
   else if(new_body < 0.0)
      direction = -1;
   return true;
  }
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_WeekAlreadyEntered(g_decision_week_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadInsideBodySignal(g_decision_week_key,
      ...
      if(!g_signal_valid)
         ...
         g_signal_state = "strict_inside_bull_body_long";
      else if(g_signal_direction < 0)
         g_signal_state = "strict_inside_bear_body_short";
      else if(!g_strict_inside)
         g_signal_state = "non_inside_flat";
      else
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"week\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_week_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"new_week_bars\":%d,\"parent_week_bars\":%d,\"new_open\":%.8f,\"new_high\":%.8f,\"new_low\":%.8f,\"new_close\":%.8f,\"parent_open\":%.8f,\"parent_high\":%.8f,\"parent_low\":%.8f,\"parent_close\":%.8f,\"strict_inside\":%s,\"state\":\"%s\"}",
                            g_decision_week_key,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41091 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41091_WTI_WINSIDE_BODY_MOM";
   ...
   req.reason = (g_signal_direction > 0)
      ? "WTI_WINSIDE_BODY_MOM_LONG"
      : "WTI_WINSIDE_BODY_MOM_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41091 ||
   ...
   g_attempt_state_key =
      StringFormat("QM5_41091_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41091\",\"ea\":\"wti-winside-body-mom\"}");
