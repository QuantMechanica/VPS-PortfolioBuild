"""Deterministic reference checks for QM5_41091 weekly inside-body momentum."""
...
def inside_body_signal(
    ...
    """Mirror bounded weekly OHLC aggregation and strict inside/body signal."""
    ...
    strict_inside = new_high < parent_high and new_low > parent_low
    direction = 0
    if strict_inside and new_close > new_open:
        direction = 1
    elif strict_inside and new_close < new_open:
        direction = -1
    values = (
        new_open,
        new_high,
    ...
    return True, direction, (len(buckets[0]), len(buckets[1])), values, strict_inside
...
def sample(
    new: tuple[float, float, float, float] = (100.0, 115.0, 85.0, 112.0),
...
class WeekInsideBodyReferenceTest(unittest.TestCase):
    def signal(self, bars: list[Bar], offset: timedelta = timedelta(0)):
        return inside_body_signal(20260824, bars, offset)

    def test_strict_long_and_short_paths(self) -> None:
        _, _, bars = sample()
        valid, direction, counts, values, inside = self.signal(bars)
        self.assertTrue(valid)
        self.assertTrue(inside)
        self.assertEqual(direction, 1)
        self.assertEqual(counts, (5, 5))
        self.assertEqual(values, (100.0, 115.0, 85.0, 112.0, 95.0, 120.0, 80.0, 100.0))

        _, _, bars = sample(new=(110.0, 115.0, 85.0, 88.0))
    ...
    def test_three_four_and_five_session_weeks_are_accepted(self) -> None:
        for new_count in (3, 4, 5):
            for parent_count in (3, 4, 5):
                ...
                valid, direction, counts, _, inside = self.signal(bars)
                self.assertTrue(valid)
                self.assertTrue(inside)
                self.assertEqual(direction, 1)
    ...
    def test_equal_endpoint_and_non_inside_geometries_are_flat(self) -> None:
        cases = [
            (100.0, 120.0, 85.0, 112.0),
            (100.0, 115.0, 80.0, 112.0),
            (100.0, 121.0, 85.0, 112.0),
            (100.0, 110.0, 75.0, 108.0),
        ]
        for new in cases:
            _, _, bars = sample(new=new)
            valid, direction, _, _, inside = self.signal(bars)
            self.assertTrue(valid)
            self.assertFalse(inside)
            self.assertEqual(direction, 0)

    def test_inside_body_equality_is_flat(self) -> None:
        _, _, bars = sample(new=(100.0, 115.0, 85.0, 100.0))
        valid, direction, _, _, inside = self.signal(bars)
        self.assertTrue(valid)
        self.assertTrue(inside)
        self.assertEqual(direction, 0)
    ...
    def test_static_build_contract_is_fixed_risk_and_completed_data_only(self) -> None:
        source = (EA_DIR / "QM5_41091_wti-winside-body-mom.mq5").read_text(encoding="utf-8")
        setfile = (EA_DIR / "sets" / "QM5_41091_wti-winside-body-mom_XTIUSD.DWX_D1_backtest.set").read_text(encoding="utf-8")
        for marker in (
        ...
        self.assertNotIn("new_midpoint", source)
        self.assertNotIn("strategy_clv_", source)
        self.assertLess(
            source.index("Strategy_RecordWeekAttempt(g_decision_week_key)"),
            source.index("Strategy_LoadInsideBodySignal(g_decision_week_key"),
        )
        for marker in (
            "qm_ea_id=41091",
            "RISK_FIXED=1000",
