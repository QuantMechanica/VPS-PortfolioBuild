...
#property description "QM5_41225 XNG Median Same-Calendar Seasonality"
...
//   - skip missing exact years and require at least five observations
//   - trade the ordinary bounded sample median's sign
...
// -----------------------------------------------------------------------------
// Exact completed-month endpoint and median helpers.
// -----------------------------------------------------------------------------
...
bool Strategy_LoadMedianSignal(const datetime decision_bar_time,
                               ...
                               int &direction)
  {
   ...
   if(!TimeToStruct(normalized_decision, decision_parts) ||
      decision_parts.year - strategy_lookback_years < 1900 ||
   ...
   double observations[];
   if(ArrayResize(observations, strategy_lookback_years) !=
         strategy_lookback_years ||
      ArraySize(observations) < strategy_lookback_years)
      return false;
   ...
   return Strategy_MedianSignal(observations,
                                sample_count,
                                seasonal_median,
                                direction);
  }
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadMedianSignal(g_decision_bar_time,
      ...
      if(!g_signal_valid)
         ...
         g_signal_state = "median_long";
      else if(g_signal_direction < 0)
         g_signal_state = "median_short";
      else
         g_signal_state = "median_tie_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"valid\":%s,\"samples\":%d,\"median\":%.12e,\"direction\":%d,\"state\":\"%s\"}",
                            g_decision_month_key,
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(!QM_InputRequireLong("qm_ea_id", qm_ea_id, 41225) ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(g_signal_direction > 0)
     {
      req.type = QM_BUY;
      req.reason = "XNG_MEDCAL_LONG";
     }
   else
     {
      req.type = QM_SELL;
      req.reason = "XNG_MEDCAL_SHORT";
     }
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41225 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XNG median same-calendar card holds to the next broker month"))
      return INIT_FAILED;
   ...
   g_attempt_state_key =
      StringFormat("QM5_41225_MEDCAL_MONTH_ATTEMPT_%d",
                   QM_FrameworkMagic());
...
void OnTick()
  {
   ...
   // gate. Both axes are locked OFF for the Q02 baseline.
   const datetime broker_now = TimeCurrent();
