...
#property description "QM5_41112 XAU/XAG Completed-Month Daily Relative-Sign Breadth Reversion"
...
// consecutive completed calendar months. Use the parent-final log ratio as the
// anchor for every daily relative return in the newest completed month. Fade
// only a strict sign majority whose endpoint displacement agrees, using one
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41112 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_MDAYBREADTH_RV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41112_XAUXAG_MDAYBREADTH_RV";
...
bool Strategy_LoadMonthlyRatioBreadth(
      ...
      int &direction)
  {
   ...
   double previous_ratio = parent_final_ratio;
   for(int series_index = completed_month_sessions - 1;
       series_index >= 0;
       --series_index)
     {
      ...
      if(relative_return > 0.0)
         ++positive_count;
      else if(relative_return < 0.0)
         ++negative_count;
      previous_ratio = newest_ratios[series_index];
     }
   ...
   // fade only a strict daily-sign majority whose endpoint displacement agrees.
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41112_XAUXAG_MDAYBREADTH_RV_HOST";
   ...
   int direction = 0;
   double endpoint_net = 0.0;
   const bool valid =
      Strategy_LoadMonthlyRatioBreadth(g_signal_month_key,
                                       completed_month_sessions,
                                       parent_month_sessions,
                                       positive_count,
                                       negative_count,
                                       endpoint_net,
                                       direction);
   string state = "monthly_ratio_breadth_failed";
   if(valid && direction > 0)
      state = "negative_daily_majority_fade_long_xau";
   else if(valid && direction < 0)
      state = "positive_daily_majority_fade_short_xau";
   else if(valid)
      state = "majority_or_endpoint_not_confirmed_flat";

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"completed_month_sessions\":%d,\"parent_month_sessions\":%d,\"positive\":%d,\"negative\":%d,\"endpoint_net\":%.12e,\"direction\":%d,\"state\":\"%s\"}",
                            g_signal_month_key,
                            completed_month_sessions,
                            parent_month_sessions,
                            positive_count,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG completed-month daily relative-sign breadth reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41112\",\"ea\":\"xauxag-mdaybreadth-rv\"}");
