...
#property description "QM5_12806 XNG Reverse Weekend Effect"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_12806 - Natural Gas Reverse Weekend Effect
// -----------------------------------------------------------------------------
// D1 structural weekday sleeve:
//   - buy XNGUSD.DWX on broker-calendar Monday
//   - sell XNGUSD.DWX on broker-calendar Friday
...
input bool   strategy_enable_monday_long   = true;
input bool   strategy_enable_friday_short  = true;
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      const int opened_dow = Strategy_DayOfWeek(opened);
      bool should_close = (current_dow != opened_dow);
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(strategy_max_hold_days <= 0)
      return true;
   if(!strategy_enable_monday_long && !strategy_enable_friday_short)
      return true;
   return false;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12806_XNG_REV_WEEKEND";
   ...
   const int dow = Strategy_DayOfWeek(current_d1_bar);
   if(dow == 1 && strategy_enable_monday_long)
      req.type = QM_BUY;
   else if(dow == 5 && strategy_enable_friday_short)
      req.type = QM_SELL;
   else
      return false;
   ...
   req.reason = (req.type == QM_BUY) ? "XNG_REVERSE_WEEKEND_MONDAY_LONG"
                                     : "XNG_REVERSE_WEEKEND_FRIDAY_SHORT";
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_12806\",\"ea\":\"xng-rev-weekend\"}");
