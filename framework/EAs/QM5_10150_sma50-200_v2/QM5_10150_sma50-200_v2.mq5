...
#property description "QM5_10150_v2 SMA 50/200 Golden-Cross Trend Filter"
...
// No Trade Filter (time, spread, news): the framework handles news, kill-switch,
// and Friday close; this hook applies the card's spread gate when configured.
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(state == 0)
      return false;
   if(state < 0 && !strategy_shorts_enabled)
      return false;
...
bool Strategy_ExitSignal()
  {
   ...
   const int state = Strategy_SmaState(1);
   if(position_type == POSITION_TYPE_BUY && state <= 0)
      return true;
   if(position_type == POSITION_TYPE_SELL && state >= 0)
      return true;
