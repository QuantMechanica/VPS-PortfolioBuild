...
#property description "QM5_41227 WTI Same-Calendar Rolling Two-Year Block Median"
...
//   - form four overlapping chronological two-year arithmetic means
//   - sort only those four means and average the two central values
//   - follow only when the even block median is beyond +/-1e-12
...
input int    strategy_rolling_years       = 2;
input int    strategy_rolling_count       = 4;
...
// -----------------------------------------------------------------------------
// Exact completed-month endpoint and rolling block-median helpers.
// -----------------------------------------------------------------------------
...
bool Strategy_RollingBlockMedianSignal(const double &observations[],
                                       ...
                                       int &direction)
  {
   ...
   if(strategy_history_years != 5 ||
      ...
      strategy_history_years != strategy_rolling_count + 1 ||
      ArraySize(observations) < sample_count)
      return false;

   double sorted_means[];
   if(ArrayResize(sorted_means, strategy_rolling_count) !=
         strategy_rolling_count)
      return false;
   ...
   for(int index = 0; index < strategy_rolling_count; ++index)
     {
      if(index + 1 >= sample_count)
         return false;
      const double rolling_mean =
         (observations[index] + observations[index + 1]) / 2.0;
      if(!MathIsValidNumber(rolling_mean))
         return false;
      sorted_means[index] = rolling_mean;
     }

   ArraySort(sorted_means);
   if(ArraySize(sorted_means) != strategy_rolling_count ||
      !MathIsValidNumber(sorted_means[1]) ||
      !MathIsValidNumber(sorted_means[2]))
      return false;

   location_value = (sorted_means[1] + sorted_means[2]) / 2.0;
...
bool Strategy_LoadRollingBlockMedianSignal(
   ...
   int &direction)
  {
   ...
   // Oldest to newest is load-bearing because rolling pairs are chronological.
   ...
   return Strategy_RollingBlockMedianSignal(observations,
                                            sample_count,
                                            location_value,
                                            direction);
  }
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   // Consume before history, rolling-mean arithmetic, news, spread, quote,
   ...
   else
     {
      g_signal_valid =
         Strategy_LoadRollingBlockMedianSignal(
            ...
            g_signal_direction);
      if(!g_signal_valid)
         g_signal_state = "endpoint_sample_or_block_median_failed";
      else if(g_signal_direction > 0)
         g_signal_state = "rolling_block_median_long";
      else if(g_signal_direction < 0)
         g_signal_state = "rolling_block_median_short";
      else
         g_signal_state = "rolling_block_median_epsilon_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"valid\":%s,\"samples\":%d,\"rolling_block_median\":%.12g,\"direction\":%d,\"state\":\"%s\"}",
                            g_decision_month_key,
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(!QM_InputRequireLong("qm_ea_id", qm_ea_id, 41227) ||
      ...
      !QM_InputRequireLong("strategy_rolling_years", strategy_rolling_years, 2) ||
      !QM_InputRequireLong("strategy_rolling_count", strategy_rolling_count, 4) ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "WTI_SAMECAL_BLOCKMED";
   ...
   if(g_signal_direction > 0)
     {
      req.type = QM_BUY;
      req.reason = "WTI_SAMECAL_BLOCKMED_LONG";
     }
   else
     {
      req.type = QM_SELL;
      req.reason = "WTI_SAMECAL_BLOCKMED_SHORT";
     }
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41227 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI same-calendar rolling two-year block-median card holds to the next broker month"))
      return INIT_FAILED;
   ...
   g_attempt_state_key =
      StringFormat("QM5_41227_SAMECAL_BLOCKMED_MONTH_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41227\",\"ea\":\"wti-samecal-blockmed\"}");
