...
#property description "QM5_41053 WTI Post-Wednesday Counter-Gap Fade"
...
//   - measure completed Wednesday event-session flow and the later frozen
//     Wednesday-close-to-Thursday-open gap
...
bool Strategy_LoadPostWednesdayCounterGapFade(const datetime current_bar_time,
                                             const datetime broker_now,
                                             ...
                                             int &direction)
  {
   ...
   // The only signal endpoints are the completed Wednesday open/close and
   // the current D1 bar's frozen opening price. Tuesday and Monday prove the
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(!QM_InputRequireLong("qm_ea_id", qm_ea_id, 41053) ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "WTI_POSTWED_GAP_FADE";
   ...
   int direction = 0;
   if(!Strategy_LoadPostWednesdayCounterGapFade(g_strategy_d1_bar_time,
                                               broker_now,
                                               event_session_flow,
                                               post_event_gap,
                                               confirmed_path,
   ...
   if(direction > 0)
     {
      req.type = QM_BUY;
      req.reason = "WTI_POSTWED_GAP_FADE_LONG";
     }
   else if(direction < 0)
     {
      req.type = QM_SELL;
      req.reason = "WTI_POSTWED_GAP_FADE_SHORT";
     }
   else
      return false;
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI post-Wednesday counter-gap-fade card exits next D1; Friday 21 is fail-safe"))
      return INIT_FAILED;
   ...
   g_attempt_state_key =
      StringFormat("QM5_41053_POSTWED_GAP_FADE_THURSDAY_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41053\",\"ea\":\"wti-postwed-gap-fade\"}");
