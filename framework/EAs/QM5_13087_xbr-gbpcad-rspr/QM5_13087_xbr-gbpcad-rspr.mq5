...
#property description "QM5_13087 XBR GBPCAD Return Spread Reversion"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_13087 - Brent/GBPCAD Return-Spread Reversion
// -----------------------------------------------------------------------------
// D1 two-leg energy/FX basket:
//   rspread = log(XBR[t] / XBR[t-L]) + beta_gbpcad * log(GBPCAD[t] / GBPCAD[t-L])
//   z > entry: short return spread = sell Brent, sell GBPCAD
//   z < -entry: long return spread = buy Brent, buy GBPCAD
...
input double strategy_beta_gbpcad         = 0.60;
...
bool Strategy_SpreadAllowed(const string symbol)
  {
   ...
   if(symbol == g_leg_gbpcad && strategy_gbpcad_max_spread_pts > 0)
      return (spread_points <= strategy_gbpcad_max_spread_pts);
   return true;
  }
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(CopyClose(g_leg_xbr, PERIOD_D1, 1, history_needed, xbr) != history_needed) // perf-allowed: called only behind the D1 new-bar gate or close-state refresh.
      return false;
   if(CopyClose(g_leg_gbpcad, PERIOD_D1, 1, history_needed, gbpcad) != history_needed) // perf-allowed: called only behind the D1 new-bar gate or close-state refresh.
      return false;
   ...
   for(int i = 0; i < z_lookback; ++i)
     {
      ...
      if(xbr[i] <= 0.0 || gbpcad[i] <= 0.0 || xbr[past_idx] <= 0.0 || gbpcad[past_idx] <= 0.0)
         return false;

      const double xbr_ret = MathLog(xbr[i] / xbr[past_idx]);
      const double gbpcad_ret = MathLog(gbpcad[i] / gbpcad[past_idx]);
      spreads[i] = xbr_ret + strategy_beta_gbpcad * gbpcad_ret;
...
bool Strategy_OpenPair(const int spread_direction)
  {
   if(spread_direction == 0 || Strategy_OpenPairLegCount() > 0)
      return false;
   if(!Strategy_SpreadAllowed(g_leg_xbr) || !Strategy_SpreadAllowed(g_leg_gbpcad))
      return false;

   const double xbr_weight = 1.0;
   const double gbpcad_weight = MathMax(0.1, MathAbs(strategy_beta_gbpcad));
   const double weight_sum = xbr_weight + gbpcad_weight;
   ...
   const string reason = long_spread ? "QM5_13087_LONG_XBR_GBPCAD_RSPREAD"
                                     : "QM5_13087_SHORT_XBR_GBPCAD_RSPREAD";
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart())
      return true;
   if(strategy_return_lookback_d1 < 5 || strategy_z_lookback_d1 < 40 || strategy_beta_gbpcad <= 0.0)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_13087_RSPREAD_HOST";
...
int OnInit()
  {
   ...
   string basket_symbols[2] = {g_leg_xbr, g_leg_gbpcad};
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_13087\",\"ea\":\"xbr-gbpcad-rspr\"}");
