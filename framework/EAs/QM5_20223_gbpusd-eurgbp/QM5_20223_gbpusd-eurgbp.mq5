...
#property description "QM5_20223 GBPUSD EURGBP Cointegration"
...
//
// Fixed spread:
//   ln(GBPUSD.DWX) - beta * ln(EURGBP.DWX)
//
...
input double strategy_beta              = -0.399228065;
...
bool Strategy_EnsureBasketScope()
  {
   ...
   string allowed[2] = {"GBPUSD.DWX", "EURGBP.DWX"};
   for(int i = 0; i < 2; ++i)
...
bool Strategy_RefreshSpreadState()
  {
   ...
   for(int i = 0; i < history_count; ++i)
     {
      if(gbpusd_time[i] <= 0 || gbpusd_time[i] != eurgbp_time[i])
         return false;
      if(gbpusd[i] <= 0.0 || eurgbp[i] <= 0.0)
         return false;

      spreads[i] =
         MathLog(gbpusd[i]) - strategy_beta * MathLog(eurgbp[i]);
...
bool Strategy_PreparePair(const int spread_direction,
                          QM_EntryRequest &host_req)
  {
   ...
   const double weight_sum = gbpusd_weight + eurgbp_weight;
   if(weight_sum <= 0.0)
      return false;
   ...
   const double eurgbp_lots =
      Strategy_LotsForLeg(g_leg_eurgbp, eurgbp_weight, gbpusd_weight);
   if(gbpusd_lots <= 0.0 || eurgbp_lots <= 0.0)
      return false;
   ...
   const string reason =
      long_spread ? "QM5_20223_LONG_SPREAD_Z_LT_NEG_ENTRY"
                  : "QM5_20223_SHORT_SPREAD_Z_GT_POS_ENTRY";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20223_PAIR_HOST";
