# QM5_20223_gbpusd-eurgbp — Strategy Spec
...
`ln(GBPUSD.DWX) - (-0.399228065) * ln(EURGBP.DWX)`. Score the newest spread
...
| `strategy_beta` | -0.399228065 | fixed | DEV-fitted structural hedge coefficient |
...
The GBPUSD traded history also supplies the GBP-to-USD conversion path for the
EURGBP contract; no order-free conversion symbol or magic slot is declared.
...
| Typical hold time | multi-day; scan half-life 149.505 D1 bars |
| Expected drawdown profile | high and regime-sensitive; conservative 30% prior |
| Regime preference | GBPUSD/EURGBP fixed-residual mean reversion |
| Win rate target | unknown; Q02 is the first platform-economic judge |

The fixed scan recorded DEV net Sharpe `-0.078883`, OOS net Sharpe
`-0.098985`, and OOS return `-0.844505%`. Both window scores and the slow
half-life are adverse priors, not a promotion claim.
...
`strategy-seeds/cards/approved/QM5_20223_gbpusd-eurgbp_card.md`

Chan supplies the deterministic pair-trading method but makes no performance
claim for GBPUSD/EURGBP.
...
`1.0` and `0.399228065`; each leg is independently bounded by its ATR stop. If
the EURGBP allocation normalizes below broker minimum volume, the whole
...
|---|---|---|---|
| v1 | 2026-08-05 | Initial build from approved card | Rank-44 first-gap fixed-scan FX basket |
