...
#property description "QM5_41390 WTI Eleven-Month Trend / Two-Month Hold"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41390 - WTI Eleven-Month Trend / Two-Month Hold
// -----------------------------------------------------------------------------
...
//   1. derive twelve consecutive completed WTI month-end closes from D1;
//   2. calculate the exact eleven-month return ln(C[11] / C[0]);
...
// The exact eleven-month endpoint pair remains independent of the two-month clock.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41390;
...
input int    strategy_return_months            = 11;
...
bool Strategy_IsEligibleDecisionMonth(const int month_key)
  {
   const int month = month_key % 100;
   if(month_key < 190001 || month < 1 || month > 12)
      return false;
   if(strategy_hold_months != 2 ||
      strategy_rebalance_month_parity != 1)
      return false;
   return ((month % strategy_hold_months) ==
...
bool Strategy_IsManagedPosition()
  {
   return (Strategy_IsOwnedPosition() &&
           PositionGetString(POSITION_SYMBOL) == strategy_symbol);
  }
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      if(ticket == 0 ||
         ...
         continue;
      if(PositionGetString(POSITION_SYMBOL) != strategy_symbol)
         return false;
...
bool Strategy_PeriodAlreadyAttempted(const int month_key)
  {
   if(!Strategy_IsEligibleDecisionMonth(month_key) ||
...
void Strategy_LoadAttemptState(const datetime reference_time)
  {
   ...
   if(current_month_key > 0 &&
      ...
      Strategy_IsEligibleDecisionMonth(stored_month_key))
     {
...
bool Strategy_RecordPeriodAttempt(const int month_key)
  {
   if(!Strategy_IsEligibleDecisionMonth(month_key) ||
      g_attempt_state_key == "")
      return false;
...
bool Strategy_AreConsecutiveMonths(const int &month_keys[],
                                   ...
                                   const int count)
  {
   if(start < 0 ||
      count <= 1 ||
      start + count > ArraySize(month_keys))
      return false;

   for(int offset = 1; offset < count; ++offset)
     {
      const int expected_key =
         Strategy_NextMonthKey(month_keys[start + offset - 1]);
      if(expected_key <= 0 ||
         month_keys[start + offset] != expected_key)
         return false;
     }
   return true;
  }
...
bool Strategy_LoadMonthlyCloses(double &closes[])
  {
   ...
   if(current_month_key <= 0 ||
      expected_previous_month <= 0)
      return false;

   MqlRates rates[];
   ArraySetAsSeries(rates, false);
   // perf-allowed: one bounded bulk read, eligible odd-month new-bar path only.
   const int copied = CopyRates( // perf-allowed: gated by the caller's new-calendar/new-bar path.
                                _Symbol, PERIOD_D1, 1,
                                strategy_history_bars_d1, rates);
   if(copied <= 0)
      return false;
   ...
   for(int i = 0; i < copied; ++i)
     {
      ...
      const int month_key =
         Strategy_MonthKeyForTime(rates[i].time);
      if(month_key <= 0 || month_key > current_month_key)
         return false;

      // Never use a partial current broker month, including restart mid-month.
      if(month_key == current_month_key)
         continue;

      if(month_count <= 0 ||
         month_keys[month_count - 1] != month_key)
        {
   ...
   for(int i = start + 1; i < month_count; ++i)
      if(month_end_times[i] <= month_end_times[i - 1])
         return false;
   if(month_keys[month_count - 1] != expected_previous_month)
      return false;
...
void Strategy_ResetCachedState()
  {
   ...
   g_cache_signal = 0;
   g_cache_eleven_month_return = 0.0;
   g_cache_state_reason = "not_evaluated";
  }

bool Strategy_ElevenMonthReturnSignal(const double &closes[],
                                      int &signal)
  {
   signal = 0;
   if(strategy_return_months != 11 ||
      ArraySize(closes) != 12)
     {
      g_cache_state_reason = "bad_eleven_month_contract";
      return false;
     }
   ...
   const double endpoint_ratio = closes[11] / closes[0];
   ...
   const double eleven_month_return = MathLog(endpoint_ratio);
   if(!MathIsValidNumber(eleven_month_return))
     {
      g_cache_state_reason = "invalid_eleven_month_return";
      return false;
     }
   ...
   if(!MathIsValidNumber(chained_return) ||
      MathAbs(eleven_month_return - chained_return) > 1.0e-10)
     {
      g_cache_state_reason = "endpoint_path_identity_failed";
      return false;
     }

   g_cache_eleven_month_return = eleven_month_return;
   if(eleven_month_return == 0.0)
     {
      g_cache_state_reason = "exact_zero_eleven_month_return";
      return true;
     }

   signal = (eleven_month_return > 0.0) ? 1 : -1;
...
bool Strategy_LoadSignalState(int &signal)
  {
   ...
   return Strategy_ElevenMonthReturnSignal(monthly_closes,
                                           signal);
  }

void Strategy_DetectBimonthlyRebalance_OnNewBar()
  {
   g_bimonthly_rebalance_bar = false;
   g_cache_month_key = 0;
   ...
   const int current_month_key =
      QM_CalendarPeriodKey(PERIOD_MN1, _Symbol, 0);
   if(!Strategy_IsEligibleDecisionMonth(current_month_key))
      return;

   g_bimonthly_rebalance_bar = true;
   g_cache_month_key = current_month_key;
  }

void Strategy_PrepareBimonthlySignal()
  {
   if(!g_bimonthly_rebalance_bar ||
      !Strategy_IsEligibleDecisionMonth(g_cache_month_key))
      return;
   if(Strategy_PeriodAlreadyAttempted(g_cache_month_key))
     {
      g_cache_state_reason = "period_already_consumed";
      return;
     }
   if(!Strategy_RecordPeriodAttempt(g_cache_month_key))
     {
   ...
   QM_LogEvent(QM_INFO,
               "MONTHLY_STATE",
               StringFormat("{\"month\":%d,\"valid\":%s,\"signal\":%d,\"return_11m\":%.10f,\"state\":\"%s\"}",
                            g_cache_month_key,
                            g_cache_signal_valid ? "true" : "false",
                            g_cache_signal,
                            g_cache_eleven_month_return,
                            g_cache_state_reason));
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41390_WTI_TSMOM11_H2";
...
void Strategy_ManageOpenPosition()
  {
   ...
   // Only an eligible odd-month edge rolls the package. Even-month changes
   // deliberately preserve the original position and hard stop.
   if(g_bimonthly_rebalance_bar &&
      Strategy_IsEligibleDecisionMonth(g_cache_month_key) &&
      Strategy_MonthKeyForTime(entry_time) != g_cache_month_key)
     {
      Strategy_CloseOwnedPositions(QM_EXIT_STRATEGY);
      return;
     }
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41390_BIMONTH_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   string warmup_symbols[1] = {strategy_symbol};
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41390\",\"ea\":\"wti-tsmom11-h2\"}");
...
void OnTick()
  {
   ...
   g_bimonthly_rebalance_bar = false;
   if(new_bar)
      Strategy_DetectBimonthlyRebalance_OnNewBar();

   // Lifecycle repair and eligible odd-month liquidation precede entry gates.
   ...
   if(!new_bar)
      return;
   if(g_bimonthly_rebalance_bar)
      Strategy_PrepareBimonthlySignal();

   // The period attempt is persisted before news, spread, quote, and sizing.
...
void OnTradeTransaction(const MqlTradeTransaction &trans,
  ...
  {
   QM_FrameworkOnTradeTransaction(trans,
                                  request,
                                  result);
  }
