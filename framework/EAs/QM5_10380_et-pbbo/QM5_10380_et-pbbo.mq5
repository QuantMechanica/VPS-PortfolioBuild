...
#property description "QM5_10380 Elite Trader previous-bar breakout"
...
input double strategy_catastrophic_atr_mult   = 1.50;
input double strategy_range_max_atr_mult      = 1.25;
input double strategy_min_range_spread_mult   = 4.00;
...
bool Strategy_IsInSession(const datetime broker_time)
  {
   MqlDateTime dt;
...
double Strategy_LongStop(const double entry_price)
  {
   ...
   const double catastrophic = (atr > 0.0 && strategy_catastrophic_atr_mult > 0.0)
                               ? entry_price - strategy_catastrophic_atr_mult * atr
                               : 0.0;
   if(bar_low > 0.0 && bar_low < entry_price)
     {
      if(catastrophic > 0.0)
         return MathMax(bar_low, catastrophic);
      return bar_low;
     }
   return catastrophic;
  }

double Strategy_ShortStop(const double entry_price)
  {
   ...
   if(bar_high > entry_price)
     {
      if(catastrophic > 0.0)
         return MathMin(bar_high, catastrophic);
      return bar_high;
     }
   return catastrophic;
  }

bool Strategy_RangeAllowsEntry()
  {
   const double high1 = iHigh(_Symbol, _Period, 1);
   const double low1 = iLow(_Symbol, _Period, 1);
   const double atr = QM_ATR(_Symbol, _Period, strategy_atr_period, 1);
   const double spread = Strategy_SpreadPrice();
   if(high1 <= 0.0 || low1 <= 0.0 || high1 <= low1 || atr <= 0.0 || spread <= 0.0)
      return false;

   const double range = high1 - low1;
   if(range < strategy_min_range_spread_mult * spread)
      return false;
   if(range > strategy_range_max_atr_mult * atr)
      return false;
   return true;
  }
...
bool Strategy_BuildStopRequest(const QM_OrderType type,
                               ...
                               QM_EntryRequest &req)
  {
   ...
   req.sl = (type == QM_BUY_STOP) ? Strategy_LongStop(req.price)
                                  : Strategy_ShortStop(req.price);
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsInSession(TimeCurrent()))
      return true;
   return (Strategy_SpreadPrice() <= 0.0);
  }

// Trade Entry: prior completed-bar high/low stop bracket.
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   Strategy_CancelPendingStops("new_bar_reprice");

   if(!Strategy_RangeAllowsEntry())
      return false;
   ...
   const double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
   const double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   if(tick <= 0.0 || high1 <= 0.0 || low1 <= 0.0 || ask <= 0.0 || bid <= 0.0)
      return false;

   const double offset = MathMax(1, strategy_entry_offset_ticks) * tick;
   const double buy_stop = high1 + offset;
   const double sell_stop = low1 - offset;
   if(buy_stop <= ask + tick || sell_stop >= bid - tick)
      return false;
   ...
   if(!Strategy_BuildStopRequest(QM_BUY_STOP, buy_stop, expiry_seconds, "ET_PBBO_BUY_STOP", buy_req))
      return false;
   if(!Strategy_BuildStopRequest(QM_SELL_STOP, sell_stop, expiry_seconds, "ET_PBBO_SELL_STOP", req))
      return false;
...
// Trade Management: OCO cleanup and bar-extreme stop maintenance.
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      const double target_sl = (ptype == POSITION_TYPE_BUY) ? Strategy_LongStop(entry)
                                                            : Strategy_ShortStop(entry);
      if(target_sl <= 0.0)
         continue;
      ...
      const bool improves = (current_sl <= 0.0) ||
                            (ptype == POSITION_TYPE_BUY
                             ...
                             : target_sl < current_sl - point * 0.5);
      if(improves)
         QM_TM_MoveSL(ticket, NormalizeDouble(target_sl, _Digits), "pbbo_bar_extreme_stop");
     }
  }

// Trade Close: exits are broker SL updates and framework Friday/session close.
