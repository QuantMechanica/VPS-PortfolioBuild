...
slug: xng-fri-trend
type: strategy
strategy_id: BOROWSKI-MOP-XNG-FRITREND-2026_S01
variant_id: BOROWSKI-MOP-XNG-FRITREND-2026_S01
source_id: BOROWSKI-MOP-XNG-FRITREND-2026
status: DRAFT
g0_status: APPROVED
execution_contract_ref: strategy-seeds/cards/approved/QM5_20160_xng-fri-trend_card.md
execution_contract_status: DRAFT
created: 2026-07-25
created_by: Research+Development
last_updated: 2026-08-16
source_authors: "Krzysztof Borowski; Tobias J. Moskowitz, Yao Hua Ooi, Lasse Heje Pedersen"
strategy_mechanic: friday-xng-short-only-when-completed-252d-return-is-negative
...
sources:
  - "[[sources/BOROWSKI-MOP-XNG-FRITREND-2026]]"
concepts:
...
timeframe: D1
timeframes: [D1]
expected_trade_frequency: "Approximately 12-30 completed Friday-session XNG packages/year when the completed 252-D1 return is strictly negative; Q02 must prove or retire the density."
...
review_focus: "Falsify whether the load-bearing conjunction of XNG Friday weakness and a negative completed 252-D1 trend clears density and costs while adding direct natural-gas exposure."
...
`strategy-seeds/sources/BOROWSKI-MOP-XNG-FRITREND-2026/source.md` preserves the
two completely read governed parent lineages.

Borowski supply the weak XNG Friday direction. Moskowitz, Ooi,
...
Thursday-close-to-Friday-open attachment, an ATR stop, or QM portfolio
...
- A genuine Friday boundary requires the current D1 bar to be Friday and the
  immediately prior completed D1 bar to be Thursday. A missing Thursday or Friday bar produces
  no shifted Friday trade.
- Entry uses the first executable quote observed within five minutes of the
  Friday D1 bar open. The Thursday-close to Friday-open return is not captured.
- `Close[1] / Close[253]`, a strict negative sign, `ATR(20) * 3.0`, the
  2,500-point spread cap, and one consumed attempt per week are fixed,
...
research cards and returned `CLEAN` for slug `xng-fri-trend`, strategy ID
`BOROWSKI-MOP-XNG-FRITREND-2026_S01`, and mechanic
`Friday XNG short only when completed 252-D1 return is negative`.

Manual semantic review resolved the closest systems:

- `QM5_20094_xng-fri-short` is an unconditional Friday short and never reads
  the slow XNG trend.
- `QM5_12603_xng-tsmom12m` is a year-round symmetric monthly trend package
  without a weekday gate.
- `QM5_12750` and `QM5_12779` condition Friday trades on the observed opening
  gap and target gap fill; this card never reads or targets that gap.
- `QM5_20110_xti-xng-fri-rv` is a two-leg relative-value Friday package;
  this card is a single-symbol, trend-conditioned XNG package.
- `QM5_12806_xng-rev-weekend` rotates fixed Monday-long and Friday-short
  directions without reading the slow XNG trend.
- `QM5_20156`, `QM5_20158`, and `QM5_20159` use the same governed
  weekday/trend interaction architecture on Wednesday, Tuesday, and Monday.
- `QM5_12567_cum-rsi2-commodity` is a two-day oscillator pullback.

The genuine Friday boundary and negative completed 252-D1 sign are jointly
...
- Timeframe: D1.
- Magic slot: 0; allocated magic `201600000`.
- Decision clock: first observed tick within five minutes of a genuine Friday
  D1 bar immediately following a Thursday D1 bar.
- Direction: short-only, and only while completed 252-D1 log return is
  strictly negative.
- Ordinary lifecycle: close through the framework Friday-close control at
  broker hour 21; first non-Friday D1 and two-day checks are stale repairs.
...
2. Evaluate entry only when the current D1 bar is broker-calendar Friday and
   the immediately prior completed D1 bar is Thursday.
3. Require the first observed tick to occur within five minutes of the
   current Friday D1 bar open. A late attachment consumes no attempt and does
   not enter.
4. Derive a stable Friday-anchored broker week key. Persist that key as
...
10. Open at most one position for magic `201600000`; no pending order,
    same-week retry, second entry, or scale-in is authorized.

## 5. Exit Rules

1. Close the Friday package on the first new D1 bar whose broker weekday is
   not Friday.
...
4. Framework Friday close at broker hour 21 is the ordinary session exit.
...
- Fail closed for a non-Friday current D1 bar, a prior bar that is not Thursday,
...
- One position for magic `201600000` and one consumed decision per broker
  week.
...
There is no baseline parameter sweep. The Friday gate and negative 252-D1 sign
...
- Forced session flatten: framework Friday close at broker hour 21; first
  non-Friday D1 bar is the stale fallback.

## Exit Precedence

1. Framework kill switch and the server-side hard stop.
2. Framework Friday close at broker hour 21.
3. First non-Friday D1 boundary or unexpected-long cleanup.
4. Two-calendar-day stale close.
...
XNG Fridays. Moskowitz, Ooi, and Pedersen report time-series momentum across
...
- `expected_dd_pct: 25.0` reflects XNG gaps, short squeezes, CFD roll/basis,
...
- Fail on any long entry, entry outside a genuine Friday/Thursday boundary,
...
ATR stop. XNG weekend gaps, short squeezes, continuous-CFD roll/basis,
...
- no_trade: exact host/D1/EA/slot, locked-input, genuine-Friday, grace,
  history, arithmetic, trend, spread, quote, stop, consumed-week, and
  owned-position guards.
- trade_entry: genuine Friday boundary, negative completed 252-D1 state, one
  SELL, and frozen ATR stop.
- trade_management: first non-Friday, wrong-side, and two-day stale closes
...
|---|---|---|---|---|
| v1 | 2026-07-25 | initial source-backed XNG Friday/trend interaction card | G0 | APPROVED |
...
| Q01 Build Validation | 2026-07-25 | PASS: strict compile 0 errors/0 warnings; targeted build check PASS | `docs/ops/evidence/2026-07-25_qm5_20160_xng_fri_trend_build_q02.md` |
| Q02 Baseline Screening | 2026-07-25 | ENQUEUED: `1a253cfa-4b9a-4ef3-a707-6589dd0f4972` | same evidence |
