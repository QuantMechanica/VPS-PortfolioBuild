...
#property description "QM5_20191 EURUSD USDCHF Cointegration"
...
//
// Fixed spread:
//   ln(EURUSD.DWX) - beta * ln(USDCHF.DWX)
//
...
input double strategy_beta              = -0.585986704;
...
bool Strategy_IsHostSymbol()
  {
   return (_Symbol == g_leg_eurusd || _Symbol == g_leg_usdchf);
  }
...
bool Strategy_EnsureBasketScope()
  {
   ...
   string allowed[2] = {"EURUSD.DWX", "USDCHF.DWX"};
   for(int i = 0; i < 2; ++i)
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(!QM_SymbolAssertOrLog(g_leg_eurusd) ||
      !QM_SymbolAssertOrLog(g_leg_usdchf))
      return false;
   ...
   for(int i = 0; i < history_count; ++i)
     {
      if(eurusd_time[i] <= 0 || eurusd_time[i] != usdchf_time[i])
         return false;
      if(eurusd[i] <= 0.0 || usdchf[i] <= 0.0)
         return false;

      spreads[i] =
         MathLog(eurusd[i]) - strategy_beta * MathLog(usdchf[i]);
   ...
   if(g_state_ready)
      g_last_state_bar = iTime(_Symbol, PERIOD_D1, 0); // perf-allowed: cache the D1 timestamp only after a successful gated refresh.
...
bool Strategy_OpenPair(const int spread_direction)
  {
   ...
   const double usdchf_weight = MathAbs(strategy_beta);
   const double weight_sum = eurusd_weight + usdchf_weight;
   if(weight_sum <= 0.0)
      return false;

   const bool long_spread = (spread_direction > 0);
   const QM_OrderType eurusd_type = long_spread ? QM_BUY : QM_SELL;
   const QM_OrderType usdchf_type =
      long_spread
      ? (strategy_beta >= 0.0 ? QM_SELL : QM_BUY)
      : (strategy_beta >= 0.0 ? QM_BUY : QM_SELL);
   const string reason =
      long_spread ? "QM5_20191_LONG_SPREAD_Z_LT_NEG_ENTRY"
                  : "QM5_20191_SHORT_SPREAD_Z_GT_POS_ENTRY";
   ...
   const bool eurusd_ok =
      Strategy_OpenLeg(g_leg_eurusd,
                       ...
                       weight_sum,
                       reason);
   const bool usdchf_ok =
      Strategy_OpenLeg(g_leg_usdchf,
                       usdchf_type,
                       usdchf_weight,
                       weight_sum,
                       reason);
   if(eurusd_ok && usdchf_ok)
     {
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20191_PAIR_HOST";
...
bool Strategy_ExitSignal()
  {
   ...
   const datetime current_d1_bar = iTime(_Symbol, PERIOD_D1, 0); // perf-allowed: cheap D1 guard before an optional spread refresh.
