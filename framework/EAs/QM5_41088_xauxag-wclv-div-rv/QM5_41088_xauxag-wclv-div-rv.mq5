...
#property description "QM5_41088 XAU/XAG Weekly Close-Location Divergence Reversion"
...
// -----------------------------------------------------------------------------
// Reconstruct synchronized XAU/XAG OHLC for the immediately completed broker
// week. When one metal closes strictly in its own upper range tercile and the
...
double   g_xau_week_high = 0.0;
double   g_xau_week_low = 0.0;
double   g_xau_week_close = 0.0;
double   g_xag_week_high = 0.0;
double   g_xag_week_low = 0.0;
double   g_xag_week_close = 0.0;
double   g_xau_clv = 0.0;
double   g_xag_clv = 0.0;
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41088 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_WCLVDIV_ATTEMPT_WEEK", qm_ea_id);
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41088_XAUXAG_WCLVDIV_RV";
...
bool Strategy_LoadWeeklyCloseLocationDivergence(const int current_week,
                                                int &direction)
  {
   ...
   g_xau_week_high = 0.0;
   g_xau_week_low = 0.0;
   g_xau_week_close = 0.0;
   g_xag_week_high = 0.0;
   g_xag_week_low = 0.0;
   g_xag_week_close = 0.0;
   g_xau_clv = 0.0;
   g_xag_clv = 0.0;
   g_week_sessions = 0;
   ...
   const int xau_copied =
      CopyRates(g_leg_xau, // perf-allowed: decision-only bounded completed-week OHLC scan.
                PERIOD_D1, 1, strategy_history_bars_d1, xau_bars);
   const int xag_copied =
      CopyRates(g_leg_xag, // perf-allowed: decision-only bounded completed-week OHLC scan.
   ...
   for(int index = 0; index < strategy_history_bars_d1; ++index)
     {
      ...
      if(xau_bars[index].open <= 0.0 || xau_bars[index].high <= 0.0 ||
         xau_bars[index].low <= 0.0 || xau_bars[index].close <= 0.0 ||
         xag_bars[index].open <= 0.0 || xag_bars[index].high <= 0.0 ||
         xag_bars[index].low <= 0.0 || xag_bars[index].close <= 0.0 ||
         !MathIsValidNumber(xau_bars[index].open) ||
         !MathIsValidNumber(xau_bars[index].high) ||
         !MathIsValidNumber(xau_bars[index].low) ||
         !MathIsValidNumber(xau_bars[index].close) ||
         !MathIsValidNumber(xag_bars[index].open) ||
         !MathIsValidNumber(xag_bars[index].high) ||
         !MathIsValidNumber(xag_bars[index].low) ||
         !MathIsValidNumber(xag_bars[index].close) ||
         xau_bars[index].high < xau_bars[index].open ||
         xau_bars[index].high < xau_bars[index].close ||
         xau_bars[index].low > xau_bars[index].open ||
         xau_bars[index].low > xau_bars[index].close ||
         xag_bars[index].high < xag_bars[index].open ||
         xag_bars[index].high < xag_bars[index].close ||
         xag_bars[index].low > xag_bars[index].open ||
         xag_bars[index].low > xag_bars[index].close)
         return false;

      if(g_week_sessions == 0)
        {
         g_xau_week_high = xau_bars[index].high;
         g_xau_week_low = xau_bars[index].low;
         g_xau_week_close = xau_bars[index].close;
         g_xag_week_high = xag_bars[index].high;
         g_xag_week_low = xag_bars[index].low;
         g_xag_week_close = xag_bars[index].close;
        }
      else
        {
         g_xau_week_high = MathMax(g_xau_week_high, xau_bars[index].high);
         g_xau_week_low = MathMin(g_xau_week_low, xau_bars[index].low);
         g_xag_week_high = MathMax(g_xag_week_high, xag_bars[index].high);
         g_xag_week_low = MathMin(g_xag_week_low, xag_bars[index].low);
        }
      ++g_week_sessions;
     }
   ...
   if(completed_week <= 0 ||
      ...
      g_xau_week_high <= 0.0 || g_xau_week_low <= 0.0 ||
      g_xau_week_close <= 0.0 || g_xag_week_high <= 0.0 ||
      g_xag_week_low <= 0.0 || g_xag_week_close <= 0.0)
      return false;

   const double xau_range = g_xau_week_high - g_xau_week_low;
   const double xag_range = g_xag_week_high - g_xag_week_low;
   if(xau_range <= 0.0 || xag_range <= 0.0 ||
      !MathIsValidNumber(xau_range) || !MathIsValidNumber(xag_range))
      return false;

   g_xau_clv = (g_xau_week_close - g_xau_week_low) / xau_range;
   g_xag_clv = (g_xag_week_close - g_xag_week_low) / xag_range;
   if(!MathIsValidNumber(g_xau_clv) || !MathIsValidNumber(g_xag_clv) ||
      g_xau_clv < 0.0 || g_xau_clv > 1.0 ||
      g_xag_clv < 0.0 || g_xag_clv > 1.0)
      return false;

   if(g_xau_clv > strategy_clv_upper &&
      g_xag_clv < strategy_clv_lower)
      direction = -1;
   else if(g_xau_clv < strategy_clv_lower &&
           g_xag_clv > strategy_clv_upper)
      direction = 1;
   return true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41088_XAUXAG_WCLVDIV_RV_HOST";
   ...
   if(Strategy_OpenOwnedPositionCount() > 0 ||
   ...
   else
     {
      ...
      if(!signal_valid)
         ...
         g_signal_state = "xau_upper_xag_lower_fade_short_xau";
      else if(direction > 0)
         g_signal_state = "xau_lower_xag_upper_fade_long_xau";
      else
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"week\":%d,\"decision_bar\":%I64d,\"late\":%s,\"valid\":%s,\"direction\":%d,\"xau_clv\":%.12f,\"xag_clv\":%.12f,\"xau_high\":%.10f,\"xau_low\":%.10f,\"xau_close\":%.10f,\"xag_high\":%.10f,\"xag_low\":%.10f,\"xag_close\":%.10f,\"sessions\":%d,\"state\":\"%s\"}",
                            g_signal_week_key,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG weekly close-location divergence package exits at the next broker week"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41088\",\"ea\":\"xauxag-wclv-div-rv\"}");
