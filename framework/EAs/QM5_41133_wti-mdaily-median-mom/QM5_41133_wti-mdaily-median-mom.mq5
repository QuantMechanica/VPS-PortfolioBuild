...
#property description "QM5_41133 WTI Completed-Month Daily-Return Median Momentum"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41133 - WTI Completed-Month Daily-Return Median Momentum
// -----------------------------------------------------------------------------
...
//   - follow the ordinary odd/even sample median of those individual returns
...
bool Strategy_LoadDailyMedianSignal(const int current_month_key,
                                    const int label_offset,
                                    int &direction,
                                    int &month_sessions,
                                    ...
                                    double &daily_median)
  {
   ...
   if((return_count % 2) == 1)
     {
      center_low = daily_returns[center];
      center_high = center_low;
      daily_median = center_low;
     }
   else
     {
      if(center <= 0)
         return false;
      center_low = daily_returns[center - 1];
      center_high = daily_returns[center];
      daily_median = (center_low + center_high) / 2.0;
     }

   if(!MathIsValidNumber(center_low) ||
      !MathIsValidNumber(center_high) ||
      !MathIsValidNumber(daily_median) ||
      center_low > center_high)
      return false;

   if(daily_median > 0.0)
      direction = 1;
   else if(daily_median < 0.0)
      direction = -1;
   return true;
  }
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadDailyMedianSignal(g_decision_month_key,
                                        ...
                                        g_daily_median);
      if(!g_signal_valid)
         g_signal_state = "monthly_daily_median_validation_failed";
      else if(g_daily_median == 0.0)
         g_signal_state = "zero_daily_median_flat";
      else if(g_signal_direction > 0)
         g_signal_state = "daily_median_completed_month_long";
      else if(g_signal_direction < 0)
         g_signal_state = "daily_median_completed_month_short";
      else
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"month_sessions\":%d,\"center_low\":%.12e,\"center_high\":%.12e,\"daily_median\":%.12e,\"boundary_close\":%.8f,\"final_close\":%.8f,\"raw_sum\":%.12e,\"endpoint_return\":%.12e,\"raw_direction\":%d,\"endpoint_agrees\":%s,\"state\":\"%s\"}",
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41133 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41133_WTI_MDAILY_MEDIAN_MOM";
   ...
   req.reason = (g_signal_direction > 0)
      ? "WTI_MDAILY_MEDIAN_MOM_LONG"
      : "WTI_MDAILY_MEDIAN_MOM_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41133 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI completed-month daily-return-median position holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41133_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41133\",\"ea\":\"wti-mdaily-median-mom\"}");
