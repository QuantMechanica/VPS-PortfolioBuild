...
#property description "QM5_20216 AUDUSD EURAUD Cointegration"
...
//
// Card:
//   strategy-seeds/cards/approved/QM5_20216_audusd-euraud_card.md
//
// Fixed spread:
//   ln(AUDUSD.DWX) - beta * ln(EURAUD.DWX)
//
...
input double strategy_beta              = -0.655175398;
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(!Strategy_EnsureBasketScope())
      return false;
   if(!QM_SymbolAssertOrLog(g_leg_audusd) ||
   ...
   for(int i = 0; i < history_count; ++i)
     {
      if(audusd_time[i] <= 0 || audusd_time[i] != euraud_time[i])
         return false;
      if(audusd[i] <= 0.0 || euraud[i] <= 0.0)
         return false;

      spreads[i] =
         MathLog(audusd[i]) - strategy_beta * MathLog(euraud[i]);
...
bool Strategy_PreparePair(const int spread_direction,
                          QM_EntryRequest &host_req)
  {
   ...
   const double weight_sum = audusd_weight + euraud_weight;
   if(weight_sum <= 0.0)
      return false;
   ...
   const double audusd_lots =
      Strategy_LotsForLeg(g_leg_audusd, audusd_weight, audusd_weight);
   const double euraud_lots =
      Strategy_LotsForLeg(g_leg_euraud, euraud_weight, audusd_weight);
   if(audusd_lots <= 0.0 || euraud_lots <= 0.0)
      return false;
   ...
   const string reason =
      long_spread ? "QM5_20216_LONG_SPREAD_Z_LT_NEG_ENTRY"
                  : "QM5_20216_SHORT_SPREAD_Z_GT_POS_ENTRY";

   const double host_entry =
      QM_OrderTypeIsBuy(audusd_type)
      ? SymbolInfoDouble(g_leg_audusd, SYMBOL_ASK)
      : SymbolInfoDouble(g_leg_audusd, SYMBOL_BID);
   const double host_atr =
      QM_ATR(g_leg_audusd, PERIOD_D1, strategy_atr_period_d1, 1);
   if(host_entry <= 0.0 || host_atr <= 0.0)
      return false;

   host_req.type = audusd_type;
   host_req.price = 0.0;
   host_req.sl = QM_StopATRFromValue(g_leg_audusd,
                                     audusd_type,
                                     host_entry,
                                     host_atr,
                                     strategy_atr_sl_mult);
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20216_PAIR_HOST";
...
void OnTick()
  {
   ...
   const bool is_new_signal_bar =
      QM_IsNewBar(g_leg_audusd, PERIOD_D1);
