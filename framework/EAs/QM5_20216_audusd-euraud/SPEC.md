# QM5_20216_audusd-euraud — Strategy Spec
...
`ln(AUDUSD.DWX) - (-0.655175398) * ln(EURAUD.DWX)`. Score the newest spread
...
| `strategy_beta` | -0.655175398 | fixed | DEV-fitted structural hedge coefficient |
...
- `AUDUSD.DWX` — fixed tester host and first traded leg.
- `EURAUD.DWX` — fixed companion and second traded leg.
- `EURUSD.DWX` — conversion-history dependency for EUR-denominated margin
  accounting; it is order-free and has no magic slot. The traded AUDUSD leg
  supplies the AUD/USD conversion path for EURAUD P/L.
...
| Bar gating | `QM_IsNewBar(AUDUSD.DWX, PERIOD_D1)` consumed once per signal bar |
...
| Typical hold time | multi-day; scan half-life 171.485 D1 bars |
| Expected drawdown profile | high and regime-sensitive; conservative 30% prior |
| Regime preference | AUDUSD/EURAUD fixed-residual mean reversion |
| Win rate target | unknown; Q02 is the first platform-economic judge |

The fixed scan recorded DEV net Sharpe `-0.025549`, OOS net Sharpe `0.160430`,
and OOS return `1.290977%`. The negative DEV score and very long half-life are
...
`1.0` and `0.655175398`; each leg is independently bounded by its ATR stop.
If the EURAUD allocation normalizes below broker minimum volume, the
...
|---|---|---|---|
| v1 | 2026-08-05 | Initial build from approved card | Next-ranked non-duplicate fixed-scan FX basket |
