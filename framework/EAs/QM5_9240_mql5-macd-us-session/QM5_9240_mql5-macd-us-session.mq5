   ...
   if(strategy_max_spread_points > 0.0 && ask > bid)
     {
      ...
      if(point > 0.0 && ((ask - bid) / point) > strategy_max_spread_points)
         return true;
     }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(atr_now <= 0.0 || adx_now <= 0.0)
      return false;

   bool atr_normal = false;
   if(strategy_atr_percentile_lookback > 1)
     {
      int valid = 0;
      int below_or_equal = 0;
      for(int i = 2; i < 2 + strategy_atr_percentile_lookback; ++i)
        {
         const double sample = QM_ATR(_Symbol, strategy_signal_tf, strategy_atr_period, i);
         if(sample <= 0.0)
            continue;
         valid++;
         if(sample <= atr_now)
      ...
      if(valid > 0)
        {
         const double percentile = 100.0 * (double)below_or_equal / (double)valid;
         atr_normal = (percentile >= strategy_atr_normal_min_pct && percentile <= strategy_atr_normal_max_pct);
        }
     }

   const bool trend_regime = (adx_now >= strategy_adx_trend_min);
   if(!trend_regime && !atr_normal)
      return false;

   const int price_vs_ema = QM_Sig_Price_Above_MA(_Symbol, strategy_signal_tf, strategy_ema_period, 0.0, 1);
   if(long_cross && price_vs_ema <= 0)
      return false;
   if(short_cross && price_vs_ema >= 0)
      return false;
