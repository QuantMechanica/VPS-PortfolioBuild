...
#property description "QM5_41022 WTI Split-Week Dual-Segment Momentum"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41022 - WTI Split-Week Dual-Segment Momentum
// -----------------------------------------------------------------------------
// Structural D1 crude-oil sleeve:
//   - require an exact completed Friday-through-Friday broker-week sequence
//   - compute disjoint Friday->Tuesday and Tuesday->Friday log returns
//   - enter on the next Monday only when both completed-return signs agree
...
bool Strategy_LoadSplitWeek(const datetime current_bar_time,
                            ...
                            int &direction)
  {
   ...
   const int copied = CopyRates(_Symbol,          // perf-allowed: exact
                                PERIOD_D1,         // six-bar completed
                                1,                 // split-week sequence
                                6,                 // one QM_IsNewBar edge.
                                bars);
   if(copied != 6)
      return false;
   ...
   opening_return = MathLog(bars[3].close / bars[5].close);
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   int direction = 0;
   if(!Strategy_LoadSplitWeek(g_strategy_d1_bar_time,
                              broker_now,
                              opening_return,
                              closing_return,
                              direction))
   ...
   if(direction > 0)
     {
      req.type = QM_BUY;
      req.reason = "WTI_WEEK_DUAL_MOM_LONG";
     }
   else if(direction < 0)
     {
      req.type = QM_SELL;
      req.reason = "WTI_WEEK_DUAL_MOM_SHORT";
     }
   else
      return false;
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"date\":%d,\"opening_return\":%.12e,\"closing_return\":%.12e,\"direction\":%d}",
                            date_key,
                            opening_return,
                            closing_return,
                            direction));
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI split-week card uses Friday 21 as ordinary exit"))
      return INIT_FAILED;
   ...
   g_attempt_state_key =
      StringFormat("QM5_41022_WEEK_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41022\",\"ea\":\"wti-wdual-mom\"}");
