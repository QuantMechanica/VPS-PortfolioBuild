...
#property description "QM5_12910 WTI Distillate Crack Spring Unwind"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_12910 - WTI Distillate Crack Spring Unwind
// -----------------------------------------------------------------------------
// D1 structural WTI sleeve:
//   - short only during the March-April post-winter distillate-crack unwind
//   - requires falling slow trend and downside channel breakdown
//   - exits on unwind-window end, trend/channel recovery, max hold, Friday close,
//     or ATR hard stop
// Runtime uses MT5 OHLC only; no EIA, crack-spread, futures-curve, CSV, or API feed.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 12910;
...
bool Strategy_InUnwindWindow(const datetime t)
  {
   MqlDateTime dt;
   TimeToStruct(t, dt);
...
bool Strategy_LoadClosedState(double &open_last,
                              ...
                              datetime &closed_time)
  {
   ...
   open_last = iOpen(_Symbol, PERIOD_D1, 1);    // perf-allowed: D1 candle confirmation.
   close_last = iClose(_Symbol, PERIOD_D1, 1);  // perf-allowed: D1 closed-bar signal math.
   if(closed_time <= 0 || open_last <= 0.0 || close_last <= 0.0)
      return false;

   double breakdown_high = 0.0;
   double exit_low = 0.0;
   if(!Strategy_Channel(strategy_breakdown_lookback, breakdown_high, breakdown_low))
      return false;
...
void Strategy_CloseOpenPositionsIfNeeded()
  {
   ...
   double breakdown_low = 0.0;
   double exit_high = 0.0;
   double trend_sma = 0.0;
   double trend_sma_prior = 0.0;
   double atr = 0.0;
   datetime closed_time = 0;
   if(!Strategy_LoadClosedState(open_last, close_last, breakdown_low, exit_high,
                                trend_sma, trend_sma_prior, atr, closed_time))
      return;

   const bool in_window = Strategy_InUnwindWindow(closed_time);
   const int magic = QM_FrameworkMagic();
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(strategy_trend_period <= 1 || strategy_sma_slope_shift <= 0)
      return true;
   if(strategy_breakdown_lookback <= 1 || strategy_exit_channel <= 1)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12910_WTI_DIST_UNWIND";
   ...
   if(!Strategy_LoadClosedState(open_last, close_last, breakdown_low, exit_high,
                                trend_sma, trend_sma_prior, atr, closed_time))
      return false;
   if(!Strategy_InUnwindWindow(closed_time))
      return false;
   if(close_last >= trend_sma || trend_sma >= trend_sma_prior)
      return false;
   if(close_last >= open_last || close_last >= breakdown_low)
      return false;
   ...
   req.reason = "WTI_DISTILLATE_UNWIND_SHORT";
   return true;
  }
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_12910\",\"ea\":\"wti-dist-unwind\"}");
