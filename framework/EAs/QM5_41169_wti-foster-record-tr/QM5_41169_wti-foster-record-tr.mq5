...
#property description "QM5_41169 WTI Monthly Foster-Stuart Record-Count Trend"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41169 - WTI Monthly Foster-Stuart Record-Count Trend
// -----------------------------------------------------------------------------
// D1 structural crude-oil sleeve:
//   - reconstruct thirteen consecutive completed broker-month-end closes
//   - count strict forward upper and lower records from the oldest frontier
//   - follow WTI only when upper-minus-lower has absolute value at least two
...
input int    strategy_record_threshold     = 2;
...
bool Strategy_FosterStuartRecordSignal(const double &closes[],
                                       Strategy_SignalMetrics &metrics)
  {
   ...
   double running_high = closes[0];
   double running_low = closes[0];
   if(running_high <= 0.0 || running_low <= 0.0 ||
   ...
   for(int index = 1; index < strategy_endpoint_count; ++index)
     {
      const double value = closes[index];
      if(value > running_high)
        {
         ++metrics.upper_count;
         running_high = value;
        }
      else if(value < running_low)
        {
         ++metrics.lower_count;
         running_low = value;
        }
   ...
   if(metrics.upper_count < 0 || metrics.lower_count < 0 ||
      metrics.neutral_count < 0 ||
      metrics.upper_count + metrics.lower_count +
         metrics.neutral_count != strategy_endpoint_count - 1)
      return false;

   metrics.record_difference = metrics.upper_count - metrics.lower_count;
   if(metrics.record_difference >= strategy_record_threshold)
      metrics.direction = 1;
   else if(metrics.record_difference <= -strategy_record_threshold)
      metrics.direction = -1;
   else
      metrics.direction = 0;
   return true;
                                        ...
                                        int &direction)
  {
   ...
   const bool valid =
      Strategy_LoadMonthlyEndpoints(current_month_key,
                                    ...
                                    closes,
                                    endpoint_times) &&
      Strategy_FosterStuartRecordSignal(closes, metrics);
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
   ...
   else
     {
      ...
      g_signal_valid =
         endpoints_valid &&
         Strategy_FosterStuartRecordSignal(closes, g_signal_metrics);
      if(!g_signal_valid)
         g_signal_state = "foster_stuart_record_validation_failed";
      else if(g_signal_metrics.direction == 0)
         g_signal_state = "record_difference_below_two_flat";
      else if(g_signal_metrics.direction > 0)
         g_signal_state = "foster_stuart_record_wti_long";
      else if(g_signal_metrics.direction < 0)
         g_signal_state = "foster_stuart_record_wti_short";
      else
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"endpoint_count\":%d,\"upper_count\":%d,\"lower_count\":%d,\"neutral_count\":%d,\"record_difference\":%d,\"oldest_endpoint\":%I64d,\"newest_endpoint\":%I64d,\"state\":\"%s\"}",
                            ...
                            g_signal_metrics.upper_count,
                            g_signal_metrics.lower_count,
                            g_signal_metrics.neutral_count,
                            g_signal_metrics.record_difference,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41169 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41169_WTI_FOSTER_RECORD";
   ...
   req.reason = (g_signal_metrics.direction > 0)
      ? "WTI_FOSTER_RECORD_LONG"
      : "WTI_FOSTER_RECORD_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41169 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI Foster-Stuart record-count trend holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41169_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41169\",\"ea\":\"wti-foster-record-tr\"}");
