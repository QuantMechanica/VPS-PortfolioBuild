"""Deterministic reference checks for QM5_41457 WTI summer NR2 up-week fade."""
...
class WtiSummerNR2UpWeekFadeReferenceTest(unittest.TestCase):
    current = date(2026, 7, 13)
    new = current - timedelta(days=7)
    old = current - timedelta(days=14)

    def bars(self, close: float = 102.0, new_range: float = 8.0,
    ...
    def test_contraction_positive_week_is_short(self) -> None:
        self.assertEqual(signal(self.current, *self.bars())[0], -1)

    def test_negative_week_is_flat(self) -> None:
        self.assertEqual(signal(self.current, *self.bars(close=98))[0], 0)
    ...
    def test_range_expansion_is_flat(self) -> None:
        self.assertEqual(signal(self.current, *self.bars(new_range=12, old_range=10))[0], 0)
    ...
    def test_three_to_five_sessions_are_valid(self) -> None:
        for count in (3, 4, 5):
            newest = package(self.new, count, 100, 104, 96, 102)
