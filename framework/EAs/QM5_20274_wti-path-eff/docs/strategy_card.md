...
slug: wti-path-eff
status: APPROVED
execution_contract_ref: strategy-seeds/cards/approved/QM5_20274_wti-path-eff_card.md
...
source_citations:
  - type: peer_reviewed_paper
    ...
    location: "DOI https://doi.org/10.1016/j.jfineco.2011.11.003; complete-paper evidence strategy-seeds/sources/MOP-TSMOM-2012/source.md; bounded extraction strategy-seeds/sources/MOP-WTI-PATHEFF-2026/source.md"
    quality_tier: A
...
sources:
  - "[[sources/MOP-WTI-PATHEFF-2026]]"
concepts:
  - "[[concepts/time-series-momentum]]"
  - "[[concepts/path-efficiency]]"
  - "[[concepts/crude-oil-structural-trend]]"
indicators:
  - "[[indicators/completed-month-log-return]]"
  - "[[indicators/net-to-absolute-path-efficiency]]"
  - "[[indicators/atr]]"
strategy_type_flags: [commodity, energy, crude-oil, structural-trend, path-efficiency, monthly-rebalance, atr-hard-stop, time-stop, symmetric-long-short, low-frequency]
...
period: D1
timeframe: D1
expected_trade_frequency: "Approximately six monthly WTI packages/year after thirteen completed month ends under a zero-drift design reference; Q02 must prove at least five completed positions/year or retire."
...
q01_status: PASS
q02_status: ENQUEUED
review_focus: "Falsify a direct WTI monthly trend whose direction is the twelve-month net log move and whose fixed qualification gate is absolute net displacement divided by the full absolute monthly path; distinct from endpoint-only, sign-count/run, vote, regression/rank, and robust-average estimators. Q09 alone may establish realized book decorrelation."
modules_used: [no_trade, trade_entry, trade_management, trade_close]
target_modules: [Strategy_NoTradeFilter, Strategy_EntrySignal, Strategy_ManageOpenPosition, Strategy_ExitSignal, Strategy_NewsFilterHook]
hard_rules_at_risk: [completed_month_reconstruction, chronological_log_return_orientation, net_return_sum, absolute_path_denominator, fixed_efficiency_threshold, monthly_attempt_state, risk_mode_dual, friday_close_disabled, cfd_futures_basis, q02_frequency_floor, portfolio_correlation]
g0_approval_reasoning: "APPROVED under decisions/2026-08-10_qm5_20274_wti_path_eff_g0.md: R1 complete-read peer-reviewed WTI source; R2 fixed thirteen month ends, twelve adjacent log returns, exact net and absolute-path sums, 0.25 threshold, direction, and lifecycle; R3 registered WTI D1 route; R4 deterministic native arithmetic. The deterministic checker returned CLEAN and manual review separated cumulative-return, sign-count/run, fixed-block-vote, regression/rank, and robust-average mechanics."
---
...
inventories, transport, refining, hedging, and demand adjust. A twelve-month
endpoint return alone cannot distinguish a relatively direct trend from a
highly reversing path that happens to finish away from its start. The ratio of
absolute net displacement to total absolute monthly path length tests whether
the prior-year move was directionally efficient before following its sign.
...
The source does not use a path-efficiency estimator. The net-to-absolute-path
ratio, `0.25` threshold, Darwinex continuous CFD, broker-month reconstruction,
...
The canonical checker scanned 4,337 EA-registry rows and 447 cards. It found no
exact identity and no fuzzy match above threshold. Manual mechanic review
resolves the closest systems:

- pure WTI TSMOM uses one cumulative endpoint return without a path-quality
  qualification gate;
- `QM5_13150_wti-signmom`, `QM5_20244_wti-trend-sign`, and
  `QM5_20273_wti-signrun-tr` use monthly signs and discard magnitude;
- `QM5_20258_wti-mom-vote` and `QM5_20272_wti-qtrvote-tr` vote nested or fixed
  block returns;
- `QM5_20261`, `QM5_20264`, and `QM5_20271` use OLS, all-pairs ordinal, and
  all-pairs median-slope estimators over price levels; and
- `QM5_20269` and `QM5_20270` sort monthly returns and aggregate a median or
  trimmed mean rather than compare net displacement with the entire path.

Generic efficiency-ratio implementations elsewhere in the repository do not
use this exact WTI monthly carrier and lifecycle. Endpoint count, adjacent-log
orientation, numerator, all-term absolute denominator, threshold, direction,
consumed attempt, and renewal are jointly load-bearing. Verdict: `CLEAN`.

## Markets, Timeframe, And Cadence

- Exact host and traded symbol: `XTIUSD.DWX`.
- Timeframe: D1; magic slot 0; intended magic `202740000`.
...
- Signal state: twelve-month net log return qualified by net-to-absolute-path
  efficiency of at least `0.25`.
...
- Expected cadence: about six positions per full post-warm-up year under the
  design reference; retire below five observed positions.
...
Require `P > 0`, finite values, and `E <= 1 + 1e-10`. BUY when `N > 0` and
`E >= 0.25`. SELL when `N < 0` and `E >= 0.25`. Anything else consumes the
month flat. Neither net return nor efficiency scales risk.

## Rules

These are the complete authorized baseline. There is no signal-parameter sweep
and no fallback to an unqualified cumulative return, sign statistic, vote,
mean, median, trimmed mean, regression, rank statistic, moving average,
oscillator, calendar direction, external series, or previous pipeline result.

## 4. Entry Rules

1. Require exact EA ID `20274`, `XTIUSD.DWX` D1, magic slot 0, and every
...
7. Calculate exactly twelve finite adjacent log returns from pairs `(0,1)`
   ...
   allowed.
8. Sum the twelve signed returns into `N` and their twelve absolute values into
   `P`; reject nonfinite values, `P <= 0`, or an efficiency outside its
   numerical bounds.
9. Buy only when `N > 0` and `abs(N)/P >= 0.25`; sell only under the symmetric
   negative-net rule. Zero net and below-threshold paths remain flat.
...
- Reject a consumed attempt, owned exposure, same-month entry history,
  ...
  nonfinite close, wrong adjacent pair or orientation, invalid logarithm,
  wrong net/path arithmetic, zero path, out-of-range efficiency, zero/below-
  threshold signal, excessive spread, invalid quote, unavailable ATR, invalid
...
| `strategy_efficiency_threshold` | 0.25 | [0.25] | minimum absolute-net / absolute-path ratio |
...
The endpoint count, adjacent-pair definition, chronological orientation,
numerator, denominator, zero handling, threshold, direction, entry clock,
risk, stop, hold, and no-retry policy are locked. Changing any requires a new
...
WTI in their commodity universe. They do not claim this path-efficiency rule
...
correlation with XNG or risk assets can dominate the premise. Efficiency can
be high because of a single shock and is a descriptive statistic, not a
...
- Fail on wrong endpoint order, nonconsecutive months, current-month leakage,
  wrong adjacent pairs or log orientation, omitted absolute-path term, wrong
  numerator/denominator, wrong `0.25` threshold, wrong-side entry, repeated
...
- Retire on nonpositive governed economics or later portfolio-correlation
  rejection.
- Do not rescue failure by changing lookback, threshold, direction, entry
  clock, stop, hold, spread cap, retry policy, or carrier.
...
- [x] R2: PASS. Fixed endpoints, adjacent returns, signed/absolute sums,
  threshold, direction, attempt, hard stop, rollover, and stale exit.
- [x] R3: PASS. Registered `XTIUSD.DWX` D1 plus native V5 execution state only.
- [x] R4: PASS. Deterministic logarithm, absolute value, sums, division,
  calendar, and ATR arithmetic; no trained model, prohibited signal indicator,
  external feed, grid, or martingale.
- [x] Dedup: deterministic checker CLEAN; endpoint-only, sign-count/run,
...
- trade_entry: month-attempt persistence, endpoint reconstruction, exact
  adjacent returns, net/absolute-path arithmetic, spread/quote/ATR/stop checks,
...
|---|---|---|---|---|
| v1 | 2026-08-10 | initial source-bounded WTI path-efficiency card | G0 | APPROVED |
| v1-q01 | 2026-08-10 | deterministic V5 build, strict compile, target build validation, and P1 artifact validation | Q01 | PASS |
| v1-q02 | 2026-08-10 | one paced current-binary WTI baseline handoff below the factory CPU ceiling | Q02 | ENQUEUED |
...
| Q02 Baseline Screening | 2026-08-10 | ENQUEUED; attempt 0, no verdict | work item `6586fea1-87ce-4bf4-a570-f49431c50a57`; `docs/ops/evidence/2026-08-10_qm5_20274_wti_path_eff_q01_q02_enqueue.md` |
