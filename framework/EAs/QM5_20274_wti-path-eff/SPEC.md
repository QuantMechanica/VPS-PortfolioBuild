...
in chronological order. Form twelve adjacent monthly log returns. Let `N` be
their signed sum and `P` the sum of their twelve absolute values. Buy when
`N > 0` and `abs(N)/P >= 0.25`; sell under the symmetric negative rule;
consume zero-path, zero-net, below-threshold, and invalid states flat.
...
frozen ATR hard stop, and no take-profit. This is a monthly net-to-path
efficiency statistic, not an unqualified cumulative return, sign statistic,
fixed-block vote, regression, rank trend, robust average, calendar rule, or
oscillator.
...
- `XTIUSD.DWX` only.
- Magic slot 0, intended magic `202740000`.
...
- Approximately six completed monthly packages per full post-warm-up year
  under the design reference; Q02 retires the EA below five.
- Symmetric long/short WTI exposure only after a `0.25`-qualified directional
  path.
...
`strategy-seeds/sources/MOP-WTI-PATHEFF-2026/source.md`. The path-efficiency
gate is a transparent QM hypothesis, not a source performance claim.
...
- **Trade Entry:** persist month attempt, reconstruct endpoints, calculate the
  twelve adjacent returns and exact net/path state, then validate spread,
...
The pre-allocation checker found no exact or fuzzy identity across 4,337
registry rows and 447 cards. The exact thirteen month ends, twelve adjacent
chronological log returns, signed numerator, full absolute-path denominator,
`0.25` threshold, symmetric mapping, monthly attempt, and renewal lifecycle are
load-bearing. Endpoint-only, sign-count/run, quarter-block, OLS, rank, median,
trimmed-mean, and Theil-Sen WTI paths use materially different estimators.
