...
`artifacts/cards_approved/QM5_10257_tv-sma-rsi-macd.md`. See that card's body for
...
`QM5_10257_tv-sma-rsi-macd.mq5`. Framework wiring (risk, magic, news, Friday close)
...
| `strategy_sma_fast_period` | 5 | (see source) | (see strategy logic) |
| `strategy_sma_mid_period` | 8 | (see source) | (see strategy logic) |
| `strategy_sma_slow_period` | 13 | (see source) | (see strategy logic) |
| `strategy_rsi_period` | 4 | (see source) | (see strategy logic) |
...
| `strategy_macd_lookback` | 3 | (see source) | (see strategy logic) |
| `strategy_atr_period` | 14 | (see source) | (see strategy logic) |
| `strategy_atr_sl_mult` | 1.5 | (see source) | (see strategy logic) |
| `strategy_take_profit_rr` | 2.0 | (see source) | (see strategy logic) |
| `strategy_session_enabled` | true | (see source) | (see strategy logic) |
| `strategy_session_start_h` | 7 | (see source) | (see strategy logic) |
| `strategy_session_end_h` | 18 | (see source) | (see strategy logic) |
...
`artifacts/cards_approved/QM5_10257_tv-sma-rsi-macd.md`
