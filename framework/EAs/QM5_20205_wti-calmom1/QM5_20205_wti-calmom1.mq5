...
#property description "QM5_20205 WTI same-calendar / one-month momentum agreement"
...
bool Strategy_LoadMomentumSignal(const datetime decision_bar_time,
                                 ...
                                 int &direction)
  {
   ...
   if(!Strategy_MonthReturn(rates,
                            ...
                            momentum_score))
      return false;
   if(!MathIsValidNumber(momentum_score))
      return false;
   ...
   if(momentum_score > threshold)
      direction = 1;
   else if(momentum_score < -threshold)
      direction = -1;
   return true;
  }
...
void Strategy_AdvanceMonthlyState()
  {
   ...
   const bool momentum_valid =
      Strategy_LoadMomentumSignal(decision_bar_time,
                                  g_momentum_score,
                                  momentum_direction);
   ...
   if(g_signal_valid &&
      ...
      seasonal_direction == momentum_direction)
...
bool Strategy_InputsValid()
  {
   if(qm_ea_id != 20205 || qm_magic_slot_offset != 0)
      return false;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20205_WTI_CALMOM1";
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_20205_MONTH_ATTEMPT_%d",
                   QM_FrameworkMagic());
