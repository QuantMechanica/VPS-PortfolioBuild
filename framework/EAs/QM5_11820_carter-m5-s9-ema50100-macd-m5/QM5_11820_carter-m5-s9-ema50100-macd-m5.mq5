...
#property description "QM5_11820 Carter M5 S9 EMA50/100 MACD"
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   const bool long_signal = (ema_fast > ema_slow && hist_2 <= 0.0 && hist_1 > 0.0);
   const bool short_signal = (ema_fast < ema_slow && hist_2 >= 0.0 && hist_1 < 0.0);
   ...
   req.sl = sl;
   req.tp = tp;
   req.reason = long_signal ? "CARTER_S9_LONG" : "CARTER_S9_SHORT";
   return true;
  }
...
bool Strategy_ExitSignal()
  {
   ...
   const double ema_fast_1 = QM_EMA(_Symbol, strategy_signal_tf, strategy_ema_fast_period, 1);
   const double ema_fast_2 = QM_EMA(_Symbol, strategy_signal_tf, strategy_ema_fast_period, 2);
   const double close_1 = QM_EMA(_Symbol, strategy_signal_tf, 1, 1);
   const double close_2 = QM_EMA(_Symbol, strategy_signal_tf, 1, 2);
   const double break_distance = QM_StopRulesPipsToPriceDistance(_Symbol, strategy_exit_break_pips);
   if(ema_fast_1 <= 0.0 || ema_fast_2 <= 0.0 || close_1 <= 0.0 || close_2 <= 0.0 || break_distance <= 0.0)
      return false;

   if(is_buy)
      return (close_2 >= ema_fast_2 && close_1 < (ema_fast_1 - break_distance));
