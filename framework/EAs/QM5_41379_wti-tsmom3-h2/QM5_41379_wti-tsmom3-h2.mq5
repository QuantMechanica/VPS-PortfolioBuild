...
#property description "QM5_41379 WTI Three-Month Trend / Two-Month Hold"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41379 - WTI Three-Month Trend / Two-Month Hold
// -----------------------------------------------------------------------------
...
//   1. derive four consecutive completed WTI month-end closes from D1;
//   2. calculate the exact three-month return ln(C[3] / C[0]);
...
input int    strategy_return_months            = 3;
...
double g_cache_three_month_return = 0.0;
...
void Strategy_ResetCachedState()
  {
   ...
   g_cache_signal = 0;
   g_cache_three_month_return = 0.0;
   g_cache_state_reason = "not_evaluated";
  }

bool Strategy_ThreeMonthReturnSignal(const double &closes[],
                                     int &signal)
  {
   signal = 0;
   if(strategy_return_months != 3 ||
      ArraySize(closes) != 4)
     {
      g_cache_state_reason = "bad_three_month_contract";
      return false;
     }
   ...
   const double endpoint_ratio = closes[3] / closes[0];
   ...
   const double three_month_return = MathLog(endpoint_ratio);
   if(!MathIsValidNumber(three_month_return))
     {
      g_cache_state_reason = "invalid_three_month_return";
      return false;
     }
   ...
   if(!MathIsValidNumber(chained_return) ||
      MathAbs(three_month_return - chained_return) > 1.0e-10)
     {
      g_cache_state_reason = "endpoint_path_identity_failed";
      return false;
     }

   g_cache_three_month_return = three_month_return;
   if(three_month_return == 0.0)
     {
      g_cache_state_reason = "exact_zero_three_month_return";
      return true;
     }

   signal = (three_month_return > 0.0) ? 1 : -1;
...
bool Strategy_LoadSignalState(int &signal)
  {
   ...
   return Strategy_ThreeMonthReturnSignal(monthly_closes,
                                          signal);
  }
...
void Strategy_PrepareBimonthlySignal()
  {
   ...
   QM_LogEvent(QM_INFO,
               "MONTHLY_STATE",
               StringFormat("{\"month\":%d,\"valid\":%s,\"signal\":%d,\"return_3m\":%.10f,\"state\":\"%s\"}",
                            g_cache_month_key,
                            g_cache_signal_valid ? "true" : "false",
                            g_cache_signal,
                            g_cache_three_month_return,
                            g_cache_state_reason));
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41379_WTI_TSMOM3_H2";
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41379_BIMONTH_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41379\",\"ea\":\"wti-tsmom3-h2\"}");
