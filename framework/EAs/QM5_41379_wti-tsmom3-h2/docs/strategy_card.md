...
type: strategy
strategy_id: MOP-TSMOM-2012_XTI_K3H2_S30
variant_id: MOP-TSMOM-2012_XTI_K3H2_S30
source_id: MOP-WTI-TSMOM3-H2-2026
ea_id: QM5_41379
slug: wti-tsmom3-h2
status: APPROVED
execution_contract_ref: strategy-seeds/cards/approved/QM5_41379_wti-tsmom3-h2_card.md
execution_contract_status: APPROVED
created: 2026-09-07
created_by: Research+Development
last_updated: 2026-09-07
g0_status: APPROVED
g0_decision: decisions/2026-09-07_qm5_41379_wti_tsmom3_h2_g0.md
source_approval: decisions/2026-09-07_wti_three_month_momentum_two_month_hold_source_approval.md
...
source_citations:
  - type: peer_reviewed_paper
    ...
    location: "DOI 10.1016/j.jfineco.2011.11.003; complete-read parent strategy-seeds/sources/MOP-TSMOM-2012/source.md; bounded extraction strategy-seeds/sources/MOP-WTI-TSMOM3-H2-2026/source.md"
    quality_tier: A
    role: primary_own_return_formation_holding_family_and_wti_membership
strategy_mechanic: bimonthly-wti-sign-of-exact-three-completed-broker-month-log-return-two-month-hold-odd-month-epoch
strategy_type_flags: [commodity, energy, wti-crude, structural-trend, time-series-momentum, three-month-return, two-month-hold, symmetric-long-short, atr-hard-stop, low-frequency]
...
pipeline_phase: Q02_HELD_CPU_CEILING
q01_status: PASS
q02_status: HELD_UNCLAIMABLE_CPU_CEILING
parameters_to_test: "Locked Q02 baseline only: D1; four consecutive completed month-end closes; exact three-month log-return sign; odd-month decisions; two-month hold; 300 D1 bars; 3.5*ATR(20,D1) frozen stop; 70-day stale repair; 1500-point spread ceiling."
review_focus: "Falsify the source-bounded WTI k=3,h=2 stream. Verify four exact completed-month endpoints, odd-month-only attempts, no even-month rollover, fixed risk, frozen stop, and next-odd-month exit. Q09 alone may establish realized decorrelation."
modules_used: [no_trade, trade_entry, trade_management, trade_close]
target_modules: [Strategy_NoTradeFilter, Strategy_EntrySignal, Strategy_ManageOpenPosition, Strategy_ExitSignal]
hard_rules_at_risk: [exact_symbol_period, completed_month_reconstruction, exact_three_month_orientation, odd_month_epoch, bimonthly_attempt_state, two_month_rollover, risk_mode_dual, hard_stop_present, q02_frequency_floor, portfolio_correlation]
---

# QM5_41379 WTI Three-Month Momentum / Two-Month Hold

## Hypothesis

A three-completed-month WTI own-return direction may persist when held as a
...
publish a standalone WTI `k=3, h=2` result or continuous-CFD evidence.

## Source Traceability And Non-Duplicate Decision

The approved packet is
`strategy-seeds/sources/MOP-WTI-TSMOM3-H2-2026/source.md`; its complete-read
...
`QM5_20055` uses the same three-month return with monthly renewal;
`QM5_20281` uses the same two-month clock with a twelve-month return. This card
requires both exact `k=3` endpoints and the fixed odd-month `h=2` lifecycle.

## Market, Clock, And Formula

- Host and traded symbol: setfile-bound `XTIUSD.DWX`; D1 only.
- Decision: first processed D1 bar of January, March, May, July, September, or
  November.
- History: four consecutive completed broker-month-end closes `C[0]..C[3]`,
  oldest to newest; newest is the immediately prior completed month.
- Formula: `r3 = ln(C[3] / C[0])`; positive buys, negative sells, equality or
  invalid state stays flat.
- Lifecycle: close and reconsider only at the next eligible odd-month edge;
  70 elapsed days is stale repair.
...
2. Require configured symbol, D1, EA 41379, slot 0, and fixed-risk backtest
   mode.
3. At a genuine odd-month transition, persist the month attempt before
   history, signal, spread, quote, ATR, sizing, news, or order gates.
4. Reconstruct exactly four positive finite consecutive completed month-end
   closes with strictly increasing timestamps and no current-month price.
5. Compute `ln(C[3]/C[0])`; buy strict positive and sell strict negative.
...
`PORTFOLIO_WEIGHT=1`. Principal risks are futures/CFD roll and basis,
financing, WTI gaps, fixed phase sensitivity, slow reversal response, hard-stop
slippage, weak carrier-specific evidence, and portfolio correlation.
...
Q01 must prove month adjacency and year boundaries, exact four endpoints,
...
|---|---|---|---|
| G0 Research Intake | 2026-09-07 | APPROVED | `decisions/2026-09-07_qm5_41379_wti_tsmom3_h2_g0.md` |
| Q01 Build Validation | 2026-09-07 | PASS | 13/13 reference vectors; PACER audit zero hits; governed `COMPILE_OK`, 0 errors/warnings; build check PASS |
| Q02 Baseline Screening | 2026-09-07 | HELD / NOT DISPATCHABLE | CPU sample max 97.7% breached 97% ceiling. `record-build` unexpectedly appended row `cbdc6809-d507-4d0f-9a27-a709dc5aa46c`; governed non-restart hold `PACER_Q02_CPU_CEILING_STOP` made it unclaimable before dispatch. |
