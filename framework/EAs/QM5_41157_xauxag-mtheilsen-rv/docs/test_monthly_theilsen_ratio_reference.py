"""Independent deterministic fixtures for QM5_41157's monthly ratio basket."""
...
def make_fixture(
    ...
    *,
    ...
    """Make 500-bar newest-first series with deliberate unmatched days."""
    ...
    for key in newest_first_keys:
        ...
        # Day 27 exists only in XAU and day 26 only in XAG. Day 25 is the
    ...
    while len(xau) < HISTORY_BARS:
        ...
        xau.append(CloseBar(opened, math.exp(-0.25)))
        filler_index += 1
    filler_index = 0
...
class MonthlyTheilSenRatioReferenceTest(unittest.TestCase):
    ...
    def test_latest_exact_pairs_are_selected_and_current_month_is_excluded(self) -> None:
        ...
        result = monthly_theilsen(current, xau, xag)
    ...
    def test_missing_synchronized_month_and_stale_endpoint_fail_closed(self) -> None:
        ...
        self.assertFalse(monthly_theilsen(current, xau, xag).valid)
        current, xau, xag = make_fixture(ratios)
        self.assertFalse(monthly_theilsen(current + timedelta(days=2), xau, xag).valid)
    ...
    def test_static_build_contract_matches_approved_card(self) -> None:
        source = (EA_DIR / "QM5_41157_xauxag-mtheilsen-rv.mq5").read_text(encoding="utf-8")
        for marker in (
        ...
        self.assertEqual(manifest["logical_symbol"], "QM5_41157_XAU_XAG_MTHEILSEN_RV_D1")
        self.assertEqual(manifest["host_symbol"], "XAUUSD.DWX")
        self.assertEqual(manifest["traded_symbols"], ["XAUUSD.DWX", "XAGUSD.DWX"])

        approved = REPO_ROOT / "strategy-seeds" / "cards" / "approved" / "QM5_41157_xauxag-mtheilsen-rv_card.md"
        canonical = REPO_ROOT / "strategy-seeds" / "cards" / "xauxag-mtheilsen-rv_card.md"
        local = EA_DIR / "docs" / "strategy_card.md"
        self.assertEqual(approved.read_bytes(), canonical.read_bytes())
        self.assertEqual(approved.read_bytes(), local.read_bytes())
        magic_rows = (REPO_ROOT / "framework" / "registry" / "magic_numbers.csv").read_text(
            encoding="utf-8-sig"
        )
        self.assertIn("41157,xauxag-mtheilsen-rv,0,XAUUSD.DWX,411570000", magic_rows)
        self.assertIn("41157,xauxag-mtheilsen-rv,1,XAGUSD.DWX,411570001", magic_rows)
        resolver = (
            REPO_ROOT / "framework" / "include" / "QM" / "QM_MagicResolver.mqh"
        ).read_text(encoding="utf-8")
        self.assertIn("411570000", resolver)
        self.assertIn("411570001", resolver)
