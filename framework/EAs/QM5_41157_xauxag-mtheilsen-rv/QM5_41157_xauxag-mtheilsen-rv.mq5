...
#property description "QM5_41157 XAU/XAG Thirteen-Month Theil-Sen Ratio Reversion"
...
// completed broker months. Form all 78 forward gold-minus-silver log-ratio
// slopes with denominator j-i, average sorted indexes 38 and 39, and fade the
// strict sign. The opposite, equal-notional package is held to the next month.
// Runtime is native, completed-price-only, deterministic, and one-attempt per
// month.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41157;
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41157 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_MTHEILSEN_RV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41157_XAUXAG_MTHEILSEN_RV";
...
bool Strategy_LoadMonthlyTheilSen(
      ...
      int &direction)
  {
      ...
      for(int j = i + 1; j < strategy_month_end_count; ++j)
        {
         ...
         if(month_distance <= 0 || slope_index < 0 ||
            slope_index >= expected_slope_count)
            return false;
         const double slope =
   ...
   slope_count = slope_index;
   if(slope_count != expected_slope_count || !ArraySort(slopes))
      return false;
   for(int index = 0; index < slope_count; ++index)
     {
      if(!MathIsValidNumber(slopes[index]) ||
         (index > 0 && slopes[index] < slopes[index - 1]))
         return false;
     }

   median_left_index = slope_count / 2 - 1;
   median_right_index = slope_count / 2;
   if(median_left_index != 38 || median_right_index != 39)
      return false;
   center_low = slopes[median_left_index];
   center_high = slopes[median_right_index];
   theilsen_slope = center_low / 2.0 + center_high / 2.0;
   if(!MathIsValidNumber(center_low) ||
      !MathIsValidNumber(center_high) || center_low > center_high ||
      !MathIsValidNumber(theilsen_slope))
      return false;

   // direction > 0 means BUY XAU / SELL XAG. Fade only the strict robust
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41157_XAUXAG_MTHEILSEN_RV_HOST";
   ...
   double center_low = 0.0;
   double center_high = 0.0;
   double theilsen_slope = 0.0;
   const bool valid =
      Strategy_LoadMonthlyTheilSen(
         g_signal_month_key,
         month_count,
         ...
         theilsen_slope,
         direction);
   string state = "monthly_theilsen_failed";
   if(valid && direction > 0)
      state = "negative_theilsen_fade_long_xau";
   else if(valid && direction < 0)
      state = "positive_theilsen_fade_short_xau";
   else if(valid && theilsen_slope == 0.0)
      state = "zero_theilsen_flat";
   else if(valid)
      state = "theilsen_sign_flat";

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"month_count\":%d,\"newest_endpoint_time\":%I64d,\"endpoint_displacement\":%.12e,\"slope_count\":%d,\"median_left_index\":%d,\"median_right_index\":%d,\"center_low\":%.12e,\"center_high\":%.12e,\"theilsen_slope\":%.12e,\"direction\":%d,\"state\":\"%s\"}",
                            g_signal_month_key,
                            month_count,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG thirteen-month Theil-Sen ratio reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41157\",\"ea\":\"xauxag-mtheilsen-rv\"}");
