...
#property description "QM5_41170 WTI Monthly Bartels Rank-Persistence Trend"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41170 - WTI Monthly Bartels Rank-Persistence Trend
// -----------------------------------------------------------------------------
// D1 structural crude-oil sleeve:
//   - reconstruct thirteen consecutive completed broker-month-end closes
//   - rank the closes and square twelve successive chronological rank moves
//   - follow the endpoint direction only when Bartels RVN is below two
...
input int    strategy_nm_boundary          = 364;
...
bool Strategy_BartelsRankSignal(const double &closes[],
                                Strategy_SignalMetrics &metrics)
  {
   ...
   for(int index = 0; index < strategy_endpoint_count; ++index)
     {
      ...
      const int centered = rank - 7;
      metrics.rank_denominator += centered * centered;
     }
   ...
   if(metrics.rank_sum != 91 || metrics.rank_denominator != 182)
      return false;

   for(int index = 0; index < strategy_endpoint_count - 1; ++index)
     {
      const int delta = ranks[index + 1] - ranks[index];
      metrics.rank_numerator += delta * delta;
     }
   if(metrics.rank_numerator <= 0)
      return false;

   metrics.rvn =
      (double)metrics.rank_numerator /
      (double)metrics.rank_denominator;
   if(!MathIsValidNumber(metrics.rvn) || metrics.rvn <= 0.0)
      return false;

   metrics.oldest_close = closes[0];
   metrics.newest_close = closes[strategy_endpoint_count - 1];
   metrics.direction = 0;
   if(metrics.rank_numerator < strategy_nm_boundary)
     {
                                        ...
                                        int &direction)
  {
   ...
   const bool valid =
      Strategy_LoadMonthlyEndpoints(current_month_key,
                                    ...
                                    closes,
                                    endpoint_times) &&
      Strategy_BartelsRankSignal(closes, metrics);
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
   ...
   else
     {
      ...
      g_signal_valid =
         endpoints_valid &&
         Strategy_BartelsRankSignal(closes, g_signal_metrics);
      if(!g_signal_valid)
         g_signal_state = "bartels_rank_validation_failed";
      else if(g_signal_metrics.direction == 0)
         g_signal_state = "bartels_rvn_not_below_two_flat";
      else if(g_signal_metrics.direction > 0)
         g_signal_state = "bartels_rank_wti_long";
      else if(g_signal_metrics.direction < 0)
         g_signal_state = "bartels_rank_wti_short";
      else
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"endpoint_count\":%d,\"rank_sum\":%d,\"rank_denominator\":%d,\"rank_numerator\":%d,\"rvn\":%.10f,\"oldest_close\":%.10f,\"newest_close\":%.10f,\"oldest_endpoint\":%I64d,\"newest_endpoint\":%I64d,\"state\":\"%s\"}",
                            ...
                            g_signal_metrics.rank_denominator,
                            g_signal_metrics.rank_numerator,
                            g_signal_metrics.rvn,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41170 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41170_WTI_BARTELS_RANK";
   ...
   req.reason = (g_signal_metrics.direction > 0)
      ? "WTI_BARTELS_RANK_LONG"
      : "WTI_BARTELS_RANK_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41170 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI Bartels rank-persistence trend holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41170_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41170\",\"ea\":\"wti-bartels-rank-tr\"}");
