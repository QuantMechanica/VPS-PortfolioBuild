...
#property description "QM5_10062 Connors CVR 1 D1"
...
input int    strategy_vix_lookback      = 5;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   for(int p = PositionsTotal() - 1; p >= 0; --p)
     {
      ...
      if(StringFind(comment, "CVR") >= 0 || foreign_ea_id == 10063)
         return false;
     }
   ...
   req.sl = sl;
   req.tp = 0.0;
   req.reason = long_signal ? "CVR1_LONG" : "CVR1_SHORT";
   return true;
  }
...
void Strategy_ManageOpenPosition()
  {
   // Card specifies fixed SL plus time/opposite-signal exits; no trailing,
