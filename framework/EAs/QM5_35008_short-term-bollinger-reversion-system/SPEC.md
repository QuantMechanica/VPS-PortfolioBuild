...
**Source:** `short-term-bollinger-reversion-system-official-source` (see `strategy-seeds/sources/short-term-bollinger-reversion-system-official-source/`)
...
| `strategy_bb_dev` | 2.50 | (see source) | (see strategy logic) |
| `strategy_rsi_period` | 14 | (see source) | (see strategy logic) |
| `strategy_rsi_oversold` | 30.0 | (see source) | (see strategy logic) |
| `strategy_rsi_overbought` | 70.0 | (see source) | (see strategy logic) |
| `strategy_atr_period` | 14 | (see source) | (see strategy logic) |
| `strategy_sl_atr_mult` | 1.50 | (see source) | (see strategy logic) |
| `strategy_entry_start_hhmm` | 1800 | (see source) | (see strategy logic) |
| `strategy_entry_end_hhmm` | 2200 | (see source) | (see strategy logic) |
| `strategy_exit_hhmm` | 2300 | (see source) | (see strategy logic) |
...
| `strategy_max_slippage_ticks` | 3.0 | 0.0 < value <= 3.0 | Maximum market-order deviation, converted from symbol ticks to MT5 points. |
...
**Pointer:** `strategy-seeds/sources/short-term-bollinger-reversion-system-official-source/`
...
| v2 | 2026-08-24 | Review rework | Closed-deal daily halt, persistent UTC time exit, fail-closed inputs, M15 contract, and three-tick deviation cap. |
