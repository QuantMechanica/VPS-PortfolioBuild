...
#property description "QM5_10588 MQL5 ForexProfitBoost Trend Color Change"
...
input int    strategy_fast_ema_period   = 7;
...
int Strategy_FPBTrendColor(const int shift)
  {
   if(strategy_fast_ema_period <= 1 ||
      strategy_slow_sma_period <= 1 ||
      strategy_bb_period <= 1 ||
      strategy_bb_deviation <= 0.0)
      return 0;

   const double fast = QM_EMA(_Symbol, strategy_signal_tf, strategy_fast_ema_period, shift);
   const double slow = QM_SMA(_Symbol, strategy_signal_tf, strategy_slow_sma_period, shift);
   const double upper = QM_BB_Upper(_Symbol, strategy_signal_tf, strategy_bb_period, strategy_bb_deviation, shift);
   const double lower = QM_BB_Lower(_Symbol, strategy_signal_tf, strategy_bb_period, strategy_bb_deviation, shift);
   if(fast <= 0.0 || slow <= 0.0 || upper <= 0.0 || lower <= 0.0)
      return 0;
...
int Strategy_FPBColorChangeSignal()
  {
   const int latest = Strategy_FPBTrendColor(1);
   const int prior = Strategy_FPBTrendColor(2);
   if(latest == 1 && prior == -1)
      return 1;
   if(latest == -1 && prior == 1)
      return -1;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   const int signal = Strategy_FPBColorChangeSignal();
   if(signal == 0)
      return false;
   ...
   req.reason = (signal > 0) ? "FPB_BLUE_COLOR_CHANGE" : "FPB_PINK_COLOR_CHANGE";
   req.symbol_slot = qm_magic_slot_offset;
...
bool Strategy_ExitSignal()
  {
   // Opposite ForexProfitBoost color changes are handled inside Strategy_EntrySignal
   // under the framework closed-bar gate so this hook stays O(1) on the tick path.
   return false;
  }
