...
#property description "QM5_41377 WTI Disjoint Weekly Momentum Agreement"
...
//   - validate four consecutive completed 3-to-5-session broker weeks
//   - compute disjoint t-1 and cumulative t-4..t-2 log-return signs
//   - follow only strict sign agreement; remain flat on disagreement
//   - consume the week before fallible gates; never enter late or retry
//   - hold the signal for one broker week
// Runtime uses MT5-native OHLC, calendar, history, and framework state only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                      = 41377;
...
double   g_week_log_return        = 0.0;
double   g_recent_week_open        = 0.0;
double   g_recent_week_close       = 0.0;
double   g_recent_week_log_return  = 0.0;
...
void Strategy_ResetDecisionState()
  {
   ...
   g_week_open = 0.0;
   g_week_close = 0.0;
   g_week_log_return = 0.0;
   g_recent_week_open = 0.0;
   g_recent_week_close = 0.0;
   g_recent_week_log_return = 0.0;
   g_signal_state = "idle";
  }
...
bool Strategy_LoadWeeklyMomentumSignal(const int current_week_key,
                                       const int label_offset,
                                       int &direction,
                                       int &week_bars,
                                       double &week_open,
                                       double &week_close,
                                       double &week_log_return)
  {
   direction = 0;
   week_bars = 0;
   week_open = 0.0;
   ...
   ArraySetAsSeries(bars, true);
   const int copied =
      CopyRates(_Symbol, // perf-allowed: bounded disjoint-horizon scan behind the sole QM_IsNewBar branch.
                PERIOD_D1,
                1,
                strategy_history_bars,
                bars);
   if(copied < 4 * strategy_min_week_bars + 1)
      return false;

   int week_keys[4];
   int week_counts[4];
   double week_opens[4];
   double week_closes[4];
   ArrayInitialize(week_keys, 0);
   ArrayInitialize(week_counts, 0);
   ArrayInitialize(week_opens, 0.0);
   ArrayInitialize(week_closes, 0.0);

   int week_index = -1;
   int active_week_key = 0;
   ...
   for(int index = 0; index < copied; ++index)
     {
      ...
      if(week_key != active_week_key)
        {
         if(active_week_key == 0)
           {
            if(Strategy_NextWeekKey(week_key) != current_week_key)
               return false;
            week_index = 0; // t-1: recent source block.
           }
         else
           {
            if(Strategy_NextWeekKey(week_key) != active_week_key)
               return false;
            if(week_index == 3)
              {
      ...
      // Series order is newest-to-oldest. First bar is the week's final close;
      // overwriting open leaves its chronologically first session open.
      if(week_counts[week_index] == 0)
         week_closes[week_index] = bars[index].close;
      week_opens[week_index] = bars[index].open;
      ++week_counts[week_index];
      if(week_counts[week_index] > strategy_max_week_bars)
         return false;
     }

   if(!older_boundary_seen || week_index != 3)
      return false;
   for(int index = 0; index < 4; ++index)
     {
      if(week_keys[index] <= 0 ||
         week_counts[index] < strategy_min_week_bars ||
         week_counts[index] > strategy_max_week_bars)
         return false;
      if(index > 0 &&
         Strategy_NextWeekKey(week_keys[index]) != week_keys[index - 1])
         return false;
     }

   // The source blocks are disjoint: t-1 stands alone, while the older
   // block runs from the first open of t-4 through the final close of t-2.
   g_recent_week_open = week_opens[0];
   g_recent_week_close = week_closes[0];
   week_open = week_opens[3];
   week_close = week_closes[1];
   week_bars = week_counts[0] + week_counts[1] + week_counts[2] + week_counts[3];
   if(g_recent_week_open <= 0.0 || g_recent_week_close <= 0.0 ||
      week_open <= 0.0 || week_close <= 0.0 ||
      !MathIsValidNumber(g_recent_week_open) ||
      !MathIsValidNumber(g_recent_week_close) ||
      !MathIsValidNumber(week_open) ||
      !MathIsValidNumber(week_close))
      return false;

   g_recent_week_log_return =
      MathLog(g_recent_week_close / g_recent_week_open);
   week_log_return = MathLog(week_close / week_open);
   if(!MathIsValidNumber(g_recent_week_log_return) ||
      !MathIsValidNumber(week_log_return))
      return false;

   const bool positive_agreement =
      (g_recent_week_log_return > strategy_return_epsilon &&
       week_log_return > strategy_return_epsilon);
   const bool negative_agreement =
      (g_recent_week_log_return < -strategy_return_epsilon &&
       week_log_return < -strategy_return_epsilon);
   if(positive_agreement)
      direction = 1;
   else if(negative_agreement)
      direction = -1;
   return true;
  }
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_WeekAlreadyEntered(g_decision_week_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadWeeklyMomentumSignal(g_decision_week_key,
                                           ...
                                           g_week_log_return);
      if(!g_signal_valid)
         g_signal_state = "disjoint_horizon_validation_failed";
      else if(g_signal_direction > 0)
         g_signal_state = "positive_disjoint_agreement_long";
      else if(g_signal_direction < 0)
         g_signal_state = "negative_disjoint_agreement_short";
      else
         g_signal_state = "disagreement_or_zero_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"week\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_week_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"formation_bars\":%d,\"recent_open_t1\":%.8f,\"recent_close_t1\":%.8f,\"recent_log_return\":%.12f,\"prior_open_t4\":%.8f,\"prior_close_t2\":%.8f,\"prior_log_return\":%.12f,\"state\":\"%s\"}",
                            g_decision_week_key,
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(RISK_PERCENT != 0.0 ||
      ...
      !MathIsValidNumber(PORTFOLIO_WEIGHT) || PORTFOLIO_WEIGHT <= 0.0)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41377_WTI_WMOM142_AGREE";
   ...
   req.reason = (g_signal_direction > 0)
       ? "WTI_WMOM142_AGREE_LONG"
       : "WTI_WMOM142_AGREE_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(strategy_symbol, true) ||
       !Strategy_IsHostChart() || qm_ea_id != 41377 ||
      qm_magic_slot_offset != 0)
   ...
   g_attempt_state_key =
      StringFormat("QM5_41377_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
                "{\"card\":\"QM5_41377\",\"ea\":\"wti-wmom142-agree\"}");
