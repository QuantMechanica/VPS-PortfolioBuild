...
#property description "QM5_10206 TradingView 9 EMA Full Candle Continuation"
...
input int             strategy_us_start_hhmm      = 1630;
input int             strategy_us_end_hhmm        = 2300;
input int             strategy_overlap_start_hhmm = 1500;
input int             strategy_overlap_end_hhmm   = 1900;
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(_Symbol == "SP500.DWX" || _Symbol == "NDX.DWX" || _Symbol == "WS30.DWX")
      session_ok = (hhmm >= strategy_us_start_hhmm && hhmm < strategy_us_end_hhmm);
   else
      session_ok = (hhmm >= strategy_overlap_start_hhmm && hhmm < strategy_overlap_end_hhmm);
   if(!session_ok)
      return true;
   ...
   return ((ask - bid) > emergency_stop_distance * strategy_spread_stop_frac);
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(strategy_ema_period <= 0 || strategy_atr_period <= 0 || strategy_atr_sl_mult <= 0.0)
      return false;
   ...
   const bool long_now = (low1 > ema1);
   const bool long_prev = (low2 > ema2);
   const bool short_now = (high1 < ema1);
   const bool short_prev = (high2 < ema2);
   ...
   if(long_now && !long_prev)
     {
      ...
      req.type = QM_BUY;
      req.sl = sl;
      req.reason = "FULL_CANDLE_ABOVE_EMA9";
      return true;
     }
   ...
   if(short_now && !short_prev)
     {
      ...
      req.type = QM_SELL;
      req.sl = sl;
      req.reason = "FULL_CANDLE_BELOW_EMA9";
      return true;
     }
...
void Strategy_ManageOpenPosition()
  {
   // Card defines no trailing, break-even, partial close, or stop adjustment.
  }
