...
#property description "QM5_12607 WTI CAD Confirmation Trend"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_12607 - WTI CAD Confirmation Trend
// -----------------------------------------------------------------------------
// D1 structural WTI sleeve:
//   - host/traded symbol: XTIUSD.DWX
//   - confirmation symbol: USDCAD.DWX
//   - weekly entry gate
//   - direction requires WTI momentum and opposite-direction USDCAD movement
// Runtime uses MT5 OHLC/broker calendar only; no macro feed, API, or ML.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 12607;
...
input string strategy_cad_symbol           = "USDCAD.DWX";
input int    strategy_oil_lookback_d1      = 63;
input int    strategy_cad_lookback_d1      = 63;
input double strategy_min_oil_return_pct   = 3.0;
input double strategy_min_cad_return_pct   = 1.0;
...
bool Strategy_LoadSignal(int &direction)
  {
   ...
   double cad_return = 0.0;
   if(!Strategy_LoadLogReturn(_Symbol, strategy_oil_lookback_d1, oil_return))
      return false;
   if(!Strategy_LoadLogReturn(strategy_cad_symbol, strategy_cad_lookback_d1, cad_return))
      return false;
   ...
   const double cad_threshold = MathMax(0.0, strategy_min_cad_return_pct) / 100.0;

   if(oil_return > oil_threshold && cad_return < -cad_threshold && oil_close > trend_sma)
      direction = 1;
   else if(oil_return < -oil_threshold && cad_return > cad_threshold && oil_close < trend_sma)
      direction = -1;
   else
      direction = 0;
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(qm_magic_slot_offset != 0)
      return true;
   if(strategy_cad_symbol != "USDCAD.DWX")
      return true;
   if(strategy_oil_lookback_d1 < 10 || strategy_cad_lookback_d1 < 10)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12607_WTI_CAD_CONFIRM";
   ...
   req.reason = (direction > 0) ? "WTI_CAD_CONFIRM_LONG" : "WTI_CAD_CONFIRM_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(strategy_cad_symbol, true))
     {
      PrintFormat("QM5_12607 failed to select confirmation symbol %s", strategy_cad_symbol);
      return INIT_FAILED;
     }
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_12607\",\"ea\":\"wti-cad-confirm\"}");
