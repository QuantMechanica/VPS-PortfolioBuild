...
#property description "QM5_1177 Quantpedia High-VIX SP500 Premia Timing"

#include <QM/QM_Common.mqh>

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 1177;
...
input string strategy_vix_csv_path         = "QM5_1177_vix_monthly.csv";
input double strategy_vix_threshold        = 25.0;
...
input int    strategy_vix_stale_months     = 3;
...
bool Strategy_ReadLatestVix(const datetime signal_day, double &vix_close, datetime &obs_date)
  {
   vix_close = 0.0;
   obs_date = 0;
   if(strategy_vix_csv_path == "")
      return false;
   ...
   while(!FileIsEnding(handle))
     {
      ...
      const string vix_field = FileReadString(handle);
      if(date_field == "" && vix_field == "")
         continue;
      ...
      const double row_vix = StringToDouble(vix_field);
      if(row_vix <= 0.0)
         continue;
   ...
   FileClose(handle);
   if(obs_date <= 0 || vix_close <= 0.0)
      return false;

   const int max_stale_months = MathMax(1, strategy_vix_stale_months);
   if(Strategy_MonthsBetween(obs_date, signal_day) > max_stale_months)
      return false;

   return true;
  }

bool Strategy_HighVixSignal(const datetime signal_day, double &vix_close)
  {
   vix_close = 0.0;
   datetime obs_date = 0;
   if(strategy_vix_threshold <= 0.0)
      return false;
   if(!Strategy_ReadLatestVix(signal_day, vix_close, obs_date))
      return false;
   return (vix_close > strategy_vix_threshold);
  }

bool Strategy_HasOpenPosition(ulong &ticket, datetime &opened_at, double &open_price)
  {
   ticket = 0;
   opened_at = 0;
   open_price = 0.0;
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(qm_magic_slot_offset != 0)
      return true;
   if(Bars(_Symbol, PERIOD_D1) < MathMax(strategy_min_d1_bars, strategy_atr_period_d1 + 5))
      return true;
   if(strategy_hold_months <= 0 || strategy_atr_period_d1 <= 0 || strategy_atr_sl_mult <= 0.0)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(Strategy_HasOpenPosition(ticket, opened_at, open_price))
      return false;

   double vix_close = 0.0;
   if(!Strategy_HighVixSignal(signal_day, vix_close))
      return false;
   ...
   req.tp = 0.0;
   req.reason = "QM5_1177_HIGH_VIX_SP500_LONG";
...
void Strategy_ManageOpenPosition()
  {
   // Card specifies an initial ATR stop, a 10% safety stop, and monthly exits.
  }
...
bool Strategy_ExitSignal()
  {
   ...
   if(bid > 0.0 && open_price > 0.0)
     {
      const double safety_level = open_price * (1.0 - strategy_safety_stop_pct / 100.0);
      if(bid <= safety_level)
         return true;
     }
   ...
   if(!Strategy_IsMonthTurnSignalDay(signal_day) || g_last_exit_signal_day == signal_day)
      return false;
   if(Strategy_MonthsBetween(opened_at, signal_day) < MathMax(1, strategy_hold_months))
      return false;

   g_last_exit_signal_day = signal_day;

   double vix_close = 0.0;
   return !Strategy_HighVixSignal(signal_day, vix_close);
  }
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_1177\",\"ea\":\"qp-vix80-sp500-premia\"}");
