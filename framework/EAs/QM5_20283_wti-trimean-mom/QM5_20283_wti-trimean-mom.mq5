...
#property description "QM5_20283 WTI Quartile-Trimean Return Trend"
...
//   2. form twelve disjoint chronological monthly log returns and sort them;
//   3. estimate Q1, median, and Q3 with fixed even-half order indexes;
//   4. combine those locations with fixed 1:2:1 weights;
//   5. trade the sign of the resulting quartile trimean.
...
double g_cache_q1               = 0.0;
double g_cache_median           = 0.0;
double g_cache_q3               = 0.0;
double g_cache_trimean          = 0.0;
...
bool Strategy_TrimeanSignal(const double &closes[], int &signal)
  {
   signal = 0;
   const int return_count = strategy_return_months;
   if(return_count != 12 || ArraySize(closes) != return_count + 1)
     {
      g_cache_state_reason = "bad_trimean_contract";
      return false;
     }
   ...
   const double q1 =
      (sorted_returns[2] + sorted_returns[3]) / 2.0;
   const double median =
      (sorted_returns[5] + sorted_returns[6]) / 2.0;
   const double q3 =
      (sorted_returns[8] + sorted_returns[9]) / 2.0;
   if(!MathIsValidNumber(q1) ||
      !MathIsValidNumber(median) ||
      !MathIsValidNumber(q3) ||
      q1 > median || median > q3)
     {
      g_cache_state_reason = "invalid_quartile_state";
      return false;
     }

   const double trimean = (q1 + 2.0 * median + q3) / 4.0;
   if(!MathIsValidNumber(trimean))
     {
      g_cache_state_reason = "invalid_trimean";
      return false;
     }
   ...
   g_cache_q3 = q3;
   g_cache_trimean = trimean;
   if(trimean == 0.0)
     {
      g_cache_state_reason = "exact_zero_trimean";
      return true;
     }

   signal = (trimean > 0.0) ? 1 : -1;
...
bool Strategy_LoadSignalState(int &signal)
  {
   ...
   return Strategy_TrimeanSignal(monthly_closes, signal);
  }
...
void Strategy_PrepareMonthlySignal()
  {
   ...
   QM_LogEvent(QM_INFO,
               "MONTHLY_STATE",
               StringFormat("{\"month\":%d,\"valid\":%s,\"signal\":%d,\"q1\":%.10f,\"median\":%.10f,\"q3\":%.10f,\"trimean\":%.10f,\"state\":\"%s\"}",
                            g_cache_month_key,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 20283 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20283_WTI_TRIMEAN_MOM";
   ...
   req.reason = (g_cache_signal > 0) ? "TRIMEAN_MOM_XTI_LONG" : "TRIMEAN_MOM_XTI_SHORT";
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_20283_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_20283\",\"ea\":\"wti-trimean-mom\"}");
