...
#property description "QM5_41045 WTI Standard-Wednesday Event / Slow-Trend Agreement"
...
//   - enter Thursday only when both nonzero returns have the same strict sign,
//     following that common sign
...
bool Strategy_LoadWednesdayTrendAgreement(const datetime current_bar_time,
                                           const datetime broker_now,
                                           double &event_return,
                                           double &slow_trend,
                                           int &direction)
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "WTI_WED_TREND_AGREE";
   ...
   int direction = 0;
   if(!Strategy_LoadWednesdayTrendAgreement(g_strategy_d1_bar_time,
                                             broker_now,
                                             event_return,
                                             slow_trend,
                                             direction))
   ...
   if(direction > 0)
     {
      req.type = QM_BUY;
      req.reason = "WTI_WED_TREND_AGREE_LONG";
     }
   else if(direction < 0)
     {
      req.type = QM_SELL;
      req.reason = "WTI_WED_TREND_AGREE_SHORT";
     }
   else
      return false;
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI Wednesday event/slow-trend card exits next D1; Friday 21 is fail-safe"))
      return INIT_FAILED;
   ...
   g_attempt_state_key =
      StringFormat("QM5_41045_WED_TREND_AGREE_THURSDAY_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41045\",\"ea\":\"wti-wed-trend-agree\"}");
