...
input int    strategy_fast_ema_period         = 2;
input int    strategy_slow_ema_period         = 5;
input int    strategy_atr_period              = 20;
input double strategy_initial_atr_sl_mult     = 1.5;
input double strategy_trailing_atr_mult       = 1.0;
input double strategy_max_spread_atr_fraction = 0.20;
input int    strategy_index_flat_hour_broker  = 21;
...
bool Strategy_IsIndexSymbol(const string symbol)
  {
...
bool Strategy_NoTradeFilter()
  {
   // No Trade Filter: spread gate plus overnight block for index CFDs.
   ...
   if(atr <= 0.0 || point <= 0.0)
      return true;
   if((double)spread_points * point > atr * strategy_max_spread_atr_fraction)
      return true;

   if(Strategy_IsIndexSymbol(_Symbol))
     {
      MqlDateTime dt;
      TimeToStruct(TimeCurrent(), dt);
      if(dt.hour >= strategy_index_flat_hour_broker && !Strategy_HasOpenPosition())
         return true;
     }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   // Trade Entry: completed-bar EMA(2/5) cross, entered on next bar.
   req.type = QM_BUY;
   req.price = 0.0;
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "ET_EMA2_5_CROSS";
   ...
   const double sl = QM_StopATR(_Symbol, side, entry, strategy_atr_period, strategy_initial_atr_sl_mult);
   if(sl <= 0.0)
      return false;
...
void Strategy_ManageOpenPosition()
  {
   // Trade Management: after +1R, trail at 1.0 ATR(20).
   const int magic = QM_FrameworkMagic();
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      const ulong ticket = PositionGetTicket(i);
      ...
      if(open_price <= 0.0 || current_sl <= 0.0 || bid <= 0.0 || ask <= 0.0)
         continue;
      ...
      if(Strategy_IsIndexSymbol(_Symbol))
        {
         MqlDateTime dt;
         TimeToStruct(TimeCurrent(), dt);
         if(dt.hour >= strategy_index_flat_hour_broker)
            return true;
        }
