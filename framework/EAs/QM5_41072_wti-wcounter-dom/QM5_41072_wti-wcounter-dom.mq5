...
#property description "QM5_41072 WTI Completed-Week Countershock Dominance"
...
//   - require outer/countershock/restoration signs and strict middle dominance
//   - prove the middle move exceeds both outer moves combined
//   - consume the week before fallible gates; never enter late or retry
//   - follow the middle and cumulative-three-week sign for one week
// Runtime uses MT5-native OHLC, calendar, history, and framework state only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                      = 41072;
...
bool Strategy_LoadCountershockSignal(const int current_week_key,
                                    const int label_offset,
                                    int &direction,
                                    ...
                                    double &week_end_c4)
  {
   ...
   const double combined_outer_magnitude =
      MathAbs(oldest_return) + MathAbs(newest_return);
   if(oldest_return > 0.0 && middle_return < 0.0 &&
      newest_return > 0.0 &&
      MathAbs(middle_return) > combined_outer_magnitude)
      direction = -1;
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_WeekAlreadyEntered(g_decision_week_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadCountershockSignal(g_decision_week_key,
      ...
      if(!g_signal_valid)
         ...
         g_signal_state = "positive_middle_net_long";
      else if(g_signal_direction < 0)
         g_signal_state = "negative_middle_net_short";
         ...
         g_signal_state = "exact_zero_flat";
              ...
              (g_oldest_return < 0.0 && g_middle_return > 0.0 &&
               g_newest_return < 0.0))
         g_signal_state = "middle_not_combined_dominant_flat";
      else
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41072 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41072_WTI_WCOUNTER_DOM";
   ...
   req.reason = (g_signal_direction > 0)
      ? "WTI_WCOUNTER_DOM_LONG"
      : "WTI_WCOUNTER_DOM_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_strategy_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41072 ||
   ...
   g_attempt_state_key =
      StringFormat("QM5_41072_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41072\",\"ea\":\"wti-wcounter-dom\"}");
