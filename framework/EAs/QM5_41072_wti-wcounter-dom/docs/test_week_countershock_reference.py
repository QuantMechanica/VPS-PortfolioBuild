"""Deterministic reference checks for QM5_41072 WTI weekly countershock dominance."""
...
def countershock_signal(
    current_week_key: int,
    completed_newest_first: list[Bar],
    offset: timedelta,
) -> tuple[bool, int, tuple[float, float, float], tuple[float, float, float, float]]:
    """Mirror four week ends and strict middle-over-combined-outer dominance."""
    ...
    newest = math.log(closes[0] / closes[1])
    middle = math.log(closes[1] / closes[2])
    oldest = math.log(closes[2] / closes[3])
    direction = 0
    combined_outer = abs(oldest) + abs(newest)
    if (
        oldest > 0.0
        and middle < 0.0
        and newest > 0.0
        and abs(middle) > combined_outer
    ):
        direction = -1
    elif (
        oldest < 0.0
        ...
        direction = 1
    return True, direction, (oldest, middle, newest), tuple(closes)
...
class WeekCountershockReferenceTest(unittest.TestCase):
    def test_positive_middle_net_sign_is_long(self) -> None:
        current, now, bars = sample(110.0, 120.0, 95.0, 100.0)
        decision, late, count, offset = decision_clock(current, now, bars)
        self.assertTrue(decision)
        self.assertFalse(late)
        self.assertEqual(count, 0)
        assert offset is not None
        valid, direction, returns, endpoints = countershock_signal(
            20260824, bars, offset
        )
        oldest, middle, newest = returns
        self.assertTrue(valid)
        self.assertEqual(direction, 1)
        self.assertLess(oldest, 0.0)
        self.assertGreater(middle, 0.0)
        self.assertLess(newest, 0.0)
        self.assertGreater(abs(middle), abs(oldest) + abs(newest))
        self.assertGreater(oldest + middle + newest, 0.0)
        self.assertEqual(endpoints, (110.0, 120.0, 95.0, 100.0))

    def test_negative_middle_net_sign_is_short(self) -> None:
        current, now, bars = sample(90.0, 80.0, 105.0, 100.0)
        offset = label_offset(current, now)
        assert offset is not None
        valid, direction, returns, _ = countershock_signal(
            20260824, bars, offset
        )
        oldest, middle, newest = returns
        self.assertTrue(valid)
        ...
        self.assertGreater(abs(middle), abs(oldest) + abs(newest))
        self.assertLess(oldest + middle + newest, 0.0)

    def test_wrong_path_zero_equal_and_nondominant_states_are_flat(self) -> None:
        cases = [
            sample(110.0, 120.0, 105.0, 100.0),
            sample(100.0, 90.0, 80.0, 100.0),
            sample(100.0, 100.0, 95.0, 100.0),
            sample(1.0, math.exp(-1.0), math.e, 1.0),
            sample(98.0, 100.0, 95.0, 100.0),
        ]
        for current, now, bars in cases:
            offset = label_offset(current, now)
            assert offset is not None
            valid, direction, _, _ = countershock_signal(
                20260824, bars, offset
            )
            self.assertTrue(valid)
    ...
    def test_uniform_prior_date_label_normalization_and_grace(self) -> None:
        current, now, bars = sample(110.0, 120.0, 95.0, 100.0, True)
        decision, late, count, offset = decision_clock(current, now, bars)
        self.assertTrue(decision)
        self.assertFalse(late)
        self.assertEqual(count, 0)
        ...
        assert offset is not None
        self.assertEqual(countershock_signal(20260824, bars, offset)[1], 1)
    ...
    def test_nonconsecutive_or_current_week_endpoint_rejected(self) -> None:
        ...
        self.assertFalse(countershock_signal(20260824, bars, offset)[0])
        _, _, bars = sample(110.0, 120.0, 95.0, 100.0)
        bars.insert(0, Bar(datetime(2026, 8, 24, tzinfo=UTC), 111.0))
        self.assertFalse(countershock_signal(20260824, bars, offset)[0])
    ...
    def test_bad_endpoint_reverse_order_and_short_history_rejected(self) -> None:
        ...
        self.assertFalse(countershock_signal(20260824, bars, offset)[0])
        _, _, bars = sample(110.0, 120.0, 95.0, 100.0)
        self.assertFalse(countershock_signal(20260824, bars[:-1], offset)[0])
