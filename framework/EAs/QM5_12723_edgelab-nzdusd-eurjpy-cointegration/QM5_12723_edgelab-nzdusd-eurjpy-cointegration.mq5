...
#property description "QM5_12723 Edge Lab NZDUSD EURJPY Cointegration"
...
bool Strategy_IsHostSymbol()
  {
   return (_Symbol == g_leg_nzdusd || _Symbol == g_leg_eurjpy);
  }
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(!Strategy_EnsureBasketScope())
      return false;
   if(!QM_SymbolAssertOrLog(g_leg_nzdusd) || !QM_SymbolAssertOrLog(g_leg_eurjpy))
      return false;
   ...
   for(int i = 0; i < lookback; ++i)
     {
      if(nzdusd[i] <= 0.0 || eurjpy[i] <= 0.0)
         return false;
      spreads[i] = MathLog(nzdusd[i]) - strategy_beta * MathLog(eurjpy[i]);
...
bool Strategy_OpenPair(const int spread_direction)
  {
   ...
   const double weight_sum = nzdusd_weight + eurjpy_weight;
   if(weight_sum <= 0.0)
      return false;
   ...
   const string reason = long_spread ? "QM5_12723_LONG_SPREAD_Z_LT_NEG_ENTRY"
                                     : "QM5_12723_SHORT_SPREAD_Z_GT_POS_ENTRY";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12723_PAIR_HOST";
