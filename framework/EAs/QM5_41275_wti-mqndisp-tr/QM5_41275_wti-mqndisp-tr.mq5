...
#property description "QM5_41275 WTI Completed-Month Qn-Core Dispersion Trend"
...
//   - sort all 120 pairwise absolute return distances
//   - use the raw 36th one-based Qn core, without consistency multipliers
//   - continue only when net return reaches an inclusive four-core boundary
...
input int    strategy_distance_count         = 120;
input int    strategy_qn_order_one_based     = 36;
input double strategy_qn_core_floor          = 0.000000000001;
input double strategy_net_core_multiplier    = 4.0;
...
// -----------------------------------------------------------------------------
// Completed-month reconstruction and exact Qn-core score.
// -----------------------------------------------------------------------------
...
bool Strategy_QnSupportValid()
  {
   ...
   const int h = n / 2 + 1;
   const int k = h * (h - 1) / 2;
   return (pair_count == 120 && h == 9 && k == 36 &&
           k - 1 == 35);
  }

bool Strategy_QnSignal(const double &closes[],
                       ...
                       Strategy_SignalMetrics &metrics)
  {
   ...
   for(int left = 0; left < 15; ++left)
     {
      for(int right = left + 1; right < 16; ++right)
        {
         if(left < 0 || right <= left || right >= 16 ||
            metrics.distance_count < 0 ||
            metrics.distance_count >= 120)
            return false;
         const double distance =
            MathAbs(log_returns[right] - log_returns[left]);
         if(distance < 0.0 || !MathIsValidNumber(distance))
            return false;
         distances[metrics.distance_count] = distance;
         ++metrics.distance_count;
        }
     }
   if(metrics.distance_count != strategy_distance_count)
      return false;
   ...
   if(!MathIsValidNumber(metrics.q_core) ||
      metrics.q_core <= strategy_qn_core_floor)
      return true;

   metrics.threshold =
      strategy_net_core_multiplier * metrics.q_core;
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   else
     {
      ...
      g_signal_valid =
         ...
         Strategy_QnSignal(closes,
                           ...
                           g_signal_metrics);
      if(!g_signal_valid)
         g_signal_state = "qn_validation_failed";
      else if(g_signal_metrics.q_core <= strategy_qn_core_floor)
         g_signal_state = "qn_core_floor_flat";
      else if(g_signal_metrics.direction > 0)
         g_signal_state = "qn_core_wti_long";
      else if(g_signal_metrics.direction < 0)
         g_signal_state = "qn_core_wti_short";
      else
         g_signal_state = "four_core_boundary_flat";
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"completed_month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"session_count\":%d,\"close_count\":%d,\"return_count\":%d,\"distance_count\":%d,\"qn_index_zero_based\":%d,\"net_return\":%.12f,\"endpoint_return\":%.12f,\"endpoint_error\":%.12f,\"q_core\":%.12f,\"threshold\":%.12f,\"closes\":\"%s\",\"state\":\"%s\"}",
                            ...
                            g_signal_metrics.distance_count,
                            g_signal_metrics.qn_index_zero_based,
                            g_signal_metrics.net_return,
                            g_signal_metrics.endpoint_return,
                            g_signal_metrics.endpoint_error,
                            g_signal_metrics.q_core,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41275 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41275_WTI_MQNDISP_TR";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41275 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI monthly Qn-core trend holds through Fridays until the next broker month"))
     {
      QM_FrameworkShutdown();
      return INIT_FAILED;
     }
   g_support_contract_valid = Strategy_QnSupportValid();
   ...
   g_attempt_state_key =
      StringFormat("QM5_41275_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41275\",\"ea\":\"wti-mqndisp-tr\"}");
