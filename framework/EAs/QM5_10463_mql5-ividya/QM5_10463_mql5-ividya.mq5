...
input int             strategy_vidya_cmo_period = 9;
input int             strategy_vidya_ema_period = 12;
input ENUM_APPLIED_PRICE strategy_vidya_price  = PRICE_CLOSE;
input int             strategy_atr_period      = 14;
input double          strategy_atr_sl_mult     = 1.5;
input double          strategy_reward_r        = 2.0;

int g_vidya_handle = INVALID_HANDLE;
...
double Strategy_Close(const int shift)
  {
   return iClose(_Symbol, strategy_signal_tf, shift);
  }

double Strategy_VIDYA(const int shift)
  {
   ...
   double value[1];
   if(CopyBuffer(g_vidya_handle, 0, shift, 1, value) != 1) // perf-allowed: single closed-bar read; framework has no VIDYA wrapper.
      return 0.0;
   return value[0];
  }
...
int Strategy_Signal()
  {
   ...
   const double vidya_1 = Strategy_VIDYA(1);
   const double vidya_2 = Strategy_VIDYA(2);
   if(close_1 <= 0.0 || close_2 <= 0.0 || vidya_1 <= 0.0 || vidya_2 <= 0.0)
      return 0;

   if(close_2 <= vidya_2 && close_1 > vidya_1)
      return 1;
   if(close_2 >= vidya_2 && close_1 < vidya_1)
      return -1;
   return 0;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = Strategy_NormalizePrice((signal > 0) ? (entry - stop_distance)
                                                 : (entry + stop_distance));
   req.tp = Strategy_NormalizePrice((signal > 0) ? (entry + stop_distance * strategy_reward_r)
                                                 : (entry - stop_distance * strategy_reward_r));
   req.reason = (signal > 0) ? "MQL5_IVIDYA_LONG" : "MQL5_IVIDYA_SHORT";
...
void Strategy_ManageOpenPosition()
  {
   // Baseline card specifies fixed SL/TP and no trailing.
  }
