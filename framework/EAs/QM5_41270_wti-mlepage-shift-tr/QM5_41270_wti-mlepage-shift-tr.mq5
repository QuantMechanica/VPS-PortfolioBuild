...
#property description "QM5_41270 WTI Lepage Location-Scale Shift Trend"
...
//   - pool exact ranks and combine Wilcoxon location plus Ansari-Bradley scale
//   - require the fixed two-component Lepage statistic at its locked gate
...
input double strategy_w_mean                       = 637.5;
input double strategy_w_variance                   = 2656.25;
input double strategy_a_mean                       = 325.0;
input double strategy_a_variance                   = 663.26530612244898;
input double strategy_statistic_gate               = 1.3862943611198906;
...
struct Strategy_SignalMetrics
  {
   ...
   int return_count;
   int block_size;
   bool distinct_returns;
   bool qualifies;
   double wilcoxon_rank_sum;
   double ansari_bradley_score;
   double location_component;
   double scale_component;
   double statistic_l;
...
// -----------------------------------------------------------------------------
// Exact Lepage joint location-scale rank arithmetic.
// -----------------------------------------------------------------------------
...
bool Strategy_LepageSignal(const double &closes[],
                           Strategy_SignalMetrics &metrics)
  {
   ...
   if(ArrayResize(returns, strategy_return_count) !=
       ...
       ArrayResize(sorted_returns, strategy_return_count) !=
          strategy_return_count ||
       ArrayResize(recent_labels, strategy_return_count) !=
          strategy_return_count)
      return false;
   for(int index = 0; index < strategy_return_count; ++index)
     {
      ...
      recent_labels[index] = (index >= strategy_block_size) ? 1 : 0;
      }

   // Sort the pooled returns while carrying the recent-block label. Exact
   // ties are rejected because the locked card does not permit midranks.
   for(int index = 1; index < strategy_return_count; ++index)
     {
      const double key_value = sorted_returns[index];
      const int key_label = recent_labels[index];
      int cursor = index - 1;
      if(cursor >= ArraySize(sorted_returns))
         return false;
      if(cursor >= ArraySize(recent_labels))
         return false;
      while(cursor >= 0 && sorted_returns[cursor] > key_value)
        {
         sorted_returns[cursor + 1] = sorted_returns[cursor];
         recent_labels[cursor + 1] = recent_labels[cursor];
         --cursor;
        }
      sorted_returns[cursor + 1] = key_value;
      recent_labels[cursor + 1] = key_label;
     }
   for(int index = 1; index < strategy_return_count; ++index)
      if(sorted_returns[index - 1] >= sorted_returns[index])
         return false;
   metrics.distinct_returns = true;

   int recent_rank_count = 0;
   for(int index = 0; index < strategy_return_count; ++index)
     {
      if(recent_labels[index] != 1)
         continue;
      const int rank = index + 1;
      const int symmetric_score =
         (rank <= strategy_block_size) ? rank : 51 - rank;
      metrics.wilcoxon_rank_sum += (double)rank;
      metrics.ansari_bradley_score += (double)symmetric_score;
      ++recent_rank_count;
     }
   if(recent_rank_count != strategy_block_size ||
      !MathIsValidNumber(metrics.wilcoxon_rank_sum) ||
      !MathIsValidNumber(metrics.ansari_bradley_score))
      return false;

   const double location_delta =
      metrics.wilcoxon_rank_sum - strategy_w_mean;
   const double scale_delta =
      metrics.ansari_bradley_score - strategy_a_mean;
   metrics.location_component =
      location_delta * location_delta / strategy_w_variance;
   metrics.scale_component =
      scale_delta * scale_delta / strategy_a_variance;
   metrics.statistic_l =
      metrics.location_component + metrics.scale_component;
   if(!MathIsValidNumber(metrics.location_component) ||
      !MathIsValidNumber(metrics.scale_component) ||
      !MathIsValidNumber(metrics.statistic_l) ||
      metrics.location_component < 0.0 ||
      metrics.scale_component < 0.0 || metrics.statistic_l < 0.0)
      return false;
                                        ...
                                        int &direction)
  {
   ...
   const bool valid =
      Strategy_LoadPreMonthCloses(current_month_key,
                                  ...
                                  closes,
                                  close_times) &&
      Strategy_LepageSignal(closes, metrics);
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
   ...
   else
     {
      ...
      g_signal_valid =
         history_valid &&
         Strategy_LepageSignal(closes, g_signal_metrics);
      if(!g_signal_valid)
         g_signal_state = "lepage_validation_failed";
      else if(!g_signal_metrics.qualifies)
         g_signal_state = "lepage_below_gate_flat";
      else if(g_signal_metrics.direction == 0)
         g_signal_state = "lepage_neutral_return_flat";
      else if(g_signal_metrics.direction > 0)
         g_signal_state = "lepage_wti_long";
      else if(g_signal_metrics.direction < 0)
         g_signal_state = "lepage_wti_short";
      else
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"close_count\":%d,\"return_count\":%d,\"block_size\":%d,\"distinct_returns\":%s,\"wilcoxon_rank_sum\":%.15f,\"ansari_bradley_score\":%.15f,\"location_component\":%.15f,\"scale_component\":%.15f,\"statistic_l\":%.15f,\"qualifies\":%s,\"recent_return\":%.15f,\"oldest_close\":%.10f,\"newest_close\":%.10f,\"oldest_time\":%I64d,\"newest_time\":%I64d,\"state\":\"%s\"}",
                            ...
                            g_signal_metrics.distinct_returns
                                ? "true" : "false",
                            g_signal_metrics.wilcoxon_rank_sum,
                            g_signal_metrics.ansari_bradley_score,
                            g_signal_metrics.location_component,
                            g_signal_metrics.scale_component,
                            g_signal_metrics.statistic_l,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41270 ||
       qm_magic_slot_offset != 0 || qm_rng_seed != 42 ||
       QM_FrameworkMagic() != 412700000)
      return true;
   ...
   if(strategy_close_count != 51 ||
      strategy_return_count != 50 ||
      strategy_block_size != 25 ||
      MathAbs(strategy_w_mean - 637.5) > 0.000000000000001 ||
      MathAbs(strategy_w_variance - 2656.25) >
         0.000000000000001 ||
      MathAbs(strategy_a_mean - 325.0) > 0.000000000000001 ||
      MathAbs(strategy_a_variance - (32500.0 / 49.0)) >
         0.000000000000001 ||
      MathAbs(strategy_statistic_gate - 1.3862943611198906) >
         0.000000000000001 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41270_WTI_MLEPAGE_SHIFT_TR";
   ...
   req.reason = (g_signal_metrics.direction > 0)
       ? "WTI_MLEPAGE_SHIFT_TR_LONG"
       : "WTI_MLEPAGE_SHIFT_TR_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41270 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI Lepage location-scale shift trend holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41270_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41270\",\"ea\":\"wti-mlepage-shift-tr\"}");
