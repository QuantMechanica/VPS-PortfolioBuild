...
#property description "QM5_13003 Edge Lab USDCAD NZDUSD Cointegration"
...
input double strategy_beta              = -0.423070732289;
...
bool Strategy_IsHostSymbol()
  {
   return (_Symbol == g_leg_usdcad || _Symbol == g_leg_nzdusd);
  }
...
bool Strategy_EnsureBasketScope()
  {
   ...
   string allowed[2] = {"USDCAD.DWX", "NZDUSD.DWX"};
   for(int i = 0; i < 2; ++i)
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(!Strategy_EnsureBasketScope())
      return false;
   if(!QM_SymbolAssertOrLog(g_leg_usdcad) || !QM_SymbolAssertOrLog(g_leg_nzdusd))
      return false;
   ...
   for(int i = 0; i < lookback; ++i)
     {
      if(usdcad[i] <= 0.0 || nzdusd[i] <= 0.0)
         return false;
      spreads[i] = MathLog(usdcad[i]) - strategy_beta * MathLog(nzdusd[i]);
...
bool Strategy_OpenPair(const int spread_direction)
  {
   ...
   const double weight_sum = usdcad_weight + nzdusd_weight;
   if(weight_sum <= 0.0)
      return false;

   const bool long_spread = (spread_direction > 0);
   const QM_OrderType usdcad_type = long_spread ? QM_BUY : QM_SELL;
   const QM_OrderType nzdusd_type = long_spread
                                    ? (strategy_beta >= 0.0 ? QM_SELL : QM_BUY)
                                    : (strategy_beta >= 0.0 ? QM_BUY : QM_SELL);
   const string reason = long_spread ? "QM5_13003_LONG_SPREAD_Z_LT_NEG_ENTRY"
                                     : "QM5_13003_SHORT_SPREAD_Z_GT_POS_ENTRY";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_13003_PAIR_HOST";
