...
#property description "QM5_12705 XNG Fall Storage Short"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_12705 - XNG Fall Storage Short
// -----------------------------------------------------------------------------
// D1 structural natural-gas sleeve:
//   - short-only in the September-October fall shoulder/storage window
//   - weekly entry gate, with D1 SMA confirmation
//   - exits on season end, SMA recovery, max hold, Friday close, or ATR stop
...
bool Strategy_IsFallStorageMonth(const int month)
  {
   return (month == 9 || month == 10);
  }

bool Strategy_IsFirstBarOfWeek()
  {
...
void Strategy_CloseOpenPositionsIfNeeded()
  {
   ...
   const bool in_fall_storage = Strategy_IsFallStorageMonth(month);
   const int magic = QM_FrameworkMagic();
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12705_XNG_FALL_STORAGE";
   req.symbol_slot = qm_magic_slot_offset;
   req.expiration_seconds = 0;

   Strategy_CloseOpenPositionsIfNeeded();

   if(!Strategy_IsFirstBarOfWeek())
      return false;

   const datetime current_bar = iTime(_Symbol, PERIOD_D1, 0); // perf-allowed: weekly rebalance key.
   const int current_week_key = Strategy_WeekKey(current_bar);
   ...
   const int month = Strategy_MonthFromTime(current_bar);
   if(!Strategy_IsFallStorageMonth(month))
      return false;
   ...
   req.reason = "XNG_FALL_STORAGE_SHORT";
   return true;
  }
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_12705\",\"ea\":\"xngusd-fall-storage-short\"}");
