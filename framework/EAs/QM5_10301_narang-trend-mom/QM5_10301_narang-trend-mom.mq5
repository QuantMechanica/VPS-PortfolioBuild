...
#property description "QM5_10301 Narang Price Trend Momentum"
...
input double          strategy_spread_atr_ratio  = 0.020;
...
bool Strategy_NoTradeFilter()
  {
   if(strategy_max_spread_points <= 0.0 && strategy_spread_atr_ratio <= 0.0)
      return false;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(strategy_entry_channel < 1 || strategy_sma_period < 1 ||
   ...
   const int breakout = QM_Sig_Range_Breakout(_Symbol, strategy_timeframe, strategy_entry_channel, 1);
   if(breakout == 0)
      return false;

   const double close_1 = QM_SMA(_Symbol, strategy_timeframe, 1, 1, PRICE_CLOSE);
   const double sma_200 = QM_SMA(_Symbol, strategy_timeframe, strategy_sma_period, 1, PRICE_CLOSE);
   const double atr = QM_ATR(_Symbol, strategy_timeframe, strategy_atr_period, 1);
   const double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
   const double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   const double point = SymbolInfoDouble(_Symbol, SYMBOL_POINT);
   ...
   const bool long_signal = (breakout > 0 && close_1 > sma_200);
   const bool short_signal = (breakout < 0 && close_1 < sma_200);
   ...
   req.type = side;
   req.sl = sl;
   req.reason = long_signal ? "NARANG_TREND_MOM_LONG_BREAKOUT"
                            : "NARANG_TREND_MOM_SHORT_BREAKOUT";
   return true;
  }
...
void Strategy_ManageOpenPosition()
  {
   ...
   if(magic <= 0 || strategy_atr_period < 1 || strategy_atr_stop_mult <= 0.0)
      return;
...
bool Strategy_ExitSignal()
  {
   const int magic = QM_FrameworkMagic();
   if(magic <= 0 || strategy_exit_channel < 1)
      return false;

   const int reversal = QM_Sig_Range_Breakout(_Symbol, strategy_timeframe, strategy_exit_channel, 1);
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      if(pos_type == POSITION_TYPE_BUY && reversal < 0)
         return true;
      if(pos_type == POSITION_TYPE_SELL && reversal > 0)
         return true;
