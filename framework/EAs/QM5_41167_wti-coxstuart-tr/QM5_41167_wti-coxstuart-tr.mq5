...
#property description "QM5_41167 WTI Monthly Cox-Stuart Paired-Sign Trend"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41167 - WTI Monthly Cox-Stuart Paired-Sign Trend
// -----------------------------------------------------------------------------
// D1 structural crude-oil sleeve:
//   - reconstruct fourteen consecutive completed broker-month-end closes
//   - compare seven disjoint ordered half-sample log-price pairs
//   - follow WTI only when at least five of seven strict signs agree
...
input int    strategy_endpoint_count       = 14;
input int    strategy_pair_count           = 7;
input int    strategy_signs_required       = 5;
...
void Strategy_ResetDecisionState()
  {
   ...
   ArrayInitialize(g_pair_signs, 0);
...
bool Strategy_LoadMonthlyEndpoints(const int current_month_key,
                                   ...
                                   datetime &endpoint_times[])
  {
   ...
   ArraySetAsSeries(bars, true);
   const int copied =
      CopyRates(_Symbol, // perf-allowed: one bounded fourteen-month scan behind a consumed monthly attempt or monthly restart validation.
                PERIOD_D1,
                1,
                strategy_history_bars_d1,
                bars);
   ...
   if(endpoint_count != strategy_endpoint_count || endpoint_count != 14)
      return false;
...
bool Strategy_CoxStuartSignal(const double &closes[],
                               Strategy_SignalMetrics &metrics)
  {
   ...
   if(strategy_endpoint_count != 14 ||
      strategy_pair_count != 7 ||
      strategy_signs_required != 5 ||
      metrics.endpoint_count != strategy_endpoint_count ||
      strategy_signs_required * 2 <= strategy_pair_count)
      return false;

   double log_prices[];
   if(ArrayResize(log_prices, strategy_endpoint_count) !=
      strategy_endpoint_count)
      return false;
   ...
   for(int pair = 0; pair < strategy_pair_count; ++pair)
     {
      const int older = pair;
      ...
      if(!MathIsValidNumber(difference))
         return false;
      const int sign = Strategy_StrictSign(difference);
      g_pair_signs[pair] = sign;
      ++metrics.pair_count;
      if(sign > 0)
   ...
   if(metrics.pair_count != 7 ||
      metrics.positive_count + metrics.negative_count +
         metrics.tie_count != metrics.pair_count)
      return false;

   if(metrics.tie_count > 0)
      metrics.direction = 0;
   else if(metrics.positive_count >= strategy_signs_required)
      metrics.direction = 1;
   else if(metrics.negative_count >= strategy_signs_required)
      metrics.direction = -1;
   else
      metrics.direction = 0;
   return true;
                                        ...
                                        int &direction)
  {
   ...
   const bool valid =
      Strategy_LoadMonthlyEndpoints(current_month_key,
                                    ...
                                    closes,
                                    endpoint_times) &&
      Strategy_CoxStuartSignal(closes, metrics);
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
   ...
   else
     {
      ...
      g_signal_valid =
         endpoints_valid &&
         Strategy_CoxStuartSignal(closes, g_signal_metrics);
      if(!g_signal_valid)
         g_signal_state = "cox_stuart_validation_failed";
      else if(g_signal_metrics.tie_count > 0)
         g_signal_state = "paired_sign_tie_consumed_flat";
      else if(g_signal_metrics.direction == 0)
         g_signal_state = "paired_sign_four_three_flat";
      else if(g_signal_metrics.direction > 0)
         g_signal_state = "cox_stuart_wti_long";
      else if(g_signal_metrics.direction < 0)
         g_signal_state = "cox_stuart_wti_short";
      else
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"endpoint_count\":%d,\"pair_count\":%d,\"positive_count\":%d,\"negative_count\":%d,\"tie_count\":%d,\"pair_signs\":[%d,%d,%d,%d,%d,%d,%d],\"oldest_endpoint\":%I64d,\"newest_endpoint\":%I64d,\"state\":\"%s\"}",
                            ...
                            g_signal_metrics.pair_count,
                            g_signal_metrics.positive_count,
                            g_signal_metrics.negative_count,
                            g_signal_metrics.tie_count,
                            g_pair_signs[0],
                            g_pair_signs[1],
                            g_pair_signs[2],
                            g_pair_signs[3],
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41167 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41167_WTI_COX_STUART";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41167 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI Cox-Stuart paired-sign trend holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41167_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41167\",\"ea\":\"wti-coxstuart-tr\"}");
