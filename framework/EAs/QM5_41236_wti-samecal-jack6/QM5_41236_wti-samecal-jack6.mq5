...
#property description "QM5_41236 WTI Same-Calendar Delete-One Sign Stability"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41236 - WTI Same-Calendar Delete-One Sign Stability
// -----------------------------------------------------------------------------
// Structural D1 crude-oil sleeve:
//   - decide only at a genuine normalized broker-month boundary
//   - reconstruct the upcoming calendar month's completed return in Y-6..Y-1
//   - require all six exact years without substitution
//   - omit each observation once and compute all six five-year means
//   - follow only when every delete-one mean has the same strict epsilon sign
...
input int    strategy_history_years       = 6;
input int    strategy_history_bars_d1     = 3000;
input int    strategy_delete_count        = 1;
input int    strategy_subset_size         = 5;
...
double   g_signal_min_mean        = 0.0;
double   g_signal_max_mean        = 0.0;
...
void Strategy_ResetDecisionState()
  {
   ...
   g_signal_min_mean = 0.0;
   g_signal_max_mean = 0.0;
   g_signal_state = "idle";
  }
...
// -----------------------------------------------------------------------------
// Exact completed-month endpoint and six-sample delete-one helpers.
// -----------------------------------------------------------------------------
...
bool Strategy_DeleteOneSignal(const double &observations[],
                              ...
                              int &direction)
  {
      ...
      for(int index = 0; index < sample_count; ++index)
        {
         ...
         ++subset_members;
         if(!MathIsValidNumber(subset_sum))
            return false;
        }

      if(subset_members != strategy_subset_size)
         return false;

      const double subset_mean =
         subset_sum / (double)strategy_subset_size;
      if(!MathIsValidNumber(subset_mean))
         return false;

      if(first_mean)
        {
         minimum_mean = subset_mean;
         maximum_mean = subset_mean;
         first_mean = false;
        }
      else
        {
         minimum_mean = MathMin(minimum_mean, subset_mean);
         maximum_mean = MathMax(maximum_mean, subset_mean);
        }
   ...
   if(first_mean || mean_count != sample_count ||
      !MathIsValidNumber(minimum_mean) ||
      !MathIsValidNumber(maximum_mean) ||
      minimum_mean > maximum_mean ||
...
bool Strategy_LoadDeleteOneSignal(
   ...
   int &direction)
  {
   ...
   // Exact year membership is load-bearing. The return vector remains in
   // chronological year order; every index is then omitted exactly once.
   ...
   return Strategy_DeleteOneSignal(observations,
                                   sample_count,
                                   minimum_mean,
                                   maximum_mean,
                                   direction);
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   // Consume before history, delete-one arithmetic, news, spread, quote,
   ...
   else
     {
      g_signal_valid =
          Strategy_LoadDeleteOneSignal(
            ...
            g_signal_direction);
      if(!g_signal_valid)
         g_signal_state = "endpoint_sample_or_delete_one_failed";
      else if(g_signal_direction > 0)
         g_signal_state = "all_delete_one_means_long";
      else if(g_signal_direction < 0)
         g_signal_state = "all_delete_one_means_short";
      else
         g_signal_state = "delete_one_sign_disagreement_or_epsilon_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"valid\":%s,\"samples\":%d,\"min_delete_one_mean\":%.12g,\"max_delete_one_mean\":%.12g,\"direction\":%d,\"state\":\"%s\"}",
                            g_decision_month_key,
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(!QM_InputRequireLong("qm_ea_id", qm_ea_id, 41236) ||
      ...
      !QM_InputRequireLong("strategy_history_years", strategy_history_years, 6) ||
      !QM_InputRequireLong("strategy_history_bars_d1", strategy_history_bars_d1, 3000) ||
      !QM_InputRequireLong("strategy_delete_count", strategy_delete_count, 1) ||
      !QM_InputRequireLong("strategy_subset_size", strategy_subset_size, 5) ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(g_signal_direction > 0)
     {
      req.type = QM_BUY;
      req.reason = "WTI_SAMECAL_JACK6_LONG";
     }
   else
     {
      req.type = QM_SELL;
      req.reason = "WTI_SAMECAL_JACK6_SHORT";
     }
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41236 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI same-calendar delete-one sign-stability card holds to the next broker month"))
      return INIT_FAILED;
   ...
   g_attempt_state_key =
      StringFormat("QM5_41236_SAMECAL_JACK6_MONTH_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41236\",\"ea\":\"wti-samecal-jack6\"}");
