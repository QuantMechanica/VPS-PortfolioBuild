...
#property description "QM5_11024 MQL5 MA Bollinger Bounce"
...
input int    strategy_d1_sma_period        = 64;
input int    strategy_bb_period            = 20;
input double strategy_bb_deviation         = 2.0;
input int    strategy_atr_period           = 20;
input double strategy_min_band_atr_mult    = 0.5;
...
bool Strategy_NoTradeFilter()
  {
   if(strategy_d1_sma_period <= 0 ||
      strategy_bb_period <= 0 ||
      strategy_bb_deviation <= 0.0 ||
      strategy_atr_period <= 0 ||
      strategy_min_band_atr_mult < 0.0 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.reason = (signal > 0) ? "MQL5_MA_BOLL_LONG" : "MQL5_MA_BOLL_SHORT";
   return true;
  }
      ...
      if(pos_type == POSITION_TYPE_BUY)
        {
         ...
         if(bid - open_price >= trail_distance && (current_sl <= 0.0 || new_sl > current_sl))
            QM_TM_MoveSL(ticket, new_sl, "MQL5_MA_BOLL_POINT_TRAIL");
        }
