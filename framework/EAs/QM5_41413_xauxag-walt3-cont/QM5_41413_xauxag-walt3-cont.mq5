...
#property description "QM5_41413 XAU/XAG Weekly Alternation Continuation"
...
// returns and trade only strict +,-,+ or -,+,- sign alternation. Fade the
// newest relative winner with an equal-notional opposite-leg package. Consume
// one attempt per week and flatten at the next week boundary.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41413;
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41413 && qm_magic_slot_offset == 0 &&
            g_leg_xau != "" && g_leg_xag != "" &&
            g_leg_xau != g_leg_xag &&
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41413_XAUXAG_WALT3_CONT";
...
bool Strategy_LoadAlternation(const int current_week,
                              int &direction)
  {
   direction = 0;
   ...
   if(g_relative_return[0] > 0.0 &&
      ...
      direction = 1; // Newest XAU winner: buy XAU, sell XAG.
   else if(g_relative_return[0] < 0.0 &&
           g_relative_return[1] > 0.0 &&
           g_relative_return[2] < 0.0)
      direction = -1; // Newest XAG winner: sell XAU, buy XAG.
   return true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41413_XAUXAG_WALT3_CONT_HOST";
   ...
   if(Strategy_OpenOwnedPositionCount() > 0 ||
     ...
     {
      signal_valid = Strategy_LoadAlternation(g_signal_week_key, direction);
      if(!signal_valid)
         ...
         g_signal_state = "no_strict_alternation_flat";
     }
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG weekly strict alternation continuation package exits at the next broker week"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41413\",\"ea\":\"xauxag-walt3-cont\"}");
