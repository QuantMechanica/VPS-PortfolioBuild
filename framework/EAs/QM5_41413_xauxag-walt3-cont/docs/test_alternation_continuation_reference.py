"""Deterministic reference checks for QM5_41413 weekly alternation continuation."""
...
MQ5 = EA_DIR / "QM5_41413_xauxag-walt3-cont.mq5"
SETFILE = (
    EA_DIR
    / "sets"
    / "QM5_41413_xauxag-walt3-cont_QM5_41413_XAU_XAG_WALT3_CONT_D1_D1_backtest.set"
)
...
class AlternationReferenceTest(unittest.TestCase):
    def test_positive_negative_positive_follows_xau(self) -> None:
        self.assertEqual(alternation_signal([0.03, -0.02, 0.01]), 1)

    def test_negative_positive_negative_follows_xag(self) -> None:
        self.assertEqual(alternation_signal([-0.03, 0.02, -0.01]), -1)
    ...
    def test_static_artifacts_lock_identity_signal_and_fixed_risk(self) -> None:
        ...
        self.assertIn("qm_ea_id                    = 41413", source)
        ...
        self.assertRegex(resolver, re.compile(r"414130000.*414130001", re.DOTALL))
