...
#property description "QM5_10480 MQL5 Three Indicators MACD Stochastic RSI"
...
int Strategy_ConfirmedSignal()
  {
   if(strategy_macd_fast <= 0 || strategy_macd_slow <= strategy_macd_fast ||
      strategy_macd_signal <= 0 || strategy_stoch_k <= 0 ||
   ...
   const double macd_main = QM_MACD_Main(_Symbol, strategy_work_tf,
                                         strategy_macd_fast,
                                         strategy_macd_slow,
                                         strategy_macd_signal,
                                         1);
   const double macd_signal = QM_MACD_Signal(_Symbol, strategy_work_tf,
                                             strategy_macd_fast,
                                             strategy_macd_slow,
                                             strategy_macd_signal,
                                             1);
   const double stoch_k = QM_Stoch_K(_Symbol, strategy_work_tf,
                                     strategy_stoch_k,
                                     strategy_stoch_d,
                                     strategy_stoch_slowing,
                                     1);
   const double stoch_d = QM_Stoch_D(_Symbol, strategy_work_tf,
                                     strategy_stoch_k,
                                     strategy_stoch_d,
                                     strategy_stoch_slowing,
                                     1);
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   const int signal = Strategy_ConfirmedSignal();
   if(signal == 0)
      return false;
   ...
   req.sl = sl;
   req.tp = tp;
   req.reason = (signal > 0) ? "TRI_IND_LONG" : "TRI_IND_SHORT";
   return true;
  }
