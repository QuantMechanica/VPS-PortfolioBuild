...
#property description "QM5_41123 XAU/XAG Completed-Month Path-Efficiency Reversion"
...
// -----------------------------------------------------------------------------
// At the first synchronized D1 boundary of a broker month, reconstruct every
// ratio close in the immediately completed 17-to-23-session calendar month.
// Sum every chronological log-ratio return into signed net displacement N and
// absolute path P. When abs(N)/P is at least 0.20, fade N with one opposite,
// equal-notional package held to the next month. Exact-zero constituent returns
// remain valid. Runtime is native, completed-price-only, deterministic, and
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41123 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
            ...
            MathAbs(strategy_efficiency_threshold - 0.20) <= 1.0e-12 &&
            MathAbs(strategy_efficiency_tolerance - 1.0e-10) <= 1.0e-18 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_MPATH_EFF_RV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41123_XAUXAG_MPATH_EFF_RV";
...
bool Strategy_LoadMonthlyPathEfficiency(
      ...
      int &direction)
  {
   ...
   if(ArrayResize(series_ratios, strategy_max_month_sessions) !=
      ...
      ArrayResize(chronological_ratios, strategy_max_month_sessions) !=
      strategy_max_month_sessions)
      return false;
   ...
   // This older synchronized pair proves that the completed-month package was
   // not truncated at the history edge and belongs to the adjacent month.
   ...
   // CopyRates series order is newest to oldest. Reverse the complete month
   // before summing each adjacent log-ratio return exactly once.
   ...
   return_count = completed_month_sessions - 1;
   if(return_count < 1)
      return false;
   ...
   for(int i = 1; i < completed_month_sessions; ++i)
     {
      ...
      // Exact-zero constituent returns are valid and contribute zero to both
      // sums. Accumulate N and P from the same bounded loop.
   ...
   const double endpoint_net =
      chronological_ratios[completed_month_sessions - 1] -
      chronological_ratios[0];
   const double consistency_scale = MathMax(1.0, absolute_path);
   if(!MathIsValidNumber(endpoint_net) ||
      MathAbs(net_displacement - endpoint_net) >
         strategy_efficiency_tolerance * consistency_scale)
      return false;

   // A zero path is explicitly flat. P must be positive before division.
   if(absolute_path <= 0.0)
      return false;

   efficiency = MathAbs(net_displacement) / absolute_path;
   if(!MathIsValidNumber(efficiency) ||
      efficiency < 0.0 ||
      efficiency > 1.0 + strategy_efficiency_tolerance)
      return false;
   if(efficiency > 1.0)
      efficiency = 1.0;

   // direction > 0 means BUY XAU / SELL XAG. Threshold equality qualifies;
   // the completed-month signed net alone determines the contrarian side.
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41123_XAUXAG_MPATH_EFF_RV_HOST";
   ...
   const bool valid =
      Strategy_LoadMonthlyPathEfficiency(g_signal_month_key,
                                         completed_month_sessions,
                                         return_count,
                                         net_displacement,
                                         absolute_path,
                                         efficiency,
                                         direction);
   string state = "monthly_path_efficiency_failed";
   if(valid && direction > 0)
      state = "efficient_negative_net_fade_long_xau";
   else if(valid && direction < 0)
      state = "efficient_positive_net_fade_short_xau";
   else if(return_count > 0 && absolute_path == 0.0)
      state = "zero_path_flat";
   else if(valid && net_displacement == 0.0)
      state = "net_zero_flat";
   else if(valid)
      state = "below_efficiency_threshold_flat";

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"completed_month_sessions\":%d,\"return_count\":%d,\"net_displacement\":%.12e,\"absolute_path\":%.12e,\"efficiency\":%.12e,\"threshold\":%.12e,\"direction\":%d,\"state\":\"%s\"}",
                            g_signal_month_key,
                            completed_month_sessions,
                            return_count,
                            net_displacement,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG completed-month path-efficiency reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41123\",\"ea\":\"xauxag-mpath-eff-rv\"}");
