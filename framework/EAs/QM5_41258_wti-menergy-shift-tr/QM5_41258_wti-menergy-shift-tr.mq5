...
#property description "QM5_41258 WTI Exact-Permutation Energy-Distance Shift Trend"
...
//   - measure the actual-magnitude energy distance between the two blocks
//   - enumerate every one of the 924 fixed-size six/six label assignments
//   - follow the recent-block median only inside the locked three-fifths tail
...
input int    strategy_month_returns        = 12;
input int    strategy_block_size            = 6;
input int    strategy_assignment_count      = 924;
input int    strategy_tail_numerator        = 3;
input int    strategy_tail_denominator      = 5;
input int    strategy_tail_count_max        = 554;
input double strategy_energy_epsilon        = 0.000000000001;
...
struct Strategy_SignalMetrics
  {
   ...
   int tail_count;
   double observed_energy;
   double comparison_epsilon;
   double mean_cross;
   double mean_old;
   double mean_recent;
   double median_old;
...
void Strategy_DetectDecisionClock_OnNewBar()
  {
   ...
   if(current_month_key <= 0 ||
      current_month_key != Strategy_MonthKeyForTime(normalized_current) ||
...
bool Strategy_LoadMonthlyEndpoints(const int current_month_key,
                                   ...
                                   datetime &endpoint_times[])
  {
   ...
   if(current_month_key <= 0 ||
      strategy_month_returns != 12 || endpoint_target != 13 ||
...
bool Strategy_EnergyForMask(const double &values[],
                            const int recent_mask,
                            double &energy,
                            double &mean_cross,
                            double &mean_old,
                            double &mean_recent)
  {
   ...
   mean_old = 0.0;
   mean_recent = 0.0;
   if(ArraySize(values) != 12 ||
      recent_mask < 0 || recent_mask >= 4096 ||
      Strategy_Popcount12(recent_mask) != strategy_block_size)
      return false;
   ...
   for(int index = 0; index < ArraySize(values); ++index)
     {
      if(!MathIsValidNumber(values[index]))
         return false;
      if((recent_mask & (1 << index)) != 0)
        {
         if(recent_count < 0 || recent_count >= 6)
            return false;
         recent_values[recent_count] = values[index];
         ++recent_count;
        }
      else
        {
         if(old_count < 0 || old_count >= 6)
            return false;
         old_values[old_count] = values[index];
         ++old_count;
        }
     }
   if(old_count != strategy_block_size ||
      recent_count != strategy_block_size)
      return false;
   ...
   for(int old_index = 0; old_index < 6; ++old_index)
     {
      for(int recent_index = 0; recent_index < 6; ++recent_index)
         cross_sum +=
            MathAbs(old_values[old_index] - recent_values[recent_index]);
      for(int peer_index = 0; peer_index < 6; ++peer_index)
        {
         // Ordered-pair means deliberately include the six zero self-distances.
         old_sum +=
            MathAbs(old_values[old_index] - old_values[peer_index]);
         recent_sum +=
            MathAbs(recent_values[old_index] -
                    recent_values[peer_index]);
        }
     }

   const double pair_count =
      (double)strategy_block_size * (double)strategy_block_size;
   if(pair_count != 36.0)
      return false;
   mean_cross = cross_sum / pair_count;
   mean_old = old_sum / pair_count;
   mean_recent = recent_sum / pair_count;
   energy = 3.0 *
            (2.0 * mean_cross - mean_old - mean_recent);
   return (MathIsValidNumber(mean_cross) &&
           MathIsValidNumber(mean_old) &&
           MathIsValidNumber(mean_recent) &&
           MathIsValidNumber(energy));
  }

bool Strategy_EnergyDistanceSignal(const double &closes[],
                                   Strategy_SignalMetrics &metrics)
  {
   ...
   if(strategy_month_returns != 12 ||
      ...
      MathAbs(strategy_energy_epsilon - 0.000000000001) >
   ...
   for(int index = 0; index < strategy_month_returns; ++index)
     {
      ...
      if(index < strategy_block_size)
      ...
      else
        {
         ...
         if(recent_index < 0 || recent_index >= 6)
            return false;
   ...
   if(!Strategy_MedianSix(old_values, metrics.median_old) ||
      !Strategy_MedianSix(recent_values, metrics.median_recent) ||
      StringLen(metrics.return_path) <= 0)
      return false;

   metrics.median_delta = metrics.median_recent - metrics.median_old;
   metrics.oldest_close = closes[0];
   metrics.newest_close = closes[metrics.endpoint_count - 1];
   if(!MathIsValidNumber(metrics.median_delta) ||
      !MathIsValidNumber(metrics.oldest_close) ||
   ...
   for(int index = strategy_block_size;
       ...
       ++index)
      observed_mask |= (1 << index);

   if(Strategy_Popcount12(observed_mask) != strategy_block_size ||
      !Strategy_EnergyForMask(returns,
                              observed_mask,
                              metrics.observed_energy,
                              metrics.mean_cross,
                              metrics.mean_old,
                              metrics.mean_recent))
      return false;

   metrics.comparison_epsilon =
      strategy_energy_epsilon *
      MathMax(1.0, MathAbs(metrics.observed_energy));
   if(metrics.comparison_epsilon <= 0.0 ||
      !MathIsValidNumber(metrics.comparison_epsilon))
      return false;
   ...
   for(int mask = 0; mask < 4096; ++mask)
     {
      if(Strategy_Popcount12(mask) != strategy_block_size)
         continue;
      double perm_energy = 0.0;
      double perm_cross = 0.0;
      double perm_old = 0.0;
      double perm_recent = 0.0;
      if(!Strategy_EnergyForMask(returns,
                                 mask,
                                 perm_energy,
                                 perm_cross,
                                 perm_old,
                                 perm_recent))
         return false;
      if(perm_energy + metrics.comparison_epsilon >=
         metrics.observed_energy)
         ++metrics.tail_count;
   ...
   if(metrics.assignment_count != strategy_assignment_count ||
      metrics.tail_count < 1 || metrics.tail_count > 924)
      return false;
   ...
   const bool count_qualified =
      (metrics.tail_count <= strategy_tail_count_max);
   const bool fraction_qualified =
      (strategy_tail_denominator * metrics.tail_count <=
       strategy_tail_numerator * metrics.assignment_count);

   if(count_qualified && fraction_qualified)
     {
      if(metrics.median_delta > strategy_direction_epsilon)
         metrics.direction = 1;
      else if(metrics.median_delta < -strategy_direction_epsilon)
         metrics.direction = -1;
     }
   return true;
  }
                                        ...
                                        int &direction)
  {
   ...
   const bool valid =
      Strategy_LoadMonthlyEndpoints(current_month_key,
                                    ...
                                    closes,
                                    endpoint_times) &&
      Strategy_EnergyDistanceSignal(closes, metrics);
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
   ...
   else
     {
      ...
      g_signal_valid =
         endpoints_valid &&
         Strategy_EnergyDistanceSignal(closes, g_signal_metrics);
      if(!g_signal_valid)
         g_signal_state = "energy_distance_validation_failed";
      else if(g_signal_metrics.direction == 0)
         g_signal_state = "energy_distance_unqualified_or_neutral_flat";
      else if(g_signal_metrics.direction > 0)
         g_signal_state = "energy_distance_shift_wti_long";
      else if(g_signal_metrics.direction < 0)
         g_signal_state = "energy_distance_shift_wti_short";
      else
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"endpoint_count\":%d,\"return_count\":%d,\"block_size\":%d,\"assignment_count\":%d,\"tail_count\":%d,\"observed_energy\":%.12f,\"comparison_epsilon\":%.16f,\"mean_cross\":%.12f,\"mean_old\":%.12f,\"mean_recent\":%.12f,\"median_old\":%.12f,\"median_recent\":%.12f,\"median_delta\":%.12f,\"tail_fraction\":%.12f,\"returns\":\"%s\",\"oldest_close\":%.10f,\"newest_close\":%.10f,\"oldest_endpoint\":%I64d,\"newest_endpoint\":%I64d,\"state\":\"%s\"}",
                            ...
                            g_signal_metrics.observed_energy,
                            g_signal_metrics.comparison_epsilon,
                            g_signal_metrics.mean_cross,
                            g_signal_metrics.mean_old,
                            g_signal_metrics.mean_recent,
                            g_signal_metrics.median_old,
                            g_signal_metrics.median_recent,
                            g_signal_metrics.median_delta,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41258 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41258_WTI_MENERGY_SHIFT";
   ...
   req.reason = (g_signal_metrics.direction > 0)
      ? "WTI_MENERGY_SHIFT_LONG"
      : "WTI_MENERGY_SHIFT_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41258 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI energy-distance distribution-shift trend holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41258_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41258\",\"ea\":\"wti-menergy-shift-tr\"}");
