...
#property description "QM5_41468 WTI Winter Lower-CLV Continuation"
...
//   - decide only in November through May during the positive WTI winter leg
//   - aggregate the immediately completed 3-to-5-session normalized week
//   - short only when that week closes in the lower third of its full range
...
input int    strategy_history_bars         = 20;
...
input double strategy_clv_cutoff           = 0.333333333333;
...
int      g_completed_week_bars    = 0;
double   g_completed_week_range   = 0.0;
double   g_completed_week_high    = 0.0;
double   g_completed_week_low     = 0.0;
double   g_completed_week_close   = 0.0;
double   g_completed_week_clv     = 0.0;
...
void Strategy_ResetDecisionState()
  {
   ...
   g_completed_week_bars = 0;
   g_completed_week_range = 0.0;
   g_completed_week_high = 0.0;
   g_completed_week_low = 0.0;
   g_completed_week_close = 0.0;
   g_completed_week_clv = 0.0;
   g_signal_state = "idle";
  }
...
bool Strategy_LoadLowerCLVSignal(const int current_week_key,
                                 const int label_offset,
                                 int &direction)
  {
   direction = 0;
   ...
   for(int index = 0; index < copied; ++index)
     {
      ...
      ++week_count;
      if(week_count > strategy_max_week_bars)
         return false;
     }

   if(completed_week_key <= 0 || !older_boundary_seen ||
      week_count < strategy_min_week_bars ||
      week_count > strategy_max_week_bars ||
      week_high <= 0.0 || week_low <= 0.0 || week_close <= 0.0 ||
      week_high < week_low || week_high < week_close || week_low > week_close ||
      !MathIsValidNumber(week_high) ||
      !MathIsValidNumber(week_low) ||
      !MathIsValidNumber(week_close))
      return false;

   const double full_range = week_high - week_low;
   if(full_range <= 0.0 || !MathIsValidNumber(full_range))
      return false;

   const double clv = (week_close - week_low) / full_range;
   if(!MathIsValidNumber(clv) || clv < 0.0 || clv > 1.0)
      return false;

   g_completed_week_bars = week_count;
   g_completed_week_range = full_range;
   g_completed_week_high = week_high;
   g_completed_week_low = week_low;
   g_completed_week_close = week_close;
   g_completed_week_clv = clv;
   if(clv < strategy_clv_cutoff)
      direction = -1;
   return true;
  }
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_WeekAlreadyEntered(g_decision_week_key))
     ...
     {
       g_signal_valid =
          Strategy_LoadLowerCLVSignal(g_decision_week_key,
                                      g_decision_label_offset,
                                      g_signal_direction);
       if(!g_signal_valid)
          g_signal_state = "completed_week_validation_failed";
       else if(g_signal_direction < 0)
          g_signal_state = "lower_tercile_continuation_short";
       else
          g_signal_state = "completed_week_not_lower_tercile_flat";
      }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"week\":%d,\"anchor_month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_week_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"completed_week_bars\":%d,\"completed_week_range\":%.8f,\"completed_week_high\":%.8f,\"completed_week_low\":%.8f,\"completed_week_close\":%.8f,\"completed_week_clv\":%.12f,\"state\":\"%s\"}",
                            g_decision_week_key,
                            anchor_month,
                            ...
                            g_signal_direction,
                            g_completed_week_bars,
                            g_completed_week_range,
                            g_completed_week_high,
                            g_completed_week_low,
                            g_completed_week_close,
                            g_completed_week_clv,
                            g_signal_state));
  }
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(strategy_symbol == "" ||
      ...
      !MathIsValidNumber(strategy_clv_cutoff) ||
      MathAbs(strategy_clv_cutoff - 0.333333333333) > 1.0e-12 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41468_WTI_WINTER_LCLV_CONT";
   ...
   req.type = QM_SELL;
   req.reason = "WTI_WINTER_LCLV_CONT_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(strategy_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41468 ||
      qm_magic_slot_offset != 0)
   ...
   g_attempt_state_key =
      StringFormat("QM5_41468_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41468\",\"ea\":\"wti-winter-lclv-cont\"}");
