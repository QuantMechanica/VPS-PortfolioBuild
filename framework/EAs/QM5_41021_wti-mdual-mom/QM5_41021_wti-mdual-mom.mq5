...
#property description "QM5_41021 WTI Month-Boundary Dual-Horizon Momentum"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41021 - WTI Month-Boundary Dual-Horizon Momentum
// -----------------------------------------------------------------------------
// D1 structural crude-oil sleeve:
//   - reconstruct the immediately completed broker-month return
//   - reconstruct that same month's final five close-to-close intervals
//   - enter at the next month boundary only when both signs strictly agree
...
bool Strategy_LoadDualHorizonSignal(const int current_month_key,
                                    const int label_offset,
                                    int &direction,
                                    double &month_return,
                                    double &closing_return)
  {
   ...
   const int copied = CopyRates(_Symbol, // perf-allowed: bounded endpoint scan behind the sole QM_IsNewBar branch.
                                ...
                                strategy_history_bars,
                                bars);
   if(copied < strategy_closing_intervals + 2)
      return false;
   ...
   const int required_prior_bars = strategy_closing_intervals + 1;
   if(prior_month_bar_count < required_prior_bars ||
   ...
   const double prior_month_close_6 =
      bars[strategy_closing_intervals].close;
   const double prior_prior_month_end =
      bars[prior_month_bar_count].close;
   if(prior_month_end <= 0.0 ||
      prior_month_close_6 <= 0.0 ||
      prior_prior_month_end <= 0.0 ||
      !MathIsValidNumber(prior_month_end) ||
      !MathIsValidNumber(prior_month_close_6) ||
      !MathIsValidNumber(prior_prior_month_end))
      return false;

   month_return = MathLog(prior_month_end / prior_prior_month_end);
   closing_return = MathLog(prior_month_end / prior_month_close_6);
   if(!MathIsValidNumber(month_return) ||
      !MathIsValidNumber(closing_return))
      return false;

   if(month_return > 0.0 && closing_return > 0.0)
      direction = 1;
   else if(month_return < 0.0 && closing_return < 0.0)
      direction = -1;
   return true;
  }
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadDualHorizonSignal(g_decision_month_key,
      ...
      else if(g_month_return == 0.0 || g_closing_return == 0.0)
   ...
   QM_LogEvent(QM_INFO,
               "MONTHLY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"month_return\":%.12e,\"closing_return\":%.12e,\"state\":\"%s\"}",
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41021 ||
   ...
   if(qm_friday_close_enabled ||
      ...
      return true;
   if(strategy_closing_intervals != 5 ||
      strategy_hold_bars != 5 ||
      strategy_entry_grace_minutes != 5 ||
      strategy_history_bars != 80 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41021_WTI_MDUAL_MOM";
...
int OnInit()
  {
   if(!SymbolSelect(g_strategy_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41021 ||
   ...
   g_attempt_state_key =
      StringFormat("QM5_41021_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41021\",\"ea\":\"wti-mdual-mom\"}");
