...
#property description "QM5_41016 WTI Month-Closing Segment Momentum"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41016 - WTI Month-Closing Segment Momentum
// -----------------------------------------------------------------------------
// D1 structural crude-oil sleeve:
//   - inspect the final five close-to-close intervals of the prior broker month
//   - on the first executable tick of the new month, follow that return sign
...
input int    strategy_formation_intervals  = 5;
...
void Strategy_DetectDecisionClock_OnNewBar()
  {
   ...
   const long elapsed = (long)(TimeCurrent() - current_bar.time);
   g_late_decision =
      (current_month_count > 0 || elapsed < 0 ||
       elapsed > (long)strategy_entry_grace_minutes * 60);
  }

bool Strategy_LoadClosingSegmentSignal(const int current_month_key,
                                       int &direction,
                                       double &formation_return)
  {
   ...
   if(current_month_key <= 0 ||
      strategy_formation_intervals != 5)
      return false;

   MqlRates bars[];
   ArraySetAsSeries(bars, true);
   const int required_bars = strategy_formation_intervals + 1;
   ...
   const int prior_month_key = Strategy_MonthKeyForTime(bars[0].time);
   if(prior_month_key <= 0 ||
      Strategy_NextMonthKey(prior_month_key) != current_month_key)
      return false;
   ...
   for(int index = 0; index < required_bars; ++index)
     {
      if(bars[index].time <= 0 || bars[index].close <= 0.0 ||
         ...
         return false;
      if(Strategy_MonthKeyForTime(bars[index].time) != prior_month_key)
         return false;
      if(index > 0 && bars[index - 1].time <= bars[index].time)
         return false;
     }

   const double close_final = bars[0].close;
   const double close_first = bars[strategy_formation_intervals].close;
   formation_return = MathLog(close_final / close_first);
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadClosingSegmentSignal(g_decision_month_key,
                                           g_signal_direction,
                                           g_formation_return);
      if(!g_signal_valid)
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41016 ||
   ...
   if(qm_friday_close_enabled ||
      ...
      return true;
   if(strategy_formation_intervals != 5 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41016_WTI_MCLOSE_MOM";
...
void Strategy_ManageOpenPosition()
  {
   ...
   const int current_month_key = Strategy_MonthKeyForTime(current_bar.time);
...
int OnInit()
  {
   if(!SymbolSelect(g_strategy_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41016 ||
   ...
   g_attempt_state_key =
      StringFormat("QM5_41016_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41016\",\"ea\":\"wti-mclose-mom\"}");
