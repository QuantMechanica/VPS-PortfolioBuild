...
#property description "QM5_12536 ICT Order-Block Retest IDX"

#include <QM/QM_Common.mqh>

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 12536;
...
input int    strategy_m15_pool_lookback     = 96;
...
bool Strategy_LoadRates(const ENUM_TIMEFRAMES tf, const int start_shift, const int count, MqlRates &rates[])
  {
   ...
   const int copied = CopyRates(_Symbol, tf, start_shift, count, rates); // perf-allowed: bounded structural sweep/OB scan, called only from the framework closed-bar entry hook.
   return (copied == count);
  }
...
bool Strategy_M15PivotPools(const double reference_price,
                            ...
                            double &nearest_low_below)
  {
   ...
   MqlRates m15[];
   const int needed = MathMax(12, strategy_m15_pool_lookback + 4);
   if(!Strategy_LoadRates(PERIOD_M15, 1, needed, m15))
      return false;
...
bool Strategy_OppositePool(const double reference_price,
                           const int day_key,
                           const bool for_long,
                           double &opposite_pool)
  {
   opposite_pool = for_long ? DBL_MAX : 0.0;

   double pdh, pdl;
   if(Strategy_PreviousDayLevels(pdh, pdl))
     {
      if(for_long && pdh > reference_price)
         opposite_pool = MathMin(opposite_pool, pdh);
      if(!for_long && pdl < reference_price)
         opposite_pool = MathMax(opposite_pool, pdl);
     }

   double asia_high, asia_low;
   if(Strategy_AsiaRange(day_key, asia_high, asia_low))
     {
      if(for_long && asia_high > reference_price)
         opposite_pool = MathMin(opposite_pool, asia_high);
      if(!for_long && asia_low < reference_price)
         opposite_pool = MathMax(opposite_pool, asia_low);
     }

   return for_long ? (opposite_pool < DBL_MAX) : (opposite_pool > 0.0);
  }
...
bool Strategy_FindBullishSetup(const MqlRates &m15[],
                               ...
                               double &ob_mid)
  {
   ...
   for(int s = 1; s <= strategy_mss_max_bars && s < ArraySize(m15); ++s)
     {
   ...
   const int origin_shift = sweep_shift - 1;
   for(int i = origin_shift + 1; i < ArraySize(m15); ++i)
     {
      if(m15[i].close < m15[i].open)
        {
         ob_far_end = MathMin(m15[i].open, m15[i].close);
         ob_mid = (m15[i].open + m15[i].close) * 0.5;
         return (ob_far_end > 0.0 && ob_mid > 0.0);
        }
     }
   return false;
  }
   ...
   for(int i = origin_shift + 1; i < ArraySize(m15); ++i)
     {
      if(m15[i].close > m15[i].open)
        {
         ob_far_end = MathMax(m15[i].open, m15[i].close);
         ob_mid = (m15[i].open + m15[i].close) * 0.5;
         return (ob_far_end > 0.0 && ob_mid > 0.0);
        }
     }
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(!Strategy_SpreadOK())
      return true;
   return !Strategy_InKillzone(TimeCurrent());
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   MqlRates m15[];
   const int needed = MathMax(128, MathMax(strategy_m15_pool_lookback + 4,
                                           strategy_m15_pivot_lookback + strategy_mss_max_bars + 8));
   if(!Strategy_LoadRates(PERIOD_M15, 1, needed, m15))
      return false;
   ...
   int sweep_shift = -1;
   double ob_far_end = 0.0;
   double entry = 0.0;
   bool is_long = false;

   if(Strategy_FindBullishSetup(m15, pool_below, sweep_shift, ob_far_end, entry))
      is_long = true;
   else if(!Strategy_FindBearishSetup(m15, pool_above, sweep_shift, ob_far_end, entry))
      return false;
   ...
   const double sl = is_long
                     ? Strategy_NormalizePrice(ob_far_end - atr * strategy_atr_buffer_mult)
                     : Strategy_NormalizePrice(ob_far_end + atr * strategy_atr_buffer_mult);
   if(sl <= 0.0)
      return false;
   ...
   double opposite_pool = 0.0;
   if(!Strategy_OppositePool(entry, day_key, is_long, opposite_pool))
      return false;
   const double rr2 = is_long ? (entry + 2.0 * risk) : (entry - 2.0 * risk);
   const double rr3 = is_long ? (entry + 3.0 * risk) : (entry - 3.0 * risk);
   const double tp1 = is_long ? MathMin(opposite_pool, rr2) : MathMax(opposite_pool, rr2);
   ...
   req.reason = is_long ? "ICT_OB_RETEST_LONG" : "ICT_OB_RETEST_SHORT";
   req.symbol_slot = qm_magic_slot_offset;
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      if((int)PositionGetInteger(POSITION_MAGIC) != magic)
         continue;
      if(Strategy_HHMM(TimeCurrent()) >= 2100)
         return true;
     }
   return false;
  }
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_12536_ict-ob-retest-idx\"}");
