...
#property description "QM5_13008 XNG AUD Return Spread Reversion"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_13008 - XNG/AUD Return-Spread Reversion
// -----------------------------------------------------------------------------
// D1 two-leg commodity-FX basket:
//   rspread = log(XNG[t] / XNG[t-L]) - beta_aud * log(AUD[t] / AUD[t-L])
//   z > entry: short return spread = sell natural gas, buy AUDUSD
//   z < -entry: long return spread = buy natural gas, sell AUDUSD
...
input double strategy_beta_aud            = 0.75;
...
input int    strategy_audusd_max_spread_pts = 90;
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(CopyClose(g_leg_xng, PERIOD_D1, 1, history_needed, xng) != history_needed) // perf-allowed: called only behind the D1 new-bar gate or close-state refresh.
      return false;
   if(CopyClose(g_leg_audusd, PERIOD_D1, 1, history_needed, audusd) != history_needed) // perf-allowed: called only behind the D1 new-bar gate or close-state refresh.
      return false;
   ...
   for(int i = 0; i < z_lookback; ++i)
     {
      ...
      if(xng[i] <= 0.0 || audusd[i] <= 0.0 || xng[past_idx] <= 0.0 || audusd[past_idx] <= 0.0)
         return false;

      const double xng_ret = MathLog(xng[i] / xng[past_idx]);
      const double aud_ret = MathLog(audusd[i] / audusd[past_idx]);
      spreads[i] = xng_ret - strategy_beta_aud * aud_ret;
...
bool Strategy_OpenPair(const int spread_direction)
  {
   if(spread_direction == 0 || Strategy_OpenPairLegCount() > 0)
      return false;
   if(!Strategy_SpreadAllowed(g_leg_xng) || !Strategy_SpreadAllowed(g_leg_audusd))
      return false;

   const double xng_weight = 1.0;
   const double aud_weight = MathMax(0.1, MathAbs(strategy_beta_aud));
   const double weight_sum = xng_weight + aud_weight;
   const bool long_spread = (spread_direction > 0);
   const QM_OrderType xng_type = long_spread ? QM_BUY : QM_SELL;
   const QM_OrderType aud_type = long_spread ? QM_SELL : QM_BUY;
   const string reason = long_spread ? "QM5_13008_LONG_XNG_AUD_RSPREAD"
                                     : "QM5_13008_SHORT_XNG_AUD_RSPREAD";
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart())
      return true;
   if(strategy_return_lookback_d1 < 5 || strategy_z_lookback_d1 < 40 || strategy_beta_aud <= 0.0)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_13008_RSPREAD_HOST";
...
int OnInit()
  {
   ...
   string basket_symbols[2] = {g_leg_xng, g_leg_audusd};
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_13008\",\"ea\":\"xng-aud-rspread\"}");
