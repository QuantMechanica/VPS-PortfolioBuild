...
#property description "QM5_41139 WTI Completed-Month Daily-Return Hodges-Lehmann Momentum"
...
//   - enumerate every inclusive self/cross-pair daily-return average
//   - follow the exact odd/even median of the 153-276 pairwise values
...
double   g_center_low              = 0.0;
double   g_center_high             = 0.0;
double   g_daily_pseudomedian      = 0.0;
...
bool Strategy_LoadDailyPseudomedianSignal(const int current_month_key,
                                          const int label_offset,
                                          int &direction,
                                          int &month_sessions,
                                          int &pair_count,
                                          ...
                                          double &daily_pseudomedian)
  {
   ...
   if(expected_pair_count < 153 ||
      expected_pair_count > strategy_max_pair_count ||
      strategy_max_pair_count != 276)
      return false;

   double pairwise_values[];
   if(ArrayResize(pairwise_values, expected_pair_count) !=
      expected_pair_count)
      return false;
   ...
   for(int index = 0; index < pair_count; ++index)
     {
      if(index >= ArraySize(pairwise_values) ||
         !MathIsValidNumber(pairwise_values[index]))
         return false;
      if(index > 0 &&
         pairwise_values[index] < pairwise_values[index - 1])
         return false;
     }

   const int center = pair_count / 2;
   if(center < 0 || center >= pair_count ||
      center >= ArraySize(pairwise_values))
      return false;
   if((pair_count % 2) == 1)
     {
      center_low = pairwise_values[center];
      center_high = center_low;
      daily_pseudomedian = center_low;
     }
   else
     {
      if(center <= 0)
         return false;
      center_low = pairwise_values[center - 1];
      center_high = pairwise_values[center];
      daily_pseudomedian = (center_low + center_high) / 2.0;
     }

   if(!MathIsValidNumber(center_low) ||
      !MathIsValidNumber(center_high) ||
      !MathIsValidNumber(daily_pseudomedian) ||
      center_low > center_high)
      return false;

   if(daily_pseudomedian > 0.0)
      direction = 1;
   else if(daily_pseudomedian < 0.0)
      direction = -1;
   return true;
  }
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadDailyPseudomedianSignal(g_decision_month_key,
                                              ...
                                              g_center_low,
                                              g_center_high,
                                              g_daily_pseudomedian);
      if(!g_signal_valid)
         g_signal_state = "monthly_daily_pseudomedian_validation_failed";
      else if(g_daily_pseudomedian == 0.0)
         g_signal_state = "zero_daily_pseudomedian_flat";
      else if(g_signal_direction > 0)
         g_signal_state = "daily_pseudomedian_completed_month_long";
      else if(g_signal_direction < 0)
         g_signal_state = "daily_pseudomedian_completed_month_short";
      else
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"month_sessions\":%d,\"pair_count\":%d,\"center_low\":%.12e,\"center_high\":%.12e,\"daily_pseudomedian\":%.12e,\"boundary_close\":%.8f,\"final_close\":%.8f,\"raw_sum\":%.12e,\"endpoint_return\":%.12e,\"raw_direction\":%d,\"endpoint_agrees\":%s,\"state\":\"%s\"}",
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41139 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41139_WTI_MDAILY_HL_MOM";
   ...
   req.reason = (g_signal_direction > 0)
      ? "WTI_MDAILY_HL_MOM_LONG"
      : "WTI_MDAILY_HL_MOM_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41139 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI completed-month daily-return-pseudomedian position holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41139_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41139\",\"ea\":\"wti-mdaily-hl-mom\"}");
