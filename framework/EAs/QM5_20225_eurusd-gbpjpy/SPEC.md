# QM5_20225_eurusd-gbpjpy — Strategy Spec
...
`ln(EURUSD.DWX) - (-0.242481866) * ln(GBPJPY.DWX)`. Score the newest spread
...
| `strategy_beta` | -0.242481866 | fixed | DEV-fitted structural hedge coefficient |
...
- `USDJPY.DWX` — conversion-history dependency used by the USD-account
  tester for GBPJPY P/L; it is order-free and has no magic slot.
...
| Typical hold time | multi-day; scan half-life 135.604 D1 bars |
| Expected drawdown profile | high and regime-sensitive; conservative 30% prior |
| Regime preference | EURUSD/GBPJPY fixed-residual mean reversion |
| Win rate target | unknown; Q02 is the first platform-economic judge |

The fixed scan recorded DEV net Sharpe `0.005490`, OOS net Sharpe `-0.128824`,
and OOS return `-1.095531%`. The negative OOS score and slow half-life are
...
`1.0` and `0.242481866`; each leg is independently bounded by its ATR stop.
If the GBPJPY allocation normalizes below broker minimum volume, the
...
|---|---|---|---|
| v1 | 2026-08-05 | Initial build from approved card | Next-rank-47 non-duplicate fixed-scan FX basket |
