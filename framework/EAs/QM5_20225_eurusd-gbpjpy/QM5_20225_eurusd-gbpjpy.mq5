...
#property description "QM5_20225 EURUSD GBPJPY Cointegration"
...
//
// Card:
//   strategy-seeds/cards/approved/QM5_20225_eurusd-gbpjpy_card.md
//
// Fixed spread:
//   ln(EURUSD.DWX) - beta * ln(GBPJPY.DWX)
//
...
input double strategy_beta              = -0.242481866;
...
string   g_leg_eurusd = "EURUSD.DWX";
...
bool Strategy_EnsureBasketScope()
  {
   ...
   string allowed[3] = {"EURUSD.DWX", "GBPJPY.DWX", "USDJPY.DWX"};
   for(int i = 0; i < 3; ++i)
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(!Strategy_EnsureBasketScope())
      return false;
   if(!QM_SymbolAssertOrLog(g_leg_eurusd) ||
   ...
   if(CopyClose(g_leg_eurusd, PERIOD_D1, 1, history_count, eurusd) != // perf-allowed: called only by the gated spread refresh.
      history_count)
      return false;
   if(CopyClose(g_leg_gbpjpy, PERIOD_D1, 1, history_count, gbpjpy) != // perf-allowed: called only by the gated spread refresh.
      history_count)
      return false;
   if(CopyTime(g_leg_eurusd, PERIOD_D1, 1, history_count, eurusd_time) != // perf-allowed: called only by the gated spread refresh.
      history_count)
      return false;
   ...
   for(int i = 0; i < history_count; ++i)
     {
      if(eurusd_time[i] <= 0 || eurusd_time[i] != gbpjpy_time[i])
         return false;
      if(eurusd[i] <= 0.0 || gbpjpy[i] <= 0.0)
         return false;

      spreads[i] =
         MathLog(eurusd[i]) - strategy_beta * MathLog(gbpjpy[i]);
...
bool Strategy_PreparePair(const int spread_direction,
                          QM_EntryRequest &host_req)
  {
   ...
   const double eurusd_weight = 1.0;
   const double gbpjpy_weight = MathAbs(strategy_beta);
   const double weight_sum = eurusd_weight + gbpjpy_weight;
   if(weight_sum <= 0.0)
      return false;
   ...
   const double eurusd_lots =
      Strategy_LotsForLeg(g_leg_eurusd, eurusd_weight, eurusd_weight);
   const double gbpjpy_lots =
      Strategy_LotsForLeg(g_leg_gbpjpy, gbpjpy_weight, eurusd_weight);
   if(eurusd_lots <= 0.0 || gbpjpy_lots <= 0.0)
      return false;

   const bool long_spread = (spread_direction > 0);
   const QM_OrderType eurusd_type = long_spread ? QM_BUY : QM_SELL;
   const QM_OrderType gbpjpy_type =
      long_spread
      ? (strategy_beta >= 0.0 ? QM_SELL : QM_BUY)
      : (strategy_beta >= 0.0 ? QM_BUY : QM_SELL);
   const string reason =
      long_spread ? "QM5_20225_LONG_SPREAD_Z_LT_NEG_ENTRY"
                  : "QM5_20225_SHORT_SPREAD_Z_GT_POS_ENTRY";

   const double host_entry =
      QM_OrderTypeIsBuy(eurusd_type)
      ? SymbolInfoDouble(g_leg_eurusd, SYMBOL_ASK)
      : SymbolInfoDouble(g_leg_eurusd, SYMBOL_BID);
   const double host_atr =
      QM_ATR(g_leg_eurusd, PERIOD_D1, strategy_atr_period_d1, 1);
   if(host_entry <= 0.0 || host_atr <= 0.0)
      return false;

   host_req.type = eurusd_type;
   host_req.price = 0.0;
   host_req.sl = QM_StopATRFromValue(g_leg_eurusd,
                                     eurusd_type,
                                     host_entry,
                                     host_atr,
                                     strategy_atr_sl_mult);
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20225_PAIR_HOST";
...
bool Strategy_NewsFilterHook(const datetime broker_time)
  {
   if(qm_news_temporal != QM_NEWS_TEMPORAL_OFF ||
     ...
     {
      if(!QM_NewsAllowsTrade2(g_leg_eurusd,
                              broker_time,
                              qm_news_temporal,
                              qm_news_compliance))
         return true;
     ...
     {
      if(!QM_NewsAllowsTrade(g_leg_eurusd,
                             broker_time,
                             qm_news_mode_legacy))
         return true;
...
int OnInit()
  {
   SymbolSelect(g_leg_eurusd, true);
...
void OnTick()
  {
   ...
   const bool is_new_signal_bar =
      QM_IsNewBar(g_leg_eurusd, PERIOD_D1);
