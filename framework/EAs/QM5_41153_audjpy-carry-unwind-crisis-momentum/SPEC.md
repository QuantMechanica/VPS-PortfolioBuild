# QM5_41153_audjpy-carry-unwind-crisis-momentum - Strategy Spec

**EA ID:** QM5_41153

**Slug:** `audjpy-carry-unwind-crisis-momentum`
...
At each new `AUDJPY.DWX` D1 bar, the EA reads the immediately completed D1
...
`AUDJPY.DWX`; the other three symbols are signal data only.
...
- Host/execution symbol: exact `AUDJPY.DWX`, D1, slot 0, magic `411530000`.
- Signal-only symbols: `NZDJPY.DWX`, `CADJPY.DWX`, and `EURJPY.DWX` plus the
  host series.
...
The entry decision runs once behind `QM_IsNewBar(AUDJPY.DWX, PERIOD_D1)`.
...
|---|---|---|---|
| v1 | 2026-08-28 | Initial build from OWNER-approved G0 card | `5eddb74a-3cdd-4af7-8be0-c9f100bda600` |
