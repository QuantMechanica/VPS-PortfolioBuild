...
#property description "QM5_41338 WTI Monthly ADF and Raw von Neumann Agreement Trend"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41338 - WTI Monthly ADF and Raw von Neumann Agreement-Gated Trend
// -----------------------------------------------------------------------------
...
//   - require ADF t >= -2.594 and raw von Neumann eta < 2.0
...
input int    strategy_vn_return_count       = 20;
input double strategy_vn_eta_max            = 2.0;
...
bool Strategy_AgreementSignal(const double &closes[],
                              Strategy_SignalMetrics &metrics)
  {
   ...
   metrics.vn_return_count = strategy_vn_return_count;
   metrics.direction = 0;
   metrics.persistence_qualified = false;
   metrics.adf_qualified = false;
   metrics.vn_qualified = false;
   ...
   if(strategy_level_count != 60 ||
      ...
      MathAbs(strategy_vn_eta_max - 2.0) >
         0.000000000000001 ||
   ...
   double returns[20];
   for(int index = 0; index < strategy_vn_return_count; ++index)
     {
   ...
   if(!Strategy_VonNeumannCore(
         returns,
         metrics.vn_return_mean,
         metrics.vn_variance_sum,
         metrics.vn_successive_sum,
         metrics.vn_eta,
         metrics.vn_path))
      return false;
   metrics.vn_qualified = (metrics.vn_eta < strategy_vn_eta_max);
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   else
     {
      ...
      if(!g_signal_valid)
         g_signal_state = "agreement_validation_failed";
              ...
              !g_signal_metrics.vn_qualified)
         g_signal_state = "both_gates_reject_consumed_flat";
      else if(!g_signal_metrics.adf_qualified)
         g_signal_state = "adf_rejects_vn_only_consumed_flat";
      else if(!g_signal_metrics.vn_qualified)
         g_signal_state = "vn_rejects_adf_only_consumed_flat";
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"endpoint_count\":%d,\"level_count\":%d,\"vn_return_count\":%d,\"mean_y\":%.16f,\"mean_z\":%.16f,\"mean_w\":%.16f,\"szz\":%.16f,\"sww\":%.16f,\"szw\":%.16f,\"szy\":%.16f,\"swy\":%.16f,\"determinant\":%.16f,\"alpha\":%.16f,\"gamma\":%.16f,\"phi\":%.16f,\"sse\":%.16f,\"residual_variance\":%.16f,\"se_gamma\":%.16f,\"adf_t\":%.12f,\"adf_qualified\":%s,\"vn_return_mean\":%.12f,\"vn_variance_sum\":%.16f,\"vn_successive_sum\":%.12f,\"vn_eta\":%.12f,\"vn_qualified\":%s,\"persistence_qualified\":%s,\"momentum_12\":%.12f,\"levels\":\"%s\",\"returns\":\"%s\",\"regression\":\"%s\",\"vn_returns\":\"%s\",\"oldest_close\":%.10f,\"newest_close\":%.10f,\"oldest_endpoint\":%I64d,\"newest_endpoint\":%I64d,\"state\":\"%s\"}",
                            ...
                            g_signal_metrics.vn_return_count,
                            ...
                            g_signal_metrics.vn_return_mean,
                            g_signal_metrics.vn_variance_sum,
                            g_signal_metrics.vn_successive_sum,
                            g_signal_metrics.vn_eta,
                            g_signal_metrics.vn_qualified ? "true" : "false",
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41338 ||
      qm_magic_slot_offset != 0 ||
      QM_FrameworkMagic() != 413380000 ||
      qm_rng_seed != 42)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41338_WTI_ADF_VN";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41338 ||
      qm_magic_slot_offset != 0)
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI monthly ADF and Raw von Neumann agreement trend holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41338_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   g_entry_month_state_key =
      StringFormat("QM5_41338_ENTRY_MONTH_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41338\",\"ea\":\"wti-adf-vn-agree-tr\"}");
