# QM5_12731_edgelab-usdcad-eurgbp-cointegration - Strategy Spec
...
The EA trades the USDCAD.DWX and EURGBP.DWX D1 cointegration spread. It computes
`S = ln(USDCAD) - beta * ln(EURGBP)` with beta defaulting to 0.49, then
...
same script ranked USDCAD/EURGBP as the strongest remaining unbuilt positive
DEV/OOS candidate after the existing 12532/12533/12624/12712/12723/12728 baskets, with
DEV Sharpe 0.46, OOS net Sharpe 0.57, OOS return +3.50%, 19 OOS state changes,
beta 0.49, and 57-day half-life.
...
| strategy_beta | 0.49 | >0 | Hedge coefficient in `ln(USDCAD) - beta * ln(EURGBP)` |
...
**Designed for:**
- USDCAD.DWX - leg 1 of the fixed USDCAD/EURGBP spread and the spread numerator.
- EURGBP.DWX - leg 2 of the fixed USDCAD/EURGBP spread and the beta-weighted spread denominator.

**Explicitly not for:**
- Other `.DWX` symbols. This card is a fixed two-leg FX-cross basket, not a portable multi-pair strategy.

The EA selects EURUSD.DWX and GBPUSD.DWX as conversion history for the EURGBP leg
...
| Regime preference | mean-reverting USD/CAD vs European/UK relative-value spread |
