...
#property description "QM5_12731 Edge Lab USDCAD EURGBP Cointegration"
...
input double strategy_beta              = 0.49;
...
bool Strategy_IsHostSymbol()
  {
   return (_Symbol == g_leg_usdcad || _Symbol == g_leg_eurgbp);
  }
...
bool Strategy_EnsureBasketScope()
  {
   ...
   // EURGBP needs EUR/GBP conversion history for USD-denominated tester
   // accounting; USDCAD is also selected as its own CAD conversion path.
   string allowed[4] = {"USDCAD.DWX", "EURGBP.DWX", "EURUSD.DWX", "GBPUSD.DWX"};
   for(int i = 0; i < 4; ++i)
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(!Strategy_EnsureBasketScope())
      return false;
   if(!QM_SymbolAssertOrLog(g_leg_usdcad) || !QM_SymbolAssertOrLog(g_leg_eurgbp))
      return false;
   ...
   for(int i = 0; i < lookback; ++i)
     {
      if(usdcad[i] <= 0.0 || eurgbp[i] <= 0.0)
         return false;
      spreads[i] = MathLog(usdcad[i]) - strategy_beta * MathLog(eurgbp[i]);
...
bool Strategy_OpenPair(const int spread_direction)
  {
   ...
   const double weight_sum = usdcad_weight + eurgbp_weight;
   if(weight_sum <= 0.0)
      return false;
   ...
   const string reason = long_spread ? "QM5_12731_LONG_SPREAD_Z_LT_NEG_ENTRY"
                                     : "QM5_12731_SHORT_SPREAD_Z_GT_POS_ENTRY";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12731_PAIR_HOST";
