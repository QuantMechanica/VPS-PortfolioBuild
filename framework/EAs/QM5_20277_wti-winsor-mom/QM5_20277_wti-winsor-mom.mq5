...
#property description "QM5_20277 WTI Winsorized-Mean Momentum"
...
//   3. sort the returns and cap the lowest/highest two at indexes 2 and 9;
//   4. trade the sign of the resulting twelve-term Winsorized mean.
// The fixed-tail return statistic is a transparent QM mechanization, not an
// author rule. Runtime uses native XTIUSD.DWX price and broker data only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 20277;
...
double g_cache_winsor_mean    = 0.0;
double g_cache_winsor_low     = 0.0;
double g_cache_winsor_high    = 0.0;
...
void Strategy_ResetCachedState()
  {
   ...
   g_cache_signal = 0;
   g_cache_winsor_mean = 0.0;
   g_cache_winsor_low = 0.0;
   g_cache_winsor_high = 0.0;
   g_cache_state_reason = "not_evaluated";
  }

bool Strategy_WinsorizedMeanSignal(const double &closes[], int &signal)
  {
   signal = 0;
   const int return_count = strategy_return_months;
   const int winsor_each_tail = strategy_winsor_each_tail;
   const int low_boundary_index = winsor_each_tail;
   const int high_boundary_index = return_count - winsor_each_tail - 1;
   if(return_count != 12 ||
      winsor_each_tail != 2 ||
      low_boundary_index != 2 ||
      high_boundary_index != 9 ||
      ArraySize(closes) != return_count + 1)
     {
      g_cache_state_reason = "bad_winsor_contract";
      return false;
     }
   ...
   for(int i = 0; i < return_count; ++i)
     {
      if(!MathIsValidNumber(sorted_returns[i]) ||
        ...
        {
         g_cache_state_reason = "invalid_sorted_state";
         return false;
        }
     }

   const double winsor_low = sorted_returns[low_boundary_index];
   const double winsor_high = sorted_returns[high_boundary_index];
   if(!MathIsValidNumber(winsor_low) ||
      !MathIsValidNumber(winsor_high) ||
      winsor_low > winsor_high)
     {
      g_cache_state_reason = "invalid_winsor_boundary";
      return false;
     }
   ...
   for(int i = 0; i < return_count; ++i)
     {
      double capped_return = sorted_returns[i];
      if(i < low_boundary_index)
         capped_return = winsor_low;
      else if(i > high_boundary_index)
         capped_return = winsor_high;
      ...
      winsor_sum += capped_return;
      if(!MathIsValidNumber(winsor_sum))
        {
         g_cache_state_reason = "invalid_winsor_sum";
         return false;
        }
     }

   const double winsor_mean = winsor_sum / (double)return_count;
   if(!MathIsValidNumber(winsor_mean))
     {
      g_cache_state_reason = "invalid_winsor_mean";
      return false;
     }

   g_cache_winsor_low = winsor_low;
   g_cache_winsor_high = winsor_high;
   g_cache_winsor_mean = winsor_mean;
   if(winsor_mean == 0.0)
     {
      g_cache_state_reason = "exact_zero_winsor_mean";
      return true;
     }

   signal = (winsor_mean > 0.0) ? 1 : -1;
...
bool Strategy_LoadSignalState(int &signal)
  {
   ...
   return Strategy_WinsorizedMeanSignal(monthly_closes, signal);
  }
...
void Strategy_PrepareMonthlySignal()
  {
   ...
   QM_LogEvent(QM_INFO,
               "MONTHLY_STATE",
               StringFormat("{\"month\":%d,\"valid\":%s,\"signal\":%d,\"winsor_mean\":%.10f,\"winsor_low\":%.10f,\"winsor_high\":%.10f,\"state\":\"%s\"}",
                            g_cache_month_key,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 20277 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20277_WTI_WINSOR_MOM";
   ...
   req.reason = (g_cache_signal > 0) ? "WINSOR_MOM_XTI_LONG" : "WINSOR_MOM_XTI_SHORT";
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_20277_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_20277\",\"ea\":\"wti-winsor-mom\"}");
