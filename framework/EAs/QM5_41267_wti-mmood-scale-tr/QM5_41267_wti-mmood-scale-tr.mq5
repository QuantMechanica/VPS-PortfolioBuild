...
#property description "QM5_41267 WTI Mood Squared-Rank Scale Non-Contraction Trend"
...
//   - pool the raw returns and reject every anchored relative-tolerance tie
//   - assign unique integer ranks 1..12 and require their sum to equal 78
//   - score the older block by squared rank distance from the locked center
//   - require the older score not to exceed its fixed null expectation
//   - compute the finite standardized Mood statistic as an invariant
...
input double strategy_rank_center             = 6.5;
input double strategy_score_expectation       = 71.5;
input double strategy_score_variance          = 364.0;
...
struct Strategy_SignalMetrics
  {
   ...
   int assigned_count;
   int rank_sum;
   int unique_rank_count;
   bool recent_scale_noncontracting;
   double rank_center;
   double score_old;
   double score_expectation;
...
bool Strategy_MoodSignal(const double &closes[],
                         Strategy_SignalMetrics &metrics)
  {
   ...
   metrics.rank_center = strategy_rank_center;
   metrics.score_expectation = strategy_score_expectation;
   metrics.score_variance = strategy_score_variance;
   metrics.direction = 0;

   if(strategy_endpoint_count != 13 ||
      strategy_return_count != 12 ||
      strategy_block_size != 6 ||
      MathAbs(strategy_rank_center - 6.5) >
         0.000000000000001 ||
      MathAbs(strategy_score_expectation - 71.5) >
         0.000000000000001 ||
      MathAbs(strategy_score_variance - 364.0) >
         0.000000000001 ||
   ...
   while(run_start < strategy_return_count)
     {
      ...
      if(!MathIsValidNumber(anchor))
         return false;
      int run_end = run_start;
      while(run_end + 1 < strategy_return_count)
        {
         const double candidate = ordered_returns[run_end + 1];
      ...
      // Mood's locked no-tie form: any anchored run wider than one
      // observation consumes the month flat.
      if(run_end != run_start)
         return false;

      const int rank = run_start + 1;
      const int original_index = ordered_indices[run_start];
      ...
      ++metrics.unique_rank_count;
      metrics.rank_sum += rank;
      run_start = run_end + 1;
     }

   if(metrics.assigned_count != strategy_return_count ||
      metrics.unique_rank_count != strategy_return_count ||
      metrics.rank_sum != 78)
      return false;

   for(int index = 0; index < strategy_return_count; ++index)
     {
      if(!assigned[index] || ranks[index] < 1 ||
         ranks[index] > strategy_return_count)
         return false;
      if(index < strategy_block_size)
        {
         const double rank_delta =
            (double)ranks[index] - metrics.rank_center;
         metrics.score_old += rank_delta * rank_delta;
         if(!MathIsValidNumber(metrics.score_old))
            return false;
        }
      if(StringLen(metrics.rank_path) > 0)
         metrics.rank_path += ",";
      metrics.rank_path += IntegerToString(ranks[index]);
     }

   if(!MathIsValidNumber(metrics.score_old) ||
      !MathIsValidNumber(metrics.score_expectation) ||
      !MathIsValidNumber(metrics.score_variance) ||
      metrics.score_old < 0.0 ||
      metrics.score_expectation <= 0.0 ||
      metrics.score_variance <= 0.0)
      return false;

   const double statistic_denominator = MathSqrt(metrics.score_variance);
   if(statistic_denominator <= 0.0 ||
      !MathIsValidNumber(statistic_denominator))
      return false;
   metrics.statistic_z =
      (metrics.score_old - metrics.score_expectation) /
      statistic_denominator;
   if(!MathIsValidNumber(metrics.statistic_z))
      return false;

   metrics.recent_scale_noncontracting =
      (metrics.score_old <= metrics.score_expectation);
   ...
   if(!MathIsValidNumber(metrics.oldest_close) ||
      ...
      return false;
   if(!metrics.recent_scale_noncontracting)
      return true;
                                        ...
                                        int &direction)
  {
   ...
   const bool valid =
      Strategy_LoadMonthlyEndpoints(current_month_key,
                                    ...
                                    closes,
                                    endpoint_times) &&
      Strategy_MoodSignal(closes, metrics);
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
   ...
   else
     {
      ...
      g_signal_valid =
         endpoints_valid &&
         Strategy_MoodSignal(closes, g_signal_metrics);
      if(!g_signal_valid)
         g_signal_state = "mood_validation_failed";
      else if(g_signal_metrics.direction == 0)
        {
         if(!g_signal_metrics.recent_scale_noncontracting)
            g_signal_state =
               "mood_recent_scale_contracting_flat";
         else
            g_signal_state = "mood_neutral_return_flat";
        }
      else if(g_signal_metrics.direction > 0)
         g_signal_state = "mood_wti_long";
      else if(g_signal_metrics.direction < 0)
         g_signal_state = "mood_wti_short";
      else
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"endpoint_count\":%d,\"return_count\":%d,\"block_size\":%d,\"assigned_count\":%d,\"rank_sum\":%d,\"unique_rank_count\":%d,\"recent_scale_noncontracting\":%s,\"rank_center\":%.15f,\"score_old\":%.15f,\"score_expectation\":%.15f,\"score_variance\":%.15f,\"statistic_z\":%.15f,\"recent_return\":%.12f,\"returns\":\"%s\",\"ranks\":\"%s\",\"oldest_close\":%.10f,\"newest_close\":%.10f,\"oldest_endpoint\":%I64d,\"newest_endpoint\":%I64d,\"state\":\"%s\"}",
                            ...
                            g_signal_metrics.unique_rank_count,
                            g_signal_metrics.recent_scale_noncontracting
                               ? "true" : "false",
                            g_signal_metrics.rank_center,
                            g_signal_metrics.score_old,
                            g_signal_metrics.score_expectation,
                            g_signal_metrics.score_variance,
                            g_signal_metrics.statistic_z,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41267 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41267_WTI_MMOOD_SCALE_TR";
   ...
   req.reason = (g_signal_metrics.direction > 0)
      ? "WTI_MMOOD_SCALE_TR_LONG"
      : "WTI_MMOOD_SCALE_TR_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41267 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI Mood scale-noncontraction trend holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41267_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41267\",\"ea\":\"wti-mmood-scale-tr\"}");
