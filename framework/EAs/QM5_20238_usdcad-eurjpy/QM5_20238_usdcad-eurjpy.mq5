...
#property description "QM5_20238 USDCAD EURJPY Cointegration"
...
//
// Card:
//   strategy-seeds/cards/approved/QM5_20238_usdcad-eurjpy_card.md
//
// Fixed spread:
//   ln(USDCAD.DWX) - beta * ln(EURJPY.DWX)
//
...
input double strategy_beta              = -0.243266890557;
...
bool Strategy_EnsureBasketScope()
  {
   ...
   string allowed[3] = {"USDCAD.DWX", "EURJPY.DWX", "USDJPY.DWX"};
   for(int i = 0; i < 3; ++i)
...
bool Strategy_RefreshSpreadState()
  {
   ...
   for(int i = 0; i < history_count; ++i)
     {
      if(usdcad_time[i] <= 0 || usdcad_time[i] != eurjpy_time[i])
         return false;
      if(usdcad[i] <= 0.0 || eurjpy[i] <= 0.0)
         return false;

      spreads[i] =
         MathLog(usdcad[i]) - strategy_beta * MathLog(eurjpy[i]);
...
bool Strategy_PreparePair(const int spread_direction,
                          QM_EntryRequest &host_req)
  {
   ...
   const double weight_sum = usdcad_weight + eurjpy_weight;
   if(weight_sum <= 0.0)
      return false;
   ...
   const double usdcad_lots =
      Strategy_LotsForLeg(g_leg_usdcad, usdcad_weight, usdcad_weight);
   const double eurjpy_lots =
      Strategy_LotsForLeg(g_leg_eurjpy, eurjpy_weight, usdcad_weight);
   if(usdcad_lots <= 0.0 || eurjpy_lots <= 0.0)
      return false;
   ...
   const string reason =
      long_spread ? "QM5_20238_LONG_SPREAD_Z_LT_NEG_ENTRY"
                  : "QM5_20238_SHORT_SPREAD_Z_GT_POS_ENTRY";

   const double host_entry =
      QM_OrderTypeIsBuy(usdcad_type)
      ? SymbolInfoDouble(g_leg_usdcad, SYMBOL_ASK)
      : SymbolInfoDouble(g_leg_usdcad, SYMBOL_BID);
   const double host_atr =
      QM_ATR(g_leg_usdcad, PERIOD_D1, strategy_atr_period_d1, 1);
   if(host_entry <= 0.0 || host_atr <= 0.0)
      return false;

   host_req.type = usdcad_type;
   host_req.price = 0.0;
   host_req.sl = QM_StopATRFromValue(g_leg_usdcad,
                                     usdcad_type,
                                     host_entry,
                                     host_atr,
                                     strategy_atr_sl_mult);
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20238_PAIR_HOST";
