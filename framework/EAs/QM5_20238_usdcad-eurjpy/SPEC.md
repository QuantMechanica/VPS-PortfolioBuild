# QM5_20238_usdcad-eurjpy — Strategy Spec
...
`ln(USDCAD.DWX) - (-0.243266890557) * ln(EURJPY.DWX)`. Score the newest spread
...
| `strategy_beta` | -0.243266890557 | fixed | DEV-fitted structural hedge coefficient |
...
- `USDJPY.DWX` — conversion-history dependency for EURJPY P/L; order-free
...
| Typical hold time | multi-day; scan half-life 66.784 D1 bars |
| Expected drawdown profile | high and regime-sensitive; conservative 30% prior |
| Regime preference | USDCAD/EURJPY fixed-residual mean reversion |
| Win rate target | unknown; Q02 is the first platform-economic judge |

The fixed scan recorded DEV net Sharpe `-0.006562`, OOS net Sharpe `-0.403385`,
and OOS return `-2.696283%`. These are adverse priors, not a promotion claim.
...
Chan supplies the method but makes no USDCAD/EURJPY performance claim.
...
`1.0` and `0.231842927`; each leg is bounded by its ATR stop. If the EURJPY
...
|---|---|---|---|
| v1 | 2026-08-06 | Initial approved-card build | Next-rank-57 non-duplicate fixed-scan FX basket |
