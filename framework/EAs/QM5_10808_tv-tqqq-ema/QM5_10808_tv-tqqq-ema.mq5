...
#property description "QM5_10808 TradingView TQQQ EMA ADX Trend"
...
input double strategy_step_trigger_r    = 2.0;
input double strategy_step_lock_r       = 1.0;
...
bool Strategy_NoTradeFilter()
  {
   // Card-authorized filters are ADX in EntrySignal plus framework spread/session/news.
   return false;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_10808_EMA20_50_ADX_LONG";
   ...
   const double fast_now = QM_EMA(_Symbol, tf, strategy_fast_ema, 1);
   const double slow_now = QM_EMA(_Symbol, tf, strategy_slow_ema, 1);
   const double fast_prev = QM_EMA(_Symbol, tf, strategy_fast_ema, 2);
   const double slow_prev = QM_EMA(_Symbol, tf, strategy_slow_ema, 2);
...
void Strategy_ManageOpenPosition()
  {
   if(strategy_be_trigger_r <= 0.0 || strategy_step_trigger_r <= 0.0 ||
      strategy_step_lock_r < 0.0 || strategy_atr_tp_mult <= 0.0)
      return;
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      if(current_tp > open_price)
         risk_distance = (current_tp - open_price) / strategy_atr_tp_mult;
      ...
      const double profit_distance = bid - open_price;
      double target_sl = 0.0;
      string reason = "";

      if(profit_distance >= strategy_step_trigger_r * risk_distance)
        {
         target_sl = open_price + strategy_step_lock_r * risk_distance;
         reason = "QM5_10808_STEP_2R_LOCK_1R";
        }
      else if(profit_distance >= strategy_be_trigger_r * risk_distance)
        {
         target_sl = open_price;
         reason = "QM5_10808_STEP_1R_BREAKEVEN";
        }
      ...
      QM_TM_MoveSL(ticket, target_sl, reason);
     }
  }
...
bool Strategy_ExitSignal()
  {
   // Card exits are handled by SL, TP, stepped stop management, and framework Friday close.
   return false;
  }
