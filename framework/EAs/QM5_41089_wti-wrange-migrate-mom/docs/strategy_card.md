...
type: strategy
strategy_id: MOP-WTI-WRANGE-MIGRATE-MOM-2026_S01
variant_id: MOP-WTI-WRANGE-MIGRATE-MOM-2026_S01
source_id: MOP-WTI-WRANGE-MIGRATE-MOM-2026
ea_id: QM5_41089
slug: wti-wrange-migrate-mom
status: APPROVED
execution_contract_ref: strategy-seeds/cards/approved/QM5_41089_wti-wrange-migrate-mom_card.md
execution_contract_status: APPROVED
created: 2026-08-21
created_by: Research+Development
last_updated: 2026-08-21
g0_status: APPROVED
g0_decision: decisions/2026-08-21_qm5_41089_wti_weekly_range_migration_momentum_g0.md
source_approval: decisions/2026-08-21_wti_weekly_range_migration_momentum_source_approval.md
...
source_citations:
  - type: academic_paper
    ...
    location: "DOI 10.1016/j.jfineco.2011.11.003; complete-read packet strategy-seeds/sources/MOP-TSMOM-2012/source.md; bounded translation strategy-seeds/sources/MOP-WTI-WRANGE-MIGRATE-MOM-2026/source.md"
    quality_tier: A
    role: own_price_continuation_and_wti_carrier_lineage
strategy_mechanic: normalized-week-boundary-wti-two-consecutive-completed-weekly-ohlc-packages-strict-higher-high-higher-low-or-lower-high-lower-low-auction-range-migration-continuation-one-week-hold
sources:
  - "[[sources/MOP-WTI-WRANGE-MIGRATE-MOM-2026]]"
concepts:
...
strategy_type_flags: [commodity, energy, wti-crude, structural-trend, completed-week-range-migration, symmetric-long-short, atr-hard-stop, time-stop, low-frequency]
...
period: D1
timeframe: D1
expected_trade_frequency: "Approximately 12-24 completed WTI positions per full post-warm-up year after exact weekly history, strict two-endpoint migration, and execution gates; Q02 must prove at least five/year or retire."
...
review_focus: "Falsify a direct-WTI completed-week auction-range migration trend outside the certified XAU/SP500/NDX/XNG book. Verify uniform energy labels, exact Monday anchors, two consecutive completed weekly OHLC packages, three-to-five sessions each, strict HH+HL or LH+LL state, mixed/equality flat, one attempt, fixed risk, and next-week exit. Q09 alone may establish realized decorrelation."
...
g0_approval_reasoning: "OWNER-authorized WTI sleeve; R1 complete-read peer-reviewed WTI source with weekly range-state translation risk disclosed; R2 exact labels, weeks, OHLC, strict HH+HL/LH+LL, attempt, risk, and lifecycle; R3 registered native WTI D1; R4 deterministic price arithmetic; canonical dedup CLEAN with manual"
---

# QM5_41089 WTI Completed-Week Auction-Range Migration Momentum

## Hypothesis

When both endpoints of WTI's completed weekly auction range migrate in the
same direction versus the parent week, the whole price-discovery region has
shifted rather than merely printing a close-to-close fluctuation. Following a
strict higher-high/higher-low state long or a strict lower-high/lower-low
state short for the next broker week may capture a structural energy trend.
...
`strategy-seeds/sources/MOP-WTI-WRANGE-MIGRATE-MOM-2026/source.md`, authorized
before extraction by
`decisions/2026-08-21_wti_weekly_range_migration_momentum_source_approval.md`
at commit `801e20b8e`. The complete parent source hash is
...
weekly higher-high/higher-low or lower-high/lower-low state, a continuous CFD,
...
scanned 4,578 registry identities and 625 root cards, and returned `CLEAN`
with no exact or fuzzy match. Manual semantic review fixes the load-bearing
boundaries:
...
- `QM5_41073_wti-woutside-settle` requires a strict higher high and lower low,
  ...
  quartile close. This card requires same-direction migration of both range
  endpoints and ignores opens and closes.
- `QM5_41080_wti-wclose-location-mom` follows parent-close-to-new-close return
  sign only when the newest close finishes in the matching edge of its own
  range. This card reads no close and has no close-location threshold.
- `QM5_41087_wti-wr4-close-mom` ranks four completed ranges by width and
  requires body/close-location agreement. This card has no range-width rank,
...
- `QM5_41061_wti-week-nr7-brk`, `QM5_13075_xti-inside-week-brk`, and
  ...
  all current-week price from the signal and enters only at the boundary.
- the WTI weekly return-path family classifies completed closes and return
  signs or magnitudes. This card classifies only completed-week highs/lows.
- `QM5_10596_mql5-highlow` counts a configurable run of individual H4 bars
  whose highs and lows step together and exits on an opposite H4 star; it is
  not an exact WTI completed-week auction-range/one-week package.
- `QM5_12567_cum-rsi2-commodity` is a long-only two-day XNG cumulative-RSI2
  pullback under a slow trend filter on a different energy carrier.
...
- Timeframe: exact D1; magic slot 0; planned magic `410890000`.
...
- Normal exit: first tick whose broker Monday anchor is later than the open
  position's anchor.
- Expected cadence: approximately 12-24 completed positions/year.
...
1. Evaluate only once on a new exact `XTIUSD.DWX` D1 bar under EA 41089 and
   magic slot zero.
...
7. Within a fixed 30-bar buffer, reconstruct exactly the immediately completed
   ...
   positive finite OHLC, and strict positive aggregate ranges.
...
- Exact host, D1, EA 41089, slot zero, and registered magic.
...
- Own at most one `XTIUSD.DWX` position under magic `410890000`.
...
membership. They do not supply the weekly horizon or range-migration state.

## QM Interpretations

`MOP-WTI-WRANGE-MIGRATE-MOM-2026_S01` fixes the weekly horizon, completed
...
entry at equality or a mixed/inside/outside state, wrong side, current-week
leakage, late or repeated attempt, missing hard stop, wrong next-week close,
nondeterminism, or invalid fixed-risk mode.

Changing the WTI carrier, weekly packages, range comparisons, direction,
attempt clock, risk, stop, or lifecycle requires a new identity, binary,
complete stream reconciliation, and portfolio requalification. A failed result
may not be rescued by accepting equality or mixed states, adding a close or
current-week gate, reversing the side, changing the hold, or adding calendar,
return, close-location, volatility, volume, moving-average, inventory, event,
or external state.
...
- [x] R1: one bounded source ID with named peer-reviewed authors, DOI,
  ...
  weekly range-state translation risk is disclosed.
...
- [x] Dedup: canonical checker clean; manual weekly WTI and generic high/low
...
|---|---|---|---|---|
| v1 | 2026-08-21 | initial WTI completed-week range-migration card | G0 | APPROVED |

## Pipeline Phase Status

| Phase | Date | Status | Evidence |
|---|---|---|---|
| G0 Research Intake | 2026-08-21 | APPROVED | `decisions/2026-08-21_qm5_41089_wti_weekly_range_migration_momentum_g0.md` |
...
`RISK_FIXED` backtest setfile, and one paced target-only Q02
