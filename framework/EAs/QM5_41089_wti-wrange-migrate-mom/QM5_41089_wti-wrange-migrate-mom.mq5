...
#property description "QM5_41089 WTI Weekly Range-Migration Momentum"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41089 - WTI Weekly Range-Migration Momentum
// -----------------------------------------------------------------------------
// D1 structural crude-oil sleeve:
//   - aggregate two consecutive completed broker-week OHLC packages
//   - compare only the aggregate high and low of the two completed weeks
//   - require both newest endpoints to migrate strictly in one direction
//   - consume the week before fallible gates; never enter late or retry
//   - follow the completed auction-range migration for one broker week
// Runtime uses MT5-native OHLC, calendar, history, and framework state only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                      = 41089;
...
bool Strategy_LoadRangeMigrationSignal(const int current_week_key,
                                       const int label_offset,
                                       int &direction,
                                       int &new_week_bars,
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_WeekAlreadyEntered(g_decision_week_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadRangeMigrationSignal(g_decision_week_key,
      ...
      if(!g_signal_valid)
         ...
         g_signal_state = "higher_high_higher_low_long";
      else if(g_signal_direction < 0)
         g_signal_state = "lower_high_lower_low_short";
      else if(g_new_high == g_parent_high ||
              g_new_low == g_parent_low)
         g_signal_state = "endpoint_equality_flat";
      else if(g_new_high < g_parent_high &&
              g_new_low > g_parent_low)
         g_signal_state = "inside_week_flat";
      else if(g_new_high > g_parent_high &&
              g_new_low < g_parent_low)
         g_signal_state = "outside_week_flat";
      else
         g_signal_state = "mixed_endpoint_migration_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"week\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_week_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"new_week_bars\":%d,\"parent_week_bars\":%d,\"new_open\":%.8f,\"new_high\":%.8f,\"new_low\":%.8f,\"new_close\":%.8f,\"parent_open\":%.8f,\"parent_high\":%.8f,\"parent_low\":%.8f,\"parent_close\":%.8f,\"state\":\"%s\"}",
                            g_decision_week_key,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41089 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41089_WTI_WRANGE_MIGRATE_MOM";
   ...
   req.reason = (g_signal_direction > 0)
      ? "WTI_WRANGE_MIGRATE_MOM_LONG"
      : "WTI_WRANGE_MIGRATE_MOM_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41089 ||
   ...
   g_attempt_state_key =
      StringFormat("QM5_41089_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41089\",\"ea\":\"wti-wrange-migrate-mom\"}");
