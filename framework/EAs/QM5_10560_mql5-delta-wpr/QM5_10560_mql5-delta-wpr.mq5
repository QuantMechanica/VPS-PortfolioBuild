...
#property description "QM5_10560 MQL5 Delta WPR Histogram Color Trend"
...
input int    strategy_wpr_fast_period   = 14;
input int    strategy_wpr_slow_period   = 30;
...
#define STRATEGY_DELTA_WPR_UP      1
#define STRATEGY_DELTA_WPR_NEUTRAL 0
#define STRATEGY_DELTA_WPR_DOWN   -1

double Strategy_WPR(const int period, const int shift)
  {
   if(period <= 1 || shift < 1)
      return EMPTY_VALUE;
...
int Strategy_DeltaWprColor(const int shift)
  {
   const double fast = Strategy_WPR(strategy_wpr_fast_period, shift);
   const double slow = Strategy_WPR(strategy_wpr_slow_period, shift);
   if(fast == EMPTY_VALUE || slow == EMPTY_VALUE)
      return STRATEGY_DELTA_WPR_NEUTRAL;

   if(slow > strategy_signal_level && fast > slow)
      return STRATEGY_DELTA_WPR_UP;
   if(slow < strategy_signal_level && fast < slow)
      return STRATEGY_DELTA_WPR_DOWN;
   return STRATEGY_DELTA_WPR_NEUTRAL;
  }

int Strategy_ColorChangeSignal()
  {
   const int color_now = Strategy_DeltaWprColor(1);
   const int color_prev = Strategy_DeltaWprColor(2);
   if(color_now == STRATEGY_DELTA_WPR_UP && color_prev != STRATEGY_DELTA_WPR_UP)
      return 1;
   if(color_now == STRATEGY_DELTA_WPR_DOWN && color_prev != STRATEGY_DELTA_WPR_DOWN)
      return -1;
   return 0;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(strategy_wpr_fast_period <= 1 || strategy_wpr_slow_period <= 1 ||
   ...
   const int signal = Strategy_ColorChangeSignal();
   if(signal == 0)
      return false;
   ...
   req.reason = (side == QM_BUY) ? "DELTA_WPR_COLOR_BLUE" : "DELTA_WPR_COLOR_ORANGE";
