...
#property description "QM5_41264 WTI Monthly Yuen20 Trimmed-Shift Trend"
...
//   - derive twenty adjacent log returns in fixed older/recent blocks of ten
//   - compare 20%-trimmed means with unequal Winsorized-variance scale
//   - follow the signed robust location shift at the inclusive score boundary
...
input int    strategy_trim_each_tail       = 2;
input int    strategy_effective_size       = 6;
input int    strategy_wvar_divisor         = 5;
input double strategy_score_floor           = 0.75;
input double strategy_min_se2               = 0.000000000000000001;
...
struct Strategy_SignalMetrics
  {
   ...
   int effective_size;
   int wvar_divisor;
   double trimmed_mean_old;
   double trimmed_mean_recent;
   double winsor_mean_old;
   double winsor_mean_recent;
   double winsor_var_old;
   double winsor_var_recent;
   double se2;
...
bool Strategy_Yuen20Block(const double &returns[],
                          const int start_index,
                          double &trimmed_mean,
                          double &winsor_mean,
                          double &winsor_variance)
  {
   trimmed_mean = 0.0;
   winsor_mean = 0.0;
   winsor_variance = 0.0;
   if(ArraySize(returns) != strategy_month_returns ||
      strategy_month_returns != 20 ||
      strategy_block_size != 10 ||
      strategy_trim_each_tail != 2 ||
      strategy_effective_size != 6 ||
      strategy_wvar_divisor != 5 ||
      (start_index != 0 && start_index != strategy_block_size))
      return false;

   double sorted[];
   if(ArrayResize(sorted, strategy_block_size) != strategy_block_size)
      return false;
   for(int index = 0; index < strategy_block_size; ++index)
     {
      const int source_index = start_index + index;
      if(source_index < 0 || source_index >= ArraySize(returns) ||
         !MathIsValidNumber(returns[source_index]))
         return false;
      sorted[index] = returns[source_index];
     }
   ArraySort(sorted);
   ...
   trimmed_mean = trimmed_sum / (double)strategy_effective_size;
   if(!MathIsValidNumber(trimmed_mean))
      return false;

   double winsorized[];
   if(ArrayResize(winsorized, strategy_block_size) != strategy_block_size)
      return false;
   double winsor_sum = 0.0;
   const int upper_index =
      strategy_block_size - strategy_trim_each_tail - 1;
   if(upper_index != 7)
      return false;
   for(int index = 0; index < strategy_block_size; ++index)
     {
      int source_index = index;
      if(index < strategy_trim_each_tail)
      ...
      if(!MathIsValidNumber(winsorized[index]) ||
         !MathIsValidNumber(winsor_sum))
         return false;
     }
   winsor_mean = winsor_sum / (double)strategy_block_size;
   if(!MathIsValidNumber(winsor_mean))
      return false;
   ...
   for(int index = 0; index < strategy_block_size; ++index)
     {
      ...
      if(!MathIsValidNumber(difference) ||
         ...
         return false;
      winsor_ss += square;
      if(!MathIsValidNumber(winsor_ss))
         return false;
     }
   winsor_variance =
      winsor_ss / (double)strategy_wvar_divisor;
   return (MathIsValidNumber(winsor_variance) &&
           winsor_variance >= 0.0);
  }

bool Strategy_Yuen20Signal(const double &closes[],
                           Strategy_SignalMetrics &metrics)
  {
   ...
   metrics.trim_each_tail = strategy_trim_each_tail;
   metrics.effective_size = strategy_effective_size;
   metrics.wvar_divisor = strategy_wvar_divisor;
   metrics.direction = 0;
   ...
   if(strategy_month_returns != 20 ||
      ...
      strategy_effective_size != 6 ||
      strategy_wvar_divisor != 5 ||
      MathAbs(strategy_score_floor - 0.75) > 0.000000000001 ||
      MathAbs(strategy_min_se2 - 0.000000000000000001) >
         0.000000000000000000000001 ||
      metrics.endpoint_count != strategy_month_returns + 1 ||
      strategy_block_size * 2 != strategy_month_returns ||
      strategy_block_size - 2 * strategy_trim_each_tail !=
         strategy_effective_size ||
      strategy_effective_size - 1 != strategy_wvar_divisor)
      return false;
   ...
   if(!Strategy_Yuen20Block(returns,
                            0,
                            metrics.trimmed_mean_old,
                            metrics.winsor_mean_old,
                            metrics.winsor_var_old) ||
      !Strategy_Yuen20Block(returns,
                            strategy_block_size,
                            metrics.trimmed_mean_recent,
                            metrics.winsor_mean_recent,
                            metrics.winsor_var_recent))
      return false;

   metrics.se2 =
      metrics.winsor_var_old / (double)strategy_effective_size +
      metrics.winsor_var_recent / (double)strategy_effective_size;
   metrics.oldest_close = closes[0];
   metrics.newest_close = closes[metrics.endpoint_count - 1];
   if(!MathIsValidNumber(metrics.trimmed_mean_old) ||
      !MathIsValidNumber(metrics.trimmed_mean_recent) ||
      !MathIsValidNumber(metrics.winsor_mean_old) ||
      !MathIsValidNumber(metrics.winsor_mean_recent) ||
      !MathIsValidNumber(metrics.winsor_var_old) ||
      !MathIsValidNumber(metrics.winsor_var_recent) ||
      !MathIsValidNumber(metrics.se2) ||
      metrics.winsor_var_old < 0.0 ||
      metrics.winsor_var_recent < 0.0 || metrics.se2 < 0.0)
      return false;

   // A degenerate or effectively zero unequal-Winsorized denominator is a
   // valid flat decision. No pooled, untrimmed, or fallback scale is allowed.
   if(metrics.se2 <= strategy_min_se2)
      return true;

   metrics.standard_error = MathSqrt(metrics.se2);
   if(metrics.standard_error <= 0.0 ||
      !MathIsValidNumber(metrics.standard_error))
      return false;
   metrics.score =
      (metrics.trimmed_mean_recent - metrics.trimmed_mean_old) /
      metrics.standard_error;
   if(!MathIsValidNumber(metrics.score))
      return false;

   if(metrics.score >= strategy_score_floor)
      metrics.direction = 1;
   else if(metrics.score <= -strategy_score_floor)
      metrics.direction = -1;
   return true;
  }
                                        ...
                                        int &direction)
  {
   ...
   const bool valid =
      Strategy_LoadMonthlyEndpoints(current_month_key,
                                    ...
                                    closes,
                                    endpoint_times) &&
      Strategy_Yuen20Signal(closes, metrics);
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
   ...
   else
     {
      ...
      g_signal_valid =
         endpoints_valid &&
         Strategy_Yuen20Signal(closes, g_signal_metrics);
       if(!g_signal_valid)
          g_signal_state = "yuen20_arithmetic_validation_failed";
       else if(g_signal_metrics.direction == 0)
          g_signal_state = "yuen20_unqualified_or_degenerate_flat";
       else if(g_signal_metrics.direction > 0)
          g_signal_state = "yuen20_positive_shift_wti_long";
       else if(g_signal_metrics.direction < 0)
          g_signal_state = "yuen20_negative_shift_wti_short";
      else
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"endpoint_count\":%d,\"return_count\":%d,\"block_size\":%d,\"trim_each_tail\":%d,\"effective_size\":%d,\"wvar_divisor\":%d,\"trimmed_mean_old\":%.12f,\"trimmed_mean_recent\":%.12f,\"winsor_mean_old\":%.12f,\"winsor_mean_recent\":%.12f,\"winsor_var_old\":%.12f,\"winsor_var_recent\":%.12f,\"se2\":%.12f,\"standard_error\":%.12f,\"score\":%.12f,\"returns\":\"%s\",\"oldest_close\":%.10f,\"newest_close\":%.10f,\"oldest_endpoint\":%I64d,\"newest_endpoint\":%I64d,\"state\":\"%s\"}",
                            ...
                            g_signal_metrics.endpoint_count,
                             ...
                             g_signal_metrics.trim_each_tail,
                             g_signal_metrics.effective_size,
                             g_signal_metrics.wvar_divisor,
                             g_signal_metrics.trimmed_mean_old,
                             g_signal_metrics.trimmed_mean_recent,
                             g_signal_metrics.winsor_mean_old,
                             g_signal_metrics.winsor_mean_recent,
                             g_signal_metrics.winsor_var_old,
                             g_signal_metrics.winsor_var_recent,
                            g_signal_metrics.se2,
                            g_signal_metrics.standard_error,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41264 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41264_WTI_MYUEN20_SHIFT";
   ...
   req.reason = (g_signal_metrics.direction > 0)
      ? "WTI_MYUEN20_UPSHIFT_LONG"
      : "WTI_MYUEN20_DOWNSHIFT_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41264 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI fixed-block Yuen20 trimmed-shift trend holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41264_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41264\",\"ea\":\"wti-myuen20-shift-tr\"}");
