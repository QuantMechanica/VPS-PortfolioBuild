...
#property description "QM5_41201 WTI Five-Year Same-Calendar Hodges-Lehmann Seasonality"
...
//   - form every inclusive pair average, require 15, sort, select index 7
...
int      g_signal_pair_count      = 0;
double   g_signal_pair_min        = 0.0;
double   g_signal_pair_max        = 0.0;
double   g_signal_hl              = 0.0;
...
void Strategy_ResetDecisionState()
  {
   ...
   g_signal_pair_count = 0;
   g_signal_pair_min = 0.0;
   g_signal_pair_max = 0.0;
   g_signal_hl = 0.0;
   g_signal_state = "idle";
  }
...
// -----------------------------------------------------------------------------
// Exact completed-month endpoint and Hodges-Lehmann helpers.
// -----------------------------------------------------------------------------
...
bool Strategy_HodgesLehmannSignal(const double &observations[],
                                  ...
                                  int &direction)
  {
   ...
   double pair_averages[];
   if(ArrayResize(pair_averages, strategy_required_pair_averages) !=
      strategy_required_pair_averages)
      return false;

   for(int left = 0; left < sample_count; ++left)
     {
      for(int right = left; right < sample_count; ++right)
        {
         ...
         if(!MathIsValidNumber(pair_average))
            return false;
         pair_averages[pair_count] = pair_average;
         ++pair_count;
        }
     }
   ...
   pair_min = pair_averages[0];
   pair_max = pair_averages[pair_count - 1];
   hl_value = pair_averages[pair_count / 2];
   if(!MathIsValidNumber(pair_min) ||
      !MathIsValidNumber(pair_max) ||
      !MathIsValidNumber(hl_value))
      return false;

   if(hl_value > strategy_signal_epsilon)
      direction = 1;
   else if(hl_value < -strategy_signal_epsilon)
      direction = -1;
   return true;
  }

bool Strategy_LoadHodgesLehmannSignal(const datetime decision_bar_time,
                                      ...
                                      int &direction)
  {
   ...
   return Strategy_HodgesLehmannSignal(observations,
                                       sample_count,
                                       pair_count,
                                       pair_min,
                                       pair_max,
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   // Consume before history, pair arithmetic, news, spread, quote, ATR,
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadHodgesLehmannSignal(g_decision_bar_time,
                                          ...
                                          g_signal_direction);
      if(!g_signal_valid)
         g_signal_state = "endpoint_exact_year_sample_pair_sort_or_hl_failed";
      else if(g_signal_direction > 0)
         g_signal_state = "hodges_lehmann_long";
      else if(g_signal_direction < 0)
         g_signal_state = "hodges_lehmann_short";
      else
         g_signal_state = "hodges_lehmann_tie_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"valid\":%s,\"samples\":%d,\"pairs\":%d,\"pair_min\":%.12g,\"pair_max\":%.12g,\"hl\":%.12g,\"direction\":%d,\"state\":\"%s\"}",
                            g_decision_month_key,
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(!QM_InputRequireLong("qm_ea_id", qm_ea_id, 41201) ||
      ...
      !QM_InputRequireLong("strategy_required_pair_averages", strategy_required_pair_averages, 15) ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(g_signal_direction > 0)
     {
      req.type = QM_BUY;
      req.reason = "WTI_SAMECAL_HL5_LONG";
     }
   else
     {
      req.type = QM_SELL;
      req.reason = "WTI_SAMECAL_HL5_SHORT";
     }
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41201 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI five-year same-calendar Hodges-Lehmann card holds to the next broker month"))
      return INIT_FAILED;
   ...
   g_attempt_state_key =
      StringFormat("QM5_41201_SAMECAL_HL5_MONTH_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41201\",\"ea\":\"wti-samecal-hl5\"}");
