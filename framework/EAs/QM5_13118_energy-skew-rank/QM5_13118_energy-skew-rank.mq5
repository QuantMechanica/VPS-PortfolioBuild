...
#property description "QM5_13118 XTI XNG Commodity Skewness Rank"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_13118 - XTI/XNG Commodity-Skewness Rank
// -----------------------------------------------------------------------------
// Monthly market-neutral energy basket:
//   - calculate each leg's Pearson skewness from the prior 12 completed months
//     of D1 log returns
//   - long the lower-skew energy leg and short the higher-skew leg
// Runtime is Darwinex-native D1 OHLC only; no external or futures-chain data.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 13118;
...
double   g_cache_xti_skew = 0.0;
double   g_cache_xng_skew = 0.0;
double   g_cache_skew_difference = 0.0;
...
bool Strategy_PearsonSkewness(const string symbol,
                              ...
                              int &observation_count)
  {
   ...
   const int requested = MathMax(strategy_history_bars, strategy_min_return_observations + 40);
   ...
   for(int i = count - 1; i >= 0; --i)
     {
      ...
      if(bar_time < window_start || bar_time >= window_end)
         continue;
      ...
      if(previous_close > 0.0)
        {
         const double value = MathLog(close_price / previous_close);
   ...
   for(int i = 0; i < observation_count; ++i)
     {
      ...
      second_moment += centered * centered;
      third_moment += centered * centered * centered;
     }
   second_moment /= (double)observation_count;
   third_moment /= (double)observation_count;
   if(second_moment <= 1.0e-16 || !MathIsValidNumber(second_moment) ||
      !MathIsValidNumber(third_moment))
      return false;

   const double denominator = MathPow(second_moment, 1.5);
   if(denominator <= 0.0 || !MathIsValidNumber(denominator))
      return false;
   skewness = third_moment / denominator;
...
bool Strategy_LoadSignalState(const datetime decision_bar_time,
  ...
  {
   pair_direction = 0;
   g_cache_xti_skew = 0.0;
   g_cache_xng_skew = 0.0;
   g_cache_skew_difference = 0.0;
   ...
   if(!Strategy_FormationWindow(decision_bar_time, window_start, window_end))
      return false;
   if(!Strategy_PearsonSkewness(g_leg_xti, window_start, window_end,
                                g_cache_xti_skew, g_cache_xti_observations))
      return false;
   if(!Strategy_PearsonSkewness(g_leg_xng, window_start, window_end,
                                g_cache_xng_skew, g_cache_xng_observations))
      return false;

   g_cache_skew_difference = g_cache_xti_skew - g_cache_xng_skew;
   if(!MathIsValidNumber(g_cache_skew_difference))
      return false;
   if(g_cache_skew_difference < -1.0e-12)
      pair_direction = 1;
   else if(g_cache_skew_difference > 1.0e-12)
      pair_direction = -1;
   return true;
  }
...
bool Strategy_OpenPair(const int pair_direction)
  {
   ...
   const string reason = long_xti_short_xng ? "QM5_13118_LONG_XTI_SHORT_XNG_LOWSKEW"
                                            : "QM5_13118_SHORT_XTI_LONG_XNG_LOWSKEW";
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(strategy_history_bars < 400 ||
      ...
      strategy_history_bars <= strategy_min_return_observations)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_13118_ENERGY_SKEW_RANK_HOST";
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_13118\",\"ea\":\"energy-skew-rank\"}");
