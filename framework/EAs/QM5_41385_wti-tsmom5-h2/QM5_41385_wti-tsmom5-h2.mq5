...
#property description "QM5_41385 WTI Five-Month Trend / Two-Month Hold"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41385 - WTI Five-Month Trend / Two-Month Hold
// -----------------------------------------------------------------------------
...
// The exact five-month endpoint pair remains independent of the two-month clock.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41385;
...
double g_cache_five_month_return   = 0.0;
...
bool Strategy_FiveMonthReturnSignal(const double &closes[],
                                   int &signal)
  {
   ...
   const double five_month_return = MathLog(endpoint_ratio);
   if(!MathIsValidNumber(five_month_return))
     {
      g_cache_state_reason = "invalid_five_month_return";
      return false;
     }
   ...
   if(!MathIsValidNumber(chained_return) ||
      MathAbs(five_month_return - chained_return) > 1.0e-10)
     {
      g_cache_state_reason = "endpoint_path_identity_failed";
      return false;
     }

   g_cache_five_month_return = five_month_return;
   if(five_month_return == 0.0)
     {
      g_cache_state_reason = "exact_zero_five_month_return";
      return true;
     }

   signal = (five_month_return > 0.0) ? 1 : -1;
...
void Strategy_PrepareBimonthlySignal()
  {
   ...
   QM_LogEvent(QM_INFO,
               "MONTHLY_STATE",
               StringFormat("{\"month\":%d,\"valid\":%s,\"signal\":%d,\"return_5m\":%.10f,\"state\":\"%s\"}",
                            g_cache_month_key,
                            g_cache_signal_valid ? "true" : "false",
                            g_cache_signal,
                            g_cache_five_month_return,
                            g_cache_state_reason));
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41385_WTI_TSMOM5_H2";
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41385_BIMONTH_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41385\",\"ea\":\"wti-tsmom5-h2\"}");
