...
#property description "QM5_41180 XTI/XNG Thirteen-Month Spearman Ratio-Rank Reversion"
...
// completed broker months. Strict-rank the oil-minus-gas log ratios,
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XTIXNG_MSPEARMAN_RV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41180_XTIXNG_MSPEARMAN_RV";
...
bool Strategy_LoadMonthlySpearman(
      ...
      int &direction)
  {
   ...
   // direction > 0 means BUY XTI / SELL XNG. Fade the qualified ratio-rank
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41180_XTIXNG_MSPEARMAN_RV_HOST";
   ...
   if(valid && direction > 0)
      state = "negative_rank_trend_fade_long_xti";
   else if(valid && direction < 0)
      state = "positive_rank_trend_fade_short_xti";
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XTI/XNG thirteen-month Spearman ratio-rank reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41180\",\"ea\":\"xtixng-mspearman-rv\"}");
