...
#property description "QM5_41054 XNG Post-Thursday Counter-Gap Fade"
...
//   - require exact completed Tuesday-Wednesday-Thursday broker sessions
//   - measure completed Thursday event-session flow and the later frozen
//     Thursday-close-to-Friday-open gap
//   - require strict opposition with event-session dominance, then fade the counter-gap
//   - consume one exact-Friday attempt before every fallible entry gate
//   - flatten at framework Friday hour 21
//   - repair malformed or stale survivors behind one frozen ATR hard stop
// Native MT5 calendar/OHLC/history only; no external runtime data.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                     = 41054;
...
bool Strategy_PrimeLateSignalAttach()
  {
   ...
   // Consume the initialization edge and persist the missed Friday
...
bool Strategy_LoadPostThursdayCounterGapFade(const datetime current_bar_time,
                                            const datetime broker_now,
                                            double &event_session_flow,
                                            double &post_event_gap,
                                            double &confirmed_path,
                                            double &total_flow,
                                            int &direction)
  {
   event_session_flow = 0.0;
   post_event_gap = 0.0;
   confirmed_path = 0.0;
   total_flow = 0.0;
   direction = 0;
   ...
   // Newest first: Thursday, Wednesday, Tuesday. Apply the current bar's one
   ...
   const int expected_days[3] = {4, 3, 2};
   ...
   // The only signal endpoints are the completed Thursday open/close and
   // the current D1 bar's frozen opening price. Wednesday and Tuesday prove the
   // exact ordinary-week sequence but supply no price to the signal.
   if(bars[0].open <= 0.0 ||
      !MathIsValidNumber(bars[0].open) ||
      bars[0].close <= 0.0 ||
      !MathIsValidNumber(bars[0].close) ||
      current_bar.open <= 0.0 ||
   ...
   post_event_gap =
      MathLog(current_bar.open / bars[0].close);
   confirmed_path =
      MathLog(current_bar.open / bars[0].open);
   total_flow = event_session_flow + post_event_gap;
   if(!MathIsValidNumber(event_session_flow) ||
      !MathIsValidNumber(post_event_gap) ||
      !MathIsValidNumber(confirmed_path) ||
      !MathIsValidNumber(total_flow))
      return false;

   if(MathAbs(total_flow - confirmed_path) >
      strategy_reconcile_tolerance)
      return false;

   const bool signs_oppose =
      ((event_session_flow > 0.0 && post_event_gap < 0.0) ||
       (event_session_flow < 0.0 && post_event_gap > 0.0));
   if(!signs_oppose)
      return true;

   const bool event_session_dominates =
      (MathAbs(event_session_flow) > MathAbs(post_event_gap));
   if(!event_session_dominates)
      return true;

   // Fade only the counter-gap by following the still-dominant event-session
   // sign; magnitude never changes risk.
   if(event_session_flow > 0.0)
      direction = 1;
   else if(event_session_flow < 0.0)
      direction = -1;
   return true;
  }
   ...
   for(int index = PositionsTotal() - 1; index >= 0; --index)
     {
      ...
      // Framework Friday hour 21 is the ordinary exit. The first observable
      // later D1 session is a deterministic repair for any survivor.
      if(!should_close &&
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(!QM_InputRequireLong("qm_ea_id", qm_ea_id, 41054) ||
      ...
      !QM_InputRequireLong("strategy_max_hold_days", strategy_max_hold_days, 4) ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "XNG_POSTTHU_GAP_FADE";
   ...
   // order gates. A blocked Friday attempt cannot retry after restart.
   ...
   double event_session_flow = 0.0;
   double post_event_gap = 0.0;
   double confirmed_path = 0.0;
   double total_flow = 0.0;
   int direction = 0;
   if(!Strategy_LoadPostThursdayCounterGapFade(g_strategy_d1_bar_time,
                                              broker_now,
                                              event_session_flow,
                                              post_event_gap,
                                              confirmed_path,
   ...
   if(direction > 0)
     {
      req.type = QM_BUY;
      req.reason = "XNG_POSTTHU_GAP_FADE_LONG";
     }
   else if(direction < 0)
     {
      req.type = QM_SELL;
      req.reason = "XNG_POSTTHU_GAP_FADE_SHORT";
     }
   else
      return false;
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"date\":%d,\"event_session_flow\":%.12e,\"post_event_gap\":%.12e,\"confirmed_path\":%.12e,\"total_flow\":%.12e,\"direction\":%d}",
                            date_key,
                            event_session_flow,
                            post_event_gap,
                            confirmed_path,
...
int OnInit()
  {
   if(!SymbolSelect(_Symbol, true))
     {
      Print("QM_INPUT_REJECT predicate=SymbolSelect observed=false required=true symbol='XNGUSD.DWX'");
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XNG post-Thursday counter-gap-fade card exits Friday 21; later D1 is repair"))
      return INIT_FAILED;
   ...
   g_attempt_state_key =
      StringFormat("QM5_41054_POSTTHU_GAP_FADE_FRIDAY_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41054\",\"ea\":\"xng-postthu-gap-fade\"}");
...
void OnTick()
  {
   ...
   // EntrySignal deliberately consumes the exact Friday before this entry-
