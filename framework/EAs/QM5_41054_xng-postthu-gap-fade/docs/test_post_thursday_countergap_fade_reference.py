"""Independent mechanic fixtures for QM5_41054.
...
cross-boundary sign opposition plus event-session dominance, return
reconciliation, counter-gap-fade direction, Friday grace/attempt identity,
the broker-hour-21 exit, and later-D1 repair. It does not invoke MT5 or
...
def select_direction(
    ...
    if abs(total_flow - confirmed_path) > tolerance:
        return 0
    signs_oppose = (event_session_flow > 0 and post_event_gap < 0) or (
        event_session_flow < 0 and post_event_gap > 0
    )
    if not signs_oppose:
        return 0
    if abs(event_session_flow) <= abs(post_event_gap):
        return 0
    return (event_session_flow > 0) - (event_session_flow < 0)
...
class PostThursdayCounterGapFadeReferenceTests(unittest.TestCase):
    ...
    def test_positive_event_dominant_countergap_is_bought(self) -> None:
        ...
        self.assertGreater(abs(event), abs(gap))
        self.assertAlmostEqual(total, confirmed)

    def test_negative_event_dominant_countergap_is_sold(self) -> None:
        direction, event, gap, confirmed, total = post_thursday_direction(
            100.0, 96.0, 98.0
        )
        self.assertEqual(direction, -1)
    ...
    def test_agreement_and_exact_zero_are_flat(self) -> None:
        self.assertEqual(post_thursday_direction(100.0, 102.0, 103.0)[0], 0)
        self.assertEqual(post_thursday_direction(100.0, 98.0, 97.0)[0], 0)
    ...
    def test_equal_or_countergap_dominance_is_flat(self) -> None:
        self.assertEqual(post_thursday_direction(100.0, 102.0, 100.0)[0], 0)
        self.assertEqual(post_thursday_direction(100.0, 102.0, 99.0)[0], 0)
        self.assertEqual(post_thursday_direction(100.0, 98.0, 101.0)[0], 0)

    def test_failed_reconciliation_and_invalid_arithmetic_are_flat(self) -> None:
        self.assertEqual(select_direction(0.1, -0.05, 0.04), 0)
        self.assertEqual(
            select_direction(
                0.1,
                -0.05,
                0.05 + 2 * RECONCILE_TOLERANCE,
            ),
            0,
        )
        self.assertEqual(select_direction(float("nan"), -0.05, 0.05), 0)
    ...
    def test_later_intrabar_price_is_not_a_signal_input(self) -> None:
        frozen = post_thursday_direction(100.0, 104.0, 102.0)
        # The reference function has no post-open price argument by design.
        after_large_intraday_move = post_thursday_direction(100.0, 104.0, 102.0)
