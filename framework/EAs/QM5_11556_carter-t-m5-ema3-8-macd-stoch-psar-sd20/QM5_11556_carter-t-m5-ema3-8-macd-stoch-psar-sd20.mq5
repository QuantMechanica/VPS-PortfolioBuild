...
#property description "QM5_11556 Carter-T M5 EMA3/8 MACD Stoch PSAR StdDev"
...
input double          strategy_stddev_threshold   = 0.00010;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(strategy_ema_fast <= 0 || strategy_ema_slow <= 0 ||
      ...
      strategy_stddev_period <= 0 || strategy_sl_lookback_bars <= 0 ||
      strategy_sl_cap_pips <= 0 || strategy_psar_step <= 0.0 || strategy_psar_max <= 0.0)
      return false;

   const ENUM_TIMEFRAMES tf = strategy_signal_tf;
   const double sd_1 = QM_StdDev(_Symbol, tf, strategy_stddev_period, 1, PRICE_CLOSE, MODE_SMA);
   if(sd_1 < strategy_stddev_threshold)
      return false;

   const double ema_fast_1 = QM_EMA(_Symbol, tf, strategy_ema_fast, 1, PRICE_CLOSE);
   const double ema_slow_1 = QM_EMA(_Symbol, tf, strategy_ema_slow, 1, PRICE_CLOSE);
   const double ema_fast_2 = QM_EMA(_Symbol, tf, strategy_ema_fast, 2, PRICE_CLOSE);
   const double ema_slow_2 = QM_EMA(_Symbol, tf, strategy_ema_slow, 2, PRICE_CLOSE);
   const double macd_1 = QM_MACD_Main(_Symbol, tf, strategy_macd_fast, strategy_macd_slow, strategy_macd_signal, 1, PRICE_CLOSE);
   ...
   const double stoch_d_2 = QM_Stoch_D(_Symbol, tf, strategy_stoch_k, strategy_stoch_d, strategy_stoch_slowing, 2);
...
bool Strategy_ExitSignal()
  {
   ...
   if(magic <= 0 || strategy_ema_fast <= 0 || strategy_ema_slow <= 0)
      return false;
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      if(position_type == POSITION_TYPE_BUY && ema_fast_1 < ema_slow_1 && ema_fast_2 >= ema_slow_2)
         return true;
      if(position_type == POSITION_TYPE_SELL && ema_fast_1 > ema_slow_1 && ema_fast_2 <= ema_slow_2)
         return true;
     }
