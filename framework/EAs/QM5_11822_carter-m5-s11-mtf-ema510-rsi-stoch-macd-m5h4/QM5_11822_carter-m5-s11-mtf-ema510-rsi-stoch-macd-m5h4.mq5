...
#property description "QM5_11822 carter-m5-s11-mtf-ema510-rsi-stoch-macd-m5h4 — H4 EMA5/10 trend + M5 RSI/Stoch-cross/MACD-hist confluence (M5 entry, H4 bias)"

#include <QM/QM_Common.mqh>

// =============================================================================
// QuantMechanica V5 EA — QM5_11822 carter-m5-s11-mtf-ema510-rsi-stoch-macd-m5h4
// -----------------------------------------------------------------------------
// Source: Thomas Carter, "20 Forex Trading Strategies (5 Minute Time Frame)",
//   Strategy #11 (self-published 2014, 367145560).
// Card: artifacts/cards_approved/QM5_11822_carter-m5-s11-mtf-ema510-rsi-stoch-macd-m5h4.md
//   (g0_status: APPROVED).
//
...
//   Trigger EVENT    : M5 Stochastic %K/%D cross — the SINGLE fresh event on the
//                      bar. LONG = %K crosses ABOVE %D (k@2 <= d@2 AND k@1 > d@1);
//                      SHORT = %K crosses BELOW %D. Per THIS card the entry rule
//                      lists no M5 EMA cross — the Stoch crossover is the trigger.
//   RSI STATE        : M5 RSI(period) > rsi_mid (LONG) / < rsi_mid (SHORT).
//   Stoch band STATE : %K < stoch_ob (LONG, not overbought) / %K > stoch_os
//                      (SHORT, not oversold).
//   MACD-hist STATE  : M5 histogram = Main - Signal. LONG = histogram > 0;
//                      SHORT = histogram < 0 (sign per the card).
...
//   - Only ONE fresh cross EVENT (the M5 Stoch %K/%D cross). H4/RSI/MACD-hist and
//     the Stoch over-extension band are STATES, never required to cross on the
//     same bar (rule 4: two-cross trap avoided).
...
input int    strategy_ema_fast_period        = 5;          // fast EMA (H4 trend)
input int    strategy_ema_slow_period        = 10;         // slow EMA (H4 trend)
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   // --- M5 Stochastic %K/%D — the SINGLE fresh cross EVENT ---
   ...
   if(k_1 <= 0.0 || d_1 <= 0.0 || k_2 <= 0.0 || d_2 <= 0.0)
      return false;
   const bool stoch_cross_up   = (k_2 <= d_2 && k_1 >  d_1);
   const bool stoch_cross_down = (k_2 >= d_2 && k_1 <  d_1);
   if(!stoch_cross_up && !stoch_cross_down)
   ...
   // --- M5 MACD histogram STATE (Main - Signal sign) ---
   ...
   // ----------------------------- LONG ------------------------------
   // H4 up + Stoch %K crosses above %D + %K not overbought + RSI>mid + MACD hist>0.
   if(stoch_cross_up && htf_up)
     {
      const bool rsi_ok   = (rsi_1 > strategy_rsi_mid);
      const bool stoch_ok = (k_1 < strategy_stoch_ob);   // not overbought
      const bool macd_ok  = (hist_1 > 0.0);
      if(rsi_ok && stoch_ok && macd_ok)
        {
         ...
         req.sl     = sl;
         req.tp     = tp;
         req.reason = "carter_s11_mtf_long";
         return true;
        }
      return false;
     }
   ...
   // ----------------------------- SHORT -----------------------------
   // H4 down + Stoch %K crosses below %D + %K not oversold + RSI<mid + MACD hist<0.
   if(stoch_cross_down && htf_down)
     {
      const bool rsi_ok   = (rsi_1 < strategy_rsi_mid);
      const bool stoch_ok = (k_1 > strategy_stoch_os);   // not oversold
      const bool macd_ok  = (hist_1 < 0.0);
      if(rsi_ok && stoch_ok && macd_ok)
        {
         ...
         req.sl     = sl;
         req.tp     = tp;
         req.reason = "carter_s11_mtf_short";
         return true;
        }
      return false;
     }
