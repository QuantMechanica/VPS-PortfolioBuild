...
#property description "QM5_41265 XAU/XAG Monthly Brown-Forsythe Scale Reversion"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_41265 - XAU/XAG Monthly Brown-Forsythe Scale Reversion
// -----------------------------------------------------------------------------
...
// returns into fixed old/recent blocks of six, center each block on its even
// median, and apply the Brown-Forsythe absolute-deviation ANOVA arithmetic.
// A finite, nondegenerate recent scale expansion fades the blocks' median
// shift with an opposite, equal-notional package held to the next month.
// Runtime is native, completed-price-only, deterministic, and one consumed
// attempt per month. W is diagnostic only; no F critical or p-value is used.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41265;
...
input double strategy_bf_multiplier            = 10.0;
input double strategy_min_within_ss            = 0.000000000000000001;
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41265 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
            strategy_xag_symbol == "XAGUSD.DWX" &&
            strategy_endpoint_count == 13 &&
            strategy_return_count == 12 &&
            strategy_block_size == 6 &&
            MathAbs(strategy_bf_multiplier - 10.0) <= 1.0e-12 &&
            MathAbs(strategy_min_within_ss - 1.0e-18) <= 1.0e-24 &&
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41265_XAUXAG_MBF_SCALE_RV";
...
bool Strategy_MedianSix(const double &values[], double &median)
  {
   median = 0.0;
   if(ArraySize(values) != strategy_block_size ||
      strategy_block_size != 6)
      return false;
   ...
   for(int index = 1; index < strategy_block_size; ++index)
     {
   ...
   median = 0.5 * (sorted[2] + sorted[3]);
   return MathIsValidNumber(median);
  }

bool Strategy_BrownForsytheSignal(
      const double &relative_returns[],
      double &old_median,
      double &recent_median,
      double &old_z_mean,
      double &recent_z_mean,
      double &grand_z_mean,
      double &ss_between,
      double &ss_within,
      double &w_statistic,
      int &direction)
  {
   ...
   double old_values[6];
   double recent_values[6];
   double old_z[6];
   double recent_z[6];
   for(int index = 0; index < strategy_block_size; ++index)
     {
      old_values[index] = relative_returns[index];
      recent_values[index] = relative_returns[index + strategy_block_size];
      if(!MathIsValidNumber(old_values[index]) ||
         !MathIsValidNumber(recent_values[index]))
         return false;
     }
   if(!Strategy_MedianSix(old_values, old_median) ||
      !Strategy_MedianSix(recent_values, recent_median))
      return false;

   double old_z_sum = 0.0;
   double recent_z_sum = 0.0;
   for(int index = 0; index < strategy_block_size; ++index)
     {
      old_z[index] = MathAbs(old_values[index] - old_median);
      recent_z[index] = MathAbs(recent_values[index] - recent_median);
      if(!MathIsValidNumber(old_z[index]) ||
         !MathIsValidNumber(recent_z[index]))
         return false;
      old_z_sum += old_z[index];
      recent_z_sum += recent_z[index];
     }
   old_z_mean = old_z_sum / (double)strategy_block_size;
   recent_z_mean = recent_z_sum / (double)strategy_block_size;
   grand_z_mean = 0.5 * (old_z_mean + recent_z_mean);
   if(!MathIsValidNumber(old_z_mean) ||
      !MathIsValidNumber(recent_z_mean) ||
      !MathIsValidNumber(grand_z_mean))
      return false;

   const double old_group_delta = old_z_mean - grand_z_mean;
   const double recent_group_delta = recent_z_mean - grand_z_mean;
   ss_between = (double)strategy_block_size *
                old_group_delta * old_group_delta +
                (double)strategy_block_size *
                recent_group_delta * recent_group_delta;
   for(int index = 0; index < strategy_block_size; ++index)
     {
      const double old_delta = old_z[index] - old_z_mean;
      const double recent_delta = recent_z[index] - recent_z_mean;
      ss_within += old_delta * old_delta + recent_delta * recent_delta;
     }
   if(!MathIsValidNumber(ss_between) || !MathIsValidNumber(ss_within) ||
      ss_between < 0.0 || ss_within < 0.0)
      return false;
   if(ss_within <= strategy_min_within_ss)
      return true; // A valid but degenerate scale state consumes the month flat.

   w_statistic = strategy_bf_multiplier * ss_between / ss_within;
   if(!MathIsValidNumber(w_statistic) || w_statistic < 0.0)
      return false;

   const double scale_tolerance =
      strategy_relative_epsilon *
      MathMax(1.0, MathMax(MathAbs(old_z_mean), MathAbs(recent_z_mean)));
   if(!MathIsValidNumber(scale_tolerance) ||
      recent_z_mean <= old_z_mean + scale_tolerance)
      return true;

   const double median_delta = recent_median - old_median;
   const double location_tolerance =
      strategy_relative_epsilon *
      MathMax(1.0, MathMax(MathAbs(old_median), MathAbs(recent_median)));
   if(!MathIsValidNumber(median_delta) ||
      !MathIsValidNumber(location_tolerance))
      return false;
   if(median_delta > location_tolerance)
      direction = -1; // SELL XAU / BUY XAG.
   else if(median_delta < -location_tolerance)
      direction = 1;  // BUY XAU / SELL XAG.
   return true;
  }

bool Strategy_LoadMonthlyBrownForsytheSignal(
      ...
      int &direction)
  {
   ...
   return Strategy_BrownForsytheSignal(relative_returns,
                                       old_median,
                                       recent_median,
                                       old_z_mean,
                                       recent_z_mean,
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41265_XAUXAG_MBF_SCALE_RV_HOST";
   ...
   double old_z_mean = 0.0;
   double recent_z_mean = 0.0;
   double grand_z_mean = 0.0;
   ...
   string return_path = "";
   const bool valid =
      Strategy_LoadMonthlyBrownForsytheSignal(
         g_signal_month_key,
         month_count,
   ...
   string state = "monthly_brown_forsythe_signal_failed";
   if(valid && direction > 0)
      state = "qualified_recent_scale_fade_long_xau";
   else if(valid && direction < 0)
      state = "qualified_recent_scale_fade_short_xau";
   else if(valid && ss_within <= strategy_min_within_ss)
      state = "degenerate_within_ss_flat";
   else if(valid && recent_z_mean <= old_z_mean + scale_tolerance)
      state = "recent_scale_not_expanded_flat";
   else if(valid && MathAbs(median_delta) <= location_tolerance)
      state = "neutral_median_shift_flat";
   else if(valid)
      state = "nonqualifying_brown_forsythe_flat";

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"month_count\":%d,\"newest_endpoint_time\":%I64d,\"old_median\":%.12e,\"recent_median\":%.12e,\"old_z_mean\":%.12e,\"recent_z_mean\":%.12e,\"grand_z_mean\":%.12e,\"ss_between\":%.12e,\"ss_within\":%.12e,\"w\":%.12e,\"direction\":%d,\"endpoints\":\"%s\",\"ratios\":\"%s\",\"returns\":\"%s\",\"state\":\"%s\"}",
                            g_signal_month_key,
                            month_count,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG monthly Brown-Forsythe scale reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41265\",\"ea\":\"xauxag-mbf-scale-rv\"}");
