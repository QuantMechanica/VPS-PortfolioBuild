"""Deterministic reference checks for QM5_41368 leader-switch reversion."""
...
MQ5 = EA_DIR / "QM5_41368_xtixng-cs-leadswitch-rv.mq5"
SETFILE = (
    EA_DIR
    / "sets"
    / "QM5_41368_xtixng-cs-leadswitch-rv_QM5_41368_XTI_XNG_CS_LEADSWITCH_RV_D1_D1_backtest.set"
)
...
def leader_switch_signal(
    ...
    if older_same and newer_same and d0 < -epsilon and d1 > epsilon:
        direction = -1  # New WTI winner is faded: sell XTI, buy XNG.
    elif older_same and newer_same and d0 > epsilon and d1 < -epsilon:
        direction = 1  # New XNG winner is faded: buy XTI, sell XNG.
...
class LeaderSwitchReferenceTest(unittest.TestCase):
    def test_positive_common_shocks_rotate_from_wti_to_xng_and_fade_xng(self) -> None:
    ...
    def test_positive_common_shocks_rotate_from_xng_to_wti_and_fade_wti(self) -> None:
    ...
    def test_static_artifacts_lock_identity_signal_and_fixed_risk(self) -> None:
        ...
        self.assertIn("qm_ea_id                    = 41368", source)
        ...
        self.assertRegex(resolver, re.compile(r"413680000.*413680001", re.DOTALL))
