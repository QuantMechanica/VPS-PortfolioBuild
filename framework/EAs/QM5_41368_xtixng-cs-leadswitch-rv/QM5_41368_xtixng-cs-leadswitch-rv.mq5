...
#property description "QM5_41368 XTI/XNG Common-Shock Leader-Switch Reversion"
...
// returns in both intervals and a strict rotation of the relative winner, then
// fade the newest winner with an equal-notional opposite-leg package. Consume
// one attempt per week and flatten at the next week boundary.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41368;
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41368 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XTIXNG_CS_LEADSWITCH_ATTEMPT_WEEK", qm_ea_id);
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41368_XTIXNG_CS_LEADSWITCH_RV";
...
bool Strategy_LoadLeaderSwitch(const int current_week,
                                    int &direction)
  {
   direction = 0;
   ...
   if(older_same_sign && newer_same_sign &&
      ...
      direction = -1; // WTI became the winner: sell XTI, buy XNG.
   else if(older_same_sign && newer_same_sign &&
           g_older_relative_return > strategy_signal_epsilon &&
           g_newer_relative_return < -strategy_signal_epsilon)
      direction = 1; // natural gas became the winner: buy XTI, sell XNG.
   return true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41368_XTIXNG_CS_LEADSWITCH_RV_HOST";
   ...
   if(Strategy_OpenOwnedPositionCount() > 0 ||
     ...
     {
      signal_valid =
         Strategy_LoadLeaderSwitch(g_signal_week_key, direction);
      if(!signal_valid)
         g_signal_state = "endpoint_validation_failed";
      else if(direction < 0)
         g_signal_state = "wti_newest_winner_faded_short_xti";
      else if(direction > 0)
         g_signal_state = "xng_newest_winner_faded_long_xti";
         ...
         g_signal_state = "no_strict_leader_switch_flat";
     }
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XTI/XNG common-shock leader-switch package exits at the next broker week"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41368\",\"ea\":\"xtixng-cs-leadswitch-rv\"}");
