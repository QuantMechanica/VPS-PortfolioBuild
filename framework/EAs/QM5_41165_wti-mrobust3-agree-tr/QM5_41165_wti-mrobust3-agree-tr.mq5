...
#property description "QM5_41165 WTI Monthly Robust-Three Consensus Trend"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41165 - WTI Monthly Robust-Three Consensus Trend
// -----------------------------------------------------------------------------
// D1 structural crude-oil sleeve:
//   - reconstruct thirteen consecutive completed broker-month-end closes
//   - compute exact Theil-Sen, LAD, and Siegel repeated-median slopes
//   - follow WTI only when all three final slopes have one strict sign
...
input double strategy_loss_tie_tolerance   = 1.0e-12;
...
struct Strategy_SignalMetrics
  {
   int    direction;
   int    endpoint_count;
   int    pair_slope_count;
   int    lad_objective_count;
   int    lad_minimizer_count;
   int    repeated_pivot_count;
   int    repeated_grouped_slope_count;
   double theilsen_slope;
   double lad_intercept;
   double lad_minimum_loss;
   double lad_slope;
   double repeated_median_slope;
  };
...
int Strategy_StrictSign(const double value)
  {
...
bool Strategy_LoadMonthlyEndpoints(const int current_month_key,
                                   ...
                                   datetime &endpoint_times[])
  {
   ...
   if(ArrayResize(reverse_closes, strategy_price_points) !=
         strategy_price_points ||
      ArrayResize(reverse_times, strategy_price_points) !=
         strategy_price_points ||
      ArrayResize(reverse_month_keys, strategy_price_points) !=
         strategy_price_points)
      return false;
   ...
   for(int index = 0;
       index < copied && endpoint_count < strategy_price_points;
       ++index)
     {
   ...
   if(endpoint_count != strategy_price_points || endpoint_count != 13)
      return false;
...
bool Strategy_RobustConsensusSignal(const double &closes[],
                                    Strategy_SignalMetrics &metrics)
  {
   ZeroMemory(metrics);
   metrics.endpoint_count = ArraySize(closes);

   const int point_count = strategy_price_points;
   const int expected_pairs = point_count * (point_count - 1) / 2;
   const int expected_grouped = point_count * (point_count - 1);
   if(point_count != 13 || metrics.endpoint_count != point_count ||
      expected_pairs != 78 || expected_grouped != 156 ||
      strategy_loss_tie_tolerance != 1.0e-12)
      return false;

   double log_prices[];
   if(ArrayResize(log_prices, point_count) != point_count)
      return false;
   for(int index = 0; index < point_count; ++index)
     {
      if(closes[index] <= 0.0 || !MathIsValidNumber(closes[index]))
         return false;
      log_prices[index] = MathLog(closes[index]);
      if(!MathIsValidNumber(log_prices[index]))
         return false;
     }

   double pair_slopes[];
   if(ArrayResize(pair_slopes, expected_pairs) != expected_pairs)
      return false;
   for(int older = 0; older < point_count - 1; ++older)
     {
      for(int newer = older + 1; newer < point_count; ++newer)
        {
         ...
         if(!MathIsValidNumber(slope))
            return false;
         pair_slopes[metrics.pair_slope_count] = slope;
         ++metrics.pair_slope_count;
        }
     }
   if(metrics.pair_slope_count != expected_pairs)
      return false;

   // Theil-Sen: central two of the exact 78 pooled forward slopes.
   double sorted_pairs[];
   if(ArrayResize(sorted_pairs, expected_pairs) != expected_pairs)
      return false;
   for(int index = 0; index < expected_pairs; ++index)
      sorted_pairs[index] = pair_slopes[index];
   ArraySort(sorted_pairs);
   for(int index = 0; index < expected_pairs; ++index)
     {
      if(!MathIsValidNumber(sorted_pairs[index]) ||
         (index > 0 && sorted_pairs[index] < sorted_pairs[index - 1]))
         return false;
     }
   metrics.theilsen_slope =
      sorted_pairs[38] / 2.0 + sorted_pairs[39] / 2.0;
   if(!MathIsValidNumber(metrics.theilsen_slope))
      return false;

   // LAD: profile the median residual intercept for every pair-slope
   // breakpoint and minimize chronological total absolute vertical loss.
   double candidate_losses[];
   if(ArrayResize(candidate_losses, expected_pairs) != expected_pairs)
      return false;
   for(int candidate = 0; candidate < expected_pairs; ++candidate)
     {
      const double slope = pair_slopes[candidate];
      double residuals[];
      if(ArrayResize(residuals, point_count) != point_count)
         return false;
      for(int index = 0; index < point_count; ++index)
        {
         residuals[index] =
            log_prices[index] - slope * (double)index;
      ...
      const double intercept = residuals[6];
      ...
      for(int index = 0; index < point_count; ++index)
        {
         const double error =
            log_prices[index] - intercept - slope * (double)index;
      ...
      if(metrics.lad_objective_count == 0 ||
         loss < metrics.lad_minimum_loss)
         metrics.lad_minimum_loss = loss;
      ++metrics.lad_objective_count;
     }
   if(metrics.lad_objective_count != expected_pairs ||
      !MathIsValidNumber(metrics.lad_minimum_loss) ||
      metrics.lad_minimum_loss < 0.0)
      return false;

   double minimizers[];
   if(ArrayResize(minimizers, expected_pairs) != expected_pairs)
      return false;
   for(int candidate = 0; candidate < expected_pairs; ++candidate)
     {
      if(MathAbs(candidate_losses[candidate] -
                 metrics.lad_minimum_loss) <=
         strategy_loss_tie_tolerance)
        {
         if(metrics.lad_minimizer_count < 0 ||
            metrics.lad_minimizer_count >= expected_pairs)
            return false;
         minimizers[metrics.lad_minimizer_count] =
            pair_slopes[candidate];
         ++metrics.lad_minimizer_count;
        }
     }
   if(metrics.lad_minimizer_count <= 0 ||
      ArrayResize(minimizers, metrics.lad_minimizer_count) !=
         metrics.lad_minimizer_count)
      return false;
   ...
   if((metrics.lad_minimizer_count % 2) == 1)
     ...
     {
      if(minimizer_center <= 0 ||
         minimizer_center >= metrics.lad_minimizer_count)
         return false;
      metrics.lad_slope =
         minimizers[minimizer_center - 1] / 2.0 +
         minimizers[minimizer_center] / 2.0;
     }
   if(!MathIsValidNumber(metrics.lad_slope))
      return false;

   double final_residuals[];
   if(ArrayResize(final_residuals, point_count) != point_count)
      return false;
   for(int index = 0; index < point_count; ++index)
     {
      final_residuals[index] =
         log_prices[index] - metrics.lad_slope * (double)index;
      if(!MathIsValidNumber(final_residuals[index]))
         return false;
     }
   ArraySort(final_residuals);
   metrics.lad_intercept = final_residuals[6];
   if(!MathIsValidNumber(metrics.lad_intercept))
      return false;
   ...
   for(int index = 0; index < point_count; ++index)
     {
      const double term =
         MathAbs(log_prices[index] - metrics.lad_intercept -
                 metrics.lad_slope * (double)index);
   ...
   if(MathAbs(final_loss - metrics.lad_minimum_loss) >
      strategy_loss_tie_tolerance)
      return false;

   // Siegel repeated median: one even median inside every endpoint pivot,
   // followed by the odd median of the thirteen pivot medians.
   double pivot_medians[];
   if(ArrayResize(pivot_medians, point_count) != point_count)
      return false;
   for(int pivot = 0; pivot < point_count; ++pivot)
     {
      double pivot_slopes[];
      if(ArrayResize(pivot_slopes, point_count - 1) !=
         point_count - 1)
         return false;
      int pivot_slope_count = 0;
      for(int other = 0; other < point_count; ++other)
        {
         if(other == pivot)
            continue;
         const int older = MathMin(pivot, other);
         const int newer = MathMax(pivot, other);
         const int month_distance = newer - older;
         ...
         if(!MathIsValidNumber(slope))
            return false;
         pivot_slopes[pivot_slope_count] = slope;
         ++pivot_slope_count;
         ++metrics.repeated_grouped_slope_count;
        }
      if(pivot_slope_count != 12)
         return false;
      ...
      pivot_medians[pivot] =
         pivot_slopes[5] / 2.0 + pivot_slopes[6] / 2.0;
      if(!MathIsValidNumber(pivot_medians[pivot]))
         return false;
      ++metrics.repeated_pivot_count;
     }
   if(metrics.repeated_pivot_count != 13 ||
      metrics.repeated_grouped_slope_count != expected_grouped)
      return false;
   ArraySort(pivot_medians);
   for(int index = 0; index < point_count; ++index)
     {
   ...
   metrics.repeated_median_slope = pivot_medians[6];
   if(!MathIsValidNumber(metrics.repeated_median_slope))
      return false;

   const int theilsen_sign = Strategy_StrictSign(metrics.theilsen_slope);
   const int lad_sign = Strategy_StrictSign(metrics.lad_slope);
   const int repeated_sign =
      Strategy_StrictSign(metrics.repeated_median_slope);
   if(theilsen_sign != 0 &&
      theilsen_sign == lad_sign &&
      theilsen_sign == repeated_sign)
      metrics.direction = theilsen_sign;
   else
      metrics.direction = 0;
   return true;
  }
                                        ...
                                        int &direction)
  {
   ...
   const bool valid =
      Strategy_LoadMonthlyEndpoints(current_month_key,
                                    ...
                                    closes,
                                    endpoint_times) &&
      Strategy_RobustConsensusSignal(closes, metrics);
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
   ...
   else
     {
      ...
      if(endpoints_valid)
        {
         ...
         if(ArraySize(endpoint_times) == g_signal_metrics.endpoint_count &&
            g_signal_metrics.endpoint_count == strategy_price_points)
           {
      ...
      g_signal_valid =
         endpoints_valid &&
         Strategy_RobustConsensusSignal(closes, g_signal_metrics);
      if(!g_signal_valid)
         g_signal_state = "robust_consensus_validation_failed";
      else if(g_signal_metrics.direction == 0)
         g_signal_state = "no_unanimous_strict_sign_flat";
      else if(g_signal_metrics.direction > 0)
         g_signal_state = "robust3_wti_long";
      else if(g_signal_metrics.direction < 0)
         g_signal_state = "robust3_wti_short";
      else
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"endpoint_count\":%d,\"pair_slope_count\":%d,\"lad_objective_count\":%d,\"lad_minimizer_count\":%d,\"repeated_pivot_count\":%d,\"repeated_grouped_slope_count\":%d,\"oldest_endpoint\":%I64d,\"newest_endpoint\":%I64d,\"theilsen_slope\":%.12e,\"lad_intercept\":%.12e,\"lad_minimum_loss\":%.12e,\"lad_slope\":%.12e,\"repeated_median_slope\":%.12e,\"state\":\"%s\"}",
                            ...
                            g_signal_metrics.pair_slope_count,
                            g_signal_metrics.lad_objective_count,
                            g_signal_metrics.lad_minimizer_count,
                            g_signal_metrics.repeated_pivot_count,
                            g_signal_metrics.repeated_grouped_slope_count,
                            (long)g_oldest_endpoint_time,
                            (long)g_newest_endpoint_time,
                            g_signal_metrics.theilsen_slope,
                            g_signal_metrics.lad_intercept,
                            g_signal_metrics.lad_minimum_loss,
                            g_signal_metrics.lad_slope,
                            g_signal_metrics.repeated_median_slope,
                            g_signal_state));
  }
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41165 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41165_WTI_ROBUST3_CONSENSUS";
   ...
   req.reason = (g_signal_metrics.direction > 0)
      ? "WTI_ROBUST3_CONSENSUS_LONG"
      : "WTI_ROBUST3_CONSENSUS_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41165 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI robust-three consensus trend holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41165_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41165\",\"ea\":\"wti-mrobust3-agree-tr\"}");
