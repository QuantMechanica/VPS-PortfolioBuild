# QM5_41118_xauxag-mlatehalf-dom-rv - Strategy Spec
...
**Source:** `SCHWEIKERT-CME-XAUXAG-MLATEHALF-DOM-RV-2026`
...
month's chronological final log ratio anchors the newest completed month. One
deterministic floor-half endpoint partitions its full relative path:
...
abs(late) > abs(early), late > 0 => SELL XAU, BUY XAG
abs(late) > abs(early), late < 0 => BUY XAU, SELL XAG
otherwise                         => FLAT
```

The early and late blocks contain exactly `h` and `n-h` adjacent returns from
`P` through `Q[n-1]`; the midpoint is an anchor, not a duplicated return.
Equality, zero late return, non-dominance, invalid split, incomplete or
nonconsecutive months, asynchronous pairs, or invalid closes consume the
month flat. Early-half sign and full-month endpoint sign do not affect the
decision.
...
- Host/traded slot 0: exact `XAUUSD.DWX`, D1, magic `411180000`.
- Companion/traded slot 1: exact `XAGUSD.DWX`, D1, magic `411180001`.
...
- Formation is the immediately completed month plus its consecutive parent.
- Signal is strict late-half absolute relative-return dominance.
- Normal hold is through the first observed next-month boundary; forty
  calendar days is a stale repair guard.

## 5. Expected Behaviour

The prior is approximately 5-8 completed packages per full post-warm-up year.
Q02 retires below five rather than changing the rule. A dominant positive late
half produces SELL XAU / BUY XAG; a dominant negative late half produces BUY
XAU / SELL XAG. Equality, zero, or non-dominance stays flat.
...
and the intermarket ratio carrier. The completed-month half partition, strict
late-half dominance, contrarian side, CFD mapping, risk, and lifecycle are
