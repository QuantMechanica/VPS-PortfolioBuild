"""Independent deterministic fixtures for QM5_41118's late-half-dominance basket."""
...
@dataclass(frozen=True)
class LateHalfDominanceResult:
    valid: bool = False
    direction: int = 0
    completed_sessions: int = 0
    parent_sessions: int = 0
    split_index: int = 0
    parent_final_ratio: float = 0.0
    midpoint_ratio: float = 0.0
    newest_final_ratio: float = 0.0
    early_return: float = 0.0
    late_return: float = 0.0
...
def half_returns(
    ...
    """Create chronological adjacent returns with the exact floor-half split."""

    split_index = count // 2
    return [early_step] * split_index + [late_step] * (count - split_index)
...
def monthly_late_half_dominance(
    current_month: int,
...
) -> LateHalfDominanceResult:
    """Mirror bounded reconstruction, floor halves, dominance, and inverse side."""
    ...
    if (
        ...
        return LateHalfDominanceResult()
    ...
    split_index = count // 2
    series_index_mid = count - split_index
    if not 0 < split_index < count or not 0 < series_index_mid < count:
        return LateHalfDominanceResult()
    ...
    direction = 0
    if abs(late_return) > abs(early_return):
        direction = -1 if late_return > 0.0 else 1 if late_return < 0.0 else 0
    return LateHalfDominanceResult(
        True,
        direction,
        count,
...
class MonthlyLateHalfDominanceReferenceTest(unittest.TestCase):
    def signal(self, returns: list[float], **kwargs) -> LateHalfDominanceResult:
        xau, xag = sample(returns, **kwargs)
        completed_key = kwargs.get("completed_key", 202607)
        return monthly_late_half_dominance(next_month_key(completed_key), xau, xag)

    def test_positive_and_negative_dominant_late_halves_are_faded(self) -> None:
        positive = self.signal(half_returns(20, 0.002, 0.010))
        self.assertTrue(positive.valid)
        self.assertEqual(positive.direction, -1)
        self.assertGreater(positive.late_return, 0.0)
        self.assertGreater(abs(positive.late_return), abs(positive.early_return))

        negative = self.signal(half_returns(20, -0.002, -0.010))
        self.assertTrue(negative.valid)
        self.assertEqual(negative.direction, 1)
        self.assertLess(negative.late_return, 0.0)
        self.assertGreater(abs(negative.late_return), abs(negative.early_return))

    def test_opposed_half_signs_remain_eligible_when_late_dominates(self) -> None:
        up_late = self.signal(half_returns(20, -0.003, 0.010))
        self.assertTrue(up_late.valid)
        self.assertLess(up_late.early_return, 0.0)
        self.assertGreater(up_late.late_return, 0.0)
        self.assertEqual(up_late.direction, -1)

        down_late = self.signal(half_returns(20, 0.003, -0.010))
        self.assertTrue(down_late.valid)
        self.assertGreater(down_late.early_return, 0.0)
        self.assertLess(down_late.late_return, 0.0)
        self.assertEqual(down_late.direction, 1)

    def test_equality_zero_and_non_dominance_are_consumed_flat(self) -> None:
        equality = self.signal(half_returns(20, 0.01, -0.01))
        self.assertTrue(equality.valid)
        self.assertAlmostEqual(abs(equality.early_return), abs(equality.late_return))
        self.assertEqual(equality.direction, 0)

        zero = self.signal(half_returns(20, 0.01, 0.0))
        self.assertTrue(zero.valid)
        self.assertEqual(zero.late_return, 0.0)
        self.assertEqual(zero.direction, 0)

        early_dominant = self.signal(half_returns(20, 0.02, 0.005))
        self.assertTrue(early_dominant.valid)
        self.assertGreater(abs(early_dominant.early_return), abs(early_dominant.late_return))
        self.assertEqual(early_dominant.direction, 0)

    def test_floor_halves_are_exhaustive_for_all_session_shapes(self) -> None:
        expected = {17: 8, 20: 10, 23: 11}
        for count, split_index in expected.items():
            result = self.signal(half_returns(count, 0.001, 0.002))
            self.assertTrue(result.valid)
            self.assertEqual(result.completed_sessions, count)
            self.assertEqual(result.split_index, split_index)
            self.assertEqual((split_index, count - split_index), (count // 2, count - count // 2))
            self.assertAlmostEqual(result.early_return, 0.001 * split_index, places=10)
            self.assertAlmostEqual(result.late_return, 0.002 * (count - split_index), places=10)
            self.assertAlmostEqual(
                result.early_return + result.late_return,
                result.newest_final_ratio - result.parent_final_ratio,
                places=10,
            )
    ...
    def test_asynchronous_and_non_descending_pairs_are_rejected(self) -> None:
        ...
        self.assertFalse(monthly_late_half_dominance(202608, xau, xag).valid)
    ...
    def test_invalid_close_current_month_leak_and_short_buffer_are_rejected(self) -> None:
        ...
        xau, xag = sample([0.01] * 20)
        self.assertFalse(monthly_late_half_dominance(202608, xau[:-1], xag[:-1]).valid)
    ...
    def test_parent_chronological_final_is_the_anchor(self) -> None:
        ...
        result = self.signal(
            half_returns(20, 0.002, 0.010),
            parent_count=20,
            parent_ratios=parent,
        )
        self.assertTrue(result.valid)
        self.assertEqual(result.direction, -1)
        self.assertAlmostEqual(result.parent_final_ratio, 4.25, places=10)
        self.assertAlmostEqual(result.early_return, 0.02, places=10)
        self.assertAlmostEqual(result.late_return, 0.10, places=10)
    ...
    def test_static_build_contract_matches_approved_card(self) -> None:
        source = (EA_DIR / "QM5_41118_xauxag-mlatehalf-dom-rv.mq5").read_text(encoding="utf-8")
        setfile = (
            EA_DIR
            / "sets"
            / "QM5_41118_xauxag-mlatehalf-dom-rv_QM5_41118_XAU_XAG_MLATEHALF_DOM_RV_D1_D1_backtest.set"
        ...
        self.assertEqual(manifest["logical_symbol"], "QM5_41118_XAU_XAG_MLATEHALF_DOM_RV_D1")
        ...
        approved = (
            ...
            / "cards"
            / "approved"
            / "QM5_41118_xauxag-mlatehalf-dom-rv_card.md"
        )
