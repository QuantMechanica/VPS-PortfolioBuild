...
#property description "QM5_41118 XAU/XAG Completed-Month Late-Half Dominance Reversion"
...
// -----------------------------------------------------------------------------
// At the first synchronized D1 boundary of a broker month, reconstruct two
// consecutive completed calendar months. Use the parent-final log ratio and
// a deterministic floor(n/2) observation boundary to split the newest
// month's relative path into two exhaustive cumulative halves. When the late
// half strictly dominates the early half in absolute magnitude, fade its sign
// with one opposite equal-notional package held to the next month. Runtime is
// native, completed-price-only, deterministic, and one-attempt-per-month.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41118;
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41118 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_MLATEHALF_DOM_RV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41118_XAUXAG_MLATEHALF_DOM_RV";
...
bool Strategy_SynchronizedPairValid(MqlRates &xau_bars[],
                                    MqlRates &xag_bars[],
                                    const int index)
  {
   if(index < 0 || index >= ArraySize(xau_bars) ||
      index >= ArraySize(xag_bars) ||
      xau_bars[index].time <= 0 ||
      xau_bars[index].time != xag_bars[index].time ||
      xau_bars[index].close <= 0.0 || xag_bars[index].close <= 0.0 ||
...
bool Strategy_LoadMonthlyLateHalfDominance(
      const int current_month_key,
      int &completed_month_sessions,
      int &parent_month_sessions,
      int &split_index,
      double &parent_final_ratio,
      double &midpoint_ratio,
      double &newest_final_ratio,
      double &early_return,
      double &late_return,
      int &direction)
  {
   ...
   if(current_month_key <= 0 ||
      ...
      strategy_history_bars_d1 < 2 * strategy_max_month_sessions + 1)
      return false;

   MqlRates xau_bars[];
   MqlRates xag_bars[];
   double newest_ratios[];
   ArraySetAsSeries(xau_bars, true);
   ArraySetAsSeries(xag_bars, true);
   if(ArrayResize(newest_ratios, strategy_max_month_sessions) !=
      strategy_max_month_sessions)
      return false;

   const int xau_copied =
      CopyRates(g_leg_xau, // perf-allowed: one bounded synchronized two-month scan behind a consumed monthly attempt.
                PERIOD_D1, 1, strategy_history_bars_d1, xau_bars);
   const int xag_copied =
      CopyRates(g_leg_xag, // perf-allowed: one bounded synchronized two-month scan behind a consumed monthly attempt.
                PERIOD_D1, 1, strategy_history_bars_d1, xag_bars);
   if(xau_copied != strategy_history_bars_d1 ||
      xag_copied != strategy_history_bars_d1 ||
      !Strategy_SynchronizedPairValid(xau_bars, xag_bars, 0))
      return false;
   ...
   while(index < strategy_history_bars_d1 &&
         Strategy_MonthKey(xau_bars[index].time) == completed_month_key)
     {
      if(completed_month_sessions >= strategy_max_month_sessions ||
         ...
         !Strategy_LogRatio(xau_bars[index].close,
                            xag_bars[index].close,
                            newest_ratios[completed_month_sessions]))
         return false;
   ...
   const int parent_month_key = Strategy_MonthKey(xau_bars[index].time);
   if(parent_month_key <= 0 ||
      Strategy_MonthKey(xag_bars[index].time) != parent_month_key ||
      Strategy_NextMonthKey(parent_month_key) != completed_month_key)
      return false;

   while(index < strategy_history_bars_d1 &&
         Strategy_MonthKey(xau_bars[index].time) == parent_month_key)
     {
      if(parent_month_sessions >= strategy_max_month_sessions ||
         ...
         return false;
      double ratio = 0.0;
      if(!Strategy_LogRatio(xau_bars[index].close,
                            xag_bars[index].close,
                            ratio))
         return false;
      // Series index order is newest to oldest. The first parent-month pair is
      // therefore the parent's chronological final close and signal anchor.
   ...
   // Seeing the next older synchronized boundary proves that the parent
   // package was complete rather than truncated at the history edge.
   ...
   // newest_ratios[] is series-ordered (newest to oldest). For chronological
   // Q[0]..Q[n-1], Q[h-1] maps to series index n-h. The shared midpoint is
   // an anchor only: the early and late halves exhaust every adjacent return
   // from the parent-final ratio through Q[n-1] exactly once.
   split_index = completed_month_sessions / 2;
   ...
   newest_final_ratio = newest_ratios[0];
   early_return = midpoint_ratio - parent_final_ratio;
   late_return = newest_final_ratio - midpoint_ratio;
   if(!MathIsValidNumber(parent_final_ratio) ||
      !MathIsValidNumber(midpoint_ratio) ||
      !MathIsValidNumber(newest_final_ratio) ||
      !MathIsValidNumber(early_return) ||
      !MathIsValidNumber(late_return))
      return false;

   // direction > 0 means BUY XAU / SELL XAG. Fade only a strictly dominant
   // late half; equality, zero, and non-dominance remain consumed flat.
   if(MathAbs(late_return) > MathAbs(early_return))
     {
      if(late_return > 0.0)
         direction = -1;
      else if(late_return < 0.0)
         direction = 1;
     }
   return true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41118_XAUXAG_MLATEHALF_DOM_RV_HOST";
   ...
   int parent_month_sessions = 0;
   int split_index = 0;
   int direction = 0;
   double parent_final_ratio = 0.0;
   double midpoint_ratio = 0.0;
   double newest_final_ratio = 0.0;
   double early_return = 0.0;
   double late_return = 0.0;
   const bool valid =
      Strategy_LoadMonthlyLateHalfDominance(g_signal_month_key,
                                    completed_month_sessions,
                                    parent_month_sessions,
                                    split_index,
                                    parent_final_ratio,
                                    ...
                                    late_return,
                                    direction);
   string state = "monthly_ratio_late_half_dominance_failed";
   if(valid && direction > 0)
      state = "negative_late_half_dominance_fade_long_xau";
   else if(valid && direction < 0)
      state = "positive_late_half_dominance_fade_short_xau";
   else if(valid && late_return == 0.0)
      state = "late_half_zero_flat";
   else if(valid && MathAbs(late_return) == MathAbs(early_return))
      state = "half_magnitude_equality_flat";
   else if(valid)
      state = "late_half_not_dominant_flat";

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"completed_month_sessions\":%d,\"parent_month_sessions\":%d,\"split_index\":%d,\"parent_final_ratio\":%.12e,\"midpoint_ratio\":%.12e,\"newest_final_ratio\":%.12e,\"early_return\":%.12e,\"late_return\":%.12e,\"direction\":%d,\"state\":\"%s\"}",
                            g_signal_month_key,
                            completed_month_sessions,
                            parent_month_sessions,
                            split_index,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG completed-month late-half dominance reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41118\",\"ea\":\"xauxag-mlatehalf-dom-rv\"}");
