"""Independent deterministic fixtures for QM5_41212.
...
class SameCalendarSignScoreReferenceTests(unittest.TestCase):
    ...
    def test_static_build_contract_matches_approved_card(self) -> None:
        source = (
            EA_DIR / "QM5_41212_wti-samecal-signscore.mq5"
        ).read_text(encoding="utf-8")
        set_path = (
            EA_DIR
            / "sets"
            / "QM5_41212_wti-samecal-signscore_XTIUSD.DWX_D1_backtest.set"
        )
        ...
        approved = (
            ...
            / "cards"
            / "approved"
            / "QM5_41212_wti-samecal-signscore_card.md"
        )
        ...
        self.assertIn(
            "41212,wti-samecal-signscore,0,XTIUSD.DWX,412120000",
            magic_rows,
        )
        resolver = (
            REPO_ROOT / "framework" / "include" / "QM" / "QM_MagicResolver.mqh"
        ).read_text(encoding="utf-8")
        self.assertIn("412120000", resolver)
