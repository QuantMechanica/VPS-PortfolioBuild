...
#property description "QM5_41452 XAU/XAG Weekly WR2 CLV Momentum"
...
// closes. Trade only when the newest ratio-close range is strictly wider
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41452 && qm_magic_slot_offset == 0 &&
            g_leg_xau != "" && g_leg_xag != "" &&
            g_leg_xau != g_leg_xag &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_WR2_CLV_MOM_ATTEMPT_WEEK", qm_ea_id);
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41452_XAUXAG_WR2_CLV_MOM";
...
bool Strategy_LoadWr2Clv(const int current_week,
                         int &direction)
  {
   direction = 0;
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41452_XAUXAG_WR2_CLV_MOM_HOST";
   ...
   if(Strategy_OpenOwnedPositionCount() > 0 ||
     ...
     {
      signal_valid = Strategy_LoadWr2Clv(g_signal_week_key, direction);
      if(!signal_valid)
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG weekly ratio range-expansion CLV continuation package exits at the next broker week"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41452\",\"ea\":\"xauxag-wr2-clv-mom\"}");
