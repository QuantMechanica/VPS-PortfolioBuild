...
#property description "QM5_41060 XAU/XAG Weekly NR7 Ratio Breakout"
...
// -----------------------------------------------------------------------------
// Group synchronized completed D1 gold/silver close ratios into complete
// Monday-Friday broker weeks. When the immediately prior week is strictly the
// narrowest of seven valid weeks, follow only a fresh next-week completed-close
// break beyond that compressed range with opposite equal-notional legs. Flatten
// the logical package Friday at broker 21.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41060;
...
input int    strategy_reference_weeks         = 7;
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41060 && qm_magic_slot_offset == 0 &&
            strategy_xag_symbol == "XAGUSD.DWX" &&
            strategy_reference_weeks == 7 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_WEEKNR7_ATTEMPT_WEEK", qm_ea_id);
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41060_XAUXAG_WEEKNR7";
...
bool Strategy_LoadWeeklyNr7(double &prior_low,
                            ...
                            int &direction)
  {
   ...
   if(g_current_host_bar <= 0 ||
      Strategy_DayOfWeek(g_current_host_bar) < 2 ||
      Strategy_DayOfWeek(g_current_host_bar) > 5 ||
      iTime(g_leg_xag, PERIOD_D1, 0) != // perf-allowed: entry-only companion current-bar synchronization.
         g_current_host_bar)
      return false;
   ...
   const int xau_copied =
      CopyRates(g_leg_xau, // perf-allowed: entry-only bounded weekly state.
                PERIOD_D1, 1, strategy_history_bars_d1, xau_bars);
   const int xag_copied =
      CopyRates(g_leg_xag, // perf-allowed: entry-only bounded weekly state.
                PERIOD_D1, 1, strategy_history_bars_d1, xag_bars);
   if(xau_copied != strategy_history_bars_d1 ||
      xag_copied != strategy_history_bars_d1)
      return false;

   if(xau_bars[0].time <= 0 || xau_bars[1].time <= 0 ||
      xau_bars[0].time != xag_bars[0].time ||
      xau_bars[1].time != xag_bars[1].time ||
      !Strategy_LogRatio(xau_bars[0].close, xag_bars[0].close,
                         latest_ratio) ||
      !Strategy_LogRatio(xau_bars[1].close, xag_bars[1].close,
                         previous_ratio))
      return false;

   const int current_week = Strategy_WeekKey(g_current_host_bar);
   const int prior_week =
      Strategy_WeekKey(g_current_host_bar - (datetime)(7 * 86400));
   if(current_week <= 0 || prior_week <= 0 ||
      Strategy_WeekKey(xau_bars[0].time) != current_week)
      return false;
   const int previous_week = Strategy_WeekKey(xau_bars[1].time);
   if(previous_week != current_week && previous_week != prior_week)
      return false;
   ...
   for(int index = 0; index < strategy_history_bars_d1; ++index)
     {
      if(xau_bars[index].time <= 0 ||
         ...
         return false;
      const int day = Strategy_DayOfWeek(xau_bars[index].time);
      if(day < 1 || day > 5)
         continue;
      ...
      if(group < 0)
        {
         ...
         week_counts[group] = 0;
         week_masks[group] = 0;
         week_lows[group] = ratio;
         week_highs[group] = ratio;
        }
      ...
      ++week_counts[group];
      week_lows[group] = MathMin(week_lows[group], ratio);
      week_highs[group] = MathMax(week_highs[group], ratio);
     }

   const int COMPLETE_WEEK_MASK = 62;
   if(group_count <= 0 || week_keys[0] != prior_week ||
      week_counts[0] != 5 || week_masks[0] != COMPLETE_WEEK_MASK)
      return false;
   prior_low = week_lows[0];
   prior_high = week_highs[0];
   prior_range = prior_high - prior_low;
   if(prior_range <= 0.0 || !MathIsValidNumber(prior_range))
      return false;

   int valid_weeks = 1;
   for(int group = 1;
       group < group_count && valid_weeks < strategy_reference_weeks;
       ++group)
     {
      if(week_counts[group] != 5 ||
         week_masks[group] != COMPLETE_WEEK_MASK)
         continue;
      const double older_range = week_highs[group] - week_lows[group];
      if(older_range <= 0.0 || !MathIsValidNumber(older_range) ||
         !(prior_range < older_range))
         return false;
      ++valid_weeks;
     }
   if(valid_weeks != strategy_reference_weeks)
      return false;

   const bool previous_inside =
      (previous_ratio >= prior_low && previous_ratio <= prior_high);
   if(!previous_inside)
      return true;
   if(latest_ratio > prior_high)
      direction = 1;
   else if(latest_ratio < prior_low)
      direction = -1;
   return true;
  }
...
bool Strategy_DecisionClockReady(int &week_key)
  {
   ...
   if(!g_is_new_bar || g_current_host_bar <= 0)
      return false;
   const int day = Strategy_DayOfWeek(g_current_host_bar);
   if(day < 2 || day > 5)
      return false;
   week_key = Strategy_WeekKey(g_current_host_bar);
   return (week_key > 0 && week_key != g_last_attempt_week_key);
  }
...
bool Strategy_EntryWindowReady(const int week_key)
  {
   ...
   if(xag_current != g_current_host_bar ||
      Strategy_WeekKey(g_current_host_bar) != week_key ||
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41060_XAUXAG_WEEKNR7_HOST";
   ...
   if(!Strategy_LoadWeeklyNr7(prior_low,
      ...
      direction == 0)
      return false;
   // The signal is now fully known. Consume the broker week before spread,
   // quote, ATR, lot sizing, or either order submission can fail.
   if(!Strategy_RecordAttemptState(g_signal_week_key))
      return false;
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"week\":%d,\"prior_low\":%.12e,\"prior_high\":%.12e,\"prior_range\":%.12e,\"previous_ratio\":%.12e,\"latest_ratio\":%.12e,\"fresh_cross\":true,\"direction\":%d}",
                            g_signal_week_key,
                            prior_low,
                            prior_high,
                            prior_range,
...
bool Strategy_PrimeLateSignalAttach()
  {
   const datetime broker_now = TimeCurrent();
   const int day = Strategy_DayOfWeek(g_current_host_bar);
   if(day < 2 || day > 5 || Strategy_WithinEntryGrace(broker_now))
      return true;

   // Prime a late attachment so the first tick cannot backfill the current
   // bar. A later D1 bar may still present its own fresh-cross event.
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG weekly NR7 ratio breakout uses paired Friday 21 exit"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41060\",\"ea\":\"xauxag-week-nr7-brk\"}");
