...
#property description "QM5_20305 XNG Self-Relative Amihud-Illiquidity Regime"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_20305 - XNG Self-Relative Amihud-Illiquidity Regime
// -----------------------------------------------------------------------------
...
// The self-relative XNG comparison below is a locked QM translation, not a
// source-tested XNG rule. At each genuine
...
// Runtime uses native XNGUSD.DWX price, tick volume, and broker data only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 20305;
...
input int    strategy_preceding_block_offset       = 252;
...
input double strategy_state_tolerance           = 1.0e-12;
...
bool Strategy_LoadSignalState(const datetime decision_bar_time,
                              int &signal)
  {
   ...
   if(decision_bar_time <= 0 ||
      strategy_returns_per_block != 252 ||
      strategy_preceding_block_offset != 252 ||
      strategy_history_bars_d1 != 505 ||
      MathAbs(strategy_aliq_scale - 1000000.0) > 1.0e-6 ||
      strategy_max_endpoint_gap_days != 10 ||
      MathAbs(strategy_state_tolerance - 1.0e-12) > 1.0e-22)
     {
   ...
   if(!Strategy_ComputeAliqBlock(rates,
                                 strategy_preceding_block_offset,
   ...
   if(g_cache_aliq_difference > strategy_state_tolerance)
      signal = 1;
   else if(g_cache_aliq_difference < -strategy_state_tolerance)
      signal = -1;
   else
     {
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 20305 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20305_XNG_ALIQ_REGIME";
   ...
   req.reason = (g_cache_signal > 0) ? "ALIQ_REGIME_XNG_LONG" : "ALIQ_REGIME_XNG_SHORT";
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_20305_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_20305\",\"ea\":\"xng-aliq-regime\",\"stat\":\"two_disjoint_252_logreturn_tickvolume_aliq_blocks\"}");
