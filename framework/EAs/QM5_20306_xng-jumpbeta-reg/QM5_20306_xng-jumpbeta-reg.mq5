...
#property description "QM5_20306 XNG Self-Relative Common-Jump-Beta Regime"
...
// aggregate-jump beta commodity characteristic and report a negative
// high-minus-low spread. This EA is a disclosed price-native time-series
// translation, not a replication of their option-derived factor.
// At each genuine broker-month transition it:
//   1. loads exactly 505 synchronized completed XTI/XNG D1 closes;
//   2. forms two disjoint blocks of exactly 252 chronological simple returns;
//   3. independently builds an inverse-volatility common-energy factor,
//      inclusive two-sigma realized-jump residual, and 252-row OLS;
//   4. buys XNG when recent jump beta is lower, sells when it is higher, and
//      consumes a tie or invalid state flat. XTI is read-only.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 20306;
...
input int    strategy_recent_block_offset      = 252;
...
const string g_factor_symbol   = "XTIUSD.DWX";
...
int    g_cache_recent_jump_days   = 0;
int    g_cache_preceding_jump_days = 0;
...
void Strategy_ResetCachedState()
  {
   ...
   g_cache_recent_jump_days = 0;
   g_cache_preceding_jump_days = 0;
   g_cache_state_reason = "not_evaluated";
  }

bool Strategy_ComputeJumpBetaBlock(const double &xti_returns[],
                                   ...
                                   int &jump_days)
  {
   ...
   const double xti_mean = xti_sum / (double)strategy_returns_per_block;
   const double xng_mean = xng_sum / (double)strategy_returns_per_block;
   ...
   const double xti_stddev =
      MathSqrt(xti_ss / (double)(strategy_returns_per_block - 1));
   const double xng_stddev =
      MathSqrt(xng_ss / (double)(strategy_returns_per_block - 1));
   ...
   const double market_mean = market_sum / (double)strategy_returns_per_block;
   ...
   const double market_stddev =
      MathSqrt(market_ss / (double)(strategy_returns_per_block - 1));
   if(market_stddev <= 1.0e-12 || !MathIsValidNumber(market_stddev))
      return false;

   const double jump_threshold =
      strategy_jump_z * market_stddev;
   ...
   for(int local = 0; local < strategy_returns_per_block; ++local)
     {
      const double market_innovation = market_returns[local] - market_mean;
      const bool jump_day = (MathAbs(market_innovation) >= jump_threshold);
      const double jump_factor = jump_day ? market_innovation : 0.0;
      if(jump_day)
         ++jump_days;
      ...
      x[2] = jump_factor;
      // The carrier extension changes only the dependent return. The common
      // energy factor and its block-local XTI/XNG weights stay locked.
      const double y = xng_returns[block_offset + local];
   ...
   if(ols_rows != strategy_returns_per_block ||
      jump_days < strategy_min_jump_days)
      return false;
   for(int row = 0; row < 3; ++row)
      for(int col = 0; col < 4; ++col)
...
bool Strategy_LoadSignalState(const datetime decision_bar_time,
                              int &signal)
  {
   ...
   const int xti_count = CopyRates(g_factor_symbol, // perf-allowed: read-only XTI factor history on the monthly decision bar.
                                   PERIOD_D1,
                                   1,
                                   strategy_history_bars_d1,
                                   xti_rates);
   const int xng_count = CopyRates(g_strategy_symbol, // perf-allowed: bounded XNG carrier history on the same monthly path.
                                   PERIOD_D1,
                                   1,
                                   strategy_history_bars_d1,
                                   xng_rates);
   ...
   // CopyRates is newest-first; write the 504 simple returns oldest-first so
   // indices 0..251 are preceding and 252..503 are recent.
   ...
   if(!Strategy_ComputeJumpBetaBlock(xti_returns,
                                     xng_returns,
                                     0,
                                     ...
                                     g_cache_preceding_jump_days))
     {
      g_cache_state_reason = "invalid_preceding_jumpbeta";
      return false;
     }
   if(!Strategy_ComputeJumpBetaBlock(xti_returns,
                                     xng_returns,
                                     ...
                                     g_cache_recent_jump_days))
     {
      g_cache_state_reason = "invalid_recent_jumpbeta";
      return false;
     }
   ...
   if(g_cache_preceding_ols_rows != strategy_returns_per_block ||
      g_cache_recent_ols_rows != strategy_returns_per_block ||
      g_cache_preceding_jump_days < strategy_min_jump_days ||
      g_cache_recent_jump_days < strategy_min_jump_days ||
   ...
   // Preserve the source's negative high-minus-low orientation in the
   // translated own-history state: lower recent jump beta is long XNG.
   if(g_cache_beta_difference < -strategy_beta_tolerance)
     ...
     {
      g_cache_state_reason = "jumpbeta_tie";
      return true;
     }
...
void Strategy_PrepareMonthlySignal()
  {
   ...
   QM_LogEvent(QM_INFO,
               "MONTHLY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"valid\":%s,\"signal\":%d,\"recent_beta\":%.12e,\"preceding_beta\":%.12e,\"difference\":%.12e,\"recent_xti_weight\":%.10f,\"recent_xng_weight\":%.10f,\"preceding_xti_weight\":%.10f,\"preceding_xng_weight\":%.10f,\"recent_ols_rows\":%d,\"preceding_ols_rows\":%d,\"recent_jump_days\":%d,\"preceding_jump_days\":%d,\"rates\":%d,\"state\":\"%s\"}",
                             g_cache_month_key,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 20306 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20306_XNG_JUMPBETA_REGIME";
   ...
   req.reason = (g_cache_signal > 0) ? "JUMPBETA_REGIME_XNG_LONG" : "JUMPBETA_REGIME_XNG_SHORT";
...
int OnInit()
  {
   ...
   g_attempt_state_key =
      StringFormat("QM5_20306_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_20306\",\"ea\":\"xng-jumpbeta-reg\",\"stat\":\"two_disjoint_252_return_block_local_common_jump_beta\",\"xti\":\"read_only\"}");
