...
#property description "QM5_41357 XTI/XNG Monthly Fixed-Tail Winsorized Reversion"
...
// completed broker months. Form twelve adjacent oil-minus-gas log-ratio
...
bool Strategy_InputsValid()
  {
   return (Strategy_DoubleInputsFinite() &&
            g_leg_xti == "XTIUSD.DWX" &&
            g_leg_xng == "XNGUSD.DWX" &&
            g_leg_xti != g_leg_xng &&
            qm_ea_id == 41357 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
...
bool Strategy_PairCompositionValid(const int expected_direction)
  {
   ...
   if(owned_count != 2 || xti_count != 1 || xng_count != 1 ||
      !opposite_sides)
      return false;
   if(expected_direction == 0)
      return true;
   if(expected_direction > 0)
      return (xti_type == POSITION_TYPE_BUY &&
              xng_type == POSITION_TYPE_SELL);
   return (xti_type == POSITION_TYPE_SELL &&
           xng_type == POSITION_TYPE_BUY);
  }
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XTIXNG_MWINSOR2_RV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41357_XTIXNG_MWINSOR2_RV";
...
bool Strategy_LoadMonthlyWinsorizedSignal(
      ...
      int &direction)
  {
   ...
   // direction > 0 means BUY XTI / SELL XNG. Fade only a robust location
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41357_XTIXNG_MWINSOR2_RV_HOST";
   ...
   if(valid && direction > 0)
      state = "negative_winsorized_mean_fade_long_xti";
   else if(valid && direction < 0)
      state = "positive_winsorized_mean_fade_short_xti";
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XTI/XNG monthly fixed-tail Winsor reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41357\",\"ea\":\"xtixng-mwinsor2-rv\"}");
