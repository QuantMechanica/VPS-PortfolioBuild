...
#property description "QM5_10471 MQL5 MA Trend Price Versus MA"
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(Bars(_Symbol, (ENUM_TIMEFRAMES)_Period) < strategy_ma_period + strategy_atr_period + 2)
      return false;
   ...
   int state = 0;
   const double ma_1 = QM_SMA(_Symbol, (ENUM_TIMEFRAMES)_Period, strategy_ma_period, 1, PRICE_CLOSE);
   const double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
   const double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   if(ma_1 <= 0.0 || ask <= 0.0 || bid <= 0.0)
      return false;
   if(ask > ma_1)
      state = 1;
   else if(bid < ma_1)
   ...
   if(req.tp <= 0.0)
      return false;
   req.reason = (side == QM_BUY) ? "ASK_ABOVE_MA_LONG" : "BID_BELOW_MA_SHORT";
   return true;
  }
...
bool Strategy_ExitSignal()
  {
   const int magic = QM_FrameworkMagic();
   if(magic <= 0 || strategy_ma_period <= 0)
      return false;
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      if(position_type == POSITION_TYPE_BUY && bid < ma_1)
         return true;
      if(position_type == POSITION_TYPE_SELL && ask > ma_1)
         return true;
     }
