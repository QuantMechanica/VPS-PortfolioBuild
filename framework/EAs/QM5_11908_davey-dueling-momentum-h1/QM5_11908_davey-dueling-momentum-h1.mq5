...
#property description "QM5_11908 Davey Dueling Momentum H1"

#include <QM/QM_Common.mqh>

// =============================================================================
// QuantMechanica V5 EA: QM5_11908
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 11908;
...
input int    strategy_short_lookback    = 5;
input int    strategy_long_lookback     = 30;
input int    strategy_atr_period        = 14;
input double strategy_atr_sl_mult       = 1.5;
input double strategy_atr_tp_mult       = 2.0;
input int    strategy_time_stop_bars    = 50;
input bool   strategy_align_exit        = true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
{
   ...
   if(close1 <= 0.0 || close_sl <= 0.0 || close_slx <= 0.0 || atr1 <= 0.0) return false;

   bool signal_long  = (close1 > close_sl && close1 < close_slx);
   bool signal_short = (close1 < close_sl && close1 > close_slx);
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
   {
      ...
      if(strategy_align_exit)
      {
         ...
         if(close1 > 0.0 && close_slx > 0.0)
         {
            ...
            if(ptype == POSITION_TYPE_BUY && close1 > close_slx) return true;
            if(ptype == POSITION_TYPE_SELL && close1 < close_slx) return true;
         }
      }
   }
   return false;
