...
#property description "QM5_20168 XNG Autumn Dual-Trend"

#include <QM/QM_Common.mqh>

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 20168;
...
bool Strategy_IsAutumnMonth(const int month)
  {
...
void Strategy_ManageOpenPosition()
  {
   ...
   const int trend_side = Strategy_TrendSide(close_last, fast_now, slow_now,
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      const bool side_valid =
         (pos_type == POSITION_TYPE_BUY && trend_side == 1) ||
         (pos_type == POSITION_TYPE_SELL && trend_side == -1);
      const datetime opened = (datetime)PositionGetInteger(POSITION_TIME);
      if(!Strategy_IsAutumnMonth(month) || !side_valid ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "XNG_AUTUMN_DUALTREND";
   ...
   if(QM_TM_OpenPositionCount(QM_FrameworkMagic()) > 0)
      return false;
   const int month_key = QM_CalendarPeriodKey(PERIOD_MN1);
   if(month_key <= 0 || !Strategy_IsAutumnMonth(month_key % 100))
      return false;
   ...
   if(trend_side == 0)
      return false;
   req.type = (trend_side > 0) ? QM_BUY : QM_SELL;
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"ea\":\"QM5_20168\",\"slug\":\"xng-autumn-dualtrend\"}");
