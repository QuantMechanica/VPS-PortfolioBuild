...
#property description "QM5_12809 EIA Jet Fuel Summer Breakout"
...
//   - long-only XTIUSD breakouts during the summer jet-fuel demand window
//   - requires a D1 trend gate so runtime stays price-only and low-frequency
// Runtime uses MT5 OHLC/calendar only; no EIA, airline, refinery, or crack data.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                   = 12809;
...
bool Strategy_Channel(const int lookback, double &highest_high, double &lowest_low)
  {
   ...
   for(int shift = 2; shift < lookback + 2; ++shift)
     {
      const double high = iHigh(_Symbol, PERIOD_D1, shift); // perf-allowed: D1 breakout state on closed bars.
      const double low = iLow(_Symbol, PERIOD_D1, shift);   // perf-allowed: D1 breakout state on closed bars.
...
bool Strategy_LoadClosedState(double &close_last,
                              ...
                              int &day_key)
  {
   ...
   close_last = iClose(_Symbol, PERIOD_D1, 1); // perf-allowed: D1 breakout state.
   if(close_last <= 0.0)
      return false;
...
void Strategy_CloseOpenPositionsIfNeeded()
  {
   ...
   int day_key = 0;
   if(!Strategy_LoadClosedState(close_last, trend_sma, entry_high, exit_low, in_window, day_key))
      return;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12809_EIA_JETFUEL_BRK";
   ...
   req.reason = "EIA_JETFUEL_SUMMER_BREAKOUT";
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_12809\",\"ea\":\"eia-jetfuel-brk\"}");
