...
#property description "QM5_41255 WTI Exact-Permutation Integrated-ECDF Shift Trend"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41255 - WTI Exact-Permutation Integrated-ECDF Shift Trend
// -----------------------------------------------------------------------------
...
//   - integrate the squared old/recent membership imbalance over pooled ranks
//   - enumerate every one of the 924 fixed-size six/six label assignments
//   - follow the recent-block median only inside the locked upper-tail set
...
input int    strategy_tail_count_max        = 460;
input int    strategy_score_min             = 22;
...
bool Strategy_PathScoreForMask(const int mask, int &score)
  {
   score = 0;
   if(mask < 0 || mask >= 4096 ||
   ...
   for(int rank = 0; rank < strategy_month_returns; ++rank)
     {
      if((mask & (1 << rank)) != 0)
         ...
         ++old_seen;
      const int delta = old_seen - recent_seen;
      score += delta * delta;
      if(score < 0)
         return false;
     }
   return (old_seen == strategy_block_size &&
           recent_seen == strategy_block_size);
  }

bool Strategy_IntegratedEcdfSignal(const double &closes[],
                                   Strategy_SignalMetrics &metrics)
  {
   ...
   if(Strategy_Popcount12(observed_mask) != strategy_block_size ||
      !Strategy_PathScoreForMask(observed_mask,
                                 metrics.observed_score) ||
      metrics.observed_score < 0 ||
   ...
   for(int mask = 0; mask < 4096; ++mask)
     {
      if(Strategy_Popcount12(mask) != strategy_block_size)
         continue;
      int perm_score = 0;
      if(!Strategy_PathScoreForMask(mask, perm_score))
         return false;
   ...
   if(tail_qualified != score_qualified)
      return false;
                                        ...
                                        int &direction)
  {
   ...
   const bool valid =
      Strategy_LoadMonthlyEndpoints(current_month_key,
                                    ...
                                    closes,
                                    endpoint_times) &&
      Strategy_IntegratedEcdfSignal(closes, metrics);
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
   ...
   else
     {
      ...
      g_signal_valid =
         endpoints_valid &&
         Strategy_IntegratedEcdfSignal(closes, g_signal_metrics);
      if(!g_signal_valid)
         g_signal_state = "integrated_ecdf_validation_failed";
      else if(g_signal_metrics.direction == 0)
         g_signal_state = g_signal_metrics.pooled_tie
            ? "integrated_ecdf_pooled_tie_flat"
            : "integrated_ecdf_unqualified_flat";
      else if(g_signal_metrics.direction > 0)
         g_signal_state = "integrated_ecdf_shift_wti_long";
      else if(g_signal_metrics.direction < 0)
         g_signal_state = "integrated_ecdf_shift_wti_short";
      else
   ...
   QM_LogEvent(QM_INFO,
               ...
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"endpoint_count\":%d,\"return_count\":%d,\"block_size\":%d,\"assignment_count\":%d,\"tail_count\":%d,\"observed_score\":%d,\"pooled_tie\":%s,\"median_old\":%.12f,\"median_recent\":%.12f,\"median_delta\":%.12f,\"tail_fraction\":%.12f,\"returns\":\"%s\",\"sorted_membership\":\"%s\",\"oldest_close\":%.10f,\"newest_close\":%.10f,\"oldest_endpoint\":%I64d,\"newest_endpoint\":%I64d,\"state\":\"%s\"}",
                            ...
                            g_signal_metrics.observed_score,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41255 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41255_WTI_MCVM_SHIFT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41255 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI integrated-ECDF distribution-shift trend holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41255_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41255\",\"ea\":\"wti-mcvm-shift-tr\"}");
