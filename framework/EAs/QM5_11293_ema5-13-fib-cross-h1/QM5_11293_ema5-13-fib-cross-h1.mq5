...
#property description "QM5_11293 EMA(5/13) Fibonacci Cross System"
...
input int    strategy_slow_ema_period   = 13;
input int    strategy_atr_period        = 14;
input double strategy_atr_sl_mult       = 1.5;
input double strategy_min_separation_pips = 1.0;
input double strategy_max_gap_pips      = 100.0;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(strategy_fast_ema_period <= 0 ||
      ...
      strategy_min_separation_pips < 0.0 ||
      strategy_max_gap_pips <= 0.0)
      return false;
   ...
   const double atr_last = QM_ATR(_Symbol, strategy_signal_tf, strategy_atr_period, 1);
   if(fast_last <= 0.0 || slow_last <= 0.0 || fast_prev <= 0.0 || slow_prev <= 0.0 || atr_last <= 0.0)
      return false;
   ...
   const int pip_factor = (digits == 3 || digits == 5) ? 10 : 1;
   const double min_separation = strategy_min_separation_pips * point * pip_factor;
   const double max_gap = strategy_max_gap_pips * point * pip_factor;
   ...
   const double separation = MathAbs(fast_last - slow_last);
   ...
   const double raw_sl = (side == QM_BUY)
                         ? (slow_last - atr_last * strategy_atr_sl_mult)
                         : (slow_last + atr_last * strategy_atr_sl_mult);
   ...
   req.sl = sl;
   req.tp = 0.0;
   req.reason = bullish_cross ? "EMA5_13_FIB_CROSS_LONG" : "EMA5_13_FIB_CROSS_SHORT";
   req.symbol_slot = qm_magic_slot_offset;
   req.expiration_seconds = 0;
...
void Strategy_ManageOpenPosition()
  {
   // Card P2 build uses the initial ATR stop only; trailing is a later P3 sweep.
  }
...
bool Strategy_ExitSignal()
  {
   ...
   const double fast_last = QM_EMA(_Symbol, strategy_signal_tf, strategy_fast_ema_period, 1);
   const double slow_last = QM_EMA(_Symbol, strategy_signal_tf, strategy_slow_ema_period, 1);
   if(fast_last <= 0.0 || slow_last <= 0.0)
      return false;
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
     {
      ...
      if(position_type == POSITION_TYPE_BUY && fast_last < slow_last)
         return true;
      if(position_type == POSITION_TYPE_SELL && fast_last > slow_last)
         return true;
     }
