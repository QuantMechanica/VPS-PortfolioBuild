...
#property description "QM5_10467 MQL5 Bollinger RSI D1 MA Reversion"
...
input int    strategy_d1_ma_period      = 200;
...
bool Strategy_ReadSignal(const int shift, bool &long_signal, bool &short_signal)
  {
   long_signal = false;
   short_signal = false;
   if(strategy_bb_period <= 1 || strategy_bb_deviation <= 0.0 ||
      strategy_rsi_period <= 1 || strategy_d1_ma_period <= 1 ||
      strategy_atr_period <= 0 || strategy_atr_sl_mult <= 0.0 ||
      strategy_rr_tp <= 0.0 || shift < 1)
      return false;

   const double close_h1 = iClose(_Symbol, _Period, shift);
   ...
   const double d1_ma = QM_SMA(_Symbol, PERIOD_D1, strategy_d1_ma_period, 1);
   if(close_h1 <= 0.0 || close_d1 <= 0.0 || lower <= 0.0 || upper <= 0.0 ||
      rsi == EMPTY_VALUE || d1_ma <= 0.0)
      return false;

   long_signal = (close_h1 < lower &&
                  rsi < strategy_rsi_oversold &&
                  close_d1 > d1_ma);
   short_signal = (close_h1 > upper &&
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.reason = long_signal ? "BB_RSI_D1MA_LONG" : "BB_RSI_D1MA_SHORT";
...
bool Strategy_ExitSignal()
  {
   ...
   bool long_signal = false;
   bool short_signal = false;
   Strategy_ReadSignal(1, long_signal, short_signal);

   if(ptype == POSITION_TYPE_BUY)
     {
      const double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
      return ((bid >= middle) || short_signal);
     }

   const double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
   return ((ask <= middle) || long_signal);
  }
