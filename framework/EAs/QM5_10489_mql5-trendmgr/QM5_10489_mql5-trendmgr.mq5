...
#property description "QM5_10489 MQL5 TrendManager color-change time-stop"
...
int Strategy_TrendManagerColor(const int shift)
  {
   ...
   if(fast <= 0.0 || slow <= 0.0 || point <= 0.0)
      return 0;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   datetime opened_at;
   ulong ticket;
   if(Strategy_HasOurPosition(position_type, opened_at, ticket))
      return false;

   const int color_now = Strategy_TrendManagerColor(1);
   const int color_prev = Strategy_TrendManagerColor(2);
   if(color_now == 0 || color_now == color_prev)
      return false;

   const bool is_long = (color_now > 0);
   ...
   req.sl = sl;
   req.tp = tp;
   req.reason = is_long ? "TRENDMANAGER_BULL_COLOR_CHANGE" : "TRENDMANAGER_BEAR_COLOR_CHANGE";
   req.symbol_slot = qm_magic_slot_offset;
   return true;
  }
...
void Strategy_ManageOpenPosition()
  {
   // Card specifies fixed initial SL/TP only; no trailing, BE, or partial logic.
  }
...
bool Strategy_ExitSignal()
  {
   ...
   if(strategy_max_hold_minutes > 0 && opened_at > 0)
     {
      const int held_seconds = (int)(TimeCurrent() - opened_at);
   ...
   if(position_type == POSITION_TYPE_BUY && color_now < 0)
      return true;
   if(position_type == POSITION_TYPE_SELL && color_now > 0)
      return true;
   return false;
  }
