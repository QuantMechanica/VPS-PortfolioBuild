...
#property description "QM5_20164 XNG Summer Dual-Trend"

#include <QM/QM_Common.mqh>

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 20164;
...
bool Strategy_IsSummerMonth(const int month)
  {
   return (month == 5 || month == 6 || month == 7 || month == 8 || month == 9);
  }
...
void Strategy_ManageOpenPosition()
  {
   ...
   const bool valid_state = Strategy_IsSummerMonth(month) &&
      Strategy_TrendIsLong(close_last, fast_now, slow_now, fast_prior, slow_prior);
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "XNG_SUMMER_DUALTREND";
   ...
   if(QM_TM_OpenPositionCount(QM_FrameworkMagic()) > 0)
      return false;
   const int month_key = QM_CalendarPeriodKey(PERIOD_MN1);
   if(month_key <= 0 || !Strategy_IsSummerMonth(month_key % 100))
      return false;
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"ea\":\"QM5_20164\",\"slug\":\"xng-summer-dualtrend\"}");
