...
#property description "QM5_12803 Edge Lab USDCAD EURAUD Cointegration"
...
input double strategy_beta              = 0.466896958;
...
bool Strategy_IsHostSymbol()
  {
   return (_Symbol == g_leg_usdcad || _Symbol == g_leg_euraud);
  }
...
bool Strategy_EnsureBasketScope()
  {
   ...
   // EURAUD needs EURUSD/AUDUSD conversion history for USD-denominated
   // tester accounting. USDCAD supplies its own CAD conversion path.
   string allowed[4] = {"USDCAD.DWX", "EURAUD.DWX", "EURUSD.DWX", "AUDUSD.DWX"};
   for(int i = 0; i < 4; ++i)
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(!Strategy_EnsureBasketScope())
      return false;
   if(!QM_SymbolAssertOrLog(g_leg_usdcad) || !QM_SymbolAssertOrLog(g_leg_euraud))
      return false;
   ...
   for(int i = 0; i < lookback; ++i)
     {
      if(usdcad[i] <= 0.0 || euraud[i] <= 0.0)
         return false;
      spreads[i] = MathLog(usdcad[i]) - strategy_beta * MathLog(euraud[i]);
...
bool Strategy_OpenPair(const int spread_direction)
  {
   ...
   const double weight_sum = usdcad_weight + euraud_weight;
   if(weight_sum <= 0.0)
      return false;
   ...
   const string reason = long_spread ? "QM5_12803_LONG_SPREAD_Z_LT_NEG_ENTRY"
                                     : "QM5_12803_SHORT_SPREAD_Z_GT_POS_ENTRY";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12803_PAIR_HOST";
