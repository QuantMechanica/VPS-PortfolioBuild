...
#property description "QM5_20157 XAU XAG Ratio Reversion"
   ...
   for(int i = 0; i < lookback; ++i)
     {
      if(xau[i] <= 0.0 || xag[i] <= 0.0)
         return false;
      spreads[i] = MathLog(xau[i]) - strategy_beta * MathLog(xag[i]);
...
bool Strategy_OpenPair(const int spread_direction)
  {
   ...
   const double xag_weight = MathAbs(strategy_beta);
   ...
   const string reason = long_spread ? "QM5_20157_LONG_SPREAD_Z_LT_NEG_ENTRY"
                                     : "QM5_20157_SHORT_SPREAD_Z_GT_POS_ENTRY";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20157_PAIR_HOST";
