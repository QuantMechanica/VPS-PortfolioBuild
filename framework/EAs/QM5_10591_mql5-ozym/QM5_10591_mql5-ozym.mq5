...
input int    strategy_ozymandias_length = 2;
input int    strategy_ozymandias_lookback_bars = 240;
...
double OzymandiasLowestLow(const int shift, const int length)
  {
   double lowest = 0.0;
   for(int i = 0; i < length; ++i)
     {
...
int OzymandiasTrendAt(const int target_shift)
  {
   ...
   int trend = 0;
   int nexttrend = 0;
   double maxl = OzymandiasLowestLow(target_shift + warmup, length);
   double minh = OzymandiasHighestHigh(target_shift + warmup, length);
   if(maxl <= 0.0 || minh <= 0.0)
      return 0;

   for(int bar = target_shift + warmup; bar >= target_shift; --bar)
     {
      const double hh = OzymandiasHighestHigh(bar, length);
      ...
      if(nexttrend == 1)
        {
         ...
         if(hma < maxl && close_bar < prev_low)
           {
            trend = 1;
            nexttrend = 0;
            minh = hh;
      ...
      if(nexttrend == 0)
        {
         ...
         if(lma > minh && close_bar > prev_high)
           {
            trend = 0;
            nexttrend = 1;
            maxl = ll;
...
int OzymandiasColorChange()
  {
   const int current_trend = OzymandiasTrendAt(1);
   const int previous_trend = OzymandiasTrendAt(2);
   if(current_trend == 0 || previous_trend == 0 || current_trend == previous_trend)
      return 0;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = sl;
   req.tp = tp;
   req.reason = (signal > 0) ? "OZYM_BULLISH_COLOR_CHANGE" : "OZYM_BEARISH_COLOR_CHANGE";
