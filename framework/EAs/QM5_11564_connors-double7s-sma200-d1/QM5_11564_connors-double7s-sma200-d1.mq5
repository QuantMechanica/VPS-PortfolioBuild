...
#property description "QM5_11564 connors-double7s-sma200-d1 — Double 7's mean reversion (D1)"

#include <QM/QM_Common.mqh>

// =============================================================================
// QuantMechanica V5 EA — QM5_11564 connors-double7s-sma200-d1
// -----------------------------------------------------------------------------
// Source: Larry Connors & Cesar Alvarez, "Short-Term Trading Strategies That
// Work: A Quantified Guide to Trading Stocks and ETFs", TradingMarkets
// Publishing LLC, 2009 — Strategy 11 "Double 7's".
// Card: artifacts/cards_approved/QM5_11564_connors-double7s-sma200-d1.md
//       (g0_status APPROVED).
//
// Mechanics (D1, closed-bar reads at shift 1; long + symmetric short, FX port):
...
//                  SHORT fires when close[1] is the HIGHEST close (new 7-day
//                  closing high). The new 7-day extreme is the SINGLE trigger
//                  EVENT; the SMA is a trend STATE, never a second cross EVENT
//                  on the same bar (avoids the two-cross zero-trade trap).
//   Exit EVENT   : LONG  exits when close[1] is the new 7-day HIGHEST close.
//                  SHORT exits when close[1] is the new 7-day LOWEST  close.
//   Stop loss    : (not in Connors original) P2 safety stop.
//                  LONG  entry - sl_atr_mult * ATR(atr_period);
//                  SHORT entry + sl_atr_mult * ATR(atr_period).
//                  Skip the trade when the stop distance exceeds max_sl_pips.
//   No-trade     : skip new entries on Friday (broker time); spread cap fail-
...
input double strategy_sl_atr_mult       = 2.0;   // stop distance = 2 * ATR (card)
input int    strategy_max_sl_pips       = 150;   // skip trade if ATR stop wider than 150p (card cap)
input bool   strategy_allow_short       = true;  // symmetric short below SMA200 (card)
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   // LONG: uptrend + new 7-day closing low (mean-reversion buy).
   if(close1 > sma && IsNewLowestClose(strategy_extreme_lookback))
     {
      ...
      if(sl <= 0.0)
         return false;
      // P2 safety cap: skip if the ATR stop is wider than max_sl_pips.
   ...
   // SHORT: downtrend + new 7-day closing high (symmetric mean-reversion sell).
...
// No active trade management beyond the fixed ATR safety stop. The 7-day-extreme
...
// Exit:
//   LONG  -> close[1] is a new 7-day HIGHEST close.
//   SHORT -> close[1] is a new 7-day LOWEST  close.
...
bool Strategy_ExitSignal()
  {
   ...
   // Find this EA's position to read its direction.
   bool is_long  = false;
   bool have_pos = false;
   ...
   // 7-day-extreme exit (mean reversion completed): long exits on a new 7-day
   // closing HIGH, short on a new 7-day closing LOW.
   if(is_long)
