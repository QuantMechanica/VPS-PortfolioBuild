...
#property description "QM5_20161 XAU XAG Ratio Reversion"
...
input int    strategy_ols_lookback_d1   = 120;
input double strategy_entry_z           = 2.25;
input double strategy_exit_z            = 0.5;
input double strategy_beta_min          = 0.25;
input double strategy_beta_max          = 2.50;
...
bool Strategy_IsHostSymbol()
  {
   return (_Symbol == g_leg_xau || _Symbol == g_leg_xag);
  }
...
bool Strategy_EnsureBasketScope()
  {
   ...
   QM_BasketWarmupHistory(allowed, PERIOD_D1, MathMax(300, strategy_ols_lookback_d1 + strategy_atr_period_d1 + 10));
...
bool Strategy_RefreshSpreadState()
  {
   g_state_ready = false;
   const int lookback = MathMax(40, strategy_ols_lookback_d1);

   if(!Strategy_EnsureBasketScope())
      return false;
   if(!QM_SymbolAssertOrLog(g_leg_xau) || !QM_SymbolAssertOrLog(g_leg_xag))
      return false;
   ...
   if(CopyClose(g_leg_xau, PERIOD_D1, 1, lookback, xau) != lookback) // perf-allowed: Strategy_EntrySignal is called only after the framework QM_IsNewBar gate.
      return false;
   if(CopyClose(g_leg_xag, PERIOD_D1, 1, lookback, xag) != lookback) // perf-allowed: Strategy_EntrySignal is called only after the framework QM_IsNewBar gate.
      return false;
   ...
   for(int i = 0; i < lookback; ++i)
     {
      if(xau[i] <= 0.0 || xag[i] <= 0.0)
         return false;
      const double y = MathLog(xau[i]);
      const double x = MathLog(xag[i]);
   ...
   for(int i = 0; i < lookback; ++i)
     {
      const double dx = MathLog(xag[i]) - mean_x;
      cov_xy += dx * (MathLog(xau[i]) - mean_y);
      var_x += dx * dx;
     }
   if(var_x <= 0.0)
      return false;
   g_rolling_beta = cov_xy / var_x;
   if(!MathIsValidNumber(g_rolling_beta) ||
      g_rolling_beta < strategy_beta_min ||
      g_rolling_beta > strategy_beta_max)
      return false;
   ...
   for(int i = 0; i < lookback; ++i)
     {
      spreads[i] = MathLog(xau[i]) - alpha - g_rolling_beta * MathLog(xag[i]);
      sum += spreads[i];
     }
...
bool Strategy_OpenPair(const int spread_direction)
  {
   ...
   const double xag_weight = MathAbs(g_rolling_beta);
   ...
   const string reason = long_spread ? "QM5_20161_LONG_SPREAD_Z_LT_NEG_ENTRY"
                                     : "QM5_20161_SHORT_SPREAD_Z_GT_POS_ENTRY";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_20161_PAIR_HOST";
