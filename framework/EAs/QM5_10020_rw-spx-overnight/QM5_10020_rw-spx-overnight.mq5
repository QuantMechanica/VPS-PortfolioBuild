...
bool Strategy_NoTradeFilter()
  {
   // Entry-specific filters live in Strategy_EntrySignal so an existing
   // overnight position can still be closed by Strategy_ExitSignal.
   return false;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(strategy_filter_sessions < 1 || strategy_atr_period_h1 < 1 ||
   ...
   if(strategy_skip_news_day &&
      !QM_NewsAllowsTrade(_Symbol, TimeCurrent(), QM_NEWS_SKIP_DAY))
      return false;
   ...
   if(samples < strategy_filter_sessions || edge_sum / (double)samples <= 0.0)
      return false;
...
void Strategy_ManageOpenPosition()
  {
   // Card specifies no trailing stop, break-even, partial close, or TP.
  }
