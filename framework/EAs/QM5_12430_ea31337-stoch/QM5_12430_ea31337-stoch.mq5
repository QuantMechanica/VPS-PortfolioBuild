...
#property description "QM5_12430 EA31337 Stochastic reversal cross"
...
//     QM_ADX / QM_ADX_PlusDI / QM_ADX_MinusDI / QM_Stoch_K / QM_Stoch_D /
...
//   - Call iATR / iMA / iRSI / iMACD / iADX / iBands / iStochastic or CopyBuffer
...
// Card §Entry: min(K, 4 bars) below 50-level, D below K, both K and D rising,
// D risen by >= level/10 percent over the 3-bar window (shift 1 vs shift 4).
bool Strategy_LongSignal()
  {
   ...
   for(int s = 1; s <= 4; ++s)
     {
      k[s] = QM_Stoch_K(_Symbol, tf, strategy_stoch_k_period, strategy_stoch_d_period, strategy_stoch_slowing, s);
      d[s] = QM_Stoch_D(_Symbol, tf, strategy_stoch_k_period, strategy_stoch_d_period, strategy_stoch_slowing, s);
      if(k[s] < 0.0 || k[s] > 100.0 || d[s] < 0.0 || d[s] > 100.0)
   ...
   if(k_min4 >= 50.0 - strategy_signal_open_level)
      return false;
   if(d[1] >= k[1])
      return false;         // signal line must be below main line
   if(k[1] <= k[2])
      return false;         // main line must be increasing
   if(d[1] <= d[2])
      return false;         // signal line must be increasing
   if(d[4] == 0.0)
      return false;

   const double change_pct = ((d[1] - d[4]) / MathAbs(d[4])) * 100.0;
   return (change_pct >= strategy_signal_open_level / 10.0);
  }

// Card §Entry: mirror of the long rule above the 50+level threshold.
bool Strategy_ShortSignal()
  {
   ...
   if(k_max4 <= 50.0 + strategy_signal_open_level)
      return false;
   if(d[1] <= k[1])
      return false;         // signal line must be above main line
   if(k[1] >= k[2])
      return false;         // main line must be decreasing
   if(d[1] >= d[2])
      return false;         // signal line must be decreasing
   if(d[4] == 0.0)
      return false;

   const double change_pct = ((d[4] - d[1]) / MathAbs(d[4])) * 100.0;
   return (change_pct >= strategy_signal_open_level / 10.0);
  }
