...
#property description "QM5_10582 mql5-ema-pred"
...
input int    strategy_fast_ema_period   = 1;
input int    strategy_slow_ema_period   = 2;
...
int EmaPredictionSignal()
  {
   if(strategy_fast_ema_period < 1 || strategy_slow_ema_period < 1 ||
      strategy_fast_ema_period >= strategy_slow_ema_period)
      return 0;

   const double fast_1 = QM_EMA(_Symbol, _Period, strategy_fast_ema_period, 1);
   const double slow_1 = QM_EMA(_Symbol, _Period, strategy_slow_ema_period, 1);
   const double fast_2 = QM_EMA(_Symbol, _Period, strategy_fast_ema_period, 2);
   const double slow_2 = QM_EMA(_Symbol, _Period, strategy_slow_ema_period, 2);
   ...
   if(fast_1 <= 0.0 || slow_1 <= 0.0 || fast_2 <= 0.0 || slow_2 <= 0.0 ||
      open_1 <= 0.0 || close_1 <= 0.0)
      return 0;

   if(fast_2 < slow_2 && fast_1 > slow_1 && open_1 < close_1)
      return 1;
   if(fast_2 > slow_2 && fast_1 < slow_1 && open_1 > close_1)
      return -1;
   return 0;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(HasOurPosition(position_type))
      return false;
   if(strategy_atr_period < 1 || strategy_atr_sl_mult <= 0.0 || strategy_take_profit_rr <= 0.0)
      return false;

   const int signal = EmaPredictionSignal();
   if(signal == 0)
      return false;
   ...
   req.reason = (signal > 0) ? "EMA_PREDICTION_BUY" : "EMA_PREDICTION_SELL";
   req.symbol_slot = qm_magic_slot_offset;
   req.expiration_seconds = 0;
   return true;
  }
