...
#property description "QM5_1195 Quantpedia SPY Overnight 09:31 Exit"
...
input double strategy_gap_risk_mult     = 2.0;
input int    strategy_entry_hhmm_broker = 2300;
input int    strategy_exit_hhmm_broker  = 1631;
input int    strategy_safety_exit_hhmm_broker = 1645;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QP_SPY_OVERNIGHT_931_LONG";
   ...
   const double sma_d1 = QM_SMA(_Symbol, PERIOD_D1, strategy_sma_period, 1);
   const double atr_d1 = QM_ATR(_Symbol, PERIOD_D1, strategy_atr_period, 1);
   const double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
   if(close_d1 <= 0.0 || sma_d1 <= 0.0 || atr_d1 <= 0.0 || ask <= 0.0)
      return false;

   if(close_d1 <= sma_d1)
      return false;

   req.sl = QM_StopATRFromValue(_Symbol, QM_BUY, ask, atr_d1, strategy_atr_stop_mult);
   if(req.sl <= 0.0)
      return false;
      ...
      if(planned_risk > 0.0)
        {
         ...
         if(pnl <= -(strategy_gap_risk_mult * planned_risk))
            return true;
        }
     }
