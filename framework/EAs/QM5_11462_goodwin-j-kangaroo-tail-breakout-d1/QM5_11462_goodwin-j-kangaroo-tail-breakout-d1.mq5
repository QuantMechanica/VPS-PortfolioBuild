...
#property description "QM5_11462 goodwin-j-kangaroo-tail-breakout-d1 — Kangaroo Tail 3-Bar Breakout (D1)"

#include <QM/QM_Common.mqh>

// =============================================================================
// QuantMechanica V5 EA — QM5_11462 goodwin-j-kangaroo-tail-breakout-d1
// -----------------------------------------------------------------------------
// Source: Jarrod Goodwin, "Beat the Markets Strategy Guidebook" (~2020), Strategy 2.
// Card: artifacts/cards_approved/QM5_11462_goodwin-j-kangaroo-tail-breakout-d1.md
//       (g0_status APPROVED).
//
// Mechanics (D1, closed-bar reads at shift 1..3; one EVENT per new daily bar):
//   The "Kangaroo Tail" is a 3-bar Williams-fractal STATE confirmed once the
//   bar at shift 1 closes:
...
//   STOP pending order so price must resume in the breakout direction to fill.
...
//
// Filters (card):
//   - 0.5% continuation filter : skip LONG if Close[1] is >filter_pct above
//     Close[2]; skip SHORT if Close[1] is >filter_pct below Close[2]. (Avoid
//     chasing a bar that already ran away from the tail.)
//   - No-Friday entry          : the breakout bar (current D1 bar) must not be a
//     Friday.
//   - Bar-range cap            : skip if the breakout bar[1] range (High[1]-Low[1])
//     exceeds range_cap_pips (stop would be too wide).
//   - Spread cap               : skip only a genuinely wide spread (fail-open on
//     .DWX zero modeled spread).
//
// .DWX invariants honoured:
//   - Gapless CFDs (open[0]==close[1]): a STOP above High[1] / below Low[1] is a
//     valid pending placement that requires fresh continuation — no gap rule.
//   - Spread guard fails OPEN on zero modeled spread (#1).
//   - All pip thresholds via QM_StopRulesPipsToPriceDistance (pip_factor) so the
//     0.5% filter, offsets and range cap are scale-correct on 5-digit / JPY (#14).
//   - QM_IsNewBar() consumed once by the framework; one entry EVENT per D1 bar (#4).
//
...
input int    strategy_range_cap_pips    = 80;    // skip if breakout bar[1] range exceeds this (pips)
input int    strategy_spread_cap_pips   = 20;    // block only a genuinely wide spread (pips)
...
// Detects the kangaroo-tail fractal and places a STOP pending order whose break
// of bar[1]'s extreme is the trigger.
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   // Closed-bar OHLC for the 3-bar fractal (shifts 1,2,3). perf-allowed:
   ...
   // Fractals are mutually exclusive in practice; if both/none, take none.
   ...
   if(tail_low)
     {
      ...
      const double sl_price    = QM_StopRulesNormalizePrice(_Symbol, low1  - offset);
   ...
   const double sl_price    = QM_StopRulesNormalizePrice(_Symbol, high1 + offset);
