...
#property description "QM5_41324 Balke GMT3 Range Breakout - DL-089 path-to-25 sibling"
...
//
// Identity: QM5_41324 is a fresh DL-089 path-to-25 instrument (_opt). Its
// direct parent is QM5_21501, which carries the A1-fixed, side-effect-free
// straddle inherited from QM5_13213. An optimization instrument must never reach a
...
bool Pattern_AllowsRequest(const QM_EntryRequest &req,
                           const QM_PermissionResult &perm)
  {
   if(!perm.valid)
      return false;
   return QM_OrderTypeIsBuy(req.type) ? perm.allow_buy : perm.allow_sell;
  }
...
void OnTick()
  {
   ...
   if(decision.place_buy && Pattern_AllowsRequest(plan.buy_req, perm))
     {
      ulong buy_ticket = 0;
      QM_TM_OpenPosition(plan.buy_req, buy_ticket);
     }
   if(decision.place_sell && Pattern_AllowsRequest(plan.sell_req, perm))
     {
      ulong sell_ticket = 0;
