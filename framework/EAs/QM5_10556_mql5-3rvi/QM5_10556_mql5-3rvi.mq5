...
#property description "QM5_10556 MQL5 Three Timeframe RVI Cross"
...
input ENUM_TIMEFRAMES strategy_lower_tf  = PERIOD_M5;
input ENUM_TIMEFRAMES strategy_medium_tf = PERIOD_M15;
input ENUM_TIMEFRAMES strategy_higher_tf = PERIOD_M30;
...
int Strategy_RviCross(const string sym, const ENUM_TIMEFRAMES tf, const int period)
  {
   const double main_1 = Strategy_RviMain(sym, tf, period, 1);
   const double sig_1 = Strategy_RviSignal(sym, tf, period, 1);
   const double main_2 = Strategy_RviMain(sym, tf, period, 2);
   const double sig_2 = Strategy_RviSignal(sym, tf, period, 2);
...
int Strategy_RviPosition(const string sym, const ENUM_TIMEFRAMES tf, const int period)
  {
   const double main = Strategy_RviMain(sym, tf, period, 1);
   const double sig = Strategy_RviSignal(sym, tf, period, 1);
   if(main > sig)
      return 1;
   if(main < sig)
      return -1;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(strategy_adx_floor > 0.0)
     {
      const double adx = QM_ADX(_Symbol, strategy_higher_tf, strategy_adx_period, 1);
      if(adx < strategy_adx_floor)
         return false;
     }

   const int medium_pos = Strategy_RviPosition(_Symbol, strategy_medium_tf, strategy_rvi_period);
   const int higher_pos = Strategy_RviPosition(_Symbol, strategy_higher_tf, strategy_rvi_period);
   const int lower_cross = Strategy_RviCross(_Symbol, strategy_lower_tf, strategy_rvi_period);
   if(lower_cross == 0)
      return false;

   QM_OrderType side = QM_BUY;
   if(medium_pos > 0 && higher_pos > 0 && lower_cross > 0)
      side = QM_BUY;
   else if(medium_pos < 0 && higher_pos < 0 && lower_cross < 0)
      side = QM_SELL;
   else
      return false;
   ...
   const double atr = QM_ATR(_Symbol, strategy_higher_tf, strategy_atr_period, 1);
   if(atr <= 0.0)
      return false;
   ...
   req.reason = (side == QM_BUY) ? "MQL5_3RVI_LONG" : "MQL5_3RVI_SHORT";
...
bool Strategy_ExitSignal()
  {
   ...
   if(!QM_IsNewBar(_Symbol, strategy_lower_tf))
      return false;

   const int lower_cross = Strategy_RviCross(_Symbol, strategy_lower_tf, strategy_rvi_period);
   if(position_type == POSITION_TYPE_BUY && lower_cross < 0)
      return true;
   if(position_type == POSITION_TYPE_SELL && lower_cross > 0)
      return true;
