...
#property description "QM5_41251 WTI Monthly Brunner-Munzel Shift Trend"
...
//   - compute exact pooled and within-sample midranks
//   - follow the corrected Brunner-Munzel studentized rank-placement shift
...
input double strategy_score_threshold      = 0.625;
...
struct Strategy_SignalMetrics
  {
   ...
   int return_count;
   int block_size;
   double mean_rank_old;
   double mean_rank_recent;
   double relative_effect;
   double placement_var_old;
   double placement_var_recent;
   double numerator;
   double denominator;
   double score;
   bool saturated;
...
bool Strategy_AverageRanks(const double &values[], double &ranks[])
  {
   const int count = ArraySize(values);
   ArrayResize(ranks, 0);
   if(count <= 0 || ArrayResize(ranks, count) != count)
      return false;
   ...
   for(int index = 0; index < count; ++index)
     {
      ...
      for(int other = 0; other < count; ++other)
        {
         if(!MathIsValidNumber(values[other]))
            return false;
         if(values[other] < values[index])
            ++less_count;
         else if(values[other] == values[index])
            ++equal_count;
        }
      if(equal_count <= 0)
         return false;
      ranks[index] = (double)less_count +
                     ((double)equal_count + 1.0) / 2.0;
      if(ranks[index] < 1.0 || ranks[index] > (double)count ||
         !MathIsValidNumber(ranks[index]))
         return false;
      rank_sum += ranks[index];
      if(!MathIsValidNumber(rank_sum))
         return false;
     }

   const double expected_sum =
      (double)count * ((double)count + 1.0) / 2.0;
   return (MathAbs(rank_sum - expected_sum) <= 0.000000001);
  }

bool Strategy_BrunnerMunzelSignal(const double &closes[],
                                  Strategy_SignalMetrics &metrics)
  {
   ...
   if(strategy_month_returns != 20 ||
      ...
      MathAbs(strategy_score_threshold - 0.625) > 0.000000000001 ||
   ...
   double recent_ranks[];
   double pooled_ranks[];
   if(!Strategy_AverageRanks(old_values, old_ranks) ||
      !Strategy_AverageRanks(recent_values, recent_ranks) ||
      !Strategy_AverageRanks(pooled_values, pooled_ranks) ||
      ArraySize(old_ranks) != strategy_block_size ||
      ArraySize(recent_ranks) != strategy_block_size ||
      ArraySize(pooled_ranks) != strategy_month_returns ||
      StringLen(metrics.return_path) <= 0)
      return false;

   double pooled_old_sum = 0.0;
   double pooled_recent_sum = 0.0;
   for(int index = 0; index < strategy_block_size; ++index)
     {
      pooled_old_sum += pooled_ranks[index];
      pooled_recent_sum += pooled_ranks[strategy_block_size + index];
      if(!MathIsValidNumber(pooled_old_sum) ||
         !MathIsValidNumber(pooled_recent_sum))
         return false;
     }

   metrics.mean_rank_old =
      pooled_old_sum / (double)strategy_block_size;
   metrics.mean_rank_recent =
      pooled_recent_sum / (double)strategy_block_size;
   metrics.relative_effect =
      (metrics.mean_rank_recent -
       ((double)strategy_block_size + 1.0) / 2.0) /
      (double)strategy_block_size;
   if(!MathIsValidNumber(metrics.mean_rank_old) ||
      !MathIsValidNumber(metrics.mean_rank_recent) ||
      !MathIsValidNumber(metrics.relative_effect) ||
      metrics.relative_effect < 0.0 || metrics.relative_effect > 1.0)
      return false;

   double old_placement_ss = 0.0;
   double recent_placement_ss = 0.0;
   const double within_rank_center =
      ((double)strategy_block_size + 1.0) / 2.0;
   for(int index = 0; index < strategy_block_size; ++index)
     {
      const double old_placement =
         pooled_ranks[index] - old_ranks[index] -
         metrics.mean_rank_old + within_rank_center;
      const double recent_placement =
         pooled_ranks[strategy_block_size + index] -
         recent_ranks[index] - metrics.mean_rank_recent +
         within_rank_center;
      const double old_square = old_placement * old_placement;
      const double recent_square = recent_placement * recent_placement;
      if(!MathIsValidNumber(old_placement) ||
         !MathIsValidNumber(recent_placement) ||
         !MathIsValidNumber(old_square) ||
         !MathIsValidNumber(recent_square) ||
         old_square < 0.0 || recent_square < 0.0)
         return false;
      old_placement_ss += old_square;
      recent_placement_ss += recent_square;
      if(!MathIsValidNumber(old_placement_ss) ||
         !MathIsValidNumber(recent_placement_ss))
         return false;
     }

   const int variance_denominator = strategy_block_size - 1;
   if(variance_denominator != 9)
      return false;
   metrics.placement_var_old =
      old_placement_ss / (double)variance_denominator;
   metrics.placement_var_recent =
      recent_placement_ss / (double)variance_denominator;
   metrics.numerator =
      (double)(strategy_block_size * strategy_block_size) *
      (metrics.mean_rank_recent - metrics.mean_rank_old) /
      (double)strategy_month_returns;
   const double denominator_square =
      (double)strategy_block_size * metrics.placement_var_old +
      (double)strategy_block_size * metrics.placement_var_recent;
   metrics.oldest_close = closes[0];
   metrics.newest_close = closes[metrics.endpoint_count - 1];
   if(!MathIsValidNumber(metrics.placement_var_old) ||
      !MathIsValidNumber(metrics.placement_var_recent) ||
      !MathIsValidNumber(metrics.numerator) ||
      !MathIsValidNumber(denominator_square) ||
      metrics.placement_var_old < 0.0 ||
      metrics.placement_var_recent < 0.0 ||
      denominator_square < 0.0)
      return false;

   metrics.denominator = MathSqrt(denominator_square);
   ...
   if(metrics.denominator <= strategy_denominator_epsilon)
     {
      const double rank_difference =
         metrics.mean_rank_recent - metrics.mean_rank_old;
      if(rank_difference > strategy_denominator_epsilon)
        {
         metrics.score = strategy_score_cap;
         metrics.saturated = true;
        }
      else if(rank_difference < -strategy_denominator_epsilon)
        {
         metrics.score = -strategy_score_cap;
         metrics.saturated = true;
        }
                                        ...
                                        int &direction)
  {
   ...
   const bool valid =
      Strategy_LoadMonthlyEndpoints(current_month_key,
                                    ...
                                    closes,
                                    endpoint_times) &&
      Strategy_BrunnerMunzelSignal(closes, metrics);
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
   ...
   else
     {
      ...
      g_signal_valid =
         endpoints_valid &&
         Strategy_BrunnerMunzelSignal(closes, g_signal_metrics);
      if(!g_signal_valid)
         g_signal_state = "brunner_munzel_arithmetic_validation_failed";
      else if(g_signal_metrics.direction == 0)
         g_signal_state = "brunner_munzel_unqualified_or_tied_flat";
      else if(g_signal_metrics.direction > 0)
         g_signal_state = "brunner_munzel_positive_shift_wti_long";
      else if(g_signal_metrics.direction < 0)
         g_signal_state = "brunner_munzel_negative_shift_wti_short";
      else
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"endpoint_count\":%d,\"return_count\":%d,\"block_size\":%d,\"mean_rank_old\":%.12f,\"mean_rank_recent\":%.12f,\"relative_effect\":%.12f,\"placement_var_old\":%.12f,\"placement_var_recent\":%.12f,\"numerator\":%.12f,\"denominator\":%.12f,\"score\":%.12f,\"saturated\":%s,\"returns\":\"%s\",\"oldest_close\":%.10f,\"newest_close\":%.10f,\"oldest_endpoint\":%I64d,\"newest_endpoint\":%I64d,\"state\":\"%s\"}",
                            ...
                            g_signal_metrics.mean_rank_old,
                            g_signal_metrics.mean_rank_recent,
                            g_signal_metrics.relative_effect,
                            g_signal_metrics.placement_var_old,
                            g_signal_metrics.placement_var_recent,
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41251 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41251_WTI_MBRUNNER_SHIFT";
   ...
   req.reason = (g_signal_metrics.direction > 0)
      ? "WTI_MBRUNNER_UPSHIFT_LONG"
      : "WTI_MBRUNNER_DOWNSHIFT_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41251 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI fixed-block Brunner-Munzel rank shift holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41251_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41251\",\"ea\":\"wti-mbrunner-shift-tr\"}");
