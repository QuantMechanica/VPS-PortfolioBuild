...
#property description "QM5_41281 XAU/XAG Monthly Conover Scale Reversion"
...
// returns into fixed old/recent blocks of six. Strict pooled ranks of the
// within-block mean-centered absolute deviations receive squared-rank Conover
// scores. An inclusive exact 924-assignment upper-half state fades the raw
// block-mean shift with an opposite equal-notional package to next month.
// Runtime is native, completed-price-only, deterministic, and one consumed
// attempt per month. The exhaustive tail is an integrity/activity gate only;
// it is not a p-value and does not change risk.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41281;
...
input int    strategy_score_total              = 650;
input int    strategy_score_expected           = 325;
input int    strategy_score_min                = 326;
input int    strategy_assignment_count         = 924;
input int    strategy_tail_count_max           = 461;
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41281 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
            ...
            MathAbs(strategy_direction_epsilon - 1.0e-12) <= 1.0e-18 &&
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41281_XAUXAG_CONOVER_RV";
...
bool Strategy_ConoverScore(const int rank, int &score)
  {
   score = 0;
   if(rank < 1 || rank > strategy_return_count)
      return false;
   score = rank * rank;
   return (score > 0);
  }

bool Strategy_ConoverSignal(
      ...
      int &direction)
  {
   ...
   for(int index = 0; index < strategy_return_count; ++index)
     {
      const double own_mean =
         (index < strategy_block_size) ? old_mean : recent_mean;
      deviations[index] = MathAbs(relative_returns[index] - own_mean);
      if(!MathIsValidNumber(deviations[index]))
         return false;
      ranked_deviations[index] = deviations[index];
      ranked_labels[index] = (index < strategy_block_size) ? 0 : 1;
      if(StringLen(deviation_path) > 0)
         deviation_path += ",";
      deviation_path +=
         StringFormat("%s:%.12e",
      ...
      for(int right = left + 1; right < strategy_return_count; ++right)
        {
         const double tie_tolerance =
            ...
            MathMax(1.0,
                    MathMax(MathAbs(deviations[left]),
                            MathAbs(deviations[right])));
         if(!MathIsValidNumber(tie_tolerance))
            return false;
         if(MathAbs(deviations[left] - deviations[right]) <= tie_tolerance)
           {
            tie_detected = true;
   ...
   for(int index = 1; index < strategy_return_count; ++index)
     {
      const double deviation = ranked_deviations[index];
      const int label = ranked_labels[index];
      int cursor = index - 1;
      while(cursor >= 0 && ranked_deviations[cursor] > deviation)
        {
         ranked_deviations[cursor + 1] = ranked_deviations[cursor];
         ranked_labels[cursor + 1] = ranked_labels[cursor];
         --cursor;
        }
      ranked_deviations[cursor + 1] = deviation;
   ...
   for(int index = 0; index < strategy_return_count; ++index)
     {
      int score = 0;
      if(!Strategy_ConoverScore(index + 1, score))
         return false;
      total_score += score;
      ...
      if(StringLen(rank_path) > 0)
         rank_path += ",";
      rank_path +=
         StringFormat("%d:%s:%.12e:%d",
                      index + 1,
                      (ranked_labels[index] == 0) ? "O" : "R",
                      ranked_deviations[index],
                      score);
     }
   if(total_score != strategy_score_total ||
      strategy_score_expected * 2 != strategy_score_total ||
      recent_score <= 0 || recent_score >= strategy_score_total)
      return false;
   ...
   while(true)
     {
      ...
      for(int item = 0; item < strategy_block_size; ++item)
        {
         int score = 0;
         if(!Strategy_ConoverScore(choice[item] + 1, score))
            return false;
         permutation_score += score;
        }
      if(permutation_score >= recent_score)
         ++tail_count;
      ++assignment_count;
   ...
   if(recent_score < strategy_score_min ||
   ...
   if(!MathIsValidNumber(mean_delta))
      return false;
   if(mean_delta > strategy_direction_epsilon)
      direction = -1; // SELL XAU / BUY XAG.
   else if(mean_delta < -strategy_direction_epsilon)
      direction = 1;  // BUY XAU / SELL XAG.
   return true;
  }

bool Strategy_LoadMonthlyConoverSignal(
      ...
      int &direction)
  {
   ...
   return Strategy_ConoverSignal(relative_returns,
                               old_mean,
                               recent_mean,
                               recent_score,
                               assignment_count,
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41281_XAUXAG_CONOVER_RV_HOST";
   ...
   string rank_path = "";
   const bool valid =
      Strategy_LoadMonthlyConoverSignal(
         g_signal_month_key,
         month_count,
   ...
   string state = "monthly_conover_signal_failed";
   if(valid && direction > 0)
      state = "qualified_conover_mean_fade_long_xau";
   else if(valid && direction < 0)
      state = "qualified_conover_mean_fade_short_xau";
   else if(valid && tie_detected)
      state = "absolute_deviation_tie_flat";
   else if(valid && recent_score < strategy_score_min)
      state = "conover_score_below_min_flat";
   else if(valid && tail_count > strategy_tail_count_max)
      state = "conover_tail_above_cap_flat";
   else if(valid && MathAbs(mean_delta) <= strategy_direction_epsilon)
      state = "neutral_raw_mean_shift_flat";
   else if(valid)
      state = "nonqualifying_conover_flat";

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"month_count\":%d,\"newest_endpoint_time\":%I64d,\"old_mean\":%.12e,\"recent_mean\":%.12e,\"recent_score\":%d,\"score_total\":%d,\"score_expected\":%d,\"score_min\":%d,\"assignments\":%d,\"tail_count\":%d,\"tie_detected\":%s,\"direction\":%d,\"endpoints\":\"%s\",\"ratios\":\"%s\",\"returns\":\"%s\",\"deviations\":\"%s\",\"ranks\":\"%s\",\"state\":\"%s\"}",
                            g_signal_month_key,
                            month_count,
                            ...
                            recent_mean,
                            recent_score,
                            strategy_score_total,
                            strategy_score_expected,
                            strategy_score_min,
                            assignment_count,
                            tail_count,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG monthly Conover scale reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41281\",\"ea\":\"xauxag-mconover-scale-rv\"}");
