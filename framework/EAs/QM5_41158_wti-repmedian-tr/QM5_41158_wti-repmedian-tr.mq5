...
#property description "QM5_41158 WTI Thirteen-Month Repeated-Median Trend"
...
//   - take logarithms and form twelve forward-oriented slopes per pivot
//   - take the even median inside each of thirteen pivot groups
//   - take the odd median across the thirteen pivot medians
//   - follow the strict repeated-median sign for one broker month
//   - consume before fallible gates; never enter late or retry
// The exact nested estimator is a transparent QM mechanization. Runtime uses
...
int      g_grouped_slope_count     = 0;
int      g_pivot_median_count      = 0;
datetime g_oldest_endpoint_time    = 0;
datetime g_newest_endpoint_time    = 0;
double   g_inner_median_min        = 0.0;
double   g_inner_median_max        = 0.0;
double   g_repeated_median         = 0.0;
...
void Strategy_ResetDecisionState()
  {
   ...
   g_endpoint_count = 0;
   g_grouped_slope_count = 0;
   g_pivot_median_count = 0;
   g_oldest_endpoint_time = 0;
   g_newest_endpoint_time = 0;
   g_inner_median_min = 0.0;
   g_inner_median_max = 0.0;
   g_repeated_median = 0.0;
   g_signal_state = "idle";
  }
...
bool Strategy_RepeatedMedianSignal(const double &closes[],
                                   int &direction,
                                   int &endpoint_count,
                                   ...
                                   double &repeated_median)
  {
   ...
   const int slopes_per_pivot = point_count - 1;
   if(slopes_per_pivot != 12)
      return false;
   for(int pivot = 0; pivot < point_count; ++pivot)
     {
      double pivot_slopes[];
      if(ArrayResize(pivot_slopes, slopes_per_pivot) !=
         slopes_per_pivot)
         return false;
      ...
      for(int other = 0; other < point_count; ++other)
        {
         ...
         if(month_distance <= 0 ||
            ...
            return false;
         const double slope =
            (log_prices[upper] - log_prices[lower]) /
      ...
      if(slope_index != slopes_per_pivot)
         return false;
      grouped_slope_count += slope_index;
   ...
   for(int index = 0; index < pivot_median_count; ++index)
     {
      if(!MathIsValidNumber(pivot_medians[index]) ||
         (index > 0 &&
          pivot_medians[index] < pivot_medians[index - 1]))
         return false;
     }
                                        ...
                                        int &direction)
  {
   ...
   const bool valid =
      Strategy_LoadMonthlyEndpoints(current_month_key,
                                    ...
                                    closes,
                                    endpoint_times) &&
      Strategy_RepeatedMedianSignal(closes,
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   else
     {
      ...
      g_signal_valid =
         ...
         Strategy_RepeatedMedianSignal(closes,
                                       g_signal_direction,
                                       g_endpoint_count,
                                       g_grouped_slope_count,
                                       g_pivot_median_count,
                                       g_inner_median_min,
                                       g_inner_median_max,
                                       g_repeated_median);
      if(!g_signal_valid)
         g_signal_state = "repeated_median_validation_failed";
      else if(g_repeated_median == 0.0)
         g_signal_state = "zero_repeated_median_flat";
      else if(g_signal_direction > 0)
         g_signal_state = "repeated_median_wti_long";
      else if(g_signal_direction < 0)
         g_signal_state = "repeated_median_wti_short";
      else
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"endpoint_count\":%d,\"grouped_slope_count\":%d,\"pivot_median_count\":%d,\"oldest_endpoint\":%I64d,\"newest_endpoint\":%I64d,\"inner_median_min\":%.12e,\"inner_median_max\":%.12e,\"repeated_median\":%.12e,\"state\":\"%s\"}",
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41158_WTI_REPMEDIAN_TREND";
   ...
   req.reason = (g_signal_direction > 0)
      ? "WTI_REPMEDIAN_TREND_LONG"
      : "WTI_REPMEDIAN_TREND_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41158 ||
      qm_magic_slot_offset != 0)
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI repeated-median trend position holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41158_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41158\",\"ea\":\"wti-repmedian-tr\"}");
