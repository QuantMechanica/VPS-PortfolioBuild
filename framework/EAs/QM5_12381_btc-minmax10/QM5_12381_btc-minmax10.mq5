...
#property description "QM5_12381 BTC MINMAX10 ported CFD daily continuation/rebound"
...
input int    strategy_lookback_days       = 10;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   for(int i = 1; i <= strategy_lookback_days; ++i)
     {
      ...
      lowest_close = MathMin(lowest_close, close_i);
     }

   const int spread_now = (int)SymbolInfoInteger(_Symbol, SYMBOL_SPREAD);
   if(spread_now > 0 && strategy_spread_median_days > 0)
     {
      double spreads[];
      ...
      for(int i = 1; i <= strategy_spread_median_days; ++i)
        {
         ...
         spreads[n] = (double)spread_i;
        }

      const int count = ArraySize(spreads);
      if(count > 0)
        {
         ArraySort(spreads);
         ...
         if(median_spread > 0.0 && (double)spread_now > 2.0 * median_spread)
            return false;
        }
     }
   ...
   const bool at_high10 = (MathAbs(signal_close - highest_close) <= point * 0.1);
   const bool at_low10 = (MathAbs(signal_close - lowest_close) <= point * 0.1);
   if(!at_high10 && !at_low10)
      return false;
   ...
   if(req.sl <= 0.0)
      return false;
   req.reason = at_high10 ? "MAX10_CONTINUATION_LONG" : "MIN10_REBOUND_LONG";
   return true;
  }
...
bool Strategy_ExitSignal()
  {
   ...
   const int hold_seconds = strategy_hold_bars * 86400;
