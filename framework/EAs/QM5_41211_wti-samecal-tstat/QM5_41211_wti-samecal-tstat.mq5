...
#property description "QM5_41211 WTI Same-Calendar One-Standard-Error Seasonality"
...
//   - compute mean, n-1 sample variance, standard error, and t=mean/SE
...
double   g_signal_mean            = 0.0;
double   g_signal_sample_variance = 0.0;
double   g_signal_standard_error  = 0.0;
double   g_signal_t_statistic     = 0.0;
...
void Strategy_ResetDecisionState()
  {
   ...
   g_signal_mean = 0.0;
   g_signal_sample_variance = 0.0;
   g_signal_standard_error = 0.0;
   g_signal_t_statistic = 0.0;
   g_signal_state = "idle";
  }
...
// -----------------------------------------------------------------------------
// Exact completed-month endpoint and one-sample t-score helpers.
// -----------------------------------------------------------------------------
...
bool Strategy_TStatisticSignal(const double &observations[],
                               const int sample_count,
                               double &mean_value,
                               double &sample_variance_value,
                               double &standard_error_value,
                               double &t_value,
                               int &direction)
  {
   mean_value = 0.0;
   sample_variance_value = 0.0;
   ...
   if(strategy_history_years != 10 ||
      ...
      MathAbs(strategy_t_threshold - 1.0) > 1.0e-12 ||
   ...
   for(int index = 0; index < sample_count; ++index)
     {
      if(index >= ArraySize(observations) ||
         ...
         return false;
      observation_sum += observations[index];
      if(!MathIsValidNumber(observation_sum))
         return false;
     }

   mean_value = observation_sum / (double)sample_count;
   if(!MathIsValidNumber(mean_value))
      return false;
   ...
   for(int index = 0; index < sample_count; ++index)
     {
      if(index >= ArraySize(observations))
         return false;
      const double deviation = observations[index] - mean_value;
   ...
   if(!MathIsValidNumber(sample_variance_value) ||
      sample_variance_value <= 0.0)
      return false;

   standard_error_value =
      MathSqrt(sample_variance_value / (double)sample_count);
   if(!MathIsValidNumber(standard_error_value) ||
      standard_error_value <= 0.0)
      return false;

   t_value = mean_value / standard_error_value;
   if(!MathIsValidNumber(t_value))
      return false;
   ...
   const double lower_gate =
      -strategy_t_threshold - strategy_signal_tolerance;
   if(t_value > upper_gate)
      direction = 1;
   else if(t_value < lower_gate)
      direction = -1;
   return true;
  }

bool Strategy_LoadTStatisticSignal(const datetime decision_bar_time,
                                   ...
                                   int &direction)
  {
   ...
   return Strategy_TStatisticSignal(observations,
                                    sample_count,
                                    mean_value,
                                    sample_variance_value,
                                    standard_error_value,
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   // Consume before history, t-statistic arithmetic, news, spread, quote, ATR,
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadTStatisticSignal(g_decision_bar_time,
                                       g_decision_month_key,
                                       g_decision_label_offset,
                                       g_signal_sample_count,
                                       g_signal_mean,
                                       g_signal_sample_variance,
                                       g_signal_standard_error,
                                       g_signal_t_statistic,
                                       g_signal_direction);
      if(!g_signal_valid)
         g_signal_state = "endpoint_sample_or_t_statistic_failed";
      else if(g_signal_direction > 0)
         g_signal_state = "t_statistic_long";
      else if(g_signal_direction < 0)
         g_signal_state = "t_statistic_short";
      else
         g_signal_state = "t_statistic_band_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"valid\":%s,\"samples\":%d,\"mean\":%.12g,\"sample_variance\":%.12g,\"standard_error\":%.12g,\"t_statistic\":%.12g,\"direction\":%d,\"state\":\"%s\"}",
                            g_decision_month_key,
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(!QM_InputRequireLong("qm_ea_id", qm_ea_id, 41211) ||
      ...
      !QM_InputRequireDouble("strategy_t_threshold", strategy_t_threshold, 1.0, 1.0e-12) ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   if(g_signal_direction > 0)
     {
      req.type = QM_BUY;
      req.reason = "WTI_SAMECAL_TSTAT_LONG";
     }
   else
     {
      req.type = QM_SELL;
      req.reason = "WTI_SAMECAL_TSTAT_SHORT";
     }
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41211 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI same-calendar one-standard-error card holds to the next broker month"))
      return INIT_FAILED;
   ...
   g_attempt_state_key =
      StringFormat("QM5_41211_SAMECAL_TSTAT_MONTH_ATTEMPT_%d",
                   QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41211\",\"ea\":\"wti-samecal-tstat\"}");
