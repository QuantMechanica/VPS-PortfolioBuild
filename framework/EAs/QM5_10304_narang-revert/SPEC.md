...
For fixed-risk tester runs on a fiat `.DWX` cross, the EA pre-loads the
direct or inverse USD-account conversion histories selected by the framework
risk sizer. For example, `EURNZD.DWX` requires `NZDUSD.DWX` for tick-value
conversion and `EURUSD.DWX` for margin conversion. These are data-only
dependencies: they are not signal inputs, order routes, or magic-number slots,
and the framework symbol guard remains in single-symbol mode.
...
|---|---|---|---|
| v1.1 | 2026-08-08 | Q02 infrastructure recovery | Pre-load deterministic fixed-risk currency-conversion histories before the first trade is sized. |
