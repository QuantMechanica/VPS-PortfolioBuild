# QM5_41263_xauxag-mkuiper-rv - Strategy Spec
...
**Source:** `AI-CODEX-XAUXAG-MKUIPER-RV-20260901`
...
changes while preserving sample labels. Across pooled cuts it computes:

```text
D_plus  = max(F_recent - F_old)
D_minus = max(F_old - F_recent)
V       = D_plus + D_minus
```
...
`V_perm + 1e-12*max(1,abs(V_observed)) >= V_observed`. The signal requires
`V_observed>=0.5` and `tail_count<=798`. For a qualified tail, recent pooled-
rank sum above neutral 39 sells XAU and buys XAG; a sum below 39 buys XAU and
sells XAG. Exact ties, a neutral sum, invalid enumeration, or a smaller
distance consume the month flat. Statistic magnitude never changes risk.
...
| `strategy_min_kuiper_v` | 0.5 | locked 0.5 | minimum opposing-extrema distance |
| `strategy_tail_count_max` | 798 | locked 798 | inclusive exact-tail count cap |
...
- `XAUUSD.DWX` - exact D1 host, traded slot 0, governed magic `412630000`.
- `XAGUSD.DWX` - exact D1 companion, traded slot 1, governed magic
  `412630001`.
- `QM5_41263_XAU_XAG_MKUIPER_RV_D1` - logical tester symbol hosted on XAU.
...
| Metric | Expected |
|---|---|
| Packages / year | approximately 9-10 combinatorial states before market and execution gates; at least 5 completed packages in every full post-warm-up year |
| Decision frequency | one consumed attempt per broker month; the market-free strict-rank reference leaves 760 directional assignments among 924, about 9.870 states per twelve attempts |
...
`strategy-seeds/cards/approved/QM5_41263_xauxag-mkuiper-rv_card.md`; the runtime
mirror is `framework/EAs/QM5_41263_xauxag-mkuiper-rv/docs/strategy_card.md`.

Schweikert (2018) and CME Group support only the state-dependent gold/silver
relationship and intermarket carrier. Kuiper (1960), DOI
`10.1016/S1385-7258(60)50006-0`, plus CRAN `twosamples` 2.0.1 pinned at commit
`4923388cdb14be4875a7041cddd69629a6bfc735`, support only the continuous
two-sample opposing-ECDF-extrema statistic and a permutation route. The
adjacent-change state, fixed split, activity boundary, contrarian side, CFD
translation, risk, and lifecycle are pre-result QM choices.
...
changes, adds the largest positive and negative ECDF gaps, and enumerates the
exact 924-assignment inclusive tail. Locked label paths establish both
decision-disagreement directions against the closest Anderson-Darling rule.
...
- `no_trade`: exact host, period, identity, slots, magics, risk, news, Friday,
  stress, strategy locks, clock, history, Kuiper arithmetic, and
  package state.
...
|---|---|---|---|
| v1 | 2026-09-01 | Initial build from approved card | OWNER commodity portfolio mission; governed magics `412630000` and `412630001` |
