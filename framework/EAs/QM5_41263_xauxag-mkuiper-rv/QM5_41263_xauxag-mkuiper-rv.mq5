...
#property description "QM5_41263 XAU/XAG Monthly Exact Kuiper Reversion"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_41263 - XAU/XAG Monthly Exact Kuiper Reversion
// -----------------------------------------------------------------------------
...
// returns into fixed old/recent blocks of six, compute the continuous no-tie
// two-sample Kuiper D-plus plus D-minus distance, and enumerate all
// C(12,6)=924 label assignments for an inclusive exact upper tail. A distance
// of at least one half with tail count at most 798 fades the recent block's
// pooled-rank displacement with an opposite, equal-notional package held to
// the next month. Runtime is native, completed-price-only, deterministic, and
// one consumed attempt per month.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41263;
...
input double strategy_min_kuiper_v             = 0.5;
input int    strategy_tail_count_max          = 798;
input double strategy_stat_epsilon            = 0.000000000001;
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41263 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
            ...
            MathAbs(strategy_min_kuiper_v - 0.5) <= 1.0e-12 &&
            strategy_tail_count_max == 798 &&
            MathAbs(strategy_stat_epsilon - 1.0e-12) <= 1.0e-18 &&
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41263_XAUXAG_MKUIPER_RV";
...
bool Strategy_KuiperFromLabels(const int &labels[],
                               double &statistic,
                               double &d_plus,
                               double &d_minus)
  {
   ...
   for(int index = 0; index < strategy_return_count; ++index)
     {
      if(labels[index] == 0)
         ...
         ++recent_seen;
      const double old_ecdf =
         (double)old_seen / (double)strategy_block_size;
      const double recent_ecdf =
         (double)recent_seen / (double)strategy_block_size;
      const double recent_minus_old = recent_ecdf - old_ecdf;
      const double old_minus_recent = old_ecdf - recent_ecdf;
      if(!MathIsValidNumber(recent_minus_old) ||
         !MathIsValidNumber(old_minus_recent))
         return false;
      d_plus = MathMax(d_plus, recent_minus_old);
      d_minus = MathMax(d_minus, old_minus_recent);
     }
   statistic = d_plus + d_minus;
   return (MathIsValidNumber(d_plus) && MathIsValidNumber(d_minus) &&
           MathIsValidNumber(statistic) && d_plus >= 0.0 && d_minus >= 0.0 &&
           statistic >= 0.0 && statistic <= 1.0 + strategy_stat_epsilon);
  }
...
bool Strategy_SortPooledReturns(double &values[], int &labels[])
  {
   ...
   for(int index = 1; index < strategy_return_count; ++index)
      if(!MathIsValidNumber(values[index - 1]) ||
         ...
         return false; // The approved continuous-distribution baseline rejects exact ties.
   return MathIsValidNumber(values[0]);
  }

bool Strategy_LoadMonthlyKuiperSignal(
      const int current_month_key,
      int &month_count,
      datetime &newest_endpoint_time,
      double &observed_v,
      double &observed_d_plus,
      double &observed_d_minus,
      ...
      int &direction)
  {
   ...
   for(int left = 0; left < strategy_return_count; ++left)
     {
      for(int right = left + 1; right < strategy_return_count; ++right)
         if(relative_returns[left] == relative_returns[right])
            return true; // Exact pooled tie: consume the month flat.
     }
   ...
   for(int index = 0; index < strategy_return_count; ++index)
     {
      if(pooled_labels[index] == 1)
        {
         recent_rank_sum += index + 1;
         label_path += "R";
        }
      else
         label_path += "O";
     }
   if(recent_rank_sum < 21 || recent_rank_sum > 57 ||
      !Strategy_KuiperFromLabels(pooled_labels,
                                 observed_v,
                                 observed_d_plus,
                                 observed_d_minus))
      return false;
   ...
   const double tail_epsilon =
      strategy_stat_epsilon * MathMax(1.0, MathAbs(observed_v));
   ...
   for(int mask = 0; mask < (1 << strategy_return_count); ++mask)
     {
      ...
      for(int index = 0; index < strategy_return_count; ++index)
         permutation_labels[index] =
            ((mask & (1 << index)) != 0) ? 1 : 0;
      double permutation_v = 0.0;
      double permutation_d_plus = 0.0;
      double permutation_d_minus = 0.0;
      if(!Strategy_KuiperFromLabels(permutation_labels,
                                    permutation_v,
                                    permutation_d_plus,
                                    permutation_d_minus))
         return false;
      ++assignment_count;
      if(permutation_v + tail_epsilon >= observed_v)
         ++tail_count;
     }
   ...
   const bool qualifies =
      observed_v + strategy_stat_epsilon >= strategy_min_kuiper_v &&
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41263_XAUXAG_MKUIPER_RV_HOST";
   ...
   double observed_v = 0.0;
   double observed_d_plus = 0.0;
   double observed_d_minus = 0.0;
   string ratio_path = "";
   string return_path = "";
   string label_path = "";
   const bool valid =
      Strategy_LoadMonthlyKuiperSignal(
         g_signal_month_key,
         month_count,
   ...
   string state = "monthly_kuiper_signal_failed";
   ...
   else if(valid && assignment_count == 0)
      state = "pooled_return_tie_flat";
   else if(valid && tail_count > strategy_tail_count_max)
      state = "exact_upper_tail_above_locked_cutoff_flat";
   else if(valid &&
           observed_v + strategy_stat_epsilon < strategy_min_kuiper_v)
      state = "kuiper_distance_below_locked_half_flat";
   else if(valid && recent_rank_sum == strategy_neutral_rank_sum)
      state = "neutral_recent_rank_sum_flat";
   else if(valid)
      state = "nonqualifying_exact_kuiper_flat";

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"month_count\":%d,\"newest_endpoint_time\":%I64d,\"observed_v\":%.12e,\"d_plus\":%.12e,\"d_minus\":%.12e,\"assignment_count\":%d,\"tail_count\":%d,\"recent_rank_sum\":%d,\"direction\":%d,\"ratios\":\"%s\",\"returns\":\"%s\",\"pooled_labels\":\"%s\",\"state\":\"%s\"}",
                            g_signal_month_key,
                            month_count,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG monthly exact Kuiper reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41263\",\"ea\":\"xauxag-mkuiper-rv\"}");
