...
#property description "QM5_41115 WTI Completed-Month Three-Block Vote Momentum"
...
//   - reconstruct two consecutive completed broker-calendar months
//   - anchor the newest-month close path at the parent final close
//   - split every adjacent return into three exhaustive chronological blocks
//   - follow the strict two-of-three cumulative block-sign majority
//   - consume the month before fallible gates; never enter late or retry
//   - follow the block-voted completed-month direction for one month
// Runtime uses MT5-native completed closes, calendar, history, and framework state only.
// =============================================================================
...
int      g_new_month_bars          = 0;
int      g_parent_month_bars       = 0;
int      g_first_boundary          = 0;
int      g_second_boundary         = 0;
double   g_parent_final_close       = 0.0;
double   g_first_boundary_close     = 0.0;
double   g_second_boundary_close    = 0.0;
double   g_newest_final_close       = 0.0;
double   g_first_block_return       = 0.0;
double   g_second_block_return      = 0.0;
double   g_third_block_return       = 0.0;
...
void Strategy_ResetDecisionState()
  {
   ...
   g_new_month_bars = 0;
   g_parent_month_bars = 0;
   g_first_boundary = 0;
   g_second_boundary = 0;
   g_parent_final_close = 0.0;
   g_first_boundary_close = 0.0;
   g_second_boundary_close = 0.0;
   g_newest_final_close = 0.0;
   g_first_block_return = 0.0;
   g_second_block_return = 0.0;
   g_third_block_return = 0.0;
   g_signal_state = "idle";
  }
...
bool Strategy_LoadThreeBlockVoteSignal(const int current_month_key,
                                       const int label_offset,
                                       int &direction,
                                       int &new_month_bars,
                                       int &parent_month_bars,
                                       int &first_boundary,
                                       int &second_boundary,
                                       double &parent_final_close,
                                       double &first_boundary_close,
                                       double &second_boundary_close,
                                       double &newest_final_close,
                                       double &first_block_return,
                                       double &second_block_return,
                                       double &third_block_return)
  {
   direction = 0;
   new_month_bars = 0;
   parent_month_bars = 0;
   first_boundary = 0;
   second_boundary = 0;
   parent_final_close = 0.0;
   first_boundary_close = 0.0;
   second_boundary_close = 0.0;
   newest_final_close = 0.0;
   first_block_return = 0.0;
   second_block_return = 0.0;
   third_block_return = 0.0;
   if(current_month_key <= 0 ||
   ...
   const int copied =
      CopyRates(_Symbol, // perf-allowed: bounded completed-month three-block scan behind the sole QM_IsNewBar branch.
                ...
                strategy_history_bars_d1,
                bars);
   if(copied < strategy_min_month_sessions * 2 + 1)
      return false;

   double newest_closes[];
   if(ArrayResize(newest_closes,
                  strategy_max_month_sessions) !=
      strategy_max_month_sessions)
      return false;

   int month_keys[2] = {0, 0};
   int month_counts[2] = {0, 0};
   int month_last_date_keys[2] = {0, 0};
   int found_months = 0;
   ...
   for(int index = 0; index < copied; ++index)
     {
      ...
      if(found_months == 0 ||
         month_key != month_keys[found_months - 1])
        {
         if(found_months >= 2)
           {
            if(Strategy_NextMonthKey(month_key) != month_keys[1])
               return false;
         ...
         else if(Strategy_NextMonthKey(month_key) !=
                 month_keys[found_months - 1])
            return false;

         month_keys[found_months] = month_key;
         ++found_months;
        }

      const int bucket = found_months - 1;
      if(bucket < 0 || bucket > 1 ||
         !Strategy_BarCloseValid(bars[index]))
         return false;
      const int session_date_key = Strategy_DateKeyForTime(normalized);
      if(session_date_key <= 0 ||
         (month_last_date_keys[bucket] > 0 &&
          session_date_key >= month_last_date_keys[bucket]))
         return false;
      month_last_date_keys[bucket] = session_date_key;

      if(bucket == 0)
        {
         if(month_counts[bucket] >= ArraySize(newest_closes))
            return false;
         newest_closes[month_counts[bucket]] = bars[index].close;
        }
      else if(month_counts[bucket] == 0)
         parent_final_close = bars[index].close;

      ++month_counts[bucket];
      if(month_counts[bucket] > strategy_max_month_sessions)
         return false;
     }

   if(found_months != 2 || !parent_boundary_seen)
      return false;
   for(int bucket = 0; bucket < 2; ++bucket)
     {
      if(month_counts[bucket] < strategy_min_month_sessions ||
         month_counts[bucket] > strategy_max_month_sessions)
         return false;
     }
   if(parent_final_close <= 0.0 ||
      !MathIsValidNumber(parent_final_close))
      return false;

   new_month_bars = month_counts[0];
   parent_month_bars = month_counts[1];
   first_boundary = new_month_bars / 3;
   second_boundary = (2 * new_month_bars) / 3;
   if(first_boundary < 1 ||
      second_boundary <= first_boundary ||
      second_boundary >= new_month_bars)
      return false;

   // newest_closes[] is stored newest-first. Chronological C[a-1] and
   // C[b-1] are therefore newest_closes[n-a] and newest_closes[n-b].
   // Shared closes are return anchors; no adjacent return is duplicated.
   ...
   newest_final_close = newest_closes[0];
   if(first_boundary_close <= 0.0 || second_boundary_close <= 0.0 ||
      newest_final_close <= 0.0 ||
      !MathIsValidNumber(first_boundary_close) ||
      !MathIsValidNumber(second_boundary_close) ||
      !MathIsValidNumber(newest_final_close))
      return false;
   ...
   if(!MathIsValidNumber(first_block_return) ||
      !MathIsValidNumber(second_block_return) ||
      !MathIsValidNumber(third_block_return))
      return false;

   int positive_votes = 0;
   int negative_votes = 0;
   if(first_block_return > 0.0)
      ++positive_votes;
   else if(first_block_return < 0.0)
      ++negative_votes;
   if(second_block_return > 0.0)
      ++positive_votes;
   else if(second_block_return < 0.0)
      ++negative_votes;
   if(third_block_return > 0.0)
      ++positive_votes;
   else if(third_block_return < 0.0)
      ++negative_votes;
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   // Consume before close validation, partition calculation, news, spread, quote,
   // ATR, sizing, or order gates. The broker-month clock is the prerequisite.
   ...
   if(Strategy_MonthAlreadyEntered(g_decision_month_key))
     ...
     {
      g_signal_valid =
         Strategy_LoadThreeBlockVoteSignal(g_decision_month_key,
                                           ...
                                           g_first_boundary,
                                           g_second_boundary,
                                           g_parent_final_close,
                                           g_first_boundary_close,
                                           g_second_boundary_close,
                                           g_newest_final_close,
                                           g_first_block_return,
                                           g_second_block_return,
                                           g_third_block_return);
      if(!g_signal_valid)
         g_signal_state = "monthly_close_path_validation_failed";
      else if(g_signal_direction > 0)
         g_signal_state = "three_block_vote_long";
      else if(g_signal_direction < 0)
         g_signal_state = "three_block_vote_short";
      else if(g_first_block_return == 0.0 ||
              g_second_block_return == 0.0 ||
              g_third_block_return == 0.0)
         g_signal_state = "zero_without_majority_flat";
      else
         g_signal_state = "no_strict_majority_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_month_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"new_month_bars\":%d,\"parent_month_bars\":%d,\"first_boundary\":%d,\"second_boundary\":%d,\"parent_final_close\":%.8f,\"first_boundary_close\":%.8f,\"second_boundary_close\":%.8f,\"newest_final_close\":%.8f,\"first_block_return\":%.12e,\"second_block_return\":%.12e,\"third_block_return\":%.12e,\"state\":\"%s\"}",
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41115 ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41115_WTI_MTHIRDVOTE_MOM";
   ...
   req.reason = (g_signal_direction > 0)
      ? "WTI_MTHIRDVOTE_MOM_LONG"
      : "WTI_MTHIRDVOTE_MOM_SHORT";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41115 ||
   ...
   g_attempt_state_key =
      StringFormat("QM5_41115_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
                "{\"card\":\"QM5_41115\",\"ea\":\"wti-mthirdvote-mom\"}");
