...
#property description "QM5_41375 WTI Pure One-Week Momentum"
...
//   - aggregate the immediately completed 3-to-5-session broker week
//   - compute log(final close / first open), with no volatility/range filter
//   - follow the completed week's strict return sign
...
bool Strategy_LoadWeeklyMomentumSignal(const int current_week_key,
                                       ...
                                       double &week_log_return)
  {
   ...
   ArraySetAsSeries(bars, true);
   const int copied =
      CopyRates(_Symbol, // perf-allowed: bounded completed-week return scan behind the sole QM_IsNewBar branch.
                PERIOD_D1,
                1,
                strategy_history_bars,
                bars);
   ...
   for(int index = 0; index < copied; ++index)
     {
      ...
      // close; overwriting open leaves the first session's open.
      if(week_bars == 0)
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   if(Strategy_WeekAlreadyEntered(g_decision_week_key))
   ...
   else
     {
      ...
      if(!g_signal_valid)
         ...
         g_signal_state = "positive_week_long";
      else if(g_signal_direction < 0)
         g_signal_state = "negative_week_short";
      else
   ...
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"week\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"completed_current_week_bars\":%d,\"late\":%s,\"valid\":%s,\"signal\":%d,\"week_bars\":%d,\"week_open\":%.8f,\"week_close\":%.8f,\"week_log_return\":%.12f,\"state\":\"%s\"}",
                            g_decision_week_key,
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41375_WTI_WMOM1";
...
int OnInit()
  {
   if(!SymbolSelect(strategy_symbol, true) ||
       !Strategy_IsHostChart() || qm_ea_id != 41375 ||
      qm_magic_slot_offset != 0)
   ...
   g_attempt_state_key =
      StringFormat("QM5_41375_WEEK_ATTEMPT_%d", QM_FrameworkMagic());
