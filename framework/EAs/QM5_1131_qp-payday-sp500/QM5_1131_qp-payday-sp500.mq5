...
#property description "QM5_1131 Quantpedia Payday Anomaly - SP500"

#include <QM/QM_Common.mqh>

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 1131;
...
input int    strategy_payday_day          = 16;
...
datetime Strategy_FirstPaydayTradeDate(const int year, const int month)
  {
   datetime candidate = Strategy_MakeDate(year, month, MathMax(1, MathMin(28, strategy_payday_day)));
   for(int i = 0; i < 10; ++i)
     {
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(!Strategy_CurrentNyDateTime(ny))
      return true;
   if(!Strategy_IsRegularUsCashDayNY(Strategy_DateFloor(Strategy_BrokerToNewYork(TimeCurrent()))))
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QP_PAYDAY_SP500_LONG";
   ...
   const datetime payday = Strategy_FirstPaydayTradeDate(ny.year, ny.mon);
   if(payday <= 0 || !Strategy_SameDate(today_ny, payday))
      return false;
...
int OnInit()
  {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK", "{\"card\":\"QM5_1131\",\"ea\":\"qp-payday-sp500\"}");
