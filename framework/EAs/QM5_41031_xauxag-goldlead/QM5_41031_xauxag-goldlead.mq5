...
#property description "QM5_41031 XAU/XAG Asymmetric Gold-Lead Catch-Up"
...
// -----------------------------------------------------------------------------
// On each new synchronized D1 session, compare the two most recent completed
// closes. A sufficiently large gold return may lead silver only when silver
// moved less than half as far and did not overshoot in absolute terms. Trade
// the one-session same-direction catch-up as opposite equal-notional legs,
// then flatten both legs at the first following XAU D1 boundary.
// =============================================================================

input group "QuantMechanica V5 Framework"
input int    qm_ea_id                    = 41031;
...
input double strategy_gold_shock_abs_return  = 0.0075;
input double strategy_silver_response_fraction = 0.50;
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41031 && qm_magic_slot_offset == 0 &&
            strategy_xag_symbol == "XAGUSD.DWX" &&
            strategy_entry_grace_minutes == 180 &&
            MathAbs(strategy_gold_shock_abs_return - 0.0075) <= 1.0e-12 &&
            MathAbs(strategy_silver_response_fraction - 0.50) <= 1.0e-12 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_GOLDLEAD_ATTEMPT_DATE", qm_ea_id);
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41031_XAUXAG_GOLDLEAD";
...
bool Strategy_LoadGoldLead(double &gold_return,
                           double &silver_return,
                           int &direction)
  {
   gold_return = 0.0;
   silver_return = 0.0;
   ...
   const int xau_copied =
      CopyRates(g_leg_xau, // perf-allowed: entry-only two-close return window.
                PERIOD_D1, 1, 2, xau_bars);
   const int xag_copied =
      CopyRates(g_leg_xag, // perf-allowed: entry-only two-close return window.
                PERIOD_D1, 1, 2, xag_bars);
   ...
   if(!Strategy_GapHoursAllowed(g_current_host_bar,
                                xau_bars[0].time, 20, 96))
      return false;
   if(!Strategy_GapHoursAllowed(xau_bars[0].time,
                                xau_bars[1].time, 20, 96))
      return false;

   gold_return = MathLog(xau_bars[0].close / xau_bars[1].close);
   silver_return = MathLog(xag_bars[0].close / xag_bars[1].close);
   if(!MathIsValidNumber(gold_return) ||
      !MathIsValidNumber(silver_return))
      return false;
   if(gold_return >= strategy_gold_shock_abs_return &&
      silver_return < strategy_silver_response_fraction * gold_return &&
      MathAbs(silver_return) <= MathAbs(gold_return))
      direction = -1; // Sell gold, buy silver for same-direction catch-up.
   else if(gold_return <= -strategy_gold_shock_abs_return &&
           silver_return > strategy_silver_response_fraction * gold_return &&
           MathAbs(silver_return) <= MathAbs(gold_return))
      direction = 1; // Buy gold, sell silver for same-direction catch-up.
   return true;
  }
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41031_XAUXAG_GOLDLEAD_HOST";
   ...
   double silver_return = 0.0;
   int direction = 0;
   if(!Strategy_LoadGoldLead(gold_return, silver_return, direction) ||
      direction == 0)
      return false;
   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"date\":%d,\"gold_return\":%.12e,\"silver_return\":%.12e,\"direction\":%d}",
                            g_signal_date_key,
                            gold_return,
                            silver_return,
                            direction));
...
bool Strategy_PrimeLateSignalAttach()
  {
   ...
   // A late attachment consumes and primes the current D1 edge so the
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved one-session XAU/XAG package uses paired Friday 21 fail-safe"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41031\",\"ea\":\"xauxag-goldlead\"}");
...
void OnTick()
  {
   ...
   // Consume this broker date before history, signal, news, spread, quote,
