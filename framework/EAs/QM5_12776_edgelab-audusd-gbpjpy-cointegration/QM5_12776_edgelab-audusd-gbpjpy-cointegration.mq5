...
#property description "QM5_12776 Edge Lab AUDUSD GBPJPY Cointegration"
...
input double strategy_beta              = 0.189701;
...
bool Strategy_IsHostSymbol()
  {
   return (_Symbol == g_leg_audusd || _Symbol == g_leg_gbpjpy);
  }
...
bool Strategy_EnsureBasketScope()
  {
   ...
   string allowed[3] = {"AUDUSD.DWX", "GBPJPY.DWX", "USDJPY.DWX"};
   for(int i = 0; i < 3; ++i)
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(!Strategy_EnsureBasketScope())
      return false;
   if(!QM_SymbolAssertOrLog(g_leg_audusd) || !QM_SymbolAssertOrLog(g_leg_gbpjpy))
      return false;
   ...
   for(int i = 0; i < lookback; ++i)
     {
      if(audusd[i] <= 0.0 || gbpjpy[i] <= 0.0)
         return false;
      spreads[i] = MathLog(audusd[i]) - strategy_beta * MathLog(gbpjpy[i]);
...
bool Strategy_OpenPair(const int spread_direction)
  {
   ...
   const double weight_sum = audusd_weight + gbpjpy_weight;
   if(weight_sum <= 0.0)
      return false;
   ...
   const string reason = long_spread ? "QM5_12776_LONG_SPREAD_Z_LT_NEG_ENTRY"
                                     : "QM5_12776_SHORT_SPREAD_Z_GT_POS_ENTRY";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12776_PAIR_HOST";
