# QM5_13000_xng-rig-fri-fade - Strategy Spec
...
The EA trades a single-symbol natural-gas sleeve tied to the weekly Baker
Hughes North America rig-count release cadence. On the first D1 bar of a new
broker week, it checks the prior completed D1 bar, which is the final workday of
the prior week and the market-reaction proxy for the rig-count window. If that
bar has a large close-to-close displacement and closes near the bar extreme,
the EA enters opposite the displacement.

This is not `QM5_12567` cumulative RSI2 commodity pullback logic. It is also
not the existing `QM5_12997_xng-rig-fri-mom` continuation card: this EA fades
the same event-proxy displacement instead of following it.
...
`130000000`. The EA rejects any other host symbol or timeframe.
...
Expected frequency is low, approximately 5-14 trades per year before Q02
confirms realized history. Entries occur only on the first new-week D1 bar
after a qualifying prior final-workday displacement. Positions close by ATR hard
stop, time stop, framework Friday close handling, favorable reversion close, or
