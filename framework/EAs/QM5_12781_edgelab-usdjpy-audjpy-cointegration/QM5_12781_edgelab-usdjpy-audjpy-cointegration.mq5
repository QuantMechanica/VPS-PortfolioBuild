...
#property description "QM5_12781 Edge Lab USDJPY AUDJPY Cointegration"
...
input double strategy_beta              = 0.763733;
...
bool Strategy_IsHostSymbol()
  {
   return (_Symbol == g_leg_USDJPY || _Symbol == g_leg_audjpy);
  }
...
bool Strategy_RefreshSpreadState()
  {
   ...
   if(!Strategy_EnsureBasketScope())
      return false;
   if(!QM_SymbolAssertOrLog(g_leg_USDJPY) || !QM_SymbolAssertOrLog(g_leg_audjpy))
      return false;
   ...
   for(int i = 0; i < lookback; ++i)
     {
      if(USDJPY[i] <= 0.0 || audjpy[i] <= 0.0)
         return false;
      spreads[i] = MathLog(USDJPY[i]) - strategy_beta * MathLog(audjpy[i]);
...
bool Strategy_OpenPair(const int spread_direction)
  {
   ...
   const double weight_sum = USDJPY_weight + audjpy_weight;
   if(weight_sum <= 0.0)
      return false;
   ...
   const string reason = long_spread ? "QM5_12781_LONG_SPREAD_Z_LT_NEG_ENTRY"
                                     : "QM5_12781_SHORT_SPREAD_Z_GT_POS_ENTRY";
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_12781_PAIR_HOST";
