...
EA_SOURCE = EA_DIR / "QM5_41336_wti-adf-kpss-agree-tr.mq5"
SETFILE = (
    EA_DIR
    / "sets"
    / "QM5_41336_wti-adf-kpss-agree-tr_XTIUSD.DWX_D1_backtest.set"
)
CANONICAL_CARD = (
    REPO_ROOT
    / "strategy-seeds"
    / "cards"
    / "approved"
    / "QM5_41336_wti-adf-kpss-agree-tr_card.md"
)
...
FIXTURE = (
    REPO_ROOT
    / "artifacts"
    / "qm5_wti_adf_kpss_agree_tr_reference_fixture_20260904.json"
)
...
def direct_kpss(levels: list[float]) -> KPSSState:
    ...
    if residual_energy <= ENERGY_FLOOR:
        raise ValueError("KPSS residual energy at or below floor")
    partial = 0.0
    eta_numerator = 0.0
    for residual in residuals:
        partial += residual
        eta_numerator += partial * partial
    eta = eta_numerator / (LEVEL_COUNT * LEVEL_COUNT)
    lag_cross = tuple(
        sum(residuals[t] * residuals[t - lag] for t in range(lag, LEVEL_COUNT))
        for lag in range(1, KPSS_LAGS + 1)
    )
    long_run_variance = (
        residual_energy
        + 2.0
        * sum(
            (1.0 - lag / (KPSS_LAGS + 1.0)) * lag_cross[lag - 1]
            for lag in range(1, KPSS_LAGS + 1)
        )
    ) / LEVEL_COUNT
    if not math.isfinite(long_run_variance) or long_run_variance <= ENERGY_FLOOR:
        raise ValueError("KPSS long-run variance at or below floor")
    statistic = eta / long_run_variance
    return KPSSState(
        mean, residual_energy, eta, lag_cross, long_run_variance, statistic
    )


def classify(adf_t: float, kpss: float, momentum_12: float) -> int:
    if (
        not math.isfinite(adf_t)
        or not math.isfinite(momentum_12)
        or not math.isfinite(kpss)
        or adf_t < ADF_T_MIN
        or kpss < KPSS_MIN
    ):
        return 0
...
def signal_from_levels(levels: list[float]) -> Signal:
    ...
    kpss = direct_kpss(levels)
    momentum_12 = levels[-1] - levels[-1 - MOMENTUM_MONTHS]
    return Signal(
        classify(adf.adf_t, kpss.statistic, momentum_12),
        adf.adf_t >= ADF_T_MIN,
        kpss.statistic >= KPSS_MIN,
        momentum_12,
        adf,
        kpss,
    )
...
def fixture_levels(name: str) -> list[float]:
    ...
    if name == "adf_only_disagreement_flat":
        return [
            4.0
            - 0.002 * t
            + 0.100 * math.sin(0.26 * t)
            + 0.009 * math.cos(1.91 * t)
            for t in range(LEVEL_COUNT)
        ]
    if name == "kpss_only_disagreement_flat":
        return [
            4.0
            - 0.002 * t
            + 0.018 * math.sin(1.19 * t)
            + 0.009 * math.cos(1.91 * t)
            for t in range(LEVEL_COUNT)
        ]
    raise KeyError(name)
...
class WtiMonthlyADFKPSSAgreementReferenceTests(unittest.TestCase):
    def test_fixture_receipt_matches_direct_formula(self) -> None:
        ...
        self.assertEqual(
            receipt["contract"]["adf_regression_observations"], OBSERVATION_COUNT
        )
        self.assertEqual(
            receipt["contract"]["adf_residual_degrees_of_freedom"], RESIDUAL_DOF
        )
        self.assertEqual(receipt["contract"]["adf_t_min_inclusive"], ADF_T_MIN)
        self.assertEqual(receipt["contract"]["kpss_covariance_lags"], KPSS_LAGS)
        self.assertEqual(receipt["contract"]["kpss_min_inclusive"], KPSS_MIN)
        for expected in receipt["fixtures"]:
            ...
            for field in ("alpha", "gamma", "phi", "sse", "se_gamma", "adf_t"):
                self.assertAlmostEqual(
                    getattr(signal.adf, field), expected["adf"][field], places=12
                )
            for field in (
                "mean",
                "residual_energy",
                ...
                self.assertAlmostEqual(
                    getattr(signal.kpss, field), expected["kpss"][field], places=12
                )
            self.assertAlmostEqual(
            ...
            self.assertEqual(signal.kpss_qualified, expected["kpss_qualified"])
    ...
    def test_log_close_orientation_and_newest_twelve_month_direction(self) -> None:
        for name, expected_direction in (
            ...
            self.assertAlmostEqual(direct.adf.adf_t, via_closes.adf.adf_t, places=12)
            self.assertAlmostEqual(
                direct.kpss.statistic, via_closes.kpss.statistic, places=12
            )
            self.assertAlmostEqual(
    ...
    def test_boundary_is_inclusive_and_direction_band_symmetric(self) -> None:
        self.assertEqual(classify(ADF_T_MIN, KPSS_MIN, 0.01), 1)
        self.assertEqual(classify(ADF_T_MIN, KPSS_MIN, -0.01), -1)
        below = math.nextafter(ADF_T_MIN, -math.inf)
        below_kpss = math.nextafter(KPSS_MIN, -math.inf)
        self.assertEqual(classify(below, KPSS_MIN, 0.01), 0)
        self.assertEqual(classify(ADF_T_MIN, below_kpss, 0.01), 0)
        self.assertEqual(
            classify(ADF_T_MIN, KPSS_MIN, DIRECTION_EPSILON), 0
        )
        self.assertEqual(
            classify(ADF_T_MIN, KPSS_MIN, -DIRECTION_EPSILON), 0
        )
    ...
    def test_setfile_is_the_one_locked_fixed_risk_backtest(self) -> None:
        ...
        self.assertEqual(headers["ea_id"], "41336")
        self.assertEqual(headers["ea_slug"], "wti-adf-kpss-agree-tr")
    ...
    def test_source_contract_attempt_order_and_agreement_guards(self) -> None:
        source = EA_SOURCE.read_text(encoding="utf-8-sig")
        for required in (
            "bool Strategy_ADFCore",
            "bool Strategy_KPSSCore",
            "bool Strategy_AgreementReferenceSelfTest",
            "bool Strategy_AgreementSignal",
            ...
            "metrics.kpss >= strategy_kpss_min",
    ...
    def test_magic_registry_and_card_copy_are_exact(self) -> None:
        with MAGIC_REGISTRY.open(encoding="utf-8-sig", newline="") as handle:
            rows = [
                ...
                if row["ea_id"] == "41336" and row["status"] == "active"
            ]
        self.assertEqual(len(rows), 1)
        self.assertEqual(rows[0]["ea_slug"], "wti-adf-kpss-agree-tr")
        self.assertEqual(rows[0]["symbol"], "XTIUSD.DWX")
        self.assertEqual(rows[0]["magic"], "413360000")
        self.assertEqual(
