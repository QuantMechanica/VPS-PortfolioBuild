...
#property description "QM5_41336 WTI Monthly ADF-KPSS Agreement Trend"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41336 - WTI Monthly ADF-KPSS Agreement-Gated Trend
// -----------------------------------------------------------------------------
...
//   - require ADF t >= -2.594 and constant-only KPSS(4) >= 0.347
...
input int    strategy_kpss_covariance_lags  = 4;
input double strategy_kpss_variance_floor   = 0.000000000000000001;
input double strategy_kpss_min              = 0.347;
...
struct Strategy_SignalMetrics
  {
   ...
   double adf_t;
   double kpss_mean;
   double kpss_residual_energy;
   double kpss_eta;
   double kpss_lag1_cross;
   double kpss_lag2_cross;
   double kpss_lag3_cross;
   double kpss_lag4_cross;
   double kpss_long_run_variance;
   double kpss;
   double momentum_12;
   string level_path;
...
bool Strategy_KPSSCore(const double &levels[],
                       ...
                       string &covariance_path)
  {
   ...
   const int level_count = ArraySize(levels);
   if(level_count != 60 || strategy_level_count != 60 ||
      strategy_kpss_covariance_lags != 4 ||
      strategy_energy_floor != 0.000000000000000001 ||
      strategy_kpss_variance_floor != 0.000000000000000001)
      return false;
   ...
   if(residual_energy <= strategy_energy_floor)
      return false;
   eta = eta_numerator / ((double)level_count * (double)level_count);
   if(!MathIsValidNumber(eta) || eta < 0.0)
      return false;

   double lag_cross[4];
   double long_run_numerator = residual_energy;
   for(int lag = 1; lag <= strategy_kpss_covariance_lags; ++lag)
     {
      lag_cross[lag - 1] = 0.0;
      ...
      const double weight =
         1.0 - (double)lag / (double)(strategy_kpss_covariance_lags + 1);
   ...
   if(!MathIsValidNumber(long_run_variance) ||
      long_run_variance <= strategy_kpss_variance_floor)
      return false;
   ...
   covariance_path =
      StringFormat("lag1=%.16f,lag2=%.16f,lag3=%.16f,lag4=%.16f",
                   lag1_cross, lag2_cross, lag3_cross, lag4_cross);
...
bool Strategy_AgreementSignal(const double &closes[],
                        Strategy_SignalMetrics &metrics)
  {
   ...
   if(strategy_level_count != 60 ||
      ...
      strategy_kpss_covariance_lags != 4 ||
      strategy_kpss_variance_floor != 0.000000000000000001 ||
      MathAbs(strategy_kpss_min - 0.347) >
         0.000000000000001 ||
   ...
   if(!Strategy_KPSSCore(levels,
                         metrics.kpss_mean,
                         metrics.kpss_residual_energy,
                         metrics.kpss_eta,
                         metrics.kpss_lag1_cross,
                         metrics.kpss_lag2_cross,
                         metrics.kpss_lag3_cross,
                         metrics.kpss_lag4_cross,
                         metrics.kpss_long_run_variance,
                         metrics.kpss,
                         metrics.covariance_path))
      return false;
   metrics.kpss_qualified = (metrics.kpss >= strategy_kpss_min);
   metrics.persistence_qualified =
      (metrics.adf_qualified && metrics.kpss_qualified);
...
bool Strategy_AgreementReferenceSelfTest()
  {
   ...
   for(int index = 0; index < 60; ++index)
     {
      ...
      const double adf_only_level =
         ...
         0.100 * MathSin(0.26 * (double)index) +
         0.009 * MathCos(1.91 * (double)index);
      const double kpss_only_level =
         4.0 - 0.002 * (double)index +
      ...
      kpss_only_closes[index] = MathExp(kpss_only_level);
   ...
   if(!Strategy_AgreementSignal(up_closes, metrics) ||
      ...
      MathAbs(metrics.adf_t - (-0.28754973622603336)) >
         0.0000000001 ||
      MathAbs(metrics.kpss - 1.2972592934714569) >
         0.0000000001 ||
   ...
   if(!Strategy_AgreementSignal(down_closes, metrics) ||
      ...
      MathAbs(metrics.adf_t - (-0.34439061991466297)) >
         0.0000000001 ||
      MathAbs(metrics.kpss - 1.3050178255898004) >
         0.0000000001 ||
   ...
   if(!Strategy_AgreementSignal(adf_only_closes, metrics) ||
      ...
      MathAbs(metrics.adf_t - (-2.3128965916890856)) >
         0.000000001 ||
      MathAbs(metrics.kpss - 0.3435465930087885) >
         0.000000001)
      return false;

   if(!Strategy_AgreementSignal(kpss_only_closes, metrics) ||
      metrics.adf_qualified || !metrics.kpss_qualified ||
      metrics.persistence_qualified || metrics.direction != 0 ||
      MathAbs(metrics.adf_t - (-2.638439000868299)) >
         0.000000001 ||
      MathAbs(metrics.kpss - 1.2994845279150933) >
         0.000000001)
      return false;
   ...
   if(!Strategy_AgreementSignal(mean_reverting_closes, metrics) ||
      ...
      MathAbs(metrics.adf_t - (-20.092593377208576)) >
         0.000000001 ||
      MathAbs(metrics.kpss - 0.1220487603596016) >
         0.000000001)
      return false;
   ...
   else
     {
      ...
      if(!g_signal_valid)
         g_signal_state = "agreement_validation_failed";
              ...
              !g_signal_metrics.kpss_qualified)
         g_signal_state = "both_gates_reject_consumed_flat";
      else if(!g_signal_metrics.adf_qualified)
         g_signal_state = "adf_rejects_kpss_only_consumed_flat";
      else if(!g_signal_metrics.kpss_qualified)
         g_signal_state = "kpss_rejects_adf_only_consumed_flat";
...
bool Strategy_NoTradeFilter()
  {
   if(!Strategy_IsHostChart() || qm_ea_id != 41336 ||
      qm_magic_slot_offset != 0 ||
      QM_FrameworkMagic() != 413360000 ||
      qm_rng_seed != 42)
      return true;
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "QM5_41336_WTI_AGREE";
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41336 ||
      qm_magic_slot_offset != 0)
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI monthly ADF-KPSS agreement trend holds through Fridays until the next broker month"))
     {
   ...
   g_attempt_state_key =
      StringFormat("QM5_41336_MONTH_ATTEMPT_%d", QM_FrameworkMagic());
   g_entry_month_state_key =
      StringFormat("QM5_41336_ENTRY_MONTH_%d", QM_FrameworkMagic());
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41336\",\"ea\":\"wti-adf-kpss-agree-tr\"}");
