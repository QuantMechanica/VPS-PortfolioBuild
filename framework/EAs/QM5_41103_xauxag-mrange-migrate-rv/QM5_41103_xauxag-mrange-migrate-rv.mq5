...
#property description "QM5_41103 XAU/XAG Monthly Ratio-Range Migration Reversion"

#include <QM/QM_Common.mqh>
#include <QM/QM_BasketOrder.mqh>

// =============================================================================
// QM5_41103 - XAU/XAG Monthly Ratio-Range Migration Reversion
// -----------------------------------------------------------------------------
// At the first synchronized D1 boundary of a broker month, reconstruct the
// daily-close XAU/XAG log-ratio ranges of the two immediately completed
// calendar months. Fade only strict same-direction migration of both range
// endpoints with opposite equal-notional legs, and hold to the next month.
...
bool Strategy_InputsValid()
  {
   return (qm_ea_id == 41103 && qm_magic_slot_offset == 0 &&
            qm_rng_seed == 42 &&
...
string Strategy_AttemptStateKey()
  {
   return StringFormat("QM5_%d_XAUXAG_MRANGE_MIGRATE_RV_ATTEMPT_MONTH_%I64d",
                       qm_ea_id,
                       Strategy_HostMagic());
  }
...
bool Strategy_OpenLeg(const string symbol,
                      ...
                      const double stop)
  {
   ...
   request.tp = 0.0;
   request.lots = lots;
   request.reason = "QM5_41103_XAUXAG_MRANGE_MIGRATE_RV";
...
bool Strategy_LoadMonthlyRatioRanges(
      const int current_month_key,
      int &new_month_sessions,
      int &parent_month_sessions,
      double &new_range_low,
      double &new_range_high,
      ...
      int &direction)
  {
   ...
   while(index < available &&
         Strategy_MonthKey(xau_bars[index].time) == newest_month_key)
     {
      ...
      if(new_month_sessions == 0)
        {
         new_range_low = ratio;
         new_range_high = ratio;
        }
      else
        {
         new_range_low = MathMin(new_range_low, ratio);
         new_range_high = MathMax(new_range_high, ratio);
        }
      ++new_month_sessions;
      ++index;
     }
   ...
   if(parent_month_sessions < strategy_min_month_sessions ||
      ...
      index >= available ||
      !MathIsValidNumber(new_range_low) ||
      !MathIsValidNumber(new_range_high) ||
      !MathIsValidNumber(parent_range_low) ||
      !MathIsValidNumber(parent_range_high) ||
      new_range_high <= new_range_low ||
   ...
   // direction > 0 means BUY XAU / SELL XAG. Fade the direction of strict
   // migration of the complete observed daily-close ratio range.
   if(new_range_high > parent_range_high &&
      new_range_low > parent_range_low)
      direction = -1;
...
bool Strategy_EntrySignal(QM_EntryRequest &request)
  {
   ...
   request.sl = 0.0;
   request.tp = 0.0;
   request.reason = "QM5_41103_XAUXAG_MRANGE_MIGRATE_RV_HOST";
   ...
   int direction = 0;
   double new_range_low = 0.0;
   double new_range_high = 0.0;
   double parent_range_low = 0.0;
   double parent_range_high = 0.0;
   const bool valid =
      Strategy_LoadMonthlyRatioRanges(g_signal_month_key,
                                      new_month_sessions,
                                      parent_month_sessions,
                                      new_range_low,
                                      new_range_high,
                                      ...
                                      parent_range_high,
                                      direction);
   string state = "monthly_ratio_range_failed";
   if(valid && direction > 0)
      state = "range_migrated_down_long_xau";
   else if(valid && direction < 0)
      state = "range_migrated_up_short_xau";
   else if(valid)
      state = "mixed_or_equal_migration_flat";

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"new_month_sessions\":%d,\"parent_month_sessions\":%d,\"new_range_low\":%.12e,\"new_range_high\":%.12e,\"parent_range_low\":%.12e,\"parent_range_high\":%.12e,\"direction\":%d,\"state\":\"%s\"}",
                            g_signal_month_key,
                            new_month_sessions,
                            parent_month_sessions,
                            new_range_low,
...
int OnInit()
  {
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved XAU/XAG monthly ratio-range migration-reversion package holds through Fridays until next broker month"))
     {
   ...
   QM_LogEvent(QM_INFO, "INIT_OK",
               "{\"card\":\"QM5_41103\",\"ea\":\"xauxag-mrange-migrate-rv\"}");
