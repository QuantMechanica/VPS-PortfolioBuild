...
the EA reconstructs the daily-close log-ratio range for each of the two
immediately completed consecutive calendar months:

```text
r[d] = log(XAU_close[d]) - log(XAG_close[d])
```

If both endpoints of the newest range are strictly above the parent range,
the package fades the upward migration by selling XAU and buying XAG. If both
are strictly below, it buys XAU and sells XAG. Equality, mixed endpoint
movement, a zero range, incomplete or non-adjacent months, asynchronous data,
or invalid arithmetic consumes the month flat.
...
- Logical basket: `QM5_41103_XAU_XAG_MRANGE_MIGRATE_RV_D1`.
- Host/traded slot 0: exact `XAUUSD.DWX`, D1, magic `411030000`.
- Companion/traded slot 1: exact `XAGUSD.DWX`, D1, magic `411030001`.
...
- Strict upward ratio-range migration produces SELL XAU / BUY XAG; strict
  downward migration produces BUY XAU / SELL XAG.
...
carrier. The completed-month endpoint-migration fade is a disclosed QM
