...
#property description "QM5_41238 WTI Same-Calendar Five-Sample Arctangent Location"

#include <QM/QM_Common.mqh>

// =============================================================================
// QM5_41238 - WTI Same-Calendar Five-Sample Arctangent-Weighted Location
// -----------------------------------------------------------------------------
...
//   - freeze 1.4826*MAD and execute exactly 32 Arctangent derivative-weight updates
...
input double strategy_scale_multiplier    = 1.4826;
input int    strategy_arctan_iterations   = 32;
...
// -----------------------------------------------------------------------------
// Exact completed-month endpoint and five-sample-Arctangent helpers.
// -----------------------------------------------------------------------------
...
bool Strategy_ArctangentSignal(const double &observations[],
                           ...
                           int &direction)
  {
   ...
   if(strategy_history_years != 5 ||
      MathAbs(strategy_scale_multiplier - 1.4826) > 1.0e-12 ||
      strategy_arctan_iterations != 32 ||
   ...
   double mu = median;
   for(int step = 0; step < strategy_arctan_iterations; ++step)
     {
      ...
      for(int index = 0; index < 5; ++index)
        {
         ...
         const double squared_u = u * u;
         const double fourth_u = squared_u * squared_u;
         if(!MathIsValidNumber(u) || !MathIsValidNumber(squared_u) ||
            !MathIsValidNumber(fourth_u) ||
            squared_u < 0.0 || fourth_u < 0.0)
            return false;

         // rho(z)=arctan(z), z=u^2, so rho'(z)=1/(1+z^2).
         const double denominator = 1.0 + fourth_u;
         if(denominator < 1.0 || !MathIsValidNumber(denominator))
            return false;
         const double weight = 1.0 / denominator;
...
bool Strategy_LoadArctangentSignal(
   ...
   int &direction)
  {
   ...
   return Strategy_ArctangentSignal(observations,
                                sample_count,
                                location_value,
                                direction);
  }
...
void Strategy_PrepareDecisionSignal()
  {
   ...
   // Consume before history, Arctangent arithmetic, news, spread, quote,
   ...
   else
     {
      g_signal_valid =
          Strategy_LoadArctangentSignal(
            ...
            g_signal_direction);
      if(!g_signal_valid)
         g_signal_state = "endpoint_sample_or_arctan_failed";
      else if(g_signal_direction > 0)
         g_signal_state = "fixed_arctan_long";
      else if(g_signal_direction < 0)
         g_signal_state = "fixed_arctan_short";
      else
         g_signal_state = "fixed_arctan_epsilon_flat";
     }

   QM_LogEvent(QM_INFO,
               "STRATEGY_STATE",
               StringFormat("{\"month\":%d,\"decision_bar\":%I64d,\"label_offset_seconds\":%d,\"valid\":%s,\"samples\":%d,\"fixed_arctan\":%.12g,\"direction\":%d,\"state\":\"%s\"}",
                            g_decision_month_key,
...
bool Strategy_NoTradeFilter()
  {
   ...
   if(!QM_InputRequireLong("qm_ea_id", qm_ea_id, 41238) ||
      ...
      !QM_InputRequireLong("strategy_arctan_iterations", strategy_arctan_iterations, 32) ||
...
bool Strategy_EntrySignal(QM_EntryRequest &req)
  {
   ...
   req.sl = 0.0;
   req.tp = 0.0;
   req.reason = "WTI_SAMECAL_ARCTAN5";
   ...
   if(g_signal_direction > 0)
     {
      req.type = QM_BUY;
      req.reason = "WTI_SAMECAL_ARCTAN5_LONG";
     }
   else
     {
      req.type = QM_SELL;
      req.reason = "WTI_SAMECAL_ARCTAN5_SHORT";
     }
...
int OnInit()
  {
   if(!SymbolSelect(g_symbol, true) ||
      !Strategy_IsHostChart() || qm_ea_id != 41238 ||
   ...
   if(!QM_FrameworkDeclareExecutionContract(
         ...
         "Approved WTI same-calendar five-sample Arctangent-location card holds to the next broker month"))
      return INIT_FAILED;
   ...
   QM_LogEvent(QM_INFO,
               "INIT_OK",
               "{\"card\":\"QM5_41238\",\"ea\":\"wti-samecal-arctan5\"}");
