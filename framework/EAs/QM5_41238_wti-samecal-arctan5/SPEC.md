# QM5_41238_wti-samecal-arctan5 - Strategy Spec
...
- source approval:
  `decisions/2026-08-31_wti_same_calendar_arctan5_source_approval.md`
- approved card:
  `strategy-seeds/cards/approved/QM5_41238_wti-samecal-arctan5_card.md`
- G0 decision:
  `decisions/2026-08-31_qm5_41238_wti_same_calendar_arctan5_g0.md`
- host and traded symbol: exact `XTIUSD.DWX`, D1, slot 0
- deterministic magic: `412380000`
...
`[-0.095,-0.045,-0.005,+0.050,+0.060]` make the locked arctangent statistic
buy at approximately `+0.006280955600`, while the otherwise matched Cauchy
location sells at approximately `-0.002436516741`; raw mean and median are
also negative. Sign reflection reverses the mapping. The quartic-tail weight
`1/(1+u^4)` therefore commits to a different signal path from Cauchy's
quadratic-tail `1/(1+u^2)` rather than renaming another same-calendar rule.
...
fixes the arctangent loss `rho(z)=arctan(z)` and its scale convention. QM fixes the
...
| exact sample, median/MAD, fixed-step Arctangent | `Strategy_LoadArctangentSignal` and `Strategy_ArctangentSignal` |
...
|---|---|---|---|
| v1 | 2026-08-31 | G0-approved WTI same-calendar Arctangent build | Q01 pending |
