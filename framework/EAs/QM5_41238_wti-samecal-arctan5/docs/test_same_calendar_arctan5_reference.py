"""Independent deterministic fixtures for QM5_41238.

The suite covers normalized D1 labels, completed WTI month endpoints, exact
Y-5..Y-1 membership, odd median/MAD arithmetic, frozen-scale Arctangent updates,
...
def arctan_weight(signed_residual: float) -> float | None:
    ...
    if (
        ...
        or not math.isfinite(fourth)
        or squared < 0.0
        or fourth < 0.0
    ):
        return None
    weight = 1.0 / (1.0 + fourth)
    if not math.isfinite(weight) or weight <= 0.0:
        return None
    return weight


def arctan_signal(observations: list[float]) -> ArctangentResult | None:
    ...
    location = median
    trace = [location]
    for _ in range(ARCTAN_ITERATIONS):
        weighted_sum = 0.0
        weight_sum = 0.0
        for value in observations:
            weight = arctan_weight((value - location) / scale)
            if weight is None:
                return None
...
class SameCalendarArctangentReferenceTests(unittest.TestCase):
    ...
    def test_locked_fixture_median_mad_scale_and_32_updates(self) -> None:
        values = [-0.095, -0.045, -0.005, 0.050, 0.060]
        result = arctan_signal(values)
        self.assertIsNotNone(result)
        assert result is not None
        self.assertEqual(result.sorted_values, tuple(values))
        self.assertEqual(result.median, -0.005)
        self.assertEqual(result.raw_mad, 0.055)
        self.assertAlmostEqual(result.scale, 0.081543)
        self.assertEqual(len(result.trace), ARCTAN_ITERATIONS + 1)
        self.assertNotEqual(result.trace[1], result.trace[-1])
        self.assertAlmostEqual(result.location, 0.006280955600, places=12)
        self.assertEqual(result.direction, 1)

    def test_arctan_weights_are_signed_symmetric_positive_and_decaying(self) -> None:
        self.assertEqual(arctan_weight(0.0), 1.0)
        self.assertEqual(arctan_weight(1.0), 0.5)
        self.assertEqual(arctan_weight(-1.0), 0.5)
        self.assertAlmostEqual(arctan_weight(2.0) or 0.0, 1.0 / 17.0)
        self.assertGreater(arctan_weight(1000.0) or 0.0, 0.0)
        self.assertLess(arctan_weight(1000.0) or 1.0, arctan_weight(100.0) or 0.0)
        self.assertIsNone(arctan_weight(math.nan))
        self.assertIsNone(arctan_weight(1.0e308))
    ...
    def test_disagreement_fixture_opposes_named_peers(self) -> None:
        ...
        self.assertAlmostEqual(result.location if result else 0.0,
                               0.006280955600,
                               places=12)
        self.assertAlmostEqual(cauchy, -0.002436516741, places=12)
        self.assertAlmostEqual(raw_mean, -0.007, places=12)
    ...
    def test_sign_reflection_and_permutation_are_exactly_symmetric(self) -> None:
        values = [-0.095, -0.045, -0.005, 0.050, 0.060]
        positive = arctan_signal(values)
        reflected = arctan_signal([-value for value in values])
        reversed_order = arctan_signal(values[::-1])
    ...
    def test_strict_epsilon_and_invalid_states_fail_closed(self) -> None:
        centered = arctan_signal([-2.0, -1.0, 0.0, 1.0, 2.0])
        positive = arctan_signal(
        ...
        self.assertIsNone(arctan_signal([0.01] * 4))
        self.assertIsNone(arctan_signal([0.01] * 5))
        self.assertIsNone(arctan_signal([0.01, 0.01, math.nan, 0.02, 0.03]))
    ...
    def test_static_build_contract_matches_approved_card(self) -> None:
        source_path = EA_DIR / "QM5_41238_wti-samecal-arctan5.mq5"
        source = source_path.read_text(encoding="utf-8")
        set_path = (
            EA_DIR
            / "sets"
            / "QM5_41238_wti-samecal-arctan5_XTIUSD.DWX_D1_backtest.set"
        )
        ...
        self.assertLess(
            ...
            prepare.index("Strategy_LoadArctangentSignal"),
        )
        ...
        approved = (
            ...
            / "cards"
            / "approved"
            / "QM5_41238_wti-samecal-arctan5_card.md"
        )
        ...
        self.assertIn(
            "41238,wti-samecal-arctan5,0,XTIUSD.DWX,412380000",
            magic_rows,
        )
        resolver = (
            REPO_ROOT
        ...
        self.assertIn("412380000", resolver)
