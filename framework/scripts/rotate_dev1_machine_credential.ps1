...
$contractPath = [System.IO.Path]::GetFullPath((Join-Path $repoRoot 'framework\registry\dev1_lane_contract.json'))
$helperPath = [System.IO.Path]::GetFullPath((Join-Path $PSScriptRoot 'dev1_machine_credential.ps1'))
$childPath = [System.IO.Path]::GetFullPath((Join-Path $PSScriptRoot 'invoke_dev1_identity_probe.ps1'))
$cleanupSourcePath = [System.IO.Path]::GetFullPath((Join-Path $PSScriptRoot 'cleanup_dev1_account_lease.ps1'))
...
$credentialPath = [System.IO.Path]::GetFullPath('C:\ProgramData\QM\DEV1\credential.machine-dpapi.json')
$canonicalRotationReceiptPath = [System.IO.Path]::GetFullPath(
    'C:\ProgramData\QM\DEV1\credential.machine-dpapi.rotation-receipt.json'
)
$legacyCredentialPath = [System.IO.Path]::GetFullPath('C:\ProgramData\QM\DEV1\credential.clixml')
$dev1Root = [System.IO.Path]::GetFullPath('D:\QM\mt5\DEV1')
$reportsRoot = [System.IO.Path]::GetFullPath('D:\QM\reports\dev1')
$rotationRoot = [System.IO.Path]::GetFullPath('D:\QM\reports\dev1\credential-rotation')
$groupsTargetPath = [System.IO.Path]::GetFullPath('D:\QM\mt5\DEV1\MQL5\Profiles\Tester\Groups\Darwinex-Live_real.txt')
$pwshPath = [System.IO.Path]::GetFullPath('C:\Program Files\PowerShell\7\pwsh.exe')
$mutexName = 'Global\QM_DEV1_SMOKE_CONTROLLER'
...
function Assert-QmRotationElevated {
    ...
    if (-not $principal.IsInRole([System.Security.Principal.WindowsBuiltInRole]::Administrator)) {
        throw 'DEV1 machine-credential rotation requires an elevated Administrator token.'
    }
}
...
function Assert-QmRotationTargetNonAdministrator {
    ...
    if (@(Get-LocalGroupMember -Group $administrators -ErrorAction Stop | Where-Object {
        ...
        throw 'QMDev1 must not be a member of BUILTIN\Administrators.'
    }
}
...
function Get-QmRotationDev1Processes {
    ...
    foreach ($process in @(Get-CimInstance -ClassName Win32_Process -Property ProcessId,ExecutablePath,CreationDate -ErrorAction Stop)) {
        $pathMatch = -not [string]::IsNullOrWhiteSpace([string]$process.ExecutablePath) -and
            (Test-QmRotationPathWithin -Path ([string]$process.ExecutablePath) -Root $dev1Root)
...
function Stop-QmRotationTargetProcesses {
    param([Parameter(Mandatory = $true)][string]$TargetSid)
    foreach ($candidate in @(Get-QmRotationDev1Processes -TargetSid $TargetSid)) {
...
function Assert-QmRotationNoTasks {
    $tasks = @(Get-ScheduledTask -TaskPath $taskPath -ErrorAction Stop | Where-Object {
            ...
            $_.TaskName.StartsWith($compileTaskPrefix, [System.StringComparison]::Ordinal) -or
            $_.TaskName.StartsWith($profileTaskPrefix, [System.StringComparison]::Ordinal)
        })
    if ($tasks.Count -ne 0) { throw "DEV1 rotation found $($tasks.Count) pre-existing smoke/cleanup/profile-init task(s)." }
}

function Ensure-QmRotationTargetDisabledAtRest {
    param([Parameter(Mandatory = $true)][string]$TargetSid)
    $sid = New-Object System.Security.Principal.SecurityIdentifier($TargetSid)
    $before = Get-LocalUser -SID $sid -ErrorAction Stop
    if ($before.Name -cne $targetUserName -or $before.SID.Value -cne $TargetSid -or
        -not $before.PasswordRequired) {
        throw 'DEV1 pre-migration containment target name/SID/password-required state drifted.'
    }
    $disabledByMigration = [bool]$before.Enabled
    if ($disabledByMigration) {
        Disable-LocalUser -SID $sid -ErrorAction Stop
    }
    # The entry preflight was process-free, but an enabled legacy account could
    # race one last process into existence before Disable-LocalUser completed.
    # Reuse the immutable-SID cleanup and prove both identity and lane root idle.
    Stop-QmRotationTargetProcesses -TargetSid $TargetSid
    $after = Get-LocalUser -SID $sid -ErrorAction Stop
    if ($after.Name -cne $targetUserName -or $after.SID.Value -cne $TargetSid -or
        $after.Enabled -or -not $after.PasswordRequired -or
        @(Get-QmRotationDev1Processes -TargetSid $TargetSid).Count -ne 0) {
        throw 'DEV1 pre-migration containment did not establish exact disabled-at-rest/process-free state.'
    }
    return $disabledByMigration
}
...
function Write-QmCanonicalRotationReceipt {
    ...
    if (-not $full.Equals($canonicalRotationReceiptPath, [System.StringComparison]::OrdinalIgnoreCase)) {
        throw 'Canonical rotation receipt path differs from the fixed DEV1 evidence path.'
    }
    $parent = [System.IO.Path]::GetDirectoryName($full)
    Assert-QmDev1CredentialExactAcl -Path $parent -Directory
    if (Test-Path -LiteralPath $full) {
        throw 'Canonical DEV1 machine-credential rotation receipt already exists.'
    }
    ...
    try {
        ...
        Set-QmDev1CredentialExactAcl -Path $temporary
        Assert-QmDev1CredentialExactAcl -Path $temporary
...
function Assert-QmRotationAdminControlledSourceFile {
    ...
    foreach ($rule in @($acl.Access)) {
        ...
        if (-not [string]::IsNullOrWhiteSpace($ForbiddenWriterSid) -and $writerSid -ceq $ForbiddenWriterSid) {
            throw "DEV1 target identity can write a sealed rotation source: $full"
        }
    }
}
...
function Assert-QmRotationJournalSchema {
    ...
    if ([int]$Journal.schema_version -ne $journalSchemaVersion -or
        ...
        [string]$Journal.target_task_name -cnotmatch '^QM_DEV1_SMOKE_[0-9a-f]{32}$' -or
        [string]$Journal.cleanup_task_name -cnotmatch '^QM_DEV1_CLEANUP_[0-9a-f]{32}$' -or
...
function Get-QmRotationJournalHistory {
    ...
    foreach ($directory in @(Get-ChildItem -LiteralPath $rotationRoot -Directory -Force -ErrorAction Stop | Where-Object {
        ...
        if ($RequireExactAcl.IsPresent) { Assert-QmDev1CredentialExactAcl -Path $journalPathLocal }
...
function Assert-QmRotationTaskContract {
    ...
    if ($Task.TaskName -cne $ExpectedTaskName -or $Task.TaskPath -cne $taskPath -or
        ...
        throw 'DEV1 rotation identity-probe task drifted from Password/Limited/triggerless.'
    }
    ...
    if (-not (ConvertTo-QmRotationFullPath -Path $action.Execute).Equals($pwshPath, [System.StringComparison]::OrdinalIgnoreCase) -or
        ...
        throw 'DEV1 rotation identity-probe task action drifted.'
    }
}
...
function Assert-QmRotationCleanupTaskContract {
    ...
    if ($Task.TaskName -cne $ExpectedTaskName -or $Task.TaskPath -cne $taskPath -or
        ...
        throw 'DEV1 rotation cleanup task drifted from the SYSTEM/Highest/two-trigger lease contract.'
    }
    $action = @($Task.Actions)[0]
    if (-not (ConvertTo-QmRotationFullPath -Path $action.Execute).Equals($pwshPath, [System.StringComparison]::OrdinalIgnoreCase) -or
        [string]$action.Arguments -cne $ExpectedArguments -or
        ...
        throw 'DEV1 rotation cleanup task action drifted before account mutation.'
    }
}
...
function Assert-QmRotationReceiptExact {
    ...
    $full = ConvertTo-QmRotationFullPath -Path $Path
    Assert-QmRotationNoReparseComponents -Path $full
    if ($RequireExactAcl.IsPresent) { Assert-QmDev1CredentialExactAcl -Path $full }
...
function Assert-QmRotationIdentityProofBindings {
    ...
    Assert-QmDev1CredentialExactAcl -Path $requestPath
    Assert-QmDev1CredentialExactAcl -Path $resultPath
    $requestFields = @(
    ...
    if ([int]$request.schema_version -ne 1 -or [string]$request.artifact_type -cne 'QM_DEV1_IDENTITY_PROBE_REQUEST' -or
    ...
    if ([int]$result.schema_version -ne 1 -or [string]$result.artifact_type -cne 'QM_DEV1_IDENTITY_PROBE_RESULT' -or
...
function Assert-QmRotationCleanupLeaseBindings {
    ...
    Assert-QmDev1CredentialExactAcl -Path $leasePath
    foreach ($exactFile in @([string]$Journal.cleanup_helper_path, [string]$Journal.tester_groups_source_path)) {
        Assert-QmDev1CredentialExactAcl -Path (ConvertTo-QmRotationFullPath -Path $exactFile)
    }
    $leaseFields = @(
        'artifact_type', 'cleanup_action_mutex', 'cleanup_task_name', 'created_utc', 'dev1_root', 'disarm_result_path',
    ...
    if ([int]$lease.schema_version -ne 1 -or [string]$lease.artifact_type -cne 'QM_DEV1_ACCOUNT_CLEANUP_LEASE' -or
        ...
        ) -or -not (ConvertTo-QmRotationFullPath -Path ([string]$lease.dev1_root)).Equals(
            $dev1Root, [System.StringComparison]::OrdinalIgnoreCase
...
function Assert-QmRotationCleanupEvidenceReceipt {
    ...
    $fields = if ($ArtifactType -ceq 'QM_DEV1_ACCOUNT_CLEANUP_RESULT') {
        @(
    ...
    try {
        ...
        $rootElement = $typeDocument.RootElement.GetProperty('dev1_root_process_count')
        ...
        if ($ownerElement.ValueKind -ne [System.Text.Json.JsonValueKind]::Number -or
            ...
            -not $rootElement.TryGetInt64([ref]$dev1RootProcessCount) -or $dev1RootProcessCount -lt -1 -or
            $dev1RootProcessCount -gt [int]::MaxValue -or
    ...
    if ([int]$payload.schema_version -ne 1 -or [string]$payload.artifact_type -cne $ArtifactType -or
        ...
        [long]$payload.dev1_root_process_count -ne $dev1RootProcessCount -or
    ...
    if ($ArtifactType -ceq 'QM_DEV1_ACCOUNT_CLEANUP_RESULT') {
    ...
    if ([bool]$payload.success) {
        if (-not [bool]$payload.containment_verified -or -not [bool]$payload.account_restored_disabled -or
            $ownerProcessCount -ne 0 -or $dev1RootProcessCount -ne 0 -or
            [bool]$payload.target_task_registered -or $failureValues.Count -ne 0 -or
            ($ArtifactType -ceq 'QM_DEV1_ACCOUNT_CLEANUP_RESULT' -and -not [bool]$payload.manifest_valid) -or
            ($ArtifactType -ceq 'QM_DEV1_ACCOUNT_CLEANUP_DISARM_RESULT' -and
    ...
    Set-QmDev1CredentialExactAcl -Path $Path
    return [pscustomobject]@{
        Payload = $payload
...
function Resolve-QmRotationCleanupEvidence {
    ...
    if (Test-Path -LiteralPath $canonical -PathType Leaf) {
        ...
        if ($canonicalRecord.Success) {
            ...
            Assert-QmDev1CredentialExactAcl -Path $archivePath
...
function Invoke-QmRotationHostContainmentPass {
    ...
    if ($verifiedUser.Name -cne $targetUserName -or $verifiedUser.SID.Value -cne [string]$Journal.target_sid -or
        ...
        @(Get-QmRotationDev1Processes -TargetSid ([string]$Journal.target_sid)).Count -ne 0 -or
...
function Invoke-QmRotationRecoveryContainment {
    ...
    $allDev1Tasks = @(Get-ScheduledTask -TaskPath $taskPath -ErrorAction Stop | Where-Object {
            $_.TaskName.StartsWith($taskPrefix, [System.StringComparison]::Ordinal) -or
            $_.TaskName.StartsWith($cleanupPrefix, [System.StringComparison]::Ordinal) -or
            $_.TaskName.StartsWith($compileTaskPrefix, [System.StringComparison]::Ordinal) -or
            $_.TaskName.StartsWith($profileTaskPrefix, [System.StringComparison]::Ordinal)
        })
    foreach ($task in $allDev1Tasks) {
        if ([string]$task.TaskName -notin $boundTaskNames) {
            throw "Recovery found an unbound DEV1 task and refuses cross-run cleanup: $($task.TaskName)"
        }
    }
    ...
    # after later lease removal must leave QMDev1 disabled, process-free, and groups-restored.
    ...
    if ($tasksAfterDrain.Count -ne 0) {
        throw 'Recovery observed DEV1 task reappearance after bounded scheduler drain.'
    }
    ...
    $resultEvidence = Resolve-QmRotationCleanupEvidence -CanonicalPath $resultPath `
        -ArtifactType 'QM_DEV1_ACCOUNT_CLEANUP_RESULT' -Journal $Journal `
    ...
    $disarmEvidence = Resolve-QmRotationCleanupEvidence -CanonicalPath $disarmPath `
        -ArtifactType 'QM_DEV1_ACCOUNT_CLEANUP_DISARM_RESULT' -Journal $Journal `
    ...
    Assert-QmDev1CredentialExactAcl -Path $resultPath
    Assert-QmDev1CredentialExactAcl -Path $disarmPath
    return [pscustomobject]@{
        AccountDisabled = $true
        PasswordRequired = $true
        OwnerProcessCount = 0
        Dev1RootProcessCount = 0
...
function Invoke-QmRotationResumeFinalize {
    ...
    Assert-QmDev1CredentialExactAcl -Path $journalPathLocal
    ...
    $expectedPaths = [ordered]@{
        ...
        cleanup_helper_path = (Join-Path $run 'control\cleanup_dev1_account_lease.ps1')
    ...
    Assert-QmDev1CredentialExactAcl -Path $legacyCredentialPath
    ...
    if ($disposition -in @(
        ...
        if (-not $forensicCredentialPath.Equals($forensicFailedPath, [System.StringComparison]::OrdinalIgnoreCase)) {
            ...
            Set-QmDev1CredentialExactAcl -Path $forensicCredentialPath
        }
        Assert-QmDev1CredentialExactAcl -Path $forensicCredentialPath
    ...
    if ($rollbackRecovery) {
        ...
        if ($forensicCredentialPath.Equals($credentialPath, [System.StringComparison]::OrdinalIgnoreCase)) {
            ...
            Set-QmDev1CredentialExactAcl -Path $forensicFailedPath
            $forensicCredentialPath = $forensicFailedPath
        }
        Assert-QmDev1CredentialExactAcl -Path $forensicCredentialPath
        if ((Get-FileHash -LiteralPath $forensicCredentialPath -Algorithm SHA256 -ErrorAction Stop).Hash.ToLowerInvariant() -cne
    ...
    Assert-QmDev1CredentialExactAcl -Path ([System.IO.Path]::GetDirectoryName($artifactPath)) -Directory
    Assert-QmDev1CredentialExactAcl -Path $artifactPath
    $envelope = Read-QmDev1MachineCredentialEnvelope -CredentialPath $artifactPath `
    ...
    $decryptedCredential = Get-QmDev1MachineCredential -CredentialPath $artifactPath `
        -ExpectedCredentialSha256 ([string]$journal.credential_sha256) -ExpectedAccount ([string]$journal.target_account) `
        -ExpectedSid ([string]$journal.target_sid) -ContractId ([string]$journal.contract_id) -Lane ([string]$journal.lane)
    try {
    ...
    if ($stagedExists) {
        ...
        Set-QmDev1CredentialExactAcl -Path $credentialPath
    }
    Assert-QmDev1CredentialExactAcl -Path $credentialPath
    ...
    if (Test-Path -LiteralPath $dynamicReceiptPath -PathType Leaf) {
        ...
        Set-QmDev1CredentialExactAcl -Path $dynamicReceiptPath
    } else {
...
foreach ($required in @($contractPath, $helperPath, $childPath, $cleanupSourcePath, $groupsCanonicalPath, $pwshPath, $legacyCredentialPath)) {
    if (-not (Test-Path -LiteralPath $required -PathType Leaf)) { throw "Required DEV1 rotation input is missing: $required" }
...
if ([int]$contract.schema_version -ne 3 -or [string]$contract.contract_id -cne $contractId -or
    ...
    [string]$contract.coordination.compile_task_prefix -cne $compileTaskPrefix -or
    ...
    -not (ConvertTo-QmRotationFullPath -Path ([string]$contract.identity.credential)).Equals(
        $credentialPath, [System.StringComparison]::OrdinalIgnoreCase
    ) -or [string]$contract.identity.credential_format -cne 'QM_DEV1_MACHINE_DPAPI_CREDENTIAL' -or
    [string]$contract.identity.dpapi_scope -cne 'LocalMachine') {
    throw 'DEV1 rotation requires the exact V3 machine-credential lane contract.'
}
...
if ($ResumeFinalize.IsPresent) {
    ...
    try {
        ...
        if (-not $resumeMutexAcquired) { throw 'Another DEV1 controller holds the exclusive rotation/recovery lock.' }
...
if (Test-Path -LiteralPath $canonicalRotationReceiptPath) {
    throw 'Canonical DEV1 machine-credential rotation receipt already exists; one-time rotation refuses replacement.'
}
...
$targetDisabledByMigration = $false
$preMigrationContainmentStarted = $false
...
try {
    ...
    if (-not $mutexAcquired) { throw 'Another DEV1 controller holds the exclusive rotation lock.' }
    $null = Assert-QmRotationFreshApplyHistory
    Assert-QmRotationNoTasks
    $user = Get-LocalUser -Name $targetUserName -ErrorAction Stop
    if ($user.Name -cne $targetUserName -or -not $user.PasswordRequired) {
        throw 'DEV1 rotation requires the exact password-required QMDev1 account.'
    }
    $targetSid = $user.SID.Value
    $targetInitiallyEnabled = [bool]$user.Enabled
    $expectedAccount = "$env:COMPUTERNAME\$targetUserName"
    if ((Resolve-QmRotationSid -AccountName $expectedAccount) -cne $targetSid) {
        throw 'DEV1 rotation target account/SID binding drifted.'
    }
    Assert-QmRotationTargetNonAdministrator -TargetSid $targetSid
    if (@(Get-QmRotationDev1Processes -TargetSid $targetSid).Count -ne 0) {
        throw 'DEV1 rotation requires zero target-SID and DEV1-root processes before pre-migration containment.'
    }
    foreach ($source in @($contractPath, $helperPath, $childPath, $legacyCredentialPath)) {
    ...
    if ($legacyCredential -isnot [System.Management.Automation.PSCredential] -or
        ...
        throw 'Legacy CurrentUser-DPAPI credential is not a nonempty PSCredential bound to QMDev1.'
    }
    ...
    Assert-QmRotationTargetNonAdministrator -TargetSid $targetSid
    if (@(Get-QmRotationDev1Processes -TargetSid $targetSid).Count -ne 0) {
        throw 'DEV1 rotation requires zero target-SID and DEV1-root processes.'
    }
    if (Test-Path -LiteralPath $credentialPath) {
        throw 'DEV1 machine credential already exists; this one-time legacy rotation refuses replacement.'
    }
    ...
    $machineCredential = New-QmDev1MachineCredentialArtifact -CredentialPath $pendingCredentialPath `
    ...
    Set-QmDev1CredentialExactAcl -Path $cleanupHelperPath
    Set-QmDev1CredentialExactAcl -Path $cleanupGroupsPath
    ...
    Set-QmDev1CredentialExactAcl -Path $cleanupLeasePath
    ...
    Register-ScheduledTask -TaskName $cleanupTaskName -TaskPath $taskPath -Action $cleanupAction `
        ...
        -Description "Fail-closed DEV1 credential rotation cleanup lease $rotationId" -ErrorAction Stop | Out-Null
    ...
    # The legacy account may enter migration enabled. Keep the durable SYSTEM
    # cleanup lease armed before the first account/process mutation so a
    # controller or OS crash during Disable/Stop still converges to the exact
    # disabled-at-rest, process-free state.
    $preMigrationContainmentStarted = $true
    $targetDisabledByMigration = Ensure-QmRotationTargetDisabledAtRest -TargetSid $targetSid
    ...
    Register-ScheduledTask -TaskName $targetTaskName -TaskPath $taskPath -Action $action -Settings $settings `
        ...
        -Description "Ephemeral DEV1 machine-credential identity proof $rotationId" -ErrorAction Stop | Out-Null
    ...
    if ($task.State.ToString() -eq 'Running' -or [int64]$info.LastTaskResult -ne 0 -or
        ...
        throw 'DEV1 rotation Limited/Password identity-only child did not complete successfully.'
    }
    ...
    if ([int]$identityResult.schema_version -ne 1 -or [string]$identityResult.artifact_type -cne 'QM_DEV1_IDENTITY_PROBE_RESULT' -or
        ...
        throw 'DEV1 rotation identity-only child returned stale or drifted proof.'
    }
    ...
    if (-not [DateTimeOffset]::TryParseExact(
        ...
        throw 'DEV1 rotation identity-only child completion time is stale or invalid.'
    }
    ...
    if ($postProofUser.Enabled -or -not $postProofUser.PasswordRequired -or
        @(Get-QmRotationDev1Processes -TargetSid $targetSid).Count -ne 0) {
        throw 'DEV1 identity proof did not return to disabled/process-free containment before publication.'
    }
    Set-QmDev1CredentialExactAcl -Path $requestPath
    Set-QmDev1CredentialExactAcl -Path $identityResultPath
    ...
    if ([string]$sealedIdentityResult.Json -cne [string]$identityResultRecord.Json) {
        throw 'DEV1 rotation identity-only result changed before target write access was removed.'
    }
    ...
    try {
        ...
        try {
            ...
            Set-QmDev1CredentialExactAcl -Path $failedCredentialPath
    ...
    if ($null -ne $primaryError -and $passwordChanged -and -not $credentialPublished) {
        try {
            ...
            Set-QmDev1CredentialExactAcl -Path $pendingCredentialPath
            Assert-QmDev1CredentialExactAcl -Path $pendingCredentialPath
            ...
            if ($null -eq $legacyCredential -or $legacyCredential.Password.Length -le 0 -or
                ...
                throw 'Legacy rollback credential is unavailable or no longer bound to QMDev1.'
            }
    ...
    # Before the durable SYSTEM lease is armed, preparation is read-only with
    # respect to the account/process lifecycle. Do not introduce an unbacked
    # first Disable/Stop from this error path.
    if ($preMigrationContainmentStarted -and -not [string]::IsNullOrWhiteSpace($targetSid)) {
        ...
        try {
            ...
            if ($verified.Enabled -or -not $verified.PasswordRequired) { throw 'QMDev1 did not return to disabled-at-rest.' }
            $accountEnabled = $false
        } catch { $cleanupErrors.Add("account_disable: $($_.Exception.Message)") }
        try {
            if (@(Get-QmRotationDev1Processes -TargetSid $targetSid).Count -ne 0) {
                throw 'DEV1 owner/root processes remain after rotation containment.'
            }
    ...
    if ($cleanupTaskRegistered -and -not [string]::IsNullOrWhiteSpace($cleanupTaskName)) {
        if ($null -ne $primaryError -or $cleanupErrors.Count -gt 0) {
            try {
                ...
                if ([string]$cleanupReceipt.artifact_type -cne 'QM_DEV1_ACCOUNT_CLEANUP_DISARM_RESULT' -or
                    ...
                    [int]$cleanupReceipt.owner_process_count -ne 0 -or [int]$cleanupReceipt.dev1_root_process_count -ne 0 -or
                ...
                if ($postCleanupUser.Enabled -or -not $postCleanupUser.PasswordRequired -or
                    @(Get-QmRotationDev1Processes -TargetSid $targetSid).Count -ne 0 -or
...
if ($null -ne $primaryError -or $cleanupErrors.Count -gt 0 -or -not $rotationSucceeded) {
    ...
    $mutex.Dispose()
    throw "DEV1 machine-credential rotation failed closed: $primaryMessage; containment=$([string]::Join(' | ', $cleanupErrors.ToArray()))"
}

try {
$credentialSha256 = (Get-FileHash -LiteralPath $credentialPath -Algorithm SHA256 -ErrorAction Stop).Hash.ToLowerInvariant()
Assert-QmDev1CredentialExactAcl -Path ([System.IO.Path]::GetDirectoryName($credentialPath)) -Directory
Assert-QmDev1CredentialExactAcl -Path $credentialPath
...
if ($finalUser.Enabled -or $finalUser.SID.Value -cne $targetSid -or -not $finalUser.PasswordRequired -or
    @(Get-QmRotationDev1Processes -TargetSid $targetSid).Count -ne 0 -or $targetTaskRegistered -or $cleanupTaskRegistered -or
    $null -ne (Get-ScheduledTask -TaskName $targetTaskName -TaskPath $taskPath -ErrorAction SilentlyContinue) -or
    ...
    throw 'DEV1 machine-credential rotation final containment proof failed.'
}
...
if ((Get-FileHash -LiteralPath $contractPath -Algorithm SHA256 -ErrorAction Stop).Hash.ToLowerInvariant() -cne
    ...
    throw 'DEV1 lane contract changed before final receipt publication.'
}
if (Test-Path -LiteralPath $canonicalRotationReceiptPath) {
    throw 'Canonical DEV1 machine-credential rotation receipt appeared before one-time publication.'
}
...
Set-QmDev1CredentialExactAcl -Path $receiptPath
Assert-QmRotationCleanupEvidenceHashBindings -Journal ([pscustomobject]$journal)
$journal.phase = 'READY_FOR_CANONICAL_RECEIPT'
$journal.updated_utc = [DateTimeOffset]::UtcNow.ToString('o')
Write-QmRotationJournalExact -Path $journalPath -Payload $journal -Replace
...
Write-Output ([ordered]@{
        ...
        target_disabled_at_rest = $true
        target_initially_enabled = [bool]$targetInitiallyEnabled
        target_disabled_by_migration = [bool]$targetDisabledByMigration
        legacy_credential_preserved = $true
    } | ConvertTo-Json -Depth 3 -Compress)
} finally {
