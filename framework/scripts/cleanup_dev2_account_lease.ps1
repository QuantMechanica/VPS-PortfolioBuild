...
param(
    ...
    [Parameter(Mandatory = $true)]
    [ValidatePattern('^Global\\QM_DEV2_CLEANUP_ACTION_[0-9a-f]{32}$')]
...
function Assert-QmExistingCleanupDisarmReceipt {
    ...
    try {
        ...
        $rootElement = $document.RootElement.GetProperty('dev2_root_process_count')
    ...
    if ($receipt.schema_version -isnot [long] -or [long]$receipt.schema_version -ne 1 -or
        ...
        $receipt.dev2_root_process_count -isnot [long] -or [long]$receipt.dev2_root_process_count -ne 0 -or
        [string]$receipt.artifact_type -cne 'QM_DEV2_ACCOUNT_CLEANUP_DISARM_RESULT' -or
...
try {
    ...
    $expectedLeaseFields = @(
        'artifact_type', 'cleanup_action_mutex', 'cleanup_task_name', 'created_utc', 'dev2_root',
    ...
    if ([string]::Join('|', $actualLeaseFields) -cne [string]::Join('|', $expectedLeaseFields) -or
        -not (Test-QmPathWithin -Path $runDirectory -Root $script:Dev2ReportsRoot) -or
...
try {
    ...
    if ($profileTasks.Count -ne 0) {
        throw "Found $($profileTasks.Count) unbound DEV2 profile-init task(s); cleanup will not claim full containment."
    }
} catch {
...
if ($script:FailureMessages.Count -ne 0 -or -not $containmentResultPersisted) {
    ...
    throw 'DEV2 cleanup lease failed before durable disarm; retry remains fail-closed.'
}
...
try {
    ...
    throw 'DEV2 cleanup lease could not unregister its retry task; retry remains fail-closed.'
}
