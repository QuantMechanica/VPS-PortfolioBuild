...
$credentialProbePath = Join-Path $repoRoot 'framework\scripts\probe_dev1_machine_credential.ps1'
$credentialRotatePath = Join-Path $repoRoot 'framework\scripts\rotate_dev1_machine_credential.ps1'
$identityProbePath = Join-Path $repoRoot 'framework\scripts\invoke_dev1_identity_probe.ps1'
...
Invoke-Expression (Get-QmFunctionTextFromAst -Ast $controllerAst -Name 'Get-QmMinimumDev1ControllerTimeoutSeconds')
$controllerMinimum = Get-QmMinimumDev1ControllerTimeoutSeconds `
...
if ((Get-QmMinimumDev1ControllerTimeoutSeconds -MaximumRunAttempts 10 -RunTimeoutSeconds 28800) -le 172800) {
...
& {
    ...
    $script:MockProcesses = @(
        [pscustomobject]@{
            ProcessId = 9001
            ExecutablePath = 'D:\QM\mt5\DEV1\metatester64.exe'
    ...
    Invoke-Expression (Get-QmFunctionTextFromAst -Ast $childAst -Name 'Update-QmDev1AgentListenerProof')
    ...
    $seen = @{}
    Update-QmDev1AgentListenerProof -Baseline $baseline -Seen $seen `
    ...
    foreach ($conflictingAddress in @('127.0.0.1', '::ffff:127.0.0.1', '0.0.0.0', '::')) {
        ...
        $rejected = $false
        try {
            Update-QmDev1AgentListenerProof -Baseline $baseline -Seen @{} `
                -ExpectedOwnerSid $script:MockOwnerSid -EarliestCreationUtc ([DateTimeOffset]::UtcNow.AddSeconds(-1)) `
                -PortContract $portContract
        } catch {
            $rejected = $true
...
Remove-Item -LiteralPath Function:\Get-QmMinimumDev1ControllerTimeoutSeconds -ErrorAction Stop
Invoke-Expression (Get-QmFunctionTextFromAst -Ast $childAst -Name 'Get-QmMinimumDev1ControllerTimeoutSeconds')
$childMinimum = Get-QmMinimumDev1ControllerTimeoutSeconds `
    -MaximumRunAttempts 4 -RunTimeoutSeconds 28800
...
$expectedResultPath = 'D:\QM\reports\dev1\runs\test\control\cleanup_lease.result.json'
$receipt = [pscustomobject]@{
    ...
    target_task_name = 'QM_DEV1_SMOKE_aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'
    cleanup_task_name = 'QM_DEV1_CLEANUP_bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb'
...
foreach ($tamper in @(
    ...
    @{ Field = 'target_task_name'; Value = 'QM_DEV1_SMOKE_cccccccccccccccccccccccccccccccc' },
    @{ Field = 'cleanup_task_name'; Value = 'QM_DEV1_CLEANUP_dddddddddddddddddddddddddddddddd' },
    @{ Field = 'containment_result_path'; Value = 'D:\QM\reports\dev1\runs\test\output\cleanup_lease.result.json' }
)) {
...
$evidenceTestRoot = Join-Path ([System.IO.Path]::GetTempPath()) ('qm-dev1-controller-evidence-' + [guid]::NewGuid().ToString('N'))
...
foreach ($marker in @(
    ...
    'QM_DEV1_PROFILE_INIT_', 'Global\QM_DEV1_CLEANUP_ACTION_',
...
foreach ($marker in @(
        ...
        'Read-QmDev1MachineCredentialEnvelope', 'Get-QmDev1MachineCredential'
    )) {
    if (-not $controllerText.Contains($marker, [System.StringComparison]::Ordinal)) {
...
foreach ($forbidden in @('Import-Clixml', 'ProtectedData', 'Get-QmDev1MachineCredential')) {
...
$credentialDecryptIndex = $controllerText.IndexOf('$credential = Get-QmDev1MachineCredential', [System.StringComparison]::Ordinal)
$accountEnableIndex = $controllerText.IndexOf('$dev1AccountEnabledByController = Enable-QmDev1ControllerAccountState', [System.StringComparison]::Ordinal)
...
$noDev1TasksText = Get-QmFunctionTextFromAst -Ast $controllerAst -Name 'Assert-QmNoDev1Tasks'
if (-not $noDev1TasksText.Contains('$script:ProfileTaskNamePrefix', [System.StringComparison]::Ordinal) -or
    -not $noDev1TasksText.Contains('$script:CompileTaskNamePrefix', [System.StringComparison]::Ordinal)) {
    throw 'DEV1 controller task preflight does not include the profile-init and compile task families.'
}
$restoreGroupsText = Get-QmFunctionTextFromAst -Ast $controllerAst -Name 'Restore-QmDev1TesterGroupsCanonical'
foreach ($marker in @(
    ...
    $surface = if ($marker.StartsWith('$testerGroups', [System.StringComparison]::Ordinal)) {
        $controllerText
    } else {
        $restoreGroupsText
    }
    if (-not $surface.Contains($marker, [System.StringComparison]::Ordinal)) {
        throw "DEV1 tester-groups evidence is not canonical lowercase SHA-256: $marker"
    }
}
...
$firstProcessSweepIndex = $controllerText.IndexOf('Stop-QmDev1ProcessesExact -ExpectedOwnerSid', $targetDrainIndex, [System.StringComparison]::Ordinal)
$accountRestoreIndex = $controllerText.IndexOf('$dev1AccountRestoredDisabled = Restore-QmDev1ControllerAccountState', $firstProcessSweepIndex, [System.StringComparison]::Ordinal)
$secondProcessSweepIndex = $controllerText.IndexOf('Stop-QmDev1ProcessesExact -ExpectedOwnerSid', $accountRestoreIndex, [System.StringComparison]::Ordinal)
$containmentReassertIndex = $controllerText.IndexOf('Assert-QmDev1ContainedBeforeCleanupLeaseDisarm', $secondProcessSweepIndex, [System.StringComparison]::Ordinal)
...
& {
    ...
    Remove-QmScheduledTaskBounded -TaskName ('QM_DEV1_CLEANUP_' + ('a' * 32)) `
...
if ($sourceWriterCheckIndex -lt 0 -or $sourceWriterCheckIndex -ge $passwordMutationIndex) {
    throw 'Initial rotation does not reject QMDev1-writable contract/helper/child/legacy sources before password mutation.'
}
...
$fixedCredential = 'C:\ProgramData\QM\DEV1\credential.machine-dpapi.json'
$fixedCanonicalReceipt = 'C:\ProgramData\QM\DEV1\credential.machine-dpapi.rotation-receipt.json'
$legacyCredential = 'C:\ProgramData\QM\DEV1\credential.clixml'
$beforePlan = [ordered]@{
...
Write-Host 'PASS Test-Dev1ControllerContracts'
