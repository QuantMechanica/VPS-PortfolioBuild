...
$journal = [ordered]@{
    ...
    lane = $lane
    lane_contract_path = 'C:\QM\repo\framework\registry\dev2_lane_contract.json'
    lane_contract_sha256 = $hashA
    target_account = "$env:COMPUTERNAME\QMDev2"
    target_sid = $sid
    target_profile = 'C:\Users\QMDev2'
    identity_probe_logon_type = 'Password'
    identity_probe_run_level = 'Limited'
    credential_staged_path = "C:\ProgramData\QM\DEV2\credential.machine-dpapi.pending.$nonce.json"
    credential_final_path = 'C:\ProgramData\QM\DEV2\credential.machine-dpapi.json'
    credential_sha256 = $hashB
    credential_generation_id = 'f' * 32
    credential_helper_path = 'C:\QM\repo\framework\scripts\dev2_machine_credential.ps1'
    credential_helper_sha256 = $hashC
    identity_probe_child_path = 'C:\QM\repo\framework\scripts\invoke_dev2_identity_probe.ps1'
    identity_probe_child_sha256 = $hashA
    identity_probe_request_path = "D:\QM\reports\dev2\credential-rotation\$rotationId\control\identity_probe_request.json"
    identity_probe_request_sha256 = $hashB
    identity_probe_result_path = "D:\QM\reports\dev2\credential-rotation\$rotationId\output\identity_probe_result.json"
    identity_probe_result_sha256 = $hashC
    identity_proof_completed_utc = $now.AddMinutes(-1).ToString('o')
    identity_proof_verified = $true
    target_task_name = 'QM_DEV2_SMOKE_' + ('1' * 32)
    cleanup_task_name = 'QM_DEV2_CLEANUP_' + ('2' * 32)
    cleanup_action_mutex = 'Global\QM_DEV2_CLEANUP_ACTION_' + $nonce
    cleanup_lease_path = "D:\QM\reports\dev2\credential-rotation\$rotationId\control\cleanup_lease.json"
    cleanup_lease_sha256 = $hashA
    cleanup_helper_path = "D:\QM\reports\dev2\credential-rotation\$rotationId\control\cleanup_dev2_account_lease.ps1"
    cleanup_helper_sha256 = $hashB
    tester_groups_source_path = "D:\QM\reports\dev2\credential-rotation\$rotationId\control\Darwinex-Live_real.canonical.txt"
    tester_groups_target_path = 'D:\QM\mt5\DEV2\MQL5\Profiles\Tester\Groups\Darwinex-Live_real.txt'
    tester_groups_sha256 = $hashC
    cleanup_result_path = "D:\QM\reports\dev2\credential-rotation\$rotationId\control\cleanup_lease.result.json"
    cleanup_disarm_path = "D:\QM\reports\dev2\credential-rotation\$rotationId\control\cleanup_lease.disarm.result.json"
    ...
    legacy_credential_path = 'C:\ProgramData\QM\DEV2\credential.clixml'
    legacy_credential_sha256 = $hashA
    legacy_credential_preserved = $true
    dynamic_receipt_path = "D:\QM\reports\dev2\credential-rotation\$rotationId\control\rotation_receipt.json"
    canonical_receipt_path = 'C:\ProgramData\QM\DEV2\credential.machine-dpapi.rotation-receipt.json'
...
foreach ($tamper in @(
        ...
        @{ Field = 'cleanup_action_mutex'; Value = 'Global\QM_DEV2_CLEANUP_ACTION_' + ('0' * 32) },
...
foreach ($required in @(
        ...
        'Invoke-QmRotationRecoveryContainment', 'Read-QmDev2MachineCredentialEnvelope',
...
$resumeDecryptIndex = $resumeText.IndexOf('Get-QmDev2MachineCredential -CredentialPath', [System.StringComparison]::Ordinal)
foreach ($marker in @(
...
if ($freshHistoryIndex -lt 0 -or $mainTaskPreflightIndex -le $freshHistoryIndex -or
    $passwordGenerationIndex -le $mainTaskPreflightIndex) {
    throw 'Fresh Apply does not reject unresolved rotation history before credential/password preparation.'
}
...
& {
    ...
    $temporaryRoot = [System.IO.Path]::GetFullPath(
        (Join-Path 'C:\QM\tmp' ('dev2-exact-bytes-test-' + [guid]::NewGuid().ToString('N')))
    )
    if (-not $temporaryRoot.StartsWith('C:\QM\tmp\dev2-exact-bytes-test-', [System.StringComparison]::OrdinalIgnoreCase)) {
...
& {
    $credentialPath = 'C:\ProgramData\QM\DEV2\credential.machine-dpapi.json'
    $canonicalRotationReceiptPath = 'C:\ProgramData\QM\DEV2\credential.machine-dpapi.rotation-receipt.json'
    $script:history = @()
...
& {
    function ConvertTo-QmRotationFullPath { param([string]$Path) [System.IO.Path]::GetFullPath($Path) }
    $currentRun = 'D:\QM\reports\dev2\credential-rotation\20260720T000000Z_' + ('1' * 32)
    ...
    $script:history = @(
        ...
        [pscustomobject]@{ RunDirectory = 'D:\QM\reports\dev2\credential-rotation\later'; Journal = $later }
    )
    function Get-QmRotationJournalHistory { param([switch]$RequireExactAcl) return $script:history }
...
& {
    ...
    function Get-ScheduledTask {
        ...
        return [pscustomobject]@{ TaskName = 'QM_DEV2_PROFILE_INIT_' + ('9' * 32) }
    }
    Invoke-Expression (Get-QmRecoveryFunctionText -Name 'Assert-QmRotationNoTasks')
    $profileTaskRejected = $false
    try { Assert-QmRotationNoTasks } catch { $profileTaskRejected = $true }
    if (-not $profileTaskRejected) { throw 'A stale DEV2 PROFILE_INIT task passed rotation preflight.' }
}
...
& {
    ...
    function Get-ScheduledTask {
        param([string]$TaskPath, [object]$ErrorAction)
        return [pscustomobject]@{ TaskName = 'QM_DEV2_CLEANUP_' + ('2' * 32) }
    }
    function Get-QmRotationDev2Processes { param([string]$TargetSid) return @() }
    Invoke-Expression (Get-QmRecoveryFunctionText -Name 'Invoke-QmRotationHostContainmentPass')
    $temporaryRoot = [System.IO.Path]::GetFullPath(
        (Join-Path 'C:\QM\tmp' ('dev2-prefence-containment-test-' + [guid]::NewGuid().ToString('N')))
    )
    if (-not $temporaryRoot.StartsWith('C:\QM\tmp\dev2-prefence-containment-test-', [System.StringComparison]::OrdinalIgnoreCase)) {
...
& {
    ...
    function Set-QmDev2CredentialExactAcl { param([string]$Path) }
    ...
    function New-QmFailureEvidencePayload {
        return [pscustomobject]@{
            ...
            dev2_root_process_count = [long]-1
    ...
    $null = Assert-QmRotationCleanupEvidenceReceipt -Path 'C:\QM\tmp\unused.json' `
        -ArtifactType 'QM_DEV2_ACCOUNT_CLEANUP_RESULT' -Journal $evidenceJournal -AllowFailure
    foreach ($malformed in @(
            ...
            @{ Field = 'dev2_root_process_count'; Value = [long]2147483648 }
        )) {
    ...
    $successPayload.dev2_root_process_count = [long]0
    ...
    $null = Assert-QmRotationCleanupEvidenceReceipt -Path 'C:\QM\tmp\unused.json' `
        -ArtifactType 'QM_DEV2_ACCOUNT_CLEANUP_RESULT' -Journal $evidenceJournal
}

& {
    $ExpectedSid = $sid
    $TargetTaskName = 'QM_DEV2_SMOKE_' + ('1' * 32)
    $CleanupTaskName = 'QM_DEV2_CLEANUP_' + ('2' * 32)
    ...
    $temporaryRoot = [System.IO.Path]::GetFullPath(
        (Join-Path 'C:\QM\tmp' ('dev2-cleanup-disarm-test-' + [guid]::NewGuid().ToString('N')))
    )
    if (-not $temporaryRoot.StartsWith('C:\QM\tmp\dev2-cleanup-disarm-test-', [System.StringComparison]::OrdinalIgnoreCase)) {
...
& {
    ...
    Remove-QmRotationScheduledTaskBounded -TaskName ('QM_DEV2_CLEANUP_' + ('3' * 32)) `
...
& {
    ...
    $rejected = $false
    try {
        Remove-QmRotationScheduledTaskBounded -TaskName ('QM_DEV2_CLEANUP_' + ('4' * 32)) `
...
& {
    function ConvertTo-QmRotationFullPath { param([string]$Path) [System.IO.Path]::GetFullPath($Path) }
    function Assert-QmDev2CredentialExactAcl { param([string]$Path) }
    ...
    $temporaryRoot = [System.IO.Path]::GetFullPath(
        (Join-Path 'C:\QM\tmp' ('dev2-rotation-recovery-test-' + [guid]::NewGuid().ToString('N')))
    )
    if (-not $temporaryRoot.StartsWith('C:\QM\tmp\dev2-rotation-recovery-test-', [System.StringComparison]::OrdinalIgnoreCase)) {
...
Write-Host 'PASS Test-Dev2RotationRecovery'
