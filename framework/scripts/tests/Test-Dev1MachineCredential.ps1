...
$helperPath = Join-Path $repoRoot 'framework\scripts\dev1_machine_credential.ps1'
$probePath = Join-Path $repoRoot 'framework\scripts\probe_dev1_machine_credential.ps1'
...
$target = Get-LocalUser -Name 'QMDev1' -ErrorAction Stop
$targetAccount = "$env:COMPUTERNAME\QMDev1"
$contractId = 'QM_DEV1_ISOLATED_MT5_LANE_V3'
$lane = 'DEV1'
$testRoot = [System.IO.Path]::GetFullPath((Join-Path 'C:\QM\tmp' ("dev1-machine-credential-test-$([guid]::NewGuid().ToString('N'))")))
$allowedTestPrefix = [System.IO.Path]::GetFullPath('C:\QM\tmp\dev1-machine-credential-test-')
...
try {
    ...
    $created = New-QmDev1MachineCredentialArtifact -CredentialPath $artifactPath -Password $dummyPassword `
    ...
    Assert-QmDev1CredentialExactAcl -Path $testRoot -Directory
    Assert-QmDev1CredentialExactAcl -Path $artifactPath
    $credential = Get-QmDev1MachineCredential -CredentialPath $artifactPath `
    ...
    $rejected = $false
    try {
        Get-QmDev1MachineCredential -CredentialPath $artifactPath -ExpectedCredentialSha256 ('0' * 64) `
    ...
    foreach ($malformed in @(
        ...
        Set-QmDev1CredentialExactAcl -Path $artifactPath
        $malformedSha = (Get-FileHash -LiteralPath $artifactPath -Algorithm SHA256).Hash.ToLowerInvariant()
        $rejected = $false
        try {
            Read-QmDev1MachineCredentialEnvelope -CredentialPath $artifactPath `
    ...
    $rejected = $false
    try {
        Get-QmDev1MachineCredential -CredentialPath $artifactPath -ExpectedCredentialSha256 $nonCanonicalSha `
            -ExpectedAccount $targetAccount -ExpectedSid $target.SID.Value -ContractId $contractId -Lane $lane | Out-Null
    } catch { $rejected = $true }
    ...
    $identityRequest = [ordered]@{
        ...
        expected_task_name = 'QM_DEV1_SMOKE_' + ('2' * 32)
    ...
    $rejected = $false
    try {
        Get-QmDev1MachineCredential -CredentialPath $artifactPath -ExpectedCredentialSha256 $tamperedSha `
            -ExpectedAccount $targetAccount -ExpectedSid $target.SID.Value -ContractId $contractId -Lane $lane | Out-Null
    } catch { $rejected = $true }
    ...
    $rejected = $false
    try { Assert-QmDev1CredentialExactAcl -Path $artifactPath } catch { $rejected = $true }
    ...
    Assert-QmDev1CredentialHelperBinding -HelperPath $helperPath -ExpectedSha256 $helperSha | Out-Null
    ...
    $expectedReceiptFields = @(
        ...
        'dev1_run_directory_created', 'metatester_started'
    )
    ...
    foreach ($forbidden in @('Enable-LocalUser', 'Register-ScheduledTask', 'Start-ScheduledTask', 'run_dev1_smoke.ps1')) {
...
Write-Host 'PASS Test-Dev1MachineCredential'
