...
if (-not (Test-QmIdentityProbePathWithin -Path $requestFull -Root $reportsRoot) -or
    ...
    throw 'Identity-probe request escaped the DEV1 report root or is missing.'
}
...
if ([int]$request.schema_version -ne 1 -or
    [string]$request.artifact_type -cne 'QM_DEV1_IDENTITY_PROBE_REQUEST' -or
    ...
    [string]$request.expected_task_name -cnotmatch '^QM_DEV1_SMOKE_[0-9a-f]{32}$' -or
...
if (-not (Test-QmIdentityProbePathWithin -Path $resultFull -Root $reportsRoot) -or
    ...
    throw 'Identity-probe result path escaped DEV1 reports or is not fresh.'
}
...
if ($null -eq $identity.User -or $identity.User.Value -cne [string]$request.expected_sid -or
    ...
    throw 'Limited identity-probe child did not run as the exact isolated QMDev1 identity.'
}
