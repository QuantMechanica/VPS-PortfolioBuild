...
$script:CleanupTaskNamePrefix = 'QM_DEV1_CLEANUP_'
...
$script:CleanupHelperSourcePath = [System.IO.Path]::GetFullPath((Join-Path $PSScriptRoot 'cleanup_dev1_account_lease.ps1'))
...
function Get-QmLaneContract {
    if (-not (Test-Path -LiteralPath $script:LaneContractPath -PathType Leaf)) {
        throw "DEV1 lane contract is missing: $($script:LaneContractPath)"
    }
    ...
    if ([int]$contract.schema_version -ne 3 -or [string]$contract.contract_id -cne 'QM_DEV1_ISOLATED_MT5_LANE_V3' -or
        ...
        [string]$contract.identity.local_user -cne $script:Dev1UserName -or
        ...
        -not (ConvertTo-QmFullPath -Path ([string]$contract.identity.credential)).Equals(
            (ConvertTo-QmFullPath -Path $script:CredentialPath), [System.StringComparison]::OrdinalIgnoreCase) -or
        ...
        -not (ConvertTo-QmFullPath -Path ([string]$contract.paths.report_root)).Equals($script:Dev1ReportsRoot, [System.StringComparison]::OrdinalIgnoreCase) -or
        ...
        [string]$contract.coordination.profile_task_prefix -cne $script:ProfileTaskNamePrefix -or
        [string]::Join('|', $actualSymbols) -cne [string]::Join('|', $expectedSymbols)) {
        throw 'DEV1 lane contract drifted from the fixed controller isolation contract.'
    }
...
function Get-QmProgramHashes {
    ...
    if ([string]::Join('|', $actualNames) -cne [string]::Join('|', @($expectedNames | Sort-Object))) {
        throw 'DEV1 lane contract must bind exactly terminal64.exe, metatester64.exe, and MetaEditor64.exe.'
    }
    $hashes = [ordered]@{}
    foreach ($name in $expectedNames) {
        $expected = ([string]$Contract.program_sha256.$name).ToLowerInvariant()
        if ($expected -notmatch '^[0-9a-f]{64}$') {
            throw "Invalid DEV1 program SHA-256 in lane contract: $name"
        }
        $path = Join-Path $script:Dev1Root $name
        ...
        if ($actual -cne $expected) {
            throw "DEV1 program hash mismatch for $name; expected=$expected actual=$actual"
        }
        $hashes[$name] = $actual
    }
    return $hashes
...
function Get-QmDev1IdentityContract {
    ...
    if ($localUser.Name -cne $script:Dev1UserName -or -not $localUser.PasswordRequired) {
    ...
    Assert-QmDev1CredentialExactAcl -Path ([System.IO.Path]::GetDirectoryName($script:CredentialPath)) -Directory
    Assert-QmDev1CredentialExactAcl -Path $script:CredentialPath
...
function Get-QmDev1ControllerAccountState {
    $localUser = Get-LocalUser -Name $script:Dev1UserName -ErrorAction Stop
    if ($localUser.Name -cne $script:Dev1UserName -or $localUser.Enabled) {
        throw 'QMDev1 must be disabled at controller entry so this controller owns the full enable/restore lifecycle.'
    }
    if (-not $localUser.PasswordRequired) {
        throw 'The local QMDev1 account must have PasswordRequired=True before controller enable.'
    }
    return [pscustomobject]@{
...
function Enable-QmDev1ControllerAccountState {
    ...
    try {
        ...
        if ($current.Name -cne $script:Dev1UserName -or $current.SID.Value -cne [string]$State.Sid -or $current.Enabled -or -not $current.PasswordRequired) {
        ...
        if ($enabled.Name -cne $script:Dev1UserName -or $enabled.SID.Value -cne [string]$State.Sid -or -not $enabled.Enabled -or -not $enabled.PasswordRequired) {
            throw 'QMDev1 temporary controller enable contract failed.'
        }
        return $true
    } catch {
        $enableError = $_
        try {
            ...
            if ($rollback.Name -cne $script:Dev1UserName -or $rollback.SID.Value -cne [string]$State.Sid) {
            ...
            if ($verified.Name -cne $script:Dev1UserName -or $verified.SID.Value -cne [string]$State.Sid -or $verified.Enabled -or -not $verified.PasswordRequired) {
                throw 'Enable-failure rollback did not restore disabled-at-rest state.'
            }
        } catch {
            throw "QMDev1 enable failed and rollback also failed. enable=$($enableError.Exception.Message); rollback=$($_.Exception.Message)"
        }
        throw $enableError
    }
}

function Restore-QmDev1ControllerAccountState {
    param([Parameter(Mandatory = $true)]$State)
    $sid = New-Object System.Security.Principal.SecurityIdentifier([string]$State.Sid)
    $current = Get-LocalUser -SID $sid -ErrorAction Stop
    if ($current.Name -cne $script:Dev1UserName -or $current.SID.Value -cne [string]$State.Sid) {
        throw 'Refusing to restore QMDev1 account state after SID drift.'
    }
    if ($current.Enabled) {
        Disable-LocalUser -SID $sid -ErrorAction Stop
    }
    $restored = Get-LocalUser -SID $sid -ErrorAction Stop
    if ($restored.Name -cne $script:Dev1UserName -or $restored.SID.Value -cne [string]$State.Sid -or $restored.Enabled -or -not $restored.PasswordRequired) {
        throw 'QMDev1 disabled-at-rest restore contract failed.'
    }
    return $true
}

function Assert-QmNoDev1Tasks {
    $stale = @(Get-ScheduledTask -TaskPath $script:TaskPath -ErrorAction Stop | Where-Object {
        ...
        $_.TaskName.StartsWith($script:TaskNamePrefix, [System.StringComparison]::Ordinal) -or
        $_.TaskName.StartsWith($script:CompileTaskNamePrefix, [System.StringComparison]::Ordinal) -or
        $_.TaskName.StartsWith($script:ProfileTaskNamePrefix, [System.StringComparison]::Ordinal)
    })
    if ($stale.Count -ne 0) {
        throw "DEV1 task preflight found $($stale.Count) stale smoke/compile/cleanup/profile-init task(s); refusing to enable the isolated account."
    }
}

function Assert-QmDev1ContainedBeforeCleanupLeaseDisarm {
    param(
        [Parameter(Mandatory = $true)]$AccountState,
        [Parameter(Mandatory = $true)][string]$AllowedCleanupTaskName
    )
    $sid = New-Object System.Security.Principal.SecurityIdentifier([string]$AccountState.Sid)
    $user = Get-LocalUser -SID $sid -ErrorAction Stop
    $tasks = @(Get-ScheduledTask -TaskPath $script:TaskPath -ErrorAction Stop | Where-Object {
        $_.TaskName.StartsWith($script:CleanupTaskNamePrefix, [System.StringComparison]::Ordinal) -or
        $_.TaskName.StartsWith($script:TaskNamePrefix, [System.StringComparison]::Ordinal) -or
        $_.TaskName.StartsWith($script:CompileTaskNamePrefix, [System.StringComparison]::Ordinal) -or
        $_.TaskName.StartsWith($script:ProfileTaskNamePrefix, [System.StringComparison]::Ordinal)
    })
    $unexpectedTasks = @($tasks | Where-Object {
        [string]$_.TaskName -cne $AllowedCleanupTaskName -or [string]$_.TaskPath -cne $script:TaskPath
    })
    if ($user.Name -cne $script:Dev1UserName -or $user.SID.Value -cne [string]$AccountState.Sid -or
        $user.Enabled -or -not $user.PasswordRequired -or
        @(Get-QmDev1IdentityProcesses -ExpectedOwnerSid ([string]$AccountState.Sid)).Count -ne 0 -or
        @(Get-QmDev1Processes).Count -ne 0 -or $unexpectedTasks.Count -ne 0) {
        throw 'DEV1 local containment is not disabled, process-free, and task-contained before cleanup-lease disarm.'
    }
}
...
function Assert-QmNoDev1IdentityProcesses {
    param(
        [Parameter(Mandatory = $true)][string]$ExpectedOwnerSid,
        [Parameter(Mandatory = $true)][string]$Stage
    )
    $running = @(Get-QmDev1IdentityProcesses -ExpectedOwnerSid $ExpectedOwnerSid)
    if ($running.Count -gt 0) {
        throw "The dedicated QMDev1 identity must be idle at $Stage; found $($running.Count) process(es)."
    }
}

function Stop-QmDev1ProcessesExact {
    param([Parameter(Mandatory = $true)][string]$ExpectedOwnerSid)
    $initial = @(Get-QmDev1IdentityProcesses -ExpectedOwnerSid $ExpectedOwnerSid)
    ...
    $remainingOwner = @(Get-QmDev1IdentityProcesses -ExpectedOwnerSid $ExpectedOwnerSid)
...
function Stop-QmScheduledTaskExact {
    ...
    if ($task.TaskName -cne $TaskName -or $task.TaskPath -cne $script:TaskPath) {
        throw 'DEV1 Scheduled Task identity drifted before stop.'
    }
    if ($task.State.ToString() -eq 'Running') {
        ...
        if ($null -ne $task -and $task.State.ToString() -eq 'Running') {
            throw "DEV1 Scheduled Task did not stop within 30 seconds: $TaskName"
        }
    }
}
...
function Unregister-QmScheduledTaskExact {
    ...
    if ($null -ne $task) {
        if ($task.TaskName -cne $TaskName -or $task.TaskPath -cne $script:TaskPath) {
            throw 'DEV1 Scheduled Task identity drifted before unregister.'
        }
    ...
    if ($null -ne (Get-ScheduledTask -TaskName $TaskName -TaskPath $script:TaskPath -ErrorAction SilentlyContinue)) {
        throw "DEV1 Scheduled Task remains registered after exact unregister: $TaskName"
    }
}

function Remove-QmScheduledTaskBounded {
    param(
        [Parameter(Mandatory = $true)][string]$TaskName,
    ...
    if ($task.TaskName -cne $TaskName -or $task.TaskPath -cne $script:TaskPath) {
        throw "Bounded DEV1 task drain observed identity drift: $TaskName"
    }
    if ($DisableBeforeStop.IsPresent) {
        Disable-ScheduledTask -TaskName $TaskName -TaskPath $script:TaskPath -ErrorAction Stop | Out-Null
        $task = Get-ScheduledTask -TaskName $TaskName -TaskPath $script:TaskPath -ErrorAction SilentlyContinue
    }
    if ($null -ne $task -and $task.State.ToString() -eq 'Running') {
        Stop-ScheduledTask -TaskName $TaskName -TaskPath $script:TaskPath -ErrorAction Stop
    ...
    if ($null -ne $task -and $task.State.ToString() -eq 'Running') {
        throw "Bounded DEV1 task drain timed out waiting for non-running state: $TaskName"
    }
    if ($null -ne $task) {
        Unregister-ScheduledTask -TaskName $TaskName -TaskPath $script:TaskPath -Confirm:$false -ErrorAction Stop
    }
    ...
    if ($null -ne $task) {
        throw "Bounded DEV1 task drain timed out waiting for task absence: $TaskName"
    }
}
...
function Enter-QmCleanupActionMutex {
    ...
    if ($Name -cnotmatch '^Global\\QM_DEV1_CLEANUP_ACTION_[0-9a-f]{32}$') {
        throw 'Cleanup action mutex name drifted from its per-run nonce binding.'
    }
    $actionMutex = New-Object System.Threading.Mutex($false, $Name)
    try {
        $acquired = $false
        try {
            $acquired = [bool]$actionMutex.WaitOne($script:CleanupActionMutexWaitMilliseconds)
        } catch {
            $cursor = $_.Exception
            while ($null -ne $cursor) {
                if ($cursor -is [System.Threading.AbandonedMutexException]) {
                    $acquired = $true
                    break
                }
        ...
        if (-not $acquired) {
            throw 'Timed out waiting for the per-run DEV1 cleanup action to finish.'
        }
        return $actionMutex
    } catch {
        $actionMutex.Dispose()
...
function Restore-QmDev1TesterGroupsCanonical {
    ...
    $canonicalHash = (Get-FileHash -LiteralPath $script:TesterGroupsCanonicalPath -Algorithm SHA256).Hash.ToLowerInvariant()
    $restoredHash = (Get-FileHash -LiteralPath $script:TesterGroupsDev1Path -Algorithm SHA256).Hash.ToLowerInvariant()
...
function Assert-QmImmediateCleanupDisarmReceipt {
    ...
    if ([string]$Receipt.artifact_type -cne 'QM_DEV1_ACCOUNT_CLEANUP_DISARM_RESULT' -or
...
function Read-QmExactImmediateCleanupEvidence {
    param(
        [Parameter(Mandatory = $true)][string]$Path,
        [Parameter(Mandatory = $true)]
        [ValidateSet('QM_DEV1_ACCOUNT_CLEANUP_RESULT', 'QM_DEV1_ACCOUNT_CLEANUP_DISARM_RESULT')]
        [string]$ArtifactType
    )
    Assert-QmNoReparseComponents -Path $Path
    $expectedFields = if ($ArtifactType -ceq 'QM_DEV1_ACCOUNT_CLEANUP_RESULT') {
        @(
            'account_restored_disabled', 'artifact_type', 'cleanup_task_name', 'cleanup_task_registered',
            'completed_utc', 'containment_verified', 'dev1_root_process_count', 'expected_sid', 'failures',
    ...
    $raw = [System.IO.File]::ReadAllText((ConvertTo-QmFullPath -Path $Path))
    $document = [System.Text.Json.JsonDocument]::Parse($raw)
    try {
        if ($document.RootElement.ValueKind -ne [System.Text.Json.JsonValueKind]::Object) {
            throw 'Immediate cleanup evidence is not a JSON object.'
        }
        $actualFields = New-Object System.Collections.Generic.List[string]
        $seen = New-Object 'System.Collections.Generic.HashSet[string]' ([System.StringComparer]::Ordinal)
        foreach ($property in $document.RootElement.EnumerateObject()) {
            if (-not $seen.Add($property.Name)) { throw 'Immediate cleanup evidence has duplicate fields.' }
            $actualFields.Add($property.Name)
        }
        $schema = $document.RootElement.GetProperty('schema_version')
        $ownerCount = $document.RootElement.GetProperty('owner_process_count')
        $rootCount = $document.RootElement.GetProperty('dev1_root_process_count')
        $failures = $document.RootElement.GetProperty('failures')
        $completed = $document.RootElement.GetProperty('completed_utc')
        if ([string]::Join('|', @($actualFields.ToArray() | Sort-Object)) -cne
                [string]::Join('|', @($expectedFields | Sort-Object)) -or
            $schema.ValueKind -ne [System.Text.Json.JsonValueKind]::Number -or $schema.GetRawText() -cne '1' -or
            $ownerCount.ValueKind -ne [System.Text.Json.JsonValueKind]::Number -or $ownerCount.GetRawText() -cne '0' -or
            $rootCount.ValueKind -ne [System.Text.Json.JsonValueKind]::Number -or $rootCount.GetRawText() -cne '0' -or
            $failures.ValueKind -ne [System.Text.Json.JsonValueKind]::Array -or $failures.GetArrayLength() -ne 0 -or
            $completed.ValueKind -ne [System.Text.Json.JsonValueKind]::String) {
            throw 'Immediate cleanup evidence fields or primitive JSON types drifted.'
        }
        $booleanFields = @(
            'success', 'containment_verified', 'lease_disarmed', 'account_restored_disabled',
            'target_task_registered', 'cleanup_task_registered'
        )
        if ($ArtifactType -ceq 'QM_DEV1_ACCOUNT_CLEANUP_RESULT') { $booleanFields += 'manifest_valid' }
        foreach ($field in $booleanFields) {
            $kind = $document.RootElement.GetProperty($field).ValueKind
            if ($kind -notin @([System.Text.Json.JsonValueKind]::True, [System.Text.Json.JsonValueKind]::False)) {
                throw "Immediate cleanup evidence field is not a JSON Boolean: $field"
            }
        }
    } finally {
        $document.Dispose()
    ...
    if (-not [DateTimeOffset]::TryParseExact([string]$payload.completed_utc, 'o', [cultureinfo]::InvariantCulture,
            [System.Globalization.DateTimeStyles]::RoundtripKind, [ref]$completedUtc) -or
        $completedUtc.Offset -ne [TimeSpan]::Zero -or $completedUtc -gt [DateTimeOffset]::UtcNow.AddMinutes(5)) {
        throw 'Immediate cleanup evidence completed_utc is invalid.'
    }
    return $payload
}

function Assert-QmCleanupEvidenceAfterActionFence {
    param(
    ...
    $disarmExists = Test-Path -LiteralPath $DisarmPath -PathType Leaf
    if ($disarmExists -and -not $resultExists) {
        throw 'Cleanup disarm evidence exists without its preceding containment result.'
    }
    if ($resultExists) {
        $result = Read-QmExactImmediateCleanupEvidence -Path $ResultPath `
            -ArtifactType 'QM_DEV1_ACCOUNT_CLEANUP_RESULT'
        if ([string]$result.artifact_type -cne 'QM_DEV1_ACCOUNT_CLEANUP_RESULT' -or
            [string]$result.expected_sid -cne $ExpectedSid -or
            [string]$result.target_task_name -cne $ExpectedTargetTaskName -or
            [string]$result.cleanup_task_name -cne $ExpectedCleanupTaskName -or
            -not [bool]$result.success -or -not [bool]$result.containment_verified -or
            [bool]$result.lease_disarmed -or -not [bool]$result.manifest_valid -or
            -not [bool]$result.account_restored_disabled -or [bool]$result.target_task_registered -or
            -not [bool]$result.cleanup_task_registered) {
            throw 'Existing cleanup containment result is failed, malformed, or identity-drifted.'
        }
    }
    if ($disarmExists) {
        $disarm = Read-QmExactImmediateCleanupEvidence -Path $DisarmPath `
            -ArtifactType 'QM_DEV1_ACCOUNT_CLEANUP_DISARM_RESULT'
        Assert-QmImmediateCleanupDisarmReceipt -Receipt $disarm -ExpectedSid $ExpectedSid `
            -ExpectedTargetTaskName $ExpectedTargetTaskName -ExpectedCleanupTaskName $ExpectedCleanupTaskName `
...
$cleanupActionMutexName = $null
$cleanupActionMutex = $null
$credential = $null
...
$dev1AccountState = $null
$dev1AccountEnabledByController = $false
$dev1AccountRestoredDisabled = $false
...
try {
    ...
    Assert-QmNoDev1Tasks
    $dev1AccountState = Get-QmDev1ControllerAccountState
    ...
    foreach ($requiredRoot in @($script:Dev1ReportsRoot, $script:CredentialPath, $script:CredentialHelperPath, $script:PwshPath, $script:CleanupHelperSourcePath, $script:TesterGroupsCanonicalPath)) {
        Assert-QmNoReparseComponents -Path $requiredRoot
    }
    $machineCredentialHelperSha256 = (Get-FileHash -LiteralPath $script:CredentialHelperPath -Algorithm SHA256 -ErrorAction Stop).Hash.ToLowerInvariant()
    if ($machineCredentialHelperSha256 -cne $ExpectedHelperSha256) {
        throw 'DEV1 machine-credential helper SHA-256 differs from the expected controller binding.'
    }
    . $script:CredentialHelperPath
    Assert-QmDev1CredentialHelperBinding -HelperPath $script:CredentialHelperPath -ExpectedSha256 $ExpectedHelperSha256 | Out-Null
    $machineCredentialSha256 = (Get-FileHash -LiteralPath $script:CredentialPath -Algorithm SHA256 -ErrorAction Stop).Hash.ToLowerInvariant()
    if ($machineCredentialSha256 -cne $ExpectedCredentialSha256) {
        throw 'DEV1 machine credential SHA-256 differs from the expected controller binding.'
    }
    ...
    if ($identityContract.Sid -cne [string]$dev1AccountState.Sid) {
        throw 'QMDev1 disabled-at-rest identity differs from the isolated lane identity.'
    }
    ...
    Assert-QmNoDev1IdentityProcesses -ExpectedOwnerSid $identityContract.Sid -Stage 'controller preflight'

    $null = Read-QmDev1MachineCredentialEnvelope -CredentialPath $script:CredentialPath `
        -ExpectedCredentialSha256 $ExpectedCredentialSha256 -ExpectedAccount $identityContract.Account `
        -ExpectedSid $identityContract.Sid -ContractId ([string]$laneContract.contract_id) `
        -Lane ([string]$laneContract.lane)
    ...
    $cleanupActionMutexName = "$($script:CleanupActionMutexPrefix)$nonce"
    ...
    $cleanupHelperPath = Join-Path $controlDirectory 'cleanup_dev1_account_lease.ps1'
    ...
    $cleanupLease = [ordered]@{
        ...
        tester_groups_target_path = $script:TesterGroupsDev1Path
    ...
    $cleanupActionArguments = '-NoLogo -NoProfile -NonInteractive -ExecutionPolicy Bypass -File "{0}" -LeasePath "{1}" -ExpectedSid "{2}" -TargetTaskName "{3}" -CleanupTaskName "{4}" -CleanupActionMutex "{5}" -ExpectedHelperSha256 "{6}"' -f `
        $cleanupHelperPath, $cleanupLeasePath, $identityContract.Sid, $taskName, $cleanupTaskName, $cleanupActionMutexName, $cleanupHelperSha256
    ...
    Register-ScheduledTask -TaskName $cleanupTaskName -TaskPath $script:TaskPath -Action $cleanupAction `
        ...
        -Description "Bounded DEV1 account containment lease $runId" -ErrorAction Stop | Out-Null
    ...
    $credential = Get-QmDev1MachineCredential -CredentialPath $script:CredentialPath `
        -ExpectedCredentialSha256 $ExpectedCredentialSha256 -ExpectedAccount $identityContract.Account `
        -ExpectedSid $identityContract.Sid -ContractId ([string]$laneContract.contract_id) `
        -Lane ([string]$laneContract.lane)
    $plainPassword = $credential.GetNetworkCredential().Password
    if ([string]::IsNullOrEmpty($plainPassword)) {
        throw 'The DEV1 machine credential decrypted an empty task password.'
    }
    $dev1AccountEnabledByController = Enable-QmDev1ControllerAccountState -State $dev1AccountState
    ...
    Assert-QmNoDev1IdentityProcesses -ExpectedOwnerSid $identityContract.Sid -Stage 'immediately before Scheduled Task start'
    ...
    if (-not (Test-Path -LiteralPath $resultPath -PathType Leaf)) {
        ...
        throw "DEV1 Scheduled Task timed out after $effectiveControllerTimeout seconds; ordered containment cleanup is armed; log=$logPath"
    }
    ...
    if ([string]$result.identity_sid -cne $identityContract.Sid -or
        ...
        [string]$result.machine_credential_sha256 -cne $machineCredentialSha256 -or
        [string]$result.machine_credential_helper_sha256 -cne $machineCredentialHelperSha256 -or
        [string]$result.child_sha256 -cne $childSha256 -or
        [string]$result.run_smoke_sha256 -cne $runSmokeSha256) {
        throw 'DEV1 result identity/task/mutex/script hash binding mismatch.'
    }
    foreach ($name in @($programSha256.Keys)) {
        $reported = [string]$result.program_sha256.$name
        if ($reported -cne [string]$programSha256[$name]) {
            throw "DEV1 result program hash mismatch for $name."
        }
    }
    ...
    if ([string]$agentProof.status -cne 'PASS' -or $proofRows.Count -lt 1 -or
        ...
        -not (ConvertTo-QmFullPath -Path ([string]$agentProof.metatester_path)).Equals(
            (ConvertTo-QmFullPath -Path (Join-Path $script:Dev1Root 'metatester64.exe')),
            [System.StringComparison]::OrdinalIgnoreCase) -or
        [string]$agentProof.metatester_sha256 -cne [string]$programSha256['metatester64.exe']) {
        throw 'DEV1 result lacks a valid exact-path, runtime-exclusive metatester listener proof.'
    }
    foreach ($listener in $proofRows) {
        if ([int]$listener.local_port -lt [int]$laneContract.agent_port_contract.minimum_port -or
            ...
            throw 'DEV1 metatester listener proof contains an out-of-contract listener.'
        }
    }
    ...
    $testerGroupsPostChildSha256 = (Get-FileHash -LiteralPath $script:TesterGroupsDev1Path -Algorithm SHA256).Hash.ToLowerInvariant()
    $testerGroupsCanonicalSha256 = (Get-FileHash -LiteralPath $script:TesterGroupsCanonicalPath -Algorithm SHA256).Hash.ToLowerInvariant()
    ...
    if (-not [string]::IsNullOrWhiteSpace($taskName)) {
        ...
        } catch { $cleanupErrors.Add("task_drain: $($_.Exception.Message)") }
    }
    if ($null -ne $dev1AccountState) {
        try { Stop-QmDev1ProcessesExact -ExpectedOwnerSid ([string]$dev1AccountState.Sid) } catch { $cleanupErrors.Add("process_cleanup: $($_.Exception.Message)") }
    }
    if ($null -ne $identityContract) {
        try { $testerGroupsRestoredSha256 = Restore-QmDev1TesterGroupsCanonical } catch { $cleanupErrors.Add("tester_groups_restore: $($_.Exception.Message)") }
    }
    if ($null -ne $dev1AccountState) {
        try {
            $dev1AccountRestoredDisabled = Restore-QmDev1ControllerAccountState -State $dev1AccountState
        } catch { $cleanupErrors.Add("account_restore: $($_.Exception.Message)") }
    }
    if ($null -ne $dev1AccountState -and -not [string]::IsNullOrWhiteSpace($cleanupTaskName)) {
        try {
            Stop-QmDev1ProcessesExact -ExpectedOwnerSid ([string]$dev1AccountState.Sid)
            Assert-QmDev1ContainedBeforeCleanupLeaseDisarm -AccountState $dev1AccountState `
                -AllowedCleanupTaskName $cleanupTaskName
    ...
    if ($cleanupTaskRegistered -and -not [string]::IsNullOrWhiteSpace($cleanupTaskName) -and $cleanupErrors.Count -eq 0) {
        try {
            ...
            $cleanupActionMutex = Enter-QmCleanupActionMutex -Name $cleanupActionMutexName
            Assert-QmCleanupEvidenceAfterActionFence -ResultPath $cleanupResultPath `
                -DisarmPath $cleanupDisarmResultPath -ExpectedSid ([string]$dev1AccountState.Sid) `
    ...
    if ($cleanupErrors.Count -gt 0 -and $cleanupTaskRegistered -and -not [string]::IsNullOrWhiteSpace($cleanupTaskName)) {
        try {
            ...
            Assert-QmCleanupEvidenceAfterActionFence -ResultPath $cleanupResultPath `
                -DisarmPath $cleanupDisarmResultPath -ExpectedSid ([string]$dev1AccountState.Sid) `
                -ExpectedTargetTaskName $taskName -ExpectedCleanupTaskName $cleanupTaskName
            $postCleanupUser = Get-LocalUser -SID (New-Object System.Security.Principal.SecurityIdentifier([string]$dev1AccountState.Sid)) -ErrorAction Stop
            $postCleanupOwnerProcesses = @(Get-QmDev1IdentityProcesses -ExpectedOwnerSid ([string]$dev1AccountState.Sid))
            $postCleanupRootProcesses = @(Get-QmDev1Processes)
            ...
            if ($postCleanupUser.Name -cne $script:Dev1UserName -or $postCleanupUser.Enabled -or -not $postCleanupUser.PasswordRequired -or
    ...
    if ($null -ne $cleanupActionMutex) {
        try {
            $cleanupActionMutex.ReleaseMutex()
        } catch {
            $cleanupErrors.Add("cleanup_action_mutex_release: $($_.Exception.Message)")
        } finally {
            $cleanupActionMutex.Dispose()
            $cleanupActionMutex = $null
        }
    }
    if ($cleanupErrors.Count -gt 0) {
        ...
        $primaryError = [System.InvalidOperationException]::new(
            "DEV1 controller/containment failure. primary=$primaryMessage; cleanup=$([string]::Join(' | ', @($cleanupErrors)))"
        )
    }
...
$finalResult | Add-Member -NotePropertyName dev1_account_initially_enabled -NotePropertyValue ([bool]$dev1AccountState.InitiallyEnabled) -Force
$finalResult | Add-Member -NotePropertyName dev1_account_enabled_by_controller -NotePropertyValue ([bool]$dev1AccountEnabledByController) -Force
$finalResult | Add-Member -NotePropertyName dev1_account_restored_disabled -NotePropertyValue ([bool]$dev1AccountRestoredDisabled) -Force
$finalResult | Add-Member -NotePropertyName cleanup_helper_sha256 -NotePropertyValue $cleanupHelperSha256 -Force
$finalResult | Add-Member -NotePropertyName machine_credential_sha256 -NotePropertyValue $machineCredentialSha256 -Force
$finalResult | Add-Member -NotePropertyName machine_credential_helper_sha256 -NotePropertyValue $machineCredentialHelperSha256 -Force
