...
from _phase_utils import ensure_dir, load_json, utc_now_iso, write_json
...
def write_aggregate_log(
    *,
    ...
    log_path = output_dir / "aggregate_phase_results_log.jsonl"
    with log_path.open("w", encoding="utf-8", newline="\n") as handle:
        for phase in EXPECTED_PHASES:
            phase_data = phases.get(phase, {})
            record = {
                "criterion": str(phase_data.get("criterion", "phase_verdict")),
                "ea_id": ea_id,
                "evidence_path": str(phase_data.get("evidence_path", "")),
                "phase": phase,
                "ts_utc": utc_now_iso(),
                "verdict": normalize_verdict(phase_data.get("verdict", "MISSING")),
            }
