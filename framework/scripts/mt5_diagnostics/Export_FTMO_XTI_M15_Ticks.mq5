...
void OnStart()
  {
   ...
   if(!SymbolSelect(symbol, true))
     {
      PrintFormat("FTMO_XTI_TICK_SELECT_FAIL error=%d", GetLastError());
      return;
     }
   const string path = "FTMO_JOINT_XTIUSD_DWX_M15_TICKS.csv";
   const int handle = FileOpen(path, FILE_WRITE | FILE_CSV | FILE_ANSI, ',');
   if(handle == INVALID_HANDLE)
     {
      PrintFormat("FTMO_XTI_TICK_OPEN_FAIL error=%d", GetLastError());
      return;
     }
   ...
   for(int year = 2018; year <= 2025; ++year)
     {
      ...
      for(int month = 1; month <= 12; ++month)
        {
         ...
         if(copied <= 0)
           {
            PrintFormat("FTMO_XTI_TICK_MONTH_EMPTY year=%d month=%d copied=%d error=%d",
      ...
      PrintFormat("FTMO_XTI_TICK_YEAR year=%d ticks=%I64d bars=%d", year, year_ticks, year_bars);
      FileFlush(handle);
     }
   FileClose(handle);
   PrintFormat("FTMO_XTI_TICK_WROTE path=%s ticks=%I64d bars=%d", path, total_ticks, total_bars);
  }
